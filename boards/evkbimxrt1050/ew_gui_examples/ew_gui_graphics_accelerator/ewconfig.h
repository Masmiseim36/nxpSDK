...
/* ******************************************************************************
   ...
   RTC_MINIMUM_TIME.
   **************************************************************************** */
#define PLATFORM_STRING       "IMXRT1050-EVKB"
...
/* ******************************************************************************
   ...
   MEMORY_POOL_SECTION - This macro has to be defined if the memory pool shall be
   declared as an array. Optionally a linker section name can be assigned to this
   macro to define where this array shall be located by the linker.
   Alternatively, a fixed memory address can be defined by setting the macro
   MEMORY_POOL_ADDR.

   MEMORY_POOL_ADDR - This macro defines the start address of the memory pool
   to a fixed address. Take care that the memory area is not used by the linker.
   Do not use this define in combination with MEMORY_POOL_SECTION.

   MEMORY_POOL_SIZE - This macro defines the size of the memory pool in bytes
   used for the Embedded Wizard heap manager.

   EXTRA_POOL_SECTION, EXTRA_POOL_ADDRESS and EXTRA_POOL_SIZE - These macros are
   used to define a second (additional) memory pool.
   **************************************************************************** */
#define MEMORY_POOL_ADDR      (FRAME_BUFFER_ADDR + FRAME_BUFFER_SIZE + DOUBLE_BUFFER_SIZE)
#define MEMORY_POOL_SIZE      SDRAM_SIZE_BYTES - FRAME_BUFFER_SIZE - DOUBLE_BUFFER_SIZE

#define EXTRA_POOL_ADDR       0
#define EXTRA_POOL_SIZE       0
