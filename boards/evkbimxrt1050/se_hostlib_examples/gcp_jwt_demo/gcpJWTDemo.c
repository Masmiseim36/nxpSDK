...
void getECDSASign_RandS(
    ...
    if( ( ret = mbedtls_pk_sign(
                    pKey,
                    MBEDTLS_MD_SHA256,
                    b64TokenBuffer, b64TokenBufferLen,
                    signature, &sig_len,
...
void addClaimb64(
        ...
        size_t max_len,
        size_t * enc_len,
        const char * aud, const unsigned long iat, const unsigned long exp)
{
    char claim_json[1024];
    snprintf(claim_json, sizeof(claim_json), CLAIM_JSON, aud, iat, exp);
    *enc_len = 0;
    mbedtls_base64_encode(pBuffer,
        max_len,
        enc_len,(unsigned char *)claim_json,strlen(claim_json));
}

#define REMOVE_TRAILING_EQUALS() do { \
    ...
    mbedtls_pk_context * pKey)
{
    ...
    getECDSASign_RandS(signature_r_and_s, &sig_len, sha256_of_header_and_claim_b64, sizeof(sha256_of_header_and_claim_b64), pKey);

    if( ( ret = mbedtls_base64_encode(
                    (unsigned char *)&b64TokenBuffer[b64TokenBuffer_writeIndex],
                    sizeof(b64TokenBuffer) - b64TokenBuffer_writeIndex,
                    &enc_len,(unsigned char *)signature_r_and_s,sig_len)) != 0 ) {
