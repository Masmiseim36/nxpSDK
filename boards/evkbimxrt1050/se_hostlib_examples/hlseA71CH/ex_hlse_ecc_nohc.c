...
//  };
    ...
    // to hold handles for eccKeyPairs
    HLSE_OBJECT_HANDLE handles[2];
...
#else
    err = hlse_GenerateEccKeyPair(keyIdx, &handles[keyIdx]);
#endif
...
#else
    err = hlse_GetEccPublicKey(pubEccKey0, &pubEccKey0Len, &handles[keyIdx]);
#endif
    result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
    err = hlse_GetEccPublicKey(pubEccKey1, &pubEccKey1Len, &handles[keyIdx]);
#endif
    result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
    err = hlse_SetEccKeyPair(keyIdx, eccPubKeyTlsNist256_0, (U16)sizeof(eccPubKeyTlsNist256_0),
                             eccPrivKeyTlsNist256_0, (U16)sizeof(eccPrivKeyTlsNist256_0),
                             &handles[keyIdx]);
#endif
    result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
    err = hlse_SetEccKeyPair(keyIdx, eccPubKeyTlsNist256_1, (U16)sizeof(eccPubKeyTlsNist256_1),
                             eccPrivKeyTlsNist256_1, (U16)sizeof(eccPrivKeyTlsNist256_1),
                             &handles[keyIdx]);
#endif
    result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
    err = hlse_GetEccPublicKey(pubEccKeyScratch, &pubEccKeyScratchLen, &handles[keyIdx]);
#endif
    result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
    err = hlse_SetEccPublicKey(keyIdx, eccPubKeyCANist256_0, (U16)sizeof(eccPubKeyCANist256_0), &handles[keyIdx]);
#endif
    result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
    err = hlse_SetEccPublicKey(keyIdx, eccPubKeyCANist256_1, (U16)sizeof(eccPubKeyCANist256_1), &handles[keyIdx]);
#endif
    result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
    err = hlse_EcdhGetSharedSecret(handles[keyIdx], eccPubKeySpareNist256_1, (U16)sizeof(eccPubKeySpareNist256_1), sharedSecret, &sharedSecretLen);
#endif
    result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
    err = hlse_EcdhGetSharedSecret(handles[keyIdx], eccPubKeyTlsNist256_1, (U16)sizeof(eccPubKeyTlsNist256_1), sharedSecret_0_1, &sharedSecret_0_1_Len);
#endif
    result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
    err = hlse_EcdhGetSharedSecret(handles[keyIdx], eccPubKeyTlsNist256_0, (U16)sizeof(eccPubKeyTlsNist256_0), sharedSecret_1_0, &sharedSecret_1_0_Len);
#endif
    result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
    err = hlse_EccSign(handles[keyIdx], hash, hashLen, signature, &signatureLen);
#endif
    result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
    err = hlse_EccVerifyWithKey(eccPubKeyTlsNist256_0, (U16)sizeof(eccPubKeyTlsNist256_0), hash, hashLen,
        signature, signatureLen, &isOk);
#endif
    result &= AX_CHECK_SW(err, SW_OK, "err");
