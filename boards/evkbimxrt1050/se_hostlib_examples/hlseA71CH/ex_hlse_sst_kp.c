...
static U8 exSstKeyPair(U8 initMode);
...
#else
        err = hlse_SetEccKeyPair((SST_Index_t) i, eccKcTls[i].pub, eccKcTls[i].pubLen, eccKcTls[i].priv, eccKcTls[i].privLen,
            &keyPairHandles[i]);
#endif
        result &= AX_CHECK_SW(err, SW_OK, "err");
    }
...
#else
        err = hlse_GetEccPublicKey(fetchedPubKey, &fetchedPubKeyLen, &keyPairHandles[i]);
#endif
        result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
        err = hlse_EccSign(keyPairHandles[i], hashSha256, hashSha256Len, signature, &signatureLen);
#endif
        result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
    err = HLSE_EraseObject(keyPairHandles[kpIndex]);
#endif
    result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
    err = hlse_GetEccPublicKey(fetchedPubKey, &fetchedPubKeyLen, &keyPairHandles[kpIndex]);
#endif
...
#else
    err = hlse_SetEccKeyPair((SST_Index_t)kpIndex, eccKcTls[kpIndex].pub, eccKcTls[kpIndex].pubLen,
        eccKcTls[kpIndex].priv, eccKcTls[kpIndex].privLen, &keyPairHandles[kpIndex]);
#endif
    result &= AX_CHECK_SW(err, SW_OK, "err");
...
#else
        err = hlse_SetEccKeyPair((SST_Index_t)kpIndex, eccKcAlt.pub, eccKcAlt.pubLen, eccKcAlt.priv, eccKcAlt.privLen,
            &keyPairHandles[kpIndex]);
#endif
...
#else
        err = hlse_EccSign(keyPairHandles[kpIndex], hashSha256, hashSha256Len, signature, &signatureLen);
#endif
        result &= AX_CHECK_SW(err, SW_OK, "err");
