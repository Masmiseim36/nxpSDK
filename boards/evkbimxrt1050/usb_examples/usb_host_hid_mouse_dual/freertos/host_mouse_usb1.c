...
static uint8_t s_MouseBuffer[HID_BUFFER_SIZE_USB1]; /*!< use to receive report descriptor and data */
usb_host_mouse_instance_t g_HostHidMouse_1;
...
void USB_HostHidMouseTask_1(void *param)
{
    ...
    switch (mouseInstance->runState)
    {
        ...
        case kUSB_HostHidRunSetIdleDone: /* 3. hid get report descriptor */
            ...
            if (mouseReportLength > HID_BUFFER_SIZE_USB1)
            {
...
usb_status_t USB_HostHidMouseEvent_1(usb_device_handle deviceHandle,
    ...
    switch (eventCode)
    {
        case kUSB_HostEventAttach:
            ...
            for (interfaceIndex = 0; interfaceIndex < configuration->interfaceCount; ++interfaceIndex)
            {
                ...
                {
                    if (g_HostHidMouse_1.deviceState == kStatus_DEV_Idle)
                    {
                        /* the interface is supported by the application */
                        g_HostHidMouse_1.mouseBuffer     = s_MouseBuffer;
                        g_HostHidMouse_1.deviceHandle    = deviceHandle;
                        g_HostHidMouse_1.interfaceHandle = interface;
                        g_HostHidMouse_1.configHandle    = configurationHandle;
        ...
        case kUSB_HostEventEnumerationDone:
            if (g_HostHidMouse_1.configHandle == configurationHandle)
            {
                if ((g_HostHidMouse_1.deviceHandle != NULL) && (g_HostHidMouse_1.interfaceHandle != NULL))
                {
                    /* the device enumeration is done */
                    if (g_HostHidMouse_1.deviceState == kStatus_DEV_Idle)
                    {
                        g_HostHidMouse_1.deviceState = kStatus_DEV_Attached;
        ...
        case kUSB_HostEventDetach:
            if (g_HostHidMouse_1.configHandle == configurationHandle)
            {
                ...
                g_HostHidMouse_1.configHandle = NULL;
                if (g_HostHidMouse_1.deviceState != kStatus_DEV_Idle)
                {
                    g_HostHidMouse_1.deviceState = kStatus_DEV_Detached;
                }
            }
            break;
