...
#define DEMO_LC3_AUDIO_SAMPLING_RATE  (kSAI_SampleRate48KHz)
...
extern osa_semaphore_handle_t xSemaphoreLEsinkAudio;
...
void BOARD_USB_Audio_TxInit_LC3(uint32_t samplingRate)
{
    sai_transceiver_t config;
    SAI_Init(BOARD_DEMO_SAI);
    SAI_TransferTxCreateHandleEDMA(BOARD_DEMO_SAI, &txHandle, txCallback, NULL, &dmaTxHandle);
    /* I2S mode configurations */
#if (defined(BEETHOVEN) || (BEETHOVEN_ENCDEC))
    SAI_GetClassicI2SConfig(&config, DEMO_AUDIO_BIT_WIDTH_BEETHOVEN, kSAI_Stereo, kSAI_Channel0Mask);
#else
...
#if (defined(BEETHOVEN) || (BEETHOVEN_ENCDEC))
    SAI_TxSetBitClockRate(BOARD_DEMO_SAI, masterClockHz, samplingRate, DEMO_AUDIO_BIT_WIDTH_BEETHOVEN, DEMO_AUDIO_DATA_CHANNEL);
#else
    SAI_TxSetBitClockRate(BOARD_DEMO_SAI, 12288750U, samplingRate, DEMO_AUDIO_BIT_WIDTH, DEMO_AUDIO_DATA_CHANNEL);
#endif
}
...
void Init_Board_Audio_LC3(void)
{
    printf("Init Audio SAI and CODEC\r\n");
    BOARD_DMA_EDMA_Config();
    BOARD_USB_Audio_TxInit_LC3(DEMO_LC3_AUDIO_SAMPLING_RATE);
    BOARD_Codec_Init();
...
void SAI_Codec_Start(UCHAR ep)
{
   ...
   if ((0 == currentFreq) && (A2DP_EP_SINK == ep))
   {
       ...
       Init_Board_Audio_LC3();
       CODEC_SetMute(&codecHandle, kCODEC_PlayChannelHeadphoneRight |kCODEC_PlayChannelHeadphoneLeft, false);
       CODEC_SetFormat(&codecHandle, DEMO_SAI_CLK_FREQ, 48000, DEMO_AUDIO_BIT_WIDTH);
       CODEC_SetVolume(&codecHandle, kCODEC_VolumeHeadphoneLeft | kCODEC_VolumeHeadphoneRight, 90);
    }
}
...
extern OSA_MSGQ_HANDLE_DEFINE(lc3_q_handle, 10, 960);
...
void a2dp_write_pl_ext (UCHAR ep, UCHAR * m_data, UINT16 m_datalen)
{
    ...
    ret = OSA_MsgQPut((osa_msgq_handle_t)lc3_q_handle, m_data);
}

void lc3_sendto_saiedma(sai_transfer_t *xfer)
{
