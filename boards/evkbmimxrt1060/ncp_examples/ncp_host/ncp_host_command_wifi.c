...
void dump_uap_prov_set_uapcfg_usage(void)
{
...
int wlan_uap_prov_set_uapcfg(int argc, char **argv)
{
    ...
    if(argc >= 2 && strlen(argv[1]) > WLAN_NETWORK_NAME_MAX_LENGTH)
    {
        (void)PRINTF("wlan_uap_prov_reset_command: invalid ssid length!\r\n");
	    return ret;
    }
	
    if(argc >= 4 && strlen(argv[3]) > NCP_WLAN_PASSWORD_MAX_LENGTH)
    {       
        (void)PRINTF("wlan_uap_prov_reset_command: invalid password length!\r\n");
        return ret;
    }
	
    NCPCmd_DS_COMMAND *uap_prov_set_uap_cfg_command = ncp_host_get_cmd_buffer_wifi();
	...
	NCP_CMD_UAP_PROV_SET_UAPCFG *set_uap_cfg = (NCP_CMD_UAP_PROV_SET_UAPCFG *)&uap_prov_set_uap_cfg_command->params.prov_set_uapcfg;
    if (argc >= 2)
    {
...
int wlan_eu_crypto_ccmp256_command(int argc, char **argv)
{
    ...
    MCU_NCPCmd_DS_COMMAND *command = ncp_host_get_cmd_buffer_wifi();
    (void)memset((uint8_t *)command, 0, NCP_HOST_COMMAND_LEN);
    command->header.cmd      = NCP_CMD_WLAN_REGULATORY_EU_CRYPTO_CCMP_256;
    command->header.size     = NCP_CMD_HEADER_LEN;
    command->header.result   = NCP_CMD_RESULT_OK;
...
int wlan_process_eu_crypto_ccmp256_response(uint8_t *res)
{
    ...
    if (cmd_res->header.result == NCP_CMD_RESULT_OK)
    {
        (void)PRINTF("EU Crypto CCMP 256 verified successfully.\r\n");
    }
    else
    {
        (void)PRINTF("EU Crypto CCMP 256 verified failed.\r\n");
    }
...
int wlan_eu_crypto_gcmp256_command(int argc, char **argv)
{
    ...
    MCU_NCPCmd_DS_COMMAND *command = ncp_host_get_cmd_buffer_wifi();
    (void)memset((uint8_t *)command, 0, NCP_HOST_COMMAND_LEN);
    command->header.cmd      = NCP_CMD_WLAN_REGULATORY_EU_CRYPTO_GCMP_256;
    command->header.size     = NCP_CMD_HEADER_LEN;
    command->header.result   = NCP_CMD_RESULT_OK;
...
int wlan_process_eu_crypto_gcmp256_response(uint8_t *res)
{
    ...
    if (cmd_res->header.result == NCP_CMD_RESULT_OK)
    {
        (void)PRINTF("EU Crypto GCMP 256 verified successfully.\r\n");
    }
    else
    {
        (void)PRINTF("EU Crypto GCMP 256 verified failed.\r\n");
    }
...
int wlan_eu_crypto_rc4_command(int argc, char **argv)
{
    ...
    MCU_NCPCmd_DS_COMMAND *command = ncp_host_get_cmd_buffer_wifi();
    (void)memset((uint8_t *)command, 0, NCP_HOST_COMMAND_LEN);
    command->header.cmd      = NCP_CMD_WLAN_REGULATORY_EU_CRYPTO_RC4;
    command->header.size     = NCP_CMD_HEADER_LEN;
    command->header.result   = NCP_CMD_RESULT_OK;
...
int wlan_process_eu_crypto_rc4_response(uint8_t *res)
{
    ...
    if (cmd_res->header.result == NCP_CMD_RESULT_OK)
    {
        (void)PRINTF("EU Crypto RC4 verified successfully.\r\n");
    }
    else
    {
        (void)PRINTF("EU Crypto RC4 verified failed.\r\n");
    }
...
int wlan_eu_crypto_aes_wrap_command(int argc, char **argv)
{
    ...
    MCU_NCPCmd_DS_COMMAND *command = ncp_host_get_cmd_buffer_wifi();
    (void)memset((uint8_t *)command, 0, NCP_HOST_COMMAND_LEN);
    command->header.cmd      = NCP_CMD_WLAN_REGULATORY_EU_CRYPTO_AES_WRAP;
    command->header.size     = NCP_CMD_HEADER_LEN;
    command->header.result   = NCP_CMD_RESULT_OK;
...
int wlan_process_eu_crypto_aes_wrap_response(uint8_t *res)
{
    ...
    if (cmd_res->header.result == NCP_CMD_RESULT_OK)
    {
        (void)PRINTF("EU Crypto AES-WRAP verified successfully.\r\n");
    }
    else
    {
        (void)PRINTF("EU Crypto AES-WRAP verified failed.\r\n");
    }
...
int wlan_eu_crypto_aes_ecb_command(int argc, char **argv)
{
    ...
    MCU_NCPCmd_DS_COMMAND *command = ncp_host_get_cmd_buffer_wifi();
    (void)memset((uint8_t *)command, 0, NCP_HOST_COMMAND_LEN);
    command->header.cmd      = NCP_CMD_WLAN_REGULATORY_EU_CRYPTO_AES_ECB;
    command->header.size     = NCP_CMD_HEADER_LEN;
    command->header.result   = NCP_CMD_RESULT_OK;
...
int wlan_process_eu_crypto_aes_ecb_response(uint8_t *res)
{
    ...
    if (cmd_res->header.result == NCP_CMD_RESULT_OK)
    {
        (void)PRINTF("EU Crypto AES-ECB verified successfully.\r\n");
    }
    else
    {
        (void)PRINTF("EU Crypto AES-ECB verified failed.\r\n");
    }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                else
                {
                    ...
                    if (scan_res_tlv->res[i].wpa2 != 0U)
                        (void)PRINTF("WPA2 ");
                    if (scan_res_tlv->res[i].wpa2_sha256 != 0U)
...
#if CONFIG_NCP_11K
                if (scan_res_tlv->res[i].neighbor_report_supported == true)
                {
                    (void)PRINTF("802.11K: YES\r\n");
                }
#endif
#if CONFIG_NCP_11V
                if (scan_res_tlv->res[i].bss_transition_supported == true)
                {
                    (void)PRINTF("802.11V: YES\r\n");
                }
#endif
                if ((scan_res_tlv->res[i].ap_mfpc == true) && (scan_res_tlv->res[i].ap_mfpr == true))
                {
                    (void)PRINTF("802.11W: Capable, Required\r\n");
                }
                if ((scan_res_tlv->res[i].ap_mfpc == true) && (scan_res_tlv->res[i].ap_mfpr == false))
                {
                    (void)PRINTF("802.11W: Capable\r\n");
                }
                if ((scan_res_tlv->res[i].ap_mfpc == false) && (scan_res_tlv->res[i].ap_mfpr == false))
                {
                    (void)PRINTF("802.11W: NA\r\n");
                }
#if CONFIG_NCP_SUPP_WPS
               if (scan_res_tlv->res[i].wps)
                {
                    if (scan_res_tlv->res[i].wps_session == WPS_SESSION_PBC)
                    {
                        (void)PRINTF("WPS: %s, Session: %s\r\n", "YES", "Push Button");
                    }
                    else if (scan_res_tlv->res[i].wps_session == WPS_SESSION_PIN)
                    {
                        (void)PRINTF("WPS: %s, Session: %s\r\n", "YES", "PIN");
                    }
                    else
...
int wlan_process_wlan_uap_prov_set_uapcfg_response(uint8_t *res)
{
    ...
    if (cmd_res->header.result == NCP_CMD_RESULT_OK)
    {
        (void)PRINTF("wlan-uap-prov-set-uapcfg success!\r\n");
    }
    else
        (void)PRINTF("wlan-uap-prov-set-uapcfg fail!\r\n");
    return WM_SUCCESS;
}
...
int wlan_process_response(uint8_t *res)
{
    ...
    switch (cmd_res->header.cmd)
    {
        ...
        case NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_GCMP_128:
            ...
            break;
        case NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_CCMP_256:
            ret = wlan_process_eu_crypto_ccmp256_response(res);
            break;
        case NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_GCMP_256:
            ret = wlan_process_eu_crypto_gcmp256_response(res);
            break;
        case NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_RC4:
            ret = wlan_process_eu_crypto_rc4_response(res);
            break;
        case NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_AES_WRAP:
            ret = wlan_process_eu_crypto_aes_wrap_response(res);
            break;
        case NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_AES_ECB:
            ret = wlan_process_eu_crypto_aes_ecb_response(res);
            break;
...
static struct ncp_host_cli_command ncp_host_app_cli_commands[] = {
    ...
    {"wlan-eu-crypto-ccmp-256", "<EncDec>", wlan_eu_crypto_ccmp256_command},
    {"wlan-eu-crypto-gcmp-256", "<EncDec>", wlan_eu_crypto_gcmp256_command},
    {"wlan-eu-crypto-rc4", "<EncDec>", wlan_eu_crypto_rc4_command},
    {"wlan-eu-crypto-aes-wrap", "<EncDec>", wlan_eu_crypto_aes_wrap_command},
    {"wlan-eu-crypto-aes-ecb", "<EncDec>", wlan_eu_crypto_aes_ecb_command},
