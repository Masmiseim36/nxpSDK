 ...
 *\brief NCP host command interfaces.
 */
/*
 *  Copyright 2024 NXP
 *
...
enum wlan_security_type
{
    ...
    WLAN_SECURITY_NONE,
    /** The network uses WEP (wired equivalent privacy) security with open key. */
    WLAN_SECURITY_WEP_OPEN,
    /** The network uses WEP security with shared key. */
    WLAN_SECURITY_WEP_SHARED,
    /** The network uses WPA (Wi-Fi protected access) security with PSK. */
    WLAN_SECURITY_WPA,
    /** The network uses WPA2 (Wi-Fi protected access 2) security with PSK. */
    WLAN_SECURITY_WPA2,
    /** The network uses WPA/WPA2 mixed security with PSK */
    WLAN_SECURITY_WPA_WPA2_MIXED,
#if CONFIG_NCP_11R
    /** The network uses WPA2 security with PSK (pre-shared key) FT (fast BSS transition). */
    WLAN_SECURITY_WPA2_FT,
#endif
    /** The network uses WPA3 security with SAE (simultaneous authentication of equals). */
    ...
    /** The network uses WPA2 Enterprise EAP-TLS (extensible authentication protocol transport layer security).
    ...
    /** The network uses DPP (device provisioning protocol) security with NAK (net access key) */
    WLAN_SECURITY_DPP,
#endif
 ...
 *                             if ant mode = 15, SAD (software antenna diversity) evaluate time interval.\n
 ...
 *                    argc should be 2.
 * \param[in] argv    Argument vector.\n
 *                    argv[0]: wlan-eu-crypto-gcmp-256\n
 *                    argv[1]: string value of decrypt or encypt option.\n
 *                             0: decrypt\n
 *                             1: encrypt
 *
