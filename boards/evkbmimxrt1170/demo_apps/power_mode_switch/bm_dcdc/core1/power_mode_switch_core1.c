...
void TypicalSetPointTransition(void)
{
    ...
    gpc_cpu_mode_t cpuMode;
    uint8_t ch, target, targetSp, i, activeSpCnt, standbySpCnt, currentSp;
    ...
    while (1)
    {
        ...
        if (target < (activeSpCnt + standbySpCnt))
        {
            ...
            currentSp = (GPC_CPU_MODE_CTRL_1->CM_SP_STAT) & (GPC_CPU_MODE_CTRL_CM_SP_STAT_CPU_SP_CURRENT_MASK);
