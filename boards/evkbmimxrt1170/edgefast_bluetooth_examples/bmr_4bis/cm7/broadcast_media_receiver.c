...
#include "lc3_codec.h"
static lc3_decoder_t decoder[MAX_AUDIO_CHANNEL_COUNT];
...
NET_BUF_POOL_FIXED_DEFINE(sdu_pool,
			  PCM_BUFF_COUNT*MAX_AUDIO_CHANNEL_COUNT,
			  sizeof(sdu_packet_t)*MAX_AUDIO_CHANNEL_COUNT,
...
static int audio_stream_decode(void)
{
	...
	sdu_packet_t *sdu;
	int frame_flags[MAX_AUDIO_CHANNEL_COUNT] = {LC3_FRAME_FLAG_BAD};
	int lc3_res;
	...
	{
		sdu = (sdu_packet_t *)(sdu_buf->data + i * sizeof(sdu_packet_t));

		if(sdu->info.flags & BT_ISO_FLAGS_VALID)
		{
			frame_flags[i] = LC3_FRAME_FLAG_GOOD;
		}

#if 1 /* used for packet lost sdu debug. */
		if((sdu->info.flags & BT_ISO_FLAGS_VALID) == 0)
		{
			PRINTF("sdu %d -> seq: %d, t: %d, flag: 0x%02x, len: %d, invalid frame: ", i, sdu->info.seq_num, sdu->info.ts, sdu->info.flags, sdu->len);

			if(sdu->info.flags & BT_ISO_FLAGS_ERROR)
			{
				PRINTF("BT_ISO_FLAGS_ERROR! ");
			}

			if(sdu->info.flags & BT_ISO_FLAGS_LOST)
			{
				PRINTF("BT_ISO_FLAGS_LOST! ");
			}

			if((sdu->info.flags & BT_ISO_FLAGS_TS) == 0)
			{
				PRINTF("time stamp invalid!");
			}
	...
	{
		sdu = (sdu_packet_t *)(sdu_buf->data + i * sizeof(sdu_packet_t));
		lc3_res = lc3_decoder(&decoder[i], sdu->buff, frame_flags[i], frame.buff[i]);
		if(lc3_res)
		{
			PRINTF("\nlc3_decoder fail!\n");
		}
...
static bool bis_cb(const struct bt_bap_base_subgroup_bis *bis, void *user_data)
{
	...
	enum bt_audio_location save_chan_allocation;
    int ret;
    ...
    if((chan_allocation == BT_AUDIO_LOCATION_FRONT_LEFT) || (chan_allocation == BT_AUDIO_LOCATION_FRONT_RIGHT))
    {
		if(AUDIO_SINK_ROLE_FRONT == le_audio_sink_role_get())
		{
			bt_audio_codec_cfg_get_chan_allocation(codec_cfg, &save_chan_allocation, true);
			save_chan_allocation |= chan_allocation;
			bt_audio_codec_cfg_set_chan_allocation(codec_cfg, save_chan_allocation);
    ...
    else if((chan_allocation == BT_AUDIO_LOCATION_BACK_LEFT) || (chan_allocation == BT_AUDIO_LOCATION_BACK_RIGHT))
    {
		if(AUDIO_SINK_ROLE_BACK == le_audio_sink_role_get())
		{
			bt_audio_codec_cfg_get_chan_allocation(codec_cfg, &save_chan_allocation, true);
			save_chan_allocation |= chan_allocation;
			bt_audio_codec_cfg_set_chan_allocation(codec_cfg, save_chan_allocation);
...
static void audio_codec_config(void)
{
	...
	if(lc3_codec_info.channels != 2)
	{
		PRINTF("There should be two channels, rather than %d channels.\n", lc3_codec_info.channels);
		while(1);
	}
	...
	for(int i = 0; i < MAX_AUDIO_CHANNEL_COUNT; i++)
	{
		int lc3_res = lc3_decoder_init(&decoder[i], lc3_codec_info.sample_rate, lc3_codec_info.frame_duration_us, lc3_codec_info.octets_per_frame, 16);
		if(lc3_res)
		{
...
static bool scan_check_and_sync_broadcast(struct bt_data *data, void *user_data)
{
	...
	if(data->type == BT_DATA_NAME_COMPLETE)
	{
		...
		if(0 == strcmp(device_name, "bms_4bis"))
		{
			PRINTF("\n[device name]:%s\n", device_name);
			PRINTF("connect...\n");
			*found = true;
