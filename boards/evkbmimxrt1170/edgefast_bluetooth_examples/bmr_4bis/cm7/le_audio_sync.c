...
/* Resampler */
SrCvtFrmCfg_t upSrcCfg[2];
SrCvtFrm_t    upSrc[2];
static int16_t resampler_buff[2][(480 + 128)] = { 0 };
volatile int resampler_buff_len = 0;

static void Resampler_Init(int instans, int sample_rate, int samples_per_frame)
{
    upSrcCfg[instans].fsIn = sample_rate;            //input sampling rate
    upSrcCfg[instans].sfOut = sample_rate;           //output sampling rate
    upSrcCfg[instans].phs = 32;                              //the filter phases
    upSrcCfg[instans].fltTaps = 32;                          //the filter taps
    upSrcCfg[instans].frmSizeIn = samples_per_frame;   //input frame size, 10ms interval
    upSrcCfg[instans].frmSizeOut = samples_per_frame;  //output frame size

    initUpCvtFrm(&upSrc[instans], &upSrcCfg[instans], 0.0);
}

/* offset: 1.0 mean one sample. */
static void Resampler_SetOffset(int instans, double offset)
{
    srCvtSetFrcSmpl(&upSrc[instans], offset);
}
...
static void Resampler_Update_FsOffset(int instans, double real_samples_per_frame, double ideal_samples_per_frame)
{
    double samples_offset = (ideal_samples_per_frame - real_samples_per_frame) / ideal_samples_per_frame;
    srCvtUpdateFreqOffset(&upSrc[instans], samples_offset);
}

static double Resampler_GetFrc(int instans)
{
...
void le_audio_sync_start(int sample_rate, int samples_per_frame)
{
    ...
    /* Init resampler. */
    Resampler_Init(0, sample_rate, samples_per_frame);
    Resampler_Init(1, sample_rate, samples_per_frame);
}
...
void le_audio_sync_process(frame_packet_t *frame)
{
    ...
    if(audio_sync_info.state == AUDIO_SYNC_STATE_STOP)
    {
        ...
        if((frame->flags & BT_ISO_FLAGS_VALID) != 0)
        {
            ...
            Resampler_SetOffset(0, - (double)(audio_sync_info.sync_offset_us / audio_sync_info.sample_duration_us));
            Resampler_SetOffset(1, - (double)(audio_sync_info.sync_offset_us / audio_sync_info.sample_duration_us));
    ...
    if((audio_sync_info.state == AUDIO_SYNC_STATE_KEEP) || (audio_sync_info.state == AUDIO_SYNC_STATE_START))
    {
        ...
        Resampler_Update_FsOffset(0, audio_sync_info.samples_per_frame + audio_sync_info.fix_output, audio_sync_info.samples_per_frame);
        Resampler_Update_FsOffset(1, audio_sync_info.samples_per_frame + audio_sync_info.fix_output, audio_sync_info.samples_per_frame);
