				  ...
				  void *rsp, uint16_t *rsp_len)
{
	...
	tester_set_bit(rp->data, BTP_GATT_RELIABLE_WRITE);
			 ...
			 struct bt_gatt_discover_params *params)
{
	...
	if (data->uuid->type == BT_UUID_TYPE_16) {
        ...
        memcpy(&service->uuid_length + sizeof(service->uuid_length), &u16, uuid_length);
	} else {
        memcpy(&service->uuid_length + sizeof(service->uuid_length), BT_UUID_128(data->uuid)->val,
		       uuid_length);
	}
				...
				struct bt_gatt_discover_params *params)
{
	...
	if (data->uuid->type == BT_UUID_TYPE_16) {
        ...
        memcpy(&(included->service.uuid_length) + sizeof(included->service.uuid_length), &u16, uuid_length);
	} else {
        memcpy(&(included->service.uuid_length) + sizeof(included->service.uuid_length), BT_UUID_128(data->uuid)->val,
		       uuid_length);
	}
			    ...
			    struct bt_gatt_discover_params *params)
{
	...
	if (data->uuid->type == BT_UUID_TYPE_16) {
        ...
        memcpy(&(chrc->uuid_length) + sizeof(chrc->uuid_length), &u16, uuid_length);
	} else {
        memcpy(&(chrc->uuid_length) + sizeof(chrc->uuid_length), BT_UUID_128(data->uuid)->val, uuid_length);
	}
				...
				struct bt_gatt_discover_params *params)
{
	...
	if (attr->uuid->type == BT_UUID_TYPE_16) {
        ...
        memcpy(&(descriptor->uuid_length) + sizeof(descriptor->uuid_length), &u16, uuid_length);
	} else {
        memcpy(&(descriptor->uuid_length) + sizeof(descriptor->uuid_length), BT_UUID_128(attr->uuid)->val,
		       uuid_length);
	}
...
static void write_reliable_rsp(struct bt_conn *conn, uint8_t err,
			   struct bt_gatt_write_params *params)
{
			  ...
			  void *rsp, uint16_t *rsp_len)
{
	...
	write_params.handle = sys_le16_to_cpu(cp->handle);
	write_params.func = write_reliable_rsp;
	write_params.offset = sys_le16_to_cpu(cp->offset);
	write_params.data = cp->data;
	write_params.length = sys_le16_to_cpu(cp->data_length);
#if (defined(CONFIG_BT_EATT) && (CONFIG_BT_EATT > 0))
