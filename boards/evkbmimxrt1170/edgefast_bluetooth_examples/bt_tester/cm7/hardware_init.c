...
#endif
#if defined(WIFI_IW612_BOARD_RD_USD) || defined(WIFI_IW416_BOARD_AW_AM510_USD)
...
#if defined(USE_USB_DEBUG_PORT) && (USE_USB_DEBUG_PORT > 0)
    USB_DeviceClockInit();
    DbgConsole_Init(BOARD_DEBUG_USBCDC_INSTANCE, 0, kSerialPort_UsbCdc, 0);
...
#elif (defined(WIFI_IW612_BOARD_RD_USD) || defined(WIFI_IW416_BOARD_AW_AM510_USD))
int controller_hci_uart_get_configuration(controller_hci_uart_config_t *config)
{
    if (NULL == config)
    {
...
#if defined(SERIAL_PORT_TYPE_USBCDC) && (SERIAL_PORT_TYPE_USBCDC > 0)
void USB_DeviceClockInit(void)
{
...
#endif
#if defined(USB_DEVICE_CONFIG_EHCI) && (USB_DEVICE_CONFIG_EHCI > 0U)
    if (BOARD_DEBUG_USBCDC_INSTANCE == kSerialManager_UsbControllerEhci0)
    {
    ...
    USB_EhciPhyInit(BOARD_DEBUG_USBCDC_INSTANCE, BOARD_XTAL0_CLK_HZ, &phyConfig);
#endif
}
#endif
