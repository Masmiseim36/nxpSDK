...
static OSA_SEMAPHORE_HANDLE_DEFINE(semaphore);
static bool semaphore_init = false;
...
int lc3_encoder_init(lc3_encoder_t *encoder, int sample_rate, int duration_us, int target_bytes, int sample_bits)
{
    ...
    if(!semaphore_init)
    {
        OSA_SemaphoreCreate(semaphore, 1);
        semaphore_init = true;
    }

    OSA_SemaphoreWait(semaphore, osaWaitForever_c);
