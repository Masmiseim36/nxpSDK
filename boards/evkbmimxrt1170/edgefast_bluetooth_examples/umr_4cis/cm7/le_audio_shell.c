...
static shell_status_t sink_init(shell_handle_t shellHandle, int32_t argc, char **argv)
{
	...
	if (0 == strcmp(role, "front"))
	{
		le_audio_sink_role_set(AUDIO_SINK_ROLE_FRONT);
		(void)strcat(new_prompt, "front>> ");
	}
	else if (0 == strcmp(role, "back"))
	{
		le_audio_sink_role_set(AUDIO_SINK_ROLE_BACK);
		(void)strcat(new_prompt, "back>> ");
	}
	else
	{
...
static shell_status_t set_sirk(shell_handle_t shellHandle, int32_t argc, char **argv)
{
	...
	{
		if((len % 2 == 0) && (len <= 2 * BT_CSIP_SIRK_SIZE))
		{
			int ret = hex2bin(input, len, sirk, BT_CSIP_SIRK_SIZE);
			if(ret != len / 2)
			{
				return kStatus_SHELL_Error;
			}
	...
	memcpy(csip_set_memeber_param.sirk, sirk, sizeof(sirk));
...
SHELL_COMMAND_DEFINE(init,           "init front|back\r\n",        sink_init,   1);
