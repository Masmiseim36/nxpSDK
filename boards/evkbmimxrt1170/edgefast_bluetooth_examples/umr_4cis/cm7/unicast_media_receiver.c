...
#include "lc3_codec.h"
lc3_decoder_t decoder[CONFIG_BT_ASCS_ASE_SNK_COUNT];
		      ...
		      struct bt_bap_ascs_rsp *rsp)
{
	...
	/* Hear we need wait the second sink stream enable before the start the play. */
	if(stream != &sink_streams[1])
	{
		return 0;
	}
...
static int set_location(void)
{
	...
	{
		case AUDIO_SINK_ROLE_FRONT:  audio_location = (enum bt_audio_location)(BT_AUDIO_LOCATION_FRONT_LEFT | BT_AUDIO_LOCATION_FRONT_RIGHT);
			break;
		case AUDIO_SINK_ROLE_BACK: audio_location = (enum bt_audio_location)(BT_AUDIO_LOCATION_BACK_LEFT | BT_AUDIO_LOCATION_BACK_RIGHT);
			break;
		default:
...
void unicast_media_receiver_task(void *param)
{
	...
	switch(le_audio_sink_role_get())
	{
		case AUDIO_SINK_ROLE_FRONT : csip_set_memeber_param.rank = 1;  break;
		case AUDIO_SINK_ROLE_BACK: csip_set_memeber_param.rank = 2;  break;
	}
