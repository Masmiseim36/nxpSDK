...
#include "lc3_codec.h"
lc3_encoder_t encoder[CONFIG_BT_BAP_UNICAST_CLIENT_ASE_SNK_COUNT];

static uint8_t audio_buff[MAX_AUDIO_CHANNEL_COUNT][MAX_AUDIO_BUFF_SIZE];
static uint8_t sdu_buff[CONFIG_BT_BAP_UNICAST_CLIENT_ASE_SNK_COUNT][LC3_FRAME_SIZE_MAX];
...
NET_BUF_POOL_FIXED_DEFINE(tx_pool2, TOTAL_BUF_NEEDED/2,
			  CONFIG_BT_ISO_TX_MTU + BT_ISO_CHAN_SEND_RESERVE, CONFIG_NET_BUF_USER_DATA_SIZE, NULL);
NET_BUF_POOL_FIXED_DEFINE(tx_pool3, TOTAL_BUF_NEEDED/2,
			  CONFIG_BT_ISO_TX_MTU + BT_ISO_CHAN_SEND_RESERVE, CONFIG_NET_BUF_USER_DATA_SIZE, NULL);
NET_BUF_POOL_FIXED_DEFINE(tx_pool4, TOTAL_BUF_NEEDED/2,
			  CONFIG_BT_ISO_TX_MTU + BT_ISO_CHAN_SEND_RESERVE, CONFIG_NET_BUF_USER_DATA_SIZE, NULL);
static struct net_buf_pool *tx_pool[] = { &tx_pool1, &tx_pool2, &tx_pool3, &tx_pool4 };
...
static int audio_stream_encode(bool mute)
{
	...
	{
		int lc3_res = lc3_encoder(&encoder[i], audio_buff[i%2], sdu_buff[i]);
		if(lc3_res)
		{
			PRINTF("\nlc3_encoder fail!\n");
		}
...
static void add_remote_sink(struct bt_bap_ep *ep, uint8_t index)
{
	...
	{
		if(sinks[index*2+i].ep == NULL)
		{
			sinks[index*2+i].ep = ep;
			break;
		}
	}
}
...
static int configure_stream(struct bt_bap_stream *stream, struct bt_bap_ep *ep)
{
	...
	if((audio_receiver_loc[i/2] & BT_AUDIO_LOCATION_FRONT_LEFT) || (audio_receiver_loc[i/2] & BT_AUDIO_LOCATION_FRONT_RIGHT))
	{
		chan_allocation_value = ((i % 2) == 0) ? BT_AUDIO_LOCATION_FRONT_LEFT : BT_AUDIO_LOCATION_FRONT_RIGHT;
	}
	else if((audio_receiver_loc[i/2] & BT_AUDIO_LOCATION_BACK_LEFT) || (audio_receiver_loc[i/2] & BT_AUDIO_LOCATION_BACK_RIGHT))
	{
		chan_allocation_value = ((i % 2) == 0) ? BT_AUDIO_LOCATION_BACK_LEFT : BT_AUDIO_LOCATION_BACK_RIGHT;
	}
	else
	{
		PRINTF("chan_allocation_value not set!\n");
	}

	bt_audio_codec_cfg_set_chan_allocation(&lc3_preset.codec_cfg, chan_allocation_value);

	err = bt_bap_stream_config(default_conn[i/2], stream, ep,
...
static int configure_streams(int index)
{
	...
	for(int i = 0; i < MAX_AUDIO_CHANNEL_COUNT; i++)
	{
		struct bt_bap_ep *ep = sinks[index*2 + i].ep;
		struct bt_bap_stream *stream = &streams[index*2 + i];
...
static int enable_streams(int index)
{
	...
	for(int i = 0; i < MAX_AUDIO_CHANNEL_COUNT; i++)
	{
		...
		switch(index*2 + i)
		{
			case 0: chan_allocation_value = (enum bt_audio_location)(audio_receiver_loc[i] & BT_AUDIO_LOCATION_FRONT_LEFT);
				break;
			case 1: chan_allocation_value = (enum bt_audio_location)(audio_receiver_loc[i] & BT_AUDIO_LOCATION_FRONT_RIGHT);
				break;
			case 2: chan_allocation_value = (enum bt_audio_location)(audio_receiver_loc[i] & BT_AUDIO_LOCATION_BACK_LEFT);
				break;
			case 3: chan_allocation_value = (enum bt_audio_location)(audio_receiver_loc[i] & BT_AUDIO_LOCATION_BACK_RIGHT);
				break;
		}
		bt_audio_codec_cfg_set_chan_allocation(&lc3_preset.codec_cfg, chan_allocation_value);

		err = bt_bap_stream_enable(&streams[index*2 + i],
...
static int disable_streams(int index)
{
	...
	for(int i = 0; i < MAX_AUDIO_CHANNEL_COUNT; i++)
	{
		err = bt_bap_stream_disable(&streams[index*2 + (1-i)]);
		if (err != 0) {
...
static int connect_streams(int index)
{
	...
	for(int i = 0; i < MAX_AUDIO_CHANNEL_COUNT; i++)
	{
		err = bt_bap_stream_connect(&streams[index*2 + i]);
		if (err != 0) {
