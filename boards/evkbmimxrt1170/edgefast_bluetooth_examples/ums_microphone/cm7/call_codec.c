...
#define SIMPLE_RX_CHANNELS_MAX 1U

#define CODEC_TX_BUFFER_SIZE_MAX     (SIMPLE_TX_CHANNELS_MAX * SIMPLE_BIT_RATE_MAX * SIMPLE_BITS_MAX * SIMPLE_DURATION_MAX / 8000U)
#define CODEC_TX_BUFFER_COUNT    (1U)

#define CODEC_RX_BUFFER_SIZE_MAX     (SIMPLE_RX_CHANNELS_MAX * SIMPLE_BIT_RATE_MAX * SIMPLE_BITS_MAX * SIMPLE_DURATION_MAX / 8000U)
...
AT_NONCACHEABLE_SECTION_ALIGN(static uint8_t codecRxBuffer[CODEC_RX_BUFFER_SIZE_MAX * CODEC_RX_BUFFER_COUNT], 4);
...
AT_NONCACHEABLE_SECTION_ALIGN(static uint8_t codecTxBuffer[CODEC_TX_BUFFER_SIZE_MAX * CODEC_TX_BUFFER_COUNT], 4);
...
static volatile uint32_t codec_rx_prime_size = CODEC_RX_BUFFER_SIZE_MAX;
...
static void APP_CodecTxCallback(hal_audio_handle_t handle, hal_audio_status_t completionStatus, void *callbackParam)
{
    ...
    tx_played_count += CODEC_TX_BUFFER_SIZE_MAX;
...
static void APP_CodecRxCallback(hal_audio_handle_t handle, hal_audio_status_t completionStatus, void *callbackParam)
{
    ...
    rx_new_count += CODEC_RX_BUFFER_SIZE_MAX;
    OSA_SemaphorePost(s_audioTaskSync);
}

int BOARD_AudioInit(uint32_t samplingRate, uint32_t bitWidth, uint32_t duration_us)
{
...
(defined FSL_FEATURE_PCC_HAS_SAI_DIVIDER && FSL_FEATURE_PCC_HAS_SAI_DIVIDER)
	...
	CODEC_SetFormat(&codec_handle, codecRxConfig.srcClock_Hz, codecRxConfig.sampleRate_Hz, codecRxConfig.bitWidth);
	...
	codec_rx_prime_size = (uint32_t)((uint64_t)samplingRate * (uint64_t)duration_us * (uint64_t)bitWidth / 8000000);
...
static void BOARD_CodecTask(void* param)
{
    ...
    while (true)
    {
        ...
        {
            xfer.data     = &codecTxBuffer[codecTxBufferPrimeIndex * CODEC_TX_BUFFER_SIZE_MAX];
            xfer.dataSize = codec_rx_prime_size;
            if (kStatus_HAL_AudioSuccess == HAL_AudioTransferSendNonBlocking((hal_audio_handle_t)&codecTxHandle[0], &xfer))
            {
                tx_queued_count += CODEC_TX_BUFFER_SIZE_MAX;
        ...
        {
            xfer.data     = codecRxBuffer + codecRxBufferPrimeIndex * CODEC_RX_BUFFER_SIZE_MAX;
            xfer.dataSize = codec_rx_prime_size;
...
uint8_t *BOARD_GetRxReadBuffer(void)
{
    uint8_t * buffer = &codecRxBuffer[codecRxBufferReadIndex * CODEC_RX_BUFFER_SIZE_MAX];
    ...
    rx_read_count += codec_rx_prime_size;
...
static uint8_t *BOARD_GetRxNotifyBuffer(void)
{
    uint8_t * buffer = &codecRxBuffer[codecRxBufferNotifyIndex * CODEC_RX_BUFFER_SIZE_MAX];
...
int BOARD_StartCodec(codec_rx_callback_t rx_cb, uint32_t simpleBitRate, uint32_t simpleBits, uint32_t duration_ms)
{
    BOARD_AudioDeinit();

    BOARD_AudioInit(simpleBitRate, simpleBits, duration_ms);
