...
static int draw_path(vg_lite_buffer_t *buffer, vg_lite_color_t back_color, vg_lite_path_t path[], uint32_t color_data[], int path_count, vg_lite_float_t translate_x, vg_lite_float_t translate_y, vg_lite_float_t scale_x, vg_lite_float_t scale_y, vg_lite_float_t rotate_deg)
{
    vg_lite_error_t error = VG_LITE_SUCCESS;
    ...
    {
        error = vg_lite_draw(buffer, &path[count], VG_LITE_FILL_EVEN_ODD, &matrix, VG_LITE_BLEND_NONE, color_data[count]);
        if (error)
        {
...
{
	rot0 += 1.0; rot1 += 2.0; rot2 += 3.0;
	rot3 += 4.0; rot4 += 5.0; rot5 +=6.0;
	if (rot0 > 360.0f)
		rot0 -= 360.0;
	if (rot1 > 360.0f)
		rot1 -= 360.0;
    ...
    if (nz0321 > 0.0f)
    {
	    draw_path(&image, 0xFFFF0000, tiger_path, tiger_color_data, tiger_path_count, 90.0f, 90.0f, 1.5f, 1.5f, rot0);
    ...
    if (nz4567 > 0.0f)
    {
    	...
    	if (draw_path(&image, 0xFFFFFFFF, nxp_path, nxp_color_data, nxp_path_count, 25.0f, 80.5f, 3.0f, 3.0f, 0.0f) != 1) {
        	PRINTF("load image1 file error\n");
	    }
        /* Compute 3x3 image transform matrix to map a rectangle image (w,h) to
           a parallelogram (x0,y0), (x1,y1), (x2,y2), (x3,y3) counterclock wise.
        */
    ...
    if (nz5126 > 0.0f)
    {
	    draw_path(&image, 0xFFE7BFC8, tiger_path, tiger_color_data, tiger_path_count, 90.0f, 90.0f, 1.5f, 1.5f, rot2);
        /* Compute 3x3 image transform matrix to map a rectangle image (w,h) to
           a parallelogram (x0,y0), (x1,y1), (x2,y2), (x3,y3) counterclock wise.
        */
    ...
    if (nz7623 > 0.0f)
    {
	    draw_path(&image, 0xFFEAD999, tiger_path, tiger_color_data, tiger_path_count, 90.0f, 90.0f, 1.5f, 1.5f, rot4);
        /* Compute 3x3 image transform matrix to map a rectangle image (w,h) to
           a parallelogram (x0,y0), (x1,y1), (x2,y2), (x3,y3) counterclock wise.
        */
