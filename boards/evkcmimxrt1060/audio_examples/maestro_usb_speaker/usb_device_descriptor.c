...
uint8_t g_UsbDeviceConfigurationDescriptor[] = {
    ...
    USB_SHORT_GET_LOW(USB_AUDIO_CONFIGURATION_DESC_TOTAL_LENGTH),  /* Total length of data returned for this
                                                                      configuration. */
    USB_SHORT_GET_HIGH(USB_AUDIO_CONFIGURATION_DESC_TOTAL_LENGTH), /* Total length of data returned for this
                                                                      configuration. */
    ...
    (USB_DESCRIPTOR_CONFIGURE_ATTRIBUTE_D7_MASK) |
        ...
        (1 << USB_DESCRIPTOR_CONFIGURE_ATTRIBUTE_SELF_POWERED_SHIFT) |
#endif
#if defined(USB_DEVICE_CONFIG_REMOTE_WAKEUP) && (USB_DEVICE_CONFIG_REMOTE_WAKEUP > 0U)       
        (1 << USB_DESCRIPTOR_CONFIGURE_ATTRIBUTE_REMOTE_WAKEUP_SHIFT) |
#endif
    0U,
    ...
    */
    ...
    USB_SHORT_GET_HIGH(USB_AUDIO_CONTROL_INTERFACE_TOTAL_LENGTH),
    ...
    USB_AUDIO_SPEAKER_CONTROL_CLOCK_SOURCE_ENTITY_ID, /* ID of the Clock Entity to which this Input Terminal is
                                                         connected.  */
    AUDIO_FORMAT_CHANNELS, /* This Terminal's output audio channel cluster has 1 logical output channel   */
    USB_SHORT_GET_LOW(((uint32_t)USB_AUDIO_SPATIAL_LOCATION) & 0xFFFFU),
    USB_SHORT_GET_HIGH(((uint32_t)USB_AUDIO_SPATIAL_LOCATION) & 0xFFFFU),
    USB_SHORT_GET_LOW((((uint32_t)USB_AUDIO_SPATIAL_LOCATION) >> 16U) & 0xFFFFU),
    USB_SHORT_GET_HIGH((((uint32_t)USB_AUDIO_SPATIAL_LOCATION) >> 16U) & 0xFFFFU),
    ...
    0x01U, 0x00U, 0x00U, 0x00U, /* The Audio Data Format that can be Used to communicate with this interface, D0:PCM */
