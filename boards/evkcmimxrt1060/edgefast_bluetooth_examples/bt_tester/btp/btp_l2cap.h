...
#define BTP_L2CAP_TRANSPORT_LE				0x00
#define BTP_L2CAP_TRANSPORT_BREDR			0x01
...
#define BTP_L2CAP_SEND_UC_DATA				0x0c
struct btp_l2cap_send_uc_data_cmd {
	bt_addr_t address;
	uint16_t data_len;
	uint8_t data[];
} __packed;
...
#define BTP_L2CAP_EV_CONNECTIONLESS_DATA_RECEIVED  0x85
struct btp_l2cap_cl_data_received_ev {
	uint8_t chan_id;
	uint16_t psm;
	uint16_t mtu_remote;
	uint16_t mps_remote;
