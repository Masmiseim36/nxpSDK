				       ...
				       void *rsp, uint16_t *rsp_len)
{
	...
	source->qos.framing = (enum bt_audio_codec_qos_framing)cp->framing;
...
uint8_t btp_bap_local_add_broadcast_src(const void *cmd, uint16_t cmd_len,
					void *rsp, uint16_t *rsp_len)
{
	int err;
	const struct btp_bap_local_add_broadcast_src_cmd *cp = cmd;
	struct bt_bap_scan_delegator_add_src_param add_src;
#if 0
	struct bt_bap_scan_delegator_mod_src_param mod_src;
#endif
	const uint8_t *ptr;

	LOG_DBG("");

	memset(add_src.subgroups, 0, sizeof(add_src.subgroups));
	bt_addr_le_copy(&add_src.addr, &cp->address);
	add_src.sid = cp->sid;
	add_src.encrypt_state = cp->encrypted ? BT_BAP_BIG_ENC_STATE_BCODE_REQ :
				BT_BAP_BIG_ENC_STATE_NO_ENC;
	add_src.broadcast_id = sys_get_le24(cp->broadcast_id);
	add_src.num_subgroups = cp->num_subgroups;

	add_src.num_subgroups = MIN(cp->num_subgroups, CONFIG_BT_BAP_BASS_MAX_SUBGROUPS);

	ptr = &cp->subgroups[0];
	for (uint8_t i = 0; i < add_src.num_subgroups; i++) {
		struct bt_bap_bass_subgroup *subgroup = &add_src.subgroups[i];
	...
	err = bt_bap_scan_delegator_add_src(&add_src);
	if (err < 0) {
		LOG_DBG("add src err %d", err);

		return BTP_STATUS_FAILED;
	}

#if 0
	memset(mod_src.subgroups, 0, sizeof(mod_src.subgroups));
	mod_src.src_id = (uint8_t)err;
	mod_src.encrypt_state = cp->encrypted ? BT_BAP_BIG_ENC_STATE_BCODE_REQ :
				BT_BAP_BIG_ENC_STATE_NO_ENC;
	mod_src.broadcast_id = sys_get_le24(cp->broadcast_id);
	mod_src.num_subgroups = cp->num_subgroups;

	mod_src.num_subgroups = MIN(cp->num_subgroups, CONFIG_BT_BAP_BASS_MAX_SUBGROUPS);

	ptr = &cp->subgroups[0];
	for (uint8_t i = 0; i < add_src.num_subgroups; i++) {
		struct bt_bap_bass_subgroup *subgroup = &mod_src.subgroups[i];
	...
	err = bt_bap_scan_delegator_mod_src(&mod_src);
	if (err != 0) {
		LOG_DBG("mod src err %d", err);
