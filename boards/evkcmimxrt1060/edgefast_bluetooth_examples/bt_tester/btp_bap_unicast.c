					...
					enum bt_audio_dir dir)
{
	...
	if (codec_cap_get_val(codec_cap, BT_AUDIO_CODEC_CAP_TYPE_FREQ, &data)) {
		/* EDGEFAST: Fix Building warning on IAR */
#if 0
		memcpy(&ev.frequencies, data, sizeof(ev.frequencies));
#else
		ev.frequencies = sys_cpu_to_le16(sys_get_le16(data));
#endif
	}
...
#else
		ev.octets_per_frame = sys_cpu_to_le32(sys_get_le32(data));
#endif
	}
...
static int client_add_ase_to_cis(struct btp_bap_unicast_connection *u_conn, uint8_t ase_id,
				  ...
				  void *rsp, uint16_t *rsp_len)
{
	...
	qos->framing = (enum bt_audio_codec_qos_framing)cp->framing;
