 ...
 *
 * This function  allocates memory for and initializes a timer.
 *
 * @return:  Handler to access the timer for start and stop operations. If timer
 *         creation failed, return NULL.
 */
 ...
 *
 * @param timerId  Pointer to timer returned by the OSA_TimerCreate function.
 * @param oneshot  One-shot timer or periodly timer.
 * @param millisec The time in milliseconds to wait before timer expires.
 * @param callback The function to be called before timer expires, in task context.
 * @param param    The parameter passed to callback function.
 *
 ...
 *
 * @param timerId  Timer handler returned by the OSA_TimerCreate function.
 *
 * @retval osaStatus_Success   The timer was successfully stopped.
 * @retval osaStatus_Error     Invalid parameter.
 */
...
/*!
 * @brief allocate a buffer from heap.
 *
 * @param bytes number of bytes to alloc.
 *
