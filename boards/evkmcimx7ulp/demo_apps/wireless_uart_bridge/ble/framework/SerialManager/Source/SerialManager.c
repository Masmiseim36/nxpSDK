...
/* If the SPI Slave has more data to transmit, restart the transfer */
...
#endif
            {
                ...
                {
                    Spi_SendDummyByte(i, &mSerials[i].rxBuffer[mSerials[i].rxIn]);
                }
            }
#endif
        }
    ...
    uint8_t InterfaceId, uint8_t *pBuf, uint16_t bufLen, pSerialCallBack_t cb, void *pTxParam)
{
    ...
    ((gSerialMgr_BlockSenderOnQueueFull_c) && (gSMGR_UseOsSemForSynchronization_c))
...
/* Wait until Tx finishes. The sem will be released by the SMGR task */
        ...
        {
#if 0
             /* Don't poll in Serial Manager task which will cause rtos dead lock,
              * sync write is not necessary in Serial Manager */
             while(pSer->state) {}
#endif
        }
#if 0 /* Don't poll if already got semaphore to avoid rtos dead lock issue */
#if (gSerialMgrUseUart_c)
...
static serialStatus_t Serial_WriteInternal(uint8_t InterfaceId)
{
    ...
    switch (mSerials[InterfaceId].serialType)
    {
        ...
        case gSerialMgrIICMaster_c:
            if (kStatus_Success !=
                I2C_MasterSend(pSer->serialChannel, pSer->txQueue[idx].pData, pSer->txQueue[idx].dataSize))
            {
                status = gSerial_InternalError_c;
            }
            break;
        ...
        case gSerialMgrIICSlave_c:
            ...
            if (kStatus_Success !=
                I2C_SlaveSend(pSer->serialChannel, pSer->txQueue[idx].pData, pSer->txQueue[idx].dataSize))
            {
                status = gSerial_InternalError_c;
                break;
            }
        ...
        case gSerialMgrSPISlave_c:
            ...
            if (Spi_AsyncTransfer(pSer->serialChannel, pSer->txQueue[idx].pData, &pSer->rxBuffer[pSer->rxIn],
        ...
        case gSerialMgrSPIMaster_c:
        {
            ...
            while ((spiStatus = Spi_AsyncTransfer(pSer->serialChannel, pSer->txQueue[idx].pData,
                                                  &pSer->rxBuffer[pSer->rxIn], pSer->txQueue[idx].dataSize)) == spiBusy)
                ;
            if (spiStatus)
            {
                status = gSerial_InternalError_c;
...
void SerialManager_RxNotify(uint32_t i)
{
    ...
    mSerial_IncIdx_d(pSer->rxIn, gSMRxBufSize_c) if (pSer->rxIn == pSer->rxOut)
    {
...
#endif
                {
                    Spi_SendDummyByte(pSer->serialChannel, &pSer->rxBuffer[pSer->rxIn]);
                }
            }
...
static void SpiMasterDapISR(void)
{
    ...
    if (GpioIsPinIntPending(&mSpiMasterDapCfg))
    {
#if gSerialMgrSlaveDapTxLogicOne_c
...
static void I2cMasterDapISR(void)
{
    ...
    if (GpioIsPinIntPending(&mI2cMasterDapCfg))
    {
#if gSerialMgrSlaveDapTxLogicOne_c
        if (GpioReadPinInput(&mI2cMasterDapCfg))
#else
