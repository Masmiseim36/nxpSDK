...
#define Ble_IsPrivateResolvableDeviceAddress(bleAddress) ((bleAddress[5] & (BIT7 | BIT6)) == BIT6)

/*! PNRA condition: check the 6th byte - MSB should be 0; 2nd MSB should be 0. */
#define Ble_IsPrivateNonresolvableDeviceAddress(bleAddress) ((bleAddress[5] & (BIT7 | BIT6)) == 0)

/*! RSA condition: check the 6th byte - MSB should be 1; 2nd MSB should be 1. */
#define Ble_IsRandomStaticDeviceAddress(bleAddress) ((bleAddress[5] & (BIT7 | BIT6)) == (BIT7 | BIT6))

/*! A macro used to compare two device addresses */
#define Ble_DeviceAddressesMatch(bleAddress1, bleAddress2)                           \
    ((bleAddress1)[0] == (bleAddress2)[0] && (bleAddress1)[1] == (bleAddress2)[1] && \
     (bleAddress1)[2] == (bleAddress2)[2] && (bleAddress1)[3] == (bleAddress2)[3] && \
     (bleAddress1)[4] == (bleAddress2)[4] && (bleAddress1)[5] == (bleAddress2)[5])

/*! A macro used to copy device addresses */
#define Ble_CopyDeviceAddress(destinationAddress, sourceAddress) \
    {                                                            \
        for (uint8_t j = 0; j < gcBleDeviceAddressSize_c; j++)   \
        {                                                        \
            destinationAddress[j] = sourceAddress[j];            \
        }                                                        \
    }
...
typedef enum
{
    ...
    gBleInvalidParameter_c =
        gBleStatusBase_c | 0x01, /*!< Parameter has an invalid value or is outside the accepted range. */
    ...
    gBleFeatureNotSupported_c =
        gBleStatusBase_c | 0x04,                 /*!< The requested feature is not supported by this stack version. */
    gBleOutOfMemory_c = gBleStatusBase_c | 0x05, /*!< An internal memory allocation failed. */
    gBleAlreadyInitialized_c =
        gBleStatusBase_c | 0x06, /*!< Ble_HostInitialize function is incorrectly called a second time. */
    ...
    gSmCommandNotSupported_c =
        gSmStatusBase_c |
        0x02, /*!< The Security Manager (SM) does not have the required features or version to support this command */
    gSmUnexpectedCommand_c =
        gSmStatusBase_c | 0x03, /*!< This command is not or cannot be handled in the current context of the SM.  */
    gSmInvalidCommandCode_c = gSmStatusBase_c | 0x04, /*!< The provided SM command code is invalid. */
    gSmInvalidCommandLength_c =
        gSmStatusBase_c | 0x05, /*!< The provided command length is not valid for the SM command code. */
    ...
    gSmInvalidInternalOperation_c =
        gSmStatusBase_c | 0x08, /*!< There is a problem with the internal state of the SM. This should not happen during
                                 * normal operation. A memory corruption or invalid operation may have occurred.  */
    gSmInvalidConnectionHandle_c =
        gSmStatusBase_c |
        0x09, /*!< The target device does not have a valid connection handle. It might be disconnected. */
    gSmInproperKeyDistributionField_c =
        gSmStatusBase_c |
        0x0A, /*!< The Responder upper layer has set to "1" one or more flags in the Initiator or Responder Key
               * Distribution Fields from the Pairing Request which were set to "0" by the peer device. */
    gSmUnexpectedKeyType_c =
        gSmStatusBase_c | 0x0B, /*!< The Responder upper layer has set a key type field in the Passkey Request Reply
                                 *   command, which is different than the field negotiated with the peer device. */
    gSmUnexpectedPairingTerminationReason_c =
        gSmStatusBase_c | 0x0C, /*!< The upper layer tried to cancel the pairing procedure with an unexpected
                                 *   pairing failure reason for the current phase of the pairing procedure. */
    gSmUnexpectedKeyset_c =
        gSmStatusBase_c |
        0x0D, /*!< The Responder upper layer is trying to distribute keys which were not requested during the pairing
               * procedure or the peer device has sent a Key Distribution packet which was not expected. */
    gSmSmpTimeoutOccurred_c =
        gSmStatusBase_c | 0x0E, /*!< An SMP timeout has occurred for the peer device.
                                 *   No more operations are accepted until a new physical link is established. */
    gSmUnknownSmpPacketType_c =
        gSmStatusBase_c | 0x0F, /*!< An SMP packet with an unknown (or invalid) type has been received. */
    gSmInvalidSmpPacketLength_c =
        gSmStatusBase_c | 0x10, /*!< An SMP packet with an invalid length for the SMP packet type has been received. */
    gSmInvalidSmpPacketParameter_c =
        gSmStatusBase_c | 0x11, /*!< An SMP packet with an invalid parameter has been received. */
    ...
    gSmReceivedSmpPacketFromUnknownDevice_c =
        gSmStatusBase_c | 0x13, /*!< An SMP packet is received but the source Device ID cannot be identified. */
    gSmReceivedUnexpectedHciEvent_c =
        gSmStatusBase_c | 0x14, /*!< An HCI event has been received which cannot be handled by the SM or cannot be
                                 *   handled in the current context. */
    gSmReceivedHciEventFromUnknownDevice_c =
        gSmStatusBase_c | 0x15, /*!< An HCI event is received but the source Device ID cannot be identified. */
    ...
    gSmLlConnectionEncryptionInProgress_c =
        gSmStatusBase_c |
        0x17, /*!< A Link Layer Connection encryption was requested by the upper layer or attempted internally by
    ...
    gSmLlConnectionEncryptionFailure_c =
        gSmStatusBase_c | 0x18, /*!< The Link Layer connection encryption procedure has failed. */
    gSmInsufficientResources_c =
        gSmStatusBase_c | 0x19, /*!< The SM could not allocate resources to perform operations (memory or timers). */
    /* LE SC statuses. */
    gSmOobDataAddressMismatch_c =
        gSmStatusBase_c | 0x1A, /*!< The address of the peer contained in the remote OOB data sent to the stack does not
                                 * match the address used by the remote device for the connection/pairing procedure. */
    gSmSmpPacketReceivedAfterTimeoutOccurred_c =
        gSmStatusBase_c |
        0x1B, /*!< A SMP packet has been received from a peer device for which a pairing priocedure has timed out.
               *   No further operations are permitted until a new connection sisestablished. */
    gSmReceivedTimerEventForUnknownDevice_c =
        gSmStatusBase_c | 0x1C, /*!< An Timer event is received but the source Device ID cannot be identified. */
    gSmUnattainableLocalDeviceSecRequirements_c =
        gSmStatusBase_c | 0x1D, /*!< The provided pairing parameters cannot lead to a Pairing Procedure which satisfies
                                   the minimum security properties for the local device. */
    gSmUnattainableLocalDeviceMinKeySize_c =
        gSmStatusBase_c | 0x1E, /*!< The provided pairing parameters cannot lead to a Pairing Procedure which satisfies
                                   the minimum encryption key size for the local device. */
    gSmUnattainableSlaveSecReqRequirements_c =
        gSmStatusBase_c |
        0x1F, /*!< The provided pairing parameters cannot lead to a Pairing Procedure which satisfies the minimum
    ...
    gSmTbResolvableAddressDoesNotMatchIrk_c =
        gSmStatusBase_c | 0x60, /*!< The provided Resolvable Private Address and IRK do not match. */
    gSmTbInvalidDataSignature_c =
        gSmStatusBase_c | 0x61, /*!< The provided data signature does not match the computed data signature. */
    ...
    gGattAnotherProcedureInProgress_c =
        gGattStatusBase_c | 0x01, /*!< Trying to start a GATT procedure while one is already in progress. */
    gGattLongAttributePacketsCorrupted_c =
        gGattStatusBase_c |
        0x02, /*!< Writing a Long Characteristic failed because Prepare Write Request packets were corrupted. */
    gGattMultipleAttributesOverflow_c =
        gGattStatusBase_c |
        0x03, /*!< Too many Characteristics are given for a Read Multiple Characteristic procedure. */
    gGattUnexpectedReadMultipleResponseLength_c =
        gGattStatusBase_c |
        0x04, /*!< Read Multiple Characteristic procedure failed because unexpectedly long data was read. */
    gGattInvalidValueLength_c =
        gGattStatusBase_c | 0x05, /*!< An invalid value length was supplied to a Characteristic Read/Write operation. */
    gGattServerTimeout_c = gGattStatusBase_c | 0x06, /*!< No response was received from the Server. */

    gGattIndicationAlreadyInProgress_c =
        gGattStatusBase_c | 0x07, /*!< A Server Indication is already waiting for Client Confirmation. */
    gGattClientConfirmationTimeout_c =
        gGattStatusBase_c | 0x08, /*!< No Confirmation was received from the Client after a Server Indication. */
    ...
    gDevDbCccdLimitReached_c =
        gDevDbStatusBase_c |
        0x01, /*!< CCCD value cannot be saved because Server's CCCD list is full for the current client. */
    gDevDbCccdNotFound_c =
        gDevDbStatusBase_c |
        0x02, /*!< CCCD with the given handle is not found in the Server's list for the current client. */
...
typedef enum
{
    ...
    gAdvDirectedHighDutyCycle_c =
        0x01, /*!< Answers only to connect requests; smaller advertising interval for quicker connection. */
...
typedef enum
{
    ...
    gBleAdvFilterAllowScanFromWLAllowConnFromWL_c =
        0x03, /*!< White List is used for both Scan and Connection Requests. */
...
typedef enum bleScanType_tag
{
    ...
    gScanTypeActive_c = 0x01 /*!< Active Scanning - the scanner sends scan requests to the advertiser and reports to the
                                Host after the scan response is received. */
} bleScanType_t;
...
typedef enum gapGenericEventType_tag
{
    ...
    gAdvertisingParametersSetupComplete_c, /*!< Advertising parameters have been successfully set. Response to
                                              Gap_SetAdvertisingParameters. */
    gAdvertisingDataSetupComplete_c, /*!< Advertising and/or scan response data has been successfully set. Response to
    ...
    gDeviceRemovedFromWhiteList_c,   /*!< Device has been removed from the White List. Response to
    ...
    gCreateConnectionCanceled_c, /*!< Connection initiation was successfully cancelled. Response to
    ...
    gAdvTxPowerLevelRead_c,      /*!< Contains the TX power on the advertising channel. Response to
                                    Gap_ReadAdvertisingTxPowerLevel. */
    gPrivateResolvableAddressVerified_c, /*!< Contains the result of PRA verification. Response to
    ...
    gLeScPublicKeyRegenerated_c,      /*!< The private/public key pair used for LE Secure Connections pairing has been
                                         regenerated. */
...
typedef struct gapInternalError_tag
{
    ...
    gapInternalErrorSource_t
        errorSource; /*!< The command that generated the error; useful when it is not obvious from the error code. */
    uint16_t hciCommandOpcode; /*!< Only for errorSource = gHciCommandStatus_c; the HCI Command that received an error
                                  status. */
} gapInternalError_t;
...
typedef struct gapControllerTestEvent_tag
{
    ...
    uint16_t
        receivedPackets; /* Valid only if testEventType == gControllerTestEnded_c and the test was a Receiver Test. */
...
typedef struct gapGenericEvent_tag
{
    ...
    union
    {
        gapInternalError_t internalError; /*!< Data for the gInternalError_c event. The error that has occurred and the
                                             command that triggered it. */
        uint8_t whiteListSize;            /*!< Data for the gWhiteListSizeReady_c event. The size of the White List. */
        bleDeviceAddress_t aAddress; /*!< Data for the gRandomAddressReady_c, gPublicAddressRead_c events. Contains the
        ...
        bool_t newControllerPrivacyState; /*!< Data for the gControllerPrivacyStateChanged_c event. TRUE if enabled,
                                             FALSE if disabled. */
        gapControllerTestEvent_t
            testEvent; /*!< Data for the gControllerTestEvent_c event. Contains test event type and received packets. */
    } eventData;       /*!< Event data, selected according to event type. */
} gapGenericEvent_t;

/*! Generic Callback prototype. */
typedef void (*gapGenericCallback_t)(gapGenericEvent_t *pGenericEvent /*!< Generic event. */
);

/*! Host-to-Controller API prototype. */
typedef bleResult_t (*hciHostToControllerInterface_t)(hciPacketType_t packetType, /*!< HCI Packet Type. */
