...
#define mGattDbDynamic_GapServiceAppearanceInitValue                  {0x00, 0x00}

#define mGattDbDynamic_BServiceLevelInitValue                         {0x5A}
#define mGattDbDynamic_BServiceCharPresFormatDescriptorInitValue      {0x04, 0x00, 0xAD, 0x27, 0x01, 0x01, 0x00}
...
*************************************************************************************
************************************************************************************/
const uint8_t uuid_service_wireless_uart[16] = {
    0xE0, 0x1C, 0x4B, 0x5E, 0x1E, 0xEB, 0xA1, 0x5C, 0xEE, 0xF4, 0x5E, 0xBA, 0x00, 0x01, 0xFF, 0x01
};

const uint8_t uuid_uart_stream[16] = {
    0xE0, 0x1C, 0x4B, 0x5E, 0x1E, 0xEB, 0xA1, 0x5C, 0xEE, 0xF4, 0x5E, 0xBA, 0x01, 0x01, 0xFF, 0x01
};
...
bleResult_t GattDbDynamic_AddServiceInDatabase(serviceInfo_t* pServiceInfo)
{
    ...
    /* Add service */
    result = GattDbDynamic_AddPrimaryServiceDeclaration(pServiceInfo->uuidType, 
                                                        (bleUuid_t*)&pServiceInfo->uuid, 
                                                        &pServiceInfo->handle);
    ...
    if( gBleSuccess_c == result )
    {
        for( i = 0; i < pServiceInfo->nbOfCharacteristics; i++ )
        {
            ...
            result = GattDbDynamic_AddCharacteristicDeclarationAndValue(pCharacteristicInfo->uuidType,
                                                                        (bleUuid_t*)&pCharacteristicInfo->uuid,
            ...
            else
            {
                for( j = 0; j < pCharacteristicInfo->nbOfDescriptors; j++ )
                {
                    ...
                    result = GattDbDynamic_AddCharacteristicDescriptor(pDescriptorInfo->uuidType,
                                                                       (bleUuid_t*)&pDescriptorInfo->uuid,
...
bleResult_t GattDbDynamic_AddGapService(gapServiceHandles_t* pOutServiceHandles)
{
    ...
    if( NULL != pServiceInfo )
    {
        ...
        pServiceInfo->pCharacteristicInfo[0].pValue             = (uint8_t*)pServiceInfo->pCharacteristicInfo + 2 * sizeof(characteristicInfo_t);        
...
serviceInfo_t* GattDbDynamic_GetWirelessUartService()
{
    ...
    if( NULL != pServiceInfo )
    {
        ...
        FLib_MemCpy(pServiceInfo->pCharacteristicInfo[0].uuid.uuid128, (void *)uuid_uart_stream, sizeof(uuid_uart_stream));
