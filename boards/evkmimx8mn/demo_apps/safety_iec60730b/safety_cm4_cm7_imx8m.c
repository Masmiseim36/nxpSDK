...
******************************************************************************/
#if (defined(__GNUC__) && ( __ARMCC_VERSION >= 6010050)) /* KEIL */
    extern uint32_t crcPostbuild; /* defined in main_imx8m.c */
...
void SafetyFlashTestInit(flash_runtime_test_parameters_t *psFlashCrc, flash_configuration_parameters_t *psFlashConfig)
{
    ...
    psFlashConfig->endAddress = (uint32_t)FLASH_TEST_END_ADDRESS + 1;
...
{
    /* 3rd argument is 0 because i.MX8M doesn't contain HW CRC module */
    psSafetyCommon->FLASH_test_result = FS_CM4_CM7_FLASH_SW32(psFlashConfig->startAddress, psFlashConfig->size, 0, psFlashConfig->startConditionSeed);
...
{
    /* CRC calculation for a given block of Flash memory, 3rd argument is 0 because i.MX8M doesn't contain HW CRC module */
    psFlashCrc->partCrc = FS_CM4_CM7_FLASH_SW32(psFlashCrc->actualAddress, psFlashCrc->blockSize, 0, psFlashCrc->partCrc);
...
void SafetyDigitalOutputTest(safety_common_t *psSafetyCommon, fs_dio_test_imx_t *pTestedPin)
{
    ...
    /* Output test */
    psSafetyCommon->DIO_output_test_result = FS_DIO_Output_IMX8M(pTestedPin, DIO_WAIT_CYCLE);
...

{
    ...
    psSafetyCommon->DIO_short_test_result = FS_DIO_ShortToAdjSet_IMX8M(pTestedPin, pAdjPin, PinValue, DIO_BACKUP);
    ...
    psSafetyCommon->DIO_input_test_result = FS_DIO_InputExt_IMX8M(pTestedPin, pAdjPin, PinValue, DIO_BACKUP);    
...
void SafetyDigitalInputOutput_ShortSupplyTest(safety_common_t *psSafetyCommon, fs_dio_test_imx_t *pTestedPin, uint8_t polarity)
{
    ...
    psSafetyCommon->DIO_short_test_result = FS_DIO_ShortToSupplySet_IMX8M(pTestedPin, polarity, DIO_BACKUP);
    if((psSafetyCommon->DIO_short_test_result) == FS_FAIL_DIO)
    {
        psSafetyCommon->safetyErrors |= DIO_TEST_ERROR;
        SafetyErrorHandling(psSafetyCommon);
    ...
    psSafetyCommon->DIO_input_test_result = FS_DIO_InputExt_IMX8M(pTestedPin, pTestedPin, polarity, DIO_BACKUP);
    if((psSafetyCommon->DIO_input_test_result) == FS_FAIL_DIO)
    {
        psSafetyCommon->safetyErrors |= DIO_TEST_ERROR;
        SafetyErrorHandling(psSafetyCommon);
