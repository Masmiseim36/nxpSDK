...
#include "project_setup_evkmimxrt1010.h"
...
void WatchdogEnable(void)   
{
    ...
    /* First disable RTWDOG */
    RTWDOG->CNT = 0xD928C520U; /* 0xD928C520U is the update key */
    RTWDOG->TOVAL = 0xFFFF;
    RTWDOG->CS = (uint32_t) ((RTWDOG->CS) & ~RTWDOG_CS_EN_MASK) | RTWDOG_CS_UPDATE_MASK;

    /* Wait for configure RTWDOG */
    for (uint32_t temp = 0; temp < 100000U; temp++)
    {
        __NOP();
    }
    
    USED_WDOG->CNT = RTWDOG_UPDATE_KEY;
    while ((USED_WDOG->CS & RTWDOG_CS_ULK_MASK) == 0)
    {
    }

    USED_WDOG->WIN   = 0;
    USED_WDOG->TOVAL = WATCHDOG_TIMEOUT_VALUE; /* Set timeout */
    
    uint32_t value        = 0U;
    
    /* Enable rtwdog, LPO clock, interrupt disabled, update enabled, 32b refresh, window mode, prescaler 255 enabled/disabled */
    value = RTWDOG_CS_EN(1U) | RTWDOG_CS_CLK(1U) |
            RTWDOG_CS_INT(0) | RTWDOG_CS_WIN(0) |
            RTWDOG_CS_UPDATE(1U) | RTWDOG_CS_DBG(0) |
            RTWDOG_CS_STOP(0) | RTWDOG_CS_WAIT(0) |
            RTWDOG_CS_PRES(0) | RTWDOG_CS_CMD32EN(1U) | RTWDOG_CS_TST(0);
...
void SerialInit(void)
{
     ...
     * LPUART_1_config.parityMode = kUART_ParityDisabled;
     * LPUART_1_config.stopBitCount = kUART_OneStopBit;
     * LPUART_1_config.txFifoWatermark = 0;
     * LPUART_1_config.rxFifoWatermark = 1;
