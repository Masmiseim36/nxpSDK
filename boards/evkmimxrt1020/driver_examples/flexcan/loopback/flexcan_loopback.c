 ...
 * if RX FIFO enables (RFEN bit in MCE set as 1) and RFFN in CTRL2 is set default as zero, the first valid TX MB Number
 * shall be 8; if RX FIFO enables (RFEN bit in MCE set as 1) and RFFN in CTRL2 is set by other values (0x1~0xF), the
 * user should consider to detail the first valid MB number; if RX FIFO disables (RFEN bit in MCE set as 0) , the first
 * valid MB number would be zero.
 */
