...
GPIO_HANDLE_DEFINE(g_GpioHandlePower);
/*******************************************************************************
 * Code
 ******************************************************************************/
...
void BOARD_SDCardDetectCallback(void *param)
{
...
void BOARD_SDCardDetectInit(sd_cd_t cd, void *userData)
{
    ...
    if (BOARD_SDMMC_SD_CD_TYPE == kSD_DetectCardByGpioCD)
    {
        ...
        config.port      = 2;
        config.pin       = BOARD_SDMMC_SD_CD_GPIO_PIN;
        config.level     = 0;
        HAL_GpioInit((hal_gpio_handle_t)g_GpioHandle, &config);
        HAL_GpioInstallCallback((hal_gpio_handle_t)g_GpioHandle, BOARD_SDCardDetectCallback, s_cd.userData);
        HAL_GpioGetInput((hal_gpio_handle_t)g_GpioHandle, &pinState);
        if (pinState != 0)
        {
            if (cd != NULL)
...
void BOARD_SDCardPowerResetInit(void)
{
    ...
    config.port      = 1;
    config.pin       = BOARD_SDMMC_SD_POWER_RESET_GPIO_PIN;
    config.level     = 1;
    HAL_GpioInit((hal_gpio_handle_t)g_GpioHandlePower, &config);
}
...
void BOARD_SDCardPowerControl(bool enable)
{
    if (enable)
    {
        HAL_GpioSetOutput((hal_gpio_handle_t)g_GpioHandlePower, 1);
    }
    else
    {
        HAL_GpioSetOutput((hal_gpio_handle_t)g_GpioHandlePower, 0);
    }
}
