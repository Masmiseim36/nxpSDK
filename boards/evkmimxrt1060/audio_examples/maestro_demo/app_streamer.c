...
#ifdef EAP_PROC
#include "eap_proc.h"
static post_proc_args eap_args;
#endif
...
status_t STREAMER_mic_Create(streamer_handle_t *handle)
{
    ...
    POST_PROCESS_DESC_T vit_proc = {VIT_Initialize_func, VIT_Execute_func, VIT_Deinit_func, NULL, 0};
...
status_t STREAMER_file_Create(streamer_handle_t *handle, char *filename, char *eap_par)
{
    STREAMER_CREATE_PARAM params;
    OsaThreadAttr thread_attr;
    int ret;
