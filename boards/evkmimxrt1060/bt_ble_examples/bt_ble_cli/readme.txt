...
NOTE:
1. For GAP PTS test cases, enable "PTS" only with no other server macros defined in preprocessor settings
...
10. Flash nRF5340 audio dk before test
11. To test any BT profile along with A2DP sink/source test, it is required to update link-policy to "0001" (no-sniff) during bt_ble_cli Init.
	>> After option-3, enter option-11->47, select default link-key policy setting:1, enter setting value:0001
...
enable advertise
nRF5340:random address:112233445566
Laird:public
Connect from remote device running as UMS

After Subevent : HCI_LE_CIS_ESTABLISHED_SUBEVENT.
...
Address type:0 public if remote device is Laird, 1 random if remote device is nRF5340
...
27. Associate Source.
address is 00 00 00 00 00 00 if remote BMS is nRF5340
29. Enable Stream.
30. Disable Stream.

Here below is the guide to enble hci_uart on nRF5340 Audio DK to make use is as LE Audio controller.

Background:
nRF Connect SDK provide one LE Audio demo project : "ncs\nrf\applications\nrf5340_audio" for nRF5340 Audio DK; The project provide one LE Audio controller firmware:  "nrf\applications\nrf5340_audio\bin\ble5-ctr-rpmsg_xxxx.hex" for nRF5340 NET Core.
So we could enable hci_uart in nRF5340 App Core to make use of its LE Audio controller firmware.
 
Install nRF Connect SDK:
Doc for install the SDK: https://developer.nordicsemi.com/nRF_Connect_SDK/doc/latest/nrf/getting_started.html#
 
Enable hci_uart for nRF5340 Audio DK:
1, Flash LE Audio controller firmware to nRF5340 Audio DK's net core.
Run command below to flash the firmware:
nrfjprog --program nrf\applications\nrf5340_audio\bin\ble5-ctr-rpmsg_3251.hex --chiperase --coprocessor CP_NETWORK -r
Note:
...
2, Modify "zephyr\samples\bluetooth\hci_uart" project to use LE Audio controller firmware.
Add below config to "zephyr\samples\bluetooth\hci_uart\CMakeLists.txt":
zephyr_library_include_directories(app PRIVATE
    ${ZEPHYR_NRF_MODULE_DIR}/boards/arm/nrf5340_audio_dk_nrf5340)
Add below config to "zephyr\samples\bluetooth\hci_uart\prj.conf":
# Avoid multi-build system build network core firmware
CONFIG_NCS_INCLUDE_RPMSG_CHILD_IMAGE=n
# provide enough buffer to receive HCI packet from Host
CONFIG_BT_BUF_ACL_TX_SIZE=255
CONFIG_BT_ISO_TX_MTU=255
# enable anyone config to enable CONFIG_BT_ISO indirectly
CONFIG_BT_ISO_UNICAST=y
CONFIG_BT_ISO_BROADCASTER=y
CONFIG_BT_ISO_SYNC_RECEIVER=y
 
 
3, Build "zephyr\samples\bluetooth\hci_uart" project:
Switch to  "zephyr\samples\bluetooth\hci_uart" project path.
Run command: west build ./ -b nrf5340_audio_dk_nrf5340_cpuapp -p
 
4, Flash "zephyr\samples\bluetooth\hci_uart" project to nRF5340 Audio DK:
Switch to  "zephyr\samples\bluetooth\hci_uart" project path.
Run command: west flash
 
The flash log:
 
 
5, Verify "zephyr\samples\bluetooth\hci_uart" project:
The hci_uart project default use "Jlink CDC UART Port" as HCI UART interface with baudrate 115200.
1, Push nRF5340 Audio DK "RESET" button.
2, Send  "Read Local Supported Commands command" hex value "01 02 10 00", if everything right then you will receive response like below:
"04 0E 44 01 02 10 00 20 00 80 00 00 C0 00 0C 00 00 04 00 00 00 28 22 00 00 00 00 00 00 04 00 00 F7 FF FF 7F 00 00 00 30 F0 FF FF FF FF FF 1F E0 F7 FF FF 07 01 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00"
You could reference "BLUETOOTH CORE SPECIFICATION Version 5.2 | Vol 4, Part E" page 1912 | 6.27 SUPPORTED COMMANDS to check which ISO commands are supported.
 
6, Change UART baudrate:
1, Copy "zephyr\samples\bluetooth\hci_uart\boards\nrf5340dk_nrf5340_cpuapp.overlay" and rename to "zephyr\samples\bluetooth\hci_uart\boards\nrf5340_audio_dk_nrf5340_cpuapp.overlay".
2, Change "current-speed" value to any value you want, here its default value is 1M:
&uart0 {
...
3, Rebuild project then flash again.
 
7, Change UART interface:
The hci_uart project default use "Jlink CDC UART Port" as HCI UART interface with UART0.
Here we will change HCI UART interface to UART1 with PIN define:
UART1 PIN define:
UART_RX, P1.8
UART_TX, P1.9
UART_RTS, P1.10
UART_CTS, P1.11
1, Add UART1 pins to device tree:
Add below config to "nrf\boards\arm\nrf5340_audio_dk_nrf5340\nrf5340_audio_dk_nrf5340_cpuapp_common-pinctrl.dtsi"
&pinctrl {
...
2, Change "zephyr\samples\bluetooth\hci_uart" project's overlay config for "nrf5340_audio_dk_nrf5340_cpuapp".
Add below config to "zephyr\samples\bluetooth\hci_uart\boards\nrf5340_audio_dk_nrf5340_cpuapp.overlay"
&uart1 {
    compatible = "nordic,nrf-uarte";
    status = "okay";
    current-speed = <1000000>;
...
3, Rebuild the project, then you will find the interface changed to UART1.
...
Jumper Setting On Murata Adapter:

Connect J13[1-2] and  J12[2-3] for VIO 3.3V supply
Connect J1  [2-3] VBAT Supply from microSD connector


------------------------------------------------------------------------------------
PIN NAME	M2-ADAPTER REV B2	I.MXRT1060	PIN NAME	GPIO NAME OF RT1060
------------------------------------------------------------------------------------
UART_RXD	J9(pin 2)	        J22(pin 2)	LPUART3_RXD	GPIO_AD_B1_07
UART_TXD	J9(pin 1)	        J22(pin 1)	LPUART3_TXD	GPIO_AD_B1_06
UART_CTS	J8(pin 4)	        J23(pin 4)	LPUART3_CTS	GPIO_AD_B1_04
UART_RTS	J8(pin 3)	        J23(pin 3)	LPUART3_RTS	GPIO_AD_B1_05
GND	        J7(pin 6)	        J25(pin 7)	GND	        GND
------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------
PIN NAME	MURATA ADAPTER M2- REV B2	I.MXRT1060	PIN NAME	GPIO NAME OF RT1060
------------------------------------------------------------------------------------
...
To run HFP feature, you must remove J47,J48,J49,J50 jumper.
Prepare the Demo
================
...
1. Enable 2. Disable
	...
	- Event type: 0x0001
	- Address type: 0x0000
	- Address: 0xD4 0xBD 0x27 0xE8 0x07 0xC0 
...
ng - Legacy. [CAP - Acceptor, GAP - Peripheral] 
   21. Enable/Disable LE Advertising - Extended. [CAP - Acceptor, GAP - Peripheral] 
   22. Enable/Disable LE Scan - Legacy. [CAP - Initiator, GAP - Central] 
   23. Enable/Disable LE Scan - Extended. [CAP - Initiator GAP - Central]  
...
status = 0x00
connection_handle = 0x0000
y, Location, Supp/Avail Contexts 
...
Your Option ?: 28
Device: 
[BD ADDRESS]  : 0xD4 0xBD 0x27 0xE8 0x07 0xC0 
Initiator: 0x01
Index: 0x01
...
Status - 0x0000
Remote keys negotiated - 0x06
Encryption Key Size negotiated - 0x10
Encryption Info:
...
BD Address : D4 BD 27 E8 07 C0
BD addr type : Public Address
Local keys negotiated - 0x06
Encryption Key Size negotiated - 0x10
...
12
GA_sep_configure
retval in 1379 0Retval - GA_SUCCESS (0x0000)
...
Your Option ?: ************************** CALLBACK: GA Client **************************
[Profile]     : ASCS (0x184E)
[BD ADDRESS]  : 0xD4 0xBD 0x27 0xE8 0x07 0xC0 
[Event Status]: GA_SUCCESS (0x0000)
...
Retval - GA_SUCCESS (0x0000)
    ...
    5. ASE QoS Configuration. 
    6. ASE Enable. 
 ASE Receiver Start Ready for Local ASE Sink. 
    8. ASE Update. 
    9. ASE Disable. 
   10. ASE Receiver Stop Ready for Local ASE Sink. 
   11. ASE Release. 
...
*********************************************************************
	...
	LE Num Command Packets: 1 (0x01)
Status: 0x00
CIG_Sync_Delay: 0x00002388
CIS_Sync_Delay: 0x00002388
...
Command 2(Set Security Mode) and 3(Get Security Mode) under SM option are not supported if BT_SSP flag is enabled in BT feature
    ...
    8.9 Wake on BLE and OOB wakeup over GPIO
        ...
        Supported Hosts are RT1170/RT1060-EVKA/RT1060-EVKB and Controllers are FC RDB, 1XK/1ZM with uSD-M2 Adapter,
        Host will initialize the RT platform specific GPIOs used for Host to Controller and Controller to Host sleep and wakeup.
...
Received HCI_COMMAND_COMPLETE_EVENT in LE.
		...
		For RT1170-EVKA, J9.12 shall be connected with J10.13 (GPIO12) of FC-RD Board for H2C trigger.
		...
		For RT1170-EVKA, J9.6 shall be connected with HD3.16 (GPIO16) of FC-RD Board for H2C trigger.
