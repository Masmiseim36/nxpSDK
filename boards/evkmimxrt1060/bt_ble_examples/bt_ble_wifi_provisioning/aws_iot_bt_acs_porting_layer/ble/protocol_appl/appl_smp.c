...
/* Application compilation switch to enable auto confirmation of Numeric Comparison Value */
...
DECL_STATIC UCHAR g_appl_pair_mode = SMP_LEGACY_MODE;
#endif /* SMP_LESC */
...
#ifdef SMP_TBX_TEST_LESC_FUNCTIONS
DECL_STATIC UCHAR appl_smp_menu[] = "\n\
============== SMP MENU ================\n\
 0.  Exit.\n\
 1.  Refresh.\n\
 \n\
...
DECL_STATIC UCHAR appl_smp_pl_menu[] = "\n\
============= SMP PL MENU ==============\n\
 0.  Exit.\n\
 1.  Refresh.\n\
 \n\
...
/* global to hold Key Distribution Flag */
DECL_STATIC UCHAR initiator_keys = 0x00;
DECL_STATIC UCHAR responder_keys = 0x00;
...
#ifdef SMP_LESC_CROSS_TXP_KEY_GEN
DECL_STATIC void appl_smp_bredr_l2cap_connection_complete
            (
...
DECL_STATIC void appl_smp_bredr_l2cap_disconnection_complete
            (
                DEVICE_HANDLE  * bd_handle,
                void          * data_param,
                UINT16          datalen
...
DECL_STATIC void appl_smp_bredr_l2cap_data_read_ind
            (
                DEVICE_HANDLE  * bd_handle,
                void          * data_param,
                UINT16          datalen
...
API_RESULT appl_smp_bredr_send_command
           (
...
void appl_smp_bredr_l2cap_event_cb
     (
         DEVICE_HANDLE * bd_handle,
         UCHAR   event_type,
         void  * data_param,
...
void appl_smp_bredr_l2cap_init (void);
DECL_STATIC void appl_bredr_send_local_keys( UCHAR local_keys);
DECL_STATIC void appl_bredr_gen_ltk(void);

void (* DECL_CONST appl_smp_bredr_event_fp[]) (DEVICE_HANDLE * , void * , UINT16) DECL_REENTRANT =
{
    /* SMP BREDR L2CAP Connection Event - L2CAP_FIXED_CH_CONNECTED_EVENT */
    appl_smp_bredr_l2cap_connection_complete,

    /* SMP BREDR L2CAP Disconnection Event - L2CAP_FIXED_CH_DISCONNECTED_EVENT */
    appl_smp_bredr_l2cap_disconnection_complete,

    /* SMP BREDR L2CAP Data Read Event - L2CAP_FIXED_CH_DATA_RX_EVENT */
    appl_smp_bredr_l2cap_data_read_ind
};

#define APPL_BR_EDR_SMP_CID                  0x0007
#define APPL_BR_EDR_SMP_MTU                  23
#define APPL_BR_EDR_SMP_IDKEY_MASK           0x02
#define APPL_BR_EDR_SMP_SIGNKEY_MASK         0x04
#define APPL_BR_EDR_SMP_INITIATOR            0x01
#define APPL_BR_EDR_SMP_RESPONDER            0x02

/* Negotiated CT2/H7 support */
DECL_STATIC UCHAR appl_smp_ct2;
DECL_STATIC UCHAR appl_smp_h7;

/* SMP_CODE_PAIRING_REQUEST or SMP_CODE_PAIRING_RESPONSE */
DECL_STATIC UCHAR appl_smp_bredr_pair_reqres[] = {0x00, 0x00, 0x00, 0x10, 0x06, 0x06};

/* SMP_CODE_IDENTITY_INFORMATION */
DECL_STATIC UCHAR appl_smp_bredr_pair_irk[]    = {0x0A, 0x2D, 0xF4, 0x65, 0xE3, 0xBD, 0x7B, 0x49,
                                                  0x1E, 0xB4, 0xC0, 0x95, 0x95, 0x13, 0x46, 0x73};

/* SMP_CODE_IDENTITY_ADDRESS_INFORMATION */
DECL_STATIC UCHAR appl_smp_bredr_pair_idinfo[] = {0x00, 0x00, 0xC0, 0xFF, 0xEE, 0xDE, 0xDA};

/* SMP_CODE_SIGNING_INFORMATION */
DECL_STATIC UCHAR appl_smp_bredr_pair_csrk[]   = {0x90, 0xD5, 0x06, 0x95, 0x92, 0xED, 0x91, 0xD7,
                                                  0xA8, 0x9E, 0x2C, 0xDC, 0x4A, 0x93, 0x5B, 0xF9};

/* global Device Handle */
DECL_STATIC DEVICE_HANDLE g_smp_br_edr_hndl;

/* global to hold smp role */
/* TODO: Needs to come from HCI */
DECL_STATIC UCHAR appl_br_edr_smp_role;

/* global to hold the keys */
SMP_LESC_LK_LTK_GEN_PL g_smp_lk_ltk;

#define APPL_BREDR_SMP_PAIR_REQ()                \
        appl_smp_bredr_send_command              \
        (                                        \
            &g_smp_br_edr_hndl,                  \
            SMP_CODE_PAIRING_REQUEST,            \
            appl_smp_bredr_pair_reqres,          \
            sizeof(appl_smp_bredr_pair_reqres)   \
        )

#define APPL_BREDR_SMP_PAIR_RES()                \
        appl_smp_bredr_send_command              \
        (                                        \
            &g_smp_br_edr_hndl,                  \
            SMP_CODE_PAIRING_RESPONSE,           \
            appl_smp_bredr_pair_reqres,          \
            sizeof(appl_smp_bredr_pair_reqres)   \
        )

#define APPL_BREDR_SMP_PAIR_IRK()                \
        appl_smp_bredr_send_command              \
        (                                        \
            &g_smp_br_edr_hndl,                  \
            SMP_CODE_IDENTITY_INFORMATION,       \
            appl_smp_bredr_pair_irk,             \
            sizeof(appl_smp_bredr_pair_irk)      \
        )

#define APPL_BREDR_SMP_PAIR_ID_INFO()              \
        appl_smp_bredr_send_command                \
        (                                          \
            &g_smp_br_edr_hndl,                    \
            SMP_CODE_IDENTITY_ADDRESS_INFORMATION, \
            appl_smp_bredr_pair_idinfo,            \
            sizeof(appl_smp_bredr_pair_idinfo)     \
        )

#define APPL_BREDR_SMP_PAIR_SIGN()                \
        appl_smp_bredr_send_command               \
        (                                         \
            &g_smp_br_edr_hndl,                   \
            SMP_CODE_SIGNING_INFORMATION,         \
            appl_smp_bredr_pair_csrk,             \
            sizeof(appl_smp_bredr_pair_csrk)      \
        )

DECL_CONST DECL_STATIC UCHAR appl_smp_br_edr_menu[] = "\n\
============= SMP BR EDR MENU ==============\n\
0.  Exit\n\
1.  Refresh\n\
\n\
2.  Send Pairing Request\n\
3.  Send Identity Keys\n\
4.  Send Sign Key\n\
5.  Generate LTK from LK\n\
6.  Set CT2 Support\n\
\n\
Enter Your Choice: ";

#if 0
void appl_smp_br_edr_operations()
{
    unsigned int  choice;
    API_RESULT    retval;
    UINT32        i;
    ...
    BT_LOOP_FOREVER()
    {
        ...
        IotLogDebug("%s", appl_smp_br_edr_menu);
        CONSOLE_IN("%u", &choice);
        ...
        switch(choice)
        {
            ...
            case 2:
                IotLogDebug("Enter CT2 support (1/0):");
                CONSOLE_IN("%d", &choice);

                appl_smp_bredr_pair_reqres[2] = (choice) ? 0x20 : 0x00;

                IotLogDebug("Sending Pairing Request...\n");
                /* Set SMP Role */
                appl_br_edr_smp_role = APPL_BR_EDR_SMP_INITIATOR;

                APPL_BREDR_SMP_PAIR_REQ();
                break;

            case 3:
                IotLogDebug("Sending Identity Keys...\n");
                BT_hci_get_local_bd_addr((appl_smp_bredr_pair_idinfo + 1));
                APPL_BREDR_SMP_PAIR_IRK();
                APPL_BREDR_SMP_PAIR_ID_INFO();
                break;

            case 4:
                IotLogDebug("Sending Sign Key...\n");
                APPL_BREDR_SMP_PAIR_SIGN();
                break;
...
void appl_smp_bredr_l2cap_init (void)
{
    /* Register event handler callback for L2CAP fixed channel */
    l2cap_register_fixed_channel_cb
    (
        APPL_BR_EDR_SMP_CID,
        APPL_BR_EDR_SMP_MTU,
        appl_smp_bredr_l2cap_event_cb
    );
}
     ...
     )
{
    ...
    IotLogDebug (
    "[SMP] Received L2CAP Event 0x%02X\n", event_type);

    if (event_type > L2CAP_FIXED_CH_DATA_RX_EVENT)
    {
        IotLogError (
        "Received Invalid L2CAP Event 0x%02X\n", event_type);

        return;
    }

    /* Call the associated handler function pointer */
    appl_smp_bredr_event_fp[event_type - 1] (bd_handle, data_param, datalen);
}
                 ...
                 )
{
    ...
    cmd_pdu = (UCHAR *)data_param;

    /* Get the peer BD Address for the device handle received */
    device_queue_get_remote_addr (bd_handle, &bd_addr);

    IotLogDebug(
    "BD Handle: %02X, BD Addr: %02X:%02X:%02X:%02X:%02X:%02X, Type: %d, Length: %d\n",
    *bd_handle, bd_addr.addr[0], bd_addr.addr[1], bd_addr.addr[2],
    bd_addr.addr[3], bd_addr.addr[4], bd_addr.addr[5], bd_addr.type, datalen);

    /* Print the DATA Received */
    IotLogInfo ("Data:\n");
    appl_dump_bytes((UCHAR*)data_param, datalen);
    IotLogInfo ("\n");
    ...
    case SMP_CODE_PAIRING_REQUEST:
        /* Set SMP Role */
        appl_br_edr_smp_role = APPL_BR_EDR_SMP_RESPONDER;

        /* Extract and store Master Key Dist */
        initiator_keys = *(cmd_pdu + 5);
        responder_keys = *(cmd_pdu + 6);

        /* Update initiator and responder keys according to Peer request */
        appl_smp_bredr_pair_reqres[4] &= initiator_keys;
        appl_smp_bredr_pair_reqres[5] &= responder_keys;

        appl_smp_ct2 = ((appl_smp_bredr_pair_reqres[2] & 0x20) & ((*(cmd_pdu + 3)) & 0x20));

        /* Send Pairing Response */
        APPL_BREDR_SMP_PAIR_RES();
        if (APPL_BR_EDR_SMP_RESPONDER == appl_br_edr_smp_role)
        {
            appl_bredr_send_local_keys(responder_keys);
        }
        break;

    case SMP_CODE_PAIRING_RESPONSE:
        /* Extract and Store Master and Slave Key Dist */
        /* Extract and store Master Key Dist */
        initiator_keys = *(cmd_pdu + 5);
        responder_keys = *(cmd_pdu + 6);
    ...
    if (APPL_BR_EDR_SMP_RESPONDER == appl_br_edr_smp_role)
    {
        if (initiator_keys & APPL_BR_EDR_SMP_SIGNKEY_MASK)
        {
            /* wait for SMP_CODE_SIGNING_INFORMATION */
            if (opcode == SMP_CODE_SIGNING_INFORMATION)
            {
                /* End of Proccess */
                appl_bredr_gen_ltk();
            }
        }
        else if (initiator_keys & APPL_BR_EDR_SMP_SIGNKEY_MASK)
        {
            /* wait for SMP_CODE_IDENTITY_ADDRESS_INFORMATION */
            if (opcode == SMP_CODE_IDENTITY_ADDRESS_INFORMATION)
            {
                /* End of Proccess */
                appl_bredr_gen_ltk();
            }
    ...
    if (APPL_BR_EDR_SMP_INITIATOR == appl_br_edr_smp_role)
    {
        if (responder_keys & APPL_BR_EDR_SMP_SIGNKEY_MASK)
        {
            /* wait for SMP_CODE_SIGNING_INFORMATION */
            if (opcode == SMP_CODE_SIGNING_INFORMATION)
            {
                appl_bredr_send_local_keys(initiator_keys);

                /* End of Proccess */
                appl_bredr_gen_ltk();
            }
        }
        else if (responder_keys & APPL_BR_EDR_SMP_SIGNKEY_MASK)
        {
            /* wait for SMP_CODE_IDENTITY_ADDRESS_INFORMATION */
            if (opcode == SMP_CODE_IDENTITY_ADDRESS_INFORMATION)
            {
    ...
    /* TODO: Store the Key Exchanges negotiated here */
    return;
}
...
{
    API_RESULT retval;
    DECL_STATIC UCHAR link_key[16];

    SMP_BD_ADDR bdaddr;

    IotLogInfo("\n\n BREDR SMP PROCEDURE COMPLETED!!!\n\n");
    IotLogInfo("\n\n GENERATING SMP LE LTK \n\n");

    /* Get the BD Address from handle */
    BT_smp_get_bd_addr (&g_smp_br_edr_hndl, &bdaddr);

    /* Get the device's LK */
    retval = BT_sm_get_device_link_key (bdaddr.addr, link_key);

    BT_smp_get_ltk_from_lk_pl(link_key,appl_smp_lesc_txp_key_gen_complete, appl_smp_ct2);
}

DECL_STATIC void appl_bredr_send_local_keys( UCHAR local_keys)
{
    if (local_keys & APPL_BR_EDR_SMP_IDKEY_MASK)
    {
        /* Send ID Keys */
        APPL_BREDR_SMP_PAIR_IRK();
        APPL_BREDR_SMP_PAIR_ID_INFO();
    }
    if (local_keys & APPL_BR_EDR_SMP_SIGNKEY_MASK)
    {
        /* Send Sign Keys */
        APPL_BREDR_SMP_PAIR_SIGN();
    }
                 ...
                 )
{
    ...
    IotLogDebug("\nConnection Received for CID 0x07 from\n");

    IotLogDebug(
    "BD Handle: %02X, BD Addr: %02X:%02X:%02X:%02X:%02X:%02X, Type: %d\n",
    *bd_handle, bd_addr.addr[0], bd_addr.addr[1], bd_addr.addr[2],
    bd_addr.addr[3], bd_addr.addr[4], bd_addr.addr[5], bd_addr.type);

    /* Populate the global Handle */
    g_smp_br_edr_hndl = *bd_handle;
}
                 ...
                 )
{
    ...
    IotLogDebug("\nDisconnection Received for CID 0x07 from\n");

    IotLogDebug(
    "[SMP] BD Handle: %02X, BD Addr: %02X:%02X:%02X:%02X:%02X:%02X, Type: 0x%02X\n",
    *bd_handle, bd_addr.addr[0], bd_addr.addr[1], bd_addr.addr[2],
    bd_addr.addr[3], bd_addr.addr[4], bd_addr.addr[5], bd_addr.type);
}
           ...
           )
{
    ...
    /* Add L2CAP header size to command */
    datalen = L2CAP_HDR_LEN + 1 + param_len;

    /* Allocate memory for command */
    d_buf = BT_alloc_mem (datalen);
    ...
    /* Copy the Opcode here */
    *(d_buf + L2CAP_HDR_LEN) = opcode;

    /* Copy the command params */
    BT_mem_copy ((d_buf + L2CAP_HDR_LEN + 1), param, param_len);
    ...
    {
        IotLogError(
        "FAILED to send command with retval 0x%04X.\n", retval);
...
void smp_pl_operations(void)
{
    ...
    BT_LOOP_FOREVER()
    {
        ...
        switch(choice)
        {
            ...
            case 2:
#ifdef SMP_HAVE_OOB_SUPPORT
                IotLogInfo ("Enter peer device handle for OOB capability: ");
                CONSOLE_IN("%u", &read);
#endif /* SMP_HAVE_OOB_SUPPORT */
...
#else /* SMP_HAVE_OOB_SUPPORT */
                IotLogInfo ("'SMP_HAVE_OOB_SUPPORT' flag not defined\n");
#endif /* SMP_HAVE_OOB_SUPPORT */
            ...
            case 10:
                ...
                IotLogInfo("Made Device Untrusted with result 0x%04X",retval);
            ...
            case 21:
                ...
                IotLogInfo ("SMP_PL_ENABLE_KEY_GENERATION not defined\n");
            ...
            case 31:
                ...
                IotLogInfo ("Remove device - 0x%04X\n", retval);
#else /* SMP_HAVE_OOB_SUPPORT */
                IotLogInfo ("'SMP_HAVE_OOB_SUPPORT' flag not defined\n");
#endif /* SMP_HAVE_OOB_SUPPORT */
                break;
    ...
    BT_LOOP_FOREVER()
    {
        ...
        switch(choice)
        {
            ...
            case 2:
...
#ifdef SMP_LESC
                IotLogInfo("0 - Legacy LE SMP Procedure\n");
                IotLogInfo("1 - LE Secure Connections Mode\n");
                IotLogInfo("Enter SMP Pairing Mode :");
                CONSOLE_IN("%u", &read);
                ...
                if (0 != appl_smp_h7)
                {
                    auth.pair_mode |= SMP_CT2_SUPPORT_MASK;
                }
#endif /* SMP_LESC_CROSS_TXP_KEY_GEN */
#endif /* SMP_LESC */

                IotLogInfo(" 0 - Encryption Only/Unauthenticated (Without MITM)\n");
                IotLogInfo(" 1 - Authenticated (With MITM)\n");
                IotLogInfo("Enter Security level required : ");
                CONSOLE_IN("%u", &read);
                auth.security = (UCHAR) ((read)?SMP_SEC_LEVEL_2: SMP_SEC_LEVEL_1);

                IotLogInfo("\n0 - non-Bonding\n1 - Bonding\n");
                IotLogInfo("Enter Bonding type : ");
                CONSOLE_IN("%u", &read);
                auth.bonding = (UCHAR) read;

                IotLogInfo("Enter Encryption Key size required : ");
                CONSOLE_IN("%u", &read);
                ...
                IotLogInfo ("BT_smp_authenticate, Retval - 0x%04X\n", retval);
            ...
            case 3:
                ...
                IotLogInfo("Enter Authentication (0 - Accept/ErrorCode - Reject)  : ");
                CONSOLE_IN("%x", &read);
            ...
            case 5:
                ...
                IotLogInfo("Enter EDIV (UINT16 in HEX): ");
                CONSOLE_IN("%x", &read);
                ediv = read;

                IotLogInfo("Enter RAND (8 octet in HEX): ");
                ...
                IotLogInfo("Enter LTK (16 octet in HEX): ");
                for (i = 0; i < SMP_LTK_SIZE; i++)
                {
                    CONSOLE_IN("%x", &read);
                    ltk[i] = read;
                ...
                IotLogInfo("BT_smp_encrypt retval - 0x%04X\n", retval);
#else /* SMP_HAVE_ENCRYPT_API */
                IotLogInfo("SMP_HAVE_ENCRYPT_API not defined\n");
#endif /* SMP_HAVE_ENCRYPT_API */
                break;
            ...
            case 10:
                ...
                IotLogInfo("Enter BD_ADDR type (0: Public, 1: Random): ");
                CONSOLE_IN("%u", &read);
                bd_addr.type = (UCHAR) read;
#else /* APPL_SMP_AUTO_FILL_BD_ADDR */
                BT_COPY_BD_ADDR_AND_TYPE(&bd_addr,&g_bd_addr);
                ...
                IotLogInfo("1 - Accept/<ERROR> - Reject)  : ");
                CONSOLE_IN("%u", &read);
                accept = (UCHAR)read;
...
#else /* SMP_LESC */
                IotLogInfo("SMP_LESC Compilation Flag not defined!!!\n");
#endif /* SMP_LESC */
                break;
            ...
            case 21:
                IotLogInfo("Enter mode (0: Non-automatic, 1: Automatic) : ");
                CONSOLE_IN("%d", &read);
                automatic_mode = (UCHAR)read;
                break;

            case 22:
                IotLogInfo("Enter Pairable Mode (1/0): ");
                CONSOLE_IN("%d", &read);
            ...
            case 31:
                ...
                IotLogInfo("Enter IRK (16 octets): ");
            ...
            case 60:
#ifdef SMP_DATA_SIGNING
                {
                    ...
                    if (NULL != data)
                    {
                        IotLogInfo ("Enter the data (In Hex): ");
                        for (i = 0; i < datalen; i++)
                        {
                            CONSOLE_IN("%x", &choice);
                            data[i] = (UCHAR)choice;
                    ...
                    IotLogInfo ("Signing Counter: %d", sign_create_counter);
                ...
                IotLogInfo ("SMP_DATA_SIGNING not defined\n");
#endif /* SMP_DATA_SIGNING */
                break;
            ...
            case 61:
#ifdef SMP_DATA_SIGNING
                {
                    ...
                    IotLogInfo ("Enter signed data length: ");
                    CONSOLE_IN("%d", &choice);
                    datalen = (UINT16) choice;
                    data = (UCHAR *) BT_alloc_mem (datalen);
                    ...
                    IotLogInfo ("Signing Counter: %d", sign_verify_counter);
            ...
            case 70:
                ...
                aes_cmac_test();
#else /* AES_CMAC_UTIL */
                IotLogInfo ("AES_CMAC_UTIL not defined\n");
#endif /* AES_CMAC_UTIL */
                break;
            ...
            case 71:
                ...
                appl_smp_test_f4();
#else
                IotLogInfo ("SMP_TBX_TEST_LESC_FUNCTIONS not defined\n");
#endif /* SMP_TBX_TEST_LESC_FUNCTIONS */
                break;
            ...
            case 100:
#ifdef SMP_LESC_CROSS_TXP_KEY_GEN
                appl_smp_br_edr_operations();
#else /* SMP_LESC_CROSS_TXP_KEY_GEN */
                IotLogInfo("SMP_LESC_CROSS_TXP_KEY_GEN not defined\n");
#endif /* SMP_LESC_CROSS_TXP_KEY_GEN */
                break;

            case 101:
#ifdef SMP_LESC_CROSS_TXP_KEY_GEN
                IotLogInfo("Enter CT2 value (0/1): ");
                CONSOLE_IN("%d", &choice);
                appl_smp_h7 = (UCHAR)choice;
#else /* SMP_LESC_CROSS_TXP_KEY_GEN */
                IotLogInfo("SMP_LESC_CROSS_TXP_KEY_GEN not defined\n");
#endif /* SMP_LESC_CROSS_TXP_KEY_GEN */
                break;
           ...
           )
{
    ...
    case SMP_AUTHENTICATION_COMPLETE:
        ...
        if (NULL != event_data)
        {
            if(API_SUCCESS == status)
            {
                ...
                /* Added device to Resolving List if needed */
                ...
                if (auth->bonding)
                {
                    ...
                    retval = BT_hci_le_add_device_to_resolving_list
                             (
                                 peer_key_info.id_addr_info[0],
                                 &peer_key_info.id_addr_info[1],
        ...
        else
        {
            ...
            else if (SMP_REMOTE_SIDE_PIN_KEY_MISSING == status)
            {
                ...
                IotLogDebug("Marked Device Untrusted with result 0x%04X",retval);
    ...
    case SMP_AUTHENTICATION_REQUEST:
        ...
        if(SMP_TRUE == automatic_mode)
        {
            ...
            {
                if (SMP_LESC_MODE != (SMP_PAIRING_MODE_MASK & auth->pair_mode))
                {
    ...
    case SMP_KEY_EXCHANGE_INFO_REQUEST:
        ...
        /* Hack for PTS/USB - to delay data sending after encryption */
        BT_sleep (1);
...
#ifdef SMP_LESC_CROSS_TXP_KEY_GEN
                    /* TODO: Have proper interface for Handle Updation @ SMP
                     * based on
                     *   1. LE Connection Complete
                     *   2. BREDR Connection Complete
                     */
                    /*
                     *  Right now, the g_smp_br_edr_hndl is expected to be
                     *  populated only for BREDR link type Connections as,
                     *  seprate fixed channel callbacks are added at application
                     *  for the BREDR SMP Fixed channel.
                     */
                    if (g_smp_br_edr_hndl == *bd_handle)
                    {
                        /* If the Handle Matches, Copy the Generated LE LTK */
                        BT_mem_copy(ltk, g_smp_lk_ltk.ltk, 16);
                    }
#endif /* SMP_LESC_CROSS_TXP_KEY_GEN */
...
void appl_smp_test_f6 (void)
{
    ...
    IotLogInfo ("Dumping DH key\n");
    appl_dump_bytes (mackey, 16);
    IotLogInfo("Dumping L RAND\n");
    appl_dump_bytes (N1, 16);
    IotLogInfo("Dumping R RAND\n");
    appl_dump_bytes (N2, 16);
    IotLogInfo ("Dumping RAND\n");
    appl_dump_bytes (rand, 16);
    IotLogInfo("Dumping IO CAPAB\n");
    appl_dump_bytes (io_cap, 3);
    IotLogInfo ("Dumping l_bd_addr\n");
    appl_dump_bytes (l_bd_addr.addr, 6);
    IotLogInfo ("Dumping r_bd_addr\n");
     ...
     )
 {
    ...
    switch(aes_op)
    {
        case SMP_AES_CMAC_SIGN_OP:
            IotLogInfo ("SMP_AES_CMAC_SIGN_OP\n");
            break;

        case SMP_AES_CMAC_LESC_CONF_VAL_GEN_OP:
            IotLogInfo ("SMP_AES_CMAC_LESC_CONF_VAL_GEN_OP\n");
            break;

        case SMP_AES_CMAC_LESC_T_KEY_GEN_OP:
            IotLogInfo ("SMP_AES_CMAC_LESC_T_KEY_GEN_OP\n");
        ...
        case SMP_AES_CMAC_LESC_LTK_GEN_OP:
            IotLogInfo ("SMP_AES_CMAC_LESC_LTK_GEN_OP\n");
            appl_smp_test_f5_mackey(appl_smp_lesc_test_f5_T);
            break;

        case SMP_AES_CMAC_LESC_MACKEY_GEN_OP:
            IotLogInfo ("SMP_AES_CMAC_LESC_MACKEY_GEN_OP\n");
            break;

        case SMP_AES_CMAC_LESC_CHK_VAL_GEN_OP:
            IotLogInfo ("SMP_AES_CMAC_LESC_CHK_VAL_GEN_OP\n");
            break;

        case SMP_AES_CMAC_LESC_NUM_COMP_VAL_GEN_OP:
            IotLogInfo ("SMP_AES_CMAC_LESC_NUM_COMP_VAL_GEN_OP\n");
            break;

        case SMP_AES_CMAC_LESC_LINK_KEY_CONV_OP:
            IotLogInfo ("SMP_AES_CMAC_LESC_LINK_KEY_CONV_OP\n");
            break;
