...
int draw_clock(vg_lite_buffer_t * buffer, vg_lite_color_t back_color)
{
    ...
    {
        error = vg_lite_draw(buffer, &bg_data[count], VG_LITE_FILL_NON_ZERO, &matrix, VG_LITE_BLEND_SRC_OVER, bg_color[count]);
        if (error)
        {
    ...
    {
        error = vg_lite_draw(buffer, &h_data[count], VG_LITE_FILL_NON_ZERO, &matrix, VG_LITE_BLEND_SRC_OVER, h_color[count]);
        if (error)
        {
    ...
    {
        error = vg_lite_draw(buffer, &m_data[count], VG_LITE_FILL_NON_ZERO, &matrix, VG_LITE_BLEND_SRC_OVER, m_color[count]);
        if (error)
        {
    ...
    {
        error = vg_lite_draw(buffer, &s_data[count], VG_LITE_FILL_NON_ZERO, &matrix, VG_LITE_BLEND_SRC_OVER, s_color[count]);
        if (error)
        {
