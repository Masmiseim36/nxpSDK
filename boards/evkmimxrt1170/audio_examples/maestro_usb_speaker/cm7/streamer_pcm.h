 ...
 *
 * This function is called from the streamer to output audio data.
 *
 * @param pcm Pointer to PCM interface handle
...
/*!
 * @brief Read audio data from the receive PCM interface (USB audio 2.0 speaker device)
 *
 * This function reads raw PCM data from the audioPlayDataBuff
 *
 * This function is called from the streamer to input raw audio data.
 *
 * @param pcm Pointer to PCM interface handle
 * @param data Pointer to data buffer to store the PCM data
 * @param next_buffer Pointer to next data buffer to store the next PCM data
 * @param size Size in bytes of the data buffer
 * @return 0 on succes, non-zero on failure
 */
