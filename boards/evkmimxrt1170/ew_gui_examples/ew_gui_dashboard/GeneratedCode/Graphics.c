  ...
  aSrcPath, XInt32 aSrcSubPathNo, XInt32 aSrcNodeNo, XInt32 aNoOfEdges )
{
  ...
  dstHandle = _this->path;
  srcHandle = aSrcPath->path;
  result = 0;
  {
    result = EwAddSubPathCopy((XPath*)dstHandle, aSubPathNo, (XPath*)srcHandle,
                               aSrcSubPathNo, aSrcNodeNo, aNoOfEdges );
  }
  return result;
}
  ...
  XFloat aScaleX, XFloat aScaleY, XFloat aDeltaX, XFloat aDeltaY, XFloat aAngle )
{
  ...
  {
    EwFreePath((XPath*)handle );
    handle = (XHandle)EwCreatePathFromSVGString( aSVGString, aScaleX, aScaleY, aDeltaX, aDeltaY, aAngle );
    status = EwGetSVGParserStatus();
  }
  _this->path = handle;
  EwPostSignal( EwNewSlot( _this, GraphicsPath_onUpdate ), ((XObject)_this ));

  if ( status == 1 )
    return GraphicsSVGParserStatusEmptyString;

  if ( status == 2 )
    return GraphicsSVGParserStatusSyntaxError;

  if ( status == 3 )
    return GraphicsSVGParserStatusInvalidArgument;

  if ( status == 4 )
    return GraphicsSVGParserStatusOutOfMemory;

  return GraphicsSVGParserStatusSuccess;
}
