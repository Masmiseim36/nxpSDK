...
void CoreRotateTouchHandler__Init( CoreRotateTouchHandler _this, XObject aLink, XHandle aArg )
{
  ...
  /* Allow the Immediate Garbage Collection to evalute the members of this class. */
  _this->_GCT = EW_CLASS_GCT( CoreRotateTouchHandler );
  ...
  aEvent )
{
      ...
      && ( _this->state > 0 )) && ( _this->state < 33554432 ))
  {
    ...
    if (((( event3 != 0 ) && event3->Down ) && ( event3->Target != (CoreView)_this )) 
        && EwIsPointInRect( _this->Super1.Bounds, CoreGroup_LocalPosition( _this->Super2.Owner, 
  ...
  if (( press || release ) || hold )
  {
    ...
    _this->HittingPos = event1->HittingPos;
    _this->Down = (XBool)( press || hold );
    pos = EwMovePointNeg( EwMovePointNeg( event1->CurrentPos, EwGetRectCenter( _this->Super1.Bounds )), 
    _this->CenterOffset );
  }
  ...
  if ( drag )
  {
    ...
    _this->HittingPos = event2->HittingPos;
    _this->Down = 1;
    pos = EwMovePointNeg( EwMovePointNeg( event2->CurrentPos, EwGetRectCenter( _this->Super1.Bounds )), 
    _this->CenterOffset );
  }
...
void CoreRotateTouchHandler_OnSetCenterOffset( CoreRotateTouchHandler _this, XPoint 
  value )
{
  if ( !EwCompPoint( value, _this->CenterOffset ))
    return;
...
EW_DEFINE_CLASS( CoreRotateTouchHandler, CoreRectView, OnRotate, OnRotate, stateRetargetReason, 
                 stateRetargetReason, stateRetargetReason, stateRetargetReason, 
...
/* 'C' function for method : 'Core::SlideTouchHandler.OnSetRubberBandEffectDuration()' */
void CoreSlideTouchHandler_OnSetRubberBandEffectDuration( CoreSlideTouchHandler _this, 
  XInt32 value )
{
  if ( value < 1 )
    value = 1;

  _this->RubberBandEffectDuration = value;
}
