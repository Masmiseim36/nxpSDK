...
static void convert_id_to_label(char* label, size_t label_size, uint32_t id);
...
static TlsTransportStatus_t prvConnectToServerWithBackoffRetries( NetworkContext_t * pxNetworkContext )
{
    ...
    xServerInfo.pHostName = (char *) AZURE_IOT_MQTT_HOST;
    xServerInfo.hostNameLength = sizeof( AZURE_IOT_MQTT_HOST ) - 1U;
    xServerInfo.port = AZURE_IOT_MQTT_PORT;
   ...
   // xSocketConfig.enableTls = true;
	...
	convert_id_to_label(certLabel, sizeof(certLabel), pex_sss_demo_tls_ctx->client_cert_index);
	convert_id_to_label(keyLabel, sizeof(keyLabel), pex_sss_demo_tls_ctx->client_keyPair_index);
    /* Set the credentials for establishing a TLS connection. */
    xNetworkCredentials.pRootCa = ( unsigned char * ) AZURE_SERVER_ROOT_CERTIFICATE_PEM;
    xNetworkCredentials.rootCaSize = AZURE_SERVER_ROOT_CERTIFICATE_PEM_LENGTH;
                                 ...
                                 MQTTEventCallback_t eventCallback)
{
    ...
    else
    {
        ...
        {   
            ...
            xConnectInfo.pClientIdentifier = (const char *)AZURE_IOT_MQTT_CLIENT_ID;
            xConnectInfo.clientIdentifierLength = (uint16_t)strlen((const char *)AZURE_IOT_MQTT_CLIENT_ID);

            /* Use the metrics string as username to report the OS and MQTT client version
             * metrics to AWS IoT. */
            xConnectInfo.pUserName = AZURE_IOT_MQTT_USERNAME;
            xConnectInfo.userNameLength = (uint16_t)strlen(AZURE_IOT_MQTT_USERNAME);
...
void azurePubSub(void)
{
    ...
    xResult = SubscribeToTopic( &xMqttContext,
            ...
            strlen(AZURE_IOT_MQTT_SUB_TOPIC) );
    if (xResult != pdPASS) {
    ...
    xResult = PublishToTopic( &xMqttContext,
            AZURE_IOT_MQTT_PUB_TOPIC,
            strlen(AZURE_IOT_MQTT_PUB_TOPIC),
            cPayload,
            strlen(cPayload) );
    if (xResult != pdPASS) {
...
exit:
    if (xResult != pdPASS) {
        LOG_E("AZURE demo failed");
    }
    else {
        LOG_I("AZURE demo passed");
    }
