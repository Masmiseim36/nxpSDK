...
uint8_t *bundleInpAddr = GLOW_GET_ADDR(mutableWeight, CIFAR10_data);
uint8_t *bundleOutAddr = GLOW_GET_ADDR(mutableWeight, CIFAR10_softmax);
...
void nn_app_task(void *param)
{
    ...
    for (int idx = 0; idx < NUM_IMAGES; idx++)
    {
        ...
        tic = get_ccount();
        cifar10(WEIGHT_DATA_START, mutableWeight, activations);
        toc     = get_ccount();
