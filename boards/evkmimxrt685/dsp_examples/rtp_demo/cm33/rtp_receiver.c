...
static void rtp_receiver_join_group(const ip_addr_t *group)
{
    uint8_t mac_addr[6];
    int result;
    ...
    mac_addr[1] = 0x00U;
    mac_addr[2] = 0x5EU;
    mac_addr[3] = (group->addr >> 8) & 0x7FU;
    mac_addr[4] = (group->addr >> 16) & 0xFFU;
    mac_addr[5] = (group->addr >> 24) & 0xFFU;

    result = wifi_add_mcast_filter(&mac_addr[0]);
    LWIP_ASSERT("failed to join group", result == WM_SUCCESS);
}
...
static void rtp_receiver_task(void *param)
{
    ...
    if (ip_addr_ismulticast(&mcast_address))
    {
        ...
        /* Wi-Fi interface lwIP adaptation layer does not automatically join group. */
        rtp_receiver_join_group(&mcast_address);
    }
