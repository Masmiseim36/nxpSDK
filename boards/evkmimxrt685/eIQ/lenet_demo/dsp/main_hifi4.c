...
/* Internal functions */
static int my_ept_read_cb(void *payload, unsigned int payload_len, unsigned long src, void *priv)
{
...
int handleMSG_GENERAL(void)
{
    ...
    switch (msg.DAT.head.command)
    {
        ...
        case SRTM_Command_ECHO:
            ...
            PRINTF("NNLib Lib version %d.%d\r\n", msg.DAT.param[6] >> 16, msg.DAT.param[6] & 0xFF);
            break;
...
int handleMSG(void)
{
    ...
    {
        PRINTF("Incoming message is NOT a request! \n");
        return -1;
    }
...
int SCHEDULER_MULTICORE(void)
{
    ...
    {
        PRINTF("rpmsg NOT link my_rpmsg\n");
    }
    PRINTF("Initialized\n");
    ...
    while (1)
    {
        if (rpmsg_user_data.has_received)
        {
            ...
            if (status == 0)
            {
                ...
                PRINTF("Response message sent msg_count=%d \r\n", msg_count);
            }
        }
        else
        {
