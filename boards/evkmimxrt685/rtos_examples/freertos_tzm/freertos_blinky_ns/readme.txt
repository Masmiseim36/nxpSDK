...
3.  Use secure project to download the program to target board
    For IAR environment go to project options/Debugger/Images. Open file dialog for first extra image. Select file trustzone_blinky_ns.out and click on open button.
    Now you have absolute path in the path box. The flash loader requires absolute path to this file.
4.  Launch the debugger in your IDE to begin running the demo.

5.  For debugging with GDB server using the ARMGCC binary use following steps:
    5.1     Run J-link GDB server, connect to the RT600 device
    5.2     Run arm-none-eabi-gdb.exe from the location of your GNU tools ARM embedded install directory
    5.3     Use following commands to connect to the GDB server, load the binaries and run the example:
        ...
        file freertos_blinky_ns/armgcc/debug/freertos_blinky_ns.elf

        load

        monitor halt

        load freertos_blinky_s/armgcc/debug/freertos_blinky_s.elf

        add-symbol-file freertos_blinky_s/armgcc/debug/freertos_blinky_s.elf 0x10080144
