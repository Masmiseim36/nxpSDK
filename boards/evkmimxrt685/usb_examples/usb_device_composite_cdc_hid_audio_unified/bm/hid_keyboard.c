...
usb_status_t USB_DeviceHidKeyboardAction(void)
{
    if (g_ButtonPress)
    {
        g_UsbDeviceComposite->hidKeyboard.buffer[0] = 0x04U;
        g_ButtonPress                               = false;
        return USB_DeviceHidSend(g_UsbDeviceComposite->hidKeyboard.hidHandle, USB_HID_KEYBOARD_ENDPOINT,
                                 g_UsbDeviceComposite->hidKeyboard.buffer, USB_HID_KEYBOARD_REPORT_LENGTH);
    }
    else if (g_CodecMuteUnmute)
    {
        g_UsbDeviceComposite->hidKeyboard.buffer[0] = 0x00U;
...
usb_status_t USB_DeviceHidKeyboardInit(usb_device_composite_struct_t *deviceComposite)
{
    ...
    g_UsbDeviceComposite->hidKeyboard.buffer = s_KeyboardBuffer;
    return kStatus_USB_Success;
}
