...
status_t A_Format_Get_ID_Parse(encoder_a_format_t *enc, a_format_res2_t *res, uint32_t *id)
{
    ...
    memcpy(&temp, res->DF, sizeof(uint32_t));
    *id = GET_ENCODER_ID(temp);
    return kStatus_Success;
}
                                          ...
                                          a_format_abs_multi_t *multiData)
{
    ...
    for (uint8_t i = 0; i < nEncoder; i++)
    {
        ...
        memcpy(&temp, res[i].DF, sizeof(uint32_t));
        multiData[i].multiTurn = (uint16_t)((temp >> (enc->singleTurnRevolution - 16)) & enc->multi_turn_sign_mask);
    }
                                           ...
                                           a_format_abs_single_t *single_data)
{
    ...
    for (uint8_t i = 0; i < nEncoder; i++)
    {
        ...
        memcpy(&temp, res[i].DF, sizeof(uint32_t));
        single_data[i].singleTurn = temp & enc->single_turn_sign_mask;
    }
                                                 ...
                                                 a_format_abs_multi_single_t *abs_data)
{
    ...
    for (uint8_t i = 0; i < nEncoder; i++)
    {
        ...
        memcpy(&temp, res[i].DF, sizeof(uint32_t));
        abs_data[i].singleTurn = temp & enc->single_turn_sign_mask;
        memcpy(&temp, &(res[i].DF[1]), sizeof(uint32_t));
        abs_data[i].multiTurn  = (uint16_t)((temp >> (enc->singleTurnRevolution - 16)) & enc->multi_turn_sign_mask);
    }
