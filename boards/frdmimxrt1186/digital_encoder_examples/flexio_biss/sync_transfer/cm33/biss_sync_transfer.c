...
void BOARD_FLEXIO_IRQHandler(void)
{
    status_t status;

    status = FLEXIO_BISS_ReadNonBlocking(&encDev, dataBuffer, BISS_FRAME_BUFFER_NUM);
    if (status == kStatus_Success)
    {
        revFlag = 2;
    }
...
int main(void)
{
    ...
    FLEXIO_BISS_UpdateTriggerMode(&encDev, kFLEXIO_BISS_USERMODE_SYNC);
    FLEXIO_BISS_EnableInterrupts(&encDev);
    EnableIRQ(BOARD_FLEXIO_IRQn);
    PWM_Trigger_Init(BOARD_PWM_INSTANCE);

    FLEXIO_BISS_GenClock(&encDev, FLEXIO_BISS_CalClockValue(&frame_g, false));
