...
/* RX MB individual mask, which will make FLEXCAN to check full 1~11 bits of the ID of the received message. */
#define RX_MB_ID_FULL_MASK 0x7FFUL
...
static void User_TransferHandleIRQ(CAN_Type *base)
{
    ...
    do
    {
                                ...
                                (uint32_t)kFLEXCAN_BusOffIntFlag | (uint32_t)kFLEXCAN_ErrorIntFlag)))
        {
            ...
            FLEXCAN_ClearStatusFlags(base, (uint32_t)kFLEXCAN_TxWarningIntFlag | 
                                           (uint32_t)kFLEXCAN_RxWarningIntFlag |
                                           (uint32_t)kFLEXCAN_BusOffIntFlag | 
        ...
        else
        {
            ...
            if (0U != FLEXCAN_GetMbStatusFlags(base, 1U << RX_QUEUE_BUFFER_END_1))
            {
                ...
                {
                    /* 
                     * Rx MB default matched ID is 0x21, and after masked by 0xFF, it can match ID 0x321.
                     * Update queue 1 MB mask to 0x7FF, it can only match ID 0x21, so queue 1 will ignore
                     * the next CAN/CANFD messages and queue 2 start to receive the messages.
                     */
            ...
            else if (0U != FLEXCAN_GetMbStatusFlags(base, 1U << RX_QUEUE_BUFFER_END_2))
            {
                /* Queue 2 end Message Buffer interrupt. */
                rxQueue2Flag = 1U;

                /* Restore queue 1 ID mask to 0xFF, make it match ID 0x321 again. */
                for (i = 0; i < RX_QUEUE_BUFFER_SIZE; i++)
                    FLEXCAN_SetRxIndividualMask(EXAMPLE_CAN, RX_QUEUE_BUFFER_BASE + i,
    ...
    while (
        (0U != FLEXCAN_GetMbStatusFlags(base, (1U << RX_QUEUE_BUFFER_END_1) | (1U << RX_QUEUE_BUFFER_END_2))) ||
...
int main(void)
{
    ...
    else
    {
        ...
        mbConfig.id     = FLEXCAN_ID_STD(RX_MB_ID);
        ...
        FLEXCAN_EnableMbInterrupts(EXAMPLE_CAN, 1U << RX_QUEUE_BUFFER_END_1);
        FLEXCAN_EnableMbInterrupts(EXAMPLE_CAN, 1U << RX_QUEUE_BUFFER_END_2);
