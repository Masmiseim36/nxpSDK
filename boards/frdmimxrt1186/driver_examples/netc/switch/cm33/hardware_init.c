...
static status_t APP_PHY_SetPort(uint32_t port, phy_config_t *phyConfig)
{
    ...
    (void)PHY_Write(&s_phy_handle[port], PHY_EXT_ADDR_REG, PHY_RGMII_CONFIG1_REG);
    (void)PHY_Read(&s_phy_handle[port], PHY_EXT_DATA_REG, &regValue);
    regValue &= ~PHY_RGMII_CONFIG1_TXDLY_MASK;
    /* 150ps per step. */
    regValue |= 0x2;
    (void)PHY_Write(&s_phy_handle[port], PHY_EXT_DATA_REG, regValue);
