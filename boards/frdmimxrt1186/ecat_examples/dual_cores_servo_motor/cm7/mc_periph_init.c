...
void M2_InitFlexIOEndat2p2(void)
{
    ...
    FLEXIO_ENDAT2_Init(&g_sM2Enc.endat2Master, &config, srcClock_Hz);

    int number = 0x10;
    
    while (number--) {
        if (FLEXIO_ENDAT2_ReadBlocking(&g_sM2Enc.endat2Master) != kStatus_Success) 
        {  
        }
    }
        
    FLEXIO_ENDAT2_Deinit(&g_sM2Enc.endat2Master);
