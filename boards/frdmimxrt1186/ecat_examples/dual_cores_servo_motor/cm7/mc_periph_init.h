 ...
 * Clock & PWM definition for motor 1
 ******************************************************************************/
#define M2_PWM_FREQ (32000)         /* PWM frequency - 16kHz */
...
/******************************************************************************
 * Define common phase currents and DC bus measurement functions for motor 2
 ******************************************************************************/
#define M2_MCDRV_CURR_3PH_VOLT_DCB_GET(par)    /* Need to be empty - SINC data are read and processed in SINC IRQ. */
         
#define M2_MCDRV_CURR_3PH_CHAN_ASSIGN(par)      /* Keep this macro to avoid project build error (macro is used in MID). */
...
/******************************************************************************
 * Define 3-ph PWM control functions for motor 2
 ******************************************************************************/
...
#define SINC2_NUM_CHANNELS_USED		(3U)
#define SINC2_PRETRIGGER_VAL            ((SINC2_ORD * (SINC2_OSR + 1U)) + ((SINC2_OSR + 1U) / 2U))      // 3*OSR + OSR/2 ... first sample (OSR*ORD)/MCLK, next samples OSR/MCLK
...
#define M2_MCDRV_SINC_INIT()            (M2_InitSinc())
#define M2_MCDRV_SINC_GET(par)          (MCDRV_SincReadPhCurrDcBusVolt_3chnl(par))

/* Init sensors pointers for M1 */
...
#define M2_MCDRV_FLEXIO_ENDAT2P2_INIT()         (M2_InitFlexIOEndat2p2())
#define M2_MCDRV_FLEXIO_ENDAT2P2_GET(par)       (MCDRV_Endat2p2DataRead(par))

#define M2_FLEXIO_INSTANCE                      FLEXIO2
#define M2_FLEXIO_CLOCK_ROOT	                kCLOCK_Root_Flexio2
#define M2_FLEXIO_CLOCK_ROOT_MUX                kCLOCK_FLEXIO2_ClockRoot_MuxSysPll3Div2 // tuto ma wenbin chybu
#define M2_FLEXIO_IRQHandler                    FLEXIO2_IRQHandler
#define M2_FLEXIO_IRQn                          FLEXIO2_IRQn
...
/* ENCODER_ECN_1325 */
#define M2_FLEXIO_ENDAT2P2_BIT_RATE             (8000000U)
#define M2_FLEXIO_ENDAT2P2_MT_LEN               (0U)
...
#define USER_J3_18_OFF()     RGPIO_PortClear(BOARD_INITPINS_TP_J3_18_GPIO, 1U << BOARD_INITPINS_TP_J3_18_GPIO_PIN)      /*!< Turn off target USER_LED */
#define USER_J3_18_ON()      RGPIO_PortSet(BOARD_INITPINS_TP_J3_18_GPIO, 1U << BOARD_INITPINS_TP_J3_18_GPIO_PIN)        /*!<Turn on target USER_LED*/
#define USER_J3_18_TOGGLE()  RGPIO_PortToggle(BOARD_INITPINS_TP_J3_18_GPIO, 1u << BOARD_INITPINS_TP_J3_18_GPIO_PIN)  
