...
#endif /* !CONFIG_BOOT_MODE_SINGLE_APPLICATION_SLOT */
...
#if defined(CONFIG_BOOT_MODE_SINGLE_APPLICATION_SLOT)
static void bl_single_app_print_info(void)
{
    static struct image_header ih;
    uint8_t sha256[32];
    struct flash_area *fa = &boot_flash_map[0];
    ...
    PRINTF("\nSINGLE APPLICATION SLOT INFO:\n");    
    if(ih.ih_magic == IMAGE_MAGIC)
    {
        PRINTF("    Slot %s; offset 0x%x; size 0x%x (%u): \n",
                    fa->fa_name, fa->fa_off, fa->fa_size, fa->fa_size);
