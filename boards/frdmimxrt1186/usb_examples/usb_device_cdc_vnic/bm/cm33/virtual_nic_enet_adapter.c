...
USB_DMA_DATA_NONCACHEABLE static uint8_t s_dataRxBuffer[NETC_EP_BD_NUM]
                                                       [ENET_FRAME_MAX_FRAMELEN + sizeof(netc_header_t)];
...
USB_DMA_DATA_NONCACHEABLE static uint8_t s_dataTxBuffer[NETC_EP_BD_NUM]
                                                       [ENET_FRAME_MAX_FRAMELEN + +sizeof(netc_header_t)];
...
uint32_t VNIC_EnetInit(void)
{
    ...
    VNIC_NetcQueueInit(&g_enetRxServiceQueue, ENET_QUEUE_MAX);
    VNIC_NetcQueueInit(&g_enetTxServiceQueue, ENET_QUEUE_MAX);

    /* Initialize the rx packet buffer */
    for (count = 0; count < NETC_EP_BD_NUM; count++)
    {
...
usb_status_t VNIC_EnetSend(uint8_t *buf, uint32_t len)
{
    ...
    else
    {
        ...
        do
        {
            error = NETCIF_Output(&packetBuffer);
            /* wait for a while */
