...
#endif /* AWS_IOT_USE_KEYS_FROM_SM */
...
void awsPubSub(const U8 ax_uid[A71CH_MODULE_UNIQUE_ID_LEN])
{
    ...
    if( xReturned == eMQTTAgentSuccess )
    {
        ...
        if( xReturned != eMQTTAgentSuccess )
        {
            if (xReturned == eMQTTAgentFailure)
            {
                  LOG_I("\nTLS Handshake failed.\nLet's register the intermediate CA certificate\n");
                  /*we are here since the certificates have been removed from AWS and we need to register them again
                  *so lets clear the flag in gp storage that indicates that registration is complete */
                  returncode = SetGetFlag_GPstorage(&flag, SET, Gpstorage_handle);
                  if (returncode == 1)
                  {
                      LED_GREEN_OFF();
                      LED_RED_ON();
            ...
            else if (xReturned == eMQTTAgentTimeout)
            {
                /*First disconnect. Restart and try again */
                LOG_I("\nFirst Disconnect received while the device certificate is registered.\nIs the device/CA certificate Activated?\n");
            }
            else
            {
