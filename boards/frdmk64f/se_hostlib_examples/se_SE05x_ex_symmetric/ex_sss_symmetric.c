...
sss_status_t ex_sss_entry(ex_sss_boot_ctx_t *pCtx)
{
    ...
    status = sss_symmetric_context_init(
        &ctx_symm_encrypt, &pCtx->session, &key, algorithm, mode);
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);
    ...
    status = sss_cipher_one_go(
        &ctx_symm_encrypt, iv, ivlen, srcData, destData, destDataLen);
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);
    ...
    status = sss_symmetric_context_init(
        &ctx_symm_decrypt, &pCtx->session, &key, algorithm, mode);
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);
    ...
    LOG_MAU8_I("Encrypted data", destData, destDataLen);
    status = sss_cipher_one_go(
        &ctx_symm_decrypt, iv, ivlen, destData, srcData, destDataLen);
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);
