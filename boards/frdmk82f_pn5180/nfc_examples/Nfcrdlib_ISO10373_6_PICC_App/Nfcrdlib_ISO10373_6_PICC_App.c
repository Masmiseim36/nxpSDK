...
/** \file
* ISO 10373-6 PICC Compliance application.
...
/* Local headers */
#include "Nfcrdlib_ISO10373_6_PICC_App.h"
...
#   warning "This application is not supported for RC663"
int main() {
    DEBUG_PRINTF("This application is not supported for RC663.");
    while (1){
 ...
 * */
uint8_t aAtsRes[5]  = {0x05, 0x77, 0x77, 0x88, 0x02}; /* ATS Response send for RATS
                                                       * |TL|T0|TA|TB|TC|
                                                       */
...
phOsal_SemObj_t  appexit;  /* Used to synchronize end of one
                                             cycle of application processing */
phOsal_SemObj_t timerwtx;  /* Used to create WTX */
...
#else /* NXPBUILD_ISO10373_6_PICC_APP_MANDATORY_LAYERS */
...
phStatus_t ConfigureCardEmulation(void)
{
    ...
    /* Enable support for proprietary command handling by application callback. */
...
* This function configures DiscLoop and PAL 14443p4 MC layers to ISO Mode.
* \return Status code
...
static phStatus_t Configure_Mode(void)
{
    ...
    /* Configure DiscLoop to ISO mode */
    ...
    /* Configure PAL 14443p4 MC to ISO mode */
    status = phpalI14443p4mC_SetConfig(
        ppalI14443p4mC,
...
void pWtoxCallBck()
{
    ...
    /* Trigger Application callback (AppProcessCmdCallback) about WTX */
