...
tsi_status_t NT_TSI_DRV_EnableElectrode(uint32_t instance, const uint32_t channel, const bool enable)
{
    ...
    {
        NT_ASSERT(!(tsiState->opModesData[tsiState->opMode].enabledElectrodes &
    ...
    while (electrode_last--)
    {
        ...
        if (_nt_electrode_get_flag(module->electrodes[electrode_last], NT_ELECTRODE_ENABLED))
        {
            ...
            if (electrodes[electrode_last]->pin_input < TF_TSI_SELF_CAP_CHANNEL_COUNT)
            {
                ...
                TSI_SetSelfCapMeasuredChannel(base, electrodes[electrode_last]->pin_input);
            }
    ...
    if (tsiState->opSatus != tsi_OpStatusSuspend)
    {
        ...
        { /* Check if the electrode is enabled */
            ...
            { /* OS: End of critical section. */
                ...
                if (tsiState->opSatus != tsi_OpStatusSuspend) /* If TSI not Suspended */
                {
...
tsi_status_t NT_TSI_DRV_MeasureAllElect(uint32_t instance, uint16_t (*counters)[])
{
    ...
    while (electrode_last--)
    {
        ...
        if (electrodes[electrode_last]->rom->pin_input < TF_TSI_SELF_CAP_CHANNEL_COUNT)
        {
            ...
            TSI_SetSelfCapMeasuredChannel(base, electrodes[electrode_last]->rom->pin_input);
        }
        /* If this electrode is a mutual-cap mdde electrode, initialize mutual-cap sensing*/
        else
        {
                                         ...
                                         uint8_t elNumber)
{
    ...
    while (elNumber--)
    {
        ...
        if (electrodes[elNumber]->pin_input < TF_TSI_SELF_CAP_CHANNEL_COUNT)
        {
            ...
            TSI_SetSelfCapMeasuredChannel(base, electrodes[elNumber]->pin_input);
        }
        /* If this electrode is a mutual-cap mdde electrode, initialize mutual-cap sensing*/
        else
        {
...
uint16_t NT_TSI_DRV_MeasureOneElect(uint32_t instance, struct nt_electrode *electrode)
{
    ...
    {
        TSI_InitSelfCapMode(base, &electrode_data->tsi_hw_config->configSelfCap);
        TSI_SetSelfCapMeasuredChannel(base, electrode->pin_input);
    }
    /* If this electrode is a mutual-cap mdde electrode, initialize mutual-cap sensing*/
    else
    {
...
tsi_status_t NT_TSI_DRV_Recalibrate(uint32_t instance, void *configuration)
{
    ...
    struct nt_electrode **electrodes        = (struct nt_electrode **)system->rom->modules[instance]->electrodes;
    NT_ASSERT(electrodes != NULL);
    struct nt_electrode *electrodesEnabledSelf[TF_TSI_SELF_CAP_CHANNEL_COUNT];
    struct nt_electrode *electrodesEnabledMutual[TF_TSI_MUTUAL_CHANNEL_COUNT];
    ...
    { /* Check wheather configuration can be modidify (is RAM located) for recalibration reasons */
        struct nt_electrode_data *electrode_data = _nt_electrode_get_data(*electrodes);
        configSelfCap    = (tsi_selfCap_config_t *)&electrode_data->tsi_hw_config->configSelfCap;
        uint32_t address = (uint32_t) & *configSelfCap;

/* variable setting for obtaining FLASH start and end adress to check if confiruration is in FLASH or RAM */
#if defined(__IAR_SYSTEMS_ICC__) /* For IAR compiler   */
...
#warning "Unsupported compiler/IDE used !"
        ...
        if (_nt_electrode_get_flag(electrode_data, NT_ELECTRODE_ENABLED))
        {
    ...
    while (elNumber--)
    {
        do
        {
            ...
            if (counter < recalib_config->SelfRangeMin) /* untouched counter value is too low */
            {
                ...
                {
                    configSelfCap->commonConfig.decimation -= 1;
                    continue;
                }
                else if (configSelfCap->chargeCurrent < kTSI_CurrentMultipleChargeValue_7 && cycleCounter > 32 &&
                         cycleCounter <= 32 + 8) /* Charge/Discharge current multiple calibration, 8 values */
                {
                    configSelfCap->chargeCurrent += 1;
                    continue;
                }
                else if (configSelfCap->inputCurrent < kTSI_CurrentMultipleInputValue_1 && cycleCounter > 32 + 8 &&
                         cycleCounter <= 32 + 8 + 2) /* Input current multiple calibration, 2 values */
                {
                    configSelfCap->inputCurrent = kTSI_CurrentMultipleInputValue_1;
                    continue;
                }
                else if (configSelfCap->commonConfig.dvolt > kTSI_DvoltOption_0 && cycleCounter > 32 + 8 + 2 &&
                         cycleCounter <= 32 + 8 + 2 + 4) /* DVOLT option value calibration, 4 values */
                {
                    configSelfCap->commonConfig.dvolt -= 1;
                    continue;
                }
                else if (configSelfCap->commonConfig.cutoff < kTSI_SincCutoffDiv_128 && cycleCounter > 32 + 8 + 2 + 4 &&
                         cycleCounter <= 32 + 8 + 2 + 4 + 8) /* Cut-off divider calibration, 8 values */
                {
                    configSelfCap->commonConfig.cutoff += 1;
                    continue;
                }
                else if (configSelfCap->commonConfig.order > kTSI_SincFilterOrder_1 &&
                         cycleCounter > 32 + 8 + 2 + 4 + 8 &&
                         cycleCounter <= 32 + 8 + 2 + 4 + 8 + 2) /* SINC filter order, 2 values */
                {
                    configSelfCap->commonConfig.order -= 1;
                    continue;
                }
            ...
            else if (counter > recalib_config->SelfRangeMax) /* untouched counter value is too high */
            {
                ...
                else if (configSelfCap->chargeCurrent > kTSI_CurrentMultipleChargeValue_0 && cycleCounter > 32 &&
                         cycleCounter <= 32 + 8) /* Charge/Discharge current multiple calibration, 8 values */
                {
                    configSelfCap->chargeCurrent -= 1;
                    continue;
                }
                else if (configSelfCap->inputCurrent > kTSI_CurrentMultipleInputValue_0 && cycleCounter > 32 + 8 &&
                         cycleCounter <= 32 + 8 + 2) /* Input current multiple calibration, 2 values */
                {
                    configSelfCap->inputCurrent = kTSI_CurrentMultipleInputValue_0;
                    continue;
                }
                else if (configSelfCap->commonConfig.dvolt < kTSI_DvoltOption_3 && cycleCounter > 32 + 8 + 2 &&
                         cycleCounter <= 32 + 8 + 2 + 4) /* DVOLT option value calibration, 4 values */
                {
                    configSelfCap->commonConfig.dvolt += 1;
                    continue;
                }
                else if (configSelfCap->commonConfig.cutoff > kTSI_SincCutoffDiv_0 && cycleCounter > 32 + 8 + 2 + 4 &&
                         cycleCounter <= 32 + 8 + 2 + 4 + 8) /* Cut-off divider calibration, 8 values */
                {
                    configSelfCap->commonConfig.cutoff -= 1;
                    continue;
        ...
        } while ((counter < recalib_config->SelfRangeMin) ||
                 (counter > recalib_config->SelfRangeMax)); /* next calibration cycle required, TSI setting changed */
        cycleCounter = 0;
    }
    ...
    while (elNumber--)
    {
        do
        {
            ...
            nt_module_TSI_TouchOneElect(electrodesEnabledSelf[elNumber]);
            uint8_t n;
            for (n = 0; n < 9; n++)
            {
                CUSTOM_DELAY()
            ...
            counter = NT_TSI_DRV_MeasureOneElect(instance, electrodesEnabledSelf[elNumber]);
            nt_module_TSI_ReleaseOneElect(electrodesEnabledSelf[elNumber]);
            ...
            if (delta < 0)
                delta = -delta;
            cycleCounter++;
            if (delta < recalib_config->SelfRangeDeltaMin)
            {
                ...
                else if (configSelfCap->ctrim < kTSI_SensitivityCtrimOption_7 && cycleCounter > 8 &&
                         cycleCounter <= 8 + 8) /* Sensitivity CTRIM option calibration, 8 values */
                {
                    configSelfCap->ctrim += 1;
                    continue;
                }
                else if (configSelfCap->commonConfig.dvolt > kTSI_DvoltOption_0 && cycleCounter > 8 + 8 &&
                         cycleCounter <= 8 + 8 + 4) /* DVOLT option value calibration, 4 values */
                {
                    configSelfCap->commonConfig.dvolt -= 1;
                    continue;
                }
                else if (configSelfCap->commonConfig.cutoff > kTSI_SincCutoffDiv_0 && cycleCounter > 8 + 8 + 4 &&
                         cycleCounter <= 8 + 8 + 4 + 8) /* Cut-off divider calibration, 8 values */
                {
                    configSelfCap->commonConfig.cutoff -= 1;
                    continue;
                }
            ...
            else if (delta > recalib_config->SelfRangeDeltaMax)
            {
                configSelfCap->enableSensitivity = true;
                ...
                else if (configSelfCap->ctrim > kTSI_SensitivityCtrimOption_0 && cycleCounter > 8 &&
                         cycleCounter <= 8 + 8) /* Sensitivity CTRIM option calibration, 8 values */
                {
                    configSelfCap->ctrim -= 1;
                    continue;
                }
                else if (configSelfCap->commonConfig.dvolt < kTSI_DvoltOption_3 && cycleCounter > 8 + 8 &&
                         cycleCounter <= 8 + 8 + 4) /* DVOLT option value calibration, 4 values */
                {
                    configSelfCap->commonConfig.dvolt += 1;
                    continue;
                }
                else if (configSelfCap->commonConfig.cutoff < kTSI_SincCutoffDiv_128 && cycleCounter > 8 + 8 + 4 &&
                         cycleCounter <= 8 + 8 + 4 + 8) /* Cut-off divider calibration, 8 values */
                {
                    configSelfCap->commonConfig.cutoff += 1;
                    continue;
        ...
        } while ((delta < recalib_config->SelfRangeDeltaMin) || (delta > recalib_config->SelfRangeDeltaMax));
        cycleCounter = 0;
    }
    ...
    while (elNumber--)
    {
        do
        {
            ...
            nt_module_TSI_TouchOneElect(electrodesEnabledMutual[elNumber]);
            if (counter == 0)
                return kStatus_TSI_MutUntouchRecalibError;
            cycleCounter++;
            if (counter < recalib_config->MutRangeMin) /* untouched counter value is too low */
            {
                if (configMutual->commonConfig.decimation < kTSI_SincDecimationValue_32 &&
                    cycleCounter <= 32) /* SINC decimation calibration, 32 values */
                {
                    configMutual->commonConfig.decimation += 1;
                    continue;
                }
                else if (configMutual->commonConfig.cutoff > kTSI_SincCutoffDiv_0 && cycleCounter > 32 &&
                         cycleCounter <= 32 + 8) /* Cut-off divider calibration, 8 values */
                {
                    configMutual->commonConfig.cutoff -= 1;
                    continue;
                }
                else if (configMutual->commonConfig.chargeNum > kTSI_SscChargeNumValue_1 && cycleCounter > 32 + 8 &&
                         cycleCounter <= 32 + 8 + 16) /* SSC output bit0's period setting calibration, 16 values */
                {
                    configMutual->commonConfig.chargeNum -= 1;
                    continue;
                }
                else if (configMutual->preResistor < kTSI_MutualPreResistor_8k && cycleCounter > 32 + 8 + 16 &&
                         cycleCounter <= 32 + 8 + 16 + 8) /* Vref generator resistor  calibration, 8 values */
                {
                    configMutual->preResistor += 1;
                    continue;
                }
                else if (configMutual->commonConfig.dvolt < kTSI_DvoltOption_3 && cycleCounter > 32 + 8 + 16 + 8 &&
                         cycleCounter <= 32 + 8 + 16 + 8 + 4) /* DVOLT option value calibration, 4 values */
                {
                    configMutual->commonConfig.dvolt += 1;
                    continue;
                }
                else if (configMutual->boostCurrent > kTSI_MutualSenseBoostCurrent_0uA &&
                         cycleCounter > 32 + 8 + 16 + 8 + 4 &&
                         cycleCounter <= 32 + 8 + 16 + 8 + 4 + 32) /* Sensitivity boost current setting, 32 values */
                {
                    configMutual->boostCurrent += 1;
                    continue;
                }
                else if (configMutual->pmosLeftCurrent > kTSI_MutualPmosCurrentMirrorLeft_4 &&
                         cycleCounter > 32 + 8 + 16 + 8 + 4 + 32 &&
                         cycleCounter <=
                             32 + 8 + 16 + 8 + 4 + 32 + 8) /* Pmos current mirror on the left side, 8 values */
                {
                    configMutual->pmosLeftCurrent += 1;
                    continue;
                }
                else if (configMutual->pmosRightCurrent < kTSI_MutualPmosCurrentMirrorRight_4 &&
                         cycleCounter > 32 + 8 + 16 + 8 + 4 + 32 + 8 &&
                         cycleCounter <=
                             32 + 8 + 16 + 8 + 4 + 32 + 8 + 4) /* Pmos current mirror on the right side, 4 values */
                {
                    configMutual->pmosRightCurrent -= 1;
                    continue;
                }
                else if (configMutual->commonConfig.order > kTSI_SincFilterOrder_1 &&
                         cycleCounter > 32 + 8 + 16 + 8 + 4 + 32 + 8 + 4 &&
                         cycleCounter <= 32 + 8 + 16 + 8 + 4 + 32 + 8 + 4 + 2) /* SINC filter order, 2 values */
                {
                    configMutual->commonConfig.order -= 1;
                    continue;
                }
            ...
            else if (counter > recalib_config->MutRangeMax) /* untouched counter value is too high */
            {
                ...
                else if (configMutual->commonConfig.cutoff < kTSI_SincCutoffDiv_128 && cycleCounter > 32 &&
                         cycleCounter <= 32 + 8) /* Cut-off divider calibration, 8 values */
                {
                    configMutual->commonConfig.cutoff += 1;
                    continue;
                }
                else if (configMutual->commonConfig.chargeNum < kTSI_SscChargeNumValue_16 && cycleCounter > 32 + 8 &&
                         cycleCounter <= 32 + 8 + 16) /* SSC output bit0's period setting calibration, 16 values */
                {
                    configMutual->commonConfig.chargeNum += 1;
                    continue;
                }
                else if (configMutual->preResistor > kTSI_MutualPreResistor_1k && cycleCounter > 32 + 8 + 16 &&
                         cycleCounter <= 32 + 8 + 16 + 8) /* Vref generator resistor  calibration, 8 values */
                {
                    configMutual->preResistor -= 1;
                    continue;
                }
                else if (configMutual->commonConfig.dvolt > kTSI_DvoltOption_0 && cycleCounter > 32 + 8 + 16 + 8 &&
                         cycleCounter <= 32 + 8 + 16 + 8 + 4) /* DVOLT option value calibration, 4 values */
                {
                    configMutual->commonConfig.dvolt -= 1;
                    continue;
        ...
        } while ((counter < recalib_config->MutRangeMin) ||
                 (counter > recalib_config->MutRangeMax)); /* next calibration cycle required, TSI setting changed */
        cycleCounter = 0;
    }
    ...
    while (elNumber--)
    {
        do
        {
            ...
            counter = NT_TSI_DRV_MeasureOneElect(instance, electrodesEnabledMutual[elNumber]);
            nt_module_TSI_ReleaseOneElect(electrodesEnabledMutual[elNumber]);
            ...
            if (delta < 0)
                delta = -delta;
            cycleCounter++;
            if (delta < recalib_config->MutRangeDeltaMin)
            {
                ...
                if (configMutual->commonConfig.decimation > kTSI_SincDecimationValue_1 && cycleCounter > 32 &&
                    cycleCounter <= 32 + 32) /* SINC decimation calibration, 32 values  */
                {
                    configMutual->commonConfig.decimation -= 1;
                    continue;
                }
                else if (configMutual->commonConfig.cutoff > kTSI_SincCutoffDiv_0 && cycleCounter > 32 + 32 &&
                         cycleCounter <= 32 + 32 + 8) /* Cut-off divider, 8 calibration values  */
                {
                    configMutual->commonConfig.cutoff -= 1;
                    continue;
                }
            ...
            else if (delta > recalib_config->MutRangeDeltaMax)
            {
                configMutual->enableNmosMirror = true;
                ...
                else if (configMutual->commonConfig.decimation < kTSI_SincDecimationValue_32 && cycleCounter > 32 &&
                         cycleCounter <= 32 + 32) /* SINC decimation calibration, 32 values  */
                {
                    configMutual->commonConfig.decimation += 1;
                    continue;
                }
                else if (configMutual->commonConfig.cutoff < kTSI_SincCutoffDiv_128 && cycleCounter > 32 + 32 &&
                         cycleCounter <= 32 + 32 + 8) /* Cut-off divider, 8 calibration values  */
                {
                    configMutual->commonConfig.cutoff += 1;
                    continue;
        ...
        } while ((delta < recalib_config->MutRangeDeltaMin) || (delta > recalib_config->MutRangeDeltaMax));
    }
    tsiState->status = kStatus_TSI_Initialized;
