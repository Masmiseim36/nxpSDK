...
int32_t nt_module_safety_check_baseline_low(const struct nt_electrode *elec)
{
    ...
    if (s_delta < 0) /* for too low baseline level must be negative delta */
    {
        ...
        if ((s_delta << 3) >
            elec->baseline_level) /* baseline neg. change is more than 12.5% of normal baseline level */
        {
            return NT_FAILURE;
        }
    }
...
int32_t nt_module_safety_check_baseline_high(const struct nt_electrode *elec)
{
    ...
    {
        if ((s_delta << 2) >
            elec->baseline_level) /* baseline positive change is more than 25% of normal baseline level */
        {
            return NT_FAILURE;
        }
    }
...
int32_t nt_module_safety_check_electrodes(struct nt_module_data *module)
{
    ...
    while (el_counter--)                 /* The main test loop */
    {
        ...
        if (nt_module_safety_check_baseline_low(elec) ==
            NT_FAILURE) /* Test each defined electrode to low limit baseline level */
        {
        ...
        if (nt_module_safety_check_baseline_high(elec) ==
            NT_FAILURE) /* Test each defined electrode to high limit baseline level */
        {
            fault_cnt++;                               /* Increment fault counter */
            context.sig_out_of_limit.electrode = elec; /* Store electrode not passed */
    ...
    if (module->safety_last_test == 0) /* Set short to GND tests */
    {
        ...
        {
            struct nt_electrode *elec = (struct nt_electrode *)module->electrodes[el_counter]->rom;
            nt_module_safety_check_electrode_short_gnd_adj_set(
    ...
    if (module->safety_last_test == 1) /* If GND tests is running check the safety delay overflow */
    {
        if (_nt_module_safety_delay(module) ==
            NT_SUCCESS) /* After applying delay the assigned electrode will be sensed */
        {
            el_counter = module->electrodes_cnt; /* Get number of electrodes */
            while (el_counter--)                 /* The main test loop */
            {
                struct nt_electrode *elec =
                    (struct nt_electrode *)module->electrodes[el_counter]->rom; /* Get electrode to test */
                if (nt_module_safety_check_electrode_short_read(elec, false) ==
                    NT_FAILURE) /* Test each assigned electrode to GPIO input with pull-up */
                {
                    fault_cnt++;                               /* Increment fault counter */
                    context.sig_out_of_limit.electrode = elec; /* Store electrode not passed */
    ...
    if (module->safety_last_test == 2) /* Set short to VDD tests */
    {
        ...
        {
            struct nt_electrode *elec = (struct nt_electrode *)module->electrodes[el_counter]->rom;
            nt_module_safety_check_electrode_short_vdd_set(
            ...
            NT_SUCCESS) /* After applying delay the assigned electrode will be sensed */
        {
            ...
            while (el_counter--)                 /* The main test loop */
            {
                ...
                if (nt_module_safety_check_electrode_short_read(elec, true) ==
                    NT_FAILURE) /* Test each assigned electrode to GPIO input */
                {
                    fault_cnt++;                               /* Increment fault counter */
                    context.sig_out_of_limit.electrode = elec; /* Store electrode not passed */
    ...
    if (module->safety_last_test >= 4) /* Set short to ADJ tests */
    {
        if (module->safety_last_adj_test ==
            module->safety_last_test - 4) /* Set short to ADJ test for assigned electrode */
        {
            ...
            {
                elec = (struct nt_electrode *)module->electrodes[module->safety_last_adj_test]
                           ->rom; /* Select elect to ADJ test set */
                if (nt_module_safety_check_electrode_short_gnd_adj_set(elec) ==
                    NT_NOT_SUPPORTED) /* Set electrode for ADJ test */
                {
                ...
                if (module->safety_last_test >=
                    module->electrodes_cnt + 4) /* Reactivate pin test if last electrode achieved */
                {
                    module->safety_last_adj_test = 0; /* Reactivate test counter  */
                    module->safety_last_test     = 0; /* Reactivate auxiliary ADJ counter */
                    _nt_module_safety_switch_electrodes_tsi(
                        module); /* Switch all pins to TSI if no next electrode to test */
                    return NT_SUCCESS;
                }
            } while (nt_module_safety_check_electrode_short_gnd_adj_set(elec) ==
                     NT_NOT_SUPPORTED); /* repeat main ADJ loop if assigned electrode exists */
        }
        if (module->safety_last_adj_test ==
            module->safety_last_test - 5) /* If ADJ test is running check the safety delay overflow */
        {
            if (_nt_module_safety_delay(module) ==
                NT_SUCCESS) /* After applying delay the assigned electrode will be sensed */
            {
