...
uint8_t nt_memory_pool[2900]; /* IAR EWARM compiler */
#else
uint8_t nt_memory_pool[2900] __attribute__((aligned(4))); /* Keil, GCC compiler */
#endif
...
{
    /* FTM2 Init */
    FTM_Init(FTM2, &my_ftm_config);
    
    /* Configure FTM2 channels 0, 1, 2 */
    FTM_SetupPwm(FTM2, my_ftm_channel_config, 3U, kFTM_EdgeAlignedPwm, 10000U, CLOCK_GetFreq(kCLOCK_CoreSysClk));
...
static void SetHueBrightness(uint32_t hue_angle, uint32_t brightness)
{
    ...
    FTM2->CONTROLS[kFTM_Chnl_0].CnV = (duty_cycle_blue * (brightness));
    FTM2->CONTROLS[kFTM_Chnl_1].CnV = (duty_cycle_green * (brightness));
    FTM2->CONTROLS[kFTM_Chnl_2].CnV = (duty_cycle_red * (brightness));
