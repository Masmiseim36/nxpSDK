...
void APP_SetClockVlpr(void)
{
    ...
    /* Errata:
     * e9865 - PMC: Current increases in VLPS mode when the PMC_REGSC[5] bit is not cleared.
     */
    PMC->REGSC &= ~(1U << 5); /* PMC->REGSC &= ~PMC_REGSC_HPSB_MASK; */
...
int main(void)
{
    ...
    /* Disable the clock for unnecessary modules. */
    CLOCK_DisableClock(kCLOCK_Vref0);
    CLOCK_DisableClock(kCLOCK_Nvm0);
    CLOCK_DisableClock(kCLOCK_Dma0);
    CLOCK_DisableClock(kCLOCK_Sysmpu0);
