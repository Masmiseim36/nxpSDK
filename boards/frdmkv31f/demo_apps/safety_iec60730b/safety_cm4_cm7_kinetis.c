...
void SafetyWatchdogTest(safety_common_t *psSafetyCommon, wd_test_t *psSafetyWdTest)
{
    ...
    {
        FS_CM4_WDOG_Check_K32W(counterLimitHigh, counterLimitLow, WATCHDOG_RESETS_LIMIT, ENDLESS_LOOP_ENABLE, WATCHDOG_TEST_VARIABLES);
    }      
...
* @return  None
*/
void SafetyFlashTestInit(fs_flash_runtime_test_parameters_t *psFlashCrc, fs_flash_configuration_parameters_t *psFlashConfig, fs_flash_dcp_state_t *psFlashDCPState)
{
#if USE_PCC
...
* @return  None
*/
void SafetyFlashAfterResetTest(safety_common_t *psSafetyCommon, fs_flash_configuration_parameters_t *psFlashConfig, fs_flash_dcp_state_t *psFlashDCPState)
{
    psSafetyCommon->FLASH_test_result = FS_CM4_CM7_FLASH_HW16(psFlashConfig->startAddress, psFlashConfig->size, CRC_BASE, psFlashConfig->startConditionSeed);
...
* @return  None
*/
void SafetyFlashRuntimeTest(safety_common_t *psSafetyCommon, fs_flash_runtime_test_parameters_t *psFlashCrc, fs_flash_configuration_parameters_t *psFlashConfig, fs_flash_dcp_state_t *psFlashDCPState)
{
    /* CRC calculation for a given block of Flash memory */
    psFlashCrc->partCrc = FS_CM4_CM7_FLASH_HW16(psFlashCrc->actualAddress, psFlashCrc->blockSize, CRC_BASE, psFlashCrc->partCrc);
...
{
#ifdef _MKV46F16_H_
  FS_AIO_InputInit_CYCLIC(&aio_Str, (fs_aio_limits_t *)FS_ADC_Limits, (uint8_t *)FS_ADC_inputs, (uint8_t *)FS_ADC_samples, FS_CFG_AIO_CHANNELS_CNT);
#elif _K32W042S1M2_CM4_H_
  FS_AIO_InputInit_K3S(&aio_Str, (fs_aio_limits_t *)FS_ADC_Limits, (uint8_t *)FS_ADC_inputs, FS_CFG_AIO_CHANNELS_CNT, ADC_COMMAND_BUFFER);
#else
