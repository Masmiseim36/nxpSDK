...
#define DEMO_LED_GPIO BOARD_LED_GREEN_GPIO
#define DEMO_LED_PIN BOARD_LED_GREEN_GPIO_PIN
#define DEMO_SLPN_GPIO GPIOC
#define DEMO_SLPN_PIN 5
#define DEMO_LED_ON() LED_GREEN_ON()
#define DEMO_LED_OFF() LED_GREEN_OFF()
...
void DEMO_SW_IRQ_HANDLER(void)
{
    ...
    if( GPIO_GetPinsInterruptFlags(DEMO_BUTTON1_GPIO) & (1U << DEMO_BUTTON1_PIN))
    {
        /* Clear external interrupt flag. */
        GPIO_ClearPinsInterruptFlags(DEMO_BUTTON1_GPIO, 1U << DEMO_BUTTON1_PIN);
        /* Turn off LED. */
        DEMO_LED_OFF();
        /* Update goto_sleep flag */
        prot_state_ptr->go_to_sleep_flg = (bool)1U;
    }
    else if(GPIO_GetPinsInterruptFlags(DEMO_BUTTON2_GPIO) & (1U << DEMO_BUTTON2_PIN))
    {
        /* Clear external interrupt flag. */
        GPIO_ClearPinsInterruptFlags(DEMO_BUTTON2_GPIO, 1U << DEMO_BUTTON2_PIN);
...
void timerGetTimeIntervalCallback0(uint32_t *ns)
{
    ...
    {
        *ns = (uint32_t)((uint32_t)(((timerCounterValue[1] - timerCounterValue[0])*TIMER_1TICK_DURATION_PS)/1000U) + (timerOverflowInterruptCount*500000U));
    }
    else /* (timerCounterValue[1]<timerCounterValue[0]) */
    {
        *ns = (uint32_t)(((moduloValue - timerCounterValue[0] + timerCounterValue[1])*TIMER_1TICK_DURATION_PS)/1000U);
...
void MasterScheduleTick(void)
{
    ...
    if(frame_delay_count<=0)
    {
        ...
        {
            /* check for error occured during last tranfer */
