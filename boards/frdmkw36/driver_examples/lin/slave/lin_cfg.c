...
uint8_t    g_lin_frame_data_buffer[LIN_FRAME_BUF_SIZE] =
{
  '*','*','*','*','*','*','*','*','S','L','A','V','E','*','*','*','*','*','*'
};

/* definition and initialization of signal array flags */
uint8_t    g_lin_flag_handle_tbl[LIN_FLAG_BUF_SIZE] =
...
static const uint16_t  LI0_lin_configuration_ROM[LI0_LIN_SIZE_OF_CFG]= {0x0000, 0x30, 0x33, 0x36, 0x2D, 0x3C, 0x3D ,0xFFFF};
/* node address */
