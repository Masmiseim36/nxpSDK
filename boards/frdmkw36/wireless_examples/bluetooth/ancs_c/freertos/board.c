...
*************************************************************************************
************************************************************************************/
#define ADC16_CHN_GROUP               (0U)   /* ADC group configuration selection */
#define ADC16_USES_HARDWARE_AVERAGE   (0U)   
#ifndef gBoard_ADC_uses_mutex_d
#define gBoard_ADC_uses_mutex_d       (0U)   
#endif
#define ADC16_BATLVL_CHN              (23U) /* Potentiometer channel */
#define ADC16_BL_LOWER_LIMIT          (0U) /* min percentage of battery charge */
#define ADC16_BL_UPPER_LIMIT          (100U) /* max percentage of battery charge */
...
#define ADC16_BANDGAP_CHN             (27U) /* ADC channel of BANDGAP Voltage reference */
...
#define M                             (1620)      /* Typical slope:uV/oC */
#define STANDARD_TEMP                 (25)
#define mNoOfPinsDisabledInLowPower_c (16U)
...
#endif
static const gpioPinId_t maPinsDisabledInLowPower[mNoOfPinsDisabledInLowPower_c]={
{gpioPort_A_c, 16},
{gpioPort_A_c, 17},
{gpioPort_B_c, 1},
{gpioPort_B_c, 2},
...
static uint32_t maPCRSave[mNoOfPinsDisabledInLowPower_c];
...
void hardware_init(void)
{
    ...
    if( 0U == initialized )
    {
        ...
        if((PMC->REGSC & PMC_REGSC_ACKISO_MASK) != 0x00U)
        {
            ...
            SMC->STOPCTRL &= (uint8_t)(~SMC_STOPCTRL_LLSM_MASK);
            SMC->PMCTRL   &= (uint8_t)(~(SMC_PMCTRL_STOPM_MASK | SMC_PMCTRL_RUNM_MASK));
        }
        ...
        XCVR_TSM->TIMING43 = 0xFFFFFFFFU;
        XCVR_TSM->TIMING49 = 0xFFFFFFFFU;
        XCVR_TSM->TIMING50 = 0xFFFFFFFFU;
...
void BOARD_GetMACAddr(uint8_t* aOutMacAddr5B)
{
    ...
    aOutMacAddr5B[3] = (uint8_t)((RSIM->MAC_LSB>>24U)&0xFFU);
    aOutMacAddr5B[2] = (uint8_t)((RSIM->MAC_LSB>>16U)&0xFFU);
    aOutMacAddr5B[1] = (uint8_t)((RSIM->MAC_LSB>>8U )&0xFFU);
...
static uint32_t ADC16_ReadValue(uint32_t chnIdx, uint8_t diffMode)
{
    ...
    chnConfig.enableDifferentialConversion = diffMode > 0U ? TRUE : FALSE;
#endif
    ...
    while (0U == ((uint32_t)kADC16_ChannelConversionDoneFlag & ADC16_GetChannelStatusFlags(ADC0, ADC16_CHN_GROUP)))
    {
        ; /* Pole flag */
    }
...
void BOARD_SetPinsLowPower(bool_t isLowPower)
{
  ...
  {
      i = 0U;
      j = (mNoOfPinsDisabledInLowPower_c-1U);
    for(; i < mNoOfPinsDisabledInLowPower_c; i++)
    {
...
uint32_t BOARD_GetLpuartClock(uint32_t instance)
{
    ...
    {
    case 1: /* MCGFLLCLK */
        clockTemp = CLOCK_GetFllFreq();
        break;
    case 2: /* OSCERCLK */
        clockTemp = CLOCK_GetOsc0ErClkFreq();
        break;
    case 3: /* MCGIRCLK */
        clockTemp = CLOCK_GetInternalRefClkFreq();
        break;
...
uint32_t BOARD_GetI2cClock(uint32_t instance)
{
    ...
    {
    case 1:
        clockTemp = CLOCK_GetFreq(kCLOCK_CoreSysClk);
        break;

    default:
        clockTemp = CLOCK_GetFreq(kCLOCK_BusClk);
        break;
    }
...
{
while((RSIM->CONTROL & RSIM_CONTROL_RF_OSC_READY_MASK) == 0U )
{}
}
