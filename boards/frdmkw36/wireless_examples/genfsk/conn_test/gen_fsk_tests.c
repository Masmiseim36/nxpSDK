...
ct_config_params_t gaConfigParams[6];
...
app_status_t CT_GenFskInit(pHookAppNotification pFunc, pTmrHookNotification pTmrFunc)
{
    ...
    gaConfigParams[4].paramType = gParamTypeNumber_c;
    FLib_MemCpy(gaConfigParams[4].paramName, "XtalTrim", 9);
    gaConfigParams[4].paramValue.decValue = (uint8_t)gHardwareParameters.xtalTrim;
    
    gaConfigParams[5].paramType = gParamTypeMaxType_c;
...
#endif /* ((cPWR_UsePowerDownMode) && (cPWR_GENFSK_LL_Enable)) */
                    u16ReceivedPackets++;
                    i32RssiSum += pIndicationInfo->rssi;
               ...
               gCtEvtRxFailed_c == evType)
        {
            ...
            {                   
                       ...
                       gRxPacket.header.lengthField == gGenFskMinPayloadLen_c) 
                {
                    ...
                    gTxPacket.payload[0] = (uint8_t)pIndicationInfo->rssi;
...
static bool_t CT_ApplyPrintConfigParams(void)
{
    ...
    if(CT_IsShortcutMenuEnabled())
    {
        ...
        if(gaConfigParams[4].paramValue.decValue != gHardwareParameters.xtalTrim)
        {
            if( XCVR_SetXtalTrim(gaConfigParams[4].paramValue.decValue) != gXcvrSuccess_c )
            {
                gaConfigParams[4].paramValue.decValue = gHardwareParameters.xtalTrim;
            }
            else
            {
                gHardwareParameters.xtalTrim = gaConfigParams[4].paramValue.decValue;
                bParamsUpdated = TRUE;
            }
        }
