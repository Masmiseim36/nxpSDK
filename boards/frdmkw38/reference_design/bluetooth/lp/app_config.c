...
gapAdvertisingParameters_t gAdvParams = {
    ...
    /* ADV interval above 10.24 sec uses ADV on wakeup (gAppAdvOnWakeUp is set to 1)
       Value set below defines the ADV interval only in connected mode which must be smaller than 0x4000   */
                              gAppAdvertisingIntervalConnected,
                              gAppAdvertisingIntervalConnected,
#else
    /* minInterval */         gAppAdvertisingInterval,
    /* maxInterval */         gAppAdvertisingInterval,
...
/* Set SCAN_RSP payload - See limitation on the SCAN_RSP payload size in comment for
      gAppUseScanRsp setting in app_preinclude.h */
gapScanResponseData_t gAppScanRspData =
{
#if defined(gAppUseScanRsp) && (gAppUseScanRsp == 1)
    NumberOfElements(advScanStruct),
    (void *)advScanStruct
#else
    0,
