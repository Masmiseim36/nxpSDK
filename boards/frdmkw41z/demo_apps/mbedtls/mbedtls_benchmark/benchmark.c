    ...
    {                                                                                  \
        ...
        {                                                                              \
            mbedtls_printf("%6.2f " TYPE "/s",                                         \
                           ((float)ii) / ((benchmark_mbedtls_timing_hardclock() - tsc) \
...
typedef struct
{
    char md4, md5, ripemd160, sha1, sha256, sha512, arc4, des3, des, aes_cbc, aes_gcm, aes_ccm, camellia, blowfish,
...
int main(int argc, char *argv[])
{
    ...
    { /* Init HW crypto acceleration module.*/
    ...
    if (todo.aes_cbc)
    {
        static uint8_t aes_ctr_test_key[] = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
                                             0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};
        static uint8_t aes_ctr_test_counter[16] = {0xf0, 0xf1, 0xf2, 0xf3, 0xf4, 0xf5, 0xf6, 0xf7,
        ...
        for (keysize = 128; keysize <= 256; keysize += 64)
        {
...
#endif
            snprintf(title, sizeof(title), "AES-CTR-%d", keysize);
            ...
            TIME_AND_TSC(
                title, mbedtls_aes_crypt_ctr(&aes, BUFSIZE, &offset, aes_ctr_test_counter, aes_ctr_test_key, buf, buf));
        }
        mbedtls_aes_free(&aes);
    }
#endif
    ...
    if (todo.rsa)
    {
#if defined(MBEDTLS_PEM_PARSE_C) && defined(MBEDTLS_PK_PARSE_C)
        mbedtls_pk_context pk;
        mbedtls_rsa_context *rsa;
        const char *rsa_keys[] = {
            RSA_PRIVATE_KEY_1024,
        #if defined(FREESCALE_PKHA_LONG_OPERANDS_ENABLE) \
            || (defined(FREESCALE_PKHA_INT_MAX_BYTES) && (FREESCALE_PKHA_INT_MAX_BYTES >= ((2048 / 8) * 2))) \
            || ((!defined(MBEDTLS_FREESCALE_LTC_PKHA)) && (!defined(MBEDTLS_FREESCALE_CAU3_PKHA)))
        ...
        {
            mbedtls_pk_init(&pk);
            mbedtls_pk_parse_key(&pk, (const unsigned char *)rsa_keys[i], strlen(rsa_keys[i]) + 1, NULL, 0);
