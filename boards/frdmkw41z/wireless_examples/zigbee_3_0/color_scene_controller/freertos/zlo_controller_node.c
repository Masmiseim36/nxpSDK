...
NVM_RegisterDataSet(&sZllState, 1, sizeof(tsZllRemoteState), PDM_ID_APP_ZLO_CONTROLLER, gNVM_MirroredInRam_c);
...
PUBLIC void vRemoveLight(uint16 u16Addr)
{
    ...
    for ( i=0; i<sEndpointTable.u8NumRecords; i++ )
    {
        if (sEndpointTable.asEndpointRecords[i].u16NwkAddr == u16Addr )
        {
            ...
            FLib_MemSet( &sEndpointTable.asEndpointRecords[sEndpointTable.u8NumRecords], 0, sizeof( tsZllEndpointInfoRecord) );
    ...
    if ( sEndpointTable.au8PingCount[sEndpointTable.u8CurrentLight] >= LIGHT_DEVICE_AGE_LIMIT )
{
        ...
        sEndpointTable.u8NumRecords--;
        if(sEndpointTable.u8NumRecords < NUM_ENDPOINT_RECORDS)
        {
            FLib_MemSet( &sEndpointTable.asEndpointRecords[sEndpointTable.u8NumRecords], 0, sizeof( tsZllEndpointInfoRecord) );
            sEndpointTable.au8PingCount[sEndpointTable.u8NumRecords] = 0;
        }
...
PUBLIC void APP_vFactoryResetRecords( void)
{
    ...
    FLib_MemSet(&sEndpointTable, 0 , sizeof(tsZllEndpointInfoTable));
    FLib_MemSet(&sGroupTable, 0, sizeof(tsZllGroupInfoTable));
