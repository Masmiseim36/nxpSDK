    ...
    DBG_vPrintf(TRACE_APP_GP, "\n before  vGP_RestorePersistedData Addr= 0x%8x %d Address = 0x%x\n",sGPDeviceInfo.sGreenPowerCustomDataStruct.asZgpsSinkProxyTable[0].uZgpdDeviceAddr.u32ZgpdSrcId,
     ...
     vAPP_GP_SetCommModeLtWt();
         ...
         if(E_GP_OUT_OF_THE_BOX_ZGPD_KEY != GP_KEYTPE)
         {
             ...
             FLib_MemCpy(&sGPDeviceInfo.sServerGreenPowerCluster.sZgpSharedSecKey,key, 16 );
         }
#endif
#ifdef CLD_GP_ATTR_ZGP_LINK_KEY
             FLib_MemCpy((sGPDeviceInfo.sServerGreenPowerCluster.sZgpLinkKey.au8Key),
                    psAib->psAplDefaultTCAPSLinkKey->au8LinkKey, E_ZCL_KEY_128_SIZE );
         ...
         {
             uint8 key[] = GP_SHARED_KEY;
             FLib_MemCpy(&sGPDeviceInfo.sClientGreenPowerCluster.sZgpSharedSecKey,key, 16 );
         }
#endif
#ifdef CLD_GP_ATTR_ZGP_LINK_KEY
             FLib_MemCpy((sGPDeviceInfo.sClientGreenPowerCluster.sZgpLinkKey.au8Key),
                    psAib->psAplDefaultTCAPSLinkKey->au8LinkKey, E_ZCL_KEY_128_SIZE );
             DBG_vPrintf(TRACE_APP_GP, "Link key : ");
             {
                 uint8 i;
...
{
    FLib_MemSet( &(sGPDeviceInfo.sGreenPowerCustomDataStruct.asZgpsSinkProxyTable),0,
               sizeof(sGPDeviceInfo.sGreenPowerCustomDataStruct.asZgpsSinkProxyTable));
    FLib_MemSet(&sGP_PDM_Data,0,
    ...
    switch(psGPMessage->eEventType)
    {
        ...
        case E_GP_DECOMM_CMD_RCVD:
            ...
            for(u8Count = 0; u8Count < GP_NUMBER_OF_TRANSLATION_TABLE_ENTRIES; u8Count++)
            {
                ...
                {
                    FLib_MemSet(psTranslationTableEntry,0,sizeof(tsGP_TranslationTableEntry));
        ...
        )
{
    ...
    for(u8Count = 0; u8Count < GP_NUMBER_OF_TRANSLATION_TABLE_ENTRIES; u8Count++)
    {
        ...
        {
            /* clear entry */
            FLib_MemSet(&psTranslationTableEntry->uZgpdDeviceAddr, 0, sizeof(tuGP_ZgpdDeviceAddr));
...
{
    uint8 i;
    uint8 u8TranslationTableIndex = 0;
    ...
    case E_GP_TRANSLATION_TABLE_REPLACE_ENTRY:
                ...
                (E_GP_TRANSLATION_TABLE_REPLACE_ENTRY == psUpdateCmd->eAction))
        {
            ...
            for(i = 0 ; i < GP_NUMBER_OF_CONFIGURABLE_CMD_INFO; i++)
            {
                if(psTranslationTableEntry->psGpToZclCmdInfo == &sGP_PDM_Data.asGpToZclCmdInfoUpdate[i])
                {
                    ...
                    FLib_MemSet(&sGP_PDM_Data.asGpToZclCmdInfoUpdate[i],
                            0,
        ...
        if(psTranslationTableEntry != NULL)
        {
            ...
            FLib_MemCpy(&psTranslationTableEntry->psGpToZclCmdInfo->au8ZbCmdPayload[0],
