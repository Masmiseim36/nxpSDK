...
#endif
    FLib_MemSet(&sGPD_MACFrame,0,sizeof(sGPD_MACFrame));
        ...
        tsMacFrame 		*psMACFrame)
{
    ...
    /* Reset nonce */
    FLib_MemSet(&aNonce, 0, sizeof(aNonce));
    FLib_MemSet(&aData, 0, sizeof(aData));
    FLib_MemSet(&ua8CheckSum, 0, sizeof(ua8CheckSum));



    if( psMACFrame->u8PayloadLength < (MAX_PHY_DATA - u8aLen))
    {
        FLib_MemCpy(&aData[u8aLen],&psMACFrame->uPayload.au8Byte[0],
    ...
    if(XCV_REG_AES_SET_MODE_CCM == u8AESmode)
    {
        FLib_MemCpy(auEncryptedOutput,&(psMACFrame->uPayload.au8Byte[u8GPDEncryptedDataIndex]),
    ...
    else
    {
        ...
        FLib_MemCpy(ua8CheckSum,&psMACFrame->uPayload.au8Byte[u8CheckSumStart],
                0x04 );
    ...
    FLib_MemCpy( &sKey, &sGPDPersistentData.sGPDSecKey, 16);
    ...
    FLib_MemCpy(&psMACFrame->uPayload.au8Byte[u8GPDEncryptedDataIndex ],auEncryptedOutput, (psMACFrame->u8PayloadLength - u8GPDEncryptedDataIndex));
    ...
    if(XCV_REG_AES_SET_MODE_CCM == u8AESmode)
    {
        ...
        #ifdef  GPD_SECURITY_LEVEL_FC_1LSB_MIC_2B_NO_ENC
            /* 2 bytes of MIC */
            FLib_MemCpy(&psMACFrame->uPayload.au8Byte[psMACFrame->u8PayloadLength ],ua8CheckSum, 0x02);
            psMACFrame->u8PayloadLength += 0x02;
        #else /* GPD_SECURITY_LEVEL_FC_1LSB_MIC_2B_NO_ENC */

            /* 4 bytes of MIC */
            FLib_MemCpy(&psMACFrame->uPayload.au8Byte[psMACFrame->u8PayloadLength ],ua8CheckSum, 0x04);
            psMACFrame->u8PayloadLength += 0x04;
        #endif
...
#if 0 //#ifdef LITTLE_ENDIAN_PROCESSOR
/****************************************************************************
 *
 * NAME:       u32Reverse
 */
