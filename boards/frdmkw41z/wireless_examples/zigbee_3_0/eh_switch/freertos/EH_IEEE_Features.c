...
void vClearPersistentData(bool bIsWriteToEEPROM)
{
    ...
    FLib_MemCpy((uint8 *)&sGPDPersistentData.sGPDSecKey, au8GPDKey,  sizeof(sGPDPersistentData.sGPDSecKey));
                    ...
                    if(sReceivedData.pu8CmdPayload[0] & SEC_KEY_BIT_INDEX_COMM_REPY)
                    {
                        ...
                        if(sReceivedData.pu8CmdPayload[0] & SEC_KEY_ENC_BIT_INDEX_COMM_REPY)
                        {
                            ...
                            if(bIsValidFrame)
                            {
                                ...
                                FLib_MemCpy((uint8 *)&sGPDPersistentData.sGPDSecKey ,&sReceivedData.pu8CmdPayload[payloadIndex], KEY_SIZE);
...
bool bEncryptDecyptKey(uint8 u8CCMMode, uint8 *pu8Key)
{
    ...
    DBG_vPrintf(DBG_DEVICE_SWITCH, "\n bEncryptDecyptKey  \n" );
    FLib_MemSet((uint8 *)&aNonce, 0, sizeof(aNonce));
#ifndef  GPD_IEEE_ADDRESS_ENABLE
...
#else
    FLib_MemCpy(&aNonce.au8[1],(uint8 *)&(sGPD_MACReceivedFrame.uDestAddr.sExt),MAC_EXT_ADDR_LEN);
#endif
    ...
    FLib_MemCpy((uint8 *)&sKey ,auTC_LK, sizeof(auTC_LK) );
...
void vSendCommissioningCmd(void)
{
    ...
    FLib_MemCpy(&(sGPD_MACFrame.uPayload.au8Byte[sGPD_MACFrame.u8PayloadLength]),
