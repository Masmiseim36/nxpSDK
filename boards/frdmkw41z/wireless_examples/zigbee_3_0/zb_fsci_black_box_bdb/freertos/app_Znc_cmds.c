...
PUBLIC void APP_vProcessIncomingSerialCommands ( uint8    u8RxByte )
{
    ...
    if (newCommand)
    {
        ...
        else
        {
            ...
            break;
#if gFsciIncluded_c
            case (E_SL_MSG_ADD_IN_OUT_CLUSTERS):
                {
                    uint8_t i, offset = 0;
                    uint16_t *pau16ClustersLists = MEM_BufferAlloc( (au8LinkRxBuffer[1] + au8LinkRxBuffer[2]) * sizeof(uint16));
                    
                    if(pau16ClustersLists)
                    {
                        for(i=0; i< (au8LinkRxBuffer[1] + au8LinkRxBuffer[2]); i++)
                        {
                            pau16ClustersLists[i] = ZNC_RTN_U16_OFFSET ( &au8LinkRxBuffer[3], offset, offset );
                        }
                        
                        u8Status = zps_eApsAfAddInOutClusters(ZPS_pvAplZdoGetAplHandle(), au8LinkRxBuffer[0],
                                                              au8LinkRxBuffer[1], au8LinkRxBuffer[2],
                                                              &pau16ClustersLists[0], &pau16ClustersLists[au8LinkRxBuffer[1]]);
                        MEM_BufferFree(pau16ClustersLists);
                    }
                }
            break;
            case (E_SL_MSG_FIND):
            {
                bool_t fbInitiator = au8LinkRxBuffer[0];
                uint8_t src_endpoint = au8LinkRxBuffer[1];
                ...
                if( fbInitiator )
                {
                    u8Status = BDB_eFbTriggerAsInitiator(src_endpoint);
                }
                else
                {
                    u8Status = BDB_eFbTriggerAsTarget(src_endpoint);
                }
            }
            break;
#endif
