Overview
========
This application represents the "FSCI Black Box BDB" Zigbee 3.0 demo application.
The FSCI Black Box BDB application contains minimal ZCL functionality and can be used for solutions where the ZCL clusters are implemented on the host.
For more information please refer to the "Kinetis FSCI Host Application Programming Interface.pdf" document.

Toolchain supported
===================
- IAR Embedded Workbench v8.32.1
- MCUXpresso IDE v10.2.1


Hardware requirements
=====================
- Mini/micro USB cable
- FRDM-KW41Z board
- Personal Computer

Board settings
==============
No special board setting.

Prepare the Demo
================
1.  Connect a mini/micro USB cable between the PC host and the OpenSDA USB port on the board.
2.  Download the program to the target board.
3.  Press the reset button, then any other switch button on your board to begin running the demo.

Running the demo
================
Follow the instructions from the "Kinetis FSCI Host Application Programming Interface.pdf" document.