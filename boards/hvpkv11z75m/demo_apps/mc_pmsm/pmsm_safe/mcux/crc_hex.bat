...
if not exist %hex_file% (
    echo # ERROR: Invalid HEX file path or name! Please fix the first argument.
...
if not exist %srec_cat_file% (
    echo # ERROR: Invalid srec_cat.exe file path! Please fix the third argument.
    echo #        CRC NOT updated.
    GOTO end_of_file
)
...
set CRC_actual=%CRC_info_table:~4,4%
set CRC_end=%CRC_info_table:~8,8%
set CRC_start=%CRC_info_table:~16,8%
set CRC_seed=%CRC_info_table:~24,4%

rem Print the success message
echo # CRC table found at: 0x%FCN_ParseLine_RetAddressHEX%
echo # CRC seed:           0x%CRC_seed%
...
rem Calculate the CRC-16/AUG-CCITT of selected region using srec_cat.exe
for /f %%i in ('%~dp0%srec_cat_loc% %hex_file%^
    -intel -crop 0x%CRC_start% 0x%CRC_end%^
    -fill 0xFF 0x%CRC_start% 0x%CRC_end%^
    -CRC16_Little_Endian 0xFFFFFFF0 -xmodem^
    -crop 0xFFFFFFF0 0xFFFFFFF2') do (
...
set /a temp_var=0x%FCN_ParseLine_RetAddressHEX%+12
...
rem Write the CRC value
%~dp0%srec_cat_loc% %hex_file%^
    -intel -crop 0x00000000 0x%crc_addr_start%^
    -generate 0x%crc_addr_start% 0x%crc_addr_end% -constant-l-e 0x5AA5%CRC_actual% 4^
