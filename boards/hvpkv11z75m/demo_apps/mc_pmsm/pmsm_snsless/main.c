...
int main(void)
{
    ...
    BOARD_InitUART(g_sClockSetup.ui32BusClock, BOARD_FMSTR_UART_BAUDRATE);
...
*           - handling the PDB sequence error interrupt: re-initiates the PDB
*		    - enables PDB Trigger on FTM0 init
*
* @param   void
*
* @return  none
...
void PDB0_PDB1_IRQHandler(void)
{
    ...
    {
        /* PDB Trigger on (FTM0 init trigger) */
        PDB0->SC = (PDB0->SC & (~PDB_SC_TRGSEL_MASK)) | PDB_SC_TRGSEL(0x8);
        PDB0->SC |= PDB_SC_LDOK_MASK;
