...
#define CLOCK_PWM_MODULO        (CLOCK_BUS_FREQ / M1_PWM_FREQ)

/* Three-phase PWM dead-time. */
#define CLOCK_PWM_DEADTIME      (CLOCK_BUS_FREQ / (1000000000U / M1_PWM_DEADTIME) / 4U)

/* Calculate the slow control loop timer (FTM2) modulo. */
#define CLOCK_SLOW_LOOP_MODULO  (CLOCK_BUS_FREQ / IRQ_SLOW_LOOP_FREQ)
...
static void fsInitADC16(void)
{
    ...
    /* Divide Bus clock get lower input ADC clock during calibration.
       -By default set to 30MHz. */
    SIM->CLKDIV1 = SIM_CLKDIV1_OUTDIV1(CLOCK_SYSTEM_OUTDIV      - 1UL) |
                   SIM_CLKDIV1_OUTDIV2(CLOCK_ADC_CAL_BUS_OUTDIV - 1UL) |
    ...
    /* -Perform single-ended 12-bit conversion.
       -Use bus clock source (by default set to 60 MHz)
       -Set clock divide ratio to 8 to obtain ~3.75 MHz input ADC clock */
    ADC0->CFG1 = ADC_CFG1_MODE(1U) | ADC_CFG1_ADICLK(0U) |
    ...
    /* Set the bus clock back to the original configuration. */
    SIM->CLKDIV1 = SIM_CLKDIV1_OUTDIV1(CLOCK_SYSTEM_OUTDIV  - 1UL) |
    ...
    /* -Perform single-ended 12-bit conversion.
       -Use bus clock clock source (by default set to 60 MHz).
       -Divide the input clock by four (by default use 15 MHz). */
    ADC0->CFG1 = ADC_CFG1_MODE(1U) | ADC_CFG1_ADICLK(0U) | ADC_CFG1_ADIV(2U);
    ADC1->CFG1 = ADC0->CFG1;
    ...
    /* Set ADC1_CH7 MUX to B channels. */
...
static void fsInitPDB(void)
{
    ...
    PDB0->CH[0U].DLY[0U] = 45U;
    PDB0->CH[1U].DLY[0U] = 45U;
    ...
    /* Setup the PDB interrupt delay. The PDB is clocked from the bus clock
       which is by default a half of system clock (results in 30 MHz).
       The calculation includes the FOC execution multiplier. */
    PDB0->IDLY = (uint32_t)(uint16_t)
        ((CLOCK_PWM_MODULO * M1_FOC_FREQ_VS_PWM_FREQ) - (CLOCK_PWM_MODULO >> 1U));
    ...
    NVIC_SetPriority(PDB0_IRQn, IRQ_PDB0_PRIO);
