...
void FSCI_BOOT_main(void)
{
    ...
    defined(CPU_MKW36Z512VFP4) || defined(CPU_MKW36Z512VHT4))
     /* KW36/38 have 256KB of FlexNVM mapped at adress 0x1000 0000 which also has an alias starting from address 0x0004 0000.
...
#endif
                {
                    ...
                    if( pPacket->structured.header.len >= gBootData_SectorsBitmap_Size_c )
                    {
                        ...
                        while (mFlashAddr < (gMcuFlashSize - gFlashErasePage_c))
                        {
                            ...
                            {
#if (defined(FSL_FEATURE_FLASH_HAS_FLEX_NVM) && (FSL_FEATURE_FLASH_HAS_FLEX_NVM == 1))
                                if (mFlashAddr >= gFLEXNVM_config.ftfxConfig.flashDesc.blockBase)
                                {
                                    status = FLEXNVM_DflashErase(&gFLEXNVM_config, 
                                                                 mFlashAddr, 
                                                                 FSL_FEATURE_FLASH_FLEX_NVM_BLOCK_SECTOR_SIZE,
                                                                 kFLASH_ApiEraseKey
                                                                 );
                                }
                                else
