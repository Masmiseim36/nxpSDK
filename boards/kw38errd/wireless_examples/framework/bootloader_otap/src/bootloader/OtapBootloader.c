...
#if (defined(FSL_FEATURE_FLASH_HAS_FLEX_NVM) && (FSL_FEATURE_FLASH_HAS_FLEX_NVM == 1))
extern flexnvm_config_t gFLEXNVM_config;
...
void Boot_LoadImage (void)
{
...
#if (defined(FSL_FEATURE_FLASH_HAS_FLEX_NVM) && (FSL_FEATURE_FLASH_HAS_FLEX_NVM == 1))
    FLEXNVM_Init(&gFLEXNVM_config);
    
#if (defined(KW38A4_SERIES) || defined(KW38Z4_SERIES) || defined(KW39A4_SERIES) || \
    defined(CPU_MKW36Z512VFP4) || defined(CPU_MKW36Z512VHT4))
     /* KW36/38/39 have 256KB of FlexNVM mapped at adress 0x1000 0000 which also has an alias starting from address 0x0004 0000.
     * Configured the Flash driver to treat the PFLASH bloxk and FlexNVM block as a continuous memory block. */
    gFLEXNVM_config.ftfxConfig.flashDesc.blockBase = FSL_FEATURE_FLASH_PFLASH_BLOCK_SIZE * FSL_FEATURE_FLASH_PFLASH_BLOCK_COUNT;
#endif
    (void)FLEXNVM_GetProperty(&gFLEXNVM_config, kFLEXNVM_PropertyDflashBlockBaseAddr, &mDflashBlockBase);
...
#endif
    {
        ...
        if( *pBitmap & bitMask )
        {
            /* Erase Flash sector */
 #if (defined(FSL_FEATURE_FLASH_HAS_FLEX_NVM) && (FSL_FEATURE_FLASH_HAS_FLEX_NVM == 1))
            if (flashAddr >= gFLEXNVM_config.ftfxConfig.flashDesc.blockBase)
            {
                status = FLEXNVM_DflashErase(&gFLEXNVM_config, 
                                             flashAddr+mDflashBlockBase, 
                                             gFlashErasePage_c,
                                             kFLASH_ApiEraseKey
                                             );
            }
            ...
            if (len)
            {
                ...
                if ((flashAddr <= gBootImageFlagsAddress_c) && (flashAddr + len > gBootImageFlagsAddress_c))
                {
                    ...
                    /* FLEXNVM: no need to check address here as gBootImageFlagsAddress_c is in the lower section */
                ...
                else
                {
                    ...
                    {
#if defined(FSL_FEATURE_FLASH_HAS_PROGRAM_SECTION_CMD) && FSL_FEATURE_FLASH_HAS_PROGRAM_SECTION_CMD
                        status = FLEXNVM_DflashProgramSection(&gFLEXNVM_config, 
                                                              flashAddr+mDflashBlockBase,
                                                              (uint8_t*)buffer, 
                                                              alignedLen);
#else
                        status = FLEXNVM_DFlashProgram(&gFLEXNVM_config, 
                                                       flashAddr+mDflashBlockBase, 
                                                       (uint8_t*)buffer, 
                                                       alignedLen);
#endif                
    ...
    status = FLASH_ProgramSection(&gFTFx_config, 
                                  ...
                                  (uint8_t*)&bootProcessCompleteFlag, 
