...
#define TSTMR_BASE CM4_0__TSTMR
...
#define CPU_RSRC SC_R_M4_1_PID0
#define IRQSTEER_RSRC SC_R_IRQSTR_M4_1
#define TSTMR_BASE CM4_1__TSTMR
#else
#error "No valid CPU core defined."
#endif

#define M4_LANDING_ZONE_SIZE (32U)
/* The IPC MU interrupt priority. Once waken up by SCU, the MU interrupt will be handled.
...
typedef struct _lpm_nvic_context
{
    ...
    uint32_t ISER[8];
    uint8_t IP[240]; /* 8QM CM4 max IRQn is 50 */
    uint8_t SHP[12];
} lpm_nvic_context_t;
...
static uint32_t s_irqEnabled = 0; /* Whether the IRQ enabled outside LPM module. */
...
static void LPM_GptInit(void)
{
    ...
    sc_pm_set_resource_power_mode((sc_ipc_t)IPC_MU, SYSTICK_RSRC, SC_PM_PW_MODE_ON);
...
static uint32_t LPM_EnterTicklessIdle(uint32_t timeoutMilliSec, uint64_t *pCounter)
{
     ...
     * equivalently increase at 8MHZ. If the MU IRQ enabled anywhere else, the IRQ should
     * be left enabled when exited tickless idle.
     */
    s_irqEnabled = NVIC_GetEnableIRQ(IPC_MU_IRQn);
    if (s_irqEnabled == 0)
    {
    ...
    /* Register for SYSCTR events to generate a wakeup. */
    sc_irq_enable((sc_ipc_t)IPC_MU, IPC_MU_RSRC, SC_IRQ_GROUP_SYSCTR, SC_IRQ_SYSCTR, true);
    ...
    sc_pm_set_resource_power_mode((sc_ipc_t)IPC_MU, SYSTICK_RSRC, SC_PM_PW_MODE_OFF);
...
static void LPM_ExitTicklessIdle(uint32_t timeoutTicks, uint64_t timeoutCounter)
{
    ...
    uint32_t expiredTicks, completeTicks, status;
    uint32_t countPerTick = SYSTICK_COUNT_PER_TICK;

    /* Clear SYSCTR IRQ and stop SYSCTR. */
    sc_irq_status((sc_ipc_t)IPC_MU, IPC_MU_RSRC, SC_IRQ_GROUP_SYSCTR, &status);
    sc_irq_enable((sc_ipc_t)IPC_MU, IPC_MU_RSRC, SC_IRQ_GROUP_SYSCTR, SC_IRQ_SYSCTR, false);
...
#endif
    mode = ASMC_GetPowerModeState(BBS_SIM);
    switch (mode)
    {
    ...
    if (timeoutTicks > 0)
    {
        ...
        switch (s_curMode)
        {
            case LPM_PowerModeWait:
                status = ASMC_SetPowerModeWait(BBS_SIM);
                break;
            case LPM_PowerModeStop:
                status = ASMC_SetPowerModeStop(BBS_SIM, kASMC_PartialStop);
                break;
            case LPM_PowerModeVlpw:
                status = ASMC_SetPowerModeVlpw(BBS_SIM);
                break;
            case LPM_PowerModeVlps:
                status = ASMC_SetPowerModeVlps(BBS_SIM);
                break;
         ...
         * up.*/
        sc_pm_set_resource_power_mode((sc_ipc_t)IPC_MU, IRQSTEER_RSRC, SC_PM_PW_MODE_ON);
#ifdef FSL_RTOS_FREE_RTOS
...
uint32_t LPM_SystemResume(bool resume)
{
    ...
    *lmpecrReg = s_lmpecr;
    ...
    /* IPC MU Interrupt enabled before enter LPM, enable MU interrupt after wakeup. */
    MU_EnableInterrupts(IPC_MU, MU_SR_GIPn_MASK);
