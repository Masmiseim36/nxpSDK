...
int main(void)
{
    ...
    if (sc_pm_set_resource_power_mode(ipcHandle, SC_R_FSPI_0, SC_PM_PW_MODE_ON) != SC_ERR_NONE)
    {
        PRINTF("Error: Failed to power on FSPI0\r\n");
    }

    CLOCK_SetIpFreq(kCLOCK_LSIO_Flexspi0, SC_100MHZ);
    CLOCK_EnableClock(kCLOCK_LSIO_Flexspi0);
