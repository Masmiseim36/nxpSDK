...
#define BOARD_ACCEL_I2C_BASEADDR DMA__LPI2C0

#define I2C_RELEASE_SDA_GPIO LSIO__GPIO2
#define I2C_RELEASE_SDA_PIN 3U
#define I2C_RELEASE_SCL_GPIO LSIO__GPIO2
...
void BOARD_I2C_ReleaseBus(void)
{
    ...
    if (sc_pad_set_mux(ipc, SC_P_HDMI_TX0_TS_SCL, 3U, SC_PAD_CONFIG_NORMAL, SC_PAD_ISO_OFF) != SC_ERR_NONE)
    {
        PRINTF("Error: Failed to configure SC_P_HDMI_TX0_TS_SCL mux\r\n");
    }
    if (sc_pad_set_mux(ipc, SC_P_HDMI_TX0_TS_SDA, 3U, SC_PAD_CONFIG_NORMAL, SC_PAD_ISO_OFF) != SC_ERR_NONE)
    {
        PRINTF("Error: Failed to configure SC_P_HDMI_TX0_TS_SDA mux\r\n");
    }

    /* Power on GPIO. */
    if (sc_pm_set_resource_power_mode(ipc, SC_R_GPIO_2, SC_PM_PW_MODE_ON) != SC_ERR_NONE)
    {
        PRINTF("Error: Failed to power on GPIO2\r\n");
    }
