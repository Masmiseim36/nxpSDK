...
int main(void)
{
    ...
    IRQSTEER_EnableInterrupt(IRQSTEER, DMA_UART2_INT_IRQn); /* Enable Debug Console uart interrupt in IRQSTEER */
    ...
    /* DMA_UART2_INT is connected to IRQSTEER master 4, configure the UART IRQ priority. */
    NVIC_SetPriority(IRQSTEER_4_IRQn, HAL_UART_ISR_PRIORITY);
