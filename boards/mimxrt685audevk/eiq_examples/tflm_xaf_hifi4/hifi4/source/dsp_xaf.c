...
#define CHECK_ERROR(err_code, action)                                \
    ...
    device_config.pframework_local_buffer = dsp_xaf_malloc(FRMWK_APP_IF_BUF_SIZE, XAF_MEM_ID_DEV);
    device_config.framework_local_buffer_size = FRMWK_APP_IF_BUF_SIZE;
    for (int i = XAF_MEM_ID_DEV; i <= XAF_MEM_ID_DEV_MAX; i++)
    {
        device_config.audio_framework_buffer_size[i] = AUDIO_FRMWK_BUF_SIZE;
...
static void dsp_xaf_process_closing(dsp_handle_t *dsp)
{
    ...
    for(int i = XAF_MEM_ID_DEV; i <= XAF_MEM_ID_DEV_MAX; i++)
    {
        dsp_xaf_free(device_config.paudio_framework_buffer[i], i);
    }
    for(int i = XAF_MEM_ID_COMP; i <= XAF_MEM_ID_COMP_MAX; i++)
    {
        dsp_xaf_free(device_config.paudio_component_buffer[i], i);
    }
    dsp_xaf_free(device_config.pframework_local_buffer, XAF_MEM_ID_DEV);
