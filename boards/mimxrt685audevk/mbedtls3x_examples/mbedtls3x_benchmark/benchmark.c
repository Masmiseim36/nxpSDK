 ...
 *  Copyright 2020 NXP. Not a Contribution
 *
...
#if !defined(MBEDTLS_TIMING_C) && !defined(MCUX_MBEDTLS)
int main(void)
{
...
{
    const mbedtls_ecp_curve_info *found = mbedtls_ecp_curve_info_from_name(string);
    if (found != NULL)
    {
        *curve = *found;
        return (1);
...
#if defined(MCUX_MBEDTLS) && !defined(MBEDTLS_TIMING_C)
...
int main(int argc, char *argv[])
{
    ...
    if (todo.aes_xts)
    {
        ...
        for (keysize = 128; keysize <= 256; keysize += 128)
        {
            ...
            TIME_AND_TSC(title, mbedtls_aes_crypt_xts(&ctx, MBEDTLS_AES_ENCRYPT, BUFSIZE, tmp, buf, buf));
    ...
    if (todo.chachapoly)
    {
        ...
        TIME_AND_TSC(title, mbedtls_chachapoly_encrypt_and_tag(&chachapoly, BUFSIZE, tmp, NULL, 0, buf, buf, tmp));
    ...
    if (todo.aria)
    {
        ...
        for (keysize = 128; keysize <= 256; keysize += 64)
        {
            ...
            TIME_AND_TSC(title, mbedtls_aria_crypt_cbc(&aria, MBEDTLS_ARIA_ENCRYPT, BUFSIZE, tmp, buf, buf));
        }
    ...
    if (todo.ctr_drbg)
    {
        ...
        if (mbedtls_ctr_drbg_seed(&ctr_drbg, myrand, NULL, NULL, 0) != 0)
            mbedtls_exit(1);
        TIME_AND_TSC("CTR_DRBG (NOPR)", mbedtls_ctr_drbg_random(&ctr_drbg, buf, BUFSIZE));
        ...
        TIME_AND_TSC("CTR_DRBG (PR)", mbedtls_ctr_drbg_random(&ctr_drbg, buf, BUFSIZE));
        mbedtls_ctr_drbg_free(&ctr_drbg);
    }
#endif
    ...
    if (todo.hmac_drbg)
    {
        ...
        if (mbedtls_hmac_drbg_seed(&hmac_drbg, md_info, myrand, NULL, NULL, 0) != 0)
            mbedtls_exit(1);
        TIME_AND_TSC("HMAC_DRBG SHA-1 (NOPR)", mbedtls_hmac_drbg_random(&hmac_drbg, buf, BUFSIZE));

        if (mbedtls_hmac_drbg_seed(&hmac_drbg, md_info, myrand, NULL, NULL, 0) != 0)
            mbedtls_exit(1);
        mbedtls_hmac_drbg_set_prediction_resistance(&hmac_drbg, MBEDTLS_HMAC_DRBG_PR_ON);
        TIME_AND_TSC("HMAC_DRBG SHA-1 (PR)", mbedtls_hmac_drbg_random(&hmac_drbg, buf, BUFSIZE));
#endif
        ...
        if (mbedtls_hmac_drbg_seed(&hmac_drbg, md_info, myrand, NULL, NULL, 0) != 0)
            mbedtls_exit(1);
        TIME_AND_TSC("HMAC_DRBG SHA-256 (NOPR)", mbedtls_hmac_drbg_random(&hmac_drbg, buf, BUFSIZE));

        if (mbedtls_hmac_drbg_seed(&hmac_drbg, md_info, myrand, NULL, NULL, 0) != 0)
            mbedtls_exit(1);
        mbedtls_hmac_drbg_set_prediction_resistance(&hmac_drbg, MBEDTLS_HMAC_DRBG_PR_ON);
        TIME_AND_TSC("HMAC_DRBG SHA-256 (PR)", mbedtls_hmac_drbg_random(&hmac_drbg, buf, BUFSIZE));
#endif
    ...
    if (todo.dhm)
    {
        ...
        static const unsigned char dhm_P_2048[] = MBEDTLS_DHM_RFC3526_MODP_2048_P_BIN;
        static const unsigned char dhm_P_3072[] = MBEDTLS_DHM_RFC3526_MODP_3072_P_BIN;
        static const unsigned char dhm_G_2048[] = MBEDTLS_DHM_RFC3526_MODP_2048_G_BIN;
        static const unsigned char dhm_G_3072[] = MBEDTLS_DHM_RFC3526_MODP_3072_G_BIN;
        ...
        for (i = 0; (size_t)i < sizeof(dhm_sizes) / sizeof(dhm_sizes[0]); i++)
        {
            ...
            TIME_PUBLIC(title, "handshake", ret |= mbedtls_dhm_make_public(&dhm, (int)n, buf, n, myrand, NULL);
    ...
    if (todo.ecdsa)
    {
        ...
        for (curve_info = curve_list; curve_info->grp_id != MBEDTLS_ECP_DP_NONE; curve_info++)
        {
            ...
            TIME_PUBLIC(title, "sign",
                        ret = mbedtls_ecdsa_write_signature(&ecdsa, MBEDTLS_MD_SHA256, buf, curve_info->bit_size, tmp,
                                                            sizeof(tmp), &sig_len, myrand, NULL));
        ...
        for (curve_info = curve_list; curve_info->grp_id != MBEDTLS_ECP_DP_NONE; curve_info++)
        {
            ...
            if (mbedtls_ecdsa_genkey(&ecdsa, curve_info->grp_id, myrand, NULL) != 0 ||
                mbedtls_ecdsa_write_signature(&ecdsa, MBEDTLS_MD_SHA256, buf, curve_info->bit_size, tmp, sizeof(tmp),
                                              &sig_len, myrand, NULL) != 0)
            {
                mbedtls_exit(1);
            }
            ecp_clear_precomputed(&ecdsa.grp);
    ...
    if (todo.ecdh)
    {
        ...
        const mbedtls_ecp_curve_info *curve_info;
        size_t olen;
        const mbedtls_ecp_curve_info *selected_montgomery_curve_list = montgomery_curve_list;
        ...
        for (curve_info = curve_list; curve_info->grp_id != MBEDTLS_ECP_DP_NONE; curve_info++)
        {
            ...
            CHECK_AND_CONTINUE(mbedtls_ecdh_make_public(&ecdh, &olen, buf, sizeof(buf), myrand, NULL));
            ...
            TIME_PUBLIC(title, "handshake",
                        CHECK_AND_CONTINUE(mbedtls_ecdh_make_public(&ecdh, &olen, buf, sizeof(buf), myrand, NULL));
                        CHECK_AND_CONTINUE(mbedtls_ecdh_calc_secret(&ecdh, &olen, buf, sizeof(buf), myrand, NULL)));
            mbedtls_ecdh_free(&ecdh);
        }

        /* Montgomery curves need to be handled separately */
        for (curve_info = selected_montgomery_curve_list; curve_info->grp_id != MBEDTLS_ECP_DP_NONE; curve_info++)
        {
            mbedtls_ecdh_init(&ecdh);
            mbedtls_mpi_init(&z);
            ...
            TIME_PUBLIC(
                ...
                CHECK_AND_CONTINUE(mbedtls_ecdh_compute_shared(&ecdh.grp, &z, &ecdh.Qp, &ecdh.d, myrand, NULL)));
