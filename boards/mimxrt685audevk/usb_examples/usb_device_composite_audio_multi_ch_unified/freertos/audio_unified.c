...
void USB_AudioSpeakerPutBuffer(uint8_t *buffer, uint32_t size)
{
    ...
    if ((size % (AUDIO_OUT_FORMAT_SIZE * AUDIO_OUT_FORMAT_CHANNELS)) != 0U)
    {
        size = (size / (AUDIO_OUT_FORMAT_SIZE * AUDIO_OUT_FORMAT_CHANNELS)) *
               (AUDIO_OUT_FORMAT_SIZE * AUDIO_OUT_FORMAT_CHANNELS);
    }
    playChannelLength = size / (AUDIO_OUT_FORMAT_CHANNELS / DEMO_TDM_AUDIO_OUTPUT_CHANNEL_COUNT);
    if (playChannelLength > remainBufferSpace) /* discard the overflow data */
    {
        playChannelLength = remainBufferSpace;
    }

    if (playChannelLength <= 0U)
    {
        return;
    }
    for (uint32_t i = 0; i < size; i += (AUDIO_OUT_FORMAT_SIZE * AUDIO_OUT_FORMAT_CHANNELS))
    {
#if (defined(USB_AUDIO_MULTI_CHANNEL_PLAY_SEL) && (0x01 == USB_AUDIO_MULTI_CHANNEL_PLAY_SEL))
        buffer_temp = buffer + i;
#elif (defined(USB_AUDIO_MULTI_CHANNEL_PLAY_SEL) && (0x02 == USB_AUDIO_MULTI_CHANNEL_PLAY_SEL))
        buffer_temp = (buffer + i + AUDIO_OUT_FORMAT_SIZE * 8U);
#endif
        fill_zero    = 1U;
        channel_data = 0U;
        for (uint32_t j = 0; j < (AUDIO_OUT_FORMAT_SIZE * DEMO_TDM_AUDIO_OUTPUT_CHANNEL_COUNT); j++)
        {
            /* make up 32 bits */
            if (1U == fill_zero)
            {
                for (uint8_t k = 0U; k < (4U - AUDIO_OUT_FORMAT_SIZE); k++)
                {
                    audioPlayDataBuff[g_deviceAudioComposite->audioUnified.tdWriteNumberPlay] = 0;
            ...
            audioPlayDataBuff[g_deviceAudioComposite->audioUnified.tdWriteNumberPlay] = *(buffer_temp + j);
            g_deviceAudioComposite->audioUnified.tdWriteNumberPlay++;
            if (g_deviceAudioComposite->audioUnified.tdWriteNumberPlay >=
                g_deviceAudioComposite->audioUnified.audioPlayBufferSize)
            {
            ...
            if (4U != AUDIO_OUT_FORMAT_SIZE)
            {
                channel_data++;
                if (channel_data == AUDIO_OUT_FORMAT_SIZE)
                {
                    fill_zero    = 1U;
                    channel_data = 0U;
                }
...
void USB_DeviceCalculateFeedback(void)
{
    ...
    if (USB_SPEED_HIGH == g_deviceAudioComposite->speed)
    {
        feedbackValue = (uint32_t)((((((uint64_t)g_deviceAudioComposite->audioUnified.audioSendCount[1]) << 32U) |
                                   ...
                                   (uint64_t)(DEMO_TDM_AUDIO_OUTPUT_CHANNEL_COUNT * DEMO_AUIDO_OUTPUT_FORMAT_SIZE));
    }
    else
    {
        feedbackValue = (uint32_t)((((((uint64_t)g_deviceAudioComposite->audioUnified.audioSendCount[1]) << 32U) |
                                     g_deviceAudioComposite->audioUnified.audioSendCount[0])) *
                                   1024UL / totalFrameValue /
                                   (uint64_t)(DEMO_TDM_AUDIO_OUTPUT_CHANNEL_COUNT * DEMO_AUIDO_OUTPUT_FORMAT_SIZE));
    }
...
usb_status_t USB_DeviceAudioCompositeInit(usb_device_composite_struct_t *device_composite)
{
    ...
    g_deviceAudioComposite->audioUnified.speakerFreqControlRange.wMIN          = 16000U;
    g_deviceAudioComposite->audioUnified.speakerFreqControlRange.wMAX          = 16000U;
    ...
    g_deviceAudioComposite->audioUnified.recorderFreqControlRange.wMIN          = 16000U;
    g_deviceAudioComposite->audioUnified.recorderFreqControlRange.wMAX          = 16000U;
