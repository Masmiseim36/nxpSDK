    ...
    for (uint32_t i = 0; i < size; i += (AUDIO_OUT_FORMAT_SIZE * AUDIO_OUT_FORMAT_CHANNELS))
    {
        ...
        for (uint32_t j = 0; j < (AUDIO_OUT_FORMAT_SIZE * DEMO_TDM_AUDIO_OUTPUT_CHANNEL_COUNT); j++)
        {
            ...
            if (1U == fill_zero)
            {
                ...
                {
                    audioPlayDataBuff[g_deviceComposite->audioUnified.tdWriteNumberPlay] = 0;
            ...
            audioPlayDataBuff[g_deviceComposite->audioUnified.tdWriteNumberPlay] = *(buffer_temp + j);
            g_deviceComposite->audioUnified.tdWriteNumberPlay++;
            if (g_deviceComposite->audioUnified.tdWriteNumberPlay >=
                g_deviceComposite->audioUnified.audioPlayBufferSize)
            {
...
usb_status_t USB_DeviceAudioUnifiedInit(usb_device_composite_struct_t *deviceComposite)
{
    ...
    g_deviceComposite->audioUnified.speakerFreqControlRange.wMIN          = 16000U;
    g_deviceComposite->audioUnified.speakerFreqControlRange.wMAX          = 16000U;
