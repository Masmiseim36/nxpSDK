...
{
    /* When current frame decode finished, set s_newFrameDone to true. */
...
int main(void)
{
    ...
    PRINTF(
        "1 slot with 2 descriptors linked with each other(ping-pong descriptor) take turns to decode a series of "
        "pictures.\r\n");
    ...
    /* Get the first frame. */
    APP_ReadNewJpegFile(0);

    /* Configure the ping-pong descriptor. */
    /* Set source buffer, buffer size for the first descriptor. */
    ...
    JPEGDEC_SetDecodeOption(&g_decoderDespt[1].config, g_decoderDespt[0].config.width, true, true);
    ...
    while (1)
    {
        ...
        /* Wait in case the last frame is not done, normally it shall already been shown by this time. */
        while (s_newFrameShown == false)
        {
        }
        ...
        /* Update the next descriptor for the file size. */
