 ...
 * Definitions
 ******************************************************************************/
#if defined(FSL_FEATURE_PUF_ACTIVATION_CODE_ADDRESS)
...
/* Some devices have provisioned PUF from NXP factory. This means that PUF activation code is already present in NXP
 ...
 * is not necessary to perform PUF enroll again.
 */
#define DO_ENROLL 0
#else
/* In case that device doen't have provisioned keystore in FFR or user application requires to have a separate PUF key
 * store (which is independent on the NXP programmed key store), it is possible to enroll PUF and store the new AC at
 * user defined location in non-volatile memory.
 */
#define DO_ENROLL 1

#endif /* FSL_FEATURE_PUF_ACTIVATION_CODE_ADDRESS */

#ifndef PUF_SCORE_ENROLL
#define PUF_SCORE_ENROLL 7u
#endif
...
static void TestSelftest(void)
{
    ...
    TEST_ASSERT_EQUAL(result, kStatus_Success);
    TEST_ASSERT_TRUE(score <= PUF_SCORE_ENROLL);
}
...
static void TestEnroll(void)
{
    ...
    TEST_ASSERT_EQUAL(result, kStatus_Success);
    TEST_ASSERT_TRUE(pufScore <= PUF_SCORE_ENROLL);
