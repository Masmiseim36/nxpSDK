...
status_t xspi_hyper_ram_dmacommand_write_data(XSPI_Type *base, uint32_t address, uint32_t *buffer, uint32_t length)
{
    ...
    while(XSPI_CheckIPAccessAsserted(base) == true)
    {}
    
    /* Disable XSPI TX EDMA. */
...
status_t xspi_hyper_ram_dmacommand_read_data(XSPI_Type *base, uint32_t address, uint32_t *buffer, uint32_t length)
{
    xspi_transfer_t flashXfer;
    status_t status;
...
status_t xspi_hyper_ram_write_mcr(XSPI_Type *base, uint32_t regAddr, uint8_t *mrVal)
{
    xspi_transfer_t flashXfer;
    status_t status;
...
status_t xspi_hyper_ram_get_mcr(XSPI_Type *base, uint32_t regAddr, uint8_t *mrVal)
{
    xspi_transfer_t flashXfer;
    status_t status;
