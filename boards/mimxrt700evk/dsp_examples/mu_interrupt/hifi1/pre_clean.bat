...
if test -f "$bin_path/dsp_vector_$build_target.bin"; then $rm_cmd "$bin_path/dsp_vector_$build_target.bin"
fi
...
if exist "%bin_path%/dsp_vector_%build_target%.bin" %rm_cmd% "%bin_path%/dsp_vector_%build_target%.bin"
