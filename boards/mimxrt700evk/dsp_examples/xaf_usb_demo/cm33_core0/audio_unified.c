...
#else
USB_DMA_NONINIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE)
uint8_t audioRecPacket[(FS_ISO_IN_ENDP_PACKET_SIZE + AUDIO_IN_FORMAT_CHANNELS * AUDIO_IN_FORMAT_SIZE)] = {0};
USB_DMA_NONINIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE)
...
void USB_DeviceAudioRecorderStatusReset(void)
{
    ...
    g_deviceAudioComposite->audioUnified.tdReadNumberRec       = g_deviceAudioComposite->audioUnified.tdWriteNumberRec;
