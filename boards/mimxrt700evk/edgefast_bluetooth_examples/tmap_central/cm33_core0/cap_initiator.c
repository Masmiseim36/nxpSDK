...
static OSA_SEMAPHORE_HANDLE_DEFINE(sem_discover_source);
static OSA_SEMAPHORE_HANDLE_DEFINE(sem_audio_start);
...
static void unicast_stream_started(struct bt_bap_stream *stream)
{
	...
	OSA_SemaphorePost(sem_audio_start);
}
...
static int discover_cas(struct bt_conn *conn)
{
	...
	err = OSA_SemaphoreWait(sem_cas_discovery, osaWaitForever_c);
	if (err != 0) {
...
static void discover_cb(struct bt_conn *conn, int err, enum bt_audio_dir dir)
{
	...
	if (dir == BT_AUDIO_DIR_SINK) {
		...
		OSA_SemaphorePost(sem_discover_sink);
	} else if (dir == BT_AUDIO_DIR_SOURCE) {
		printk("Discover sources complete: err %d\n", err);
		OSA_SemaphorePost(sem_discover_source);
	}
}
...
static int discover_sinks(struct bt_conn *conn)
{
	...
	err = OSA_SemaphoreWait(sem_discover_sink, osaWaitForever_c);
	if (err != 0) {
...
static int discover_sources(struct bt_conn *conn)
{
	...
	err = OSA_SemaphoreWait(sem_discover_source, osaWaitForever_c);
	if (err != 0) {
...
static void audio_timer_timeout(struct k_work *work)
{
	...
	k_work_schedule(&audio_send_work, 1000);
}
...
int cap_initiator_init(void)
{
    ...
    (void)OSA_SemaphoreCreate(sem_cas_discovery, 0);
    (void)OSA_SemaphoreCreate(sem_discover_sink, 0);
    (void)OSA_SemaphoreCreate(sem_discover_source, 0);
    (void)OSA_SemaphoreCreate(sem_audio_start, 0);
...
int cap_initiator_setup(struct bt_conn *conn)
{
	...
	struct bt_bap_unicast_group *unicast_group;
        ...
        (void)OSA_SemaphoreDestroy(sem_cas_discovery);
        (void)OSA_SemaphoreDestroy(sem_discover_sink);
        (void)OSA_SemaphoreDestroy(sem_discover_source);
        (void)OSA_SemaphoreDestroy(sem_audio_start);
	...
	OSA_SemaphoreWait(sem_audio_start, osaWaitForever_c);

	k_work_schedule(&audio_send_work, 0);
