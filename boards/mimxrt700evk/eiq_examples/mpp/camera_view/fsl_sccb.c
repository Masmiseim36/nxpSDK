...
{
    //PRINTF("write:regAddr=0x%x, regValue=0x%x\r\n", reg, value);
...
{
    status_t status;
    status = i2cSendFunc(i2cAddr, startReg, addrType, value, len);
    return status;
}
                        ...
                        sccb_i2c_send_func_t i2cSendFunc)
{
    ...
    //PRINTF("read:regAddr=0x%x, regValue=0x%x\r\n", reg, value);
    regVal &= ~clrMask;
    regVal |= value & clrMask;
    //PRINTF("write:regAddr=0x%x, regValue=0x%x\r\n", reg, regVal);
    status =  SCCB_WriteReg(i2cAddr, addrType, reg, regVal, i2cSendFunc);
    return status;
}
