...
void boxes_to_rects(box_data boxes[], uint32_t num_boxes, uint32_t max_boxes, mpp_labeled_rect_t *rects) {
    ...
    for (uint32_t i = 0; i < num_boxes && box_counter < max_boxes; i++) {
        ...
        rects[box_counter].left = (int)((boxes[i].left * DETECTION_ZONE_RECT_WIDTH)/ NANODET_WIDTH) + BOXES_OFFSET_TOP;
        rects[box_counter].right = (int)((boxes[i].right * DETECTION_ZONE_RECT_WIDTH)/ NANODET_WIDTH) + BOXES_OFFSET_TOP;
        rects[box_counter].bottom = (int)((boxes[i].bottom * DETECTION_ZONE_RECT_HEIGHT)/NANODET_HEIGHT) + BOXES_OFFSET_LEFT;
        rects[box_counter].top = (int)((boxes[i].top * DETECTION_ZONE_RECT_HEIGHT)/NANODET_HEIGHT) + BOXES_OFFSET_LEFT;
