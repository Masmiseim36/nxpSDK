...
extern void BOARD_SetCodecVolume(uint32_t volume);
extern usb_status_t USB_DeviceInterruptInTask(void);
extern usb_status_t USB_DeviceInterruptOutTask(tfa9xxx_handle_t *handle);
...
extern volatile bool g_UsbDeviceInterruptInReady;
extern volatile bool g_UsbDeviceInterruptOutReady;
/*******************************************************************************
 * Code
 ******************************************************************************/
/* Initialize the structure information for sai. */
void Init_Board_Sai(void)
{
    usb_echo("Init Audio SAI and I2C\r\n");
...
void USB_AudioCodecTask(void)
{
    ...
    uint16_t maxVolume;
    tfa9xxx_handle_t *tfa9xxxHandle = (tfa9xxx_handle_t *)((uint32_t)(codecHandle.codecDevHandle));
    ...
    if (g_deviceComposite->audioUnified.codecTask & VOLUME_CHANGE_TASK)
    {
        ...
        audioVolume = (uint16_t)(g_deviceComposite->audioUnified.curVolume[1] << 8U) |
                      g_deviceComposite->audioUnified.curVolume[0];
        maxVolume = (uint16_t)(g_deviceComposite->audioUnified.maxVolume[1] << 8U) |
                    g_deviceComposite->audioUnified.maxVolume[0];
        BOARD_SetCodecVolume(100 * audioVolume / maxVolume);
        g_deviceComposite->audioUnified.codecTask &= ~VOLUME_CHANGE_TASK;
    }

    if (g_UsbDeviceInterruptOutReady)
    {
        USB_DeviceInterruptOutTask(tfa9xxxHandle);
        g_UsbDeviceInterruptOutReady = false;
    }

    if (g_UsbDeviceInterruptInReady)
    {
        USB_DeviceInterruptInTask();
        g_UsbDeviceInterruptInReady = false;
    }
}
