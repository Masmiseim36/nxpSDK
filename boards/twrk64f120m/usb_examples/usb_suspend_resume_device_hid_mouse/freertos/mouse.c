...
{
    if ((1U << BOARD_SW1_GPIO_PIN) & PORT_GetPinsInterruptFlags(BOARD_SW1_PORT))
    {
        /* Disable interrupt. */
...
void SW_Init(void)
{
    NVIC_SetPriority(BOARD_SW1_IRQ, 1U);
...
#endif
{
    ...
    BOARD_InitDebugConsole();
    /* Set the LLWU pin */
    GPIO_PinInit(BOARD_SW1_GPIO, BOARD_SW1_GPIO_PIN, &pinConfig);
