...
/* RTC initialization */
        ...
        if ((RTC->CR & RTC_CR_OSCE_MASK) == 0x00U)
        { /* Only if the OSCILLATOR is not already enabled */
            RTC->CR = (uint32_t)((RTC->CR & (uint32_t) ~(uint32_t)(RTC_CR_SC2P_MASK | RTC_CR_SC4P_MASK |
...
/* Internal reference clock trim initialization */
    ...
    SIM->SOPT2 =
        ((SIM->SOPT2) & (uint32_t)(~(SIM_SOPT2_TPMSRC_MASK))) |
        ((SYSTEM_SIM_SOPT2_VALUE) & (SIM_SOPT2_TPMSRC_MASK)); /* Selects the clock source for the TPM counter clock. */
#if ((MCG_MODE == MCG_MODE_FEI) || (MCG_MODE == MCG_MODE_FBI) || (MCG_MODE == MCG_MODE_BLPI))
/* Set MCG and OSC */
#if ((((SYSTEM_OSC_CR_VALUE)&OSC_CR_ERCLKEN_MASK) != 0x00U) ||   \
     ((((SYSTEM_MCG_C5_VALUE)&MCG_C5_PLLCLKEN_MASK) != 0x00U) && \
...
/* Set MCG and OSC */
    ...
    MCG->C5 = (SYSTEM_MCG_C5_VALUE) &
              (uint8_t)(~(MCG_C5_PLLCLKEN_MASK)); /* Set C5 (PLL settings, PLL reference divider etc.) */
...
/* Very-low-power run mode enable */
    ...
    SIM->CLKDIV3 = ((SIM->CLKDIV3) & (uint32_t)(~(SIM_CLKDIV3_PLLFLLFRAC_MASK | SIM_CLKDIV3_PLLFLLDIV_MASK))) |
                   ((SYSTEM_SIM_CLKDIV3_VALUE) &
                    (SIM_CLKDIV3_PLLFLLFRAC_MASK | SIM_CLKDIV3_PLLFLLDIV_MASK)); /* Selects the PLLFLL clock divider. */
#endif
...
/* Copy RW data for KEIL */
#if defined(__CC_ARM) || defined(__ARMCC_VERSION)
void init_rw(void)
{
    uint32_t n = 0;
    extern uint32_t Image$$RW_m_data$$RW$$Base[];
    extern uint32_t Load$$RW_m_data$$RW$$Base[];
    extern uint32_t Load$$RW_m_data$$RW$$Length[];
    extern uint32_t Image$$RW_m_data_2$$RW$$Base[];
    extern uint32_t Load$$RW_m_data_2$$RW$$Base[];
    extern uint32_t Load$$RW_m_data_2$$RW$$Length[];
    /* Copy RW_m_data */
    uint32_t *data_ram = Image$$RW_m_data$$RW$$Base;
    uint32_t *data_rom = Load$$RW_m_data$$RW$$Base;
    n                  = (uint32_t)Load$$RW_m_data$$RW$$Length;
    while (n--)
    {
        *data_ram++ = *data_rom++;
    }
    /* Copy RW_m_data2 */
    data_ram = Image$$RW_m_data_2$$RW$$Base;
    data_rom = Load$$RW_m_data_2$$RW$$Base;
    n        = (uint32_t)Load$$RW_m_data_2$$RW$$Length;
    while (n--)
    {
        *data_ram++ = *data_rom++;
    }
    /* ZI data init */
    extern uint32_t Image$$RW_m_data$$ZI$$Base[];
    extern uint32_t Image$$RW_m_data$$ZI$$Length[];
    extern uint32_t Image$$RW_m_data_2$$ZI$$Base[];
    extern uint32_t Image$$RW_m_data_2$$ZI$$Length[];
    data_ram = Image$$RW_m_data$$ZI$$Base;
    n        = (uint32_t)Image$$RW_m_data$$ZI$$Length;
    while (n--)
    {
        *data_ram++ = 0;
    }
    data_ram = Image$$RW_m_data_2$$ZI$$Base;
    n        = (uint32_t)Image$$RW_m_data_2$$ZI$$Length;
    while (n--)
    {
        *data_ram++ = 0;
    }
