...
void InitClock(void)
{
    ...
    g_sClockSetup.ui16M1PwmFreq = M1_PWM_FREQ; /* 10kHz */
                                              ...
                                              * PWM_MOD = FTM_CLOCK / PWM_FREQUNCY = 84 MHz / 10 kHz = 8400   */
...
void InitFTM0(void)
{
    ...
    */
    ...
    /* Fault 2 polarity setting - for MC33937 is polarity active high */
    FTM0->FLTPOL &= (~FTM_FLTPOL_FLT2POL_MASK);

    /* Enable Fault 2 - connected to MC33937 over current comparator output */
    FTM0->FLTCTRL |= FTM_FLTCTRL_FAULT2EN_MASK;
...
void InitADC16(void)
{
    ...
    PCC->CLKCFG[PCC_ADC2_INDEX] = (PCC_CLKCFG_CGC(1) | PCC_CLKCFG_PCS(2));
    ...
    /* set divide ratio to 8 */
    /* input clock select - SIRC/2 clock  */
    ADC2->CFG1 = (ADC_CFG1_MODE(1) | ADC_CFG1_ADIV(2));
    ...
    PCC->CLKCFG[PCC_ADC2_INDEX] = (PCC_CLKCFG_CGC(1) | PCC_CLKCFG_PCS(6));    
    ...
    /* ADC2, channel Ib = M1_ADC1_PH_B, , SAMPLE & RESULT = 0 */
    g_sM1AdcSensor.sCurrSec16.pui32AdcBasePhaB = (ADC_Type *)ADC2;
    ...
    g_sM1AdcSensor.sCurrSec23.pui32AdcBasePhaA = (ADC_Type *)ADC2;
    ...
    /* ADC1, channel Ia = M1_ADC1_PH_A, SAMPLE & RESULT = 0  */
    g_sM1AdcSensor.sCurrSec45.pui32AdcBasePhaB = (ADC_Type *)ADC2;
    ...
    g_sM1AdcSensor.pui32UdcbAdcBase = (ADC_Type *)ADC2;
...
void InitPDB(void)
{
    ...
    PCC->CLKCFG[PCC_PDB2_INDEX] = PCC_CLKCFG_CGC(TRUE); /* Sys Clock - 168MHz */
    ...
    PDB2->SC = (PDB_SC_PDBEN_MASK | PDB_SC_LDMOD(2) | PDB_SC_TRGSEL(0) | PDB_SC_PDBEIE_MASK);
