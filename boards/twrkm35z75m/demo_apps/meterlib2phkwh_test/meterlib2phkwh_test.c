...
static tMETERLIB2PH_DATA mlib = METERLIB2PH_CFG;
...
void main (void)
{
  ...
  while (1)
  {
    ...
    {
      METERLIB2PH_CalcURMS (&mlib);
      METERLIB2PH_CalcIRMS (&mlib);
      METERLIB2PH_CalcPAVG (&mlib);
    }

    METERLIB2PH_ReadURMS  (&mlib,&U_RMS_Ph1,&U_RMS_Ph2);
    METERLIB2PH_ReadIRMS  (&mlib,&I_RMS_Ph1,&I_RMS_Ph2);
    METERLIB2PH_ReadPAVG  (&mlib,&P_Ph1,&P_Ph2);
    METERLIB2PH_ReadS     (&mlib,&S_Ph1,&S_Ph2);
    cycle++;
  }
}
