...
static tMETERLIB3PH_DATA mlib = METERLIB3PH_CFG;
...
void main (void)
{
  ...
  while (1)
  {
    ...
    {
      METERLIB3PH_CalcURMS (&mlib);
      METERLIB3PH_CalcIRMS (&mlib);
      METERLIB3PH_CalcPAVG (&mlib);
    }

    METERLIB3PH_ReadURMS  (&mlib,&U_RMS_Ph1,&U_RMS_Ph2,&U_RMS_Ph3);
    METERLIB3PH_ReadIRMS  (&mlib,&I_RMS_Ph1,&I_RMS_Ph2,&I_RMS_Ph3);
    METERLIB3PH_ReadPAVG  (&mlib,&P_Ph1,&P_Ph2,&P_Ph3);
    METERLIB3PH_ReadS     (&mlib,&S_Ph1,&S_Ph2,&S_Ph3);
    cycle++;
  }
}
