...
void main (void)
{
  ...
  while (1)
  {
    ...
    {
      METERLIBLP1PH_CalcVarHours (&mlib,&varh_cnt,METERLIBLP_KVARH_PR(IMP_PER_KVARH));
    }

    if (!(cycle % DECIM_FACTOR))
    {
      METERLIBLP1PH_CalcAuxiliary(&mlib);
    }

    METERLIBLP1PH_ReadResults  (&mlib,&U_RMS,&I_RMS,&P,&Q,&S);
    cycle++;
  }
}
