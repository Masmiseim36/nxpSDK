...
void main (void)
{
  ...
  while (1)
  {
    ...
    {
      METERLIBLP2PH_CalcVarHours(&mlib,&varh_cnt,METERLIBLP_KVARH_PR(IMP_PER_KVARH));
    }

    if (!(cycle % DECIM_FACTOR))
    {
      METERLIBLP2PH_CalcAuxiliary(&mlib);
    }

    METERLIBLP2PH_ReadResultsPh1(&mlib,&U_RMS_Ph1,&I_RMS_Ph1,&P_Ph1,&Q_Ph1,&S_Ph1);
    METERLIBLP2PH_ReadResultsPh2(&mlib,&U_RMS_Ph2,&I_RMS_Ph2,&P_Ph2,&Q_Ph2,&S_Ph2);
    cycle++;
  }
}
