...
/* Configuration and the trigger source of LPTRM */
...
static void InitTriggerSource(void)
{
    ...
    LPTMR_Init(DEMO_LPTMR_BASEADDR, &lptmrConfig);

    frepUs = 1000000U / (INPUT_SIGNAL_FREQ * NR_SAMPLES) * 2;
    frepUs = ((frepUs * LPTMR_SOURCE_CLOCK) / 1000000U);

    LPTMR_SetTimerPeriod(DEMO_LPTMR_BASEADDR, frepUs);
    LPTMR_StartTimer(DEMO_LPTMR_BASEADDR);
    SIM->SOPT7 |= (SIM_SOPT7_ADCAALTTRGEN(0x2U) | SIM_SOPT7_ADCATRGSEL(0xEU));
}
...
int main(void)
{
    ...
    PRINTF("\r\ncadc_lptmr_trigger demo running...\r\n\r\n");
    ...
    LPTMR_StopTimer(DEMO_LPTMR_BASEADDR);
    LPTMR_Deinit(DEMO_LPTMR_BASEADDR);
