...
int main(void)
{
    ...
    PRINTF("CADC dual separate conversion example.\r\n");
    ...
    /* For converter A. */
    sampleConvAMask = CADC_SAMPLE_MASK(0U) | CADC_SAMPLE_MASK(1U);
    /* For converter B. */
    sampleConvBMask = CADC_SAMPLE_MASK(8U) | CADC_SAMPLE_MASK(9U);
    ...
    while (1)
    {
        ...
        if (sampleConvAMask == (sampleMask & CADC_GetSampleReadyStatusFlags(DEMO_CADC_BASEADDR)))
        {
        ...
        /* Wait the conversion to be done. */
        while (kCADC_ConverterBEndOfScanFlag !=
               (kCADC_ConverterBEndOfScanFlag & CADC_GetStatusFlags(DEMO_CADC_BASEADDR)))
        {
        }
        if (sampleConvBMask == (sampleMask & CADC_GetSampleReadyStatusFlags(DEMO_CADC_BASEADDR)))
        {
