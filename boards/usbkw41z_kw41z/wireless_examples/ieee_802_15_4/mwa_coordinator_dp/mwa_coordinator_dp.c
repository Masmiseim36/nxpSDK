...
static const uint16_t maShortAddress[] = {mDefaultValueOfShortAddress_c, mDefaultValueOfShortAddress_PAN1_c};
static const uint16_t maPanId[] = {mDefaultValueOfPanId_c, mDefaultValueOfPanId_PAN1_c};

/* The current logical channel (frequency band) */
static uint8_t maLogicalChannel[gMacInstancesCnt_c];

/* These byte arrays stores an associated
   devices long and short addresses. */
static uint16_t maDeviceShortAddress[] = {0xFFFF, 0xFFFF};
static uint64_t maDeviceLongAddress[gMacInstancesCnt_c];
...
static anchor_t mMlmeNwkInputQueue[gMacInstancesCnt_c];
static anchor_t mMcpsNwkInputQueue[gMacInstancesCnt_c];

static uint8_t mCurrentNwk;

static const uint64_t mExtendedAddress[]  = {mMacExtendedAddress_c, mMacExtendedAddress_PAN1_c};
static instanceId_t   macInstances[gMacInstancesCnt_c];
...
void main_task(uint32_t param)
{
    ...
    if( !initialized )
    {
        ...
        {
            macInstances[i] = BindToMAC( (instanceId_t)i );
            Mac_RegisterSapHandlers( MCPS_NWK_SapHandler, MLME_NWK_SapHandler, macInstances[i] );
        }
...
void App_init( void )
{
    ...
    for( i=0; i<gMacInstancesCnt_c; i++ )
    {
        ...
        MSG_InitQueue(&mMlmeNwkInputQueue[i]); 
        MSG_InitQueue(&mMcpsNwkInputQueue[i]);

        /* Initialize the MAC 802.15.4 extended address */
        Mac_SetExtendedAddress( (uint8_t*)&mExtendedAddress[i], i );
    }
...
{ 
    ...
    while(1)
    {
        ...
        if( ev & gAppEvtMessageFromMLME_c )
        {
          ...
          for( i=0; i<gMacInstancesCnt_c; i++ )
          {
              if (MSG_Pending(&mMlmeNwkInputQueue[i]))
              {
                  pMsgIn = MSG_DeQueue(&mMlmeNwkInputQueue[i]);
                  mCurrentNwk = i;
                  break;
              }
          }
...
#if gMpmIncluded_d
          Serial_Print(interfaceId,"MyWirelessApp Demo Coordinator Dual PAN application is initialized and ready.\n\r\n\r", gAllowToBlock_d);
#else
      ...
      case stateStartCoordinator:
          if (ev & gAppEvtStartCoordinator_c)
          {
              ...
              Serial_Print(interfaceId,"\n\rStarting as PAN coordinator on channel(s): ", gAllowToBlock_d);
              for( i=0; i<gMacInstancesCnt_c; i++)
              {
                  Serial_Print(interfaceId,"0x", gAllowToBlock_d);
                  Serial_PrintHex(interfaceId,&maLogicalChannel[i], 1, gPrtHexNoFormat_c);

                  if( (i+1) != gMacInstancesCnt_c )
              ...
              {        
                  ...
                  if(ret == errorNoError)
                  {
                      ...
                      for( i=0; i<gMacInstancesCnt_c; i++)
                      {
                          ...
                          Serial_PrintHex(interfaceId,(uint8_t *)&maPanId[i], 2, gPrtHexNoFormat_c);
                          Serial_Print(interfaceId,", and short address 0x", gAllowToBlock_d);
                          Serial_PrintHex(interfaceId,(uint8_t *)&maShortAddress[i], 2, gPrtHexNoFormat_c);
                          Serial_Print(interfaceId,".\n\r", gAllowToBlock_d);
                      }
      ...
      case stateListen:
          ...
          if (ev & gAppEvtMessageFromMLME_c)
          {
              ...
              {      
                  /* Process it */
                  ret = App_HandleMlmeInput(pMsgIn, mCurrentNwk);
      ...
      {      
          ...
          for( i=0; i<gMacInstancesCnt_c; i++ )
          {
              if( MSG_Pending(&mMcpsNwkInputQueue[i]) )
              {
                  pMsgIn = MSG_DeQueue(&mMcpsNwkInputQueue[i]);
                  mCurrentNwk = i;
                  break;
              }
          }
          ...
          {
              /* Process it */
              App_HandleMcpsInput(pMsgIn, mCurrentNwk);
...
static uint8_t App_StartScan(macScanType_t scanType, uint8_t appInstance)
{
  ...
  if(pMsg != NULL)
  {
    ...
    for( uint32_t i=1; i < ( sizeof(channelMask_t) / sizeof(uint32_t) ) ; i++ )
    {  
      pScanReq->scanChannels[i] = 0x00;
    }
#else
    ...
    if( NWK_MLME_SapHandler( pMsg, macInstances[appInstance] ) == gSuccess_c )
    {
...
{  
  ...
  Serial_PrintHex(interfaceId,&maLogicalChannel[0], 1, gPrtHexNoFormat_c);
#if !gMpmIncluded_d
  Serial_Print(interfaceId," was selected\n\r", gAllowToBlock_d);
#else
  Serial_Print(interfaceId," was selected for PAN0,\n\r", gAllowToBlock_d);
  Serial_Print(interfaceId,"and logical cannel 0x", gAllowToBlock_d);
  Serial_PrintHex(interfaceId,&maLogicalChannel[1], 1, 0);
  Serial_Print(interfaceId," was selected for PAN1.\n\r", gAllowToBlock_d);
#endif
...
static uint8_t App_StartCoordinator( uint8_t appInstance )
{
  ...
  if(pMsg != NULL)
  {
    ...
    pMsg->msgData.setReq.pibAttributeValue = (uint8_t *)&mExtendedAddress[appInstance];
    (void)NWK_MLME_SapHandler( pMsg, macInstances[appInstance] );
    ...
    pMsg->msgData.setReq.pibAttributeValue = (uint8_t *)&maShortAddress[appInstance];
    (void)NWK_MLME_SapHandler( pMsg, macInstances[appInstance] );
    ...
    FLib_MemCpy(&pStartReq->panId, (void *)&maPanId[appInstance], 2);
    /* Logical Channel - the default of 11 will be overridden */
    pStartReq->logicalChannel = maLogicalChannel[appInstance];
#ifdef gPHY_802_15_4g_d
...
static uint8_t App_SendAssociateResponse(nwkMessage_t *pMsgIn, uint8_t appInstance)
{
  ...
  if(pMsg != NULL)
  {
    ...
    /* Save device info. */
    FLib_MemCpy(&maDeviceShortAddress[appInstance], &pAssocRes->assocShortAddress, 2);
    FLib_MemCpy(&maDeviceLongAddress[appInstance],  &pAssocRes->deviceAddress,     8);
    
    /* Send the Associate Response to the MLME. */
    if( gSuccess_c == NWK_MLME_SapHandler( pMsg, macInstances[appInstance] ) )
    {
      Serial_Print( interfaceId,"Done\n\r", gAllowToBlock_d );
      return errorNoError;
    }
...
#if gMpmIncluded_d
      Serial_Print( interfaceId,"\n\rPAN", gAllowToBlock_d );
      Serial_PrintDec( interfaceId, appInstance );
      Serial_Print( interfaceId,":", gAllowToBlock_d );
#endif
...
static void App_TransmitUartData(void)
{
    ...
    for( i=0; i<gMacInstancesCnt_c; i++ )
    {
        ...
        {
            if( maDeviceShortAddress[i] != 0xFFFF )
            {
            ...
            if( 0 == i )
            {
                Serial_Read(interfaceId, keysBuffer, count, &count);
            }
        }
        ...
        if(mpPacket != NULL)
        {
            ...
            FLib_MemCpy(mpPacket->msgData.dataReq.pMsdu, keysBuffer, count);
            ...
            FLib_MemCpy(&mpPacket->msgData.dataReq.dstAddr, (void*)&maDeviceShortAddress[i], 2);
            FLib_MemCpy(&mpPacket->msgData.dataReq.srcAddr, (void*)&maShortAddress[0], 2);
            FLib_MemCpy(&mpPacket->msgData.dataReq.dstPanId, (void*)&maPanId[i], 2);
            FLib_MemCpy(&mpPacket->msgData.dataReq.srcPanId, (void*)&maPanId[i], 2);
            ...
            (void)NWK_MCPS_SapHandler(mpPacket, macInstances[i]);
