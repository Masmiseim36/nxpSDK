...
status_t WM8962_Init(wm8962_handle_t *handle, const wm8962_config_t *config)
{
    ...
    if (config->sysclkSource == kWM8962_SysClkSourceFLL)
    {
        /* Internal FLL config */
...
status_t WM8962_SetProtocol(wm8962_handle_t *handle, wm8962_protocol_t protocol)
{
    ...
    {
        WM8962_CHECK_RET(WM8962_ModifyReg(handle, WM8962_IFACE0, WM8962_IFACE0_FORMAT_MASK, (uint16_t)protocol) | 0x10U,
                         ret);
    }
    else
    {
