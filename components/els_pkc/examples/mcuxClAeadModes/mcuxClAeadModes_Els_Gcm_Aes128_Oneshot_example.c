...
/**
 * @file:  mcuxClAeadModes_Els_Gcm_Aes128_Oneshot_example.c
...
static ALIGNED uint8_t aes128_key[16u] = {0x2BU, 0x7EU, 0x15U, 0x16U,
                                          0x28U, 0xAEU, 0xD2U, 0xA6U,
                                          0xABU, 0xF7U, 0x15U, 0x88U,
                                          0x09U, 0xCFU, 0x4FU, 0x3CU};

/** IV of the AES encryption. */
static ALIGNED uint8_t aes128_iv[12u] = {0xF8U, 0xD2U, 0x68U, 0x76U,
                                         0x81U, 0x6FU, 0x0FU, 0xBAU,
                                         0x86U, 0x2BU, 0xD8U, 0xA3U};

/** Plaintext input for the AES encryption. */
static ALIGNED uint8_t const msg_plain[16u] = {0x6BU, 0xC1U, 0xBEU, 0xE2U,
                                               0x2EU, 0x40U, 0x9FU, 0x96U,
                                               0xE9U, 0x3DU, 0x7EU, 0x11U,
                                               0x73U, 0x93U, 0x17U, 0x2AU};

/** Additional authenticated data. */
static ALIGNED uint8_t const msg_adata[16u] = {0xCAU, 0xEAU, 0x07U, 0x26U,
...
static ALIGNED uint8_t const msg_enc_expected[16u] = {0x4FU, 0x74U, 0x2DU, 0xF6U,
                                                      0x9DU, 0x1CU, 0x03U, 0x6BU,
                                                      0x56U, 0xBCU, 0xC2U, 0x81U,
                                                      0x5FU, 0xDAU, 0x8DU, 0x6DU};

/** Expected authentication tag output. */
static ALIGNED uint8_t const msg_tag_expected[16u] = {0xB2U, 0xC5U, 0xCFU, 0xC3U,
...
MCUXCLEXAMPLE_FUNCTION(mcuxClAeadModes_Els_Gcm_Aes128_Oneshot_example)
{
    ...
    MCUXCLEXAMPLE_ALLOCATE_AND_INITIALIZE_SESSION(session, MCUXCLEXAMPLE_MAX_WA(MCUXCLAEAD_CRYPT_CPU_WA_BUFFER_SIZE, MCUXCLRANDOMMODES_NCINIT_WACPU_SIZE), 0u);
    ...
    ALIGNED uint8_t msg_enc[sizeof(msg_enc_expected)];
    uint32_t msg_enc_size = 0u;

    ALIGNED uint8_t msg_tag[32];
    ...
    ALIGNED uint8_t msg_dec[sizeof(msg_plain)];
    ...
    for (size_t i = 0U; i < sizeof(msg_plain); i++)
    {
