...
#define SET_GINT_USED_FLAG(x)   (s_GINT_usedFlag |= 1 << x)
#define GET_GINT_USED_FLAG(x)   ((s_GINT_usedFlag & (1 << x)) ? 1 : 0)
#define CLEAR_GINT_USED_FLAG(x) (s_GINT_usedFlag &= ~(1 << x))
...
hal_gpio_status_t HAL_GpioInit(hal_gpio_handle_t gpioHandle, hal_gpio_pin_config_t *pinConfig)
{
    ...
    for (int i = 0; i < FSL_FEATURE_SOC_GINT_COUNT; i++)
    {
        if (0 == GET_GINT_USED_FLAG(i))
        {
            gpioStateHandle->gintInstance = i;
            SET_GINT_USED_FLAG(i);
            break;
        }
    }
    ...
    gpioStateHandle->pin.direction = pinConfig->direction;
...
hal_gpio_status_t HAL_GpioDeinit(hal_gpio_handle_t gpioHandle)
{
    ...
    /* Relase the instance */
    CLEAR_GINT_USED_FLAG(gpioStateHandle->gintInstance);
...
hal_gpio_status_t HAL_GpioSetTriggerMode(hal_gpio_handle_t gpioHandle, hal_gpio_interrupt_trigger_t gpioTrigger)
{
    ...
    if (!s_GINT_initialized)
    {
        s_GINT_initialized = 1;
        GINT_Init(gintList[gpioStateHandle->gintInstance]);
    }

    /* Set trigger type to GINT */
    GINT_SetCtrl(gintList[gpioStateHandle->gintInstance], kGINT_CombineOr, triggerType,
                 (gint_cb_t)gpioStateHandle->callback);
