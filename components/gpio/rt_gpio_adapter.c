...
#define SET_GPIO_PORT_INIT_FLAG(port)   (s_GPIO_PORT_initFlag |= 1 << port)
#define GET_GPIO_PORT_INIT_FLAG(port)   ((s_GPIO_PORT_initFlag & (1 << port)) ? 1 : 0)
#define CLEAR_GPIO_PORT_INIT_FLAG(port) (s_GPIO_PORT_initFlag &= ~(1 << port))

/*******************************************************************************
 * Code
 ******************************************************************************/

void HAL_GpioInterruptHandle(uint32_t intNum)
{
    ...
    while (NULL != head)
    {
        ...
        if (portIntFlags & (1 << head->pin.pin))
        {
            ...
            if (kHAL_GpioInterruptEitherEdge == head->pin.trigger)
            {
                pinLevel               = GPIO_PinRead(GPIO, head->pin.port, head->pin.pin);
                triggerConfig.mode     = kGPIO_PinIntEnableEdge;
                triggerConfig.polarity = (pinLevel) ? kGPIO_PinIntEnableLowOrFall : kGPIO_PinIntEnableHighOrRise;
...
hal_gpio_status_t HAL_GpioInit(hal_gpio_handle_t gpioHandle, hal_gpio_pin_config_t *pinConfig)
{
    ...
    /* Critical protection */
    regPrimask = DisableGlobalIRQ();
    if (0 == GET_GPIO_PORT_INIT_FLAG(gpioState->pin.port))
    {
...
hal_gpio_status_t HAL_GpioGetInput(hal_gpio_handle_t gpioHandle, uint8_t *pinState)
{
    ...
    *pinState = GPIO_PinRead(GPIO, gpioStateHandle->pin.port, gpioStateHandle->pin.pin);
...
hal_gpio_status_t HAL_GpioSetOutput(hal_gpio_handle_t gpioHandle, uint8_t pinState)
{
    ...
    GPIO_PinWrite(GPIO, gpioStateHandle->pin.port, gpioStateHandle->pin.pin, (pinState) ? 1 : 0);
...
hal_gpio_status_t HAL_GpioSetTriggerMode(hal_gpio_handle_t gpioHandle, hal_gpio_interrupt_trigger_t gpioTrigger)
{
    ...
    assert(kHAL_GpioDirectionOut != gpioStateHandle->pin.direction);

    switch (gpioTrigger)
    {
        case kHAL_GpioInterruptLogicZero:
            triggerConfig.mode     = kGPIO_PinIntEnableLevel;
            triggerConfig.polarity = kGPIO_PinIntEnableLowOrFall;
            break;
        case kHAL_GpioInterruptLogicOne:
            triggerConfig.mode     = kGPIO_PinIntEnableLevel;
            triggerConfig.polarity = kGPIO_PinIntEnableHighOrRise;
            break;
