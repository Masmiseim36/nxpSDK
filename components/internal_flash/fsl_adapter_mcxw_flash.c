...
*************************************************************************************
********************************************************************************** */
#ifndef HAL_FLASH_ROMAPI_DRIVER
#define HAL_FLASH_ROMAPI_DRIVER (1U)
#endif
#if (defined(HAL_FLASH_ROMAPI_DRIVER) && (HAL_FLASH_ROMAPI_DRIVER == 1U))
#include "fsl_iap.h"
#else
#error Internal flash can only be used in combination with ROM API
#endif
#include "fsl_adapter_flash.h"

#define FSL_FEATURE_FLASH_BLOCK_PHRASE_SIZE (16U)
...
static flash_property_tag_t HAL_FlashGetPropertyTag(hal_flash_property_tag_t tag)
{
    ...
    switch (tag)
    {
        case kHAL_Flash_PropertyPflashSectorSize:
            ret = kFLASH_PropertyPflashSectorSize;
            break;
        case kHAL_Flash_PropertyPflashTotalSize:
            ret = kFLASH_PropertyPflashTotalSize;
            break;
        case kHAL_Flash_PropertyPflashBlockCount:
            ret = kFLASH_PropertyPflashBlockCount;
            break;
        case kHAL_Flash_PropertyPflashBlockBaseAddr:
            ret = kFLASH_PropertyPflashBlockBaseAddr;
            break;
        case kHAL_Flash_PropertyPflashBlockSize:
            ret = kFLASH_PropertyPflashBlockSize;
            break;
        default:
            /*MISRA rule 16.4*/
...
static hal_flash_status_t HAL_FlashProgramAdaptation(uint32_t dest, uint32_t size, uint8_t *pData)
{
    ...
    status = FLASH_Program(&s_flashConfig, dest, (uint8_t *)pData, size);
...
hal_flash_status_t HAL_FlashVerifyErase(uint32_t start, uint32_t lengthInBytes, hal_flash_margin_value_t margin)
{
    ...
    (void)margin;
    status = FLASH_VerifyErase(&s_flashConfig, start, lengthInBytes);
    EnableGlobalIRQ(regPrimask);
    return HAL_FlashGetStatus(status);
}
