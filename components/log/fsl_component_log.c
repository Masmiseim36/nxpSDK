...
{
    log_print_buffer_t *buffer = (log_print_buffer_t *)buf;
    int i                      = 0;

    for (i = 0; i < len; i++)
    {
        if (((*indicator) + buffer->sofar + 1) >= buffer->length)
        {
            break;
        }

        if ((val == '\n') && ((0 == (*indicator)) || ('\r' != buffer->buffer[(*indicator) + buffer->sofar - 1])))
        {
            buffer->buffer[(*indicator) + buffer->sofar] = '\r';
            (*indicator)++;
        }

        buffer->buffer[(*indicator) + buffer->sofar] = val;
        (*indicator)++;
    }
}
...
log_status_t LOG_Init(void)
{
    ...
    for (i = 1; i < LOG_MAX_BUFF_LOG_COUNT; i++)
    {
                     ...
                     LOG_ARGUMENT_TYPE argv[])
{
    ...
    assert(((LOG_MAX_ACTUAL_ARGUMENT_COUNT) >= argc));
    ...
    if (NULL != node)
    {
        ...
        node->argc         = argc;
        node->next         = NULL;
        memcpy(node->argv, argv, sizeof(argv[0]) * argc);
...
void LOG_Printf(log_module_t const *module, log_level_t level, unsigned int timeStamp, char const *format, ...)
{
    ...
    {
        memcpy(&buffer, &s_logContext.printBuffer, sizeof(buffer));
    }
    else
#endif
    {
...
void LOG_Dump(uint8_t *buffer, size_t length, size_t *outLength)
{
    ...
    s_logContext.printBuffer.sofar  = 0;
    (void)LOG_DumpInternal();
    if ((NULL != buffer) && (0 != length) && (NULL != outLength))
    {
