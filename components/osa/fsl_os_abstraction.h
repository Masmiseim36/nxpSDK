...
externosa_task_def_t os_thread_def_##name
/*  ==== Timer Management  ====
...
osa_time_def_t os_timer_def_##name = \
{ (function), NULL }
 ...
 * @param semaphoreHandle      Pointer to a memory space of size #OSA_SEM_HANDLE_SIZE allocated by the caller, semaphore handle.
 ...
 *
 * @param mutexHandle       Pointer to a memory space of size #OSA_MUTEX_HANDLE_SIZE allocated by the caller, mutex handle.
 *
 ...
 * be cleared manually.
 * @param eventHandle       Pointer to a memory space of size #OSA_EVENT_HANDLE_SIZE allocated by the caller, Event handle.
 ...
 * @param msgqHandle    Pointer to a memory space of size #(OSA_MSGQ_HANDLE_SIZE + msgNo*OSA_MSG_HANDLE_SIZE) allocated by the caller, message queue handle.
