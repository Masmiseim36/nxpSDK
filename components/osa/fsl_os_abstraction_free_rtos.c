...
{
#if (defined(FSL_OSA_FREERTOS_TASK_ENABLE) && (FSL_OSA_FREERTOS_TASK_ENABLE > 0U))
...
osa_status_t OSA_MutexCreate(osa_mutex_handle_t mutexHandle)
{
    ...
    xMutexHandle.mutex = xSemaphoreCreateMutex();
    if (xMutexHandle.mutex != NULL)
    {
