...
#if (defined(PANIC_ENABLE_LOG) && (PANIC_ENABLE_LOG > 0U))
    PRINTF("System Panic happen: \r\n");
    PRINTF("Panic id: 0x%x \r\n", id);
    PRINTF("Panic location: 0x%x \r\n", location);
    PRINTF("Panic extra1: 0x%x \r\n", extra1);
    PRINTF("Panic extra2: 0x%x \r\n", extra2);
    PRINTF("Panic linkRegister: 0x%x \r\n", panic_data.linkRegister);
#endif

    DisableGlobalIRQ(); /* disable interrupts */
