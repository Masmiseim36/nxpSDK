...
#define gHalTimerInstanceNum_c (0x4)
...
hal_timer_status_t HAL_TimerInit(hal_timer_handle_t halTimerHandle, hal_timer_config_t* halTimerConfig)
{
    ...
    PIT_SetTimerPeriod(instanceList[halTimerState->instance], kPIT_Chnl_0, USEC_TO_COUNT(halTimerState->timeout, halTimerState->timerClock_Hz));
...
uint32_t HAL_TimerGetCurrentTimerCount(hal_timer_handle_t halTimerHandle)
{
    ...
    assert(halTimerHandle);
    hal_timer_handle_struct_t* halTimerState = halTimerHandle;
    return COUNT_TO_USEC(PIT_GetCurrentTimerCount(instanceList[halTimerState->instance],kPIT_Chnl_0), halTimerState->timerClock_Hz);   
}
