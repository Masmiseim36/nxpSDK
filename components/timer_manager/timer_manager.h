...
*****************************************************************************/
/*
 * @brief   Configures the common task enable.If set to 1, then timer will use common task and consume less ram/flash size.
 */
...
typedef enum _timer_mode{
    kTimerModeSingleShot     = 0x01,     /**< The timer will expire only once. */
    kTimerModeIntervalTimer  = 0x02,     /**< The timer will restart each time it expires. */
    kTimerModeSetMinuteTimer = 0x04,     /**< The timer will one minute timer. */
    kTimerModeSetSecondTimer = 0x08,     /**< The timer will one second timer. */
    kTimerModeLowPowerTimer  = 0x10,     /**< The timer will low power mode timer. */
} timer_mode_t;
 ...
 *
 */
timer_status_t TM_InstallCallback
(
    timer_handle_t timerHandle,
 ...
 * @retval kStatus_TimerError      An error occurred.
 */
timer_status_t TM_Start
(
    timer_handle_t timerHandle,
    timer_mode_t timeMode,
 ...
 */ 
uint32_t TM_GetFirstExpireTime
(
     uint8_t timerType
);
...
*/ 
timer_handle_t TM_GetFirstTimerWithParam
(
    void * param
);
