 ...
 ******************************************************************************/
#ifndef NDEBUG
#if (defined(DEBUG_CONSOLE_ASSERT_DISABLE) &&                                  \
     (DEBUG_CONSOLE_ASSERT_DISABLE > 0U))
#undef assert
#define assert(n)
#endif
#endif
...
#if defined(FSL_FEATURE_LPUART_HAS_SEPARATE_RX_TX_IRQ) &&                      \
...
#if !(defined(HAL_UART_TRANSFER_MODE) && (HAL_UART_TRANSFER_MODE > 0U))
static hal_uart_state_t
    *s_UartState[sizeof(s_LpuartAdapterBase) / sizeof(LPUART_Type *)];
#endif
...
#if (defined(HAL_UART_TRANSFER_MODE) && (HAL_UART_TRANSFER_MODE > 0U))
static void HAL_UartCallback(LPUART_Type *base, lpuart_handle_t *handle,
                             status_t status, void *callbackParam) {
...
static void HAL_UartInterruptHandle(uint8_t instance) {
  ...
  if ((LPUART_STAT_RDRF_MASK & status) &&
      (LPUART_GetEnabledInterrupts(s_LpuartAdapterBase[instance]) &
       kLPUART_RxDataRegFullInterruptEnable)) {
    if (uartHandle->rx.buffer) {
      uartHandle->rx.buffer[uartHandle->rx.bufferSofar++] =
          LPUART_ReadByte(s_LpuartAdapterBase[instance]);
      if (uartHandle->rx.bufferSofar >= uartHandle->rx.bufferLength) {
        ...
        if (uartHandle->callback) {
          uartHandle->callback(uartHandle, kStatus_HAL_UartRxIdle,
  ...
  if ((LPUART_STAT_TDRE_MASK & status) &&
    ...
    if (uartHandle->tx.buffer) {
      LPUART_WriteByte(s_LpuartAdapterBase[instance],
      ...
      if (uartHandle->tx.bufferSofar >= uartHandle->tx.bufferLength) {
        ...
        if (uartHandle->callback) {
          uartHandle->callback(uartHandle, kStatus_HAL_UartTxIdle,
                               uartHandle->callbackParam);
        }
      }
    }
...
hal_uart_status_t HAL_UartInit(hal_uart_handle_t handle,
  ...
  assert(config->instance <
         (sizeof(s_LpuartAdapterBase) / sizeof(LPUART_Type *)));
  ...
  status = LPUART_Init(s_LpuartAdapterBase[config->instance], &lpuartConfig,
                       config->srcClock_Hz);
  ...
  LPUART_TransferCreateHandle(
      s_LpuartAdapterBase[config->instance], &uartHandle->hardwareHandle,
...
hal_uart_status_t HAL_UartReceiveBlocking(hal_uart_handle_t handle,
  ...
  status = LPUART_ReadBlocking(s_LpuartAdapterBase[uartHandle->instance], data,
                               length);
...
hal_uart_status_t
HAL_UartTransferInstallCallback(hal_uart_handle_t handle,
                                hal_uart_transfer_callback_t callback,
                                void *callbackParam) {
  hal_uart_state_t *uartHandle;
...
hal_uart_status_t
HAL_UartTransferReceiveNonBlocking(hal_uart_handle_t handle,
  ...
  status = LPUART_TransferReceiveNonBlocking(
      s_LpuartAdapterBase[uartHandle->instance], &uartHandle->hardwareHandle,
...
hal_uart_status_t
HAL_UartTransferSendNonBlocking(hal_uart_handle_t handle,
                                hal_uart_transfer_t *transfer) {
  hal_uart_state_t *uartHandle;
  status_t status;
  assert(handle);
  ...
  status = LPUART_TransferSendNonBlocking(
      s_LpuartAdapterBase[uartHandle->instance], &uartHandle->hardwareHandle,
...
hal_uart_status_t HAL_UartTransferGetReceiveCount(hal_uart_handle_t handle,
  ...
  status =
      LPUART_TransferGetReceiveCount(s_LpuartAdapterBase[uartHandle->instance],
                                     &uartHandle->hardwareHandle, count);
...
hal_uart_status_t HAL_UartTransferGetSendCount(hal_uart_handle_t handle,
  ...
  status =
      LPUART_TransferGetSendCount(s_LpuartAdapterBase[uartHandle->instance],
                                  &uartHandle->hardwareHandle, count);
...
hal_uart_status_t HAL_UartTransferAbortReceive(hal_uart_handle_t handle) {
  ...
  LPUART_TransferAbortReceive(s_LpuartAdapterBase[uartHandle->instance],
                              &uartHandle->hardwareHandle);
...
hal_uart_status_t HAL_UartTransferAbortSend(hal_uart_handle_t handle) {
  ...
  LPUART_TransferAbortSend(s_LpuartAdapterBase[uartHandle->instance],
                           &uartHandle->hardwareHandle);
...
#endif
#endif
  LPUART_TransferHandleIRQ(s_LpuartAdapterBase[uartHandle->instance],
                           &uartHandle->hardwareHandle);
#if 0
#if defined(FSL_FEATURE_LPUART_HAS_SEPARATE_RX_TX_IRQ) &&                      \
    FSL_FEATURE_LPUART_HAS_SEPARATE_RX_TX_IRQ
...
#if defined(FSL_FEATURE_LPUART_HAS_SHARED_IRQ0_IRQ1) &&                        \
    FSL_FEATURE_LPUART_HAS_SHARED_IRQ0_IRQ1
#if defined(FSL_FEATURE_LPUART_HAS_SEPARATE_RX_TX_IRQ) &&                      \
    FSL_FEATURE_LPUART_HAS_SEPARATE_RX_TX_IRQ
void LPUART0_LPUART1_RX_IRQHandler(void) {
  if ((s_UartState[0])) {
    if ((LPUART_STAT_OR_MASK & LPUART0->STAT) ||
        ((LPUART_STAT_RDRF_MASK & LPUART0->STAT) &&
         (LPUART_CTRL_RIE_MASK & LPUART0->CTRL))) {
      HAL_UartInterruptHandle(0);
    }
  }
  if ((s_UartState[1])) {
    if ((LPUART_STAT_OR_MASK & LPUART1->STAT) ||
        ((LPUART_STAT_RDRF_MASK & LPUART1->STAT) &&
         (LPUART_CTRL_RIE_MASK & LPUART1->CTRL))) {
