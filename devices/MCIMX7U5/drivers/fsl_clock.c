...
uint32_t CLOCK_GetIpFreq(clock_ip_name_t name)
{
    ...
    else
    {
        ...
        {
            case kCLOCK_IpSrcNone:
                freq = 0U;
                break;
            case kCLOCK_IpSrcPllPfdAsync:
                if (kCLOCK_Qspi == name)
                {
                    freq = CLOCK_GetSysPllPfdFreq(kSCG_SysPllPfd3Clk);
                }
            ...
            case kCLOCK_IpSrcSysPllAsync:
                ...
                break;
            case kCLOCK_IpSrcSystem:
                freq = CLOCK_GetSysClkFreq(sysClk);
                break;
            case kCLOCK_IpSrcRtcAuxPllAsync:
                if (kCLOCK_Qspi == name)
                {
 ...
 *                                              .prediv = 5U,
 *                                              .pfdClkout = kSCG_AuxPllPfd3Clk, // Configure APLL PFD3 as the output
 * clock
 *                                              .mult = 20U,
 ...
 *                                              .prediv = 0U,
 *                                              .pfdClkout = kSCG_SysPllPfd0Clk, // No need to configure pfdClkout; only
 * needed for initialization
 *                                              .mult = 3U};
 ...
 *                                            .prediv = 5U,
 *                                            .pfdClkout = kSCG_AuxPllPfd3Clk, // Selects the SPLL PFD3 as the output
 * clock
 *                                            .mult = 20U};
 * CLOCK_InitSysPll(&g_scgSysPllConfig);
