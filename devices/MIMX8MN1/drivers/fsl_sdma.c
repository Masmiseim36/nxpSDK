...
#define SDMA_P2P_HIGH_WATERMARK_MASK (0xFFU << SDMA_P2P_HIGH_WATERMARK_SHIFT)
...
#define SDMA_P2P_SOURCE_SPBA_MASK (1 << SDMA_P2P_SOURCE_SPBA_SHIFT)
...
#define SDMA_P2P_DEST_SPBA_MASK (1 << SDMA_P2P_DEST_SPBA_SHIFT)
...
#define SDMA_P2P_LOWER_EVENT_REG_MASK (1U << SDMA_P2P_LOWER_EVENT_REG_SHIFT)
...
#define SDMA_P2P_HIGHER_EVENT_REG_MASK (1U << SDMA_P2P_HIGHER_EVENT_REG_SHIFT)
...
#define SDMA_P2P_CONT_MASK (1U << SDMA_P2P_CONT_SHIFT)
...
static void SDMA_LoadContext(sdma_handle_t *handle, const sdma_transfer_config_t *config)
{
    ...
    else
    {
        ...
        context->GeneralReg[7] =
            ...
            ((config->swDone.enableSwDone & (uint32_t)kSDMA_MultiFifoSwDoneMask) << kSDMA_MultiFifoSwDoneShift) |
                             ...
                             sdma_p2p_config_t *p2p)
{
    ...
    if (p2p->sourceWatermark > p2p->destWatermark)
    {
        ...
        config->eventMask0 = (1 << (config->eventSource % 32));
        config->eventMask1 = (1 << (config->eventSource1 % 32));
    }
    else
    {
        config->watermarkLevel |=
            SDMA_P2P_LOW_WATERMARK(p2p->sourceWatermark) | SDMA_P2P_HIGH_WATERMARK(p2p->destWatermark);
        config->eventMask0 = (1 << (config->eventSource1 % 32));
        config->eventMask1 = (1 << (config->eventSource % 32));
    }
}
...
void SDMA_StartTransfer(sdma_handle_t *handle)
{
    ...
    if ((handle->eventSource != 0) || (handle->eventSource1 != 0))
    {
