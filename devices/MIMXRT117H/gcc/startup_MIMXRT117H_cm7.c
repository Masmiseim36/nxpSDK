//*****************************************************************************
// MIMXRT117H_cm7 startup code for use with MCUXpresso IDE
//
// Version : 140421
//*****************************************************************************
//
...
// FlexRAM sections
extern void __base_SRAM_DTC_cm7(void);
extern void __top_SRAM_DTC_cm7(void);
extern void __base_SRAM_ITC_cm7(void);
extern void __top_SRAM_ITC_cm7(void);

// Map managed linker file configuration to FlexRAM config
static void map_flexram(void)
{
    unsigned int dtcSize = (unsigned int)__top_SRAM_DTC_cm7 - (unsigned int)__base_SRAM_DTC_cm7;
    unsigned int itcSize = (unsigned int)__top_SRAM_ITC_cm7 - (unsigned int)__base_SRAM_ITC_cm7;
    unsigned int ocSize  = 0;

    unsigned int dtcBankCount = dtcSize / 0x8000U;
    unsigned int itcBankCount = itcSize / 0x8000U;
    unsigned int ocBankCount  = 0;
...
#define SAFE_STACK_TOP (0x202C0000) // Temporary stack pointer to end of first block of SRAM OC1 (always there)

#else

#define SAFE_STACK_TOP __StackTop // Safe stack pointer entry if not reconfiguring
...
// library.
//*****************************************************************************
__attribute__((section(".after_vectors.reset"))) void Reset_Handler(void)
{
    // Disable interrupts
    __asm volatile("cpsid i");

#if defined(CONFIG_FLEXRAM_AT_STARTUP) && (CONFIG_FLEXRAM_AT_STARTUP == 1)
    __asm volatile("MSR MSP, %0" : : "r"(SAFE_STACK_TOP) :);
    __asm volatile("MSR PSP, %0" : : "r"(SAFE_STACK_TOP) :);
    // Map FlexRAM
    map_flexram();
    ...
    volatile unsigned int vCompilerStackTop     = (unsigned int)&__StackTop;
