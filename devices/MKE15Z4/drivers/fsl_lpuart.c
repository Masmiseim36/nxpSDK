...
#endif
        {
            statusFlag = (int32_t)LPUART_GetStatusFlags(base);

            if (kLPUART_RxOverrunFlag == (statusFlag & kLPUART_RxOverrunFlag))
            {
                return ((kStatus_Success == LPUART_ClearStatusFlags(base, kLPUART_RxOverrunFlag)) ?
                            (kStatus_LPUART_RxHardwareOverrun) :
                            (kStatus_LPUART_FlagCannotClearManually));
            }

            if (kLPUART_NoiseErrorFlag == (statusFlag & kLPUART_NoiseErrorFlag))
            {
                return ((kStatus_Success == LPUART_ClearStatusFlags(base, kLPUART_NoiseErrorFlag)) ?
                            (kStatus_LPUART_NoiseError) :
                            (kStatus_LPUART_FlagCannotClearManually));
            }

            if (kLPUART_FramingErrorFlag == (statusFlag & kLPUART_FramingErrorFlag))
            {
                return ((kStatus_Success == LPUART_ClearStatusFlags(base, kLPUART_FramingErrorFlag)) ?
                            (kStatus_LPUART_FramingError) :
                            (kStatus_LPUART_FlagCannotClearManually));
            }

            if (kLPUART_ParityErrorFlag == (statusFlag & kLPUART_ParityErrorFlag))
            {
                return ((kStatus_Success == LPUART_ClearStatusFlags(base, kLPUART_ParityErrorFlag)) ?
...
void LPUART_TransferHandleIRQ(LPUART_Type *base, lpuart_handle_t *handle)
{
    ...
    /* If RX overrun. */
    if (kLPUART_RxOverrunFlag == (kLPUART_RxOverrunFlag & status))
    {
