...
void RTC_Init(RTC_Type *base, const rtc_config_t *config)
{
    ...
    if (kRTC_TimeInvalidFlag == (RTC_GetStatusFlags(RTC) & kRTC_TimeInvalidFlag))
    {
        RTC_Reset(RTC);
    }
...
void RTC_ClearStatusFlags(RTC_Type *base, uint32_t mask)
{
    ...
    if (kRTC_AlarmFlag == (mask & kRTC_AlarmFlag))
    {
        base->TAR = 0U;
    }
     ...
     */
    if (kRTC_TimeOverflowFlag == (mask & kRTC_TimeOverflowFlag))
    {
        base->TSR = 1U;
    }
     ...
     * The time counter should be disabled for this write to be successful
     */
    if (kRTC_TimeInvalidFlag == (mask & kRTC_TimeInvalidFlag))
    {
        base->TSR = 1U;
    }
