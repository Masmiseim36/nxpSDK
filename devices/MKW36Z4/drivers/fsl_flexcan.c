                                           ...
                                           flexcan_timing_config_t *pTimingConfig)
{
    ...
    do
    {
        ...
        if (clk < sourceClock_Hz)
        {
            break; /* Non-supporting: the frequency of clock source is not divisible by target baud rate, the user
...
void FLEXCAN_SetRxFifoConfig(CAN_Type *base, const flexcan_rx_fifo_config_t *config, bool enable)
{
    ...
    volatile uint32_t *idFilterRegion;
    uint8_t setup_mb, i, j = 0, k, rffn = 0, numMbOccupy;
    ...
    if (enable)
    {
        ...
        for (i = 6U; i < numMbOccupy; i++)
        {
            ...
            for (k = 0; k < 4U; k++)
            {
                 ...
                 * as 0xffffffff.
                 */
                idFilterRegion[k] = (j < config->idFilterNum) ? (config->idFilterTable[j]) : 0xFFFFFFFFU;
...
status_t FLEXCAN_TransferSendBlocking(CAN_Type *base, uint8_t mbIdx, flexcan_frame_t *txFrame)
{
    ...
    if (kStatus_Success == FLEXCAN_WriteTxMb(base, mbIdx, txFrame))
    {
        ...
        txFrame->timestamp = (base->MB[mbIdx].CS & CAN_CS_TIME_STAMP_MASK) >> CAN_CS_TIME_STAMP_SHIFT;
...
status_t FLEXCAN_TransferFDSendBlocking(CAN_Type *base, uint8_t mbIdx, flexcan_fd_frame_t *txFrame)
{
    ...
    if (kStatus_Success == FLEXCAN_WriteFDTxMb(base, mbIdx, txFrame))
    {
        ...
        volatile uint32_t *mbAddr = &(base->MB[0].CS);
        uint32_t offset = FLEXCAN_GetFDMailboxOffset(base, mbIdx);
        txFrame->timestamp = (mbAddr[offset] & CAN_CS_TIME_STAMP_MASK) >> CAN_CS_TIME_STAMP_SHIFT;
...
status_t FLEXCAN_TransferFDSendNonBlocking(CAN_Type *base, flexcan_handle_t *handle, flexcan_mb_transfer_t *xfer)
{
    ...
    {
        /* Distinguish transmit type. */
        if (kFLEXCAN_FrameTypeRemote == xfer->framefd->type)
        {
            handle->mbState[xfer->mbIdx] = (uint8_t)kFLEXCAN_StateTxRemote;
        }
        else
...
void FLEXCAN_TransferAbortSend(CAN_Type *base, flexcan_handle_t *handle, uint8_t mbIdx)
{
    ...
    assert(handle);
    assert(mbIdx <= (base->MCR & CAN_MCR_MAXMB_MASK));
    assert(handle->mbFrameBuf[mbIdx]);
    assert(!FLEXCAN_IsMbOccupied(base, mbIdx));
    ...
    uint16_t timestamp = (uint16_t)((base->MB[mbIdx].CS & CAN_CS_TIME_STAMP_MASK) >> CAN_CS_TIME_STAMP_SHIFT);
...
void FLEXCAN_TransferFDAbortSend(CAN_Type *base, flexcan_handle_t *handle, uint8_t mbIdx)
{
    ...
    assert(handle);
    assert(mbIdx <= (base->MCR & CAN_MCR_MAXMB_MASK));
    assert(handle->mbFDFrameBuf[mbIdx]);
    assert(!FLEXCAN_IsMbOccupied(base, mbIdx));
    ...
    volatile uint32_t *mbAddr = &(base->MB[0].CS);
    uint32_t offset           = FLEXCAN_GetFDMailboxOffset(base, mbIdx);
    uint16_t timestamp        = (uint16_t)((mbAddr[offset] & CAN_CS_TIME_STAMP_MASK) >> CAN_CS_TIME_STAMP_SHIFT);
