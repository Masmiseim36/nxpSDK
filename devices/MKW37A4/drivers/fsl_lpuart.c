...
/*!
 * @brief Handle idle line in IRQ.
 *
 * @param base LPUART peripheral base address.
 * @param handle LPUART handle pointer.
 */
static void LPUART_TransferHandleIdleLine(LPUART_Type *base, lpuart_handle_t *handle);

/*!
 * @brief Handle RX data in IRQ.
 *
 * @param base LPUART peripheral base address.
 * @param handle LPUART handle pointer.
 */
static void LPUART_TransferHandleRX(LPUART_Type *base, lpuart_handle_t *handle);

/*!
 * @brief Handle TX data in IRQ.
 *
 * @param base LPUART peripheral base address.
 * @param handle LPUART handle pointer.
 */
static void LPUART_TransferHandleTX(LPUART_Type *base, lpuart_handle_t *handle);
...
status_t LPUART_SetBaudRate(LPUART_Type *base, uint32_t baudRate_Bps, uint32_t srcClock_Hz)
{
    ...
    for (osrTemp = 4u; osrTemp <= 32u; osrTemp++)
    {
        ...
        /* Calculate the baud rate based on the temporary OSR and SBR values */
        calculatedBaud = (srcClock_Hz / (osrTemp * (uint32_t)sbrTemp));
...
status_t LPUART_ClearStatusFlags(LPUART_Type *base, uint32_t mask)
{
    ...
    temp |= mask & (LPUART_STAT_RXEDGIF_MASK | LPUART_STAT_IDLE_MASK | LPUART_STAT_OR_MASK | LPUART_STAT_NF_MASK |
                    LPUART_STAT_FE_MASK | LPUART_STAT_PF_MASK);
#if defined(FSL_FEATURE_LPUART_HAS_ADDRESS_MATCHING) && FSL_FEATURE_LPUART_HAS_ADDRESS_MATCHING
    temp &= (uint32_t)(~((uint32_t)((uint32_t)LPUART_STAT_MA2F_MASK | (uint32_t)LPUART_STAT_MA1F_MASK)));
...
void LPUART_TransferHandleIRQ(LPUART_Type *base, lpuart_handle_t *handle)
{
    ...
    {
        LPUART_TransferHandleIdleLine(base, handle);
    }
    ...
    {
        LPUART_TransferHandleRX(base, handle);
    }
    ...
    {
        LPUART_TransferHandleTX(base, handle);
    }
}
...
void LPUART0_LPUART1_RX_DriverIRQHandler(void)
{
    ...
    if (CLOCK_isEnabledClock(s_lpuartClock[0]))
    {
        stat = LPUART0->STAT;
        ctrl = LPUART0->CTRL;
        if (((LPUART_STAT_OR_MASK & stat) != 0U) ||
            (((LPUART_STAT_RDRF_MASK & stat) != 0U) && ((LPUART_CTRL_RIE_MASK & ctrl) != 0U)) ||
            (((LPUART_STAT_IDLE_MASK & stat) != 0U) && ((LPUART_CTRL_ILIE_MASK & ctrl) != 0U)))
        {
...
void LPUART0_LPUART1_DriverIRQHandler(void)
{
    ...
    if (CLOCK_isEnabledClock(s_lpuartClock[0]))
    {
        ...
        if ((0U != (LPUART_STAT_OR_MASK & stat)) ||
            ...
            ((0U != (LPUART_STAT_TDRE_MASK & stat)) && (0U != (LPUART_CTRL_TIE_MASK & ctrl))) ||
            ((0U != (LPUART_STAT_IDLE_MASK & stat)) && (0U != (LPUART_CTRL_ILIE_MASK & ctrl))))
        {
            s_lpuartIsr(LPUART0, s_lpuartHandle[0]);
        }
    }
