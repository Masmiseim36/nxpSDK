...
/*!
 * brief Sub Configures the PWM signal parameters for all channels
 *
 ...
 * param modValue    PWM MOD value
 *
 * return kStatus_Success if the PWM setup was successful,
 *         kStatus_Error on failure
 */
static status_t TPM_SubSetupPwmChannels(TPM_Type *base,
...

{
    ...
    for (i = 0U; i < numOfChnls; i++)
    {
        ...
        if (mode == kTPM_CombinedPwm)
        {
            ...
            {
                cnvFirstEdge = (uint16_t)((modValue * chnlParams->firstEdgeDelayPercent) / 100U);
            }
            /* Configure dutycycle */
            ...
            {
                cnv = (uint16_t)((modValue * chnlParams->dutyCyclePercent) / 100U);
                /* For 100% duty cycle */
                if (cnv >= modValue)
                {
                    cnv = (uint16_t)(modValue + 1u);
                }
            }
        ...
        {
#endif /* FSL_FEATURE_TPM_HAS_COMBINE*/
            if (chnlParams->dutyCyclePercent == 0U)
            {
                /* Signal stays low */
                cnv = 0U;
                      ...
                      uint32_t srcClock_Hz)
{
    ...
    switch (mode)
    {
        ...
        case kTPM_CombinedPwm:
            ...
            modValue = (tpmClock / pwmFreq_Hz) - 1u;
            break;
        case kTPM_CenterAlignedPwm:
            base->SC |= TPM_SC_CPWMS_MASK;
            modValue = tpmClock / (pwmFreq_Hz * 2u);
            break;
        default:
    ...
    /* Setup each TPM channel */
    return TPM_SubSetupPwmChannels(base, chnlParams, numOfChnls, mode, modValue);
}
...
{
    assert((int32_t)chnlNumber < FSL_FEATURE_TPM_CHANNEL_COUNTn(base));
#if defined(FSL_FEATURE_TPM_HAS_COMBINE) && FSL_FEATURE_TPM_HAS_COMBINE
