...
#define AT_NOINIT_SECTION(var) __no_init var
#elif (defined(__CC_ARM))
#define AT_NOINIT_SECTION(var) __attribute__((section("NOINIT"), zero_init)) var
#elif (defined(__GNUC__))
#define AT_NOINIT_SECTION(var) __attribute__((section("NOINIT,\"aw\",%nobits @"))) var
#else
...
static inline void CLOCK_EnableClock(clock_ip_name_t name)
{
    ...
    (*(volatile uint32_t *)regAddr) |= (u32flag << (uint16_t)CLK_GATE_ABSTRACT_BITS_SHIFT((uint32_t)name));
}
...
/*!
 * @brief Returns true if the clock for specific IP is enabled.
 *
 * @param name  Which clock to check for, see \ref clock_ip_name_t.
 */
static inline bool CLOCK_isEnabledClock(clock_ip_name_t name)
{
    uint32_t u32flag = 1;
    uint32_t regAddr = SIM_BASE + CLK_GATE_ABSTRACT_REG_OFFSET((uint32_t)name);
    return (((*(volatile uint32_t *)regAddr) & (u32flag << (uint16_t)CLK_GATE_ABSTRACT_BITS_SHIFT((uint8_t)name))) !=
            0U);
}
