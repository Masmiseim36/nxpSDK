...
status_t FLASH_Init(flash_config_t *config)
{
    ...
    for (uint8_t flashIndex = 0U; flashIndex < FTFx_FLASH_COUNT; flashIndex++)
    {
        ...
        /* Initialize the current flash index */
                             ...
                             flash_xacc_state_t *access_state)
{
    ...
    while (start < endAddress)
    {
        ...
        {
            /* For primary with eight XACC registers allow up to 64 restricted segments of equal memory size.
             */
            if (segmentIndex < 32U)
            {
...
status_t FLASH_PflashSetProtection(flash_config_t *config, pflash_prot_status_t *protectStatus)
{
...
#if defined(FTFx_FLASH0_HAS_HIGH_PROT_REG) && FTFx_FLASH0_HAS_HIGH_PROT_REG
        /* For some primary flash, which will have eight PROT registers allow up to 64 protected segments of equal
         * memory size. */
    ...
    {
        /* For secondary flash, The two FPROT registers allow up to 16 protected segments of equal memory size. */
...
#endif
    {
#if (FSL_FEATURE_FLASH_PFLASH_PROTECTION_REGION_COUNT >= 32)
        /* gat the flash protect status */
        if (config->ftfxConfig[0].flashDesc.feature.ProtRegBits >= 32U)
        {
            protectStatus->protl = *kFPROTL;
        }
#endif
#if FTFx_FLASH0_HAS_HIGH_PROT_REG
...
static void flash_access_init(flash_config_t *config, uint8_t flashIndex)
{
    ...
    if (flashIndex == 1U)
    {
#if FTFx_FLASH1_HAS_INT_XACC_REG
        facssValue = FTFx_FACSSS_REG; /* store the value of secondary flash access segment Size Register */
        facsnValue = FTFx_FACSNS_REG; /* store the value of secondary flash access segment number Register */
#endif
    }
    else
    {
#if FTFx_FLASH0_HAS_INT_XACC_REG
        facssValue = FTFx_FACSS_REG; /* store the value of primary flash access segment size register */
        facsnValue = FTFx_FACSN_REG; /* store the value of primary flash access segment number register */
#endif
    }
...
#endif // #if (FTFx_FLASH_COUNT > 1)
    {
        config->ftfxConfig[flashIndex].flashDesc.feature.ProtRegBits = pflashProtectionRegionCount;
        specMem->base  = config->ftfxConfig[flashIndex].flashDesc.blockBase;
...
static status_t flash_validate_swap_indicator_address(ftfx_config_t *config, uint32_t address)
{
...
#else
    {
        ...
        returnCode = FTFx_CMD_ReadOnce(config, kFLASH_RecordIndexSwapAddr, (uint8_t *)returnValue, 4U);
        if (returnCode != kStatus_FTFx_Success)
        {
            return returnCode;
        }
        flashSwapIfrFieldData.swapIndicatorAddress = (uint16_t)returnValue[0];
        returnCode = FTFx_CMD_ReadOnce(config, kFLASH_RecordIndexSwapEnable, (uint8_t *)returnValue, 4U);
        if (returnCode != kStatus_FTFx_Success)
        {
            return returnCode;
        }
        flashSwapIfrFieldData.swapEnableWord = (uint16_t)returnValue[0];
        returnCode = FTFx_CMD_ReadOnce(config, kFLASH_RecordIndexSwapDisable, (uint8_t *)returnValue, 4U);
        if (returnCode != kStatus_FTFx_Success)
        {
            return returnCode;
        }
