...
AT_NOINIT_SECTION(volatile uint32_t g_xtal0Freq);
/* External XTAL32K clock frequency. */
AT_NOINIT_SECTION(volatile uint32_t g_xtal32Freq);
...
status_t CLOCK_SetInternalRefClkConfig(uint8_t enableMode, mcg_irc_mode_t ircs, uint8_t fcrdiv)
{
    ...
    uint32_t curIrcs        = (uint32_t)MCG_S_IRCST_VAL;
    ...
    if ((uint32_t)kMCG_ClkOutStatInt == mcgOutClkState)
    {
        ...
        if ((((uint32_t)kMCG_IrcFast == curIrcs) && (fcrdiv != curFcrdiv)) || ((uint32_t)ircs != curIrcs))
        {
            return kStatus_MCG_SourceUsed;
        }
    }
...
uint32_t CLOCK_GetStatusFlags(void)
{
    ...
    if (0U != (MCG->SC & MCG_SC_LOCS0_MASK))
    {
        ret |= (uint32_t)kMCG_Osc0LostFlag;
    }
    if (0U != (mcg_s & MCG_S_OSCINIT0_MASK))
    {
...
void CLOCK_ClearStatusFlags(uint32_t mask)
{
    ...
    if (0U != (mask & (uint32_t)kMCG_Osc0LostFlag))
    {
        MCG->SC &= ~(uint8_t)MCG_SC_ATMF_MASK;
    }
