 ...
 */
static status_t I2C_WaitForStatusReady(I2C_Type *base, uint8_t statusMask);
...
/*!
 * @brief Master checks bus status after sending out address.
 *
 * @param base I2C peripheral base address.
 * @retval kStatus_Success Successfully complete the address transmission.
 * @retval kStatus_I2C_Timeout Transfer error, wait signal timeout.
 * @retval kStatus_I2C_Addr_Nak Transfer error, receive NAK during transfer.
 */
static status_t I2C_MasterCheckAddrNack(I2C_Type *base);
...
/*!
 * brief Master handle the start condition for master polling transfer
 *.
 * @param base I2C peripheral base address.
 * @param xfer Pointer to the transfer structure.
 * @retval kStatus_Success Successfully handle the start condition.
 * @retval kStatus_I2C_Busy Previous transmission still not finished.
 * @retval kStatus_InvalidArgument Argument does not support.
 */
status_t I2C_MasterHandleStartCondition(I2C_Type *base, i2c_master_transfer_t *xfer);

#ifdef I2C_HAS_STOP_DETECT
/*!
 * @brief Slave handles stop signal detect event.
 *
 * @param base I2C base pointer.
 * @param handle pointer to i2c_slave_handle_t structure.
 */
static void I2C_SlaveHandleStopDetect(I2C_Type *base, i2c_slave_handle_t *handle);
#endif

#if defined(FSL_FEATURE_I2C_HAS_START_STOP_DETECT) && FSL_FEATURE_I2C_HAS_START_STOP_DETECT
/*!
 * @brief Slave handles start signal detect event.
 *
 * @param base I2C base pointer.
 * @param handle pointer to i2c_slave_handle_t structure.
 */
static void I2C_SlaveHandleStartDetect(I2C_Type *base, i2c_slave_handle_t *handle);
#endif

/*!
 * @brief Slave handles receive data NACK event.
 *
 * @param base I2C base pointer.
 * @param handle pointer to i2c_slave_handle_t structure.
 */
static void I2C_SlaveHandleReceiveNack(I2C_Type *base, i2c_slave_handle_t *handle);

/*!
 * @brief Slave handles receive data complete event.
 *
 * @param base I2C base pointer.
 * @param handle pointer to i2c_slave_handle_t structure.
 */
static void I2C_SlaveHandleReceiveComplete(I2C_Type *base, i2c_slave_handle_t *handle);
...
static status_t I2C_WaitForStatusReady(I2C_Type *base, uint8_t statusMask)
{
    ...
    /* Wait until status flag is set. */
    while ((0U == (base->S & statusMask)) && (--waitTimes))
    {
    }
    if (waitTimes == 0)
    {
...
static status_t I2C_MasterHandleCheckAddress(I2C_Type *base, i2c_master_handle_t *handle)
{
    status_t result = (status_t)kStatus_Success;
    ...
    {
        if (0U != (base->S & (uint8_t)kI2C_ReceiveNakFlag))
        {
            result = (status_t)kStatus_I2C_Addr_Nak;
        }
        else
        {
            if (handle->transfer.subaddressSize > 0U)
...
static status_t I2C_MasterTransferRunStateMachine(I2C_Type *base, i2c_master_handle_t *handle, bool *isDone)
{
    ...
    *isDone              = false;
    ...
    /* Handle Check address state to check the slave address is Acked in slave
       probe application. */
    result = I2C_MasterHandleCheckAddress(base, handle);
    if (result != 0)
    {
        return result;
    }
...
status_t I2C_MasterWriteBlocking(I2C_Type *base, const uint8_t *txBuff, size_t txSize, uint32_t flags)
{
    ...
    result = I2C_WaitForStatusReady(base, (uint8_t)kI2C_TransferCompleteFlag);
    if (result != 0)
    {
        return result;
    }
    ...
    while (0U != (txSize--))
    {
        ...
        /* Wait until data transfer complete. */
        result = I2C_WaitForStatusReady(base, (uint8_t)kI2C_IntPendingFlag);
        if (result != 0)
        {
            return result;
        }
...
static status_t I2C_MasterCheckAddrNack(I2C_Type *base)
{
    ...
    if (0 != result)
    {
        if (result == (status_t)kStatus_I2C_Nak)
        {
            result = (status_t)kStatus_I2C_Addr_Nak;
...
status_t I2C_MasterTransferBlocking(I2C_Type *base, i2c_master_transfer_t *xfer)
{
    ...
    result = I2C_MasterHandleStartCondition(base, xfer);
    if (result != 0)
    {
        return result;
    }
    ...
    if (0U == (xfer->flags & (uint32_t)kI2C_TransferNoStartFlag))
    {
        /* Wait until data transfer complete and check if there's transfer error. */
        result = I2C_MasterCheckAddrNack(base);
        if (result != 0)
        {
            return result;
        }
...
status_t I2C_SlaveWriteBlocking(I2C_Type *base, const uint8_t *txBuff, size_t txSize)
{
    ...
    result = I2C_WaitForStatusReady(base, (uint8_t)kI2C_AddressMatchFlag);
    if (result != 0)
    {
        return result;
    }
...
static void I2C_SlaveHandleStopDetect(I2C_Type *base, i2c_slave_handle_t *handle)
{
    ...
    uint32_t eventMask         = handle->eventMask;
    ...
    if (true == handle->isBusy)
    {
        ...
        if ((0U != (eventMask & (uint32_t)xfer->event)) && (NULL != handle->callback))
        {
...
static void I2C_SlaveHandleReceiveNack(I2C_Type *base, i2c_slave_handle_t *handle)
{
    ...
    {
        xfer->event            = kI2C_SlaveCompletionEvent;
        xfer->completionStatus = (status_t)kStatus_I2C_Nak;
        handle->isBusy         = false;
...
void I2C_SlaveTransferHandleIRQ(I2C_Type *base, void *i2cHandle)
{
    ...
    {
        I2C_SlaveHandleStopDetect(base, handle);
    ...
    {
        I2C_SlaveHandleStartDetect(base, handle);
    ...
    {
        I2C_SlaveHandleReceiveNack(base, handle);
    }
    /* Check address match. */
    ...
    else if (0U != (status & (uint16_t)kI2C_TransferCompleteFlag))
    {
        ...
        {
            I2C_SlaveHandleReceiveComplete(base, handle);
        }
    }
    else
    {
        /* Read data to release bus. */
        (void)base->D;
        doTransmit = false;
    }
    ...
    if (doTransmit)
    {
        ...
        else
        {
            ...
            if ((eventMask & (uint32_t)xfer->event) && (handle->callback))
            {
                handle->callback(base, xfer, handle->userData);
            }
#endif /* !FSL_FEATURE_I2C_HAS_START_STOP_DETECT or !FSL_FEATURE_I2C_HAS_STOP_DETECT */
