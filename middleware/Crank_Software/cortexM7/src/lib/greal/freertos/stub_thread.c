...
//This provides a stub pthread* implementation that does nothing
...
struct _private_thread {
	struct _private_thread *	next;		//Used for pthread_self lookup
...
int
greal_pthread_detach(pthread_t thread) {
...
{
//	struct _private_thread *pt = (struct _private_thread *) tid;

//	taskENTER_CRITICAL();
//	vTaskDelete(pt->task_handle);
//	taskEXIT_CRITICAL();
