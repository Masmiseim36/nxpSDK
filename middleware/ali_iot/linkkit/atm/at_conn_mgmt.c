...
extern int HAL_AT_CONN_Init(void);
extern int HAL_AT_CONN_Start(at_conn_t *conn);
extern int HAL_AT_CONN_Send(int fd, uint8_t *data, uint32_t len, char remote_ip[16],
                     int32_t remote_port, int32_t timeout);
extern int HAL_AT_CONN_DomainToIp(char *domain, char ip[16]);
extern int HAL_AT_CONN_Close(int fd, int32_t remote_port);
extern int HAL_AT_CONN_Deinit(void);
