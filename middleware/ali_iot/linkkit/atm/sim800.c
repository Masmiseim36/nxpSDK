...
static int sim800_send_with_retry(const char *cmd, int cmdlen, bool delimiter, const char *data,
                                  int datalen, char *rspbuf, int buflen, const char *expectrsp, int retryCounter)
{
    int retry = 0;
...
int sim800_uart_selfadaption(const char *command, const char *rsp, uint32_t rsplen)
{
    ...
    if (sim800_send_with_retry(command, strlen(command), true, NULL, 0,
                               buffer, SIM800_DEFAULT_RSP_LEN, rsp, SIM800_RETRY_MAX) < 0) {
        return -1;
    }
...
static int sim800_uart_init(void)
{
    ...
    /*turn off echo*/
    if (sim800_send_with_retry(AT_CMD_ECHO_OFF, strlen(AT_CMD_ECHO_OFF), true, NULL, 0,
                               rsp, SIM800_DEFAULT_RSP_LEN, SIM800_AT_CMD_SUCCESS_RSP, SIM800_RETRY_MAX) < 0)
    {
    ...
    if (sim800_send_with_retry(cmd, strlen(cmd), true, NULL, 0,
                               rsp, SIM800_DEFAULT_RSP_LEN, SIM800_AT_CMD_SUCCESS_RSP, SIM800_RETRY_MAX) < 0)
    {
        at_conn_hal_err("%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
        return -1;
    ...
    memset(rsp, 0, SIM800_DEFAULT_RSP_LEN);
    /*save configuration */
    if (sim800_send_with_retry(AT_CMD_SAVE_CONFIG, strlen(AT_CMD_SAVE_CONFIG), true, NULL, 0,
                               rsp, SIM800_DEFAULT_RSP_LEN, SIM800_AT_CMD_SUCCESS_RSP, SIM800_RETRY_MAX) < 0)
    {
        at_conn_hal_err("%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
        return -1;
...
static int sim800_gprs_status_check(void)
{
    ...
    if (sim800_send_with_retry(AT_CMD_SIM_PIN_CHECK, strlen(AT_CMD_SIM_PIN_CHECK), true,
                               NULL, 0, rsp, SIM800_DEFAULT_RSP_LEN, SIM800_AT_CMD_SUCCESS_RSP, SIM800_RETRY_MAX * 20) < 0) {
    ...
    if (sim800_send_with_retry(AT_CMD_NETWORK_REG_CHECK, strlen(AT_CMD_NETWORK_REG_CHECK), true,
                               NULL, 0, rsp, SIM800_DEFAULT_RSP_LEN, "CREG: 0,1", SIM800_RETRY_MAX * 20) < 0) {
        at_conn_hal_err("sim card status check failed\n");
        return -1;
    }
    ...
    if (sim800_send_with_retry(AT_CMD_GPRS_ATTACH_CHECK, strlen(AT_CMD_GPRS_ATTACH_CHECK), true,
                               NULL, 0, rsp, SIM800_DEFAULT_RSP_LEN, "CGATT: 1", SIM800_RETRY_MAX * 20U) < 0) {
        at_conn_hal_err("sim card status check failed\n");
        return -1;
    }
...
static int sim800_gprs_ip_init(void)
{
    ...
    if (sim800_send_with_retry(AT_CMD_GPRS_PDP_DEACTIVE, strlen(AT_CMD_GPRS_PDP_DEACTIVE), true,
                               NULL, 0, rsp, SIM800_DEFAULT_RSP_LEN, "SHUT OK", SIM800_RETRY_MAX * 2) < 0) {
    ...
    if(sim800_send_with_retry(cmd, strlen(cmd), true, NULL, 0, rsp, SIM800_DEFAULT_RSP_LEN, SIM800_AT_CMD_SUCCESS_RSP, SIM800_RETRY_MAX) < 0)
    {
        at_conn_hal_err("%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
        return -1;
    }
...
static int sim800_gprs_got_ip(void)
{
    ...
    if (sim800_send_with_retry(AT_CMD_START_TASK, strlen(AT_CMD_START_TASK), true,
                               NULL, 0, rsp, SIM800_DEFAULT_RSP_LEN, SIM800_AT_CMD_SUCCESS_RSP, SIM800_RETRY_MAX) < 0) {
        at_conn_hal_err("%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
        return -1;
    }
    ...
    /* delay 2s may helpful to aovid PDP DEACT error */
    HAL_SleepMs(2000);

    memset(rsp, 0, SIM800_DEFAULT_RSP_LEN);
    /* Do not check below command response intentional,See SIM800 Series_TCPIP_Application Note_V1.02.pdf for
     * the GPRS states documentation.*/
    if(sim800_send_with_retry(AT_CMD_BRING_UP_GPRS_CONNECT, strlen(AT_CMD_BRING_UP_GPRS_CONNECT), true,
                       NULL, 0, rsp, SIM800_DEFAULT_RSP_LEN, SIM800_AT_CMD_SUCCESS_RSP, SIM800_RETRY_MAX) < 0)
    {
        at_conn_hal_err("%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
    }
...
static int sim800_root_cert_init()
{
    ...
    if(sim800_send_with_retry(cmd, strlen(cmd), true, iotx_ca_crt, cert_len, rsp, SIM800_DEFAULT_RSP_LEN, SIM800_AT_CMD_FAIL_RSP, SIM800_RETRY_MAX) < 0){
...
restart:
    ...
    memset(rsp, 0, SIM800_DEFAULT_RSP_LEN);
    if(sim800_send_with_retry(pccmd, strlen(pccmd), true, NULL, 0, rsp,
                       SIM800_DEFAULT_RSP_LEN, SIM800_AT_CMD_SUCCESS_RSP, SIM800_RETRY_MAX) < 0)
    {
        at_conn_hal_err("%s %d failed rsp %s\r\n", __func__, __LINE__, rsp);
        goto err;
    }
        ...
        case TCP_CLIENT:
            ...
            do
            {
                at_send_wait_reply(pccmd, strlen(pccmd), true, NULL, 0, rsp, SIM800_DEFAULT_RSP_LEN,
                                   &atcmd_config_client);
                if ((strstr(rsp, AT_CMD_CLIENT_CONNECT_OK) != NULL) || (strstr(rsp, "ALREADY CONNECT"))) {
                    break;
                }
...
int HAL_AT_CONN_Close(int fd, int32_t remote_port)
{
    ...
    if(sim800_send_with_retry(cmd, strlen(cmd), true, NULL, 0,
                       rsp, SIM800_DEFAULT_RSP_LEN, "CLOSE OK", SIM800_RETRY_MAX) < 0)
    {
                     ...
                     int32_t timeout)
{
    ...
    ret = sim800_send_with_retry((const char *)cmd, strlen(cmd), true, (const char *)data, len,
                                 rsp, SIM800_DEFAULT_RSP_LEN, SIM800_AT_CMD_SUCCESS_RSP, 10);
