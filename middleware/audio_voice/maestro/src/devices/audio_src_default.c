...
/*!
 * @file    audio_src_default.c
 * @brief   This file implement audio source element of default.
 */

#include "pipeline.h"
#include "audio_cfg.h"
#include "audio_src_default.h"
#include <string.h>
 ...
 */
AudioSrcStreamErrorType audiosrc_default_init_params(ElementAudioSrc *audio_src_element)
{
    DefaultSrcDeviceInfo *dev_info;
    int ret;
    ...
    dev_info = (DefaultSrcDeviceInfo *)audio_src_element->device_info;
...
AudioSrcStreamErrorType audio_src_default_init_device(ElementAudioSrc *audio_src_element)
{
    ...
    dev_info = OSA_MemoryAllocate(sizeof(DefaultSrcDeviceInfo));
    if (!dev_info)
        return AUDIOSRC_ERROR_OUT_OF_HEAP_MEMORY;

    memset(dev_info, 0, sizeof(DefaultSrcDeviceInfo));
                                                      ...
                                                      StreamBuffer *buf)
{
    ...
    DefaultSrcDeviceInfo *dev_info = NULL;
    ...
    if (!dev_info->init_params_done)
    {
        ...
        if (audiosrc_default_init_params(audio_src_element) != AUDIOSRC_SUCCESS)
        {
