...
char* amqpvalue_to_string(AMQP_VALUE amqp_value)
{
    ...
    if (amqp_value != NULL)
    {
        ...
        case AMQP_TYPE_CHAR:
        {
            ...
            {
                char str_value[25];
                if ((snprintf(str_value, sizeof(str_value), "U%02X%02X%02X%02X", (unsigned int)char_code >> 24, (unsigned int)(char_code >> 16) & 0xFF, (unsigned int)(char_code >> 8) & 0xFF, (unsigned int)char_code & 0xFF) < 0) ||
                    (string_concat(&result, str_value) != 0))
                {
                    LogError("Failure building amqp value string");
                    free(result);
