...
void vec_rsqrtf         (float32_t* restrict z, const float32_t* restrict x, int N)
{
    ...
    while(M>1)
    {
        ...
        for ( n=0; n<(N>>1); n++ )
        {
            ae_int32x2 X;
            xtfloat Yl,Yh;
            AE_LA32X2_IP(X,ax0,px0);
            Yl=XT_RSQRT_S(XT_LOW_S (XT_AE_MOVINT32X2_FROMXTFLOATX2(X)));
            Yh=XT_RSQRT_S(XT_HIGH_S(XT_AE_MOVINT32X2_FROMXTFLOATX2(X)));
    ...
    if(M&1)
    {
        ...
        ux = XT_AE_MOVINT32_FROMXTFLOAT(xn); /* return +/- inf */
        ux = AE_AND32(ux,0x80000000);
        ux = AE_OR32 (ux,0x7f800000);
        y0 = XT_AE_MOVXTFLOAT_FROMINT32(ux);
