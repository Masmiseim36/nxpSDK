...
/*
 *  First stage of FFT 16x16, radix-3, dynamic scaling
 */
...
int fft_16x16_stage_first_scl2_DFT5(const int16_t *tw, const int16_t *x, int16_t *y, int N, int *v, int tw_step, int *bexp)
{
  ...
  ae_int16x4 tw012, tw034, tw112, tw134, accbits;
  ...
  for (i = 0; i <(stride>>1); i++)
  {
    ...
    accbits = AE_OR16(accbits, AE_ABS16S(x4));
  }
  accbits = AE_OR16(accbits, AE_SEL16_2301(accbits, accbits));
  accbits = AE_OR16(accbits, AE_SHORTSWAP(accbits));
  sumbits = AE_MOVAD16_0(accbits);
...
{ 
  ...
  for (j = 0; j < (_v>>1); j++)
  {
    ...
    for (i = 0; i < ((stride/_v)>>1); i++)
    {
      ...
      accbits = AE_OR16(accbits, AE_ABS16S(y0));
      accbits = AE_OR16(accbits, AE_ABS16S(y1));
  ...
  if (_v&1)
  {
    ...
    for (i = 0; i < (stride/_v); i++)
    {
      ...
      accbits = AE_OR16(accbits, AE_ABS16S(AE_MOVINT16X4_FROMINT32X2(y01)));
      accbits = AE_OR16(accbits, AE_ABS16S(AE_MOVINT16X4_FROMINT32X2(y11)));
    }
  }
...
/*
 *  First stage of IFFT 16x16, radix-3, dynamic scaling
 */
int ifft_16x16_stage_first_scl2_DFT3(const int16_t *tw, const int16_t *x, int16_t *y, int N, int *v, int tw_step, int *bexp)
{
  ...
  ae_int16x4 tw012, tw112, c1, accbits;
  int32_t sumbits;
  NASSERT_ALIGN8(x);
  NASSERT_ALIGN8(y);
...
int ifft_16x16_stage_first_scl2_DFT5(const int16_t *tw, const int16_t *x, int16_t *y, int N, int *v, int tw_step, int *bexp)
{
  ...
  ae_int16x4 tw012, tw034, tw112, tw134, c1, accbits;
  ae_int16x4 w1, w2, w3, w4;
  int32_t sumbits;
  NASSERT_ALIGN8(x);
  NASSERT_ALIGN8(y);
