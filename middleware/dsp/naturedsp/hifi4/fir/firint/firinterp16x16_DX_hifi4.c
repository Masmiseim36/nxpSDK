                            ...
                            int wrIx, int D, int N, int M)
{
    ...
    for (n = 0; n < (N >> 3); n++)
    {
        ...
        for (d = 0; d < D; d++)
        {
            ...
            AE_ADDCIRC_XC(castxcc(ae_int64, D_tmp), 4*sizeof(int16_t));
            D_rd0_1 = D_tmp;
            ...
            for (m = 0; m < (M >> 2) + 1; m++)
            {
                ...
                AE_L16X4_XC (d0, D_rd0_1, 8);
                AE_LA16X4_IC(d1, D_va1, D_rd1);
                AE_LA16X4_IC(d2, D_va2, D_rd2);
                AE_LA16X4_IC(d3, D_va3, D_rd3);

                AE_MULAAAAQ16(q4, d0, c0);
                AE_MULAAAAQ16(q5, d1, c0);
                AE_MULAAAAQ16(q6, d2, c0);
                AE_MULAAAAQ16(q7, d3, c0);
            }
