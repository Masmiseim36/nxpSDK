                ...
                const int32_t *                x , int N)
{
  ...
  for ( m=0; m<M/2; m++ )
  {
    ...
    if ((uintptr_t)px & ((int)sizeof(ae_int32x2)-1)) // unaligned
    {
      // process the first sample
      AE_L32_IP( x01, castxcc(ae_int32,px), sizeof(ae_int32) );
      ...
      AE_S32_L_IP( st2, castxcc(ae_int32,pr), sizeof(ae_int32) );

      st0 = AE_SEL32_LH(st0, x01);

      for ( n=0; n<((N>>1)-1); n++ )
      {
        acc00 = AE_MULZAAFD32S_HH_LL( cf0_b21, st0 );

        AE_L32X2_IP( x01, px, sizeof(ae_int32x2) );
      ...
      AE_PKSRF32_( st0, acc00, 1 );
      st1 = AE_SEL32_LL(st1, st0);

      AE_MULSSFD32S_HH_LL( acc10, cf1_a21, st2 );
      AE_MULAAFD32S_HH_LL( acc10, cf1_b10, st1 );
      AE_PKSRF32_( st2, acc10, 1 );

      AE_S32_L_I( st2, (ae_int32*)pr, 0 );

      st0 = AE_SEL32_HH(st0, x01);
    }
    else
    {
      for ( n=0; n<(N>>1); n++ )
