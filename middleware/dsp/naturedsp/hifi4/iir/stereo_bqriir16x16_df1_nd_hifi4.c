...
{
	if (M<=0) return 0;
    return (2 * bqriir16x16_df1_nd_alloc(M) + ALIGNED_SIZE(sizeof(stereo_bqriir16x16_df1_nd_t), 8));
                                                             ...
                                                             int16_t         gainr )
{
	...
	void * ptr;
    size_t sz_bqriir = bqriir16x16_df1_nd_alloc(M);
    ...
    stereo_bqriir = (stereo_bqriir16x16_df1_nd_ptr_t)(ptr);
    ...
    stereo_bqriir->bqriir_left = bqriir16x16_df1_nd_init(stereo_bqriir->bqriir_left_mem, M, coef_sosl, coef_gl, gainl);
    stereo_bqriir->bqriir_right = bqriir16x16_df1_nd_init(stereo_bqriir->bqriir_right_mem, M, coef_sosr, coef_gr, gainr);
