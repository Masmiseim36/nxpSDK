...
{
    stereo_bqriir16x16_df1_nd_ptr_t stereo_bqriir = (stereo_bqriir16x16_df1_nd_ptr_t)_bqriir;
	...
	NASSERT(stereo_bqriir->magic == STEREO_BQRIIR16X16_DF1_ND_MAGIC);
	NASSERT_ALIGN(s,8);
	if(N<=0) return;
	NASSERT(N%2==0);
	rv = AE_ZALIGN64();
	...
	for (b = 0; b < N; b+=MAX_BUFFER_SZ)
	{
        ...
        bqriir16x16_df1_nd(stereo_bqriir->bqriir_left, s, (int16_t *)pBxl, (const int16_t *)pBxl, NN);

		// Aggregate right channel.
        bqriir16x16_df1_nd(stereo_bqriir->bqriir_right, s, (int16_t *)pBxr, (const int16_t *)pBxr, NN);
