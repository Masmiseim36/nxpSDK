               ...
               int32_t mean, const imgsize_t* sz)
{
    ...
    */
    ...
    ae_int16x4 t; 
    /* Convert int8_t samples to int16 and subtract the mean value */
    for (i = 0; i < M; i++)
    {
        ae_int16x4 _0x8000 = AE_MOVDA16(0x8000);
        pIn = (const signed char  *)(x + i*stride);
        pOut = (ae_int16x4 *)(tmpX2 + 2 * i*N); 
        for (j = 0; j < (N>>2); j++)
        {
            //tmpX2[2 * i*N + j] = ((int16_t)x[i*stride + j] << 7) - dcBias;
            AE_L8X4F_IP(t, pIn, 4 * sizeof(int8_t));
            t = AE_XOR16(t, _0x8000);
            t = AE_MOVINT16X4_FROMINT32X2(AE_SRLI32(AE_MOVINT32X2_FROMINT16X4(t), 1));
            t = AE_SUB16S(t, dcBias); 
            AE_S16X4_IP(t, pOut, sizeof(t)); 
        }
    }
