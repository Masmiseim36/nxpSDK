...
void imgfastnorm_gu8_nonlinear ( void * restrict outImg, const void * restrict inImg, const imgsize_t* sz, const int16_t* tbl)
{
    ...
    for (i = 0; i < h; i++)
    {
        ...
        for (j = 0; j < (w >> 3); j++)
        {
            ...
            ae_int16x4 x, ix, x0, z, y0, y1, t0, t1;
            ae_int32x2 d0, d1;
            ...
            AE_L8X4F_IP(x_, pIn, 4 * sizeof(int8_t));
            x_ = AE_MOVINT16X4_FROMINT32X2(AE_SRLI32(AE_MOVINT32X2_FROMINT16X4(x_), 8));
            ...
            t0 = AE_SEL16_7531(z, z_);
            t1 = AE_SEL16_6420(z, z_);
            t1 = AE_MOVINT16X4_FROMINT32X2(AE_SRLA32(AE_MOVINT32X2_FROMINT16X4(t1), -8));
            t0 = AE_OR16(t0, t1);
