...
void img_gs16_rot90_16(void* pScr,void* outImg, imgsize_t * restrict outSz, const void* inImg, const imgsize_t * restrict inSz, int fillColor)
{
  ...
  for (i = 0; i<(h>>1); i++)
  {
    ...
    for (j = 0; j<(w >> 2); j++) 
    {
      ...
      AE_S16X4_IP(x0_, dst0, sizeof(ae_int16x4));
      AE_S16X4_IP(x0, dst1, sizeof(ae_int16x4));
    }
  }
  if (h&1)
  ...
  for (i = 0; i<(w & 3); i++)
  {
    ...
    dst0 = (ae_int16x4 *)((uintptr_t)outImg + (((h & 3) + 0)* ostride + (PADDING*ostride + (ALIGNMENT / 2)) + (w - 1 - i))*sizeof(int16_t));
    dst1 = (ae_int16x4 *)((uintptr_t)outImg + (((h & 3) + 1)* ostride + (PADDING*ostride + (ALIGNMENT / 2)) + (w - 1 - i))*sizeof(int16_t));
    dst2 = (ae_int16x4 *)((uintptr_t)outImg + (((h & 3) + 2)* ostride + (PADDING*ostride + (ALIGNMENT / 2)) + (w - 1 - i))*sizeof(int16_t));
    dst3 = (ae_int16x4 *)((uintptr_t)outImg + (((h & 3) + 3)* ostride + (PADDING*ostride + (ALIGNMENT / 2)) + (w - 1 - i))*sizeof(int16_t));
    aS0 = AE_LA64_PP(src0);
    for (j = 0; j<(h >> 2); j++)
    {
