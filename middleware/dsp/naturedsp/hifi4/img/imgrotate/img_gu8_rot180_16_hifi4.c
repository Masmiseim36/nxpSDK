...
void img_gu8_rot180_16(void* pScr,void* outImg, imgsize_t * restrict outSz, const void* inImg, const imgsize_t * restrict inSz, int fillColor)
{
  ...
  for (i = 0; i<h; i++)
  {
      src = (const ae_int16x4 *)((uintptr_t)inImg + ((h - 1 - i)*istride)*sizeof(uint8_t));
      dst0 = (ae_int16x4 *)((uintptr_t)outImg + (PADDING*ostride + (ALIGNMENT / 2) + i * ostride + w-1)*sizeof(int16_t));
      s_ = (((uintptr_t)src) & 3);
      if (s_) s_ = 4 - s_;
      for (j = 0; j<s_; j++)
      {
          ...
          AE_S16_0_IP(p, castxcc(ae_int16, dst0), -((int)sizeof(int16_t)));
      }
      aD = AE_ZALIGN64();
      for (j = 0; j<((w-s_) >> 2); j++)
      {
