...
void imgfast_gs16_rot90 (void* pScr, void* outImg, const imgsize_t * restrict outSz, const void* inImg, const imgsize_t * restrict inSz)
{
  ...
  for (i=0; i<(w>>2); i++)
  {
    ...
    dst0 = (ae_int16x4 *)((uintptr_t)outImg + (((h & 3) + 0)* ostride)*sizeof(int16_t)) + i;
    dst1 = (ae_int16x4 *)((uintptr_t)outImg + (((h & 3) + 1)* ostride)*sizeof(int16_t)) + i;
    dst2 = (ae_int16x4 *)((uintptr_t)outImg + (((h & 3) + 2)* ostride)*sizeof(int16_t)) + i;
    dst3 = (ae_int16x4 *)((uintptr_t)outImg + (((h & 3) + 3)* ostride)*sizeof(int16_t)) + i;
    for (j=0; j<(h>>2); j++)
    {
  ...
  for (j = 0; j<(h&3); j++)
  {
    ...
    dst0 = (ae_int16x4 *)((uintptr_t)outImg + (j * ostride)*sizeof(int16_t));  
    for (i = 0; i<(w >> 2); i++)
    {
