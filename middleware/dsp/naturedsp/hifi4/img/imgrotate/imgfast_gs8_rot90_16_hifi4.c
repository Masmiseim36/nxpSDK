...
void imgfast_gs8_rot90_16(void* pScr, void* outImg, imgsize_t * restrict outSz, const void* inImg, const imgsize_t * restrict inSz, int fillColor)
{
  ...
  for (i=0; i<(w>>2); i++)
  {
    src0 = (const uint8_t *)((uintptr_t)inImg + ((0+4*i) * istride + (h)-(h & 3)) * sizeof(uint8_t)) - 4;
    src1 = (const uint8_t *)((uintptr_t)inImg + ((1+4*i) * istride + (h)-(h & 3)) * sizeof(uint8_t)) - 4;
    src2 = (const uint8_t *)((uintptr_t)inImg + ((2+4*i) * istride + (h)-(h & 3)) * sizeof(uint8_t)) - 4;
    src3 = (const uint8_t *)((uintptr_t)inImg + ((3+4*i) * istride + (h)-(h & 3)) * sizeof(uint8_t)) - 4;
    ...
    for (j=0; j<(h>>2); j++)
    {
      ...
      AE_L8X4F_IP(x1,castxcc(signed char,src1), -4 * (int)sizeof(uint8_t));
      AE_L8X4F_IP(x2,castxcc(signed char,src2), -4 * (int)sizeof(uint8_t));
      AE_L8X4F_IP(x3,castxcc(signed char,src3), -4 * (int)sizeof(uint8_t));
      ...
      x1 = AE_XOR16(x1, _8000);
      x2 = AE_XOR16(x2, _8000);
      x3 = AE_XOR16(x3, _8000);
  ...
  for (j = 0; j<(h&3); j++)
  {
    src0 = (const uint8_t *)((uintptr_t)inImg + (0 * istride + (h - 1 - j)) * sizeof(uint8_t));
    src1 = (const uint8_t *)((uintptr_t)inImg + (1 * istride + (h - 1 - j)) * sizeof(uint8_t));
    src2 = (const uint8_t *)((uintptr_t)inImg + (2 * istride + (h - 1 - j)) * sizeof(uint8_t));
    src3 = (const uint8_t *)((uintptr_t)inImg + (3 * istride + (h - 1 - j)) * sizeof(uint8_t));
