...
void imgfast_gu8_rot270 (void* pScr, void* outImg, const imgsize_t * restrict outSz, const void* inImg, const imgsize_t * restrict inSz)
{
  ...
  for (i = 0; i<(w >> 2); i++)
  {
    src0 = (const int8_t *)((uintptr_t)inImg + ((w - 1 - 4 * i - 0) * istride) * sizeof(uint8_t));
    src1 = (const int8_t *)((uintptr_t)inImg + ((w - 1 - 4 * i - 1) * istride) * sizeof(uint8_t));
    src2 = (const int8_t *)((uintptr_t)inImg + ((w - 1 - 4 * i - 2) * istride) * sizeof(uint8_t));
    src3 = (const int8_t *)((uintptr_t)inImg + ((w - 1 - 4 * i - 3) * istride) * sizeof(uint8_t));

    dst0 = (ae_int32 *)((uintptr_t)outImg + (0 * ostride + 4 * i)*sizeof(uint8_t));
    dst1 = (ae_int32 *)((uintptr_t)outImg + (1 * ostride + 4 * i)*sizeof(uint8_t));
    dst2 = (ae_int32 *)((uintptr_t)outImg + (2 * ostride + 4 * i)*sizeof(uint8_t));
    dst3 = (ae_int32 *)((uintptr_t)outImg + (3 * ostride + 4 * i)*sizeof(uint8_t));
    for (j = 0; j<(h >> 2); j++)
    {
      ...
      x0 = AE_OR16(y1, AE_MOVINT16X4_FROMINT64(AE_SRLI64(AE_MOVINT64_FROMINT16X4(y0), 8)));
      x1 = AE_OR16(y3, AE_MOVINT16X4_FROMINT64(AE_SRLI64(AE_MOVINT64_FROMINT16X4(y2), 8)));

      AE_S32_L_XP(AE_MOVINT32X2_FROMINT16X4(AE_SEL16_7632(x0, x0)), dst0, 4 * ostride*sizeof(uint8_t));
      AE_S32_L_XP(AE_MOVINT32X2_FROMINT16X4(x0), dst1, 4 * ostride*sizeof(uint8_t));
      AE_S32_L_XP(AE_MOVINT32X2_FROMINT16X4(AE_SEL16_7632(x1, x1)), dst2, 4 * ostride*sizeof(uint8_t) );
      AE_S32_L_XP(AE_MOVINT32X2_FROMINT16X4(x1), dst3, 4 * ostride*sizeof(uint8_t) );
    }
  }
  __Pragma("no_reorder");
  for (j = 0; j<(h & 3); j++)
  {
    src0 = (const int8_t *)((uintptr_t)inImg + ((w - 1 - 0) * istride + (h - 1 - j)) * sizeof(uint8_t));
    src1 = (const int8_t *)((uintptr_t)inImg + ((w - 1 - 1) * istride + (h - 1 - j)) * sizeof(uint8_t));
    src2 = (const int8_t *)((uintptr_t)inImg + ((w - 1 - 2) * istride + (h - 1 - j)) * sizeof(uint8_t));
    src3 = (const int8_t *)((uintptr_t)inImg + ((w - 1 - 3) * istride + (h - 1 - j)) * sizeof(uint8_t));

    dst0 = (ae_int32 *)((uintptr_t)outImg + ((h - 1 - j) * ostride)*sizeof(uint8_t));  
    for (i = 0; i<(w >> 2); i++)
    {
