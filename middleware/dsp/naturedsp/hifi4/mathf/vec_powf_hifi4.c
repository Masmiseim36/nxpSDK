            ...
            int N )
{
  ...
  float32_t ALIGN(16) scr[blkSize];
  int M;
  if ( N<=0 ) return;
  M=N&~1;
  if ( M )
  ...
  {     // processing the tail
    ...
    /* Take sign of x and y */
    {
      ...
      b_yinf = (XT_OEQ_S(XT_ABS_SX2(y0), plusInff.f));     /* |y|==Inf    */
      ...
      XT_MOVT_S(spec, plusInff.f, xtbool2_extract_0(b_Inf));
      XT_MOVT_S(spec, qNaNf.f, xtbool2_extract_0(b_NaN2));
