...
void vec_dot_batch8x8_fast  (int16_t   *z, const int8_t      * x,const cint8ptr_t    * y,int rsh, int N, int M)
{
	...
	const signed char * restrict py2;
	const signed char * restrict py3;
	...
	for (m = 0; m < M; m += 4)
	{
		...
		px = (const signed char *)x;
		py0 = (const signed char *)y[m + 0];
		py1 = (const signed char *)y[m + 1];
		py2 = (const signed char *)y[m + 2];
		py3 = (const signed char *)y[m + 3];

		// load first 4 values
		AE_L8X4F_IP(vxh, px, 4);

		AE_L8X4F_IP(vyh0, py0, 4);
		AE_L8X4F_IP(vyh1, py1, 4);
		AE_L8X4F_IP(vyh2, py2, 4);
		AE_L8X4F_IP(vyh3, py3, 4);
