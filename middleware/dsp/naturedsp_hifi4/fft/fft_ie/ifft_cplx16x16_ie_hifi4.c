...
static int stage_first_iDFT4_16x16_ie(const int16_t *tw, const int16_t *x, int16_t *y, int N, int *v, int tw_step, int *bexp)
{
    ...
    for (i = 0; i < N / 2 / R; i++)
    {
        ...
        ae_int32x2  tmp0, tmp1, tmp2, tmp3;
        ...
        AE_L32X2_XP(tmp0, _px, stride*sizeof(complex_fract16));
        AE_L32X2_XP(tmp1, _px, stride*sizeof(complex_fract16));
        AE_L32X2_XP(tmp2, _px, stride*sizeof(complex_fract16));
        AE_L32X2_XP(tmp3, _px, sizeof(*_px) - 3 * stride*sizeof(complex_fract16));

        _x0 = AE_MOVINT16X4_FROMINT32X2(tmp0);
        _x1 = AE_MOVINT16X4_FROMINT32X2(tmp1);
        _x2 = AE_MOVINT16X4_FROMINT32X2(tmp2);
        _x3 = AE_MOVINT16X4_FROMINT32X2(tmp3);
    ...
    if (log2N & 1)
    {
        ...
        if (bexp == 0)
        {
            ...
            for (i = 0; i < N / 4; i++)
            {
                ...
                z0 = AE_MOVINT32X2_FROMINT16X4(tmp0);
                z1 = AE_MOVINT32X2_FROMINT16X4(x1);

                AE_S32X2_XP(z0, py, stride*sizeof(complex_fract16));
                AE_S32X2_XP(z1, py, sizeof(*px) - stride*sizeof(complex_fract16));
            }
        }
        else
        {
