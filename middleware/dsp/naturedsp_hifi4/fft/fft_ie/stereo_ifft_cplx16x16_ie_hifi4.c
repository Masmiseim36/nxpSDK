...
int stereo_ifft_cplx16x16_ie(complex_fract16* y, complex_fract16* x, const complex_fract16* twd, int twdstep, int N, int scalingOpt)
{
    ...
    ae_int32x2 x00, x10, x20, x30, x11, x21, x31;
    ae_int16x4 accbits;
    int32_t sumbits;
    ...
    /*
     * First FFT stage, radix-4
     * real and imaginary parts are swapped in the first stage to make inverse FFT
     */
    {
        ...
        for (i = 0; i < N4; i++)
        {
            ...
            /* load input data */
            AE_L32X2_XP(x00, castxcc(ae_int32x2,px), N4*2*sizeof(complex_fract16));
            AE_L32X2_XP(x10, castxcc(ae_int32x2,px), N4*2*sizeof(complex_fract16));
            AE_L32X2_XP(x20, castxcc(ae_int32x2,px), N4*2*sizeof(complex_fract16));
            AE_L32X2_XP(x30, castxcc(ae_int32x2,px), sizeof(ae_int16x4) - 3*N4*2*sizeof(complex_fract16));
            x0 = AE_MOVINT16X4_FROMINT32X2(x00);
            x1 = AE_MOVINT16X4_FROMINT32X2(x10);
            x2 = AE_MOVINT16X4_FROMINT32X2(x20);
            x3 = AE_MOVINT16X4_FROMINT32X2(x30);
     ...
     * real and imaginary parts are swapped in the last stage to make inverse FFT
     */
    if (y != pdest)
    {
    ...
    if (stridey == N2)/* radix-2 */
    {
        ...
        if (bexp == 0)/* butterfly with scaling */
        {
            ...
            for (i = 0; i < N2; i++)
            {
                ...
                x00 = AE_MOVINT32X2_FROMINT16X4(y0);
                x10 = AE_MOVINT32X2_FROMINT16X4(y1);
                AE_S32X2_XP(x00, castxcc(ae_int32x2,py), N2*2*sizeof(complex_fract16));
                AE_S32X2_XP(x10, castxcc(ae_int32x2,py), sizeof(ae_int16x4) - N2*2*sizeof(complex_fract16));
            }
        }
    ...
    else/* radix-4 */
    {
        ...
        for (i = 0; i < N4; i++)
        {
            ...
            x00 = AE_MOVINT32X2_FROMINT16X4(x0);
            x10 = AE_MOVINT32X2_FROMINT16X4(x1);
            x20 = AE_MOVINT32X2_FROMINT16X4(x2);
            x30 = AE_MOVINT32X2_FROMINT16X4(x3);
            AE_S32X2_XP(x00, castxcc(ae_int32x2,py), N4*2*sizeof(complex_fract16));
            AE_S32X2_XP(x10, castxcc(ae_int32x2,py), N4*2*sizeof(complex_fract16));
            AE_S32X2_XP(x20, castxcc(ae_int32x2,py), N4*2*sizeof(complex_fract16));
            AE_S32X2_XP(x30, castxcc(ae_int32x2,py), sizeof(ae_int16x4) - 3*N4*2*sizeof(complex_fract16));
        }
        shift += stage_shift;
    }
    return shift;
