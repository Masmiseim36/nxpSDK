...
float32_t * fir_interpf_3x(float32_t * restrict z, float32_t * restrict delay, float32_t * restrict p, const float32_t * restrict h, const float32_t * restrict x, int M, int N)
{
  ...
  /* Process by 4 input samples (12 output samples) */
  ...
  for (n = 0; n < (N>>2); n++)
  {
      ...
      for (j = 0; j < 3; j++)
      {
          ...
          XT_SSIP(s0, pZ, 3*sizeof(float32_t));
          XT_SSIP(s1, pZ, 3*sizeof(float32_t));
          XT_SSIP(s2, pZ, 3*sizeof(float32_t));
          XT_SSIP(s3, pZ, -8*(int)sizeof(float32_t));
      }
