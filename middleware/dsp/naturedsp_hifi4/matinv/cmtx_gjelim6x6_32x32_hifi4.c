...
int  cmtx_gjelim6x6_32x32  (void* pScr, complex_fract32 *y, const complex_fract32* A,const complex_fract32 * x, int qA, int qX)
{
    ...
    for (k=0; k<N; k++)
    {
        ...
        else // k=3..5
        {
            ...
            pB=(B+k*N+3);
            pT=(T+k);
            Bk4=AE_L32X2_I(pB,1*sizeof(ae_int32x2));
            Bk5=AE_L32X2_I(pB,2*sizeof(ae_int32x2));
            AE_L32X2_XC(Bk3,pB,N*sizeof(ae_int32x2));
            AE_ADDCIRC32X2_XC1(pT,sizeof(ae_int32x2));
            pBwr=pB;
            esh_coef=AE_SLAA32S(0x40000000,-e+1);
            for (i=0; i<N-1; i++)
            {
                ...
                AE_S32X2_X (Cin,pT,(int)((uintptr_t)C-(uintptr_t)T));
                AE_ADDCIRC32X2_XC1(pT,sizeof(ae_int32x2));
                Bin4=AE_L32X2_I(pB,1*sizeof(ae_int32x2));
                Bin5=AE_L32X2_I(pB,2*sizeof(ae_int32x2));
                AE_L32X2_XC(Bin3,pB,N*sizeof(ae_int32x2));
                Bin3=AE_MULFP32X2RAS(Bin3,esh_coef);
                Bin4=AE_MULFP32X2RAS(Bin4,esh_coef);
                Bin5=AE_MULFP32X2RAS(Bin5,esh_coef);
                AE_MULAFC32RAS(Bin3,Bk3,Ti);
                AE_MULAFC32RAS(Bin4,Bk4,Ti);
                AE_MULAFC32RAS(Bin5,Bk5,Ti);
                AE_S32X2_I(Bin4,pBwr,1*sizeof(ae_int32x2));
                AE_S32X2_I(Bin5,pBwr,2*sizeof(ae_int32x2));
                AE_S32X2_XC(Bin3,pBwr,N*sizeof(ae_int32x2));
            }
            Ti=AE_L32X2_I(pT,0);
            Ckn=AE_MULFC32RAS(Ckn,Ti);
            AE_S32X2_X (Ckn,pT,(int)((uintptr_t)C-(uintptr_t)T));
            ...
            Bk4=AE_MULFC32RAS(Bk4,Ti);
            Bk5=AE_MULFC32RAS(Bk5,Ti);
            AE_S32X2_I(Bk4,pBwr,1*sizeof(ae_int32x2));
            AE_S32X2_I(Bk5,pBwr,2*sizeof(ae_int32x2));
            AE_S32X2_XC(Bk3,pBwr,N*sizeof(ae_int32x2));
            pBwr=(B+k);
            for (i=0; i<N; i++)  AE_S32X2_XP(AE_ZERO32(),pBwr,N*sizeof(ae_int32x2));
            __Pragma("no_reorder");
        }
...
// scratch allocation
size_t cmtx_gjelim6x6_32x32_getScratchSize   () { return  (6*6+2*6)*sizeof(complex_fract32);  }
