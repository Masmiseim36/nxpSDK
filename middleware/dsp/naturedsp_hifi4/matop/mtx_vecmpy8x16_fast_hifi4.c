                 ...
                 int M, int N, int lsh)
{
	...
	if ((N&4)&&(N&8))
	{
		...
		for (m=0; m<(M>>2); m++)
		{
			...
			{
				AE_L16X4_IP(y0,pY,4*sizeof(int16_t));
				AE_L16X4_IP(y1,pY,4*sizeof(int16_t));
			...
			/* shift with saturation and pack results */
			{
				ae_int32x2 a01,a23;
				ae_int16x4 t;
				a01=AE_TRUNCA32X2F64S(A0,A1,40-rsh);
				a23=AE_TRUNCA32X2F64S(A2,A3,40-rsh);
				t=AE_ROUND16X4F32SASYM(a01,a23);
				AE_S16X4_IP(t,pZ,4*sizeof(int16_t));
			}
			pX0+=3*N;
			pX1+=3*N;
			pX2+=3*N;
