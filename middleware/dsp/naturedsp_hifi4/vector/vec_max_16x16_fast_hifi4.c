...
int16_t vec_max16x16_fast (const int16_t* restrict x, int N)
{
    ...
    for (n=0;n<(N>>3); n++)
    {
        ...
        cprt = AE_LT16(h0, vxh);
        AE_MOVT16X4(h0, vxh, cprt);

        AE_L16X4_IP(vxh, px, +8);
        cprt = AE_LT16(h1, vxh);
        AE_MOVT16X4(h1, vxh, cprt);
    }                                
    if(N&4)
    {
        AE_L16X4_IP(vxh, px, +8);
    ...
    cprt = AE_LT16(h0, h1);
    AE_MOVT16X4(h0, h1, cprt);

    vxh = AE_SEL16_4321(h0, h0);
    cprt = AE_LT16(h0, vxh);
    AE_MOVT16X4(h0, vxh, cprt);
    vxh = AE_SEL16_5432(h0, h0);
    cprt = AE_LT16(h0, vxh);
    AE_MOVT16X4(h0, vxh, cprt);

    return (int16_t)AE_MOVAD16_0(h0);
}
