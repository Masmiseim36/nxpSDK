...
struct net_buf {
	union {
		/** Allow placing the buffer into bt_list_t */
		bt_list_t node;
	...
	uint8_t user_data[CONFIG_NET_BUF_USER_DATA_SIZE];
};

struct net_buf_data_cb {
	uint8_t * (*alloc)(struct net_buf *buf, size_t *size, size_t timeout);
 ...
 * the timeout passed to net_buf_alloc will be always treated as osaWaitNone_c
 ...
 * always treated as osaWaitNone_c when trying to allocate the data. This means
 ...
 *        If osaWaitNone_c, then return immediately. If osaWaitForever_c, then
...
struct net_buf *net_buf_alloc_fixed_debug(struct net_buf_pool *pool,
					  size_t timeout, const char *func,
					  int line);
 ...
 *        If osaWaitNone_c, then return immediately. If osaWaitForever_c, then wait as
 *        long as necessary. Otherwise, wait until the specified timeout.
 *
 * @return New buffer or NULL if the FIFO is empty.
 */
#if defined(CONFIG_NET_BUF_LOG)
struct net_buf *net_buf_get_debug(osa_msgq_handle_t fifo, size_t timeout,
				  const char *func, int line);
#define	net_buf_get(_fifo, _timeout) \
	net_buf_get_debug(_fifo, _timeout, __func__, __LINE__)
#else
struct net_buf *net_buf_get(osa_msgq_handle_t fifo, size_t timeout);
#endif
 ...
 * @param buf Buffer.
 */
void net_buf_slist_put(bt_list_t *list, struct net_buf *buf);
 ...
 * @return New buffer or NULL if the FIFO is empty.
 */
struct net_buf *net_buf_slist_get(bt_list_t *list);
#endif
 ...
 */
struct net_buf *net_buf_clone(struct net_buf *buf, size_t timeout);
 ...
 */
typedef struct net_buf *(*net_buf_allocator_cb)(size_t timeout,
						void *user_data);
 ...
 *         length if other timeout than osaWaitForever_c was used, and there
 *         were no free fragments in a pool to accommodate all data.
 */
size_t net_buf_append_bytes(struct net_buf *buf, size_t len,
			    const void *value, size_t timeout,
