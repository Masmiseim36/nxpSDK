    ...
    int nbits_budget, float nbits_off, int g_off, bool *reset_off)
{
    ...
    int nbits = (int)(nbits_budget + nbits_off + 0.5f);
    int g_int = 255 - g_off;
    ...
    int g_min = x_max == 0 ? -g_off :
        (int)ceilf(28 * log10f(x_max / (32768 - 0.375f)));
    ...
    int g_int, float *x, int16_t *xq, int *nq)
{
    ...
    *nq = ne;
    ...
    for (int i = 0; i < ne; i += 2) {
        ...
        x[i+0] *= g_inv;
        x0 = (int16_t)fminf(floorf(fabsf(x[i+0]) + 6.f/16), INT16_MAX);
        xq[i+0] = x[i+0] < 0 ? -x0 : x0;

        x[i+1] *= g_inv;
        x1 = (int16_t)fminf(floorf(fabsf(x[i+1]) + 6.f/16), INT16_MAX);
