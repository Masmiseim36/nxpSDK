...
#define LOG_ENABLE IS_ENABLED(CONFIG_BT_DEBUG_ASCS)
...
#define BT_BAP_ASCS_RSP_NULL ((struct bt_bap_ascs_rsp[]) { BT_BAP_ASCS_RSP((enum bt_bap_ascs_rsp_code)0, (enum bt_bap_ascs_reason)0) })
...
static void ascs_app_rsp_warn_valid(const struct bt_bap_ascs_rsp *rsp)
{
	...
	case BT_BAP_ASCS_RSP_CODE_CONF_UNSUPPORTED:
	case BT_BAP_ASCS_RSP_CODE_CONF_REJECTED:
#if 0 /* result is always true. */
		if (!IN_RANGE((int)rsp->reason, BT_BAP_ASCS_REASON_NONE, BT_BAP_ASCS_REASON_CIS)) {
			LOG_WRN("Invalid reason %u for code %u", rsp->reason, rsp->code);
		}
#endif
		break;
...
{
	return (enum bt_bap_ep_state)ep->status.state;
}
			     ...
			     uint8_t *cc, uint8_t len, struct bt_bap_ascs_rsp *rsp)
{
	...
	bt_pacs_cap_foreach((enum bt_audio_dir)ep->dir, codec_lookup_id, &lookup_data);
...
static int ase_config(struct bt_ascs_ase *ase, const struct bt_ascs_config *cfg)
{
...
#else
	err = ascs_ep_set_codec(&ase->ep, cfg->codec.id, sys_le16_to_cpu(cfg->codec.cid),
				sys_le16_to_cpu(cfg->codec.vid), (uint8_t *)cfg + sizeof(struct bt_ascs_config), cfg->cc_len,
				&rsp);
#endif
	if (err) {
	...
	if (ase->ep.stream != NULL) {
		if (unicast_server_cb != NULL &&
			...
			err = unicast_server_cb->reconfig(ase->ep.stream, (enum bt_audio_dir)ase->ep.dir,
		...
		if (unicast_server_cb != NULL &&
			...
			err = unicast_server_cb->config(ase->conn, &ase->ep, (enum bt_audio_dir)ase->ep.dir,
		    ...
		    struct bt_audio_codec_qos *qos, struct bt_bap_ascs_rsp *rsp)
{
	...
	if (ep->iso == NULL) {
		...
		if (bt_bap_iso_get_ep(false, iso, (enum bt_audio_dir)ep->dir) != NULL) {
...
static ssize_t ascs_qos(struct bt_conn *conn, struct net_buf_simple *buf)
{
	...
	for (uint8_t i = 0; i < req->num_ases; i++) {
		...
		cqos.framing = (enum bt_audio_codec_qos_framing)qos->framing;
		cqos.phy = qos->phy;
...
static bool ascs_parse_metadata(struct bt_data *data, void *user_data)
{
	...
	if (!BT_AUDIO_METADATA_TYPE_IS_KNOWN((int)data_type)) {
	...
	case BT_AUDIO_METADATA_TYPE_STREAM_CONTEXT: {
		...
		if (IS_ENABLED(CONFIG_BT_CAP_ACCEPTOR) &&
			...
			if (!is_context_available(result->conn, (enum bt_audio_dir)ep->dir, context)) {
				LOG_WRN("Context 0x%04x is unavailable", context);
				*result->rsp = BT_BAP_ASCS_RSP(
					BT_BAP_ASCS_RSP_CODE_METADATA_REJECTED, (enum bt_bap_ascs_reason)data_type);
				result->err = -EACCES;
				return false;
			}
		}
...
#else
	err = bt_audio_data_parse(((uint8_t *)meta + sizeof(struct bt_ascs_metadata)), meta->len, ascs_parse_metadata, &result);
#endif
...
#else
		err = unicast_server_cb->metadata(stream, ((uint8_t *)meta + sizeof(struct bt_ascs_metadata)), meta->len, &rsp);
#endif
	} else {
		err = -ENOTSUP;
		rsp = BT_BAP_ASCS_RSP(BT_BAP_ASCS_RSP_CODE_UNSPECIFIED,
...
#else
	(void)memcpy(ep->codec_cfg.meta, ((uint8_t *)meta + sizeof(struct bt_ascs_metadata)), meta->len);
#endif
...
#else
		err = unicast_server_cb->enable(stream, ((uint8_t *)meta + sizeof(struct bt_ascs_metadata)), meta->len, &rsp);
#endif
	} else {
		err = -ENOTSUP;
		rsp = BT_BAP_ASCS_RSP(BT_BAP_ASCS_RSP_CODE_UNSPECIFIED,
