...
#define LOG_ENABLE IS_ENABLED(CONFIG_BT_DEBUG_BAP_BROADCAST_ASSISTANT)
...
/*EDGEFAST: Do not give initialization value. */
static sys_slist_t broadcast_assistant_cbs;
			    ...
			    struct bt_bap_scan_delegator_recv_state *recv_state)
{
	...
	recv_state->pa_sync_state = (enum bt_bap_pa_state)net_buf_simple_pull_u8(&buf);
	recv_state->encrypt_state = (enum bt_bap_big_enc_state)net_buf_simple_pull_u8(&buf);
