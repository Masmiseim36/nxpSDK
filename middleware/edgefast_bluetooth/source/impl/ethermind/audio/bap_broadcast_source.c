...
static void broadcast_source_set_state(struct bt_bap_broadcast_source *source, uint8_t state)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&source->subgroups, subgroup, _node, struct bt_bap_broadcast_subgroup) {
		struct bt_bap_stream *stream;

		SYS_SLIST_FOR_EACH_CONTAINER(&subgroup->streams, stream, _node, struct bt_bap_stream) {
...
static enum bt_bap_ep_state broadcast_source_get_state(struct bt_bap_broadcast_source *source)
{
	...
	return (enum bt_bap_ep_state)stream->ep->status.state;
}
				     ...
				     struct bt_bap_broadcast_source_param *param)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&source->subgroups, subgroup, _node, struct bt_bap_broadcast_subgroup) {
		...
		for (size_t i = 0U; i < subgroup_param->params_count; i++) {
			...
			SYS_SLIST_FOR_EACH_CONTAINER(&subgroup->streams, subgroup_stream, _node, struct bt_bap_stream) {
	...
	for (size_t i = 0U; i < param->params_count; i++) {
		...
		if (i == 0) {
			subgroup =
				SYS_SLIST_PEEK_HEAD_CONTAINER(&source->subgroups, subgroup, _node, struct bt_bap_broadcast_subgroup);
		} else {
			subgroup = SYS_SLIST_PEEK_NEXT_CONTAINER(subgroup, _node, struct bt_bap_broadcast_subgroup);
		}
