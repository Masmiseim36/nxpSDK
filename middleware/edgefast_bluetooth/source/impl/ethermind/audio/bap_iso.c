...
#define LOG_ENABLE IS_ENABLED(CONFIG_BT_DEBUG_BAP_ISO)
...
void bt_bap_iso_configure_data_path(struct bt_bap_ep *ep, struct bt_audio_codec_cfg *codec_cfg)
{
	...
	struct bt_bap_iso_dir *iso_dir = bap_iso_get_iso_dir(is_unicast_client, bap_iso, (enum bt_audio_dir)ep->dir);
...
void bt_bap_iso_bind_ep(struct bt_bap_iso *iso, struct bt_bap_ep *ep)
{
	...
	iso_dir = bap_iso_get_iso_dir(is_unicast_client_ep(ep), iso, (enum bt_audio_dir)ep->dir);
