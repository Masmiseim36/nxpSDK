...
#define LOG_ENABLE IS_ENABLED(CONFIG_BT_DEBUG_BAP_UNICAST_CLIENT)
#define LOG_MODULE_NAME bt_bap_unicast_client
...
BUILD_ASSERT_MSG(CONFIG_BT_BAP_UNICAST_CLIENT_ASE_SNK_COUNT > 0 ||
...
BUILD_ASSERT_MSG(CONFIG_BT_BAP_UNICAST_CLIENT_ASE_SRC_COUNT == 0 ||
...
#if defined(CONFIG_BT_BAP_DEBUG_STREAM_SEQ_NUM) && (CONFIG_BT_BAP_DEBUG_STREAM_SEQ_NUM)
					...
					uint8_t old_state)
{
	...
	stream->qos->framing = (enum bt_audio_codec_qos_framing)qos->framing;
					...
					struct bt_audio_codec_cap *codec_cap)
{
	...
	if (meta_len > 0U) {
#if CONFIG_BT_AUDIO_CODEC_CAP_MAX_METADATA_SIZE < 255
	...
	for (uint8_t i = 0U; i < rsp->num_ase; i++) {
		...
		case BT_ASCS_CONFIG_OP:
			if (unicast_client_cbs->config != NULL) {
				unicast_client_cbs->config(stream, (enum bt_bap_ascs_rsp_code)ase_rsp->code, (enum bt_bap_ascs_reason)ase_rsp->reason);
			}
			break;
		case BT_ASCS_QOS_OP:
			if (unicast_client_cbs->qos != NULL) {
				unicast_client_cbs->qos(stream, (enum bt_bap_ascs_rsp_code)ase_rsp->code, (enum bt_bap_ascs_reason)ase_rsp->reason);
			}
			break;
		case BT_ASCS_ENABLE_OP:
			if (unicast_client_cbs->enable != NULL) {
				unicast_client_cbs->enable(stream, (enum bt_bap_ascs_rsp_code)ase_rsp->code, (enum bt_bap_ascs_reason)ase_rsp->reason);
			}
			break;
		case BT_ASCS_START_OP:
			if (unicast_client_cbs->start != NULL) {
				unicast_client_cbs->start(stream, (enum bt_bap_ascs_rsp_code)ase_rsp->code, (enum bt_bap_ascs_reason)ase_rsp->reason);
			}
			break;
		case BT_ASCS_DISABLE_OP:
			if (unicast_client_cbs->disable != NULL) {
				unicast_client_cbs->disable(stream, (enum bt_bap_ascs_rsp_code)ase_rsp->code, (enum bt_bap_ascs_reason)ase_rsp->reason);
			}
			break;
		case BT_ASCS_STOP_OP:
			if (unicast_client_cbs->stop != NULL) {
				unicast_client_cbs->stop(stream, (enum bt_bap_ascs_rsp_code)ase_rsp->code, (enum bt_bap_ascs_reason)ase_rsp->reason);
			}
			break;
		case BT_ASCS_METADATA_OP:
			if (unicast_client_cbs->metadata != NULL) {
				unicast_client_cbs->metadata(stream, (enum bt_bap_ascs_rsp_code)ase_rsp->code,
							     (enum bt_bap_ascs_reason)ase_rsp->reason);
			}
			break;
		case BT_ASCS_RELEASE_OP:
			...
			if (client_ep != NULL && client_ep->release_requested) {
				...
				if (unicast_client_cbs->release != NULL) {
					unicast_client_cbs->release(stream, (enum bt_bap_ascs_rsp_code)ase_rsp->code,
								    (enum bt_bap_ascs_reason)ase_rsp->reason);
				}
			}
			break;
...
static void long_ase_read(struct bt_bap_unicast_client_ep *client_ep)
{
	...
	if (client->busy) {
		...
		err = k_work_reschedule(&client_ep->ase_read_work,
					BT_CONN_INTERVAL_TO_US(conn_info.le.interval) / 1000);
		if (err < 0) {
...
static void discover_cb(struct bt_conn *conn, int err)
{
	...
	client->dir = (enum bt_audio_dir)0U;
	reset_att_buf(client);
	client->busy = false;
				     ...
				     struct bt_bap_unicast_group *group)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&group->streams, stream, _node, struct bt_bap_stream) {
		if (stream->conn != conn) {
...
static void unicast_group_free(struct bt_bap_unicast_group *group)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&group->streams, stream, next, _node, struct bt_bap_stream) {
				       ...
				       size_t num_param)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&unicast_group->streams, tmp_stream, _node, struct bt_bap_stream) {
		total_stream_cnt++;
	}
...
int bt_bap_unicast_group_delete(struct bt_bap_unicast_group *unicast_group)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&unicast_group->streams, stream, _node, struct bt_bap_stream) {
						       ...
						       const void *data, uint16_t length)
{
	...
	if (unicast_client_cbs != NULL && unicast_client_cbs->available_contexts != NULL) {
		unicast_client_cbs->available_contexts(conn, (enum bt_audio_context)context.snk, (enum bt_audio_context)context.src);
	}
