...
#include "../host/bt_pal_keys.h"

//#include "common/bt_str.h"

#define LOG_ENABLE IS_ENABLED(CONFIG_BT_DEBUG_CSIP_SET_COORDINATOR)
...
#if defined(CONFIG_ASSERT) && (CONFIG_ASSERT > 0)
...
static void lock_changed(struct bt_csip_set_coordinator_csis_inst *inst, bool locked)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&csip_set_coordinator_cbs, listener, _node, struct bt_csip_set_coordinator_cb) {
			     ...
			     struct bt_gatt_discover_params *params)
{
	...
	if (params->type == BT_GATT_DISCOVER_CHARACTERISTIC &&
		...
		if (bt_uuid_cmp(chrc->uuid, BT_UUID_CSIS_SET_SIRK) == 0) {
			...
			notify_handler = (void*)sirk_notify_func;
			...
			notify_handler = (void*)size_notify_func;
			...
			notify_handler = (void*)lock_notify_func;
		...
		if (sub_params != NULL && notify_handler != NULL) {
			...
			if (sub_params->value != 0) {
				...
				sub_params->notify = (bt_gatt_notify_func_t)notify_handler;
