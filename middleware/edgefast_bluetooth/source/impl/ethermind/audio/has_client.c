...
{
	enum bt_has_capabilities caps = (enum bt_has_capabilities)0;
...
static void handle_read_preset_rsp(struct bt_has_client *inst, struct net_buf_simple *buf)
{
	...
	record.properties = (enum bt_has_properties)pdu->properties;
	record.name = name;
	...
	utf8_lcpy(name, (const char *)pdu->name, name_len);
...
static void discover_complete(struct bt_has_client *inst)
{
	...
	client_cb->discover(inst->conn, 0, &inst->has,
			    (enum bt_has_hearing_aid_type)(inst->has.features & BT_HAS_FEAT_HEARING_AID_TYPE_MASK),
...
static void discover_failed(struct bt_conn *conn, int err)
{
	...
	client_cb->discover(conn, err, NULL, (enum bt_has_hearing_aid_type)0, (enum bt_has_capabilities)0);
}
