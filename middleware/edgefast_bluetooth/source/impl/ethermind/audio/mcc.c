...
#include "ots/ots_client_internal.h"
#endif
...
#define LOG_ENABLE IS_ENABLED(CONFIG_BT_DEBUG_MCC)
...
int bt_mcc_set_current_track_obj_id(struct bt_conn *conn, uint64_t obj_id)
{
	...
	sys_put_le48(obj_id, (uint8_t *)mcs_inst->write_buf);
	mcs_inst->write_params.offset = 0;
	mcs_inst->write_params.data = mcs_inst->write_buf;
...
#if (defined(CONFIG_BT_MCC_SHELL) && (CONFIG_BT_MCC_SHELL > 0))
