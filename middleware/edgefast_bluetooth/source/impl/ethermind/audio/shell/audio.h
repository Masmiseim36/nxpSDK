...
static inline void print_qos(const struct shell *sh, const struct bt_audio_codec_qos *qos)
{
	...
	(defined(CONFIG_BT_BAP_UNICAST) && (CONFIG_BT_BAP_UNICAST > 0))
	shell_print(sh,
						 ...
						 enum bt_audio_context context)
{
	...
	for (size_t i = 0U; i < 16; i++) {
		...
		if (context & bit_val) {
			shell_print(sh, "%*s%s (0x%04X)", indent, "", context_bit_to_str((enum bt_audio_context)bit_val),
				    bit_val);
		}
	}
}
	...
	for (size_t i = 0; i < 16; i++) {
		...
		if (freq & bit_val) {
			shell_print(sh, "%*s%s (0x%04X)", indent, "",
				    codec_cap_freq_bit_to_str((enum bt_audio_codec_cap_freq)bit_val), bit_val);
		}
	}
}
	...
	for (size_t i = 0; i < 8; i++) {
		...
		if (frame_dur & bit_val) {
			shell_print(sh, "%*s%s (0x%02X)", indent, "",
				    codec_cap_frame_dur_bit_to_str((enum bt_audio_codec_cap_frame_dur)bit_val), bit_val);
		}
	}
}
					      ...
					      enum bt_audio_codec_cap_chan_count chan_count)
{
	...
	for (size_t i = 0; i < 8; i++) {
		...
		if (chan_count & bit_val) {
			shell_print(sh, "%*s%s (0x%02X)", indent, "",
				    codec_cap_chan_count_bit_to_str((enum bt_audio_codec_cap_chan_count)bit_val), bit_val);
		}
	}
}
		...
		for (size_t i = 0; i < 32; i++) {
			...
			if (chan_allocation & bit_val) {
				shell_print(sh, "%*s%s (0x%08X)", indent, "",
					    chan_location_bit_to_str((enum bt_audio_location)bit_val), bit_val);
			}
		}
	}
}
