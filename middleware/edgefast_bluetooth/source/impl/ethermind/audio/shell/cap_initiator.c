					   ...
					   char *argv[])
{
	struct bt_bap_unicast_group_stream_param
		group_stream_params[CAP_UNICAST_CLIENT_STREAM_COUNT] = {NULL};
	struct bt_bap_unicast_group_stream_pair_param
		pair_params[CAP_UNICAST_CLIENT_STREAM_COUNT] = {NULL};
	struct bt_cap_unicast_audio_start_stream_param
		stream_param[CAP_UNICAST_CLIENT_STREAM_COUNT] = {NULL};
	struct bt_conn *connected_conns[CONFIG_BT_MAX_CONN] = {NULL};
	struct bt_cap_unicast_audio_start_param start_param = {(enum bt_cap_set_type)0};
					    ...
					    char *argv[])
{
	struct bt_cap_unicast_audio_update_stream_param
		stream_params[CAP_UNICAST_CLIENT_STREAM_COUNT] = {NULL};
	struct bt_cap_unicast_audio_update_param param = {(enum bt_cap_set_type)0};
	int err = 0;
					  ...
					  char *argv[])
{
	...
	struct bt_cap_unicast_audio_stop_param param = {(enum bt_cap_set_type)0};
	int err = 0;
			    ...
			    size_t chan_cnt, size_t conn_index, size_t ep_index)
{
	...
	if (chan_cnt == 1U) {
		...
		new_chan_alloc = (enum bt_audio_location)(BT_AUDIO_LOCATION_FRONT_LEFT | BT_AUDIO_LOCATION_FRONT_RIGHT);
