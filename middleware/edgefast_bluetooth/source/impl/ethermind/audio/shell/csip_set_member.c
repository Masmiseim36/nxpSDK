...
{
    if ((shell_status_t)kStatus_Success != SHELL_RegisterCommand(shell, &g_shellCommandcsip_set_member))
    {
        shell_print(shell, "Shell register command %s failed!", g_shellCommandcsip_set_member.pcCommand);
    }
}
