...
{
    if ((shell_status_t)kStatus_Success != SHELL_RegisterCommand(shell, &g_shellCommandhas_client))
    {
        shell_print(shell, "Shell register command %s failed!", g_shellCommandhas_client.pcCommand);
    }
}
