...
{
    if ((shell_status_t)kStatus_Success != SHELL_RegisterCommand(shell, &g_shellCommandmedia))
    {
        shell_print(shell, "Shell register command %s failed!", g_shellCommandmedia.pcCommand);
    }
}
