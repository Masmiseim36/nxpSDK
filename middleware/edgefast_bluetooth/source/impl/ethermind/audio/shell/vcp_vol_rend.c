...
#if (defined(CONFIG_BT_VCP_VOL_REND_VOCS) && (CONFIG_BT_VCP_VOL_REND_VOCS > 0))
...
#endif /* CONFIG_BT_VCP_VOL_REND_VOCS */
#if (defined(CONFIG_BT_VCP_VOL_REND_AICS) && (CONFIG_BT_VCP_VOL_REND_AICS > 0))
...
{
    if ((shell_status_t)kStatus_Success != SHELL_RegisterCommand(shell, &g_shellCommandvcp_vol_rend))
    {
        shell_print(shell, "Shell register command %s failed!", g_shellCommandvcp_vol_rend.pcCommand);
    }
}
