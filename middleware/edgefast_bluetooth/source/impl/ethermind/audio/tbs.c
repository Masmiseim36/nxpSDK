...
#define LOG_ENABLE IS_ENABLED(CONFIG_BT_DEBUG_TBS)
...
#if (defined(CONFIG_BT_GTBS) && (CONFIG_BT_GTBS > 0))
			  ...
			  uint16_t uri_len, uint8_t *call_index)
{
	...
	for (int i = 0; i < CONFIG_BT_TBS_MAX_CALLS; i++) {
#if 1
		/* Workaround: fix the issue that the state is busy but the call has been terminated. */
		if ((inst->calls[i].state == BT_TBS_CALL_STATE_ALERTING) && (inst->calls[i].index != BT_TBS_FREE_CALL_INDEX)){
#else
	...
	if (!bt_tbs_valid_uri((const char *)ccp->uri, uri_len)) {
		return BT_TBS_RESULT_CODE_INVALID_URI;
	}

	call = call_alloc(inst, BT_TBS_CALL_STATE_DIALING, (const char *)ccp->uri, uri_len);
	if (call == NULL) {
			     ...
			     uint16_t offset, uint8_t flags)
{
	...
	case BT_TBS_CALL_OPCODE_ORIGINATE:
	{
		...
		if (is_gtbs) {
			tbs = lookup_inst_by_uri_scheme((const char *)ccp->originate.uri, uri_len);
			if (tbs == NULL) {
