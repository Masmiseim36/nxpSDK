...
#if (defined(EGDEFAST_BT_PAL_EXT_EATT) && (EGDEFAST_BT_PAL_EXT_EATT > 0))
...
/** The thread ATT response eatt_handlers likely run on.
 *
...
static int bt_eatt_chan_send(struct bt_att_chan *chan, struct net_buf *buf);

#if 0
static void att_chan_mtu_updated(struct bt_att_chan *updated_chan);
#endif

static void bt_eatt_disconnected(struct bt_l2cap_chan *chan);

#if 0
struct net_buf *bt_att_create_rsp_pdu(struct bt_att_chan *chan, uint8_t op);

static void bt_eatt_sent(struct bt_l2cap_chan *ch);
...
static void att_sent(void *user_data)
{
	...
	/* For EATT, `bt_eatt_sent` is assigned to the `.sent` L2 callback.
...
static bool eatt_chan_matches_chan_opt(struct bt_att_chan *chan, enum bt_att_chan_opt chan_opt)
{
...
static struct net_buf *eatt_get_first_buf_matching_chan(struct k_fifo *fifo, struct bt_att_chan *chan)
{
	if (IS_ENABLED(CONFIG_BT_EATT)) {
		...
		while ((buf = net_buf_get(fifo, K_NO_WAIT))) {
			...
			if (!ret &&
			    eatt_chan_matches_chan_opt(chan, meta->chan_opt)) {
				ret = buf;
			} else {
...
static struct bt_att_req *eatt_get_first_req_matching_chan(sys_slist_t *reqs, struct bt_att_chan *chan)
{
	if (IS_ENABLED(CONFIG_BT_EATT)) {
		...
		SYS_SLIST_FOR_EACH_NODE(reqs, curr) {
			...
			if (eatt_chan_matches_chan_opt(chan, meta->chan_opt)) {
				break;
			}
...
static int eatt_process_queue(struct bt_att_chan *chan, struct k_fifo *queue)
{
	...
	if (buf) {
		err = bt_eatt_chan_send(chan, buf);
		if (err) {
...
static void bt_eatt_sent(struct bt_l2cap_chan *ch)
{
	...
	err = eatt_process_queue(chan, &chan->tx_queue);
	if (!err) {
		return;
	}

	/* Process global queue */
	(void)eatt_process_queue(chan, &att->tx_queue);
}
...
static int bt_eatt_chan_send(struct bt_att_chan *chan, struct net_buf *buf)
{
	...
	if (IS_ENABLED(CONFIG_BT_EATT) &&
	    !eatt_chan_matches_chan_opt(chan, bt_att_get_tx_meta_data(buf)->chan_opt)) {
		return -EINVAL;
	}
...
static void att_send_process(struct bt_att *att)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&att->chans, chan, tmp, node) {
		...
		err = eatt_process_queue(chan, &att->tx_queue);
		if (!err) {
			/* Success */
			return;
		}
...
static void bt_eatt_chan_send_rsp(struct bt_att_chan *chan, struct net_buf *buf)
{
	int err;
			 ...
			 uint8_t err)
{
	...
	bt_eatt_chan_send_rsp(chan, buf);
}

static uint8_t eatt_mtu_req(struct bt_att_chan *chan, struct net_buf *buf)
{
	...
	bt_eatt_chan_send_rsp(chan, pdu);
...
static int bt_eatt_chan_req_send(struct bt_att_chan *chan,
				struct bt_att_req *req)
{
...
static void att_req_send_process(struct bt_att *att)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&att->chans, chan, tmp, node) {
		...
		if (bt_eatt_chan_req_send(chan, req) >= 0) {
			return;
		}
...
static uint8_t eatt_handle_rsp(struct bt_att_chan *chan, void *pdu, uint16_t len,
			      int err)
{
...
#if defined(CONFIG_BT_GATT_CLIENT)
static uint8_t eatt_mtu_rsp(struct bt_att_chan *chan, struct net_buf *buf)
{
	struct bt_att_exchange_mtu_rsp *rsp;
	uint16_t mtu;
	...
	if (mtu < BT_ATT_DEFAULT_LE_MTU) {
		return eatt_handle_rsp(chan, NULL, 0, BT_ATT_ERR_INVALID_PDU);
	}
	...
	return eatt_handle_rsp(chan, rsp, buf->len, 0);
}
#endif /* CONFIG_BT_GATT_CLIENT */
...
static uint8_t eat_att_find_info_rsp(struct bt_att_chan *chan, uint16_t start_handle,
			      uint16_t end_handle)
{
	...
	bt_eatt_chan_send_rsp(chan, data.buf);

	return 0;
}

static uint8_t eatt_find_info_req(struct bt_att_chan *chan, struct net_buf *buf)
{
	...
	return eat_att_find_info_rsp(chan, start_handle, end_handle);
}
...
static uint8_t eat_att_find_type_rsp(struct bt_att_chan *chan, uint16_t start_handle,
...
static uint8_t eatt_find_type_req(struct bt_att_chan *chan, struct net_buf *buf)
{
	...
	return eat_att_find_type_rsp(chan, start_handle, end_handle, value,
				 buf->len);
}
...
static uint8_t eatt_read_type_rsp(struct bt_att_chan *chan, struct bt_uuid *uuid,
...
static uint8_t eatt_read_type_req(struct bt_att_chan *chan, struct net_buf *buf)
{
	...
	return eatt_read_type_rsp(chan, &u.uuid, start_handle, end_handle);
}
...
static uint8_t eatt_read_rsp(struct bt_att_chan *chan, uint8_t op, uint8_t rsp,
...
static uint8_t eatt_read_req(struct bt_att_chan *chan, struct net_buf *buf)
{
	...
	return eatt_read_rsp(chan, BT_ATT_OP_READ_REQ, BT_ATT_OP_READ_RSP,
			    handle, 0);
}

static uint8_t eatt_read_blob_req(struct bt_att_chan *chan, struct net_buf *buf)
{
	...
	return eatt_read_rsp(chan, BT_ATT_OP_READ_BLOB_REQ,
			    BT_ATT_OP_READ_BLOB_RSP, handle, offset);
}

#if defined(CONFIG_BT_GATT_READ_MULTIPLE)
static uint8_t eatt_read_mult_req(struct bt_att_chan *chan, struct net_buf *buf)
{
	struct eatt_read_data data;
	uint16_t handle;
...
static uint8_t eatt_read_mult_vl_req(struct bt_att_chan *chan, struct net_buf *buf)
{
	struct eatt_read_data data;
	uint16_t handle;
...
static uint8_t eatt_read_group_rsp(struct bt_att_chan *chan, struct bt_uuid *uuid,
			       uint16_t start_handle, uint16_t end_handle)
{
...
static uint8_t eatt_read_group_req(struct bt_att_chan *chan, struct net_buf *buf)
{
	...
	return eatt_read_group_rsp(chan, &u.uuid, start_handle, end_handle);
}
...
static uint8_t eatt_write_rsp(struct bt_att_chan *chan, uint8_t req, uint8_t rsp,
...
static uint8_t eatt_write_req(struct bt_att_chan *chan, struct net_buf *buf)
{
	uint16_t handle;
	...
	return eatt_write_rsp(chan, BT_ATT_OP_WRITE_REQ, BT_ATT_OP_WRITE_RSP,
...
static uint8_t eatt_prep_write_rsp(struct bt_att_chan *chan, uint16_t handle,
...
static uint8_t eatt_prepare_write_req(struct bt_att_chan *chan, struct net_buf *buf)
{
#if CONFIG_BT_ATT_PREPARE_COUNT == 0
	return BT_ATT_ERR_NOT_SUPPORTED;
#else
	...
	return eatt_prep_write_rsp(chan, handle, offset, buf->data, buf->len);
#endif /* CONFIG_BT_ATT_PREPARE_COUNT */
}
...
static uint8_t eatt_exec_write_rsp(struct bt_att_chan *chan, uint8_t flags)
{
	struct net_buf *buf;
	uint8_t err = 0U;
	...
	while (!sys_slist_is_empty(&chan->att->prep_queue)) {
		...
		if (!err && flags == BT_ATT_FLAG_EXEC) {
			err = eatt_write_rsp(chan, BT_ATT_OP_EXEC_WRITE_REQ, 0,
					    handle, data->offset,
...
static uint8_t eatt_exec_write_req(struct bt_att_chan *chan, struct net_buf *buf)
{
#if CONFIG_BT_ATT_PREPARE_COUNT == 0
	return BT_ATT_ERR_NOT_SUPPORTED;
#else
	...
	return eatt_exec_write_rsp(chan, req->flags);
#endif /* CONFIG_BT_ATT_PREPARE_COUNT */
}

static uint8_t eatt_write_cmd(struct bt_att_chan *chan, struct net_buf *buf)
{
	uint16_t handle;
	...
	return eatt_write_rsp(chan, 0, 0, handle, 0, buf->data, buf->len);
}

#if defined(CONFIG_BT_SIGNING)
static uint8_t eatt_signed_write_cmd(struct bt_att_chan *chan, struct net_buf *buf)
{
	struct bt_conn *conn = chan->chan.chan.conn;
	...
	return eatt_write_rsp(chan, 0, 0, handle, 0, buf->data,
...
static int eatt_change_security(struct bt_conn *conn, uint8_t err)
{
	bt_security_t sec;
...
static uint8_t eatt_error_rsp(struct bt_att_chan *chan, struct net_buf *buf)
{
	struct bt_att_error_rsp *rsp;
	uint8_t err;
	...
	ret = eatt_change_security(chan->chan.chan.conn, err);
	if (ret == 0 || ret == -EBUSY) {
		/* ATT timeout work is normally cancelled in eatt_handle_rsp.
	...
	return eatt_handle_rsp(chan, NULL, 0, err);
}

static uint8_t eatt_handle_find_info_rsp(struct bt_att_chan *chan,
				     struct net_buf *buf)
{
	LOG_DBG("");

	return eatt_handle_rsp(chan, buf->data, buf->len, 0);
}

static uint8_t eatt_handle_find_type_rsp(struct bt_att_chan *chan,
				     struct net_buf *buf)
{
	LOG_DBG("");
...
static uint8_t eatt_handle_read_type_rsp(struct bt_att_chan *chan,
				     struct net_buf *buf)
{
	LOG_DBG("");
...
static uint8_t eatt_handle_read_rsp(struct bt_att_chan *chan,
				struct net_buf *buf)
{
	LOG_DBG("");
...
static uint8_t eatt_handle_read_blob_rsp(struct bt_att_chan *chan,
				     struct net_buf *buf)
{
	LOG_DBG("");
...
#if defined(CONFIG_BT_GATT_READ_MULTIPLE)
static uint8_t eatt_handle_read_mult_rsp(struct bt_att_chan *chan,
				     struct net_buf *buf)
{
	LOG_DBG("");
...
#if defined(CONFIG_BT_GATT_READ_MULT_VAR_LEN)
static uint8_t eatt_handle_read_mult_vl_rsp(struct bt_att_chan *chan,
					struct net_buf *buf)
{
	LOG_DBG("");
...
static uint8_t eatt_handle_read_group_rsp(struct bt_att_chan *chan,
				      struct net_buf *buf)
{
	LOG_DBG("");
...
static uint8_t eatt_handle_write_rsp(struct bt_att_chan *chan,
				 struct net_buf *buf)
{
	LOG_DBG("");
...
static uint8_t eatt_handle_prepare_write_rsp(struct bt_att_chan *chan,
					 struct net_buf *buf)
{
	LOG_DBG("");
...
static uint8_t eatt_handle_exec_write_rsp(struct bt_att_chan *chan,
				      struct net_buf *buf)
{
	LOG_DBG("");
...
static uint8_t eatt_notify(struct bt_att_chan *chan, struct net_buf *buf)
{
	uint16_t handle;
...
static uint8_t eatt_indicate(struct bt_att_chan *chan, struct net_buf *buf)
{
	uint16_t handle;
...
static uint8_t eatt_notify_mult(struct bt_att_chan *chan, struct net_buf *buf)
{
	LOG_DBG("chan %p", chan);
...
static uint8_t eatt_confirm(struct bt_att_chan *chan, struct net_buf *buf)
{
	LOG_DBG("");
...
static const struct eatt_handler {
	uint8_t		op;
	uint8_t		expect_len;
	att_type_t	type;
...
static int bt_eatt_recv(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
	struct bt_att_chan *att_chan = ATT_CHAN(chan);
	struct bt_conn *conn = get_conn(att_chan);
	struct bt_att_hdr *hdr;
	const struct eatt_handler *handler;
	uint8_t err;
	size_t i;
...
static void eatt_chan_detach(struct bt_att_chan *chan)
{
	struct net_buf *buf;
	...
	if (chan->req) {
		...
		eatt_handle_rsp(chan, NULL, 0, -ECONNRESET);
	}
...
static void eatt_timeout(struct k_work *work)
{
	 ...
	 */
	bt_eatt_disconnected(&chan->chan.chan);
}
...
static void bt_eatt_connected(struct bt_l2cap_chan *chan)
{
	struct bt_att_chan *att_chan = ATT_CHAN(chan);
	...
	k_work_init_delayable(&att_chan->timeout_work, eatt_timeout);
...
static uint8_t eatt_req_retry(struct bt_att_chan *att_chan)
{
...
static void bt_eatt_encrypt_change(struct bt_l2cap_chan *chan,
				  uint8_t hci_status)
{
	struct bt_att_chan *att_chan = ATT_CHAN(chan);
	struct bt_l2cap_le_chan *le_chan = BT_L2CAP_LE_CHAN(chan);
	...
	if (hci_status) {
		if (att_chan->req && att_chan->req->retrying) {
			eatt_handle_rsp(att_chan, NULL, 0,
	...
	err = eatt_req_retry(att_chan);
	if (err) {
		LOG_DBG("Retry failed (%d)", err);
		eatt_handle_rsp(att_chan, NULL, 0, err);
	}
}
#endif /* CONFIG_BT_SMP */

#if defined(CONFIG_BT_EATT)
static void bt_eatt_reconfigured(struct bt_l2cap_chan *l2cap_chan);
#endif

static void bt_eatt_status(struct bt_l2cap_chan *ch, atomic_t *status)
{
	struct bt_att_chan *chan = ATT_CHAN(ch);
	sys_snode_t *node;
	...
	if (bt_eatt_chan_req_send(chan, ATT_REQ(node)) >= 0) {
		return;
	}
...
static void bt_eatt_released(struct bt_l2cap_chan *ch)
{
	struct bt_att_chan *chan = ATT_CHAN(ch);
...
static struct bt_att_chan *eatt_chan_new(struct bt_att *att, atomic_val_t flags)
{
	int quota = 0;
	static struct bt_l2cap_chan_ops ops = {
		.connected = bt_eatt_connected,
		.disconnected = bt_eatt_disconnected,
		.recv = bt_eatt_recv,
		.sent = bt_eatt_sent,
		.status = bt_eatt_status,
	#if defined(CONFIG_BT_SMP)
		.encrypt_change = bt_eatt_encrypt_change,
	#endif /* CONFIG_BT_SMP */
		.released = bt_eatt_released,
	#if defined(CONFIG_BT_EATT)
		.reconfigured = bt_eatt_reconfigured,
	#endif /* CONFIG_BT_EATT */
	};
	struct bt_att_chan *chan;
 ...
 * placed as the last one to ensure that SMP channel is properly initialized before bt_eatt_connected
...
int bt_eatt_connect(struct bt_conn *conn, size_t num_channels)
{
	...
	while (num_channels--) {
		att_chan = eatt_chan_new(att, BIT(ATT_ENHANCED));
		if (!att_chan) {
			break;
		}
...
#endif /* CONFIG_BT_EATT */
#endif /* EGDEFAST_BT_PAL_EXT_EATT */