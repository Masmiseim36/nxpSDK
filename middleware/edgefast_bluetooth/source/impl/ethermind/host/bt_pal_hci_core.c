...
#if !(defined(CONFIG_BT_EXT_ADV_LEGACY_SUPPORT) && (CONFIG_BT_EXT_ADV_LEGACY_SUPPORT > 0))
...
#define RPA_TIMEOUT          BT_MSEC(RPA_TIMEOUT_MS)
...
static struct bt_br_discovery_result *discovery_results;
...
#if ((defined(CONFIG_BT_OBSERVER) && ((CONFIG_BT_OBSERVER) > 0U)) || (defined(CONFIG_BT_BROADCASTER) && ((CONFIG_BT_BROADCASTER) > 0U)))
...
static struct bt_le_ext_adv *bt_adv_lookup_legacy(void)
{
#if (defined(CONFIG_BT_EXT_ADV) && ((CONFIG_BT_EXT_ADV) > 0U))
	return bt_dev.adv;
#else
...
static int le_set_private_addr(uint8_t id)
{
	...
	BT_INFO("RPA ADDR %02X:%02X:%02X:%02X:%02X:%02X\r\n", rpa.val[0], rpa.val[1], rpa.val[2], rpa.val[3], rpa.val[4], rpa.val[5]);
	return err;
}
...
{
	struct adv_id_check_data *check_data = (struct adv_id_check_data *)data;
...
static int bt_le_create_conn_ext(const struct bt_conn *conn)
{
...
#if ((defined(CONFIG_BT_CENTRAL) && (CONFIG_BT_CENTRAL > 0U)) && !(defined(CONFIG_BT_WHITELIST) && (CONFIG_BT_WHITELIST > 0U)))
...
static int reject_conn(const bt_addr_t *bdaddr, uint8_t reason)
{
	...
	cp = (struct bt_hci_cp_reject_conn_req *)net_buf_add(buf, sizeof(*cp));
...
static int accept_sco_conn(const bt_addr_t *bdaddr, struct bt_conn *sco_conn)
{
	...
	cp = (struct bt_hci_cp_accept_sync_conn_req *)net_buf_add(buf, sizeof(*cp));
	bt_addr_copy(&cp->bdaddr, bdaddr);
...
static int accept_conn(const bt_addr_t *bdaddr)
{
	...
	cp = (struct bt_hci_cp_accept_conn_req *)net_buf_add(buf, sizeof(*cp));
	bt_addr_copy(&cp->bdaddr, bdaddr);
...
{
	struct bt_hci_evt_conn_request *evt = (struct bt_hci_evt_conn_request *)buf->data;
	struct bt_conn *conn;
...
static void appl_hci_esco_audio_config(UCHAR coding)
{
    API_RESULT retval;
...
{
	struct bt_hci_evt_sync_conn_complete *evt = (struct bt_hci_evt_sync_conn_complete *)buf->data;
	struct bt_conn *sco_conn;
	uint16_t handle = sys_le16_to_cpu(evt->handle);
...
{
	struct bt_hci_evt_conn_complete *evt = (struct bt_hci_evt_conn_complete *)buf->data;
	struct bt_conn *conn;
	...
	cp = (struct bt_hci_cp_read_remote_features *)net_buf_add(buf, sizeof(*cp));
...
static int request_name(const bt_addr_t *addr, uint8_t pscan, uint16_t offset)
{
	...
	cp = (struct bt_hci_cp_remote_name_request *)net_buf_add(buf, sizeof(*cp));
...
{
	struct bt_hci_evt_inquiry_complete *evt = (struct bt_hci_evt_inquiry_complete *)buf->data;
...
static void inquiry_result_with_rssi(struct net_buf *buf)
{
	...
	while (num_reports--) {
		...
		evt = (struct bt_hci_evt_inquiry_result_with_rssi *)net_buf_pull_mem(buf, sizeof(*evt));
...
{
	struct bt_hci_evt_extended_inquiry_result *evt = (struct bt_hci_evt_extended_inquiry_result *)buf->data;
	struct bt_br_discovery_result *result;
	struct discovery_priv *priv;
...
{
	struct bt_hci_evt_remote_name_req_complete *evt = (struct bt_hci_evt_remote_name_req_complete *)buf->data;
	struct bt_br_discovery_result *result;
	struct discovery_priv *priv;
	int eir_len = 240;
	uint8_t *eir;
...
{
	struct bt_hci_evt_remote_features *evt = (struct bt_hci_evt_remote_features *)buf->data;
	uint16_t handle = sys_le16_to_cpu(evt->handle);
	...
	cp = (struct bt_hci_cp_read_remote_ext_features *)net_buf_add(buf, sizeof(*cp));
	cp->handle = evt->handle;
	cp->page = 0x01;
...
{
	struct bt_hci_evt_remote_ext_features *evt = (struct bt_hci_evt_remote_ext_features *)buf->data;
	uint16_t handle = sys_le16_to_cpu(evt->handle);
	struct bt_conn *conn;
...
{
	struct bt_hci_evt_role_change *evt = (struct bt_hci_evt_role_change *)buf->data;
	struct bt_conn *conn;
...
static int hci_id_add(uint8_t id, const bt_addr_le_t *addr, uint8_t peer_irk[16])
{
	...
	cp = (struct bt_hci_cp_le_add_dev_to_rl *)net_buf_add(buf, sizeof(*cp));
...
{
	struct bt_hci_evt_le_p256_public_key_complete *evt = (struct bt_hci_evt_le_p256_public_key_complete *)buf->data;
...
{
	struct bt_hci_evt_le_generate_dhkey_complete *evt = (struct bt_hci_evt_le_generate_dhkey_complete *)buf->data;
			     ...
			     uint16_t duration)
{
	...
	set_param = (struct bt_hci_cp_le_set_ext_scan_param *)net_buf_add(buf, sizeof(*set_param));
...
{
	struct bt_hci_rp_read_buffer_size *rp = (struct bt_hci_rp_read_buffer_size *)buf->data;
	uint16_t pkts;
    osa_status_t ret;
...
static void read_buffer_size_v2_complete(struct net_buf *buf)
{
	...
	OSA_SemaphoreCreate(bt_dev.le.acl_pkts, max_num);
	...
	OSA_SemaphoreCreate(bt_dev.le.iso_pkts, max_num);
#endif /* CONFIG_BT_ISO */
}
...
static int read_ext_features(void)
{
	...
	for (i = 1; i < LMP_FEAT_PAGES_COUNT; i++) {
		...
		cp = (struct bt_hci_cp_read_local_ext_features *)net_buf_add(buf, sizeof(*cp));
		cp->page = i;
		...
		rp = (struct bt_hci_rp_read_local_ext_features *)rsp->data;
...
static void device_supported_pkt_type(void)
{
...
static int br_init(void)
{
	...
	ssp_cp = (struct bt_hci_cp_write_ssp_mode *)net_buf_add(buf, sizeof(*ssp_cp));
	ssp_cp->mode = 0x01;
	...
	inq_cp = (struct bt_hci_cp_write_inquiry_mode *)net_buf_add(buf, sizeof(*inq_cp));
	inq_cp->mode = 0x02;
	...
	name_cp = (struct bt_hci_write_local_name *)net_buf_add(buf, sizeof(*name_cp));
	...
	if (BT_FEAT_SC(bt_dev.features)) {
		...
		sc_cp = (struct bt_hci_cp_write_sc_host_supp *)net_buf_add(buf, sizeof(*sc_cp));
    ...
    case BT_BUF_CMD:
		{
			...
			ret = BT_hci_generic_command(BT_OGF(hdr->opcode), BT_OCF(hdr->opcode), &(((uint8_t *)buf->data)[sizeof(*hdr)]), buf->len - sizeof(*hdr));
		}
        break;
    case BT_BUF_ACL_OUT:
        break;
...
static int hci_set_ad(uint16_t hci_op, const struct bt_ad *ad, size_t ad_len)
{
	...
	set_data = (struct bt_hci_cp_le_set_adv_data *)net_buf_add(buf, sizeof(*set_data));
			  ...
			  const struct bt_ad *ad, size_t ad_len)
{
	...
	set_data = (struct bt_hci_cp_le_set_ext_adv_data *)net_buf_add(buf, sizeof(*set_data));
	(void)memset(set_data, 0, sizeof(*set_data));
...
{
#if ((defined(CONFIG_BT_HCI_VS_EXT) && ((CONFIG_BT_HCI_VS_EXT) > 0U)) || (defined(CONFIG_BT_CTLR) && ((CONFIG_BT_CTLR) > 0U)))
				...
				bool  has_scan_data)
{
	...
	cp = (struct bt_hci_cp_le_set_ext_adv_param *)net_buf_add(buf, sizeof(*cp));
	(void)memset(cp, 0, sizeof(*cp));
...
#if (defined(CONFIG_BT_WHITELIST) && ((CONFIG_BT_WHITELIST) > 0U))
...
int bt_le_whitelist_add(const bt_addr_le_t *addr)
{
	...
	cp = (struct bt_hci_cp_le_add_dev_to_wl *)net_buf_add(buf, sizeof(*cp));
	bt_addr_le_copy(&cp->addr, addr);
...
int bt_le_whitelist_rem(const bt_addr_le_t *addr)
{
	...
	cp = (struct bt_hci_cp_le_rem_dev_from_wl *)net_buf_add(buf, sizeof(*cp));
	bt_addr_le_copy(&cp->addr, addr);
...
static int br_start_inquiry(const struct bt_br_discovery_param *param)
{
	...
	cp = (struct bt_hci_op_inquiry *)net_buf_add(buf, sizeof(*cp));
...
int bt_br_discovery_stop(void)
{
	...
	for (i = 0; i < discovery_results_count; i++) {
		...
		cp = (struct bt_hci_cp_remote_name_cancel *)net_buf_add(buf, sizeof(*cp));
...
int bt_dh_key_gen(const uint8_t remote_pk[64], bt_dh_key_cb_t cb)
{
	...
	cp = (struct bt_hci_cp_le_generate_dhkey *)net_buf_add(buf, sizeof(*cp));
