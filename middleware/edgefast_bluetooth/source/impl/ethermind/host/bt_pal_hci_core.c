...
static void init_work(struct bt_work *work);
...
static void cmd_state_set_init(struct cmd_state_set *state, atomic_t *target, int bit,
			bool val)
{
	state->target = target;
	state->bit = bit;
...
#define NUM_COMPLETE_EVENT_SIZE BT_BUF_SIZE(\
 ...
 */
NET_BUF_POOL_FIXED_DEFINE(num_complete_pool, 1, NUM_COMPLETE_EVENT_SIZE, NULL);
#endif /* CONFIG_BT_CONN */

#if (defined(CONFIG_BT_DISCARDABLE_BUF_COUNT) && ((CONFIG_BT_DISCARDABLE_BUF_COUNT) > 0U))
...
static void slave_update_conn_param(struct bt_conn *conn)
{
	if (IS_ENABLED(CONFIG_BT_PERIPHERAL)) {    
         ...
         */
        bt_delayed_work_submit(&conn->update_work, CONN_UPDATE_TIMEOUT);
	}
    return;
}
			...
			struct net_buf *buf, uint8_t len)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&scan_cbs, listener, node, struct bt_le_scan_cb) {
		net_buf_simple_save(&buf->b, &state);
            ...
            uint8_t  event_datalen)
{
    ...
    if (0x06 == event_type)
    {
        ...
        PRINTF("HCI_AUTHENTICATION_COMPLETE_EVENT %d\r\n", status);
...
#else
	buf = bt_buf_get_evt(event_type, uint8_t false, osaWaitForever_c);
#endif
    hdr.evt = event_type;
...
int bt_enable(bt_ready_cb_t cb)
{
	...
	bt_work_init(&bt_dev.init, init_work);
...
{
#if 0 /* TODO: defined(CONFIG_BT_DEVICE_NAME_DYNAMIC) */
	struct bt_le_ext_adv *adv = bt_adv_lookup_legacy();
...
struct net_buf *bt_buf_get_rx(enum bt_buf_type type, size_t timeout)
{
	struct net_buf *buf;
...
struct net_buf *bt_buf_get_cmd_complete(size_t timeout)
{
	struct net_buf *buf;
	unsigned int key;
...
#if 0
static struct net_buf *bt_buf_get_cmd_complete_with_op_check(uint8_t evt, uint8_t *data, size_t len, size_t timeout)
{
	struct net_buf *buf;
	unsigned int key;
	struct bt_hci_evt_cmd_complete *evtCmd = (struct bt_hci_evt_cmd_complete *)data;

	key = DisableGlobalIRQ();
	buf = bt_dev.sent_cmd;
	if (NULL != buf)
	{
		assert(evtCmd->opcode == cmd(buf)->opcode);
		if (evtCmd->opcode != cmd(buf)->opcode)
		{
			buf = NULL;
		}
		else
...
struct net_buf *bt_buf_get_evt(uint8_t evt, bool discardable, size_t timeout)
{
	switch (evt) {
#if (defined(CONFIG_BT_CONN) && ((CONFIG_BT_CONN) > 0U))
	case BT_HCI_EVT_NUM_COMPLETED_PACKETS:
			  ...
			  bt_br_discovery_cb_t cb)
{
	...
	if (!valid_br_discov_param(param, count)) {
		return -EINVAL;
	}
	...
	(void)memset(results, 0, sizeof(*results) * count);

	discovery_cb = cb;
	discovery_results = results;
	discovery_results_size = count;
	discovery_results_count = 0;
