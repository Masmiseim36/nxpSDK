...
Z_STRUCT_SECTION_DEFINE(bt_gatt_service_static);
			    ...
			    const struct bt_gatt_attr *attr, uint8_t err)
{
	...
	if (atomic_test_bit(gatt_sc.flags, SC_RANGE_CHANGED)) {
		...
		bt_delayed_work_submit(&gatt_sc.work, osaWaitNone_c);
#endif
	}
...
#if 1
static void sc_process(struct bt_work *work)
{
...
static void gatt_ccc_conn_enqueue(struct bt_conn *conn)
{
	if ((!gatt_ccc_conn_is_queued(conn)) &&
		...
		bt_delayed_work_submit(&gatt_ccc_store.work, CCC_STORE_DELAY);
	}
}
...
#if (defined(CONFIG_BT_GATT_CACHING) && ((CONFIG_BT_GATT_CACHING) > 0U))
	bt_delayed_work_init(&db_hash_work, db_hash_process);

	/* Submit work to Generate initial hash as there could be static
	 * services already in the database.
	 */
	bt_delayed_work_submit(&db_hash_work, DB_HASH_TIMEOUT);
#endif /* CONFIG_BT_GATT_CACHING */

	if (IS_ENABLED(CONFIG_BT_GATT_SERVICE_CHANGED)) {
#if 1
		bt_delayed_work_init(&gatt_sc.work, sc_process);
#endif
		if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
...
#if (defined(CONFIG_BT_SETTINGS_CCC_STORE_ON_WRITE) && ((CONFIG_BT_SETTINGS_CCC_STORE_ON_WRITE) > 0U))
	bt_delayed_work_init(&gatt_ccc_store.work, ccc_delayed_store);
#endif
}
...
submit:
	...
	bt_delayed_work_submit(&gatt_sc.work, SC_TIMEOUT);
}
				      ...
				      void *user_data)
{
	...
	write = net_buf_append_bytes(buf, length, data, osaWaitNone_c, NULL, NULL);
	if (write != length) {
			      ...
			      struct bt_gatt_write_params *params)
{
	...
	write = net_buf_append_bytes(buf, len, params->data, osaWaitNone_c,
				     NULL, NULL);
			...
			struct bt_gatt_subscribe_params *params)
{
	...
	/* Lookup existing subscriptions */
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&sub->list, tmp, next, node, struct bt_gatt_subscribe_params) {
		/* Remove subscription */
		if (params == tmp) {
			found = true;
