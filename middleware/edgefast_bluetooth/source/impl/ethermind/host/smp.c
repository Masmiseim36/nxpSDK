           ...
           )
{
    ...
    case SMP_AUTHENTICATION_COMPLETE:
        ...
        if (API_SUCCESS == status)
        {
            ...
            bondingFlag = true;
            retval = BT_smp_get_device_security_info (bd_handle, &le_auth_info);
            if (API_SUCCESS == retval)
            {
                if (NULL != conn->le.keys)
                {
    ...
    case SMP_KEY_EXCHANGE_INFO:
        ...
        ethermind_dump_bytes(key_info->enc_info, sizeof (key_info->enc_info));
        BT_DBG ("Master Identification Info:");
        ethermind_dump_bytes(key_info->mid_info, sizeof (key_info->mid_info));
        BT_DBG ("Identity Info:");
        ethermind_dump_bytes(key_info->id_info, sizeof (key_info->id_info));
        BT_DBG ("Identity Address Info:");
        ethermind_dump_bytes(key_info->id_addr_info, sizeof (key_info->id_addr_info));
        BT_DBG ("Signature Info:");
        ethermind_dump_bytes(key_info->sign_info, sizeof (key_info->sign_info));
#endif
        break;
