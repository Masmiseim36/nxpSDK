				...
				enum bt_gatt_ots_olcp_res_code result_code)
{
	...
	(void)lit_olcp_request;
	(void)lit_olcp_result;
}
			     ...
			     const void *data, uint16_t length)
{
	...
	if (op_code == BT_GATT_OTS_OLCP_PROC_RESP) {
		...
		enum bt_gatt_ots_olcp_res_code result_code =
			(enum bt_gatt_ots_olcp_res_code)net_buf_simple_pull_u8(&net_buf);
			     ...
			     const void *data, uint16_t length)
{
	...
	op_code = (enum bt_gatt_ots_oacp_proc_type)net_buf_simple_pull_u8(&net_buf);

	LOG_DBG("OACP indication");

	if (op_code == BT_GATT_OTS_OACP_PROC_RESP) {
		if (net_buf.len >= (sizeof(req_opcode) + sizeof(result_code))) {
			req_opcode = (enum bt_gatt_ots_oacp_proc_type)net_buf_simple_pull_u8(&net_buf);
			result_code = (enum bt_gatt_ots_oacp_res_code)net_buf_simple_pull_u8(&net_buf);
		} else {
