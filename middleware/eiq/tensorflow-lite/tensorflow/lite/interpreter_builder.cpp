...
#if !defined(__ANDROID__) && !defined(__APPLE__) && !defined(_WIN32) && !defined(TFLITE_MCU)
#include <dlfcn.h>
#endif
...
TfLiteStatus InterpreterBuilder::ParseTensors(
  ...
  for (int i = 0; i < tensors->size(); ++i) {
    ...
    TfLiteQuantization quantization = {.type = kTfLiteNoQuantization, .params = nullptr};
...
#else
      dims_signature_data = reinterpret_cast<const int*>(tensor->shape_signature()->data());
#endif
    }
