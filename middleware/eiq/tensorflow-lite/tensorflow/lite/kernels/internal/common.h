...
log_x_for_x_greater_than_or_equal_to_1_impl(
  ...
  FixedPoint0 r_a_tmp =
      SaturatingRoundingMultiplyByPOTParam<std::int32_t>(z_a, (z_a_headroom_plus_1 - 1));
  const int32 r_a_raw =
      SaturatingRoundingMultiplyByPOTParam<std::int32_t>((r_a_tmp * sqrt_half).raw(), 1);
  // z_pow_2_adj = max(z_pow_2_a - 0.75, z_pow_2_b - 0.25);
  // z_pow_2_adj = max(InputIntegerBits - z_a_headroom_plus_1 + 0.25,
  //                   InputIntegerBits - z_b_headroom - 0.25);
  const FixedPointAccum z_a_pow_2_adj = SaturatingAddNonGemmlowp(
      FixedPointAccum::FromRaw(SaturatingRoundingMultiplyByPOTParam<std::int32_t>(
  ...
  const int32 r_b_raw =
      SaturatingRoundingMultiplyByPOTParam<std::int32_t>(z_a.raw(), z_b_headroom);
