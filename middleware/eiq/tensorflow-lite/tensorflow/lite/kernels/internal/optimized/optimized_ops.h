  ...
  if (batches == 1) {
    ...
    for (int c = 0; c < output_depth; c += 4) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        acc = std::max<int32>(acc, -32768);
        acc = std::min<int32>(acc, 32767);
        output_ptr[c + i] = acc;
      }
    }
  } else if (batches == 4) {
...
inline void MeanImpl(const tflite::MeanParams& op_params,
  ...
  for (int out_b = 0; out_b < output_batch; ++out_b) {
    ...
    for (; out_d < end_depth; ++out_d) {
      ...
      acc += bias;
      acc = std::min<int32>(std::max(acc, kMinValue), kMaxValue);
      output_data[Offset(output_shape, out_b, 0, 0, out_d)] =
  ...
  if (unextended_input2_shape.FlatSize() == 1) {
    ...
    if ((std::abs(input2_data[0] - int_exponent) < epsilon) &&
      ...
      if (std::is_same<T, float>::value) {
        ...
        params.quantized_activation_max = std::numeric_limits<int32_t>::max();
        params.quantized_activation_min = std::numeric_limits<int32_t>::lowest();
#endif
      }
