...
//#include "third_party/eigen/blas/common.h"
#include "third_party/eigen/unsupported/Eigen/CXX11/src/Tensor/Tensor.h"
...
inline void L2Normalization(const tflite::L2NormalizationParams& op_params,
  ...
  for (int i = 0; i < outer_size; ++i) {
    ...
    for (int c = 0; c < depth; c++) {
      ...
      int32 output_val = std::min<int>(255, std::max<int>(0, unclamped_output_val));
    ...
    // exponentials.
    {
      ...
      for (; c < depth; ++c) {
        ...
        if (input_diff >= diff_min) {
          ...
          output_data_ptr[c] = std::max<int32>(std::min<int32>(unsat_output, 255), 0);
    ...
    for (int c = 0; c < depth; ++c) {
      ...
      if (input_diff > adjusted_diff_min) {
        ...
        block_output_data[c] = static_cast<uint8>(
            std::max<int>(std::min<int>(unsat_output, static_cast<int32>(255)), 0));
      } else {
  ...
  for (int b = 0; b < batches; b++) {
    for (int y0 = 0, y = 0; y <= output_height - 2; y += 2, y0++) {
      for (int x0 = 0, x = 0; x <= output_width - 2; x += 2, x0++) {
        int32 x1 = std::min<int>(x0 + 1, input_width - 1);
        int32 y1 = std::min<int>(y0 + 1, input_height - 1);
