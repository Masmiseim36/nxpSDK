...
inline void FullyConnectedSparseWeight1x4Impl(
  ...
  const int output_depth = MatchingDim(weights_shape, weights_dims_count - 2,
                                       output_shape, output_dims_count - 1);
#ifdef TFLITE_MCU
  const int32_t* w1_segments = reinterpret_cast<int32_t*>(sparsity.dim_metadata[1].array_segments->data);
  const int32_t* w1_indices = reinterpret_cast<int32_t*>(sparsity.dim_metadata[1].array_indices->data);
#else
  const int* w1_segments = sparsity.dim_metadata[1].array_segments->data;
  const int* w1_indices = sparsity.dim_metadata[1].array_indices->data;
#endif
