      ...
      if (input_zeropoint[i] == output_zeropoint &&
        ...
        for (int j = 0; j < copy_size; ++j) {
          const int32_t value =
              ...
              output_zeropoint;
          output_ptr[j] =
              static_cast<uint8_t>(std::max<int32>(std::min<int32>(255, value), 0));
        }
      }
...
inline void LocalResponseNormalization(
  ...
  for (int i = 0; i < outer_size; ++i) {
    for (int c = 0; c < depth; ++c) {
      const int begin_input_c = std::max<int>(0, c - op_params.range);
      const int end_input_c = std::min<int>(depth, c + op_params.range);
      float accum = 0.f;
...
inline void LogSoftmax(const SoftmaxParams& params,
  ...
  for (int i = 0; i < outer_size; ++i) {
    ...
    for (int c = 0; c < depth; ++c) {
      max = std::max<float>(max, input_data[i * depth + c]);
    }
...
inline void LogSoftmax(const SoftmaxParams& params,
  ...
  for (int i = 0; i < outer_size; ++i) {
    ...
    const int adjusted_diff_min =
        std::max<int>(diff_min - 1,  // Note use of > below instead of >= above.
                 MultiplyByQuantizedMultiplierSmallerThanOneExp(
    ...
    for (int c = 0; c < depth; ++c) {
      ...
      if (input_diff > adjusted_diff_min) {
        ...
        output_data[i * depth + c] = static_cast<uint8>(
            std::max<int32>(std::min<int32>(unsat_output, static_cast<int32>(255)), 0));
      } else {
...
inline void ComputeInterpolationValues(const int32 value, const int32 scale_10,
  ...
  *lower_bound = std::max<int32>(*scaled_value / (1 << 10), 0);
  *upper_bound = std::min<int32>(*scaled_value / (1 << 10) + 1, input_size - 1);
}
