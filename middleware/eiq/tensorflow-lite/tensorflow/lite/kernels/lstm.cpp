...
TfLiteStatus PopulateQuantizedLstmParams8x8_16(
  ...
  if (!use_cifg) {
    integer_lstm_param->input_variance_guard =
        std::max<int32_t>(1, 10000 * layer_norm_input_scale);
  }
  integer_lstm_param->forget_variance_guard =
      std::max<int32_t>(1, 10000 * layer_norm_forget_scale);
  integer_lstm_param->cell_variance_guard =
      std::max<int32_t>(1, 10000 * layer_norm_cell_scale);
  integer_lstm_param->output_variance_guard =
      std::max<int32_t>(1, 10000 * layer_norm_output_scale);
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input->type == kTfLiteFloat32 &&
    ...
    int32 accum_multiplier, accum_shift;
