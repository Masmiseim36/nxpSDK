...
/**
 * @file fxls896xaf_freemaster_demo.c
 * @brief The fxls896xaf_freemaster_demo.c file implements FreeMASTER demo using the ISSDK
 *        FXLS896xAF sensor driver example demonstration with interrupt mode.
 */
...
const registerwritelist_t cFxls896xConfigNormal[] = {
    ...
    /*! Disable Self-Test. */
    {FXLS896x_SENS_CONFIG1, FXLS896x_SENS_CONFIG1_ST_AXIS_SEL_DISABLED, FXLS896x_SENS_CONFIG1_ST_AXIS_SEL_MASK},
    /*! Set Wake Mode ODR Rate as 12.5Hz. */
    {FXLS896x_SENS_CONFIG3, FXLS896x_SENS_CONFIG3_WAKE_ODR_12_5HZ, FXLS896x_SENS_CONFIG3_WAKE_ODR_MASK},
    /*! Enable Interrupts for Data Ready Events. */
    {FXLS896x_INT_EN, FXLS896x_INT_EN_DRDY_EN_EN, FXLS896x_INT_EN_DRDY_EN_MASK},
    /*! Enable Temperature sensor. */
    {FXLS896x_SENS_CONFIG2,FXLS896x_SENS_CONFIG2_ANIC_TEMP_EN,FXLS896x_SENS_CONFIG2_ANIC_TEMP_MASK},
...
const registerwritelist_t cFxls896xSTXP[] = {
    /* Set Self Test Axis. */
    {FXLS896x_SENS_CONFIG1, FXLS896x_SENS_CONFIG1_FSR_16G, FXLS896x_SENS_CONFIG1_FSR_MASK},
    {FXLS896x_SENS_CONFIG1, FXLS896x_SENS_CONFIG1_ST_AXIS_SEL_EN_X, FXLS896x_SENS_CONFIG1_ST_AXIS_SEL_MASK},
    {FXLS896x_SENS_CONFIG1, FXLS896x_SENS_CONFIG1_ST_POL_POSITIVE,  FXLS896x_SENS_CONFIG1_ST_POL_MASK},
    __END_WRITE_DATA__};
...
const registerwritelist_t cFxls896xSTXN[] = {
    ...
    {FXLS896x_SENS_CONFIG1, FXLS896x_SENS_CONFIG1_FSR_16G, FXLS896x_SENS_CONFIG1_FSR_MASK},
    {FXLS896x_SENS_CONFIG1, FXLS896x_SENS_CONFIG1_ST_AXIS_SEL_EN_X, FXLS896x_SENS_CONFIG1_ST_AXIS_SEL_MASK},
    {FXLS896x_SENS_CONFIG1, FXLS896x_SENS_CONFIG1_ST_POL_NEGATIVE, FXLS896x_SENS_CONFIG1_ST_POL_MASK},
    __END_WRITE_DATA__};
...
const registerwritelist_t cFxls896xSTYP[] = {
    /* Set Self Test Axis. */
    {FXLS896x_SENS_CONFIG1, FXLS896x_SENS_CONFIG1_FSR_16G, FXLS896x_SENS_CONFIG1_FSR_MASK},
    {FXLS896x_SENS_CONFIG1, FXLS896x_SENS_CONFIG1_ST_AXIS_SEL_EN_Y, FXLS896x_SENS_CONFIG1_ST_AXIS_SEL_MASK},
    {FXLS896x_SENS_CONFIG1, FXLS896x_SENS_CONFIG1_ST_POL_POSITIVE,FXLS896x_SENS_CONFIG1_ST_POL_MASK},
    __END_WRITE_DATA__};
...
const registerwritelist_t cFxls896xSTZP[] = {
    /* Set Self Test Axis. */
    {FXLS896x_SENS_CONFIG1, FXLS896x_SENS_CONFIG1_FSR_16G, FXLS896x_SENS_CONFIG1_FSR_MASK},
    {FXLS896x_SENS_CONFIG1, FXLS896x_SENS_CONFIG1_ST_AXIS_SEL_EN_Z, FXLS896x_SENS_CONFIG1_ST_AXIS_SEL_MASK},
    {FXLS896x_SENS_CONFIG1, FXLS896x_SENS_CONFIG1_ST_POL_POSITIVE, FXLS896x_SENS_CONFIG1_ST_POL_MASK},
    __END_WRITE_DATA__};
...
const registerreadlist_t FXLS896x_ALL_REG_READ[] = {{.readFrom = FXLS896x_INT_STATUS, .numBytes = FXLS896x_NUM_REGISTERS},
                                                      __END_READ_DATA__};
...
/*! @brief This structure defines the fxls896x host operation type.*/
 ...
 *  @param[in]       fxls896x_i2c_sensorhandle_t fxls896xDriver, FXLS896x sensor I2C handle.
 ...
 *  @param[in]       fxls896x_acceldata_t *rawData, pointer to FXLS896x rawdata metadata.
 *  @param[in/out]   fxls896x_offset_noise_t *offnoiseptr, pointer to FXLS896x offset noise metadata.
...
FMSTR_TSA_TABLE_BEGIN(main_table)
	FMSTR_TSA_STRUCT(fxls896x_acceldata_t)
    ...
    FMSTR_TSA_MEMBER(fxls896x_allregs_t, accelraw, FMSTR_TSA_SINT16)
    ...
    FMSTR_TSA_MEMBER(fxls896x_allregs_t, timestamp, FMSTR_TSA_UINT32)
    ...
    FMSTR_TSA_RO_VAR(rawData, FMSTR_TSA_USERTYPE(fxls896x_acceldata_t))
...
int main(void)
{
    ...
    pGpioDriver->pin_init(&FXLS896x_INT1, GPIO_DIRECTION_IN, NULL, &fxls896x_isr_callback, NULL);
    ...
    for (;;) /* Forever loop */
    {
		...
		if (registers.trigger == 1)
		{
			...
			status = apply_register_write(fxls896xDriver, registers.offset, registers.value);
		    if (SENSOR_ERROR_NONE != status)
		    {
                return status;
		    }
		...
		{
		    /*! Apply Register Write */
			status = apply_register_read(fxls896xDriver, registers.read_offset, &(registers.read_value));
		    if (SENSOR_ERROR_NONE != status)
		    {
                return status;
		    }
		...
		{
		    /*! Apply Register Write */
			status = apply_register_readall(fxls896xDriver);
		    if (SENSOR_ERROR_NONE != status)
		    {
                return status;
		    }
        ...
        status = FXLS896x_I2C_ReadData(&fxls896xDriver, FXLS896x_ALL_REG_READ, registers.reg_addr);
        if (ARM_DRIVER_OK != status)
        {
            return -1;
        }
        ...
        status = FXLS896x_I2C_ReadData(&fxls896xDriver, cFXLS896x_whoami, (uint8_t *)&registers.reg_addr[13]);

        /*! Check the FS and apply sensitivity */
        status = FXLS896x_I2C_ReadData(&fxls896xDriver, cFXLS896x_fs_src, &regdata);
        if ((regdata & FXLS896x_SENS_CONFIG1_FSR_MASK) == 2)
        {
            sensitivity = ACCEL_4G_SENS;
        }
        else if ((regdata & FXLS896x_SENS_CONFIG1_FSR_MASK) == 4)
        {
            sensitivity = ACCEL_8G_SENS;
        }
        else if ((regdata & FXLS896x_SENS_CONFIG1_FSR_MASK) == 6)
        {
        ...
        {
        	perform_selftest(fxls896xDriver, &selftest);
...
int32_t apply_register_write(fxls896x_i2c_sensorhandle_t fxls896xDriver, uint8_t offset, uint8_t value)
{
    ...
    status = FXLS896x_I2C_Configure(&fxls896xDriver, fxls896x_register_write);
    if (SENSOR_ERROR_NONE != status)
    {
 ...
 */
int32_t apply_register_read(fxls896x_i2c_sensorhandle_t fxls896xDriver, uint8_t read_offset, uint8_t *read_value)
{
    int32_t status;
    ...
    status = FXLS896x_I2C_ReadData(&fxls896xDriver, fxls896x_register_read, read_value);
    if (SENSOR_ERROR_NONE != status)
    {
        return SENSOR_ERROR_WRITE;
    }
...
int32_t apply_register_readall(fxls896x_i2c_sensorhandle_t fxls896xDriver)
{
	...
	for (int reg_offset = FXLS896x_INT_STATUS; reg_offset <= FXLS896x_SELF_TEST_CONFIG2; reg_offset++)
	{
		registerreadlist_t fxls896x_register_readall[] = {
				 /*! Set register offset with provided value */
				{.readFrom = reg_offset, .numBytes = 1}, __END_READ_DATA__};

		status = FXLS896x_I2C_ReadData(&fxls896xDriver, fxls896x_register_readall, &(registers.readall_value[reg_offset]));
		if (SENSOR_ERROR_NONE != status)
		{
...
int32_t update_dropdown_selection(fxls896x_allregs_t *registers, uint8_t caller)
{
	...
	switch (caller)
	{
		case FXLS896x_REG_WRITE:
		    ...
		    if(FXLS896x_SENS_CONFIG1 == registers->offset) //FS Selection
		    {
			    registers->fs_value = registers->value;
		    }
		    else if (FXLS896x_SENS_CONFIG2 == registers->offset)
		    {
			    registers->mods_value = registers->value;
		    }
		    else if (FXLS896x_SENS_CONFIG3 == registers->offset)
		    {
		...
		case FXLS896x_REG_READ: //Called from Register Read
            ...
            /*! Update drop down option based on updated read value */
		    if(FXLS896x_SENS_CONFIG1 == registers->read_offset) //FS Selection
		    {
			    registers->fs_value = registers->read_value;
		    }
		    else if (FXLS896x_SENS_CONFIG2 == registers->read_offset)
		    {
			    registers->mods_value = registers->read_value;
		    }
		    else if (FXLS896x_SENS_CONFIG3 == registers->read_offset)
		    {
		...
		case FXLS896x_ALLREG_READ: //Called from Register ReadAll
			...
			registers->fs_value   = registers->reg_addr[FXLS896x_SENS_CONFIG1];
			registers->mods_value = registers->reg_addr[FXLS896x_SENS_CONFIG2];
			registers->odr_value = registers->reg_addr[FXLS896x_SENS_CONFIG3];
			break;
        default:
...
int32_t perform_selftest(fxls896x_i2c_sensorhandle_t fxls896xDriver, fxls896x_selftest_t *selftest)
{
	...
	while (axis<6)
	{
		...
		switch(axis)
		{
			case 0:status = FXLS896x_I2C_Configure(&fxls896xDriver, cFxls896xSTXP);break;
			case 1:status = FXLS896x_I2C_Configure(&fxls896xDriver, cFxls896xSTXN);break;
			case 2:status = FXLS896x_I2C_Configure(&fxls896xDriver, cFxls896xSTYP);break;
			case 3:status = FXLS896x_I2C_Configure(&fxls896xDriver, cFxls896xSTYN);break;
			case 4:status = FXLS896x_I2C_Configure(&fxls896xDriver, cFxls896xSTZP);break;
			case 5:status = FXLS896x_I2C_Configure(&fxls896xDriver, cFxls896xSTZN);break;
			default:break;
		}
		...
		for(counter=0;counter<1;counter++)
		{
			...
			Register_I2C_Write(&I2C_S_DRIVER,&(fxls896xDriver.deviceInfo),FXLS896x_I2C_ADDR, FXLS896x_SENS_CONFIG1,0x00,0x00,0x00);
			if (ARM_DRIVER_OK != status)
			{
				return SENSOR_ERROR_WRITE;
			}
			...
			/*! Read new raw sensor data from the FXLS896x. */
			status = FXLS896x_I2C_ReadData(&fxls896xDriver, cFxls896xOutputNormal, &data_.byte_data[0]);
			if (ARM_DRIVER_OK != status)
