...
static int lfs_alloc(lfs_t *lfs, lfs_block_t *block) {
    while (true) {
        ...
        if (lfs->free.off == lfs->free.ack - lfs->free.begin) {
            LFS_WARN("No more free space %ld", (long)(lfs->free.off + lfs->free.begin));
...
static int lfs_dir_fetch(lfs_t *lfs,
    ...
    if (!valid) {
        LFS_ERROR("Corrupted dir pair at %ld %ld", (long)tpair[0], (long)tpair[1]);
        return LFS_ERR_CORRUPT;
    }
...
relocate:
        ...
        LFS_DEBUG("Bad block at %ld", (long)dir->pair[0]);
        ...
        if (lfs_paircmp(oldpair, (const lfs_block_t[2]){0, 1}) == 0) {
            LFS_WARN("Superblock %ld has become unwritable", (long)oldpair[0]);
            return LFS_ERR_CORRUPT;
        }
    ...
    if (relocated) {
        ...
        LFS_DEBUG("Relocating %ld %ld to %ld %ld",
                (long)oldpair[0], (long)oldpair[1], (long)dir->pair[0], (long)dir->pair[1]);
...
relocate:
        LFS_DEBUG("Bad block at %ld", (long)nblock);
...
relocate:
    LFS_DEBUG("Bad block at %ld", (long)file->block);
...
int lfs_mount(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    if (err || memcmp(superblock.d.magic, "littlefs", 8) != 0) {
        LFS_ERROR("Invalid superblock at %ld %ld", (long)dir.pair[0], (long)dir.pair[1]);
        return LFS_ERR_CORRUPT;
    }
...
static int lfs_relocate(lfs_t *lfs,
    ...
    if (res) {
        ...
        if (lfs_paircmp(oldpair, lfs->root) == 0) {
            LFS_DEBUG("Relocating root %ld %ld", (long)newpair[0], (long)newpair[1]);
        ...
        if (!(0x80000000 & pdir.d.size)) {
            ...
            if (!res) {
                ...
                LFS_DEBUG("Found orphan %ld %ld",
                        (long)pdir.d.tail[0], (long)pdir.d.tail[1]);
            ...
            if (!lfs_pairsync(entry.d.u.dir, pdir.d.tail)) {
                ...
                LFS_DEBUG("Found desync %ld %ld",
                        (long)entry.d.u.dir[0], (long)entry.d.u.dir[1]);
