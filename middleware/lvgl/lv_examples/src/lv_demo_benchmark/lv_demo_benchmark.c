...
//                lv_table_set_cell_type(table, row, 1, 2);
            ...
            PRINTF("\n\r %s(fps): %d : +opa: %d", scenes[i].name, scenes[i].fps_normal, scenes[i].fps_opa);
        }
...
static void fall_anim(lv_obj_t * obj)
{
    /* Add so that lv_obj_get_width(obj) and lv_obj_get_height(obj) can get the right result.  */
