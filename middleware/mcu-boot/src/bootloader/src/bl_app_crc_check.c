...
static void assert_pin_to_indicate_crc_check_failure(void)
{
    ...
    // Set pin as output
    CRC_CHECK_FAILURE_PIN_GPIO->PDDR |= ( 1u << CRC_CHECK_FAILURE_PIN_NUMBER);
    // Toggle pin (the inverse of default logic state) for crc check failure
    CRC_CHECK_FAILURE_PIN_GPIO->PTOR = 1u << CRC_CHECK_FAILURE_PIN_NUMBER;
}
...
void init_crc_check_status(property_store_t *propertyStore)
{
    ...
    if (!((propertyStore->configurationData.crcStartAddress == 0xffffffffu) &&
          (propertyStore->configurationData.crcByteCount == 0xffffffffu) &&
          (propertyStore->configurationData.crcExpectedValue == 0xffffffffu)))
    {
        propertyStore->crcCheckStatus = (uint32_t)kStatus_AppCrcCheckInactive;
    }
    else
    {
        propertyStore->crcCheckStatus = (uint32_t)kStatus_AppCrcCheckInvalid;
    }
}
...
static bool is_crc_check_address_valid(bootloader_configuration_data_t *config)
{
    ...
    isAddressValid = (config->crcByteCount != 0u) && (config->crcStartAddress <= 0xffffffffu - config->crcByteCount + 1u);
    if (isAddressValid)
    {
        ...
        map = (const memory_map_entry_t *)&g_bootloaderContext.memoryMap[(uint32_t)kIndexFlashArray];
        isAddressValid = ((config->crcStartAddress >= map->startAddress) &&
                          (config->crcStartAddress + config->crcByteCount - 1u <= map->endAddress));
#else
...
bool is_application_crc_check_pass(void)
{
    ...
    if ((uint32_t)kStatus_AppCrcCheckInvalid != propertyStore->crcCheckStatus)
    {
        ...
        {
            void * ptr = &propertyStore->configurationData;
            uint32_t calculated_crc = calculate_application_crc32(
                (crc_checksum_header_t *)ptr, (uint32_t)kBootloaderConfigAreaAddress);

            if (calculated_crc != propertyStore->configurationData.crcExpectedValue)
            {
                propertyStore->crcCheckStatus = (uint32_t)kStatus_AppCrcCheckFailed;
            }
            else
            {
                isCrcCheckPassed = true;
                propertyStore->crcCheckStatus = (uint32_t)kStatus_AppCrcCheckPassed;
            }
        }
        else
        {
            propertyStore->crcCheckStatus = (uint32_t)kStatus_AppCrcCheckOutOfRange;
        }
#if defined(BL_FEATURE_CRC_ASSERT) && BL_FEATURE_CRC_ASSERT
        if (!isCrcCheckPassed)
        {
