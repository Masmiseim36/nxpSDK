...
/* Array of LPUART peripheral clock name */
static const clock_ip_name_t s_lpuartPeriphClock[] = LPUART_PERIPH_CLOCKS;
#endif
...
status_t LPUART_Init(LPUART_Type *base, const lpuart_config_t *config, uint32_t srcClock_Hz)
{
...
#if defined(K32H844P_SERIES)
    CLOCK_EnableClock(s_lpuartPeriphClock[LPUART_GetInstance(base)]);
#endif
    /* Enable lpuart clock */
    ...
    base->CTRL |= (uint8_t)config->parityMode; /* merge to once C1 register write */
...
{
    assert(s_lpuartHandle[0]);

    LPUART_HandleIRQ(LPUART0, s_lpuartHandle[0]);
}
...
{
    assert(s_lpuartHandle[1]);

    LPUART_HandleIRQ(LPUART1, s_lpuartHandle[1]);
}
...
{
    assert(s_lpuartHandle[2]);

    LPUART_HandleIRQ(LPUART2, s_lpuartHandle[2]);
}
...
{
    assert(s_lpuartHandle[3]);

    LPUART_HandleIRQ(LPUART3, s_lpuartHandle[3]);
}
...
{
    assert(s_lpuartHandle[4]);

    LPUART_HandleIRQ(LPUART4, s_lpuartHandle[4]);
}
...
{
    assert(s_lpuartHandle[5]);

    LPUART_HandleIRQ(LPUART5, s_lpuartHandle[5]);
}
