...
static status_t semc_nor_issue_unlock_bypass_buffer_program(semc_nor_config_t *config,
                                                            uint32_t ipgCmdAddr,
                                                            uint8_t *data,
                                                            uint32_t length);
...
void semc_nor_cmd_sequence_cycle_info_init(semc_nor_config_t *config)
{
    ...
    {
        s_norIpgCmdSeqCycleInfo.addrDataCycle[index].address = semc_ipg_command_convert_nor_address(
            &config->memConfig, s_norIpgCmdSeqCycleInfo.addrDataCycle[index].address);
    }
...
status_t semc_nor_make_use_of_cfi_parameter(semc_nor_config_t *config)
{
    ...
    status =
        semc_axi_memory_read(cfiStartAddress, (uint8_t *)&s_norCfiParameterConfig, sizeof(s_norCfiParameterConfig));
    // Exit CFI Read mode
    ...
    s_norOperationInfo.bufferProgramTimeInUs =
        (1 << (s_norCfiParameterConfig.querySystemInterfaceInformation.typBufferProgramTimeInUs_2n +
               s_norCfiParameterConfig.querySystemInterfaceInformation.maxBufferProgramTimeOfTyp_2n));
    s_norOperationInfo.blockEraseTimeInUs =
        1000 * (1 << (s_norCfiParameterConfig.querySystemInterfaceInformation.typBlockEraseTimeInMs_2n +
    ...
    {
        config->blockSizeInBytes =
            (s_norCfiParameterConfig.deviceGeometryDefinition.eraseBlockRegionInfo[0][2] +
    ...
    config->pageSizeInBytes =
        (1 << (s_norCfiParameterConfig.deviceGeometryDefinition.maxProgramBufferInBytes_2n[0] +
...
void semc_nor_issue_reset_to_read(semc_nor_config_t *config)
{
    ...
    semc_ipg_command_device_write(s_norIpgCmdSeqCycleInfo.addrDataCycle[kNorCmdSeqCycleIndex_1st].address,
                                  ...
                                  s_norIpgCmdSeqCycleInfo.addrDataCycle[kNorCmdSeqCycleIndex_1st].data, ipgCmdDataSize);

    semc_ipg_command_device_write(s_norIpgCmdSeqCycleInfo.addrDataCycle[kNorCmdSeqCycleIndex_2nd].address,
                                  SEMC_IPCMD_NOR_IPG_WRITE_ARRAY,
                                  s_norIpgCmdSeqCycleInfo.addrDataCycle[kNorCmdSeqCycleIndex_2nd].data, ipgCmdDataSize);

    semc_ipg_command_device_write(anyAddress, SEMC_IPCMD_NOR_IPG_WRITE_ARRAY, kNorDeviceCmd_EPSCD_ResetToRead,
                                  ipgCmdDataSize);
}
...
void semc_nor_issue_auto_select(semc_nor_config_t *config)
{
    ...
    semc_ipg_command_device_write(s_norIpgCmdSeqCycleInfo.addrDataCycle[kNorCmdSeqCycleIndex_3rd].address,
                                  SEMC_IPCMD_NOR_IPG_WRITE_ARRAY, kNorDeviceCmd_EPSCD_AutoSelect, ipgCmdDataSize);
}
...
void semc_nor_issue_read_cfi(semc_nor_config_t *config)
{
    ...
    semc_ipg_command_device_write(ipgCmdAddr, SEMC_IPCMD_NOR_IPG_WRITE_ARRAY, kNorDeviceCmd_EPSCD_ReadCfi,
                                  ipgCmdDataSize);
}
...
void semc_nor_issue_exit_read_cfi_auto_select(semc_nor_config_t *config)
{
    ...
    semc_ipg_command_device_write(anyAddress, SEMC_IPCMD_NOR_IPG_WRITE_ARRAY, kNorDeviceCmd_EPSCD_ExitReadCfiAutoSlect,
                                  ipgCmdDataSize);
}
...
void semc_nor_issue_unlock_bypass(semc_nor_config_t *config)
{
    ...
    semc_ipg_command_device_write(s_norIpgCmdSeqCycleInfo.addrDataCycle[kNorCmdSeqCycleIndex_3rd].address,
                                  SEMC_IPCMD_NOR_IPG_WRITE_ARRAY, kNorDeviceCmd_EPSCD_UnlockBypass, ipgCmdDataSize);
}
...
void semc_nor_issue_unlock_bypass_reset(semc_nor_config_t *config)
{
    uint8_t ipgCmdDataSize = semc_ipg_command_get_data_size();
    uint32_t anyAddress = semc_ipg_command_convert_nor_address(&config->memConfig, 0);
    semc_ipg_command_device_write(anyAddress, SEMC_IPCMD_NOR_IPG_WRITE_ARRAY,
                                  kNorDeviceCmd_EPSCD_UnlockBypassResetSetup, ipgCmdDataSize);
    semc_ipg_command_device_write(anyAddress, SEMC_IPCMD_NOR_IPG_WRITE_ARRAY,
                                  kNorDeviceCmd_EPSCD_UnlockBypassResetConfirm, ipgCmdDataSize);
}

status_t semc_nor_issue_unlock_bypass_buffer_program(semc_nor_config_t *config,
                                                     uint32_t ipgCmdAddr,
                                                     uint8_t *data,
                                                     uint32_t length)
{
    ...
    do
    {
        ...
        semc_ipg_command_device_write(firstOffsetInBuffer, SEMC_IPCMD_NOR_IPG_WRITE_ARRAY,
                                      kNorDeviceCmd_EPSCD_BufferProgramSetup, ipgCmdDataSize);
        semc_ipg_command_device_write(firstOffsetInBuffer, SEMC_IPCMD_NOR_IPG_WRITE_ARRAY, currentCycles - 1,
                                      ipgCmdDataSize);
        ...
        semc_ipg_command_device_write(firstOffsetInBuffer, SEMC_IPCMD_NOR_IPG_WRITE_ARRAY,
                                      kNorDeviceCmd_EPSCD_BufferProgramConfirm, ipgCmdDataSize);
...
status_t semc_nor_issue_unlock_bypass_block_erase(semc_nor_config_t *config, uint32_t ipgCmdAddr)
{
    ...
    semc_ipg_command_device_write(anyAddress, SEMC_IPCMD_NOR_IPG_WRITE_ARRAY, kNorDeviceCmd_EPSCD_EraseBlockSetup,
                                  ipgCmdDataSize);
    semc_ipg_command_device_write(ipgCmdAddr, SEMC_IPCMD_NOR_IPG_WRITE_ARRAY, kNorDeviceCmd_EPSCD_EraseBlockConfirm,
                                  ipgCmdDataSize);
...
status_t semc_nor_flash_init(semc_nor_config_t *config)
{
    ...
    // From Spec:
    //   Reset command may cause a return to read mode from undefined states that might result from invalid command
    //   sequences. A hardware reset may be required to return to normal operation from some undefined states.
    semc_nor_issue_reset_to_read(config);
...
status_t semc_nor_flash_page_program(semc_nor_config_t *config, uint32_t dstAddr, const uint8_t *src, uint32_t length)
{
    status_t status;
    ...
    if ((axiCmdAddr < config->memConfig.norMemConfig.comMemBaseAddress) ||
        ((axiCmdAddr + length) >
         (config->memConfig.norMemConfig.comMemBaseAddress + config->memConfig.norMemConfig.comMemSizeInByte)))
    {
