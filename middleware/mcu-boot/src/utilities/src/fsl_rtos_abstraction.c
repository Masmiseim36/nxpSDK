...
{
    return (uint32_t)((tickStop - tickStart) / s_ticksPerMs);
}
...
osa_status_t OSA_SemaCreate(semaphore_t *pSem, uint8_t initValue)
{
    ...
    if (s_ticksPerMs == 0u)
    {
        s_ticksPerMs = microseconds_convert_to_ticks(1000u);
    }

    pSem->semCount = initValue;
    pSem->isWaiting = (_Bool)false;
    pSem->tickStart = 0u;
    pSem->timeout = 0u;
...
osa_status_t OSA_SemaWait(semaphore_t *pSem, uint32_t timeout)
{
    ...
    else
    {
        ...
        else if (timeout != (uint32_t)kSyncWaitForever) /* If don't wait forever, start the timer */
        {
...
osa_status_t OSA_SemaPost(semaphore_t *pSem)
{
    ...
    if (pSem->semCount == 0xFFu)
    {
        status = kStatus_OSA_Error;
    }
    else
    {
        __disable_irq();
...
{
    microseconds_delay(delay * 1000u);
}

void sync_init(sync_object_t *obj, bool state)
{
    *obj = state ? (int32_t)kSyncLocked : (int32_t)kSyncUnlocked;
}
