...
void flexspi_iomux_config(uint32_t instance, flexspi_mem_config_t *config)
{
    ...
    // Intentionally set the readSampleClkSrc to 0 because ROM support this option only
    config->readSampleClkSrc = kFlexSPIReadSampleClk_LoopbackInternally;
    // Restrict the maximum frequency to 60MHz
    if (config->serialClkFreq > 3)
    {
        config->serialClkFreq = 3;
    }
}
