...
void init_hardware(void)
{
    ...
    tmp &= (uint32_t) ~(SIM_SOPT2_LPUARTSRC_MASK | SIM_SOPT2_PLLFLLSEL_MASK);
    tmp |= SIM_SOPT2_LPUARTSRC(1); // Set the LPUART clock source to the IRC48Mhz clock
    tmp |= SIM_SOPT2_PLLFLLSEL(3); // Enable IRC48M
    SIM->SOPT2 = tmp;
