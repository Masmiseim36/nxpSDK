...
DataSource *ELFSourceFile::createDataSource(StringMatcher &matcher)
{
    ...
    for (; index < m_file->getSectionCount(); ++index)
    {
        ...
        if (!(header.sh_type == SHT_PROGBITS || header.sh_type == SHT_NOBITS || 
			  header.sh_type == SHT_INITARRAY ||
			  header.sh_type == SHT_FINIARRAY ||
			  header.sh_type == SHT_PREINITARRAY))
        {
            continue;
        }
		...
		// Ignore sections that have the write flag set.
		if ((header.sh_flags & SHF_WRITE) == SHF_WRITE && !(header.sh_type == SHT_INITARRAY ||
			header.sh_type == SHT_FINIARRAY ||
			header.sh_type == SHT_PREINITARRAY))
		{
			continue;
...
void ELFSourceFile::ELFDataSource::addSection(unsigned sectionIndex)
{
    ...
    if (section.sh_type == SHT_PROGBITS || 
		section.sh_type == SHT_INITARRAY ||
		section.sh_type == SHT_FINIARRAY ||
		section.sh_type == SHT_PREINITARRAY)
    {
