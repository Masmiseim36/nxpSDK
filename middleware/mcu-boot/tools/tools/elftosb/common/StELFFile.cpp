...
void StELFFile::readFileHeaders()
{
	...
	else
	{
		...
		m_stream.seekg(0, std::ios_base::beg);
		m_stream.read(reinterpret_cast<char *>(&elf64_hdr), sizeof(elf64_hdr));
		if (m_stream.bad())
		{
			throw StELFFileException("could not read file header");
		}

		m_header.e_type = ENDIAN_LITTLE_TO_HOST_U16(elf64_hdr.e_type);
		m_header.e_machine = ENDIAN_LITTLE_TO_HOST_U16(elf64_hdr.e_machine);
		m_header.e_version = ENDIAN_LITTLE_TO_HOST_U32(elf64_hdr.e_version);
		m_header.e_entry = ENDIAN_LITTLE_TO_HOST_U32(elf64_hdr.e_entry);
		m_header.e_phoff = ENDIAN_LITTLE_TO_HOST_U32(elf64_hdr.e_phoff);
		m_header.e_shoff = ENDIAN_LITTLE_TO_HOST_U32(elf64_hdr.e_shoff);
		m_header.e_flags = ENDIAN_LITTLE_TO_HOST_U32(elf64_hdr.e_flags);
		m_header.e_ehsize = ENDIAN_LITTLE_TO_HOST_U16(elf64_hdr.e_ehsize);
		m_header.e_phentsize = ENDIAN_LITTLE_TO_HOST_U16(elf64_hdr.e_phentsize);
		m_header.e_phnum = ENDIAN_LITTLE_TO_HOST_U16(elf64_hdr.e_phnum);
		m_header.e_shentsize = ENDIAN_LITTLE_TO_HOST_U16(elf64_hdr.e_shentsize);
		m_header.e_shnum = ENDIAN_LITTLE_TO_HOST_U16(elf64_hdr.e_shnum);
		m_header.e_shstrndx = ENDIAN_LITTLE_TO_HOST_U16(elf64_hdr.e_shstrndx);
	}
    // check magic number
    ...
    try
    {
        ...
        if (m_header.e_shoff != 0 && m_header.e_shnum > 0)
        {
            ...
            for (i = 0; i < m_header.e_shnum; ++i)
            {
				...
				{
					m_stream.read(reinterpret_cast<char *>(&sectionElf64Header), sizeof(sectionElf64Header));
				}
                if (m_stream.bad())
                {
				...
				{
					sectionHeader.sh_name = ENDIAN_LITTLE_TO_HOST_U32(sectionElf64Header.sh_name);
					sectionHeader.sh_type = ENDIAN_LITTLE_TO_HOST_U32(sectionElf64Header.sh_type);
					sectionHeader.sh_flags = ENDIAN_LITTLE_TO_HOST_U32(sectionElf64Header.sh_flags);
					sectionHeader.sh_addr = ENDIAN_LITTLE_TO_HOST_U32(sectionElf64Header.sh_addr);
					sectionHeader.sh_offset = ENDIAN_LITTLE_TO_HOST_U32(sectionElf64Header.sh_offset);
					sectionHeader.sh_size = ENDIAN_LITTLE_TO_HOST_U32(sectionElf64Header.sh_size);
					sectionHeader.sh_link = ENDIAN_LITTLE_TO_HOST_U32(sectionElf64Header.sh_link);
					sectionHeader.sh_info = ENDIAN_LITTLE_TO_HOST_U32(sectionElf64Header.sh_info);
					sectionHeader.sh_addralign = ENDIAN_LITTLE_TO_HOST_U32(sectionElf64Header.sh_addralign);
					sectionHeader.sh_entsize = ENDIAN_LITTLE_TO_HOST_U32(sectionElf64Header.sh_entsize);
				}
        ...
        if (m_header.e_phoff != 0 && m_header.e_phnum > 0)
        {
            ...
            for (i = 0; i < m_header.e_phnum; ++i)
            {
				...
				{
					m_stream.read(reinterpret_cast<char *>(&programElf64Header), sizeof(programElf64Header));
				}
				if (m_stream.bad())
                {
				...
				else
				{
					...
					programHeader.p_memsz = ENDIAN_LITTLE_TO_HOST_U32(programElf64Header.p_memsz);
					programHeader.p_flags = ENDIAN_LITTLE_TO_HOST_U32(programElf64Header.p_flags);
					programHeader.p_align = ENDIAN_LITTLE_TO_HOST_U32(programElf64Header.p_align);
				}
...
void StELFFile::byteSwapSymbolTable(const Elf32_Shdr &header, SectionDataInfo &info)
{
    ...
    for (; i < symbolCount; ++i, symbolOffset += header.sh_entsize)
    {
		...
		{
			Elf64_Sym *symbol = reinterpret_cast<Elf64_Sym *>(&info.m_data[symbolOffset]);
			symbol->st_name = ENDIAN_LITTLE_TO_HOST_U32(symbol->st_name);
			symbol->st_value = ENDIAN_LITTLE_TO_HOST_U32(symbol->st_value);
			symbol->st_size = ENDIAN_LITTLE_TO_HOST_U32(symbol->st_size);
			symbol->st_shndx = ENDIAN_LITTLE_TO_HOST_U16(symbol->st_shndx);
