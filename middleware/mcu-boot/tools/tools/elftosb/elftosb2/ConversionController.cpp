...
OperationSequence *ConversionController::convertLoadStatement(LoadStatementASTNode *statement)
{
    ...
    try
    {
        ...
        if (loadOption)
        {
            uint32_t memoryId = EncoreBootImage::ROM_MEM_ID_Internal;
            ...
            if (!strNode)
            {
                ...
                memoryId = static_cast<uint32_t>(IntNode->getValue());
            }
            else
            {
                ...
                {
                    memoryId = getMemoryId(statement, *optStr);
                }
            }

            if (memoryId == EncoreBootImage::ROM_MEM_ID_IFR_FUSE)
            {
                progOp = new ProgramOperation(EncoreBootImage::ROM_MEM_ID_IFR_FUSE);
                op = progOp;
            }
            else if (GROUPID(memoryId) == EncoreBootImage::ROM_MEM_GROUP_INTERNAL)
            {
                if (memoryId != EncoreBootImage::ROM_MEM_ID_Internal)
                {
                    Log::log(Logger::WARNING, "warning: line %d: memory(ID: %#x) locates at internal memory space, use Internal(ID: %#x) instead.\n", statement->getFirstLine(), memoryId, EncoreBootImage::ROM_MEM_ID_Internal);
                }
                op = new LoadOperation();
            }
            else
            {
                extOp = new LoadExternalOperation(memoryId);
                op = extOp;
                isExternalLoad = true;
            }
        }
...
OperationSequence *ConversionController::convertEraseStatement(EraseStatementASTNode *statement)
{
    ...
    if (!memOption)
    {
        // Memory ID is not specified. Select internal by default. 
        memoryId = EncoreBootImage::ROM_MEM_ID_Internal;
    }
    else
    {
    ...
    // create target
    if (GROUPID(memoryId) == EncoreBootImage::ROM_MEM_GROUP_INTERNAL)
    {
        if (memoryId != EncoreBootImage::ROM_MEM_ID_Internal)
        {
            Log::log(Logger::WARNING, "warning: line %d: memory(ID: %#x) locates at internal memory space, use Internal(ID: %#x) instead.\n", statement->getFirstLine(), memoryId, EncoreBootImage::ROM_MEM_ID_Internal);
    ...
    {
        FlashEraseExternalOperation *op = new FlashEraseExternalOperation;
        if (hasEndAddress)
        {
            op->setRange(beginAddress, endAddress - beginAddress);
        }
...
uint32_t ConversionController::getMemoryId(ASTNode *statement, std::string &memoryName)
{
    if (memoryName.empty())
    {
        throw semantic_error(format_string("line %d: invalid memory id (memory id is empty.)", statement->getFirstLine()));
    }

    std::pair<uint32_t, std::string> memoryIdList[] = { make_pair(EncoreBootImage::ROM_MEM_ID_Internal, "internal"),
                                                        make_pair(EncoreBootImage::ROM_MEM_ID_QSPI, "qspi"),
                                                        make_pair(EncoreBootImage::ROM_MEM_ID_IFR_FUSE, "ifr"),
                                                        make_pair(EncoreBootImage::ROM_MEM_ID_IFR_FUSE, "fuse"),
                                                        make_pair(EncoreBootImage::ROM_MEM_ID_SEMCNOR, "semcnor"),
                                                        make_pair(EncoreBootImage::ROM_MEM_ID_FLEXSPINOR, "flexspinor"),

                                                        make_pair(EncoreBootImage::ROM_MEM_ID_SEMCNAND, "semcnand"),
                                                        make_pair(EncoreBootImage::ROM_MEM_ID_SPINAND, "spinand"),
                                                        make_pair(EncoreBootImage::ROM_MEM_ID_SPIEEPROM, "spieeprom"),
                                                        make_pair(EncoreBootImage::ROM_MEM_ID_I2CEEPROM, "i2ceeprom"),
                                                        make_pair(EncoreBootImage::ROM_MEM_ID_SD, "sdcard"),
                                                        make_pair(EncoreBootImage::ROM_MEM_ID_MMC, "mmccard"),
                                                      };

    for (size_t i = 0; i < sizeof(memoryIdList) / sizeof(std::pair<uint32_t, std::string>); i++)
    {
        if (memoryName.compare(memoryIdList[i].second) == 0)
        {
            return memoryIdList[i].first;
        }
    }

    throw semantic_error(format_string("line %d: invalid memory id %s.(Unknow meory id string, please try '@' + memoryId digits instead.)", statement->getFirstLine(), memoryName.c_str()));
