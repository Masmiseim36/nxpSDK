...
typedef struct _mmc_user_config
{
    ...
    bool enableAck;                 /*!< Enable ACK for boot mode. */
    bool noPreIdle;                 /*!< Not send Pre-Idle command before start boot mode. (Not supported by eMMC4.3. */
} mmc_user_config_t;
...
extern "C"
{
    ...
    /*!
     * @brief Initializes the MMC card and host.(For bootloader use only)
     *
     * @param card Card descriptor.
     *
     * @retval kStatus_SDMMC_HostNotReady host is not ready.
    ...
    /*!
     * @brief MMC card start boot.(For bootloader use only)
     *
     * @param card Card descriptor.
     * @param buffer address to recieve data.
     * @param hostConfig host boot configurations.
