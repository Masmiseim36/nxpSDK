...
status_t flexspi_nand_get_fcb_search_cfg(uint32_t *searchCount, uint32_t *searchStride)
{
    ...
    *searchCount = 1 << (FUSE_BOOT_SEARCH_COUNT_VALUE + 1);
    // Calculate Search Stride in terms of pages for FCB
    *searchStride = 1 << (FUSE_BOOT_SEARCH_STRIDE_VALUE + 6);
...
status_t flexspi_nand_get_default_cfg_blk(flexspi_nand_config_t *config)
{
    ...
    if (FUSE_SPI_NAND_BOOT_CACHE_RD_CMD_VALUE)
    {
        readCacheCommand = FUSE_SPI_NAND_BOOT_CACHE_RD_CMD_VALUE;
    }
    ...
    if (FUSE_SPI_NAND_BOOT_PAGE_RD_CMD_VALUE)
    {
        readPageCommand = FUSE_SPI_NAND_BOOT_PAGE_RD_CMD_VALUE;
    }
    ...
    switch (FUSE_COL_ADDRESS_WIDTH_VALUE)
    {
        default:
    ...
    if (!FUSE_BYPASS_READ_STATUS_VALUE)
    {
    ...
    else
    {
        ...
        tempVal = FUSE_BYPASS_READ_STATUS_VALUE;
        if (tempVal == 0)
        {
    ...
    if (FUSE_SPI_NAND_BOOT_BUSY_BIT_OFFSET_OVERRIDE_VALUE)
    {
        memCfg->busyOffset = FUSE_SPI_NAND_BOOT_BUSY_BIT_OFFSET_OVERRIDE_VALUE;
    }

    // Get the command interval
    switch (FUSE_CS_INTERVAL_VALUE)
    {
        default:
