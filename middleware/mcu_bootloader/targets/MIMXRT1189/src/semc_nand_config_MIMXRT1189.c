...
// Definitions
////////////////////////////////////////////////////////////////////////////////
#define SEMC_NAND_FUSE_MAP_VERSION (1)
...
enum
{
    ...
    kFuseSemcNand_PcsSelection_A8  = 4U,
};
...
enum
{
    kFuseSemcNand_EccType_ByNAND = 0U,
    kFuseSemcNand_EccType_BySEMC = 1U,
};
...
status_t semc_nand_get_fcb_search_cfg(uint32_t *searchCount, uint32_t *searchStride)
{
    ...
    *searchCount = 1 << (FUSE_SEMC_NAND_SEARCH_COUNT_VALUE);
    // Calculate Search Stride in terms of pages for FCB
    if (FUSE_SEMC_NAND_SEARCH_STRIDE_VALUE)
    {
        *searchStride = 1 << (FUSE_SEMC_NAND_SEARCH_STRIDE_VALUE);
    }
    else
    {
...
void semc_nand_get_config_data_from_fuse(semc_nand_config_t *config)
{
    ...
    // Get ONFI flag
    fuseValue = FUSE_SEMC_NAND_ONFI_COMPLIANT_VALUE;
    switch (fuseValue)
    {
    ...
    // Get ECC check type
//    fuseValue = FUSE_SEMC_NAND_ECC_TYPE_VALUE;
//    switch (fuseValue)
//    {
...
//    }
    ...
    switch (fuseValue)
    {
        case kFuseSemcNand_EccType_BySEMC:
            pNandConfig->eccCheckType = kSemcNandEccCheckType_SemcBchECC;
            break;
        case kFuseSemcNand_EccType_ByNAND:
        default:
            pNandConfig->eccCheckType = kSemcNandEccCheckType_DeviceECC;
            break;
    }    

    pNandConfig->memConfig.nandMemConfig.semcBchMode = 0u;
    pNandConfig->memConfig.nandMemConfig.semcBchSectorNumber = 0u;
    pNandConfig->memConfig.nandMemConfig.semcBchSectorSize = 0u;
    if (pNandConfig->eccCheckType == kSemcNandEccCheckType_SemcBchECC)
    {
        // Sector number = 4 sectors, sector-size = 512 bytes
        pNandConfig->memConfig.nandMemConfig.semcBchSectorNumber = 4u;
        pNandConfig->memConfig.nandMemConfig.semcBchSectorSize = 512u;
        // Get ECC mode
        fuseValue = FUSE_SEMC_NAND_ECC_MODE_VALUE;
        switch (fuseValue)
        {
            case kFuseSemcNand_EccMode_BCH8:
                pNandConfig->memConfig.nandMemConfig.semcBchMode = 2u;
                break;
            case kFuseSemcNand_EccMode_BCH4:
            default:
                pNandConfig->memConfig.nandMemConfig.semcBchMode = 1u;
                break;
        }
    }    
    ...
    // Get I/O Port Size
    fuseValue = FUSE_SEMC_NAND_IO_PORT_WIDTH_VALUE;
    switch (fuseValue)
    {
    ...
    // Get EDO mode
    fuseValue = FUSE_SEMC_NAND_EDO_MODE_VALUE;
    switch (fuseValue)
    {
    ...
    // Get RDY Polarity
    fuseValue = FUSE_SEMC_NAND_RDY_POLARITY_VALUE;
    switch (fuseValue)
    {
    ...
    fuseValue = FUSE_SEMC_NAND_READY_CHECK_TYPE_VALUE;
    switch (fuseValue)
    {
    ...
    fuseValue = FUSE_SEMC_NAND_ROW_COL_ADDR_MODE_VALUE;
    switch (fuseValue)
    {
    ...
    fuseValue = FUSE_SEMC_NAND_COL_ADDRESS_WIDTH_VALUE;
    switch (fuseValue)
    {
    ...
    fuseValue = FUSE_SEMC_NAND_STATUS_CMD_TYPE_VALUE;
    switch (fuseValue)
    {
    ...
    fuseValue = FUSE_SEMC_NAND_ACCESS_COMMAND_VALUE;
    switch (fuseValue)
    {
    ...
    // Get Pages in block
    fuseValue = FUSE_SEMC_NAND_PAGES_IN_BLOCK_VALUE;
    switch (fuseValue)
    {
    ...
    /* Get PCS selection */
    fuseValue = FUSE_SEMC_NAND_PCS_SELECTION_VALUE;
    switch (fuseValue)
    {
        case kFuseSemcNand_PcsSelection_A8:
            pNandConfig->memConfig.nandMemConfig.cePortOutputSelection = kFuseSemcNand_PcsSelection_A8;
            break;      
        case kFuseSemcNand_PcsSelection_CSX3:
            pNandConfig->memConfig.nandMemConfig.cePortOutputSelection = kSemcCeOutputSelection_MUX_CSX3;
            break;
    ...
    if (pNandConfig->onfiVersion != kNandOnfiVersion_None)
    {
        ...
        fuseValue = FUSE_SEMC_NAND_DEVICE_ECC_STATUS_VALUE;
        switch (fuseValue)
        {
        ...
        fuseValue = FUSE_SEMC_NAND_TIMING_MODE_VALUE;
        switch (fuseValue)
        {
