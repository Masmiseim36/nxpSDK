...
status_t bl_get_update_partition_info(uint32_t image, partition_t *ptn)
{
    ...
    {
        PRINTF("Test state detected, cannot determine update partition\n");
        goto error;
    }
...
status_t bl_get_active_image_info(uint32_t image, bl_image_info_t *info)
{   
    if (image >= MCUBOOT_IMAGE_NUMBER)
    {
        return kStatus_InvalidArgument;
    ...
    static struct image_header ih;     //align
    struct flash_area *fa;
    uint32_t slot;
    int faid;
    
    
    slot = get_active_image(image);
    faid = (image * 2) + slot;
    fa = &boot_flash_map[faid];     
