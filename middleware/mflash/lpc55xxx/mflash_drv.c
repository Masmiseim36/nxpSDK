...
int32_t mflash_drv_init(void)
{
    ...
    /* Disable this check, because the MFLASH_SECTOR_SIZE was decreased so it is possible to store more files in mflash
     * files system. */
...
int32_t mflash_drv_write(void *any_addr, uint8_t *a_data, uint32_t a_len, uint8_t *b_data, uint32_t b_len)
{
    ...
    for (block_i = 0; block_i < block_max; block_i++)
    {
        ...
        else if ((tmp_a_len && tmp_a_len < (MFLASH_WORD_SIZE)) || (tmp_b_len && tmp_b_len < (MFLASH_WORD_SIZE)))
        {
