...
static int32_t mflash_drv_page_program_internal(uint32_t page_addr, uint32_t *data)
{
    ...
    DCACHE_InvalidateByRange(FlexSPI_AMBA_BASE + page_addr, MFLASH_PAGE_SIZE);
...
{
    memcpy(buffer, (void *)(addr + FlexSPI_AMBA_BASE), len);
    return kStatus_Success;
}
...
{
    /* FLASH starts at FlexSPI_AMBA_BASE */
    return (void *)(addr + FlexSPI_AMBA_BASE);
}
