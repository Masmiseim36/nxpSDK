...
BaseType_t mflash_save_file(char *pcFileName, uint8_t *pucData, uint32_t ulDataSize)
{
    ...
    if (0 != mflash_drv_write(tmp_file.flash_addr + sizeof(tmp_meta), pucData, ulDataSize))
        return pdFALSE;
    /* Set & write meta data before a real data */
    tmp_meta.magic_no  = MFLASH_META_MAGIC_NO;
    tmp_meta.file_size = ulDataSize;
    if (0 != mflash_drv_write(tmp_file.flash_addr, (uint8_t *)&tmp_meta, sizeof(tmp_meta)))
        return pdFALSE;
    return pdTRUE;
}
