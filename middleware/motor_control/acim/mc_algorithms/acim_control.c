...
{
    /* required speed ramp */
    psScalarACIM->fltSpdMeRamp = GFLIB_Ramp_FLT(
        psScalarACIM->fltSpdMeReq, &psScalarACIM->sRampParSpdMe);
    
    /* frequency generation by integrating required speed */
    psScalarACIM->fltPosEl += psScalarACIM->fltGainSpdMe2PosEl *
                              psScalarACIM->fltSpdMeRamp;
    ...
    psFocACIM->sUDQReq.fltQ = GFLIB_Limit_FLT(MLIB_Mul_FLT(
        psScalarACIM->fltGainRpm2Volt, MLIB_Abs_FLT(psScalarACIM->fltSpdMeRamp)), 
        psScalarACIM->fltUMin, 
        MLIB_Mul_FLT(psFocACIM->fltDutyLim, psFocACIM->fltUDcBus));
...
{   
    ...
    psFocACIM->sUDQReq.fltD = GFLIB_UpperLimit_FLT(psFocACIM->sUDQReq.fltD, 
        MLIB_Mul_FLT(psFocACIM->fltDutyLim, psFocACIM->fltUDcBus));
...
void MCS_ACIMFocCtrlVoltageA1(mcs_acim_foc_a1_t *psFocACIM)
{
    ...
    AMCLIB_ACIMRotFluxObsrv_FLT_FC(&psFocACIM->sIAlBe,
                                   &psFocACIM->sUAlBe,
                                   &psFocACIM->sRFO);

    /* run MRAS speed estimator on background */
    AMCLIB_ACIMSpeedMRAS_FLT_FC(&psFocACIM->sIAlBe,
                                &psFocACIM->sRFO.sPsiRotSAlBe,
                                 psFocACIM->sSpdObs.a32RotPos,
                                &psFocACIM->sSpdObs);
    ...
    GMCLIB_Park_FLT(&psFocACIM->sUAlBe, 
                    &sSCFOC, 
    ...
    if(psFocACIM->bFlagDTComp)
    {
        MCS_DTComp(&psFocACIM->sUAlBeDTComp,
                   &psFocACIM->sIABC,
                   psFocACIM->fltUDcBusFilt);
    }


    /* DC-bus voltage ripple elimination */
    GMCLIB_ElimDcBusRipFOC_F16ff(psFocACIM->fltUDcBus, 
                                 &psFocACIM->sUAlBeDTComp,
                                 &psFocACIM->sUAlBeCmpFrac);
    
    /* space vector modulation */
    psFocACIM->ui16SectorSVM = GMCLIB_SvmStd_F16(&psFocACIM->sUAlBeCmpFrac, 
                      &psFocACIM->sDutyABC);
...
void MCS_ACIMFocCtrlCurrentA1(mcs_acim_foc_a1_t *psFocACIM)
{
    ...
    if (!psFocACIM->bFlagSpdStart)
    {
        ...
        psFocACIM->sSCFOC.fltSin = 
            GFLIB_Sin_FLTa(psFocACIM->sRFO.a32RotFluxPos);
        psFocACIM->sSCFOC.fltCos = 
            GFLIB_Cos_FLTa(psFocACIM->sRFO.a32RotFluxPos);
    }
    else
    {
    ...
    psFocACIM->sPIParId.fltLowerLim = 
        MLIB_MulNeg_FLT(psFocACIM->fltDutyLim, psFocACIM->fltUDcBus);
    psFocACIM->sPIParId.fltUpperLim =  
        MLIB_Mul_FLT(psFocACIM->fltDutyLim,psFocACIM->fltUDcBus);
    
    /* calculate d-axis current control error */
    psFocACIM->sIDQErr.fltD = MLIB_Sub_FLT(psFocACIM->sIDQReq.fltD,
                                           psFocACIM->sIDQ.fltD);
    
    /* calculate d-axis required voltage */
    psFocACIM->sUDQReq.fltD = GFLIB_CtrlPIpAW_FLT(
                                   psFocACIM->sIDQErr.fltD, 
                                  &psFocACIM->bFlagPIIdStopInt, 
                                  &psFocACIM->sPIParId);
    
    /* calculate q-axis voltage limitation */
    psFocACIM->sPIParIq.fltUpperLim = GFLIB_Sqrt_FLT(
        psFocACIM->sPIParId.fltUpperLim *
        psFocACIM->sPIParId.fltUpperLim -
    ...
    psFocACIM->sIDQErr.fltQ = MLIB_Sub_FLT(psFocACIM->sIDQReq.fltQ, 
                                           psFocACIM->sIDQ.fltQ);
    
    /* calculate q-axis required voltage */
    psFocACIM->sUDQReq.fltQ  = GFLIB_CtrlPIpAW_FLT(
                                    psFocACIM->sIDQErr.fltQ,
                                   &psFocACIM->bFlagPIIqStopInt, 
                                   &psFocACIM->sPIParIq);
...
{ 
    ...
    else
    {
        ...
        /* field weakening */
        fltIdTmp = AMCLIB_CtrlFluxWkng_FLT(psFocACIM->sIDQErr.fltQ,
                                          psFocACIM->sUDQReq.fltQ,
                                          psFocACIM->sPIParIq.fltUpperLim,
                                          &psSpdFlux->sFluxWkng);  
 
        /* max torque per ampere calculation including exponential filter */
        psSpdFlux->fltIdMTPA = MLIB_Mac_FLT(psSpdFlux->fltIdMTPA, 
            psSpdFlux->fltIdMTPAFiltCoef, 
            MLIB_Sub_FLT(MLIB_Abs_FLT(psFocACIM->sIDQ.fltQ), 
                         psSpdFlux->fltIdMTPA));
        
        /* limit required MTPA current */
        psSpdFlux->fltIdMTPA = GFLIB_Limit_FLT(psSpdFlux->fltIdMTPA, 
                                               psSpdFlux->fltIdReqMin, 
                                               psSpdFlux->fltIdReqMax);
        
        /* limit d-axis current in case of flux-weakening */
        psFocACIM->sIDQReq.fltD = GFLIB_UpperLimit_FLT(psSpdFlux->fltIdMTPA, 
                                                       fltIdTmp);
    }
    ...
    if(psSpdFlux->fltSpdMeReq != psSpdFlux->sRampParSpdMe.fltTarget)
    {
        ...
        GFLIB_FlexSRampCalcIncr_FLT(psSpdFlux->fltSpdMeReq, 
                                    psSpdFlux->fltSRampDuration, 
                                   &psSpdFlux->sRampParSpdMe);   
    }
    ...
    psSpdFlux->bFlagPISpdMeStopInt = (psFocACIM->sPIParId.bLimFlag || 
                                     ...
                                     (MLIB_Abs_FLT(psSpdFlux->fltSpdMeRamp) >  
                                      MLIB_Abs_FLT(psSpdFlux->fltSpdMeFilt));
    
    /* calculate control error */
    psSpdFlux->fltSpdMeErr = MLIB_Sub_FLT(psSpdFlux->fltSpdMeRamp, 
                                          psSpdFlux->fltSpdMeFilt);
 
    /* compensation for changing d-axis current */
    fltIdTmp = 1.0F / psFocACIM->sIDQReq.fltD;
        
    /* speed controller */
    psFocACIM->sIDQReq.fltQ = GFLIB_CtrlBetaIPpAW_FLT(
        psSpdFlux->fltSpdMeRamp * fltIdTmp, psSpdFlux->fltSpdMeFilt * fltIdTmp, 
                ...
                float_t fltUDcBusFilt)
{
    ...
    /* compensate phase A */
    i16CurrSign = (sIABC->fltA > DTCOMP_I_RANGE) - 
                  (sIABC->fltA < -DTCOMP_I_RANGE);
    if (!i16CurrSign)
        ...
        sUABCErr.fltA = i16CurrSign * 
            ((MLIB_Abs_FLT(sIABC->fltA) - DTCOMP_I_RANGE) * 
             DTCOMP_LINCOEFF - fltUerrMax);

    /* compensate phase B */
    i16CurrSign = (sIABC->fltB > DTCOMP_I_RANGE) - 
                  (sIABC->fltB < -DTCOMP_I_RANGE);
    if (!i16CurrSign)
        ...
        sUABCErr.fltB = i16CurrSign * 
          ((MLIB_Abs_FLT(sIABC->fltB) - DTCOMP_I_RANGE) * 
           DTCOMP_LINCOEFF - fltUerrMax);

    /* compensate phase C */
    i16CurrSign = (sIABC->fltC > DTCOMP_I_RANGE) - 
                  (sIABC->fltC < -DTCOMP_I_RANGE);
    if (!i16CurrSign)
        ...
        sUABCErr.fltC = i16CurrSign * 
          ((MLIB_Abs_FLT(sIABC->fltC) - DTCOMP_I_RANGE) * 
           DTCOMP_LINCOEFF - fltUerrMax);
