...
void MCDRV_Curr3Ph2ShChanAssignInit(mcdrv_adc16_t *this)
{
    ...
    this->pui32AuxAdcBase->SC1[2] =
        ((this->pui32AuxAdcBase->SC1[2] & ~(uint16_t)ADC_SC1_ADCH_MASK) | this->ui16ChanNumAux);
#else
    /* Aux sampling - select SC1[1] (sample number 1) */
...
#if (defined(KE18F16_SERIES))
/*!
 * @brief Function reads and passes auxiliary2 sample (temp at HVP-KE18)
 *
 * @param this   Pointer to the current object
 *
 * @return none
