...
{
    /* data processing */
    METERLIBLP3PH_ProcSamples(&mlib, 
                              FRAC24(sCalcPower->fltUphA * 
                                     (1.0F / (float)U_MAX)),
                              FRAC24(*sCalcPower->pfltIphA * 
                                     (1.0F / (float)I_MAX)),
                              FRAC24(sCalcPower->fltUphB * 
                                     (1.0F / (float)U_MAX)), 
                              FRAC24(*sCalcPower->pfltIphB * 
                                     (1.0F / (float)I_MAX)),
                              FRAC24(sCalcPower->fltUphC * 
                                     (1.0F / (float)U_MAX)), 
                              FRAC24(*sCalcPower->pfltIphC * 
                                     (1.0F / (float)I_MAX)), 
                              NULL);

    /* calculate watt hours */
    METERLIBLP3PH_CalcWattHours(&mlib, &wh_cnt, 
                                METERLIBLP_KWH_PR(IMP_PER_KWH));
    
    /* calculate var hours */
    METERLIBLP3PH_CalcVarHours(&mlib, &varh_cnt, 
                               METERLIBLP_KVARH_PR(IMP_PER_KVARH));
...
void MID_ReadPwr(mid_calc_power_a1_t *sCalcPower)
{
    ...
    sCalcPower->fltUrmsAvg = (float)((sUrms.dblA + sUrms.dblB + sUrms.dblC) 
                                     * MID_ONETHIRD);
    sCalcPower->fltIrmsAvg = (float)((sIrms.dblA + sIrms.dblB + sIrms.dblC) 
                                     * MID_ONETHIRD);
    sCalcPower->fltPAvg    = (float)((sP.dblA + sP.dblB + sP.dblC) 
                                     * MID_ONETHIRD);
    sCalcPower->fltQAvg    = (float)((sQ.dblA + sQ.dblB + sQ.dblC) 
                                     * MID_ONETHIRD);
    sCalcPower->fltSAvg    = (float)((sS.dblA + sS.dblB + sS.dblC) 
                                     * MID_ONETHIRD);
}
