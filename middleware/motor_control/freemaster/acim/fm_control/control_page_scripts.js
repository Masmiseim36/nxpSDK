...
function Init()
{
    ...
    actSpeedID      = pcm.SubscribeVariable("Speed Actual Filtered", 100);
    ...
    reqSpeedID      = pcm.SubscribeVariable("Speed Required",1100);
    ...
    spdReqLimHighID = pcm.SubscribeVariable("Speed Required Max",1500);
    spdReqLimLowID  = pcm.SubscribeVariable("Speed Required Min",1500);
    controlMode    = pcm.SubscribeVariable("MCAT Control",1500);
    
    
    succ = pcm.ReadVariable("MCAT Control", vValue0, tValue0, retMsg)
    if(pcm.LastVariable_vValue !== 3)
    {
         ...
         succ = pcm.WriteVariable("MCAT Control", 3, retMsg);
    }
    /* read speed scales */
    ...
    succ = pcm.ReadVariable("Speed Required Max", vValue0, tValue0, retMsg)
	if (succ)
    	speedNominal = pcm.LastVariable_vValue/100;
    succ = pcm.ReadVariable("Speed Required Min", vValue0, tValue0, retMsg)
	if (succ)
    ...
    if((dcbVoltScale>50)&&(dcbVoltScale<=100))
        dcbVoltInterMaj = 10;
    if((dcbVoltScale>100)&&(dcbVoltScale<=200))
        dcbVoltInterMaj = 20;
    if((dcbVoltScale>200))
        dcbVoltInterMaj = 50;
    ...
    /* current gauge major interval calculation */
    if(currentScale<=2)
        currentInterMaj = 0.5;
...
function SpeedSliderInit()
{
    $('#jqxSliderSetSpeed').jqxSlider(
    {
        ...
        height: 30,
        width: 300,
        min: -speedScale*100, max: speedScale*100,
        step: 500,
        ticksFrequency: 500,
        values: [-speedScale*100, speedScale*100]
    });
...
$(document).ready(function(){
    $('#jqxSliderSetSpeed').on('change', function (event)
    {
        if(demoMode == 0)
        {
            ...
            if(speedReqNew > speedNominal*100) 
            {    
                $('#jqxSliderSetSpeed').jqxSlider('setValue', speedNominal*100);  
                speedReqNew = speedNominal*100;
            }
            else if(speedReqNew < -speedNominal*100) 
            {
                speedReqNew = -speedNominal*100;
                $('#jqxSliderSetSpeed').jqxSlider('setValue', -speedNominal*100); 
            }
            succ = pcm.WriteVariable("Speed Required", speedReqNew, retMsg);
            $('#gaugeContainerSpeedRequired').jqxGauge({value: speedReqNew*0.01});
...
function Event_OnVariableChanged(a,IDsubscribedVariable)
{
    ...
    case speedLimLowID:
        ...
        CurrentLimitUpdate();
        break;
    case spdReqLimHighID:
    case spdReqLimLowID:
        // Speed gauge limits update
        SpeedLimitUpdate();
        break;
    default:
    }
...
function ApplicationSwitchUpdate()
{
    ...
    {
        $("#jqxAppSwitchButton").jqxButton('val', "NO CONNECTION");
        $("#jqxAppSwitchButton").jqxButton({template: 'danger'});
    }
}
...
{
    var speedAct = 0;
    succ = pcm.ReadVariable("Speed Actual Filtered", vValue0, tValue0, retMsg)
    if (succ)
...
/******************************************************************************
@brief   Update speed gauuge limits

@param   void

@return  none
******************************************************************************/
function SpeedLimitUpdate()
{
    var succ;
    var ranges;
    
    if(pcm.ReadVariable("Speed Required Max", vValue0, tValue0, retMsg))
        speedNominal = pcm.LastVariable_vValue/100;
    
    if (pcm.ReadVariable("Speed Required Min", vValue0, tValue0, retMsg))
        speedMinimum = pcm.LastVariable_vValue/100;
    ...
    /* update ranges */
    $('#gaugeContainerSpeedActual').jqxGauge('ranges', ranges);
