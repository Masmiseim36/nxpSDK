...
var tabHTMLFiles = [
        ...
        "inner_FLoop.html",  // Flux Loop
        "inner_PoSpeSensor.html", // PoSpes Sensors
        "inner_PoSpeRFO.html", // Sensorless
...
function build_multi_motor_selector(defaultChecking)
{
    ...
    // motor 3 
      ...
      string = string + "<div id =\"idModeSelector\" style=\" font-size: 15px; font-family: Arial; color:black; background-color: rgb(195,199,204); height: 30px; width: 200px; position: absolute; left: 644px; top: 69px;\">";    
      string = string + "<font style=\"padding-left: 0.2cm; font-size: 15px; position: relative; left: 0px; top: 5px; \">Tuning Mode:</font>";
      string = string + "<select id=\"idTunningMode\"  onChange =\"setActiveMode()\"  size=\"1\" style=\" border:0px; width:85px; font-style: italic; font-size: 15px; position: relative; left: 10px; top: 6px;\">";  
...
function build_parameter_line( varLabel, varID, varUnit, varComment, inObj, inObjDescr)
{
    ...
    if(optionalObject == 4)
    {
      string = string + "</td><td><img src='images/info.png' id='img_inf_"+varID+"' style='visibility: hidden;' title='message' width=12 height=12></td><td>";
    }
...
function build_parameter_line_SL(varLabel, varID, varUnit, varComment)
{
    ...
    if(prefixM!='')
	 ...
	 // parameter name + comment when mouse moves over
    string = string + "<td align=\"left\" width=\"10px\" style=\"font-family:Arial; font-size:14px; padding-left:0.4cm; \"> <label id = \"parText" + varLabel + "\" title=\"" + varComment + "\" >" + varLabel + "</label> </td>";
	 // next column
    string = string + "<td valign=\"top\" width=\"110px\">";
	 // input box with event "onchage"
...
function build_constant_line( varLabel, varID, valComment)
{
	 var string = "<tr>";
  	...
  	string = string + "<td align=\"left\" width=\"20px\" style=\"font-family: Arial; font-size: 14px; padding-left:0.4cm\"> <label id = \"constText" + varLabel + "\" title=\"" + valComment + "\" >" + varLabel + "</label> </td>";
      	string = string + "<td valign=\"top\" width=\"20px\">";
...
function build_VHz_control_structure_line( varID, lineName,inputLabel1, unit1, inputLabel2, unit2, inputComment1,inputComment2)
{
    ...
    // Uq_required line
    var string = string + "<tr>"; 
    var string = string + "<td><div class=\"fontControlLabelS\" title=\"Resulting supply voltage\" style=\"padding-left:0.2cm;\">Usq_req</div></td>";
...
function build_line_with_selector(varLabel, varID,varComment)
{
    ...
    // parameter name + comment when mouse moves over
    string = string + "<td align=\"left\" valign=\"middle\" style=\"font-family:Arial; font-size:14px; padding-left:0.4cm\"> <label id = \"parText" + varLabel + "\" title=\"" + varComment + "\" >" + varLabel + "</label> </td>";
 	  // next column
    //string = string + "<td valign=\"top\" width=25%>";
    // input box with event "onchage"
    
    string = string + "<td  width=\"58%\"  valign=\"center\" style=\"overflow: visible; position: relative;\"><div style=\"margin-right:-100px; width:40px; overflow: visible; position: relative;\"><select style=\"width: 140px; margin: 0px;\" id=\"" + varIDprefix + "\" onChange =\"HWboardSelector(this.value);\" size=\"1\">"; 
    if(getBoardType() == "HVP")
    {
      string = string + "<option value=\"0\">FSL HVP-MC3PH</option>";
      string = string + "<option value=\"1\">User HW</option></select></div></td><td colspan=\"3\"></td>";
    }
    else if(getBoardType() == "TWR")
    {
      string = string + "<option value=\"0\">FSL TWR-LV3PH</option>";
      string = string + "<option value=\"1\">User HW</option></select></div></td><td colspan=\"3\"></td>";
    }
    else if(getBoardType() == "FRDM")
    {
      string = string + "<option value=\"0\">FSL FRDM-LV-MCPMSM</option>"; 
      string = string + "<option value=\"1\">User HW</option></select></div></td><td colspan=\"3\"></td>";
    }
    else
...
function checkIt(id) {
    ...
    if(isNaN(object.value) || object.value==='') 
    {
        ...
        var regex = /(&nbsp;|<([^>]+)>)/ig;
        alert("The field '" + objname.replace(regex, "") + "' accepts numbers only!");
