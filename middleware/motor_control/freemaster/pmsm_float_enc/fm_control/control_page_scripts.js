...
function Init()
{
    ...
    appSwitchID     = pcm.SubscribeVariable("M1 Application Switch", 1000);
    appStateID      = pcm.SubscribeVariable("M1 Application State", 1000);
    ...
    speedLimHighID  = pcm.SubscribeVariable("M1 Speed Loop Limit High",1500);
    speedLimLowID   = pcm.SubscribeVariable("M1 Speed Loop Limit Low",1500);
	
	Motor_pp = Number(parent.document.getElementById("M1_pp").innerHTML)
	...
	if (succ)
        speedScale = ((pcm.LastVariable_vValue * SpeedAngScale) - ((pcm.LastVariable_vValue*SpeedAngScale)%500 ) + 500)/100;
	
    succ = pcm.ReadVariable("M1 Fault Threshold SpeedNom", vValue0, tValue0, retMsg)
	if (succ)
    	speedNominal = ((pcm.LastVariable_vValue * SpeedAngScale) - (pcm.LastVariable_vValue * SpeedAngScale)%100 +100) /100;
	
    succ = pcm.ReadVariable("M1 Fault Threshold SpeedMin", vValue0, tValue0, retMsg)
	if (succ)
    	speedMinimum = ((pcm.LastVariable_vValue * SpeedAngScale) - (pcm.LastVariable_vValue * SpeedAngScale)%100 +100) /100;		
    ...
    succ = pcm.ReadVariable("M1 Fault Threshold DcBusOver", vValue0, tValue0, retMsg)
    if (succ)
    	dcbVoltOver = pcm.LastVariable_vValue;
    succ = pcm.ReadVariable("M1 Fault Threshold DcBusUnder", vValue0, tValue0, retMsg)
	if (succ)
    ...
    succ = pcm.ReadVariable("M1 Speed Loop Limit High", vValue0, tValue0, retMsg);
    if (succ)
       currentLimHigh = Math.abs(pcm.LastVariable_vValue);
    succ = pcm.ReadVariable("M1 Speed Loop Limit Low", vValue0, tValue0, retMsg);
    if (succ)
...
$(document).ready(function(){
    $('#jqxSliderSetSpeed').on('change', function (event)
    {
        if(demoMode == 0)
        {
            ...
            succ = pcm.WriteVariable("M1 Speed Required", speedReqNew, retMsg);
...
$(document).ready(function(){
    $('#jqxSliderSetCurrentLimit').on('change', function (event)
    {
        ...
        succ = pcm.WriteVariable("M1 Speed Loop Limit High", currentLimNew, retMsg);
        succ = pcm.WriteVariable("M1 Speed Loop Limit Low", -currentLimNew, retMsg);
    });
});
...
$("#jqxDemoButton").on('click',function()
    {
        ...
        {
            succ = pcm.WriteVariable("Demo Mode Speed",1,retMsg);
        }
    });
});
...
$(document).ready(function(){
    $("#jqxAppSwitchButton").on('click', function ()
    {
        ...
        if(appState==0)
        {
            succ = pcm.WriteVariable("M1 Fault Clear", 1, retMsg);
        }
        else
        {
...
function DemoModeUpdate()
{
    ...
    if (demoMode)
        ...
        succ = pcm.WriteVariable("M1 Speed Required",0, retMsg);
