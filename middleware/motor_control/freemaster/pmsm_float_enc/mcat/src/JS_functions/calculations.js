...
catch(err){window.alert("Error: Failed to set inner HTML value of "+varID);}
...
}catch(err){window.alert(varID.toString());}
}    
...
catch(err){window.alert("Error: Unknown ID " + valueId.toString());}
}
...
{   
    ...
    {
        alert("InnerHTML could not be set \n\n Caller is:\n " + arguments.callee.caller.toString());
    }
...
{
alert("Error in UpdateFMVariable.\nParams:"+varId.toString()+", "+valueFrac.toString()+", "+valueFloat.toString()+"\n caller is" + arguments.callee.caller.toString());
}
}
