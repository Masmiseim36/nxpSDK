...
function clickCalculateSloop()
{
   ...
   // alert(W_CC1f+'\n'+W_CC2f); 
    ...
    rampIncUp_float = RAMP_Up*SLOOP_Ts;
    rampIncDown_float = RAMP_Down*SLOOP_Ts;
    ...
    W_IIR_B0_fl =  Math.round((2*Math.PI*SPEED_IIR_filt*SLOOP_Ts)/(2+(2*Math.PI*SPEED_IIR_filt*SLOOP_Ts))*1000000000000)/1000000000000; 
    W_IIR_B1_fl =  Math.round((2*Math.PI*SPEED_IIR_filt*SLOOP_Ts)/(2+(2*Math.PI*SPEED_IIR_filt*SLOOP_Ts))*1000000000000)/1000000000000;
    W_IIR_A1_fl =  Math.round((2*Math.PI*SPEED_IIR_filt*SLOOP_Ts-2)/(2+(2*Math.PI*SPEED_IIR_filt*SLOOP_Ts))*1000000000000)/1000000000000;
    W_IIR_B0_out = Math.round(4.0*W_IIR_B0_fl/IIRxCoefsScaleType*1000000000000)/1000000000000;
    W_IIR_B1_out = Math.round(4.0*W_IIR_B1_fl/IIRxCoefsScaleType*1000000000000)/1000000000000;
    W_IIR_A1_out = Math.round(-4.0*W_IIR_A1_fl/IIRxCoefsScaleType*1000000000000)/1000000000000;
    ...
    if(document.getElementById("SLoop") != undefined)
    {
     ...
     {	
        ...
        {
          // write values to forms in current Html page
          setInnerHtmlValueAsText("SPEED_PI_PROP_GAIN",10,W_Kpz_f, W_Kpz);
          setInnerHtmlValueAsText("SPEED_PI_INTEG_GAIN",10,W_Kiz_f, W_Kiz);
        }
        ...
        {
          //speed IIR filter
          setInnerHtmlValueAsText("SPEED_IIR_B0",7,W_IIR_B0_out,W_IIR_B0_out);
          setInnerHtmlValueAsText("SPEED_IIR_B1",7,W_IIR_B1_out,W_IIR_B1_out);
          setInnerHtmlValueAsText("SPEED_IIR_A1",7,W_IIR_A1_out,W_IIR_A1_out);
        }
