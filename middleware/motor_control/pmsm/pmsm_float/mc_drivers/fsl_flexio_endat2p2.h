...
typedef struct _flexio_endat2_type
{
    ...
    uint8_t TxdPinIndex;       /*!< Pin select for data output. */
    uint8_t RxdPinIndex;       /*!< Pin select for data input. */
    uint8_t ClkPinIndex;     /*!< Pin select for clock. */
    uint8_t DirPinIndex;       /*!< Pin select for direction. */

    uint32_t preambleNum;

    uint16_t mtLen; /* Multi-Turn bit length */
    uint16_t stLen; /* Single-Turn bit length */
