  ...
  float_t	    fltZtotal, fltRs_square, fltZtotal_square, fltXLd;
    ...
    if(sLsMeasFcn->i16AmplitudeOK != TRUE)
    {
        ...
        if(sLsMeasFcn->ui16LoopCounter < MID_TIME_300MS)
            *(sLsMeasFcn->pfltUdReq) = MLIB_Mul_FLT(sLsMeasFcn->fltUdAmplitude, GFLIB_Sin_FLTa((acc32_t)sLsMeasFcn->f16Angle));
        
        /* If current f16Idfbck > f16MeasCurrentAmp, proceed to frequency adjusting */
        if((sLsMeasFcn->ui16LoopCounter > MID_TIME_100MS) && (*(sLsMeasFcn->pfltIdfbck) > sLsMeasFcn->fltIdAmplitudeReq))
        {
    ...
    if(sLsMeasFcn->i16AmplitudeOK == TRUE && sLsMeasFcn->i16FrequencyOK == TRUE)
    {
        ...
        if(sLsMeasFcn->ui16LoopCounter <= MID_TIME_300MS)
        {
            ...
            *(sLsMeasFcn->pfltUdReq) = MLIB_Mul_FLT(sLsMeasFcn->fltUdAmplitude, GFLIB_Sin_FLTa((acc32_t)sLsMeasFcn->f16Angle));
            ...
            if((sLsMeasFcn->ui16LoopCounter > MID_TIME_100MS) && (*(sLsMeasFcn->pfltIdfbck) > sLsMeasFcn->fltIdAmplitude))
        ...
        if(sLsMeasFcn->ui16LoopCounter > MID_TIME_300MS)
        {
            ...
            sLsMeasFcn->fltLs = MLIB_Div_FLT(fltXLd, MLIB_Mul_FLT(2.0, MLIB_Mul_FLT(FLOAT_PI, sLsMeasFcn->fltFreqActual)));
