...
void MID_getKe(mid_get_ke_a1_t *sKeMeasFcn)
{
    ...
    *sKeMeasFcn->pf16PosEl = GFLIB_Integrator_F16(sKeMeasFcn->f16SpeedElRamp, &sKeMeasFcn->sSpeedIntegrator);

    /* Bemf filtering */
    f32EdFilt = MLIB_Conv_F32s(GDFLIB_FilterMA_F16(MLIB_Conv_F16l(*(sKeMeasFcn->pf32Ed)), &sKeMeasFcn->sEdMA32Filter));
    f32EqFilt = MLIB_Conv_F32s(GDFLIB_FilterMA_F16(MLIB_Conv_F16l(*(sKeMeasFcn->pf32Eq)), &sKeMeasFcn->sEqMA32Filter));
    ...
    if (sKeMeasFcn->f16SpeedElRamp == sKeMeasFcn->f16SpeedElReq)
    {
        ...
        if (sKeMeasFcn->ui16LoopCounter > M1_TIME_2400MS)
        {
            ...
            /* Divide U/I and always increase i16ShiftRsMax until the Rs result is positive and f32UdCorrectShifted is
             * not saturated */
            do
            {
                sKeMeasFcn->i16ShiftKeMax++;
                f32EtotalShifted = MLIB_ShLBiSat_F32(f32Etotal, -sKeMeasFcn->i16ShiftKeMax);
