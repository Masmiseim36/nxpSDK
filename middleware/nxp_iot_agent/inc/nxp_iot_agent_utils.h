...
/*! @brief Writes the key reference to PEM.
 * The function is used when OpenSSL crypto is selected to write the key reference generated by the Open SSL
 * to a file. The key reference provided as input depends on the used OpenSSL varsion
 * - Version < 3.x.x: EVP_PKEY
 * - Version >= 3.x.x: OSSL_PARAM
 *
 * @param[in] gen_key_ref	The key reference to be written
...
/*! @brief Gets the keystore from service descriptor.
 *
 * @param[in] ctx					Agent context
 * @param[in] service_descriptor	Input service descriptor
