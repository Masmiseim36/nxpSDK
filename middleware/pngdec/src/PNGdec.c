...
// For IAR only as in GCC
#if defined( __IAR_SYSTEMS_ICC__ )
uint16_t __builtin_bswap16(uint16_t usPixel)
{
  return ((usPixel<<8)|(usPixel>>8));
}
#endif
//
...
PNG_STATIC int PNGInit(PNGIMAGE *);
PNG_STATIC int32_t seekMem(PNGFILE *, int32_t );
PNG_STATIC int32_t readFLASH(PNGFILE *, uint8_t *, int32_t );
PNG_STATIC int32_t readRAM(PNGFILE *, uint8_t *, int32_t );
...
{
  return (int) pPNG->ucPixelType;
}; /* PNG_getPixelType() */

int PNG_getBpp(PNGIMAGE *pPNG)
{
  return (int) pPNG->ucBpp;
}; /* PNG_getBpp() */
...
PNG_STATIC int PNGParseInfo(PNGIMAGE *pPage)
{
    ...
    if (MOTOLONG(&s[12]) == 0x49484452/*'IHDR'*/) {
        ...
        {
            pPage->iError = PNG_UNSUPPORTED_INTERLACED;
            return pPage->iError;
        }
        if ((pPage->ucBpp < 8U) || (pPage->ucBpp != 8U) || (pPage->ucBpp != 16U))
        {
            pPage->iError = PNG_BPP_NOT_SUPPORTED;
            return pPage->iError;
        }
#else
        ...
        switch (pPage->ucPixelType) {
            ...
            case PNG_PIXEL_TRUECOLOR_ALPHA: // truecolor + alpha
                ...
                break;
            default:
              return PNG_UNSUPPORTED_COLORTYPE;
        } // switch
    }
#if defined(USE_PNGDEC_DRIVER) && USE_PNGDEC_DRIVER
...
#else
    if (pPage->iPitch > ((PNG_MAX_BUFFERED_PIXELS - 1 ) / 2 ) ){
      pPage->iError = PNG_WIDTH_TOO_BIG;
      return pPage->iError; // TODO
    }

    if( (pPage->ucBpp != 1 ) && (pPage->ucBpp != 2 ) && (pPage->ucBpp != 4 ) && (pPage->ucBpp != 8 )){
      pPage->iError = PNG_BPP_NOT_SUPPORTED; // 1,4 & 8 bit per pixel/color are supported TODO
      return pPage->iError;
    }
#endif
    return PNG_SUCCESS;
            ...
            if (iBpp == 1) {
                ...
                *pCurr++ = (uint8_t)a;
                ...
                while (pCurr < pEnd) {
                   ...
                   c = pPrev[-iBpp];//c = *(pPrev-iBpp);// TODO
                   a = pCurr[-iBpp];//a = *(pCurr-iBpp);//
                   b = *pPrev++;
                   p = b - c;
                   pc = a - c;
...
int PNG_decode(PNGIMAGE *pPage, void *pUser, int iOptions)
{
    ...
    {
        if (pPage->pfnDraw != NULL)
        {
            /* When using the driver for decoding, callback is not supported. */
            pPage->iError = PNG_INVALID_PARAMETER;
            return pPage->iError;
        }
        else
        {
            /* Need a valid buffer for output data. */
    ...
    PNGDEC_Init(PNGDEC, &config);
    PNGDEC_SetPngBuffer(PNGDEC, pPage->PNGFile.pData, pPage->PNGFile.iSize);
    PNGDEC_SetOutputBuffer(PNGDEC, pPage->pImage, NULL);
    result = PNGDEC_Decode(PNGDEC, &image, NULL);

    pPage->iError = PNG_SUCCESS;
    if (result != kStatus_Success)
    {
        pPage->iError = PNG_DECODE_ERROR;
        if (result == kStatus_PNGDEC_NotSupported)
        {
            pPage->iError = PNG_UNSUPPORTED_FEATURE;
        }
        if (result == kStatus_PNGDEC_WidthTooLarge)
        {
            pPage->iError = PNG_WIDTH_TOO_BIG;
        }
    }
...
#if USE_DMA_MEMCPY
    int align_32byte = (int32_t)(pPage->ucPixels) % 32;
    pCurr = pPage->ucPixels + 32 - 1 - align_32byte;
    pPrev = pCurr + (pPage->iPitch + 32) / 32 * 32; //&(pPage->ucPixels[pPage->iPitch+align_32byte+3/*+1*/]); // TODO remove 1 (filter?) or keep?
#else
...
//        err = mz_inflateInit2(&d_stream, 15);
        ...
        switch (iMarker)
        {
            ...
            case 0x504c5445: //'PLTE' palette colors
                ...
                if (iOptions & PNG_FAST_PALETTE) { // create a RGB565 palette
                    ...
                    uint16_t usPixel, *d;
                    uint8_t *pal = pPage->ucPalette;
                    d = (uint16_t *)&pPage->ucPixels[sizeof(pPage->ucPixels)-512];
                    for (i=0; i<iColors; i++) {
                    usPixel = (pal[2] >> 3); // blue
                    usPixel |= ((pal[1] >> 2) << 5); // green
                    usPixel |= ((pal[0] >> 3) << 11); // red
                    *d++ = usPixel;
                    pal += 3;
                    }
                }
...
#if USE_DMA_MEMCPY
                                dma_memcpy(&pPage->pImage[y * pPage->iPitch], &pCurr[1], pPage->iPitch, false);//TODO
#else
    ...
    if(!marker_IDAT){ // IDAT is missing
      pPage->iError = PNG_MARKER_IDAT_MISSING;
    }
