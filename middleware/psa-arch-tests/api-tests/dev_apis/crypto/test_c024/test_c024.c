...
int32_t psa_aead_encrypt_test(caller_security_t caller)
{
    ...
    for (i = 0; i < num_checks; i++)
    {
        ...
        size_t                      additional_data_length = check1[i].additional_data_length;
        size_t                      nonce_length = check1[i].nonce_length;
        ...
        if (is_buffer_empty(check1[i].nonce, nonce_length) == TRUE)
        {
            nonce = NULL;
            nonce_length = 0;
        }
        ...
        if (is_buffer_empty(check1[i].additional_data, additional_data_length) == TRUE)
        {
        ...
        status = val->crypto_function(VAL_CRYPTO_AEAD_ENCRYPT, key_handle,
                    check1[i].key_alg, nonce, nonce_length, additional_data,
                    additional_data_length, check1[i].plaintext,
