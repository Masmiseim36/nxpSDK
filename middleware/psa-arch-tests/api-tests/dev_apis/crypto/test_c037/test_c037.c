...
int32_t psa_cipher_finish_test(caller_security_t caller __UNUSED)
{
    ...
    for (i = 0; i < num_checks; i++)
    {
        ...
        //NXP TEST_ASSERT_EQUAL(status, PSA_SUCCESS, TEST_CHECKPOINT_NUM(7));
        if(check1[i].expected_status == PSA_ERROR_BAD_STATE)
        {    
            TEST_ASSERT_EQUAL(status, PSA_ERROR_CONNECTION_REFUSED, TEST_CHECKPOINT_NUM(7)); //NXP
