...
U16 SST_SetScpKeyset(SST_Item_t keyId, U8 *keyEnc, U16 keyEncLen, U8 *keyMac, U16 keyMacLen, U8 *keyDek, U16 keyDekLen)
{
    ...
    assert(keyMac != NULL);
    assert(keyDek != NULL);
    assert(keyEncLen == AES_KEY_LEN_nBYTE);
    assert(keyMacLen == AES_KEY_LEN_nBYTE);
    assert(keyDekLen == AES_KEY_LEN_nBYTE);
...
U16 SST_GetCplcData(U8 *cplcData, U16 *cplcDataLen)
{
    ...
    assert(cplcData != NULL);
    assert(cplcDataLen != NULL);
