    ...
    uint8_t hasle)
{
    ...
    if (*rspLength >= 2)
    {
        ...
        {    /*Counter to be increament only in case of authentication is all kind of SCP
              and response is not 9000 */
            if ((rv != SM_OK) && (pSessionCtx->pdynScp03Ctx != NULL))
            {
                if (((pSessionCtx->pdynScp03Ctx->authType == kSSS_AuthType_AppletSCP03) ||
                        (pSessionCtx->pdynScp03Ctx->authType == kSSS_AuthType_FastSCP)) ||
    ...
    uint8_t hasle)
{
    ...
    if (pSession->hasSession) {
        ...
        * Copy the Wrapped header in the outhdr buffer */
        ...
        se05xApdu.wsSe05x_tag1W = ((se05xApdu.wsSe05x_tag1Len <= 0x7F) ? 1 : (se05xApdu.wsSe05x_tag1Len <= 0xFF) ? 2 : 3);
        ...
        se05xApdu.wsSe05x_tag1CmdLen = sizeof(*(se05xApdu.se05xCmd_hdr)) + se05xApdu.se05xCmdLCW + se05xApdu.se05xCmdLen;
    ...
    else
    {
        ...
        se05xApdu.dataToMacLen = sizeof(*(se05xApdu.se05xCmd_hdr)) + se05xApdu.se05xCmdLCW + se05xApdu.se05xCmdLC - SCP_GP_IU_CARD_CRYPTOGRAM_LEN;
    ...
    sss_status = nxpSCP03_CalculateMac_CommandAPDU(pSession->pdynScp03Ctx, se05xApdu.dataToMac, se05xApdu.dataToMacLen, macToAdd, &macLen);
    ENSURE_OR_GO_CLEANUP(sss_status == kStatus_SSS_Success);
