...
#define SSS_SUBSYSTEM_TYPE_IS_SSCP(subsystem) \
    ((subsystem == kType_SSS_SE_A71CH) || (subsystem == kType_SSS_SE_A71CL))

#define SSS_SESSION_TYPE_IS_SSCP(session) \
    (session && SSS_SUBSYSTEM_TYPE_IS_SSCP(session->subsystem))

#define SSS_KEY_STORE_TYPE_IS_SSCP(keyStore) \
    (keyStore && SSS_SESSION_TYPE_IS_SSCP(keyStore->session))

#define SSS_OBJECT_TYPE_IS_SSCP(pObject) \
    (pObject && SSS_KEY_STORE_TYPE_IS_SSCP(pObject->keyStore))

#define SSS_DERIVE_KEY_TYPE_IS_SSCP(context) \
    (context && SSS_SESSION_TYPE_IS_SSCP(context->session))

#define SSS_ASYMMETRIC_TYPE_IS_SSCP(context) \
    (context && SSS_SESSION_TYPE_IS_SSCP(context->session))

#define SSS_SYMMETRIC_TYPE_IS_SSCP(context) \
    (context && SSS_SESSION_TYPE_IS_SSCP(context->session))

#define SSS_MAC_TYPE_IS_SSCP(context) \
    (context && SSS_SESSION_TYPE_IS_SSCP(context->session))

#define SSS_RNG_CONTEXT_TYPE_IS_SSCP(context) \
    (context && SSS_SESSION_TYPE_IS_SSCP(context->session))

#define SSS_DIGEST_TYPE_IS_SSCP(context) \
    (context && SSS_SESSION_TYPE_IS_SSCP(context->session))

#define SSS_AEAD_TYPE_IS_SSCP(context) \
    (context && SSS_SESSION_TYPE_IS_SSCP(context->session))
...
typedef struct _sss_sscp_symmetric
{
    ...
    sss_algorithm_t
        algorithm;      /*!< What eventual operation algorithm be performed */
