...
#if SSS_HAVE_SE05X
#if SSS_HAVE_SCP_SCP03_SSS && SSSFTR_SE05X_AuthFastSCP
...
/* ************************************************************************** */
static sss_status_t nxFastScp_InternalAuthenticate(pSe05xSession_t se05xSession,
    SE05x_AuthCtx_FastScp_t *pAuthFScp,
...
static sss_status_t nxFastScp_calculate_master_secret(
    SE05x_AuthCtx_FastScp_t *pAuthFScp,
    uint8_t *rnd,
    size_t rndLen,
...
static sss_status_t nxFastScp_HostLocal_CalculateSessionKeys(
    SE05x_AuthCtx_FastScp_t *pAuthFScp);

static sss_status_t nxFastScp_Calculate_Initial_Mac_Chaining_Value(
    SE05x_AuthCtx_FastScp_t *pAuthFScp);

static sss_status_t nxFastScp_Calculate_Shared_secret(
    SE05x_AuthCtx_FastScp_t *pAuthFScp,
    uint8_t *sharedSecret,
    size_t *sharedSecretLen);
...
static sss_status_t nxFastScp_GetVerify_SE_Ecka_Public(
...
int get_u8buf_2bTag(uint8_t *buf,
    size_t *pBufIndex,
    const size_t bufLen,
    uint16_t tag,
    uint8_t *rsp,
    ...
    pSe05xSession_t se05xSession, SE05x_AuthCtx_FastScp_t *pAuthFScp)
{
    ...
    uint8_t hostEckaPub
        [100]; // Host public key to send to the SE for internal authenticate
    size_t hostEckaPubLen = sizeof(hostEckaPub);
    size_t hostEckabitLen;
    uint8_t
        drSE[20]; // Random bytes to retrive from SE in internal authenticate
    ...
    int offset = 0;
    NXFastSCP03_StaticCtx_t *pStatic_ctx = pAuthFScp->pStatic_ctx;
    ...
    status = sss_host_key_store_get_key(pStatic_ctx->HostEcKeypair.keyStore,
    ...
    */
    ...
    memcpy(hostEckaPub + offset,
        hostPubkey + ASN_ECC_NIST_256_HEADER_LEN,
        hostEckaPubLen - ASN_ECC_NIST_256_HEADER_LEN);
    ...
    status = nxFastScp_GetVerify_SE_Ecka_Public(
        se05xSession, sePubkey, &sePubkeyLen);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    ...
    sePubkeyLen = sePubkeyLen -
                  2; // Exclude last three bytes Key parameter tag len and value
                     /*Set the key in Fast scp Host context*/
    status = sss_host_key_store_set_key(pStatic_ctx->SeEcPubKey.keyStore,
        &pStatic_ctx->SeEcPubKey,
        pkSeEcka,
        sePubkeyLen,
        256,
        NULL,
    ...
    status = nxFastScp_InternalAuthenticate(se05xSession,
        pAuthFScp,
        hostEckaPub,
        hostEckaPubLen,
        drSE,
    ...
    status =
        nxFastScp_Calculate_Shared_secret(pAuthFScp, shsSecret, &shsSecretLen);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    ...
    status = nxFastScp_calculate_master_secret(
        pAuthFScp, drSE, drSELen, shsSecret, shsSecretLen);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);

    status = nxFastScp_HostLocal_CalculateSessionKeys(pAuthFScp);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    ...
    status = nxFastScp_Calculate_Initial_Mac_Chaining_Value(pAuthFScp);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    ...
    SE05x_AuthCtx_FastScp_t *pAuthFScp)
{
    ...
    nxScp03_setDerivationData(ddA,
        &ddALen,
        DATA_DERIVATION_INITIAL_MCV,
        DATA_DERIVATION_L_128BIT,
        DATA_DERIVATION_KDF_CTR,
        NULL,
        0);
    // Calculate the Initial MCV value
    status = nxScp03_Generate_SessionKey(
        &pStatic_ctx->masterSec, ddA, ddALen, iniMacChaining, &signatureLen);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    ...
    SE05x_AuthCtx_FastScp_t *pAuthFScp)
{
    ...
    status = nxScp03_Generate_SessionKey(
        &pStatic_ctx->masterSec, ddA, ddALen, sessionEncKey, &signatureLen);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    ...
    status = nxScp03_Generate_SessionKey(
        &pStatic_ctx->masterSec, ddA, ddALen, sessionMacKey, &signatureLen);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    ...
    status = nxScp03_Generate_SessionKey(
        &pStatic_ctx->masterSec, ddA, ddALen, sessionRmacKey, &signatureLen);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    ...
    size_t sharedSecretLen)
{
    sss_status_t status = kStatus_SSS_Fail;
    sss_digest_t md;
    uint8_t derivationInput[100];
    uint8_t masterSk[32];
    size_t masterSkLen = sizeof(masterSk);
    size_t derivationInputLen = 0;
    NXFastSCP03_StaticCtx_t *pStatic_ctx = pAuthFScp->pStatic_ctx;

    memcpy(derivationInput, sharedSecret, sharedSecretLen);
    derivationInputLen += sharedSecretLen;
    memcpy(&derivationInput[sharedSecretLen], rnd, rndLen);
...
sss_status_t nxFastScp_InternalAuthenticate(pSe05xSession_t se05xSession,
    SE05x_AuthCtx_FastScp_t *pAuthFScp,
    uint8_t *hostEckaPubKey,
    size_t hostEckaPubKeyLen,
    uint8_t *rndData,
    ...
    size_t *receiptLen)
{
    ...
    const tlvHeader_t hdr = {{CLA_GP_7816 | CLA_GP_SECURITY_BIT,
        INS_GP_INTERNAL_AUTHENTICATE,
        00,
        00}};
    ...
    size_t cntrlRefTemp_Len = 0 + 1 + 1 + APPLET_NAME_LEN + 1 /*TLV AID */ + 1 +
                              1 + sizeof(scpParms) /* TLV SCP Params */ + 1 +
                              1 + 1 /* TLV Keytype */ + 1 + 1 +
                              1 /* TLV KeyLEN */;

#if NX_LOG_ENABLE_SCP_DEBUG
    nLog("APDU", NX_LEVEL_DEBUG, "FastScp Internal authenticate []");
    ...
    tlvRet = TLVSET_u8buf("SCP parameters",
        &pCmdbuf,
        &cmdbufLen,
        kSE05x_GP_TAG_SCP_PARMS,
        scpParms,
        sizeof(scpParms));
    ENSURE_OR_GO_CLEANUP(tlvRet == 0);
    tlvRet = TLVSET_U8("Key Type",
        &pCmdbuf,
        &cmdbufLen,
        kSE05x_GP_TAG_KEY_TYPE,
        GPCS_KEY_TYPE_AES);
    ENSURE_OR_GO_CLEANUP(tlvRet == 0);
    tlvRet = TLVSET_U8("Key length",
        &pCmdbuf,
        &cmdbufLen,
        kSE05x_GP_TAG_KEY_LEN,
        GPCS_KEY_LEN_AES);
    ENSURE_OR_GO_CLEANUP(tlvRet == 0);
    ...
    *pCmdbuf++ = (uint8_t)hostEckaPubKeyLen;
    ...
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);
    status = sss_host_digest_one_go(
        &md, cmdbuf, cmdbufLen, md_host5F37, &md_host5F37Len);
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);
    sss_host_digest_context_free(&md);
    ...
    status = sss_host_asymmetric_context_init(&asym,
        ...
        kMode_SSS_Sign);
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);
    status = sss_host_asymmetric_sign_digest(
        &asym, md_host5F37, md_host5F37Len, sig_host5F37, &sig_host5F37Len);
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);
    ...
    *pCmdbuf++ = (uint8_t)sig_host5F37Len;
    ...
    retStatus = DoAPDUTxRx_s_Case3(
        se05xSession, &hdr, cmdbuf, cmdbufLen, rspbuf, &rspbufLen);
    if (retStatus == SM_OK) {
        size_t rspIndex = 0;
        tlvRet = tlvGet_u8buf(pRspbuf,
            ...
            kSE05x_GP_TAG_DR_SE,
            rndData,
            rndDataLen); /* Get the Random No */
        ENSURE_OR_GO_CLEANUP(tlvRet == 0);
        tlvRet = tlvGet_u8buf(pRspbuf,
            &rspIndex,
            rspbufLen,
            kSE05x_GP_TAG_RECEIPT,
            receipt,
            receiptLen); /* Get the Receipt */
        ENSURE_OR_GO_CLEANUP(tlvRet == 0);
...
sss_status_t nxFastScp_GetVerify_SE_Ecka_Public(
    pSe05xSession_t se05xSession, uint8_t *pSePubEcka, size_t *pSePubEckaLen)
{
    smStatus_t retStatus = SM_NOT_OK;
    sss_status_t status = kStatus_SSS_Fail;
    ...
    const tlvHeader_t hdr = {
        {CLA_GP_7816, INS_GP_GET_DATA, P1_GP_GET_DATA, P2_GP_GET_DATA}};
    size_t cntrlRefTemp_Len = 0 + 1 + 1 + 2; /*TLV Key */

    cmdbuf[i++] =
        kSE05x_TAG_GP_CONTRL_REF_PARM; // Tag Control reference template
    ...
    retStatus =
        DoAPDUTxRx_s_Case3(se05xSession, &hdr, cmdbuf, i, rspbuf, &rspbufLen);
    ENSURE_OR_GO_CLEANUP(retStatus == SM_OK);

    i = 0;
    /* Get the Public Key*/
    tlvRet = get_u8buf_2bTag(pRspbuf,
        &i,
        rspbufLen,
        (uint16_t)TAG_PK_SE_ECKA,
        pSePubEcka,
        pSePubEckaLen);
    if (0 != tlvRet) {
        goto cleanup;
...
sss_status_t nxFastScp_Calculate_Shared_secret(
    SE05x_AuthCtx_FastScp_t *pAuthFScp,
    uint8_t *sharedSecret,
    size_t *sharedSecretLen)
{
...
#if SSS_HAVE_MBEDTLS
    sss_mbedtls_object_t *hostShsSecretObj = (sss_mbedtls_object_t *)&shsSecret;
#elif SSS_HAVE_OPENSSL
    sss_openssl_object_t *hostShsSecretObj = (sss_openssl_object_t *)&shsSecret;
#else
#error "No Host Crypto Selected select either Mbedtls or openssl"
#endif

    status =
        sss_host_key_object_init(&shsSecret, pStatic_ctx->SeEcPubKey.keyStore);
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);
    status = sss_host_key_object_allocate_handle(&shsSecret,
        __LINE__,
    ...
    status =
        sss_host_derive_key_dh(&dervCtx, &pStatic_ctx->SeEcPubKey, &shsSecret);
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);

    memcpy(sharedSecret,
        hostShsSecretObj->contents,
        hostShsSecretObj->contents_size);
    *sharedSecretLen = hostShsSecretObj->contents_size;
