...
uint32_t se05x_sssKeyTypeLenToCurveId(
    sss_cipher_type_t cipherType, size_t keyBits)
{
...
smStatus_t Se05x_API_EC_CurveGetId(
    pSe05xSession_t session_ctx, uint32_t objectID, SE05x_ECCurve_t *pcurveId)
{
    ...
    sss_session_t *sess, SE05x_I2CM_cmd_t *p, uint8_t noOftags)
{
    ...
    for (iCnt = 0; iCnt < noOftags; iCnt++) {
        if (p[iCnt].type == kSE05x_I2CM_Configure) {
            ...
            tlvRet = TLVSET_u8buf_I2CM("I2CM Configure",
                &pCmdbuf,
                &bufferLen,
        ...
        //}
            ...
            tlvRet = TLVSET_u8buf_I2CM("I2CM Read",
                &pCmdbuf,
                &bufferLen,
    ...
    SendLen = bufferLen;
    retval = Se05x_API_I2CM_Send(
        se050session_id, pSendbuf, SendLen, rspbuffer, &rspbufferLen);
        ...
        for (iCnt = 0; iCnt < noOftags; iCnt++) {
            ...
            //}
                ...
                if (p[iCnt].cmd.rd.rdStatus == kSE05x_I2CM_Success) {
                    ...
                    uint16_t reportedRead =
                        (rspbuffer[rspPos + 1] << 8) + rspbuffer[rspPos + 2];
                    rspPos += 2;
                    if (reportedRead < p[iCnt].cmd.rd.readLength) {
                        LOG_W(
                            "kSE05x_I2CM_Read: Requested %d, Received %d byte",
