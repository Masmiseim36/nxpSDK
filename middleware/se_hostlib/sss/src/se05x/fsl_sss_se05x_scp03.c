...
sss_status_t nxScp03_AuthenticateChannel(
    pSe05xSession_t se05xSession, NXSCP03_AuthCtx_t *pAuthScp03)
{
    ...
    if ((pStatic_ctx->Enc.keyStore == NULL) ||
        (pStatic_ctx->Mac.keyStore == NULL) ||
        (pStatic_ctx->Dek.keyStore == NULL) ||
        (pDyn_ctx->Enc.keyStore == NULL) || (pDyn_ctx->Mac.keyStore == NULL) ||
        (pDyn_ctx->Rmac.keyStore == NULL)) {
        LOG_E("nxScp03_GP_InitializeUpdate fails Invalid objects sent %04X",
            status);
        return status;
    }
    ...
    status =
        sss_host_rng_context_init(&rngctx, pStatic_ctx->Enc.keyStore->session);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);

    status = sss_host_rng_get_random(
        &rngctx, hostChallenge, SCP_GP_HOST_CHALLENGE_LEN);
    LOG_MAU8_D(
        " Output: hostChallenge", hostChallenge, SCP_GP_HOST_CHALLENGE_LEN);
    ...
    status = nxScp03_HostLocal_CalculateSessionKeys(
        pAuthScp03, hostChallenge, cardChallenge);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);

    status = nxScp03_HostLocal_VerifyCardCryptogram(
        &pDyn_ctx->Mac, hostChallenge, cardChallenge, cardCryptoGram);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    ...
    status = nxScp03_HostLocal_CalculateHostCryptogram(
        &pDyn_ctx->Mac, hostChallenge, cardChallenge, hostCryptogram);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    LOG_AU8_D(hostCryptogram, SCP_GP_IU_CARD_CRYPTOGRAM_LEN);

    status = nxScp03_GP_ExternalAuthenticate(
        se05xSession, &pDyn_ctx->Mac, pDyn_ctx->MCV, hostCryptogram);
    if (status != kStatus_SSS_Success) {
        ...
        pDyn_ctx->SecurityLevel = (C_MAC | R_MAC);
#else
    ...
    uint8_t *hostCryptogram)
{
    ...
    sss_status_t status = kStatus_SSS_Fail;
#if SSS_HAVE_SE050_M_SR1
    tlvHeader_t hdr = {{CLA_GP_7816 | CLA_GP_SECURITY_BIT,
        INS_SE050M_EXTERNAL_AUTHENTICATE,
        C_MAC | R_MAC,
        0x00}};
#elif SSS_HAVE_SE050_M_SR2
    tlvHeader_t hdr = {{CLA_GP_7816 | CLA_GP_SECURITY_BIT,
        INS_SE050M_EXTERNAL_AUTHENTICATE,
        SECLVL_CDEC_RENC_CMAC_RMAC,
        0x00}};
#else
    tlvHeader_t hdr = {{CLA_GP_7816 | CLA_GP_SECURITY_BIT,
        INS_GP_EXTERNAL_AUTHENTICATE,
        SECLVL_CDEC_RENC_CMAC_RMAC,
        0x00}};
#endif
...
#if SSS_HAVE_SE050_M_SR1
    txBuf[1] = INS_SE050M_EXTERNAL_AUTHENTICATE; //Set INS Byte
    txBuf[2] = C_MAC | R_MAC;                    //Set Security Level
#elif SSS_HAVE_SE050_M_SR2
    txBuf[1] = INS_SE050M_EXTERNAL_AUTHENTICATE; //Set INS Byte
    ...
    txBuf[3] = 0x00;
    txBuf[4] =
        0x10; // The Lc value is set as-if the MAC has already been appended (SCP03 spec p16. Fig.6-1)
    ...
    */
    ...
    memcpy(&txBuf[5 + SCP_GP_IU_CARD_CRYPTOGRAM_LEN],
        macToAdd,
        SCP_GP_IU_CARD_CRYPTOGRAM_LEN);

    LOG_D("Sending GP External Authenticate Command !!!");
    st = DoAPDUTxRx_s_Case3(
        se05xSession, &hdr, &txBuf[5], 16, response, &responseLen);
    if (st != SM_OK) {
        ...
        if (responseLen != 2) {
            st = ERR_WRONG_RESPONSE;
            status = kStatus_SSS_Fail;
        }
        else {
            st = (response[0] << 8) + response[1];
        }
    }
    if (st == SM_OK) {
        status = kStatus_SSS_Success;
...
sss_status_t nxScp03_HostLocal_CalculateHostCryptogram(sss_object_t *keyObj,
    uint8_t *hostChallenge,
    uint8_t *cardChallenge,
    uint8_t *hostCryptogram)
{
    ...
    LOG_D("FN: %s", __FUNCTION__);
    LOG_MAU8_D(
        " Input:hostChallenge", hostChallenge, SCP_GP_HOST_CHALLENGE_LEN);
    LOG_MAU8_D(
        " Input:cardChallenge", cardChallenge, SCP_GP_CARD_CHALLENGE_LEN);

    memcpy(context, hostChallenge, SCP_GP_HOST_CHALLENGE_LEN);
    memcpy(&context[SCP_GP_HOST_CHALLENGE_LEN],
        cardChallenge,
    ...
    nxScp03_setDerivationData(ddA,
        &ddALen,
        DATA_HOST_CRYPTOGRAM,
        DATA_DERIVATION_L_64BIT,
        DATA_DERIVATION_KDF_CTR,
        context,
        contextLen);

    status = nxScp03_Generate_SessionKey(
        keyObj, ddA, ddALen, hostCryptogramFullLength, &signatureLen);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);

    LOG_MAU8_D(
        " Output:hostCryptogram", hostCryptogramFullLength, AES_KEY_LEN_nBYTE);

    // Chop of the tail of the hostCryptogramFullLength
    memcpy(hostCryptogram,
...
sss_status_t nxScp03_HostLocal_VerifyCardCryptogram(sss_object_t *keyObj,
    uint8_t *hostChallenge,
    uint8_t *cardChallenge,
    uint8_t *cardCryptogram)
{
    ...
    status = nxScp03_Generate_SessionKey(
        keyObj, ddA, ddALen, cardCryptogramFullLength, &signatureLen);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);

    LOG_MAU8_D(
        " Output:cardCryptogram", cardCryptogramFullLength, AES_KEY_LEN_nBYTE);

    // Verify whether the 8 left most byte of cardCryptogramFullLength match cardCryptogram
    if (memcmp(cardCryptogramFullLength,
            cardCryptogram,
            SCP_GP_IU_CARD_CRYPTOGRAM_LEN) != 0)
        status = kStatus_SSS_Fail;
exit:
    return status;
}
    ...
    uint8_t *cardChallenge)
{
    ...
    status = nxScp03_Generate_SessionKey(
        &pStatic_ctx->Enc, ddA, ddALen, sessionEncKey, &signatureLen);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    ...
    status = nxScp03_Generate_SessionKey(
        &pStatic_ctx->Mac, ddA, ddALen, sessionMacKey, &signatureLen);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
    ...
    status = nxScp03_Generate_SessionKey(
        &pStatic_ctx->Mac, ddA, ddALen, sessionRmacKey, &signatureLen);
    ENSURE_OR_GO_EXIT(status == kStatus_SSS_Success);
...
sss_status_t nxScp03_Generate_SessionKey(sss_object_t *keyObj,
    uint8_t *inData,
    uint32_t inDataLen,
    uint8_t *outSignature,
    uint32_t *outSignatureLen)
{
    sss_mac_t macCtx;
    sss_algorithm_t algorithm = kAlgorithm_SSS_CMAC_AES;
    sss_mode_t mode = kMode_SSS_Mac;
    ...
    status =
        sss_host_mac_one_go(&macCtx, inData, inDataLen, outSignature, &sigLen);
    ...
    uint8_t keyVerNo)
{
    ...
    uint16_t parsePos = 0;
    uint16_t sw = 0;
    uint32_t iuResponseLenSmall = SCP_GP_IU_KEY_DIV_DATA_LEN +
                                  SCP_GP_IU_KEY_INFO_LEN +
                                  SCP_GP_CARD_CHALLENGE_LEN +
                                  SCP_GP_IU_CARD_CRYPTOGRAM_LEN + SCP_GP_SW_LEN;
    uint32_t iuResponseLenBig =
        SCP_GP_IU_KEY_DIV_DATA_LEN + SCP_GP_IU_KEY_INFO_LEN +
        SCP_GP_CARD_CHALLENGE_LEN + SCP_GP_IU_CARD_CRYPTOGRAM_LEN +
        SCP_GP_IU_SEQ_COUNTER_LEN + SCP_GP_SW_LEN;
    sss_status_t status = kStatus_SSS_Fail;
    /* Default Key version no for applet scp is 0x00*/
    uint8_t keyVersion = 0x00;
    if (se05xSession->authType == kSSS_AuthType_SCP03) {
        /* Default Key version no for platform scp is 0x30*/
        keyVersion = keyVerNo;
    ...
    tlvHeader_t hdr = {
        {CLA_GP_7816, INS_SE050M_INITIALIZE_UPDATE, keyVersion, 0x00}};
#else
    tlvHeader_t hdr = {
        {CLA_GP_7816, INS_GP_INITIALIZE_UPDATE, keyVersion, 0x00}};
#endif
    uint8_t cmdBuf[60];
    ...
    st = DoAPDUTxRx_s_Case3(
        se05xSession, &hdr, cmdBuf, hostChallengeLen, response, &responseLen);
    if (st != SM_OK) {
    ...
    if ((responseLen != iuResponseLenSmall) &&
        (responseLen != iuResponseLenBig)) {
        // Note: A response of length 2 (a proper SW) is also collapsed into return code SCP_FAIL
        LOG_E("GP_InitializeUpdate Unexpected amount of data returned: %04X",
            responseLen);
        return status;
    }
    ...
    memcpy(
        cardCryptoGram, &(response[parsePos]), SCP_GP_IU_CARD_CRYPTOGRAM_LEN);
    ...
    if (sw == SM_OK) {
        ...
        LOG_MAU8_D(
            " Output: cardCryptoGram", cardCryptoGram, *pCardCryptoGramLen);
        status = kStatus_SSS_Success;
    }
cleanup:
    return status;
