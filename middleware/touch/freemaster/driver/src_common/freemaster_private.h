...
#if defined(FMSTR_PLATFORM_QN908x)
    #if FMSTR_PLATFORM_QN908x
    #include "freemaster_QN908x.h"
    #define FMSTR_PLATFORM QN908X
    #else
    #undef FMSTR_PLATFORM_QN908x
    #endif
#endif
...
/* MQX IO communication interface warnings and errors */
    ...
    /* by default, the new USB stack from MCUXpresso SDK is used, set to 1 for to use legacy NXP 'Medical USB' stack */
    ...
    /* USB interface selection (MCUXpresso SDK USB stack only) */
        ...
        /* pick one of USB_CONTROLLER_ identifiers as defined by MCUXpresso SDK USB driver */
