...
static void _nt_control_rotary_calc_dynamic(struct nt_control_data *control,
                                   uint32_t position);
/* Calculates position */
...
static void _nt_control_rotary_adjust_direction(struct nt_control_data *control,
                                       uint32_t position);
...
static int32_t _nt_control_rotary_process(struct nt_control_data *control)
{
    ...
    if (!elec_state) {
        ...
        if (_nt_control_get_flag(control, NT_ROTARY_TOUCH_FLAG)) {
            ...
            _nt_control_clear_flag(control, NT_ROTARY_TOUCH_FLAG | NT_ROTARY_MOVEMENT_FLAG | NT_ROTARY_INVALID_POSITION_FLAG);
            _nt_control_rotary_invoke_callback(control, NT_ROTARY_ALL_RELEASE,
    ...
    if(touch_count <= 2)
    {
      ...
      if (!_nt_control_get_flag(control, NT_ROTARY_TOUCH_FLAG)) {
          _nt_control_rotary_invoke_callback(control, NT_ROTARY_INITIAL_TOUCH,
                                    (uint32_t)ram->position);
      }
