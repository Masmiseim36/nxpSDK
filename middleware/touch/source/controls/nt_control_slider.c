...
static void _nt_control_slider_invoke_callback(const struct nt_control_data *control,
                                               ...
                                               uint32_t position);
/* Invokes a callback depending on the event */
static void _nt_control_slider_calc_dynamic(struct nt_control_data *control,
                                   uint32_t position);
/* Calculates position */
...
static int32_t _nt_control_slider_process(struct nt_control_data *control)
{
    ...
    if (!elec_state) {
        ...
        if (_nt_control_get_flag(control, NT_SLIDER_TOUCH_FLAG)) {
            ...
            _nt_control_clear_flag(control, NT_SLIDER_MOVEMENT_FLAG);
            _nt_control_slider_invoke_callback(control, NT_SLIDER_ALL_RELEASE,
    ...
    if (!_nt_control_get_flag(control, NT_SLIDER_TOUCH_FLAG)) {
        _nt_control_slider_invoke_callback(control, NT_SLIDER_INITIAL_TOUCH,
                                   (uint32_t)ram->position);
    }
