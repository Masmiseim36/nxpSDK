...
struct nt_control_data *_nt_control_init(const struct nt_control *control)
{
    ...
    if (_nt_freemaster_add_variable(control->interface->name, "nt_control_interface", (void *)control->interface,
                                    sizeof(struct nt_control_interface)) != NT_SUCCESS)
    {
        return NULL;
    }
...
uint32_t _nt_control_get_electrodes_digital_state(struct nt_control_data *control)
{
    ...
    while (elec_counter--)
    {
        ...
        if (_nt_electrode_get_flag(control->electrodes[elec_counter],
...
/* Internal function */
void _nt_control_set_flag_all_elec(struct nt_control_data *control, uint32_t flag)
{
    NT_ASSERT(control != NULL);
    uint32_t elec_counter = control->electrodes_size;
