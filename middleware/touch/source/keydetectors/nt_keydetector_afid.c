...
static void _nt_keydetector_afid_measure(struct nt_electrode_data *electrode, uint32_t signal)
{
    ...
    if (ram->filter_state == NT_FILTER_STATE_INIT)
    {
        ...
        _nt_filter_moving_average_init(&rom->base_avrg, &ram->base_avrg, signal);
    ...
    if (release_reset_counter > 0U)
    {
        ...
        {
            if (!(_nt_electrode_get_time_offset_period(electrode, rom->asc.touch_threshold_fall_rate)))
            {
        ...
        /* Update the baseline */
        electrode->baseline = _nt_filter_moving_average_process(&rom->base_avrg, &ram->base_avrg, signal);
    }
}
