...
static void _nt_keydetector_safa_measure(struct nt_electrode_data *electrode, uint32_t signal)
{
    ...
    {
        _nt_reset_keydetector_safa_reset(electrode, signal,
...
static void _nt_keydetector_safa_process(struct nt_electrode_data *electrode)
{
    ...
    int32_t delta  = (int32_t)_nt_filter_abs(electrode->baseline - signal);
    ...
    switch (_nt_electrode_get_last_status(electrode))
    {
        ...
        case (int32_t)NT_ELECTRODE_STATE_TOUCH:
            if ((delta < ((int32_t)_nt_filter_abs(((int16_t)ram->predicted_signal - (int16_t)electrode->baseline) * 4U / 5U)) &&
        ...
        case (int32_t)NT_ELECTRODE_STATE_RELEASE:
            if (((delta > (_nt_filter_abs(ram->predicted_signal - electrode->baseline) >> 2))) &&
                (ram->deadband_cnt == 0) &&
                (!(bool)_nt_filter_is_deadrange_u(signal, electrode->baseline, (uint16_t)(ram->noise * rom->signal_to_noise_ratio))))
            {
                ...
                if (ram->entry_event_cnt > (int32_t)(rom->entry_event_cnt)) /* debouncing */
                {
                    ...
                    _nt_filter_moving_average_init(
                        ...
                        _nt_filter_deadrange_u(signal, electrode->baseline, (uint16_t)(ram->noise * rom->signal_to_noise_ratio)));
                    _nt_electrode_set_status(electrode, (int32_t) NT_ELECTRODE_STATE_TOUCH);
                }
            }
                                              ...
                                              uint16_t signal)
{
    ...
    if (!(bool)_nt_electrode_get_flag(electrode,(int32_t) NT_ELECTRODE_LOCK_BASELINE_FLAG))
    {
        if (_nt_filter_abs((int16_t)(electrode->baseline - signal)) < (ram->noise * rom->signal_to_noise_ratio))
        {
            ram->noise = _nt_filter_moving_average_process(
                &ram->noise_avrg_init, &ram->noise_avrg,
                _nt_filter_limit_u((int32_t)_nt_filter_abs((int16_t)(electrode->baseline - signal)), rom->min_noise_limit,
                                   (uint16_t)(rom->min_noise_limit * rom->signal_to_noise_ratio *
                                    2U))); /* max noise level step adaptation factor */
    ...
    {
        ram->predicted_signal = _nt_filter_deadrange_u((uint16_t)
