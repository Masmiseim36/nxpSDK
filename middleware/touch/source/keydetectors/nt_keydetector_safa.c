...
static void _nt_keydetector_safa_process(struct nt_electrode_data *electrode)
{
    ...
    switch (_nt_electrode_get_last_status(electrode))
    {
        ...
        case NT_ELECTRODE_STATE_TOUCH:
            if ((delta < (_nt_filter_abs(ram->predicted_signal - electrode->baseline) * 4 / 5)) &&
                ...
                (!_nt_filter_is_deadrange_u(signal, electrode->baseline, (ram->noise * rom->signal_to_noise_ratio))))
            {
                ...
                if (ram->entry_event_cnt > rom->entry_event_cnt) /* debouncing */
                {
                    ...
                    _nt_filter_moving_average_init(
                        ...
                        _nt_filter_deadrange_u(signal, electrode->baseline, (ram->noise * rom->signal_to_noise_ratio)));
                    _nt_electrode_set_status(electrode, NT_ELECTRODE_STATE_TOUCH);
                }
            }
                                              ...
                                              uint16_t signal)
{
    ...
    if (!_nt_electrode_get_flag(electrode, NT_ELECTRODE_LOCK_BASELINE_FLAG))
    {
        if (_nt_filter_abs(electrode->baseline - signal) < (ram->noise * rom->signal_to_noise_ratio))
        {
            ram->noise = _nt_filter_moving_average_process(
                ...
                _nt_filter_limit_u(_nt_filter_abs(electrode->baseline - signal), rom->min_noise_limit,
                                   (rom->min_noise_limit * rom->signal_to_noise_ratio *
                                    2))); /* max noise level step adaptation factor */

            ram->f_noise = _nt_filter_moving_average_process(
                &ram->f_noise_avrg_init, &ram->f_noise_avrg,
                _nt_filter_limit_u(_nt_filter_abs(electrode->baseline - signal), rom->min_noise_limit,
                                   (rom->min_noise_limit * rom->signal_to_noise_ratio * 2))); /* Fast Noise */
    ...
    {
        ram->predicted_signal = _nt_filter_deadrange_u(
