...
static void _nt_keydetector_usafa_measure(struct nt_electrode_data *electrode,
                      uint32_t signal);
...
static int32_t _nt_keydetector_usafa_init(struct nt_electrode_data *electrode)
{
  ...
  electrode->keydetector_data.usafa = (struct nt_keydetector_usafa_data *) _nt_mem_alloc(sizeof(struct nt_keydetector_usafa_data));
  ...
  electrode->keydetector_data.usafa->f_noise_avrg_init.n2_order = electrode->keydetector_data.usafa->noise_avrg_init.n2_order - 4;
...
static void _nt_keydetector_usafa_measure(struct nt_electrode_data *electrode, uint32_t signal)
{
    ...
    if (ram->filter_state == NT_FILTER_STATE_INIT) {
        _nt_reset_keydetector_usafa_reset(electrode, signal, _nt_electrode_get_flag(electrode, NT_ELECTRODE_AFTER_INIT_TOUCH_FLAG));
    ...
    if(rom->dc_track_enabled)
    {
      ...
      switch (_nt_electrode_get_last_status(electrode))
      {
        case NT_ELECTRODE_STATE_INIT:
            ...
            if (_nt_electrode_get_time_offset(electrode) >= system->rom->init_time)
            {
              ...
              { 
                ...
                _nt_reset_keydetector_usafa_reset(electrode, signal, _nt_electrode_get_flag(electrode, NT_ELECTRODE_AFTER_INIT_TOUCH_FLAG)); /* Initial signal warm-up (settling) Completed */
                
              }              
            }
          break;
        case NT_ELECTRODE_STATE_TOUCH:
            if((delta < (_nt_filter_pos(ram->predicted_signal - electrode->baseline) * 4/5)) && (ram->deadband_cnt == 0)) /* 80% release thresh */             
            {
              ram->entry_event_cnt = 0;
              ram->deadband_cnt = rom->deadband_cnt;
              _nt_electrode_set_status(electrode, NT_ELECTRODE_STATE_RELEASE); 
        ...
        case NT_ELECTRODE_STATE_RELEASE:
              if(((delta > (_nt_filter_pos(ram->predicted_signal - electrode->baseline) / 4))) &&
                 (ram->deadband_cnt == 0) &&
...
static void _nt_keydetector_usafa_lock_baseline(struct nt_electrode_data *electrode, const struct nt_keydetector_usafa *rom, struct nt_keydetector_usafa_data *ram, uint16_t signal)
{
...
static void _nt_keydetector_usafa_unlock_baseline(struct nt_electrode_data *electrode, const struct nt_keydetector_usafa *rom, struct nt_keydetector_usafa_data *ram, uint16_t signal)
{
  if(_nt_electrode_get_flag(electrode, NT_ELECTRODE_AFTER_INIT_TOUCH_FLAG))
  {
...
static void _nt_keydetector_usafa_signal_track(struct nt_electrode_data *electrode, const struct nt_keydetector_usafa *rom, struct nt_keydetector_usafa_data *ram, uint16_t signal)
{
  ...
  if(!_nt_electrode_get_flag(electrode, NT_ELECTRODE_LOCK_BASELINE_FLAG))
  {
    ...
    {            
        ...
        ram->noise = _nt_filter_moving_average_process(&ram->noise_avrg_init, 
                                               &ram->noise_avrg,
                                              _nt_filter_limit_u(_nt_filter_pos(signal - electrode->baseline), rom->min_noise_limit, (rom->min_noise_limit * rom->signal_to_noise_ratio * 4))); 
  ...
  {
    ram->predicted_signal = _nt_filter_deadrange_p(_nt_filter_moving_average_process(&ram->predicted_signal_avrg_init, 
                                               &ram->predicted_signal_avrg,
