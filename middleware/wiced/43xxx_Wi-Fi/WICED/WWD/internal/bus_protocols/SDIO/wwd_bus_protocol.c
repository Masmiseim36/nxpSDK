...
#define WICED_PLATFORM_MASKS_BUS_IRQ 1
/******************************************************
 *             Structures
 ******************************************************/
...
wwd_result_t wwd_bus_init( void )
{
    ...
    result = host_platform_sdio_enumerate();
    VERIFY_RESULT(result);
    WPRINT_WWD_DEBUG(("WWD : Enumeration Complete\n"));
    ...
    VERIFY_RESULT( wwd_bus_read_register_value (BUS_FUNCTION, SDIOD_CCCR_CAPABLITIES, (uint8_t) 1, &byte_data) );
    WPRINT_WWD_DEBUG(("Reading card capability : %x", byte_data));
...
{
	//printf("wwd_bus_write_backplane_value address = %u, register_length=%u value = %u\n",(unsigned int)address,(unsigned int)register_length,(unsigned int)value);
...
static wwd_result_t wwd_bus_sdio_transfer( wwd_bus_transfer_direction_t direction, wwd_bus_function_t function, uint32_t address, uint16_t data_size, /*@in@*/ /*@out@*/ uint8_t* data, sdio_response_needed_t response_expected )
{
    /* Note: this function had broken retry logic (never retried), which has been removed.
       ...
       */
	//WPRINT_WWD_DEBUG(("wwd_bus_sdio_transfer: direction=%d function=%d response_expected=%d data_size=%d\n ", direction,function,response_expected,data_size ));
    ...
    else
    {
        ...
        return wwd_bus_sdio_cmd53( direction, function, ( data_size >= (uint16_t) SDIO_64B_BLOCK ) ? SDIO_BLOCK_MODE : SDIO_BYTE_MODE, address, data_size, data, response_expected, NULL );
    }
}
...
static wwd_result_t wwd_bus_sdio_cmd52( wwd_bus_transfer_direction_t direction, wwd_bus_function_t function, uint32_t address, uint8_t value, sdio_response_needed_t response_expected, uint8_t* response )
{
    ...
    result = host_platform_sdio_transfer( direction, SDIO_CMD_52, SDIO_BYTE_MODE, SDIO_1B_BLOCK, arg.value, 0, 0, response_expected,kCARD_ResponseTypeR5, 0, &sdio_response );
...
static wwd_result_t wwd_bus_sdio_cmd53( wwd_bus_transfer_direction_t direction, wwd_bus_function_t function, sdio_transfer_mode_t mode, uint32_t address, uint16_t data_size, /*@in@*/ /*@out@*/ uint8_t* data, sdio_response_needed_t response_expected, /*@null@*/ uint32_t* response )
{
    ...
    if ( mode == SDIO_BYTE_MODE )
    {
        ...
        result = host_platform_sdio_transfer( direction, SDIO_CMD_53, mode, SDIO_64B_BLOCK, arg.value, (uint32_t*) data, data_size, response_expected,kCARD_ResponseTypeR5,0, response );
        if( result != WWD_SUCCESS )
        {
            goto done;
        }
    ...
    {
//#define WICED_PLATFORM_DOESNT_USE_TEMP_DMA_BUFFER
        ...
        result = host_platform_sdio_transfer( direction, SDIO_CMD_53, SDIO_BLOCK_MODE, SDIO_64B_BLOCK, arg.value, (uint32_t*) data, sent_data_size, response_expected,kCARD_ResponseTypeR5, 0,response );
        if( result != WWD_SUCCESS )
        {
            goto done;
        }
        ...
        if ( data_size > sent_data_size ) /* Send the remaining bytes using CMD53 Byte mode */
        {
            ...
            result = host_platform_sdio_transfer( direction, SDIO_CMD_53, SDIO_BYTE_MODE, SDIO_1B_BLOCK, arg.value, (uint32_t*) ( data + sent_data_size ), (uint16_t)( data_size - sent_data_size ), response_expected,kCARD_ResponseTypeR5,0, response );
            if( result != WWD_SUCCESS )
            {
                goto done;
            }
