...
{
#if defined (RWL_SERIAL) || (RWL_SOCKET)
	...
	if (remote_type == REMOTE_SOCKET) {
		...
		/* Send CDC header first */
		        if ((ret = rwl_write_eth_port(wl, (char *)rem_ptr,
		            REMOTE_SIZE, &numwritten)) == -1) {
		            DPRINT_ERR(ERR, "CDC_Tx: Data: Write failed \n");
		            return (FAIL);
		        }
		        ...
		        /* Send data second */
		        if ((ret = rwl_write_eth_port(wl, (char*)buf,
		            data_len, &numwritten)) == -1) {
		            DPRINT_ERR(ERR, "CDC_Tx: Data: Write failed \n");
		            return (FAIL);
		        }
...
remote_CDC_rx_hdr(void *remote, int debug)
{
	...
	switch (remote_type)	{
		...
		case REMOTE_SOCKET:
#ifdef TARGET_wiced
		    ret = rwl_read_eth_port(remote, (char *)rem_ptr, sizeof(rem_ioctl_t),
                    &numread);
		    if ( ret < 0) {
		                    DPRINT_ERR(ERR, "remote_CDC_rx_hdr: Header Read failed \n");
		                    return (NULL);
...
remote_CDC_rx(void *wl, rem_ioctl_t *rem_ptr, uchar *readbuf, uint buflen, int debug)
{
	...
	if (remote_type == REMOTE_SOCKET) {
#ifdef TARGET_wiced
	    if (((ret = rwl_read_eth_port(wl, (char*)readbuf, rem_ptr->data_len,
	         &numread)) < 0)) {
	         DPRINT_ERR(ERR, "remote_CDC_rx_hdr: Data Receivefailed \n");
	         return (FAIL);
	     }
...
#if (defined (RWL_SOCKET) && !defined (TARGET_wiced))
int
...
#endif /*(defined (RWL_SOCKET) && !defined (TARGET_wiced)) */
void
