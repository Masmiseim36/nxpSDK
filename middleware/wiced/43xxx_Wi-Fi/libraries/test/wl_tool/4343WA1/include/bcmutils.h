 ...
 *
 * $Id: bcmutils.h 543267 2015-03-24 04:53:30Z sungjoon $
 */
...
#define bcm_strncpy_s(dst, tot_buf_size, src, bytes2copy)    strncpy((dst), (src), (bytes2copy))
#define bcm_snprintf_s snprintf

/*
 * bcm_strcpy_s: Use bcm_strncpy_s as a safer option.
 * bcm_strcat_s: Use bcm_strncat_s as a safer option
 * strncat : use snprintf or strncpy after adjusting the dest starting offset
 */
...
typedef struct pktpool {
	...
	pktpool_cbinfo_t cbs_reserv[PKTPOOL_CB_MAX];
...
#define UNSIGNEDDEC(x) do { if (x > 0) x--; } while (0)
