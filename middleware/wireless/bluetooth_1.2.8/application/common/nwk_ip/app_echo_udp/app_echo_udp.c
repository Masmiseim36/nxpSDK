...
* file
*
* rief 	This is a public source file for the echo protocol application.
*
...
)
{
    ...
    if (pmEchoUdpMsgQueue == NULL)
    {
        ...
        if (mEchoUdpSrvSockFd == -1)
        {
			...
			((sockaddrIn6_t*)&portAddr)->sin6_scope_id = gIpIfUndef_c;
...
)
{
    ...
    tempTimestamp /= 1000;
#if SHELL_ENABLED
    ntop(AF_INET6, (ipAddr_t*)&pSessionPacket->remAddr.ss_addr, ipAddr, INET6_ADDRSTRLEN);
...
static void ECHO_TimerCallback(void *param)
{
    (void)NWKU_SendMsg((nwkMsgHandler)ECHO_HandleTimerCallback, NULL, pmEchoUdpMsgQueue);
}
