...
static tmrTimerID_t mTransferStatusCheckTmrId = gTmrInvalidTimerID_c;
...
bool_t OtapClient_Config(void)
{
    ...
    (void)GattServer_RegisterHandlesForWriteNotifications (NumberOfElements(otapWriteNotifHandles),
    ...
    /* Start OTA service */
    (void)OtapCS_Start(&otapServiceConfig);

    /* Allocate timer that checks periodically the transfer status */
    mTransferStatusCheckTmrId = TMR_AllocateTimer();
    if (mTransferStatusCheckTmrId == gTmrInvalidTimerID_c)
    {
        retStatus = FALSE;
    }
                                    ...
                                    uint8_t*    pValue)
{
    ...
    if (handle == (uint16_t)value_otap_control_point)
    {
        ...
        case gOtapCmdIdStopImageTransfer_c:
            ...
            (void)TMR_StopTimer(mTransferStatusCheckTmrId);
            /* Cancel OTA transfer */
...
********************************************************************************** */
void OtapClient_HandlePsmConnectionComplete (l2caLeCbConnectionComplete_t *pConnComplete)
{
...
void OtapClient_HandleDisconnectionEvent (deviceId_t deviceId)
{
    ...
    (void)TMR_StartSingleShotTimer(mTransferStatusCheckTmrId, gOTAPCancelIntervalMs, OtapClient_CancelOTA, NULL);
}
...
static void OtapClient_HandleDataChunk (deviceId_t deviceId, uint16_t length, uint8_t* pData)
{
    ...
    otapCmdImgChunkCoc_t* pDataChunk = (otapCmdImgChunkCoc_t*)(&(otapCommandVars.otapCommandTemp)->cmd); //use the CoC Data Chunk type but observe the length
    ...
    if (deviceId == otapClientData.peerOtapServer)
    {
           ...
           )
        {
                ...
                (pDataChunk->seqNumber < otapClientData.totalBlockChunks))
            {
                ...
                if (((dataLen == otapClientData.chunkSize) && ((pDataChunk->seqNumber < (otapClientData.totalBlockChunks - 1U)) || (otapClientData.totalBlockSize % otapClientData.chunkSize == 0U))) ||
                    ((dataLen < otapClientData.chunkSize) && (pDataChunk->seqNumber == (otapClientData.totalBlockChunks - 1U)) && (dataLen == otapClientData.totalBlockSize % otapClientData.chunkSize))
                   )
                {
    ...
    if (otapStatus == gOtapStatusSuccess_c)
    {
        ...
        while (dataLen > 0U)
        {
            ...
            if (otapClientData.currentPos < sizeof(bleOtaImageFileHeader_t))
            {
                ...
                {
                    uint16_t residualHeaderLen = (uint16_t)(sizeof(bleOtaImageFileHeader_t) - otapClientData.currentPos);
            ...
            else
            {
                ...
                if (currentImgElemRcvdLen < sizeof(subElementHeader_t))
                {
                    ...
                    {
                        uint16_t residualSubElemHdrLen = (uint16_t)(sizeof(subElementHeader_t) - currentImgElemRcvdLen);
                        ...
                        if (subElemHdr.tagId != (uint16_t)gBleOtaSubElemTagIdImageFileCrc_c)
                        {
                    ...
                    case (uint16_t)gBleOtaSubElemTagIdUpgradeImage_c:
                        ...
                        if (currentImgElemRcvdLen == sizeof(subElementHeader_t))
                        {
                            ...
                            if (gOtaSucess_c != otaStartImageResult)
                            {
                                ...
                                {
                                    /* The image is too large and does not fit into the non-volatile storage. */
                                    otapStatus = gOtapStatusImageSizeTooLarge_c;
                                }
                                else
                                {
                                    /* There was an error when trying to initiate the image storage in non-volatile memory */
                        ...
                        otapClientData.imgComputedCrc = OTA_CrcCompute (pData,
                                                                        ...
                                                                        otapClientData.imgComputedCrc);
                        if (gOtaSucess_c != OTA_PushImageChunk (pData, (uint16_t)elementChunkLength, NULL, NULL))
                        {
                            otapStatus = gOtapStatusImageStorageError_c;
                            otapClientData.currentPos = 0;
                            OTA_CancelImage();
...
static bleResult_t OtapClient_SendImageRequest(void)
{
    ...
    if (otapClientData.currentPos < otapClientData.imgSize)
    {
        ...
        if (bAbortOTA == FALSE)
        {
            ...
            otapCommand.cmdId = (uint8_t)gOtapCmdIdImageBlockRequest_c;
            ...
            if (gBleSuccess_c == bleResult)
            {
                ...
                (void)TMR_StartIntervalTimer(mTransferStatusCheckTmrId, gOTAPTransferCheckIntervalMs, OtapClient_TimerCb, NULL);
