 ...
 * For further details see the silicon datasheet.
 ********************************************************************************** */
bleResult_t Controller_BumpPaPowerUp(uint8_t val);
...
/*! *********************************************************************************
 * \brief  Callback function to be implemented in the BLE application. This function 
 * is called when LL fails to allocate memory. The usual action is to reset CPU when
 * this function is called.
 *
 * \return void
 ********************************************************************************** */
void BleApp_LlMemoryAllocationError(void);

/*! *********************************************************************************
 * \brief  Set the local sleep clock accuracy (SCA). This needs to be called before 
 *         the connection setup.
 *
 * \param[in]  sca  SCA value 0 0- 7 corresponding to 500ppm - 20ppm.
 *
 * \return gBleSuccess_c or gBleInvalidParameter_c
 ********************************************************************************** */
bleResult_t Controller_SetLocalSca(uint8_t sca);

/*! *********************************************************************************
 * \brief  Read the supervision timeout values
 *
 * \param[in]  connHandle         Connection handle
 * \param[in]  pSupTimeout625us   Pointer to save the configured supervison timeout value 
 *                                connSupervisionTimeout*16 (unit 625us)
 * \param[in]  pRemaining625us     Pointer to save the remaining time to reach the supervison timeout (unit 625us)
 *
 * \return gBleSuccess_c or gBleInvalidParameter_c
 ********************************************************************************** */
bleResult_t Controller_GetSupervisionTimeout(uint16_t connHandle, uint16_t *pSupTimeout625us, uint16_t *pRemaining625us);

/*! *********************************************************************************
 * \brief   Disconnect immediately the connection without sending the control PDU 
 *          TERMINATE_IND.
 *
 * \param[in]  connHandle       Connection handle
 ...
 * \return gBleSuccess_c or gBleInvalidParameter_c
 ********************************************************************************** */
bleResult_t Controller_Disconnect(uint16_t connHandle, uint8_t reason);

/*! *********************************************************************************
 * \brief  Install the window widening callback. This applies to all connections.
 *         The provided callback function is called each time the window widening value changes.
 *         The callback is called in the LL task so the procession spent in the function must be 
 *         limited to not interfere with the LL activities.
 *         
 * \param[in]  pCallback     Callback function pointer which is called with the following parameters:  
 *                            - connHandle         Connection handle of the connection
 *                            - windowWidening1us  Current window widening value (unit 1 us)
 *                                                 The actual radio reception window increase is 2 times
 *                            - supTimeout625us    Configured supervison timeout value (unit 625us)
 *                            - remaining625us     Remaining time before the supervison timeout (unit 625us)
 *
 *                           NULL value will disable the callback.
 *
 * \return gBleSuccess_c or gBleInvalidParameter_c
 ********************************************************************************** */
bleResult_t Controller_WindowWideningCallback(void (*pCallback)(uint16_t connHandle, uint16_t windowWidening1us, uint16_t supTimeout625us, uint16_t remaining625us));
