...
static tmrTimerID_t gAppTimerId;
    ...
    else
    {
        ...
        if(TRUE == bFsciBleGapEnabled)
        {
            ...
            switch(pClientPacket->structured.header.opCode)
            {
                ...
                case (uint8_t)gBleGapCmdSetExtAdvertisingParametersOpCode_c:
                    {
                ...
                case (uint8_t)gBleGapCmdSetExtAdvertisingDataOpCode_c:
                    {
                        uint8_t                 handle;
                ...
                case (uint8_t)gBleGapCmdSetPeriodicAdvDataOpCode_c:
                    {
                        uint8_t                 handle;
                        bleResult_t             status = gBleSuccess_c;
                ...
                case (uint8_t)gBleGapCmdStartAdvertisingOpCode_c:
                    ...
                    break;
#if defined(gBLE50_d) && (gBLE50_d == 1U)
                case (uint8_t)gBleGapCmdStartExtAdvertisingOpCode_c:
                    {
                        uint8_t  handle;
                        uint16_t duration;
                ...
                case (uint8_t)gBleGapCmdUpdatePeriodicAdvListOpCode_c:
                    {
                ...
                case (uint8_t)gBleGapCmdPeriodicAdvTerminateSyncOpCode_c:
                    {
                        uint16_t  syncHandle;
                ...
                case (uint8_t)gBleCtrlCmdConfigureAdvCodingSchemeOpCode_c:
                    {
                        advCodingScheme_t coding_scheme;
                        
                        fsciBleGetEnumValueFromBuffer(coding_scheme, pBuffer, advCodingScheme_t);
                        
                        fsciBleGapCallApiFunction(Controller_ConfigureAdvCodingScheme(coding_scheme));
                    }
                    break;
#endif
...
#if defined(gBLE50_d) && (gBLE50_d == 1U)
                case (uint8_t)gBleGapCmdStopExtAdvertisingOpCode_c:
                    {
                        uint8_t handle;
                ...
                case (uint8_t)gBleGapCmdRemoveAdvSetOpCode_c:
                    {
                        uint8_t handle;
                        fsciBleGetUint8ValueFromBuffer(handle, pBuffer);
                ...
                case (uint8_t)gBleGapCmdStartPeriodicAdvOpCode_c:
                    {
                        uint8_t handle;
                ...
                case (uint8_t)gBleGapCmdStopPeriodicAdvOpCode_c:
                    {
                        uint8_t handle;
                ...
                case (uint8_t)gBleGapCmdPeriodicAdvCreateSyncOpCode_c:
                    {
                ...
                case (uint8_t)gBleGapCmdCheckNvmIndexOpCode_c:
                    {
                        uint8_t  nvmIndex;
                        bool_t   bOutIsFree;
                ...
                case (uint8_t)gBleGapCmdGetDeviceIdFromConnHandleOpCode_c:
                    {
                        uint16_t connHandle;
                        deviceId_t deviceId = 0U;
                ...
                case (uint8_t)gBleGapCmdGetConnectionHandleFromDeviceIdOpCode_c:
                    {
                        deviceId_t deviceId;
                ...
                case (uint8_t)gBleGapCmdReadControllerLocalRPAOpCode_c:
                    {
                        bleIdentityAddress_t bleIdentityAddress;
                        /* Get command parameters from buffer */
...
#if defined(gBLE50_d) && (gBLE50_d == 1U)
                case (uint8_t)gBleGapCmdSetPeriodicAdvParametersOpCode_c:
                    {
        ...
        defined(CPU_MKW35A512VFP4) || defined(CPU_MKW35Z512VHT4) || defined(CPU_MKW36A512VFP4) || \
        defined(CPU_MKW36A512VHT4) || defined(CPU_MKW36Z512VFP4) || defined(CPU_MKW36Z512VHT4))
                case (uint8_t)gBleCtrlCmdSetScanDupFiltModeOpCode_c:
                    {
                        uint8_t mode;
                    ...
                    break;
#endif
				case gBleCtrlWritePublicDeviceAddressOpCode_c:
					{
						...
						fsciBleGetArrayFromBuffer(deviceAddress, pBuffer, gcBleDeviceAddressSize_c);
						...
						if(status == gBleSuccess_c)
						{
						    if(reset == TRUE)
						    {
						        ...
						        gAppTimerId = TMR_AllocateTimer();
						        if (gAppTimerId != gTmrInvalidTimerID_c)
						        {
						            /* reset MCU after 1 seconds */
						            (void)TMR_StartSingleShotTimer(gAppTimerId, TmrSeconds(1U), WritePublicDeviceAddress_TmrCb, NULL);
						        }
						        else
						        {
                ...
                case gBleGapEvtCheckNvmIndexOpCode_c:
                    {
...
void fsciBleGapSendSlaveSecurityRequestCmdMonitor(deviceId_t deviceId, const gapPairingParameters_t* pPairingParameters)
{
    ...
    /* Allocate the packet to be sent over UART */
    pClientPacket = fsciBleGapAllocFsciPacket((uint8_t)gBleGapCmdSendSlaveSecurityRequestOpCode_c, fsciBleGetDeviceIdBufferSize(&deviceId) +
                                                                                          fsciBleGapGetPairingParametersBufferSize(pPairingParameters));
...
*
* \param[in]    deviceId    The id of the connected peer.
*
********************************************************************************** */
...
*
* \param[in]    connHandle    The connection identifier.
*
********************************************************************************** */
...
void fsciBleGapGetBondedDevIdentityInfoEvtMonitor(const gapIdentityInformation_t* aOutIdentityAddresses, const uint8_t* pOutActualCount)
{
   ...
   /* If GAP is disabled the event must be not monitored */
    ...
    pClientPacket = fsciBleGapAllocFsciPacket((uint8_t)gBleGapEvtGetBondedDevicesIdentityInformationOpCode_c,
                                              sizeof(uint8_t) +
                                              *pOutActualCount * (sizeof(bleAddressType_t) +
                                                                            gcBleDeviceAddressSize_c +
                                                                            gcSmpIrkSize_c +
                                                                            sizeof(blePrivacyMode_t)));
...
void fsciBleGapGenericEvtMonitor(gapGenericEvent_t* pGenericEvent)
{
    ...
    switch(pGenericEvent->eventType)
    {
        ...
        case gControllerLocalRPARead_c:
            {
                opCode = gBleGapEvtGenericEventControllerLocalRPAReadOpCode_c;
            }
            break;            
        ...
        case gPeriodicAdvertisingStateChanged_c:
            {
                opCode = gBleGapEvtGenericEventPeriodicAdvertisingStateChangedOpCode_c;
            }
            break;
...
static bleResult_t fsciWritePublicDeviceAddress(bleDeviceAddress_t bdAddress)
{
    ...
    if(NV_ReadHWParameters(&gHardwareParameters) != 0U)
    {
        retStatus = gBleOsError_c; 
    }
    else
    {
        if (!FLib_MemCmp(bdAddress, gHardwareParameters.bluetooth_address, gcBleDeviceAddressSize_c))
        {
            FLib_MemCpy(gHardwareParameters.bluetooth_address, bdAddress, gcBleDeviceAddressSize_c);
            if(NV_WriteHWParameters(&gHardwareParameters) != 0U)
            {
                retStatus = gBleOsError_c;
            }
        }
