...
API_RESULT gatt_db_cscs_battery_level_handler
           (
           ...
           )
{
     ...
     *  GATT_CHAR_CSCS_BATTERY_LVL_INST set to 0xFF in corresponding gatt_db.h to
     *  resolve the build issue.
     */
    if ((GATT_DB_CHAR_PEER_CLI_CNFG_WRITE_REQ == params->db_op) &&
       (GATT_CHAR_CSCS_BATTERY_LVL_INST == handle->char_id))
    {
           ...
           )
{
    ...
    {
        BT_UNPACK_LE_2_BYTE(&config, params->value.val);
        if (GATT_CHAR_CSCS_SC_CNTRL_PNT_INST == handle->char_id)
        {
           ...
           )
{
    ...
    if (GATT_SER_CSCS_GAP_INST == handle->service_id)
    {
        retval = API_SUCCESS;
    }
    /* GATT Service Specific GATT Handlers */
    else if (GATT_SER_CSCS_GATT_INST == handle->service_id)
    {
        retval = API_SUCCESS;
    }
    /* Battery Service Specific GATT Handlers */
    else if (GATT_SER_CSCS_BATTERY_INST == handle->service_id)
    {
        retval = gatt_db_cscs_battery_level_handler(handle, params);
    }
    /* Device Info Service Specific GATT Handlers */
    else if (GATT_SER_CSCS_DEV_INFO_INST == handle->service_id)
    {
        retval = API_SUCCESS;
    }
    /* CSCS Specific GATT Handlers */
    else if (GATT_SER_CSCS_CSC_INST == handle->service_id)
    {
        retval = gatt_db_cscs_handler(handle, params);
    }
    else
    {
