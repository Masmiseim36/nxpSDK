...
API_RESULT gatt_db_hids_battery_level_handler
           (
           ...
           )
{
    ...
    if ((GATT_DB_CHAR_PEER_CLI_CNFG_WRITE_REQ == params->db_op) &&
       (GATT_CHAR_HID_BATTERY_LVL_INST == handle->char_id))
    {
           ...
           )
{
    ...
    if (GATT_SER_HID_GAP_INST == handle->service_id)
    {
        retval = API_SUCCESS;
    }
    /* GATT Service Specific GATT Handlers */
    else if (GATT_SER_HID_GATT_INST == handle->service_id)
    {
        retval = API_SUCCESS;
    }
    /* Battery Service Specific GATT Handlers */
    else if (GATT_SER_HID_BATTERY_INST == handle->service_id)
    {
        retval = gatt_db_hids_battery_level_handler(handle, params);
    }
    /* Device Info Service Specific GATT Handlers */
    else if (GATT_SER_HID_DEV_INFO_INST == handle->service_id)
    {
        retval = API_SUCCESS;
    }
