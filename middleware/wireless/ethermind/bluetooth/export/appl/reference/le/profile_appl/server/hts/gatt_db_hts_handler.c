           ...
           )
{
    ...
    if ((GATT_DB_CHAR_PEER_CLI_CNFG_WRITE_REQ == params->db_op) &&
       (GATT_CHAR_HTS_BATTERY_LVL_INST == handle->char_id))
    {
           ...
           )
{
    ...
    {
        if (GATT_CHAR_HTS_MSRMT_INTERVAL_INST == handle->char_id)
        {
            BT_UNPACK_LE_2_BYTE (&config,params->value.val);
           ...
           )
{
    ...
    if (GATT_SER_HTS_GAP_INST == handle->service_id)
    {
        retval = API_SUCCESS;
    }
    /* GATT Service Specific GATT Handlers */
    else if (GATT_SER_HTS_GATT_INST == handle->service_id)
    {
        retval = API_SUCCESS;
    }
    /* Battery Service Specific GATT Handlers */
    else if (GATT_SER_HTS_BATTERY_INST == handle->service_id)
    {
        retval = gatt_db_hts_battery_level_handler(handle, params);
    }
    /* Device Info Service Specific GATT Handlers */
    else if (GATT_SER_HTS_DEV_INFO_INST == handle->service_id)
    {
        retval = API_SUCCESS;
    }
    /* HTS Specific GATT Handlers */
    else if (GATT_SER_HTS_HEALTH_THERM_INST == handle->service_id)
    {
        retval = gatt_db_temperature_measurment_handler(handle, params);
    }
    else
    {
