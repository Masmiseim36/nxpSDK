           ...
           )
{
    ...
    retval              = API_SUCCESS;
    vcard_start_index   = 0;
    vcard_prop_selector_mask = 0;
    vcard_prop_bit_mask = 0;
    ...
    if (NULL != appl_params)
    {
        if (PBAP_GET_APPL_PARAM_FLAG(appl_params->appl_param_flag, PBAP_FLAG_LIST_START_OFFSET))
        {
        ...
        */
        if (0x00 == PBAP_GET_APPL_PARAM_FLAG(appl_params->appl_param_flag, PBAP_FLAG_VCARD_SELECTOR_OPERATOR))
        {
            appl_params->vcard_selector_operator = 0x00;
        }

        /* Apply VCARD selector filtering */
        if (PBAP_GET_APPL_PARAM_FLAG(appl_params->appl_param_flag, PBAP_FLAG_VCARD_SELECTOR))
        {
            for (i = 0; i < vcard_count; i++)
            {
                ...
                vcard_prop_bit_mask = (UINT32 )appl_params->vcard_selector[1];
                vcard_prop_bit_mask =  (vcard_prop_bit_mask << 16) |(UINT32 )appl_params->vcard_selector[0];

                vcard_prop_selector_mask =   vcard_prop_bit_mask &  vcards[i].prop_bit_mask;

                if (((0x00 == appl_params->vcard_selector_operator) &&
                     (0x00 == vcard_prop_selector_mask)) ||
                    ((0x01 == appl_params->vcard_selector_operator) &&
            ...
            PBAP_GET_APPL_PARAM_FLAG(appl_params->appl_param_flag, PBAP_FLAG_SEARCH_VALUE))
        {
            ...
            for (i = 0; i < vcard_count; i++)
            {
                ...
                if (0x00 == appl_params->search_attr)
                {
                    ret = BT_str_str ((CHAR *)vcards[i].n, (CHAR *)appl_params->search_value.value);
                }
                else if (0x01 == appl_params->search_attr)
                {
    ...
    BT_str_copy (obj_name, dir_entry);
    BT_str_cat (obj_name, BT_FOPS_PATH_SEP);
    BT_str_cat (obj_name, listingfile);

    /* Open the file */
    BT_fops_file_open (obj_name, (UCHAR *)"wb", &xml_fd);
    if (NULL == xml_fd)
    {
        BT_debug_error(BT_MODULE_ID_PBAP,
        "[PBAP_PL] Invalid Parameter\n");
    ...
    for (i = vcard_start_index; i < vcard_count; i++)
    {
        ...
        BT_fops_file_print(xml_fd, "  <vcard handle = \"%s\"", vcards[i].vcard_file_name);
           ...
           )
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            BT_str_copy(vcards[vcard_count].vcard_file_name, info.fname);
    ...
    for (i = 0; i < vcard_count; i++)
    {
        for (j = i+1; j < vcard_count; j++)
        {
            ...
            if (handle1  > handle2)
            {
                BT_str_copy(temp, vcards[i].vcard_file_name);

                BT_str_copy(vcards[i].vcard_file_name, vcards[j].vcard_file_name);

                BT_str_copy(vcards[j].vcard_file_name, temp);
            }
        }
    }
           ...
           )
{
    ...
    for (j = 0; j < num_properties; j++)
    {
        ...
        if(vcard->info[j].num_params > 0)
        {
            for (k = 0; k < vcard->info[j].num_params; k++)
            {
                if (0x00 == vcard->info[j].parm_info[k].param_name_len)
                {
                    out_vcard_data[out_offset] = ';';
                    out_offset ++;
     ...
     )
{
    ...
    for (i = 0; i < vcard_count; i++)
    {
        for (j = i+1; j < vcard_count; j++)
        {
            if (BT_str_cmp (vcards[i].n, vcards[j].n) > 0 )
            {
