...
bool_t FSCI_EnableBootloaderFunc(clientPacket_t *pData, uint32_t fsciInterface)
{
    ...
    if (FLib_MemCmpToVal((uint8_t const *)gBootImageFlagsAddress_c, 0xFF,
                         (uint8_t)FSL_FEATURE_FLASH_PFLASH_BLOCK_WRITE_UNIT_SIZE) == TRUE)
    {
        (void)HAL_FlashProgramUnaligned(gBootImageFlagsAddress_c, sizeof(flag), flag.aBootloaderTrigger);
    }
#endif
    HAL_ResetMCU();
#endif
