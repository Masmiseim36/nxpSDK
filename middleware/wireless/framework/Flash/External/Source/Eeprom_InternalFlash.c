...
ee_err_t EEPROM_ChipErase(void)
{
    ...
    endAddr = gEepromParams_TotalSize_c;

    for( i= 0x0; i<endAddr; i+=gEepromParams_SectorSize_c )
    {
...
static ee_err_t EEPROM_PrepareForWrite(uint32_t NoOfBytes, uint32_t Addr)
{
    ...
    if((uint32_t)((uint64_t)(Addr + NoOfBytes) & (uint64_t)(gEepromParams_SectorSize_c)))
    {
        endBlk++;
    }
