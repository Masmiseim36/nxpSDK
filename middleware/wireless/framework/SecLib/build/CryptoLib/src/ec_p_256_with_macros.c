...
************************************************************************************/
#include "ec_p_256.h"
#if (defined(mUseBigIntMacros_d) && (mUseBigIntMacros_d == 1))
#include "big_integer_macros.h"
...
{
    uint32_t retval;
    Bi_Add(x, y, result, KEY_UINT32_ARRAY_LENGTH, retval);
    if (retval)
    { // add prime if carry is still set!
        Bi_Add(result, reducer, result, KEY_UINT32_ARRAY_LENGTH, retval);
    }
}
...
{
    uint32_t retval;
    Bi_Sub(x, y, result, KEY_UINT32_ARRAY_LENGTH, retval);
    if (retval)
    { // add modulus if carry is set
        Bi_Add(result, modulus, result, KEY_UINT32_ARRAY_LENGTH, retval);
    }
}
...
void EcP256_FieldModO(const uint32_t *A, uint32_t *result, uint8_t length)
{
    ...
    uint32_t retval;
    Bi_Compare(A, P256_order_m, KEY_UINT32_ARRAY_LENGTH, retval);
    if (length == KEY_UINT32_ARRAY_LENGTH && retval <= 0)
    {
        if (A != result)
    ...
    Bi_RightShiftBy(A, length, q1_q3, 9, P256_order_k - 1);

    EcP256_FieldMult(P256_order_mu, q1_q3, q2_tmp, 9);

    Bi_RightShiftBy(q2_tmp, 18, q1_q3, 8, P256_order_k + 1);
    ...
    Bi_Sub(A, q2_tmp, tmpresult, 9, retval);
    int retval_cmp;
    Bi_Compare(tmpresult, P256_order_m, 9, retval_cmp);
    while (retval_cmp >= 0)
    {
        Bi_Sub(tmpresult, P256_order_m, tmpresult, 9, retval);
        Bi_Compare(tmpresult, P256_order_m, 9, retval_cmp);
    }
...
void EcP256_FieldInv(const uint32_t *A, uint32_t *B)
{
    ...
    x1[0] = 1;
    /* While u !=1 and v !=1 */
    uint32_t retval_u, retval_v;
    Bi_IsOne(u, retval_u);
    Bi_IsOne(v, retval_v);
    while ((retval_u || retval_v) == 0)
    {
        while (!(u[0] & 1))
        {                     /* While u is even */
            ...
            {
                Bi_Add(x1, P256_prime_m, x1, KEY_UINT32_ARRAY_LENGTH, n);
                Bi_RightShift(x1);
                x1[7] |= (n << (SIZE - 1));
            }
        }
        ...
        {                     /* While v is even */
            ...
            {
                Bi_Add(x2, P256_prime_m, x2, KEY_UINT32_ARRAY_LENGTH, n);
                Bi_RightShift(x2);
                x2[7] |= (n << (SIZE - 1));
            }
        }
        int32_t retval_cmp;
        Bi_Compare(u, v, KEY_UINT32_ARRAY_LENGTH, retval_cmp);
        if (retval_cmp >= 0)
        {                                                         /* If u > 0 */
            Bi_Sub(u, v, u, KEY_UINT32_ARRAY_LENGTH, retval_cmp); /* u=u-v */
            EcP256_FieldSub(x1, x2, P256_prime_m, x1);            /* x1=x1-x2 */
        }
        else
        {
            Bi_Sub(v, u, v, KEY_UINT32_ARRAY_LENGTH, retval_cmp); /* v=v-u */
...
void EcP256_PointDouble(const uint32_t *px, const uint32_t *py, uint32_t *Dx, uint32_t *Dy)
{
    ...
    uint32_t tempC[8];
    uint32_t tempD[16];
    uint32_t retval_x, retval_y;
    Bi_IsZero(px, retval_x);
    Bi_IsZero(py, retval_y);
    if (retval_x && retval_y)
    {
    ...
    const uint32_t *px, const uint32_t *py, const uint32_t *qx, const uint32_t *qy, uint32_t *Sx, uint32_t *Sy)
{
    ...
    {
        Bi_IsZero(qx, retval_x);
        Bi_IsZero(qy, retval_y);
        if (retval_x && retval_y)
        {
    ...
    Bi_IsSame(px, qx, KEY_UINT32_ARRAY_LENGTH, retval_x);
    if (retval_x)
    {
        Bi_IsSame(py, qy, KEY_UINT32_ARRAY_LENGTH, retval_y);
        if (!retval_y)
        {
            Bi_SetZero(Sx, 8);
            Bi_SetZero(Sy, 8);
                      ...
                      void *          pMultiplicationBuffer)
{
    ...
    uint32_t *pointer          = (uint32_t *)((uint8_t *)buffer + sizeof(multiplicationCtx_t));
    buffer->multiplicationTemp = pointer;
    pointer += 2 * KEY_UINT32_ARRAY_LENGTH;
    ...
    Bi_SetZero(buffer->pR0x, KEY_UINT32_ARRAY_LENGTH);
    Bi_SetZero(buffer->pR0y, KEY_UINT32_ARRAY_LENGTH);
    Bi_SetZero(buffer->pR0z, KEY_UINT32_ARRAY_LENGTH);

    for (i = 255; i >= 0; i--)
    {
        ec_JacobiDouble(buffer);

        if ((((secret[i / 32]) >> (i % 32)) & 0x01))
        {
...
bool_t EcP256_IsPointOnCurve(const uint32_t *X, const uint32_t *Y)
{
    ...
    Bi_Compare(X, P256_prime_m, KEY_UINT32_ARRAY_LENGTH, retval_cmp);
    ...
    {
        Bi_Compare(Y, P256_prime_m, KEY_UINT32_ARRAY_LENGTH, retval_cmp);
    ...
    if (bIsValid)
    {
        ...
        /* modulus = (X)^2 (mod p) */
        EcP256_FieldMult(X, X, lhs, KEY_UINT32_ARRAY_LENGTH);
        EcP256_FieldModP(modulus, lhs);
        Bi_SetZero(lhs, 16);

        /* lhs = modulus + a = (X)^2 + a (mod p) */
        EcP256_FieldAdd(modulus, P256_curve_a, P256_prime_r, lhs);
        Bi_Compare(lhs, P256_prime_m, KEY_UINT32_ARRAY_LENGTH, retval_cmp);

        /* if result is larger than p, apply mod */
        if ((lhs[KEY_UINT32_ARRAY_LENGTH] > 0) || (retval_cmp > 0))
        {
        ...
        Bi_Compare(rhs, P256_prime_m, KEY_UINT32_ARRAY_LENGTH, retval_cmp);

        /* if result is larger than p, apply mod */
        if ((rhs[KEY_UINT32_ARRAY_LENGTH] > 0) || (retval_cmp > 0))
        {
        ...
        Bi_Compare(modulus, rhs, KEY_UINT32_ARRAY_LENGTH, retval_cmp);
        bIsValid = (retval_cmp == 0) ? TRUE : FALSE;
    }
...
{
    uint32_t retval;
    Bi_IsZero(ctx->pR0y, retval);
    if (retval)
    {
        Bi_Copy(qx, ctx->pR0x, KEY_UINT32_ARRAY_LENGTH);
        Bi_Copy(qy, ctx->pR0y, KEY_UINT32_ARRAY_LENGTH);
        Bi_SetZero(ctx->pR0z, KEY_UINT32_ARRAY_LENGTH);
        ctx->pR0z[0] = 0x00000001;
        // Bi_copy(qz,Sz,KEY_UINT32_ARRAY_LENGTH);
        return;
    }
...
static void ec_JacobiDouble(multiplicationCtx_t *ctx)
{
    ...
    if (retval)
    {
        Bi_SetZero(ctx->pR0x, KEY_UINT32_ARRAY_LENGTH);
        Bi_SetZero(ctx->pR0y, KEY_UINT32_ARRAY_LENGTH);
        Bi_SetZero(ctx->pR0z, KEY_UINT32_ARRAY_LENGTH);
    }
    else
    {
...
#endif /* (defined(mUseBigIntMacros_d) && (mUseBigIntMacros_d == 1)) */
