 ...
 *
 * Important note: the OTA module assumes that any OTA procedure shall be done on
 * a Permanent image. If the image state is RunCandidate, then this function will
 * transition the image to Permanent. The application must make sure to call this
 * function in a stage where the Candidate image has been validated as safe to
 * make Permanent. The application can also make the image Permanent manually if
 * preferred, in such case this function will not do anything.
 *
 ********************************************************************************** */
