 ...
 * Zephyr environment.
 * Some APIs missing for FSAbstraction were also added.
 *
 *
 ********************************************************************************** */
...
#ifdef NO_FLASH_CONTROLLER_BLANK_CHECK
...
static int nvs_flash_area_is_blank(struct nvs_fs *fs, uint32_t addr, size_t len)
{
	int rc;
#ifdef NO_FLASH_CONTROLLER_BLANK_CHECK
	rc = nvs_flash_cmp_const(fs, addr, fs->flash_parameters->erase_value, len);
#else
	off_t offset;
	uint32_t sect_nb;

	sect_nb = (addr & ADDR_SECT_MASK) >> ADDR_SECT_SHIFT;

	offset = fs->offset;
	offset += (fs->sector_size * sect_nb) + (addr & 0xffff);
	rc = flash_blank_check(fs->flash_device, offset, len);
#endif
	return rc;
}
...
static int nvs_flash_erase_sector(struct nvs_fs *fs, uint32_t addr)
{
    ...
    if (nvs_flash_area_is_blank(fs, addr, fs->sector_size)) {
		rc = -ENXIO;
	}
...
static int nvs_startup(struct nvs_fs *fs)
{
	...
	for (i = 0; i < fs->sector_count; i++) {
	    ...
	    rc = nvs_flash_area_is_blank(fs, addr, sizeof(struct nvs_ate));
	    if (rc) {
			/* closed sector */
			closed_sectors++;
	...
	if (i == fs->sector_count) {
		 ...
		 */
		rc = nvs_flash_area_is_blank(fs, addr - ate_size, sizeof(struct nvs_ate));
		if (!rc) {
			/* empty ate */
			nvs_sector_advance(fs, &addr);
		}
	...
	nvs_sector_advance(fs, &addr);
	rc = nvs_flash_area_is_blank(fs, addr, fs->sector_size);
	if (rc < 0) {
		goto end;
	}
	if (rc) {
	...
	while (fs->ate_wra > fs->data_wra) {
		...
		rc = nvs_flash_area_is_blank(fs, fs->data_wra,  empty_len);
		if (rc < 0) {
			goto end;
		}
		if (!rc) {
...
int nvs_unmount(struct nvs_fs *fs)
{
    /* Take mutex so ensure it is free when destroying it */
    (void)k_mutex_lock(&fs->nvs_lock, osaWaitForever_c);
    (void)k_mutex_unlock(&fs->nvs_lock);
    (void)k_mutex_destroy(&fs->nvs_lock);
    fs->ready = false;
	return 0;
}
