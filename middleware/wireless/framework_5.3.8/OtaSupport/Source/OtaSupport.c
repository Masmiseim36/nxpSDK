...
otaResult_t OTA_CommitImage(uint8_t* pBitmap)
{
    ...
    else
    {
        ...
        if(EEPROM_WriteData(gBootData_Marker_Size_c, gBootData_StartMarker_Offset_c, (uint8_t *)&start_marker) != ee_ok)
        {
            status = gOtaExternalFlashError_c;
        }
        else
...
void OTA_SetNewImageFlag(void)
{
    ...
    if( mNewImageReady )
    {
        ...
        if( (status == kStatus_FLASH_Success) &&
             FLib_MemCmpToVal((void*)gBootFlags.internalStorageAddr, 0xFF, sizeof(gBootFlags.internalStorageAddr)) )
        {
