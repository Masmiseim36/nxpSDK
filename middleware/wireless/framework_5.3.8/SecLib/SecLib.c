...
#if USE_RTOS && (FSL_FEATURE_SOC_LTC_COUNT || FSL_FEATURE_SOC_MMCAU_COUNT) && gSecLibUseMutex_c
                           ...
                           bool_t        *pbChecksumVerify)
{
    ...
    if (bEncrypt)
    {
        flags = gSecLib_CCM_Encrypt_c;
    }
    else
    {
        flags = gSecLib_CCM_Decrypt_c;
    }

    status = AES_128_CCM(pau8Data,
                         u8mlength,
                         pau8authenticationData,
                         u8alength,
                         &puNonce->au8[1],
                         13,
                         (uint8_t*)&sKey,
    ...
    if (pbChecksumVerify && (flags & gSecLib_CCM_Decrypt_c))
    {
        if (status == 0)
        {
