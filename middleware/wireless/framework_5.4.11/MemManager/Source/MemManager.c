...
static pfMemAllocFailCb_t mpfMemAllocFailCb = NULL;
#endif
/*! *********************************************************************************
*************************************************************************************
* Public functions
...
#ifdef MEM_ALLOC_FAIL_CB
/*! *********************************************************************************
* \brief    Register callback for buffer allocation failure notifications
*
* \param[in] callback - Callback function for buffer allocation failure notifications.
*
* \return void
*
********************************************************************************** */
void MEM_RegisterAllocFailCb
(
    pfMemAllocFailCb_t callback
)
{
    mpfMemAllocFailCb = callback;
}
#endif
/*! *********************************************************************************
...
{
#if defined(MEM_TRACKING) || defined(MEM_ALLOC_FAIL_CB)
    /* Save the Link Register */
    volatile uint32_t savedLR = (uint32_t) __get_LR();
#endif
#if defined(MEM_TRACKING) || defined(MEM_DEBUG_OUT_OF_MEMORY) || defined(MEM_ALLOC_FAIL_CB)
...
#ifdef MEM_ALLOC_FAIL_CB
    if (NULL != mpfMemAllocFailCb)
    {
        mpfMemAllocFailCb(poolId, (void*)savedLR, requestedSize);
    }
#endif
    return NULL;
}
