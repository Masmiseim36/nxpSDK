 ...
 ******************************************************************************/
static bool_t __NvIsDataSetDirty
(
  void* ptrData
);
/******************************************************************************
...
)
{
    ...
    if (0 == gFlashConfig.FlexRAMTotalSize)
    {
    ...
    if(gFlashConfig.FlexRAMTotalSize == 0)
    {
...
)
{
    ...
    if(flexMetaInfo.rawValue == gNvFlexGuardValue_c) /* no meta found for this table entry ID */
    {
        ...
        if(lastFlexMetaInfoAddress < gFlashConfig.FlexRAMBlockBase) /* FlexRAM empty */
        {
            /* set dataset offset related to FlexRAM start address */
            flexMetaInfo.fields.NvDataOffset = gFlashConfig.FlexRAMTotalSize - datasetSize;
            /* set last meta info address */
            lastFlexMetaInfoAddress = gFlashConfig.FlexRAMBlockBase;
        }
        ...
        if(tblIndexes->saveRestoreAll)
        {
            destRecordAddress = gFlashConfig.FlexRAMBlockBase + flexMetaInfo.fields.NvDataOffset;
            pData = (uint8_t*)(pNVM_DataTable[tableEntryIdx].pData);
        }
        else
        {
            destRecordAddress = gFlashConfig.FlexRAMBlockBase + flexMetaInfo.fields.NvDataOffset + ((tblIndexes->elementIndex) * pNVM_DataTable[tableEntryIdx].ElementSize);
        ...
        /* write record */
        if(kStatus_FLASH_Success != EEEWrite(&gFlashConfig, destRecordAddress, recordSize, pData))
        {
            return gNVM_RecordWriteError_c;
        }
...
#if FSL_FEATURE_FLASH_IS_FTFE
        /* write record */
        if(kStatus_FLASH_Success != FLASH_EepromWrite(&gFlashConfig, destRecordAddress, pData, recordSize))
        {
            return gNVM_RecordWriteError_c;
        }
    ...
    else /* table entry ID already in FlexRAM, update the requested record */
    {
        /* set destination address where the record will be written and the source data pointer */
        if(tblIndexes->saveRestoreAll)
        {
...
)
{
    ...
    EERamAddress = gFlashConfig.FlexRAMBlockBase;
...
#endif
                /* read all elements */
                NV_FlashRead((gFlashConfig.FlexRAMBlockBase + flexMetaInfo.fields.NvDataOffset),
...
#endif
                /* read element */
                NV_FlashRead(gFlashConfig.FlexRAMBlockBase + flexMetaInfo.fields.NvDataOffset + (tblIdx->elementIndex * pNVM_DataTable[tableEntryIdx].ElementSize),
...
static void __NvCompletePendingOperations(void)
{
  ...
  {
    __NvIdle();
  } while(mNvPendingSavesQueue.EntriesCount);
#endif
}
