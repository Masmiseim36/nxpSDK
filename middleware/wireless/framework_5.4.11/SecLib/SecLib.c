                                       ...
                                       uint8_t* pOutput)
{
    ...
    SecLib_DisallowToSleep();
    SECLIB_MUTEX_LOCK();
    LTC_AES_DecryptCbc(LTC0, pInput, pOutput, inputLen, pInitVector, pKey, AES_BLOCK_SIZE, kLTC_EncryptKey);
    SECLIB_MUTEX_UNLOCK();
    SecLib_AllowToSleep();
#else
                  ...
                  uint8_t* pOutput)
{
    ...
    n = (uint32_t) ((inputLen + 15) / 16); /* n is number of rounds */
...
static void AES_128_CMAC_HW(AES_param_t* CMAC_p)
{
    ...
    n = (uint32_t) ((CMAC_p->Len + 15) / 16); /* n is number of rounds */
