...
static void Serial_AsyncWriteCallback(void *pPacket);
...
serialStatus_t Serial_RxBufferByteCount( uint8_t InterfaceId, uint16_t *bytesCount )
{
    ...
    {
        OSA_InterruptDisable();
        rxIn = mSerials[InterfaceId].rxIn;
        rxOut = mSerials[InterfaceId].rxOut;
        OSA_InterruptEnable();

        if( rxIn >= rxOut )
        {
            *bytesCount = rxIn - rxOut;
        }
        else
        {
            *bytesCount = gSMRxBufSize_c - rxOut + rxIn;
        }
    }
#else
    (void)bytesCount;
...
serialStatus_t Serial_Print( uint8_t InterfaceId, const char* pString, serialBlock_t allowToBlock )
{
    ...
    packetSize = FLib_StrLen(pString);

    if ( allowToBlock )
    {
        status = Serial_SyncWrite( InterfaceId, (uint8_t*)pString, packetSize);
    }
    else
    {
        pSerialPacket = MEM_BufferAlloc(packetSize);

        if( NULL != pSerialPacket )
        {
            FLib_MemCpy(pSerialPacket, (uint8_t*)pString, packetSize);
            status = Serial_AsyncWrite( InterfaceId, pSerialPacket, packetSize, Serial_AsyncWriteCallback, pSerialPacket );
...
/*! *********************************************************************************
* \brief   Asynchronous write callback.
*
* \param[in] pPacket     pointer to the packet to be free
*
********************************************************************************** */
#if gSerialManagerMaxInterfaces_c
static void Serial_AsyncWriteCallback
(
    void *pPacket
)
{
