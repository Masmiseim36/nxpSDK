...
uint32_t StackTimer_GetInputFrequency(void)
{
    ...
    (void)refClk;     /* suppress warnings */
      ...
      (defined(gCFC_XTAL1_USE_CALIB_c) && (gCFC_XTAL1_USE_CALIB_c == 0))
    ...
    (!defined(gCFC_XTAL1_USE_CALIB_c))
     result = CLOCK_GetFreq(kCLOCK_32KClk);  //32,768Khz crystal is used
  #else
    #if (defined(gCFG_MCU_HAS_FPU_c) && (gCFG_MCU_HAS_FPU_c > 0)) /* make use of the existing FPU */
    int32_t ppm = 0;
    result = 32000;

    if ( RTC->CTRL & RTC_CTRL_CAL_EN_MASK) /* is calibration enabled ? */
    {
        /* Get the current calibration value and obtain the rounded uncalibrated
         * clock frequency using the formula:
         * fUncal = 32000 - (ppm * 0.03125 +/- 0.5) where 0.03125 is the number
        ...
        if (RTC->CAL & RTC_CAL_DIR_MASK)
        {
            ...
            result -= (int32_t)((float)ppm * (float)0.03125 - (float)0.5);
        }
        else
        {
            /* Forward calibration */
