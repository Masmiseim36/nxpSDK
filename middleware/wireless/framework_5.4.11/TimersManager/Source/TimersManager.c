...
)
{
    ...
    mMinTmrScheduleTicks = TmrTicksFromMilliseconds(4);
#else
    /* The TMR_Task()event will not be issued faster than 1ms*/
    mMinTmrScheduleTicks = TmrTicksFromMilliseconds(1);
#endif
}
...
/*! -------------------------------------------------------------------------
 * \brief     Returns the first low power timer to expire. It is intended to be used
 *            from low power module only.
 * \param[in] none
 * \return    timer ID of the first low power timer to expire.
 *---------------------------------------------------------------------------*/
tmrTimerID_t TMR_LpmGetFirstLowPowerTimerToExpire(void)
{
    tmrTimerTicks64_t remainingTicks = 0xFFFFFFFFFFFFFFFFU;
    tmrTimerID_t timerID;
    tmrTimerID_t retID = gTmrInvalidTimerID_c;
    for( timerID = 0; timerID < NumberOfElements(maTmrTimerTable); ++timerID )
    {
        if( TMR_IsTimerActive(timerID) && ((gTmrLowPowerTimer_c & TMR_GetTimerType(timerID)) > 0) )
        {
            if(retID == gTmrInvalidTimerID_c)
            {
                retID = timerID;
                remainingTicks = maTmrTimerTable[timerID].remainingTicks;
            }
            else
            {
                if(maTmrTimerTable[timerID].remainingTicks < remainingTicks)
                {
                    remainingTicks = maTmrTimerTable[timerID].remainingTicks;
                    retID = timerID;
                }
...
/*! -------------------------------------------------------------------------
 * \brief     Returns the remaining ticks of the timer. It is intended to be used
 *            from low power module only in pair with TMR_LpmGetFirstLowPowerTimerToExpire
 *            so it doesn't perform any parameters check.
 * \param[in] timerID 
 * \return    the remaining ticks for the timer ID.
 *---------------------------------------------------------------------------*/
tmrTimerTicks64_t TMR_LpmGetRemainingTicks(tmrTimerID_t timerID)
{
    tmrTimerTicks64_t remainingTicks;
    tmrTimerTicks_t elapsedRemainingTicks ;
    tmrTimerTicks_t currentTimeInTicks = StackTimer_GetCounterValue();
    elapsedRemainingTicks = (tmrTimerTicks_t)(currentTimeInTicks - previousTimeInTicks);
    if(maTmrTimerTable[timerID].remainingTicks > elapsedRemainingTicks )
    {
        remainingTicks = maTmrTimerTable[timerID].remainingTicks - elapsedRemainingTicks;
    }
    else
    {
        remainingTicks = 0;
...
/*! -------------------------------------------------------------------------
 * \brief      This function is called by the Low Power module when 
 *             cPWR_EnableTimerAccuracyOptimization is set each time the MCU wakes up.
 * \param[in]  sleep duration in TMR ticks.
 * \return    Whether the timer task needs to be scheduled at low power exit. 
 *---------------------------------------------------------------------------*/
bool_t TMR_SyncLpmTimersAccurate
(
uint32_t sleepDurationTmrTicks
)
{
    bool_t makeTmrTaskReady = FALSE;
#if (gTMR_EnableLowPowerTimers_d)
#ifndef CPU_QN908X
    uint32_t  timerID;
    tmrTimerType_t timerType;
    tmrTimerTicks_t     nextInterruptTime;
    tmrTimerTicks_t timeStampAdj = sleepDurationTmrTicks; 
    /* Check if there are low power active timer */
    if (numberOfLowPowerActiveTimers)
    {
        nextInterruptTime = mMaxToCountDown_c;
        ...
        for (timerID = 0; timerID < NumberOfElements(maTmrTimerTable); ++timerID)
        {
                ...
                && (IsLowPowerTimer(timerType)) )
            {
                ...
                {
                    maTmrTimerTable[timerID].remainingTicks -= sleepDurationTmrTicks;
                    maTmrTimerTable[timerID].timestamp += timeStampAdj;
                    if (nextInterruptTime > maTmrTimerTable[timerID].remainingTicks)
                    {
                        nextInterruptTime = maTmrTimerTable[timerID].remainingTicks;
                    }
                ...
                {
                    maTmrTimerTable[timerID].remainingTicks = 0;
                    makeTmrTaskReady = TRUE;
                }
            }
        }/* end for (timerID = 0;.... */
        if(makeTmrTaskReady == FALSE)
        {
            tmrTimerTicks_t     currentTimeInTicks;
            currentTimeInTicks = StackTimer_GetCounterValue();
            nextInterruptTime += currentTimeInTicks;
...
#ifdef RTC_CLOCKS
        {
            clock_ip_name_t rtcClocks[] = RTC_CLOCKS;  
            CLOCK_EnableClock(rtcClocks[gTmrRtcInstance_c]);
        }
#endif
