...
#endif
{
    {gBootFlagUnprogrammed_c}, /* new image available */
    {gBootValueForTRUE_c},     /* boot process completed */
#ifdef CPU_QN9080C
    {gBootFlagUnprogrammed_c}, /* bootloader version */
#else
	{0x00, 0x02},
#endif	    
    {gBootFlagUnprogrammed_c}, /* internal storage start address */
...
otaResult_t OTA_CommitImage(uint8_t* pBitmap)
{
    ...
    {
#if ((gEepromType_d == gEepromDevice_InternalFlash_c) && !gSupportLegacyBootloader_d)
        ...
        if(EEPROM_WriteData((uint32_t)gBootData_Marker_Size_c, (uint32_t)gBootData_StartMarker_Offset_c,(uint8_t*)&start_marker) != ee_ok)
        {
            status = gOtaExternalFlashError_c;
        }
        else
...
void OTA_SetNewImageFlag(void)
{
    ...
    if( mNewImageReady )
    {
        ...
        if( (status == kStatus_FLASH_Success) && 
             FLib_MemCmpToVal((const void*)gBootFlags.internalStorageAddr, 0xFF, sizeof(gBootFlags.internalStorageAddr)))
        {
