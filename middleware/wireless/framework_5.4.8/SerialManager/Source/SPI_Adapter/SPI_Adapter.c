...
********************************************************************************** */
#if defined(CPU_QN9080C)
/* On QN908x, the SPI_ReadData inline function is not working correctly when optimized */
...
#else
    #if defined(CPU_QN9080C) && defined(__CC_ARM)
    /* On QN908x, the SPI_ReadData inline function is not working correctly when optimized
       using __CC_ARM #pragma O0 so skip the inline function implementation and
       rewrite it in place */
    volatile uint32_t data = 0xFF;
    if(NULL != baseAddr)
    {
        data = baseAddr->FIFORD;
    }
    return data;
    #else
