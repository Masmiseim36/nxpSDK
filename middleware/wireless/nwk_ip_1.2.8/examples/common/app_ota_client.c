...
static void OtaClient_CoapMsgProcess(uint8_t *param);
...
static void OtaClient_ProcessBlock(uint8_t *pParam);
...
)
{
    ...
    if (gOtaClientState_NotInit_c == mOtaClientSession.state)
    {
        ...
        mOtaClientSetup.coapInstanceId = COAP_CreateInstance(NULL, &coapParams,
...
)
{
    if ((pData) && (sessionStatus == gCoapSuccess_c))
    {
        ...
        if (NULL != pOtaServerInf)
        {
            ...
            FLib_MemCpy(&pOtaServerInf->remoteAddr, &pSession->remoteAddrStorage.ss_addr, sizeof(ipAddr_t));
    ...
    else
    {
        ...
        else
        {
            ...
            if (status == gOtaStatus_Success_c)
            {
                ...
                if (gOtaMulticast_c == mOtaClientSession.transferType)
                {
                    bstatus = NWKU_SendMsg((nwkMsgHandler)OtaClientMulticast_ProcessBlock, pOtaServerInfo, mpOtaClientMsgQueue);
                }
                else
                {
...
)
{
    ...
    if (pSession)
    {
        ...
        NWKU_SetSockAddrInfo(&pSession->remoteAddrStorage, &mOtaClientSession.serverIpAddr,
        ...
        /* send coap message */
        status = COAP_Send(pSession, gCoapMsgTypeUseSessionValues_c, pData, dataLen);
    }
...
)
{
    ...
    if ((pParam) && (-1 != mOtaClientSetup.mOtaUdpSrvSockFd))
    {
        ...
        (void)sendto(mOtaClientSetup.mOtaUdpSrvSockFd, (uint8_t*)&blockReq, sizeof(otaCmd_BlockReq_t), 0,
...
)
{
    ...
    if (pSession)
    {
        ...
        pSession->pCallback = NULL;
        NWKU_SetSockAddrInfo(&pSession->remoteAddrStorage, &in6addr_realmlocal_allthreadnodes,
                             AF_INET6, COAP_DEFAULT_PORT, gNoIPv6FlowInfo_c, gIpIfSlp0_c);
        COAP_SetUriPath(pSession, (coapUriPath_t *)&gOTA_CLIENT_URI_PATH);
        (void)THR_GetIP6Addr(0, OTA_ADDR_TYPE, &pSession->localAddr, NULL);
        pSession->msgType = gCoapNonConfirmable_c;
        pSession->code = gCoapGET_c;
        /* Send command */
        (void)COAP_Send(pSession, gCoapMsgTypeNonPost_c, (uint8_t*)&serverDiscoveryCmd, sizeof(otaCmd_ServerDiscovery_t));
    }
...
)
{
    ...
    else
    {
        ...
        (void)NWKU_SendMsg((nwkMsgHandler)OtaClientMulticast_ProcessImage, NULL, mpOtaClientMsgQueue);
    }
}
...
)
{
    ...
    if (TRUE == onOff)
    {
        if (mOtaClientSetup.mOtaUdpSrvSockFd == -1)
        {
            ...
            FLib_MemSet(&portAddr, 0, sizeof(sockaddrStorage_t));
            NWKU_SetSockAddrInfo(&portAddr, (ipAddr_t*)&in6addr_any, AF_INET6, OTA_CLIENT_DEFAULT_PORT, gNoIPv6FlowInfo_c, gIpIfSlp0_c);
            ...
            if (mOtaClientSetup.mOtaUdpSrvSockFd != -1)
            {
                ...
                Session_RegisterCb(mOtaClientSetup.mOtaUdpSrvSockFd, OtaClient_UdpServerService,
                                   mpOtaClientMsgQueue, mSessCbData_c);
            }
            else
            {
                status = gOtaStatus_Failed_c;
    ...
    else
    {
        if (mOtaClientSetup.mOtaUdpSrvSockFd != -1)
        {
            ...
            (void)closesock(mOtaClientSetup.mOtaUdpSrvSockFd);
