...
static void OtaServer_ClientProcess(uint8_t *param);
static void OtaServer_CmdProcess(uint8_t *param);
...
static void OtaServer_CoapSendQueryImageRsp(uint8_t *pParam);
...
static void OtaServer_CloseThciSessionTimeout(uint8_t *pParam);
...
static void OtaServerDongle_SendBlockRsp(uint8_t *pParam);
...
)
{
    ...
    if (gOtaCmd_BlockReq_c == otaCommand)
    {
        ...
        if (NULL != pOtaClientInfo)
        {
            ...
            OtaServer_ClientProcess((uint8_t *)pOtaClientInfo);
        }
    }
...
)
{
    ...
    if (!mpOtaServerMsgQueue)
    {
        ...
        mOtaServerSetup.coapInstanceId = COAP_CreateInstance(NULL, &coapParams,
    ...
    if (gCoapSuccess_c == sessionStatus)
    {
        if (gOtaStatus_Success_c == OtaServer_CmdCheck(pData, dataLen))
        {
            ...
            if (NULL != pOtaClientInfo)
            {
                ...
                /* Save client info params */
                IP_AddrCopy(&pOtaClientInfo->remoteAddr, (ipAddr_t*)&pSession->remoteAddrStorage.ss_addr);

                if (FLib_MemCmp(&pSession->remoteAddrStorage.ss_addr, &nullAddr, sizeof(pOtaClientInfo->sourceAddr)))
                {
    ...
    else
    {
                ...
                (gOtaStatus_Success_c == OtaServer_HandleBlockSocket(TRUE)))
        {
            if (mOtaServerSetup.opMode == gOtaServerOpMode_Dongle_c)
            {
                ...
                {
                    /* Process client */
                    OtaServer_CmdProcess((uint8_t*)pOtaClientInfo);
                }
                else
                {
...
)
{
    ...
    if (pSession)
    {
        ...
        NWKU_SetSockAddrInfo(&pSession->remoteAddrStorage, pDestAddr,
                             AF_INET6, COAP_DEFAULT_PORT, gNoIPv6FlowInfo_c, gIpIfSlp0_c);
        pSession->msgType = gCoapNonConfirmable_c;
        pSession->code = gCoapPOST_c;
        (void)THR_GetIP6Addr(0, OTA_ADDR_TYPE, &pSession->localAddr, NULL);
        /* Complete command */
        ...
        /* Send command */
        if (gNwkStatusSuccess_c == COAP_Send(pSession, gCoapMsgTypeNonPost_c, (uint8_t*)&imageNotify, sizeof(otaServerCmd_ImageNotify_t)))
        {
            status = gOtaStatus_Success_c;
        }
    }
...
)
{
    ...
    if (pCoapSession)
    {
        NWKU_SetSockAddrInfo(&pCoapSession->remoteAddrStorage, &pOtaClientInfo->remoteAddr,
                             AF_INET6, COAP_DEFAULT_PORT, gNoIPv6FlowInfo_c, gIpIfSlp0_c);
        ...
        (void)THR_GetIP6Addr(0, OTA_ADDR_TYPE, &pCoapSession->localAddr, NULL);
...
)
{
    ...
    while ((currentAddr < gFlashParams_MaxImageLength_c) && (index < gOtaServer_MaxOtaImages_c))
    {
        ...
        /* Read OTA Header */
        (void)OTA_ReadExternalMemory((uint8_t *)&otaHeader, sizeof(otaFileHeader_t), currentAddr);
...
)
{
    ...
    if (TRUE == onOff)
    {
        if (mOtaServerSetup.mOtaUdpSrvSockFd == -1)
        {
            ...
            FLib_MemSet(&portAddr, 0, sizeof(sockaddrStorage_t));
            NWKU_SetSockAddrInfo(&portAddr, (ipAddr_t*)&in6addr_any, AF_INET6, OTA_SERVER_DEFAULT_PORT, gNoIPv6FlowInfo_c, gIpIfSlp0_c);
            ...
            if (mOtaServerSetup.mOtaUdpSrvSockFd != -1)
            {
                ...
                Session_RegisterCb(mOtaServerSetup.mOtaUdpSrvSockFd, OtaClient_UdpServerService,
                                   mpOtaServerMsgQueue, mSessCbData_c);
            }
            else
            {
    ...
    else
    {
        if (mOtaServerSetup.mOtaUdpSrvSockFd != -1)
        {
            ...
            (void)closesock(mOtaServerSetup.mOtaUdpSrvSockFd);
...
{
    if (FALSE == NWKU_SendMsg((nwkMsgHandler)OtaServer_MulticastMngr, pParam, mpOtaServerMsgQueue))
    {
