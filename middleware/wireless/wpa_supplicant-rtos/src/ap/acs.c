...
static long double acs_survey_interference_factor(struct freq_survey *survey, s8 min_nf)
{
    ...
    factor = pow(10, survey->nf / 5.0L) +
             (total ? (busy / total) : 0) *
                 pow(2, pow(10, (long double)survey->nf / 10.0L) - pow(10, (long double)min_nf / 10.0L));
...
static void acs_survey_chan_interference_factor(struct hostapd_iface *iface, struct hostapd_channel_data *chan)
{
    ...
    dl_list_for_each(survey, &chan->survey_list, struct freq_survey, list)
    {
        ...
        wpa_printf(MSG_DEBUG, "ACS: %d: min_nf=%d interference_factor=%Lg nf=%d time=%lu busy=%lu rx=%lu", i,
                   chan->min_nf, int_factor, survey->nf, (unsigned long)survey->channel_time,
                   (unsigned long)survey->channel_time_busy, (unsigned long)survey->channel_time_rx);
    }
...
{
    const int allowed[] = {5180, 5220, 5260, 5300, 5500, 5540, 5580, 5620, 5660, 5745, 5785, 4920, 4960, 5955,
                           5995, 6035, 6075, 6115, 6155, 6195, 6235, 6275, 6315, 6355, 6395, 6435, 6475, 6515,
                           6555, 6595, 6635, 6675, 6715, 6755, 6795, 6835, 6875, 6915, 6955, 6995, 7035, 7075};
    unsigned int i;
...
{
    const int allowed[] = {5180, 5260, 5500, 5580, 5660, 5745, 5955, 6035, 6115, 6195,
...
static int acs_surveys_are_sufficient_mode(struct hostapd_hw_modes *mode)
{
    ...
    {
        chan = &mode->channels[i];
        if (!(chan->flag & HOSTAPD_CHAN_DISABLED) && acs_survey_list_is_sufficient(chan))
            return 1;
    }
...
static int acs_surveys_are_sufficient(struct hostapd_iface *iface)
{
    ...
    for (i = 0; i < iface->num_hw_features; i++)
    {
        ...
        if (!hostapd_hw_skip_mode(iface, mode) && acs_surveys_are_sufficient_mode(mode))
            return 1;
    }
...
{
    return !dl_list_empty(&chan->survey_list) && !(chan->flag & HOSTAPD_CHAN_DISABLED) &&
...
static int is_in_freqlist(struct hostapd_iface *iface, struct hostapd_channel_data *chan)
{
...
static void acs_survey_mode_interference_factor(struct hostapd_iface *iface, struct hostapd_hw_modes *mode)
{
    int i;
    struct hostapd_channel_data *chan;
...
static void acs_survey_all_chans_interference_factor(struct hostapd_iface *iface)
{
    int i;
    struct hostapd_hw_modes *mode;
...
static struct hostapd_channel_data *acs_find_chan_mode(struct hostapd_hw_modes *mode, int freq)
{
    struct hostapd_channel_data *chan;
    int i;
...
static void acs_find_ideal_chan_mode(struct hostapd_iface *iface,
                                     struct hostapd_hw_modes *mode,
                                     int n_chans,
                                     u32 bw,
                                     ...
                                     long double *ideal_factor)
{
    ...
    for (i = 0; i < mode->num_channels; i++)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "ACS: Channel %d: BW %u is not supported", chan->chan, bw);
            continue;
        }
        ...
        if (mode->mode == HOSTAPD_MODE_IEEE80211A &&
            ((iface->conf->ieee80211n && iface->conf->secondary_channel) || is_6ghz_freq(chan->freq)) &&
        ...
        if (mode->mode == HOSTAPD_MODE_IEEE80211A && (iface->conf->ieee80211ac || iface->conf->ieee80211ax))
        {
            if (hostapd_get_oper_chwidth(iface->conf) == CHANWIDTH_80MHZ && !acs_usable_bw80_chan(chan))
            {
                wpa_printf(MSG_DEBUG, "ACS: Channel %d: not allowed as primary channel for 80 MHz bandwidth",
                           chan->chan);
                continue;
            }

            if (hostapd_get_oper_chwidth(iface->conf) == CHANWIDTH_160MHZ && !acs_usable_bw160_chan(chan))
            {
                wpa_printf(MSG_DEBUG, "ACS: Channel %d: not allowed as primary channel for 160 MHz bandwidth",
                           chan->chan);
                continue;
            }
        }
        ...
        for (j = 1; j < n_chans; j++)
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "ACS: PRI Channel %d: secondary channel %d BW %u is not supported", chan->chan,
                           adj_chan->chan, bw);
                break;
            }
        ...
        else if (is_24ghz_mode(mode->mode) && is_common_24ghz_chan(chan->chan))
        {
        ...
        {
            factor *= bias->bias;
            wpa_printf(MSG_DEBUG, "ACS:  * channel %d: total interference = %Lg (%f bias)", chan->chan, factor,
                       bias->bias);
        }
        else
        {
...
{
    struct hostapd_channel_data *ideal_chan = NULL, *rand_chan = NULL;
...
bw_selected:
    ...
    wpa_printf(MSG_DEBUG, "ACS: Survey analysis for selected bandwidth %d MHz", bw);

    for (i = 0; i < iface->num_hw_features; i++)
    {
        mode = &iface->hw_features[i];
        if (!hostapd_hw_skip_mode(iface, mode))
            acs_find_ideal_chan_mode(iface, mode, n_chans, bw, &rand_chan, &ideal_chan, &ideal_factor);
    }
...
static void acs_adjust_center_freq(struct hostapd_iface *iface)
{
    ...
    switch (hostapd_get_oper_chwidth(iface->conf))
    {
        ...
        default:
             ...
             * acs_find_ideal_chan() */
            wpa_printf(MSG_INFO, "ACS: Only VHT20/40/80/160 is supported now");
            return;
    }
...
static int *acs_request_scan_add_freqs(struct hostapd_iface *iface, struct hostapd_hw_modes *mode, int *freq)
{
    struct hostapd_channel_data *chan;
    int i;
