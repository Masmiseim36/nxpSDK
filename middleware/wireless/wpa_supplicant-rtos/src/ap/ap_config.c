...
#undef ecw2cw
    ...
    conf->he_op.he_rts_threshold = HE_OPERATION_RTS_THRESHOLD_MASK >> HE_OPERATION_RTS_THRESHOLD_OFFSET;
...
static int hostapd_config_read_wpa_psk(const char *fname, struct hostapd_ssid *ssid)
{
    ...
    while (fgets(buf, sizeof(buf), f))
    {
        ...
        while ((token = str_token(buf, " ", &context)))
        {
            ...
            {
                wpa_printf(MSG_ERROR, "Unrecognized '%s=%s' on line %d in '%s'", name, value, line, fname);
                ret = -1;
                break;
            }
        }
        ...
        {
            wpa_printf(MSG_ERROR, "Invalid MAC address '%s' on line %d in '%s'", token, line, fname);
            ret = -1;
            break;
        }
        ...
        ok  = 0;
        len = os_strlen(pos);
        if (len == 2 * PMK_LEN && hexstr2bin(pos, psk->psk, PMK_LEN) == 0)
            ok = 1;
        else if (len >= 8 && len < 64 && pbkdf2_sha1(pos, ssid->ssid, ssid->ssid_len, 4096, psk->psk, PMK_LEN) == 0)
            ok = 1;
        if (!ok)
        {
        ...
        if (keyid)
        {
            ...
            {
                wpa_printf(MSG_ERROR, "PSK keyid too long on line %d in '%s'", line, fname);
                os_free(psk);
                ret = -1;
                break;
            }
...
static int hostapd_derive_psk(struct hostapd_ssid *ssid)
{
    ...
    wpa_hexdump_ascii_key(MSG_DEBUG, "PSK (ASCII passphrase)", (u8 *)ssid->wpa_passphrase,
...
int hostapd_setup_sae_pt(struct hostapd_bss_config *conf)
{
    ...
    if ((conf->sae_pwe == 0 && !hostapd_sae_pw_id_in_use(conf) && !hostapd_sae_pk_in_use(conf)) || conf->sae_pwe == 3 ||
    ...
    {
        ssid->pt = sae_derive_pt(conf->sae_groups, ssid->ssid, ssid->ssid_len, (const u8 *)ssid->wpa_passphrase,
                                 os_strlen(ssid->wpa_passphrase), NULL);
        if (!ssid->pt)
            return -1;
    }
    ...
    {
        sae_deinit_pt(pw->pt);
        pw->pt = sae_derive_pt(conf->sae_groups, ssid->ssid, ssid->ssid_len, (const u8 *)pw->password,
                               os_strlen(pw->password), pw->identifier);
        if (!pw->pt)
            return -1;
    }
#endif /* CONFIG_SAE */
...
int hostapd_setup_wpa_psk(struct hostapd_bss_config *conf)
{
    ...
    return 0;//hostapd_config_read_wpa_psk(ssid->wpa_psk_file, &conf->ssid);
}
...
static void hostapd_config_free_fils_realms(struct hostapd_bss_config *conf)
{
    ...
    while ((realm = dl_list_first(&conf->fils_realms, struct fils_realm, list)))
    {
 ...
 */
int hostapd_maclist_found(struct mac_acl_entry *list, int num_entries, const u8 *addr, struct vlan_description *vlan_id)
{
...
int hostapd_vlan_valid(struct hostapd_vlan *vlan, struct vlan_description *vlan_desc)
{
    struct hostapd_vlan *v = vlan;
    int i;

    if (!vlan_desc->notempty || vlan_desc->untagged < 0 || vlan_desc->untagged > MAX_VLAN_ID)
        return 0;
    ...
    while (v)
    {
        if (!vlan_compare(&v->vlan_desc, vlan_desc) || v->vlan_id == VLAN_ID_WILDCARD)
            return 1;
        v = v->next;
    }
    return 0;
...
const u8 *hostapd_get_psk(
    const struct hostapd_bss_config *conf, const u8 *addr, const u8 *p2p_dev_addr, const u8 *prev_psk, int *vlan_id)
{
    struct hostapd_wpa_psk *psk;
    ...
    {
        wpa_printf(MSG_DEBUG, "Searching a PSK for " MACSTR " p2p_dev_addr=" MACSTR " prev_psk=%p", MAC2STR(addr),
                   MAC2STR(p2p_dev_addr), prev_psk);
    ...
    {
        if (next_ok && (psk->group || (addr && os_memcmp(psk->addr, addr, ETH_ALEN) == 0) ||
                        (!addr && p2p_dev_addr && os_memcmp(psk->p2p_dev_addr, p2p_dev_addr, ETH_ALEN) == 0)))
        {
            if (vlan_id)
                *vlan_id = psk->vlan_id;
            return psk->psk;
...
static bool hostapd_sae_pk_password_without_pk(struct hostapd_bss_config *bss)
{
    ...
    for (pw = bss->sae_passwords; pw; pw = pw->next)
    {
        ...
        if (bss->ssid.wpa_passphrase && res && pw->pk && os_strcmp(bss->ssid.wpa_passphrase, pw->password) == 0)
            res = false;
    }
...
static bool hostapd_config_check_bss_6g(struct hostapd_bss_config *bss)
{
    ...
    {
        wpa_printf(MSG_ERROR, "Pre-RSNA security methods are not allowed in 6 GHz");
        return false;
    }

    if (bss->ieee80211w != MGMT_FRAME_PROTECTION_REQUIRED)
    {
        wpa_printf(MSG_ERROR, "Management frame protection is required in 6 GHz");
        return false;
    }

    if (bss->wpa_key_mgmt & (WPA_KEY_MGMT_PSK | WPA_KEY_MGMT_FT_PSK | WPA_KEY_MGMT_PSK_SHA256))
    {
        wpa_printf(MSG_ERROR, "Invalid AKM suite for 6 GHz");
        return false;
    }

    if (bss->rsn_pairwise & (WPA_CIPHER_WEP40 | WPA_CIPHER_WEP104 | WPA_CIPHER_TKIP))
    {
        wpa_printf(MSG_ERROR, "Invalid pairwise cipher suite for 6 GHz");
        return false;
    }

    if (bss->wpa_group & (WPA_CIPHER_WEP40 | WPA_CIPHER_WEP104 | WPA_CIPHER_TKIP))
    {
...
{
    if (full_config && is_6ghz_op_class(conf->op_class) && !hostapd_config_check_bss_6g(bss))
        return -1;
    ...
    if (full_config && bss->wpa && (bss->wpa_key_mgmt & WPA_KEY_MGMT_PSK) && bss->ssid.wpa_psk == NULL &&
        bss->ssid.wpa_passphrase == NULL && bss->ssid.wpa_psk_file == NULL &&
    ...
    if (full_config && !is_zero_ether_addr(bss->bssid))
    {
        ...
        for (i = 0; i < conf->num_bss; i++)
        {
            ...
            {
                wpa_printf(MSG_ERROR, "Duplicate BSSID " MACSTR " on interface '%s' and '%s'.", MAC2STR(bss->bssid),
    ...
    if (full_config && conf->ieee80211n && bss->wpa && !(bss->wpa_pairwise & WPA_CIPHER_CCMP) &&
        !(bss->rsn_pairwise & (WPA_CIPHER_CCMP | WPA_CIPHER_GCMP | WPA_CIPHER_CCMP_256 | WPA_CIPHER_GCMP_256)))
    {
        bss->disable_11n = true;
        wpa_printf(MSG_ERROR,
...
#ifdef CONFIG_WEP
    if (full_config && conf->ieee80211ac && bss->ssid.security_policy == SECURITY_STATIC_WEP)
    {
        bss->disable_11ac = true;
        wpa_printf(MSG_ERROR, "VHT (IEEE 802.11ac) with WEP is not allowed, disabling VHT capabilities");
    }
#endif /* CONFIG_WEP */

    if (full_config && conf->ieee80211ac && bss->wpa && !(bss->wpa_pairwise & WPA_CIPHER_CCMP) &&
        !(bss->rsn_pairwise & (WPA_CIPHER_CCMP | WPA_CIPHER_GCMP | WPA_CIPHER_CCMP_256 | WPA_CIPHER_GCMP_256)))
    {
        bss->disable_11ac = true;
        wpa_printf(MSG_ERROR,
...
#ifdef CONFIG_IEEE80211AX
#ifdef CONFIG_WEP
    if (full_config && conf->ieee80211ax && bss->ssid.security_policy == SECURITY_STATIC_WEP)
    {
        bss->disable_11ax = true;
        wpa_printf(MSG_ERROR, "HE (IEEE 802.11ax) with WEP is not allowed, disabling HE capabilities");
    }
#endif /* CONFIG_WEP */

    if (full_config && conf->ieee80211ax && bss->wpa && !(bss->wpa_pairwise & WPA_CIPHER_CCMP) &&
        !(bss->rsn_pairwise & (WPA_CIPHER_CCMP | WPA_CIPHER_GCMP | WPA_CIPHER_CCMP_256 | WPA_CIPHER_GCMP_256)))
    {
        bss->disable_11ax = true;
        wpa_printf(MSG_ERROR,
...
#ifdef CONFIG_MBO
    if (full_config && bss->mbo_enabled && (bss->wpa & 2) && bss->ieee80211w == NO_MGMT_FRAME_PROTECTION)
    {
        wpa_printf(MSG_ERROR, "MBO: PMF needs to be enabled whenever using WPA2 with MBO");
        return -1;
    }
#endif /* CONFIG_MBO */

#ifdef CONFIG_OCV
    if (full_config && bss->ieee80211w == NO_MGMT_FRAME_PROTECTION && bss->ocv)
    {
        wpa_printf(MSG_ERROR, "OCV: PMF needs to be enabled whenever using OCV");
        return -1;
    }
#endif /* CONFIG_OCV */

#ifdef CONFIG_SAE_PK
    if (full_config && hostapd_sae_pk_in_use(bss) && hostapd_sae_pk_password_without_pk(bss))
    {
        wpa_printf(MSG_ERROR, "SAE-PK: SAE password uses SAE-PK style, but does not have PK configured");
        return -1;
    }
#endif /* CONFIG_SAE_PK */
...
static int hostapd_config_check_cw(struct hostapd_config *conf, int queue)
{
    ...
    {
        wpa_printf(MSG_ERROR, "Invalid TX queue cwMin/cwMax values. cwMin(%d) greater than cwMax(%d)", tx_cwmin,
                   tx_cwmax);
        return -1;
    }
    if (ac_cwmin > ac_cwmax)
    {
        wpa_printf(MSG_ERROR, "Invalid WMM AC cwMin/cwMax values. cwMin(%d) greater than cwMax(%d)", ac_cwmin,
                   ac_cwmax);
        return -1;
    }
    return 0;
...
void hostapd_set_security_params(struct hostapd_bss_config *bss, int full_config)
{
    ...
    if (!bss->wpa_group_rekey_set)
        bss->wpa_group_rekey = bss->wpa_group == WPA_CIPHER_TKIP ? 600 : 86400;
