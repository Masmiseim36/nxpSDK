                               ...
                               struct wpabuf **assocresp_ret)
{
    ...
    *beacon_ret = *proberesp_ret = *assocresp_ret = NULL;
    ...
    if (add_buf_data(&beacon, buf, pos - buf) < 0 || add_buf_data(&proberesp, buf, pos - buf) < 0)
        goto fail;

#ifdef CONFIG_FST
    if (add_buf(&beacon, hapd->iface->fst_ies) < 0 || add_buf(&proberesp, hapd->iface->fst_ies) < 0 ||
    ...
    if (add_buf(&beacon, hapd->wps_beacon_ie) < 0 || add_buf(&proberesp, hapd->wps_probe_resp_ie) < 0)
        goto fail;

#ifdef CONFIG_P2P
    if (add_buf(&beacon, hapd->p2p_beacon_ie) < 0 || add_buf(&proberesp, hapd->p2p_probe_resp_ie) < 0)
        goto fail;
#endif /* CONFIG_P2P */
...
#ifdef CONFIG_MBO
    if (hapd->conf->mbo_enabled || OCE_STA_CFON_ENABLED(hapd) || OCE_AP_ENABLED(hapd))
    {
...
int hostapd_set_sta_flags(struct hostapd_data *hapd, struct sta_info *sta)
{
    ...
    if (((!hapd->conf->ieee802_1x && !hapd->conf->wpa) || sta->auth_alg == WLAN_AUTH_FT) &&
...
int hostapd_sta_auth(struct hostapd_data *hapd, const u8 *addr, u16 seq, u16 status, const u8 *ie, size_t len)
{
    ...
    if (!sta)
    {
        wpa_printf(MSG_DEBUG, "Station " MACSTR " not found for sta_auth processing", MAC2STR(addr));
        return 0;
    }
    ...
    {
        params.fils_auth = 1;
        wpa_auth_get_fils_aead_params(sta->wpa_sm, params.fils_anonce, params.fils_snonce, params.fils_kek,
                                      &params.fils_kek_len);
    }
#endif /* CONFIG_FILS */
                   ...
                   int use_existing)
{
    if (hapd->driver == NULL || hapd->driver->if_add == NULL)
        return -1;
    return hapd->driver->if_add(hapd->drv_priv, type, ifname, addr, bss_ctx, drv_priv, force_ifname, if_addr, bridge,
                                use_existing, 1);
}
                     ...
                     int center_segment1)
{
    ...
    if (hostapd_set_freq_params(&data, mode, freq, channel, edmg, edmg_channel, ht_enabled, vht_enabled, he_enabled,
                                sec_channel_offset, oper_chwidth, center_segment0, center_segment1,
                                cmode ? cmode->vht_capab : 0, cmode ? &cmode->he_capab[IEEE80211_MODE_AP] : NULL))
        return -1;
...
int hostapd_sta_set_airtime_weight(struct hostapd_data *hapd, const u8 *addr, unsigned int weight)
{
                                                     ...
                                                     u8 *dfs_domain)
{
    if (hapd->driver == NULL || hapd->driver->get_hw_feature_data == NULL)
        return NULL;
    return hapd->driver->get_hw_feature_data(hapd->drv_priv, num_modes, flags, dfs_domain);
}
                          ...
                          int no_encrypt)
{
    if (!hapd->driver || !hapd->driver->send_mlme || !hapd->drv_priv)
        return 0;
    return hapd->driver->send_mlme(hapd->drv_priv, msg, len, noack, 0, csa_offs, csa_offs_len, no_encrypt, 0);
}
...
int hostapd_drv_send_action(
    struct hostapd_data *hapd, unsigned int freq, unsigned int wait, const u8 *dst, const u8 *data, size_t len)
{
    const u8 *bssid;
    ...
    if (!hapd->driver || !hapd->driver->send_action || !hapd->drv_priv)
        return 0;
    bssid = hapd->own_addr;
    if (!is_multicast_ether_addr(dst) && len > 0 && data[0] == WLAN_ACTION_PUBLIC)
    {
        struct sta_info *sta;
    ...
    else if (is_broadcast_ether_addr(dst) && len > 0 && data[0] == WLAN_ACTION_PUBLIC)
    {
        /*
    ...
    return hapd->driver->send_action(hapd->drv_priv, freq, wait, dst, hapd->own_addr, bssid, data, len, 0);
}

int hostapd_drv_send_action_addr3_ap(
    struct hostapd_data *hapd, unsigned int freq, unsigned int wait, const u8 *dst, const u8 *data, size_t len)
{
                          ...
                          int center_segment1)
{
    ...
    if (hostapd_set_freq_params(&data, mode, freq, channel, 0, 0, ht_enabled, vht_enabled, he_enabled,
                                sec_channel_offset, oper_chwidth, center_segment0, center_segment1, cmode->vht_capab,
                                             ...
                                             int **freq_list)
{
    ...
    for (i = 0; i < mode->num_channels; i++)
    {
        ...
        if (!acs_ch_list_all && (hapd->iface->conf->acs_freq_list.num &&
                                 !freq_range_list_includes(&hapd->iface->conf->acs_freq_list, chan->freq)))
            continue;
        if (!acs_ch_list_all && (!hapd->iface->conf->acs_freq_list_present && hapd->iface->conf->acs_ch_list.num &&
                                 !freq_range_list_includes(&hapd->iface->conf->acs_ch_list, chan->chan)))
            continue;
        if (is_6ghz_freq(chan->freq) && hapd->iface->conf->acs_exclude_6ghz_non_psc &&
            !is_6ghz_psc_frequency(chan->freq))
            continue;
        if (!(chan->flag & HOSTAPD_CHAN_DISABLED) &&
            !(hapd->iface->conf->acs_exclude_dfs && (chan->flag & HOSTAPD_CHAN_RADAR)) &&
...
void hostapd_get_ext_capa(struct hostapd_iface *iface)
{
    ...
    hapd->driver->get_ext_capab(hapd->drv_priv, WPA_IF_AP_BSS, &iface->extended_capa, &iface->extended_capa_mask,
...
int hostapd_drv_do_acs(struct hostapd_data *hapd)
{
    ...
    for (i = 0; i < hapd->iface->num_hw_features; i++)
    {
        ...
        if (selected_mode != HOSTAPD_MODE_IEEE80211ANY && selected_mode != mode->mode)
            continue;
     ...
     */
    if ((hapd->iface->conf->ieee80211ax || hapd->iface->conf->ieee80211ac) && params.ht40_enabled)
    {
    ...
    if (hapd->iface->conf->op_class)
        params.ch_width = op_class_to_bandwidth(hapd->iface->conf->op_class);
...
int hostapd_drv_update_dh_ie(struct hostapd_data *hapd, const u8 *peer, u16 reason_code, const u8 *ie, size_t ielen)
{
    if (!hapd->driver || !hapd->driver->update_dh_ie || !hapd->drv_priv)
        return 0;
    return hapd->driver->update_dh_ie(hapd->drv_priv, peer, reason_code, ie, ielen);
}
