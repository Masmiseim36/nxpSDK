...
static u8 *hostapd_eid_country_add(
    u8 *pos, u8 *end, int chan_spacing, struct hostapd_channel_data *start, struct hostapd_channel_data *prev)
{
    if (end - pos < 3)
        return pos;
...
static const u8 *hostapd_vendor_wpa_ie(struct hostapd_data *hapd, u32 vendor_type)
{
    const u8 *ies;
    size_t ies_len;
...
static u8 *hostapd_eid_supported_op_classes(struct hostapd_data *hapd, u8 *eid)
{
    ...
    if (ieee80211_freq_to_channel_ext(hapd->iface->freq, hapd->iconf->secondary_channel,
...
#define MAX_PROBERESP_LEN 768
    ...
    {
        buflen += 3 + sizeof(struct ieee80211_he_capabilities) + 3 + sizeof(struct ieee80211_he_operation) + 3 +
                  sizeof(struct ieee80211_he_mu_edca_parameter_set) + 3 + sizeof(struct ieee80211_spatial_reuse);
        if (is_6ghz_op_class(hapd->iconf->op_class))
            buflen += sizeof(struct ieee80211_he_6ghz_oper_info) + 3 + sizeof(struct ieee80211_he_6ghz_band_cap);
    }
    ...
    *pos++ = hapd->conf->ssid.ssid_len;
    ...
    if (hapd->iface->fst_ies)
    {
        os_memcpy(pos, wpabuf_head(hapd->iface->fst_ies), wpabuf_len(hapd->iface->fst_ies));
...
#ifdef CONFIG_IEEE80211AX
    if (hapd->iconf->ieee80211ax && !hapd->conf->disable_11ax && is_6ghz_op_class(hapd->iconf->op_class))
        pos = hostapd_eid_txpower_envelope(hapd, pos);
#endif /* CONFIG_IEEE80211AX */
                                         ...
                                         size_t short_ssid_list_len)
{
    ...
    if (!iface->interfaces || iface->interfaces->count <= 1 || is_6ghz_op_class(hapd->iconf->op_class))
    ...
    for (i = 0; i < iface->interfaces->count; i++)
    {
        ...
        if (colocated == iface || !is_6ghz_op_class(colocated->conf->op_class))
            continue;
        ...
        for (j = 0; j < colocated->num_bss; j++)
        {
            ...
            if (ssid_len == conf->ssid.ssid_len && os_memcmp(ssid, conf->ssid.ssid, ssid_len) == 0)
...
void sta_track_expire(struct hostapd_iface *iface, int force)
{
    ...
    {
        if (!force && !os_reltime_expired(&now, &info->last_seen, iface->conf->track_sta_max_age))
            break;
        force = 0;

        wpa_printf(MSG_MSGDUMP, "%s: Expire STA tracking entry for " MACSTR, iface->bss[0]->conf->iface,
                   MAC2STR(info->addr));
...
static struct hostapd_sta_info *sta_track_get(struct hostapd_iface *iface, const u8 *addr)
{
    ...
    dl_list_for_each(info, &iface->sta_seen, struct hostapd_sta_info,
                     list) if (os_memcmp(addr, info->addr, ETH_ALEN) == 0) return info;
...
void sta_track_add(struct hostapd_iface *iface, const u8 *addr, int ssi_signal)
{
    ...
    wpa_printf(MSG_MSGDUMP, "%s: Add STA tracking entry for " MACSTR, iface->bss[0]->conf->iface, MAC2STR(addr));
    dl_list_add_tail(&iface->sta_seen, &info->list);
    iface->num_sta_seen++;
}

struct hostapd_data *sta_track_seen_on(struct hostapd_iface *iface, const u8 *addr, const char *ifname)
{
...
void sta_track_claim_taxonomy_info(struct hostapd_iface *iface, const u8 *addr, struct wpabuf **probe_ie_taxonomy)
{
    struct hostapd_sta_info *info;
...
void handle_probe_req(struct hostapd_data *hapd, const struct ieee80211_mgmt *mgmt, size_t len, int ssi_signal)
{
    ...
    if (hapd->iconf->rssi_ignore_probe_request && ssi_signal && ssi_signal < hapd->iconf->rssi_ignore_probe_request)
        return;
    ...
    ret = hostapd_allowed_address(hapd, mgmt->sa, (const u8 *)mgmt, len, &rad_info, 1);
    if (ret == HOSTAPD_ACL_REJECT)
    {
        wpa_msg(hapd->msg_ctx, MSG_DEBUG, "Ignore Probe Request frame from " MACSTR " due to ACL reject ",
                MAC2STR(mgmt->sa));
        return;
    }

    for (i = 0; hapd->probereq_cb && i < hapd->num_probereq_cb; i++)
        if (hapd->probereq_cb[i].cb(hapd->probereq_cb[i].ctx, mgmt->sa, mgmt->da, mgmt->bssid, ie, ie_len, ssi_signal) >
            0)
            return;
    ...
    {
        wpa_printf(MSG_DEBUG, "Ignore Probe Request due to DS Params mismatch: chan=%u != ds.chan=%u",
                   hapd->iconf->channel, elems.ds_params[0]);
        return;
    }
...
#ifdef CONFIG_P2P
    if ((hapd->conf->p2p & P2P_GROUP_OWNER) && elems.ssid_len == P2P_WILDCARD_SSID_LEN &&
    ...
    res = ssid_match(hapd, elems.ssid, elems.ssid_len, elems.ssid_list, elems.ssid_list_len, elems.short_ssid_list,
                     elems.short_ssid_list_len);
    if (res == NO_SSID_MATCH)
    {
        if (!(mgmt->da[0] & 0x01))
        {
            wpa_printf(MSG_MSGDUMP, "Probe Request from " MACSTR " for foreign SSID '%s' (DA " MACSTR ")%s",
                       MAC2STR(mgmt->sa), wpa_ssid_txt(elems.ssid, elems.ssid_len), MAC2STR(mgmt->da),
    ...
    if (hapd->conf->interworking && elems.interworking && elems.interworking_len >= 1)
    {
        ...
        {
            wpa_printf(MSG_MSGDUMP, "Probe Request from " MACSTR " for mismatching ANT %u ignored", MAC2STR(mgmt->sa),
                       ant);
            return;
        }
    }
    ...
    if (hapd->conf->interworking && elems.interworking && (elems.interworking_len == 7 || elems.interworking_len == 9))
    {
        ...
        {
            wpa_printf(MSG_MSGDUMP, "Probe Request from " MACSTR " for mismatching HESSID " MACSTR " ignored",
    ...
    if ((hapd->conf->p2p & P2P_GROUP_OWNER) && supp_rates_11b_only(&elems))
    {
        ...
        wpa_printf(MSG_EXCESSIVE, "P2P: Ignore Probe Request from " MACSTR " with only 802.11b rates",
                   MAC2STR(mgmt->sa));
        return;
    }
#endif /* CONFIG_P2P */
    ...
    if (hapd->conf->no_probe_resp_if_seen_on && is_multicast_ether_addr(mgmt->da) &&
        is_multicast_ether_addr(mgmt->bssid) &&
        sta_track_seen_on(hapd->iface, mgmt->sa, hapd->conf->no_probe_resp_if_seen_on))
    {
        wpa_printf(MSG_MSGDUMP, "%s: Ignore Probe Request from " MACSTR " since STA has been seen on %s",
                   hapd->conf->iface, MAC2STR(mgmt->sa), hapd->conf->no_probe_resp_if_seen_on);
        return;
    }

    if (hapd->conf->no_probe_resp_if_max_sta && is_multicast_ether_addr(mgmt->da) &&
        is_multicast_ether_addr(mgmt->bssid) && hapd->num_sta >= hapd->conf->max_num_sta && !ap_get_sta(hapd, mgmt->sa))
    {
        wpa_printf(MSG_MSGDUMP, "%s: Ignore Probe Request from " MACSTR " since no room for additional STA",
    ...
    wpa_msg_ctrl(hapd->msg_ctx, MSG_INFO, RX_PROBE_REQUEST "sa=" MACSTR " signal=%d", MAC2STR(mgmt->sa), ssi_signal);
    ...
    ret = hostapd_drv_send_mlme(hapd, resp, resp_len, noack, csa_offs_len ? csa_offs : NULL, csa_offs_len, 0);
...
static u8 *hostapd_probe_resp_offloads(struct hostapd_data *hapd, size_t *resp_len)
{
    ...
    if (hapd->conf->wps_state && hapd->wps_probe_resp_ie &&
        (!(hapd->iface->probe_resp_offloads &
           (WPA_DRIVER_PROBE_RESP_OFFLOAD_WPS | WPA_DRIVER_PROBE_RESP_OFFLOAD_WPS2))))
...
static u8 *hostapd_unsol_bcast_probe_resp(struct hostapd_data *hapd, struct wpa_driver_ap_params *params)
{
    if (!is_6ghz_op_class(hapd->iconf->op_class))
        return NULL;

    params->unsol_bcast_probe_resp_interval = hapd->conf->unsol_bcast_probe_resp_interval;
...
static u16 hostapd_fils_discovery_cap(struct hostapd_data *hapd)
{
...
#endif /* CONFIG_IEEE80211AX */
#ifdef CONFIG_IEEE80211AC
        if (!phy_index && hapd->iconf->ieee80211ac && !hapd->conf->disable_11ac)
            phy_index = FD_CAP_PHY_INDEX_VHT;
#endif /* CONFIG_IEEE80211AC */
        if (!phy_index && hapd->iconf->ieee80211n && !hapd->conf->disable_11n)
    ...
    if (mode)
    {
        ...
        for (i = 0; i < HE_NSS_MAX_STREAMS; i++)
        {
            ...
            if (mcs_nss_size == 4 && (((mcs[0] & nss_mask) == nss_mask) || ((mcs[1] & nss_mask) == nss_mask)))
                continue;

            if (mcs_nss_size == 8 && (((mcs[2] & nss_mask) == nss_mask) || ((mcs[3] & nss_mask) == nss_mask)))
                continue;

            if (mcs_nss_size == 12 && (((mcs[4] & nss_mask) == nss_mask) || ((mcs[5] & nss_mask) == nss_mask)))
                continue;
...
static u8 *hostapd_gen_fils_discovery(struct hostapd_data *hapd, size_t *len)
{
    ...
    ctl = (sizeof(hapd->conf->ssid.short_ssid) - 1) | FD_FRAME_CTL_SHORT_SSID_PRESENT | FD_FRAME_CTL_LENGTH_PRESENT |
    ...
    head->frame_control = IEEE80211_FC(WLAN_FC_TYPE_MGMT, WLAN_FC_STYPE_ACTION);
    ...
    *len = pos - (u8 *)head;
    wpa_hexdump(MSG_DEBUG, "FILS Discovery frame template", head, pos - (u8 *)head);
    return (u8 *)head;
}

/* Configure FILS Discovery frame transmission parameters */
static u8 *hostapd_fils_discovery(struct hostapd_data *hapd, struct wpa_driver_ap_params *params)
{
    params->fd_max_int = hapd->conf->fils_discovery_max_int;
    if (is_6ghz_op_class(hapd->iconf->op_class) && params->fd_max_int > FD_MAX_INTERVAL_6GHZ)
    ...
    if (params->fd_max_int)
        return hostapd_gen_fils_discovery(hapd, &params->fd_frame_tmpl_len);
...
#define BEACON_TAIL_BUF_SIZE 512
    ...
    if (hapd->iconf->ieee80211ax && !hapd->conf->disable_11ax)
    {
        tail_len += 3 + sizeof(struct ieee80211_he_capabilities) + 3 + sizeof(struct ieee80211_he_operation) + 3 +
                    sizeof(struct ieee80211_he_mu_edca_parameter_set) + 3 + sizeof(struct ieee80211_spatial_reuse);
        if (is_6ghz_op_class(hapd->iconf->op_class))
            tail_len += sizeof(struct ieee80211_he_6ghz_oper_info) + 3 + sizeof(struct ieee80211_he_6ghz_band_cap);
    }
#endif /* CONFIG_IEEE80211AX */
    ...
    *pos++ = WLAN_EID_SSID;
    ...
    tailpos = hostapd_eid_rm_enabled_capab(hapd, tailpos, tailend - tailpos);
    ...
    if (hapd->iface->fst_ies)
    {
        os_memcpy(tailpos, wpabuf_head(hapd->iface->fst_ies), wpabuf_len(hapd->iface->fst_ies));
    ...
    tailpos = hostapd_eid_owe_trans(hapd, tailpos, tail + tail_len - tailpos);
...
#ifdef CONFIG_WEP
    params->privacy |=
        hapd->conf->ssid.wep.keys_set ||
        (hapd->conf->ieee802_1x && (hapd->conf->default_wep_key_len || hapd->conf->individual_wep_key_len));
#endif /* CONFIG_WEP */
    ...
    if (hapd->conf->ftm_responder)
    {
        ...
        {
            wpa_printf(MSG_WARNING, "Not configuring FTM responder as the driver doesn't advertise support for it");
        }
    }
...
static int __ieee802_11_set_beacon(struct hostapd_data *hapd)
{
    ...
    params.he_spr_non_srg_obss_pd_max_offset = hapd->iface->conf->spr.non_srg_obss_pd_max_offset;
    ...
    os_memcpy(params.he_spr_bss_color_bitmap, hapd->iface->conf->spr.srg_bss_color_bitmap, 8);
    os_memcpy(params.he_spr_partial_bssid_bitmap, hapd->iface->conf->spr.srg_partial_bssid_bitmap, 8);
    params.he_bss_color_disabled       = hapd->iface->conf->he_op.he_bss_color_disabled;
    params.he_bss_color_partial        = hapd->iface->conf->he_op.he_bss_color_partial;
    ...
    params.unsol_bcast_probe_resp_tmpl = hostapd_unsol_bcast_probe_resp(hapd, &params);
#endif /* CONFIG_IEEE80211AX */
    ...
    if (cmode && hostapd_set_freq_params(&freq, iconf->hw_mode, iface->freq, iconf->channel, iconf->enable_edmg,
                                         iconf->edmg_channel, iconf->ieee80211n, iconf->ieee80211ac, iconf->ieee80211ax,
                                         iconf->secondary_channel, hostapd_get_oper_chwidth(iconf),
                                         hostapd_get_oper_centr_freq_seg0_idx(iconf),
                                         hostapd_get_oper_centr_freq_seg1_idx(iconf), cmode->vht_capab,
