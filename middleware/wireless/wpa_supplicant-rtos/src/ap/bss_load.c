...
static int get_bss_load_update_timeout(struct hostapd_data *hapd, unsigned int *sec, unsigned int *usec)
{
    ...
    {
        wpa_printf(MSG_ERROR, "BSS Load: Invalid BSS load update configuration (period=%u beacon_int=%u)",
...
static void update_channel_utilization(void *eloop_data, void *user_data)
{
    ...
    if (hapd->conf->chan_util_avg_period)
    {
        ...
        {
            iface->chan_util_average = iface->chan_util_samples_sum /
                                       (iface->chan_util_num_sample_periods / hapd->conf->bss_load_update_period);
