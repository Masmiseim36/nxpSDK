...
static size_t hostapd_write_ht_mcs_bitmask(char *buf, size_t buflen, size_t curr_len, const u8 *mcs_set)
{
    int ret;
    size_t len = curr_len;

    ret = os_snprintf(buf + len, buflen - len, "ht_mcs_bitmask=");
...
static int hostapd_get_sta_info(struct hostapd_data *hapd, struct sta_info *sta, char *buf, size_t buflen)
{
    struct hostap_sta_driver_data data;
    int ret;
    int len = 0;
    ...
    ret = os_snprintf(buf, buflen,
                      ...
                      "rx_bytes=%llu\ntx_bytes=%llu\ninactive_msec=%lu\n"
                      "signal=%d\n",
                      data.rx_packets, data.tx_packets, data.rx_bytes, data.tx_bytes, data.inactive_msec, data.signal);
    if (os_snprintf_error(buflen, ret))
        return 0;
    len += ret;

    ret = os_snprintf(buf + len, buflen - len, "rx_rate_info=%lu", data.current_rx_rate);
    if (os_snprintf_error(buflen - len, ret))
        return len;
    len += ret;
    if (data.flags & STA_DRV_DATA_RX_MCS)
    {
        ret = os_snprintf(buf + len, buflen - len, " mcs %u", data.rx_mcs);
        if (!os_snprintf_error(buflen - len, ret))
            len += ret;
    }
    if (data.flags & STA_DRV_DATA_RX_VHT_MCS)
    {
        ret = os_snprintf(buf + len, buflen - len, " vhtmcs %u", data.rx_vhtmcs);
        if (!os_snprintf_error(buflen - len, ret))
            len += ret;
    }
    if (data.flags & STA_DRV_DATA_RX_VHT_NSS)
    {
        ret = os_snprintf(buf + len, buflen - len, " vhtnss %u", data.rx_vht_nss);
        if (!os_snprintf_error(buflen - len, ret))
            len += ret;
    }
    ...
    ret = os_snprintf(buf + len, buflen - len, "tx_rate_info=%lu", data.current_tx_rate);
    if (os_snprintf_error(buflen - len, ret))
        return len;
    len += ret;
    if (data.flags & STA_DRV_DATA_TX_MCS)
    {
        ret = os_snprintf(buf + len, buflen - len, " mcs %u", data.tx_mcs);
        if (!os_snprintf_error(buflen - len, ret))
            len += ret;
    }
    if (data.flags & STA_DRV_DATA_TX_VHT_MCS)
    {
        ret = os_snprintf(buf + len, buflen - len, " vhtmcs %u", data.tx_vhtmcs);
        if (!os_snprintf_error(buflen - len, ret))
            len += ret;
    }
    if (data.flags & STA_DRV_DATA_TX_VHT_NSS)
    {
        ret = os_snprintf(buf + len, buflen - len, " vhtnss %u", data.tx_vht_nss);
        if (!os_snprintf_error(buflen - len, ret))
            len += ret;
    }
    ...
    {
        len = hostapd_write_ht_mcs_bitmask(buf, buflen, len, sta->ht_capabilities->supported_mcs_set);
    }
...
static int hostapd_ctrl_iface_sta_mib(struct hostapd_data *hapd, struct sta_info *sta, char *buf, size_t buflen)
{
    ...
    {
        ret = os_snprintf(buf + len, buflen - len, "%02x%s", sta->supported_rates[i],
    ...
    if (sta->sae && sta->sae->tmp)
    {
        ...
        res = os_snprintf(buf + len, buflen - len, "sae_rejected_groups=");
        if (!os_snprintf_error(buflen - len, res))
            len += res;
        ...
        {
            res = os_snprintf(buf + len, buflen - len, "%s%d", j == 0 ? "" : " ", WPA_GET_LE16(pos));
            if (!os_snprintf_error(buflen - len, res))
                len += res;
            pos += 2;
        }
    ...
    if (sta->supp_op_classes && buflen - len > (unsigned)(17 + 2 * sta->supp_op_classes[0]))
    {
        len += os_snprintf(buf + len, buflen - len, "supp_op_classes=");
        len += wpa_snprintf_hex(buf + len, buflen - len, sta->supp_op_classes + 1, sta->supp_op_classes[0]);
    ...
    if ((sta->flags & WLAN_STA_VHT) && sta->vht_capabilities)
    {
        res = os_snprintf(buf + len, buflen - len, "vht_caps_info=0x%08lx\n",
    ...
    if (sta->ext_capability && buflen - len > (unsigned)(11 + 2 * sta->ext_capability[0]))
    {
        len += os_snprintf(buf + len, buflen - len, "ext_capab=");
        len += wpa_snprintf_hex(buf + len, buflen - len, sta->ext_capability + 1, sta->ext_capability[0]);
        len += os_snprintf(buf + len, buflen - len, "\n");
    }

    if (sta->flags & WLAN_STA_WDS && sta->ifname_wds)
    {
        ret = os_snprintf(buf + len, buflen - len, "wds_sta_ifname=%s\n", sta->ifname_wds);
        if (!os_snprintf_error(buflen - len, ret))
            len += ret;
    }
...
static int p2p_manager_disconnect(struct hostapd_data *hapd, u16 stype, u8 minor_reason_code, const u8 *addr)
{
    ...
    wpa_dbg(hapd->msg_ctx, MSG_DEBUG, "P2P: Disconnect STA " MACSTR " with minor reason code %u (stype=%u (%s))",
            MAC2STR(addr), minor_reason_code, stype, fc2str(le_to_host16(mgmt->frame_control)));
    ...
    *pos++ = minor_reason_code;
    ...
    ret = hostapd_drv_send_mlme(hapd, mgmt, pos - (u8 *)mgmt, 0, NULL, 0, 0);
    os_free(mgmt);
...
int hostapd_ctrl_iface_deauthenticate(struct hostapd_data *hapd, const char *txtaddr)
{
    ...
    if (pos)
    {
        ...
        if (hostapd_drv_send_mlme(hapd, (u8 *)&mgmt, IEEE80211_HDRLEN + sizeof(mgmt.u.deauth), 0, NULL, 0, !encrypt) <
            0)
            return -1;
        return 0;
    }
...
int hostapd_ctrl_iface_signature(struct hostapd_data *hapd, const char *txtaddr, char *buf, size_t buflen)
{
    u8 addr[ETH_ALEN];
    struct sta_info *sta;
...
int hostapd_ctrl_iface_poll_sta(struct hostapd_data *hapd, const char *txtaddr)
{
    ...
    hostapd_drv_poll_client(hapd, hapd->own_addr, addr, sta->flags & WLAN_STA_WMM);
    return 0;
}
...
int hostapd_ctrl_iface_status(struct hostapd_data *hapd, char *buf, size_t buflen)
{
    ...
    ret = os_snprintf(buf + len, buflen - len,
                      ...
                      "olbc_ht=%d\n"
                      "ht_op_mode=0x%x\n",
                      hostapd_state_text(iface->state), iface->phy, iface->freq, iface->num_sta_non_erp,
                      iface->num_sta_no_short_slot_time, iface->num_sta_no_short_preamble, iface->olbc,
                      iface->num_sta_ht_no_gf, iface->num_sta_no_ht, iface->num_sta_ht_20mhz,
                      iface->num_sta_ht40_intolerant, iface->olbc_ht, iface->ht_op_mode);
    if (os_snprintf_error(buflen - len, ret))
        return len;
    len += ret;
    ...
    ret = os_snprintf(
        ...
        "beacon_int=%u\n"
        "dtim_period=%d\n",
        iface->conf->channel, iface->conf->enable_edmg, iface->conf->edmg_channel,
        iface->conf->ieee80211n && !hapd->conf->disable_11n ? iface->conf->secondary_channel : 0,
        iface->conf->ieee80211n && !hapd->conf->disable_11n, iface->conf->ieee80211ac && !hapd->conf->disable_11ac,
        iface->conf->ieee80211ax && !hapd->conf->disable_11ax, iface->conf->beacon_int, hapd->conf->dtim_period);
    if (os_snprintf_error(buflen - len, ret))
        return len;
    len += ret;
    ...
    if (iface->conf->ieee80211ax && !hapd->conf->disable_11ax)
    {
        ret = os_snprintf(buf + len, buflen - len,
                          ...
                          iface->conf->he_oper_chwidth, iface->conf->he_oper_centr_freq_seg0_idx,
    ...
    if (iface->conf->ieee80211ac && !hapd->conf->disable_11ac)
    {
        ret = os_snprintf(buf + len, buflen - len,
                          ...
                          iface->conf->vht_oper_centr_freq_seg1_idx, iface->conf->vht_capab);
        if (os_snprintf_error(buflen - len, ret))
            return len;
        len += ret;
    }
    ...
    {
        ret = os_snprintf(buf + len, buflen - len, "ht_caps_info=%04x\n", hapd->iconf->ht_capab);
        if (os_snprintf_error(buflen - len, ret))
            return len;
        len += ret;
    }
    ...
    {
        len = hostapd_write_ht_mcs_bitmask(buf, buflen, len, mode->mcs_set);
    }
    ...
    if (iface->current_rates && iface->num_rates)
    {
        ...
        {
            ret = os_snprintf(buf + len, buflen - len, "%s%02x", j > 0 ? " " : "", iface->current_rates[j].rate / 5);
            if (os_snprintf_error(buflen - len, ret))
                return len;
            len += ret;
        }
    ...
    for (i = 0; i < iface->num_bss; i++)
    {
        ...
        ret                      = os_snprintf(buf + len, buflen - len,
                          ...
                          wpa_ssid_txt(bss->conf->ssid.ssid, bss->conf->ssid.ssid_len), (int)i, bss->num_sta);
        if (os_snprintf_error(buflen - len, ret))
            return len;
        len += ret;
    }
...
int hostapd_ctrl_iface_pmksa_list(struct hostapd_data *hapd, char *buf, size_t len)
{
...
int hostapd_ctrl_iface_pmksa_add(struct hostapd_data *hapd, char *cmd)
{
    ...
    if (!pos2)
        return -1;
    pmk_len = (pos2 - pos) / 2;
    if (pmk_len < PMK_LEN || pmk_len > PMK_LEN_MAX || hexstr2bin(pos, pmk, pmk_len) < 0)
        return -1;
    ...
    return wpa_auth_pmksa_add2(hapd->wpa_auth, spa, pmk, pmk_len, pmkid, expiration, akmp);
}

#ifdef CONFIG_PMKSA_CACHE_EXTERNAL
#ifdef CONFIG_MESH

int hostapd_ctrl_iface_pmksa_list_mesh(struct hostapd_data *hapd, const u8 *addr, char *buf, size_t len)
{
