...
struct eth_p_oui_ctx
{
    ...
    void (*rx_callback)(void *ctx, const u8 *src_addr, const u8 *dst_addr, u8 oui_suffix, const u8 *buf, size_t len);
    void *rx_callback_ctx;
};

void eth_p_oui_deliver(struct eth_p_oui_ctx *ctx, const u8 *src_addr, const u8 *dst_addr, const u8 *buf, size_t len)
{
    ctx->rx_callback(ctx->rx_callback_ctx, src_addr, dst_addr, ctx->oui_suffix, buf, len);
}
...
static void eth_p_rx(void *ctx, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    dl_list_for_each(receiver, &iface->receiver, struct eth_p_oui_ctx, list)
    {
        if (buf[0] != receiver->oui_suffix)
            continue;

        eth_p_oui_deliver(receiver, ethhdr->h_source, ethhdr->h_dest, buf + 1, len - 1);
    }
}

struct eth_p_oui_ctx *eth_p_oui_register(
    ...
    void *rx_callback_ctx)
{
    ...
    dl_list_for_each(iface, &interfaces->eth_p_oui, struct eth_p_oui_iface, list)
    {
    ...
    if (!found)
    {
        ...
        iface->l2 = l2_packet_init(ifname, NULL, ETH_P_OUI, eth_p_rx, iface, 1);
        if (!iface->l2)
        {
            os_free(iface);
            goto err;
