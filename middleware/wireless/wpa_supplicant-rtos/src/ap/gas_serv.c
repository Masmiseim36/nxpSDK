...
static struct gas_dialog_info *gas_dialog_create(struct hostapd_data *hapd, const u8 *addr, u8 dialog_token)
{
    ...
    if (!sta)
    {
         ...
         */
        ap_sta_session_timeout(hapd, sta, hapd->conf->gas_comeback_delay / 1024 + 5);
    }
    else
    {
...
static struct anqp_element *get_anqp_elem(struct hostapd_data *hapd, u16 infoid)
{
...
static void anqp_add_elem(struct hostapd_data *hapd, struct wpabuf *buf, u16 infoid)
{
    struct anqp_element *elem;
...
static int anqp_add_override(struct hostapd_data *hapd, struct wpabuf *buf, u16 infoid)
{
...
static void anqp_add_capab_list(struct hostapd_data *hapd, struct wpabuf *buf)
{
    ...
    if (hapd->conf->roaming_consortium || get_anqp_elem(hapd, ANQP_ROAMING_CONSORTIUM))
        wpabuf_put_le16(buf, ANQP_ROAMING_CONSORTIUM);
    if (hapd->conf->ipaddr_type_configured || get_anqp_elem(hapd, ANQP_IP_ADDR_TYPE_AVAILABILITY))
        wpabuf_put_le16(buf, ANQP_IP_ADDR_TYPE_AVAILABILITY);
    if (hapd->conf->nai_realm_data || get_anqp_elem(hapd, ANQP_NAI_REALM))
        wpabuf_put_le16(buf, ANQP_NAI_REALM);
    if (hapd->conf->anqp_3gpp_cell_net || get_anqp_elem(hapd, ANQP_3GPP_CELLULAR_NETWORK))
...
#ifdef CONFIG_FILS
    if (!dl_list_empty(&hapd->conf->fils_realms) || get_anqp_elem(hapd, ANQP_FILS_REALM_INFO))
...
static void anqp_add_nai_realm(struct hostapd_data *hapd,
                               struct wpabuf *buf,
                               const u8 *home_realm,
                               size_t home_realm_len,
                               int nai_realm,
                               int nai_home_realm)
{
    if (nai_realm && !nai_home_realm && anqp_add_override(hapd, buf, ANQP_NAI_REALM))
        return;
...
#ifdef CONFIG_FILS
static void anqp_add_fils_realm_info(struct hostapd_data *hapd, struct wpabuf *buf)
{
    size_t count;
    ...
    if (count)
    {
        ...
        dl_list_for_each(realm, &hapd->conf->fils_realms, struct fils_realm, list)
        {
            if (count == 0)
                break;
...
static void anqp_add_icon(struct wpabuf *buf, struct hostapd_bss_config *bss, const char *name)
{
    size_t j;
...
static void anqp_add_osu_provider_nai(struct wpabuf *buf, struct hs20_osu_provider *p)
{
...
static void anqp_add_osu_providers_nai_list(struct hostapd_data *hapd, struct wpabuf *buf)
{
    if (hapd->conf->hs20_osu_providers_nai_count)
    {
        size_t i;
        ...
        for (i = 0; i < hapd->conf->hs20_osu_providers_count; i++)
        {
            anqp_add_osu_provider_nai(buf, &hapd->conf->hs20_osu_providers[i]);
        }
...
static void anqp_add_operator_icon_metadata(struct hostapd_data *hapd, struct wpabuf *buf)
{
...
#ifdef CONFIG_MBO
static void anqp_add_mbo_cell_data_conn_pref(struct hostapd_data *hapd, struct wpabuf *buf)
{
...
static size_t anqp_get_required_len(struct hostapd_data *hapd, const u16 *infoid, unsigned int num_infoid)
{
    size_t len = 0;
    unsigned int i;
...
static struct wpabuf *gas_serv_build_gas_resp_payload(struct hostapd_data *hapd,
                                                      ...
                                                      unsigned int num_extra_req)
{
    ...
    if (request & (ANQP_REQ_NAI_REALM | ANQP_REQ_NAI_HOME_REALM))
        anqp_add_nai_realm(hapd, buf, home_realm, home_realm_len, request & ANQP_REQ_NAI_REALM,
...
static void rx_anqp_query_list_id(struct hostapd_data *hapd, u16 info_id, struct anqp_query_info *qi)
{
    switch (info_id)
    {
        ...
        case ANQP_EMERGENCY_CALL_NUMBER:
            set_anqp_req(ANQP_REQ_EMERGENCY_CALL_NUMBER, "Emergency Call Number", get_anqp_elem(hapd, info_id) != NULL,
                         qi);
            break;
        ...
        case ANQP_IP_ADDR_TYPE_AVAILABILITY:
            set_anqp_req(ANQP_REQ_IP_ADDR_TYPE_AVAILABILITY, "IP Addr Type Availability",
        ...
        case ANQP_AP_CIVIC_LOCATION:
            set_anqp_req(ANQP_REQ_AP_CIVIC_LOCATION, "AP Civic Location", get_anqp_elem(hapd, info_id) != NULL, qi);
            break;
        ...
        case ANQP_EMERGENCY_ALERT_URI:
            set_anqp_req(ANQP_REQ_EMERGENCY_ALERT_URI, "Emergency Alert URI", get_anqp_elem(hapd, info_id) != NULL, qi);
            break;
        case ANQP_TDLS_CAPABILITY:
            set_anqp_req(ANQP_REQ_TDLS_CAPABILITY, "TDLS Capability", get_anqp_elem(hapd, info_id) != NULL, qi);
            break;
        case ANQP_EMERGENCY_NAI:
            set_anqp_req(ANQP_REQ_EMERGENCY_NAI, "Emergency NAI", get_anqp_elem(hapd, info_id) != NULL, qi);
            break;
        default:
#ifdef CONFIG_FILS
            if (info_id == ANQP_FILS_REALM_INFO && !dl_list_empty(&hapd->conf->fils_realms))
            {
                wpa_printf(MSG_DEBUG, "ANQP: FILS Realm Information (local)");
            }
            else
#endif /* CONFIG_FILS */
                if (info_id == ANQP_VENUE_URL && hapd->conf->venue_url)
            {
                wpa_printf(MSG_DEBUG, "ANQP: Venue URL (local)");
            }
            ...
            {
                wpa_printf(MSG_DEBUG, "ANQP: No more room for extra requests - ignore Info Id %u", info_id);
                break;
            }
...
static void rx_anqp_hs_query_list(struct hostapd_data *hapd, u8 subtype, struct anqp_query_info *qi)
{
    switch (subtype)
    {
        ...
        case HS20_STYPE_OPERATOR_ICON_METADATA:
            set_anqp_req(ANQP_REQ_OPERATOR_ICON_METADATA, "Operator Icon Metadata",
                         hapd->conf->hs20_operator_icon_count, qi);
            break;
        case HS20_STYPE_OSU_PROVIDERS_NAI_LIST:
            set_anqp_req(ANQP_REQ_OSU_PROVIDERS_NAI_LIST, "OSU Providers NAI List",
...
#ifdef CONFIG_P2P
static void rx_anqp_vendor_specific_p2p(struct hostapd_data *hapd, struct anqp_query_info *qi)
{
    /*
     ...
     */
    wpa_printf(MSG_DEBUG, "ANQP: Ignore WFA vendor type %u (P2P SD) in generic GAS server", P2P_OUI_TYPE);
    qi->p2p_sd = 1;
    return;
}
#endif /* CONFIG_P2P */
...
static void rx_anqp_mbo_query_list(struct hostapd_data *hapd, u8 subtype, struct anqp_query_info *qi)
{
    switch (subtype)
    {
        case MBO_ANQP_SUBTYPE_CELL_CONN_PREF:
            set_anqp_req(ANQP_REQ_MBO_CELL_DATA_CONN_PREF, "Cellular Data Connection Preference",
                                        ...
                                        struct anqp_query_info *qi)
{
    ...
    switch (subtype)
    {
        case MBO_ANQP_SUBTYPE_QUERY_LIST:
            ...
            break;
        default:
            wpa_printf(MSG_DEBUG, "ANQP: Unsupported MBO query subtype %u", subtype);
            break;
    }
}
...
static void gas_serv_req_local_processing(
    struct hostapd_data *hapd, const u8 *sa, u8 dialog_token, struct anqp_query_info *qi, int prot, int std_addr3)
{
    struct wpabuf *buf, *tx_buf;

    buf = gas_serv_build_gas_resp_payload(hapd, qi->request, qi->home_realm_query, qi->home_realm_query_len,
    ...
    if (wpabuf_len(buf) > hapd->conf->gas_frag_limit || hapd->conf->gas_comeback_delay)
    {
    ...
    if (std_addr3)
        ...
        hostapd_drv_send_action_addr3_ap(hapd, hapd->iface->freq, 0, sa, wpabuf_head(tx_buf), wpabuf_len(tx_buf));
    wpabuf_free(tx_buf);
}

#ifdef CONFIG_DPP
void gas_serv_req_dpp_processing(struct hostapd_data *hapd, const u8 *sa, u8 dialog_token, int prot, struct wpabuf *buf)
{
    ...
    if (wpabuf_len(buf) > hapd->conf->gas_frag_limit || hapd->conf->gas_comeback_delay)
    {
        ...
        wpa_printf(MSG_DEBUG, "DPP: Too long response to fit in initial response - use GAS comeback");
        di = gas_dialog_create(hapd, sa, dialog_token);
        if (!di)
        {
            wpa_printf(MSG_INFO, "DPP: Could not create dialog for " MACSTR " (dialog token %u)", MAC2STR(sa),
                       dialog_token);
            wpabuf_free(buf);
            tx_buf = gas_build_initial_resp(dialog_token, WLAN_STATUS_UNSPECIFIED_FAILURE, 0, 10);
            if (tx_buf)
        ...
        else
        {
            ...
            di->sd_resp_pos = 0;
            di->dpp         = 1;
            tx_buf          = gas_build_initial_resp(dialog_token, WLAN_STATUS_SUCCESS, comeback_delay, 10 + 2);
            if (tx_buf)
            {
                gas_serv_write_dpp_adv_proto(tx_buf);
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: GAS Initial response (no comeback)");
        tx_buf = gas_build_initial_resp(dialog_token, WLAN_STATUS_SUCCESS, 0, 10 + 2 + wpabuf_len(buf));
        if (tx_buf)
        {
            gas_serv_write_dpp_adv_proto(tx_buf);
...
static void gas_serv_rx_gas_initial_req(
    struct hostapd_data *hapd, const u8 *sa, const u8 *data, size_t len, int prot, int std_addr3)
{
    const u8 *pos = data;
    ...
    dialog_token = *pos++;
    wpa_msg(hapd->msg_ctx, MSG_DEBUG, "GAS: GAS Initial Request from " MACSTR " (dialog token %u) ", MAC2STR(sa),
            dialog_token);
        ...
        if (*pos != ACCESS_NETWORK_QUERY_PROTOCOL)
    {
        ...
        if (std_addr3)
            ...
            hostapd_drv_send_action_addr3_ap(hapd, hapd->iface->freq, 0, sa, wpabuf_head(buf), wpabuf_len(buf));
        wpabuf_free(buf);
        return;
    }
    ...
    if (dpp)
    {
        ...
        msg = hostapd_dpp_gas_req_handler(hapd, sa, pos, slen, data, len);
        if (!msg)
            return;
...
static void gas_serv_rx_gas_comeback_req(
    struct hostapd_data *hapd, const u8 *sa, const u8 *data, size_t len, int prot, int std_addr3)
{
    ...
    {
        tx_buf =
            gas_build_comeback_resp(dialog_token, WLAN_STATUS_SUCCESS, dialog->sd_frag_id, more, 0, 10 + 2 + frag_len);
        if (tx_buf)
        {
            gas_serv_write_dpp_adv_proto(tx_buf);
