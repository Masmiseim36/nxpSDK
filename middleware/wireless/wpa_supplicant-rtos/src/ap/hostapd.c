...
static void hostapd_interface_setup_failure_handler(void *eloop_ctx, void *timeout_ctx);
...
static int hostapd_iface_conf_changed(struct hostapd_config *newconf, struct hostapd_config *oldconf)
{
    size_t i;
    ...
    {
        if (os_strcmp(newconf->bss[i]->iface, oldconf->bss[i]->iface) != 0)
            return 1;
    }
...
int hostapd_reload_config(struct hostapd_iface *iface)
{
    ...
    if (hostapd_iface_conf_changed(newconf, oldconf))
    {
        ...
        if (!iface)
        {
            wpa_printf(MSG_ERROR, "Failed to initialize interface on config reload");
            return -1;
        }
...
static void hostapd_broadcast_key_clear_iface(struct hostapd_data *hapd, const char *ifname)
{
    int i;
    ...
    {
        if (hostapd_drv_set_key(ifname, hapd, WPA_ALG_NONE, NULL, i, 0, 0, NULL, 0, NULL, 0, KEY_FLAG_GROUP))
        {
            wpa_printf(MSG_DEBUG,
...
static int hostapd_broadcast_wep_set(struct hostapd_data *hapd)
{
    ...
    if (ssid->wep.default_len && ssid->wep.key[idx] &&
        hostapd_drv_set_key(hapd->conf->iface, hapd, WPA_ALG_WEP, broadcast_ether_addr, idx, 0, 1, NULL, 0,
                            ssid->wep.key[idx], ssid->wep.len[idx], KEY_FLAG_GROUP_RX_TX_DEFAULT))
    {
...
void hostapd_free_hapd_data(struct hostapd_data *hapd)
{
    ...
    if (!hapd->started)
    {
        wpa_printf(MSG_ERROR, "%s: Interface %s wasn't started", __func__, hapd->conf ? hapd->conf->iface : "N/A");
        return;
    }
    hapd->started         = 0;
...
#ifdef CONFIG_SAE
    {
        ...
        while ((q = dl_list_first(&hapd->sae_commit_queue, struct hostapd_sae_commit_queue, list)))
        {
            dl_list_del(&q->list);
            os_free(q);
        }
...
{
    wpa_printf(MSG_DEBUG, "%s(hapd=%p (%s))", __func__, hapd, hapd->conf ? hapd->conf->iface : "N/A");
...
static void hostapd_cleanup_iface(struct hostapd_iface *iface)
{
    ...
    eloop_cancel_timeout(hostapd_interface_setup_failure_handler, iface, NULL);
...
static int hostapd_setup_encryption(char *iface, struct hostapd_data *hapd)
{
    ...
    for (i = 0; i < 4; i++)
    {
        if (hapd->conf->ssid.wep.key[i] &&
            hostapd_drv_set_key(iface, hapd, WPA_ALG_WEP, NULL, i, 0, i == hapd->conf->ssid.wep.idx, NULL, 0,
                                hapd->conf->ssid.wep.key[i], hapd->conf->ssid.wep.len[i],
                                i == hapd->conf->ssid.wep.idx ? KEY_FLAG_GROUP_RX_TX_DEFAULT : KEY_FLAG_GROUP_RX_TX))
        {
...
skip_mask_ext:
    wpa_printf(MSG_DEBUG, "BSS count %lu, BSSID mask " MACSTR " (%d bits)", (unsigned long)iface->conf->num_bss,
               MAC2STR(mask), bits);
    ...
    for (i = 0; i < ETH_ALEN; i++)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Invalid BSSID mask " MACSTR " for start address " MACSTR ".", MAC2STR(mask),
...
static int hostapd_das_nas_mismatch(struct hostapd_data *hapd, struct radius_das_attrs *attr)
{
    if (attr->nas_identifier &&
        (!hapd->conf->nas_identifier || os_strlen(hapd->conf->nas_identifier) != attr->nas_identifier_len ||
         os_memcmp(hapd->conf->nas_identifier, attr->nas_identifier, attr->nas_identifier_len) != 0))
    {
    ...
    if (attr->nas_ip_addr &&
        (hapd->conf->own_ip_addr.af != AF_INET || os_memcmp(&hapd->conf->own_ip_addr.u.v4, attr->nas_ip_addr, 4) != 0))
    {
    ...
    if (attr->nas_ipv6_addr && (hapd->conf->own_ip_addr.af != AF_INET6 ||
                                os_memcmp(&hapd->conf->own_ip_addr.u.v6, attr->nas_ipv6_addr, 16) != 0))
    {
...
static enum radius_das_res hostapd_das_coa(void *ctx, struct radius_das_attrs *attr)
{
    ...
    wpa_printf(MSG_DEBUG, "RADIUS DAS: Found a matching session " MACSTR " - CoA", MAC2STR(sta->addr));
...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
    ...
    if (!first || first == -1)
    {
        ...
        hapd->interface_added = 1;
        if (hostapd_if_add(hapd->iface->bss[0], WPA_IF_AP_BSS, conf->iface, addr, hapd, &hapd->drv_priv, force_ifname,
                           if_addr, conf->bridge[0] ? conf->bridge : NULL, first == -1))
        {
    ...
    if (!hostapd_drv_none(hapd))
    {
        wpa_printf(MSG_DEBUG, "Using interface %s with hwaddr " MACSTR " and ssid \"%s\"", conf->iface,
                   MAC2STR(hapd->own_addr), wpa_ssid_txt(conf->ssid.ssid, conf->ssid.ssid_len));
    }
    ...
    if (conf->radius_req_attr_sqlite)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "Could not open SQLite file '%s'", conf->radius_req_attr_sqlite);
            return -1;
        }

        wpa_printf(MSG_DEBUG, "Opening RADIUS attribute database: %s", conf->radius_req_attr_sqlite);
    ...
    if (conf->radius_das_port)
    {
        ...
        das_conf.require_message_authenticator = conf->radius_das_require_message_authenticator;
        das_conf.ctx                           = hapd;
    ...
    if (flush_old_stations && !conf->start_disabled && conf->broadcast_deauth)
    {
        u8 addr[ETH_ALEN];
         ...
         * operate. */
        wpa_dbg(hapd->msg_ctx, MSG_DEBUG, "Deauthenticate all stations at BSS start");
        os_memset(addr, 0xff, ETH_ALEN);
...
static int configured_fixed_chan_to_freq(struct hostapd_iface *iface)
{
    ...
    {
        freq = ieee80211_chan_to_freq(NULL, iface->conf->op_class, iface->conf->channel);
        if (freq < 0)
        {
            wpa_printf(MSG_INFO, "Could not convert op_class %u channel %u to operating frequency",
    ...
    for (j = 0; j < iface->num_hw_features; j++)
    {
        ...
        if (iface->conf->hw_mode != HOSTAPD_MODE_IEEE80211ANY && iface->conf->hw_mode != mode->mode)
            continue;
        for (i = 0; i < mode->num_channels; i++)
        {
            struct hostapd_channel_data *chan = &mode->channels[i];

            if (chan->chan == iface->conf->channel && !is_6ghz_freq(chan->freq))
            {
...
static void fst_hostapd_get_channel_info_cb(void *ctx, enum hostapd_hw_mode *hw_mode, u8 *channel)
{
    struct hostapd_data *hapd = ctx;
...
static int fst_hostapd_send_action_cb(void *ctx, const u8 *da, struct wpabuf *buf)
{
    ...
    return hostapd_drv_send_action(hapd, hapd->iface->freq, 0, da, wpabuf_head(buf), wpabuf_len(buf));
}
...
static void fst_hostapd_update_mb_ie_cb(void *ctx, const u8 *addr, const u8 *buf, size_t size)
{
    struct hostapd_data *hapd = ctx;
    struct sta_info *sta      = ap_get_sta(hapd, addr);
...
static const u8 *fst_hostapd_get_sta(struct fst_get_peer_ctx **get_ctx, bool mb_only)
{
...
static const u8 *fst_hostapd_get_peer_first(void *ctx, struct fst_get_peer_ctx **get_ctx, bool mb_only)
{
    struct hostapd_data *hapd = ctx;
...
static const u8 *fst_hostapd_get_peer_next(void *ctx, struct fst_get_peer_ctx **get_ctx, bool mb_only)
{
    return fst_hostapd_get_sta(get_ctx, mb_only);
}

void fst_hostapd_fill_iface_obj(struct hostapd_data *hapd, struct fst_wpa_obj *iface_obj)
{
    iface_obj->ctx              = hapd;
...
static int hostapd_owe_iface_iter(struct hostapd_iface *iface, void *ctx)
{
    ...
    for (i = 0; i < iface->num_bss; i++)
    {
        ...
        if (os_strcmp(hapd->conf->owe_transition_ifname, bss->conf->iface) != 0)
            continue;

        wpa_printf(MSG_DEBUG, "OWE: ifname=%s found transition mode ifname=%s BSSID " MACSTR " SSID %s",
                   hapd->conf->iface, bss->conf->iface, MAC2STR(bss->own_addr),
                   wpa_ssid_txt(bss->conf->ssid.ssid, bss->conf->ssid.ssid_len));
        if (!bss->conf->ssid.ssid_set || !bss->conf->ssid.ssid_len || is_zero_ether_addr(bss->own_addr))
            continue;

        os_memcpy(hapd->conf->owe_transition_bssid, bss->own_addr, ETH_ALEN);
        os_memcpy(hapd->conf->owe_transition_ssid, bss->conf->ssid.ssid, bss->conf->ssid.ssid_len);
        hapd->conf->owe_transition_ssid_len = bss->conf->ssid.ssid_len;
        wpa_printf(MSG_DEBUG, "OWE: Copied transition mode information");
        return 1;
    }
...
{
    if (hapd->conf->owe_transition_ssid_len > 0 && !is_zero_ether_addr(hapd->conf->owe_transition_bssid))
        return 0;
    ...
    if (!hapd->iface->interfaces || !hapd->iface->interfaces->for_each_interface)
        ...
        return hapd->iface->interfaces->for_each_interface(hapd->iface->interfaces, hostapd_owe_iface_iter, hapd);
}
...
static int hostapd_owe_iface_iter2(struct hostapd_iface *iface, void *ctx)
{
    ...
    for (i = 0; i < iface->num_bss; i++)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "OWE: Interface %s state %s - defer beacon update", bss->conf->iface,
        ...
        if (res == 0)
            continue;
        wpa_printf(MSG_DEBUG, "OWE: Matching transition mode interface enabled - update beacon data for %s",
                   bss->conf->iface);
...
static void hostapd_owe_update_trans(struct hostapd_iface *iface)
{
    ...
    if (!iface->interfaces || !iface->interfaces->for_each_interface)
        hostapd_owe_iface_iter2(iface, NULL);
    else
        iface->interfaces->for_each_interface(iface->interfaces, hostapd_owe_iface_iter2, NULL);
#endif /* CONFIG_OWE */
}
...
static int hostapd_setup_interface_complete_sync(struct hostapd_iface *iface, int err)
{
    struct hostapd_data *hapd = iface->bss[0];
    size_t j;
    u8 *prev_addr;
    ...
    if (iface->freq)
    {
        ...
        wpa_printf(MSG_DEBUG,
                   "Mode: %s  Channel: %d  "
                   "Frequency: %d MHz",
                   hostapd_hw_mode_txt(iface->conf->hw_mode), iface->conf->channel, iface->freq);
        ...
        if (iface->mconf != NULL)
        {
            wpa_printf(MSG_DEBUG, "%s: Mesh configuration will be applied while joining the mesh network",
        ...
        if (!delay_apply_cfg &&
            hostapd_set_freq(hapd, hapd->iconf->hw_mode, iface->freq, hapd->iconf->channel, hapd->iconf->enable_edmg,
                             hapd->iconf->edmg_channel, hapd->iconf->ieee80211n, hapd->iconf->ieee80211ac,
                             hapd->iconf->ieee80211ax, hapd->iconf->secondary_channel,
                             hostapd_get_oper_chwidth(hapd->iconf), hostapd_get_oper_centr_freq_seg0_idx(hapd->iconf),
...
dfs_offload:
    ...
    if (hapd->iconf->fst_cfg.group_id[0])
    {
        ...
        iface->fst = fst_attach(hapd->conf->iface, hapd->own_addr, &iface_obj, &hapd->iconf->fst_cfg);
        if (!iface->fst)
        {
            wpa_printf(MSG_ERROR, "Could not attach to FST %s", hapd->iconf->fst_cfg.group_id);
            goto fail;
        }
    }
#endif /* CONFIG_FST */
...
fail:
    ...
    else if (hapd->setup_complete_cb)
    {
         ...
         */
        eloop_register_timeout(0, 0, hostapd_interface_setup_failure_handler, iface, NULL);
    }
...
int hostapd_setup_interface_complete(struct hostapd_iface *iface, int err)
{
    ...
    for (i = 0; i < interfaces->count; i++)
    {
        if (interfaces->iface[i]->need_to_start_in_sync && !interfaces->iface[i]->ready_to_start_in_sync)
    ...
    if (not_ready_in_sync_ifaces > 1 && iface->state == HAPD_IFACE_DFS)
    {
         ...
         */
        iface->need_to_start_in_sync = 0;
        wpa_printf(MSG_INFO, "%s: Finished CAC - bypass sync and start interface", iface->bss[0]->conf->iface);
        return hostapd_setup_interface_complete_sync(iface, err);
    }
    ...
    if (not_ready_in_sync_ifaces > 1)
    {
        ...
        wpa_printf(MSG_INFO, "%s: Interface waiting to sync with other interfaces", iface->bss[0]->conf->iface);
        return 0;
    }

    wpa_printf(MSG_INFO, "%s: Last interface to sync - starting all interfaces", iface->bss[0]->conf->iface);
    iface->need_to_start_in_sync = 0;
...
int hostapd_setup_interface(struct hostapd_iface *iface)
{
    ...
    if (ret)
    {
        wpa_printf(MSG_ERROR, "%s: Unable to setup interface.", iface->conf->bss[0]->iface);
        return -1;
    }
...
static void hostapd_bss_deinit(struct hostapd_data *hapd)
{
    if (!hapd)
        return;
    wpa_printf(MSG_DEBUG, "%s: deinit bss %s", __func__, hapd->conf ? hapd->conf->iface : "N/A");
    hostapd_bss_deinit_no_free(hapd);
    wpa_msg(hapd->msg_ctx, MSG_INFO, AP_EVENT_DISABLED);
#ifdef CONFIG_SQLITE
    if (hapd->rad_attr_db)
...
void hostapd_interface_deinit_free(struct hostapd_iface *iface)
{
    ...
    if (iface == NULL)
        return;
    wpa_printf(MSG_DEBUG, "%s: num_bss=%u conf->num_bss=%u", __func__, (unsigned int)iface->num_bss,
...
int hostapd_enable_iface(struct hostapd_iface *hapd_iface)
{
    ...
    if (hapd_iface->interfaces == NULL || hapd_iface->interfaces->driver_init == NULL ||
...
static struct hostapd_config *hostapd_config_alloc(struct hapd_interfaces *interfaces,
                                                   const char *ifname,
                                                   ...
                                                   const char *driver)
{
    ...
    if (driver)
    {
        ...
        wpa_printf(MSG_ERROR, "Invalid/unknown driver '%s' - registering the default driver", driver);
    }
        ...
        *pos++    = '\0';
        ...
        else
        {
            ...
            if (start_ctrl_iface_bss(hapd) < 0 ||
                (hapd_iface->state == HAPD_IFACE_ENABLED && hostapd_setup_bss(hapd, -1)))
            {
                hostapd_cleanup(hapd);
...
void hostapd_new_assoc_sta(struct hostapd_data *hapd, struct sta_info *sta, int reassoc)
{
    ...
    if (reassoc)
    {
        if (sta->auth_alg != WLAN_AUTH_FT && sta->auth_alg != WLAN_AUTH_FILS_SK &&
            sta->auth_alg != WLAN_AUTH_FILS_SK_PFS && sta->auth_alg != WLAN_AUTH_FILS_PK &&
    ...
    if (hapd->iface->drv_flags & WPA_DRIVER_FLAGS_WIRED)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "%s: %s: canceled wired ap_handle_timer timeout for " MACSTR, hapd->conf->iface,
                       __func__, MAC2STR(sta->addr));
        }
    }
    else if (!(hapd->iface->drv_flags & WPA_DRIVER_FLAGS_INACTIVITY_TIMER))
    {
        wpa_printf(MSG_DEBUG,
                   "%s: %s: reschedule ap_handle_timer timeout for " MACSTR " (%d seconds - ap_max_inactivity)",
                   hapd->conf->iface, __func__, MAC2STR(sta->addr), hapd->conf->ap_max_inactivity);
        eloop_cancel_timeout(ap_handle_timer, hapd, sta);
        eloop_register_timeout(hapd->conf->ap_max_inactivity, 0, ap_handle_timer, hapd, sta);
    }

#ifdef CONFIG_MACSEC
    if (hapd->conf->wpa_key_mgmt == WPA_KEY_MGMT_NONE && hapd->conf->mka_psk_set)
...
{
    wpa_printf(MSG_INFO, "%s: interface state %s->%s", iface->conf ? iface->conf->bss[0]->iface : "N/A",
...
static int hostapd_build_beacon_data(struct hostapd_data *hapd, struct beacon_data *beacon)
{
    ...
    {
        beacon->probe_resp = os_memdup(params.proberesp, params.proberesp_len);
    ...
    if (proberesp_extra)
    {
        beacon->proberesp_ies = os_memdup(proberesp_extra->buf, wpabuf_len(proberesp_extra));
    ...
    if (assocresp_extra)
    {
        beacon->assocresp_ies = os_memdup(assocresp_extra->buf, wpabuf_len(assocresp_extra));
                                      ...
                                      struct hostapd_freq_params *old_params)
{
    ...
    if (old_params &&
        hostapd_set_freq_params(old_params, conf->hw_mode, hostapd_hw_get_freq(hapd, conf->channel), conf->channel,
                                conf->enable_edmg, conf->edmg_channel, conf->ieee80211n, conf->ieee80211ac,
                                conf->ieee80211ax, conf->secondary_channel, hostapd_get_oper_chwidth(conf),
                                hostapd_get_oper_centr_freq_seg0_idx(conf), hostapd_get_oper_centr_freq_seg1_idx(conf),
                                conf->vht_capab, mode ? &mode->he_capab[IEEE80211_MODE_AP] : NULL))
        return -1;
...
static int hostapd_fill_csa_settings(struct hostapd_data *hapd, struct csa_settings *settings)
{
    ...
    if (ieee80211_freq_to_channel_ext(settings->freq_params.freq, settings->freq_params.sec_channel_offset, bandwidth,
                                      &hapd->iface->cs_oper_class, &chan) == NUM_HOSTAPD_MODES)
    {
        wpa_printf(
            MSG_DEBUG,
            "invalid frequency for channel switch (freq=%d, sec_channel_offset=%d, vht_enabled=%d, he_enabled=%d)",
            settings->freq_params.freq, settings->freq_params.sec_channel_offset, settings->freq_params.vht_enabled,
...
void hostapd_chan_switch_config(struct hostapd_data *hapd, struct hostapd_freq_params *freq_params)
{
    ...
    hostapd_logger(hapd, NULL, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_INFO,
                   "CHAN_SWITCH HE config 0x%x VHT config 0x%x", hapd->iconf->ch_switch_he_config,
...
struct hostapd_data *hostapd_get_iface(struct hapd_interfaces *interfaces, const char *ifname)
{
    size_t i, j;
...
void hostapd_ocv_check_csa_sa_query(void *eloop_ctx, void *timeout_ctx)
{
    ...
    for (sta = hapd->sta_list; sta; sta = sta->next)
    {
        ...
        wpa_printf(MSG_DEBUG, "OCV: OCVC STA " MACSTR " did not start SA Query after CSA - disconnect",
                   MAC2STR(sta->addr));
