...
static u8 ieee80211_he_ppet_size(u8 ppe_thres_hdr, const u8 *phy_cap_info)
{
    ...
    if ((phy_cap_info[HE_PHYCAP_PPE_THRESHOLD_PRESENT_IDX] & HE_PHYCAP_PPE_THRESHOLD_PRESENT) == 0)
        return 0;
...
static int ieee80211_invalid_he_cap_size(const u8 *buf, size_t len)
{
    ...
    cap_len += ieee80211_he_ppet_size(ppe_thres_hdr, cap->he_phy_capab_info);

    return len < cap_len;
}

u8 *hostapd_eid_he_capab(struct hostapd_data *hapd, u8 *eid, enum ieee80211_op_mode opmode)
{
    struct ieee80211_he_capabilities *cap;
    ...
    ppet_size = ieee80211_he_ppet_size(mode->he_capab[opmode].ppet[0], mode->he_capab[opmode].phy_cap);
    ...
    switch (hapd->iface->conf->he_oper_chwidth)
    {
        ...
        case CHANWIDTH_USE_HT:
            he_oper_chwidth |= HE_PHYCAP_CHANNEL_WIDTH_SET_40MHZ_IN_2G | HE_PHYCAP_CHANNEL_WIDTH_SET_40MHZ_80MHZ_IN_5G;
            break;
    }
    ...
    *pos++ = WLAN_EID_EXT_HE_CAPABILITIES;
    ...
    os_memcpy(cap->he_mac_capab_info, mode->he_capab[opmode].mac_cap, HE_MAX_MAC_CAPAB_SIZE);
    os_memcpy(cap->he_phy_capab_info, mode->he_capab[opmode].phy_cap, HE_MAX_PHY_CAPAB_SIZE);
    os_memcpy(cap->optional, mode->he_capab[opmode].mcs, mcs_nss_size);
    if (ppet_size)
        os_memcpy(&cap->optional[mcs_nss_size], mode->he_capab[opmode].ppet, ppet_size);

    if (hapd->iface->conf->he_phy_capab.he_su_beamformer)
        cap->he_phy_capab_info[HE_PHYCAP_SU_BEAMFORMER_CAPAB_IDX] |= HE_PHYCAP_SU_BEAMFORMER_CAPAB;
    else
        cap->he_phy_capab_info[HE_PHYCAP_SU_BEAMFORMER_CAPAB_IDX] &= ~HE_PHYCAP_SU_BEAMFORMER_CAPAB;

    if (hapd->iface->conf->he_phy_capab.he_su_beamformee)
        cap->he_phy_capab_info[HE_PHYCAP_SU_BEAMFORMEE_CAPAB_IDX] |= HE_PHYCAP_SU_BEAMFORMEE_CAPAB;
    else
        cap->he_phy_capab_info[HE_PHYCAP_SU_BEAMFORMEE_CAPAB_IDX] &= ~HE_PHYCAP_SU_BEAMFORMEE_CAPAB;

    if (hapd->iface->conf->he_phy_capab.he_mu_beamformer)
        cap->he_phy_capab_info[HE_PHYCAP_MU_BEAMFORMER_CAPAB_IDX] |= HE_PHYCAP_MU_BEAMFORMER_CAPAB;
    else
        cap->he_phy_capab_info[HE_PHYCAP_MU_BEAMFORMER_CAPAB_IDX] &= ~HE_PHYCAP_MU_BEAMFORMER_CAPAB;
...
u8 *hostapd_eid_he_operation(struct hostapd_data *hapd, u8 *eid)
{
    ...
    *pos++ = WLAN_EID_EXT_HE_OPERATION;
    ...
    if (hapd->iface->conf->he_op.he_default_pe_duration)
        params |= (hapd->iface->conf->he_op.he_default_pe_duration << HE_OPERATION_DFLT_PE_DURATION_OFFSET);

    if (hapd->iface->conf->he_op.he_twt_required)
        params |= HE_OPERATION_TWT_REQUIRED;

    if (hapd->iface->conf->he_op.he_rts_threshold)
        params |= (hapd->iface->conf->he_op.he_rts_threshold << HE_OPERATION_RTS_THRESHOLD_OFFSET);
    ...
    params |= hapd->iface->conf->he_op.he_bss_color << HE_OPERATION_BSS_COLOR_OFFSET;

    /* HE minimum required basic MCS and NSS for STAs */
    oper->he_mcs_nss_set = host_to_le16(hapd->iface->conf->he_op.he_basic_mcs_nss_set);
...
u8 *hostapd_eid_he_mu_edca_parameter_set(struct hostapd_data *hapd, u8 *eid)
{
    ...
    *pos++ = WLAN_EID_EXT_HE_MU_EDCA_PARAMS;
    ...
    wpa_hexdump(MSG_DEBUG, "HE: MU EDCA Parameter Set element", pos, sizeof(*edca));
...
u8 *hostapd_eid_spatial_reuse(struct hostapd_data *hapd, u8 *eid)
{
    ...
    if (hapd->iface->conf->spr.sr_control & SPATIAL_REUSE_NON_SRG_OFFSET_PRESENT)
        sz++;

    if (hapd->iface->conf->spr.sr_control & SPATIAL_REUSE_SRG_INFORMATION_PRESENT)
        sz += 18;
    ...
    *pos++ = WLAN_EID_EXT_SPATIAL_REUSE;
    ...
    {
        *spr_param++ = hapd->iface->conf->spr.non_srg_obss_pd_max_offset;
        pos++;
    }
    if (spr->sr_ctrl & SPATIAL_REUSE_SRG_INFORMATION_PRESENT)
    {
        ...
        *spr_param++ = hapd->iface->conf->spr.srg_obss_pd_max_offset;
        os_memcpy(spr_param, hapd->iface->conf->spr.srg_bss_color_bitmap, 8);
        spr_param += 8;
        os_memcpy(spr_param, hapd->iface->conf->spr.srg_partial_bssid_bitmap, 8);
        pos += 18;
    }
...
u8 *hostapd_eid_he_6ghz_band_cap(struct hostapd_data *hapd, u8 *eid)
{
    ...
    if (!mode || !is_6ghz_op_class(hapd->iconf->op_class) || !is_6ghz_freq(hapd->iface->freq))
        return eid;

    he_cap = &mode->he_capab[IEEE80211_MODE_AP];
    capab  = he_cap->he_6ghz_capa & HE_6GHZ_BAND_CAP_MIN_MPDU_START;
    capab |= (conf->he_6ghz_max_ampdu_len_exp << HE_6GHZ_BAND_CAP_MAX_AMPDU_LEN_EXP_SHIFT) &
             HE_6GHZ_BAND_CAP_MAX_AMPDU_LEN_EXP_MASK;
    capab |= (conf->he_6ghz_max_mpdu << HE_6GHZ_BAND_CAP_MAX_MPDU_LEN_SHIFT) & HE_6GHZ_BAND_CAP_MAX_MPDU_LEN_MASK;
...
static int check_valid_he_mcs(struct hostapd_data *hapd, const u8 *sta_he_capab, enum ieee80211_op_mode opmode)
{
    ...
    for (i = 0; i < mcs_count; i++)
    {
        ...
        ap_tx_mcs_set  = WPA_GET_LE16((const u8 *)&ap_mcs_set[(i * 2) + 1]);
    ...
    wpa_printf(MSG_DEBUG, "No matching HE MCS found between AP TX and STA RX");

    return 0;
}

u16 copy_sta_he_capab(struct hostapd_data *hapd,
                      struct sta_info *sta,
                      enum ieee80211_op_mode opmode,
                      const u8 *he_capab,
                      size_t he_capab_len)
{
    if (!he_capab || !(sta->flags & WLAN_STA_WMM) || !hapd->iconf->ieee80211ax || hapd->conf->disable_11ax ||
        !check_valid_he_mcs(hapd, he_capab, opmode) || ieee80211_invalid_he_cap_size(he_capab, he_capab_len) ||
...
u16 copy_sta_he_6ghz_capab(struct hostapd_data *hapd, struct sta_info *sta, const u8 *he_6ghz_capab)
{
    if (!he_6ghz_capab || !hapd->iconf->ieee80211ax || hapd->conf->disable_11ax ||
    ...
    {
        sta->he_6ghz_capab = os_zalloc(sizeof(struct ieee80211_he_6ghz_band_cap));
        if (!sta->he_6ghz_capab)
            return WLAN_STATUS_UNSPECIFIED_FAILURE;
    }

    sta->flags |= WLAN_STA_6GHZ;
    os_memcpy(sta->he_6ghz_capab, he_6ghz_capab, sizeof(struct ieee80211_he_6ghz_band_cap));
...
int hostapd_get_he_twt_responder(struct hostapd_data *hapd, enum ieee80211_op_mode mode)
{
    ...
    if (!hapd->iface->current_mode || !hapd->iface->current_mode->he_capab[mode].he_supported)
        return 0;

    mac_cap = hapd->iface->current_mode->he_capab[mode].mac_cap;

    return !!(mac_cap[HE_MAC_CAPAB_0] & HE_MACCAP_TWT_RESPONDER) && hapd->iface->conf->he_op.he_twt_responder;
}
#endif /* CONFIG_AP */
