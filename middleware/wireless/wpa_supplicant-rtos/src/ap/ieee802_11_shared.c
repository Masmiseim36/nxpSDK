...
void ieee802_11_send_sa_query_req(struct hostapd_data *hapd, const u8 *addr, const u8 *trans_id)
{
    ...
    if (sta && wpa_auth_uses_ocv(sta->wpa_sm))
    {
        ...
        {
            wpa_printf(MSG_WARNING, "Failed to get channel info for OCI element in SA Query Request");
            return;
        }
        ...
        if (hapd->conf->oci_freq_override_saquery_req)
        {
            ...
            ci.frequency = hapd->conf->oci_freq_override_saquery_req;
        }
        ...
        if (!oci_ie)
        {
            wpa_printf(MSG_WARNING, "Failed to allocate buffer for OCI element in SA Query Request");
            return;
        }
    ...
    if (!mgmt)
    {
        wpa_printf(MSG_DEBUG, "Failed to allocate buffer for SA Query Response frame");
        os_free(oci_ie);
        return;
    }
...
#endif /* CONFIG_OCV */
    if (hostapd_drv_send_mlme(hapd, mgmt, end - (u8 *)mgmt, 0, NULL, 0, 0) < 0)
...
static void ieee802_11_send_sa_query_resp(struct hostapd_data *hapd, const u8 *sa, const u8 *trans_id)
{
    ...
    if (wpa_auth_uses_ocv(sta->wpa_sm))
    {
        ...
        if (hostapd_drv_channel_info(hapd, &ci) != 0)
        {
            wpa_printf(MSG_WARNING, "Failed to get channel info for OCI element in SA Query Response");
            return;
        }
#ifdef CONFIG_TESTING_OPTIONS
        ...
        if (!oci_ie)
        {
            wpa_printf(MSG_WARNING, "Failed to allocate buffer for for OCI element in SA Query Response");
            return;
        }
...
#endif /* CONFIG_OCV */
    if (hostapd_drv_send_mlme(hapd, resp, end - (u8 *)resp, 0, NULL, 0, 0) < 0)
...
void ieee802_11_sa_query_action(struct hostapd_data *hapd, const struct ieee80211_mgmt *mgmt, size_t len)
{
    struct sta_info *sta;
    int i;
    const u8 *sa         = mgmt->sa;
    ...
    if (((const u8 *)mgmt) + len < mgmt->u.action.u.sa_query_resp.variable)
    {
    ...
    if (sta && wpa_auth_uses_ocv(sta->wpa_sm))
    {
        ...
        if (get_sta_tx_parameters(sta->wpa_sm, channel_width_to_int(ci.chanwidth), ci.seg1_idx, &tx_chanwidth,
                                  &tx_seg1_idx) < 0)
            return;

        if (ocv_verify_tx_params(elems.oci, elems.oci_len, &ci, tx_chanwidth, tx_seg1_idx) != OCI_SUCCESS)
        {
            wpa_msg(hapd->msg_ctx, MSG_INFO, OCV_FAILURE "addr=" MACSTR " frame=saquery%s error=%s", MAC2STR(sa),
                    action_type == WLAN_SA_QUERY_REQUEST ? "req" : "resp", ocv_errorstr);
            return;
        }
    }
#endif /* CONFIG_OCV */
...
static void hostapd_ext_capab_byte(struct hostapd_data *hapd, u8 *pos, int idx)
{
    *pos = 0x00;
    ...
    switch (idx)
    {
        ...
        case 9: /* Bits 72-79 */
#ifdef CONFIG_FILS
            if ((hapd->conf->wpa & WPA_PROTO_RSN) && wpa_key_mgmt_fils(hapd->conf->wpa_key_mgmt))
                *pos |= 0x01;
#endif /* CONFIG_FILS */
#ifdef CONFIG_IEEE80211AX
            if (hapd->iconf->ieee80211ax && hostapd_get_he_twt_responder(hapd, IEEE80211_MODE_AP))
        ...
        case 11: /* Bits 88-95 */
#ifdef CONFIG_SAE_PK
            if (hapd->conf->wpa && wpa_key_mgmt_sae(hapd->conf->wpa_key_mgmt) && hostapd_sae_pk_exclusively(hapd->conf))
...
u8 *hostapd_eid_mbo_rssi_assoc_rej(struct hostapd_data *hapd, u8 *eid, size_t len, int delta)
{
    u8 mbo[4];
...
{
    return hapd->conf->owe_transition_ssid_len > 0 && !is_zero_ether_addr(hapd->conf->owe_transition_bssid);
}
#endif /* CONFIG_OWE */
...
u8 *hostapd_eid_owe_trans(struct hostapd_data *hapd, u8 *eid, size_t len)
{
    ...
    if (hapd->conf->owe_transition_ifname[0] && !hostapd_eid_owe_trans_enabled(hapd))
    ...
    if (len < elen)
    {
        wpa_printf(MSG_DEBUG, "OWE: Not enough room in the buffer for OWE IE");
        return pos;
    }
    ...
    *pos++ = hapd->conf->owe_transition_ssid_len;
    os_memcpy(pos, hapd->conf->owe_transition_ssid, hapd->conf->owe_transition_ssid_len);
    pos += hapd->conf->owe_transition_ssid_len;
...
void ap_copy_sta_supp_op_classes(struct sta_info *sta, const u8 *supp_op_classes, size_t supp_op_classes_len)
{
    if (!supp_op_classes)
        return;
...
#ifdef CONFIG_OCV
int get_tx_parameters(struct sta_info *sta, int ap_max_chanwidth, int ap_seg1_idx, int *bandwidth, int *seg1_idx)
{
    int ht_40mhz  = 0;
    int vht_80p80 = 0;
    int requested_bw;
    ...
    if (sta->vht_operation)
    {
        ...
        else
        {
            ...
            requested_bw = 160;
            diff         = abs((int)oper->vht_op_info_chan_center_freq_seg0_idx -
                       (int)oper->vht_op_info_chan_center_freq_seg1_idx);
            vht_80p80    = oper->vht_op_info_chan_center_freq_seg1_idx != 0 && diff > 16;
        }
    }
    else if (sta->vht_capabilities)
    {
         ...
         * bandwidth.
         */
        vht_chanwidth = capab->vht_capabilities_info & VHT_CAP_SUPP_CHAN_WIDTH_MASK;
...
u8 *hostapd_eid_rsnxe(struct hostapd_data *hapd, u8 *eid, size_t len)
{
    ...
    if (wpa_key_mgmt_sae(hapd->conf->wpa_key_mgmt) &&
        (hapd->conf->sae_pwe == 1 || hapd->conf->sae_pwe == 2 || hostapd_sae_pw_id_in_use(hapd->conf) || sae_pk) &&
        hapd->conf->sae_pwe != 3)
    {
