...
u8 *hostapd_eid_vht_capabilities(struct hostapd_data *hapd, u8 *eid, u32 nsts)
{
    ...
    if (mode->mode == HOSTAPD_MODE_IEEE80211G && hapd->conf->vendor_vht && mode->vht_capab == 0 &&
    ...
    *pos++ = sizeof(*cap);
    ...
    if (nsts != 0)
    {
        ...
        cap->vht_capabilities_info &= ~(host_to_le32(hapd_nsts << VHT_CAP_BEAMFORMEE_STS_OFFSET));
        cap->vht_capabilities_info |= host_to_le32(nsts << VHT_CAP_BEAMFORMEE_STS_OFFSET);
    }
...
u8 *hostapd_eid_vht_operation(struct hostapd_data *hapd, u8 *eid)
{
    ...
    *pos++ = sizeof(*oper);
    ...
    if (hapd->iconf->vht_oper_chwidth == 2)
    {
        ...
        oper->vht_op_info_chan_center_freq_seg1_idx = oper->vht_op_info_chan_center_freq_seg0_idx;
...
u16 copy_sta_vht_capab(struct hostapd_data *hapd, struct sta_info *sta, const u8 *vht_capab)
{
    ...
    if (!vht_capab || !(sta->flags & WLAN_STA_WMM) || !hapd->iconf->ieee80211ac || hapd->conf->disable_11ac ||
...
u16 copy_sta_vht_oper(struct hostapd_data *hapd, struct sta_info *sta, const u8 *vht_oper)
{
    ...
    {
        sta->vht_operation = os_zalloc(sizeof(struct ieee80211_vht_operation));
        if (!sta->vht_operation)
            return WLAN_STATUS_UNSPECIFIED_FAILURE;
    }

    os_memcpy(sta->vht_operation, vht_oper, sizeof(struct ieee80211_vht_operation));
