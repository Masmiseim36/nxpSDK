...
static void ieee802_1x_send(struct hostapd_data *hapd, struct sta_info *sta, u8 type, const u8 *data, size_t datalen)
{
    ...
    if (!buf)
    {
        wpa_printf(MSG_ERROR, "malloc() failed for %s(len=%lu)", __func__, (unsigned long)len);
        return;
    }
...
void ieee802_1x_set_sta_authorized(struct hostapd_data *hapd, struct sta_info *sta, int authorized)
{
    ...
    if (res && errno != ENOENT)
    {
        wpa_printf(MSG_DEBUG, "Could not set station " MACSTR " flags for kernel driver (errno=%d).",
...
static void ieee802_1x_tx_key_one(
    struct hostapd_data *hapd, struct sta_info *sta, int idx, int broadcast, u8 *key_data, size_t key_len)
{
    u8 *buf, *ekey;
    ...
    if (os_memcmp(key->replay_counter, hapd->last_1x_eapol_key_replay_counter, IEEE8021X_REPLAY_COUNTER_LEN) <= 0)
    {
        /* NTP timestamp did not increment from last EAPOL-Key frame;
         * use previously used value + 1 instead. */
        inc_byte_array(hapd->last_1x_eapol_key_replay_counter, IEEE8021X_REPLAY_COUNTER_LEN);
        os_memcpy(key->replay_counter, hapd->last_1x_eapol_key_replay_counter, IEEE8021X_REPLAY_COUNTER_LEN);
    }
    else
    {
        os_memcpy(hapd->last_1x_eapol_key_replay_counter, key->replay_counter, IEEE8021X_REPLAY_COUNTER_LEN);
    }
    ...
    wpa_printf(MSG_DEBUG, "IEEE 802.1X: Sending EAPOL-Key to " MACSTR " (%s index=%d)", MAC2STR(sm->addr),
...
static void ieee802_1x_tx_key(struct hostapd_data *hapd, struct sta_info *sta)
{
    ...
    {
        ieee802_1x_tx_key_one(hapd, sta, eapol->default_wep_key_idx, 1, eapol->default_wep_key,
    ...
    if (hapd->conf->individual_wep_key_len > 0)
    {
        ...
        if (!ikey || random_get_bytes(ikey, hapd->conf->individual_wep_key_len))
        {
        ...
        if (hostapd_drv_set_key(hapd->conf->iface, hapd, WPA_ALG_WEP, sta->addr, 0, 0, 1, NULL, 0, ikey,
                                hapd->conf->individual_wep_key_len, KEY_FLAG_PAIRWISE_RX_TX))
        {
                                      ...
                                      size_t len)
{
    ...
    if (len <= sizeof(struct eap_hdr) ||
        (hdr->code == EAP_CODE_RESPONSE && eap[sizeof(struct eap_hdr)] != EAP_TYPE_IDENTITY) ||
        (hdr->code == EAP_CODE_INITIATE && eap[sizeof(struct eap_hdr)] != EAP_ERP_TYPE_REAUTH) ||
                                          ...
                                          struct radius_msg *msg)
{
    ...
    if (val != -1 && !hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_WLAN_PAIRWISE_CIPHER) &&
        !radius_msg_add_attr_int32(msg, RADIUS_ATTR_WLAN_PAIRWISE_CIPHER, suite))
    {
        wpa_printf(MSG_ERROR, "Could not add WLAN-Pairwise-Cipher");
        return -1;
    }

    suite = wpa_cipher_to_suite(((hapd->conf->wpa & 0x2) || hapd->conf->osen) ? WPA_PROTO_RSN : WPA_PROTO_WPA,
                                hapd->conf->wpa_group);
    if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_WLAN_GROUP_CIPHER) &&
    ...
    if (val != -1 && !hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_WLAN_AKM_SUITE) &&
    ...
    if (hapd->conf->ieee80211w != NO_MGMT_FRAME_PROTECTION)
    {
        ...
        if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_WLAN_GROUP_MGMT_CIPHER) &&
            !radius_msg_add_attr_int32(msg, RADIUS_ATTR_WLAN_GROUP_MGMT_CIPHER, suite))
        {
                                      ...
                                      struct radius_msg *msg)
{
    ...
    if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_NAS_PORT) && sta->aid > 0 &&
    ...
    {
        os_snprintf(buf, sizeof(buf), "CONNECT %d%sMbps %s", radius_sta_rate(hapd, sta) / 2,
                    (radius_sta_rate(hapd, sta) & 1) ? ".5" : "", radius_mode_txt(hapd));
        buf[sizeof(buf) - 1] = '\0';
    }
    if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_CONNECT_INFO) &&
        !radius_msg_add_attr(msg, RADIUS_ATTR_CONNECT_INFO, (u8 *)buf, os_strlen(buf)))
    {
    ...
    {
        os_snprintf(buf, sizeof(buf), "%016llX", (unsigned long long)sta->acct_session_id);
    ...
    if ((hapd->conf->wpa & 2) && !hapd->conf->disable_pmksa_caching && sta->eapol_sm &&
        sta->eapol_sm->acct_multi_session_id)
    {
        os_snprintf(buf, sizeof(buf), "%016llX", (unsigned long long)sta->eapol_sm->acct_multi_session_id);
    ...
    if (hapd->conf->wpa && wpa_key_mgmt_ft(hapd->conf->wpa_key_mgmt) && sta->wpa_sm &&
        (wpa_key_mgmt_ft(wpa_auth_sta_key_mgmt(sta->wpa_sm)) || sta->auth_alg == WLAN_AUTH_FT) &&
        !hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_MOBILITY_DOMAIN_ID) &&
        !radius_msg_add_attr_int32(msg, RADIUS_ATTR_MOBILITY_DOMAIN_ID, WPA_GET_BE16(hapd->conf->mobility_domain)))
    {
                           ...
                           struct radius_msg *msg)
{
    ...
    if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_NAS_IP_ADDRESS) &&
        ...
        !radius_msg_add_attr(msg, RADIUS_ATTR_NAS_IP_ADDRESS, (u8 *)&hapd->conf->own_ip_addr.u.v4, 4))
    {
    ...
    if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_NAS_IPV6_ADDRESS) &&
        ...
        !radius_msg_add_attr(msg, RADIUS_ATTR_NAS_IPV6_ADDRESS, (u8 *)&hapd->conf->own_ip_addr.u.v6, 16))
    {
    ...
    if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_NAS_IDENTIFIER) && hapd->conf->nas_identifier &&
        !radius_msg_add_attr(msg, RADIUS_ATTR_NAS_IDENTIFIER, (u8 *)hapd->conf->nas_identifier,
    ...
    len = os_snprintf(buf, sizeof(buf), RADIUS_802_1X_ADDR_FORMAT ":", MAC2STR(hapd->own_addr));
    os_memcpy(&buf[len], hapd->conf->ssid.ssid, hapd->conf->ssid.ssid_len);
    len += hapd->conf->ssid.ssid_len;
    if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_CALLED_STATION_ID) &&
        !radius_msg_add_attr(msg, RADIUS_ATTR_CALLED_STATION_ID, (u8 *)buf, len))
    {
    ...
    if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_NAS_PORT_TYPE) &&
        !radius_msg_add_attr_int32(msg, RADIUS_ATTR_NAS_PORT_TYPE, RADIUS_NAS_PORT_TYPE_IEEE_802_11))
    {
    ...
    if (hapd->conf->interworking && !is_zero_ether_addr(hapd->conf->hessid))
    {
        ...
        if (!hostapd_config_get_radius_attr(req_attr, RADIUS_ATTR_WLAN_HESSID) &&
            !radius_msg_add_attr(msg, RADIUS_ATTR_WLAN_HESSID, (u8 *)buf, os_strlen(buf)))
        {
...
int add_sqlite_radius_attr(struct hostapd_data *hapd, struct sta_info *sta, struct radius_msg *msg, int acct)
{
#ifdef CONFIG_SQLITE
    const char *attrtxt;
    ...
    if (sqlite3_prepare_v2(hapd->rad_attr_db, sql, os_strlen(sql), &stmt, NULL) != SQLITE_OK)
    {
        wpa_printf(MSG_ERROR, "DB: Failed to prepare SQL statement: %s", sqlite3_errmsg(hapd->rad_attr_db));
        return -1;
    }
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if (!attr)
        {
            wpa_printf(MSG_ERROR, "Skipping invalid attribute from SQL: %s", attrtxt);
            continue;
        }
        wpa_printf(MSG_DEBUG, "Adding RADIUS attribute from SQL: %s", attrtxt);
        hdr = radius_msg_add_attr(msg, attr->type, wpabuf_head(attr->val), wpabuf_len(attr->val));
        hostapd_config_free_radius_attr(attr);
        if (!hdr)
        {
            wpa_printf(MSG_ERROR, "Could not add RADIUS attribute from SQL");
            continue;
        }
    }
...
void ieee802_1x_encapsulate_radius(struct hostapd_data *hapd, struct sta_info *sta, const u8 *eap, size_t len)
{
    struct radius_msg *msg;
    struct eapol_state_machine *sm = sta->eapol_sm;
    ...
    {
        int res = radius_msg_copy_attr(msg, sm->last_recv_radius, RADIUS_ATTR_STATE);
        if (res < 0)
        {
    ...
    if (hapd->conf->hs20)
    {
        ...
        if (sta->roaming_consortium &&
            !radius_msg_add_wfa(msg, RADIUS_VENDOR_ATTR_WFA_HS20_ROAMING_CONSORTIUM,
                                wpabuf_head(sta->roaming_consortium), wpabuf_len(sta->roaming_consortium)))
        {
            wpa_printf(MSG_ERROR, "Could not add HS 2.0 Roaming Consortium");
            goto fail;
        }
        ...
        if (hapd->conf->t_c_filename)
        {
            ...
            if (!radius_msg_add_wfa(msg, RADIUS_VENDOR_ATTR_WFA_HS20_T_C_FILENAME, (const u8 *)hapd->conf->t_c_filename,
                                    os_strlen(hapd->conf->t_c_filename)))
            {
                wpa_printf(MSG_ERROR, "Could not add HS 2.0 T&C Filename");
                goto fail;
            }

            timestamp = host_to_be32(hapd->conf->t_c_timestamp);
            if (!radius_msg_add_wfa(msg, RADIUS_VENDOR_ATTR_WFA_HS20_TIMESTAMP, (const u8 *)&timestamp,
                                    sizeof(timestamp)))
            {
                wpa_printf(MSG_ERROR, "Could not add HS 2.0 Timestamp");
                goto fail;
            }
        }
    }
...
static void handle_eap_initiate(struct hostapd_data *hapd, struct sta_info *sta, struct eap_hdr *eap, size_t len)
{
    ...
    hostapd_logger(hapd, sm->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_DEBUG,
                   "received EAP packet (code=%d id=%d len=%d) from STA: EAP Initiate type %u", eap->code,
                   eap->identifier, be_to_host16(eap->length), type);
...
static void handle_eap(struct hostapd_data *hapd, struct sta_info *sta, u8 *buf, size_t len)
{
    ...
    wpa_printf(MSG_DEBUG, "EAP: code=%d (%s) identifier=%d length=%d", eap->code, eap_code_str(eap->code),
...
struct eapol_state_machine *ieee802_1x_alloc_eapol_sm(struct hostapd_data *hapd, struct sta_info *sta)
{
    int flags = 0;
    ...
    return eapol_auth_alloc(hapd->eapol_auth, sta->addr, flags, sta->wps_ie, sta->p2p_ie, sta, sta->identity,
                            sta->radius_cui);
}

static void ieee802_1x_save_eapol(struct sta_info *sta, const u8 *buf, size_t len)
{
...
void ieee802_1x_receive(struct hostapd_data *hapd, const u8 *sa, const u8 *buf, size_t len)
{
    ...
    if (!sta ||
        (!(sta->flags & (WLAN_STA_ASSOC | WLAN_STA_PREAUTH)) && !(hapd->iface->drv_flags & WPA_DRIVER_FLAGS_WIRED)))
    {
        wpa_printf(MSG_DEBUG, "IEEE 802.1X data frame from not associated/Pre-authenticating STA");

        if (sta && (sta->flags & WLAN_STA_AUTH))
        {
            wpa_printf(MSG_DEBUG, "Saving EAPOL frame from " MACSTR " for later use", MAC2STR(sta->addr));
    ...
    if (len - sizeof(*hdr) > datalen)
    {
        wpa_printf(MSG_DEBUG, "   ignoring %lu extra octets after IEEE 802.1X packet",
    ...
    key = (struct ieee802_1x_eapol_key *)(hdr + 1);
    if (datalen >= sizeof(struct ieee802_1x_eapol_key) && hdr->type == IEEE802_1X_TYPE_EAPOL_KEY &&
    ...
    if (key_mgmt != -1 &&
        (wpa_key_mgmt_wpa_psk(key_mgmt) || key_mgmt == WPA_KEY_MGMT_OWE || key_mgmt == WPA_KEY_MGMT_DPP))
    {
...
void ieee802_1x_new_station(struct hostapd_data *hapd, struct sta_info *sta)
{
    ...
    if (hapd->conf->wps_state &&
        ((hapd->conf->wpa && (sta->flags & WLAN_STA_MAYBE_WPS)) || (sta->flags & WLAN_STA_WPS)))
    {
        /*
    ...
    if (!sta->eapol_sm)
    {
        ...
        if (!sta->eapol_sm)
        {
            hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_INFO,
...
#ifdef CONFIG_FILS
    if (sta->auth_alg == WLAN_AUTH_FILS_SK || sta->auth_alg == WLAN_AUTH_FILS_SK_PFS ||
...
static void ieee802_1x_decapsulate_radius(struct hostapd_data *hapd, struct sta_info *sta)
{
    ...
    if (!eap)
    {
         ...
         * attribute */
        hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_WARNING,
    ...
    buf[sizeof(buf) - 1] = '\0';
    hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_DEBUG,
                   "decapsulated EAP packet (code=%d id=%d len=%d) from RADIUS server: %s", hdr->code, hdr->identifier,
                   be_to_host16(hdr->length), buf);
                                ...
                                size_t shared_secret_len)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "MS-MPPE: 1x_get_keys, could not get keys: %p  send: %p  recv: %p", keys,
                   keys ? keys->send : NULL, keys ? keys->recv : NULL);
    }
    ...
    if (radius_msg_get_attr_ptr(msg, RADIUS_ATTR_EAP_KEY_NAME, &buf, &len, NULL) == 0)
    {
        ...
        if (sm->eap_if->eapSessionId)
        {
            ...
            wpa_hexdump(MSG_DEBUG, "EAP-Key Name", sm->eap_if->eapSessionId, sm->eap_if->eapSessionIdLen);
        }
    }
    else
    {
...
static void ieee802_1x_store_radius_class(struct hostapd_data *hapd, struct sta_info *sta, struct radius_msg *msg)
{
    ...
    wpa_printf(MSG_DEBUG, "IEEE 802.1X: Stored %lu RADIUS Class attributes for " MACSTR,
...
static void ieee802_1x_hs20_sub_rem(struct sta_info *sta, u8 *pos, size_t len)
{
    ...
    if (len > 2)
    {
        ...
        wpa_printf(MSG_DEBUG, "HS 2.0: Subscription remediation needed for " MACSTR " - server method %u URL %s",
                   MAC2STR(sta->addr), sta->remediation_method, sta->remediation_url);
    }
    else
    {
...
static void ieee802_1x_hs20_session_info(
    struct hostapd_data *hapd, struct sta_info *sta, u8 *pos, size_t len, int session_timeout)
{
    unsigned int swt;
    ...
    wpa_printf(MSG_DEBUG, "HS 2.0: Session Information URL='%s' SWT=%u (session_timeout=%d)",
...
static void ieee802_1x_hs20_t_c_filtering(struct hostapd_data *hapd, struct sta_info *sta, u8 *pos, size_t len)
{
    if (len < 4)
        return; /* Malformed information */
    wpa_printf(MSG_DEBUG, "HS 2.0: Terms and Conditions filtering %02x %02x %02x %02x", pos[0], pos[1], pos[2], pos[3]);
    hs20_t_c_filtering(hapd, sta, pos[0] & BIT(0));
}

static void ieee802_1x_hs20_t_c_url(struct hostapd_data *hapd, struct sta_info *sta, u8 *pos, size_t len)
{
    os_free(sta->t_c_url);
    ...
    sta->t_c_url[len] = '\0';
    wpa_printf(MSG_DEBUG, "HS 2.0: Terms and Conditions URL %s", sta->t_c_url);
}
...
#ifndef CONFIG_NO_VLAN
static int ieee802_1x_update_vlan(struct radius_msg *msg, struct hostapd_data *hapd, struct sta_info *sta)
{
    struct vlan_description vlan_desc;

    os_memset(&vlan_desc, 0, sizeof(vlan_desc));
    vlan_desc.notempty = !!radius_msg_get_vlanid(msg, &vlan_desc.untagged, MAX_NUM_TAGGED_VLAN, vlan_desc.tagged);

    if (vlan_desc.notempty && !hostapd_vlan_valid(hapd->conf->vlan, &vlan_desc))
    {
        sta->eapol_sm->authFail = true;
        hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_INFO,
                       "Invalid VLAN %d%s received from RADIUS server", vlan_desc.untagged,
                       vlan_desc.tagged[0] ? "+" : "");
        os_memset(&vlan_desc, 0, sizeof(vlan_desc));
    ...
    if (hapd->conf->ssid.dynamic_vlan == DYNAMIC_VLAN_REQUIRED && !vlan_desc.notempty)
    {
        sta->eapol_sm->authFail = true;
        hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_INFO,
 ...
 */
static RadiusRxResult ieee802_1x_receive_auth(
    struct radius_msg *msg, struct radius_msg *req, const u8 *shared_secret, size_t shared_secret_len, void *data)
{
    ...
    if (hapd->conf->acct_interim_interval == 0 && hdr->code == RADIUS_CODE_ACCESS_ACCEPT &&
        radius_msg_get_attr_int32(msg, RADIUS_ATTR_ACCT_INTERIM_INTERVAL, &acct_interim_interval) == 0)
    {
        if (acct_interim_interval < 60)
        {
            hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_INFO,
                           "ignored too small Acct-Interim-Interval %d", acct_interim_interval);
        }
        else
    ...
    switch (hdr->code)
    {
...
#ifndef CONFIG_NO_VLAN
            if (hapd->conf->ssid.dynamic_vlan != DYNAMIC_VLAN_DISABLED && ieee802_1x_update_vlan(msg, hapd, sta) < 0)
                break;
        ...
        case RADIUS_CODE_ACCESS_REJECT:
            ...
            override_eapReq     = 1;
            if (radius_msg_get_attr_int32(msg, RADIUS_ATTR_WLAN_REASON_CODE, &reason_code) == 0)
            {
                wpa_printf(MSG_DEBUG, "RADIUS server indicated WLAN-Reason-Code %u in Access-Reject for " MACSTR,
                           reason_code, MAC2STR(sta->addr));
        ...
        case RADIUS_CODE_ACCESS_CHALLENGE:
            ...
            if (session_timeout_set)
            {
                ...
                hostapd_logger(hapd, sm->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_DEBUG,
                               "using EAP timeout of %d seconds (from RADIUS)", sm->eap_if->aaaMethodTimeout);
            }
            else
            {
                /*
...
#ifdef CONFIG_FILS
#ifdef NEED_AP_MLME
    if (sta->flags & (WLAN_STA_PENDING_FILS_ERP | WLAN_STA_PENDING_PASN_FILS_ERP))
    {
        /* TODO: Add a PMKSA entry on success? */
        ieee802_11_finish_fils_auth(hapd, sta, hdr->code == RADIUS_CODE_ACCESS_ACCEPT, sm->eap_if->aaaEapReqData,
                                    sm->eap_if->aaaEapKeyData, sm->eap_if->aaaEapKeyDataLen);
    }
#endif /* NEED_AP_MLME */
#endif /* CONFIG_FILS */
...
static int ieee802_1x_rekey_broadcast(struct hostapd_data *hapd)
{
    ...
    if (!eapol->default_wep_key || random_get_bytes(eapol->default_wep_key, hapd->conf->default_wep_key_len))
    {
    ...
    wpa_hexdump_key(MSG_DEBUG, "IEEE 802.1X: New default WEP key", eapol->default_wep_key,
                    hapd->conf->default_wep_key_len);
...
static void ieee802_1x_rekey(void *eloop_ctx, void *timeout_ctx)
{
    ...
    if (hostapd_drv_set_key(hapd->conf->iface, hapd, WPA_ALG_WEP, broadcast_ether_addr, eapol->default_wep_key_idx, 0,
...
static void ieee802_1x_eapol_send(void *ctx, void *sta_ctx, u8 type, const u8 *data, size_t datalen)
{
    ...
    if ((sta->flags & (WLAN_STA_WPS | WLAN_STA_MAYBE_WPS)) == WLAN_STA_MAYBE_WPS)
    {
        ...
        identity = eap_get_identity(sm->eap, &identity_len);
        if (identity &&
            ((identity_len == WSC_ID_ENROLLEE_LEN && os_memcmp(identity, WSC_ID_ENROLLEE, WSC_ID_ENROLLEE_LEN) == 0) ||
...
static int ieee802_1x_get_eap_user(
    void *ctx, const u8 *identity, size_t identity_len, int phase2, struct eap_user *user)
{
    struct hostapd_data *hapd = ctx;
    ...
    {
        user->password = os_memdup(eap_user->password, eap_user->password_len);
        if (!user->password)
            goto out;
        ...
        {
            user->salt = os_memdup(eap_user->salt, eap_user->salt_len);
            if (!user->salt)
                goto out;
...
int ieee802_1x_init(struct hostapd_data *hapd)
{
    ...
    if (hapd->conf->default_wep_key_len)
    {
        ...
        for (i = 0; i < 4; i++)
            hostapd_drv_set_key(hapd->conf->iface, hapd, WPA_ALG_NONE, NULL, i, 0, 0, NULL, 0, NULL, 0, KEY_FLAG_GROUP);
...
void ieee802_1x_deinit(struct hostapd_data *hapd)
{
    ...
    if (hapd->driver && hapd->drv_priv && (hapd->conf->ieee802_1x || hapd->conf->wpa))
...
int ieee802_1x_eapol_tx_status(struct hostapd_data *hapd, struct sta_info *sta, const u8 *buf, int len, int ack)
{
    ...
    if (len < (int)sizeof(*xhdr))
        return 0;
    wpa_printf(MSG_DEBUG, "IEEE 802.1X: " MACSTR " TX status - version=%d type=%d length=%d - ack=%d",
               MAC2STR(sta->addr), xhdr->version, xhdr->type, be_to_host16(xhdr->length), ack);

#ifdef CONFIG_WPS
    if (xhdr->type == IEEE802_1X_TYPE_EAP_PACKET && ack && (sta->flags & WLAN_STA_WPS) &&
        ap_sta_pending_delayed_1x_auth_fail_disconnect(hapd, sta))
    {
        wpa_printf(MSG_DEBUG, "WPS: Indicate EAP completion on ACK for EAP-Failure");
    ...
    if (!ack && pos + sizeof(*key) <= buf + len)
    {
        ...
        hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE8021X, HOSTAPD_LEVEL_DEBUG,
                       "did not Ack EAPOL-Key frame (%scast index=%d)", key->key_index & BIT(7) ? "uni" : "broad",
                       key->key_index & ~BIT(7));
...
{
    if (!sm || !sm->radius_class.attr || idx >= (int)sm->radius_class.count)
        return NULL;
...
#ifdef CONFIG_MACSEC
const u8 *ieee802_1x_get_session_id(struct eapol_state_machine *sm, size_t *len)
{
    *len = 0;
    if (!sm || !sm->eap_if)
        return NULL;
...
int ieee802_1x_get_mib_sta(struct hostapd_data *hapd, struct sta_info *sta, char *buf, size_t buflen)
{
    ...
    ret = os_snprintf(buf + len, buflen - len,
                      ...
                      sm->auth_pae_state + 1, sm->be_auth_state + 1, sm->adminControlledDirections,
                      sm->operControlledDirections, sm->authPortStatus, sm->portControl, sm->quietPeriod,
                      sm->serverTimeout, sm->reAuthPeriod, bool_txt(sm->reAuthEnabled), bool_txt(sm->keyTxEnabled));
    if (os_snprintf_error(buflen - len, ret))
        return len;
    len += ret;
    ...
    ret =
        os_snprintf(buf + len, buflen - len,
                    ...
                    sm->dot1xAuthEapolLogoffFramesRx, sm->dot1xAuthEapolRespIdFramesRx, sm->dot1xAuthEapolRespFramesRx,
                    sm->dot1xAuthEapolReqIdFramesTx, sm->dot1xAuthEapolReqFramesTx, sm->dot1xAuthInvalidEapolFramesRx,
                    sm->dot1xAuthEapLengthErrorFramesRx, sm->dot1xAuthLastEapolFrameVersion, MAC2STR(sm->addr));
    if (os_snprintf_error(buflen - len, ret))
        return len;
    len += ret;
    ...
    ret = os_snprintf(buf + len, buflen - len,
                      ...
                      sm->authEntersConnecting, sm->authEapLogoffsWhileConnecting, sm->authEntersAuthenticating,
                      sm->authAuthSuccessesWhileAuthenticating, sm->authAuthTimeoutsWhileAuthenticating,
                      sm->authAuthFailWhileAuthenticating, sm->authAuthEapStartsWhileAuthenticating,
                      sm->authAuthEapLogoffWhileAuthenticating, sm->authAuthReauthsWhileAuthenticated,
                      sm->authAuthEapStartsWhileAuthenticated, sm->authAuthEapLogoffWhileAuthenticated,
                      sm->backendResponses, sm->backendAccessChallenges, sm->backendOtherRequestsToSupplicant,
    ...
    ret = os_snprintf(buf + len, buflen - len,
                      ...
                      (wpa_key_mgmt_wpa_ieee8021x(wpa_auth_sta_key_mgmt(sta->wpa_sm))) ? 1 : 2, (unsigned int)diff.sec,
                      sm->identity ? (char *)sm->identity : (identity_buf ? identity_buf : "N/A"));
...
static void ieee802_1x_wnm_notif_send(void *eloop_ctx, void *timeout_ctx)
{
    ...
    if (sta->remediation)
    {
        wpa_printf(MSG_DEBUG, "HS 2.0: Send WNM-Notification to " MACSTR " to indicate Subscription Remediation",
                   MAC2STR(sta->addr));
    ...
    if (sta->hs20_deauth_req)
    {
        wpa_printf(MSG_DEBUG, "HS 2.0: Send WNM-Notification to " MACSTR " to indicate imminent deauthentication",
                   MAC2STR(sta->addr));
    ...
    {
        wpa_printf(MSG_DEBUG, "HS 2.0: Send WNM-Notification to " MACSTR " to indicate Terms and Conditions filtering",
                   MAC2STR(sta->addr));
...
static void ieee802_1x_finished(struct hostapd_data *hapd, struct sta_info *sta, int success, int remediation)
{
    ...
    if (success && (sta->remediation || sta->hs20_deauth_req || sta->hs20_t_c_filtering))
    {
        wpa_printf(MSG_DEBUG, "HS 2.0: Schedule WNM-Notification to " MACSTR " in 100 ms", MAC2STR(sta->addr));
        eloop_cancel_timeout(ieee802_1x_wnm_notif_send, hapd, sta);
    ...
    if (success && key && len >= PMK_LEN && !sta->remediation && !sta->hs20_deauth_requested &&
        wpa_auth_pmksa_add(sta->wpa_sm, key, len, session_timeout, sta->eapol_sm) == 0)
    {
        hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_WPA, HOSTAPD_LEVEL_DEBUG,
