...
void pmksa_cache_auth_flush(struct rsn_pmksa_cache *pmksa)
{
    while (pmksa->pmksa)
    {
        wpa_printf(MSG_DEBUG, "RSN: Flush PMKSA cache entry for " MACSTR, MAC2STR(pmksa->pmksa->spa));
                                                   ...
                                                   int akmp)
{
    ...
    entry = pmksa_cache_auth_create_entry(pmk, pmk_len, pmkid, kck, kck_len, aa, spa, session_timeout, eapol, akmp);
 ...
 */
struct rsn_pmksa_cache_entry *pmksa_cache_auth_create_entry(const u8 *pmk,
                                                            size_t pmk_len,
                                                            const u8 *pmkid,
                                                            const u8 *kck,
                                                            size_t kck_len,
 ...
 */
int pmksa_cache_auth_add_entry(struct rsn_pmksa_cache *pmksa, struct rsn_pmksa_cache_entry *entry)
{
                                                  ...
                                                  const u8 *pmkid)
{
    ...
    for (entry = pmksa->pmksa; entry; entry = entry->next)
    {
        if (os_memcmp(entry->spa, spa, ETH_ALEN) != 0)
            continue;
        if (wpa_key_mgmt_sae(entry->akmp) || wpa_key_mgmt_fils(entry->akmp))
        {
            if (os_memcmp(entry->pmkid, pmkid, PMKID_LEN) == 0)
                return entry;
            continue;
        }
        rsn_pmkid(entry->pmk, entry->pmk_len, aa, spa, new_pmkid, entry->akmp);
 ...
 */
struct rsn_pmksa_cache *pmksa_cache_auth_init(void (*free_cb)(struct rsn_pmksa_cache_entry *entry, void *ctx),
                                              void *ctx)
{
...
static int das_attr_match(struct rsn_pmksa_cache_entry *entry, struct radius_das_attrs *attr)
{
    ...
    if (attr->acct_multi_session_id)
    {
        ...
        if (attr->acct_multi_session_id_len != 16)
            return 0;
        os_snprintf(buf, sizeof(buf), "%016llX", (unsigned long long)entry->acct_multi_session_id);
...
int pmksa_cache_auth_list(struct rsn_pmksa_cache *pmksa, char *buf, size_t len)
{
    ...
    os_get_reltime(&now);
    ret = os_snprintf(pos, buf + len - pos, "Index / SPA / PMKID / expiration (in seconds) / opportunistic\n");
    ...
    while (entry)
    {
        ret = os_snprintf(pos, buf + len - pos, "%d " MACSTR " ", i, MAC2STR(entry->spa));
        if (os_snprintf_error(buf + len - pos, ret))
            return pos - buf;
        pos += ret;
 ...
 * in external storage.
 */
int pmksa_cache_auth_list_mesh(struct rsn_pmksa_cache *pmksa, const u8 *addr, char *buf, size_t len)
{
    int ret;
    char *pos, *end;
    struct rsn_pmksa_cache_entry *entry;
    ...
    for (entry = pmksa->pmksa; entry; entry = entry->next)
    {
        ...
        ret = os_snprintf(pos, end - pos, MACSTR " ", MAC2STR(entry->spa));
