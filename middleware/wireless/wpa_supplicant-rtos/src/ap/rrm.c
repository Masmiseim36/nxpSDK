...
static void hostapd_lci_rep_timeout_handler(void *eloop_data, void *user_ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "RRM: LCI request (token %u) timed out", hapd->lci_req_token);
    hapd->lci_req_active = 0;
}

static void hostapd_handle_lci_report(struct hostapd_data *hapd, u8 token, const u8 *pos, size_t len)
{
...
static void hostapd_range_rep_timeout_handler(void *eloop_data, void *user_ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "RRM: Range request (token %u) timed out", hapd->range_req_token);
    hapd->range_req_active = 0;
}

static void hostapd_handle_range_report(struct hostapd_data *hapd, u8 token, const u8 *pos, size_t len)
{
...
static void hostapd_handle_beacon_report(
    struct hostapd_data *hapd, const u8 *addr, u8 token, u8 rep_mode, const u8 *pos, size_t len)
{
    char report[2 * 255 + 1];

    wpa_printf(MSG_DEBUG, "Beacon report token %u len %zu from " MACSTR, token, len, MAC2STR(addr));
    ...
    if (wpa_snprintf_hex(report, sizeof(report), pos, len) < 0)
        return;
    wpa_msg(hapd->msg_ctx, MSG_INFO, BEACON_RESP_RX MACSTR " %u %02x %s", MAC2STR(addr), token, rep_mode, report);
}

static void hostapd_handle_radio_msmt_report(struct hostapd_data *hapd, const u8 *buf, size_t len)
{
    ...
    while ((ie = get_ie(pos, end - pos, WLAN_EID_MEASURE_REPORT)))
    {
        ...
        rep_mode = ie[3];
        wpa_printf(MSG_DEBUG, "Measurement report mode 0x%x type %u", rep_mode, ie[4]);
        ...
        switch (ie[4])
        {
            ...
            case MEASURE_TYPE_BEACON:
                hostapd_handle_beacon_report(hapd, mgmt->sa, token, rep_mode, ie + 2, ie[1]);
                break;
            default:
                wpa_printf(MSG_DEBUG, "Measurement report type %u is not supported", ie[4]);
                break;
        }
                                         ...
                                         u16 lci_max_age)
{
    ...
    dl_list_for_each(nr, &hapd->nr_db, struct hostapd_neighbor_entry, list)
    {
        ...
        if (ssid->ssid_len != nr->ssid.ssid_len || os_memcmp(ssid->ssid, nr->ssid.ssid, ssid->ssid_len) != 0)
            continue;
        ...
        if (len - 2 > 0xff)
        {
            wpa_printf(MSG_DEBUG, "NR entry for " MACSTR " exceeds 0xFF bytes", MAC2STR(nr->bssid));
            continue;
        }
    ...
    hostapd_drv_send_action(hapd, hapd->iface->freq, 0, addr, wpabuf_head(buf), wpabuf_len(buf));
    wpabuf_free(buf);
}

static void hostapd_handle_nei_report_req(struct hostapd_data *hapd, const u8 *buf, size_t len)
{
    const struct ieee80211_mgmt *mgmt = (const struct ieee80211_mgmt *)buf;
    const u8 *pos, *ie, *end;
    ...
    hostapd_send_nei_report_resp(hapd, mgmt->sa, token, &ssid, lci, civic, lci_max_age);
}

void hostapd_handle_radio_measurement(struct hostapd_data *hapd, const u8 *buf, size_t len)
{
    const struct ieee80211_mgmt *mgmt = (const struct ieee80211_mgmt *)buf;
    ...
    wpa_printf(MSG_DEBUG, "Radio measurement frame, action %u from " MACSTR, mgmt->u.action.u.rrm.action,
               MAC2STR(mgmt->sa));
    ...
    switch (mgmt->u.action.u.rrm.action)
    {
        ...
        case WLAN_RRM_NEIGHBOR_REPORT_REQUEST:
            ...
            break;
        default:
            wpa_printf(MSG_DEBUG, "RRM action %u is not supported", mgmt->u.action.u.rrm.action);
            break;
    }
}
...
int hostapd_send_lci_req(struct hostapd_data *hapd, const u8 *addr)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "Request LCI: LCI request is already in process, overriding");
        hapd->lci_req_active = 0;
        eloop_cancel_timeout(hostapd_lci_rep_timeout_handler, hapd, NULL);
    }
    ...
    eloop_register_timeout(HOSTAPD_RRM_REQUEST_TIMEOUT, 0, hostapd_lci_rep_timeout_handler, hapd, NULL);

    return 0;
}

int hostapd_send_range_req(struct hostapd_data *hapd,
                           const u8 *addr,
                           u16 random_interval,
                           u8 min_ap,
                           const u8 *responders,
                           unsigned int n_responders)
{
    ...
    wpa_printf(MSG_DEBUG, "Request range: dest addr " MACSTR " rand interval %u min AP %u n_responders %u",
    ...
    {
        wpa_printf(MSG_ERROR, "Request range: Destination station does not support FTM range report in RRM");
        return -1;
    }

    if (hapd->range_req_active)
    {
        wpa_printf(MSG_DEBUG, "Request range: Range request is already in process; overriding");
        hapd->range_req_active = 0;
        eloop_cancel_timeout(hostapd_range_rep_timeout_handler, hapd, NULL);
    }
    ...
    for (i = 0; i < n_responders; i++)
    {
        ...
        if (!nr)
        {
            wpa_printf(MSG_INFO, "Missing neighbor report for " MACSTR, MAC2STR(responders + ETH_ALEN * i));
            wpabuf_free(buf);
            return -1;
        }
    ...
    *len = wpabuf_len(buf) - 7;
    ...
    eloop_register_timeout(HOSTAPD_RRM_REQUEST_TIMEOUT, 0, hostapd_range_rep_timeout_handler, hapd, NULL);
...
int hostapd_send_beacon_req(struct hostapd_data *hapd, const u8 *addr, u8 req_mode, const struct wpabuf *req)
{
    struct wpabuf *buf;
    struct sta_info *sta = ap_get_sta(hapd, addr);
    int ret;
    ...
    if (!sta || !(sta->flags & WLAN_STA_AUTHORIZED))
    {
        wpa_printf(MSG_INFO, "Beacon request: " MACSTR " is not connected", MAC2STR(addr));
        return -1;
    }
    ...
    switch (mode)
    {
        case BEACON_REPORT_MODE_PASSIVE:
            if (!(sta->rrm_enabled_capa[0] & WLAN_RRM_CAPS_BEACON_REPORT_PASSIVE))
            {
                wpa_printf(MSG_INFO, "Beacon request: " MACSTR " does not support passive beacon report",
                           MAC2STR(addr));
                return -1;
            }
            break;
        case BEACON_REPORT_MODE_ACTIVE:
            if (!(sta->rrm_enabled_capa[0] & WLAN_RRM_CAPS_BEACON_REPORT_ACTIVE))
            {
                wpa_printf(MSG_INFO, "Beacon request: " MACSTR " does not support active beacon report", MAC2STR(addr));
                return -1;
            }
            break;
        case BEACON_REPORT_MODE_TABLE:
            if (!(sta->rrm_enabled_capa[0] & WLAN_RRM_CAPS_BEACON_REPORT_TABLE))
            {
                wpa_printf(MSG_INFO, "Beacon request: " MACSTR " does not support table beacon report", MAC2STR(addr));
                return -1;
            }
            break;
        default:
            wpa_printf(MSG_INFO, "Beacon request: Unknown measurement mode %d", mode);
            return -1;
    }
...
void hostapd_rrm_beacon_req_tx_status(struct hostapd_data *hapd, const struct ieee80211_mgmt *mgmt, size_t len, int ok)
{
    if (len < 24 + 3)
        return;
    wpa_msg(hapd->msg_ctx, MSG_INFO, BEACON_REQ_TX_STATUS MACSTR " %u ack=%d", MAC2STR(mgmt->da),
