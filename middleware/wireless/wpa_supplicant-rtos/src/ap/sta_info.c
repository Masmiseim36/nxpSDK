...
int ap_for_each_sta(struct hostapd_data *hapd,
                    int (*cb)(struct hostapd_data *hapd, struct sta_info *sta, void *ctx),
                    void *ctx)
{
    struct sta_info *sta;
...
void ap_free_sta_pasn(struct hostapd_data *hapd, struct sta_info *sta)
{
    if (sta->pasn)
    {
        wpa_printf(MSG_DEBUG, "PASN: Free PASN context: " MACSTR, MAC2STR(sta->addr));
...
void ap_handle_timer(void *eloop_ctx, void *timeout_ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "%s: %s: " MACSTR " flags=0x%x timeout_next=%d", hapd->conf->iface, __func__,
               MAC2STR(sta->addr), sta->flags, sta->timeout_next);
    ...
    if ((sta->flags & WLAN_STA_ASSOC) && (sta->timeout_next == STA_NULLFUNC || sta->timeout_next == STA_DISASSOC))
    {
        ...
        else if (inactive_sec < hapd->conf->ap_max_inactivity)
        {
            ...
            wpa_msg(hapd->msg_ctx, MSG_DEBUG, "Station " MACSTR " has been active %is ago", MAC2STR(sta->addr),
                    inactive_sec);
...
skip_poll:
    ...
    else if (sta->timeout_next != STA_REMOVE)
    {
        ...
        {
            reason = (sta->timeout_next == STA_DISASSOC) ? WLAN_REASON_DISASSOC_DUE_TO_INACTIVITY :
...
static void ap_handle_session_timer(void *eloop_ctx, void *timeout_ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "%s: Session timer for STA " MACSTR, hapd->conf->iface, MAC2STR(sta->addr));
...
static void ap_handle_session_warning_timer(void *eloop_ctx, void *timeout_ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "%s: WNM: Session warning time reached for " MACSTR, hapd->conf->iface, MAC2STR(sta->addr));
...
static int ap_sta_remove(struct hostapd_data *hapd, struct sta_info *sta)
{
    ...
    wpa_printf(MSG_DEBUG, "%s: Removing STA " MACSTR " from kernel driver", hapd->conf->iface, MAC2STR(sta->addr));
    if (hostapd_drv_sta_remove(hapd, sta->addr) && sta->flags & WLAN_STA_ASSOC)
    {
        wpa_printf(MSG_DEBUG, "%s: Could not remove station " MACSTR " from kernel driver", hapd->conf->iface,
                   MAC2STR(sta->addr));
        return -1;
    }
    sta->added_unassoc = 0;
...
static void ap_sta_remove_in_other_bss(struct hostapd_data *hapd, struct sta_info *sta)
{
    ...
    for (i = 0; i < iface->num_bss; i++)
    {
        ...
        wpa_printf(MSG_DEBUG, "%s: disconnect old STA " MACSTR " association from another BSS %s", hapd->conf->iface,
                   MAC2STR(sta2->addr), bss->conf->iface);
...
void ap_sta_disassociate(struct hostapd_data *hapd, struct sta_info *sta, u16 reason)
{
    ...
    sta->last_seq_ctrl = WLAN_INVALID_MGMT_SEQ;
    if (hapd->iface->current_mode && hapd->iface->current_mode->mode == HOSTAPD_MODE_IEEE80211AD)
    {
    ...
    eloop_register_timeout(hapd->iface->drv_flags & WPA_DRIVER_FLAGS_DEAUTH_TX_STATUS ? 2 : 0, 0,
...
int ap_sta_set_vlan(struct hostapd_data *hapd, struct sta_info *sta, struct vlan_description *vlan_desc)
{
    ...
    else if (hapd->conf->ssid.dynamic_vlan == DYNAMIC_VLAN_DISABLED && !hapd->conf->ssid.per_sta_vif && sta->vlan_id)
    {
    ...
    else if (vlan_desc && vlan_desc->notempty)
    {
        ...
        {
            hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG,
                           "missing vlan and wildcard for vlan=%d%s", vlan_desc->untagged,
                           vlan_desc->tagged[0] ? "+" : "");
            vlan_id = 0;
            ret     = -1;
            goto done;
    ...
    if (vlan && vlan->vlan_id == VLAN_ID_WILDCARD)
    {
        ...
        {
            hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG,
                           "could not add dynamic VLAN interface for vlan=%d%s", vlan_desc ? vlan_desc->untagged : -1,
        ...
        hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG,
                       "added new dynamic VLAN interface '%s'", vlan->ifname);
    }
...
int ap_sta_bind_vlan(struct hostapd_data *hapd, struct sta_info *sta)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "Do not override WDS VLAN assignment for STA " MACSTR, MAC2STR(sta->addr));
        return 0;
    }
    ...
    if (sta->vlan_id > 0 && !vlan && !(hapd->iface->drv_flags & WPA_DRIVER_FLAGS_VLAN_OFFLOAD))
    {
        hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG,
...
static void ap_sa_query_timer(void *eloop_ctx, void *timeout_ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "%s: SA Query timer for STA " MACSTR " (count=%d)", hapd->conf->iface, MAC2STR(sta->addr),
               sta->sa_query_count);
    ...
    hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG, "association SA Query attempt %d",
                   sta->sa_query_count);
...
const char *ap_sta_wpa_get_keyid(struct hostapd_data *hapd, struct sta_info *sta)
{
...
void ap_sta_set_authorized(struct hostapd_data *hapd, struct sta_info *sta, int authorized)
{
    ...
    if (authorized)
    {
        ...
        {
            os_snprintf(ip_addr, sizeof(ip_addr), " ip_addr=%u.%u.%u.%u", ip_addr_buf[0], ip_addr_buf[1],
        ...
        if (keyid)
        {
            os_snprintf(keyid_buf, sizeof(keyid_buf), " keyid=%s", keyid);
        }

        wpa_msg(hapd->msg_ctx, MSG_INFO, AP_STA_CONNECTED "%s%s%s", buf, ip_addr, keyid_buf);

        if (hapd->msg_ctx_parent && hapd->msg_ctx_parent != hapd->msg_ctx)
            wpa_msg_no_global(hapd->msg_ctx_parent, MSG_INFO, AP_STA_CONNECTED "%s%s%s", buf, ip_addr, keyid_buf);
    }
    else
    {
...
{
    if (sta)
        wpa_printf(MSG_DEBUG, "%s: %s STA " MACSTR " reason=%u", hapd->conf->iface, __func__, MAC2STR(sta->addr),
                   reason);
    else if (addr)
        wpa_printf(MSG_DEBUG, "%s: %s addr " MACSTR " reason=%u", hapd->conf->iface, __func__, MAC2STR(addr), reason);
    ...
    wpa_printf(MSG_DEBUG,
               ...
               " (%d seconds - "
               "AP_MAX_INACTIVITY_AFTER_DEAUTH)",
               hapd->conf->iface, __func__, MAC2STR(sta->addr), AP_MAX_INACTIVITY_AFTER_DEAUTH);
    eloop_cancel_timeout(ap_handle_timer, hapd, sta);
    eloop_register_timeout(AP_MAX_INACTIVITY_AFTER_DEAUTH, 0, ap_handle_timer, hapd, sta);
    sta->timeout_next = STA_REMOVE;
...
void ap_sta_clear_disconnect_timeouts(struct hostapd_data *hapd, struct sta_info *sta)
{
    if (eloop_cancel_timeout(ap_sta_deauth_cb_timeout, hapd, sta) > 0)
        wpa_printf(MSG_DEBUG, "%s: Removed ap_sta_deauth_cb_timeout timeout for " MACSTR, hapd->conf->iface,
                   MAC2STR(sta->addr));
    if (eloop_cancel_timeout(ap_sta_disassoc_cb_timeout, hapd, sta) > 0)
        wpa_printf(MSG_DEBUG, "%s: Removed ap_sta_disassoc_cb_timeout timeout for " MACSTR, hapd->conf->iface,
                   MAC2STR(sta->addr));
    if (eloop_cancel_timeout(ap_sta_delayed_1x_auth_fail_cb, hapd, sta) > 0)
    {
        wpa_printf(MSG_DEBUG, "%s: Removed ap_sta_delayed_1x_auth_fail_cb timeout for " MACSTR, hapd->conf->iface,
                   MAC2STR(sta->addr));
...
int ap_sta_flags_txt(u32 flags, char *buf, size_t buflen)
{
    ...
    buf[0] = '\0';
    res = os_snprintf(buf, buflen, "%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s", (flags & WLAN_STA_AUTH ? "[AUTH]" : ""),
                      (flags & WLAN_STA_ASSOC ? "[ASSOC]" : ""), (flags & WLAN_STA_AUTHORIZED ? "[AUTHORIZED]" : ""),
                      (flags & WLAN_STA_PENDING_POLL ? "[PENDING_POLL" : ""),
                      (flags & WLAN_STA_SHORT_PREAMBLE ? "[SHORT_PREAMBLE]" : ""),
                      (flags & WLAN_STA_PREAUTH ? "[PREAUTH]" : ""), (flags & WLAN_STA_WMM ? "[WMM]" : ""),
                      (flags & WLAN_STA_MFP ? "[MFP]" : ""), (flags & WLAN_STA_WPS ? "[WPS]" : ""),
                      (flags & WLAN_STA_MAYBE_WPS ? "[MAYBE_WPS]" : ""), (flags & WLAN_STA_WDS ? "[WDS]" : ""),
                      (flags & WLAN_STA_NONERP ? "[NonERP]" : ""), (flags & WLAN_STA_WPS2 ? "[WPS2]" : ""),
                      (flags & WLAN_STA_GAS ? "[GAS]" : ""), (flags & WLAN_STA_HT ? "[HT]" : ""),
                      (flags & WLAN_STA_VHT ? "[VHT]" : ""), (flags & WLAN_STA_HE ? "[HE]" : ""),
                      (flags & WLAN_STA_6GHZ ? "[6GHZ]" : ""), (flags & WLAN_STA_VENDOR_VHT ? "[VENDOR_VHT]" : ""),
...
static void ap_sta_delayed_1x_auth_fail_cb(void *eloop_ctx, void *timeout_ctx)
{
    ...
    wpa_dbg(hapd->msg_ctx, MSG_DEBUG, "IEEE 802.1X: Scheduled disconnection of " MACSTR " after EAP-Failure",
            MAC2STR(sta->addr));
...
{
    wpa_dbg(hapd->msg_ctx, MSG_DEBUG, "IEEE 802.1X: Force disconnection of " MACSTR " after EAP-Failure in 10 ms",
            MAC2STR(sta->addr));
    ...
    eloop_register_timeout(0, 10000, ap_sta_delayed_1x_auth_fail_cb, hapd, sta);
}

int ap_sta_pending_delayed_1x_auth_fail_disconnect(struct hostapd_data *hapd, struct sta_info *sta)
{
    return eloop_is_timeout_registered(ap_sta_delayed_1x_auth_fail_cb, hapd, sta);
}
...
int ap_sta_re_add(struct hostapd_data *hapd, struct sta_info *sta)
{
    ...
    if (hostapd_sta_add(hapd, sta->addr, 0, 0, sta->supported_rates, sta->supported_rates_len, 0, NULL, NULL, NULL, 0,
