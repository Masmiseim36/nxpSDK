...
static void wmm_set_regulatory_limit(const struct hostapd_wmm_ac_params *wmm_conf,
                                     ...
                                     const struct hostapd_wmm_rule *wmm_reg)
{
    ...
    for (ac = 0; ac < WMM_AC_NUM; ac++)
    {
        ...
        wmm[ac].txop_limit                  = MIN(wmm_conf[ac].txop_limit, wmm_reg[ac].max_txop);
        wmm[ac].admission_control_mandatory = wmm_conf[ac].admission_control_mandatory;
    }
}
 ...
 */
static void wmm_calc_regulatory_limit(struct hostapd_data *hapd, struct hostapd_wmm_ac_params *acp)
{
    struct hostapd_hw_modes *mode = hapd->iface->current_mode;
    int c;

    os_memcpy(acp, hapd->iconf->wmm_ac_params, sizeof(hapd->iconf->wmm_ac_params));
    ...
    for (c = 0; mode && c < mode->num_channels; c++)
    {
        ...
        if (chan->wmm_rules_valid)
            wmm_set_regulatory_limit(hapd->iconf->wmm_ac_params, acp, chan->wmm_rules);
        break;
    }
     ...
     */
    if (os_memcmp(acp, hapd->iface->prev_wmm, sizeof(hapd->iconf->wmm_ac_params)) != 0)
    {
        os_memcpy(hapd->iface->prev_wmm, acp, sizeof(hapd->iconf->wmm_ac_params));
...
int hostapd_eid_wmm_valid(struct hostapd_data *hapd, const u8 *eid, size_t len)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               wmm->oui[0], wmm->oui[1], wmm->oui[2], wmm->oui_type, wmm->oui_subtype, wmm->version, wmm->qos_info);
                            ...
                            u8 status_code)
{
    ...
    struct wmm_tspec_element *t = (struct wmm_tspec_element *)m->u.action.u.wmm_action.variable;
    int len;

    hostapd_logger(hapd, addr, HOSTAPD_MODULE_IEEE80211, HOSTAPD_LEVEL_DEBUG, "action response - reason %d",
                   status_code);
...
int wmm_process_tspec(struct wmm_tspec_element *tspec)
{
    ...
    duration =
        (le_to_host16(tspec->nominal_msdu_size) & 0x7fff) * 8 / (le_to_host32(tspec->minimum_phy_rate) / 1000000) +
                          ...
                          size_t len)
{
    ...
    wmm_send_action(hapd, mgmt->sa, &tspec_resp, WMM_ACTION_CODE_ADDTS_RESP, mgmt->u.action.u.wmm_action.dialog_token,
                    res);
}
...
void hostapd_wmm_action(struct hostapd_data *hapd, const struct ieee80211_mgmt *mgmt, size_t len)
{
    ...
    switch (action_code)
    {
        case WMM_ACTION_CODE_ADDTS_REQ:
            wmm_addts_req(hapd, mgmt, (struct wmm_tspec_element *)(elems.wmm_tspec - 2), len);
            return;
#if 0
