 ...
 */
static int wpa_ft_rrb_decrypt(const u8 *key,
                              ...
                              size_t *plain_size)
{
    ...
    wpa_printf(MSG_DEBUG, "FT(RRB): src_addr=" MACSTR " type=%u", MAC2STR(src_addr), type);
    ...
    *plain = os_zalloc(enc_len - AES_BLOCK_SIZE);
    ...
    if (aes_siv_decrypt(key, key_len, enc, enc_len, 3, ad, ad_len, *plain) < 0)
    {
    ...
    *plain_size = enc_len - AES_BLOCK_SIZE;
    wpa_hexdump_key(MSG_DEBUG, "FT(RRB): decrypted TLVs", *plain, *plain_size);
    return 0;
err:
    os_free(*plain);
    *plain      = NULL;
 ...
 */
static int wpa_ft_rrb_get_tlv(const u8 *plain, size_t plain_len, u16 type, size_t *tlv_len, const u8 **tlv_data)
{
...
static void wpa_ft_rrb_dump(const u8 *plain, const size_t plain_len)
{
    ...
    while (left >= sizeof(*f))
    {
        ...
        wpa_printf(MSG_DEBUG, "FT: RRB TLV type = %d, len = %zu", le_to_host16(f->type), len);

        if (left < len)
        {
            wpa_printf(MSG_DEBUG, "FT: RRB message truncated: left %zu bytes, need %zu", left, len);
            break;
        }
...
static int wpa_ft_rrb_get_tlv_vlan(const u8 *plain, const size_t plain_len, struct vlan_description *vlan)
{
    struct ft_rrb_tlv *f;
    size_t left;
    size_t len;
    ...
    while (left >= sizeof(*f))
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "FT: RRB VLAN_UNTAGGED invalid length");
            return -1;
        }

        if (type == FT_RRB_VLAN_TAGGED && len % sizeof(le16) != 0)
        {
            wpa_printf(MSG_DEBUG, "FT: RRB VLAN_TAGGED invalid length");
            return -1;
        }
        ...
        while (len >= sizeof(le16))
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "FT: RRB VLAN ID invalid %d", vlan_id);
                continue;
            }
            ...
            if (type == FT_RRB_VLAN_TAGGED && taggedidx < MAX_NUM_TAGGED_VLAN)
            {
...
static size_t wpa_ft_tlv_lin(const struct tlv_list *tlvs, u8 *start, u8 *endpos)
{
    int i;
    size_t tlv_len;
...
static size_t wpa_ft_vlan_lin(const struct vlan_description *vlan, u8 *start, u8 *endpos)
{
    size_t tlv_len;
    int i, len;
    struct ft_rrb_tlv *hdr;
...
static int wpa_ft_rrb_encrypt(const u8 *key,
                              const size_t key_len,
                              const u8 *plain,
                              ...
                              u8 *enc)
{
    ...
    wpa_printf(MSG_DEBUG, "FT(RRB): src_addr=" MACSTR " type=%u", MAC2STR(src_addr), type);
    wpa_hexdump_key(MSG_DEBUG, "FT(RRB): plaintext message", plain, plain_len);
    ...
    else if (aes_siv_encrypt(key, key_len, plain, plain_len, 3, ad, ad_len, enc) < 0)
    {
        wpa_printf(MSG_ERROR, "FT: Failed to encrypt RRB-OUI message");
        return -1;
    }
    wpa_hexdump(MSG_DEBUG, "FT(RRB): encrypted TLVs", enc, plain_len + AES_BLOCK_SIZE);
 ...
 * Returns: 0 on success, -1 on error
 */
static int wpa_ft_rrb_build(const u8 *key,
                            const size_t key_len,
...
#define RRB_MIN_MSG_LEN 64
    if (*packet_len < RRB_MIN_MSG_LEN)
    {
        ...
        wpa_printf(MSG_DEBUG, "FT: Pad message to minimum Ethernet frame length (%d --> %d)", (int)*packet_len,
                   (int)(*packet_len + pad_len));
        *packet_len += pad_len;
    ...
    *packet = os_zalloc(*packet_len);
    ...
    pos += auth_len;
    if (wpa_ft_rrb_encrypt(key, key_len, plain, plain_len, auth, auth_len, src_addr, type, pos) < 0)
        goto out;
...
#define RRB_GET_SRC(srcfield, type, field, txt, checklength)                                                    \
    do                                                                                                          \
    {                                                                                                           \
        if (wpa_ft_rrb_get_tlv(srcfield, srcfield##_len, type, &f_##field##_len, &f_##field) < 0 ||             \
            (checklength > 0 && ((size_t)checklength) != f_##field##_len))                                      \
        {                                                                                                       \
            wpa_printf(MSG_INFO, "FT: Missing required " #field " in %s from " MACSTR, txt, MAC2STR(src_addr)); \
...
#define RRB_GET_OPTIONAL_SRC(srcfield, type, field, txt, checklength)                                            \
    do                                                                                                           \
    {                                                                                                            \
        if (wpa_ft_rrb_get_tlv(srcfield, srcfield##_len, type, &f_##field##_len, &f_##field) < 0 ||              \
            (checklength > 0 && ((size_t)checklength) != f_##field##_len))                                       \
        {                                                                                                        \
            wpa_printf(MSG_DEBUG, "FT: Missing optional " #field " in %s from " MACSTR, txt, MAC2STR(src_addr)); \
            f_##field##_len = 0;                                                                                 \
            f_##field       = NULL;                                                                              \
        }                                                                                                        \
    } while (0)
...
static int wpa_ft_rrb_send(struct wpa_authenticator *wpa_auth, const u8 *dst, const u8 *data, size_t data_len)
{
    ...
    return wpa_auth->cb->send_ether(wpa_auth->cb_ctx, dst, ETH_P_RRB, data, data_len);
}

static int wpa_ft_rrb_oui_send(
    struct wpa_authenticator *wpa_auth, const u8 *dst, u8 oui_suffix, const u8 *data, size_t data_len)
{
    if (!wpa_auth->cb->send_oui)
        return -1;
    wpa_printf(MSG_DEBUG, "FT: RRB-OUI type %u send to " MACSTR " (len=%u)", oui_suffix, MAC2STR(dst),
               (unsigned int)data_len);
    return wpa_auth->cb->send_oui(wpa_auth->cb_ctx, dst, oui_suffix, data, data_len);
}
...
static int wpa_ft_action_send(struct wpa_authenticator *wpa_auth, const u8 *dst, const u8 *data, size_t data_len)
{
    if (wpa_auth->cb->send_ft_action == NULL)
        return -1;
    return wpa_auth->cb->send_ft_action(wpa_auth->cb_ctx, dst, data, data_len);
}
                                ...
                                const u8 *prev_psk)
{
    if (wpa_auth->cb->get_psk == NULL)
        return NULL;
    return wpa_auth->cb->get_psk(wpa_auth->cb_ctx, addr, p2p_dev_addr, prev_psk, NULL, NULL);
}
...
static int wpa_ft_set_vlan(struct wpa_authenticator *wpa_auth, const u8 *sta_addr, struct vlan_description *vlan)
{
...
static int wpa_ft_get_vlan(struct wpa_authenticator *wpa_auth, const u8 *sta_addr, struct vlan_description *vlan)
{
...
static int wpa_ft_set_identity(struct wpa_authenticator *wpa_auth,
                               const u8 *sta_addr,
                               const u8 *identity,
                               size_t identity_len)
{
    if (!wpa_auth->cb->set_identity)
        return -1;
    return wpa_auth->cb->set_identity(wpa_auth->cb_ctx, sta_addr, identity, identity_len);
}

static size_t wpa_ft_get_identity(struct wpa_authenticator *wpa_auth, const u8 *sta_addr, const u8 **buf)
{
    *buf = NULL;
...
static int wpa_ft_set_radius_cui(struct wpa_authenticator *wpa_auth,
                                 const u8 *sta_addr,
                                 const u8 *radius_cui,
                                 size_t radius_cui_len)
{
    if (!wpa_auth->cb->set_radius_cui)
        return -1;
    return wpa_auth->cb->set_radius_cui(wpa_auth->cb_ctx, sta_addr, radius_cui, radius_cui_len);
}

static size_t wpa_ft_get_radius_cui(struct wpa_authenticator *wpa_auth, const u8 *sta_addr, const u8 **buf)
{
    *buf = NULL;
...
static void wpa_ft_set_session_timeout(struct wpa_authenticator *wpa_auth, const u8 *sta_addr, int session_timeout)
{
    if (!wpa_auth->cb->set_session_timeout)
        return;
    wpa_auth->cb->set_session_timeout(wpa_auth->cb_ctx, sta_addr, session_timeout);
}

static int wpa_ft_get_session_timeout(struct wpa_authenticator *wpa_auth, const u8 *sta_addr)
{
...
static int wpa_ft_add_tspec(struct wpa_authenticator *wpa_auth, const u8 *sta_addr, u8 *tspec_ie, size_t tspec_ielen)
{
    ...
    return wpa_auth->cb->add_tspec(wpa_auth->cb_ctx, sta_addr, tspec_ie, tspec_ielen);
}

#ifdef CONFIG_OCV
static int wpa_channel_info(struct wpa_authenticator *wpa_auth, struct wpa_channel_info *ci)
{
...
int wpa_write_ftie(struct wpa_auth_config *conf,
                   int use_sha384,
                   const u8 *r0kh_id,
                   size_t r0kh_id_len,
                   const u8 *anonce,
...
{
    u8 *pos       = buf, *ielen;
    size_t hdrlen = use_sha384 ? sizeof(struct rsn_ftie_sha384) : sizeof(struct rsn_ftie);

    if (len < 2 + hdrlen + 2 + FT_R1KH_ID_LEN + 2 + r0kh_id_len + subelem_len)
        return -1;
...
static void wpa_ft_rrb_seq_flush(struct wpa_authenticator *wpa_auth, struct ft_remote_seq *rkh_seq, int cb)
{
    struct ft_remote_item *item, *n;

    dl_list_for_each_safe(item, n, &rkh_seq->rx.queue, struct ft_remote_item, list)
    {
        if (cb && item->cb)
            item->cb(wpa_auth, item->src_addr, item->enc, item->enc_len, item->auth, item->auth_len, 1);
...
static int wpa_ft_rrb_seq_req(struct wpa_authenticator *wpa_auth,
                                        ...
                                        int no_defer))
{
    ...
    struct tlv_list seq_req_auth[] = {
        {.type = FT_RRB_NONCE, .len = FT_RRB_NONCE_LEN, .data = NULL /* to be filled: item->nonce */},
        {.type = FT_RRB_R0KH_ID, .len = f_r0kh_id_len, .data = f_r0kh_id},
        {.type = FT_RRB_R1KH_ID, .len = FT_R1KH_ID_LEN, .data = f_r1kh_id},
    ...
    wpa_printf(MSG_DEBUG, "FT: Send sequence number request from " MACSTR " to " MACSTR, MAC2STR(wpa_auth->addr),
               MAC2STR(src_addr));
    ...
    eloop_register_timeout(ftRRBseqTimeout, 0, wpa_ft_rrb_seq_timeout, wpa_auth, item);

    seq_req_auth[0].data = item->nonce;

    if (wpa_ft_rrb_build(key, key_len, NULL, NULL, seq_req_auth, NULL, wpa_auth->addr, FT_PACKET_R0KH_R1KH_SEQ_REQ,
                         &packet, &packet_len) < 0)
    {
    ...
    wpa_ft_rrb_oui_send(wpa_auth, src_addr, FT_PACKET_R0KH_R1KH_SEQ_REQ, packet, packet_len);
...
static int wpa_ft_rrb_seq_chk(struct ft_remote_seq *rkh_seq,
                              const u8 *src_addr,
                              const u8 *enc,
                              size_t enc_len,
                              const u8 *auth,
                              ...
                              int no_defer)
{
    ...
    if (os_get_reltime(&now) == 0)
    {
        ...
        msg_ts_off        = le_to_host32(msg_both->ts) - (msg_ts_now_remote - ftRRBseqTimeout);
...
defer:
    ...
    wpa_printf(MSG_DEBUG, "FT: Possibly invalid sequence number in %s from " MACSTR, msgtype, MAC2STR(src_addr));

    return FT_RRB_SEQ_DEFER;
out:
    wpa_printf(MSG_DEBUG, "FT: Invalid sequence number in %s from " MACSTR, msgtype, MAC2STR(src_addr));

    return FT_RRB_SEQ_DROP;
}

static void wpa_ft_rrb_seq_accept(struct wpa_authenticator *wpa_auth,
                                  struct ft_remote_seq *rkh_seq,
                                  const u8 *src_addr,
                                  const u8 *auth,
                                  size_t auth_len,
...
static int wpa_ft_new_seq(struct ft_remote_seq *rkh_seq, struct ft_rrb_seq *f_seq)
{
    struct os_reltime now;
    ...
    if (!rkh_seq->tx.dom)
    {
        if (random_get_bytes((u8 *)&rkh_seq->tx.seq, sizeof(rkh_seq->tx.seq)))
        {
            wpa_printf(MSG_ERROR, "FT: Failed to get random data for sequence number initialization");
            rkh_seq->tx.seq = now.usec;
        }
        if (random_get_bytes((u8 *)&rkh_seq->tx.dom, sizeof(rkh_seq->tx.dom)))
        {
            wpa_printf(MSG_ERROR, "FT: Failed to get random data for sequence number initialization");
...
static void wpa_ft_expire_pmk_r0(void *eloop_ctx, void *timeout_ctx)
{
     ...
     */
    if ((!r0->expiration || expires_in > 0) && (!r0->session_timeout || session_timeout > 0))
    {
        wpa_printf(MSG_ERROR, "FT: %s() called for non-expired entry %p", __func__, r0);
        eloop_cancel_timeout(wpa_ft_expire_pmk_r0, r0, NULL);
        if (r0->expiration && expires_in > 0)
            eloop_register_timeout(expires_in + 1, 0, wpa_ft_expire_pmk_r0, r0, NULL);
        if (r0->session_timeout && session_timeout > 0)
            eloop_register_timeout(session_timeout + 1, 0, wpa_ft_expire_pmk_r0, r0, NULL);
        return;
    }
...
void wpa_ft_pmk_cache_deinit(struct wpa_ft_pmk_cache *cache)
{
    ...
    dl_list_for_each_safe(r0, r0prev, &cache->pmk_r0, struct wpa_ft_pmk_r0_sa, list) wpa_ft_free_pmk_r0(r0);

    dl_list_for_each_safe(r1, r1prev, &cache->pmk_r1, struct wpa_ft_pmk_r1_sa, list) wpa_ft_free_pmk_r1(r1);
                               ...
                               size_t radius_cui_len)
{
    ...
    if (expires_in > 0)
        eloop_register_timeout(expires_in + 1, 0, wpa_ft_expire_pmk_r1, r1, NULL);
    if (session_timeout > 0)
        eloop_register_timeout(session_timeout + 1, 0, wpa_ft_expire_pmk_r1, r1, NULL);
    ...
    *r0kh_out      = NULL;
    ...
    for (; r0kh; r0kh = r0kh->next)
    {
        if (r0kh->id_len == 1 && r0kh->id[0] == '*')
            *r0kh_wildcard = r0kh;
        if (f_r0kh_id && r0kh->id_len == f_r0kh_id_len && os_memcmp_const(f_r0kh_id, r0kh->id, f_r0kh_id_len) == 0)
            *r0kh_out = r0kh;
    }
                                   ...
                                   struct ft_remote_r1kh **r1kh_wildcard)
{
    ...
    *r1kh_out      = NULL;
    ...
    {
        if (is_zero_ether_addr(r1kh->addr) && is_zero_ether_addr(r1kh->id))
            *r1kh_wildcard = r1kh;
        if (f_r1kh_id && os_memcmp_const(r1kh->id, f_r1kh_id, FT_R1KH_ID_LEN) == 0)
            *r1kh_out = r1kh;
    }
...
static int wpa_ft_rrb_check_r0kh(struct wpa_authenticator *wpa_auth, const u8 *f_r0kh_id, size_t f_r0kh_id_len)
{
    if (f_r0kh_id_len != wpa_auth->conf.r0_key_holder_len ||
        os_memcmp_const(f_r0kh_id, wpa_auth->conf.r0_key_holder, f_r0kh_id_len) != 0)
        return -1;

    return 0;
}

static int wpa_ft_rrb_check_r1kh(struct wpa_authenticator *wpa_auth, const u8 *f_r1kh_id)
{
    if (os_memcmp_const(f_r1kh_id, wpa_auth->conf.r1_key_holder, FT_R1KH_ID_LEN) != 0)
        return -1;
...
static void wpa_ft_rrb_r0kh_replenish(struct wpa_authenticator *wpa_auth, struct ft_remote_r0kh *r0kh, int timeout)
{
    if (timeout > 0)
        eloop_replenish_timeout(timeout, 0, wpa_ft_rrb_del_r0kh, wpa_auth, r0kh);
}

static void wpa_ft_rrb_r0kh_timeout(struct wpa_authenticator *wpa_auth, struct ft_remote_r0kh *r0kh, int timeout)
{
    eloop_cancel_timeout(wpa_ft_rrb_del_r0kh, wpa_auth, r0kh);

    if (timeout > 0)
        eloop_register_timeout(timeout, 0, wpa_ft_rrb_del_r0kh, wpa_auth, r0kh);
}

static struct ft_remote_r0kh *wpa_ft_rrb_add_r0kh(struct wpa_authenticator *wpa_auth,
                                                  struct ft_remote_r0kh *r0kh_wildcard,
                                                  const u8 *src_addr,
                                                  const u8 *r0kh_id,
                                                  size_t id_len,
...
static void wpa_ft_rrb_r1kh_replenish(struct wpa_authenticator *wpa_auth, struct ft_remote_r1kh *r1kh, int timeout)
{
    if (timeout > 0)
        eloop_replenish_timeout(timeout, 0, wpa_ft_rrb_del_r1kh, wpa_auth, r1kh);
}

static struct ft_remote_r1kh *wpa_ft_rrb_add_r1kh(struct wpa_authenticator *wpa_auth,
                                                  struct ft_remote_r1kh *r1kh_wildcard,
                                                  const u8 *src_addr,
                                                  const u8 *r1kh_id,
                                                  int timeout)
{
    ...
    *wpa_auth->conf.r1kh_list = r1kh;
    ...
    if (timeout > 0)
        eloop_register_timeout(timeout, 0, wpa_ft_rrb_del_r1kh, wpa_auth, r1kh);
...
static void wpa_ft_deinit_rkh_tmp(struct wpa_authenticator *wpa_auth)
{
    ...
    {
        r0kh_next = r0kh->next;
        if (eloop_cancel_timeout(wpa_ft_rrb_del_r0kh, wpa_auth, r0kh) > 0)
        {
            if (r0kh_prev)
    ...
    {
        r1kh_next = r1kh->next;
        if (eloop_cancel_timeout(wpa_ft_rrb_del_r1kh, wpa_auth, r1kh) > 0)
        {
            if (r1kh_prev)
...
static void wpa_ft_block_r0kh(struct wpa_authenticator *wpa_auth, const u8 *f_r0kh_id, size_t f_r0kh_id_len)
{
    ...
    wpa_ft_rrb_lookup_r0kh(wpa_auth, f_r0kh_id, f_r0kh_id_len, &r0kh, &r0kh_wildcard);
    ...
    wpa_hexdump(MSG_DEBUG, "FT: Temporarily block R0KH-ID", f_r0kh_id, f_r0kh_id_len);

    if (r0kh)
    {
        wpa_ft_rrb_r0kh_timeout(wpa_auth, r0kh, wpa_auth->conf.rkh_neg_timeout);
        os_memset(r0kh->addr, 0, ETH_ALEN);
    }
    else
        wpa_ft_rrb_add_r0kh(wpa_auth, r0kh_wildcard, NULL, f_r0kh_id, f_r0kh_id_len, wpa_auth->conf.rkh_neg_timeout);
}
...
static void wpa_ft_expire_pull(void *eloop_ctx, void *timeout_ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "FT: Timeout pending pull request for " MACSTR, MAC2STR(sm->addr));
...
static int wpa_ft_pull_pmk_r1(struct wpa_state_machine *sm, const u8 *ies, size_t ies_len, const u8 *pmk_r0_name)
{
    ...
    struct tlv_list req_enc[] = {
        {.type = FT_RRB_PMK_R0_NAME, .len = WPA_PMK_NAME_LEN, .data = pmk_r0_name},
        {.type = FT_RRB_S1KH_ID, .len = ETH_ALEN, .data = sm->addr},
        {.type = FT_RRB_LAST_EMPTY, .len = 0, .data = NULL},
    };
    struct tlv_list req_auth[] = {
        {.type = FT_RRB_NONCE, .len = FT_RRB_NONCE_LEN, .data = sm->ft_pending_pull_nonce},
        {.type = FT_RRB_SEQ, .len = sizeof(f_seq), .data = (u8 *)&f_seq},
        {.type = FT_RRB_R0KH_ID, .len = sm->r0kh_id_len, .data = sm->r0kh_id},
        {.type = FT_RRB_R1KH_ID, .len = FT_R1KH_ID_LEN, .data = f_r1kh_id},
        {.type = FT_RRB_LAST_EMPTY, .len = 0, .data = NULL},
    };
    ...
    if (sm->ft_pending_pull_left_retries <= 0)
        return -1;
    first = sm->ft_pending_pull_left_retries == sm->wpa_auth->conf.rkh_pull_retries;
    sm->ft_pending_pull_left_retries--;

    wpa_ft_rrb_lookup_r0kh(sm->wpa_auth, sm->r0kh_id, sm->r0kh_id_len, &r0kh, &r0kh_wildcard);

    /* Keep r0kh sufficiently long in the list for seq num check */
    r0kh_timeout = sm->wpa_auth->conf.rkh_pull_timeout / 1000 + 1 + ftRRBseqTimeout;
    if (r0kh)
    {
    ...
    else if (r0kh_wildcard)
    {
        ...
        r0kh = wpa_ft_rrb_add_r0kh(sm->wpa_auth, r0kh_wildcard, r0kh_wildcard->addr, sm->r0kh_id, sm->r0kh_id_len,
                                   r0kh_timeout);
    }
    if (r0kh == NULL)
    {
    ...
    {
        wpa_hexdump(MSG_DEBUG, "FT: R0KH-ID is temporarily blocked", sm->r0kh_id, sm->r0kh_id_len);
        return -1;
    }
    if (os_memcmp(r0kh->addr, sm->wpa_auth->addr, ETH_ALEN) == 0)
    {
        wpa_printf(MSG_DEBUG, "FT: R0KH-ID points to self - no matching key available");
        return -1;
    }
    ...
    if (r0kh->seq->rx.num_last == 0)
    {
        ...
        wpa_ft_rrb_seq_req(sm->wpa_auth, r0kh->seq, r0kh->addr, r0kh->id, r0kh->id_len, f_r1kh_id, key, key_len, NULL,
                           0, NULL, 0, NULL);
    }

    wpa_printf(MSG_DEBUG, "FT: Send PMK-R1 pull request from " MACSTR " to remote R0KH address " MACSTR,
               MAC2STR(sm->wpa_auth->addr), MAC2STR(r0kh->addr));

    if (first && random_get_bytes(sm->ft_pending_pull_nonce, FT_RRB_NONCE_LEN) < 0)
    {
    ...
    if (wpa_ft_rrb_build(key, key_len, req_enc, NULL, req_auth, NULL, sm->wpa_auth->addr, FT_PACKET_R0KH_R1KH_PULL,
                         &packet, &packet_len) < 0)
        return -1;
    ...
    wpa_ft_rrb_oui_send(sm->wpa_auth, r0kh->addr, FT_PACKET_R0KH_R1KH_PULL, packet, packet_len);
...
int wpa_ft_store_pmk_fils(struct wpa_state_machine *sm, const u8 *pmk_r0, const u8 *pmk_r0_name)
{
    ...
    int session_timeout;
    size_t pmk_r0_len = wpa_key_mgmt_sha384(sm->wpa_key_mgmt) ? SHA384_MAC_LEN : PMK_LEN;

    if (wpa_ft_get_vlan(sm->wpa_auth, sm->addr, &vlan) < 0)
    {
        wpa_printf(MSG_DEBUG, "FT: vlan not available for STA " MACSTR, MAC2STR(sm->addr));
        return -1;
    }

    identity_len    = wpa_ft_get_identity(sm->wpa_auth, sm->addr, &identity);
    radius_cui_len  = wpa_ft_get_radius_cui(sm->wpa_auth, sm->addr, &radius_cui);
    session_timeout = wpa_ft_get_session_timeout(sm->wpa_auth, sm->addr);

    return wpa_ft_store_pmk_r0(sm->wpa_auth, sm->addr, pmk_r0, pmk_r0_len, pmk_r0_name, sm->pairwise, &vlan, expires_in,
                               session_timeout, identity, identity_len, radius_cui, radius_cui_len);
}
...
int wpa_auth_derive_ptk_ft(struct wpa_state_machine *sm, struct wpa_ptk *ptk)
{
    ...
    if (wpa_derive_pmk_r0(mpmk, mpmk_len, ssid, ssid_len, mdid, r0kh, r0kh_len, sm->addr, pmk_r0, pmk_r0_name,
                          wpa_key_mgmt_sha384(sm->wpa_key_mgmt)) < 0)
        return -1;
    if (!psk_local || !wpa_key_mgmt_ft_psk(sm->wpa_key_mgmt))
        wpa_ft_store_pmk_r0(sm->wpa_auth, sm->addr, pmk_r0, pmk_r0_len, pmk_r0_name, sm->pairwise, &vlan, expires_in,
                            session_timeout, identity, identity_len, radius_cui, radius_cui_len);

    if (wpa_derive_pmk_r1(pmk_r0, pmk_r0_len, pmk_r0_name, r1kh, sm->addr, pmk_r1, sm->pmk_r1_name) < 0)
        return -1;
    if (!psk_local || !wpa_key_mgmt_ft_psk(sm->wpa_key_mgmt))
        wpa_ft_store_pmk_r1(sm->wpa_auth, sm->addr, pmk_r1, pmk_r1_len, sm->pmk_r1_name, sm->pairwise, &vlan,
                            expires_in, session_timeout, identity, identity_len, radius_cui, radius_cui_len);

    return wpa_pmk_r1_to_ptk(pmk_r1, pmk_r1_len, sm->SNonce, sm->ANonce, sm->addr, sm->wpa_auth->addr, sm->pmk_r1_name,
                             ptk, ptk_name, sm->wpa_key_mgmt, sm->pairwise, 0);
}
...
static u8 *wpa_ft_gtk_subelem(struct wpa_state_machine *sm, size_t *len)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "FT: GTK subelem encryption failed: kek_len=%d", (int)kek_len);
...
static u8 *wpa_ft_igtk_subelem(struct wpa_state_machine *sm, size_t *len)
{
    ...
    *pos++ = igtk_len;
    ...
    {
        wpa_printf(MSG_DEBUG, "FT: IGTK subelem encryption failed: kek_len=%d", (int)kek_len);
        os_free(subelem);
        return NULL;
    }
...
static u8 *wpa_ft_bigtk_subelem(struct wpa_state_machine *sm, size_t *len)
{
    ...
    *pos++ = bigtk_len;
    ...
    {
        wpa_printf(MSG_DEBUG, "FT: BIGTK subelem encryption failed: kek_len=%d", (int)kek_len);
        os_free(subelem);
        return NULL;
    }
...
static u8 *wpa_ft_process_rdie(
    struct wpa_state_machine *sm, u8 *pos, u8 *end, u8 id, u8 descr_count, const u8 *ies, size_t ies_len)
{
...
u8 *wpa_sm_write_assoc_resp_ies(struct wpa_state_machine *sm,
                                u8 *pos,
                                size_t max_len,
                                int auth_alg,
                                const u8 *req_ies,
                                ...
                                int omit_rsnxe)
{
    ...
    if (auth_alg == WLAN_AUTH_FT && sm->wpa_auth->conf.rsne_override_ft_set)
    {
        wpa_printf(MSG_DEBUG, "TESTING: RSNE FT override for MIC calculation");
        ...
        if (rsnie_len > PMKID_LEN && sm->pmk_r1_name_valid)
        {
            ...
            if (idx == PMKID_LEN)
                os_memcpy(&rsnie[rsnie_len - PMKID_LEN], sm->pmk_r1_name, WPA_PMK_NAME_LEN);
        }
    }
    else
#endif /* CONFIG_TESTING_OPTIONS */
        if (auth_alg == WLAN_AUTH_FT ||
            ((auth_alg == WLAN_AUTH_FILS_SK || auth_alg == WLAN_AUTH_FILS_SK_PFS || auth_alg == WLAN_AUTH_FILS_PK) &&
             (sm->wpa_key_mgmt & (WPA_KEY_MGMT_FT_FILS_SHA256 | WPA_KEY_MGMT_FT_FILS_SHA384))))
    {
        if (!sm->pmk_r1_name_valid)
        {
            wpa_printf(MSG_ERROR, "FT: PMKR1Name is not valid for Assoc Resp RSNE");
            return NULL;
        }
        wpa_hexdump(MSG_DEBUG, "FT: PMKR1Name for Assoc Resp RSNE", sm->pmk_r1_name, WPA_PMK_NAME_LEN);
        /*
    ...
    if (auth_alg == WLAN_AUTH_FT)
    {
        ...
        if (sm->mgmt_frame_prot)
        {
            ...
            if (igtk == NULL)
            {
                wpa_printf(MSG_DEBUG, "FT: Failed to add IGTK subelement");
                os_free(subelem);
                return NULL;
            }
        ...
        if (sm->mgmt_frame_prot && conf->beacon_prot)
        {
            ...
            if (!bigtk)
            {
                wpa_printf(MSG_DEBUG, "FT: Failed to add BIGTK subelement");
                os_free(subelem);
                return NULL;
            }
        ...
        if (wpa_auth_uses_ocv(sm))
        {
            ...
            {
                wpa_printf(MSG_WARNING, "Failed to get channel info for OCI element");
                os_free(subelem);
                return NULL;
            }
#ifdef CONFIG_TESTING_OPTIONS
            ...
            {
                wpa_printf(MSG_INFO, "TEST: Override OCI frequency %d -> %u MHz", ci.frequency,
    ...
    rsnxe_used = (auth_alg == WLAN_AUTH_FT) && (conf->sae_pwe == 1 || conf->sae_pwe == 2);
#ifdef CONFIG_TESTING_OPTIONS
    if (sm->wpa_auth->conf.ft_rsnxe_used)
    {
        rsnxe_used = sm->wpa_auth->conf.ft_rsnxe_used == 1;
        wpa_printf(MSG_DEBUG, "TESTING: FT: Force RSNXE Used %d", rsnxe_used);
    }
#endif /* CONFIG_TESTING_OPTIONS */
    ...
    if (use_sha384)
    {
        struct rsn_ftie_sha384 *_ftie = (struct rsn_ftie_sha384 *)(ftie + 2);
    ...
    ric_start = pos;
    if (wpa_ft_parse_ies(req_ies, req_ies_len, &parse, use_sha384) == 0 && parse.ric)
    {
        pos = wpa_ft_process_ric(sm, pos, end, parse.ric, parse.ric_len);
        if (auth_alg == WLAN_AUTH_FT)
            *elem_count += ieee802_11_ie_count(ric_start, pos - ric_start);
    }
    if (ric_start == pos)
        ric_start = NULL;
...
#ifdef CONFIG_TESTING_OPTIONS
    if (auth_alg == WLAN_AUTH_FT && sm->wpa_auth->conf.rsnxe_override_ft_set)
    {
        wpa_printf(MSG_DEBUG, "TESTING: RSNXE FT override for MIC calculation");
    ...
    if (auth_alg == WLAN_AUTH_FT &&
        wpa_ft_mic(kck, kck_len, sm->addr, sm->wpa_auth->addr, 6, mdie, mdie_len, ftie, ftie_len, rsnie, rsnie_len,
                   ric_start, ric_start ? pos - ric_start : 0, rsnxe_len ? rsnxe : NULL, rsnxe_len, fte_mic) < 0)
    {
                                   ...
                                   enum key_flag key_flag)
{
    if (wpa_auth->cb->set_key == NULL)
        return -1;
    return wpa_auth->cb->set_key(wpa_auth->cb_ctx, vlan_id, alg, addr, idx, key, key_len, key_flag);
}

static inline int wpa_auth_add_sta_ft(struct wpa_authenticator *wpa_auth, const u8 *addr)
{
...
void wpa_ft_install_ptk(struct wpa_state_machine *sm, int retry)
{
     ...
     */
    if (wpa_auth_set_key(sm->wpa_auth, 0, alg, sm->addr, sm->keyidx_active, sm->PTK.tk, klen, KEY_FLAG_PAIRWISE_RX_TX))
        return;
                             ...
                             int *out_session_timeout)
{
    ...
    for (;;)
    {
        pmk = wpa_ft_get_psk(wpa_auth, sm->addr, sm->p2p_dev_addr, pmk);
        if (pmk == NULL)
            break;

        if (wpa_derive_pmk_r0(pmk, PMK_LEN, ssid, ssid_len, mdid, r0kh, r0kh_len, sm->addr, pmk_r0, pmk_r0_name, 0) <
                0 ||
            wpa_derive_pmk_r1(pmk_r0, PMK_LEN, pmk_r0_name, r1kh, sm->addr, pmk_r1, pmk_r1_name) < 0 ||
            os_memcmp_const(pmk_r1_name, req_pmk_r1_name, WPA_PMK_NAME_LEN) != 0)
            continue;

        /* We found a PSK that matches the requested pmk_r1_name */
        wpa_printf(MSG_DEBUG, "FT: Found PSK to generate PMK-R1 locally");
        os_memcpy(out_pmk_r1, pmk_r1, PMK_LEN);
        if (out_pairwise)
            *out_pairwise = pairwise;
        os_memcpy(sm->PMK, pmk, PMK_LEN);
        sm->pmk_len = PMK_LEN;
        if (out_vlan && wpa_ft_get_vlan(sm->wpa_auth, sm->addr, out_vlan) < 0)
        {
            wpa_printf(MSG_DEBUG, "FT: vlan not available for STA " MACSTR, MAC2STR(sm->addr));
            return -1;
        }
        ...
        {
            *out_identity_len = wpa_ft_get_identity(sm->wpa_auth, sm->addr, out_identity);
        }

        if (out_radius_cui && out_radius_cui_len)
        {
            *out_radius_cui_len = wpa_ft_get_radius_cui(sm->wpa_auth, sm->addr, out_radius_cui);
        }
    ...
    wpa_printf(MSG_DEBUG, "FT: Did not find PSK to generate PMK-R1 locally");
    return -1;
}
 ...
 */
static int wpa_ft_set_key_mgmt(struct wpa_state_machine *sm, struct wpa_ft_ies *parse)
{
    ...
    if (!key_mgmt)
    {
        wpa_printf(MSG_DEBUG, "FT: Invalid key mgmt (0x%x) from " MACSTR, parse->key_mgmt, MAC2STR(sm->addr));
        return -1;
    }
    ...
    if (!ciphers)
    {
        wpa_printf(MSG_DEBUG, "FT: Invalid pairwise cipher (0x%x) from " MACSTR, parse->pairwise_cipher,
                   MAC2STR(sm->addr));
        return -1;
    }
                                      ...
                                      int *out_session_timeout)
{
    ...
    if (conf->r0_key_holder_len != r0kh_id_len || os_memcmp(conf->r0_key_holder, r0kh_id, conf->r0_key_holder_len) != 0)
        return -1; /* not our R0KH-ID */

    wpa_printf(MSG_DEBUG, "FT: STA R0KH-ID matching local configuration");
    if (wpa_ft_fetch_pmk_r0(sm->wpa_auth, sm->addr, req_pmk_r0_name, &r0) < 0)
        return -1; /* no matching PMKR0Name in local cache */

    wpa_printf(MSG_DEBUG, "FT: Requested PMKR0Name found in local cache");

    if (wpa_derive_pmk_r1(r0->pmk_r0, r0->pmk_r0_len, r0->pmk_r0_name, conf->r1_key_holder, sm->addr, out_pmk_r1,
                          pmk_r1_name) < 0)
        return -1;
    ...
    wpa_ft_store_pmk_r1(wpa_auth, sm->addr, out_pmk_r1, r0->pmk_r0_len, pmk_r1_name, sm->pairwise, r0->vlan, expires_in,
                        session_timeout, r0->identity, r0->identity_len, r0->radius_cui, r0->radius_cui_len);
...
static int wpa_ft_process_auth_req(
    struct wpa_state_machine *sm, const u8 *ies, size_t ies_len, u8 **resp_ies, size_t *resp_ies_len)
{
    ...
    *resp_ies_len = 0;
    ...
    if (mdie == NULL || parse.mdie_len < sizeof(*mdie) ||
        os_memcmp(mdie->mobility_domain, sm->wpa_auth->conf.mobility_domain, MOBILITY_DOMAIN_ID_LEN) != 0)
    {
    ...
    if (wpa_derive_pmk_r1_name(parse.rsn_pmkid, sm->wpa_auth->conf.r1_key_holder, sm->addr, pmk_r1_name, use_sha384) <
        0)
        return WLAN_STATUS_UNSPECIFIED_FAILURE;

    if (conf->ft_psk_generate_local && wpa_key_mgmt_ft_psk(sm->wpa_key_mgmt))
    {
        if (wpa_ft_psk_pmk_r1(sm, pmk_r1_name, pmk_r1, &pairwise, &vlan, &identity, &identity_len, &radius_cui,
                              &radius_cui_len, &session_timeout) < 0)
            return WLAN_STATUS_INVALID_PMKID;
        wpa_printf(MSG_DEBUG, "FT: Generated PMK-R1 for FT-PSK locally");
    }
    else if (wpa_ft_fetch_pmk_r1(sm->wpa_auth, sm->addr, pmk_r1_name, pmk_r1, &pmk_r1_len, &pairwise, &vlan, &identity,
                                 &identity_len, &radius_cui, &radius_cui_len, &session_timeout) < 0)
    {
        wpa_printf(MSG_DEBUG, "FT: No PMK-R1 available in local cache for the requested PMKR1Name");
        if (wpa_ft_local_derive_pmk_r1(sm->wpa_auth, sm, parse.r0kh_id, parse.r0kh_id_len, parse.rsn_pmkid, pmk_r1_name,
                                       pmk_r1, &pairwise, &vlan, &identity, &identity_len, &radius_cui, &radius_cui_len,
                                       &session_timeout) == 0)
        {
            wpa_printf(MSG_DEBUG, "FT: Generated PMK-R1 based on local PMK-R0");
            goto pmk_r1_derived;
        }
...
pmk_r1_derived:
    ...
    if (sm->wpa_auth->conf.force_kdk_derivation ||
        (sm->wpa_auth->conf.secure_ltf && ieee802_11_rsnx_capab(sm->rsnxe, WLAN_RSNX_CAPAB_SECURE_LTF)))
        kdk_len = WPA_KDK_MAX_LEN;
    else
        kdk_len = 0;

    if (wpa_pmk_r1_to_ptk(pmk_r1, pmk_r1_len, sm->SNonce, sm->ANonce, sm->addr, sm->wpa_auth->addr, pmk_r1_name,
                          &sm->PTK, ptk_name, sm->wpa_key_mgmt, pairwise, kdk_len) < 0)
        return WLAN_STATUS_UNSPECIFIED_FAILURE;
    ...
    if (wpa_ft_set_identity(sm->wpa_auth, sm->addr, identity, identity_len) < 0 ||
        wpa_ft_set_radius_cui(sm->wpa_auth, sm->addr, radius_cui, radius_cui_len) < 0)
    {
    ...
    *resp_ies = os_zalloc(buflen);
    ...
    ret = wpa_write_ftie(conf, use_sha384, parse.r0kh_id, parse.r0kh_id_len, sm->ANonce, sm->SNonce, pos, end - pos,
                         NULL, 0, 0);
    if (ret < 0)
        goto fail;
    pos += ret;
...
void wpa_ft_process_auth(
    ...
    size_t ies_len,
    void (*cb)(
        void *ctx, const u8 *dst, const u8 *bssid, u16 auth_transaction, u16 status, const u8 *ies, size_t ies_len),
    void *ctx)
{
    u16 status;
    u8 *resp_ies;
    ...
    wpa_printf(MSG_DEBUG, "FT: Received authentication frame: STA=" MACSTR " BSSID=" MACSTR " transaction=%d",
    ...
    wpa_printf(MSG_DEBUG, "FT: FT authentication response: dst=" MACSTR " auth_transaction=%d status=%u (%s)",
               MAC2STR(sm->addr), auth_transaction + 1, status, status2str(status));
    wpa_hexdump(MSG_DEBUG, "FT: Response IEs", resp_ies, resp_ies_len);
    cb(ctx, sm->addr, bssid, auth_transaction + 1, status, resp_ies, resp_ies_len);
    os_free(resp_ies);
}

int wpa_ft_validate_reassoc(struct wpa_state_machine *sm, const u8 *ies, size_t ies_len)
{
    struct wpa_ft_ies parse;
    struct rsn_mdie *mdie;
    ...
    if (os_memcmp_const(parse.rsn_pmkid, sm->pmk_r1_name, WPA_PMK_NAME_LEN) != 0)
    {
        wpa_printf(MSG_DEBUG,
    ...
    mdie = (struct rsn_mdie *)parse.mdie;
    if (mdie == NULL || parse.mdie_len < sizeof(*mdie) ||
        os_memcmp(mdie->mobility_domain, conf->mobility_domain, MOBILITY_DOMAIN_ID_LEN) != 0)
    {
        wpa_printf(MSG_DEBUG, "FT: Invalid MDIE");
        return WLAN_STATUS_INVALID_MDIE;
    }
    ...
    if (os_memcmp(snonce, sm->SNonce, WPA_NONCE_LEN) != 0)
    {
        ...
        wpa_hexdump(MSG_DEBUG, "FT: Received SNonce", snonce, WPA_NONCE_LEN);
    ...
    if (os_memcmp(anonce, sm->ANonce, WPA_NONCE_LEN) != 0)
    {
        ...
        wpa_hexdump(MSG_DEBUG, "FT: Received ANonce", anonce, WPA_NONCE_LEN);
    ...
    if (os_memcmp_const(parse.r1kh_id, conf->r1_key_holder, FT_R1KH_ID_LEN) != 0)
    {
        wpa_printf(MSG_DEBUG,
                   "FT: Unknown R1KH-ID used in "
                   "ReassocReq");
        wpa_hexdump(MSG_DEBUG, "FT: R1KH-ID in FTIE", parse.r1kh_id, FT_R1KH_ID_LEN);
        wpa_hexdump(MSG_DEBUG, "FT: Expected R1KH-ID", conf->r1_key_holder, FT_R1KH_ID_LEN);
        return WLAN_STATUS_INVALID_FTIE;
    }
    ...
    if (wpa_ft_mic(kck, kck_len, sm->addr, sm->wpa_auth->addr, 5, parse.mdie - 2, parse.mdie_len + 2, parse.ftie - 2,
                   parse.ftie_len + 2, parse.rsn - 2, parse.rsn_len + 2, parse.ric, parse.ric_len,
                   parse.rsnxe ? parse.rsnxe - 2 : NULL, parse.rsnxe ? parse.rsnxe_len + 2 : 0, mic) < 0)
    {
        wpa_printf(MSG_DEBUG, "FT: Failed to calculate MIC");
        return WLAN_STATUS_UNSPECIFIED_FAILURE;
    }
    ...
    if (os_memcmp_const(mic, fte_mic, mic_len) != 0)
    {
        ...
        wpa_hexdump(MSG_MSGDUMP, "FT: Received MIC", fte_mic, mic_len);
        ...
        wpa_hexdump(MSG_MSGDUMP, "FT: RSNXE", parse.rsnxe ? parse.rsnxe - 2 : NULL,
                    parse.rsnxe ? parse.rsnxe_len + 2 : 0);
        return WLAN_STATUS_INVALID_FTIE;
    }

    if (rsnxe_used && (conf->sae_pwe == 1 || conf->sae_pwe == 2) && !parse.rsnxe)
    {
        wpa_printf(MSG_INFO, "FT: FTE indicated that STA uses RSNXE, but RSNXE was not included");
    ...
    if (wpa_auth_uses_ocv(sm))
    {
        ...
        if (wpa_channel_info(sm->wpa_auth, &ci) != 0)
        {
            wpa_printf(MSG_WARNING, "Failed to get channel info to validate received OCI in (Re)Assoc Request");
            return WLAN_STATUS_UNSPECIFIED_FAILURE;
        }

        if (get_sta_tx_parameters(sm, channel_width_to_int(ci.chanwidth), ci.seg1_idx, &tx_chanwidth, &tx_seg1_idx) < 0)
            return WLAN_STATUS_UNSPECIFIED_FAILURE;
        ...
        if (wpa_auth_uses_ocv(sm) == 2 && res == OCI_NOT_FOUND)
        {
            ...
            wpa_printf(MSG_INFO, "Disable OCV with a STA that does not send OCI");
        ...
        else if (res != OCI_SUCCESS)
        {
            ...
            if (sm->wpa_auth->conf.msg_ctx)
                wpa_msg(sm->wpa_auth->conf.msg_ctx, MSG_INFO,
                        OCV_FAILURE "addr=" MACSTR " frame=ft-reassoc-req error=%s", MAC2STR(sm->addr), ocv_errorstr);
            return WLAN_STATUS_INVALID_FTIE;
        }
    }
#endif /* CONFIG_OCV */
...
int wpa_ft_action_rx(struct wpa_state_machine *sm, const u8 *data, size_t len)
{
    ...
    wpa_printf(MSG_DEBUG, "FT: Received FT Action frame (STA=" MACSTR " Target AP=" MACSTR " Action=%d)",
...
static void wpa_ft_rrb_rx_request_cb(
    void *ctx, const u8 *dst, const u8 *bssid, u16 auth_transaction, u16 resp, const u8 *ies, size_t ies_len)
{
...
static int wpa_ft_rrb_rx_request(
    struct wpa_authenticator *wpa_auth, const u8 *current_ap, const u8 *sta_addr, const u8 *body, size_t len)
{
    struct wpa_state_machine *sm;
    u16 status;
    u8 *resp_ies;
                                     ...
                                     size_t resp_ies_len)
{
    ...
    wpa_printf(MSG_DEBUG, "FT: RRB authentication response: STA=" MACSTR " CurrentAP=" MACSTR " status=%u (%s)",
               MAC2STR(sm->addr), MAC2STR(current_ap), status, status2str(status));
    wpa_hexdump(MSG_DEBUG, "FT: Response IEs", resp_ies, resp_ies_len);
...
static int wpa_ft_rrb_build_r0(const u8 *key,
                               const size_t key_len,
                               ...
                               size_t *packet_len)
{
    ...
    struct tlv_list sess_tlv[] = {
        {.type = FT_RRB_PMK_R1, .len = pmk_r1_len, .data = pmk_r1},
        {.type = FT_RRB_PMK_R1_NAME, .len = sizeof(pmk_r1_name), .data = pmk_r1_name},
        {.type = FT_RRB_PAIRWISE, .len = sizeof(f_pairwise), .data = f_pairwise},
        {.type = FT_RRB_EXPIRES_IN, .len = sizeof(f_expires_in), .data = f_expires_in},
        {.type = FT_RRB_IDENTITY, .len = pmk_r0->identity_len, .data = pmk_r0->identity},
        {.type = FT_RRB_RADIUS_CUI, .len = pmk_r0->radius_cui_len, .data = pmk_r0->radius_cui},
        {.type = FT_RRB_SESSION_TIMEOUT, .len = sizeof(f_session_timeout), .data = f_session_timeout},
        {.type = FT_RRB_LAST_EMPTY, .len = 0, .data = NULL},
    };

    wpa_printf(MSG_DEBUG, "FT: Derive PMK-R1 for peer AP");
    if (wpa_derive_pmk_r1(pmk_r0->pmk_r0, pmk_r0->pmk_r0_len, pmk_r0->pmk_r0_name, r1kh_id, s1kh_id, pmk_r1,
                          pmk_r1_name) < 0)
        return -1;
    ...
    ret = wpa_ft_rrb_build(key, key_len, tlvs, sess_tlv, tlv_auth, pmk_r0->vlan, src_addr, type, packet, packet_len);
                              ...
                              int no_defer)
{
    ...
    if (r1kh)
        seq_ret = wpa_ft_rrb_seq_chk(r1kh->seq, src_addr, enc, enc_len, auth, auth_len, msgtype, no_defer);
    if (!no_defer && r1kh_wildcard && (!r1kh || os_memcmp(r1kh->addr, src_addr, ETH_ALEN) != 0))
    {
    ...
    if (wpa_ft_rrb_decrypt(key, key_len, enc, enc_len, auth, auth_len, src_addr, FT_PACKET_R0KH_R1KH_PULL, &plain,
                           &plain_len) < 0)
        goto out;

    if (!r1kh)
        r1kh = wpa_ft_rrb_add_r1kh(wpa_auth, r1kh_wildcard, src_addr, f_r1kh_id, wpa_auth->conf.rkh_pos_timeout);
    if (!r1kh)
        goto out;

    if (seq_ret == FT_RRB_SEQ_DEFER)
    {
        wpa_ft_rrb_seq_req(wpa_auth, r1kh->seq, src_addr, f_r0kh_id, f_r0kh_id_len, f_r1kh_id, key, key_len, enc,
                           enc_len, auth, auth_len, &wpa_ft_rrb_rx_pull);
        goto out;
    }

    wpa_ft_rrb_seq_accept(wpa_auth, r1kh->seq, src_addr, auth, auth_len, msgtype);
    ...
    wpa_printf(MSG_DEBUG, "FT: Send PMK-R1 pull response from " MACSTR " to " MACSTR, MAC2STR(wpa_auth->addr),
               MAC2STR(src_addr));
    ...
    if (wpa_ft_fetch_pmk_r0(wpa_auth, f_s1kh_id, f_pmk_r0_name, &r0) < 0)
    {
        ...
        ret = wpa_ft_rrb_build(key, key_len, resp, NULL, resp_auth, NULL, wpa_auth->addr, FT_PACKET_R0KH_R1KH_RESP,
                               &packet, &packet_len);
    }
    else
    {
        ret = wpa_ft_rrb_build_r0(key, key_len, resp, r0, f_r1kh_id, f_s1kh_id, resp_auth, wpa_auth->addr,
                                  FT_PACKET_R0KH_R1KH_RESP, &packet, &packet_len);
    }

    if (!ret)
        wpa_ft_rrb_oui_send(wpa_auth, src_addr, FT_PACKET_R0KH_R1KH_RESP, packet, packet_len);
                                      ...
                                      int no_defer))
{
    ...
    if (r0kh)
    {
        seq_ret = wpa_ft_rrb_seq_chk(r0kh->seq, src_addr, enc, enc_len, auth, auth_len, msgtype, cb ? 0 : 1);
    }
    if (cb && r0kh_wildcard && (!r0kh || os_memcmp(r0kh->addr, src_addr, ETH_ALEN) != 0))
    {
    ...
    if (wpa_ft_rrb_decrypt(key, key_len, enc, enc_len, auth, auth_len, src_addr, type, &plain, &plain_len) < 0)
        goto out;

    if (!r0kh)
        r0kh = wpa_ft_rrb_add_r0kh(wpa_auth, r0kh_wildcard, src_addr, f_r0kh_id, f_r0kh_id_len,
    ...
    if (seq_ret == FT_RRB_SEQ_DEFER)
    {
        wpa_ft_rrb_seq_req(wpa_auth, r0kh->seq, src_addr, f_r0kh_id, f_r0kh_id_len, f_r1kh_id, key, key_len, enc,
                           enc_len, auth, auth_len, cb);
        goto out;
    }

    wpa_ft_rrb_seq_accept(wpa_auth, r0kh->seq, src_addr, auth, auth_len, msgtype);
    ...
    wpa_hexdump(MSG_DEBUG, "FT: PMKR1Name", f_pmk_r1_name, WPA_PMK_NAME_LEN);

    pmk_r1_len = PMK_LEN;
    if (wpa_ft_rrb_get_tlv(plain, plain_len, FT_RRB_PMK_R1, &f_pmk_r1_len, &f_pmk_r1) == 0 &&
    ...
    RRB_GET_OPTIONAL(FT_RRB_EXPIRES_IN, expires_in, msgtype, sizeof(le16));
    if (f_expires_in)
        expires_in = WPA_GET_LE16(f_expires_in);
    else
        expires_in = 0;

    wpa_printf(MSG_DEBUG, "FT: PMK-R1 %s - expires_in=%d", msgtype, expires_in);
    ...
    wpa_printf(MSG_DEBUG, "FT: vlan %d%s", le_to_host16(vlan.untagged), vlan.tagged[0] ? "+" : "");

    RRB_GET_OPTIONAL(FT_RRB_IDENTITY, identity, msgtype, -1);
    if (f_identity)
        wpa_hexdump_ascii(MSG_DEBUG, "FT: Identity", f_identity, f_identity_len);

    RRB_GET_OPTIONAL(FT_RRB_RADIUS_CUI, radius_cui, msgtype, -1);
    if (f_radius_cui)
        wpa_hexdump_ascii(MSG_DEBUG, "FT: CUI", f_radius_cui, f_radius_cui_len);

    RRB_GET_OPTIONAL(FT_RRB_SESSION_TIMEOUT, session_timeout, msgtype, sizeof(le32));
    ...
    if (wpa_ft_store_pmk_r1(wpa_auth, f_s1kh_id, f_pmk_r1, pmk_r1_len, f_pmk_r1_name, pairwise, &vlan, expires_in,
                            session_timeout, f_identity, f_identity_len, f_radius_cui, f_radius_cui_len) < 0)
        goto out;
...
static void ft_finish_pull(struct wpa_state_machine *sm)
{
    ...
    res = wpa_ft_process_auth_req(sm, wpabuf_head(sm->ft_pending_req_ies), wpabuf_len(sm->ft_pending_req_ies),
    ...
    sm->ft_pending_cb(sm->ft_pending_cb_ctx, sm->addr, sm->wpa_auth->addr, sm->ft_pending_auth_transaction + 1, status,
                      resp_ies, resp_ies_len);
    os_free(resp_ies);
}
...
static int ft_get_sta_cb(struct wpa_state_machine *sm, void *ctx)
{
    ...
    if ((info->s1kh_id && os_memcmp(info->s1kh_id, sm->addr, ETH_ALEN) != 0) ||
        os_memcmp(info->nonce, sm->ft_pending_pull_nonce, FT_RRB_NONCE_LEN) != 0 || sm->ft_pending_cb == NULL ||
        sm->ft_pending_req_ies == NULL)
        return 0;
                              ...
                              int no_defer)
{
    ...
    ret = wpa_ft_rrb_rx_r1(wpa_auth, src_addr, FT_PACKET_R0KH_R1KH_RESP, enc, enc_len, auth, auth_len, msgtype, s1kh_id,
    ...
    if (wpa_auth_for_each_sta(wpa_auth, ft_get_sta_cb, &ctx))
    {
        wpa_printf(MSG_DEBUG, "FT: Response to a pending pull request for " MACSTR, MAC2STR(ctx.sm->addr));
                              ...
                              int no_defer)
{
    ...
    if (wpa_ft_rrb_rx_r1(wpa_auth, src_addr, FT_PACKET_R0KH_R1KH_PUSH, enc, enc_len, auth, auth_len, msgtype, NULL,
                             ...
                             struct ft_remote_r1kh **r1kh_wildcard_out)
{
    ...
    {
        wpa_ft_rrb_lookup_r0kh(wpa_auth, f_r0kh_id, f_r0kh_id_len, &r0kh, &r0kh_wildcard);
        if (!r0kh_wildcard && (!r0kh || os_memcmp(r0kh->addr, src_addr, ETH_ALEN) != 0))
        {
            wpa_hexdump(MSG_DEBUG, "FT: Did not find R0KH-ID", f_r0kh_id, f_r0kh_id_len);
            goto out;
        }
        if (r0kh)
        {
    ...
    {
        wpa_ft_rrb_lookup_r1kh(wpa_auth, f_r1kh_id, &r1kh, &r1kh_wildcard);
        if (!r1kh_wildcard && (!r1kh || os_memcmp(r1kh->addr, src_addr, ETH_ALEN) != 0))
        {
            wpa_hexdump(MSG_DEBUG, "FT: Did not find R1KH-ID", f_r1kh_id, FT_R1KH_ID_LEN);
            goto out;
        }
        if (r1kh)
        {
    ...
    {
        if (!r0kh)
            r0kh = wpa_ft_rrb_add_r0kh(wpa_auth, r0kh_wildcard, src_addr, f_r0kh_id, f_r0kh_id_len, ftRRBseqTimeout);
        if (!r0kh)
            goto out;
    ...
    {
        if (!r1kh)
            r1kh = wpa_ft_rrb_add_r1kh(wpa_auth, r1kh_wildcard, src_addr, f_r1kh_id, ftRRBseqTimeout);
        if (!r1kh)
            goto out;
                                 ...
                                 int no_defer)
{
    ...
    if (wpa_ft_rrb_rx_seq(wpa_auth, src_addr, FT_PACKET_R0KH_R1KH_SEQ_REQ, enc, enc_len, auth, auth_len, &rkh_seq, &key,
    ...
    wpa_printf(MSG_DEBUG, "FT: Send sequence number response from " MACSTR " to " MACSTR, MAC2STR(wpa_auth->addr),
               MAC2STR(src_addr));
    ...
    if (wpa_ft_rrb_build(key, key_len, NULL, NULL, seq_resp_auth, NULL, wpa_auth->addr, FT_PACKET_R0KH_R1KH_SEQ_RESP,
                         &packet, &packet_len) < 0)
        goto out;

    wpa_ft_rrb_oui_send(wpa_auth, src_addr, FT_PACKET_R0KH_R1KH_SEQ_RESP, packet, packet_len);
                                  ...
                                  int no_defer)
{
    ...
    if (wpa_ft_rrb_rx_seq(wpa_auth, src_addr, FT_PACKET_R0KH_R1KH_SEQ_RESP, enc, enc_len, auth, auth_len, &rkh_seq,
                          &key, &key_len, &r0kh, &r1kh, &r0kh_wildcard, &r1kh_wildcard) < 0)
        goto out;
    ...
    found = 0;
    dl_list_for_each(item, &rkh_seq->rx.queue, struct ft_remote_item, list)
    {
        if (os_memcmp_const(f_nonce, item->nonce, FT_RRB_NONCE_LEN) != 0 || os_get_reltime(&now) < 0 ||
    ...
    seq_ret = wpa_ft_rrb_seq_chk(rkh_seq, src_addr, enc, enc_len, auth, auth_len, "seq response", 1);
    if (seq_ret == FT_RRB_SEQ_OK)
    {
        wpa_printf(MSG_DEBUG, "FT: seq response - valid seq number");
        wpa_ft_rrb_seq_accept(wpa_auth, rkh_seq, src_addr, auth, auth_len, "seq response");
    }
    else
    {
        wpa_printf(MSG_DEBUG, "FT: seq response - reset seq number");

        RRB_GET_AUTH(FT_RRB_SEQ, seq, "seq response", sizeof(*msg_both));
        msg_both = (const struct ft_rrb_seq *)f_seq;
        ...
        rkh_seq->rx.last[0] = msg_seq - FT_REMOTE_SEQ_BACKLOG - dl_list_len(&rkh_seq->rx.queue);
...
int wpa_ft_rrb_rx(struct wpa_authenticator *wpa_auth, const u8 *src_addr, const u8 *data, size_t data_len)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               frame->frame_type, frame->packet_type, alen, MAC2STR(frame->ap_address));
    ...
    target_ap_addr = pos;
    pos += ETH_ALEN;
    wpa_printf(MSG_DEBUG, "FT: RRB Action Frame: action=%d sta_addr=" MACSTR " target_ap_addr=" MACSTR, action,
               MAC2STR(sta_addr), MAC2STR(target_ap_addr));
...
void wpa_ft_rrb_oui_rx(struct wpa_authenticator *wpa_auth,
                       const u8 *src_addr,
                       const u8 *dst_addr,
                       u8 oui_suffix,
                       const u8 *data,
                       size_t data_len)
{
    ...
    wpa_printf(MSG_DEBUG, "FT: RRB-OUI(" MACSTR ") received frame from remote AP " MACSTR " oui_suffix=%u dst=" MACSTR,
               MAC2STR(wpa_auth->addr), MAC2STR(src_addr), oui_suffix, MAC2STR(dst_addr));
    wpa_hexdump(MSG_MSGDUMP, "FT: RRB frame payload", data, data_len);

    if (is_multicast_ether_addr(src_addr))
    {
        wpa_printf(MSG_DEBUG, "FT: RRB-OUI received frame from multicast address " MACSTR, MAC2STR(src_addr));
        return;
    }
    ...
    switch (oui_suffix)
    {
        case FT_PACKET_R0KH_R1KH_PULL:
            wpa_ft_rrb_rx_pull(wpa_auth, src_addr, enc, elen, auth, alen, no_defer);
            break;
        case FT_PACKET_R0KH_R1KH_RESP:
            wpa_ft_rrb_rx_resp(wpa_auth, src_addr, enc, elen, auth, alen, no_defer);
            break;
        case FT_PACKET_R0KH_R1KH_PUSH:
            wpa_ft_rrb_rx_push(wpa_auth, src_addr, enc, elen, auth, alen, no_defer);
            break;
        case FT_PACKET_R0KH_R1KH_SEQ_REQ:
            wpa_ft_rrb_rx_seq_req(wpa_auth, src_addr, enc, elen, auth, alen, no_defer);
            break;
                                  ...
                                  const u8 *s1kh_id)
{
    ...
    struct tlv_list push[] = {
        {.type = FT_RRB_S1KH_ID, .len = ETH_ALEN, .data = s1kh_id},
        {.type = FT_RRB_PMK_R0_NAME, .len = WPA_PMK_NAME_LEN, .data = pmk_r0->pmk_r0_name},
        {.type = FT_RRB_LAST_EMPTY, .len = 0, .data = NULL},
    };
    struct tlv_list push_auth[] = {
        {.type = FT_RRB_SEQ, .len = sizeof(f_seq), .data = (u8 *)&f_seq},
        {.type = FT_RRB_R0KH_ID, .len = wpa_auth->conf.r0_key_holder_len, .data = wpa_auth->conf.r0_key_holder},
        {.type = FT_RRB_R1KH_ID, .len = FT_R1KH_ID_LEN, .data = r1kh->id},
        {.type = FT_RRB_LAST_EMPTY, .len = 0, .data = NULL},
    };
    ...
    wpa_printf(MSG_DEBUG, "FT: Send PMK-R1 push from " MACSTR " to remote R0KH address " MACSTR,
               MAC2STR(wpa_auth->addr), MAC2STR(r1kh->addr));

    if (wpa_ft_rrb_build_r0(r1kh->key, sizeof(r1kh->key), push, pmk_r0, r1kh->id, s1kh_id, push_auth, wpa_auth->addr,
                            FT_PACKET_R0KH_R1KH_PUSH, &packet, &packet_len) < 0)
        return -1;

    wpa_ft_rrb_oui_send(wpa_auth, r1kh->addr, FT_PACKET_R0KH_R1KH_PUSH, packet, packet_len);
