...
int wpa_write_rsn_ie(struct wpa_auth_config *conf, u8 *buf, size_t len, const u8 *pmkid)
{
    ...
    if (conf->ieee80211w != NO_MGMT_FRAME_PROTECTION && conf->group_mgmt_cipher != WPA_CIPHER_NONE)
    {
        if (2 + 4 > buf + len - pos)
            return -1;
        if (pmkid == NULL)
...
int wpa_write_rsnxe(struct wpa_auth_config *conf, u8 *buf, size_t len)
{
    ...
    if (wpa_key_mgmt_sae(conf->wpa_key_mgmt) && (conf->sae_pwe == 1 || conf->sae_pwe == 2 || conf->sae_pk))
    {
...
int wpa_auth_gen_wpa_ie(struct wpa_authenticator *wpa_auth)
{
    ...
    {
        wpa_hexdump(MSG_DEBUG, "WPA: Forced own IE(s) for testing", wpa_auth->conf.own_ie_override,
                    wpa_auth->conf.own_ie_override_len);
        os_free(wpa_auth->wpa_ie);
        wpa_auth->wpa_ie = os_malloc(wpa_auth->conf.own_ie_override_len);
        if (wpa_auth->wpa_ie == NULL)
            return -1;
        os_memcpy(wpa_auth->wpa_ie, wpa_auth->conf.own_ie_override, wpa_auth->conf.own_ie_override_len);
                                             ...
                                             size_t owe_dh_len)
{
    ...
    if (version == WPA_PROTO_RSN)
    {
        ...
        if (wpa_key_mgmt_ft(data.key_mgmt) && !mdie && !wpa_key_mgmt_only_ft(data.key_mgmt))
        {
            ...
            wpa_printf(MSG_DEBUG,
                       "RSN: FT set in RSNE AKM but MDE is missing from " MACSTR
    ...
    if (!ciphers)
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   "from " MACSTR,
                   version == WPA_PROTO_RSN ? "RSN" : "WPA", data.pairwise_cipher, MAC2STR(sm->addr));
...
#ifdef CONFIG_SAE
    if (wpa_auth->conf.ieee80211w == MGMT_FRAME_PROTECTION_OPTIONAL && wpa_auth->conf.sae_require_mfp &&
        wpa_key_mgmt_sae(sm->wpa_key_mgmt) && !(data.capabilities & WPA_CAPABILITY_MFPC))
    {
        wpa_printf(MSG_DEBUG, "Management frame protection required with SAE, but client did not enable it");
        return WPA_MGMT_FRAME_PROTECTION_VIOLATION;
    }
#endif /* CONFIG_SAE */

#ifdef CONFIG_OCV
    if (wpa_auth->conf.ocv && (data.capabilities & WPA_CAPABILITY_OCVC) && !(data.capabilities & WPA_CAPABILITY_MFPC))
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "Allow connecting MFP incapable and OCV capable STA without enabling OCV");
            wpa_auth_set_ocv(sm, 0);
        }
        else
        {
            wpa_printf(MSG_DEBUG, "Management frame protection required with OCV, but client did not enable it");
            return WPA_MGMT_FRAME_PROTECTION_VIOLATION;
        }
    }
    else
    {
        wpa_auth_set_ocv(sm, (data.capabilities & WPA_CAPABILITY_OCVC) ? wpa_auth->conf.ocv : 0);
    }
#endif /* CONFIG_OCV */
    ...
    {
        wpa_printf(MSG_DEBUG, "RSN: Trying to use non-FT AKM suite, but MDIE included");
        return WPA_INVALID_AKMP;
    }
#endif /* CONFIG_IEEE80211R_AP */
...
#ifdef CONFIG_DPP2
    if (sm->wpa_key_mgmt == WPA_KEY_MGMT_DPP && ((conf->dpp_pfs == 1 && !owe_dh) || (conf->dpp_pfs == 2 && owe_dh)))
    {
        wpa_printf(MSG_DEBUG, "DPP: PFS %s", conf->dpp_pfs == 1 ? "required" : "not allowed");
    ...
    if ((sm->wpa_key_mgmt == WPA_KEY_MGMT_FT_FILS_SHA256 || sm->wpa_key_mgmt == WPA_KEY_MGMT_FT_FILS_SHA384) &&
        (sm->auth_alg == WLAN_AUTH_FILS_SK || sm->auth_alg == WLAN_AUTH_FILS_SK_PFS ||
         sm->auth_alg == WLAN_AUTH_FILS_PK) &&
        (data.num_pmkid != 1 || !data.pmkid || !sm->pmk_r1_name_valid ||
         os_memcmp_const(data.pmkid, sm->pmk_r1_name, WPA_PMK_NAME_LEN) != 0))
    {
        wpa_auth_vlogger(wpa_auth, sm->addr, LOGGER_DEBUG, "No PMKR1Name match for FILS+FT");
    ...
    if (sm->pmksa && pmkid)
    {
        ...
        wpa_auth_vlogger(wpa_auth, sm->addr, LOGGER_DEBUG, "PMKID found from PMKSA cache eap_type=%d vlan=%d%s",
                         sm->pmksa->eap_type_authsrv, vlan ? vlan->untagged : 0, (vlan && vlan->tagged[0]) ? "+" : "");
        os_memcpy(wpa_auth->dot11RSNAPMKIDUsed, pmkid, PMKID_LEN);
    }

#ifdef CONFIG_SAE
    if (sm->wpa_key_mgmt == WPA_KEY_MGMT_SAE && data.num_pmkid && !sm->pmksa)
    {
        wpa_auth_vlogger(wpa_auth, sm->addr, LOGGER_DEBUG, "No PMKSA cache entry found for SAE");
        return WPA_INVALID_PMKID;
    }
#endif /* CONFIG_SAE */
    ...
    {
        wpa_auth_vlogger(wpa_auth, sm->addr, LOGGER_DEBUG, "No PMKSA cache entry found for DPP");
        return WPA_INVALID_PMKID;
    }
#endif /* CONFIG_DPP */

    if (conf->extended_key_id && sm->wpa == WPA_VERSION_WPA2 && sm->pairwise != WPA_CIPHER_TKIP &&
        (data.capabilities & WPA_CAPABILITY_EXT_KEY_ID_FOR_UNICAST))
    {
        sm->use_ext_key_id = true;
        if (conf->extended_key_id == 2 && !wpa_key_mgmt_ft(sm->wpa_key_mgmt) && !wpa_key_mgmt_fils(sm->wpa_key_mgmt))
            sm->keyidx_active = 1;
        else
            sm->keyidx_active = 0;
        wpa_printf(MSG_DEBUG, "RSN: Extended Key ID supported (start with %d)", sm->keyidx_active);
    }
    else
    {
    ...
    struct wpa_state_machine *sm, u8 *pos, size_t max_len, const u8 *req_ies, size_t req_ies_len)
{
    ...
    if (conf->own_ie_override_len)
    {
        if (max_len < conf->own_ie_override_len)
            return NULL;
        wpa_hexdump(MSG_DEBUG, "WPA: Forced own IE(s) for testing", conf->own_ie_override, conf->own_ie_override_len);
        os_memcpy(pos, conf->own_ie_override, conf->own_ie_override_len);
        return pos + conf->own_ie_override_len;
    }
#endif /* CONFIG_TESTING_OPTIONS */

    res = wpa_write_rsn_ie(conf, pos, max_len, sm->pmksa ? sm->pmksa->pmkid : NULL);
    if (res < 0)
        return pos;
    return pos + res;
}
    ...
    struct wpa_state_machine *sm, u8 *pos, size_t max_len, const u8 *req_ies, size_t req_ies_len)
{
    ...
    if (!sm || sm->wpa_key_mgmt & (WPA_KEY_MGMT_FT_FILS_SHA256 | WPA_KEY_MGMT_FT_FILS_SHA384))
        return pos;
...
bool wpa_auth_write_fd_rsn_info(struct wpa_authenticator *wpa_auth, u8 *fd_rsn_info)
{
    ...
    if (res == 1 && tmp[0] == 0x00 && tmp[1] == 0x0f && tmp[2] == 0xac && tmp[3] <= 13)
        cipher = tmp[3];
    selectors |= cipher << 12;

    /* AKM Suite Selector (B34..B39) */
    selector = 0; /* default to AKM from RSNE in Beacon/Probe Response */
    mask     = WPA_KEY_MGMT_FILS_SHA256 | WPA_KEY_MGMT_FILS_SHA384 | WPA_KEY_MGMT_FT_FILS_SHA384;
    if ((conf->wpa_key_mgmt & mask) && (conf->wpa_key_mgmt & ~mask) == 0)
    {
        ...
        else if (suite == (WPA_KEY_MGMT_FILS_SHA256 | WPA_KEY_MGMT_FILS_SHA384))
