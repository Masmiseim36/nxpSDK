...
static int hostapd_wps_probe_req_rx(
    void *ctx, const u8 *addr, const u8 *da, const u8 *bssid, const u8 *ie, size_t ie_len, int ssi_signal);
...
static void hostapd_wps_pin_needed_cb(void *ctx, const u8 *uuid_e, const struct wps_device_data *dev)
{
    ...
    len = os_snprintf(txt, sizeof(txt), WPS_EVENT_PIN_NEEDED "%s " MACSTR " [%s|%s|%s|%s|%s|%s]", uuid,
    ...
    if (hapd->conf->wps_pin_requests)
    {
        ...
        fprintf(f,
                ...
                "\t%s\t%s\t%s\t%s\t%s"
                "\t%s\n",
                t.sec, uuid, MAC2STR(dev->mac_addr), dev->device_name, dev->manufacturer, dev->model_name,
...
static void hostapd_wps_reg_success_cb(
    void *ctx, const u8 *mac_addr, const u8 *uuid_e, const u8 *dev_pw, size_t dev_pw_len)
{
    struct hostapd_data *hapd = ctx;
    char uuid[40];
                                         ...
                                         const char *dev_name)
{
    ...
    if (dev_name == NULL)
        dev_name = "";
    wpa_msg_ctrl(hapd->msg_ctx, MSG_INFO, WPS_EVENT_ENROLLEE_SEEN MACSTR " %s %s 0x%x %u %u [%s]", MAC2STR(addr), uuid,
                 wps_dev_type_bin2str(pri_dev_type, devtype, sizeof(devtype)), config_methods, dev_password_id,
                 request_type, dev_name);
}

static int hostapd_wps_lookup_pskfile_cb(void *ctx, const u8 *mac_addr, const u8 **psk)
{
    ...
    for (wpa_psk = hapd->conf->ssid.wpa_psk; wpa_psk; wpa_psk = wpa_psk->next)
    {
        ...
        if (mac_addr && !dev_psk && os_memcmp(mac_addr, wpa_psk->addr, ETH_ALEN) == 0)
        {
            dev_psk = wpa_psk->psk;
            break;
        }
    ...
    {
        *psk = NULL;
        wpa_printf(MSG_DEBUG, "WPS: No appropriate PSK in wpa_psk_file");
        return 0;
    }
...
static int hapd_wps_reconfig_in_memory(struct hostapd_data *hapd, const struct wps_credential *cred)
{
    ...
    if (cred->auth_type & (WPS_AUTH_WPA2 | WPS_AUTH_WPA2PSK | WPS_AUTH_WPA | WPS_AUTH_WPAPSK))
        bss->wpa = 2;
    else
        bss->wpa = 0;
#else  /* CONFIG_NO_TKIP */
    ...
    if (bss->wpa)
    {
        ...
        if (hapd->conf->wps_cred_add_sae && (cred->auth_type & WPS_AUTH_WPA2PSK) && cred->key_len != 2 * PMK_LEN)
        {
...
static int hapd_wps_cred_cb(struct hostapd_data *hapd, void *ctx)
{
    ...
    {
        wpa_printf(MSG_INFO, "WPS: Invalid key length %lu for WPA/WPA2", (unsigned long)cred->key_len);
        return -1;
    }
    else
    {
    ...
    while (fgets(buf, sizeof(buf), oconf))
    {
        if (os_strncmp(buf, "bss=", 4) == 0)
            multi_bss = 1;
        if (!multi_bss && (str_starts(buf, "ssid=") || str_starts(buf, "ssid2=") || str_starts(buf, "auth_algs=") ||
#ifdef CONFIG_WEP
                           str_starts(buf, "wep_default_key=") || str_starts(buf, "wep_key") ||
#endif /* CONFIG_WEP */
                           str_starts(buf, "wps_state=") || (pmf_changed && str_starts(buf, "ieee80211w=")) ||
                           str_starts(buf, "wpa=") || str_starts(buf, "wpa_psk=") || str_starts(buf, "wpa_pairwise=") ||
                           str_starts(buf, "rsn_pairwise=") || str_starts(buf, "wpa_key_mgmt=") ||
...
static void hostapd_wps_event_fail(struct hostapd_data *hapd, struct wps_event_fail *fail)
{
    ...
    {
        wpa_msg(hapd->msg_ctx, MSG_INFO, WPS_EVENT_FAIL "msg=%d config_error=%d reason=%d (%s)", fail->msg,
...
static int hostapd_wps_rf_band_cb(void *ctx)
{
    ...
    return hapd->iconf->hw_mode == HOSTAPD_MODE_IEEE80211A ?
               WPS_RF_50GHZ :
               hapd->iconf->hw_mode == HOSTAPD_MODE_IEEE80211AD ? WPS_RF_60GHZ : WPS_RF_24GHZ; /* FIX: dualband AP */
}
...
static int hostapd_wps_set_application_ext(struct hostapd_data *hapd, struct wps_context *wps)
{
...
int hostapd_init_wps(struct hostapd_data *hapd, struct hostapd_bss_config *conf)
{
    ...
    if ((wps->config_methods & (WPS_CONFIG_DISPLAY | WPS_CONFIG_VIRT_DISPLAY | WPS_CONFIG_PHY_DISPLAY)) ==
        WPS_CONFIG_DISPLAY)
    {
        wpa_printf(MSG_INFO,
    ...
    if ((wps->config_methods & (WPS_CONFIG_PUSHBUTTON | WPS_CONFIG_VIRT_PUSHBUTTON | WPS_CONFIG_PHY_PUSHBUTTON)) ==
    ...
    if (hostapd_wps_set_vendor_ext(hapd, wps) < 0 || hostapd_wps_set_application_ext(hapd, wps) < 0)
        goto fail;
    ...
    if (conf->wpa & WPA_PROTO_RSN)
    {
        ...
        if (conf->rsn_pairwise & (WPA_CIPHER_CCMP | WPA_CIPHER_GCMP | WPA_CIPHER_CCMP_256 | WPA_CIPHER_GCMP_256))
        {
    ...
    if ((hapd->conf->multi_ap & FRONTHAUL_BSS) && hapd->conf->multi_ap_backhaul_ssid.ssid_len)
    {
        cfg.multi_ap_backhaul_ssid_len = hapd->conf->multi_ap_backhaul_ssid.ssid_len;
        cfg.multi_ap_backhaul_ssid     = hapd->conf->multi_ap_backhaul_ssid.ssid;

        if (conf->multi_ap_backhaul_ssid.wpa_passphrase)
        {
            cfg.multi_ap_backhaul_network_key     = (const u8 *)conf->multi_ap_backhaul_ssid.wpa_passphrase;
        ...
        else if (conf->multi_ap_backhaul_ssid.wpa_psk)
        {
            ...
            if (!multi_ap_netw_key)
                goto fail;
            wpa_snprintf_hex((char *)multi_ap_netw_key, 2 * PMK_LEN + 1, conf->multi_ap_backhaul_ssid.wpa_psk->psk,
                             PMK_LEN);
...
#ifdef CONFIG_P2P
    if ((hapd->conf->p2p & P2P_ENABLED) && is_6ghz_op_class(hapd->iconf->op_class))
...
static int hostapd_wps_update_multi_ap(struct hostapd_data *hapd, struct wps_registrar *reg)
{
    ...
    if (!(conf->multi_ap & FRONTHAUL_BSS) || !conf->multi_ap_backhaul_ssid.ssid_len)
        return 0;
    ...
    if (conf->multi_ap_backhaul_ssid.wpa_passphrase)
    {
        ...
        if (!multi_ap_backhaul_network_key)
            return -1;
        multi_ap_backhaul_network_key_len = os_strlen(conf->multi_ap_backhaul_ssid.wpa_passphrase);
    }
    else if (conf->multi_ap_backhaul_ssid.wpa_psk)
    {
        ...
        if (!multi_ap_backhaul_network_key)
            return -1;
        wpa_snprintf_hex((char *)multi_ap_backhaul_network_key, 2 * PMK_LEN + 1,
                         conf->multi_ap_backhaul_ssid.wpa_psk->psk, PMK_LEN);
        multi_ap_backhaul_network_key_len = 2 * PMK_LEN;
    }

    ret = wps_registrar_update_multi_ap(reg, conf->multi_ap_backhaul_ssid.ssid, conf->multi_ap_backhaul_ssid.ssid_len,
                                        multi_ap_backhaul_network_key, multi_ap_backhaul_network_key_len);
...
static int wps_button_pushed(struct hostapd_data *hapd, void *ctx)
{
    ...
    {
        data->count++;
        return wps_registrar_button_pushed(hapd->wps->registrar, data->p2p_dev_addr);
    }
...
static int hostapd_rx_req_put_wlan_response(void *priv,
...
int hostapd_wps_config_ap(
    struct hostapd_data *hapd, const char *ssid, const char *auth, const char *encr, const char *key)
{
...
int hostapd_wps_nfc_report_handover(struct hostapd_data *hapd, const struct wpabuf *req, const struct wpabuf *sel)
{
    ...
    ret = wps_registrar_add_nfc_pw_token(hapd->wps->registrar, attr.oob_dev_password, DEV_PW_NFC_CONNECTION_HANDOVER,
                                         NULL, 0, 1);
...
struct wpabuf *hostapd_wps_nfc_token_gen(struct hostapd_data *hapd, int ndef)
{
    ...
    {
        return wps_nfc_token_build(ndef, hapd->conf->wps_nfc_dev_pw_id, hapd->conf->wps_nfc_dh_pubkey,
                                   hapd->conf->wps_nfc_dev_pw);
    }

    return wps_nfc_token_gen(ndef, &hapd->conf->wps_nfc_dev_pw_id, &hapd->conf->wps_nfc_dh_pubkey,
...
int hostapd_wps_nfc_token_enable(struct hostapd_data *hapd)
{
    ...
    if (!hapd->conf->wps_nfc_dh_pubkey || !hapd->conf->wps_nfc_dh_privkey || !hapd->conf->wps_nfc_dev_pw ||
        !hapd->conf->wps_nfc_dev_pw_id)
        return -1;

    hostapd_wps_nfc_clear(wps);
    wpa_printf(MSG_DEBUG, "WPS: Enable NFC Tag (Dev Pw Id %u) for AP interface %s (context %p)",
    ...
    {
        wpa_snprintf_hex_uppercase((char *)wpabuf_put(wps->ap_nfc_dev_pw, wpabuf_len(pw) * 2), wpabuf_len(pw) * 2 + 1,
