...
struct wpabuf *dpp_alloc_msg(enum dpp_public_action_frame_type type, size_t len)
{
    struct wpabuf *msg;
...
static const u8 *dpp_get_attr_next(const u8 *prev, const u8 *buf, size_t len, u16 req_id, u16 *ret_len)
{
    u16 id, alen;
...
int dpp_check_attrs(const u8 *buf, size_t len)
{
    ...
    while (end - pos >= 4)
    {
        ...
        alen = WPA_GET_LE16(pos);
        pos += 2;
        wpa_printf(MSG_MSGDUMP, "DPP: Attribute ID %04x len %u", id, alen);
        if (alen > end - pos)
        {
            wpa_printf(MSG_DEBUG, "DPP: Truncated message - not enough room for the attribute - dropped");
            return -1;
        }
        if (wrapped_data)
        {
            wpa_printf(MSG_DEBUG, "DPP: An unexpected attribute included after the Wrapped Data attribute");
            return -1;
        }
    ...
    if (end != pos)
    {
        wpa_printf(MSG_DEBUG, "DPP: Unexpected octets (%d) after the last attribute", (int)(end - pos));
        return -1;
    }
...
int dpp_parse_uri_chan_list(struct dpp_bootstrap_info *bi, const char *chan_list)
{
    ...
    while (pos && *pos && *pos != ';')
    {
        ...
        wpa_printf(MSG_DEBUG, "DPP: URI channel-list: opclass=%d channel=%d ==> freq=%d", opclass, channel, freq);
        bi->channels_listed = true;
        if (freq < 0)
        {
            wpa_printf(MSG_DEBUG, "DPP: Ignore unknown URI channel-list channel (opclass=%d channel=%d)", opclass,
                       channel);
        }
        else if (bi->num_freq == DPP_BOOTSTRAP_MAX_FREQ)
        {
            wpa_printf(MSG_DEBUG, "DPP: Too many channels in URI channel-list - ignore list");
            bi->num_freq = 0;
            break;
        }
        else
...
static int dpp_parse_uri_pk(struct dpp_bootstrap_info *bi, const char *info)
{
    ...
    data = wpa_base64_decode(info, end - info, &data_len);
    if (!data)
    {
        wpa_printf(MSG_DEBUG, "DPP: Invalid base64 encoding on URI public-key");
        return -1;
    }
    wpa_hexdump(MSG_DEBUG, "DPP: Base64 decoded URI public-key", data, data_len);
...
static struct dpp_bootstrap_info *dpp_parse_uri(const char *uri)
{
    ...
    for (;;)
    {
        ...
        if (pos[0] == 'C' && pos[1] == ':' && !chan_list)
            ...
            wpa_hexdump_ascii(MSG_DEBUG, "DPP: Ignore unrecognized URI parameter", pos, end - pos);
        pos = end + 1;
    }
    ...
    if (dpp_clone_uri(bi, uri) < 0 || dpp_parse_uri_chan_list(bi, chan_list) < 0 || dpp_parse_uri_mac(bi, mac) < 0 ||
        dpp_parse_uri_info(bi, info) < 0 || dpp_parse_uri_version(bi, version) < 0 || dpp_parse_uri_pk(bi, pk) < 0)
    {
...
static int dpp_channel_ok_init(struct hostapd_hw_modes *own_modes, u16 num_modes, unsigned int freq)
{
    u16 m;
    int c, flag;
    ...
    for (m = 0; m < num_modes; m++)
    {
        ...
        {
            if ((unsigned int)own_modes[m].channels[c].freq != freq)
                continue;
            flag = own_modes[m].channels[c].flag;
            if (!(flag & (HOSTAPD_CHAN_DISABLED | HOSTAPD_CHAN_NO_IR | HOSTAPD_CHAN_RADAR)))
                return 1;
        }
    }
...
static int freq_included(const unsigned int freqs[], unsigned int num, unsigned int freq)
{
    while (num > 0)
    {
        if (freqs[--num] == freq)
...
static void freq_to_start(unsigned int freqs[], unsigned int num, unsigned int freq)
{
    unsigned int i;
...
static int dpp_channel_intersect(struct dpp_authentication *auth, struct hostapd_hw_modes *own_modes, u16 num_modes)
{
    ...
    if (!auth->num_freq)
    {
        wpa_printf(MSG_INFO, "DPP: No available channels for initiating DPP Authentication");
        return -1;
    }
...
static int dpp_channel_local_list(struct dpp_authentication *auth, struct hostapd_hw_modes *own_modes, u16 num_modes)
{
    u16 m;
    int c, flag;
    unsigned int freq;
                             ...
                             u16 num_modes)
{
    ...
    *pos = '\0';
    wpa_printf(MSG_DEBUG, "DPP: Possible frequencies for initiating:%s", freqs);
...
int dpp_gen_uri(struct dpp_bootstrap_info *bi)
{
    ...
    if (is_zero_ether_addr(bi->mac_addr))
        ...
        os_snprintf(macstr, sizeof(macstr), "M:" COMPACT_MACSTR ";", MAC2STR(bi->mac_addr));
    ...
    if (!bi->uri)
        return -1;
    os_snprintf(bi->uri, len + 1, "DPP:%s%s%s%s%s%s%s%sK:%s;;", bi->chan ? "C:" : "", bi->chan ? bi->chan : "",
                bi->chan ? ";" : "", macstr, bi->info ? "I:" : "", bi->info ? bi->info : "", bi->info ? ";" : "",
                DPP_VERSION == 3 ? "V:3;" : (DPP_VERSION == 2 ? "V:2;" : ""), bi->pk);
    return 0;
}
...
static struct wpabuf *dpp_build_conf_req_attr(struct dpp_authentication *auth, const char *json)
{
    size_t nonce_len;
...
skip_conf_attr_obj:
    ...
    if (aes_siv_encrypt(auth->ke, auth->curve->hash_len, wpabuf_head(clear), wpabuf_len(clear), 0, NULL, NULL,
                        wrapped) < 0)
        goto fail;
    wpa_hexdump(MSG_DEBUG, "DPP: AES-SIV ciphertext", wrapped, wpabuf_len(clear) + AES_BLOCK_SIZE);
...
skip_wrapped_data:
    ...
    wpa_hexdump_buf(MSG_DEBUG, "DPP: Configuration Request frame attributes", msg);
out:
    wpabuf_free(clear);
    wpabuf_free(pe);
    return msg;
...
struct wpabuf *dpp_build_conf_req(struct dpp_authentication *auth, const char *json)
{
...
struct wpabuf *dpp_build_conf_req_helper(
    struct dpp_authentication *auth, const char *name, enum dpp_netrole netrole, const char *mud_url, int *opclasses)
{
    ...
    if (auth->csr)
    {
        ...
        csr = base64_encode_no_lf(wpabuf_head(auth->csr), wpabuf_len(auth->csr), &csr_len);
        if (!csr)
            goto fail;
        len += 30 + csr_len;
    }
...
struct dpp_configuration *dpp_configuration_alloc(const char *type)
{
    ...
    else if (bin_str_eq(type, len, "psk-sae") || bin_str_eq(type, len, "psk+sae"))
        conf->akm = DPP_AKM_PSK_SAE;
    else if (bin_str_eq(type, len, "sae-dpp") || bin_str_eq(type, len, "dpp+sae"))
        conf->akm = DPP_AKM_SAE_DPP;
    else if (bin_str_eq(type, len, "psk-sae-dpp") || bin_str_eq(type, len, "dpp+psk+sae"))
...
{
    return akm == DPP_AKM_PSK || akm == DPP_AKM_PSK_SAE || akm == DPP_AKM_PSK_SAE_DPP;
}

int dpp_akm_sae(enum dpp_akm akm)
{
    return akm == DPP_AKM_SAE || akm == DPP_AKM_PSK_SAE || akm == DPP_AKM_SAE_DPP || akm == DPP_AKM_PSK_SAE_DPP;
}

int dpp_akm_legacy(enum dpp_akm akm)
{
    return akm == DPP_AKM_PSK || akm == DPP_AKM_PSK_SAE || akm == DPP_AKM_SAE;
}

int dpp_akm_dpp(enum dpp_akm akm)
{
    return akm == DPP_AKM_DPP || akm == DPP_AKM_SAE_DPP || akm == DPP_AKM_PSK_SAE_DPP;
}
...
static int dpp_configuration_parse_helper(struct dpp_authentication *auth, const char *cmd, int idx)
{
    const char *pos, *end;
    ...
    if (pos)
    {
        ...
        conf->ssid_len /= 2;
        if (conf->ssid_len > sizeof(conf->ssid) || hexstr2bin(pos, conf->ssid, conf->ssid_len) < 0)
            goto fail;
    }
    else
    {
    ...
    if (pos)
    {
        if (conf_ap)
        {
            wpa_printf(MSG_INFO, "DPP: ssid64 option (ssid_charset param) not allowed for AP enrollee");
            goto fail;
        }
    ...
    if (pos)
    {
        ...
        if (!conf->passphrase || hexstr2bin(pos, (u8 *)conf->passphrase, pass_len) < 0)
            goto fail;
    }
...
static int dpp_configuration_parse(struct dpp_authentication *auth, const char *cmd)
{
    const char *pos;
    char *tmp;
    size_t len;
...
static struct dpp_configurator *dpp_configurator_get_id(struct dpp_global *dpp, unsigned int id)
{
    ...
    dl_list_for_each(conf, &dpp->configurator, struct dpp_configurator, list)
    {
        if (conf->id == id)
            return conf;
    }
...
int dpp_set_configurator(struct dpp_authentication *auth, const char *cmd)
{
    ...
    if (!auth->conf && pos)
    {
        ...
        if (!auth->conf)
        {
            wpa_printf(MSG_INFO, "DPP: Could not find the specified configurator");
            goto fail;
        }
    }
    ...
    {
        wpa_msg(auth->msg_ctx, MSG_INFO, "DPP: Failed to set configurator parameters");
        goto fail;
    }
    ret = 0;
fail:
                                           ...
                                           size_t tailroom)
{
    ...
    if (auth->discovery_override)
    {
        wpa_printf(MSG_DEBUG, "DPP: TESTING - discovery override: '%s'", auth->discovery_override);
...
int dpp_build_jwk(struct wpabuf *buf,
                  const char *name,
...
static void dpp_build_legacy_cred_params(struct wpabuf *buf, struct dpp_configuration *conf)
{
    if (conf->passphrase && os_strlen(conf->passphrase) < 64)
    {
        json_add_string_escape(buf, "pass", conf->passphrase, os_strlen(conf->passphrase));
    }
    else if (conf->psk_set)
    {
        char psk[2 * sizeof(conf->psk) + 1];

        wpa_snprintf_hex(psk, sizeof(psk), conf->psk, sizeof(conf->psk));
...
static struct wpabuf *dpp_build_conf_obj_dpp(struct dpp_authentication *auth, struct dpp_configuration *conf)
{
    ...
    if (!auth->conf)
    {
        wpa_printf(MSG_INFO, "DPP: No configurator specified - cannot generate DPP config object");
        goto fail;
    }
    ...
    if (auth->groups_override)
    {
        ...
        if (auth->groups_override)
        {
            wpa_printf(MSG_DEBUG, "DPP: TESTING - groups override: '%s'", auth->groups_override);
    ...
    json_start_object(dppcon, NULL);
    json_add_string(dppcon, "groupId", conf->group_id ? conf->group_id : "*");
    json_value_sep(dppcon);
...
skip_groups:
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: No peer protocol key available to build netAccessKey JWK");
        goto fail;
    }
#ifdef CONFIG_DPP3
    if (auth->conf->net_access_key_curve && auth->curve != auth->conf->net_access_key_curve && !auth->new_key_received)
    {
        wpa_printf(MSG_DEBUG,
                   "DPP: Peer protocol key curve (%s) does not match the required netAccessKey curve (%s) - %s",
                   auth->curve->name, auth->conf->net_access_key_curve->name,
                   auth->waiting_new_key ? "the required key not received" : "request a new key");
        if (auth->waiting_new_key)
...
#endif /* CONFIG_DPP3 */
    if (dpp_build_jwk(dppcon, "netAccessKey", auth->peer_protocol_key, NULL, nak_curve) < 0)
    {
    ...
    if (conf->netaccesskey_expiry)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "DPP: Failed to generate expiry string");
            goto fail;
        }
        os_snprintf(expiry, sizeof(expiry), "%04u-%02u-%02uT%02u:%02u:%02uZ", tm.year, tm.month, tm.day, tm.hour,
                    tm.min, tm.sec);
        json_value_sep(dppcon);
    ...
    if (akm == DPP_AKM_DOT1X)
    {
        ...
        if (!auth->certbag)
            goto fail;
        json_add_base64(buf, "certBag", wpabuf_head(auth->certbag), wpabuf_len(auth->certbag));
        if (auth->cacert)
        {
            json_value_sep(buf);
            json_add_base64(buf, "caCert", wpabuf_head(auth->cacert), wpabuf_len(auth->cacert));
        }
        if (auth->trusted_eap_server_name)
        {
            json_value_sep(buf);
            json_add_string(buf, "trustedEapServerName", auth->trusted_eap_server_name);
        }
        json_value_sep(buf);
    ...
    wpabuf_put_str(buf, "\"");
    json_value_sep(buf);
    if (dpp_build_jwk(buf, "csign", auth->conf->csign, auth->conf->kid, curve) < 0)
    {
    ...
    {
        json_value_sep(buf);
        if (dpp_build_jwk(buf, "ppKey", auth->conf->pp_key, NULL, curve) < 0)
        {
    ...
    wpa_hexdump_ascii_key(MSG_DEBUG, "DPP: Configuration Object", wpabuf_head(buf), wpabuf_len(buf));
    ...
    if (!auth->conf->net_access_key_curve)
    {
         ...
         * curve to use. */
        wpa_printf(MSG_DEBUG, "DPP: Update Configurator to require netAccessKey curve %s based on first provisioning",
                   nak_curve->name);
...
static struct wpabuf *dpp_build_conf_obj_legacy(struct dpp_authentication *auth, struct dpp_configuration *conf)
{
    struct wpabuf *buf;
    const char *akm_str;
    ...
    wpa_hexdump_ascii_key(MSG_DEBUG, "DPP: Configuration Object (legacy)", wpabuf_head(buf), wpabuf_len(buf));
...
static struct wpabuf *dpp_build_conf_obj(struct dpp_authentication *auth,
                                         ...
                                         bool cert_req)
{
    ...
    if (auth->config_obj_override)
    {
        ...
        return wpabuf_alloc_copy(auth->config_obj_override, os_strlen(auth->config_obj_override));
    }
#endif /* CONFIG_TESTING_OPTIONS */
    ...
    if (!conf)
    {
        if (idx == 0)
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "DPP: No configuration available for Enrollee(%s) - waiting for configuration",
                           dpp_netrole_str(netrole));
                auth->waiting_config = true;
                dpp_get_peer_bi_id(auth);
                return NULL;
            }
            wpa_printf(MSG_DEBUG, "DPP: No configuration available for Enrollee(%s) - reject configuration request",
                       dpp_netrole_str(netrole));
        }
        return NULL;
    }
    ...
    if (conf->akm == DPP_AKM_DOT1X)
    {
        if (!auth->conf)
        {
            wpa_printf(MSG_DEBUG, "DPP: No Configurator data available");
            return NULL;
        }
        if (!cert_req && !auth->certbag)
        {
            wpa_printf(MSG_DEBUG, "DPP: No certificate data available for dot1x configuration");
            return NULL;
        }
...
struct wpabuf *dpp_build_conf_resp(
    struct dpp_authentication *auth, const u8 *e_nonce, u16 e_nonce_len, enum dpp_netrole netrole, bool cert_req)
{
    ...
    else
    {
        ...
        if (conf)
        {
            wpa_hexdump_ascii(MSG_DEBUG, "DPP: configurationObject JSON", wpabuf_head(conf), wpabuf_len(conf));
    ...
    else if (!cert_req && netrole == DPP_NETROLE_STA && auth->conf_sta && auth->conf_sta->akm == DPP_AKM_DOT1X &&
             !auth->waiting_csr)
...
forced_status:
    ...
    if (auth->peer_version >= 2 && auth->send_conn_status && netrole == DPP_NETROLE_STA)
        clear_len += 4;
    if (status == DPP_STATUS_CSR_NEEDED && auth->conf_sta && auth->conf_sta->csrattrs)
    ...
    if (status == DPP_STATUS_NEW_KEY_NEEDED)
    {
        ...
        wpa_printf(MSG_DEBUG, "DPP: Generate a new own protocol key for the curve %s",
...
skip_e_nonce:
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Second Config Object available, but peer does not support more than one");
    }
    if (env_data)
    {
    ...
    if (auth->peer_version >= 2 && auth->send_conn_status && netrole == DPP_NETROLE_STA && status == DPP_STATUS_OK)
    {
...
#ifdef CONFIG_DPP3
    if (status == DPP_STATUS_NEW_KEY_NEEDED && auth->conf && auth->conf->net_access_key_curve)
    {
        u16 ike_group = auth->conf->net_access_key_curve->ike_group;

        /* Finite Cyclic Group attribute */
        wpa_printf(MSG_DEBUG, "DPP: Finite Cyclic Group: %u", ike_group);
...
skip_status:
    ...
    wpa_hexdump_buf(MSG_DEBUG, "DPP: AES-SIV cleartext", clear);
    if (aes_siv_encrypt(auth->ke, auth->curve->hash_len, wpabuf_head(clear), wpabuf_len(clear), 1, addr, len, wrapped) <
        0)
        goto fail;
    wpa_hexdump(MSG_DEBUG, "DPP: AES-SIV ciphertext", wrapped, wpabuf_len(clear) + AES_BLOCK_SIZE);
...
skip_wrapped_data:
    ...
    wpa_hexdump_buf(MSG_DEBUG, "DPP: Configuration Response attributes", msg);
out:
...
struct wpabuf *dpp_conf_req_rx(struct dpp_authentication *auth, const u8 *attr_start, size_t attr_len)
{
    ...
    {
        wpa_printf(MSG_INFO, "DPP: TESTING - stop at Config Request");
        return NULL;
    }
#endif /* CONFIG_TESTING_OPTIONS */
    ...
    {
        dpp_auth_fail(auth, "Missing or invalid required Wrapped Data attribute");
        return NULL;
    }

    wpa_hexdump(MSG_DEBUG, "DPP: AES-SIV ciphertext", wrapped_data, wrapped_data_len);
    unwrapped_len = wrapped_data_len - AES_BLOCK_SIZE;
    unwrapped     = os_malloc(unwrapped_len);
    if (!unwrapped)
        return NULL;
    if (aes_siv_decrypt(auth->ke, auth->curve->hash_len, wrapped_data, wrapped_data_len, 0, NULL, NULL, unwrapped) < 0)
    {
        dpp_auth_fail(auth, "AES-SIV decryption failed");
        goto fail;
    }
    wpa_hexdump(MSG_DEBUG, "DPP: AES-SIV cleartext", unwrapped, unwrapped_len);
    ...
    {
        dpp_auth_fail(auth, "Missing or invalid Enrollee Nonce attribute");
        goto fail;
    }
...
#ifdef CONFIG_DPP3
    i_proto = dpp_get_attr(unwrapped, unwrapped_len, DPP_ATTR_I_PROTOCOL_KEY, &i_proto_len);
    if (i_proto && !auth->waiting_new_key)
    {
        dpp_auth_fail(auth, "Enrollee included a new protocol key even though one was not expected");
        goto fail;
    }
    if (i_proto)
    {
        ...
        wpa_hexdump(MSG_MSGDUMP, "DPP: Initiator Protocol Key (new Pe)", i_proto, i_proto_len);

        pe = dpp_set_pubkey_point(auth->own_protocol_key, i_proto, i_proto_len);
        if (!pe)
        {
            dpp_auth_fail(auth, "Invalid Initiator Protocol Key (Pe)");
            goto fail;
        }
        ...
        rx_auth_i = dpp_get_attr(unwrapped, unwrapped_len, DPP_ATTR_I_AUTH_TAG, &rx_auth_i_len);
        if (!rx_auth_i)
        {
            dpp_auth_fail(auth, "Missing Initiator Authentication Tag");
            goto fail;
        }
        if (rx_auth_i_len != auth->curve->hash_len || os_memcmp(rx_auth_i, auth_i, auth->curve->hash_len) != 0)
        {
            dpp_auth_fail(auth, "Mismatch in Initiator Authenticating Tag");
            wpa_hexdump(MSG_DEBUG, "DPP: Received Auth-I", rx_auth_i, rx_auth_i_len);
            wpa_hexdump(MSG_DEBUG, "DPP: Derived Auth-I'", auth_i, auth->curve->hash_len);
            goto fail;
        }
    }
#endif /* CONFIG_DPP3 */
    ...
    if (!config_attr)
    {
        dpp_auth_fail(auth, "Missing or invalid Config Attributes attribute");
        goto fail;
    }
    wpa_hexdump_ascii(MSG_DEBUG, "DPP: Config Attributes", config_attr, config_attr_len);
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Unsupported wi-fi_tech '%s'", token->string);
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Unsupported netRole '%s'", token->string);
    ...
    if (token && token->type == JSON_STRING)
    {
        ...
        wpa_msg(auth->msg_ctx, MSG_INFO, DPP_EVENT_MUD_URL "%s", token->string);
        os_free(auth->e_mud_url);
    ...
    if (token && token->type == JSON_ARRAY)
    {
        ...
        while (token)
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "DPP: Invalid bandSupport array member type");
            }
            else
            {
                wpa_printf(MSG_DEBUG, "DPP: Supported global operating class: %d", token->number);
        ...
        {
            res = os_snprintf(pos, end - pos, "%s%d", pos == txt ? "" : ",", opclass[i]);
            if (os_snprintf_error(end - pos, res))
            {
                *pos = '\0';
                break;
        ...
        wpa_msg(auth->msg_ctx, MSG_INFO, DPP_EVENT_BAND_SUPPORT "%s", txt);
    }
    ...
    if (cert_req)
    {
        ...
        txt = base64_encode_no_lf(wpabuf_head(cert_req), wpabuf_len(cert_req), NULL);
        if (!txt)
            goto fail;

        wpa_msg(auth->msg_ctx, MSG_INFO, DPP_EVENT_CSR "peer=%d csr=%s", id, txt);
        os_free(txt);
...
cont:
    ...
    resp = dpp_build_conf_resp(auth, e_nonce, e_nonce_len, netrole, cert_req);
...
static int dpp_parse_cred_legacy(struct dpp_config_obj *conf, struct json_token *cred)
{
    ...
    if (pass && pass->type == JSON_STRING)
    {
        ...
        wpa_hexdump_ascii_key(MSG_DEBUG, "DPP: Legacy passphrase", pass->string, len);
        if (len < 8 || len > 63)
            return -1;
        os_strlcpy(conf->passphrase, pass->string, sizeof(conf->passphrase));
    }
    else if (psk_hex && psk_hex->type == JSON_STRING)
    {
        if (dpp_akm_sae(conf->akm) && !dpp_akm_psk(conf->akm))
        {
            wpa_printf(MSG_DEBUG, "DPP: Unexpected psk_hex with akm=sae");
            return -1;
        }
        if (os_strlen(psk_hex->string) != PMK_LEN * 2 || hexstr2bin(psk_hex->string, conf->psk, PMK_LEN) < 0)
        {
            wpa_printf(MSG_DEBUG, "DPP: Invalid psk_hex encoding");
            return -1;
        }
        wpa_hexdump_key(MSG_DEBUG, "DPP: Legacy PSK", conf->psk, PMK_LEN);
        conf->psk_set = 1;
    }
    else
    {
...
struct crypto_ec_key *dpp_parse_jwk(struct json_token *jwk, const struct dpp_curve_params **key_curve)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Unexpected JWK kty '%s'", token->string);
        goto fail;
    }
    ...
    if (!curve)
    {
        wpa_printf(MSG_DEBUG, "DPP: Unsupported JWK crv '%s'", token->string);
        goto fail;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Unexpected JWK x length %u (expected %u for curve %s)", (unsigned int)wpabuf_len(x),
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Unexpected JWK y length %u (expected %u for curve %s)", (unsigned int)wpabuf_len(y),
                   (unsigned int)curve->prime_len, curve->name);
        goto fail;
    }

    key = crypto_ec_key_set_pub(curve->ike_group, wpabuf_head(x), wpabuf_head(y), wpabuf_len(x));
    if (!key)
        goto fail;
...
int dpp_key_expired(const char *timestamp, os_time_t *expiry)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Too short timestamp - assume expired key");
        return 1;
    }
    if (sscanf(timestamp, "%04u-%02u-%02uT%02u:%02u:%02u", &year, &month, &day, &hour, &min, &sec) != 6)
    {
        wpa_printf(MSG_DEBUG, "DPP: Failed to parse expiration day - assume expired key");
        return 1;
    }

    if (os_mktime(year, month, day, hour, min, sec, &utime) < 0)
    {
        wpa_printf(MSG_DEBUG, "DPP: Invalid date/time information - assume expired key");
        return 1;
    }
    ...
    else if (*pos == '-' || *pos == '+')
    {
        ...
        if (items < 1)
        {
            wpa_printf(MSG_DEBUG, "DPP: Invalid time zone designator (%s) - assume expired key", pos);
            return 1;
        }
        if (*pos == '-')
            utime += 3600 * hour;
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Cannot get current time - assume expired key");
        return 1;
    }

    if (now.sec > utime)
    {
        wpa_printf(MSG_DEBUG, "DPP: Key has expired (%lu < %lu)", utime, now.sec);
        return 1;
    }
                               ...
                               u16 payload_len)
{
    ...
    for (token = groups->child; token; token = token->sibling)
    {
        ...
        wpa_printf(MSG_DEBUG, "DPP: connector group: groupId='%s' netRole='%s'", id->string, role->string);
        rules++;
    }
skip_groups:
    ...
    if (!rules)
    {
        wpa_printf(MSG_DEBUG, "DPP: Connector includes no groups");
        goto fail;
    }
    ...
    if (!token || token->type != JSON_STRING)
    {
        wpa_printf(MSG_DEBUG, "DPP: No expiry string found - connector does not expire");
    }
    else
    {
        wpa_printf(MSG_DEBUG, "DPP: expiry = %s", token->string);
        if (dpp_key_expired(token->string, &auth->net_access_key_expiry))
        {
            wpa_printf(MSG_DEBUG, "DPP: Connector (netAccessKey) has expired");
            goto fail;
        }
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: netAccessKey in connector does not match own protocol key");
#ifdef CONFIG_TESTING_OPTIONS
        if (auth->ignore_netaccesskey_mismatch)
        {
            wpa_printf(MSG_DEBUG, "DPP: TESTING - skip netAccessKey mismatch");
        }
        else
        {
            goto fail;
...
static void dpp_copy_netaccesskey(struct dpp_authentication *auth, struct dpp_config_obj *conf)
{
...
#ifdef CONFIG_DPP2
    if (auth->reconfig_connector_key == DPP_CONFIG_REUSEKEY && auth->reconfig_old_protocol_key)
...
static int dpp_parse_cred_dpp(struct dpp_authentication *auth, struct dpp_config_obj *conf, struct json_token *cred)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Legacy credential included in Connector credential");
    ...
    if (ppkey && ppkey->type == JSON_OBJECT)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "DPP: C-sign-key and ppKey do not use the same curve");
            goto fail;
        }
    }
    ...
    wpa_hexdump_ascii(MSG_DEBUG, "DPP: signedConnector", token->string, os_strlen(token->string));
    signed_connector = token->string;

    if (os_strchr(signed_connector, '"') || os_strchr(signed_connector, '\n'))
    {
        wpa_printf(MSG_DEBUG, "DPP: Unexpected character in signedConnector");
        goto fail;
    }

    if (dpp_process_signed_connector(&info, csign_pub, signed_connector) != DPP_STATUS_OK)
        goto fail;

    if (dpp_parse_connector(auth, conf, info.payload, info.payload_len) < 0)
    {
...
#ifdef CONFIG_DPP2
static int dpp_parse_cred_dot1x(struct dpp_authentication *auth, struct dpp_config_obj *conf, struct json_token *cred)
{
    ...
    if (conf->cacert)
        wpa_hexdump_buf(MSG_MSGDUMP, "DPP: Received caCert", conf->cacert);

    name = json_get_member(ent, "trustedEapServerName");
    if (name && (name->type != JSON_STRING || has_ctrl_char((const u8 *)name->string, os_strlen(name->string))))
    {
        dpp_auth_fail(auth, "Invalid trustedEapServerName type in JSON");
        return -1;
    }
    if (name && name->string)
    {
        wpa_printf(MSG_DEBUG, "DPP: Received trustedEapServerName: %s", name->string);
...
static int dpp_parse_conf_obj(struct dpp_authentication *auth, const u8 *conf_obj, u16 conf_obj_len)
{
    int ret = -1;
    ...
    if (os_strcmp(token->string, "infra") != 0)
    {
        wpa_printf(MSG_DEBUG, "DPP: Unsupported wi-fi_tech value: '%s'", token->string);
    ...
    if (ssid64)
    {
        wpa_hexdump_ascii(MSG_DEBUG, "DPP: discovery::ssid64", wpabuf_head(ssid64), wpabuf_len(ssid64));
    ...
    else
    {
        ...
        wpa_hexdump_ascii(MSG_DEBUG, "DPP: discovery::ssid", token->string, os_strlen(token->string));
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: No room for this many Config Objects - ignore this one");
        ret = 0;
        goto fail;
    }
    ...
    if (token && token->type == JSON_NUMBER)
    {
        ...
        wpa_printf(MSG_DEBUG, "DPP: ssid_charset=%d", conf->ssid_charset);
    }
    ...
    if (legacy && auth->peer_version >= 2)
    {
        ...
        if (csign && csign->type == JSON_OBJECT && s_conn && s_conn->type == JSON_STRING)
            legacy = 0;
    }
    if (legacy)
    {
    ...
    else if (dpp_akm_dpp(conf->akm) || (auth->peer_version >= 2 && dpp_akm_legacy(conf->akm)))
    {
    ...
    {
        if (dpp_parse_cred_dot1x(auth, conf, cred) < 0 || dpp_parse_cred_dpp(auth, conf, cred) < 0)
            goto fail;
#endif /* CONFIG_DPP2 */
    }
    else
    {
        wpa_printf(MSG_DEBUG, "DPP: Unsupported akm: %s", token->string);
...
static u8 *dpp_get_csr_attrs(const u8 *attrs, size_t attrs_len, size_t *len)
{
    ...
    if (!b64)
        return NULL;
    return wpa_base64_decode((const char *)b64, b64_len, len);
}
#endif /* CONFIG_DPP2 */

int dpp_conf_resp_rx(struct dpp_authentication *auth, const struct wpabuf *resp)
{
    ...
    wrapped_data = dpp_get_attr(wpabuf_head(resp), wpabuf_len(resp), DPP_ATTR_WRAPPED_DATA, &wrapped_data_len);
    if (!wrapped_data || wrapped_data_len < AES_BLOCK_SIZE)
    {
        dpp_auth_fail(auth, "Missing or invalid required Wrapped Data attribute");
        return -1;
    ...
    if (aes_siv_decrypt(auth->ke, auth->curve->hash_len, wrapped_data, wrapped_data_len, 1, addr, len, unwrapped) < 0)
    {
        dpp_auth_fail(auth, "AES-SIV decryption failed");
        goto fail;
    }
    ...
    status = dpp_get_attr(wpabuf_head(resp), wpabuf_len(resp), DPP_ATTR_STATUS, &status_len);
    if (!status || status_len < 1)
    {
        dpp_auth_fail(auth, "Missing or invalid required DPP Status attribute");
        goto fail;
    }
    ...
    if (status[0] == DPP_STATUS_CSR_NEEDED)
    {
        ...
        csrattrs = dpp_get_csr_attrs(unwrapped, unwrapped_len, &csrattrs_len);
        if (!csrattrs)
        {
            dpp_auth_fail(auth, "Missing or invalid CSR Attributes Request attribute");
            goto fail;
        }
    ...
    if (status[0] == DPP_STATUS_NEW_KEY_NEEDED)
    {
        ...
        fcgroup = dpp_get_attr(unwrapped, unwrapped_len, DPP_ATTR_FINITE_CYCLIC_GROUP, &fcgroup_len);
        if (!fcgroup || fcgroup_len != 2)
        {
            dpp_auth_fail(auth, "Missing or invalid required Finite Cyclic Group attribute");
            goto fail;
        }
        group = WPA_GET_LE16(fcgroup);

        wpa_printf(MSG_DEBUG, "DPP: Configurator requested a new protocol key from group %u", group);
        curve = dpp_get_curve_ike_group(group);
        if (!curve)
        {
            dpp_auth_fail(auth, "Unsupported group for new protocol key");
            goto fail;
        }
        ...
        if (!new_pe)
        {
            dpp_auth_fail(auth, "Failed to generate a new protocol key");
            goto fail;
        }
        ...
        r_proto = dpp_get_attr(unwrapped, unwrapped_len, DPP_ATTR_R_PROTOCOL_KEY, &r_proto_len);
        if (!r_proto)
        {
            dpp_auth_fail(auth, "Missing required Responder Protocol Key attribute (Pc)");
            goto fail;
        }
        wpa_hexdump(MSG_MSGDUMP, "DPP: Responder Protocol Key (new Pc)", r_proto, r_proto_len);
    ...
    env_data = dpp_get_attr(unwrapped, unwrapped_len, DPP_ATTR_ENVELOPED_DATA, &env_data_len);
#ifdef CONFIG_DPP2
    if (env_data && dpp_conf_resp_env_data(auth, env_data, env_data_len) < 0)
        goto fail;
#endif /* CONFIG_DPP2 */

    conf_obj = dpp_get_attr(unwrapped, unwrapped_len, DPP_ATTR_CONFIG_OBJ, &conf_obj_len);
    if (!conf_obj && !env_data)
    {
        dpp_auth_fail(auth, "Missing required Configuration Object attribute");
        goto fail;
    }
    while (conf_obj)
    {
        wpa_hexdump_ascii(MSG_DEBUG, "DPP: configurationObject JSON", conf_obj, conf_obj_len);
...
#ifdef CONFIG_DPP2
    status = dpp_get_attr(unwrapped, unwrapped_len, DPP_ATTR_SEND_CONN_STATUS, &status_len);
    if (status)
    {
        wpa_printf(MSG_DEBUG, "DPP: Configurator requested connection status result");
                                         ...
                                         size_t attr_len)
{
    ...
    wpa_hexdump(MSG_DEBUG, "DPP: Wrapped data", wrapped_data, wrapped_data_len);
    ...
    if (!unwrapped)
        goto fail;
    if (aes_siv_decrypt(auth->ke, auth->curve->hash_len, wrapped_data, wrapped_data_len, 2, addr, len, unwrapped) < 0)
    {
        dpp_auth_fail(auth, "AES-SIV decryption failed");
        goto fail;
    }
    ...
    {
        dpp_auth_fail(auth, "Enrollee Nonce mismatch");
        wpa_hexdump(MSG_DEBUG, "DPP: Expected Enrollee Nonce", auth->e_nonce, e_nonce_len);
        goto fail;
    }
...
struct wpabuf *dpp_build_conf_result(struct dpp_authentication *auth, enum dpp_status_error status)
{
    ...
    wpa_hexdump_buf(MSG_DEBUG, "DPP: AES-SIV cleartext", clear);
    if (aes_siv_encrypt(auth->ke, auth->curve->hash_len, wpabuf_head(clear), wpabuf_len(clear), 2, addr, len, wrapped) <
        0)
        goto fail;
                                                ...
                                                char **channel_list)
{
    ...
    *channel_list = NULL;
    ...
    status = dpp_get_attr(unwrapped, unwrapped_len, DPP_ATTR_CONN_STATUS, &status_len);
    if (!status)
    {
        dpp_auth_fail(auth, "Missing required DPP Connection Status attribute");
        goto fail;
    }
    wpa_hexdump_ascii(MSG_DEBUG, "DPP: connStatus JSON", status, status_len);
    ...
    if (token && token->type == JSON_STRING && valid_channel_list(token->string))
                                     ...
                                     const char *channel_list)
{
    ...
    json_end_object(json);
    wpa_hexdump_ascii(MSG_DEBUG, "DPP: connStatus JSON", wpabuf_head(json), wpabuf_len(json));
                                            ...
                                            const char *channel_list)
{
    ...
    wpa_hexdump_buf(MSG_DEBUG, "DPP: Connection Status Result attributes", msg);
    wpabuf_free(json);
    wpabuf_free(clear);
    return msg;
fail:
...
int dpp_configurator_get_key(const struct dpp_configurator *conf, char *buf, size_t buflen)
{
    struct wpabuf *key;
    int ret = -1;
...
static int dpp_configurator_gen_kid(struct dpp_configurator *conf)
{
    ...
    if (res < 0)
    {
        wpa_printf(MSG_DEBUG, "DPP: Failed to derive kid for C-sign-key");
        return -1;
    }

    conf->kid = base64_url_encode(conf->kid_hash, sizeof(conf->kid_hash), NULL);
    return conf->kid ? 0 : -1;
}

static struct dpp_configurator *dpp_keygen_configurator(
    const char *curve, const u8 *privkey, size_t privkey_len, const u8 *pp_key, size_t pp_key_len)
{
    struct dpp_configurator *conf;
    ...
    if (privkey)
        conf->csign = dpp_set_keypair(&conf->curve, privkey, privkey_len);
    else
        conf->csign = dpp_gen_keypair(conf->curve);
    if (pp_key)
        conf->pp_key = dpp_set_keypair(&conf->curve, pp_key, pp_key_len);
    else
...
int dpp_configurator_own_config(struct dpp_authentication *auth, const char *curve, int ap)
{
    ...
    wpa_printf(MSG_DEBUG, "DPP: Building own configuration/connector with curve %s", auth->curve->name);
    ...
    ret = dpp_parse_conf_obj(auth, wpabuf_head(conf_obj), wpabuf_len(conf_obj));
fail:
                                          ...
                                          bool reconfig)
{
    ...
    for (token = groups->child; token; token = token->sibling)
    {
        ...
        if (os_strcmp(id->string, "*") != 0 && os_strcmp(group_id, "*") != 0 && os_strcmp(id->string, group_id) != 0)
            continue;
...
int dpp_connector_match_groups(struct json_token *own_root, struct json_token *peer_root, bool reconfig)
{
    struct json_token *groups, *token;
    ...
    for (token = groups->child; token; token = token->sibling)
    {
        ...
        if (!id || id->type != JSON_STRING)
        {
            wpa_printf(MSG_DEBUG, "DPP: Missing peer groupId string");
            continue;
        }
        ...
        if (!role || role->type != JSON_STRING)
        {
            wpa_printf(MSG_DEBUG, "DPP: Missing peer groups::netRole string");
            continue;
        }
        wpa_printf(MSG_DEBUG, "DPP: peer connector group: groupId='%s' netRole='%s'", id->string, role->string);
        if (dpp_connector_compatible_group(own_root, id->string, role->string, reconfig))
        {
            wpa_printf(MSG_DEBUG, "DPP: Compatible group/netRole in own connector");
            return 1;
        }
    }
...
struct json_token *dpp_parse_own_connector(const char *own_connector)
{
    ...
    if (!own_conn)
    {
        wpa_printf(MSG_DEBUG, "DPP: Failed to base64url decode own signedConnector JWS Payload");
        return NULL;
    }
...
enum dpp_status_error dpp_peer_intro(struct dpp_introduction *intro,
                                     const char *own_connector,
                                     const u8 *net_access_key,
                                     size_t net_access_key_len,
                                     const u8 *csign_key,
                                     size_t csign_key_len,
                                     const u8 *peer_connector,
                                     ...
                                     os_time_t *expiry)
{
    ...
    res = dpp_check_signed_connector(&info, csign_key, csign_key_len, peer_connector, peer_connector_len);
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Peer connector does not include compatible group netrole with own connector");
    ...
    if (own_curve != curve)
    {
        wpa_printf(MSG_DEBUG, "DPP: Mismatching netAccessKey curves (%s != %s)", own_curve->name, curve->name);
        ret = DPP_STATUS_INVALID_CONNECTOR;
        goto fail;
    }
    ...
    wpa_hexdump_key(MSG_DEBUG, "DPP: ECDH shared secret (N.x)", Nx, Nx_len);
...
static int dpp_bootstrap_del(struct dpp_global *dpp, unsigned int id)
{
    ...
    dl_list_for_each_safe(bi, tmp, &dpp->bootstrap, struct dpp_bootstrap_info, list)
    {
        if (id && bi->id != id)
            continue;
        found = 1;
...
struct dpp_bootstrap_info *dpp_add_qr_code(struct dpp_global *dpp, const char *uri)
{
    struct dpp_bootstrap_info *bi;
...
struct dpp_bootstrap_info *dpp_add_nfc_uri(struct dpp_global *dpp, const char *uri)
{
    struct dpp_bootstrap_info *bi;
...
int dpp_bootstrap_gen(struct dpp_global *dpp, const char *cmd)
{
    ...
    if (key)
    {
        ...
        if (!privkey || hexstr2bin(key, privkey, privkey_len) < 0)
            goto fail;
    }

    if (dpp_keygen(bi, curve, privkey, privkey_len) < 0 || dpp_parse_uri_chan_list(bi, bi->chan) < 0 ||
        dpp_parse_uri_mac(bi, mac) < 0 || dpp_parse_uri_info(bi, info) < 0 || dpp_gen_uri(bi) < 0)
        goto fail;
...
int dpp_bootstrap_info(struct dpp_global *dpp, int id, char *reply, int reply_size)
{
    struct dpp_bootstrap_info *bi;
    ...
    if (!bi)
        return -1;
    wpa_snprintf_hex(pkhash, sizeof(pkhash), bi->pubkey_hash, SHA256_MAC_LEN);
    return os_snprintf(reply, reply_size,
                       ...
                       "pkhash=%s\n"
                       "version=%d\n",
                       dpp_bootstrap_type_txt(bi->type), MAC2STR(bi->mac_addr), bi->info ? bi->info : "", bi->num_freq,
                       bi->num_freq == 1 ? bi->freq[0] : 0, bi->curve->name, pkhash, bi->version);
}
...
void dpp_bootstrap_find_pair(struct dpp_global *dpp,
                             ...
                             struct dpp_bootstrap_info **peer_bi)
{
    ...
    *peer_bi = NULL;
    ...
    dl_list_for_each(bi, &dpp->bootstrap, struct dpp_bootstrap_info, list)
    {
        if (!*own_bi && bi->own && os_memcmp(bi->pubkey_hash, r_bootstrap, SHA256_MAC_LEN) == 0)
        {
            wpa_printf(MSG_DEBUG, "DPP: Found matching own bootstrapping information");
            *own_bi = bi;
        }

        if (!*peer_bi && !bi->own && os_memcmp(bi->pubkey_hash, i_bootstrap, SHA256_MAC_LEN) == 0)
        {
            wpa_printf(MSG_DEBUG, "DPP: Found matching peer bootstrapping information");
            *peer_bi = bi;
        }
...
#ifdef CONFIG_DPP2
struct dpp_bootstrap_info *dpp_bootstrap_find_chirp(struct dpp_global *dpp, const u8 *hash)
{
    struct dpp_bootstrap_info *bi;
    ...
    dl_list_for_each(bi, &dpp->bootstrap, struct dpp_bootstrap_info, list)
    {
        if (!bi->own && os_memcmp(bi->pubkey_hash_chirp, hash, SHA256_MAC_LEN) == 0)
            return bi;
    }
...
static int dpp_nfc_update_bi_channel(struct dpp_bootstrap_info *own_bi, struct dpp_bootstrap_info *peer_bi)
{
    ...
    for (i = 0; i < peer_bi->num_freq; i++)
    {
        if ((own_bi->num_freq == 0 && !own_bi->channels_listed) ||
            freq_included(own_bi->freq, own_bi->num_freq, peer_bi->freq[i]))
        {
            freq = peer_bi->freq[i];
            break;
        }
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Could not determine operating class or channel number for %u MHz", freq);
    }

    wpa_printf(MSG_DEBUG,
               "DPP: Selected %u MHz (op_class %u channel %u) as the negotiation channel based on information from NFC "
               "negotiated handover",
...
static int dpp_nfc_update_bi_key(struct dpp_bootstrap_info *own_bi, struct dpp_bootstrap_info *peer_bi)
{
    if (peer_bi->curve == own_bi->curve)
        return 0;

    wpa_printf(MSG_DEBUG, "DPP: Update own bootstrapping key to match peer curve from NFC handover");

    crypto_ec_key_deinit(own_bi->pubkey);
    own_bi->pubkey = NULL;

    if (dpp_keygen(own_bi, peer_bi->curve->name, NULL, 0) < 0 || dpp_gen_uri(own_bi) < 0)
        goto fail;
...
int dpp_nfc_update_bi(struct dpp_bootstrap_info *own_bi, struct dpp_bootstrap_info *peer_bi)
{
    if (dpp_nfc_update_bi_channel(own_bi, peer_bi) < 0 || dpp_nfc_update_bi_key(own_bi, peer_bi) < 0)
        return -1;
    return 0;
}
...
int dpp_configurator_add(struct dpp_global *dpp, const char *cmd)
{
    ...
    if (curve)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "DPP: Unsupported net_access_key_curve: %s", curve);
            goto fail;
        }
        os_free(curve);
    }
    ...
    if (ppkey)
    {
        ...
        if (!pp_key || hexstr2bin(ppkey, pp_key, pp_key_len) < 0)
            goto fail;
    }

    conf = dpp_keygen_configurator(curve, privkey, privkey_len, pp_key, pp_key_len);
    if (!conf)
        goto fail;
...
static int dpp_configurator_del(struct dpp_global *dpp, unsigned int id)
{
    ...
    dl_list_for_each_safe(conf, tmp, &dpp->configurator, struct dpp_configurator, list)
    {
        if (id && conf->id != id)
            continue;
        found = 1;
...
int dpp_configurator_get_key_id(struct dpp_global *dpp, unsigned int id, char *buf, size_t buflen)
{
    struct dpp_configurator *conf;
...
int dpp_configurator_from_backup(struct dpp_global *dpp, struct dpp_asymmetric_key *key)
{
    struct dpp_configurator *conf;
    ...
    if (curve != curve_pp)
    {
        wpa_printf(MSG_INFO, "DPP: Mismatch in c-sign-key and ppKey groups");
        return -1;
    }
...
struct dpp_configurator *dpp_configurator_find_kid(struct dpp_global *dpp, const u8 *kid)
{
    struct dpp_configurator *conf;
...
struct wpabuf *dpp_build_presence_announcement(struct dpp_bootstrap_info *bi)
{
    ...
    wpa_hexdump_buf(MSG_DEBUG, "DPP: Presence Announcement frame attributes", msg);
    return msg;
}

void dpp_notify_chirp_received(void *msg_ctx, int id, const u8 *src, unsigned int freq, const u8 *hash)
{
    char hex[SHA256_MAC_LEN * 2 + 1];

    wpa_snprintf_hex(hex, sizeof(hex), hash, SHA256_MAC_LEN);
    wpa_msg(msg_ctx, MSG_INFO, DPP_EVENT_CHIRP_RX "id=%d src=" MACSTR " freq=%u hash=%s", id, MAC2STR(src), freq, hex);
}
