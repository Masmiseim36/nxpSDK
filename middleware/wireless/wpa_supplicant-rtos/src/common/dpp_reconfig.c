                                               ...
                                               struct dpp_reconfig_id *id)
{
    ...
    if (res < 0)
        goto fail;
    wpa_hexdump(MSG_DEBUG, "DPP: kid = SHA256(uncompressed C-sign key)", hash, SHA256_MAC_LEN);
    ...
    wpa_printf(MSG_DEBUG, "DPP: Finite Cyclic Group: %u", own_curve->ike_group);
    ...
    wpa_hexdump_buf(MSG_DEBUG, "DPP: Reconfig Announcement frame attributes", msg);
fail:
    wpabuf_free(a_nonce);
    wpabuf_free(e_id);
...
static struct wpabuf *dpp_reconfig_build_req(struct dpp_authentication *auth)
{
    ...
    attr_len = 4 + 1 + 4 + 1 + 4 + os_strlen(auth->conf->connector) + 4 + auth->curve->nonce_len;
...
skip_proto_ver:
    ...
    wpa_hexdump_buf(MSG_DEBUG, "DPP: Reconfig Authentication Request frame attributes", msg);

    return msg;
}

static int dpp_configurator_build_own_connector(struct dpp_configurator *conf, const struct dpp_curve_params *curve)
{
    ...
    wpa_printf(MSG_DEBUG, "DPP: Sign own Configurator Connector for reconfiguration with curve %s", conf->curve->name);
    ...
    json_value_sep(dppcon);
    if (dpp_build_jwk(dppcon, "netAccessKey", conf->connector_key, NULL, curve) < 0)
    {
...
struct dpp_authentication *dpp_reconfig_init(struct dpp_global *dpp,
                                             void *msg_ctx,
                                             struct dpp_configurator *conf,
                                             unsigned int freq,
                                             u16 group,
                                             const u8 *a_nonce_attr,
                                             size_t a_nonce_len,
                                             const u8 *e_id_attr,
                                             size_t e_id_len)
{
    ...
    if (!curve)
    {
        wpa_printf(MSG_DEBUG, "DPP: Unsupported group %u - cannot reconfigure", group);
        return NULL;
    }
                                   ...
                                   struct wpabuf *conn_status)
{
    ...
    clear_len = 4 + auth->curve->nonce_len + 4 + wpabuf_len(conn_status);
    ...
    attr_len = 4 + 1 + 4 + 1 + 4 + os_strlen(own_connector) + 4 + auth->curve->nonce_len + 4 + wpabuf_len(pr) + 4 +
               wpabuf_len(clear) + AES_BLOCK_SIZE;
    ...
    wpa_hexdump_buf(MSG_DEBUG, "DPP: Reconfig Authentication Response frame attributes", msg);
...
struct dpp_authentication *dpp_reconfig_auth_req_rx(struct dpp_global *dpp,
                                                    void *msg_ctx,
                                                    const char *own_connector,
                                                    const u8 *net_access_key,
                                                    size_t net_access_key_len,
                                                    ...
                                                    size_t attr_len)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Missing or invalid Protocol Version attribute");
        goto fail;
    }
    ...
    wpa_hexdump_ascii(MSG_DEBUG, "DPP: I-Connector", i_connector, i_connector_len);

    c_nonce = dpp_get_attr(attr_start, attr_len, DPP_ATTR_CONFIGURATOR_NONCE, &c_nonce_len);
    if (!c_nonce || c_nonce_len > DPP_MAX_NONCE_LEN)
    {
        wpa_printf(MSG_DEBUG, "DPP: Missing or invalid C-nonce attribute");
        goto fail;
    }
    wpa_hexdump(MSG_DEBUG, "DPP: C-nonce", c_nonce, c_nonce_len);

    res = dpp_check_signed_connector(&info, csign_key, csign_key_len, i_connector, i_connector_len);
    ...
    if (!root || !own_root || !dpp_connector_match_groups(own_root, root, true))
    {
        wpa_printf(MSG_DEBUG, "DPP: I-Connector does not include compatible group netrole with own connector");
        goto fail;
    }

    token = json_get_member(root, "expiry");
    if (token && token->type == JSON_STRING && dpp_key_expired(token->string, NULL))
    {
        wpa_printf(MSG_DEBUG, "DPP: I-Connector (netAccessKey) has expired");
        goto fail;
    }
    ...
    if (dpp_reconfig_derive_ke_responder(auth, net_access_key, net_access_key_len, token) < 0)
        goto fail;

    if (c_nonce_len != auth->curve->nonce_len)
    {
        wpa_printf(MSG_DEBUG, "DPP: Unexpected C-nonce length %u (curve nonce len %zu)", c_nonce_len,
...
struct wpabuf *dpp_reconfig_build_conf(struct dpp_authentication *auth)
{
    ...
    clear_len = 4 + 1 + 4 + 1 + 2 * (4 + auth->curve->nonce_len) + 4 + 1;
    clear     = wpabuf_alloc(clear_len);
    if (!clear)
        goto fail;
    ...
    wpa_hexdump_buf(MSG_DEBUG, "DPP: Reconfig Authentication Confirm frame attributes", msg);
...
struct wpabuf *dpp_reconfig_auth_resp_rx(struct dpp_authentication *auth,
                                         const u8 *hdr,
                                         const u8 *attr_start,
                                         size_t attr_len)
{
    const u8 *trans_id, *version, *r_connector, *r_proto, *wrapped_data, *c_nonce, *e_nonce, *conn_status;
    u16 trans_id_len, version_len, r_connector_len, r_proto_len, wrapped_data_len, c_nonce_len, e_nonce_len,
        conn_status_len;
    ...
    if (!version || version_len < 1 || version[0] < 2)
    {
        dpp_auth_fail(auth, "Missing or invalid Protocol Version attribute");
        goto fail;
    }
    auth->peer_version = version[0];
    wpa_printf(MSG_DEBUG, "DPP: Peer protocol version %u", auth->peer_version);
    ...
    wpa_hexdump_ascii(MSG_DEBUG, "DPP: R-Connector", r_connector, r_connector_len);
    ...
    res = dpp_process_signed_connector(&info, auth->conf->csign, signed_connector);
    if (res != DPP_STATUS_OK)
    {
    ...
    if (dpp_reconfig_derive_ke_initiator(auth, r_proto, r_proto_len, token) < 0)
        goto fail;
    ...
    if (!c_nonce || c_nonce_len != auth->curve->nonce_len || os_memcmp(c_nonce, auth->c_nonce, c_nonce_len) != 0)
    {
    ...
    wpa_hexdump_ascii(MSG_DEBUG, "DPP: connStatus", conn_status, conn_status_len);

    conn_status_json = json_parse((const char *)conn_status, conn_status_len);
    if (!conn_status_json)
    {
...
int dpp_reconfig_auth_conf_rx(struct dpp_authentication *auth, const u8 *hdr, const u8 *attr_start, size_t attr_len)
{
    const u8 *trans_id, *version, *wrapped_data, *c_nonce, *e_nonce, *reconfig_flags, *status;
    u16 trans_id_len, version_len, wrapped_data_len, c_nonce_len, e_nonce_len, reconfig_flags_len, status_len;
    const u8 *addr[2];
    size_t len[2];
    u8 *unwrapped        = NULL;
    size_t unwrapped_len = 0;
    ...
    {
        dpp_auth_fail(auth, "Reconfiguration did not complete successfully");
        goto fail;
    }
    ...
    if (!trans_id || trans_id_len != 1 || trans_id[0] != auth->transaction_id)
    {
        dpp_auth_fail(auth, "Peer did not include valid Transaction ID");
        goto fail;
    }
    ...
    if (!e_nonce || e_nonce_len != auth->curve->nonce_len || os_memcmp(e_nonce, auth->e_nonce, e_nonce_len) != 0)
    {
        dpp_auth_fail(auth, "Missing or invalid E-nonce");
        goto fail;
    }
    ...
    reconfig_flags = dpp_get_attr(unwrapped, unwrapped_len, DPP_ATTR_RECONFIG_FLAGS, &reconfig_flags_len);
