...
struct dpp_relay_controller
{
    ...
    void *msg_ctx;
    void *cb_ctx;
    void (*tx)(void *ctx, const u8 *addr, unsigned int freq, const u8 *msg, size_t len);
    void (*gas_resp_tx)(void *ctx, const u8 *addr, u8 dialog_token, int prot, struct wpabuf *buf);
...
static void dpp_controller_auth_success(struct dpp_connection *conn, int initiator);
...
static void dpp_connection_free(struct dpp_connection *conn)
{
    if (conn->sock >= 0)
    {
        wpa_printf(MSG_DEBUG, "DPP: Close Controller socket %d", conn->sock);
        eloop_unregister_sock(conn->sock, EVENT_TYPE_READ);
        eloop_unregister_sock(conn->sock, EVENT_TYPE_WRITE);
        close(conn->sock);
    }
    eloop_cancel_timeout(dpp_controller_conn_status_result_wait_timeout, conn, NULL);
...
static struct dpp_relay_controller *dpp_relay_controller_get(struct dpp_global *dpp, const u8 *pkhash)
{
    struct dpp_relay_controller *ctrl;
    ...
    dl_list_for_each(ctrl, &dpp->controllers, struct dpp_relay_controller, list)
    {
...
static int dpp_tcp_send(struct dpp_connection *conn)
{
    ...
    res = send(conn->sock, wpabuf_head_u8(conn->msg_out) + conn->msg_out_pos,
               wpabuf_len(conn->msg_out) - conn->msg_out_pos, 0);
    if (res < 0)
    {
        wpa_printf(MSG_DEBUG, "DPP: Failed to send buffer: %s", strerror(errno));
        dpp_connection_remove(conn);
        return -1;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: %u/%u bytes of message sent to Controller", (unsigned int)conn->msg_out_pos,
                   (unsigned int)wpabuf_len(conn->msg_out));
        if (!conn->write_eloop && eloop_register_sock(conn->sock, EVENT_TYPE_WRITE, dpp_conn_tx_ready, conn, NULL) == 0)
            conn->write_eloop = 1;
        return 1;
    }
    ...
    eloop_unregister_sock(conn->sock, EVENT_TYPE_WRITE);
    conn->write_eloop = 0;
    if (!conn->read_eloop && eloop_register_sock(conn->sock, EVENT_TYPE_READ, dpp_controller_rx, conn, NULL) == 0)
        conn->read_eloop = 1;
    if (conn->on_tcp_tx_complete_remove)
    {
        if (conn->auth && conn->auth->connect_on_tx_status && conn->tcp_msg_sent &&
            conn->tcp_msg_sent(conn->cb_ctx, conn->auth))
            return 0;
        dpp_connection_remove(conn);
    }
    else if (conn->auth && (conn->ctrl || conn->auth->configurator) && conn->on_tcp_tx_complete_gas_done)
    {
...
static int dpp_tcp_send_msg(struct dpp_connection *conn, const struct wpabuf *msg)
{
    ...
    wpabuf_put_data(conn->msg_out, wpabuf_head_u8(msg) + 1, wpabuf_len(msg) - 1);

    if (dpp_tcp_send(conn) == 1)
    {
        if (!conn->write_eloop)
        {
            if (eloop_register_sock(conn->sock, EVENT_TYPE_WRITE, dpp_conn_tx_ready, conn, NULL) < 0)
                return -1;
            conn->write_eloop = 1;
        }
    }
...
static void dpp_controller_start_gas_client(struct dpp_connection *conn)
{
    ...
    buf      = dpp_build_conf_req_helper(auth, dpp_name, conn->netrole, NULL, NULL);
    if (!buf)
    {
...
static void dpp_controller_auth_success(struct dpp_connection *conn, int initiator)
{
    ...
    wpa_msg(conn->msg_ctx, MSG_INFO, DPP_EVENT_AUTH_SUCCESS "init=%d", initiator);
...
static int dpp_ipaddr_to_sockaddr(struct sockaddr *addr,
                                  socklen_t *addrlen,
                                  ...
                                  int port)
{
    ...
    switch (ipaddr->af)
    {
        ...
        case AF_INET6:
            ...
            os_memcpy(&dst6->sin6_addr, &ipaddr->u.v6, sizeof(struct in6_addr));
...
static void dpp_relay_conn_timeout(void *eloop_ctx, void *timeout_ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "DPP: Timeout while waiting for relayed connection to complete");
    dpp_connection_remove(conn);
}

static struct dpp_connection *dpp_relay_new_conn(struct dpp_relay_controller *ctrl, const u8 *src, unsigned int freq)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Too many ongoing Relay connections to the Controller - cannot start a new one");
        return NULL;
    }

    if (dpp_ipaddr_to_sockaddr((struct sockaddr *)&addr, &addrlen, &ctrl->ipaddr, DPP_TCP_PORT) < 0)
        return NULL;
    ...
    if (conn->sock < 0)
        goto fail;
    wpa_printf(MSG_DEBUG, "DPP: TCP relay socket %d connection to %s", conn->sock,
               hostapd_ip_txt(&ctrl->ipaddr, txt, sizeof(txt)));

    if (fcntl(conn->sock, F_SETFL, O_NONBLOCK) != 0)
    {
        wpa_printf(MSG_DEBUG, "DPP: fnctl(O_NONBLOCK) failed: %s", strerror(errno));
        goto fail;
    }
    ...
    if (connect(conn->sock, (struct sockaddr *)&addr, addrlen) < 0)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "DPP: Failed to connect: %s", strerror(errno));
            goto fail;
        }
...
static int dpp_relay_tx(struct dpp_connection *conn, const u8 *hdr, const u8 *buf, size_t len)
{
    u8 type = hdr[DPP_HDR_LEN - 1];

    wpa_printf(MSG_DEBUG, "DPP: Continue already established Relay/Controller connection for this session");
    wpabuf_free(conn->msg_out);
    conn->msg_out_pos = 0;
...
int dpp_relay_rx_action(struct dpp_global *dpp,
                        const u8 *src,
                        const u8 *hdr,
                        const u8 *buf,
                        size_t len,
                        ...
                        void *cb_ctx)
{
     ...
     */
    if (type != DPP_PA_PEER_DISCOVERY_REQ && type != DPP_PA_PEER_DISCOVERY_RESP &&
        type != DPP_PA_PRESENCE_ANNOUNCEMENT && type != DPP_PA_RECONFIG_ANNOUNCEMENT)
    {
        dl_list_for_each(ctrl, &dpp->controllers, struct dpp_relay_controller, list)
        {
    ...
    if (type == DPP_PA_PRESENCE_ANNOUNCEMENT || type == DPP_PA_RECONFIG_ANNOUNCEMENT)
    {
    ...
    wpa_printf(MSG_DEBUG, "DPP: Authentication Request for a configured Controller");
...
static void dpp_controller_free(struct dpp_controller *ctrl)
{
    ...
    dl_list_for_each_safe(conn, tmp, &ctrl->conn, struct dpp_connection, list) dpp_connection_remove(conn);
...
static int dpp_controller_rx_auth_req(struct dpp_connection *conn, const u8 *hdr, const u8 *buf, size_t len)
{
    ...
    {
        wpa_printf(MSG_INFO, "Missing or invalid required Responder Bootstrapping Key Hash attribute");
        return -1;
    }
    ...
    {
        wpa_printf(MSG_INFO, "Missing or invalid required Initiator Bootstrapping Key Hash attribute");
        return -1;
    }
    ...
    dpp_bootstrap_find_pair(conn->ctrl->global, i_bootstrap, r_bootstrap, &own_bi, &peer_bi);
    if (!own_bi)
    {
        wpa_printf(MSG_INFO, "No matching own bootstrapping key found - ignore message");
        return -1;
    }

    if (conn->auth)
    {
        wpa_printf(MSG_INFO, "Already in DPP authentication exchange - ignore new one");
        return 0;
    }

    conn->auth = dpp_auth_req_rx(conn->ctrl->global, conn->msg_ctx, conn->ctrl->allowed_roles, conn->ctrl->qr_mutual,
    ...
    if (dpp_set_configurator(conn->auth, conn->ctrl->configurator_params) < 0)
        return -1;

    return dpp_tcp_send_msg(conn, conn->auth->resp_msg);
}

static int dpp_controller_rx_auth_resp(struct dpp_connection *conn, const u8 *hdr, const u8 *buf, size_t len)
{
    struct dpp_authentication *auth = conn->auth;
    struct wpabuf *msg;
    int res;
...
static int dpp_controller_rx_auth_conf(struct dpp_connection *conn, const u8 *hdr, const u8 *buf, size_t len)
{
    struct dpp_authentication *auth = conn->auth;
...
void dpp_controller_conn_status_result_wait_timeout(void *eloop_ctx, void *timeout_ctx)
{
    struct dpp_connection *conn = eloop_ctx;
    ...
    wpa_msg(conn->msg_ctx, MSG_INFO, DPP_EVENT_CONN_STATUS_RESULT "timeout");
    dpp_connection_remove(conn);
}

static int dpp_controller_rx_conf_result(struct dpp_connection *conn, const u8 *hdr, const u8 *buf, size_t len)
{
    struct dpp_authentication *auth = conn->auth;
    ...
    {
        wpa_msg(msg_ctx, MSG_INFO, DPP_EVENT_CONF_SENT "wait_conn_status=1");
        wpa_printf(MSG_DEBUG, "DPP: Wait for Connection Status Result");
        auth->waiting_conn_status_result = 1;
        eloop_cancel_timeout(dpp_controller_conn_status_result_wait_timeout, conn, NULL);
        eloop_register_timeout(16, 0, dpp_controller_conn_status_result_wait_timeout, conn, NULL);
        return 0;
    }
...
static int dpp_controller_rx_conn_status_result(struct dpp_connection *conn, const u8 *hdr, const u8 *buf, size_t len)
{
    struct dpp_authentication *auth = conn->auth;
    enum dpp_status_error status;
    ...
    wpa_msg(conn->msg_ctx, MSG_INFO, DPP_EVENT_CONN_STATUS_RESULT "result=%d ssid=%s channel_list=%s", status,
                                                   ...
                                                   size_t len)
{
    ...
    auth = dpp_auth_init(dpp, conn->msg_ctx, peer_bi, NULL, DPP_CAPAB_CONFIGURATOR, -1, NULL, 0);
    if (!auth)
        return -1;
                                                   ...
                                                   size_t len)
{
    ...
    {
        wpa_msg(conn->msg_ctx, MSG_INFO, DPP_EVENT_FAIL "Missing or invalid required Finite Cyclic Group attribute");
        return -1;
    }
    ...
    auth = dpp_reconfig_init(dpp, conn->msg_ctx, conf, 0, group, a_nonce, a_nonce_len, e_id, e_id_len);
    if (!auth)
        return -1;
    if (dpp_set_configurator(auth, conn->ctrl->configurator_params) < 0)
    {
...
static int dpp_controller_rx_reconfig_auth_resp(struct dpp_connection *conn, const u8 *hdr, const u8 *buf, size_t len)
{
    struct dpp_authentication *auth = conn->auth;
    struct wpabuf *conf;
    int res;
...
static int dpp_controller_rx_pkex_exchange_req(struct dpp_connection *conn, const u8 *hdr, const u8 *buf, size_t len)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Already in PKEX/Authentication session - ignore new PKEX request");
        return 0;
    }

    conn->pkex = dpp_pkex_rx_exchange_req(conn->ctrl->global, ctrl->pkex_bi, NULL, NULL, ctrl->pkex_identifier,
                                          ctrl->pkex_code, buf, len, true);
    if (!conn->pkex)
    {
...
static int dpp_controller_rx_pkex_exchange_resp(struct dpp_connection *conn, const u8 *hdr, const u8 *buf, size_t len)
{
...
static int dpp_controller_rx_pkex_commit_reveal_resp(struct dpp_connection *conn,
                                                     const u8 *hdr,
                                                     const u8 *buf,
                                                     size_t len)
{
...
static int dpp_controller_rx_action(struct dpp_connection *conn, const u8 *msg, size_t len)
{
    const u8 *pos, *end;
    u8 type;
    ...
    if (end - pos < DPP_HDR_LEN || WPA_GET_BE24(pos) != OUI_WFA || pos[3] != DPP_OUI_TYPE)
    {
    ...
    if (pos[4] != 1)
    {
        wpa_printf(MSG_DEBUG, "DPP: Unsupported Crypto Suite %u", pos[4]);
        return -1;
    }
    type = pos[5];
    ...
    wpa_hexdump(MSG_MSGDUMP, "DPP: Received message attributes", pos, end - pos);
    ...
    if (conn->relay)
    {
        ...
        conn->relay->tx(conn->relay->cb_ctx, conn->mac_addr, conn->freq, msg, len);
        return 0;
    }
    ...
    switch (type)
    {
        ...
        case DPP_PA_CONNECTION_STATUS_RESULT:
            return dpp_controller_rx_conn_status_result(conn, msg, pos, end - pos);
        case DPP_PA_PRESENCE_ANNOUNCEMENT:
            return dpp_controller_rx_presence_announcement(conn, msg, pos, end - pos);
        case DPP_PA_RECONFIG_ANNOUNCEMENT:
            return dpp_controller_rx_reconfig_announcement(conn, msg, pos, end - pos);
        case DPP_PA_RECONFIG_AUTH_RESP:
            return dpp_controller_rx_reconfig_auth_resp(conn, msg, pos, end - pos);
        case DPP_PA_PKEX_V1_EXCHANGE_REQ:
            wpa_printf(MSG_DEBUG, "DPP: Ignore PKEXv1 Exchange Request - not supported over TCP");
            return -1;
        case DPP_PA_PKEX_EXCHANGE_REQ:
            return dpp_controller_rx_pkex_exchange_req(conn, msg, pos, end - pos);
        case DPP_PA_PKEX_EXCHANGE_RESP:
            return dpp_controller_rx_pkex_exchange_resp(conn, msg, pos, end - pos);
        case DPP_PA_PKEX_COMMIT_REVEAL_REQ:
            return dpp_controller_rx_pkex_commit_reveal_req(conn, msg, pos, end - pos);
        case DPP_PA_PKEX_COMMIT_REVEAL_RESP:
            return dpp_controller_rx_pkex_commit_reveal_resp(conn, msg, pos, end - pos);
        default:
            /* TODO: missing messages types */
            wpa_printf(MSG_DEBUG, "DPP: Unsupported frame subtype %d", type);
            return -1;
    }
}
...
static int dpp_tcp_send_gas_resp(struct dpp_connection *conn, u8 action, struct wpabuf *resp)
{
    struct wpabuf *buf;
    size_t len;
...
static int dpp_controller_rx_gas_req(struct dpp_connection *conn, const u8 *msg, size_t len)
{
    ...
    wpa_printf(MSG_DEBUG, "DPP: Received DPP Configuration Request over TCP");

    if (!auth || (!conn->ctrl && !auth->configurator) || (!auth->auth_success && !auth->reconfig_success))
    {
    ...
    adv_proto              = pos++;
    slen                   = *pos++;
    if (*adv_proto != WLAN_EID_ADV_PROTO || slen > end - pos || slen < 2)
        return -1;

    next = pos + slen;
    pos++; /* skip QueryRespLenLimit and PAME-BI */

    if (slen != 8 || *pos != WLAN_EID_VENDOR_SPECIFIC || pos[1] != 5 || WPA_GET_BE24(&pos[2]) != OUI_WFA ||
    ...
    if (!resp && auth->waiting_cert)
    {
        ...
        return dpp_tcp_send_comeback_delay(conn, WLAN_PA_GAS_INITIAL_RESP);
    }
    ...
    if (!resp && auth->waiting_config && auth->peer_bi)
    {
        ...
        if (auth->e_name)
        {
            ...
            if (buf)
            {
                printf_encode(buf, len * 4 + 1, (const u8 *)auth->e_name, e_len);
                name = buf;
            }
        }
        band[0] = '\0';
        ...
        {
            res = os_snprintf(b_pos, b_end - b_pos, "%s%d", b_pos == band ? "" : ",", opclass[i]);
        ...
        wpa_msg(conn->msg_ctx, MSG_INFO, DPP_EVENT_CONF_NEEDED "peer=%d net_role=%s name=\"%s\" opclass=%s mud_url=%s",
                auth->peer_bi->id, dpp_netrole_str(auth->e_netrole), name, band, mud_url);
        os_free(buf);
...
static int dpp_controller_rx_gas_comeback_req(struct dpp_connection *conn, const u8 *msg, size_t len)
{
    u8 dialog_token;
    struct dpp_authentication *auth = conn->auth;
    struct wpabuf *resp;
    ...
    wpa_printf(MSG_DEBUG, "DPP: Received DPP Configuration Request over TCP (comeback)");
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Dialog token mismatch (%u != %u)", dialog_token, conn->gas_dialog_token);
        return -1;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: Certificate not yet ready");
        return dpp_tcp_send_comeback_delay(conn, WLAN_PA_GAS_COMEBACK_RESP);
    }

    wpa_printf(MSG_DEBUG, "DPP: Configuration response is ready to be sent out");
...
static int dpp_tcp_rx_gas_resp(struct dpp_connection *conn, struct wpabuf *resp)
{
    ...
    wpa_printf(MSG_DEBUG, "DPP: Configuration Response for local stack from TCP");
    ...
    if (res == -3)
    {
        ...
        eloop_register_timeout(0, 0, dpp_tcp_build_new_key, conn, NULL);
        return 0;
    }
#endif /* CONFIG_DPP3 */
    if (res < 0)
...
static int dpp_rx_gas_resp(struct dpp_connection *conn, const u8 *msg, size_t len, bool comeback)
{
    struct wpabuf *buf;
    u8 dialog_token;
    ...
    wpa_printf(MSG_DEBUG, "DPP: Received DPP Configuration Response over TCP");
    ...
    if (comeback_delay)
    {
        ...
        wpa_printf(MSG_DEBUG, "DPP: Comeback delay: %u", comeback_delay);
        eloop_cancel_timeout(dpp_tcp_gas_query_comeback, conn, NULL);
        eloop_register_timeout(secs, usecs, dpp_tcp_gas_query_comeback, conn, NULL);
        return 0;
    }
    ...
    if (!conn->relay && (!conn->ctrl || (conn->ctrl->allowed_roles & DPP_CAPAB_ENROLLEE)))
    ...
    wpa_printf(MSG_DEBUG, "DPP: Relay - send over WLAN");
    conn->relay->gas_resp_tx(conn->relay->cb_ctx, conn->mac_addr, dialog_token, 0, buf);
...
static void dpp_controller_rx(int sd, void *eloop_ctx, void *sock_ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "DPP: TCP data available for reading (sock %d)", sd);

    if (conn->msg_len_octets < 4)
    {
        u32 msglen;

        res = recv(sd, &conn->msg_len[conn->msg_len_octets], 4 - conn->msg_len_octets, 0);
        if (res < 0)
        {
        ...
        wpa_printf(MSG_DEBUG, "DPP: Received %d/%d octet(s) of message length field", res,
                   (int)(4 - conn->msg_len_octets));
        conn->msg_len_octets += res;

        if (conn->msg_len_octets < 4)
        {
            wpa_printf(MSG_DEBUG, "DPP: Need %d more octets of message length field", (int)(4 - conn->msg_len_octets));
            return;
        }
    ...
    if (!conn->msg)
    {
        wpa_printf(MSG_DEBUG, "DPP: No buffer available for receiving the message");
        dpp_connection_remove(conn);
        return;
    }

    wpa_printf(MSG_DEBUG, "DPP: Need %u more octets of message payload", (unsigned int)wpabuf_tailroom(conn->msg));
    ...
    switch (*pos)
    {
        ...
        case WLAN_PA_GAS_INITIAL_REQ:
            if (dpp_controller_rx_gas_req(conn, pos + 1, wpabuf_len(conn->msg) - 1) < 0)
                dpp_connection_remove(conn);
            break;
        case WLAN_PA_GAS_INITIAL_RESP:
        case WLAN_PA_GAS_COMEBACK_RESP:
            if (dpp_rx_gas_resp(conn, pos + 1, wpabuf_len(conn->msg) - 1, *pos == WLAN_PA_GAS_COMEBACK_RESP) < 0)
                dpp_connection_remove(conn);
            break;
        case WLAN_PA_GAS_COMEBACK_REQ:
            if (dpp_controller_rx_gas_comeback_req(conn, pos + 1, wpabuf_len(conn->msg) - 1) < 0)
                dpp_connection_remove(conn);
            break;
        default:
            wpa_printf(MSG_DEBUG, "DPP: Ignore unsupported message type %u", *pos);
            break;
    }
}
...
static void dpp_controller_tcp_cb(int sd, void *eloop_ctx, void *sock_ctx)
{
    ...
    if (fd < 0)
    {
        wpa_printf(MSG_DEBUG, "DPP: Failed to accept new connection: %s", strerror(errno));
        return;
    }
    wpa_printf(MSG_DEBUG, "DPP: Connection from %s:%d", inet_ntoa(addr.sin_addr), ntohs(addr.sin_port));
    ...
    if (eloop_register_sock(conn->sock, EVENT_TYPE_READ, dpp_controller_rx, conn, NULL) < 0)
        goto fail;
    conn->read_eloop = 1;
...
int dpp_tcp_pkex_init(struct dpp_global *dpp,
                      struct dpp_pkex *pkex,
                      ...
                      int (*pkex_done)(void *ctx, void *conn, struct dpp_bootstrap_info *bi))
{
    ...
    wpa_printf(MSG_DEBUG, "DPP: Initialize TCP connection to %s port %d", hostapd_ip_txt(addr, txt, sizeof(txt)), port);
    if (dpp_ipaddr_to_sockaddr((struct sockaddr *)&saddr, &addrlen, addr, port) < 0)
    {
        dpp_pkex_free(pkex);
        return -1;
    }
...
static int dpp_tcp_auth_start(struct dpp_connection *conn, struct dpp_authentication *auth)
{
    const u8 *hdr, *pos, *end;
...
int dpp_tcp_init(struct dpp_global *dpp,
                 struct dpp_authentication *auth,
                 const struct hostapd_ip_addr *addr,
                 int port,
                 const char *name,
...
int dpp_tcp_auth(struct dpp_global *dpp,
                 void *_conn,
                 struct dpp_authentication *auth,
                 const char *name,
                 enum dpp_netrole netrole,
...
int dpp_controller_start(struct dpp_global *dpp, struct dpp_controller_config *config)
{
    ...
    if (config->configurator_params)
        ctrl->configurator_params = os_strdup(config->configurator_params);
    dl_list_init(&ctrl->conn);
    ...
    if (setsockopt(ctrl->sock, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) < 0)
    {
        wpa_printf(MSG_DEBUG, "DPP: setsockopt(SO_REUSEADDR) failed: %s", strerror(errno));
        /* try to continue anyway */
    }

    if (fcntl(ctrl->sock, F_SETFL, O_NONBLOCK) < 0)
    {
        wpa_printf(MSG_INFO, "DPP: fnctl(O_NONBLOCK) failed: %s", strerror(errno));
        goto fail;
    }
    ...
    {
        wpa_printf(MSG_INFO, "DPP: Failed to bind Controller TCP port: %s", strerror(errno));
        goto fail;
    }
    if (listen(ctrl->sock, 10 /* max backlog */) < 0 || fcntl(ctrl->sock, F_SETFL, O_NONBLOCK) < 0 ||
        eloop_register_sock(ctrl->sock, EVENT_TYPE_READ, dpp_controller_tcp_cb, ctrl, NULL))
        goto fail;
...
static bool dpp_tcp_peer_id_match(struct dpp_authentication *auth, unsigned int id)
{
    return auth && ((auth->peer_bi && auth->peer_bi->id == id) || (auth->tmp_peer_bi && auth->tmp_peer_bi->id == id));
}

static struct dpp_authentication *dpp_tcp_get_auth(struct dpp_global *dpp, unsigned int id)
{
    struct dpp_connection *conn;
...
struct dpp_authentication *dpp_controller_get_auth(struct dpp_global *dpp, unsigned int id)
{
...
void dpp_controller_new_qr_code(struct dpp_global *dpp, struct dpp_bootstrap_info *bi)
{
    ...
    dl_list_for_each(conn, &ctrl->conn, struct dpp_connection, list)
    {
        ...
        if (!auth->response_pending || dpp_notify_new_qr_code(auth, bi) != 1)
            continue;
...
void dpp_tcp_init_flush(struct dpp_global *dpp)
{
    ...
    dl_list_for_each_safe(conn, tmp, &dpp->tcp_init, struct dpp_connection, list) dpp_connection_remove(conn);
}
...
void dpp_relay_flush_controllers(struct dpp_global *dpp)
{
    ...
    dl_list_for_each_safe(ctrl, tmp, &dpp->controllers, struct dpp_relay_controller, list)
    {
                                         ...
                                         const char *channel_list)
{
    ...
    msg = dpp_build_conn_status_result(auth, result, ssid, ssid_len, channel_list);
    if (!msg)
    {
        dpp_connection_remove(conn);
        return;
...
void dpp_tcp_send_conn_status(
    struct dpp_global *dpp, enum dpp_status_error result, const u8 *ssid, size_t ssid_len, const char *channel_list)
{
    struct dpp_connection *conn;
    ...
    dl_list_for_each(conn, &dpp->tcp_init, struct dpp_connection, list)
    {
        if (conn->auth && conn->auth->conn_status_requested)
        {
            dpp_tcp_send_conn_status_msg(conn, result, ssid, ssid_len, channel_list);
            break;
        }
    }
}
