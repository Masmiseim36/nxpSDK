...
int dragonfly_suitable_group(int group, int ecc_only)
{
    ...
    return group == 19 || group == 20 || group == 21 ||
           (!ecc_only && (group == 15 || group == 16 || group == 17 || group == 18));
}
...
unsigned int dragonfly_min_pwe_loop_iter(int group)
{
    ...
    if (group == 1 || group == 2 || group == 5 || group == 14 || group == 15 || group == 16 || group == 17 ||
        group == 18)
    {
...
static struct crypto_bignum *dragonfly_get_rand_1_to_p_1(const struct crypto_bignum *prime)
{
    ...
    if (!tmp || !pm1 || !one || crypto_bignum_sub(prime, one, pm1) < 0 || crypto_bignum_rand(tmp, pm1) < 0 ||
                                         ...
                                         const struct crypto_bignum *val)
{
    ...
    if (!num || crypto_bignum_mulmod(val, r, prime, num) < 0 || crypto_bignum_mulmod(num, r, prime, num) < 0)
        goto fail;
    ...
    if (!qr_or_qnr || crypto_bignum_mulmod(num, qr_or_qnr, prime, num) < 0)
        goto fail;
...
static int dragonfly_get_rand_2_to_r_1(struct crypto_bignum *val, const struct crypto_bignum *order)
{
    return crypto_bignum_rand(val, order) == 0 && !crypto_bignum_is_zero(val) && !crypto_bignum_is_one(val);
}
                              ...
                              struct crypto_bignum *scalar)
{
    ...
    {
        if (dragonfly_get_rand_2_to_r_1(_rand, order) && dragonfly_get_rand_2_to_r_1(_mask, order) &&
            crypto_bignum_add(_rand, _mask, scalar) == 0 && crypto_bignum_mod(scalar, order, scalar) == 0 &&
            !crypto_bignum_is_zero(scalar) && !crypto_bignum_is_one(scalar))
            return 0;
    }

    /* This should not be reachable in practice if the random number
     * generation is working. */
    wpa_printf(MSG_INFO, "dragonfly: Unable to get randomness for own scalar");
    return -1;
}

/* res = sqrt(val) */
int dragonfly_sqrt(struct crypto_ec *ec, const struct crypto_bignum *val, struct crypto_bignum *res)
{
    const struct crypto_bignum *prime;
    ...
    if (crypto_bignum_to_bin(prime, prime_bin, sizeof(prime_bin), prime_len) < 0 ||
        (prime_bin[prime_len - 1] & 0x03) != 3 || !tmp || !one ||
        /* tmp = (p+1)/4 */
        crypto_bignum_add(prime, one, tmp) < 0 || crypto_bignum_rshift(tmp, 2, tmp) < 0 ||
        /* res = sqrt(val) */
