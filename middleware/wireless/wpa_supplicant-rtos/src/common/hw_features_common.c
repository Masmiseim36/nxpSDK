...
struct hostapd_channel_data *hw_mode_get_channel(struct hostapd_hw_modes *mode, int freq, int *chan)
{
    int i;
...
struct hostapd_channel_data *hw_get_channel_freq(
    enum hostapd_hw_mode mode, int freq, int *chan, struct hostapd_hw_modes *hw_features, int num_hw_features)
{
...
int hw_get_chan(enum hostapd_hw_mode mode, int freq, struct hostapd_hw_modes *hw_features, int num_hw_features)
{
    int chan;
...
{
    int ok, first;
    int allowed[] = {36, 44, 52, 60, 100, 108, 116, 124, 132, 140, 149, 157, 165, 173, 184, 192};
    size_t k;
    ...
    if (pri_chan == sec_chan || !sec_chan)
    {
        ...
        wpa_printf(MSG_ERROR, "Channel %d is not allowed as primary", pri_chan);
        return 0;
    }

    wpa_printf(MSG_DEBUG, "HT40: control channel: %d (%d MHz), secondary channel: %d (%d MHz)", pri_chan, p_chan->freq,
               sec_chan, s_chan->freq);

    /* Verify that HT40 secondary channel is an allowed 20 MHz
     * channel */
    if ((s_chan->flag & HOSTAPD_CHAN_DISABLED) || (ht40_plus && !(p_chan->allowed_bw & HOSTAPD_CHAN_WIDTH_40P)) ||
                   ...
                   struct hostapd_channel_data *sec_chan)
{
    ...
    if (!scan_res || !pri_chan || !sec_chan || pri_chan->freq == sec_chan->freq)
        return 0;
    ...
    for (i = 0; i < scan_res->num; i++)
    {
        ...
        if (pri_chan->chan == bss_pri_chan && sec_chan->chan == bss_sec_chan)
        {
            match = 1;
            break;
        }
    ...
    if (!match)
    {
        for (i = 0; i < scan_res->num; i++)
        {
            struct wpa_scan_res *bss = scan_res->res[i];
            get_pri_sec_chan(bss, &bss_pri_chan, &bss_sec_chan);
            if (pri_chan->chan == bss_sec_chan && sec_chan->chan == bss_pri_chan)
            {
                wpa_printf(MSG_INFO,
...
int check_40mhz_2g4(struct hostapd_hw_modes *mode, struct wpa_scan_results *scan_res, int pri_chan, int sec_chan)
{
    ...
    for (i = 0; i < scan_res->num; i++)
    {
        ...
        wpa_printf(MSG_DEBUG, "Neighboring BSS: " MACSTR " freq=%d pri=%d sec=%d", MAC2STR(bss->bssid), bss->freq,
                   pri_chan, sec_chan);
        ...
        if (sec_chan)
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "40 MHz pri/sec mismatch with BSS " MACSTR " <%d,%d> (chan=%d%c) vs. <%d,%d>",
                           MAC2STR(bss->bssid), pri, sec, pri_chan, sec > pri ? '+' : '-', pri_freq, sec_freq);
                return 0;
            }
        }
        ...
        if (elems.ht_capabilities)
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "40 MHz Intolerant is set on channel %d in BSS " MACSTR, pri,
                           MAC2STR(bss->bssid));
                return 0;
            }
        }
                            ...
                            struct he_capabilities *he_cap)
{
    ...
    if (oper_chwidth == CHANWIDTH_80MHZ)
        data->bandwidth = 80;
    else if (oper_chwidth == CHANWIDTH_160MHZ || oper_chwidth == CHANWIDTH_80P80MHZ)
        data->bandwidth = 160;
    ...
    hostapd_encode_edmg_chan(enable_edmg, edmg_channel, channel, &data->edmg);

    if (is_6ghz_freq(freq))
    {
        if (!data->he_enabled)
        {
            wpa_printf(MSG_ERROR, "Can't set 6 GHz mode - HE isn't enabled");
            return -1;
        }

        if (center_idx_to_bw_6ghz(channel) < 0)
        {
            wpa_printf(MSG_ERROR, "Invalid control channel for 6 GHz band");
            return -1;
        }
        ...
        if (!center_segment0)
        {
            if (center_segment1)
            {
                wpa_printf(MSG_ERROR, "Segment 0 center frequency isn't set");
                return -1;
            }
        ...
        else
        {
            ...
            if (bw < 0)
            {
                wpa_printf(MSG_ERROR, "Invalid center frequency index for 6 GHz");
                return -1;
            }

            freq1 = ieee80211_chan_to_freq(NULL, 131, center_segment0);
            if (freq1 < 0)
            {
                wpa_printf(MSG_ERROR, "Invalid segment 0 center frequency for 6 GHz");
                return -1;
            }

            if (center_segment1)
            {
                if (center_idx_to_bw_6ghz(center_segment1) != 2 || bw != 2)
                {
                    wpa_printf(MSG_ERROR, "6 GHz 80+80 MHz configuration doesn't use valid 80 MHz channels");
                    return -1;
                }

                freq2 = ieee80211_chan_to_freq(NULL, 131, center_segment1);
                if (freq2 < 0)
                {
                    wpa_printf(MSG_ERROR, "Invalid segment 1 center frequency for UHB");
                    return -1;
                }
            }
    ...
    if (data->he_enabled)
        switch (oper_chwidth)
        {
            case CHANWIDTH_USE_HT:
                ...
                if (mode == HOSTAPD_MODE_IEEE80211G)
                {
                    ...
                    {
                        wpa_printf(MSG_ERROR, "40 MHz channel width is not supported in 2.4 GHz");
                        return -1;
                    }
                    break;
                }
            ...
            case CHANWIDTH_80MHZ:
                if (mode == HOSTAPD_MODE_IEEE80211A)
                {
                    ...
                    {
                        wpa_printf(MSG_ERROR, "40/80 MHz channel width is not supported in 5/6 GHz");
                        return -1;
                    }
                }
                break;
            case CHANWIDTH_80P80MHZ:
                if (!(he_cap->phy_cap[HE_PHYCAP_CHANNEL_WIDTH_SET_IDX] & HE_PHYCAP_CHANNEL_WIDTH_SET_80PLUS80MHZ_IN_5G))
                {
                    wpa_printf(MSG_ERROR, "80+80 MHz channel width is not supported in 5/6 GHz");
                    return -1;
                }
                break;
            case CHANWIDTH_160MHZ:
                if (!(he_cap->phy_cap[HE_PHYCAP_CHANNEL_WIDTH_SET_IDX] & HE_PHYCAP_CHANNEL_WIDTH_SET_160MHZ_IN_5G))
                {
        ...
        switch (oper_chwidth)
        {
            ...
            case CHANWIDTH_160MHZ:
                ...
                {
                    wpa_printf(MSG_ERROR, "160 MHz channel width is not supported!");
                    return -1;
                }
                break;
        }

    if (data->he_enabled || data->vht_enabled)
        switch (oper_chwidth)
        {
            case CHANWIDTH_USE_HT:
                if (center_segment1 || (center_segment0 != 0 && 5000 + center_segment0 * 5 != data->center_freq1 &&
                                        2407 + center_segment0 * 5 != data->center_freq1))
                {
                    wpa_printf(MSG_ERROR, "20/40 MHz: center segment 0 (=%d) and center freq 1 (=%d) not in sync",
            ...
            case CHANWIDTH_80MHZ:
                ...
                if (!sec_channel_offset)
                {
                    wpa_printf(MSG_ERROR, "80/80+80 MHz: no second channel offset");
                    return -1;
                }
                if (oper_chwidth == CHANWIDTH_80MHZ && center_segment1)
                {
                    wpa_printf(MSG_ERROR, "80 MHz: center segment 1 configured");
                    return -1;
                }
                if (oper_chwidth == CHANWIDTH_80P80MHZ && !center_segment1)
                {
                    wpa_printf(MSG_ERROR, "80+80 MHz: center segment 1 not configured");
                    return -1;
                }
                if (!center_segment0)
                {
...
void set_disable_ht40(struct ieee80211_ht_capabilities *htcaps, int disabled)
{
...
static int _ieee80211ac_cap_check(u32 hw, u32 conf, u32 cap, const char *name)
{
...
static int ieee80211ac_cap_check_max(u32 hw, u32 conf, u32 mask, unsigned int shift, const char *name)
{
    ...
    if (conf_val > hw_max)
    {
        wpa_printf(MSG_ERROR, "Configured VHT capability [%s] exceeds max value supported by the driver (%lu > %lu)",
...
int chan_bw_allowed(const struct hostapd_channel_data *chan, u32 bw, int ht40_plus, int pri)
{
    u32 bw_mask;
...
{
    return !(chan->flag & HOSTAPD_CHAN_DISABLED) && (chan->allowed_bw & HOSTAPD_CHAN_WIDTH_20);
}
