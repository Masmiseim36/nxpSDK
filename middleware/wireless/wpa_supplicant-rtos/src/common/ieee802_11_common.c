        ...
        case OUI_QCA:
            switch (pos[3])
            {
                case QCA_VENDOR_ELEM_P2P_PREF_CHAN_LIST:
                    ...
                    break;
                default:
                    wpa_printf(MSG_EXCESSIVE, "Unknown QCA information element ignored (type=%d len=%lu)", pos[3],
                               (unsigned long)elen);
                    return -1;
            }
            break;
...
static int ieee802_11_parse_extension(const u8 *pos, size_t elen, struct ieee802_11_elems *elems, int show_errors)
{
    u8 ext_id;
    ...
    if (elen < 1)
    {
        if (show_errors)
        {
            wpa_printf(MSG_MSGDUMP, "short information element (Ext)");
        }
        return -1;
    }
    ...
    switch (ext_id)
    {
        ...
        default:
            ...
            {
                wpa_printf(MSG_MSGDUMP,
                           "IEEE 802.11 element parsing ignored unknown element extension (ext_id=%u elen=%u)", ext_id,
                           (unsigned int)elen);
            }
            return -1;
    }
...
static void ieee802_11_parse_fragment(struct frag_ies_info *frag_ies, const u8 *pos, u8 elen)
{
    ...
    {
        wpa_printf(MSG_MSGDUMP, "Fragment without a valid last element - skip");
        return;
    }
...
ParseRes ieee802_11_parse_elems(const u8 *start, size_t len, struct ieee802_11_elems *elems, int show_errors)
{
    ...
    for_each_element(elem, start, len)
    {
        ...
        switch (id)
        {
            case WLAN_EID_SSID:
                ...
                if (elems->ssid)
                {
                    wpa_printf(MSG_MSGDUMP, "Ignored duplicated SSID element");
                    break;
                }
                elems->ssid     = pos;
                elems->ssid_len = elen;
            ...
            case WLAN_EID_MULTI_BAND:
                ...
                {
                    wpa_printf(MSG_MSGDUMP, "IEEE 802.11 element parse ignored MB IE (id=%d elen=%d)", id, elen);
                    break;
                }
            ...
            case WLAN_EID_EXTENSION:
                if (ieee802_11_parse_extension(pos, elen, elems, show_errors))
                    unknown++;
                break;
            default:
                unknown++;
    ...
    if (!for_each_element_completed(elem, start, len))
    {
        if (show_errors)
        {
            wpa_printf(MSG_DEBUG, "IEEE 802.11 element parse failed @%d", (int)(start + len - (const u8 *)elem));
...
{
    return (cw == 1 || cw == 3 || cw == 7 || cw == 15 || cw == 31 || cw == 63 || cw == 127 || cw == 255 || cw == 511 ||
            cw == 1023 || cw == 2047 || cw == 4095 || cw == 8191 || cw == 16383 || cw == 32767);
}

int hostapd_config_tx_queue(struct hostapd_tx_queue_params tx_queue[], const char *name, const char *val)
{
    int num;
    const char *pos;
    ...
    pos = name + 9;
    if (os_strncmp(pos, "data", 4) == 0 && pos[4] >= '0' && pos[4] <= '9' && pos[5] == '_')
    {
        num = pos[4] - '0';
        pos += 6;
    }
    else if (os_strncmp(pos, "after_beacon_", 13) == 0 || os_strncmp(pos, "beacon_", 7) == 0)
    {
...
enum hostapd_hw_mode ieee80211_freq_to_chan(int freq, u8 *channel)
{
    ...
    return ieee80211_freq_to_channel_ext(freq, 0, CHANWIDTH_USE_HT, &op_class, channel);
}
 ...
 */
enum hostapd_hw_mode ieee80211_freq_to_channel_ext(
    unsigned int freq, int sec_channel, int chanwidth, u8 *op_class, u8 *channel)
{
    u8 vht_opclass;
...
int ieee80211_chaninfo_to_channel(
    unsigned int freq, enum chan_width chanwidth, int sec_channel, u8 *op_class, u8 *channel)
{
    ...
    if (ieee80211_freq_to_channel_ext(freq, sec_channel, cw, op_class, channel) == NUM_HOSTAPD_MODES)
    {
        wpa_printf(MSG_WARNING, "Cannot determine operating class and channel (freq=%u chanwidth=%d sec_channel=%d)",
...
static const char *const eu_op_class_cc[] = {"AL", "AM", "AT", "AZ", "BA", "BE", "BG", "BY", "CH", "CY", "CZ", "DE",
...
int ieee80211_is_dfs(int freq, const struct hostapd_hw_modes *modes, u16 num_modes)
{
    ...
    for (i = 0; i < num_modes; i++)
    {
        ...
        {
            if (modes[i].channels[j].freq == freq && (modes[i].channels[j].flag & HOSTAPD_CHAN_RADAR))
                return 1;
        }
    }
...
int mb_ies_info_by_ies(struct mb_ies_info *info, const u8 *ies_buf, size_t ies_len)
{
    ...
    for_each_element_id(elem, WLAN_EID_MULTI_BAND, ies_buf, ies_len)
    {
        ...
        wpa_printf(MSG_DEBUG, "MB IE of %u bytes found", elem->datalen + 2);
...
const struct oper_class_map global_op_class[] = {{HOSTAPD_MODE_IEEE80211G, 81, 1, 13, 1, BW20, P2P_SUPP},
...
size_t mbo_add_ie(u8 *buf, size_t len, const u8 *attr, size_t attr_len)
{
    ...
    if (len < 6 + attr_len)
    {
        wpa_printf(MSG_DEBUG, "MBO: Not enough room in buffer for MBO IE: buf len = %zu, attr_len = %zu", len,
                   attr_len);
        return 0;
    }
...
static const struct country_op_class us_op_class[] = {
    {1, 115},  {2, 118},  {3, 124},  {4, 121},  {5, 125},  {12, 81},  {22, 116}, {23, 119}, {24, 122}, {25, 126},
    {26, 126}, {27, 117}, {28, 120}, {29, 123}, {30, 127}, {31, 127}, {32, 83},  {33, 84},  {34, 180},
};

static const struct country_op_class eu_op_class[] = {
    {1, 115}, {2, 118}, {3, 121},  {4, 81},  {5, 116}, {6, 119},  {7, 122},
    {8, 117}, {9, 120}, {10, 123}, {11, 83}, {12, 84}, {17, 125}, {18, 180},
};

static const struct country_op_class jp_op_class[] = {
    {1, 115},  {30, 81},  {31, 82},  {32, 118}, {33, 118}, {34, 121}, {35, 121},
    {36, 116}, {37, 119}, {38, 119}, {39, 122}, {40, 122}, {41, 117}, {42, 120},
    {43, 120}, {44, 123}, {45, 123}, {56, 83},  {57, 84},  {58, 121}, {59, 180},
};

static const struct country_op_class cn_op_class[] = {
    {1, 115}, {2, 118}, {3, 125}, {4, 116}, {5, 119}, {6, 126}, {7, 81}, {8, 83}, {9, 84},
};

static u8 global_op_class_from_country_array(u8 op_class,
                                             size_t array_size,
...
u8 country_to_global_op_class(const char *country, u8 op_class)
{
    ...
    g_op_class = global_op_class_from_country_array(op_class, size, country_array);
...
int ieee802_11_parse_candidate_list(const char *pos, u8 *nei_rep, size_t nei_rep_len)
{
    u8 *nei_pos = nei_rep;
    const char *end;
...
bool ieee802_11_rsnx_capab_len(const u8 *rsnxe, size_t rsnxe_len, unsigned int capab)
{
    const u8 *end;
    size_t flen, i;
    u32 capabs = 0;
...
{
    return ieee802_11_rsnx_capab_len(rsnxe ? rsnxe + 2 : NULL, rsnxe ? rsnxe[1] : 0, capab);
}

void hostapd_encode_edmg_chan(int edmg_enable, u8 edmg_channel, int primary_channel, struct ieee80211_edmg_config *edmg)
{
    if (!edmg_enable)
    {
        edmg->channels  = 0;
 ...
 * EDMG configuration. */
int ieee802_edmg_is_allowed(struct ieee80211_edmg_config allowed, struct ieee80211_edmg_config requested)
{
    /*
    ...
    if (((requested.channels & allowed.channels) != requested.channels) ||
        ((requested.bw_config % 4) > (allowed.bw_config % 4)) || requested.bw_config > allowed.bw_config)
        return 0;
...
struct wpabuf *ieee802_11_defrag_data(struct ieee802_11_elems *elems, u8 eid, u8 eid_ext, const u8 *data, u8 len)
{
    ...
    for (i = 0; i < frag_ies->n_frags; i++)
    {
        ...
        if (frag_ies->frags[i].eid != eid || frag_ies->frags[i].eid_ext != eid_ext)
            continue;
        ...
        wpabuf_put_data(buf, frag_ies->frags[i].ie, frag_ies->frags[i].ie_len);
    }

    return buf;
}

struct wpabuf *ieee802_11_defrag(struct ieee802_11_elems *elems, u8 eid, u8 eid_ext)
{
    const u8 *data;
    u8 len;
        ...
        case WLAN_EID_EXTENSION:
            switch (eid_ext)
            {
                ...
                case WLAN_EID_EXT_WRAPPED_DATA:
                    ...
                    break;
                default:
                    wpa_printf(MSG_DEBUG, "Defragmentation not supported. eid_ext=%u", eid_ext);
                    return NULL;
            }
            break;
        default:
            wpa_printf(MSG_DEBUG, "Defragmentation not supported. eid=%u", eid);
            return NULL;
    }
