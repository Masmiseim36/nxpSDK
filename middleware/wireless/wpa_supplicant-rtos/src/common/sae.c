...
int sae_set_group(struct sae_data *sae, int group)
{
    ...
    if (tmp->ec)
    {
        wpa_printf(MSG_DEBUG, "SAE: Selecting supported ECC group %d", group);
        sae->group     = group;
    ...
    if (tmp->dh)
    {
        wpa_printf(MSG_DEBUG, "SAE: Selecting supported FFC group %d", group);
        sae->group     = group;
    ...
    /* Unsupported group */
    wpa_printf(MSG_DEBUG, "SAE: Group %d not supported by the crypto library", group);
    return -1;
}
...
static int sae_test_pwd_seed_ecc(
    struct sae_data *sae, const u8 *pwd_seed, const u8 *prime, const u8 *qr, const u8 *qnr, u8 *pwd_value)
{
    ...
    if (sha256_prf_bits(pwd_seed, SHA256_MAC_LEN, "SAE Hunting and Pecking", prime, sae->tmp->prime_len, pwd_value,
                        bits) < 0)
        return -1;
    if (bits % 8)
...
static int sae_test_pwd_seed_ffc(struct sae_data *sae, const u8 *pwd_seed, struct crypto_bignum *pwe)
{
    ...
    /* pwd-value = KDF-z(pwd-seed, "SAE Hunting and Pecking", p) */
    if (sha256_prf_bits(pwd_seed, SHA256_MAC_LEN, "SAE Hunting and Pecking", sae->tmp->dh->prime, sae->tmp->prime_len,
                        pwd_value, bits) < 0)
        return -1;
    wpa_hexdump_key(MSG_DEBUG, "SAE: pwd-value", pwd_value, sae->tmp->prime_len);

    /* Check whether pwd-value < p */
    res = const_time_memcmp(pwd_value, sae->tmp->dh->prime, sae->tmp->prime_len);
...
static int sae_derive_pwe_ecc(
    struct sae_data *sae, const u8 *addr1, const u8 *addr2, const u8 *password, size_t password_len)
{
    u8 counter, k;
    u8 addrs[2 * ETH_ALEN];
    const u8 *addr[2];
    ...
    if (!stub_password || !tmp_password || random_get_bytes(stub_password, password_len) < 0)
        goto fail;
    ...
    for (counter = 1; counter <= k || !found; counter++)
    {
        ...
        const_time_select_bin(found, stub_password, password, password_len, tmp_password);
        if (hmac_sha256_vector(addrs, sizeof(addrs), 2, addr, len, pwd_seed) < 0)
            break;

        res = sae_test_pwd_seed_ecc(sae, pwd_seed, prime, qr_bin, qnr_bin, x_cand_bin);
        const_time_select_bin(found, x_bin, x_cand_bin, prime_len, x_bin);
        pwd_seed_odd = const_time_select_u8(found, pwd_seed_odd, pwd_seed[SHA256_MAC_LEN - 1] & 0x01);
         ...
         */
        found |= res * 0xff;
        wpa_printf(MSG_DEBUG, "SAE: pwd-seed result %d found=0x%02x", res, found);
    }
    ...
    if (!y || dragonfly_sqrt(sae->tmp->ec, y, y) < 0 ||
        crypto_bignum_to_bin(y, x_y, SAE_MAX_ECC_PRIME_LEN, prime_len) < 0 ||
        crypto_bignum_sub(sae->tmp->prime, y, y) < 0 ||
        crypto_bignum_to_bin(y, x_y + SAE_MAX_ECC_PRIME_LEN, SAE_MAX_ECC_PRIME_LEN, prime_len) < 0)
    {
    ...
    const_time_select_bin(is_eq, x_y, x_y + SAE_MAX_ECC_PRIME_LEN, prime_len, x_y + prime_len);
    ...
    struct sae_data *sae, const u8 *addr1, const u8 *addr2, const u8 *password, size_t password_len)
{
    ...
    for (counter = 1; counter <= k || !found; counter++)
    {
         ...
         * time selection. */
        if (crypto_bignum_to_bin(pwe, pwe_buf + prime_len, prime_len, prime_len) < 0)
            break;
        const_time_select_bin(found, pwe_buf, pwe_buf + prime_len, prime_len, pwe_buf);
...
static int hkdf_extract(
    size_t hash_len, const u8 *salt, size_t salt_len, size_t num_elem, const u8 *addr[], const size_t len[], u8 *prk)
{
    if (hash_len == 32)
        return hmac_sha256_vector(salt, salt_len, num_elem, addr, len, prk);
#ifdef CONFIG_SHA384
    if (hash_len == 48)
        return hmac_sha384_vector(salt, salt_len, num_elem, addr, len, prk);
#endif /* CONFIG_SHA384 */
#ifdef CONFIG_SHA512
    if (hash_len == 64)
        return hmac_sha512_vector(salt, salt_len, num_elem, addr, len, prk);
#endif /* CONFIG_SHA512 */
    return -1;
}

static int hkdf_expand(size_t hash_len, const u8 *prk, size_t prk_len, const char *info, u8 *okm, size_t okm_len)
{
    size_t info_len = os_strlen(info);

    if (hash_len == 32)
        return hmac_sha256_kdf(prk, prk_len, NULL, (const u8 *)info, info_len, okm, okm_len);
#ifdef CONFIG_SHA384
    if (hash_len == 48)
        return hmac_sha384_kdf(prk, prk_len, NULL, (const u8 *)info, info_len, okm, okm_len);
#endif /* CONFIG_SHA384 */
#ifdef CONFIG_SHA512
    if (hash_len == 64)
        return hmac_sha512_kdf(prk, prk_len, NULL, (const u8 *)info, info_len, okm, okm_len);
#endif /* CONFIG_SHA512 */
    return -1;
}
...
static void debug_print_bignum(const char *title, const struct crypto_bignum *a, size_t prime_len)
{
    u8 *bin;
...
static struct crypto_ec_point *sswu(struct crypto_ec *ec, int group, const struct crypto_bignum *u)
{
    int z_int;
    const struct crypto_bignum *a, *b, *prime;
    struct crypto_bignum *u2, *t1, *t2, *z, *t, *zero, *one, *two, *three, *x1a, *x1b, *y = NULL;
    ...
    if (!u2 || !t1 || !t2 || !z || !t || !zero || !one || !two || !three || !x1a || !x1b || !x2 || !gx1 || !gx2)
        goto fail;
     ...
     * t2 = t1^2
     * m = t1 = t1 + t2 */
    if (crypto_bignum_sqrmod(u, prime, u2) < 0 || crypto_bignum_mulmod(z, u2, prime, t1) < 0 ||
        crypto_bignum_sqrmod(t1, prime, t2) < 0 || crypto_bignum_addmod(t1, t2, prime, t1) < 0)
        goto fail;
    ...
    /* t = m^(p-2) modulo p */
    if (crypto_bignum_sub(prime, two, t2) < 0 || crypto_bignum_exptmod(t1, t2, prime, t) < 0)
        goto fail;
    debug_print_bignum("SSWU: t", t, prime_len);

    /* b / (z * a) */
    if (crypto_bignum_mulmod(z, a, prime, t1) < 0 || crypto_bignum_inverse(t1, prime, t1) < 0 ||
        crypto_bignum_mulmod(b, t1, prime, x1a) < 0)
        goto fail;
    debug_print_bignum("SSWU: x1a = b / (z * a)", x1a, prime_len);

    /* (-b/a) * (1 + t) */
    if (crypto_bignum_sub(prime, b, t1) < 0 || crypto_bignum_inverse(a, prime, t2) < 0 ||
        crypto_bignum_mulmod(t1, t2, prime, t1) < 0 || crypto_bignum_addmod(one, t, prime, t2) < 0 ||
    ...
    /* gx1 = x1^3 + a * x1 + b */
    if (crypto_bignum_exptmod(x1, three, prime, t1) < 0 || crypto_bignum_mulmod(a, x1, prime, t2) < 0 ||
        crypto_bignum_addmod(t1, t2, prime, t1) < 0 || crypto_bignum_addmod(t1, b, prime, gx1) < 0)
        goto fail;
    debug_print_bignum("SSWU: gx1 = x1^3 + a * x1 + b", gx1, prime_len);

    /* x2 = z * u^2 * x1 */
    if (crypto_bignum_mulmod(z, u2, prime, t1) < 0 || crypto_bignum_mulmod(t1, x1, prime, x2) < 0)
        goto fail;
    debug_print_bignum("SSWU: x2 = z * u^2 * x1", x2, prime_len);

    /* gx2 = x2^3 + a * x2 + b */
    if (crypto_bignum_exptmod(x2, three, prime, t1) < 0 || crypto_bignum_mulmod(a, x2, prime, t2) < 0 ||
        crypto_bignum_addmod(t1, t2, prime, t1) < 0 || crypto_bignum_addmod(t1, b, prime, gx2) < 0)
        goto fail;
    ...
    if (crypto_bignum_sub(prime, one, t1) < 0 || crypto_bignum_rshift(t1, 1, t1) < 0 ||
        crypto_bignum_exptmod(gx1, t1, prime, t1) < 0)
        goto fail;
    debug_print_bignum("SSWU: gx1^((p-1)/2) modulo p", t1, prime_len);
    is_qr = const_time_eq(crypto_bignum_is_zero(t1) | crypto_bignum_is_one(t1), 1);
    ...
    if (crypto_bignum_to_bin(u, bin1, sizeof(bin1), prime_len) < 0 ||
        ...
        goto fail;
    is_eq = const_time_eq(bin1[prime_len - 1] & 0x01, bin2[prime_len - 1] & 0x01);
...
static int sae_pwd_seed(size_t hash_len,
                        const u8 *ssid,
                        size_t ssid_len,
                        const u8 *password,
                        size_t password_len,
                        ...
                        u8 *pwd_seed)
{
    ...
    if (identifier)
    {
        wpa_printf(MSG_DEBUG, "SAE: password identifier: %s", identifier);
        addr[num_elem] = (const u8 *)identifier;
        len[num_elem]  = os_strlen(identifier);
        num_elem++;
    }
    if (hkdf_extract(hash_len, ssid, ssid_len, num_elem, addr, len, pwd_seed) < 0)
        return -1;
...
static struct crypto_ec_point *sae_derive_pt_ecc(struct crypto_ec *ec,
                                                 int group,
                                                 const u8 *ssid,
                                                 size_t ssid_len,
                                                 const u8 *password,
                                                 ...
                                                 const char *identifier)
{
    ...
    if (sae_pwd_seed(hash_len, ssid, ssid_len, password, password_len, identifier, pwd_seed) < 0)
        goto fail;

    /* pwd-value = HKDF-Expand(pwd-seed, "SAE Hash to Element u1 P1", len)
     */
    if (hkdf_expand(hash_len, pwd_seed, hash_len, "SAE Hash to Element u1 P1", pwd_value, pwd_value_len) < 0)
        goto fail;
    wpa_hexdump_key(MSG_DEBUG, "SAE: pwd-value (u1 P1)", pwd_value, pwd_value_len);

    /* u1 = pwd-value modulo p */
    bn = crypto_bignum_init_set(pwd_value, pwd_value_len);
    if (!bn || crypto_bignum_mod(bn, prime, bn) < 0 ||
        crypto_bignum_to_bin(bn, pwd_value, sizeof(pwd_value), prime_len) < 0)
        goto fail;
     ...
     */
    if (hkdf_expand(hash_len, pwd_seed, hash_len, "SAE Hash to Element u2 P2", pwd_value, pwd_value_len) < 0)
        goto fail;
    wpa_hexdump_key(MSG_DEBUG, "SAE: pwd-value (u2 P2)", pwd_value, pwd_value_len);
...
static struct crypto_bignum *sae_derive_pt_ffc(const struct dh_group *dh,
                                               int group,
                                               const u8 *ssid,
                                               size_t ssid_len,
                                               const u8 *password,
                                               ...
                                               const char *identifier)
{
    ...
    struct crypto_bignum *one = NULL, *two = NULL, *bn = NULL, *tmp = NULL, *pt = NULL;
    u8 pwd_seed[64];
    ...
    if (hkdf_expand(hash_len, pwd_seed, hash_len, "SAE Hash to Element", pwd_value, pwd_value_len) < 0)
        goto fail;
    wpa_hexdump_key(MSG_DEBUG, "SAE: pwd-value", pwd_value, pwd_value_len);
    ...
    if (!bn || !one || !two || !tmp || crypto_bignum_sub(prime, two, tmp) < 0 || crypto_bignum_mod(bn, tmp, bn) < 0 ||
        crypto_bignum_add(bn, two, bn) < 0 || crypto_bignum_to_bin(bn, pwd_value, sizeof(pwd_value), prime_len) < 0)
        goto fail;
    wpa_hexdump_key(MSG_DEBUG, "SAE: pwd-value(reduced)", pwd_value, prime_len);

    /* PT = pwd-value^((p-1)/q) modulo p */
    pt = crypto_bignum_init();
    if (!pt || crypto_bignum_sub(prime, one, tmp) < 0 || crypto_bignum_div(tmp, order, tmp) < 0 ||
...
static struct sae_pt *sae_derive_pt_group(
    int group, const u8 *ssid, size_t ssid_len, const u8 *password, size_t password_len, const char *identifier)
{
    struct sae_pt *pt;
    ...
    if (pt->ec)
    {
        pt->ecc_pt = sae_derive_pt_ecc(pt->ec, group, ssid, ssid_len, password, password_len, identifier);
        if (!pt->ecc_pt)
        {
    ...
    pt->ffc_pt = sae_derive_pt_ffc(pt->dh, group, ssid, ssid_len, password, password_len, identifier);
    if (!pt->ffc_pt)
    {
        wpa_printf(MSG_DEBUG, "SAE: Failed to derive PT");
        goto fail;
...
struct sae_pt *sae_derive_pt(
    int *groups, const u8 *ssid, size_t ssid_len, const u8 *password, size_t password_len, const char *identifier)
{
    ...
    {
        tmp = sae_derive_pt_group(groups[i], ssid, ssid_len, password, password_len, identifier);
        if (!tmp)
            continue;
...
static void sae_max_min_addr(const u8 *addr[], size_t len[], const u8 *addr1, const u8 *addr2)
{
    len[0] = ETH_ALEN;
    len[1] = ETH_ALEN;
    if (os_memcmp(addr1, addr2, ETH_ALEN) > 0)
...
struct crypto_ec_point *sae_derive_pwe_from_pt_ecc(const struct sae_pt *pt, const u8 *addr1, const u8 *addr2)
{
    ...
    if (crypto_ec_point_to_bin(pt->ec, pt->ecc_pt, bin, bin + prime_len) < 0)
        return NULL;
    ...
    one   = crypto_bignum_init_uint(1);
    if (!tmp || !val || !one || crypto_bignum_sub(order, one, tmp) < 0 || crypto_bignum_mod(val, tmp, val) < 0 ||
    ...
    if (!pwe || crypto_ec_point_mul(pt->ec, pt->ecc_pt, val, pwe) < 0 ||
...
struct crypto_bignum *sae_derive_pwe_from_pt_ffc(const struct sae_pt *pt, const u8 *addr1, const u8 *addr2)
{
    size_t prime_len;
    const u8 *addr[2];
    size_t len[2];
...
static int sae_derive_commit_element_ecc(struct sae_data *sae, struct crypto_bignum *mask)
{
    ...
    if (crypto_ec_point_mul(sae->tmp->ec, sae->tmp->pwe_ecc, mask, sae->tmp->own_commit_element_ecc) < 0 ||
...
static int sae_derive_commit_element_ffc(struct sae_data *sae, struct crypto_bignum *mask)
{
    ...
    if (crypto_bignum_exptmod(sae->tmp->pwe_ffc, mask, sae->tmp->prime, sae->tmp->own_commit_element_ffc) < 0 ||
        crypto_bignum_inverse(sae->tmp->own_commit_element_ffc, sae->tmp->prime, sae->tmp->own_commit_element_ffc) < 0)
    {
        wpa_printf(MSG_DEBUG, "SAE: Could not compute commit-element");
        return -1;
    }
...
{
    if (sae->tmp == NULL || (sae->tmp->ec && sae_derive_pwe_ecc(sae, addr1, addr2, password, password_len) < 0) ||
        (sae->tmp->dh && sae_derive_pwe_ffc(sae, addr1, addr2, password, password_len) < 0))
        return -1;
...
int sae_prepare_commit_pt(struct sae_data *sae,
                          ...
                          const struct sae_pk *pk)
{
    ...
    if (!pt)
    {
        wpa_printf(MSG_INFO, "SAE: Could not find PT for group %u", sae->group);
        return -1;
    }
    ...
    {
        crypto_ec_point_deinit(sae->tmp->pwe_ecc, 1);
        sae->tmp->pwe_ecc = sae_derive_pwe_from_pt_ecc(pt, addr1, addr2);
        if (!sae->tmp->pwe_ecc)
            return -1;
    }
    ...
    {
        crypto_bignum_deinit(sae->tmp->pwe_ffc, 1);
        sae->tmp->pwe_ffc = sae_derive_pwe_from_pt_ffc(pt, addr1, addr2);
        if (!sae->tmp->pwe_ffc)
            return -1;
    }
...
static int sae_derive_k_ecc(struct sae_data *sae, u8 *k)
{
    ...
    if (crypto_ec_point_mul(sae->tmp->ec, sae->tmp->pwe_ecc, sae->peer_commit_scalar, K) < 0 ||
        crypto_ec_point_add(sae->tmp->ec, K, sae->tmp->peer_commit_element_ecc, K) < 0 ||
        crypto_ec_point_mul(sae->tmp->ec, K, sae->tmp->sae_rand, K) < 0 ||
        crypto_ec_point_is_at_infinity(sae->tmp->ec, K) || crypto_ec_point_to_bin(sae->tmp->ec, K, k, NULL) < 0)
    {
...
static int sae_derive_k_ffc(struct sae_data *sae, u8 *k)
{
    ...
    if (crypto_bignum_exptmod(sae->tmp->pwe_ffc, sae->peer_commit_scalar, sae->tmp->prime, K) < 0 ||
        crypto_bignum_mulmod(K, sae->tmp->peer_commit_element_ffc, sae->tmp->prime, K) < 0 ||
        crypto_bignum_exptmod(K, sae->tmp->sae_rand, sae->tmp->prime, K) < 0 || crypto_bignum_is_one(K) ||
...
static int sae_kdf_hash(
    size_t hash_len, const u8 *k, const char *label, const u8 *context, size_t context_len, u8 *out, size_t out_len)
{
    if (hash_len == 32)
        return sha256_prf(k, hash_len, label, context, context_len, out, out_len);
#ifdef CONFIG_SHA384
    if (hash_len == 48)
        return sha384_prf(k, hash_len, label, context, context_len, out, out_len);
#endif /* CONFIG_SHA384 */
#ifdef CONFIG_SHA512
    if (hash_len == 64)
        return sha512_prf(k, hash_len, label, context, context_len, out, out_len);
#endif /* CONFIG_SHA512 */
    return -1;
}
...
static int sae_derive_keys(struct sae_data *sae, const u8 *k)
{
    ...
    wpa_hexdump(MSG_DEBUG, "SAE: salt for keyseed derivation", salt, salt_len);
    addr[0] = k;
    len[0]  = prime_len;
    ...
    if (crypto_bignum_add(sae->tmp->own_commit_scalar, sae->peer_commit_scalar, tmp) < 0 ||
    ...
    if (sae->pk)
    {
        ...
        wpa_hexdump_key(MSG_DEBUG, "SAE: KEK for SAE-PK", sae->tmp->kek, sae->tmp->kek_len);
    }
#endif /* CONFIG_SAE_PK */
    forced_memzero(keys, sizeof(keys));
    wpa_hexdump_key(MSG_DEBUG, "SAE: KCK", sae->tmp->kck, sae->tmp->kck_len);
...
int sae_process_commit(struct sae_data *sae)
{
    ...
    if (sae->tmp == NULL || (sae->tmp->ec && sae_derive_k_ecc(sae, k) < 0) ||
        (sae->tmp->dh && sae_derive_k_ffc(sae, k) < 0) || sae_derive_keys(sae, k) < 0)
        return -1;
    return 0;
}

int sae_write_commit(struct sae_data *sae, struct wpabuf *buf, const struct wpabuf *token, const char *identifier)
{
    u8 *pos;
    ...
    if (crypto_bignum_to_bin(sae->tmp->own_commit_scalar, pos, sae->tmp->prime_len, sae->tmp->prime_len) < 0)
        return -1;
    wpa_hexdump(MSG_DEBUG, "SAE: own commit-scalar", pos, sae->tmp->prime_len);
    if (sae->tmp->ec)
    {
        pos = wpabuf_put(buf, 2 * sae->tmp->prime_len);
        if (crypto_ec_point_to_bin(sae->tmp->ec, sae->tmp->own_commit_element_ecc, pos, pos + sae->tmp->prime_len) < 0)
            return -1;
    ...
    {
        pos = wpabuf_put(buf, sae->tmp->prime_len);
        if (crypto_bignum_to_bin(sae->tmp->own_commit_element_ffc, pos, sae->tmp->prime_len, sae->tmp->prime_len) < 0)
            return -1;
    ...
    if (identifier)
    {
        ...
        wpa_printf(MSG_DEBUG, "SAE: own Password Identifier: %s", identifier);
    }

    if (sae->h2e && sae->tmp->own_rejected_groups)
    {
        wpa_hexdump_buf(MSG_DEBUG, "SAE: own Rejected Groups", sae->tmp->own_rejected_groups);
        wpabuf_put_u8(buf, WLAN_EID_EXTENSION);
        wpabuf_put_u8(buf, 1 + wpabuf_len(sae->tmp->own_rejected_groups));
...
{
    return end - pos >= 3 && pos[0] == WLAN_EID_EXTENSION && pos[1] >= 2 && end - pos - 2 >= pos[1] &&
...
static void sae_parse_commit_token(
    struct sae_data *sae, const u8 **pos, const u8 *end, const u8 **token, size_t *token_len, int h2e)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "SAE: Too short optional data (%u octets) to include our Anti-Clogging Token",
                   (unsigned int)tlen);
        return;
    }
...
static void sae_parse_token_container(
    struct sae_data *sae, const u8 *pos, const u8 *end, const u8 **token, size_t *token_len)
{
    ...
    *token     = pos + 3;
    *token_len = pos[1] - 1;
    wpa_hexdump(MSG_DEBUG, "SAE: Anti-Clogging Token (in container)", *token, *token_len);
}
...
static u16 sae_parse_commit_scalar(struct sae_data *sae, const u8 **pos, const u8 *end)
{
    ...
    if (sae->state == SAE_ACCEPTED && sae->peer_commit_scalar_accepted &&
        crypto_bignum_cmp(sae->peer_commit_scalar_accepted, peer_scalar) == 0)
    {
        wpa_printf(MSG_DEBUG,
...
static u16 sae_parse_commit_element_ffc(struct sae_data *sae, const u8 **pos, const u8 *end)
{
    ...
    if (crypto_bignum_exptmod(sae->tmp->peer_commit_element_ffc, sae->tmp->order, sae->tmp->prime, res) < 0 ||
...
static int sae_parse_password_identifier(struct sae_data *sae, const u8 **pos, const u8 *end)
{
    const u8 *epos;
    u8 len;
    ...
    if (!sae_is_password_id_elem(*pos, end))
    {
        if (sae->tmp->pw_id)
        {
            wpa_printf(MSG_DEBUG, "SAE: No Password Identifier included, but expected one (%s)", sae->tmp->pw_id);
    ...
    if (sae->tmp->pw_id && (len != os_strlen(sae->tmp->pw_id) || os_memcmp(sae->tmp->pw_id, epos, len) != 0))
    {
        wpa_printf(MSG_DEBUG, "SAE: The included Password Identifier does not match the expected one (%s)",
                   sae->tmp->pw_id);
        return WLAN_STATUS_UNKNOWN_PASSWORD_IDENTIFIER;
    }
    ...
    sae->tmp->pw_id[len] = '\0';
    wpa_hexdump_ascii(MSG_DEBUG, "SAE: Received Password Identifier", sae->tmp->pw_id, len);
    *pos = epos + len;
    return WLAN_STATUS_SUCCESS;
}

static int sae_parse_rejected_groups(struct sae_data *sae, const u8 **pos, const u8 *end)
{
    const u8 *epos;
    u8 len;
    ...
    wpa_hexdump_buf(MSG_DEBUG, "SAE: Received Rejected Groups list", sae->tmp->peer_rejected_groups);
    *pos = epos + len;
    return WLAN_STATUS_SUCCESS;
}
    ...
    struct sae_data *sae, const u8 *data, size_t len, const u8 **token, size_t *token_len, int *allowed_groups, int h2e)
{
     ...
     */
    if (!sae->tmp->own_commit_scalar || crypto_bignum_cmp(sae->tmp->own_commit_scalar, sae->peer_commit_scalar) != 0 ||
        (sae->tmp->dh &&
         (!sae->tmp->own_commit_element_ffc ||
          crypto_bignum_cmp(sae->tmp->own_commit_element_ffc, sae->tmp->peer_commit_element_ffc) != 0)) ||
        (sae->tmp->ec &&
         (!sae->tmp->own_commit_element_ecc ||
          crypto_ec_point_cmp(sae->tmp->ec, sae->tmp->own_commit_element_ecc, sae->tmp->peer_commit_element_ecc) != 0)))
...
static int sae_cn_confirm(struct sae_data *sae,
                          const u8 *sc,
                          ...
                          u8 *confirm)
{
     ...
     */
    if (crypto_bignum_to_bin(scalar1, scalar_b1, sizeof(scalar_b1), sae->tmp->prime_len) < 0 ||
        crypto_bignum_to_bin(scalar2, scalar_b2, sizeof(scalar_b2), sae->tmp->prime_len) < 0)
        return -1;
    addr[0] = sc;
    len[0]  = 2;
    addr[1] = scalar_b1;
    ...
    addr[4] = element2;
    len[4]  = element2_len;
    return hkdf_extract(sae->tmp->kck_len, sae->tmp->kck, sae->tmp->kck_len, 5, addr, len, confirm);
}

static int sae_cn_confirm_ecc(struct sae_data *sae,
                              const u8 *sc,
                              const struct crypto_bignum *scalar1,
                              ...
                              u8 *confirm)
{
    ...
    if (crypto_ec_point_to_bin(sae->tmp->ec, element1, element_b1, element_b1 + sae->tmp->prime_len) < 0 ||
        crypto_ec_point_to_bin(sae->tmp->ec, element2, element_b2, element_b2 + sae->tmp->prime_len) < 0 ||
        sae_cn_confirm(sae, sc, scalar1, element_b1, 2 * sae->tmp->prime_len, scalar2, element_b2,
                       2 * sae->tmp->prime_len, confirm) < 0)
        return -1;
    return 0;
}

static int sae_cn_confirm_ffc(struct sae_data *sae,
                              const u8 *sc,
                              const struct crypto_bignum *scalar1,
                              ...
                              u8 *confirm)
{
    ...
    if (crypto_bignum_to_bin(element1, element_b1, sizeof(element_b1), sae->tmp->prime_len) < 0 ||
        crypto_bignum_to_bin(element2, element_b2, sizeof(element_b2), sae->tmp->prime_len) < 0 ||
        sae_cn_confirm(sae, sc, scalar1, element_b1, sae->tmp->prime_len, scalar2, element_b2, sae->tmp->prime_len,
                       confirm) < 0)
        return -1;
    return 0;
}
...
int sae_write_confirm(struct sae_data *sae, struct wpabuf *buf)
{
    ...
    if (sae->tmp->ec)
        res = sae_cn_confirm_ecc(sae, sc, sae->tmp->own_commit_scalar, sae->tmp->own_commit_element_ecc,
                                 sae->peer_commit_scalar, sae->tmp->peer_commit_element_ecc, wpabuf_put(buf, hash_len));
    else
        res = sae_cn_confirm_ffc(sae, sc, sae->tmp->own_commit_scalar, sae->tmp->own_commit_element_ffc,
                                 sae->peer_commit_scalar, sae->tmp->peer_commit_element_ffc, wpabuf_put(buf, hash_len));
    if (res)
        return res;
...
int sae_check_confirm(struct sae_data *sae, const u8 *data, size_t len)
{
    ...
    if (sae->tmp->ec)
    {
        if (!sae->tmp->peer_commit_element_ecc || !sae->tmp->own_commit_element_ecc ||
            sae_cn_confirm_ecc(sae, data, sae->peer_commit_scalar, sae->tmp->peer_commit_element_ecc,
                               sae->tmp->own_commit_scalar, sae->tmp->own_commit_element_ecc, verifier) < 0)
            return -1;
    }
    else
    {
        if (!sae->tmp->peer_commit_element_ffc || !sae->tmp->own_commit_element_ffc ||
            sae_cn_confirm_ffc(sae, data, sae->peer_commit_scalar, sae->tmp->peer_commit_element_ffc,
                               sae->tmp->own_commit_scalar, sae->tmp->own_commit_element_ffc, verifier) < 0)
            return -1;
    }
    ...
    if (os_memcmp_const(verifier, data + 2, hash_len) != 0)
    {
        ...
        wpa_hexdump(MSG_DEBUG, "SAE: Received confirm", data + 2, hash_len);
        wpa_hexdump(MSG_DEBUG, "SAE: Calculated verifier", verifier, hash_len);
        return -1;
    }

#ifdef CONFIG_SAE_PK
    if (sae_check_confirm_pk(sae, data + 2 + hash_len, len - 2 - hash_len) < 0)
        return -1;
#endif /* CONFIG_SAE_PK */
