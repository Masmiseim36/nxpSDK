...
{
    return akmp == WPA_KEY_MGMT_OSEN || akmp == WPA_KEY_MGMT_OWE || akmp == WPA_KEY_MGMT_DPP ||
           akmp == WPA_KEY_MGMT_FT_IEEE8021X_SHA384 || wpa_key_mgmt_sae(akmp) || wpa_key_mgmt_suite_b(akmp) ||
...
{
    return akmp == WPA_KEY_MGMT_OSEN || akmp == WPA_KEY_MGMT_OWE || akmp == WPA_KEY_MGMT_DPP || wpa_key_mgmt_ft(akmp) ||
           wpa_key_mgmt_sha256(akmp) || wpa_key_mgmt_sae(akmp) || wpa_key_mgmt_suite_b(akmp);
}
...
int wpa_eapol_key_mic(const u8 *key, size_t key_len, int akmp, int ver, const u8 *buf, size_t len, u8 *mic)
{
    ...
    if (key_len == 0)
    {
        wpa_printf(MSG_DEBUG, "WPA: KCK not set - cannot calculate MIC");
        return -1;
    }
        ...
        case WPA_KEY_INFO_TYPE_AKM_DEFINED:
            switch (akmp)
            {
                ...
                case WPA_KEY_MGMT_FT_SAE:
                    wpa_printf(MSG_DEBUG, "WPA: EAPOL-Key MIC using AES-CMAC (AKM-defined - SAE)");
                    return omac1_aes_128(key, buf, len, mic);
                case WPA_KEY_MGMT_SAE_EXT_KEY:
                    wpa_printf(MSG_DEBUG, "WPA: EAPOL-Key MIC using HMAC-SHA%u (AKM-defined - SAE-EXT-KEY)",
                    ...
                    {
                        wpa_printf(MSG_INFO, "SAE: Unsupported KCK length: %u",
                ...
                case WPA_KEY_MGMT_OSEN:
                    wpa_printf(MSG_DEBUG, "WPA: EAPOL-Key MIC using AES-CMAC (AKM-defined - OSEN)");
                    return omac1_aes_128(key, buf, len, mic);
#endif /* CONFIG_HS20 */
#ifdef CONFIG_SUITEB
                case WPA_KEY_MGMT_IEEE8021X_SUITE_B:
                    wpa_printf(MSG_DEBUG, "WPA: EAPOL-Key MIC using HMAC-SHA256 (AKM-defined - Suite B)");
                    if (hmac_sha256(key, key_len, buf, len, hash))
                        return -1;
                    os_memcpy(mic, hash, MD5_MAC_LEN);
                    break;
                ...
                case WPA_KEY_MGMT_IEEE8021X_SUITE_B_192:
                    wpa_printf(MSG_DEBUG, "WPA: EAPOL-Key MIC using HMAC-SHA384 (AKM-defined - Suite B 192-bit)");
                    if (hmac_sha384(key, key_len, buf, len, hash))
                        return -1;
...
#ifdef CONFIG_OWE
                case WPA_KEY_MGMT_OWE:
                    wpa_printf(MSG_DEBUG, "WPA: EAPOL-Key MIC using HMAC-SHA%u (AKM-defined - OWE)",
                               (unsigned int)key_len * 8 * 2);
                    if (key_len == 128 / 8)
                    {
                        if (hmac_sha256(key, key_len, buf, len, hash))
                    ...
                    {
                        wpa_printf(MSG_INFO, "OWE: Unsupported KCK length: %u", (unsigned int)key_len);
                        return -1;
                    }
                    os_memcpy(mic, hash, key_len);
                    break;
...
#ifdef CONFIG_DPP
                case WPA_KEY_MGMT_DPP:
                    wpa_printf(MSG_DEBUG, "WPA: EAPOL-Key MIC using HMAC-SHA%u (AKM-defined - DPP)",
                               (unsigned int)key_len * 8 * 2);
                    if (key_len == 128 / 8)
                    {
                        if (hmac_sha256(key, key_len, buf, len, hash))
                    ...
                    {
                        wpa_printf(MSG_INFO, "DPP: Unsupported KCK length: %u", (unsigned int)key_len);
                        return -1;
                    }
                    os_memcpy(mic, hash, key_len);
                    break;
                ...
                case WPA_KEY_MGMT_FT_IEEE8021X_SHA384:
                    wpa_printf(MSG_DEBUG, "WPA: EAPOL-Key MIC using HMAC-SHA384 (AKM-defined - FT 802.1X SHA384)");
                    if (hmac_sha384(key, key_len, buf, len, hash))
                        return -1;
                    os_memcpy(mic, hash, 24);
                    break;
                ...
                default:
                    wpa_printf(MSG_DEBUG, "WPA: EAPOL-Key MIC algorithm not known (AKM-defined - akmp=0x%x)", akmp);
                    return -1;
            }
            break;
        default:
            wpa_printf(MSG_DEBUG, "WPA: EAPOL-Key MIC algorithm not known (ver=%d)", ver);
            return -1;
    }
...
#define MAX_Z_LEN 66 /* with NIST P-521 */
    ...
    {
        wpa_printf(MSG_ERROR, "WPA: KDK len=%zu exceeds max supported len", kdk_len);
        return -1;
    }
    ...
    if (ptk->tk_len == 0)
    {
        wpa_printf(MSG_ERROR, "WPA: Unsupported cipher (0x%x) used in PTK derivation", cipher);
        return -1;
    }
    ...
    if (wpa_key_mgmt_sha384(akmp))
    {
        ...
        if (sha384_prf(pmk, pmk_len, label, data, data_len, tmp, ptk_len) < 0)
            return -1;
    ...
    else if (wpa_key_mgmt_sha256(akmp))
    {
        ...
        if (sha256_prf(pmk, pmk_len, label, data, data_len, tmp, ptk_len) < 0)
            return -1;
#ifdef CONFIG_OWE
    }
    else if (akmp == WPA_KEY_MGMT_OWE && (pmk_len == 32 || owe_ptk_workaround))
    {
        wpa_printf(MSG_DEBUG, "WPA: PTK derivation using PRF(SHA256)");
        if (sha256_prf(pmk, pmk_len, label, data, data_len, tmp, ptk_len) < 0)
            return -1;
    ...
    else if (akmp == WPA_KEY_MGMT_OWE && pmk_len == 64)
    {
        ...
        if (sha512_prf(pmk, pmk_len, label, data, data_len, tmp, ptk_len) < 0)
            return -1;
    }
    else if (akmp == WPA_KEY_MGMT_OWE)
    {
        wpa_printf(MSG_INFO, "OWE: Unknown PMK length %u", (unsigned int)pmk_len);
        return -1;
#endif /* CONFIG_OWE */
#ifdef CONFIG_DPP
    }
    ...
    {
        wpa_printf(MSG_INFO, "DPP: Unknown PMK length %u", (unsigned int)pmk_len);
        return -1;
#endif /* CONFIG_DPP */
    }
#ifdef CONFIG_SAE
    else if (wpa_key_mgmt_sae_ext_key(akmp))
    {
        if (pmk_len == 32)
        {
            wpa_printf(MSG_DEBUG, "SAE: PTK derivation using PRF(SHA256)");
            if (sha256_prf(pmk, pmk_len, label, data, data_len, tmp, ptk_len) < 0)
                return -1;
        }
#ifdef CONFIG_SHA384
        ...
        {
            wpa_printf(MSG_DEBUG, "SAE: PTK derivation using PRF(SHA384)");
            if (sha384_prf(pmk, pmk_len, label, data, data_len, tmp, ptk_len) < 0)
                return -1;
        }
#endif /* CONFIG_SHA384 */
        ...
        {
            wpa_printf(MSG_DEBUG, "SAE: PTK derivation using PRF(SHA512)");
            if (sha512_prf(pmk, pmk_len, label, data, data_len, tmp, ptk_len) < 0)
                return -1;
        }
#endif /* CONFIG_SHA512 */
        ...
        {
            wpa_printf(MSG_INFO, "SAE: Unknown PMK length %u", (unsigned int) pmk_len);
            return -1;
        }
    }
#endif /* CONFIG_SAE */
    else
    {
        ...
        if (sha1_prf(pmk, pmk_len, label, data, data_len, tmp, ptk_len) < 0)
            return -1;
    }
    ...
    if (kdk_len)
    {
        os_memcpy(ptk->kdk, tmp + ptk->kck_len + ptk->kek_len + ptk->tk_len, ptk->kdk_len);
                     ...
                     size_t *pmk_len)
{
    ...
    if (wpa_key_mgmt_sha384(akmp))
        res = hmac_sha384_vector(nonces, 2 * FILS_NONCE_LEN, num_elem, addr, len, pmk);
    else
        res = hmac_sha256_vector(nonces, 2 * FILS_NONCE_LEN, num_elem, addr, len, pmk);
    if (res == 0)
...
int fils_pmkid_erp(int akmp, const u8 *reauth, size_t reauth_len, u8 *pmkid)
{
    const u8 *addr[1];
    size_t len[1];
...
int fils_pmk_to_ptk(const u8 *pmk,
                    size_t pmk_len,
                    const u8 *spa,
                    const u8 *aa,
                    const u8 *snonce,
                    ...
                    size_t kdk_len)
{
    u8 *data, *pos;
    size_t data_len;
    u8 tmp[FILS_ICK_MAX_LEN + WPA_KEK_MAX_LEN + WPA_TK_MAX_LEN + FILS_FT_MAX_LEN + WPA_KDK_MAX_LEN];
    size_t key_data_len;
    ...
    if (kdk_len)
    {
        if (kdk_len > WPA_KDK_MAX_LEN)
        {
            wpa_printf(MSG_ERROR, "FILS: KDK len=%zu too big", kdk_len);
            goto err;
        }
    ...
    if (wpa_key_mgmt_sha384(akmp))
    {
        ...
        if (sha384_prf(pmk, pmk_len, label, data, data_len, tmp, key_data_len) < 0)
            goto err;
    }
    else
    {
        wpa_printf(MSG_DEBUG, "FILS: PTK derivation using PRF(SHA256)");
        if (sha256_prf(pmk, pmk_len, label, data, data_len, tmp, key_data_len) < 0)
            goto err;
    }

    wpa_printf(MSG_DEBUG, "FILS: PTK derivation - SPA=" MACSTR " AA=" MACSTR, MAC2STR(spa), MAC2STR(aa));
    wpa_hexdump(MSG_DEBUG, "FILS: SNonce", snonce, FILS_NONCE_LEN);
    wpa_hexdump(MSG_DEBUG, "FILS: ANonce", anonce, FILS_NONCE_LEN);
    if (dhss)
    ...
    if (fils_ft && fils_ft_len)
    {
        ...
        wpa_hexdump_key(MSG_DEBUG, "FILS: FILS-FT", fils_ft, *fils_ft_len);
        offset += *fils_ft_len;
    }
...
int fils_key_auth_sk(const u8 *ick,
                     size_t ick_len,
                     const u8 *snonce,
                     const u8 *anonce,
                     const u8 *sta_addr,
                     ...
                     size_t *key_auth_len)
{
    ...
    wpa_printf(MSG_DEBUG, "FILS: Key-Auth derivation: STA-MAC=" MACSTR " AP-BSSID=" MACSTR, MAC2STR(sta_addr),
               MAC2STR(bssid));
    wpa_hexdump_key(MSG_DEBUG, "FILS: ICK", ick, ick_len);
    wpa_hexdump(MSG_DEBUG, "FILS: SNonce", snonce, FILS_NONCE_LEN);
    wpa_hexdump(MSG_DEBUG, "FILS: ANonce", anonce, FILS_NONCE_LEN);
    ...
    {
        *key_auth_len = 48;
        res           = hmac_sha384_vector(ick, ick_len, num_elem, addr, len, key_auth_sta);
    }
    else if (wpa_key_mgmt_sha256(akmp))
    {
        *key_auth_len = 32;
        res           = hmac_sha256_vector(ick, ick_len, num_elem, addr, len, key_auth_sta);
    }
    else
    {
        return -1;
    ...
    if (wpa_key_mgmt_sha384(akmp))
        res = hmac_sha384_vector(ick, ick_len, num_elem, addr, len, key_auth_ap);
    else if (wpa_key_mgmt_sha256(akmp))
        res = hmac_sha256_vector(ick, ick_len, num_elem, addr, len, key_auth_ap);
    if (res < 0)
        return res;

    wpa_hexdump(MSG_DEBUG, "FILS: Key-Auth (STA)", key_auth_sta, *key_auth_len);
    wpa_hexdump(MSG_DEBUG, "FILS: Key-Auth (AP)", key_auth_ap, *key_auth_len);
...
#endif /* CONFIG_SHA384 */
    if (kck_len == 16 && omac1_aes_128_vector(kck, num_elem, addr, len, mic))
        return -1;

    return 0;
}

static int wpa_ft_parse_ftie(const u8 *ie, size_t ie_len, struct wpa_ft_ies *parse, int use_sha384)
{
    const u8 *end, *pos;
    ...
    pos = ie + (use_sha384 ? sizeof(struct rsn_ftie_sha384) : sizeof(struct rsn_ftie));
    end = ie + ie_len;
            ...
            case WLAN_EID_FAST_BSS_TRANSITION:
                ...
                if (use_sha384)
                {
                    ...
                    if (len < sizeof(*ftie_sha384))
                        return -1;
                    ftie_sha384 = (const struct rsn_ftie_sha384 *)pos;
                    wpa_hexdump(MSG_DEBUG, "FT: FTE-MIC Control", ftie_sha384->mic_control, 2);
                    wpa_hexdump(MSG_DEBUG, "FT: FTE-MIC", ftie_sha384->mic, sizeof(ftie_sha384->mic));
                    parse->fte_anonce = ftie_sha384->anonce;
                    wpa_hexdump(MSG_DEBUG, "FT: FTE-ANonce", ftie_sha384->anonce, WPA_NONCE_LEN);
                    parse->fte_snonce = ftie_sha384->snonce;
                    wpa_hexdump(MSG_DEBUG, "FT: FTE-SNonce", ftie_sha384->snonce, WPA_NONCE_LEN);
                ...
                wpa_hexdump(MSG_DEBUG, "FT: FTE-MIC Control", ftie->mic_control, 2);
                wpa_hexdump(MSG_DEBUG, "FT: FTE-MIC", ftie->mic, sizeof(ftie->mic));
                parse->fte_anonce = ftie->anonce;
                wpa_hexdump(MSG_DEBUG, "FT: FTE-ANonce", ftie->anonce, WPA_NONCE_LEN);
                parse->fte_snonce = ftie->snonce;
                wpa_hexdump(MSG_DEBUG, "FT: FTE-SNonce", ftie->snonce, WPA_NONCE_LEN);
                prot_ie_count = ftie->mic_control[1];
                if (wpa_ft_parse_ftie(pos, len, parse, 0) < 0)
                    return -1;
                break;
            case WLAN_EID_TIMEOUT_INTERVAL:
                wpa_hexdump(MSG_DEBUG, "FT: Timeout Interval", pos, len);
                if (len != 5)
                    break;
                parse->tie     = pos;
                parse->tie_len = len;
...
{
    return (akmp == WPA_KEY_MGMT_PASN && (cipher == WPA_CIPHER_CCMP_256 || cipher == WPA_CIPHER_GCMP_256)) ||
 ...
 * Returns: 0 on success, -1 on failure
 */
int pasn_pmk_to_ptk(const u8 *pmk,
                    size_t pmk_len,
                    const u8 *spa,
                    const u8 *bssid,
                    const u8 *dhss,
                    ...
                    size_t kdk_len)
{
    ...
    if (ptk->tk_len == 0)
    {
        wpa_printf(MSG_ERROR, "PASN: Unsupported cipher (0x%x) used in PTK derivation", cipher);
        goto err;
    }
    ...
    wpa_printf(MSG_DEBUG, "PASN: PTK derivation: SPA=" MACSTR " BSSID=" MACSTR, MAC2STR(spa), MAC2STR(bssid));
    ...
    if (kdk_len)
    {
        os_memcpy(ptk->kdk, tmp + WPA_PASN_KCK_LEN + ptk->tk_len, ptk->kdk_len);
        wpa_hexdump_key(MSG_DEBUG, "PASN: KDK:", ptk->kdk, ptk->kdk_len);
    }
 ...
 * Returns: 0 on success, -1 on failure
 */
int pasn_auth_frame_hash(int akmp, int cipher, const u8 *data, size_t len, u8 *hash)
{
    if (pasn_use_sha384(akmp, cipher))
    {
...
{
    return cipher == WPA_CIPHER_GTK_NOT_USED || cipher == WPA_CIPHER_AES_128_CMAC ||
           cipher == WPA_CIPHER_BIP_GMAC_128 || cipher == WPA_CIPHER_BIP_GMAC_256 || cipher == WPA_CIPHER_BIP_CMAC_256;
}
...
int wpa_parse_wpa_ie_rsn(const u8 *rsn_ie, size_t rsn_ie_len, struct wpa_ie_data *data)
{
    ...
    if (left >= RSN_SELECTOR_LEN)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "%s: invalid group cipher 0x%x (%08x)", __func__, data->group_cipher,
                       WPA_GET_BE32(pos));
            return -1;
        }
    ...
    if (left >= 4)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "%s: Unsupported management group cipher 0x%x (%08x)", __func__,
                       data->mgmt_group_cipher, WPA_GET_BE32(pos));
            return -10;
        }
        pos += RSN_SELECTOR_LEN;
        left -= RSN_SELECTOR_LEN;
...
int wpa_parse_wpa_ie_wpa(const u8 *wpa_ie, size_t wpa_ie_len, struct wpa_ie_data *data)
{
    ...
    if (hdr->elem_id != WLAN_EID_VENDOR_SPECIFIC || hdr->len != wpa_ie_len - 2 ||
        RSN_SELECTOR_GET(hdr->oui) != WPA_OUI_TYPE || WPA_GET_LE16(hdr->version) != WPA_VERSION)
    {
        wpa_printf(MSG_DEBUG, "%s: malformed ie or unknown version", __func__);
        return -2;
    }
 ...
 */
int wpa_derive_pmk_r0(const u8 *xxkey,
                      size_t xxkey_len,
                      const u8 *ssid,
                      size_t ssid_len,
                      const u8 *mdid,
                      ...
                      int use_sha384)
{
    ...
    if (ssid_len > SSID_MAX_LEN || r0kh_id_len > FT_R0KH_ID_MAX_LEN)
        return -1;
    wpa_printf(MSG_DEBUG, "FT: Derive PMK-R0 using KDF-%s", use_sha384 ? "SHA384" : "SHA256");
    ...
    *pos++ = r0kh_id_len;
    ...
    if (use_sha384)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "FT: Unexpected XXKey length %d (expected %d)", (int)xxkey_len, SHA384_MAC_LEN);
            return -1;
        }
        if (sha384_prf(xxkey, xxkey_len, "FT-R0", buf, pos - buf, r0_key_data, r0_key_data_len) < 0)
            return -1;
    }
#endif /* CONFIG_SHA384 */
    if (!use_sha384)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "FT: Unexpected XXKey length %d (expected %d)", (int)xxkey_len, PMK_LEN);
            return -1;
        }
        if (sha256_prf(xxkey, xxkey_len, "FT-R0", buf, pos - buf, r0_key_data, r0_key_data_len) < 0)
            return -1;
    }
 ...
 */
int wpa_derive_pmk_r1_name(const u8 *pmk_r0_name, const u8 *r1kh_id, const u8 *s1kh_id, u8 *pmk_r1_name, int use_sha384)
{
    u8 hash[48];
    const u8 *addr[4];
    size_t len[4];
 ...
 * IEEE Std 802.11r-2008 - 8.5.1.5.4
 */
int wpa_derive_pmk_r1(const u8 *pmk_r0,
                      size_t pmk_r0_len,
                      ...
                      u8 *pmk_r1_name)
{
    ...
    wpa_printf(MSG_DEBUG, "FT: Derive PMK-R1 using KDF-%s", pmk_r0_len == SHA384_MAC_LEN ? "SHA384" : "SHA256");
...
#ifdef CONFIG_SHA384
    if (pmk_r0_len == SHA384_MAC_LEN && sha384_prf(pmk_r0, pmk_r0_len, "FT-R1", buf, pos - buf, pmk_r1, pmk_r0_len) < 0)
        return -1;
#endif /* CONFIG_SHA384 */
    if (pmk_r0_len == PMK_LEN && sha256_prf(pmk_r0, pmk_r0_len, "FT-R1", buf, pos - buf, pmk_r1, pmk_r0_len) < 0)
        return -1;
    if (pmk_r0_len != SHA384_MAC_LEN && pmk_r0_len != PMK_LEN)
    {
        wpa_printf(MSG_ERROR, "FT: Unexpected PMK-R0 length %d", (int)pmk_r0_len);
        return -1;
    }
    wpa_hexdump_key(MSG_DEBUG, "FT: PMK-R1", pmk_r1, pmk_r0_len);

    return wpa_derive_pmk_r1_name(pmk_r0_name, r1kh_id, s1kh_id, pmk_r1_name, pmk_r0_len == SHA384_MAC_LEN);
}
 ...
 */
int wpa_pmk_r1_to_ptk(const u8 *pmk_r1,
                      size_t pmk_r1_len,
                      const u8 *snonce,
                      const u8 *anonce,
                      const u8 *sta_addr,
                      ...
                      size_t kdk_len)
{
    ...
    const u8 *addr[6];
    size_t len[6];
    u8 tmp[2 * WPA_KCK_MAX_LEN + 2 * WPA_KEK_MAX_LEN + WPA_TK_MAX_LEN + WPA_KDK_MAX_LEN];
    size_t ptk_len, offset;
    int use_sha384 = wpa_key_mgmt_sha384(akmp);

    if (kdk_len > WPA_KDK_MAX_LEN)
    {
        wpa_printf(MSG_ERROR, "FT: KDK len=%zu exceeds max supported len", kdk_len);
        return -1;
    }
     ...
     *                  BSSID || STA-ADDR)
     */
    wpa_printf(MSG_DEBUG, "FT: Derive PTK using KDF-%s", use_sha384 ? "SHA384" : "SHA256");
    ...
    wpa_printf(MSG_DEBUG, "FT: BSSID=" MACSTR " STA-ADDR=" MACSTR, MAC2STR(bssid), MAC2STR(sta_addr));
    pos = buf;
    ...
    ptk->tk_len   = wpa_cipher_key_len(cipher);
    ptk->kdk_len  = kdk_len;
    ptk_len       = ptk->kck_len + ptk->kek_len + ptk->tk_len + ptk->kck2_len + ptk->kek2_len + ptk->kdk_len;
    ...
    if (use_sha384)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "FT: Unexpected PMK-R1 length %d (expected %d)", (int)pmk_r1_len, SHA384_MAC_LEN);
            return -1;
        }
        if (sha384_prf(pmk_r1, pmk_r1_len, "FT-PTK", buf, pos - buf, tmp, ptk_len) < 0)
            return -1;
    }
#endif /* CONFIG_SHA384 */
    if (!use_sha384)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "FT: Unexpected PMK-R1 length %d (expected %d)", (int)pmk_r1_len, PMK_LEN);
            return -1;
        }
        if (sha256_prf(pmk_r1, pmk_r1_len, "FT-PTK", buf, pos - buf, tmp, ptk_len) < 0)
            return -1;
    }
    ...
    if (ptk->kck2_len)
        wpa_hexdump_key(MSG_DEBUG, "FT: KCK2", ptk->kck2, ptk->kck2_len);
    if (ptk->kek2_len)
        wpa_hexdump_key(MSG_DEBUG, "FT: KEK2", ptk->kek2, ptk->kek2_len);
    if (ptk->kdk_len)
 ...
 */
void rsn_pmkid(const u8 *pmk, size_t pmk_len, const u8 *aa, const u8 *spa, u8 *pmkid, int akmp)
{
    char *title = "PMK Name";
    const u8 *addr[3];
...
int wpa_compare_rsn_ie(int ft_initial_assoc, const u8 *ie1, size_t ie1len, const u8 *ie2, size_t ie2len)
{
    ...
    if (ft_initial_assoc)
    {
        ...
        if (wpa_parse_wpa_ie_rsn(ie1, ie1len, &ie1d) < 0 || wpa_parse_wpa_ie_rsn(ie2, ie2len, &ie2d) < 0)
            return -1;
        if (ie1d.proto == ie2d.proto && ie1d.pairwise_cipher == ie2d.pairwise_cipher &&
            ie1d.group_cipher == ie2d.group_cipher && ie1d.key_mgmt == ie2d.key_mgmt &&
            ie1d.capabilities == ie2d.capabilities && ie1d.mgmt_group_cipher == ie2d.mgmt_group_cipher)
            return 0;
    }
#endif /* CONFIG_IEEE80211R */
...
int wpa_insert_pmkid(u8 *ies, size_t *ies_len, const u8 *pmkid)
{
    ...
    wpa_hexdump(MSG_DEBUG, "RSN: RSNE before modification", start, 2 + start[1]);
    ...
    else
    {
        ...
        if (rpos > rend)
        {
            wpa_printf(MSG_ERROR, "RSN: Could not parse RSNE in IEs data");
            return -1;
        }
    }
    ...
    else
    {
        ...
        if (num_pmkid != 0)
        {
             ...
             */
            wpa_printf(MSG_DEBUG, "RSN: Remove %u old PMKID(s) from RSNE", num_pmkid);
    ...
    wpa_hexdump(MSG_DEBUG, "RSN: RSNE after modification (PMKID inserted)", start, 2 + start[1]);
...
#else  /* CONFIG_NO_TKIP */
    return cipher == WPA_CIPHER_CCMP_256 || cipher == WPA_CIPHER_GCMP_256 || cipher == WPA_CIPHER_CCMP ||
           cipher == WPA_CIPHER_GCMP || cipher == WPA_CIPHER_TKIP;
...
int wpa_write_ciphers(char *start, char *end, int ciphers, const char *delim)
{
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sAES-128-CMAC", pos == start ? "" : delim);
        if (os_snprintf_error(end - pos, ret))
            return -1;
        pos += ret;
    }
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sBIP-GMAC-128", pos == start ? "" : delim);
        if (os_snprintf_error(end - pos, ret))
            return -1;
        pos += ret;
    }
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sBIP-GMAC-256", pos == start ? "" : delim);
        if (os_snprintf_error(end - pos, ret))
            return -1;
        pos += ret;
    }
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sBIP-CMAC-256", pos == start ? "" : delim);
        if (os_snprintf_error(end - pos, ret))
            return -1;
        pos += ret;
    }
 ...
 */
static void wpa_parse_vendor_specific(const u8 *pos, const u8 *end, struct wpa_eapol_ie_parse *ie)
{
    unsigned int oui;
...
static int wpa_parse_generic(const u8 *pos, struct wpa_eapol_ie_parse *ie)
{
    ...
    if (pos[1] >= 6 && RSN_SELECTOR_GET(pos + 2) == WPA_OUI_TYPE && pos[2 + WPA_SELECTOR_LEN] == 1 &&
    ...
    if (pos[1] >= RSN_SELECTOR_LEN + PMKID_LEN && RSN_SELECTOR_GET(pos + 2) == RSN_KEY_DATA_PMKID)
    {
    ...
    if (pos[1] >= RSN_SELECTOR_LEN + 2 && RSN_SELECTOR_GET(pos + 2) == RSN_KEY_DATA_KEYID)
    {
        ...
        wpa_hexdump(MSG_DEBUG, "WPA: KeyID in EAPOL-Key", pos, pos[1] + 2);
        return 0;
    }
    ...
    if (pos[1] > RSN_SELECTOR_LEN + 2 && RSN_SELECTOR_GET(pos + 2) == RSN_KEY_DATA_BIGTK)
    {
        ie->bigtk     = pos + 2 + RSN_SELECTOR_LEN;
        ie->bigtk_len = pos[1] - RSN_SELECTOR_LEN;
        wpa_hexdump_key(MSG_DEBUG, "WPA: BIGTK in EAPOL-Key", pos, pos[1] + 2);
        return 0;
    }
    ...
    if (pos[1] > RSN_SELECTOR_LEN + 2 && RSN_SELECTOR_GET(pos + 2) == RSN_KEY_DATA_OCI)
    {
        ie->oci     = pos + 2 + RSN_SELECTOR_LEN;
        ie->oci_len = pos[1] - RSN_SELECTOR_LEN;
        wpa_hexdump(MSG_DEBUG, "WPA: OCI KDE in EAPOL-Key", pos, pos[1] + 2);
        return 0;
    }

    if (pos[1] >= RSN_SELECTOR_LEN + 1 && RSN_SELECTOR_GET(pos + 2) == WFA_KEY_DATA_TRANSITION_DISABLE)
    {
        ie->transition_disable     = pos + 2 + RSN_SELECTOR_LEN;
        ie->transition_disable_len = pos[1] - RSN_SELECTOR_LEN;
        wpa_hexdump(MSG_DEBUG, "WPA: Transition Disable KDE in EAPOL-Key", pos, pos[1] + 2);
        return 0;
    }

    if (pos[1] >= RSN_SELECTOR_LEN + 2 && RSN_SELECTOR_GET(pos + 2) == WFA_KEY_DATA_DPP)
    {
        ie->dpp_kde     = pos + 2 + RSN_SELECTOR_LEN;
        ie->dpp_kde_len = pos[1] - RSN_SELECTOR_LEN;
        wpa_hexdump(MSG_DEBUG, "WPA: DPP KDE in EAPOL-Key", pos, pos[1] + 2);
        return 0;
    }
...
int wpa_parse_kde_ies(const u8 *buf, size_t len, struct wpa_eapol_ie_parse *ie)
{
    ...
    for (pos = buf, end = pos + len; end - pos > 1; pos += 2 + pos[1])
    {
        ...
        if (2 + pos[1] > end - pos)
        {
            wpa_printf(MSG_DEBUG, "WPA: EAPOL-Key Key Data underflow (ie=%d len=%d pos=%d)", pos[0], pos[1],
                       (int)(pos - buf));
        ...
        else if (*pos == WLAN_EID_RSNX)
        {
            ...
            wpa_hexdump(MSG_DEBUG, "WPA: RSNXE in EAPOL-Key", ie->rsnxe, ie->rsnxe_len);
        }
        ...
        else if (*pos == WLAN_EID_EXTENSION && pos[1] >= 1 + IEEE80211_HE_CAPAB_MIN_LEN &&
                 pos[2] == WLAN_EID_EXT_HE_CAPABILITIES)
        {
            ie->he_capabilities = pos + 3;
            ie->he_capab_len    = pos[1] - 1;
        }
        else if (*pos == WLAN_EID_EXTENSION && pos[1] >= 1 + sizeof(struct ieee80211_he_6ghz_band_cap) &&
 ...
 */
void wpa_pasn_build_auth_header(
    struct wpabuf *buf, const u8 *bssid, const u8 *src, const u8 *dst, u8 trans_seq, u16 status)
{
    struct ieee80211_mgmt *auth;

    wpa_printf(MSG_DEBUG, "PASN: Add authentication header. trans_seq=%u", trans_seq);

    auth = wpabuf_put(buf, offsetof(struct ieee80211_mgmt, u.auth.variable));

    auth->frame_control = host_to_le16((WLAN_FC_TYPE_MGMT << 2) | (WLAN_FC_STYPE_AUTH << 4));
...
int wpa_pasn_add_rsne(struct wpabuf *buf, const u8 *pmkid, int akmp, int cipher)
{
    ...
    rsne_len = sizeof(*hdr) + RSN_SELECTOR_LEN + 2 + RSN_SELECTOR_LEN + 2 + RSN_SELECTOR_LEN + 2 + RSN_SELECTOR_LEN +
               2 + (pmkid ? PMKID_LEN : 0);
 ...
 */
void wpa_pasn_add_parameter_ie(struct wpabuf *buf,
                               u16 pasn_group,
                               ...
                               int after)
{
    ...
    if (pubkey)
    {
        wpa_printf(MSG_DEBUG, "PASN: Adding public key and group ID %u", pasn_group);
        ...
        wpabuf_put_u8(buf, compressed ? WPA_PASN_PUBKEY_COMPRESSED_0 : WPA_PASN_PUBKEY_UNCOMPRESSED);
 ...
 */
int wpa_pasn_add_wrapped_data(struct wpabuf *buf, struct wpabuf *wrapped_data_buf)
{
    const u8 *data;
    size_t data_len;
    u8 len;
...
int wpa_pasn_validate_rsne(const struct wpa_ie_data *data)
{
    ...
    if (!data->has_pairwise || !data->pairwise_cipher || (data->pairwise_cipher & (data->pairwise_cipher - 1)))
    {
    ...
    switch (data->key_mgmt)
    {
        ...
        case WPA_KEY_MGMT_PASN:
            break;
        default:
            wpa_printf(MSG_ERROR, "PASN: invalid key_mgmt: 0x%0x", data->key_mgmt);
            return -1;
    }
...
int wpa_pasn_parse_parameter_ie(const u8 *data, u8 len, bool from_ap, struct wpa_pasn_params_data *pasn_params)
{
    ...
    if (!params || ((size_t)(params->len + 2) < sizeof(*params)) || len < sizeof(*params) || params->len + 2 != len)
    {
        wpa_printf(MSG_DEBUG, "PASN: Invalid parameters IE. len=(%u, %u)", params ? params->len : 0, len);
        return -1;
    }
    ...
    if (params->control & WPA_PASN_CTRL_COMEBACK_INFO_PRESENT)
    {
        if (from_ap)
        {
            if (len < 2)
            {
                wpa_printf(MSG_DEBUG, "PASN: Invalid Parameters IE: Truncated Comeback After");
                return -1;
            }
        ...
        if (len < 1 || len < 1 + *pos)
        {
            wpa_printf(MSG_DEBUG, "PASN: Invalid Parameters IE: comeback len");
            return -1;
        }
    ...
    if (params->control & WPA_PASN_CTRL_GROUP_AND_KEY_PRESENT)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "PASN: Invalid Parameters IE: group and key");
            return -1;
        }
    ...
    if (len)
    {
        wpa_printf(MSG_DEBUG, "PASN: Invalid Parameters IE. Bytes left=%u", len);
        return -1;
    }
