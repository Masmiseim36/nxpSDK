...
try_again:
    if (cli_path && cli_path[0] == '/')
    {
        ret = os_snprintf(ctrl->local.sun_path, sizeof(ctrl->local.sun_path),
                          "%s/" CONFIG_CTRL_IFACE_CLIENT_PREFIX "%d-%d", cli_path, (int)getpid(), counter);
    }
    else
    {
        ret = os_snprintf(ctrl->local.sun_path, sizeof(ctrl->local.sun_path),
                          CONFIG_CTRL_IFACE_CLIENT_DIR "/" CONFIG_CTRL_IFACE_CLIENT_PREFIX "%d-%d", (int)getpid(),
                          counter);
    }
...
struct wpa_ctrl *wpa_ctrl_open(const char *ctrl_path)
{
    ...
    if (connect(ctrl->s, (struct sockaddr *)&ctrl->dest, sizeof(ctrl->dest)) < 0)
    {
        ...
        wpa_printf(MSG_ERROR, "connect(%s:%d) failed: %s",
                   inet_ntop(AF_INET6, &ctrl->dest.sin6_addr, addr, sizeof(ctrl->dest)), ntohs(ctrl->dest.sin6_port),
                   strerror(errno));
#else  /* CONFIG_CTRL_IFACE_UDP_IPV6 */
        wpa_printf(MSG_ERROR, "connect(%s:%d) failed: %s", inet_ntoa(ctrl->dest.sin_addr), ntohs(ctrl->dest.sin_port),
                   strerror(errno));
#endif /* CONFIG_CTRL_IFACE_UDP_IPV6 */
        close(ctrl->s);
        os_free(ctrl->remote_ip);
...
retry_send:
    ...
    for (;;)
    {
        ...
        if (FD_ISSET(ctrl->s, &rfds))
        {
            ...
            if (res < 0)
                return res;
            if ((res > 0 && reply[0] == '<') || (res > 6 && strncmp(reply, "IFNAME=", 7) == 0))
            {
