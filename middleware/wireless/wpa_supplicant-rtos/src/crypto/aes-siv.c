...
static int aes_s2v(const u8 *key, size_t key_len, size_t num_elem, const u8 *addr[], size_t *len, u8 *mac)
{
                    ...
                    u8 *out)
{
    ...
    if (num_elem > ARRAY_SIZE(_addr) - 1 || (key_len != 32 && key_len != 48 && key_len != 64))
        return -1;
