...
#ifdef MBEDTLS_SHA512_C
#include <mbedtls/sha512.h>
__attribute_noinline__ static int sha384_512_vector(
    size_t num_elem, const u8 *addr[], const size_t *len, u8 *mac, int is384)
{
    if (TEST_FAIL())
        return -1;
...
#if defined(CRYPTO_MBEDTLS_HMAC_KDF_SHA256) || defined(CRYPTO_MBEDTLS_HMAC_KDF_SHA384) || \
...
int fips186_2_prf(const u8 *seed, size_t seed_len, u8 *x, size_t xlen)
{
    ...
    u32 xstate[5];          /*!< The intermediate digest state.  */
    u8 xkey[64];   /*!< The data block being processed. */
...
int crypto_ec_point_is_on_curve(struct crypto_ec *e, const struct crypto_ec_point *p)
{
    ...
    mbedtls_mpi *cy2      = (mbedtls_mpi *)crypto_ec_point_compute_y_sqr(e, (const struct crypto_bignum *)px);
    if (cy2 == NULL)
        return 0;
    ...
    int is_on_curve       = mbedtls_mpi_mul_mpi(&y2, py, py) /* y^2 mod P */
                              || mbedtls_mpi_mod_mpi(&y2, &y2, CRYPTO_EC_P(e)) || mbedtls_mpi_cmp_mpi(&y2, cy2) != 0 ?
                          0 :
                          1;
...
struct crypto_ec_key *crypto_ec_key_set_priv(int group, const u8 *raw, size_t raw_len)
{
    ...
    if (ctx == NULL)
        return NULL;
    mbedtls_pk_init(ctx);
    if (mbedtls_pk_setup(ctx, pk_info) == 0 && mbedtls_ecp_read_key(grp_id, mbedtls_pk_ec(*ctx), raw, raw_len) == 0)
    {
        return (struct crypto_ec_key *)ctx;
    }
