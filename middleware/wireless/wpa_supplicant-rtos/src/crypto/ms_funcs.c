 ...
 */
int challenge_hash(
    const u8 *peer_challenge, const u8 *auth_challenge, const u8 *username, size_t username_len, u8 *challenge)
{
 ...
 * Returns: 0 on success, -1 on failure
 */
int challenge_response(const u8 *challenge, const u8 *password_hash, u8 *response)
{
    u8 zpwd[7];
                                           ...
                                           u8 *response)
{
    ...
    if (hash_nt_password_hash(password_hash, password_hash_hash) || sha1_vector(3, addr1, len1, response) ||
        challenge_hash(peer_challenge, auth_challenge, username, username_len, challenge))
        return -1;
                                    ...
                                    u8 *response)
{
    ...
    if (nt_password_hash(password, password_len, password_hash))
        return -1;
    return generate_authenticator_response_pwhash(password_hash, peer_challenge, auth_challenge, username, username_len,
...
{
    static const u8 magic2[84]   = {0x4f, 0x6e, 0x20, 0x74, 0x68, 0x65, 0x20, 0x63, 0x6c, 0x69, 0x65, 0x6e, 0x74, 0x20,
    ...
    static const u8 magic3[84]   = {0x4f, 0x6e, 0x20, 0x74, 0x68, 0x65, 0x20, 0x63, 0x6c, 0x69, 0x65, 0x6e, 0x74, 0x20,
                                  0x73, 0x69, 0x64, 0x65, 0x2c, 0x20, 0x74, 0x68, 0x69, 0x73, 0x20, 0x69, 0x73, 0x20,
    ...
    static const u8 shs_pad1[40] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

    static const u8 shs_pad2[40] = {0xf2, 0xf2, 0xf2, 0xf2, 0xf2, 0xf2, 0xf2, 0xf2, 0xf2, 0xf2, 0xf2, 0xf2, 0xf2, 0xf2,
...
int encrypt_pw_block_with_password_hash(const u8 *password, size_t password_len, const u8 *password_hash, u8 *pw_block)
{
    ...
    if (utf8_to_ucs2(password, password_len, pw_block, 512, &ucs2_len) < 0 || ucs2_len > 256)
        return -1;
 ...
 * Returns: 0 on success, -1 on failure
 */
int nt_password_hash_encrypted_with_block(const u8 *password_hash, const u8 *block, u8 *cypher)
{
    if (des_encrypt(password_hash, block, cypher) < 0 || des_encrypt(password_hash + 8, block + 7, cypher + 8) < 0)
        return -1;
    return 0;
}

/**
 * old_nt_password_hash_encrypted_with_new_nt_password_hash - OldNtPasswordHashEncryptedWithNewNtPasswordHash() - RFC
 * 2759, Sect. 8.12
 * @new_password: 0-to-256-unicode-char NewPassword (IN; UTF-8)
 * @new_password_len: Length of new_password
 * @old_password: 0-to-256-unicode-char OldPassword (IN; UTF-8)
