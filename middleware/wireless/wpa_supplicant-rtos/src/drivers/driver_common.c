...
#define CHECK_TRIGGER(trigger) \
    wpa_check_wowlan_trigger(start, #trigger, capa->wowlan_triggers.trigger, &triggers->trigger)
    ...
    while (*start != '\0')
    {
        ...
        *end = '\0';
        ...
        if (!CHECK_TRIGGER(any) && !CHECK_TRIGGER(disconnect) && !CHECK_TRIGGER(magic_pkt) &&
            !CHECK_TRIGGER(gtk_rekey_failure) && !CHECK_TRIGGER(eap_identity_req) &&
