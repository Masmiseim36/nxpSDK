...
#define TLS_MASTER_SECRET_LEN 48
    ...
    sha1_t_prf(pac_key, EAP_FAST_PAC_KEY_LEN, "PAC to master secret label hash", seed, sizeof(seed), master_secret,
...
int eap_fast_derive_eap_msk(const u8 *simck, u8 *msk)
{
    ...
    if (sha1_t_prf(simck, EAP_FAST_SIMCK_LEN, "Session Key Generating Function", (u8 *)"", 0, msk, EAP_FAST_KEY_LEN) <
        0)
        return -1;
...
int eap_fast_derive_eap_emsk(const u8 *simck, u8 *emsk)
{
    ...
    if (sha1_t_prf(simck, EAP_FAST_SIMCK_LEN, "Extended Session Key Generating Function", (u8 *)"", 0, emsk,
                   EAP_EMSK_LEN) < 0)
        return -1;
...
int eap_fast_parse_tlv(struct eap_fast_tlv_parse *tlv, int tlv_type, u8 *pos, size_t len)
{
    switch (tlv_type)
    {
        ...
        case EAP_TLV_CRYPTO_BINDING_TLV:
            ...
            tlv->crypto_binding = (struct eap_tlv_crypto_binding_tlv *)(pos - sizeof(struct eap_tlv_hdr));
            break;
