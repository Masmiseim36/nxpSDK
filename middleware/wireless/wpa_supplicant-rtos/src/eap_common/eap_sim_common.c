...
int eap_sim_derive_keys_reauth(
    u16 _counter, const u8 *identity, size_t identity_len, const u8 *nonce_s, const u8 *mk, u8 *msk, u8 *emsk)
{
...
int eap_sim_verify_mac(const u8 *k_aut, const struct wpabuf *req, const u8 *mac, const u8 *extra, size_t extra_len)
{
    ...
    if (mac == NULL || wpabuf_len(req) < EAP_SIM_MAC_LEN || mac < wpabuf_head_u8(req) ||
...
static void prf_prime(const u8 *k,
                      const char *seed1,
                      const u8 *seed2,
                      size_t seed2_len,
                      const u8 *seed3,
...
void eap_aka_prime_derive_keys(const u8 *identity,
                               size_t identity_len,
                               const u8 *ik,
                               const u8 *ck,
                               u8 *k_encr,
...
int eap_aka_prime_derive_keys_reauth(
    const u8 *k_re, u16 counter, const u8 *identity, size_t identity_len, const u8 *nonce_s, u8 *msk, u8 *emsk)
{
...
int eap_sim_verify_mac_sha256(
    const u8 *k_aut, const struct wpabuf *req, const u8 *mac, const u8 *extra, size_t extra_len)
{
...
u8 *eap_sim_parse_encr(
    const u8 *k_encr, const u8 *encr_data, size_t encr_data_len, const u8 *iv, struct eap_sim_attrs *attr, int aka)
{
    u8 *decrypted;
...
#ifdef TEST_FUZZ
    wpa_printf(MSG_INFO, "TEST: Skip AES-128-CBC decryption for fuzz testing");
#else  /* TEST_FUZZ */
...
struct wpabuf *eap_sim_msg_finish(struct eap_sim_msg *msg, int type, const u8 *k_aut, const u8 *extra, size_t extra_len)
{
    ...
    {
        eap_sim_add_mac_sha256(k_aut, (u8 *)wpabuf_head(msg->buf), wpabuf_len(msg->buf),
...
int eap_sim_msg_add_encr_end(struct eap_sim_msg *msg, u8 *k_encr, int attr_pad)
{
    ...
    return aes_128_cbc_encrypt(k_encr, wpabuf_head_u8(msg->buf) + msg->iv, wpabuf_mhead_u8(msg->buf) + msg->encr + 4,
                               encr_len);
}
...
int eap_sim_anonymous_username(const u8 *id, size_t id_len)
{
    ...
    if (id_len > anonymous_id_len && os_memcmp(id, anonymous_id_prefix, anonymous_id_len) == 0)
        return 1; /* 'anonymous@realm' */

    if (id_len > anonymous_id_len + 1 && os_memcmp(id + 1, anonymous_id_prefix, anonymous_id_len) == 0)
    ...
    {
        decorated++;
        return eap_sim_anonymous_username(decorated, id + id_len - decorated);
    }

    return 0;
}

#endif /* CONFIG_EAP_SIM_COMMON */
