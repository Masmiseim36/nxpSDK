...
static bool eap_sm_allowMethod(struct eap_sm *sm, int vendor, enum eap_type method);
...
static void eapol_set_bool(struct eap_sm *sm, enum eapol_bool_var var, bool value)
{
 ...
 */
static int eap_config_allowed_method(struct eap_sm *sm, struct eap_peer_config *config, int vendor, u32 method)
{
    int i;
...
{
    return eap_config_allowed_method(sm, eap_get_config(sm), vendor, method);
}

#if defined(PCSC_FUNCS) || defined(CONFIG_EAP_PROXY)
static int eap_sm_append_3gpp_realm(struct eap_sm *sm, char *imsi, size_t max_len, size_t *imsi_len, int mnc_len)
{
    char *pos, mnc[4];
    ...
    pos = imsi + *imsi_len;
    pos +=
        os_snprintf(pos, imsi + max_len - pos, "@wlan.mnc%s.mcc%c%c%c.3gppnetwork.org", mnc, imsi[0], imsi[1], imsi[2]);
    *imsi_len = pos - imsi;
...
SM_STATE(EAP, INITIALIZE)
{
    ...
    if (sm->fast_reauth && sm->m && sm->m->has_reauth_data && sm->m->has_reauth_data(sm, sm->eap_method_priv) &&
        !sm->prev_failure && sm->last_config == eap_get_config(sm))
    {
        wpa_printf(MSG_DEBUG,
...
SM_STATE(EAP, GET_METHOD)
{
    ...
    if (!eap_sm_allowMethod(sm, sm->reqVendor, method))
    {
        ...
        wpa_msg(sm->msg_ctx, MSG_INFO, WPA_EVENT_EAP_PROPOSED_METHOD "vendor=%u method=%u -> NAK", sm->reqVendor,
                method);
     ...
     * is allocated here.
     */
    if (sm->fast_reauth && sm->m && sm->m->vendor == sm->reqVendor && sm->m->method == method &&
        sm->m->has_reauth_data && sm->m->has_reauth_data(sm, sm->eap_method_priv))
    {
        wpa_printf(MSG_DEBUG,
    ...
    wpa_msg(sm->msg_ctx, MSG_INFO, WPA_EVENT_EAP_METHOD "EAP vendor %u method %u (%s) selected", sm->reqVendor, method,
            sm->m->name);
    return;
...
static char *eap_get_realm(struct eap_sm *sm, struct eap_peer_config *config)
{
     ...
     */
    if (!config->identity && !config->anonymous_identity && sm->eapol_cb->get_imsi &&
        (eap_config_allowed_method(sm, config, EAP_VENDOR_IETF, EAP_TYPE_SIM) ||
         eap_config_allowed_method(sm, config, EAP_VENDOR_IETF, EAP_TYPE_AKA) ||
         eap_config_allowed_method(sm, config, EAP_VENDOR_IETF, EAP_TYPE_AKA_PRIME)))
    {
        char imsi[100];
        size_t imsi_len;
        int mnc_len, pos;
        ...
        mnc_len = sm->eapol_cb->get_imsi(sm->eapol_ctx, config->sim_num, imsi, &imsi_len);
        if (mnc_len < 0)
            return NULL;

        pos = imsi_len + 1; /* points to the beginning of the realm */
        if (eap_sm_append_3gpp_realm(sm, imsi, sizeof(imsi), &imsi_len, mnc_len) < 0)
        {
...
int eap_peer_update_erp_next_seq_num(struct eap_sm *sm, u16 next_seq_num)
{
    ...
    if (!erp)
    {
        wpa_printf(MSG_DEBUG, "EAP: Failed to find ERP key for realm: %s", home_realm);
        os_free(home_realm);
        return -1;
    }
...
int eap_peer_get_erp_info(struct eap_sm *sm,
 ...
 * unconditionally. */
void eap_peer_erp_init(
    struct eap_sm *sm, u8 *ext_session_id, size_t ext_session_id_len, u8 *ext_emsk, size_t ext_emsk_len)
{
#ifdef CONFIG_ERP
    u8 *emsk              = NULL;
    size_t emsk_len       = 0;
    ...
    {
        wpa_printf(MSG_DEBUG, "EAP: No suitable session id available for ERP");
        goto fail;
    }

    WPA_PUT_BE16(len, EAP_EMSK_NAME_LEN);
    if (hmac_sha256_kdf(session_id, session_id_len, "EMSK", len, sizeof(len), EMSKname, EAP_EMSK_NAME_LEN) < 0)
    {
    ...
    if (hmac_sha256_kdf(emsk, emsk_len, "EAP Re-authentication Root Key@ietf.org", len, sizeof(len), erp->rRK,
                        emsk_len) < 0)
    {
    ...
    if (hmac_sha256_kdf(erp->rRK, erp->rRK_len, "Re-authentication Integrity Key@ietf.org", ctx, sizeof(ctx), erp->rIK,
                        erp->rRK_len) < 0)
    {
...
struct wpabuf *eap_peer_build_erp_reauth_start(struct eap_sm *sm, u8 eap_id)
{
    ...
    msg = eap_msg_alloc(EAP_VENDOR_IETF, (enum eap_type)EAP_ERP_TYPE_REAUTH,
                        1 + 2 + 2 + os_strlen(erp->keyname_nai) + 1 + 16, EAP_CODE_INITIATE, eap_id);
    if (msg == NULL)
        return NULL;
...
SM_STATE(EAP, METHOD)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               ret.ignore ? "TRUE" : "FALSE", eap_sm_method_state_txt(ret.methodState),
...
SM_STATE(EAP, SUCCESS)
{
    ...
    if (!config || !sm->m)
    {
         ...
         */
        wpa_printf(MSG_DEBUG, "EAP: State machine not configured - cannot initialize ERP");
        return;
    }
    if (config->erp && sm->m->get_emsk && sm->eapSessionId && sm->m->isKeyAvailable &&
...
static void eap_peer_sm_step_idle(struct eap_sm *sm)
{
    ...
    else if ((eapol_get_bool(sm, EAPOL_altAccept) && sm->decision != DECISION_FAIL) ||
             (eapol_get_int(sm, EAPOL_idleWhile) == 0 && sm->decision == DECISION_UNCOND_SUCC))
        SM_ENTER(EAP, SUCCESS);
    else if (eapol_get_bool(sm, EAPOL_altReject) ||
             (eapol_get_int(sm, EAPOL_idleWhile) == 0 && sm->decision != DECISION_UNCOND_SUCC) ||
             (eapol_get_bool(sm, EAPOL_altAccept) && sm->methodState != METHOD_CONT && sm->decision == DECISION_FAIL))
        SM_ENTER(EAP, FAILURE);
    else if (sm->selectedMethod == EAP_TYPE_LEAP && sm->leap_done && sm->decision != DECISION_FAIL &&
             sm->methodState == METHOD_DONE)
        SM_ENTER(EAP, SUCCESS);
    else if (sm->selectedMethod == EAP_TYPE_PEAP && sm->peap_done && sm->decision != DECISION_FAIL &&
             sm->methodState == METHOD_DONE)
        SM_ENTER(EAP, SUCCESS);
}
...
static int eap_peer_req_is_duplicate(struct eap_sm *sm)
{
    ...
    if (sm->workaround && duplicate && os_memcmp(sm->req_sha1, sm->last_sha1, 20) != 0)
    {
        /*
...
static void eap_peer_sm_step_received(struct eap_sm *sm)
{
    ...
    if (sm->rxSuccess && sm->decision != DECISION_FAIL &&
        (sm->reqId == sm->lastId || eap_success_workaround(sm, sm->reqId, sm->lastId)))
        SM_ENTER(EAP, SUCCESS);
    else if (sm->workaround && sm->lastId == -1 && sm->rxSuccess && !sm->rxFailure && !sm->rxReq &&
             eap_peer_sm_allow_canned(sm))
        SM_ENTER(EAP, SUCCESS); /* EAP-Success prior any EAP method */
    else if (sm->workaround && sm->lastId == -1 && sm->rxFailure && !sm->rxReq && sm->methodState != METHOD_CONT &&
             eap_peer_sm_allow_canned(sm))
        SM_ENTER(EAP, FAILURE); /* EAP-Failure prior any EAP method */
    else if (sm->workaround && sm->rxSuccess && !sm->rxFailure && !sm->rxReq && sm->methodState != METHOD_CONT &&
             eap_peer_sm_allow_canned(sm))
        SM_ENTER(EAP, SUCCESS); /* EAP-Success after Identity */
             ...
             ((sm->rxFailure && sm->decision != DECISION_UNCOND_SUCC) ||
              (sm->rxSuccess && sm->decision == DECISION_FAIL &&
               (sm->selectedMethod != EAP_TYPE_LEAP || sm->methodState != METHOD_MAY_CONT))) &&
             (sm->reqId == sm->lastId || eap_success_workaround(sm, sm->reqId, sm->lastId)))
        SM_ENTER(EAP, FAILURE);
...
SM_STEP(EAP)
{
    ...
    else if (sm->num_rounds_short > EAP_MAX_AUTH_ROUNDS_SHORT)
    {
        ...
        {
            wpa_msg(sm->msg_ctx, MSG_INFO, "EAP: more than %d authentication rounds (short) - abort",
...
struct wpabuf *eap_sm_buildIdentity(struct eap_sm *sm, int id, int encrypted)
{
    ...
    if (sm->m && sm->m->get_identity &&
        (identity = sm->m->get_identity(sm, sm->eap_method_priv, &identity_len)) != NULL)
    {
    ...
    else if (sm->use_machine_cred)
    {
        ...
        wpa_hexdump_ascii(MSG_DEBUG, "EAP: using machine identity", identity, identity_len);
    }
    else
    {
...
static struct wpabuf *eap_sm_buildNotify(int id)
{
    ...
    return eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_NOTIFICATION, 0, EAP_CODE_RESPONSE, id);
}
...
no_auth_tag:
    ...
    if (hmac_sha256_kdf(erp->rRK, erp->rRK_len, "Re-authentication Master Session Key@ietf.org", seed, sizeof(seed),
...
static void eap_peer_sm_tls_event(void *ctx, enum tls_event ev, union tls_event_data *data)
{
    ...
    switch (ev)
    {
        ...
        case TLS_CERT_CHAIN_FAILURE:
            wpa_msg(sm->msg_ctx, MSG_INFO, WPA_EVENT_EAP_TLS_CERT_ERROR "reason=%d depth=%d subject='%s' err='%s'",
                    data->cert_fail.reason, data->cert_fail.depth, data->cert_fail.subject, data->cert_fail.reason_txt);
        ...
        case TLS_PEER_CERTIFICATE:
            ...
            sm->eapol_cb->notify_cert(sm->eapol_ctx, &data->peer_cert, hash_hex);
            break;
        case TLS_ALERT:
...
int eap_sm_get_status(struct eap_sm *sm, char *buf, size_t buflen, int verbose)
{
    ...
    if (sm->selectedMethod != EAP_TYPE_NONE)
    {
        ...
        {
            const struct eap_method *m = eap_peer_get_eap_method(EAP_VENDOR_IETF, sm->selectedMethod);
            if (m)
                name = m->name;
            else
                name = "?";
    ...
    if (verbose)
    {
        ret = os_snprintf(buf + len, buflen - len,
                          ...
                          "decision=%s\n"
                          "ClientTimeout=%d\n",
                          sm->reqMethod, eap_sm_method_state_txt(sm->methodState), eap_sm_decision_txt(sm->decision),
                          sm->ClientTimeout);
        if (os_snprintf_error(buflen - len, ret))
            return len;
        len += ret;
...
static int eap_allowed_phase2_type(int vendor, int type)
{
    ...
    if (vendor != EAP_VENDOR_IETF)
        return 0;
    return type != EAP_TYPE_PEAP && type != EAP_TYPE_TTLS && type != EAP_TYPE_FAST && type != EAP_TYPE_TEAP;
}
...
struct eap_method_type *eap_get_phase2_types(struct eap_peer_config *config, size_t *count)
{
    ...
    *count = 0;
    ...
    for (m = methods; m; m = m->next)
    {
        ...
        {
            if (vendor == EAP_VENDOR_IETF && method == EAP_TYPE_TLS && config && !config->phase2_cert.private_key)
                continue;
...
const u8 *eap_get_config_password(struct eap_sm *sm, size_t *len)
{
    ...
    if ((sm->use_machine_cred && (config->flags & EAP_CONFIG_FLAGS_EXT_MACHINE_PASSWORD)) ||
        (!sm->use_machine_cred && (config->flags & EAP_CONFIG_FLAGS_EXT_PASSWORD)))
    {
...
const u8 *eap_get_config_password2(struct eap_sm *sm, size_t *len, int *hash)
{
    ...
    if (sm->use_machine_cred)
    {
        *len = config->machine_password_len;
        if (hash)
            *hash = !!(config->flags & EAP_CONFIG_FLAGS_MACHINE_PASSWORD_NTHASH);
        return config->machine_password;
    }
