...
static int eap_aka_umts_auth(struct eap_sm *sm, struct eap_aka_data *data)
{
    ...
    if (conf->pcsc)
    {
        return scard_umts_auth(sm->scard_ctx, data->rand, data->autn, data->res, &data->res_len, data->ik, data->ck,
                               data->auts);
    }
    ...
    if (conf->password)
    {
        ...
        return milenage_check(opc, k, sqn, data->rand, data->autn, data->ik, data->ck, data->res, &data->res_len,
                              data->auts);
    }
    ...
    wpa_printf(MSG_DEBUG,
               "EAP-AKA: No UMTS authentication algorithm "
               "enabled");
    return -1;
...
static int eap_aka_learn_ids(struct eap_sm *sm, struct eap_aka_data *data, struct eap_sim_attrs *attr)
{
    if (attr->next_pseudonym)
    {
        ...
        wpa_hexdump_ascii(MSG_DEBUG, "EAP-AKA: (encr) AT_NEXT_PSEUDONYM", attr->next_pseudonym,
    ...
    {
        os_free(data->reauth_id);
        data->reauth_id = os_memdup(attr->next_reauth_id, attr->next_reauth_id_len);
...
static int eap_aka_add_id_msg(struct eap_aka_data *data, const struct wpabuf *msg1, const struct wpabuf *msg2)
{
    size_t len;
...
static struct wpabuf *eap_aka_synchronization_failure(struct eap_aka_data *data, u8 id, struct eap_sim_attrs *attr)
{
    struct eap_sim_msg *msg;
                                                ...
                                                enum eap_sim_id_req id_req)
{
    ...
    if (id_req == ANY_ID && data->reauth_id)
    {
        ...
        data->reauth = 1;
             ...
             !eap_sim_anonymous_username(data->pseudonym, data->pseudonym_len))
    {
    ...
    else if (id_req != NO_ID_REQ)
    {
        ...
        if (identity)
        {
            ...
            if (data->pseudonym && eap_sim_anonymous_username(data->pseudonym, data->pseudonym_len))
                ids &= ~CLEAR_PSEUDONYM;
...
static struct wpabuf *eap_aka_process_identity(
    struct eap_sm *sm, struct eap_aka_data *data, u8 id, const struct wpabuf *reqData, struct eap_sim_attrs *attr)
{
    int id_error;
    struct wpabuf *buf;
    ...
    if (data->prev_id != id)
    {
        ...
        {
            wpa_printf(MSG_INFO, "EAP-AKA: Failed to store ID messages");
            wpabuf_free(buf);
            return eap_aka_client_error(data, id, EAP_AKA_UNABLE_TO_PROCESS_PACKET);
        }
        data->prev_id = id;
...
static int eap_aka_prime_kdf_valid(struct eap_aka_data *data, struct eap_sim_attrs *attr)
{
    ...
    if (data->kdf_negotiation)
    {
        ...
        if (attr->kdf_count > EAP_AKA_PRIME_KDF_MAX || attr->kdf_count != data->last_kdf_count + 1)
        {
...
static struct wpabuf *eap_aka_process_challenge(
    struct eap_sm *sm, struct eap_aka_data *data, u8 id, const struct wpabuf *reqData, struct eap_sim_attrs *attr)
{
    const u8 *identity;
    size_t identity_len;
    ...
    if (attr->checkcode && eap_aka_verify_checkcode(data, attr->checkcode, attr->checkcode_len))
    {
        wpa_printf(MSG_WARNING,
                   ...
                   "message");
#ifdef TEST_FUZZ
        wpa_printf(MSG_INFO, "TEST: Ignore AT_CHECKCODE mismatch for fuzz testing");
#else  /* TEST_FUZZ */
        return eap_aka_client_error(data, id, EAP_AKA_UNABLE_TO_PROCESS_PACKET);
#endif /* TEST_FUZZ */
    }
    ...
    if (data->eap_method == EAP_TYPE_AKA_PRIME)
    {
        ...
        os_free(data->network_name);
        data->network_name = os_memdup(attr->kdf_input, attr->kdf_input_len);
    ...
    if (!attr->mac || !attr->rand || !attr->autn)
    {
        wpa_printf(MSG_WARNING,
                   ...
                   !attr->mac ? " AT_MAC" : "", !attr->rand ? " AT_RAND" : "", !attr->autn ? " AT_AUTN" : "");
        return eap_aka_client_error(data, id, EAP_AKA_UNABLE_TO_PROCESS_PACKET);
    }
    ...
    else if (data->pseudonym && !eap_sim_anonymous_username(data->pseudonym, data->pseudonym_len))
    {
        identity     = data->pseudonym;
        identity_len = data->pseudonym_len;
    }
    ...
    if (data->eap_method == EAP_TYPE_AKA_PRIME)
    {
        eap_aka_prime_derive_keys(identity, identity_len, data->ik, data->ck, data->k_encr, data->k_aut, data->k_re,
                                  data->msk, data->emsk);
    }
    else
    {
    ...
    if (eap_aka_verify_mac(data, reqData, attr->mac, (u8 *)"", 0))
    {
        wpa_printf(MSG_WARNING,
                   ...
                   "used invalid AT_MAC");
#ifdef TEST_FUZZ
        wpa_printf(MSG_INFO, "TEST: Ignore AT_MAC mismatch for fuzz testing");
#else  /* TEST_FUZZ */
        return eap_aka_client_error(data, id, EAP_AKA_UNABLE_TO_PROCESS_PACKET);
#endif /* TEST_FUZZ */
    }
    ...
    struct eap_sm *sm, struct eap_aka_data *data, u8 id, const struct wpabuf *reqData, struct eap_sim_attrs *attr)
{
    ...
    wpa_hexdump(MSG_DEBUG, "EAP-AKA: Server MAC", data->reauth_mac, EAP_SIM_MAC_LEN);
    ...
    {
        wpa_printf(MSG_INFO, "EAP-AKA: (encr) No%s%s in reauth packet", !eattr.nonce_s ? " AT_NONCE_S" : "",
    ...
    if (data->eap_method == EAP_TYPE_AKA_PRIME)
    {
        eap_aka_prime_derive_keys_reauth(data->k_re, data->counter, data->reauth_id, data->reauth_id_len, data->nonce_s,
                                         data->msk, data->emsk);
    }
    else
    {
        eap_sim_derive_keys_reauth(data->counter, data->reauth_id, data->reauth_id_len, data->nonce_s, data->mk,
                                   data->msk, data->emsk);
    }
    eap_aka_clear_identities(sm, data, CLEAR_REAUTH_ID | CLEAR_EAP_ID);
    eap_aka_learn_ids(sm, data, &eattr);
                                      ...
                                      const struct wpabuf *reqData)
{
    ...
    if (eap_sim_parse_attr(pos, wpabuf_head_u8(reqData) + len, &attr, data->eap_method == EAP_TYPE_AKA_PRIME ? 2 : 1,
                           0))
    {
...
#endif /* CONFIG_EAP_AKA || CONFIG_EAP_AKA_PRIME */
