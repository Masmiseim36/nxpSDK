...
static void eap_fast_parse_phase1(struct eap_fast_data *data, const char *phase1)
{
    const char *pos;
...
static void *eap_fast_init(struct eap_sm *sm)
{
    ...
    if (eap_peer_select_phase2_methods(config, "auth=", &data->phase2_types, &data->num_phase2_types, 0) < 0)
    {
...
{
    os_free(data->key_block_p);
    data->key_block_p = (struct eap_fast_key_block_provisioning *)eap_fast_derive_key(sm->ssl_ctx, data->ssl.conn,
     ...
     * S-IMCK[0] = session_key_seed
     */
    wpa_hexdump_key(MSG_DEBUG, "EAP-FAST: session_key_seed (SKS = S-IMCK[0])", data->key_block_p->session_key_seed,
                    sizeof(data->key_block_p->session_key_seed));
    data->simck_idx = 0;
    os_memcpy(data->simck, data->key_block_p->session_key_seed, EAP_FAST_SIMCK_LEN);
    wpa_hexdump_key(MSG_DEBUG, "EAP-FAST: server_challenge", data->key_block_p->server_challenge,
                    sizeof(data->key_block_p->server_challenge));
    wpa_hexdump_key(MSG_DEBUG, "EAP-FAST: client_challenge", data->key_block_p->client_challenge,
...
static int eap_fast_select_phase2_method(struct eap_fast_data *data, int vendor, enum eap_type type)
{
    size_t i;
    ...
    if (data->anon_provisioning && (vendor != EAP_VENDOR_IETF || type != EAP_TYPE_MSCHAPV2))
    {
        wpa_printf(
            MSG_INFO,
    ...
    {
        if (data->phase2_types[i].vendor != vendor || data->phase2_types[i].method != type)
            continue;
    ...
    if (vendor != data->phase2_type.vendor || type != data->phase2_type.method ||
                                   ...
                                   struct wpabuf **resp)
{
    ...
    if (method == EAP_TYPE_EXPANDED)
    {
        ...
        {
            wpa_printf(MSG_INFO, "EAP-FAST: Too short Phase 2 request (expanded header) (len=%lu)", (unsigned long)len);
            return -1;
        }
        vendor = WPA_GET_BE24(pos + 1);
        method = WPA_GET_BE32(pos + 4);
    }
    wpa_printf(MSG_DEBUG, "EAP-FAST: Phase 2 Request: type=%u:%u", vendor, method);
    ...
    if (data->phase2_priv && data->phase2_method &&
        (vendor != data->phase2_type.vendor || method != data->phase2_type.method))
    {
        wpa_printf(MSG_DEBUG,
    ...
    {
        wpa_printf(MSG_INFO, "EAP-FAST: Failed to initialize Phase 2 EAP method %u:%u", vendor, method);
    ...
    *resp = data->phase2_method->process(sm, data->phase2_priv, &iret, &msg);
    ...
    else if ((iret.methodState == METHOD_DONE || iret.methodState == METHOD_MAY_CONT) &&
    ...
    if (*resp == NULL && config &&
        (config->pending_req_identity || config->pending_req_password || config->pending_req_otp ||
...
static struct wpabuf *eap_fast_tlv_result(int status, int intermediate)
{
    ...
    result->tlv_type =
        host_to_be16(EAP_TLV_TYPE_MANDATORY | (intermediate ? EAP_TLV_INTERMEDIATE_RESULT_TLV : EAP_TLV_RESULT_TLV));
...
static int eap_fast_validate_crypto_binding(struct eap_tlv_crypto_binding_tlv *_bind)
{
    ...
    if (_bind->version != EAP_FAST_VERSION || _bind->received_version != EAP_FAST_VERSION ||
...
static int eap_fast_get_phase2_key(struct eap_sm *sm, struct eap_fast_data *data, u8 *isk, size_t isk_len)
{
    ...
    if (!data->phase2_method->isKeyAvailable(sm, data->phase2_priv) ||
        (key = data->phase2_method->getKey(sm, data->phase2_priv, &key_len)) == NULL)
    {
        wpa_printf(MSG_DEBUG,
...
static int eap_fast_get_cmk(struct eap_sm *sm, struct eap_fast_data *data, u8 *cmk)
{
    ...
    if (sha1_t_prf(data->simck, EAP_FAST_SIMCK_LEN, "Inner Methods Compound Keys", isk, sizeof(isk), imck,
                   sizeof(imck)) < 0)
        return -1;
    data->simck_idx++;
                                                      ...
                                                      size_t bind_len)
{
    ...
    eap_fast_write_crypto_binding((struct eap_tlv_crypto_binding_tlv *)pos, _bind, cmk);
...
static int eap_fast_parse_pac_info(struct eap_fast_pac *entry, int type, u8 *pos, size_t len)
{
    ...
    switch (type & 0x7fff)
    {
        case PAC_TYPE_CRED_LIFETIME:
            ...
            os_get_time(&now);
            wpa_printf(MSG_DEBUG,
                       "EAP-FAST: PAC-Info - CRED_LIFETIME %ld "
                       "(%ld days)",
        ...
        case PAC_TYPE_PAC_TYPE:
            ...
            if (pac_type != PAC_TYPE_TUNNEL_PAC && pac_type != PAC_TYPE_USER_AUTHORIZATION &&
...
static struct wpabuf *eap_fast_process_pac(
    struct eap_sm *sm, struct eap_fast_data *data, struct eap_method_ret *ret, u8 *pac, size_t pac_len)
{
    struct eap_peer_config *config = eap_get_config(sm);
...
static int eap_fast_encrypt_response(
    struct eap_sm *sm, struct eap_fast_data *data, struct wpabuf *resp, u8 identifier, struct wpabuf **out_data)
{
    if (resp == NULL)
        return 0;
...
static struct wpabuf *eap_fast_pac_request(void)
{
    ...
    tmp = wpabuf_alloc(sizeof(struct eap_tlv_hdr) + sizeof(struct eap_tlv_request_action_tlv) +
                                      ...
                                      struct wpabuf **out_data)
{
    ...
    if (data->current_pac == NULL && data->provisioning && !data->anon_provisioning && !tlv.pac &&
        (tlv.iresult == EAP_TLV_RESULT_SUCCESS || tlv.result == EAP_TLV_RESULT_SUCCESS))
    {
        /*
...
static void eap_fast_select_pac(struct eap_fast_data *data, const u8 *a_id, size_t a_id_len)
{
    ...
    if (data->current_pac)
    {
        ...
        wpa_hexdump_ascii(MSG_MSGDUMP, "EAP-FAST: A-ID-Info", data->current_pac->a_id_info,
...
static int eap_fast_use_pac_opaque(struct eap_sm *sm, struct eap_fast_data *data, struct eap_fast_pac *pac)
{
    ...
    if (tlv == NULL ||
        tls_connection_client_hello_ext(sm->ssl_ctx, data->ssl.conn, TLS_EXT_PAC_OPAQUE, tlv, tlv_len) < 0)
    {
        wpa_printf(MSG_DEBUG,
...
static int eap_fast_process_start(struct eap_sm *sm, struct eap_fast_data *data, u8 flags, const u8 *pos, size_t left)
{
    ...
    wpa_printf(MSG_DEBUG, "EAP-FAST: Start (server ver=%d, own ver=%d)", flags & EAP_TLS_VERSION_MASK,
               data->fast_version);
                                       ...
                                       const struct wpabuf *reqData)
{
    ...
    else
    {
        if (sm->waiting_ext_cert_check && data->pending_resp)
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "EAP-FAST: External certificate check succeeded - continue handshake");
            ...
            {
                wpa_printf(MSG_DEBUG, "EAP-FAST: External certificate check failed - force authentication failure");
                ret->methodState           = METHOD_DONE;
                ret->decision              = DECISION_FAIL;
                sm->waiting_ext_cert_check = 0;
                return NULL;
            ...
            wpa_printf(MSG_DEBUG, "EAP-FAST: Continuing to wait external server certificate validation");
            return NULL;
        }
        ...
        {
            wpa_printf(MSG_DEBUG, "EAP-FAST: Waiting external server certificate validation");
            wpabuf_clear_free(data->pending_resp);
        ...
        if (tls_connection_established(sm->ssl_ctx, data->ssl.conn))
        {
            ...
            if (data->provisioning && (!(data->provisioning_allowed & EAP_FAST_PROV_AUTH) ||
                                       tls_get_cipher(sm->ssl_ctx, data->ssl.conn, cipher, sizeof(cipher)) < 0 ||
