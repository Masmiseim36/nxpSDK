...
static void eap_fast_remove_pac(
    struct eap_fast_pac **pac_root, struct eap_fast_pac **pac_current, const u8 *a_id, size_t a_id_len, u16 pac_type)
{
...
int eap_fast_add_pac(struct eap_fast_pac **pac_root, struct eap_fast_pac **pac_current, struct eap_fast_pac *entry)
{
    ...
    if (eap_fast_copy_buf(&pac->pac_opaque, &pac->pac_opaque_len, entry->pac_opaque, entry->pac_opaque_len) < 0 ||
        eap_fast_copy_buf(&pac->pac_info, &pac->pac_info_len, entry->pac_info, entry->pac_info_len) < 0 ||
        eap_fast_copy_buf(&pac->a_id, &pac->a_id_len, entry->a_id, entry->a_id_len) < 0 ||
        eap_fast_copy_buf(&pac->i_id, &pac->i_id_len, entry->i_id, entry->i_id_len) < 0 ||
        eap_fast_copy_buf(&pac->a_id_info, &pac->a_id_info_len, entry->a_id_info, entry->a_id_info_len) < 0)
    {
        eap_fast_free_pac(pac);
        return -1;
    }
...
static const char *eap_fast_parse_pac_type(struct eap_fast_pac *pac, char *pos)
{
    ...
    if (pac->pac_type != PAC_TYPE_TUNNEL_PAC && pac->pac_type != PAC_TYPE_USER_AUTHORIZATION &&
...
static void eap_fast_write(
    char **buf, char **pos, size_t *buf_len, const char *field, const u8 *data, size_t len, int txt)
{
    size_t i, need;
    int ret;
    char *end;
