...
static void *eap_mschapv2_init(struct eap_sm *sm)
{
    ...
    {
        data->peer_challenge = os_memdup(sm->peer_challenge, MSCHAPV2_CHAL_LEN);
    ...
    {
        data->auth_challenge = os_memdup(sm->auth_challenge, MSCHAPV2_CHAL_LEN);
...
static struct wpabuf *eap_mschapv2_challenge_reply(
    struct eap_sm *sm, struct eap_mschapv2_data *data, u8 id, u8 mschapv2_id, const u8 *auth_challenge)
{
    struct wpabuf *resp;
    ...
    if (mschapv2_derive_response(identity, identity_len, password, password_len, pwhash, auth_challenge, peer_challenge,
                                 r->nt_response, data->auth_response, data->master_key))
    {
...
static int eap_mschapv2_failure_txt(struct eap_sm *sm, struct eap_mschapv2_data *data, char *txt)
{
    ...
    if (data->prev_error == ERROR_AUTHENTICATION_FAILURE && retry && config && config->phase2 &&
...
{
#ifdef CONFIG_NO_RC4
    wpa_printf(MSG_ERROR, "EAP-MSCHAPV2: RC4 not support in the build - cannot change password");
    return NULL;
#else  /* CONFIG_NO_RC4 */
    struct wpabuf *resp;
    int ms_len;
    ...
    {
        if (new_password_encrypted_with_old_nt_password_hash(new_password, new_password_len, password, password_len,
                                                             cp->encr_password))
            goto fail;
    }
    ...
    {
        if (old_nt_password_hash_encrypted_with_new_nt_password_hash(new_password, new_password_len, password,
                                                                     password_len, cp->encr_hash))
            goto fail;
    }
    ...
    generate_nt_response(data->passwd_change_challenge, cp->peer_challenge, username, username_len, new_password,
    ...
    generate_authenticator_response(new_password, new_password_len, cp->peer_challenge, data->passwd_change_challenge,
                                    username, username_len, cp->nt_response, data->auth_response);
    data->auth_response_valid = 1;
...
static u8 *eap_mschapv2_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
    ...
    if (get_asymetric_start_key(data->master_key, key, MSCHAPV2_KEY_LEN, 1, 0) < 0 ||
        get_asymetric_start_key(data->master_key, key + MSCHAPV2_KEY_LEN, MSCHAPV2_KEY_LEN, 0, 0) < 0)
    {
        os_free(key);
        return NULL;
    }
