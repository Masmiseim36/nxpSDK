...
static int eap_sim_gsm_auth(struct eap_sm *sm, struct eap_sim_data *data)
{
    ...
    if (conf->pcsc)
    {
        if (scard_gsm_auth(sm->scard_ctx, data->rand[0], data->sres[0], data->kc[0]) ||
            scard_gsm_auth(sm->scard_ctx, data->rand[1], data->sres[1], data->kc[1]) ||
            (data->num_chal > 2 && scard_gsm_auth(sm->scard_ctx, data->rand[2], data->sres[2], data->kc[2])))
        {
            wpa_printf(MSG_DEBUG,
                       "EAP-SIM: GSM SIM "
...
static int eap_sim_learn_ids(struct eap_sm *sm, struct eap_sim_data *data, struct eap_sim_attrs *attr)
{
    if (attr->next_pseudonym)
    {
        ...
        wpa_hexdump_ascii(MSG_DEBUG, "EAP-SIM: (encr) AT_NEXT_PSEUDONYM", attr->next_pseudonym,
...
static struct wpabuf *eap_sim_process_challenge(
    struct eap_sm *sm, struct eap_sim_data *data, u8 id, const struct wpabuf *reqData, struct eap_sim_attrs *attr)
{
    const u8 *identity;
    size_t identity_len;
    ...
    {
        wpa_printf(MSG_DEBUG, "EAP-SIM: Unexpected Challenge in state %s", eap_sim_state_txt(data->state));
        return eap_sim_client_error(data, id, EAP_SIM_UNABLE_TO_PROCESS_PACKET);
    }
    data->reauth = 0;
    ...
    if (os_memcmp(attr->rand, attr->rand + GSM_RAND_LEN, GSM_RAND_LEN) == 0 ||
        (attr->num_chal > 2 &&
         (os_memcmp(attr->rand, attr->rand + 2 * GSM_RAND_LEN, GSM_RAND_LEN) == 0 ||
          os_memcmp(attr->rand + GSM_RAND_LEN, attr->rand + 2 * GSM_RAND_LEN, GSM_RAND_LEN) == 0)))
    {
    ...
    eap_sim_derive_mk(identity, identity_len, data->nonce_mt, data->selected_version, data->ver_list,
                      data->ver_list_len, data->num_chal, (const u8 *)data->kc, data->mk);
    ...
    struct eap_sm *sm, struct eap_sim_data *data, u8 id, const struct wpabuf *reqData, struct eap_sim_attrs *attr)
{
    ...
    os_memcpy(data->reauth_mac, attr->mac, EAP_SIM_MAC_LEN);
#endif /* TEST_FUZZ */
    wpa_hexdump(MSG_DEBUG, "EAP-SIM: Server MAC", data->reauth_mac, EAP_SIM_MAC_LEN);
    ...
    {
        wpa_printf(MSG_INFO, "EAP-SIM: (encr) No%s%s in reauth packet", !eattr.nonce_s ? " AT_NONCE_S" : "",
    ...
    eap_sim_derive_keys_reauth(data->counter, data->reauth_id, data->reauth_id_len, data->nonce_s, data->mk, data->msk,
                               data->emsk);
    eap_sim_clear_identities(sm, data, CLEAR_REAUTH_ID | CLEAR_EAP_ID);
    eap_sim_learn_ids(sm, data, &eattr);
...
static struct wpabuf *eap_sim_process(struct eap_sm *sm,
                                      void *priv,
