...
static struct wpabuf *eap_tls_failure(
    struct eap_sm *sm, struct eap_tls_data *data, struct eap_method_ret *ret, int res, struct wpabuf *resp, u8 id)
{
                                      ...
                                      const struct wpabuf *reqData)
{
    ...
    if (sm->waiting_ext_cert_check && data->pending_resp)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "EAP-TLS: External certificate check succeeded - continue handshake");
        ...
        {
            wpa_printf(MSG_DEBUG, "EAP-TLS: External certificate check failed - force authentication failure");
            ret->methodState           = METHOD_DONE;
            ret->decision              = DECISION_FAIL;
            sm->waiting_ext_cert_check = 0;
            return NULL;
        ...
        wpa_printf(MSG_DEBUG, "EAP-TLS: Continuing to wait external server certificate validation");
        return NULL;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "EAP-TLS: Waiting external server certificate validation");
        wpabuf_free(data->pending_resp);
    ...
    if (res == 2 && data->ssl.tls_v13 && wpabuf_len(resp) == 1 && *wpabuf_head_u8(resp) == 0)
    {
...
int eap_peer_unauth_tls_register(void)
{
    ...
    eap = eap_peer_method_alloc(EAP_PEER_METHOD_INTERFACE_VERSION, EAP_VENDOR_UNAUTH_TLS, EAP_VENDOR_TYPE_UNAUTH_TLS,
                                "UNAUTH-TLS");
    if (eap == NULL)
        return -1;
...
int eap_peer_wfa_unauth_tls_register(void)
{
    ...
    eap = eap_peer_method_alloc(EAP_PEER_METHOD_INTERFACE_VERSION, EAP_VENDOR_WFA_NEW, EAP_VENDOR_WFA_UNAUTH_TLS,
                                "WFA-UNAUTH-TLS");
    if (eap == NULL)
        return -1;
