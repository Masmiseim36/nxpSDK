...
static struct wpabuf *eap_tls_msg_alloc(enum eap_type type, size_t payload_len, u8 code, u8 identifier)
{
...
static void eap_tls_cert_params_from_conf(struct tls_connection_params *params, struct eap_peer_cert_config *config)
{
...
static void eap_tls_params_from_conf2m(struct tls_connection_params *params, struct eap_peer_config *config)
{
                                    ...
                                    int phase2)
{
    ...
    if (sm->workaround && data->eap_type != EAP_TYPE_FAST && data->eap_type != EAP_TYPE_TEAP)
    {
        /*
    ...
    if (data->eap_type == EAP_TYPE_FAST || data->eap_type == EAP_TYPE_TEAP || data->eap_type == EAP_TYPE_TTLS ||
     ...
     * file as-is.
     */
    if (eap_tls_check_blob(sm, &params->ca_cert, &params->ca_cert_blob, &params->ca_cert_blob_len) ||
        eap_tls_check_blob(sm, &params->client_cert, &params->client_cert_blob, &params->client_cert_blob_len) ||
        eap_tls_check_blob(sm, &params->private_key, &params->private_key_blob, &params->private_key_blob_len) ||
        eap_tls_check_blob(sm, &params->dh_file, &params->dh_blob, &params->dh_blob_len))
    {
 ...
 */
u8 *eap_peer_tls_derive_key(
    struct eap_sm *sm, struct eap_ssl_data *data, const char *label, const u8 *context, size_t context_len, size_t len)
{
    u8 *out;
    ...
    if (tls_connection_export_key(data->ssl_ctx, data->conn, label, context, context_len, out, len))
    {
        os_free(out);
        return NULL;
    }
...
u8 *eap_peer_tls_derive_session_id(struct eap_sm *sm, struct eap_ssl_data *data, u8 eap_type, size_t *len)
{
    ...
    if (data->tls_v13)
    {
        ...
        *len = 1 + 64;
        ...
        if (!id)
            return NULL;
        method_id = eap_peer_tls_derive_key(sm, data, "EXPORTER_EAP_TLS_Method-Id", context, 1, 64);
        if (!method_id)
        {
            os_free(id);
            return NULL;
    ...
    if (tls_connection_get_random(sm->ssl_ctx, data->conn, &keys) || keys.client_random == NULL ||
        keys.server_random == NULL)
        return NULL;
...
static int eap_peer_tls_reassemble_fragment(struct eap_ssl_data *data, const struct wpabuf *in_data)
{
    ...
    if (tls_in_len + in_len == 0)
    {
        ...
        wpa_printf(MSG_WARNING,
                   ...
                   (unsigned long)data->tls_in_left, (unsigned long)tls_in_len, (unsigned long)in_len);
        eap_peer_tls_reset_input(data);
        return -1;
    }
 ...
 */
static const struct wpabuf *eap_peer_tls_data_reassemble(struct eap_ssl_data *data,
 ...
 */
static int eap_tls_process_output(
    struct eap_ssl_data *data, enum eap_type eap_type, int peap_version, u8 id, int ret, struct wpabuf **out_data)
{
    size_t len;
    u8 *flags;
    ...
    length_included =
        data->tls_out_pos == 0 && (wpabuf_len(data->tls_out) > data->tls_out_limit || data->include_tls_length);
                                ...
                                struct wpabuf **out_data)
{
    ...
    *out_data = NULL;
    ...
    if (data->tls_out == NULL || wpabuf_len(data->tls_out) == 0)
    {
        ...
        if (tls_get_version(data->ssl_ctx, data->conn, buf, sizeof(buf)) == 0)
        {
 ...
 */
struct wpabuf *eap_peer_tls_build_ack(u8 id, enum eap_type eap_type, int peap_version)
{
    struct wpabuf *resp;
...
int eap_peer_tls_status(struct eap_sm *sm, struct eap_ssl_data *data, char *buf, size_t buflen, int verbose)
{
    ...
    ret = os_snprintf(buf + len, buflen - len,
                      ...
                      version, name, tls_connection_resumed(data->ssl_ctx, data->conn));
