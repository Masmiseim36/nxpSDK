...
static void *eap_ttls_init(struct eap_sm *sm)
{
    ...
    if (config && config->phase2)
    {
        ...
        while ((token = cstr_token(config->phase2, " \t", &last)))
        {
            ...
            if (selected_non_eap)
            {
                wpa_printf(MSG_ERROR, "EAP-TTLS: Only one Phase2 type can be specified");
                eap_ttls_deinit(sm, data);
                return NULL;
            }
        ...
        if (os_strstr(config->phase2, "autheap="))
        {
            if (selected_non_eap)
            {
                wpa_printf(MSG_ERROR, "EAP-TTLS: Both auth= and autheap= params cannot be specified");
                eap_ttls_deinit(sm, data);
                return NULL;
            }
            selected          = "EAP";
...
static u8 *eap_ttls_avp_add(
    u8 *start, u8 *avphdr, u32 avp_code, u32 vendor_id, int mandatory, const u8 *data, size_t len)
{
    u8 *pos;
...
static int eap_ttls_v0_derive_key(struct eap_sm *sm, struct eap_ttls_data *data)
{
    ...
    eap_ttls_free_key(data);
    data->key_data =
        eap_peer_tls_derive_key(sm, &data->ssl, label, context, context_len, EAP_TLS_KEY_LEN + EAP_EMSK_LEN);
    if (!data->key_data)
    {
...
{
    return eap_peer_tls_derive_key(sm, &data->ssl, "ttls challenge", NULL, 0, len);
}
#endif /* CONFIG_FIPS */
...
static void eap_ttls_phase2_select_eap_method(struct eap_ttls_data *data, int vendor, enum eap_type method)
{
    ...
    {
        if (data->phase2_eap_types[i].vendor != vendor || data->phase2_eap_types[i].method != method)
            continue;
                                       ...
                                       struct wpabuf **resp)
{
    ...
    *resp = data->phase2_method->process(sm, data->phase2_priv, &iret, &msg);
    if ((iret.methodState == METHOD_DONE || iret.methodState == METHOD_MAY_CONT) &&
        (iret.decision == DECISION_UNCOND_SUCC || iret.decision == DECISION_COND_SUCC ||
...
{
#ifdef EAP_TNC
    if (data->tnc_started && data->phase2_method && data->phase2_priv && vendor == EAP_VENDOR_IETF &&
        method == EAP_TYPE_TNC && data->phase2_eap_type.method == EAP_TYPE_TNC)
        return eap_ttls_phase2_eap_process(sm, data, ret, hdr, len, resp);

    if (data->ready_for_tnc && !data->tnc_started && vendor == EAP_VENDOR_IETF && method == EAP_TYPE_TNC)
    {
        wpa_printf(MSG_DEBUG,
                   "EAP-TTLS: Start TNC after completed "
                   "EAP method");
        data->tnc_started = 1;
    }
    ...
    if (vendor != data->phase2_eap_type.vendor || method != data->phase2_eap_type.method ||
    ...
    {
        wpa_printf(MSG_INFO, "EAP-TTLS: failed to initialize Phase 2 EAP method %u:%u", vendor, method);
        return -1;
    }
                                       ...
                                       struct wpabuf **resp)
{
    ...
    switch (*pos)
    {
        ...
        case EAP_TYPE_EXPANDED:
            ...
            {
                wpa_printf(MSG_INFO, "EAP-TTLS: Too short Phase 2 request (expanded header) (len=%lu)",
                           (unsigned long)len);
                return -1;
            }
            if (eap_ttls_phase2_request_eap_method(sm, data, ret, hdr, len, WPA_GET_BE24(pos + 1),
                                                   WPA_GET_BE32(pos + 4), resp) < 0)
                return -1;
            break;
        default:
            if (eap_ttls_phase2_request_eap_method(sm, data, ret, hdr, len, EAP_VENDOR_IETF, *pos, resp) < 0)
                return -1;
            break;
    }
                                            ...
                                            struct wpabuf **resp)
{
    ...
    pos = eap_ttls_avp_add(buf, pos, RADIUS_ATTR_MS_CHAP_CHALLENGE, RADIUS_VENDOR_ID_MICROSOFT, 1, challenge,
                           EAP_TTLS_MSCHAPV2_CHALLENGE_LEN);

    /* MS-CHAP2-Response */
    pos         = eap_ttls_avp_hdr(pos, RADIUS_ATTR_MS_CHAP2_RESPONSE, RADIUS_VENDOR_ID_MICROSOFT, 1,
    ...
    *pos++      = 0; /* Flags */
    if (os_get_random(pos, EAP_TTLS_MSCHAPV2_CHALLENGE_LEN) < 0)
    {
        ...
        wpabuf_clear_free(msg);
        wpa_printf(MSG_ERROR,
                   "EAP-TTLS/MSCHAPV2: Failed to get "
    ...
    pos += 8;
    if (mschapv2_derive_response(identity, identity_len, password, password_len, pwhash, challenge, peer_challenge, pos,
                                          ...
                                          struct wpabuf **resp)
{
    ...
    /* MS-CHAP-Response */
    pos         = eap_ttls_avp_hdr(pos, RADIUS_ATTR_MS_CHAP_RESPONSE, RADIUS_VENDOR_ID_MICROSOFT, 1,
    ...
    *pos++      = 1;       /* Flags: Use NT style passwords */
    ...
    if (pwhash)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "EAP-TTLS/MSCHAP: Failed derive password hash");
            wpabuf_clear_free(msg);
            os_free(challenge);
            return -1;
        }
    ...
    {
        /* NT-Response */
        if (nt_challenge_response(challenge, password, password_len, pos))
        {
            wpa_printf(MSG_ERROR, "EAP-TTLS/MSCHAP: Failed derive password");
            wpabuf_clear_free(msg);
            os_free(challenge);
            return -1;
        }
                                   ...
                                   struct wpabuf **resp)
{
    ...
    if (phase2_type == EAP_TTLS_PHASE2_MSCHAPV2 || phase2_type == EAP_TTLS_PHASE2_MSCHAP ||
        phase2_type == EAP_TTLS_PHASE2_PAP || phase2_type == EAP_TTLS_PHASE2_CHAP)
    {
...
static int eap_ttls_encrypt_response(
    struct eap_sm *sm, struct eap_ttls_data *data, struct wpabuf *resp, u8 identifier, struct wpabuf **out_data)
{
    if (resp == NULL)
        return 0;
    ...
    {
        wpa_printf(MSG_INFO,
                   "EAP-TTLS: Failed to encrypt a Phase 2 "
                   "frame");
        wpabuf_clear_free(resp);
        return -1;
    }
                                       ...
                                       struct wpabuf **resp)
{
    ...
    {
        wpa_printf(MSG_WARNING,
                   "EAP-TTLS: No EAP Message in the "
                   "packet - dropped");
        return -1;
    }
                                            ...
                                            struct ttls_parse_avp *parse)
{
...
#endif /* EAP_TNC */
        wpa_printf(MSG_WARNING,
                   "EAP-TTLS: no MS-CHAP2-Success AVP "
    ...
    {
        wpa_printf(MSG_WARNING,
                   "EAP-TTLS: Invalid authenticator "
                                      ...
                                      struct wpabuf **out_data)
{
    ...
    else if (config->pending_req_identity || config->pending_req_password || config->pending_req_otp ||
             config->pending_req_new_password || config->pending_req_sim)
    {
        wpabuf_clear_free(data->pending_phase2_req);
...
static int eap_ttls_implicit_identity_request(
    struct eap_sm *sm, struct eap_ttls_data *data, struct eap_method_ret *ret, u8 identifier, struct wpabuf **out_data)
{
    int retval = 0;
    struct eap_hdr *hdr;
    struct wpabuf *resp;
...
static int eap_ttls_phase2_start(
    struct eap_sm *sm, struct eap_ttls_data *data, struct eap_method_ret *ret, u8 identifier, struct wpabuf **out_data)
{
    data->phase2_start = 0;
                            ...
                            struct wpabuf **out_data)
{
    ...
    if (data->ssl.tls_v13 && wpabuf_len(in_decrypted) == 1 && *wpabuf_head_u8(in_decrypted) == 0)
    {
                                      ...
                                      struct wpabuf **out_data)
{
    ...
    if (sm->waiting_ext_cert_check && data->pending_resp)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "EAP-TTLS: External certificate check succeeded - continue handshake");
        ...
        {
            wpa_printf(MSG_DEBUG, "EAP-TTLS: External certificate check failed - force authentication failure");
            ret->methodState           = METHOD_DONE;
            ret->decision              = DECISION_FAIL;
            sm->waiting_ext_cert_check = 0;
            return 0;
        ...
        wpa_printf(MSG_DEBUG, "EAP-TTLS: Continuing to wait external server certificate validation");
        return 0;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "EAP-TTLS: Waiting external server certificate validation");
        wpabuf_clear_free(data->pending_resp);
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "EAP-TTLS: TLS done, proceed to "
                   "Phase 2");
        if (data->resuming)
        {
            wpa_printf(MSG_DEBUG,
                       "EAP-TTLS: fast reauth - may "
                       "skip Phase 2");
            ret->decision    = DECISION_COND_SUCC;
            ret->methodState = METHOD_MAY_CONT;
        }
...
static void eap_ttls_check_auth_status(struct eap_sm *sm, struct eap_ttls_data *data, struct eap_method_ret *ret)
{
    ...
    else if (data->decision_succ != DECISION_FAIL && data->phase2_success && !data->ssl.tls_out)
    {
        /*
         ...
         */
        wpa_printf(MSG_DEBUG, "EAP-TTLS: Restore success decision after fragmented frame sent completely");
...
static void eap_ttls_deinit_for_reauth(struct eap_sm *sm, void *priv)
{
    ...
    if (data->phase2_priv && data->phase2_method && data->phase2_method->deinit_for_reauth)
