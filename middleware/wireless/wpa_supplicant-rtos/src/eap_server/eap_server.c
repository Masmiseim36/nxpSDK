...
static struct wpabuf *eap_sm_buildInitiateReauthStart(struct eap_sm *sm, u8 id)
{
    ...
    msg = eap_msg_alloc(EAP_VENDOR_IETF, (enum eap_type)EAP_ERP_TYPE_REAUTH_START, plen, EAP_CODE_INITIATE, id);
    if (msg == NULL)
        return NULL;
...
SM_STATE(EAP, PICK_UP_METHOD)
{
    ...
    wpa_msg(sm->cfg->msg_ctx, MSG_INFO, WPA_EVENT_EAP_PROPOSED_METHOD "method=%u", sm->currentMethod);
}
...
SM_STATE(EAP, IDLE)
{
    ...
    sm->eap_if.retransWhile =
        eap_sm_calculateTimeout(sm, sm->retransCount, sm->eap_if.eapSRTT, sm->eap_if.eapRTTVAR, sm->methodTimeout);
}
...
SM_STATE(EAP, RETRANSMIT)
{
    ...
    wpa_msg(sm->cfg->msg_ctx, MSG_INFO, WPA_EVENT_EAP_RETRANSMIT MACSTR, MAC2STR(sm->peer_addr));
}
...
static void eap_server_erp_init(struct eap_sm *sm)
{
    ...
    if (hmac_sha256_kdf(sm->eap_if.eapSessionId, sm->eap_if.eapSessionIdLen, "EMSK", len, sizeof(len), EMSKname,
                        EAP_EMSK_NAME_LEN) < 0)
    {
...
try_another_method:
    ...
    wpa_msg(sm->cfg->msg_ctx, MSG_INFO, WPA_EVENT_EAP_PROPOSED_METHOD "vendor=%u method=%u", vendor, sm->currentMethod);
...
SM_STATE(EAP, TIMEOUT_FAILURE)
{
    ...
    wpa_msg(sm->cfg->msg_ctx, MSG_INFO, WPA_EVENT_EAP_TIMEOUT_FAILURE MACSTR, MAC2STR(sm->peer_addr));
}
...
SM_STATE(EAP, FAILURE)
{
    ...
    wpa_msg(sm->cfg->msg_ctx, MSG_INFO, WPA_EVENT_EAP_FAILURE MACSTR, MAC2STR(sm->peer_addr));
}
...
SM_STATE(EAP, SUCCESS)
{
    ...
    wpa_msg(sm->cfg->msg_ctx, MSG_INFO, WPA_EVENT_EAP_SUCCESS MACSTR, MAC2STR(sm->peer_addr));
}
...
static void erp_send_finish_reauth(
    struct eap_sm *sm, struct eap_server_erp_key *erp, u8 id, u8 flags, u16 seq, const char *nai)
{
    size_t plen;
    struct wpabuf *msg;
    ...
    if (hash_len)
        plen += 1 + hash_len;
    msg = eap_msg_alloc(EAP_VENDOR_IETF, (enum eap_type)EAP_ERP_TYPE_REAUTH, plen, EAP_CODE_FINISH, id);
    if (msg == NULL)
        return;
...
SM_STATE(EAP, INITIATE_RECEIVED)
{
    ...
    pos = eap_hdr_validate(EAP_VENDOR_IETF, (enum eap_type)EAP_ERP_TYPE_REAUTH, sm->eap_if.eapRespData, &len);
    if (pos == NULL)
    {
...
SM_STATE(EAP, RETRANSMIT2)
{
    ...
    wpa_msg(sm->cfg->msg_ctx, MSG_INFO, WPA_EVENT_EAP_RETRANSMIT2 MACSTR, MAC2STR(sm->peer_addr));
}
...
SM_STATE(EAP, TIMEOUT_FAILURE2)
{
    ...
    wpa_msg(sm->cfg->msg_ctx, MSG_INFO, WPA_EVENT_EAP_TIMEOUT_FAILURE2 MACSTR, MAC2STR(sm->peer_addr));
}
...
SM_STATE(EAP, FAILURE2)
{
    ...
    wpa_msg(sm->cfg->msg_ctx, MSG_INFO, WPA_EVENT_EAP_FAILURE2 MACSTR, MAC2STR(sm->peer_addr));
}
...
SM_STATE(EAP, SUCCESS2)
{
    ...
    wpa_msg(sm->cfg->msg_ctx, MSG_INFO, WPA_EVENT_EAP_SUCCESS2 MACSTR, MAC2STR(sm->peer_addr));
}
...
SM_STEP(EAP)
{
    ...
    {
        if (sm->num_rounds_short == sm->cfg->max_auth_rounds_short + 1)
        {
            wpa_printf(MSG_DEBUG, "EAP: more than %d authentication rounds (short) - abort",
                ...
                if (sm->cfg->backend_auth)
                {
                    if (!sm->rxResp)
                        SM_ENTER(EAP, SELECT_ACTION);
                                            ...
                                            (sm->respMethod == EAP_TYPE_EXPANDED && sm->respVendor == EAP_VENDOR_IETF &&
            ...
            case EAP_RECEIVED:
                ...
                else
                {
                    wpa_printf(MSG_DEBUG,
                               ...
                               sm->rxResp, sm->respId, sm->currentId, sm->respMethod, sm->currentMethod);
...
static void eap_sm_parseEapResp(struct eap_sm *sm, const struct wpabuf *resp)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               sm->rxResp, sm->rxInitiate, sm->respId, sm->respMethod, sm->respVendor, sm->respVendorMethod);
}
...
void eap_sm_process_nak(struct eap_sm *sm, const u8 *nak_list, size_t len)
{
    ...
    while (i < EAP_MAX_METHODS &&
           (sm->user->methods[i].vendor != EAP_VENDOR_IETF || sm->user->methods[i].method != EAP_TYPE_NONE))
    {
    ...
    wpa_hexdump(MSG_MSGDUMP, "EAP: new list of configured methods", (u8 *)sm->user->methods,
                EAP_MAX_METHODS * sizeof(sm->user->methods[0]));
}
    ...
    if (sm->identity == NULL || sm->currentId == -1)
    {
        *vendor         = EAP_VENDOR_IETF;
        ...
        sm->update_user = true;
             ...
             (sm->user->methods[idx].vendor != EAP_VENDOR_IETF || sm->user->methods[idx].method != EAP_TYPE_NONE))
    {
...
static int eap_sm_Policy_getDecision(struct eap_sm *sm)
{
    ...
    if ((sm->user == NULL || sm->update_user) && sm->identity && !sm->start_reauth)
    {
         ...
         */
        int id_req = 0;
        if (sm->user && sm->currentMethod == EAP_TYPE_IDENTITY && sm->user->methods[0].vendor == EAP_VENDOR_IETF &&
...
void eap_erp_update_identity(struct eap_sm *sm, const u8 *eap, size_t len)
{
    ...
    if (erp_parse_tlvs(pos, end, &parse, 1) < 0 || !parse.keyname)
        return;
    wpa_hexdump_ascii(MSG_DEBUG, "EAP: Update identity based on EAP-Initiate/Re-auth keyName-NAI", parse.keyname,
                      parse.keyname_len);
    os_free(sm->identity);
...
#ifdef CONFIG_TESTING_OPTIONS
void eap_server_mschap_rx_callback(struct eap_sm *sm,
                                   const char *source,
                                   const u8 *username,
                                   size_t username_len,
                                   const u8 *challenge,
