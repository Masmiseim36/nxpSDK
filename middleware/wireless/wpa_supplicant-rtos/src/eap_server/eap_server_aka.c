...
static int eap_aka_check_identity_reauth(struct eap_sm *sm, struct eap_aka_data *data, const char *username)
{
    ...
    data->reauth = eap_sim_db_get_reauth_entry(sm->cfg->eap_sim_db_priv, username);
    if (data->reauth == NULL)
    {
...
static void eap_aka_check_identity(struct eap_sm *sm, struct eap_aka_data *data)
{
    ...
    if ((data->eap_method == EAP_TYPE_AKA_PRIME && username[0] == EAP_AKA_PRIME_PSEUDONYM_PREFIX) ||
        (data->eap_method == EAP_TYPE_AKA && username[0] == EAP_AKA_PSEUDONYM_PREFIX))
    {
        const char *permanent;
        wpa_printf(MSG_DEBUG, "EAP-AKA: Pseudonym username '%s'", username);
        permanent = eap_sim_db_get_permanent(sm->cfg->eap_sim_db_priv, username);
        if (permanent == NULL)
        {
            os_free(username);
            wpa_printf(MSG_DEBUG,
...
static void eap_aka_add_checkcode(struct eap_aka_data *data, struct eap_sim_msg *msg)
{
    ...
    eap_sim_msg_add(msg, EAP_SIM_AT_CHECKCODE, 0, hash,
                    data->eap_method == EAP_TYPE_AKA_PRIME ? EAP_AKA_PRIME_CHECKCODE_LEN : EAP_AKA_CHECKCODE_LEN);
}
...
static struct wpabuf *eap_aka_build_identity(struct eap_sm *sm, struct eap_aka_data *data, u8 id)
{
    ...
    else if (data->identity_round > 3)
    {
        ...
        return NULL;
             ...
             (sm->identity[0] == EAP_AKA_REAUTH_ID_PREFIX || sm->identity[0] == EAP_AKA_PRIME_REAUTH_ID_PREFIX))
    {
...
static int eap_aka_build_encr(
    struct eap_sm *sm, struct eap_aka_data *data, struct eap_sim_msg *msg, u16 counter, const u8 *nonce_s)
{
    os_free(data->next_pseudonym);
    ...
    else if (!nonce_s)
    {
        data->next_pseudonym = eap_sim_db_get_next_pseudonym(
            sm->cfg->eap_sim_db_priv, data->eap_method == EAP_TYPE_AKA_PRIME ? EAP_SIM_DB_AKA_PRIME : EAP_SIM_DB_AKA);
    }
    else
    {
    ...
    if (data->next_pseudonym)
    {
        ...
        eap_sim_msg_add(msg, EAP_SIM_AT_NEXT_PSEUDONYM, os_strlen(data->next_pseudonym), (u8 *)data->next_pseudonym,
    ...
    if (data->next_reauth_id)
    {
        ...
        eap_sim_msg_add(msg, EAP_SIM_AT_NEXT_REAUTH_ID, os_strlen(data->next_reauth_id), (u8 *)data->next_reauth_id,
...
static struct wpabuf *eap_aka_build_reauth(struct eap_sm *sm, struct eap_aka_data *data, u8 id)
{
    ...
    if (data->eap_method == EAP_TYPE_AKA_PRIME)
    {
        eap_aka_prime_derive_keys_reauth(data->k_re, data->counter, sm->identity, sm->identity_len, data->nonce_s,
                                         data->msk, data->emsk);
    }
    else
    {
        eap_sim_derive_keys(data->mk, data->k_encr, data->k_aut, data->msk, data->emsk);
        eap_sim_derive_keys_reauth(data->counter, sm->identity, sm->identity_len, data->nonce_s, data->mk, data->msk,
                                   data->emsk);
    }
    ...
    os_memcpy(data->reauth_mac, wpabuf_head_u8(buf) + wpabuf_len(buf) - EAP_SIM_MAC_LEN, EAP_SIM_MAC_LEN);
...
static bool eap_aka_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    struct eap_aka_data *data = priv;
    const u8 *pos;
    size_t len;
...
static void eap_aka_determine_identity(struct eap_sm *sm, struct eap_aka_data *data)
{
    ...
    if (((data->eap_method == EAP_TYPE_AKA_PRIME && username[0] == EAP_AKA_PRIME_REAUTH_ID_PREFIX) ||
         (data->eap_method == EAP_TYPE_AKA && username[0] == EAP_AKA_REAUTH_ID_PREFIX)) &&
    ...
    else if ((data->eap_method == EAP_TYPE_AKA_PRIME && username[0] == EAP_AKA_PRIME_PERMANENT_PREFIX) ||
             (data->eap_method == EAP_TYPE_AKA && username[0] == EAP_AKA_PERMANENT_PREFIX))
    {
...
static void eap_aka_fullauth(struct eap_sm *sm, struct eap_aka_data *data)
{
    ...
    res = eap_sim_db_get_aka_auth(sm->cfg->eap_sim_db_priv, data->permanent, data->rand, data->autn, data->ik, data->ck,
    ...
    if (data->permanent[0] == EAP_AKA_PERMANENT_PREFIX || data->permanent[0] == EAP_AKA_PRIME_PERMANENT_PREFIX)
    ...
    if (data->eap_method == EAP_TYPE_AKA_PRIME)
    {
        eap_aka_prime_derive_keys(sm->identity, identity_len, data->ik, data->ck, data->k_encr, data->k_aut, data->k_re,
                                  data->msk, data->emsk);
    }
    else
    {
...
static int eap_aka_verify_mac(
    struct eap_aka_data *data, const struct wpabuf *req, const u8 *mac, const u8 *extra, size_t extra_len)
{
    if (data->eap_method == EAP_TYPE_AKA_PRIME)
                                      ...
                                      struct eap_sim_attrs *attr)
{
     ...
     */
    if (attr->res == NULL || attr->res_len < data->res_len || attr->res_len_bits != data->res_len * 8 ||
        os_memcmp_const(attr->res, data->res, data->res_len) != 0)
    {
        wpa_printf(MSG_WARNING,
                   ...
                   (unsigned long)attr->res_len, (unsigned long)attr->res_len_bits, (unsigned long)data->res_len * 8);
        data->notification = EAP_SIM_GENERAL_FAILURE_BEFORE_AUTH;
        eap_aka_state(data, NOTIFICATION);
        return;
    }
    ...
    if (data->next_pseudonym)
    {
        eap_sim_db_add_pseudonym(sm->cfg->eap_sim_db_priv, data->permanent, data->next_pseudonym);
        data->next_pseudonym = NULL;
    }
    if (data->next_reauth_id)
    {
        ...
        {
#ifdef EAP_SERVER_AKA_PRIME
            eap_sim_db_add_reauth_prime(sm->cfg->eap_sim_db_priv, data->permanent, data->next_reauth_id,
                                        data->counter + 1, data->k_encr, data->k_aut, data->k_re);
#endif /* EAP_SERVER_AKA_PRIME */
        }
        else
        {
            eap_sim_db_add_reauth(sm->cfg->eap_sim_db_priv, data->permanent, data->next_reauth_id, data->counter + 1,
                                  data->mk);
        }
        data->next_reauth_id = NULL;
    }
                                         ...
                                         struct eap_sim_attrs *attr)
{
    ...
    if (!data->auts_reported &&
        eap_sim_db_resynchronize(sm->cfg->eap_sim_db_priv, data->permanent, attr->auts, data->rand))
    {
...
static u8 *eap_aka_get_session_id(struct eap_sm *sm, void *priv, size_t *len)
{
    ...
    else
    {
        ...
        os_memcpy(id + 1 + EAP_SIM_NONCE_S_LEN, data->reauth_mac, EAP_SIM_MAC_LEN);
    }
