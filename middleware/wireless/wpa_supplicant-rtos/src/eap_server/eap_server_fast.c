...
static enum eap_type eap_fast_req_failure(struct eap_sm *sm, struct eap_fast_data *data)
{
...
static void eap_fast_derive_key_auth(struct eap_sm *sm, struct eap_fast_data *data)
{
    ...
    sks = eap_fast_derive_key(sm->cfg->ssl_ctx, data->ssl.conn, EAP_FAST_SKS_LEN);
    if (sks == NULL)
    {
        wpa_printf(MSG_DEBUG,
...
static void eap_fast_derive_key_provisioning(struct eap_sm *sm, struct eap_fast_data *data)
{
    ...
    data->key_block_p = (struct eap_fast_key_block_provisioning *)eap_fast_derive_key(sm->cfg->ssl_ctx, data->ssl.conn,
...
static void *eap_fast_init(struct eap_sm *sm)
{
    ...
    if (tls_connection_set_cipher_list(sm->cfg->ssl_ctx, data->ssl.conn, ciphers) < 0)
    {
        wpa_printf(MSG_INFO,
    ...
    if (tls_connection_set_session_ticket_cb(sm->cfg->ssl_ctx, data->ssl.conn, eap_fast_session_ticket_cb, data) < 0)
    {
        wpa_printf(MSG_INFO,
    ...
    os_memcpy(data->pac_opaque_encr, sm->cfg->pac_opaque_encr_key, sizeof(data->pac_opaque_encr));
    ...
    data->srv_id = os_memdup(sm->cfg->eap_fast_a_id, sm->cfg->eap_fast_a_id_len);
    if (data->srv_id == NULL)
    {
        eap_fast_reset(sm, data);
        return NULL;
...
static struct wpabuf *eap_fast_build_start(struct eap_sm *sm, struct eap_fast_data *data, u8 id)
{
    ...
    req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_FAST, 1 + sizeof(struct pac_tlv_hdr) + data->srv_id_len,
                        EAP_CODE_REQUEST, id);
    if (req == NULL)
    {
...
static int eap_fast_phase1_done(struct eap_sm *sm, struct eap_fast_data *data)
{
    ...
    if (tls_get_cipher(sm->cfg->ssl_ctx, data->ssl.conn, cipher, sizeof(cipher)) < 0)
    {
        wpa_printf(MSG_DEBUG,
...
static struct wpabuf *eap_fast_build_pac(struct eap_sm *sm, struct eap_fast_data *data)
{
    ...
    *pos++ = 4;
    ...
    buf_len = sizeof(*pac_tlv) + sizeof(struct pac_tlv_hdr) + EAP_FAST_PAC_KEY_LEN + sizeof(struct pac_tlv_hdr) +
              pac_len + data->srv_id_len + srv_id_info_len + 100 + sizeof(*result);
...
static int eap_fast_encrypt_phase2(struct eap_sm *sm, struct eap_fast_data *data, struct wpabuf *plain, int piggyback)
{
    ...
    if (data->ssl.tls_out && piggyback)
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   "used=%d)",
                   (int)wpabuf_len(encr), (int)wpabuf_len(data->ssl.tls_out), (int)data->ssl.tls_out_pos);
...
static bool eap_fast_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    const u8 *pos;
    size_t len;
...
static int eap_fast_phase2_init(struct eap_sm *sm, struct eap_fast_data *data, int vendor, enum eap_type eap_type)
{
    if (data->phase2_priv && data->phase2_method)
    {
        data->phase2_method->reset(sm, data->phase2_priv);
...
static void eap_fast_process_phase2_response(struct eap_sm *sm, struct eap_fast_data *data, u8 *in_data, size_t in_len)
{
    ...
    if (in_len > sizeof(*hdr) && *pos == EAP_TYPE_NAK)
    {
            ...
            sm->user->methods[sm->user_eap_method_index].method != EAP_TYPE_NONE)
        {
            ...
            wpa_printf(MSG_DEBUG, "EAP-FAST: try EAP type %u:%u", next_vendor, next_type);
        }
        else
        {
            next_vendor = EAP_VENDOR_IETF;
...
static int eap_fast_pac_type(u8 *pac, size_t len, u16 type)
{
    ...
    return be_to_host16(tlv->tlv_type) == PAC_TYPE_PAC_TYPE && be_to_host16(tlv->length) == 2 &&
...
static void eap_fast_process_phase2_tlvs(struct eap_sm *sm, struct eap_fast_data *data, struct wpabuf *in_data)
{
    ...
    if (check_crypto_binding)
    {
        ...
        if (data->anon_provisioning && sm->cfg->eap_fast_prov != ANON_PROV && sm->cfg->eap_fast_prov != BOTH_PROV)
        {
            wpa_printf(MSG_DEBUG,
        ...
        if (sm->cfg->eap_fast_prov != AUTH_PROV && sm->cfg->eap_fast_prov != BOTH_PROV &&
        ...
        if (data->anon_provisioning || (tlv.request_action == EAP_TLV_ACTION_PROCESS_TLV &&
                                        eap_fast_pac_type(tlv.pac, tlv.pac_len, PAC_TYPE_TUNNEL_PAC)))
        {
            wpa_printf(MSG_DEBUG,
...
static int eap_fast_process_phase1(struct eap_sm *sm, struct eap_fast_data *data)
{
    ...
    if (!tls_connection_established(sm->cfg->ssl_ctx, data->ssl.conn) || wpabuf_len(data->ssl.tls_out) > 0)
        return 1;
...
{
    struct eap_fast_data *data = priv;
    if (eap_server_tls_process(sm, &data->ssl, respData, data, EAP_TYPE_FAST, eap_fast_process_version,
