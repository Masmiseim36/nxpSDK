...
static void *eap_mschapv2_init(struct eap_sm *sm)
{
    ...
    {
        data->peer_challenge = os_memdup(sm->peer_challenge, CHALLENGE_LEN);
...
static struct wpabuf *eap_mschapv2_build_success_req(struct eap_sm *sm, struct eap_mschapv2_data *data, u8 id)
{
    ...
    wpabuf_put_u8(req, 'S');
    wpabuf_put_u8(req, '=');
    wpa_snprintf_hex_uppercase(wpabuf_put(req, sizeof(data->auth_response) * 2), sizeof(data->auth_response) * 2 + 1,
...
static bool eap_mschapv2_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    struct eap_mschapv2_data *data = priv;
...
static void eap_mschapv2_process_response(struct eap_sm *sm, struct eap_mschapv2_data *data, struct wpabuf *respData)
{
    ...
    {
        res = generate_nt_response_pwhash(data->auth_challenge, peer_challenge, username, username_len,
                                          sm->user->password, expected);
    }
    else
    {
        res = generate_nt_response(data->auth_challenge, peer_challenge, username, username_len, sm->user->password,
    ...
    if (os_memcmp_const(nt_response, expected, 24) == 0)
    {
        ...
        if (generate_authenticator_response_pwhash(pw_hash, peer_challenge, data->auth_challenge, username,
...
static u8 *eap_mschapv2_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
    ...
    *len = 2 * MSCHAPV2_KEY_LEN;
    ...
    if (get_asymetric_start_key(data->master_key, key, MSCHAPV2_KEY_LEN, 0, 1) < 0 ||
        get_asymetric_start_key(data->master_key, key + MSCHAPV2_KEY_LEN, MSCHAPV2_KEY_LEN, 1, 1) < 0)
    {
        os_free(key);
        return NULL;
    }
