...
static void eap_peap_valid_session(struct eap_sm *sm, struct eap_peap_data *data)
{
    struct wpabuf *buf;

    if (!sm->cfg->tls_session_lifetime || tls_connection_resumed(sm->cfg->ssl_ctx, data->ssl.conn))
        return;
...
{
    if (data->state == FAILURE || data->state == FAILURE_REQ || data->state == SUCCESS_REQ ||
        data->tlv_request != TLV_REQ_NONE)
    {
        eap_peap_state(data, FAILURE);
        return;
    }
...
static int eap_peap_derive_cmk(struct eap_sm *sm, struct eap_peap_data *data)
{
     ...
     */
    tk = eap_server_tls_derive_key(sm, &data->ssl, label, context, context_len, EAP_TLS_KEY_LEN);
    if (tk == NULL)
        return -1;
    ...
    res = peap_prfplus(data->peap_version, tk, 40, "Inner Methods Compound Keys", isk, sizeof(isk), imck, sizeof(imck));
...
static struct wpabuf *eap_peap_build_phase2_term(struct eap_sm *sm, struct eap_peap_data *data, u8 id, int success)
{
    ...
    {
        wpa_printf(MSG_INFO, "EAP-PEAP: Failed to resize output buffer");
        wpabuf_free(encr_req);
        return NULL;
    }
    wpabuf_put_buf(data->ssl.tls_out, encr_req);
    wpa_hexdump_buf(MSG_DEBUG, "EAP-PEAP: Data appended to the message", encr_req);
    os_free(encr_req);
...
static bool eap_peap_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    const u8 *pos;
    size_t len;
...
static int eap_peap_phase2_init(struct eap_sm *sm, struct eap_peap_data *data, int vendor, enum eap_type eap_type)
{
    if (data->phase2_priv && data->phase2_method)
    {
        data->phase2_method->reset(sm, data->phase2_priv);
...
#ifdef EAP_SERVER_TNC
            if (data->state != PHASE2_SOH && sm->cfg->tnc && data->peap_version == 0)
            {
...
static void eap_peap_process(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    ...
    if (eap_server_tls_process(sm, &data->ssl, respData, data, EAP_TYPE_PEAP, eap_peap_process_version,
    ...
    if (data->state == SUCCESS || !tls_connection_established(sm->cfg->ssl_ctx, data->ssl.conn) ||
    ...
    {
        wpa_printf(MSG_DEBUG, "EAP-PEAP: No success data in resumed session - reject attempt");
        eap_peap_state(data, FAILURE);
        return;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "EAP-PEAP: Resumed session for another EAP type (%u) - reject attempt", *pos);
        eap_peap_state(data, FAILURE);
        return;
    }
    ...
    pos++;
    id_len = *pos++;
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-PEAP: Identity from cached session", pos, id_len);
    os_free(sm->identity);
    ...
    if (eap_user_get(sm, sm->identity, sm->identity_len, 1) != 0)
    {
        wpa_hexdump_ascii(MSG_DEBUG, "EAP-PEAP: Phase2 Identity not found in the user database", sm->identity,
                          sm->identity_len);
        eap_peap_state(data, FAILURE);
        return;
    }
...
static u8 *eap_peap_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
    ...
    if (data->crypto_binding_used)
    {
         ...
         */
        if (peap_prfplus(data->peap_version, data->ipmk, 40, "Session Key Generating Function", (u8 *)"\00", 1, csk,
                         sizeof(csk)) < 0)
            return NULL;
...
static u8 *eap_peap_get_emsk(struct eap_sm *sm, void *priv, size_t *len)
{
    ...
    if (eapKeyData)
    {
        ...
        if (!emsk)
            return NULL;
        *len = EAP_EMSK_LEN;
        wpa_hexdump(MSG_DEBUG, "EAP-PEAP: Derived EMSK", emsk, EAP_EMSK_LEN);
    }
    else
    {
