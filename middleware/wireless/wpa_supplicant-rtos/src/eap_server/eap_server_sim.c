...
static int eap_sim_build_encr(
    struct eap_sm *sm, struct eap_sim_data *data, struct eap_sim_msg *msg, u16 counter, const u8 *nonce_s)
{
    os_free(data->next_pseudonym);
    ...
    {
        data->next_pseudonym = eap_sim_db_get_next_pseudonym(sm->cfg->eap_sim_db_priv, EAP_SIM_DB_SIM);
    }
    else
    {
    ...
    {
        data->next_reauth_id = eap_sim_db_get_next_reauth_id(sm->cfg->eap_sim_db_priv, EAP_SIM_DB_SIM);
    }
    else
    {
...
static bool eap_sim_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    const u8 *pos;
    size_t len;
...
static bool eap_sim_unexpected_subtype(struct eap_sim_data *data, u8 subtype)
{
...
skip_id_update:
    ...
    data->num_chal = eap_sim_db_get_gsm_triplets(sm->cfg->eap_sim_db_priv, data->permanent, EAP_SIM_MAX_CHAL,
    ...
    eap_sim_derive_mk(sm->identity, identity_len, attr->nonce_mt, attr->selected_version, ver_list, sizeof(ver_list),
...
{
    if (attr->mac == NULL ||
        eap_sim_verify_mac(data->k_aut, respData, attr->mac, (u8 *)data->sres, data->num_chal * EAP_SIM_SRES_LEN))
    {
        wpa_printf(MSG_WARNING,
...
static void eap_sim_process(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    ...
    if (eap_sim_parse_attr(pos, end, &attr, 0, 0))
    {
        ...
        if (subtype != EAP_SIM_SUBTYPE_CLIENT_ERROR &&
            (data->state == START || data->state == CHALLENGE || data->state == REAUTH))
        {
            data->notification = EAP_SIM_GENERAL_FAILURE_BEFORE_AUTH;
            eap_sim_state(data, NOTIFICATION);
            return;
