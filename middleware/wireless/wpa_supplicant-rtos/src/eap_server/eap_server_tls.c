...
check_established:
    if (data->established && data->ssl.state != WAIT_FRAG_ACK)
    {
        ...
        if (sm->serial_num)
        {
            ...
            user_len = os_snprintf(user, sizeof(user), "cert-%s", sm->serial_num);
            if (eap_user_get(sm, (const u8 *)user, user_len, data->phase2) < 0)
...
static bool eap_tls_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    struct eap_tls_data *data = priv;
    const u8 *pos;
    size_t len;
...
static void eap_tls_process(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "EAP-TLS: No success data in resumed session - reject attempt");
        eap_tls_state(data, FAILURE);
        return;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "EAP-TLS: Resumed session for another EAP type (%u) - reject attempt", *pos);
        eap_tls_state(data, FAILURE);
        return;
    }
...
int eap_server_unauth_tls_register(void)
{
    ...
    eap = eap_server_method_alloc(EAP_SERVER_METHOD_INTERFACE_VERSION, EAP_VENDOR_UNAUTH_TLS,
...
int eap_server_wfa_unauth_tls_register(void)
{
    ...
    eap = eap_server_method_alloc(EAP_SERVER_METHOD_INTERFACE_VERSION, EAP_VENDOR_WFA_NEW, EAP_VENDOR_WFA_UNAUTH_TLS,
                                  "WFA-UNAUTH-TLS");
    if (eap == NULL)
        return -1;
