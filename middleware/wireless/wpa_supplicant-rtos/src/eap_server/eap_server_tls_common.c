...
struct wpabuf *eap_tls_msg_alloc(enum eap_type type, size_t payload_len, u8 code, u8 identifier)
{
...
int eap_server_tls_ssl_init(struct eap_sm *sm, struct eap_ssl_data *data, int verify_peer, int eap_type)
{
    ...
    if (tls_connection_set_verify(sm->cfg->ssl_ctx, data->conn, verify_peer, flags, session_ctx, sizeof(session_ctx)))
    {
        wpa_printf(MSG_INFO,
    ...
    data->tls_out_limit = sm->cfg->fragment_size > 0 ? sm->cfg->fragment_size : 1398;
    if (data->phase2)
    {
    ...
    struct eap_sm *sm, struct eap_ssl_data *data, const char *label, const u8 *context, size_t context_len, size_t len)
{
    ...
    if (tls_connection_export_key(sm->cfg->ssl_ctx, data->conn, label, context, context_len, out, len))
    {
        os_free(out);
        return NULL;
    }
...
u8 *eap_server_tls_derive_session_id(struct eap_sm *sm, struct eap_ssl_data *data, u8 eap_type, size_t *len)
{
    ...
    if (data->tls_v13)
    {
        ...
        *len = 1 + 64;
        ...
        if (!id)
            return NULL;
        method_id = eap_server_tls_derive_key(sm, data, "EXPORTER_EAP_TLS_Method-Id", context, 1, 64);
        if (!method_id)
        {
            os_free(id);
            return NULL;
...
struct wpabuf *eap_server_tls_build_msg(struct eap_ssl_data *data, int eap_type, int version, u8 id)
{
    ...
    else
    {
        wpa_printf(MSG_DEBUG,
                   "SSL: Sending out %lu bytes "
                   "(%lu more to send)",
                   (unsigned long)send_len, (unsigned long)wpabuf_len(data->tls_out) - data->tls_out_pos);
...
static int eap_server_tls_process_fragment(
    struct eap_ssl_data *data, u8 flags, u32 message_length, const u8 *buf, size_t len)
{
...
int eap_server_tls_phase1(struct eap_sm *sm, struct eap_ssl_data *data)
{
    ...
    data->tls_out = tls_connection_server_handshake(sm->cfg->ssl_ctx, data->conn, data->tls_in, NULL);
    if (data->tls_out == NULL)
    {
    ...
    if (tls_get_version(sm->cfg->ssl_ctx, data->conn, buf, sizeof(buf)) == 0)
    {
        wpa_printf(MSG_DEBUG, "SSL: Using TLS version %s", buf);
        data->tls_v13 = os_strcmp(buf, "TLSv1.3") == 0;
    }

    if (!sm->serial_num && tls_connection_established(sm->cfg->ssl_ctx, data->conn))
        sm->serial_num = tls_connection_peer_serial_num(sm->cfg->ssl_ctx, data->conn);
     ...
     * even on resumption.
     */
    if (data->tls_v13 && tls_connection_established(sm->cfg->ssl_ctx, data->conn))
    {
        ...
        switch (sm->currentMethod)
        {
            case EAP_TYPE_PEAP:
                break;
            default:
                if (!tls_connection_resumed(sm->cfg->ssl_ctx, data->conn))
                    break;
                /* fallthrough */
            case EAP_TYPE_TLS:
                ...
                {
                    wpa_printf(MSG_INFO, "EAP-TLS: Failed to resize output buffer");
                    wpabuf_free(encr);
                    return -1;
                }
                wpabuf_put_buf(data->tls_out, encr);
                wpa_hexdump_buf(MSG_DEBUG, "EAP-TLS: Data appended to the message", encr);
                wpabuf_free(encr);
        }
    }
...
int eap_server_tls_process(struct eap_sm *sm,
                           ...
                           void *priv,
                           int eap_type,
                           int (*proc_version)(struct eap_sm *sm, void *priv, int peer_version),
                           void (*proc_msg)(struct eap_sm *sm, void *priv, const struct wpabuf *respData))
{
    const u8 *pos;
    u8 flags;
    size_t left;
