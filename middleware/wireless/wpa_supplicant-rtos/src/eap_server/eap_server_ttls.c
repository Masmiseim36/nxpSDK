...
static void eap_ttls_valid_session(struct eap_sm *sm, struct eap_ttls_data *data)
{
    struct wpabuf *buf;
...
{
    return eap_server_tls_derive_key(sm, &data->ssl, "ttls challenge", NULL, 0, len);
}
    ...
    if (data->mschapv2_resp_ok)
    {
        ...
        *pos++ = data->mschapv2_ident;
        ...
        if (!os_snprintf_error(end - pos, ret))
            pos += ret;
        pos += wpa_snprintf_hex_uppercase((char *)pos, end - pos, data->mschapv2_auth_response,
...
static bool eap_ttls_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    const u8 *pos;
    size_t len;
                                         ...
                                         size_t password_len)
{
    ...
    if (challenge == NULL || password == NULL || challenge_len != EAP_TTLS_CHAP_CHALLENGE_LEN ||
                                           ...
                                           size_t response_len)
{
    ...
    if (challenge == NULL || response == NULL || challenge_len != EAP_TTLS_MSCHAP_CHALLENGE_LEN ||
    ...
    if ((sm->user->password_hash && challenge_response(challenge, sm->user->password, nt_response)) ||
                                             ...
                                             size_t response_len)
{
    ...
    if (challenge == NULL || response == NULL || challenge_len != EAP_TTLS_MSCHAPV2_CHALLENGE_LEN ||
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "EAP-TTLS/MSCHAPV2: No user identity "
                   "known");
        eap_ttls_state(data, FAILURE);
        return;
    }
    ...
    {
        generate_nt_response_pwhash(auth_challenge, peer_challenge, username, username_len, sm->user->password,
                                    nt_response);
    }
    else
    {
        generate_nt_response(auth_challenge, peer_challenge, username, username_len, sm->user->password,
    ...
    if (os_memcmp_const(nt_response, rx_resp, 24) == 0)
    {
        ...
        if (sm->user->password_hash)
        {
            generate_authenticator_response_pwhash(sm->user->password, peer_challenge, auth_challenge, username,
                                                   username_len, nt_response, data->mschapv2_auth_response);
        }
        else
        {
            generate_authenticator_response(sm->user->password, sm->user->password_len, peer_challenge, auth_challenge,
                                            username, username_len, nt_response, data->mschapv2_auth_response);
        }
    }
    else
    {
...
static int eap_ttls_phase2_eap_init(struct eap_sm *sm, struct eap_ttls_data *data, int vendor, enum eap_type eap_type)
{
    if (data->phase2_priv && data->phase2_method)
    {
        data->phase2_method->reset(sm, data->phase2_priv);
                                                 ...
                                                 size_t in_len)
{
    ...
    if (in_len > sizeof(*hdr) && *pos == EAP_TYPE_NAK)
    {
            ...
            sm->user->methods[sm->user_eap_method_index].method != EAP_TYPE_NONE)
        {
            ...
            wpa_printf(MSG_DEBUG, "EAP-TTLS: try EAP type %u:%u", next_vendor, next_type);
            if (eap_ttls_phase2_eap_init(sm, data, next_vendor, next_type))
            {
                wpa_printf(MSG_DEBUG, "EAP-TTLS: Failed to initialize EAP type %u:%u", next_vendor, next_type);
                eap_ttls_state(data, FAILURE);
                return;
            }
        }
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "EAP-TTLS/EAP: Phase2 method is in "
...
static void eap_ttls_process_phase2_eap(struct eap_sm *sm, struct eap_ttls_data *data, const u8 *eap, size_t eap_len)
{
    ...
    if (data->state == PHASE2_START)
    {
        ...
        if (eap_ttls_phase2_eap_init(sm, data, EAP_VENDOR_IETF, EAP_TYPE_IDENTITY) < 0)
        {
            wpa_printf(MSG_DEBUG,
...
static void eap_ttls_process_phase2(struct eap_sm *sm, struct eap_ttls_data *data, struct wpabuf *in_buf)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "EAP-TTLS: Pending Phase 2 EAP response "
                   "- skip decryption and use old data");
        eap_ttls_process_phase2_eap(sm, data, wpabuf_head(data->pending_phase2_eap_resp),
    ...
    in_decrypted = tls_connection_decrypt(sm->cfg->ssl_ctx, data->ssl.conn, in_buf);
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "EAP-TTLS: TNC started but no EAP "
                   "response from peer");
        eap_ttls_state(data, FAILURE);
        goto done;
    }
    ...
    {
        eap_ttls_process_phase2_chap(sm, data, parse.chap_challenge, parse.chap_challenge_len, parse.chap_password,
                                     parse.chap_password_len);
    }
    else if (parse.mschap_response)
    {
        eap_ttls_process_phase2_mschap(sm, data, parse.mschap_challenge, parse.mschap_challenge_len,
                                       parse.mschap_response, parse.mschap_response_len);
    }
    else if (parse.mschap2_response)
    {
        eap_ttls_process_phase2_mschapv2(sm, data, parse.mschap_challenge, parse.mschap_challenge_len,
...
static void eap_ttls_process(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    ...
    if (eap_server_tls_process(sm, &data->ssl, respData, data, EAP_TYPE_TTLS, eap_ttls_process_version,
    ...
    {
        wpa_printf(MSG_DEBUG, "EAP-TTLS: No success data in resumed session - reject attempt");
        eap_ttls_state(data, FAILURE);
        return;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "EAP-TTLS: Resumed session for another EAP type (%u) - reject attempt", *pos);
        eap_ttls_state(data, FAILURE);
        return;
    }
    ...
    pos++;
    id_len = *pos++;
    wpa_hexdump_ascii(MSG_DEBUG, "EAP-TTLS: Identity from cached session", pos, id_len);
    os_free(sm->identity);
    ...
    if (eap_user_get(sm, sm->identity, sm->identity_len, 1) != 0)
    {
        wpa_hexdump_ascii(MSG_DEBUG, "EAP-TTLS: Phase2 Identity not found in the user database", sm->identity,
                          sm->identity_len);
        eap_ttls_state(data, FAILURE);
        return;
    }
...
static u8 *eap_ttls_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
    ...
    eapKeyData = eap_server_tls_derive_key(sm, &data->ssl, label, context, context_len, EAP_TLS_KEY_LEN + EAP_EMSK_LEN);
    if (eapKeyData)
    {
        *len = EAP_TLS_KEY_LEN;
