...
static void *eap_wsc_init(struct eap_sm *sm)
{
    ...
    if (sm->assoc_p2p_ie)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "EAP-WSC: Prefer PSK format for non-6 GHz P2P client");
            cfg.use_psk_key = 1;
        }
    ...
    data->fragment_size = sm->cfg->fragment_size > 0 ? sm->cfg->fragment_size : WSC_FRAGMENT_SIZE;
...
static bool eap_wsc_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
    const u8 *pos;
    size_t len;
...
static int eap_wsc_process_fragment(
    struct eap_wsc_data *data, u8 flags, u8 op_code, u16 message_length, const u8 *buf, size_t len)
{
