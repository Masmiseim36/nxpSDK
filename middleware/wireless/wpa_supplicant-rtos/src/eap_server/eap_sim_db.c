...
static int db_table_create_pseudonym(sqlite3 *db)
{
    ...
    wpa_printf(MSG_DEBUG,
               "EAP-SIM DB: Adding database table for "
...
static int db_add_reauth(struct eap_sim_db_data *data,
                         ...
                         const u8 *k_re)
{
    ...
    pos += os_snprintf(pos, end - pos,
                       ...
                       "VALUES ('%s', '%s', %u",
                       mk ? ", mk" : "", k_encr ? ", k_encr" : "", k_aut ? ", k_aut" : "", k_re ? ", k_re" : "",
...
static void eap_sim_db_free_pending(struct eap_sim_db_data *data, struct eap_sim_db_pending *entry)
{
...
static void eap_sim_db_del_pending(struct eap_sim_db_data *data, struct eap_sim_db_pending *entry)
{
...
static void process_eap_sim_db_event(struct eap_sim_db_data *data)
{
    void *mem;
    char *buf = NULL;
    char *pos, *cmd, *imsi;
...
extern int send_hlr_event(char *msg);
...
{
    eloop_register_timeout(data->eap_sim_db_timeout, 0, eap_sim_db_query_timeout, data, entry);
}
 ...
 */
int eap_sim_db_get_gsm_triplets(
    struct eap_sim_db_data *data, const char *username, int max_chal, u8 *_rand, u8 *kc, u8 *sres, void *cb_session_ctx)
{
    struct eap_sim_db_pending *entry;
    int len, ret;
    char msg[40];
    ...
    if (username == NULL || username[0] != EAP_SIM_PERMANENT_PREFIX || username[1] == '\0' ||
        os_strlen(username) > sizeof(entry->imsi))
    {
 ...
 * anymore.
 */
int eap_sim_db_add_reauth(
    struct eap_sim_db_data *data, const char *permanent, char *reauth_id, u16 counter, const u8 *mk)
{
    struct eap_sim_reauth *r;
                            ...
                            void *cb_session_ctx)
{
    ...
    if (username == NULL ||
        (username[0] != EAP_AKA_PERMANENT_PREFIX && username[0] != EAP_AKA_PRIME_PERMANENT_PREFIX) ||
