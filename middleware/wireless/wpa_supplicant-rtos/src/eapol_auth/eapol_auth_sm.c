...
PRINTF_FORMAT(4, 5)
static void eapol_auth_vlogger(
    struct eapol_authenticator *eapol, const u8 *addr, eapol_logger_level level, const char *fmt, ...)
{
    char *format;
    int maxlen;
    va_list ap;
...
static void eapol_auth_tx_canned_eap(struct eapol_state_machine *sm, int success)
{
    ...
    eapol_auth_vlogger(sm->eapol, sm->addr, EAPOL_LOGGER_DEBUG, "Sending canned EAP packet %s (identifier %d)",
...
static void eapol_auth_tx_req(struct eapol_state_machine *sm)
{
    ...
    sm->eapol->cb.eapol_send(sm->eapol->conf.ctx, sm->sta, IEEE802_1X_TYPE_EAP_PACKET,
...
SM_STATE(AUTH_PAE, HELD)
{
    ...
    eapol_auth_vlogger(sm->eapol, sm->addr, EAPOL_LOGGER_WARNING, "authentication failed - EAP type: %d (%s)",
                       sm->eap_type_authsrv, eap_server_get_name(0, sm->eap_type_authsrv));
...
SM_STATE(AUTH_PAE, AUTHENTICATED)
{
    ...
    if (sm->flags & EAPOL_SM_PREAUTH)
        ...
        extra = " (PMKSA cache)";
    eapol_auth_vlogger(sm->eapol, sm->addr, EAPOL_LOGGER_INFO, "authenticated - EAP type: %d (%s)%s",
                       sm->eap_type_authsrv, eap_server_get_name(0, sm->eap_type_authsrv), extra);
...
SM_STEP(AUTH_PAE)
{
    ...
    else if (sm->portControl == ForceAuthorized && sm->portMode != sm->portControl &&
             !(sm->initialize || !sm->eap_if->portEnabled))
        SM_ENTER_GLOBAL(AUTH_PAE, FORCE_AUTH);
    else if (sm->portControl == ForceUnauthorized && sm->portMode != sm->portControl &&
             !(sm->initialize || !sm->eap_if->portEnabled))
        ...
        switch (sm->auth_pae_state)
        {
            ...
            case AUTH_PAE_CONNECTING:
                if (sm->eapolLogoff || sm->reAuthCount > sm->reAuthMax)
                    SM_ENTER(AUTH_PAE, DISCONNECTED);
                else if ((sm->eap_if->eapReq && sm->reAuthCount <= sm->reAuthMax) || sm->eap_if->eapSuccess ||
                         sm->eap_if->eapFail)
                                             ...
                                             const char *radius_cui)
{
    ...
    sm->eap               = eap_server_sm_init(sm, &eapol_cb, eapol->conf.eap_cfg, &eap_sess);
    if (sm->eap == NULL)
    {
        eapol_auth_free(sm);
        return NULL;
    ...
    if (radius_gen_session_id((u8 *)&sm->acct_multi_session_id, sizeof(sm->acct_multi_session_id)) < 0)
    {
        eapol_auth_free(sm);
        return NULL;
    }
...
restart:
    ...
    if (prev_auth_pae != sm->auth_pae_state || prev_be_auth != sm->be_auth_state ||
        prev_reauth_timer != sm->reauth_timer_state || prev_auth_key_tx != sm->auth_key_tx_state ||
    ...
    if (eapol_sm_sta_entry_alive(eapol, addr) && sm->eap)
    {
        ...
        if (sm->eap_if->aaaEapResp)
        {
            ...
            sm->eapol->cb.aaa_send(sm->eapol->conf.ctx, sm->sta, wpabuf_head(sm->eap_if->aaaEapRespData),
...
{
    wpa_printf(MSG_DEBUG, "EAPOL: External reauthentication trigger for " MACSTR, MAC2STR(sm->addr));
    sm->reAuthenticate = true;
    eapol_auth_step(sm);
}
...
{
    wpa_printf(MSG_DEBUG, "EAPOL: External configuration operation for " MACSTR " - param=%s value=%s",
...
static int eapol_auth_conf_clone(struct eapol_auth_config *dst, struct eapol_auth_config *src)
{
    ...
    {
        dst->eap_req_id_text = os_memdup(src->eap_req_id_text, src->eap_req_id_text_len);
