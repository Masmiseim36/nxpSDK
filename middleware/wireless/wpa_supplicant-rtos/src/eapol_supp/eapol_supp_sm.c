...
static void eapol_port_timers_tick(void *eloop_ctx, void *timeout_ctx)
{
    ...
    {
        if (eloop_register_timeout(1, 0, eapol_port_timers_tick, eloop_ctx, sm) < 0)
...
{
    int send_start = sm->SUPP_PAE_state == SUPP_PAE_CONNECTING || sm->SUPP_PAE_state == SUPP_PAE_HELD;
...
SM_STEP(SUPP_PAE)
{
    ...
    else if ((sm->portControl == ForceAuthorized) && (sm->sPortMode != sm->portControl) &&
             !(sm->initialize || !sm->portEnabled))
        SM_ENTER_GLOBAL(SUPP_PAE, S_FORCE_AUTH);
    else if ((sm->portControl == ForceUnauthorized) && (sm->sPortMode != sm->portControl) &&
             !(sm->initialize || !sm->portEnabled))
...
SM_STATE(SUPP_BE, SUCCESS)
{
    ...
    if (sm->use_eap_proxy)
    {
        if (eap_proxy_key_available(sm->eap_proxy))
        {
            ...
            session_id = eap_proxy_get_eap_session_id(sm->eap_proxy, &session_id_len);
            emsk       = eap_proxy_get_emsk(sm->eap_proxy, &emsk_len);
            if (sm->config->erp && session_id && emsk)
            {
                eap_peer_erp_init(sm->eap, session_id, session_id_len, emsk, emsk_len);
            }
            else
            {
                os_free(session_id);
...
static void eapol_sm_processKey(struct eapol_sm *sm)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               hdr->version, hdr->type, be_to_host16(hdr->length), key->type, rx_key_length, key->key_index);
    ...
    if (sm->replay_counter_valid &&
        os_memcmp(sm->last_replay_counter, key->replay_counter, IEEE8021X_REPLAY_COUNTER_LEN) >= 0)
    {
        wpa_printf(MSG_WARNING,
    ...
    hmac_md5(keydata.sign_key, sign_key_len, sm->last_rx_key, sizeof(*hdr) + be_to_host16(hdr->length),
             key->key_signature);
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "len %d",
               key->key_index & IEEE8021X_KEY_INDEX_FLAG ? "unicast" : "broadcast",
               key->key_index & IEEE8021X_KEY_INDEX_MASK, key_len);

    if (sm->ctx->set_wep_key && sm->ctx->set_wep_key(sm->ctx->ctx, !!(key->key_index & IEEE8021X_KEY_INDEX_FLAG),
    ...
    else
    {
        ...
        if ((sm->unicast_key_received || !(sm->conf.required_keys & EAPOL_REQUIRE_KEY_UNICAST)) &&
            (sm->broadcast_key_received || !(sm->conf.required_keys & EAPOL_REQUIRE_KEY_BROADCAST)))
        {
            wpa_printf(MSG_DEBUG,
...
int eapol_sm_get_status(struct eapol_sm *sm, char *buf, size_t buflen, int verbose)
{
    ...
    if (verbose)
    {
        ret = os_snprintf(buf + len, buflen - len,
                          ...
                          sm->heldPeriod, sm->authPeriod, sm->startPeriod, sm->maxStart,
                          eapol_port_control(sm->portControl), eapol_supp_be_state(sm->SUPP_BE_state));
...
int eapol_sm_get_mib(struct eapol_sm *sm, char *buf, size_t buflen)
{
    ...
    ret = os_snprintf(buf, buflen,
                      ...
                      sm->SUPP_PAE_state, sm->heldPeriod, sm->authPeriod, sm->startPeriod, sm->maxStart,
                      sm->suppPortStatus == Authorized ? "Authorized" : "Unauthorized", sm->SUPP_BE_state);
    ...
    ret = os_snprintf(buf + len, buflen - len,
                      ...
                      sm->dot1xSuppEapolLogoffFramesTx, sm->dot1xSuppEapolRespFramesTx, sm->dot1xSuppEapolReqIdFramesRx,
...
#ifdef CONFIG_WPS
    if (sm->conf.wps && sm->conf.workaround && plen < len - sizeof(*hdr) && hdr->type == IEEE802_1X_TYPE_EAP_PACKET &&
...
static void eapol_sm_set_bool(void *ctx, enum eapol_bool_var variable, bool value)
{
    struct eapol_sm *sm = ctx;
    if (sm == NULL)
        return;
...
static void eapol_sm_notify_cert(void *ctx, struct tls_cert_data *cert, const char *cert_hash)
{
    struct eapol_sm *sm = ctx;
    if (sm->ctx->cert_cb)
...
static void eapol_sm_eap_proxy_notify_sim_status(void *ctx, enum eap_proxy_sim_state sim_state)
{
    struct eapol_sm *sm = ctx;
...
static const struct eapol_callbacks eapol_cb = {eapol_sm_get_config,
                                                eapol_sm_get_bool,
                                                eapol_sm_set_bool,
                                                eapol_sm_get_int,
                                                eapol_sm_set_int,
...
void eapol_sm_process_erp_finish(struct eapol_sm *sm, const u8 *buf, size_t len)
{
#ifdef CONFIG_ERP
    if (!sm)
        return;
...
int eapol_sm_get_erp_info(struct eapol_sm *sm,
                          struct eap_peer_config *config,
                          const u8 **username,
                          size_t *username_len,
                          const u8 **realm,
                          ...
                          size_t *rrk_len)
{
    ...
    if (!sm)
        return -1;
    return eap_peer_get_erp_info(sm->eap, config, username, username_len, realm, realm_len, erp_next_seq_num, rrk,
                                 rrk_len);
#else  /* CONFIG_ERP */
    return -1;
#endif /* CONFIG_ERP */
