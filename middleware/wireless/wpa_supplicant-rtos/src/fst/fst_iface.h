...
struct fst_iface *fst_iface_create(struct fst_group *g,
                                   const char *ifname,
                                   const u8 *own_addr,
...
static inline void fst_iface_get_channel_info(struct fst_iface *i, enum hostapd_hw_mode *hw_mode, u8 *channel)
{
    i->iface_obj.get_channel_info(i->iface_obj.ctx, hw_mode, channel);
}

static inline int fst_iface_get_hw_modes(struct fst_iface *i, struct hostapd_hw_modes **modes)
{
    return i->iface_obj.get_hw_modes(i->iface_obj.ctx, modes);
}

static inline void fst_iface_set_ies(struct fst_iface *i, const struct wpabuf *fst_ies)
{
    i->iface_obj.set_ies(i->iface_obj.ctx, fst_ies);
}

static inline int fst_iface_send_action(struct fst_iface *i, const u8 *addr, struct wpabuf *data)
{
    return i->iface_obj.send_action(i->iface_obj.ctx, addr, data);
}

static inline const struct wpabuf *fst_iface_get_peer_mb_ie(struct fst_iface *i, const u8 *addr)
{
    return i->iface_obj.get_mb_ie(i->iface_obj.ctx, addr);
}

static inline void fst_iface_update_mb_ie(struct fst_iface *i, const u8 *addr, const u8 *buf, size_t size)
{
    i->iface_obj.update_mb_ie(i->iface_obj.ctx, addr, buf, size);
}

static inline const u8 *fst_iface_get_peer_first(struct fst_iface *i, struct fst_get_peer_ctx **ctx, bool mb_only)
{
    return i->iface_obj.get_peer_first(i->iface_obj.ctx, ctx, mb_only);
}

static inline const u8 *fst_iface_get_peer_next(struct fst_iface *i, struct fst_get_peer_ctx **ctx, bool mb_only)
{
    return i->iface_obj.get_peer_next(i->iface_obj.ctx, ctx, mb_only);
}

bool fst_iface_is_connected(struct fst_iface *iface, const u8 *addr, bool mb_only);
