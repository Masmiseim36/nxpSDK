...
static void p2p_process_presence_req(
    struct p2p_data *p2p, const u8 *da, const u8 *sa, const u8 *data, size_t len, int rx_freq);
...
static struct p2p_device *p2p_create_device(struct p2p_data *p2p, const u8 *addr)
{
    ...
    {
        p2p_dbg(p2p, "Remove oldest peer entry to make room for a new peer " MACSTR,
                MAC2STR(oldest->info.p2p_device_addr));
...
{
    p2p_copy_filter_devname(dev->info.device_name, sizeof(dev->info.device_name), cli->dev_name, cli->dev_name_len);
...
static int p2p_add_group_clients(struct p2p_data *p2p,
...
static void p2p_copy_wps_info(struct p2p_data *p2p,
                              struct p2p_device *dev,
                              int probe_req,
                              const struct p2p_message *msg)
{
    ...
    if (!probe_req)
    {
        ...
        {
            p2p_dbg(p2p, "Update peer " MACSTR " config_methods 0x%x -> 0x%x", MAC2STR(dev->info.p2p_device_addr),
...
static void p2p_update_peer_vendor_elems(struct p2p_device *dev, const u8 *ies, size_t ies_len)
{
    ...
    for (pos = ies; end - pos > 1; pos += len)
    {
        ...
        if (len >= 4)
        {
            ...
            if (type == WPA_IE_VENDOR_TYPE || type == WMM_IE_VENDOR_TYPE || type == WPS_IE_VENDOR_TYPE ||
                type == P2P_IE_VENDOR_TYPE || type == WFD_IE_VENDOR_TYPE)
                continue;
        }
                   ...
                   int scan_res)
{
    ...
    if (dev->last_seen.sec > 0 && os_reltime_before(rx_time, &dev->last_seen) &&
        (!(dev->flags & P2P_DEV_LAST_SEEN_AS_GROUP_CLIENT) ||
         os_reltime_expired(&dev->last_seen, rx_time, P2P_DEV_GROUP_CLIENT_RESP_THRESHOLD)))
    {
        p2p_dbg(p2p, "Do not update peer entry based on old frame (rx_time=%u.%06u last_seen=%u.%06u flags=0x%x)",
                (unsigned int)rx_time->sec, (unsigned int)rx_time->usec, (unsigned int)dev->last_seen.sec,
                (unsigned int)dev->last_seen.usec, dev->flags);
        p2p_parse_free(&msg);
        return -1;
    }
    ...
    {
        p2p_dbg(p2p, "Update Listen frequency based on scan results (" MACSTR " %d -> %d MHz (DS param %d)",
                MAC2STR(dev->info.p2p_device_addr), dev->listen_freq, freq, msg.ds_params ? *msg.ds_params : -1);
    }
    if (scan_res)
    {
    ...
    dev_name_changed = os_strncmp(dev->info.device_name, msg.device_name, WPS_DEV_NAME_MAX_LEN) != 0;
    ...
    if (dev->flags & P2P_DEV_REPORTED && !wfd_changed && !dev_name_changed &&
        (!msg.adv_service_instance || (dev->flags & P2P_DEV_P2PS_REPORTED)))
        return 0;

    p2p_dbg(p2p, "Peer found with Listen frequency %d MHz (rx_time=%u.%06u)", freq, (unsigned int)rx_time->sec,
...
static void p2p_search(struct p2p_data *p2p)
{
    ...
    else if ((p2p->find_type == P2P_FIND_PROGRESSIVE && (freq = p2p_get_next_prog_freq(p2p)) > 0) ||
             (p2p->find_type == P2P_FIND_START_WITH_FULL && (freq = p2p->find_specified_freq) > 0))
    {
    ...
    res = p2p->cfg->p2p_scan(p2p->cfg->cb_ctx, type, freq, p2p->num_req_dev_types, p2p->req_dev_types, p2p->find_dev_id,
                             pw_id, p2p->include_6ghz);
    if (res < 0)
    {
             ...
             bool include_6ghz)
{
    ...
    {
        p2p->req_dev_types = os_memdup(req_dev_types, num_req_dev_types * WPS_DEV_TYPE_LEN);
        ...
        case P2P_FIND_START_WITH_FULL:
            if (freq > 0)
            {
                ...
                res = p2p->cfg->p2p_scan(p2p->cfg->cb_ctx, P2P_SCAN_SPECIFIC, freq, p2p->num_req_dev_types,
                                         p2p->req_dev_types, dev_id, DEV_PW_DEFAULT, p2p->include_6ghz);
                break;
            }
            /* fall through */
        case P2P_FIND_PROGRESSIVE:
            res = p2p->cfg->p2p_scan(p2p->cfg->cb_ctx, P2P_SCAN_FULL, 0, p2p->num_req_dev_types, p2p->req_dev_types,
                                     dev_id, DEV_PW_DEFAULT, p2p->include_6ghz);
            break;
        case P2P_FIND_ONLY_SOCIAL:
            res = p2p->cfg->p2p_scan(p2p->cfg->cb_ctx, P2P_SCAN_SOCIAL, 0, p2p->num_req_dev_types, p2p->req_dev_types,
                                     dev_id, DEV_PW_DEFAULT, p2p->include_6ghz);
            break;
        default:
            return -1;
    ...
    if (res != 0 && p2p->p2p_scan_running)
    {
        ...
        if (type == P2P_FIND_PROGRESSIVE || (type == P2P_FIND_START_WITH_FULL && freq == 0))
...
{
    if (freq > 0 && ((p2p->drv_in_listen == freq && p2p->in_listen) || p2p->pending_listen_freq == (unsigned int)freq))
    {
...
static int p2p_prepare_channel_pref(struct p2p_data *p2p, unsigned int force_freq, unsigned int pref_freq, int go)
{
    ...
    if (!p2p_channels_includes(&p2p->cfg->channels, op_class, op_channel) &&
        (go || !p2p_channels_includes(&p2p->cfg->cli_channels, op_class, op_channel)))
    {
...
static void p2p_prepare_channel_best(struct p2p_data *p2p)
{
    ...
    if (!p2p->cfg->cfg_op_channel && p2p->best_freq_overall > 0 && p2p_supported_freq(p2p, p2p->best_freq_overall) &&
        p2p_freq_to_channel(p2p->best_freq_overall, &op_class, &op_channel) == 0)
    {
        p2p_dbg(p2p, "Select best overall channel as operating channel preference");
        p2p->op_reg_class = op_class;
        p2p->op_channel   = op_channel;
    }
    else if (!p2p->cfg->cfg_op_channel && p2p->best_freq_5 > 0 && p2p_supported_freq(p2p, p2p->best_freq_5) &&
             p2p_freq_to_channel(p2p->best_freq_5, &op_class, &op_channel) == 0)
    {
        p2p_dbg(p2p, "Select best 5 GHz channel as operating channel preference");
        p2p->op_reg_class = op_class;
        p2p->op_channel   = op_channel;
    }
    else if (!p2p->cfg->cfg_op_channel && p2p->best_freq_24 > 0 && p2p_supported_freq(p2p, p2p->best_freq_24) &&
             p2p_freq_to_channel(p2p->best_freq_24, &op_class, &op_channel) == 0)
    {
        ...
        p2p->op_channel   = op_channel;
             ...
             p2p_channels_includes(&p2p->cfg->channels, p2p->cfg->pref_chan[0].op_class, p2p->cfg->pref_chan[0].chan))
    {
    ...
    else if (p2p_channel_select(&p2p->cfg->channels, op_classes_edmg, &p2p->op_reg_class, &p2p->op_channel) == 0)
    {
        p2p_dbg(p2p, "Select possible EDMG channel (op_class %u channel %u) as operating channel preference",
             ...
             (p2p_channel_select(&p2p->cfg->channels, op_classes_6ghz, &p2p->op_reg_class, &p2p->op_channel) == 0))
    {
    ...
    else if (p2p_channel_random_social(&p2p->cfg->channels, &p2p->op_reg_class, &p2p->op_channel, NULL, NULL) == 0)
    {
...
int p2p_prepare_channel(
    struct p2p_data *p2p, struct p2p_device *dev, unsigned int force_freq, unsigned int pref_freq, int go)
{
    ...
    p2p_dbg(p2p, "Own preference for operation channel: Operating Class %u Channel %u%s", p2p->op_reg_class,
            p2p->op_channel, force_freq ? " (forced)" : "");
                ...
                u16 oob_pw_id)
{
    ...
    p2p_dbg(p2p,
            "Request to start group negotiation - peer=" MACSTR "  GO Intent=%d  Intended Interface Address=" MACSTR
            " wps_method=%d persistent_group=%d pd_before_go_neg=%d "
            "oob_pw_id=%u allow_6ghz=%d",
            MAC2STR(peer_addr), go_intent, MAC2STR(own_interface_addr), wps_method, persistent_group, pd_before_go_neg,
            oob_pw_id, p2p->allow_6ghz);
    ...
    if (dev->flags & P2P_DEV_GROUP_CLIENT_ONLY)
    {
        ...
        {
            p2p_dbg(p2p, "Cannot connect to P2P Device " MACSTR " that is in a group and is not discoverable",
                    MAC2STR(peer_addr));
            return -1;
        }
        if (dev->oper_freq <= 0)
                  ...
                  u16 oob_pw_id)
{
    ...
    p2p_dbg(p2p,
            "Request to authorize group negotiation - peer=" MACSTR "  GO Intent=%d  Intended Interface Address=" MACSTR
            " wps_method=%d  persistent_group=%d oob_pw_id=%u allow_6ghz=%d",
            MAC2STR(peer_addr), go_intent, MAC2STR(own_interface_addr), wps_method, persistent_group, oob_pw_id,
            p2p->allow_6ghz);
...
void p2p_add_dev_info(struct p2p_data *p2p, const u8 *addr, struct p2p_device *dev, struct p2p_message *msg)
{
    ...
    if (msg->listen_channel)
    {
        ...
        if (freq < 0)
        {
            p2p_dbg(p2p,
                    ...
                    msg->listen_channel[0], msg->listen_channel[1], msg->listen_channel[2], msg->listen_channel[3],
                    msg->listen_channel[4]);
        }
        else
        {
            p2p_dbg(p2p, "Update peer " MACSTR " Listen channel: %u -> %u MHz", MAC2STR(dev->info.p2p_device_addr),
                    dev->listen_freq, freq);
            dev->listen_freq = freq;
        }
    }
    ...
    else
    {
        p2p_dbg(p2p,
                ...
                "listen_freq=%d",
                MAC2STR(dev->info.p2p_device_addr), dev->info.dev_capab, dev->info.group_capab, dev->info.device_name,
                dev->listen_freq);
    }
...
void p2p_go_complete(struct p2p_data *p2p, struct p2p_device *peer)
{
    ...
    p2p_dbg(p2p, "GO Negotiation with " MACSTR " completed (%s will be GO)", MAC2STR(peer->info.p2p_device_addr),
            go ? "local end" : "peer");
...
static void p2p_rx_action_public(
    struct p2p_data *p2p, const u8 *da, const u8 *sa, const u8 *bssid, const u8 *data, size_t len, int freq)
{
    if (len < 1)
        return;
...
static void p2p_add_dev_from_probe_req(struct p2p_data *p2p, const u8 *addr, const u8 *ie, size_t ie_len)
{
    ...
    if (dev)
    {
        if (msg.listen_channel)
        {
            ...
            {
                p2p_dbg(p2p, "Updated peer " MACSTR " Listen channel (Probe Request): %d -> %d MHz", MAC2STR(addr),
                        dev->listen_freq, freq);
                dev->listen_freq = freq;
            }
        }
...
static int dev_type_match(const u8 *dev_type, const u8 *req_dev_type)
{
    if (os_memcmp(dev_type, req_dev_type, WPS_DEV_TYPE_LEN) == 0)
        return 1;
    if (os_memcmp(dev_type, req_dev_type, 2) == 0 && WPA_GET_BE32(&req_dev_type[2]) == 0 &&
...
static int p2p_build_probe_resp_buf(
    struct p2p_data *p2p, struct wpabuf *buf, struct wpabuf *ies, const u8 *addr, int rx_freq)
{
    ...
    resp->u.probe_resp.capab_info =
        host_to_le16(WLAN_CAPABILITY_SHORT_PREAMBLE | WLAN_CAPABILITY_PRIVACY | WLAN_CAPABILITY_SHORT_SLOT_TIME);
                                                 ...
                                                 unsigned int rx_freq)
{
    ...
    if (msg.service_hash && msg.service_hash_count)
    {
        ...
        p2p_dbg(p2p,
                "in_listen=%d drv_in_listen=%d when received P2PS Probe Request at %u MHz; own Listen channel %u, "
                "pending listen freq %u MHz",
                p2p->in_listen, p2p->drv_in_listen, rx_freq, p2p->cfg->channel, p2p->pending_listen_freq);

        if (!p2p->in_listen && !p2p->drv_in_listen && p2p->pending_listen_freq && rx_freq &&
            rx_freq != p2p->pending_listen_freq)
        {
            p2p_dbg(p2p,
                    "Do not reply to Probe Request frame that was received on %u MHz while waiting to start Listen "
                    "state on %u MHz",
                    rx_freq, p2p->pending_listen_freq);
            p2p_parse_free(&msg);
    ...
    else
    {
        ...
        if (!p2p->in_listen || !p2p->drv_in_listen)
        {
            ...
            p2p_dbg(p2p, "Not in Listen state (in_listen=%d drv_in_listen=%d) - ignore Probe Request", p2p->in_listen,
                    p2p->drv_in_listen);
            p2p_parse_free(&msg);
            return P2P_PREQ_NOT_LISTEN;
        }
                                           ...
                                           int p2p_lo_started)
{
     ...
     */
    if ((p2p->state == P2P_CONNECT || p2p->state == P2P_CONNECT_LISTEN) && p2p->go_neg_peer &&
    ...
    if ((p2p->state == P2P_INVITE || p2p->state == P2P_INVITE_LISTEN) && p2p->invite_peer &&
...
int p2p_service_add_asp(struct p2p_data *p2p,
                        int auto_accept,
                        u32 adv_id,
                        const char *adv_str,
                        u8 svc_state,
...
inserted:
    p2p_dbg(p2p, "Added ASP advertisement adv_id=0x%x config_methods=0x%x svc_state=0x%x adv_str='%s' cpt_mask=0x%x",
            adv_id, adv_data->config_methods, svc_state, adv_str, adv_data->cpt_mask);
...
void p2p_wps_success_cb(struct p2p_data *p2p, const u8 *mac_addr)
{
    ...
    {
        p2p_dbg(p2p, "Ignore WPS registration success notification for " MACSTR " (GO Negotiation peer " MACSTR ")",
...
static int p2p_pre_find_operation(struct p2p_data *p2p, struct p2p_device *dev)
{
    ...
    {
        p2p_dbg(p2p, "Send pending Provision Discovery Request to " MACSTR " (config methods 0x%x)",
...
static void p2p_sd_cb(struct p2p_data *p2p, int success)
{
    ...
    if (!success)
    {
        if (p2p->sd_peer)
        {
            ...
            {
                os_memcpy(p2p->sd_query_no_ack, p2p->sd_peer->info.p2p_device_addr, ETH_ALEN);
                p2p_dbg(p2p, "First SD Query no-ACK in this search iteration: " MACSTR, MAC2STR(p2p->sd_query_no_ack));
            }
...
static void p2p_prov_disc_cb(struct p2p_data *p2p, int success)
{
     ...
     */
    if (p2p->p2ps_prov && !p2p->p2ps_prov->pd_seeker && p2p->p2ps_prov->status != P2P_SC_SUCCESS_DEFERRED)
    {
        ...
        {
            p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, p2p->p2ps_prov->status, p2p->p2ps_prov->adv_mac,
                                         p2p->p2ps_prov->adv_mac, p2p->p2ps_prov->session_mac, NULL,
                                         p2p->p2ps_prov->adv_id, p2p->p2ps_prov->session_id, 0, 0, NULL, 0, 0, 0, NULL,
                                         NULL, 0, 0, NULL, 0);
        }
                         ...
                         size_t ies_len)
{
    if (os_reltime_before(rx_time, &p2p->find_start))
    {
         ...
         */
        p2p_dbg(p2p, "Ignore old scan result for " MACSTR " (rx_time=%u.%06u find_start=%u.%06u)", MAC2STR(bssid),
                (unsigned int)rx_time->sec, (unsigned int)rx_time->usec, (unsigned int)p2p->find_start.sec,
...
void p2p_scan_ie(struct p2p_data *p2p, struct wpabuf *ies, const u8 *dev_id, unsigned int bands)
{
    u8 dev_capab;
    u8 *len;
...
static void p2p_go_neg_req_cb(struct p2p_data *p2p, int success)
{
    ...
    if (!success && (dev->info.dev_capab & P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY) &&
...
static void p2p_go_neg_conf_cb(struct p2p_data *p2p, enum p2p_send_action_result result)
{
    ...
    if (result == P2P_SEND_ACTION_NO_ACK)
    {
        ...
        if (dev && dev->go_neg_conf && dev->go_neg_conf_sent <= P2P_GO_NEG_CNF_MAX_RETRY_COUNT)
        {
            ...
            if (p2p_send_action(p2p, dev->go_neg_conf_freq, dev->info.p2p_device_addr, p2p->cfg->dev_addr,
                                dev->info.p2p_device_addr, wpabuf_head(dev->go_neg_conf), wpabuf_len(dev->go_neg_conf),
                                0) >= 0)
            {
                        ...
                        enum p2p_send_action_result result)
{
    ...
    p2p_dbg(p2p,
            "Action frame TX callback (state=%d freq=%u dst=" MACSTR " src=" MACSTR " bssid=" MACSTR
            " result=%d p2p_state=%s)",
            p2p->pending_action_state, freq, MAC2STR(dst), MAC2STR(src), MAC2STR(bssid), result,
...
void p2p_listen_cb(struct p2p_data *p2p, unsigned int freq, unsigned int duration)
{
    ...
    {
        p2p_dbg(p2p, "Unexpected listen callback for freq=%u duration=%u (pending_listen_freq=%u)", freq, duration,
...
int p2p_listen_end(struct p2p_data *p2p, unsigned int freq)
{
    ...
    if (p2p->state == P2P_WAIT_PEER_CONNECT && p2p->go_neg_peer && p2p->pending_listen_freq)
    {
        /*
         ...
         */
        p2p_dbg(p2p, "Listen operation did not seem to start - delay idle phase to avoid busy loop");
...
static void p2p_timeout_connect(struct p2p_data *p2p)
{
    ...
    if (p2p->go_neg_peer && (p2p->go_neg_peer->flags & P2P_DEV_PEER_WAITING_RESPONSE) &&
    ...
    else
    {
        ...
        if (p2p->cfg->prov_disc_fail)
            p2p->cfg->prov_disc_fail(p2p->cfg->cb_ctx, p2p->pending_pd_devaddr,
                                     for_join ? P2P_PROV_DISC_TIMEOUT_JOIN : P2P_PROV_DISC_TIMEOUT, adv_id, adv_mac,
                                     NULL);
        p2p_reset_pending_pd(p2p);
    }
}
    ...
    else
    {
        if (p2p->invite_peer)
        {
            ...
            if (p2p->cfg->invitation_result)
                p2p->cfg->invitation_result(p2p->cfg->cb_ctx, -1, NULL, NULL, p2p->invite_peer->info.p2p_device_addr, 0,
                                            0);
        }
        p2p_set_state(p2p, P2P_IDLE);
    }
...
int p2p_get_peer_info_txt(const struct p2p_peer_info *info, char *buf, size_t buflen)
{
    ...
    res = os_snprintf(
        ...
        "status=%d\n"
        "invitation_reqs=%u\n",
        (int)(now.sec - dev->last_seen.sec), dev->listen_freq, p2p_wps_method_text(dev->wps_method),
        MAC2STR(dev->interface_addr), MAC2STR(dev->member_in_go_dev), MAC2STR(dev->member_in_go_iface),
        dev->go_neg_req_sent, p2p_go_state_text(dev->go_state), dev->dialog_token, MAC2STR(dev->intended_addr),
        dev->country[0] ? dev->country[0] : '_', dev->country[1] ? dev->country[1] : '_', dev->oper_freq,
        dev->req_config_methods, dev->flags & P2P_DEV_PROBE_REQ_ONLY ? "[PROBE_REQ_ONLY]" : "",
        dev->flags & P2P_DEV_REPORTED ? "[REPORTED]" : "", dev->flags & P2P_DEV_NOT_YET_READY ? "[NOT_YET_READY]" : "",
        ...
        dev->flags & P2P_DEV_FORCE_FREQ ? "[FORCE_FREQ]" : "", dev->flags & P2P_DEV_PD_FOR_JOIN ? "[PD_FOR_JOIN]" : "",
        dev->flags & P2P_DEV_LAST_SEEN_AS_GROUP_CLIENT ? "[LAST_SEEN_AS_GROUP_CLIENT]" : "", dev->status,
        dev->invitation_reqs);
    ...
    if (dev->info.wfd_subelems)
    {
        ...
        pos +=
            wpa_snprintf_hex(pos, end - pos, wpabuf_head(dev->info.wfd_subelems), wpabuf_len(dev->info.wfd_subelems));
                     ...
                     u32 interval2)
{
    ...
    p2p_dbg(p2p,
            ...
            "dur2=%u int2=%u",
            MAC2STR(go_interface_addr), MAC2STR(own_interface_addr), freq, duration1, interval1, duration2, interval2);
    ...
    p2p->pending_action_state = P2P_NO_PENDING_ACTION;
    if (p2p_send_action(p2p, freq, go_interface_addr, own_interface_addr, go_interface_addr, wpabuf_head(req),
...
static void p2p_ext_listen_timeout(void *eloop_ctx, void *timeout_ctx)
{
    ...
    if (p2p->ext_listen_interval)
    {
        ...
        eloop_register_timeout(p2p->ext_listen_interval_sec, p2p->ext_listen_interval_usec, p2p_ext_listen_timeout, p2p,
                               NULL);
    }
...
{
    if (period > 65535 || interval > 65535 || period > interval || (period == 0 && interval > 0) ||
        (period > 0 && interval == 0))
    {
...
void p2p_deauth_notif(struct p2p_data *p2p, const u8 *bssid, u16 reason_code, const u8 *ie, size_t ie_len)
{
    ...
    p2p_dbg(p2p, "Deauthentication notification BSSID " MACSTR " reason_code=%u minor_reason_code=%u", MAC2STR(bssid),
            reason_code, *msg.minor_reason_code);
...
void p2p_disassoc_notif(struct p2p_data *p2p, const u8 *bssid, u16 reason_code, const u8 *ie, size_t ie_len)
{
    ...
    p2p_dbg(p2p, "Disassociation notification BSSID " MACSTR " reason_code=%u minor_reason_code=%u", MAC2STR(bssid),
            reason_code, *msg.minor_reason_code);
...
{
    return p2p_channel_random_social(&p2p->channels, op_class, op_channel, avoid_list, disallow_list);
}
...
int p2p_set_pref_chan(struct p2p_data *p2p, unsigned int num_pref_chan, const struct p2p_channel *pref_chan)
{
    ...
    if (pref_chan)
    {
        n = os_memdup(pref_chan, num_pref_chan * sizeof(struct p2p_channel));
        if (n == NULL)
            return -1;
    }
    else
...
int p2p_send_action(struct p2p_data *p2p,
                    unsigned int freq,
                    const u8 *dst,
                    const u8 *src,
                    const u8 *bssid,
                    ...
                    unsigned int wait_time)
{
    ...
    res = p2p->cfg->send_action(p2p->cfg->cb_ctx, freq, dst, src, bssid, buf, len, wait_time, &scheduled);
    if (res == 0 && scheduled && p2p->in_listen && freq > 0 && p2p->drv_in_listen > 0 &&
        (unsigned int)p2p->drv_in_listen != freq)
    {
        p2p_dbg(p2p, "Stop listen on %d MHz to allow a frame to be sent immediately on %d MHz", p2p->drv_in_listen,
                freq);
        p2p_stop_listen_for_freq(p2p, freq);
    }
    return res;
...
void p2p_loop_on_known_peers(struct p2p_data *p2p,
                             void (*peer_callback)(struct p2p_peer_info *peer, void *user_data),
                             void *user_data)
{
    struct p2p_device *dev, *n;
...
static struct wpabuf *p2p_build_nfc_handover(
    struct p2p_data *p2p, int client_freq, const u8 *go_dev_addr, const u8 *ssid, size_t ssid_len)
{
    struct wpabuf *buf;
    u8 op_class, channel;
...
struct wpabuf *p2p_build_nfc_handover_req(
    struct p2p_data *p2p, int client_freq, const u8 *go_dev_addr, const u8 *ssid, size_t ssid_len)
{
    return p2p_build_nfc_handover(p2p, client_freq, go_dev_addr, ssid, ssid_len);
}

struct wpabuf *p2p_build_nfc_handover_sel(
    struct p2p_data *p2p, int client_freq, const u8 *go_dev_addr, const u8 *ssid, size_t ssid_len)
{
    return p2p_build_nfc_handover(p2p, client_freq, go_dev_addr, ssid, ssid_len);
}
...
void p2p_set_own_pref_freq_list(struct p2p_data *p2p, const unsigned int *pref_freq_list, unsigned int size)
{
    ...
    for (i = 0; i < size; i++)
    {
        ...
        p2p_dbg(p2p, "Own preferred frequency list[%u]=%u MHz", i, p2p->pref_freq_list[i]);
    }
}

void p2p_set_override_pref_op_chan(struct p2p_data *p2p, u8 op_class, u8 chan)
{
    p2p->override_pref_op_class = op_class;
    p2p->override_pref_channel  = chan;
}

struct wpabuf *p2p_build_probe_resp_template(struct p2p_data *p2p, unsigned int freq)
{
