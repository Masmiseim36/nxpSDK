...
void p2p_buf_add_pref_channel_list(struct wpabuf *buf, const unsigned int *preferred_freq_list, unsigned int size)
{
    ...
    for (i = 0; i < size; i++)
    {
        if (p2p_freq_to_channel(preferred_freq_list[i], &op_class, &op_channel) == 0)
            count++;
    }
    ...
    for (i = 0; i < size; i++)
    {
        if (p2p_freq_to_channel(preferred_freq_list[i], &op_class, &op_channel) < 0)
        {
            wpa_printf(MSG_DEBUG, "Unsupported frequency %u MHz", preferred_freq_list[i]);
            continue;
        }
...
void p2p_buf_add_device_info(struct wpabuf *buf, struct p2p_data *p2p, struct p2p_device *peer)
{
    ...
    else if (p2p->cfg->config_methods)
    {
        methods |= p2p->cfg->config_methods &
                   (WPS_CONFIG_PUSHBUTTON | WPS_CONFIG_DISPLAY | WPS_CONFIG_KEYPAD | WPS_CONFIG_P2PS);
    }
    else
    {
        methods |= WPS_CONFIG_PUSHBUTTON;
...
void p2p_buf_add_noa(
    struct wpabuf *buf, u8 noa_index, u8 opp_ps, u8 ctwindow, struct p2p_noa_desc *desc1, struct p2p_noa_desc *desc2)
{
    /* Notice of Absence */
...
void p2p_buf_add_oob_go_neg_channel(
    struct wpabuf *buf, const char *country, u8 oper_class, u8 channel, enum p2p_role_indication role)
{
                                    ...
                                    u8 *attr_len)
{
    ...
    if (svc_len > 255)
    {
        p2p_dbg(p2p, "Invalid service name length (%u bytes), failed to add advertised service info",
...
void p2p_buf_add_service_instance(
    struct wpabuf *buf, struct p2p_data *p2p, u8 hash_count, const u8 *hash, struct p2ps_advertisement *adv_list)
{
    struct p2ps_advertisement *adv;
    int p2ps_wildcard;
    size_t total_len;
    ...
    for (adv = adv_list; adv && total_len <= MAX_SVC_ADV_LEN; adv = adv->next)
    {
        ...
        for (i = 0; i < hash_count; i++)
        {
            ...
            if (os_memcmp(test, adv->hash, P2PS_HASH_LEN) == 0 &&
                p2p_buf_add_service_info(tmp_buf, p2p, adv->id, adv->config_methods, adv->svc_name, &ie_len, &pos,
                                         &total_len, attr_len))
                break;
...
int p2p_build_wps_ie(struct p2p_data *p2p, struct wpabuf *buf, int pw_id, int all_attr)
{
    ...
    if (all_attr)
    {
        ...
        if (wps_build_uuid_e(buf, p2p->cfg->uuid) < 0 ||
            p2p_add_wps_string(buf, ATTR_MANUFACTURER, p2p->cfg->manufacturer) < 0 ||
            p2p_add_wps_string(buf, ATTR_MODEL_NAME, p2p->cfg->model_name) < 0 ||
            p2p_add_wps_string(buf, ATTR_MODEL_NUMBER, p2p->cfg->model_number) < 0 ||
            p2p_add_wps_string(buf, ATTR_SERIAL_NUMBER, p2p->cfg->serial_number) < 0)
            return -1;
