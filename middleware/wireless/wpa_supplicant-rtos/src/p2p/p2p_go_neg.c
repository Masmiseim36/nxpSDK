...
int p2p_peer_channels_check(struct p2p_data *p2p,
                            struct p2p_channels *own,
                            ...
                            size_t channel_list_len)
{
    ...
    {
        p2p_info(p2p, "Mismatching country (ours=%c%c peer's=%c%c)", p2p->cfg->country[0], p2p->cfg->country[1], pos[0],
                 pos[1]);
        return -1;
    }
    pos += 3;
    ...
    p2p_dbg(p2p, "Own reg_classes %d peer reg_classes %d intersection reg_classes %d", (int)own->reg_classes,
            (int)dev->channels.reg_classes, (int)intersection.reg_classes);
...
static int p2p_peer_channels(struct p2p_data *p2p,
                             struct p2p_device *dev,
                             const u8 *channel_list,
                             size_t channel_list_len)
{
...
int p2p_connect_send(struct p2p_data *p2p, struct p2p_device *dev)
{
    ...
    if (freq <= 0)
    {
        p2p_dbg(p2p, "No Listen/Operating frequency known for the peer " MACSTR " to send GO Negotiation Request",
...
static struct wpabuf *p2p_build_go_neg_resp(
    struct p2p_data *p2p, struct p2p_device *peer, u8 dialog_token, u8 status, u8 tie_breaker)
{
    struct wpabuf *buf;
    u8 *len;
    u8 group_capab;
    ...
    if (p2p->override_pref_op_class)
    {
        ...
        p2p_buf_add_operating_channel(buf, p2p->cfg->country, p2p->override_pref_op_class, p2p->override_pref_channel);
    }
...
void p2p_reselect_channel(struct p2p_data *p2p, struct p2p_channels *intersection)
{
    ...
    if (p2p->own_freq_preference > 0 &&
        p2p_freq_to_channel(p2p->own_freq_preference, &op_reg_class, &op_channel) == 0 &&
        p2p_channels_includes(intersection, op_reg_class, op_channel))
    {
        p2p_dbg(p2p, "Pick own channel preference (reg_class %u channel %u) from intersection", op_reg_class,
                op_channel);
    ...
    if (p2p->best_freq_overall > 0 && p2p_freq_to_channel(p2p->best_freq_overall, &op_reg_class, &op_channel) == 0 &&
        p2p_channels_includes(intersection, op_reg_class, op_channel))
    {
    ...
    for (i = 0; p2p->cfg->pref_chan && i < p2p->cfg->num_pref_chan; i++)
    {
        if (p2p_channels_includes(intersection, p2p->cfg->pref_chan[i].op_class, p2p->cfg->pref_chan[i].chan))
        {
            ...
            p2p_dbg(p2p, "Pick highest preferred channel (op_class %u channel %u) from intersection", p2p->op_reg_class,
                    p2p->op_channel);
            return;
        }
    }
    ...
    if (p2p_channel_select(intersection, op_classes_edmg, &p2p->op_reg_class, &p2p->op_channel) == 0)
    {
        p2p_dbg(p2p, "Pick possible EDMG channel (op_class %u channel %u) from intersection", p2p->op_reg_class,
                p2p->op_channel);
        return;
    }
    ...
    {
        p2p_dbg(p2p, "Pick possible VHT channel (op_class %u channel %u) from intersection", p2p->op_reg_class,
                p2p->op_channel);
        return;
    }
    ...
    {
        p2p_dbg(p2p, "Pick possible HT40 channel (op_class %u channel %u) from intersection", p2p->op_reg_class,
                p2p->op_channel);
        return;
    }
    ...
    {
        p2p_dbg(p2p, "Pick possible 5 GHz channel (op_class %u channel %u) from intersection", p2p->op_reg_class,
                p2p->op_channel);
        return;
    }
...
int p2p_go_select_channel(struct p2p_data *p2p, struct p2p_device *dev, u8 *status)
{
    ...
    {
        p2p_dbg(p2p,
                "Try to optimize channel selection with peer information received; previously selected op_class %u "
                "channel %u",
                p2p->op_reg_class, p2p->op_channel);
        p2p_reselect_channel(p2p, &intersection);
    }
...
static void p2p_check_pref_chan_no_recv(struct p2p_data *p2p,
                                        int go,
                                        struct p2p_device *dev,
                                        ...
                                        unsigned int size)
{
    ...
    p2p_dbg(p2p,
            "Peer didn't provide a preferred frequency list, see if any of our preferred channels are supported by "
            "peer device");
    ...
    for (i = 0; i < size && !found; i++)
    {
        ...
        {
            if (!msg->channel_list || op_channel != msg->channel_list[j])
                continue;
    ...
    {
        p2p_dbg(p2p, "None of our preferred channels are supported by peer!");
    }
}

static void p2p_check_pref_chan_recv(struct p2p_data *p2p,
                                     int go,
                                     struct p2p_device *dev,
                                     struct p2p_message *msg,
                                     unsigned freq_list[],
                                     unsigned int size)
{
    ...
    for (i = 0; i < size && !found; i++)
    {
        ...
        {
            oper_freq = p2p_channel_to_freq(msg->pref_freq_list[2 * j], msg->pref_freq_list[2 * j + 1]);
    ...
    if (found)
    {
        p2p_dbg(p2p, "Freq %d MHz is a common preferred channel for both peer and local, use it as operating channel",
                oper_freq);
    }
    else
    {
...
void p2p_check_pref_chan(struct p2p_data *p2p, int go, struct p2p_device *dev, struct p2p_message *msg)
{
     ...
     */
    if (!p2p->cfg->get_pref_freq_list || p2p->cfg->num_pref_chan || (dev->flags & P2P_DEV_FORCE_FREQ) ||
    ...
    while (i < size)
    {
        if (p2p_freq_to_channel(freq_list[i], &op_class, &op_channel) < 0 ||
            (!p2p_channels_includes(&p2p->cfg->channels, op_class, op_channel) &&
             (go || !p2p_channels_includes(&p2p->cfg->cli_channels, op_class, op_channel))))
        {
            p2p_dbg(p2p,
                    ...
                    freq_list[i], go);
            if (size - i - 1 > 0)
                os_memmove(&freq_list[i], &freq_list[i + 1], (size - i - 1) * sizeof(unsigned int));
            size--;
            continue;
        }
    ...
    *pos = '\0';
    p2p_dbg(p2p, "Local driver frequency preference (size=%u):%s", size, txt);
    ...
    if (i != size && p2p_freq_to_channel(freq_list[i], &op_class, &op_channel) == 0)
    {
    ...
    p2p_dbg(p2p, "Peer operating channel preference: %d MHz is not in our preferred channel list", dev->oper_freq);
...
void p2p_process_go_neg_req(struct p2p_data *p2p, const u8 *sa, const u8 *data, size_t len, int rx_freq)
{
    ...
    {
        p2p_dbg(p2p, "Unexpected GO Negotiation Request SA=" MACSTR " != dev_addr=" MACSTR, MAC2STR(sa),
    ...
    else if (!dev->listen_freq && !dev->oper_freq)
    {
         ...
         */
        p2p_dbg(p2p, "Update peer " MACSTR " based on GO Neg Req since listen/oper freq not known",
                MAC2STR(dev->info.p2p_device_addr));
        p2p_add_dev_info(p2p, sa, dev, &msg);
    }
    ...
    if (dev && dev->flags & P2P_DEV_USER_REJECTED)
    {
        ...
        status = P2P_SC_FAIL_REJECTED_BY_USER;
             ...
             (dev->wps_method == WPS_NOT_READY &&
              (p2p->authorized_oob_dev_pw_id == 0 || p2p->authorized_oob_dev_pw_id != msg.dev_password_id)))
    {
    ...
    else
    {
        ...
        {
            p2p_dbg(p2p,
                    "Do not reply since peer is waiting for us to start a new GO Negotiation and GO Neg Request "
                    "already sent");
            p2p_parse_free(&msg);
            return;
        }
...
static struct wpabuf *p2p_build_go_neg_conf(
    struct p2p_data *p2p, struct p2p_device *peer, u8 dialog_token, u8 status, const u8 *resp_chan, int go)
{
    struct wpabuf *buf;
    u8 *len;
    struct p2p_channels res;
...
fail:
    ...
    if (p2p_send_action(p2p, freq, sa, p2p->cfg->dev_addr, sa, wpabuf_head(dev->go_neg_conf),
...
void p2p_process_go_neg_conf(struct p2p_data *p2p, const u8 *sa, const u8 *data, size_t len)
{
    ...
    {
        int oper_freq = p2p_channel_to_freq(msg.operating_channel[3], msg.operating_channel[4]);
        if (oper_freq != dev->oper_freq)
        {
            p2p_dbg(p2p, "Updated peer (GO) operating channel preference from %d MHz to %d MHz", dev->oper_freq,
                    oper_freq);
