...
int p2p_group_notif_assoc(struct p2p_group *group, const u8 *addr, const u8 *ie, size_t len)
{
    ...
    group->members = m;
    group->num_members++;
    p2p_dbg(group->p2p, "Add client " MACSTR " to group (p2p=%d wfd=%d client_info=%d); num_members=%u/%u",
            MAC2STR(addr), m->p2p_ie ? 1 : 0, m->wfd_ie ? 1 : 0, m->client_info ? 1 : 0, group->num_members,
...
void p2p_group_notif_disassoc(struct p2p_group *group, const u8 *addr)
{
    ...
    {
        p2p_dbg(group->p2p, "Remove client " MACSTR " from group; num_members=%u/%u", MAC2STR(addr), group->num_members,
                group->cfg->max_clients);
...
int p2p_group_go_discover(struct p2p_group *group, const u8 *dev_id, const u8 *searching_dev, int rx_freq)
{
    ...
    if (p2p->cfg->send_action(p2p->cfg->cb_ctx, freq, m->addr, group->cfg->interface_addr, group->cfg->interface_addr,
                              wpabuf_head(req), wpabuf_len(req), 200, NULL) < 0)
    {
...
u8 p2p_group_presence_req(struct p2p_group *group, const u8 *client_interface_addr, const u8 *noa, size_t noa_len)
{
    ...
    if (group->p2p->cfg->get_noa)
        curr_noa_len =
            group->p2p->cfg->get_noa(group->p2p->cfg->cb_ctx, group->cfg->interface_addr, curr_noa, sizeof(curr_noa));
    else
        curr_noa_len = -1;
    if (curr_noa_len < 0)
...

{
    ...
    p2p_channels_dump(group->p2p, "Group common freqs before iterating members", &intersect);
