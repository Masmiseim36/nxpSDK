...
void p2p_process_invitation_req(struct p2p_data *p2p, const u8 *sa, const u8 *data, size_t len, int rx_freq)
{
    ...
    {
        status =
            p2p->cfg->invitation_process(p2p->cfg->cb_ctx, sa, msg.group_bssid, msg.group_id, msg.group_id + ETH_ALEN,
                                         msg.group_id_len - ETH_ALEN, &go, group_bssid, &op_freq, persistent,
                                         &intersection, msg.dev_password_id_present ? msg.dev_password_id : -1);
    }
    ...
    if (op_freq)
    {
        ...
        {
            p2p_dbg(p2p, "forced freq %d MHz not in the supported channels intersection", op_freq);
            status = P2P_SC_FAIL_NO_COMMON_CHANNELS;
            goto fail;
        }
    ...
    else
    {
        ...
        if (msg.operating_channel)
        {
            ...
            if (req_freq > 0 &&
                p2p_channels_includes(&intersection, msg.operating_channel[3], msg.operating_channel[4]))
            {
        ...
        {
            p2p_dbg(p2p,
                    "Try to reselect channel selection with peer information received; previously selected op_class %u "
                    "channel %u",
                    p2p->op_reg_class, p2p->op_channel);
            p2p_reselect_channel(p2p, &intersection);
        }
        ...
        if (op_freq < 0)
        {
            p2p_dbg(p2p, "Unknown operational channel (country=%c%c reg_class=%u channel=%u)", p2p->cfg->country[0],
                    p2p->cfg->country[1], p2p->op_reg_class, p2p->op_channel);
            status = P2P_SC_FAIL_NO_COMMON_CHANNELS;
            goto fail;
        }
...
void p2p_process_invitation_resp(struct p2p_data *p2p, const u8 *sa, const u8 *data, size_t len)
{
     ...
     */
    if (*msg.status == P2P_SC_FAIL_NO_COMMON_CHANNELS && p2p->retry_invite_req_sent &&
    ...
    if (*msg.status == P2P_SC_FAIL_NO_COMMON_CHANNELS && p2p->retry_invite_req &&
        p2p_channel_random_social(&p2p->cfg->channels, &p2p->op_reg_class, &p2p->op_channel, NULL, NULL) == 0)
    {
        p2p->retry_invite_req = 0;
        p2p->cfg->send_action_done(p2p->cfg->cb_ctx);
        p2p->cfg->stop_listen(p2p->cfg->cb_ctx);
        p2p_set_state(p2p, P2P_INVITE);
        p2p_dbg(p2p, "Resend Invitation Request setting op_class %u channel %u as operating channel", p2p->op_reg_class,
                p2p->op_channel);
...
int p2p_invite_send(struct p2p_data *p2p, struct p2p_device *dev, const u8 *go_dev_addr, int dev_pw_id)
{
    ...
    if (freq <= 0)
    {
        p2p_dbg(p2p, "No Listen/Operating frequency known for the peer " MACSTR " to send Invitation Request",
...
void p2p_invitation_resp_cb(struct p2p_data *p2p, int success)
{
    ...
    {
        p2p->cfg->invitation_received(p2p->cfg->cb_ctx, p2p->inv_sa, p2p->inv_group_bssid_ptr, p2p->inv_ssid,
                                      p2p->inv_ssid_len, p2p->inv_go_dev_addr, p2p->inv_status, p2p->inv_op_freq);
    }
}
               ...
               int dev_pw_id)
{
    ...
    p2p_dbg(p2p,
            "Request to invite peer " MACSTR
            " role=%d persistent=%d "
            "force_freq=%u allow_6ghz=%d",
            MAC2STR(peer), role, persistent_group, force_freq, p2p->allow_6ghz);
    if (bssid)
    ...
    if (dev->flags & P2P_DEV_GROUP_CLIENT_ONLY)
    {
        ...
        {
            p2p_dbg(p2p, "Cannot invite a P2P Device " MACSTR " that is in a group and is not discoverable",
                    MAC2STR(peer));
        }
