...
static int p2p_parse_attribute(u8 id, const u8 *data, u16 len, struct p2p_message *msg)
{
    ...
    switch (id)
    {
        ...
        case P2P_ATTR_DEVICE_INFO:
            ...
            p2p_copy_filter_devname(msg->device_name, sizeof(msg->device_name), pos, nlen);
            wpa_printf(MSG_DEBUG,
                       "P2P: * Device Info: addr " MACSTR
                       " primary device type %s device name '%s' "
                       "config methods 0x%x",
                       MAC2STR(msg->p2p_device_addr), wps_dev_type_bin2str(msg->pri_dev_type, devtype, sizeof(devtype)),
        ...
        case P2P_ATTR_GROUP_ID:
            ...
            wpa_hexdump_ascii(MSG_DEBUG, "P2P: * P2P Group ID: SSID", msg->group_id + ETH_ALEN,
        ...
        case P2P_ATTR_EXT_LISTEN_TIMING:
            ...
            wpa_printf(MSG_DEBUG,
                       ...
                       WPA_GET_LE16(msg->ext_listen_timing), WPA_GET_LE16(msg->ext_listen_timing + 2));
            break;
        ...
        case P2P_ATTR_PERSISTENT_GROUP:
        {
            ...
            wpa_printf(MSG_DEBUG, "P2P: * Persistent Group: " MACSTR " %s", MAC2STR(msg->persistent_dev),
...
static int p2p_group_info_text(const u8 *gi, size_t gi_len, char *buf, char *end)
{
    ...
    for (i = 0; i < info.num_clients; i++)
    {
        ...
        ret = os_snprintf(pos, end - pos,
                          ...
                          MAC2STR(cli->p2p_device_addr), MAC2STR(cli->p2p_interface_addr));
        ...
        for (s = 0; s < cli->num_sec_dev_types; s++)
        {
            ret = os_snprintf(pos, end - pos, " dev_type=%s",
                              wps_dev_type_bin2str(&cli->sec_dev_types[s * 8], devtype, sizeof(devtype)));
            if (os_snprintf_error(end - pos, ret))
                return pos - buf;
            pos += ret;
        }
