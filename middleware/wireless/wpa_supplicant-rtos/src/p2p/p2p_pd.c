...
static void p2ps_add_new_group_info(struct p2p_data *p2p, struct p2p_device *dev, struct wpabuf *buf)
{
    int found;
    ...
    found = p2p->cfg->get_go_info(p2p->cfg->cb_ctx, intended_addr, ssid, &ssid_len, &group_iface, &force_freq);
    if (found)
    {
        if (force_freq > 0)
        {
...
static void p2ps_add_pd_req_attrs(struct p2p_data *p2p, struct p2p_device *dev, struct wpabuf *buf, u16 config_methods)
{
    ...
    u8 intended_addr[ETH_ALEN];
    int follow_on_req_fail = prov->status >= 0 && prov->status != P2P_SC_SUCCESS_DEFERRED;
    ...
    if (!follow_on_req_fail)
    {
        ...
        {
            shared_group = p2p->cfg->get_persistent_group(p2p->cfg->cb_ctx, dev->info.p2p_device_addr, NULL, 0,
    ...
    if (shared_group)
    {
        ...
        if ((prov->conncap == P2PS_SETUP_NONE || prov->conncap == P2PS_SETUP_CLIENT) &&
                                               ...
                                               u16 fcap_len)
{
    ...
    if (p2p->p2ps_prov && p2p->p2ps_prov->adv_id == adv_id)
    {
            ...
            (status == P2P_SC_SUCCESS || status == P2P_SC_SUCCESS_DEFERRED))
        {
            ...
            persist = p2p->cfg->get_persistent_group(p2p->cfg->cb_ctx, dev->info.p2p_device_addr, persist_ssid,
...
static int p2ps_setup_p2ps_prov(
    struct p2p_data *p2p, u32 adv_id, u32 session_id, u16 method, const u8 *session_mac, const u8 *adv_mac)
{
...
static int p2ps_validate_pd_req(struct p2p_data *p2p, struct p2p_message *msg, const u8 *addr)
{
    u8 group_id          = 0;
    u8 intended_addr     = 0;
...
void p2p_process_prov_disc_req(struct p2p_data *p2p, const u8 *sa, const u8 *data, size_t len, int rx_freq)
{
    ...
    p2p_dbg(p2p, "Received Provision Discovery Request from " MACSTR " with config methods 0x%x (freq=%d)", MAC2STR(sa),
            msg.wps_config_methods, rx_freq);
    ...
    if (dev == NULL || (dev->flags & P2P_DEV_PROBE_REQ_ONLY))
    {
        ...
        if (!dev)
        {
            p2p_dbg(p2p, "Provision Discovery device not found " MACSTR, MAC2STR(sa));
            goto out;
        }
    }
    ...
    {
        p2p->cfg->remove_stale_groups(p2p->cfg->cb_ctx, dev->info.p2p_device_addr, msg.persistent_dev,
    ...
    if (!msg.status)
    {
        ...
        conncap     = p2p->cfg->p2ps_group_capability(p2p->cfg->cb_ctx, conncap, auto_accept, &forced_freq, &pref_freq);
        ...
        resp_fcap.cpt = p2ps_own_preferred_cpt(p2ps_adv->cpt_priority, req_fcap->cpt);

        p2p_dbg(p2p, "cpt: service:0x%x remote:0x%x result:0x%x", p2ps_adv->cpt_mask, req_fcap->cpt, resp_fcap.cpt);
        ...
        else if (p2ps_adv->config_methods && !(msg.wps_config_methods & p2ps_adv->config_methods))
        {
            p2p_dbg(p2p, "Unsupported config methods in Provision Discovery Request (own=0x%x peer=0x%x)",
        ...
        if ((remote_conncap & (P2PS_SETUP_NEW | P2PS_SETUP_CLIENT) || msg.persistent_dev) &&
            conncap != P2PS_SETUP_NEW && msg.channel_list && msg.channel_list_len &&
            p2p_peer_channels_check(p2p, &p2p->channels, dev, msg.channel_list, msg.channel_list_len) < 0)
        {
            p2p_dbg(p2p, "No common channels - force deferred flow");
            auto_accept = 0;
        }

        if (((remote_conncap & P2PS_SETUP_GROUP_OWNER) || msg.persistent_dev) && msg.operating_channel)
        {
            ...
            if (dev->channels.reg_classes == 0 ||
                !p2p_channels_includes(&dev->channels, msg.operating_channel[3], msg.operating_channel[4]))
            {
    ...
    if (p2p->p2ps_prov->adv_id != adv_id || os_memcmp(p2p->p2ps_prov->adv_mac, msg.adv_mac, ETH_ALEN))
    {
        p2p_dbg(p2p, "P2PS Follow-on PD with mismatch Advertisement ID/MAC");
        goto out;
    }

    if (p2p->p2ps_prov->session_id != session_id || os_memcmp(p2p->p2ps_prov->session_mac, msg.session_mac, ETH_ALEN))
    {
    ...
    resp_fcap.cpt = p2ps_own_preferred_cpt(p2p->p2ps_prov->cpt_priority, req_fcap->cpt);

    p2p_dbg(p2p, "cpt: local:0x%x remote:0x%x result:0x%x", p2p->p2ps_prov->cpt_mask, req_fcap->cpt, resp_fcap.cpt);

    p2p_prepare_channel(p2p, dev, p2p->p2ps_prov->force_freq, p2p->p2ps_prov->pref_freq, 0);
    ...
    else if ((remote_conncap & (P2PS_SETUP_NEW | P2PS_SETUP_CLIENT) || msg.persistent_dev) &&
             conncap != P2PS_SETUP_NEW && msg.channel_list && msg.channel_list_len &&
             p2p_peer_channels_check(p2p, &p2p->channels, dev, msg.channel_list, msg.channel_list_len) < 0)
    {
    ...
    if (reject == P2P_SC_SUCCESS || reject == P2P_SC_SUCCESS_DEFERRED)
    {
        ...
        if ((conncap & P2PS_SETUP_GROUP_OWNER) && p2p_go_select_channel(p2p, dev, &tmp) < 0)
            reject = P2P_SC_FAIL_NO_COMMON_CHANNELS;
    }
...
out:
    ...
    if (!msg.status || *msg.status == P2P_SC_SUCCESS_DEFERRED)
    {
        resp = p2p_build_prov_disc_resp(p2p, dev, msg.dialog_token, reject, config_methods, adv_id, msg.group_id,
                                        msg.group_id_len, msg.persistent_ssid, msg.persistent_ssid_len,
        ...
        if (p2p_send_action(p2p, freq, sa, p2p->cfg->dev_addr, p2p->cfg->dev_addr, wpabuf_head(resp), wpabuf_len(resp),
                            50) < 0)
    ...
    {
        reject = *msg.status;
        p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, reject, sa, adv_mac, session_mac, NULL, adv_id, session_id, 0, 0,
                                     msg.persistent_ssid, msg.persistent_ssid_len, 0, 0, NULL, NULL, 0, freq, NULL, 0);
    }
    else if (msg.status && *msg.status == P2P_SC_SUCCESS_DEFERRED && p2p->p2ps_prov)
    {
        ...
        if (reject != P2P_SC_SUCCESS)
            p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, reject, sa, adv_mac, session_mac, NULL, adv_id, session_id,
                                         conncap, 0, msg.persistent_ssid, msg.persistent_ssid_len, 0, 0, NULL, NULL, 0,
                                         freq, NULL, 0);
        else
            p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, *msg.status, sa, adv_mac, session_mac, group_mac, adv_id,
                                         session_id, conncap, passwd_id, msg.persistent_ssid, msg.persistent_ssid_len,
                                         0, 0, NULL, (const u8 *)&resp_fcap, sizeof(resp_fcap), freq, NULL, 0);
    }
    else if (msg.status && p2p->p2ps_prov)
    {
        p2p->p2ps_prov->status = P2P_SC_SUCCESS;
        p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, *msg.status, sa, adv_mac, session_mac, group_mac, adv_id,
                                     session_id, conncap, passwd_id, msg.persistent_ssid, msg.persistent_ssid_len, 0, 0,
                                     NULL, (const u8 *)&resp_fcap, sizeof(resp_fcap), freq, NULL, 0);
    }
    else if (msg.status)
    {
    }
    ...
    {
        p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, P2P_SC_SUCCESS, sa, adv_mac, session_mac, group_mac, adv_id,
                                     session_id, conncap, passwd_id, msg.persistent_ssid, msg.persistent_ssid_len, 0, 0,
                                     NULL, (const u8 *)&resp_fcap, sizeof(resp_fcap), freq,
    ...
    else if (reject == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE && (!msg.session_info || !msg.session_info_len))
    {
        ...
        p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, P2P_SC_SUCCESS, sa, adv_mac, session_mac, group_mac, adv_id,
                                     session_id, conncap, passwd_id, msg.persistent_ssid, msg.persistent_ssid_len, 0, 1,
                                     NULL, (const u8 *)&resp_fcap, sizeof(resp_fcap), freq, NULL, 0);
    }
    else if (reject == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE)
    {
        ...
        if (buf)
        {
            ...
            p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, P2P_SC_SUCCESS, sa, adv_mac, session_mac, group_mac, adv_id,
                                         session_id, conncap, passwd_id, msg.persistent_ssid, msg.persistent_ssid_len,
                                         0, 1, buf, (const u8 *)&resp_fcap, sizeof(resp_fcap), freq, NULL, 0);
    ...
    if (p2p->cfg->prov_disc_req &&
        ((reject == P2P_SC_SUCCESS && !msg.adv_id) ||
         (!msg.status && (reject == P2P_SC_SUCCESS || reject == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE) &&
          passwd_id == DEV_PW_USER_SPECIFIED) ||
         (!msg.status && reject == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE && passwd_id == DEV_PW_REGISTRAR_SPECIFIED) ||
         (reject == P2P_SC_SUCCESS && msg.status && *msg.status == P2P_SC_SUCCESS_DEFERRED &&
          passwd_id == DEV_PW_REGISTRAR_SPECIFIED)))
    {
        ...
        p2p->cfg->prov_disc_req(p2p->cfg->cb_ctx, sa, msg.wps_config_methods, dev_addr, msg.pri_dev_type,
                                msg.device_name, msg.config_methods, msg.capability ? msg.capability[0] : 0,
...
static int p2p_validate_p2ps_pd_resp(struct p2p_data *p2p, struct p2p_message *msg)
{
    u8 conn_cap_go  = 0;
    u8 conn_cap_cli = 0;
    u32 session_id;
...
#define P2PS_PD_RESP_CHECK(_val, _attr)                       \
    ...
    if (*msg->status == P2P_SC_SUCCESS && !(!!msg->conn_cap ^ !!msg->persistent_dev))
    {
        p2p_dbg(p2p, "Ignore malformed PD Response - either conn_cap or persistent group should be present");
        return -1;
    }

    if (msg->persistent_dev && *msg->status != P2P_SC_SUCCESS)
    {
        p2p_dbg(p2p, "Ignore malformed PD Response - persistent group is present, but the status isn't success");
        return -1;
    }
    ...
    P2PS_PD_RESP_CHECK(msg->persistent_dev || conn_cap_go || conn_cap_cli, channel_list);
    P2PS_PD_RESP_CHECK(msg->persistent_dev || conn_cap_go || conn_cap_cli, config_timeout);
...
void p2p_process_prov_disc_resp(struct p2p_data *p2p, const u8 *sa, const u8 *data, size_t len)
{
    ...
    p2p_dbg(p2p, "Received Provision Discovery Response from " MACSTR " with config methods 0x%x", MAC2STR(sa),
    ...
    if ((status == P2P_SC_SUCCESS || status == P2P_SC_SUCCESS_DEFERRED) && p2p->p2ps_prov)
    {
        dev->oper_freq = 0;
        ...
        if (msg.operating_channel)
        {
            if (p2p_channels_includes(&p2p->channels, msg.operating_channel[3], msg.operating_channel[4]) &&
                p2p_channels_includes(&dev->channels, msg.operating_channel[3], msg.operating_channel[4]))
            {
                dev->oper_freq = p2p_channel_to_freq(msg.operating_channel[3], msg.operating_channel[4]);
            }
        ...
        if (p2p->cfg->p2ps_prov_complete)
        {
            ...
            p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, status, sa, adv_mac, p2p->p2ps_prov->session_mac, group_mac,
                                         adv_id, p2p->p2ps_prov->session_id, conncap, passwd_id, msg.persistent_ssid,
                                         msg.persistent_ssid_len, 1, 0, NULL, msg.feature_cap, msg.feature_cap_len,
                                         freq, msg.group_id ? msg.group_id + ETH_ALEN : NULL,
                                         msg.group_id ? msg.group_id_len - ETH_ALEN : 0);
        }
        p2ps_prov_free(p2p);
    }
    else if (status != P2P_SC_SUCCESS && status != P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE &&
             status != P2P_SC_SUCCESS_DEFERRED && p2p->p2ps_prov)
    {
        if (p2p->cfg->p2ps_prov_complete)
            p2p->cfg->p2ps_prov_complete(p2p->cfg->cb_ctx, status, sa, adv_mac, p2p->p2ps_prov->session_mac, group_mac,
                                         adv_id, p2p->p2ps_prov->session_id, 0, 0, NULL, 0, 1, 0, NULL, NULL, 0, 0,
                                         NULL, 0);
        p2ps_prov_free(p2p);
    }
    ...
    if (status == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE)
    {
        ...
        if (msg.session_info && msg.session_info_len)
        {
            ...
            if (p2p->cfg->prov_disc_fail)
                p2p->cfg->prov_disc_fail(p2p->cfg->cb_ctx, sa, P2P_PROV_DISC_INFO_UNAVAILABLE, adv_id, adv_mac,
                                         deferred_sess_resp);
...
out:
    ...
    {
        p2p_dbg(p2p, "Start GO Neg after the PD-before-GO-Neg workaround with " MACSTR,
    ...
    if (p2p->cfg->prov_disc_resp &&
        ((status == P2P_SC_SUCCESS && !adv_id) ||
         (p2ps_seeker && status == P2P_SC_SUCCESS && passwd_id == DEV_PW_REGISTRAR_SPECIFIED) ||
         (p2ps_seeker && status == P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE && passwd_id == DEV_PW_USER_SPECIFIED)))
...
int p2p_send_prov_disc_req(struct p2p_data *p2p, struct p2p_device *dev, int join, int force_freq)
{
    ...
    if (freq <= 0)
    {
        p2p_dbg(p2p, "No Listen/Operating frequency known for the peer " MACSTR " to send Provision Discovery Request",
                MAC2STR(dev->info.p2p_device_addr));
        return -1;
    }
    ...
    if (dev->flags & P2P_DEV_GROUP_CLIENT_ONLY)
    {
        ...
        {
            p2p_dbg(p2p, "Cannot use PD with P2P Device " MACSTR " that is in a group and is not discoverable",
                    MAC2STR(dev->info.p2p_device_addr));
            return -1;
        }
    ...
    if (p2p->p2ps_prov)
    {
        ...
        p2p_dbg(p2p, "Building PD Request based on P2PS config method 0x%x status %d --> req_config_methods 0x%x",
                p2p->p2ps_prov->method, p2p->p2ps_prov->status, dev->req_config_methods);

        if (p2p_prepare_channel(p2p, dev, p2p->p2ps_prov->force_freq, p2p->p2ps_prov->pref_freq, 1) < 0)
            return -1;
    }
    ...
    if (p2p_send_action(p2p, freq, dev->info.p2p_device_addr, p2p->cfg->dev_addr, dev->info.p2p_device_addr,
                      ...
                      int user_initiated_pd)
{
    ...
    p2p_dbg(p2p, "Provision Discovery Request with " MACSTR " (config methods 0x%x)", MAC2STR(peer_addr),
            config_methods);
    ...
    {
        p2p_dbg(p2p,
                "Busy with other operations; postpone Provision Discovery Request with " MACSTR
                " (config methods 0x%x)",
