...
found:
    if (dev->sd_reqs > 100)
    {
        p2p_dbg(p2p, "Too many SD request attempts to " MACSTR " - skip remaining queries",
...
static struct wpabuf *p2p_build_sd_response(
    u8 dialog_token, u16 status_code, u16 comeback_delay, u16 update_indic, const struct wpabuf *tlvs)
{
    struct wpabuf *buf;
    u8 *len_pos;
...
static struct wpabuf *p2p_build_gas_comeback_resp(
    u8 dialog_token, u16 status_code, u16 update_indic, const u8 *data, size_t len, u8 frag_id, u8 more, u16 total_len)
{
    struct wpabuf *buf;
...
int p2p_start_sd(struct p2p_data *p2p, struct p2p_device *dev)
{
    ...
    if (freq <= 0)
    {
        p2p_dbg(p2p, "No Listen/Operating frequency known for the peer " MACSTR " to send SD Request",
                MAC2STR(dev->info.p2p_device_addr));
        return -1;
    }
    ...
    if (query == NULL)
        return -1;
    if (p2p->state == P2P_SEARCH && os_memcmp(p2p->sd_query_no_ack, dev->info.p2p_device_addr, ETH_ALEN) == 0)
    {
        p2p_dbg(p2p,
...
void p2p_sd_response(struct p2p_data *p2p, int freq, const u8 *dst, u8 dialog_token, const struct wpabuf *resp_tlvs)
{
    ...
    p2p->pending_action_state = P2P_NO_PENDING_ACTION;
    if (p2p_send_action(p2p, freq, dst, p2p->cfg->dev_addr, p2p->cfg->dev_addr, wpabuf_head(resp), wpabuf_len(resp),
                        wait_time) < 0)
        p2p_dbg(p2p, "Failed to send Action frame");
...
void p2p_rx_gas_comeback_req(struct p2p_data *p2p, const u8 *sa, const u8 *data, size_t len, int rx_freq)
{
    ...
    resp = p2p_build_gas_comeback_resp(dialog_token, WLAN_STATUS_SUCCESS, p2p->srv_update_indic,
                                       wpabuf_head_u8(p2p->sd_resp) + p2p->sd_resp_pos, frag_len, p2p->sd_frag_id, more,
    ...
    p2p->pending_action_state = P2P_NO_PENDING_ACTION;
    if (p2p_send_action(p2p, rx_freq, sa, p2p->cfg->dev_addr, p2p->cfg->dev_addr, wpabuf_head(resp), wpabuf_len(resp),
                        wait_time) < 0)
        p2p_dbg(p2p, "Failed to send Action frame");
...
skip_nqp_header:
    ...
    if (p2p->cfg->sd_response)
        p2p->cfg->sd_response(p2p->cfg->cb_ctx, sa, p2p->sd_rx_update_indic, wpabuf_head(p2p->sd_rx_resp),
