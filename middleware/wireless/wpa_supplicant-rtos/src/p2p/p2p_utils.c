...
int p2p_supported_freq_cli(struct p2p_data *p2p, unsigned int freq)
{
    ...
    if (p2p_freq_to_channel(freq, &op_reg_class, &op_channel) < 0)
        return 0;
    return p2p_channels_includes(&p2p->cfg->channels, op_reg_class, op_channel) ||
           p2p_channels_includes(&p2p->cfg->cli_channels, op_reg_class, op_channel);
}
                              ...
                              struct wpa_freq_range_list *disallow_list)
{
    ...
    if (p2p_channels_includes(chans, 81, 1) ||
        (freq_range_list_includes(avoid_list, 2412) && !freq_range_list_includes(disallow_list, 2412)))
        chan[num_channels++] = 1;
    if (p2p_channels_includes(chans, 81, 6) ||
        (freq_range_list_includes(avoid_list, 2437) && !freq_range_list_includes(disallow_list, 2437)))
        chan[num_channels++] = 6;
    if (p2p_channels_includes(chans, 81, 11) ||
        (freq_range_list_includes(avoid_list, 2462) && !freq_range_list_includes(disallow_list, 2462)))
...
void p2p_copy_channels(struct p2p_channels *dst, const struct p2p_channels *src, bool allow_6ghz)
{
    ...
    for (i = 0, j = 0; i < P2P_MAX_REG_CLASSES; i++)
    {
        if (is_6ghz_op_class(src->reg_class[i].reg_class))
            continue;
        os_memcpy(&dst->reg_class[j], &src->reg_class[i], sizeof(struct p2p_reg_class));
        j++;
    }
    dst->reg_classes = j;
}
...
int p2p_remove_6ghz_channels(unsigned int *pref_freq_list, int size)
{
    ...
    for (i = 0; i < size; i++)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "P2P: Remove 6 GHz channel %d", pref_freq_list[i]);
            size--;
            os_memmove(&pref_freq_list[i], &pref_freq_list[i + 1], (size - i) * sizeof(pref_freq_list[0]));
            i--;
        }
    }
    return i;
