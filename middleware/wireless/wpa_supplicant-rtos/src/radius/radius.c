...
{
    return (struct radius_attr_hdr *)(wpabuf_mhead_u8(msg->buf) + msg->attr_pos[idx]);
}
...
static const struct radius_attr_type radius_attrs[] = {
    ...
    {RADIUS_ATTR_ACCT_MULTI_SESSION_ID, "Acct-Multi-Session-Id", RADIUS_ATTR_TEXT},
    {RADIUS_ATTR_ACCT_LINK_COUNT, "Acct-Link-Count", RADIUS_ATTR_INT32},
    {RADIUS_ATTR_ACCT_INPUT_GIGAWORDS, "Acct-Input-Gigawords", RADIUS_ATTR_INT32},
    ...
    {RADIUS_ATTR_MESSAGE_AUTHENTICATOR, "Message-Authenticator", RADIUS_ATTR_UNDIST},
    {RADIUS_ATTR_TUNNEL_PRIVATE_GROUP_ID, "Tunnel-Private-Group-Id", RADIUS_ATTR_HEXDUMP},
    {RADIUS_ATTR_ACCT_INTERIM_INTERVAL, "Acct-Interim-Interval", RADIUS_ATTR_INT32},
    {RADIUS_ATTR_CHARGEABLE_USER_IDENTITY, "Chargeable-User-Identity", RADIUS_ATTR_TEXT},
    ...
    {RADIUS_ATTR_BASIC_LOCATION_POLICY_RULES, "Basic-Location-Policy-Rules", RADIUS_ATTR_HEXDUMP},
    {RADIUS_ATTR_EXTENDED_LOCATION_POLICY_RULES, "Extended-Location-Policy-Rules", RADIUS_ATTR_HEXDUMP},
    {RADIUS_ATTR_LOCATION_CAPABLE, "Location-Capable", RADIUS_ATTR_INT32},
    {RADIUS_ATTR_REQUESTED_LOCATION_INFO, "Requested-Location-Info", RADIUS_ATTR_INT32},
    ...
    {RADIUS_ATTR_WLAN_PAIRWISE_CIPHER, "WLAN-Pairwise-Cipher", RADIUS_ATTR_HEXDUMP},
    ...
    {RADIUS_ATTR_WLAN_GROUP_MGMT_CIPHER, "WLAN-Group-Mgmt-Pairwise-Cipher", RADIUS_ATTR_HEXDUMP},
};
...
void radius_msg_dump(struct radius_msg *msg)
{
    ...
    wpa_printf(MSG_INFO, "RADIUS message: code=%d (%s) identifier=%d length=%d", msg->hdr->code,
               radius_code_string(msg->hdr->code), msg->hdr->identifier, be_to_host16(msg->hdr->length));
...
int radius_msg_finish_das_resp(struct radius_msg *msg,
                               const u8 *secret,
                               size_t secret_len,
...
void radius_msg_finish_acct_resp(struct radius_msg *msg,
                                 const u8 *secret,
                                 size_t secret_len,
...
int radius_msg_verify_das_req(struct radius_msg *msg,
                              const u8 *secret,
                              size_t secret_len,
                              int require_message_authenticator)
{
    ...
    if (attr == NULL)
    {
        ...
        {
            wpa_printf(MSG_WARNING, "Missing Message-Authenticator attribute in RADIUS message");
            return 1;
        }
        return 0;
    }
...
int radius_msg_verify(
    struct radius_msg *msg, const u8 *secret, size_t secret_len, struct radius_msg *sent_msg, int auth)
{
    const u8 *addr[4];
    size_t len[4];
    u8 hash[MD5_MAC_LEN];
    ...
    addr[3] = secret;
    len[3]  = secret_len;
    if (md5_vector(4, addr, len, hash) < 0 || os_memcmp_const(hash, msg->hdr->authenticator, MD5_MAC_LEN) != 0)
    {
...
{
    return os_get_random((u8 *)&msg->hdr->authenticator, sizeof(msg->hdr->authenticator));
}
...
static u8 *decrypt_ms_key(
    const u8 *key, size_t len, const u8 *req_authenticator, const u8 *secret, size_t secret_len, size_t *reslen)
{
    u8 *plain, *ppos, *res;
    const u8 *pos;
    size_t left, plen;
...
static void encrypt_ms_key(const u8 *key,
                           size_t key_len,
                           u16 salt,
                           const u8 *req_authenticator,
                           const u8 *secret,
                                                      ...
                                                      size_t secret_len)
{
    ...
    {
        keys->recv =
            decrypt_ms_key(key + 17, keylen - 17, sent_msg->hdr->authenticator, secret, secret_len, &keys->recv_len);
    }
    os_free(key);
...
struct radius_attr_hdr *radius_msg_add_attr_user_password(
    struct radius_msg *msg, const u8 *data, size_t data_len, const u8 *secret, size_t secret_len)
{
    u8 buf[128];
    int res;
 ...
 */
char *radius_msg_get_tunnel_password(
    struct radius_msg *msg, int *keylen, const u8 *secret, size_t secret_len, struct radius_msg *sent_msg, size_t n)
{
    u8 *buf = NULL;
    size_t buflen;
    const u8 *salt;
