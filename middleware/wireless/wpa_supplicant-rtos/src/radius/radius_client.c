...
static int radius_client_retransmit(struct radius_client_data *radius, struct radius_msg_list *entry, os_time_t now)
{
    ...
    if (entry->msg_type == RADIUS_ACCT &&
        radius_msg_get_attr_ptr(entry->msg, RADIUS_ATTR_ACCT_DELAY_TIME, &acct_delay_time, &acct_delay_time_len,
                                NULL) == 0 &&
        acct_delay_time_len == 4)
    {
        ...
        wpa_printf(MSG_DEBUG, "RADIUS: Updated Acct-Delay-Time to %u for retransmission", delay_time);
        radius_msg_finish_acct(entry->msg, entry->shared_secret, entry->shared_secret_len);
    ...
    entry->attempts++;
    entry->accu_attempts++;
    hostapd_logger(radius->ctx, entry->addr, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_DEBUG,
                   "Resending RADIUS message (id=%d)", radius_msg_get_hdr(entry->msg)->identifier);
...
static void radius_client_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
    ...
    while (req)
    {
        ...
        if ((req->msg_type == msg_type || (req->msg_type == RADIUS_ACCT_INTERIM && msg_type == RADIUS_ACCT)) &&
    ...
    hostapd_logger(radius->ctx, req->addr, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_DEBUG,
    ...
    hostapd_logger(radius->ctx, req->addr, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_DEBUG,
                   ...
                   msg_type, hdr->code, hdr->identifier, invalid_authenticator ? " [INVALID AUTHENTICATOR]" : "");
    radius_client_msg_free(req);
                                ...
                                int auth)
{
    ...
    hostapd_logger(radius->ctx, NULL, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_INFO, "%s server %s:%d",
                   auth ? "Authentication" : "Accounting", hostapd_ip_txt(&nserv->addr, abuf, sizeof(abuf)),
                   nserv->port);
    ...
    if (oserv && oserv != nserv &&
        (nserv->shared_secret_len != oserv->shared_secret_len ||
         os_memcmp(nserv->shared_secret, oserv->shared_secret, nserv->shared_secret_len) != 0))
    {
    ...
    if (sel_sock < 0)
    {
        wpa_printf(MSG_INFO, "RADIUS: No server socket available (af=%d sock=%d sock6=%d auth=%d", nserv->addr.af, sock,
                   sock6, auth);
        return -1;
    }

    /* Force a reconnect by disconnecting the socket first */
    if (connect(sel_sock, (struct sockaddr *)&disconnect_addr, sizeof(disconnect_addr)) < 0)
        wpa_printf(MSG_INFO, "disconnect[radius]: %s", strerror(errno));

#ifdef __linux__
    if (conf->force_client_dev && conf->force_client_dev[0])
    {
        if (setsockopt(sel_sock, SOL_SOCKET, SO_BINDTODEVICE, conf->force_client_dev,
                       os_strlen(conf->force_client_dev)) < 0)
        {
            wpa_printf(MSG_ERROR, "RADIUS: setsockopt[SO_BINDTODEVICE]: %s", strerror(errno));
            /* Probably not a critical error; continue on and hope
             * for the best. */
        }
        else
        {
            wpa_printf(MSG_DEBUG, "RADIUS: Bound client socket to device: %s", conf->force_client_dev);
        }
    }
#endif /* __linux__ */
    ...
    switch (nserv->addr.af)
    {
        case AF_INET:
            ...
            {
                wpa_printf(MSG_DEBUG, "RADIUS local address: %s:%u", inet_ntoa(claddr.sin_addr),
        ...
        case AF_INET6:
        {
            ...
            if (getsockname(sel_sock, (struct sockaddr *)&claddr6, &claddrlen) == 0)
            {
                wpa_printf(MSG_DEBUG, "RADIUS local address: %s:%u",
                           inet_ntop(AF_INET6, &claddr6.sin6_addr, abuf, sizeof(abuf)), ntohs(claddr6.sin6_port));
            }
            break;
        }
#endif /* CONFIG_IPV6 */
...
static void radius_retry_primary_timer(void *eloop_ctx, void *timeout_ctx)
{
    ...
    if (radius->auth_sock >= 0 && conf->auth_servers && conf->auth_server != conf->auth_servers)
    {
        ...
        if (radius_change_server(radius, conf->auth_server, oserv, radius->auth_serv_sock, radius->auth_serv_sock6, 1) <
            0)
        {
    ...
    if (radius->acct_sock >= 0 && conf->acct_servers && conf->acct_server != conf->acct_servers)
    {
        ...
        if (radius_change_server(radius, conf->acct_server, oserv, radius->acct_serv_sock, radius->acct_serv_sock6, 0) <
            0)
        {
...
static int radius_client_init_auth(struct radius_client_data *radius)
{
    ...
    if (radius->auth_serv_sock >= 0 &&
        eloop_register_read_sock(radius->auth_serv_sock, radius_client_receive, radius, (void *)RADIUS_AUTH))
    {
...
#ifdef CONFIG_IPV6
    if (radius->auth_serv_sock6 >= 0 &&
        eloop_register_read_sock(radius->auth_serv_sock6, radius_client_receive, radius, (void *)RADIUS_AUTH))
    {
        wpa_printf(MSG_INFO, "RADIUS: Could not register read socket for authentication server");
        radius_close_auth_sockets(radius);
        return -1;
...
static int radius_client_init_acct(struct radius_client_data *radius)
{
    ...
    if (radius->acct_serv_sock >= 0 &&
        eloop_register_read_sock(radius->acct_serv_sock, radius_client_receive, radius, (void *)RADIUS_ACCT))
    {
...
#ifdef CONFIG_IPV6
    if (radius->acct_serv_sock6 >= 0 &&
        eloop_register_read_sock(radius->acct_serv_sock6, radius_client_receive, radius, (void *)RADIUS_ACCT))
    {
        wpa_printf(MSG_INFO, "RADIUS: Could not register read socket for accounting server");
        radius_close_acct_sockets(radius);
        return -1;
...
struct radius_client_data *radius_client_init(void *ctx, struct hostapd_radius_servers *conf)
{
    ...
    radius->ctx            = ctx;
    radius->conf           = conf;
    radius->auth_serv_sock = radius->acct_serv_sock = radius->auth_serv_sock6 = radius->acct_serv_sock6 =
...
void radius_client_flush_auth(struct radius_client_data *radius, const u8 *addr)
{
    ...
    while (entry)
    {
        ...
        {
            hostapd_logger(radius->ctx, addr, HOSTAPD_MODULE_RADIUS, HOSTAPD_LEVEL_DEBUG,
                                          ...
                                          struct radius_client_data *cli)
{
    ...
    return os_snprintf(buf, buflen,
                       ...
                       "radiusAuthClientAccessRequests=%lu\n"
                       "radiusAuthClientAccessRetransmissions=%lu\n"
                       "radiusAuthClientAccessAccepts=%lu\n"
                       "radiusAuthClientAccessRejects=%lu\n"
                       "radiusAuthClientAccessChallenges=%lu\n"
                       "radiusAuthClientMalformedAccessResponses=%lu\n"
                       "radiusAuthClientBadAuthenticators=%lu\n"
                       "radiusAuthClientPendingRequests=%u\n"
                       "radiusAuthClientTimeouts=%lu\n"
                       "radiusAuthClientUnknownTypes=%lu\n"
                       "radiusAuthClientPacketsDropped=%lu\n",
                       serv->index, hostapd_ip_txt(&serv->addr, abuf, sizeof(abuf)), serv->port, serv->round_trip_time,
                       serv->requests, serv->retransmissions, serv->access_accepts, serv->access_rejects,
                       serv->access_challenges, serv->malformed_responses, serv->bad_authenticators, pending,
                                          ...
                                          struct radius_client_data *cli)
{
    ...
    return os_snprintf(buf, buflen,
                       ...
                       "radiusAccClientRequests=%lu\n"
                       "radiusAccClientRetransmissions=%lu\n"
                       "radiusAccClientResponses=%lu\n"
                       "radiusAccClientMalformedResponses=%lu\n"
                       "radiusAccClientBadAuthenticators=%lu\n"
                       "radiusAccClientPendingRequests=%u\n"
                       "radiusAccClientTimeouts=%lu\n"
                       "radiusAccClientUnknownTypes=%lu\n"
                       "radiusAccClientPacketsDropped=%lu\n",
                       serv->index, hostapd_ip_txt(&serv->addr, abuf, sizeof(abuf)), serv->port, serv->round_trip_time,
                       serv->requests, serv->retransmissions, serv->responses, serv->malformed_responses,
                       serv->bad_authenticators, pending, serv->timeouts, serv->unknown_types, serv->packets_dropped);
}
