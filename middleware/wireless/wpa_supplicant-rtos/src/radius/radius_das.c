                                         ...
                                         int from_port)
{
    ...
    if (attr)
    {
        wpa_printf(MSG_INFO, "DAS: Unsupported attribute %u in CoA-Request from %s:%d", attr, abuf, from_port);
        error = 401;
        goto fail;
    }
...
#ifdef CONFIG_HS20
    if (radius_msg_get_attr_ptr(msg, RADIUS_ATTR_VENDOR_SPECIFIC, &buf, &len, NULL) == 0)
    {
        if (len < 10 || WPA_GET_BE32(buf) != RADIUS_VENDOR_ID_WFA ||
            buf[4] != RADIUS_VENDOR_ATTR_WFA_HS20_T_C_FILTERING || buf[5] < 6)
        {
            wpa_printf(MSG_INFO, "DAS: Unsupported attribute %u in CoA-Request from %s:%d", attr, abuf, from_port);
            error = 401;
            goto fail;
    ...
    {
        wpa_printf(MSG_INFO, "DAS: No supported authorization change attribute in CoA-Request from %s:%d", abuf,
                   from_port);
        error = 402;
        goto fail;
    }
    ...
    switch (res)
    {
        ...
        case RADIUS_DAS_MULTI_SESSION_MATCH:
            ...
            break;
        case RADIUS_DAS_COA_FAILED:
            wpa_printf(MSG_INFO, "DAS: CoA failed for request from %s:%d", abuf, from_port);
            error = 407;
            break;
        case RADIUS_DAS_SUCCESS:
            error = 0;
...
fail:
    reply = radius_msg_new(error ? RADIUS_CODE_COA_NAK : RADIUS_CODE_COA_ACK, hdr->identifier);
    if (!reply)
        return NULL;

    if (error && !radius_msg_add_attr_int32(reply, RADIUS_ATTR_ERROR_CAUSE, error))
    {
        radius_msg_free(reply);
        return NULL;
    }
...
static void radius_das_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
    ...
    if (das->client_addr.u.v4.s_addr && das->client_addr.u.v4.s_addr != from.sin.sin_addr.s_addr)
    {
    ...
    if (radius_msg_verify_das_req(msg, das->shared_secret, das->shared_secret_len, das->require_message_authenticator))
    {
        wpa_printf(MSG_DEBUG, "DAS: Invalid authenticator or Message-Authenticator in packet from %s:%d - drop", abuf,
                   from_port);
        goto fail;
    }
...
struct radius_das_data *radius_das_init(struct radius_das_conf *conf)
{
    ...
    das->shared_secret = os_memdup(conf->shared_secret, conf->shared_secret_len);
