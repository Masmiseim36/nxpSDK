...
static struct radius_session *radius_server_new_session(struct radius_server_data *data, struct radius_client *client)
{
    ...
    {
        RADIUS_DEBUG(
            "Maximum number of existing session - no room "
            "for a new session");
        return NULL;
    }
...
static void radius_server_testing_options(struct radius_session *sess, struct eap_session_data *eap_conf)
{
#ifdef CONFIG_TESTING_OPTIONS
    const char *pos;
...
#ifdef CONFIG_ERP
static struct eap_server_erp_key *radius_server_erp_find_key(struct radius_server_data *data, const char *keyname)
{
                                                            ...
                                                            const char *from_addr)
{
    ...
    if (radius_msg_get_attr_ptr(msg, RADIUS_ATTR_CALLING_STATION_ID, &id, &id_len, NULL) == 0)
    {
        char buf[3 * ETH_ALEN];
    ...
    sess->eap = eap_server_sm_init(sess, &radius_server_eapol_cb, data->eap_cfg, &eap_sess);
    if (sess->eap == NULL)
    {
        RADIUS_DEBUG(
            "Failed to initialize EAP state machine for the "
            "new session");
        radius_server_session_remove(data, sess);
        return NULL;
    }
...
static void radius_srv_hs20_t_c_pending(struct radius_session *sess)
{
    ...
    sql = sqlite3_mprintf("INSERT OR REPLACE INTO pending_tc (mac_addr,identity) VALUES (%Q,%Q)", addr, id_str);
    os_free(id_str);
    if (!sql)
        return;
    ...
    if (sqlite3_exec(sess->server->db, sql, NULL, NULL, NULL) != SQLITE_OK)
    {
        RADIUS_ERROR("Failed to add pending_tc entry into sqlite database: %s", sqlite3_errmsg(sess->server->db));
    }
    sqlite3_free(sql);
#endif /* CONFIG_SQLITE */
}
...
static void radius_server_add_session(struct radius_session *sess)
{
    ...
    os_get_time(&now);
    sql = sqlite3_mprintf(
        "INSERT OR REPLACE INTO current_sessions(mac_addr,identity,start_time,nas,hs20_t_c_filtering) VALUES "
        "(%Q,%Q,%d,%Q,%u)",
        addr_txt, sess->username, now.sec, sess->nas_ip, sess->t_c_filtering);
    if (sql)
    {
        if (sqlite3_exec(sess->server->db, sql, NULL, NULL, NULL) != SQLITE_OK)
        {
...
static void db_update_last_msk(struct radius_session *sess, const char *msk)
{
    ...
    sql = sqlite3_mprintf("UPDATE users SET last_msk=%Q WHERE identity=%Q", msk, id_str);
    os_free(id_str);
    if (!sql)
        return;
    ...
    if (sqlite3_exec(sess->server->db, sql, NULL, NULL, NULL) != SQLITE_OK)
    {
        RADIUS_DEBUG("Failed to update last_msk: %s", sqlite3_errmsg(sess->server->db));
    }
    sqlite3_free(sql);
#endif /* CONFIG_SQLITE */
#endif /* CONFIG_RADIUS_TEST */
...
static int radius_server_is_sim_method(struct radius_session *sess)
{
    ...
    return name && (os_strcmp(name, "SIM") == 0 || os_strcmp(name, "AKA") == 0 || os_strcmp(name, "AKA'") == 0);
}
...
static int radius_server_hs20_missing_sim_pps(struct radius_msg *request)
{
    ...
    for (;;)
    {
        if (radius_msg_get_attr_ptr(request, RADIUS_ATTR_VENDOR_SPECIFIC, &buf, &len, buf) < 0)
            return 0;
        if (len < 6)
            continue;
        pos = buf;
...
static int radius_server_sim_provisioning_session(struct radius_session *sess, const u8 *hash)
{
#ifdef CONFIG_SQLITE
    char *sql;
    char addr_txt[ETH_ALEN * 3];
    ...
    if (!sess->server->db || (!db_table_exists(sess->server->db, "sim_provisioning") &&
    ...
    os_get_time(&now);
    sql = sqlite3_mprintf(
        "INSERT INTO sim_provisioning(mobile_identifier_hash,imsi,mac_addr,eap_method,timestamp) VALUES "
        "(%Q,%Q,%Q,%Q,%u)",
    ...
    if (sqlite3_exec(sess->server->db, sql, NULL, NULL, NULL) != SQLITE_OK)
    {
        RADIUS_ERROR("Failed to add SIM provisioning entry into sqlite database: %s", sqlite3_errmsg(sess->server->db));
        res = -1;
    }
    else
    {
                                                        ...
                                                        struct radius_msg *request)
{
    ...
    if (code == RADIUS_CODE_ACCESS_CHALLENGE &&
        !radius_msg_add_attr(msg, RADIUS_ATTR_STATE, (u8 *)&sess_id, sizeof(sess_id)))
    {
        RADIUS_DEBUG("Failed to add State attribute");
    }

    if (sess->eap_if->eapReqData &&
        !radius_msg_add_eap(msg, wpabuf_head(sess->eap_if->eapReqData), wpabuf_len(sess->eap_if->eapReqData)))
    {
    ...
    if (code == RADIUS_CODE_ACCESS_ACCEPT && sess->eap_if->eapKeyData)
    {
        ...
        if (!radius_msg_add_mppe_keys(msg, hdr->authenticator, (u8 *)client->shared_secret, client->shared_secret_len,
                                      sess->eap_if->eapKeyData + len, len, sess->eap_if->eapKeyData, len))
        {
            RADIUS_DEBUG("Failed to add MPPE key attributes");
        }

        if (sess->eap_if->eapSessionId &&
            !radius_msg_add_attr(msg, RADIUS_ATTR_EAP_KEY_NAME, sess->eap_if->eapSessionId,
    ...
    else if (code == RADIUS_CODE_ACCESS_ACCEPT && data->hs20_sim_provisioning_url &&
             radius_server_is_sim_method(sess) && radius_server_hs20_missing_sim_pps(request))
    {
        ...
        RADIUS_DUMP("hotspot2dot0-mobile-identifier-hash", hash, HS20_MOBILE_ID_HASH_LEN);
        ...
        *pos++ = data->subscr_remediation_method;
        ...
        pos += prefix_len;
        wpa_snprintf_hex((char *)pos, 2 * HS20_MOBILE_ID_HASH_LEN + 1, hash, HS20_MOBILE_ID_HASH_LEN);
    ...
    if (code == RADIUS_CODE_ACCESS_ACCEPT && sess->t_c_filtering)
    {
        ...
        if (!radius_msg_add_wfa(msg, RADIUS_VENDOR_ATTR_WFA_HS20_T_C_FILTERING, buf, sizeof(buf)))
        {
        ...
        if (!radius_msg_add_wfa(msg, RADIUS_VENDOR_ATTR_WFA_HS20_T_C_URL, (const u8 *)url2, url_len))
        {
    ...
    {
        if (radius_msg_add_attr_int32(msg, RADIUS_ATTR_WLAN_REASON_CODE, reason) < 0)
        {
                                               ...
                                               struct radius_msg *request)
{
    ...
    else
    {
        ...
        {
            u8 buf[128];
            res = radius_user_password_hide(request, tmp.password, tmp.password_len, (u8 *)client->shared_secret,
...
static void radius_server_hs20_t_c_check(struct radius_session *sess, struct radius_msg *msg)
{
#ifdef CONFIG_HS20
                                 ...
                                 struct radius_session *force_sess)
{
    ...
    if (sess->last_from_port == from_port && sess->last_identifier == radius_msg_get_hdr(msg)->identifier &&
        os_memcmp(sess->last_authenticator, radius_msg_get_hdr(msg)->authenticator, 16) == 0)
    {
        ...
        RADIUS_DEBUG(
            "No previous reply available for duplicate "
            "message");
        return -1;
    }
    ...
    {
        RADIUS_DUMP("EAP data from the state machine", wpabuf_head(sess->eap_if->eapReqData),
                    wpabuf_len(sess->eap_if->eapReqData));
    }
    else if (sess->eap_if->eapFail)
    {
        RADIUS_DEBUG(
            "No EAP data from the state machine, but eapFail "
            "set");
    }
    ...
    {
        RADIUS_DEBUG(
            "No EAP data from the state machine - ignore this"
...
send_reply:
    ...
    if (is_complete)
    {
        ...
        eloop_cancel_timeout(radius_server_session_remove_timeout, data, sess);
        eloop_register_timeout(RADIUS_SESSION_MAINTAIN, 0, radius_server_session_remove_timeout, data, sess);
    }

    return 0;
}

static void radius_server_receive_disconnect_resp(struct radius_server_data *data,
                                                  struct radius_client *client,
                                                  struct radius_msg *msg,
                                                  int ack)
{
    ...
    if (radius_msg_verify(msg, (const u8 *)client->shared_secret, client->shared_secret_len,
                                           ...
                                           int ack)
{
    ...
    {
        RADIUS_DEBUG("Ignore unexpected CoA response with unexpected identifier %u (expected %u)", hdr->identifier,
    ...
    RADIUS_DEBUG("CoA-%s received for " MACSTR, ack ? "ACK" : "NAK", MAC2STR(client->pending_dac_coa_addr));
    ...
    os_snprintf(addrtxt, sizeof(addrtxt), MACSTR, MAC2STR(client->pending_dac_coa_addr));

    if (ack)
    {
        sql = sqlite3_mprintf(
            "UPDATE current_sessions SET hs20_t_c_filtering=0, waiting_coa_ack=0, coa_ack_received=1 WHERE mac_addr=%Q",
            addrtxt);
    }
    else
    {
        sql = sqlite3_mprintf("UPDATE current_sessions SET waiting_coa_ack=0 WHERE mac_addr=%Q", addrtxt);
    }
    if (!sql)
        return;
    ...
    if (res != SQLITE_OK)
    {
        RADIUS_ERROR("Failed to update current_sessions entry: %s", sqlite3_errmsg(data->db));
        return;
    }
#endif /* CONFIG_SQLITE */
}
...
static void radius_server_receive_auth(int sock, void *eloop_ctx, void *sock_ctx)
{
    ...
    if (data->ipv6)
    {
        ...
        client = radius_server_get_client(data, (struct in_addr *)&from.sin6.sin6_addr, 1);
    }
#endif /* CONFIG_IPV6 */
...
static struct radius_client *radius_server_read_clients(const char *client_file, int ipv6)
{
    ...
    while (fgets(buf, buf_size, f))
    {
        ...
        pos = buf;
        while ((*pos >= '0' && *pos <= '9') || *pos == '.' || (*pos >= 'a' && *pos <= 'f') || *pos == ':' ||
               (*pos >= 'A' && *pos <= 'F'))
        {
            pos++;
        }
...
struct radius_server_data *radius_server_init(struct radius_server_conf *conf)
{
    ...
    {
        data->hs20_sim_provisioning_url = os_strdup(conf->hs20_sim_provisioning_url);
...
int radius_server_get_mib(struct radius_server_data *data, char *buf, size_t buflen)
{
    ...
    ret = os_snprintf(pos, end - pos,
                      "radiusAuthServTotalAccessRequests=%lu\n"
                      "radiusAuthServTotalInvalidRequests=%lu\n"
                      "radiusAuthServTotalDupAccessRequests=%lu\n"
                      "radiusAuthServTotalAccessAccepts=%lu\n"
                      "radiusAuthServTotalAccessRejects=%lu\n"
                      "radiusAuthServTotalAccessChallenges=%lu\n"
                      "radiusAuthServTotalMalformedAccessRequests=%lu\n"
                      "radiusAuthServTotalBadAuthenticators=%lu\n"
                      "radiusAuthServTotalPacketsDropped=%lu\n"
                      "radiusAuthServTotalUnknownTypes=%lu\n"
                      "radiusAccServTotalRequests=%lu\n"
                      "radiusAccServTotalInvalidRequests=%lu\n"
                      "radiusAccServTotalResponses=%lu\n"
                      "radiusAccServTotalMalformedRequests=%lu\n"
                      "radiusAccServTotalBadAuthenticators=%lu\n"
                      "radiusAccServTotalUnknownTypes=%lu\n",
                      data->counters.access_requests, data->counters.invalid_requests,
                      data->counters.dup_access_requests, data->counters.access_accepts, data->counters.access_rejects,
                      data->counters.access_challenges, data->counters.malformed_access_requests,
                      data->counters.bad_authenticators, data->counters.packets_dropped, data->counters.unknown_types,
                      data->counters.acct_requests, data->counters.invalid_acct_requests, data->counters.acct_responses,
                      data->counters.malformed_acct_requests, data->counters.acct_bad_authenticators,
                      data->counters.unknown_acct_types);
    if (os_snprintf_error(end - pos, ret))
    {
        *pos = '\0';
    ...
    for (cli = data->clients, idx = 0; cli; cli = cli->next, idx++)
    {
        ...
        ret = os_snprintf(pos, end - pos,
                          ...
                          "radiusAuthServAccessRequests=%lu\n"
                          "radiusAuthServDupAccessRequests=%lu\n"
                          "radiusAuthServAccessAccepts=%lu\n"
                          "radiusAuthServAccessRejects=%lu\n"
                          "radiusAuthServAccessChallenges=%lu\n"
                          "radiusAuthServMalformedAccessRequests=%lu\n"
                          "radiusAuthServBadAuthenticators=%lu\n"
                          "radiusAuthServPacketsDropped=%lu\n"
                          "radiusAuthServUnknownTypes=%lu\n"
                          "radiusAccServTotalRequests=%lu\n"
                          "radiusAccServTotalInvalidRequests=%lu\n"
                          "radiusAccServTotalResponses=%lu\n"
                          "radiusAccServTotalMalformedRequests=%lu\n"
                          ...
                          cli->counters.access_accepts, cli->counters.access_rejects, cli->counters.access_challenges,
                          cli->counters.malformed_access_requests, cli->counters.bad_authenticators,
                          cli->counters.packets_dropped, cli->counters.unknown_types, cli->counters.acct_requests,
                          cli->counters.invalid_acct_requests, cli->counters.acct_responses,
                          cli->counters.malformed_acct_requests, cli->counters.acct_bad_authenticators,
...
static int radius_server_get_eap_user(
...
void radius_server_eap_pending_cb(struct radius_server_data *data, void *ctx)
{
    ...
    if (radius_server_request(data, msg, (struct sockaddr *)&sess->last_from, sess->last_fromlen, cli,
                              sess->last_from_addr, sess->last_from_port, sess) == -2)
        return; /* msg was stored with the session */
...
int radius_server_dac_request(struct radius_server_data *data, const char *req)
{
    ...
    sql = sqlite3_mprintf("SELECT * FROM current_sessions WHERE mac_addr=%Q", addrtxt);
    if (!sql)
        return -1;
    ...
    msg = radius_msg_new(disconnect ? RADIUS_CODE_DISCONNECT_REQUEST : RADIUS_CODE_COA_REQUEST, identifier);
    if (!msg)
        return -1;

    os_snprintf(addrtxt, sizeof(addrtxt), RADIUS_802_1X_ADDR_FORMAT, MAC2STR(addr));
    if (!radius_msg_add_attr(msg, RADIUS_ATTR_CALLING_STATION_ID, (u8 *)addrtxt, os_strlen(addrtxt)))
    {
    ...
    if (!disconnect && t_c_clear)
    {
        ...
        if (!radius_msg_add_wfa(msg, RADIUS_VENDOR_ATTR_WFA_HS20_T_C_FILTERING, val, sizeof(val)))
        {
            RADIUS_DEBUG("Failed to add WFA-HS20-T-C-Filtering");
            radius_msg_free(msg);
            return -1;
    ...
    radius_msg_finish_acct(msg, (u8 *)client->shared_secret, client->shared_secret_len);

    if (wpa_debug_level <= MSG_MSGDUMP)
        radius_msg_dump(msg);

    buf = radius_msg_get_buf(msg);
    if (sendto(data->auth_sock, wpabuf_head(buf), wpabuf_len(buf), 0, (struct sockaddr *)&das, sizeof(das)) < 0)
    {
