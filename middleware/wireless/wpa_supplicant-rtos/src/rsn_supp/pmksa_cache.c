...
struct rsn_pmksa_cache
{
    ...
    bool (*is_current_cb)(struct rsn_pmksa_cache_entry *entry, void *ctx);
    void *ctx;
};
...
{
    wpa_sm_remove_pmkid(pmksa->sm, entry->network_ctx, entry->aa, entry->pmkid,
...
static void pmksa_cache_expire(void *eloop_ctx, void *timeout_ctx)
{
    ...
    {
        if (wpa_key_mgmt_sae(entry->akmp) && pmksa->is_current_cb(entry, pmksa->ctx))
        {
             ...
             * has been lost. */
            wpa_printf(MSG_DEBUG, "RSN: postpone PMKSA cache entry expiration for SAE with " MACSTR,
                       MAC2STR(entry->aa));
            prev  = entry;
            entry = entry->next;
            continue;
...
static void pmksa_cache_set_expiration(struct rsn_pmksa_cache *pmksa)
{
    ...
    {
        sec = 0;
        if (wpa_key_mgmt_sae(pmksa->pmksa->akmp) && pmksa->is_current_cb(pmksa->pmksa, pmksa->ctx))
        {
    ...
    entry = pmksa->sm->cur_pmksa ? pmksa->sm->cur_pmksa : pmksa_cache_get(pmksa, pmksa->sm->bssid, NULL, NULL, 0);
                                              ...
                                              const u8 *cache_id)
{
    ...
    entry->reauth_time =
        now.sec + pmksa->sm->dot11RSNAConfigPMKLifetime * pmksa->sm->dot11RSNAConfigPMKReauthThreshold / 100;
    entry->akmp = akmp;
    if (cache_id)
    {
...
struct rsn_pmksa_cache_entry *pmksa_cache_add_entry(struct rsn_pmksa_cache *pmksa, struct rsn_pmksa_cache_entry *entry)
{
    ...
    while (pos)
    {
        ...
        {
            if (pos->pmk_len == entry->pmk_len && os_memcmp_const(pos->pmk, entry->pmk, entry->pmk_len) == 0 &&
            ...
            if (!pos->opportunistic)
                pmksa_cache_flush(pmksa, entry->network_ctx, pos->pmk, pos->pmk_len, false);
    ...
    pmksa->pmksa_count++;
    wpa_printf(MSG_DEBUG, "RSN: Added PMKSA cache entry for " MACSTR " network_ctx=%p akmp=0x%x", MAC2STR(entry->aa),
               entry->network_ctx, entry->akmp);
    wpa_sm_add_pmkid(pmksa->sm, entry->network_ctx, entry->aa, entry->pmkid,
                     entry->fils_cache_id_set ? entry->fils_cache_id : NULL, entry->pmk, entry->pmk_len,
                     pmksa->sm->dot11RSNAConfigPMKLifetime, pmksa->sm->dot11RSNAConfigPMKReauthThreshold, entry->akmp);
 ...
 * applications
 */
void pmksa_cache_flush(
    struct rsn_pmksa_cache *pmksa, void *network_ctx, const u8 *pmk, size_t pmk_len, bool external_only)
{
    ...
    while (entry)
    {
        if ((entry->network_ctx == network_ctx || network_ctx == NULL) &&
            (pmk == NULL || (pmk_len == entry->pmk_len && os_memcmp(pmk, entry->pmk, pmk_len) == 0)) &&
 ...
 */
struct rsn_pmksa_cache_entry *pmksa_cache_get(
    struct rsn_pmksa_cache *pmksa, const u8 *aa, const u8 *pmkid, const void *network_ctx, int akmp)
{
    struct rsn_pmksa_cache_entry *entry = pmksa->pmksa;
    while (entry)
    {
        if ((aa == NULL || os_memcmp(entry->aa, aa, ETH_ALEN) == 0) &&
            (pmkid == NULL || os_memcmp(entry->pmkid, pmkid, PMKID_LEN) == 0) && (!akmp || akmp == entry->akmp) &&
                                                             ...
                                                             const u8 *aa)
{
    ...
    if (wpa_key_mgmt_sae(old_entry->akmp) || wpa_key_mgmt_fils(old_entry->akmp))
        pmkid = old_entry->pmkid;
    new_entry = pmksa_cache_add(pmksa, old_entry->pmk, old_entry->pmk_len, pmkid, NULL, 0, aa, pmksa->sm->own_addr,
                                old_entry->network_ctx, old_entry->akmp,
                                old_entry->fils_cache_id_set ? old_entry->fils_cache_id : NULL);
    if (new_entry == NULL)
        return NULL;
                                                            ...
                                                            int akmp)
{
    ...
    while (entry)
    {
        if (entry->network_ctx == network_ctx && (!akmp || entry->akmp == akmp))
        {
            struct os_reltime now;

            if (wpa_key_mgmt_sae(entry->akmp) && os_get_reltime(&now) == 0 && entry->reauth_time < now.sec)
            {
                wpa_printf(MSG_DEBUG,
                           "RSN: Do not clone PMKSA cache entry for " MACSTR " since its reauth threshold has passed",
                           MAC2STR(entry->aa));
                entry = entry->next;
                continue;
            }
...
static struct rsn_pmksa_cache_entry *pmksa_cache_get_fils_cache_id(struct rsn_pmksa_cache *pmksa,
                                                                   ...
                                                                   const u8 *cache_id)
{
    ...
    {
        if (network_ctx == entry->network_ctx && entry->fils_cache_id_set &&
            os_memcmp(cache_id, entry->fils_cache_id, FILS_CACHE_ID_LEN) == 0)
            return entry;
    }
...
void pmksa_cache_clear_current(struct wpa_sm *sm)
{
    if (sm == NULL)
        return;
    if (sm->cur_pmksa)
        wpa_printf(MSG_DEBUG, "RSN: Clear current PMKSA entry selection");
    sm->cur_pmksa = NULL;
}
                            ...
                            int akmp)
{
    ...
    if (fils_cache_id)
        wpa_printf(MSG_DEBUG, "RSN: Search for FILS Cache Identifier %02x%02x", fils_cache_id[0], fils_cache_id[1]);

    sm->cur_pmksa = NULL;
    if (pmkid)
        sm->cur_pmksa = pmksa_cache_get(pmksa, NULL, pmkid, network_ctx, akmp);
    if (sm->cur_pmksa == NULL && bssid)
        sm->cur_pmksa = pmksa_cache_get(pmksa, bssid, NULL, network_ctx, akmp);
    if (sm->cur_pmksa == NULL && try_opportunistic && bssid)
        sm->cur_pmksa = pmksa_cache_get_opportunistic(pmksa, network_ctx, bssid, akmp);
    if (sm->cur_pmksa == NULL && fils_cache_id)
        sm->cur_pmksa = pmksa_cache_get_fils_cache_id(pmksa, network_ctx, fils_cache_id);
    if (sm->cur_pmksa)
    {
        struct os_reltime now;

        if (wpa_key_mgmt_sae(sm->cur_pmksa->akmp) && os_get_reltime(&now) == 0 && sm->cur_pmksa->reauth_time < now.sec)
        {
            wpa_printf(MSG_DEBUG,
                       "RSN: Do not allow PMKSA cache entry for " MACSTR
...
int pmksa_cache_list(struct rsn_pmksa_cache *pmksa, char *buf, size_t len)
{
    ...
    while (entry)
    {
        ...
        ret = os_snprintf(pos, buf + len - pos, " %d %d", (int)(entry->expiration - now.sec), entry->opportunistic);
        if (os_snprintf_error(buf + len - pos, ret))
            return pos - buf;
        pos += ret;
        if (entry->fils_cache_id_set)
        {
            ret = os_snprintf(pos, buf + len - pos, " %02x%02x", entry->fils_cache_id[0], entry->fils_cache_id[1]);
            if (os_snprintf_error(buf + len - pos, ret))
                return pos - buf;
            pos += ret;
        }
...
void pmksa_cache_reconfig(struct rsn_pmksa_cache *pmksa)
{
    ...
    for (entry = pmksa->pmksa; entry; entry = entry->next)
    {
        ...
        if (entry->expiration - now.sec < 1 || entry->reauth_time - now.sec < 1)
            continue;

        life_time        = entry->expiration - now.sec;
        reauth_threshold = (entry->reauth_time - now.sec) * 100 / life_time;
        if (!reauth_threshold)
            continue;

        wpa_sm_add_pmkid(pmksa->sm, entry->network_ctx, entry->aa, entry->pmkid,
                         entry->fils_cache_id_set ? entry->fils_cache_id : NULL, entry->pmk, entry->pmk_len, life_time,
