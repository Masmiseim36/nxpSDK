...
{
    return !!(akmp & (WPA_KEY_MGMT_IEEE8021X | WPA_KEY_MGMT_IEEE8021X_SHA256 | WPA_KEY_MGMT_IEEE8021X_SUITE_B |
...
static void rsn_preauth_eapol_cb(struct eapol_sm *eapol, enum eapol_supp_result result, void *ctx)
{
    ...
    if (result == EAPOL_SUPP_RESULT_SUCCESS)
    {
        ...
        if (res == 0)
        {
            ...
            sm->pmk_len = pmk_len;
            pmksa_cache_add(sm->pmksa, pmk, pmk_len, NULL, NULL, 0, sm->preauth_bssid, sm->own_addr, sm->network_ctx,
    ...
    wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "RSN: pre-authentication with " MACSTR " %s", MAC2STR(sm->preauth_bssid),
            result == EAPOL_SUPP_RESULT_SUCCESS ? "completed successfully" : "failed");
...
static void rsn_preauth_timeout(void *eloop_ctx, void *timeout_ctx)
{
    ...
    wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "RSN: pre-authentication with " MACSTR " timed out",
...
int rsn_preauth_init(struct wpa_sm *sm, const u8 *dst, struct eap_peer_config *eap_conf)
{
    ...
    {
        sm->l2_preauth_br =
            l2_packet_init(sm->bridge_ifname, sm->own_addr, ETH_P_RSN_PREAUTH, rsn_preauth_receive, sm, 0);
...
void rsn_preauth_candidate_process(struct wpa_sm *sm)
{
    ...
    wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG,
            ...
            "list");
    if (sm->preauth_eapol || sm->proto != WPA_PROTO_RSN || wpa_sm_get_state(sm) != WPA_COMPLETED ||
    ...
    dl_list_for_each_safe(candidate, n, &sm->pmksa_candidates, struct rsn_pmksa_candidate, list)
    {
        ...
        wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: PMKSA candidate " MACSTR " does not need pre-authentication anymore",
                MAC2STR(candidate->bssid));
        /* Some drivers (e.g., NDIS) expect to get notified about the
         * PMKIDs again, so report the existing data now. */
        if (p)
        {
            wpa_sm_add_pmkid(sm, NULL, candidate->bssid, p->pmkid, NULL, p->pmk, p->pmk_len, 0, 0, p->akmp);
        }
...
void pmksa_candidate_add(struct wpa_sm *sm, const u8 *bssid, int prio, int preauth)
{
    ...
    if (sm->network_ctx && sm->proactive_key_caching)
        pmksa_cache_get_opportunistic(sm->pmksa, sm->network_ctx, bssid, 0);
