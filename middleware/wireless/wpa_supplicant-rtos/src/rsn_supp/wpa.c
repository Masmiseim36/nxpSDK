 ...
 */
int wpa_eapol_key_send(
    struct wpa_sm *sm, struct wpa_ptk *ptk, int ver, const u8 *dest, u16 proto, u8 *msg, size_t msg_len, u8 *key_mic)
{
    int ret        = -1;
    size_t mic_len = wpa_mic_len(sm->key_mgmt, sm->pmk_len);

    wpa_printf(MSG_DEBUG, "WPA: Send EAPOL-Key frame to " MACSTR " ver=%d mic_len=%d key_mgmt=0x%x", MAC2STR(dest), ver,
               (int)mic_len, sm->key_mgmt);
    if (is_zero_ether_addr(dest) && is_zero_ether_addr(sm->bssid))
    {
        ...
        {
            dest = sm->bssid;
            wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Use BSSID (" MACSTR ") as the destination for EAPOL-Key",
                    MAC2STR(dest));
        }
    }
    ...
    if (mic_len)
    {
        ...
        if (key_mic && wpa_eapol_key_mic(ptk->kck, ptk->kck_len, sm->key_mgmt, ver, msg, msg_len, key_mic))
        {
            wpa_msg(sm->ctx->msg_ctx, MSG_ERROR, "WPA: Failed to generate EAPOL-Key version %d key_mgmt 0x%x MIC", ver,
                    sm->key_mgmt);
            goto out;
        }
        if (ptk)
    ...
    else
    {
        ...
        key_data_len = msg_len - sizeof(struct ieee802_1x_hdr) - sizeof(struct wpa_eapol_key) - 2;
        ...
        wpa_hexdump_key(MSG_DEBUG, "WPA: Plaintext Key Data", s_key_data, key_data_len);
        ...
        if (aes_siv_encrypt(ptk->kek, ptk->kek_len, s_key_data, key_data_len, 1, aad, aad_len, key_data) < 0)
        {
            os_free(buf);
            goto out;
        }

        wpa_hexdump(MSG_DEBUG, "WPA: Encrypted Key Data from SIV", key_data, AES_BLOCK_SIZE + key_data_len);
...
void wpa_sm_key_request(struct wpa_sm *sm, int error, int pairwise)
{
    ...
    if (pairwise && sm->wpa_deny_ptk0_rekey && !sm->use_ext_key_id && wpa_sm_get_state(sm) == WPA_COMPLETED)
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: PTK0 rekey not allowed, reconnecting");
        wpa_sm_reconnect(sm);
        return;
    }
    ...
    wpa_eapol_key_send(sm, &sm->ptk, ver, bssid, ETH_P_EAPOL, rbuf, rlen, key_mic);
}
...
static int wpa_supplicant_get_pmk(struct wpa_sm *sm, const unsigned char *src_addr, const u8 *pmkid)
{
    ...
    if (pmkid && !sm->cur_pmksa)
    {
        ...
        sm->cur_pmksa = pmksa_cache_get(sm->pmksa, src_addr, pmkid, NULL, 0);
        if (sm->cur_pmksa)
        {
    ...
    else if (wpa_key_mgmt_wpa_ieee8021x(sm->key_mgmt) && sm->eapol)
    {
...
#ifdef CONFIG_IEEE80211R
        if (res == 0 && eapol_sm_get_key(sm->eapol, buf, 2 * PMK_LEN) == 0)
        {
            if (wpa_key_mgmt_sha384(sm->key_mgmt))
            {
            ...
            if (sm->proto == WPA_PROTO_RSN && wpa_key_mgmt_ft(sm->key_mgmt))
            {
...
#endif /* CONFIG_FILS */
                wpa_hexdump_key(MSG_DEBUG, "FT: Cache XXKey/MPMK", sm->xxkey, sm->xxkey_len);
                sa = pmksa_cache_add(sm->pmksa, sm->xxkey, sm->xxkey_len, NULL, NULL, 0, src_addr, sm->own_addr,
                                     sm->network_ctx, sm->key_mgmt, fils_cache_id);
                if (!sm->cur_pmksa)
                    sm->cur_pmksa = sa;
            }
        }
        ...
        if (res == 0)
        {
            ...
            {
                sa = pmksa_cache_add(sm->pmksa, sm->pmk, pmk_len, NULL, NULL, 0, src_addr, sm->own_addr,
                                     sm->network_ctx, sm->key_mgmt, fils_cache_id);
            }
            if (!sm->cur_pmksa && pmkid && pmksa_cache_get(sm->pmksa, src_addr, pmkid, NULL, 0))
            {
                wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
        ...
        {
            wpa_printf(MSG_DEBUG, "FT: Continue 4-way handshake without PMK/PMKID for association using FT protocol");
#endif /* CONFIG_IEEE80211R */
        }
        else
        {
    ...
    if (abort_cached && wpa_key_mgmt_wpa_ieee8021x(sm->key_mgmt) && !wpa_key_mgmt_suite_b(sm->key_mgmt) &&
                               ...
                               struct wpa_ptk *ptk)
{
    ...
    if (wpa_key_mgmt_ft(sm->key_mgmt))
    {
        ...
        wpa_hexdump(MSG_DEBUG, "WPA: WPA IE before FT processing", wpa_ie, wpa_ie_len);
        /*
        ...
        wpa_hexdump(MSG_DEBUG, "WPA: WPA IE after PMKID[PMKR1Name] addition into RSNE", rsn_ie_buf, wpa_ie_len);

        if (sm->assoc_resp_ies)
        {
            wpa_hexdump(MSG_DEBUG, "WPA: Add assoc_resp_ies", sm->assoc_resp_ies, sm->assoc_resp_ies_len);
                          ...
                          struct wpa_ptk *ptk)
{
    ...
    akmp = sm->key_mgmt;
#ifdef CONFIG_OWE
    if (sm->owe_ptk_workaround && akmp == WPA_KEY_MGMT_OWE && sm->pmk_len > 32)
    {
        wpa_printf(MSG_DEBUG, "OWE: Force SHA256 for PTK derivation");
        akmp |= WPA_KEY_MGMT_PSK_SHA256;
    }
#endif /* CONFIG_OWE */

    if (sm->force_kdk_derivation || (sm->secure_ltf && ieee802_11_rsnx_capab(sm->ap_rsnxe, WLAN_RSNX_CAPAB_SECURE_LTF)))
        kdk_len = WPA_KDK_MAX_LEN;
    else
        kdk_len = 0;

    return wpa_pmk_to_ptk(sm->pmk, sm->pmk_len, "Pairwise key expansion", sm->own_addr, sm->bssid, sm->snonce,
                          key->key_nonce, ptk, akmp, sm->pairwise_cipher, z, z_len, kdk_len);
}

static int wpa_handle_ext_key_id(struct wpa_sm *sm, struct wpa_eapol_ie_parse *kde)
{
    if (sm->ext_key_id)
    {
        u16 key_id;
        ...
        if (!kde->key_id)
        {
            wpa_msg(sm->ctx->msg_ctx, sm->use_ext_key_id ? MSG_INFO : MSG_DEBUG,
        ...
        if (key_id > 1)
        {
            wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "RSN: Invalid Extended Key ID: %d", key_id);
            return -1;
        }
        wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: Using Extended Key ID %d", key_id);
    ...
    else
    {
        ...
        {
            wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "RSN: Non-zero Extended Key ID Key ID in PTK0 handshake");
            return -1;
        }
        ...
        if (kde->key_id)
        {
            ...
            wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: Extended Key ID Key ID 0 in PTK0 handshake");
        }
        sm->keyidx_active  = 0;
        sm->use_ext_key_id = 0;
    }
                                          ...
                                          size_t key_data_len)
{
    ...
    if (sm->wpa_deny_ptk0_rekey && !sm->use_ext_key_id && wpa_sm_get_state(sm) == WPA_COMPLETED)
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: PTK0 rekey not allowed, reconnecting");
        wpa_sm_reconnect(sm);
        return;
    ...
    kde_buf = os_malloc(kde_len + 2 + RSN_SELECTOR_LEN + 3 + sm->assoc_rsnxe_len + 2 + RSN_SELECTOR_LEN + 1 + 2 +
                        RSN_SELECTOR_LEN + 2);
    if (!kde_buf)
        goto failed;
    ...
    if (wpa_sm_ocv_enabled(sm))
    {
        ...
        {
            wpa_printf(MSG_WARNING, "Failed to get channel info for OCI element in EAPOL-Key 2/4");
            goto failed;
        }
#ifdef CONFIG_TESTING_OPTIONS
        if (sm->oci_freq_override_eapol)
        {
            wpa_printf(MSG_INFO, "TEST: Override OCI KDE frequency %d -> %d MHz", ci.frequency,
                       sm->oci_freq_override_eapol);
    ...
    if (wpa_supplicant_send_2_of_4(sm, sm->bssid, key, ver, sm->snonce, kde, kde_len, ptk) < 0)
        goto failed;
...
{
    wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: Key negotiation completed with " MACSTR " [PTK=%s GTK=%s]", MAC2STR(addr),
            wpa_cipher_txt(sm->pairwise_cipher), wpa_cipher_txt(sm->group_cipher));
    ...
    if (secure)
    {
        ...
        if (wpa_key_mgmt_wpa_psk(sm->key_mgmt) || sm->key_mgmt == WPA_KEY_MGMT_DPP || sm->key_mgmt == WPA_KEY_MGMT_OWE)
...
static int wpa_supplicant_install_ptk(struct wpa_sm *sm, const struct wpa_eapol_key *key, enum key_flag key_flag)
{
    int keylen, rsclen;
    enum wpa_alg alg;
    const u8 *key_rsc;
    ...
    {
        wpa_printf(MSG_DEBUG, "WPA: TK length mismatch: %d != %lu", keylen, (long unsigned int)sm->ptk.tk_len);
        return -1;
    }
    ...
    if (wpa_sm_set_key(sm, alg, sm->bssid, sm->keyidx_active, 1, key_rsc, rsclen, sm->ptk.tk, keylen,
                       KEY_FLAG_PAIRWISE | key_flag) < 0)
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
                "WPA: Failed to set PTK to the driver (alg=%d keylen=%d bssid=" MACSTR " idx=%d key_flag=0x%x)", alg,
                keylen, MAC2STR(sm->bssid), sm->keyidx_active, key_flag);
        return -1;
    }

    wpa_sm_store_ptk(sm, sm->bssid, sm->pairwise_cipher, sm->dot11RSNAConfigPMKLifetime, &sm->ptk);
...
{
    wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Activate PTK (idx=%d bssid=" MACSTR ")", sm->keyidx_active,
            MAC2STR(sm->bssid));

    if (wpa_sm_set_key(sm, 0, sm->bssid, sm->keyidx_active, 0, NULL, 0, NULL, 0, KEY_FLAG_PAIRWISE_RX_TX_MODIFY) < 0)
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Failed to activate PTK for TX (idx=%d bssid=" MACSTR ")",
...
static int wpa_supplicant_check_group_cipher(
    struct wpa_sm *sm, int group_cipher, int keylen, int maxkeylen, int *key_rsc_len, enum wpa_alg *alg)
{
    int klen;
    ...
    *key_rsc_len = wpa_cipher_rsc_len(group_cipher);
    ...
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Unsupported %s Group Cipher key length %d (%d)",
                                      ...
                                      int wnm_sleep)
{
    ...
    if ((sm->gtk.gtk_len == (size_t)gd->gtk_len && os_memcmp(sm->gtk.gtk, gd->gtk, sm->gtk.gtk_len) == 0) ||
        (sm->gtk_wnm_sleep.gtk_len == (size_t)gd->gtk_len &&
         os_memcmp(sm->gtk_wnm_sleep.gtk, gd->gtk, sm->gtk_wnm_sleep.gtk_len) == 0))
    {
        wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
                "WPA: Not reinstalling already in-use GTK to the driver (keyidx=%d tx=%d len=%d)", gd->keyidx, gd->tx,
                gd->gtk_len);
        return 0;
    }

    wpa_hexdump_key(MSG_DEBUG, "WPA: Group Key", gd->gtk, gd->gtk_len);
    wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Installing GTK to the driver (keyidx=%d tx=%d len=%d)", gd->keyidx,
            gd->tx, gd->gtk_len);
    ...
    if (sm->pairwise_cipher == WPA_CIPHER_NONE)
    {
        if (wpa_sm_set_key(sm, gd->alg, NULL, gd->keyidx, 1, key_rsc, gd->key_rsc_len, _gtk, gd->gtk_len,
    ...
    else if (wpa_sm_set_key(sm, gd->alg, broadcast_ether_addr, gd->keyidx, gd->tx, key_rsc, gd->key_rsc_len, _gtk,
...
static int wpa_supplicant_rsc_relaxation(const struct wpa_sm *sm, const u8 *rsc)
{
    ...
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "RSC %02x%02x%02x%02x%02x%02x%02x%02x is likely bogus, using 0", rsc[0],
...
static int wpa_supplicant_pairwise_gtk(
    struct wpa_sm *sm, const struct wpa_eapol_key *key, const u8 *gtk, size_t gtk_len, int key_info)
{
...
static int wpa_supplicant_install_igtk(struct wpa_sm *sm, const struct wpa_igtk_kde *igtk, int wnm_sleep)
{
    ...
    /* Detect possible key reinstallation */
    if ((sm->igtk.igtk_len == len && os_memcmp(sm->igtk.igtk, igtk->igtk, sm->igtk.igtk_len) == 0) ||
        (sm->igtk_wnm_sleep.igtk_len == len &&
         os_memcmp(sm->igtk_wnm_sleep.igtk, igtk->igtk, sm->igtk_wnm_sleep.igtk_len) == 0))
    {
        wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Not reinstalling already in-use IGTK to the driver (keyidx=%d)",
                keyidx);
        return 0;
    }

    wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: IGTK keyid %d pn " COMPACT_MACSTR, keyidx, MAC2STR(igtk->pn));
    ...
    if (wpa_sm_set_key(sm, wpa_cipher_to_alg(sm->mgmt_group_cipher), broadcast_ether_addr, keyidx, 0, igtk->pn,
                       sizeof(igtk->pn), igtk->igtk, len, KEY_FLAG_GROUP_RX) < 0)
    {
...
static int wpa_supplicant_install_bigtk(struct wpa_sm *sm, const struct wpa_bigtk_kde *bigtk, int wnm_sleep)
{
    size_t len = wpa_cipher_key_len(sm->mgmt_group_cipher);
    u16 keyidx = WPA_GET_LE16(bigtk->keyid);

    /* Detect possible key reinstallation */
    if ((sm->bigtk.bigtk_len == len && os_memcmp(sm->bigtk.bigtk, bigtk->bigtk, sm->bigtk.bigtk_len) == 0) ||
        (sm->bigtk_wnm_sleep.bigtk_len == len &&
         os_memcmp(sm->bigtk_wnm_sleep.bigtk, bigtk->bigtk, sm->bigtk_wnm_sleep.bigtk_len) == 0))
    {
        wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Not reinstalling already in-use BIGTK to the driver (keyidx=%d)",
                keyidx);
        return 0;
    }

    wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: BIGTK keyid %d pn " COMPACT_MACSTR, keyidx, MAC2STR(bigtk->pn));
    wpa_hexdump_key(MSG_DEBUG, "WPA: BIGTK", bigtk->bigtk, len);
    if (keyidx < 6 || keyidx > 7)
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Invalid BIGTK KeyID %d", keyidx);
        return -1;
    }
    if (wpa_sm_set_key(sm, wpa_cipher_to_alg(sm->mgmt_group_cipher), broadcast_ether_addr, keyidx, 0, bigtk->pn,
                       sizeof(bigtk->pn), bigtk->bigtk, len, KEY_FLAG_GROUP_RX) < 0)
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Failed to configure BIGTK to the driver");
        return -1;
    }
    ...
    {
        sm->bigtk_wnm_sleep.bigtk_len = len;
        os_memcpy(sm->bigtk_wnm_sleep.bigtk, bigtk->bigtk, sm->bigtk_wnm_sleep.bigtk_len);
    }
    else
    {
...
static int ieee80211w_set_keys(struct wpa_sm *sm, struct wpa_eapol_ie_parse *ie)
{
    ...
    if (!wpa_cipher_valid_mgmt_group(sm->mgmt_group_cipher) || sm->mgmt_group_cipher == WPA_CIPHER_GTK_NOT_USED)
        return 0;
                            ...
                            const u8 *assoc_resp_mdie)
{
    ...
    if (ie->mdie == NULL || ie->mdie_len < 2 + sizeof(*mdie) ||
        os_memcmp(mdie->mobility_domain, sm->mobility_domain, MOBILITY_DOMAIN_ID_LEN) != 0)
    {
        wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
    ...
    if (assoc_resp_mdie &&
        (assoc_resp_mdie[1] != ie->mdie[1] || os_memcmp(assoc_resp_mdie, ie->mdie, 2 + ie->mdie[1]) != 0))
    {
                                         ...
                                         struct wpa_eapol_ie_parse *ie)
{
    ...
    if (ft_validate_mdie(sm, src_addr, ie, mdie) < 0 || ft_validate_ftie(sm, src_addr, ie, ftie) < 0 ||
...
static int wpa_supplicant_validate_ie(struct wpa_sm *sm, const unsigned char *src_addr, struct wpa_eapol_ie_parse *ie)
{
    ...
    if (ie->wpa_ie == NULL && ie->rsn_ie == NULL && (sm->ap_wpa_ie || sm->ap_rsn_ie))
    {
        wpa_report_ie_mismatch(sm,
                               ...
                               src_addr, ie->wpa_ie, ie->wpa_ie_len, ie->rsn_ie, ie->rsn_ie_len);
        return -1;
    }

    if ((ie->wpa_ie && sm->ap_wpa_ie &&
         (ie->wpa_ie_len != sm->ap_wpa_ie_len || os_memcmp(ie->wpa_ie, sm->ap_wpa_ie, ie->wpa_ie_len) != 0)) ||
        (ie->rsn_ie && sm->ap_rsn_ie &&
         wpa_compare_rsn_ie(wpa_key_mgmt_ft(sm->key_mgmt), sm->ap_rsn_ie, sm->ap_rsn_ie_len, ie->rsn_ie,
                            ie->rsn_ie_len)))
    {
        wpa_report_ie_mismatch(sm,
                               "IE in 3/4 msg does not match "
    ...
    if (sm->proto == WPA_PROTO_RSN &&
        ((sm->ap_rsnxe && !ie->rsnxe) || (!sm->ap_rsnxe && ie->rsnxe) ||
         (sm->ap_rsnxe && ie->rsnxe &&
          (sm->ap_rsnxe_len != ie->rsnxe_len || os_memcmp(sm->ap_rsnxe, ie->rsnxe, sm->ap_rsnxe_len) != 0))))
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: RSNXE mismatch between Beacon/ProbeResp and EAPOL-Key msg 3/4");
        wpa_hexdump(MSG_INFO, "RSNXE in Beacon/ProbeResp", sm->ap_rsnxe, sm->ap_rsnxe_len);
        wpa_hexdump(MSG_INFO, "RSNXE in EAPOL-Key msg 3/4", ie->rsnxe, ie->rsnxe_len);
        wpa_sm_deauthenticate(sm, WLAN_REASON_IE_IN_4WAY_DIFFERS);
        return -1;
    }
...
static void wpa_supplicant_process_3_of_4(
    struct wpa_sm *sm, const struct wpa_eapol_key *key, u16 ver, const u8 *key_data, size_t key_data_len)
{
    u16 key_info, keylen;
    struct wpa_eapol_ie_parse ie;
    ...
    if (ie.igtk && sm->mgmt_group_cipher != WPA_CIPHER_GTK_NOT_USED &&
    ...
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Invalid %s key length %d (src=" MACSTR ")",
    ...
    if (wpa_sm_ocv_enabled(sm))
    {
        ...
        if (ocv_verify_tx_params(ie.oci, ie.oci_len, &ci, channel_width_to_int(ci.chanwidth), ci.seg1_idx) !=
            OCI_SUCCESS)
        {
            wpa_msg(sm->ctx->msg_ctx, MSG_INFO, OCV_FAILURE "addr=" MACSTR " frame=eapol-key-m3 error=%s",
    ...
    {
        wpa_printf(MSG_DEBUG, "DPP: peer Protocol Version %u Flags 0x%x", ie.dpp_kde[0], ie.dpp_kde[1]);
        if (sm->key_mgmt == WPA_KEY_MGMT_DPP && sm->dpp_pfs != 2 && (ie.dpp_kde[1] & DPP_KDE_PFS_ALLOWED) && !sm->dpp_z)
        {
            wpa_printf(MSG_INFO,
    ...
    if (sm->use_ext_key_id && wpa_supplicant_install_ptk(sm, key, KEY_FLAG_RX))
        goto failed;

    if (wpa_supplicant_send_4_of_4(sm, sm->bssid, key, ver, key_info, &sm->ptk) < 0)
    {
        goto failed;
    }
    ...
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "RSN: No GTK KDE included in EAPOL-Key msg 3/4");
        goto failed;
    }
     ...
     * same PMK. */
    if (sm->proto == WPA_PROTO_RSN && wpa_key_mgmt_suite_b(sm->key_mgmt) && !sm->cur_pmksa)
    {
        struct rsn_pmksa_cache_entry *sa;

        sa = pmksa_cache_add(sm->pmksa, sm->pmk, sm->pmk_len, NULL, sm->ptk.kck, sm->ptk.kck_len, sm->bssid,
                             sm->own_addr, sm->network_ctx, sm->key_mgmt, NULL);
        if (!sm->cur_pmksa)
            sm->cur_pmksa = sa;
    }
...
static int wpa_supplicant_process_1_of_2_rsn(
    struct wpa_sm *sm, const u8 *keydata, size_t keydatalen, u16 key_info, struct wpa_gtk_data *gd)
{
    int maxkeylen;
    struct wpa_eapol_ie_parse ie;
    u16 gtk_len;

    wpa_hexdump_key(MSG_DEBUG, "RSN: msg 1/2 key data", keydata, keydatalen);
    ...
    if (gtk_len < 2)
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "RSN: Invalid GTK KDE length (%u) in Group Key msg 1/2", gtk_len);
        return -1;
    }
    gtk_len -= 2;
    if (gtk_len > sizeof(gd->gtk))
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "RSN: Too long GTK in GTK KDE (len=%u)", gtk_len);
        return -1;
    }
    ...
    if (wpa_sm_ocv_enabled(sm))
    {
        ...
        if (ocv_verify_tx_params(ie.oci, ie.oci_len, &ci, channel_width_to_int(ci.chanwidth), ci.seg1_idx) !=
            OCI_SUCCESS)
        {
            wpa_msg(sm->ctx->msg_ctx, MSG_INFO, OCV_FAILURE "addr=" MACSTR " frame=eapol-key-g1 error=%s",
                    MAC2STR(sm->bssid), ocv_errorstr);
            return -1;
        }
    }
    ...
    if (wpa_supplicant_check_group_cipher(sm, sm->group_cipher, gtk_len, maxkeylen, &gd->key_rsc_len, &gd->alg))
        return -1;

    wpa_hexdump_key(MSG_DEBUG, "RSN: received GTK in group key handshake", ie.gtk, 2 + gtk_len);
                                             ...
                                             struct wpa_gtk_data *gd)
{
    ...
    if (gtk_len > maxkeylen ||
        wpa_supplicant_check_group_cipher(sm, sm->group_cipher, gtk_len, maxkeylen, &gd->key_rsc_len, &gd->alg))
        return -1;
    ...
    {
#ifdef CONFIG_NO_RC4
        wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: RC4 not supported in the build");
        return -1;
#else  /* CONFIG_NO_RC4 */
        u8 ek[32];
...
static int wpa_supplicant_send_2_of_2(struct wpa_sm *sm, const struct wpa_eapol_key *key, int ver, u16 key_info)
{
    ...
    mic_len = wpa_mic_len(sm->key_mgmt, sm->pmk_len);
    hdrlen  = sizeof(*reply) + mic_len + 2;
    rbuf    = wpa_sm_alloc_eapol(sm, IEEE802_1X_TYPE_EAPOL_KEY, NULL, hdrlen + kde_len, &rlen, (void *)&reply);
    if (rbuf == NULL)
        return -1;
    ...
    if (wpa_sm_ocv_enabled(sm))
    {
        ...
        if (wpa_sm_channel_info(sm, &ci) != 0)
        {
            wpa_printf(MSG_WARNING, "Failed to get channel info for OCI element in EAPOL-Key 2/2");
            os_free(rbuf);
            return -1;
        }
#ifdef CONFIG_TESTING_OPTIONS
                                          ...
                                          u16 ver)
{
    ...
    if (wpa_supplicant_install_gtk(sm, &gd, key_rsc, 0) || wpa_supplicant_send_2_of_2(sm, key, ver, key_info) < 0)
        goto failed;
    forced_memzero(&gd, sizeof(gd));
...
static int wpa_supplicant_verify_eapol_key_mic(
    struct wpa_sm *sm, struct wpa_eapol_key *key, u16 ver, const u8 *buf, size_t len)
{
    ...
    if (sm->tptk_set)
    {
        ...
        if (wpa_eapol_key_mic(sm->tptk.kck, sm->tptk.kck_len, sm->key_mgmt, ver, buf, len, (u8 *)(key + 1)) < 0 ||
    ...
    {
        os_memset(key + 1, 0, mic_len);
        if (wpa_eapol_key_mic(sm->ptk.kck, sm->ptk.kck_len, sm->key_mgmt, ver, buf, len, (u8 *)(key + 1)) < 0 ||
            os_memcmp_const(mic, key + 1, mic_len) != 0)
        {
            wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
                    "WPA: Invalid EAPOL-Key MIC - "
...
static int wpa_supplicant_decrypt_key_data(
    struct wpa_sm *sm, struct wpa_eapol_key *key, size_t mic_len, u16 ver, u8 *key_data, size_t *key_data_len)
{
    ...
    else if (ver == WPA_KEY_INFO_TYPE_HMAC_SHA1_AES || ver == WPA_KEY_INFO_TYPE_AES_128_CMAC ||
             wpa_use_aes_key_wrap(sm->key_mgmt))
    {
        u8 *buf;

        wpa_printf(MSG_DEBUG, "WPA: Decrypt Key Data using AES-UNWRAP (KEK length %u)", (unsigned int)sm->ptk.kek_len);
        ...
        *key_data_len -= 8; /* AES-WRAP adds 8 bytes */
        ...
        {
#ifdef TEST_FUZZ
            wpa_printf(MSG_INFO, "TEST: Ignore AES unwrap failure for fuzz testing");
            goto continue_fuzz;
#endif /* TEST_FUZZ */
...
void wpa_sm_aborted_external_cached(struct wpa_sm *sm)
{
    ...
    {
        wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: Cancelling external PMKSA caching attempt");
        sm->cur_pmksa = NULL;
    }
}

static void wpa_eapol_key_dump(
    struct wpa_sm *sm, const struct wpa_eapol_key *key, unsigned int key_data_len, const u8 *mic, unsigned int mic_len)
{
#ifndef CONFIG_NO_STDOUT_DEBUG
    u16 key_info = WPA_GET_BE16(key->key_info);

    wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "  EAPOL-Key type=%d", key->type);
    wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "  key_info 0x%x (ver=%d keyidx=%d rsvd=%d %s%s%s%s%s%s%s%s)", key_info,
            key_info & WPA_KEY_INFO_TYPE_MASK, (key_info & WPA_KEY_INFO_KEY_INDEX_MASK) >> WPA_KEY_INFO_KEY_INDEX_SHIFT,
            (key_info & (BIT(13) | BIT(14) | BIT(15))) >> 13, key_info & WPA_KEY_INFO_KEY_TYPE ? "Pairwise" : "Group",
            key_info & WPA_KEY_INFO_INSTALL ? " Install" : "", key_info & WPA_KEY_INFO_ACK ? " Ack" : "",
            key_info & WPA_KEY_INFO_MIC ? " MIC" : "", key_info & WPA_KEY_INFO_SECURE ? " Secure" : "",
            key_info & WPA_KEY_INFO_ERROR ? " Error" : "", key_info & WPA_KEY_INFO_REQUEST ? " Request" : "",
            key_info & WPA_KEY_INFO_ENCR_KEY_DATA ? " Encr" : "");
    wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "  key_length=%u key_data_length=%u", WPA_GET_BE16(key->key_length),
            key_data_len);
...
#ifdef CONFIG_FILS
static int wpa_supp_aead_decrypt(struct wpa_sm *sm, u8 *buf, size_t buf_len, size_t *key_data_len)
{
    struct wpa_ptk *ptk;
...
int wpa_sm_rx_eapol(struct wpa_sm *sm, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "IEEE 802.1X RX: version=%d type=%d length=%lu", hdr->version, hdr->type,
            (unsigned long)plen);
    ...
    if (data_len < len)
    {
        wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: ignoring %lu bytes after the IEEE 802.1X data",
    ...
    if (ver != WPA_KEY_INFO_TYPE_HMAC_MD5_RC4 && ver != WPA_KEY_INFO_TYPE_AES_128_CMAC &&
        ver != WPA_KEY_INFO_TYPE_HMAC_SHA1_AES && !wpa_use_akm_defined(sm->key_mgmt))
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: Unsupported EAPOL-Key descriptor version %d", ver);
        goto out;
    }

    if (wpa_use_akm_defined(sm->key_mgmt) && ver != WPA_KEY_INFO_TYPE_AKM_DEFINED)
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
                "RSN: Unsupported EAPOL-Key descriptor version %d (expected AKM defined = 0)", ver);
        goto out;
    }
    ...
    if (wpa_key_mgmt_ft(sm->key_mgmt))
    {
        ...
        if (ver != WPA_KEY_INFO_TYPE_AES_128_CMAC && !wpa_use_akm_defined(sm->key_mgmt))
        {
    ...
    else if (sm->pairwise_cipher == WPA_CIPHER_CCMP && !wpa_use_akm_defined(sm->key_mgmt) &&
             ver != WPA_KEY_INFO_TYPE_HMAC_SHA1_AES)
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
        ...
        {
            wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
                    "WPA: Interoperability workaround: allow incorrect (should have been HMAC-SHA1), but stronger (is "
                    "AES-128-CMAC), descriptor version to be used");
        }
        else
            goto out;
    }
    else if (sm->pairwise_cipher == WPA_CIPHER_GCMP && !wpa_use_akm_defined(sm->key_mgmt) &&
             ver != WPA_KEY_INFO_TYPE_HMAC_SHA1_AES)
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
    ...
    if (sm->rx_replay_counter_set && os_memcmp(key->replay_counter, sm->rx_replay_counter, WPA_REPLAY_COUNTER_LEN) <= 0)
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
    ...
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: Unsupported SMK bit in key_info");
        goto out;
    }
    ...
    if ((key_info & WPA_KEY_INFO_MIC) && wpa_supplicant_verify_eapol_key_mic(sm, key, ver, tmp, data_len))
        goto out;
        ...
        mic_len)
    {
        ...
        if (wpa_supplicant_decrypt_key_data(sm, key, mic_len, ver, key_data, &key_data_len))
            goto out;
    }
    ...
    {
        if ((mic_len && (key_info & WPA_KEY_INFO_MIC)) || (!mic_len && (key_info & WPA_KEY_INFO_ENCR_KEY_DATA)))
        {
...
static u32 wpa_key_mgmt_suite(struct wpa_sm *sm)
{
    switch (sm->key_mgmt)
    {
        case WPA_KEY_MGMT_IEEE8021X:
            return ((sm->proto == WPA_PROTO_RSN || sm->proto == WPA_PROTO_OSEN) ? RSN_AUTH_KEY_MGMT_UNSPEC_802_1X :
...
int wpa_sm_get_mib(struct wpa_sm *sm, char *buf, size_t buflen)
{
    ...
    rsna =
        (wpa_key_mgmt_wpa_psk(sm->key_mgmt) || wpa_key_mgmt_wpa_ieee8021x(sm->key_mgmt)) && sm->proto == WPA_PROTO_RSN;
    ...
    ret = os_snprintf(
        ...
        RSN_SUITE_ARG(wpa_key_mgmt_suite(sm)), RSN_SUITE_ARG(wpa_cipher_to_suite(sm->proto, sm->pairwise_cipher)),
        RSN_SUITE_ARG(wpa_cipher_to_suite(sm->proto, sm->group_cipher)), pmkid_txt,
        RSN_SUITE_ARG(wpa_key_mgmt_suite(sm)), RSN_SUITE_ARG(wpa_cipher_to_suite(sm->proto, sm->pairwise_cipher)),
        RSN_SUITE_ARG(wpa_cipher_to_suite(sm->proto, sm->group_cipher)), sm->dot11RSNA4WayHandshakeFailures);
...
static void wpa_sm_pmksa_free_cb(struct rsn_pmksa_cache_entry *entry, void *ctx, enum pmksa_free_reason reason)
{
    ...
    wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: PMKSA cache entry free_cb: " MACSTR " reason=%d", MAC2STR(entry->aa),
            reason);
...
static bool wpa_sm_pmksa_is_current_cb(struct rsn_pmksa_cache_entry *entry, void *ctx)
{
    struct wpa_sm *sm = ctx;
...
struct wpa_sm *wpa_sm_init(struct wpa_sm_ctx *ctx)
{
    ...
    sm->pmksa = pmksa_cache_init(wpa_sm_pmksa_free_cb, wpa_sm_pmksa_is_current_cb, sm, sm);
    if (sm->pmksa == NULL)
    {
 ...
 */
void wpa_sm_set_pmk(struct wpa_sm *sm, const u8 *pmk, size_t pmk_len, const u8 *pmkid, const u8 *bssid)
{
    if (sm == NULL)
        return;

    wpa_hexdump_key(MSG_DEBUG, "WPA: Set PMK based on external data", pmk, pmk_len);
    sm->pmk_len = pmk_len;
    ...
    if (bssid)
    {
        sm->cur_pmksa = pmksa_cache_add(sm->pmksa, pmk, pmk_len, pmkid, NULL, 0, bssid, sm->own_addr, sm->network_ctx,
                                        sm->key_mgmt, NULL);
    }
}
...
void wpa_sm_set_pmk_from_pmksa(struct wpa_sm *sm)
{
    ...
    if (sm->cur_pmksa)
    {
        wpa_hexdump_key(MSG_DEBUG, "WPA: Set PMK based on current PMKSA", sm->cur_pmksa->pmk, sm->cur_pmksa->pmk_len);
...
void wpa_sm_set_config(struct wpa_sm *sm, struct rsn_supp_config *config)
{
    ...
    if (config)
    {
        ...
        if (config->fils_cache_id)
        {
            ...
            os_memcpy(sm->fils_cache_id, config->fils_cache_id, FILS_CACHE_ID_LEN);
        }
        else
        {
...
int wpa_sm_ocv_enabled(struct wpa_sm *sm)
{
    ...
    return wpa_parse_wpa_ie_rsn(sm->ap_rsn_ie, sm->ap_rsn_ie_len, &rsn) >= 0 &&
...
int wpa_sm_set_assoc_wpa_ie_default(struct wpa_sm *sm, u8 *wpa_ie, size_t *wpa_ie_len)
{
    ...
    if (sm->test_assoc_ie)
    {
        ...
        if (*wpa_ie_len < wpabuf_len(sm->test_assoc_ie))
            return -1;
        os_memcpy(wpa_ie, wpabuf_head(sm->test_assoc_ie), wpabuf_len(sm->test_assoc_ie));
    ...
    *wpa_ie_len = res;
    ...
    {
        wpa_hexdump(MSG_DEBUG, "WPA: Leave previously set WPA IE default", sm->assoc_wpa_ie, sm->assoc_wpa_ie_len);
    }
 ...
 * Returns: 0 on success, -1 on failure
 */
int wpa_sm_set_assoc_rsnxe_default(struct wpa_sm *sm, u8 *rsnxe, size_t *rsnxe_len)
{
    int res;
    ...
    *rsnxe_len = res;
    ...
    if (sm->assoc_rsnxe)
    {
        wpa_hexdump(MSG_DEBUG, "RSN: Leave previously set RSNXE default", sm->assoc_rsnxe, sm->assoc_rsnxe_len);
    }
    else if (*rsnxe_len > 0)
    {
        /*
...
int wpa_sm_set_assoc_rsnxe(struct wpa_sm *sm, const u8 *ie, size_t len)
{
    ...
    if (!ie || len == 0)
    {
        wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: clearing own RSNXE");
        sm->assoc_rsnxe     = NULL;
...
struct rsn_pmksa_cache_entry *wpa_sm_pmksa_cache_add_entry(struct wpa_sm *sm, struct rsn_pmksa_cache_entry *entry)
{
    return pmksa_cache_add_entry(sm->pmksa, entry);
}

void wpa_sm_pmksa_cache_add(
    struct wpa_sm *sm, const u8 *pmk, size_t pmk_len, const u8 *pmkid, const u8 *bssid, const u8 *fils_cache_id)
{
    sm->cur_pmksa = pmksa_cache_add(sm->pmksa, pmk, pmk_len, pmkid, NULL, 0, bssid, sm->own_addr, sm->network_ctx,
                                    sm->key_mgmt, fils_cache_id);
}

int wpa_sm_pmksa_exists(struct wpa_sm *sm, const u8 *bssid, const void *network_ctx)
{
    return pmksa_cache_get(sm->pmksa, bssid, NULL, network_ctx, 0) != NULL;
}

struct rsn_pmksa_cache_entry *wpa_sm_pmksa_cache_get(
    struct wpa_sm *sm, const u8 *aa, const u8 *pmkid, const void *network_ctx, int akmp)
{
...
int wpa_wnmsleep_install_key(struct wpa_sm *sm, u8 subelem_id, u8 *buf)
{
    ...
    else if (subelem_id == WNM_SLEEP_SUBELEM_BIGTK)
    {
        ...
        if (sm->beacon_prot && wpa_supplicant_install_bigtk(sm, bigtk, 1) < 0)
            return -1;
    }
    else
    {
...
void wpa_sm_set_ptk_kck_kek(
    struct wpa_sm *sm, const u8 *ptk_kck, size_t ptk_kck_len, const u8 *ptk_kek, size_t ptk_kek_len)
{
...
struct wpabuf *fils_build_auth(struct wpa_sm *sm, int dh_group, const u8 *md)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "FILS: Neither ERP EAP-Initiate/Re-auth nor PMKSA cache entry is available - skip FILS");
        goto fail;
    }

    wpa_printf(MSG_DEBUG, "FILS: Try to use FILS (erp=%d pmksa_cache=%d)", erp_msg != NULL, sm->cur_pmksa != NULL);
    ...
    wpa_hexdump(MSG_DEBUG, "FILS: Generated FILS Nonce", sm->fils_nonce, FILS_NONCE_LEN);
    wpa_hexdump(MSG_DEBUG, "FILS: Generated FILS Session", sm->fils_session, FILS_SESSION_LEN);
    ...
    if (dh_group)
    {
        ...
        if (!sm->fils_ecdh)
        {
            wpa_printf(MSG_INFO, "FILS: Could not initialize ECDH with group %d", dh_group);
            goto fail;
        }
        pub = crypto_ecdh_get_pubkey(sm->fils_ecdh, 1);
        if (!pub)
            goto fail;
        wpa_hexdump_buf(MSG_DEBUG, "FILS: Element (DH public key)", pub);
        sm->fils_dh_elem_len = wpabuf_len(pub);
    }
#endif /* CONFIG_FILS_SK_PFS */

    buf = wpabuf_alloc(1000 + sm->assoc_wpa_ie_len + (pub ? wpabuf_len(pub) : 0));
    if (!buf)
        goto fail;
    ...
    /* RSNE */
    wpa_hexdump(MSG_DEBUG, "FILS: RSNE in FILS Authentication frame", sm->assoc_wpa_ie, sm->assoc_wpa_ie_len);
    ...
    if (erp_msg)
    {
        ...
        if (fils_pmkid_erp(sm->key_mgmt, wpabuf_head(erp_msg), wpabuf_len(erp_msg), sm->fils_erp_pmkid) == 0)
            sm->fils_erp_pmkid_set = 1;
    }

    wpa_hexdump_buf(MSG_DEBUG, "RSN: FILS fields for Authentication frame", buf);
...
int fils_process_auth(struct wpa_sm *sm, const u8 *bssid, const u8 *data, size_t len)
{
    const u8 *pos, *end;
    ...
    wpa_hexdump(MSG_DEBUG, "FILS: Authentication frame fields", data, len);
    pos = data;
    end = data + len;
    ...
    if (sm->fils_dh_group)
    {
        ...
        if (end - pos < 2)
        {
            wpa_printf(MSG_DEBUG, "FILS: No room for Finite Cyclic Group");
            goto fail;
        }
        ...
        {
            wpa_printf(MSG_DEBUG, "FILS: Unexpected change in Finite Cyclic Group: %u (expected %u)", group,
                       sm->fils_dh_group);
            goto fail;
        }
        ...
        dh_ss = crypto_ecdh_set_peerkey(sm->fils_ecdh, 1, pos, sm->fils_dh_elem_len);
        if (!dh_ss)
        {
    ...
    /* RSNE */
    wpa_hexdump(MSG_DEBUG, "FILS: RSN element", elems.rsn_ie, elems.rsn_ie_len);
    if (!elems.rsn_ie || wpa_parse_wpa_ie_rsn(elems.rsn_ie - 2, elems.rsn_ie_len + 2, &rsn) < 0)
    {
    ...
    if (wpa_key_mgmt_ft(sm->key_mgmt))
    {
        ...
        if (wpa_ft_parse_ies(pos, end - pos, &parse, wpa_key_mgmt_sha384(sm->key_mgmt)) < 0)
        {
        ...
        if (!parse.r0kh_id)
        {
            wpa_printf(MSG_DEBUG, "FILS+FT: No R0KH-ID subelem in FTE");
            goto fail;
        }
        os_memcpy(sm->r0kh_id, parse.r0kh_id, parse.r0kh_id_len);
        sm->r0kh_id_len = parse.r0kh_id_len;
        wpa_hexdump_ascii(MSG_DEBUG, "FILS+FT: R0KH-ID", sm->r0kh_id, sm->r0kh_id_len);

        if (!parse.r1kh_id)
        {
            wpa_printf(MSG_DEBUG, "FILS+FT: No R1KH-ID subelem in FTE");
            goto fail;
        }
        os_memcpy(sm->r1kh_id, parse.r1kh_id, FT_R1KH_ID_LEN);
        wpa_hexdump(MSG_DEBUG, "FILS+FT: R1KH-ID", sm->r1kh_id, FT_R1KH_ID_LEN);

        /* TODO: Check MDE and FTE payload */

        wpabuf_free(sm->fils_ft_ies);
        sm->fils_ft_ies = wpabuf_alloc(2 + elems.mdie_len + 2 + elems.ftie_len);
        if (!sm->fils_ft_ies)
            goto fail;
        wpabuf_put_data(sm->fils_ft_ies, elems.mdie - 2, 2 + elems.mdie_len);
        wpabuf_put_data(sm->fils_ft_ies, elems.ftie - 2, 2 + elems.ftie_len);
    }
    else
    {
        wpabuf_free(sm->fils_ft_ies);
    ...
    {
        wpa_hexdump(MSG_DEBUG, "FILS: PMKID List", rsn.pmkid, rsn.num_pmkid * PMKID_LEN);
        ...
        if (os_memcmp(sm->cur_pmksa->pmkid, rsn.pmkid, PMKID_LEN) != 0)
        {
            ...
            wpa_hexdump(MSG_DEBUG, "FILS: Expected PMKID", sm->cur_pmksa->pmkid, PMKID_LEN);
            goto fail;
        }
        wpa_printf(MSG_DEBUG, "FILS: Matching PMKID - continue using PMKSA caching");
        pmkid_match = 1;
    }
    if (!pmkid_match && sm->cur_pmksa)
    {
        wpa_printf(MSG_DEBUG, "FILS: No PMKID match - cannot use cached PMKSA entry");
        sm->cur_pmksa = NULL;
    }
    ...
    wpa_hexdump(MSG_DEBUG, "FILS: FILS Session", elems.fils_session, FILS_SESSION_LEN);
    if (os_memcmp(sm->fils_session, elems.fils_session, FILS_SESSION_LEN) != 0)
    {
        wpa_printf(MSG_DEBUG, "FILS: Session mismatch");
        wpa_hexdump(MSG_DEBUG, "FILS: Expected FILS Session", sm->fils_session, FILS_SESSION_LEN);
        goto fail;
    }
    ...
    if (!sm->cur_pmksa && elems.wrapped_data)
    {
        ...
        wpa_hexdump(MSG_DEBUG, "FILS: Wrapped Data", elems.wrapped_data, elems.wrapped_data_len);
        eapol_sm_process_erp_finish(sm->eapol, elems.wrapped_data, elems.wrapped_data_len);
        ...
        res = fils_rmsk_to_pmk(sm->key_mgmt, rmsk, rmsk_len, sm->fils_nonce, sm->fils_anonce,
                               dh_ss ? wpabuf_head(dh_ss) : NULL, dh_ss ? wpabuf_len(dh_ss) : 0, sm->pmk, &sm->pmk_len);
        ...
        wpa_hexdump(MSG_DEBUG, "FILS: PMKID", sm->fils_erp_pmkid, PMKID_LEN);
        wpa_printf(MSG_DEBUG, "FILS: ERP processing succeeded - add PMKSA cache entry for the result");
        sm->cur_pmksa = pmksa_cache_add(sm->pmksa, sm->pmk, sm->pmk_len, sm->fils_erp_pmkid, NULL, 0, sm->bssid,
                                        sm->own_addr, sm->network_ctx, sm->key_mgmt, NULL);
    }

    if (!sm->cur_pmksa)
    {
        wpa_printf(MSG_DEBUG, "FILS: No remaining options to continue FILS authentication");
        goto fail;
    }
    ...
    if (fils_pmk_to_ptk(sm->pmk, sm->pmk_len, sm->own_addr, sm->bssid, sm->fils_nonce, sm->fils_anonce,
                        dh_ss ? wpabuf_head(dh_ss) : NULL, dh_ss ? wpabuf_len(dh_ss) : 0, &sm->ptk, ick, &ick_len,
                        sm->key_mgmt, sm->pairwise_cipher, sm->fils_ft, &sm->fils_ft_len, kdk_len) < 0)
    {
    ...
    res =
        fils_key_auth_sk(ick, ick_len, sm->fils_nonce, sm->fils_anonce, sm->own_addr, sm->bssid, g_sta, g_sta_len, g_ap,
                         g_ap_len, sm->key_mgmt, sm->fils_key_auth_sta, sm->fils_key_auth_ap, &sm->fils_key_auth_len);
    wpabuf_free(pub);
...
static int fils_ft_build_assoc_req_rsne(struct wpa_sm *sm, struct wpabuf *buf)
{
    ...
    {
        wpa_printf(MSG_WARNING, "FILS+FT: Invalid key management type (%d)", sm->key_mgmt);
        return -1;
    }
    ...
    wpa_hexdump_key(MSG_DEBUG, "FILS+FT: XXKey (FILS-FT)", sm->fils_ft, sm->fils_ft_len);
    wpa_hexdump_ascii(MSG_DEBUG, "FILS+FT: SSID", sm->ssid, sm->ssid_len);
    wpa_hexdump(MSG_DEBUG, "FILS+FT: MDID", sm->mobility_domain, MOBILITY_DOMAIN_ID_LEN);
    wpa_hexdump_ascii(MSG_DEBUG, "FILS+FT: R0KH-ID", sm->r0kh_id, sm->r0kh_id_len);
    if (wpa_derive_pmk_r0(sm->fils_ft, sm->fils_ft_len, sm->ssid, sm->ssid_len, sm->mobility_domain, sm->r0kh_id,
                          sm->r0kh_id_len, sm->own_addr, sm->pmk_r0, sm->pmk_r0_name, use_sha384) < 0)
    {
        wpa_printf(MSG_WARNING, "FILS+FT: Could not derive PMK-R0");
        return -1;
    }
    sm->pmk_r0_len = use_sha384 ? SHA384_MAC_LEN : PMK_LEN;
    wpa_printf(MSG_DEBUG, "FILS+FT: R1KH-ID: " MACSTR, MAC2STR(sm->r1kh_id));
    pos = wpabuf_put(buf, WPA_PMK_NAME_LEN);
    if (wpa_derive_pmk_r1_name(sm->pmk_r0_name, sm->r1kh_id, sm->own_addr, sm->pmk_r1_name, use_sha384) < 0)
    {
                                    ...
                                    unsigned int num_hlp)
{
    ...
    *snonce = sm->fils_nonce;
    wpa_hexdump(MSG_DEBUG, "FILS: SNonce for AEAD AAD", *snonce, FILS_NONCE_LEN);
    *anonce = sm->fils_anonce;
    wpa_hexdump(MSG_DEBUG, "FILS: ANonce for AEAD AAD", *anonce, FILS_NONCE_LEN);
...
static void fils_process_hlp_resp(struct wpa_sm *sm, const u8 *resp, size_t len)
{
    ...
    if (end - pos >= 6 && os_memcmp(pos, "\xaa\xaa\x03\x00\x00\x00", 6) == 0)
        pos += 6; /* Remove SNAP/LLC header */
    wpa_sm_fils_hlp_rx(sm, resp, resp + ETH_ALEN, pos, end - pos);
}

static void fils_process_hlp_container(struct wpa_sm *sm, const u8 *pos, size_t len)
{
    ...
    while (end - pos >= 2)
    {
        if (2 + pos[1] > end - pos)
            return;
        if (pos[0] == WLAN_EID_EXTENSION && pos[1] >= 1 + 2 * ETH_ALEN && pos[2] == WLAN_EID_EXT_FILS_HLP_CONTAINER)
            break;
        pos += 2 + pos[1];
    }
    if (end - pos < 2)
    ...
    while (end - pos >= 2)
    {
        if (2 + pos[1] > end - pos || pos[0] != WLAN_EID_EXTENSION || pos[1] < 1 + 2 * ETH_ALEN ||
        ...
        while (end - pos >= 2 && pos[0] == WLAN_EID_FRAGMENT && 2 + pos[1] <= end - pos)
        {
...
int fils_process_assoc_resp(struct wpa_sm *sm, const u8 *resp, size_t len)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "FILS: Association has already been completed for this FILS authentication - ignore unexpected "
                   "retransmission");
        return -1;
    }

    wpa_hexdump(MSG_DEBUG, "FILS: (Re)Association Response frame", resp, len);
    ...
    if (ieee802_11_parse_elems(ie_start, end - ie_start, &elems, 1) == ParseFailed)
    {
    ...
    if (os_memcmp(elems.fils_session, sm->fils_session, FILS_SESSION_LEN) != 0)
    {
        wpa_printf(MSG_DEBUG, "FILS: FILS Session mismatch");
        wpa_hexdump(MSG_DEBUG, "FILS: Received FILS Session", elems.fils_session, FILS_SESSION_LEN);
        wpa_hexdump(MSG_DEBUG, "FILS: Expected FILS Session", sm->fils_session, FILS_SESSION_LEN);
    }

    if (!elems.rsn_ie)
    {
        wpa_printf(MSG_DEBUG, "FILS: No RSNE in (Re)Association Response");
    ...
    else if (wpa_compare_rsn_ie(wpa_key_mgmt_ft(sm->key_mgmt), sm->ap_rsn_ie, sm->ap_rsn_ie_len, elems.rsn_ie - 2,
                                elems.rsn_ie_len + 2))
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
                "FILS: RSNE mismatch between Beacon/Probe Response and (Re)Association Response");
        wpa_hexdump(MSG_DEBUG, "FILS: RSNE in Beacon/Probe Response", sm->ap_rsn_ie, sm->ap_rsn_ie_len);
        wpa_hexdump(MSG_DEBUG, "FILS: RSNE in (Re)Association Response", elems.rsn_ie, elems.rsn_ie_len);
        goto fail;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "FILS: Unexpected Key-Auth length %d (expected %d)", elems.fils_key_confirm_len,
                   (int)sm->fils_key_auth_len);
        goto fail;
    }
    if (os_memcmp(elems.fils_key_confirm, sm->fils_key_auth_ap, sm->fils_key_auth_len) != 0)
    {
        ...
        wpa_hexdump(MSG_DEBUG, "FILS: Expected Key-Auth", sm->fils_key_auth_ap, sm->fils_key_auth_len);
        goto fail;
    }
    ...
    if (wpa_sm_ocv_enabled(sm))
    {
        ...
        if (ocv_verify_tx_params(elems.oci, elems.oci_len, &ci, channel_width_to_int(ci.chanwidth), ci.seg1_idx) !=
            OCI_SUCCESS)
        {
            wpa_msg(sm->ctx->msg_ctx, MSG_INFO, OCV_FAILURE "addr=" MACSTR " frame=fils-assoc error=%s",
                    MAC2STR(sm->bssid), ocv_errorstr);
            goto fail;
        }
    }
    ...
    if (wpa_key_mgmt_ft(sm->key_mgmt) && sm->fils_ft_ies)
    {
        ...
        if (!elems.rsn_ie || wpa_parse_wpa_ie_rsn(elems.rsn_ie - 2, elems.rsn_ie_len + 2, &rsn) < 0 || !rsn.pmkid ||
            rsn.num_pmkid != 1 || os_memcmp(rsn.pmkid, sm->pmk_r1_name, WPA_PMK_NAME_LEN) != 0)
        {
            wpa_printf(MSG_DEBUG, "FILS+FT: No RSNE[PMKR1Name] match in AssocResp");
            goto fail;
        }
    }
#endif /* CONFIG_IEEE80211R */
    ...
    os_memset(&gd, 0, sizeof(gd));
    if (wpa_supplicant_parse_ies(elems.key_delivery + WPA_KEY_RSC_LEN, elems.key_delivery_len - WPA_KEY_RSC_LEN, &kde) <
        0)
    {
    ...
    gd.tx     = wpa_supplicant_gtk_tx_bit_workaround(sm, !!(kde.gtk[0] & BIT(2)));
    if (kde.gtk_len - 2 > sizeof(gd.gtk))
    {
        wpa_printf(MSG_DEBUG, "FILS: Too long GTK in GTK KDE (len=%lu)", (unsigned long)kde.gtk_len - 2);
        goto fail;
    }
    ...
    if (keylen <= 0 || (unsigned int)keylen != sm->ptk.tk_len)
    {
        wpa_printf(MSG_DEBUG, "FILS: TK length mismatch: %u != %lu", keylen, (long unsigned int)sm->ptk.tk_len);
        goto fail;
    }

    rsclen = wpa_cipher_rsc_len(sm->pairwise_cipher);
    wpa_hexdump_key(MSG_DEBUG, "FILS: Set TK to driver", sm->ptk.tk, keylen);
    if (wpa_sm_set_key(sm, alg, sm->bssid, 0, 1, null_rsc, rsclen, sm->ptk.tk, keylen, KEY_FLAG_PAIRWISE_RX_TX) < 0)
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
                "FILS: Failed to set PTK to the driver (alg=%d keylen=%d bssid=" MACSTR ")", alg, keylen,
                MAC2STR(sm->bssid));
        goto fail;
    }
...
struct wpabuf *owe_build_assoc_req(struct wpa_sm *sm, u16 group)
{
    ...
    wpabuf_free(pub);
    wpa_hexdump_buf(MSG_DEBUG, "OWE: Diffie-Hellman Parameter element", ie);
...
int owe_process_assoc_resp(struct wpa_sm *sm, const u8 *bssid, const u8 *resp_ies, size_t resp_ies_len)
{
    ...
    if (!resp_ies || ieee802_11_parse_elems(resp_ies, resp_ies_len, &elems, 1) == ParseFailed)
    {
        wpa_printf(MSG_INFO, "OWE: Could not parse Association Response frame elements");
        return -1;
    }

    if (sm->cur_pmksa && elems.rsn_ie && wpa_parse_wpa_ie_rsn(elems.rsn_ie - 2, 2 + elems.rsn_ie_len, &data) == 0 &&
        data.num_pmkid == 1 && data.pmkid && os_memcmp(sm->cur_pmksa->pmkid, data.pmkid, PMKID_LEN) == 0)
    {
    ...
    if (!elems.owe_dh)
    {
        wpa_printf(MSG_INFO, "OWE: No Diffie-Hellman Parameter element found in Association Response frame");
        return -1;
    }
    ...
    {
        wpa_printf(MSG_INFO, "OWE: Unexpected Diffie-Hellman group in response: %u", group);
        return -1;
    }
    ...
    if (group == 19)
        res = hmac_sha256(wpabuf_head(hkey), wpabuf_len(hkey), wpabuf_head(secret), wpabuf_len(secret), prk);
    else if (group == 20)
        res = hmac_sha384(wpabuf_head(hkey), wpabuf_len(hkey), wpabuf_head(secret), wpabuf_len(secret), prk);
    else if (group == 21)
        res = hmac_sha512(wpabuf_head(hkey), wpabuf_len(hkey), wpabuf_head(secret), wpabuf_len(secret), prk);
    ...
    if (group == 19)
        res = hmac_sha256_kdf(prk, hash_len, NULL, (const u8 *)info, os_strlen(info), sm->pmk, hash_len);
    else if (group == 20)
        res = hmac_sha384_kdf(prk, hash_len, NULL, (const u8 *)info, os_strlen(info), sm->pmk, hash_len);
    else if (group == 21)
        res = hmac_sha512_kdf(prk, hash_len, NULL, (const u8 *)info, os_strlen(info), sm->pmk, hash_len);
    ...
    pmksa_cache_add(sm->pmksa, sm->pmk, sm->pmk_len, pmkid, NULL, 0, bssid, sm->own_addr, sm->network_ctx, sm->key_mgmt,
                    NULL);
...
#ifdef CONFIG_PASN
void wpa_pasn_pmksa_cache_add(
    struct wpa_sm *sm, const u8 *pmk, size_t pmk_len, const u8 *pmkid, const u8 *bssid, int key_mgmt)
{
    sm->cur_pmksa = pmksa_cache_add(sm->pmksa, pmk, pmk_len, pmkid, NULL, 0, bssid, sm->own_addr, NULL, key_mgmt, 0);
}
#endif /* CONFIG_PASN */
