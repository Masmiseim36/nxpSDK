                      ...
                      struct wpa_ptk *ptk)
{
    ...
    if (wpa_derive_pmk_r0(mpmk, mpmk_len, sm->ssid, sm->ssid_len, sm->mobility_domain, sm->r0kh_id, sm->r0kh_id_len,
                          sm->own_addr, sm->pmk_r0, sm->pmk_r0_name, use_sha384) < 0)
        return -1;
    sm->pmk_r1_len = sm->pmk_r0_len;
    if (wpa_derive_pmk_r1(sm->pmk_r0, sm->pmk_r0_len, sm->pmk_r0_name, sm->r1kh_id, sm->own_addr, sm->pmk_r1,
                          sm->pmk_r1_name) < 0)
        return -1;
    ...
    return wpa_pmk_r1_to_ptk(sm->pmk_r1, sm->pmk_r1_len, sm->snonce, anonce, sm->own_addr, sm->bssid, sm->pmk_r1_name,
                             ptk, ptk_name, sm->key_mgmt, sm->pairwise_cipher, kdk_len);
}
 ...
 */
static u8 *wpa_ft_gen_req_ies(struct wpa_sm *sm,
                              size_t *len,
                              const u8 *anonce,
                              const u8 *pmk_name,
                              const u8 *kck,
                              ...
                              int omit_rsnxe)
{
    ...
    buf_len =
        2 + sizeof(struct rsn_mdie) + 2 + sizeof(struct rsn_ftie_sha384) + 2 + sm->r0kh_id_len + ric_ies_len + 100;
    ...
    *pos++     = WLAN_EID_FAST_BSS_TRANSITION;
    ftie_len   = pos++;
    rsnxe_used = wpa_key_mgmt_sae(sm->key_mgmt) && anonce && (sm->sae_pwe == 1 || sm->sae_pwe == 2);
    ...
    *pos++ = sm->r0kh_id_len;
    ...
    if (kck && wpa_sm_ocv_enabled(sm))
    {
        ...
        {
            wpa_printf(MSG_WARNING, "Failed to get channel info for OCI element in FTE");
            os_free(buf);
            return NULL;
        }
#ifdef CONFIG_TESTING_OPTIONS
    ...
    *ftie_len = pos - ftie_len - 1;
    ...
    if (kck)
    {
        ...
        *elem_count = 3 + ieee802_11_ie_count(ric_ies, ric_ies_len);
        if (rsnxe_len)
            *elem_count += 1;
        if (wpa_ft_mic(kck, kck_len, sm->own_addr, target_ap, 5, ((u8 *)mdie) - 2, 2 + sizeof(*mdie), ftie_pos,
                       2 + *ftie_len, (u8 *)rsnie, 2 + rsnie->len, ric_ies, ric_ies_len, rsnxe_len ? rsnxe : NULL,
                       rsnxe_len, fte_mic) < 0)
        {
...
static int wpa_ft_install_ptk(struct wpa_sm *sm, const u8 *bssid)
{
    ...
    if (wpa_sm_set_key(sm, alg, bssid, 0, 1, null_rsc, sizeof(null_rsc), (u8 *)sm->ptk.tk, keylen,
...
int wpa_ft_prepare_auth_request(struct wpa_sm *sm, const u8 *mdie)
{
    ...
    ft_ies = wpa_ft_gen_req_ies(sm, &ft_ies_len, NULL, sm->pmk_r0_name, NULL, 0, sm->bssid, NULL, 0, mdie, 0);
    if (ft_ies)
    {
...
int wpa_ft_add_mdie(struct wpa_sm *sm, u8 *buf, size_t buf_len, const u8 *ap_mdie)
{
    u8 *pos = buf;
    struct rsn_mdie *mdie;
    ...
    {
        wpa_printf(MSG_INFO, "FT: Failed to add MDIE: short buffer, length=%zu", buf_len);
        return 0;
    }
...
int wpa_ft_process_response(struct wpa_sm *sm,
                            const u8 *ies,
                            size_t ies_len,
                            int ft_action,
                            const u8 *target_ap,
                            ...
                            size_t ric_ies_len)
{
    ...
    {
        wpa_printf(MSG_INFO, "FT: Target AP does not support PMF, but local configuration requires that");
        return -1;
    }
    ...
    if (wpa_pmk_r1_to_ptk(sm->pmk_r1, sm->pmk_r1_len, sm->snonce, anonce, sm->own_addr, bssid, sm->pmk_r1_name,
                          &sm->ptk, ptk_name, sm->key_mgmt, sm->pairwise_cipher, kdk_len) < 0)
        return -1;
    ...
    ft_ies = wpa_ft_gen_req_ies(sm, &ft_ies_len, anonce, sm->pmk_r1_name, kck, kck_len, bssid, ric_ies, ric_ies_len,
                                parse.mdie ? parse.mdie - 2 : NULL, !sm->ap_rsnxe);
    if (ft_ies)
    {
        wpa_sm_update_ft_ies(sm, sm->mobility_domain, ft_ies, ft_ies_len);
        os_free(ft_ies);
...
static int wpa_ft_process_gtk_subelem(struct wpa_sm *sm, const u8 *gtk_elem, size_t gtk_elem_len)
{
    ...
    if (wpa_sm_set_key(sm, alg, broadcast_ether_addr, keyidx, 0, gtk_elem + 3, rsc_len, gtk, keylen,
                       KEY_FLAG_GROUP_RX) < 0)
    {
        wpa_printf(MSG_WARNING,
...
static int wpa_ft_process_igtk_subelem(struct wpa_sm *sm, const u8 *igtk_elem, size_t igtk_elem_len)
{
    ...
    if (sm->mgmt_group_cipher != WPA_CIPHER_AES_128_CMAC && sm->mgmt_group_cipher != WPA_CIPHER_BIP_GMAC_128 &&
        sm->mgmt_group_cipher != WPA_CIPHER_BIP_GMAC_256 && sm->mgmt_group_cipher != WPA_CIPHER_BIP_CMAC_256)
        return 0;
    ...
    wpa_hexdump_key(MSG_DEBUG, "FT: IGTK from Reassoc Resp", igtk, igtk_len);
    if (wpa_sm_set_key(sm, wpa_cipher_to_alg(sm->mgmt_group_cipher), broadcast_ether_addr, keyidx, 0, igtk_elem + 2, 6,
                       igtk, igtk_len, KEY_FLAG_GROUP_RX) < 0)
    {
        wpa_printf(MSG_WARNING,
...
static int wpa_ft_process_bigtk_subelem(struct wpa_sm *sm, const u8 *bigtk_elem, size_t bigtk_elem_len)
{
    ...
    if (!sm->beacon_prot || !bigtk_elem ||
        (sm->mgmt_group_cipher != WPA_CIPHER_AES_128_CMAC && sm->mgmt_group_cipher != WPA_CIPHER_BIP_GMAC_128 &&
         sm->mgmt_group_cipher != WPA_CIPHER_BIP_GMAC_256 && sm->mgmt_group_cipher != WPA_CIPHER_BIP_CMAC_256))
        return 0;
    ...
    wpa_hexdump_key(MSG_DEBUG, "FT: Received BIGTK in Reassoc Resp", bigtk_elem, bigtk_elem_len);

    bigtk_len = wpa_cipher_key_len(sm->mgmt_group_cipher);
    if (bigtk_elem_len != 2 + 6 + 1 + bigtk_len + 8)
    {
        wpa_printf(MSG_DEBUG, "FT: Invalid BIGTK sub-elem length %lu", (unsigned long)bigtk_elem_len);
        return -1;
    }
    if (bigtk_elem[8] != bigtk_len)
    {
        wpa_printf(MSG_DEBUG, "FT: Invalid BIGTK sub-elem Key Length %d", bigtk_elem[8]);
        return -1;
    }

    if (aes_unwrap(kek, kek_len, bigtk_len / 8, bigtk_elem + 9, bigtk))
    {
        wpa_printf(MSG_WARNING, "FT: AES unwrap failed - could not decrypt BIGTK");
        return -1;
    }
    ...
    wpa_hexdump_key(MSG_DEBUG, "FT: BIGTK from Reassoc Resp", bigtk, bigtk_len);
    if (wpa_sm_set_key(sm, wpa_cipher_to_alg(sm->mgmt_group_cipher), broadcast_ether_addr, keyidx, 0, bigtk_elem + 2, 6,
                       bigtk, bigtk_len, KEY_FLAG_GROUP_RX) < 0)
    {
        wpa_printf(MSG_WARNING, "WPA: Failed to set BIGTK to the driver");
...
int wpa_ft_validate_reassoc_resp(struct wpa_sm *sm, const u8 *ies, size_t ies_len, const u8 *src_addr)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "FT: Reassociation has already been completed for this FT protocol instance - ignore unexpected "
                   "retransmission");
        return 0;
    }
    ...
    if (wpa_ft_mic(kck, kck_len, sm->own_addr, src_addr, 6, parse.mdie - 2, parse.mdie_len + 2, parse.ftie - 2,
    ...
    if (!sm->ap_rsn_ie)
    {
        wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG, "FT: No RSNE for this AP known - trying to get from scan results");
        if (wpa_sm_get_beacon_ie(sm) < 0)
        {
            wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "FT: Could not find AP from the scan results");
            return -1;
        }
        wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "FT: Found the current AP from updated scan results");
    }

    if (sm->ap_rsn_ie && wpa_compare_rsn_ie(wpa_key_mgmt_ft(sm->key_mgmt), sm->ap_rsn_ie, sm->ap_rsn_ie_len,
                                            parse.rsn - 2, parse.rsn_len + 2))
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
                "FT: RSNE mismatch between Beacon/ProbeResp and FT protocol Reassociation Response frame");
        wpa_hexdump(MSG_INFO, "RSNE in Beacon/ProbeResp", sm->ap_rsn_ie, sm->ap_rsn_ie_len);
        wpa_hexdump(MSG_INFO, "RSNE in FT protocol Reassociation Response frame", parse.rsn ? parse.rsn - 2 : NULL,
                    parse.rsn ? parse.rsn_len + 2 : 0);
        return -1;
    }

    own_rsnxe_used = wpa_key_mgmt_sae(sm->key_mgmt) && (sm->sae_pwe == 1 || sm->sae_pwe == 2);
    if ((sm->ap_rsnxe && !parse.rsnxe && own_rsnxe_used) || (!sm->ap_rsnxe && parse.rsnxe) ||
        (sm->ap_rsnxe && parse.rsnxe &&
         (sm->ap_rsnxe_len != 2 + parse.rsnxe_len || os_memcmp(sm->ap_rsnxe, parse.rsnxe - 2, sm->ap_rsnxe_len) != 0)))
    {
        wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
        ...
        wpa_hexdump(MSG_INFO, "RSNXE in FT protocol Reassociation Response frame", parse.rsnxe ? parse.rsnxe - 2 : NULL,
    ...
    if (wpa_sm_ocv_enabled(sm))
    {
        ...
        if (wpa_sm_channel_info(sm, &ci) != 0)
        {
            wpa_printf(MSG_WARNING, "Failed to get channel info to validate received OCI in (Re)Assoc Response");
            return -1;
        }

        if (ocv_verify_tx_params(parse.oci, parse.oci_len, &ci, channel_width_to_int(ci.chanwidth), ci.seg1_idx) !=
            OCI_SUCCESS)
        {
            wpa_msg(sm->ctx->msg_ctx, MSG_INFO, OCV_FAILURE "addr=" MACSTR " frame=ft-assoc error=%s",
...
int wpa_ft_start_over_ds(struct wpa_sm *sm, const u8 *target_ap, const u8 *mdie)
{
    ...
    ft_ies = wpa_ft_gen_req_ies(sm, &ft_ies_len, NULL, sm->pmk_r0_name, NULL, 0, target_ap, NULL, 0, mdie, 0);
    if (ft_ies)
    {
...
static struct pasn_ft_r1kh *wpa_ft_pasn_get_r1kh(struct wpa_sm *sm, const u8 *bssid)
{
    size_t i;
...
static void wpa_ft_pasn_store_r1kh(struct wpa_sm *sm, const u8 *bssid)
{
    ...
    tmp = os_realloc_array(sm->pasn_r1kh, sm->n_pasn_r1kh + 1, sizeof(*tmp));
    if (!tmp)
    {
    ...
    wpa_printf(MSG_DEBUG, "PASN: FT: Store R1KH for " MACSTR, MAC2STR(bssid));
...
int wpa_pasn_ft_derive_pmk_r1(
    struct wpa_sm *sm, int akmp, const u8 *bssid, u8 *pmk_r1, size_t *pmk_r1_len, u8 *pmk_r1_name)
{
    struct pasn_ft_r1kh *r1kh_entry;

    if (sm->key_mgmt != (unsigned int)akmp)
    {
        wpa_printf(MSG_DEBUG, "PASN: FT: Key management mismatch: %u != %u", sm->key_mgmt, akmp);
        return -1;
    }
    ...
    if (!r1kh_entry)
    {
        wpa_printf(MSG_DEBUG, "PASN: FT: Cannot find R1KH-ID for " MACSTR, MAC2STR(bssid));
        return -1;
    }
     ...
     */
    if (wpa_derive_pmk_r1(sm->pmk_r0, sm->pmk_r0_len, sm->pmk_r0_name, r1kh_entry->r1kh_id, sm->own_addr, pmk_r1,
                          pmk_r1_name) < 0)
        return -1;
    ...
    *pmk_r1_len = sm->pmk_r0_len;
    ...
    wpa_hexdump(MSG_DEBUG, "PASN: FT: PMKR1Name", pmk_r1_name, WPA_PMK_NAME_LEN);
