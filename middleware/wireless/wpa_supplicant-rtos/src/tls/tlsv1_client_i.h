...
int tls_derive_pre_master_secret(struct tlsv1_client *conn, u8 *pre_master_secret);
...
int tlsv1_client_process_handshake(
    struct tlsv1_client *conn, u8 ct, const u8 *buf, size_t *len, u8 **out_data, size_t *out_len);
...
enum tls_ocsp_result tls_process_ocsp_response(struct tlsv1_client *conn, const u8 *resp, size_t len);
