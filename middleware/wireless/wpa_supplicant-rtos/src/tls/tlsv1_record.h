...
#define TLS_MAX_KEY_BLOCK_LEN        (2 * (TLS_MAX_WRITE_MAC_SECRET_LEN + TLS_MAX_WRITE_KEY_LEN + TLS_MAX_IV_LEN))
...
int tlsv1_record_send(struct tlsv1_record_layer *rl,
                      ...
                      size_t payload_len,
                      size_t *out_len);
int tlsv1_record_receive(
    struct tlsv1_record_layer *rl, const u8 *in_data, size_t in_len, u8 *out_data, size_t *out_len, u8 *alert);
