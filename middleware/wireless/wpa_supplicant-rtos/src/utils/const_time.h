...
#define NO_UBSAN_UINT_OVERFLOW __attribute__((no_sanitize("unsigned-integer-overflow")))
#else
...
static inline unsigned int const_time_is_zero(unsigned int val) NO_UBSAN_UINT_OVERFLOW
{
 ...
 * length.
 */
static inline unsigned int const_time_eq_bin(const void *a, const void *b, size_t len)
{
    const u8 *aa = a;
    const u8 *bb = b;
    size_t i;
 ...
 */
static inline unsigned int const_time_select(unsigned int mask, unsigned int true_val, unsigned int false_val)
{
...
{
    return (s8)const_time_select(mask, (unsigned int)true_val, (unsigned int)false_val);
}
 ...
 * is being copied.
 */
static inline void const_time_select_bin(u8 mask, const u8 *true_val, const u8 *false_val, size_t len, u8 *dst)
{
    size_t i;
