    ...
    for (i = 0; i < len; i++)
    {
        ...
        switch (data[i])
        {
            ...
            default:
                ...
                {
                    txt += os_snprintf(txt, end - txt, "\\u%04x", (unsigned char)data[i]);
                }
                break;
        }
    }
    ...
    for (; pos < end; pos++)
    {
        ...
        switch (*pos)
        {
            ...
            case '\\':
                ...
                if (pos >= end)
                {
                    wpa_printf(MSG_DEBUG, "JSON: Truncated \\ escape");
                    goto fail;
                }
                switch (*pos)
                {
                    ...
                    case 't':
                        *spos++ = '\t';
                        break;
                    case 'u':
                        if (end - pos < 5 || hexstr2bin(pos + 1, bin, 2) < 0 || bin[1] == 0x00)
                        {
                            wpa_printf(MSG_DEBUG, "JSON: Invalid \\u escape");
                            goto fail;
                        }
                        if (bin[0] == 0x00)
                        {
                        ...
                        break;
                    default:
                        wpa_printf(MSG_DEBUG, "JSON: Unknown escape '%c'", *pos);
                        goto fail;
                }
                break;
            default:
...
static int json_parse_number(const char **json_pos, const char *end, int *ret_val)
{
    const char *pos = *json_pos;
    size_t len;
    char *str;
...
static int json_check_tree_state(struct json_token *token)
{
    if (!token)
        return 0;
    if (json_check_tree_state(token->child) < 0 || json_check_tree_state(token->sibling) < 0)
        return -1;
    if (token->state != JSON_COMPLETED)
    {
        wpa_printf(MSG_DEBUG, "JSON: Unexpected token state %d (name=%s type=%d)", token->state,
                   token->name ? token->name : "N/A", token->type);
        return -1;
    }
    return 0;
}
...
struct json_token *json_parse(const char *data, size_t data_len)
{
    ...
    for (; pos < end; pos++)
    {
        switch (*pos)
        {
            ...
            case '{': /* start object */
                ...
                else if (curr_token->state == JSON_WAITING_VALUE)
                {
                    token = curr_token;
                         ...
                         curr_token->parent->state == JSON_STARTED && curr_token->state == JSON_EMPTY)
                {
                    token = curr_token;
                }
                else
                {
                    wpa_printf(MSG_DEBUG, "JSON: Invalid state for start array/object");
                    goto fail;
                }
                depth++;
                if (depth > JSON_MAX_DEPTH)
                {
                    wpa_printf(MSG_DEBUG, "JSON: Max depth exceeded");
                    goto fail;
                }
            ...
            case ']': /* end array */
            case '}': /* end object */
                if (!curr_token || !curr_token->parent || curr_token->parent->state != JSON_STARTED)
                {
                    wpa_printf(MSG_DEBUG, "JSON: Invalid state for end array/object");
                    goto fail;
                }
                depth--;
                curr_token = curr_token->parent;
                if ((*pos == ']' && curr_token->type != JSON_ARRAY) || (*pos == '}' && curr_token->type != JSON_OBJECT))
                {
                    wpa_printf(MSG_DEBUG, "JSON: Array/Object mismatch");
                    goto fail;
                }
                if (curr_token->child->state == JSON_EMPTY && !curr_token->child->child && !curr_token->child->sibling)
                {
            ...
            case '\"': /* string */
                         ...
                         curr_token->parent->state == JSON_STARTED && curr_token->state == JSON_EMPTY)
                {
                    ...
                    wpa_printf(MSG_MSGDUMP, "JSON: String value: '%s'", curr_token->string);
                }
                ...
                else if (curr_token->state == JSON_WAITING_VALUE)
                {
                    ...
                    curr_token->state  = JSON_COMPLETED;
                    curr_token->type   = JSON_STRING;
                    wpa_printf(MSG_MSGDUMP, "JSON: String value: '%s' = '%s'", curr_token->name, curr_token->string);
                }
                else
                {
                    wpa_printf(MSG_DEBUG, "JSON: Invalid state for a string");
                    os_free(str);
                    goto fail;
                }
                break;
            ...
            case 'f': /* false */
            case 'n': /* null */
                if (!((end - pos >= 4 && os_strncmp(pos, "true", 4) == 0) ||
                      (end - pos >= 5 && os_strncmp(pos, "false", 5) == 0) ||
                      (end - pos >= 4 && os_strncmp(pos, "null", 4) == 0)))
                {
                    wpa_printf(MSG_DEBUG, "JSON: Invalid literal name");
                    goto fail;
                }
                if (!curr_token)
                {
                ...
                {
                    wpa_printf(MSG_MSGDUMP, "JSON: Literal name: '%s' = %c", curr_token->name, *pos);
                }
                else if (curr_token->parent && curr_token->parent->type == JSON_ARRAY &&
                         curr_token->parent->state == JSON_STARTED && curr_token->state == JSON_EMPTY)
                {
                    wpa_printf(MSG_MSGDUMP, "JSON: Literal name: %c", *pos);
                }
                else
                {
                    wpa_printf(MSG_DEBUG, "JSON: Invalid state for a literal name");
                    goto fail;
                }
                switch (*pos)
                {
            ...
            case '9':
                ...
                else if (curr_token->state == JSON_WAITING_VALUE)
                {
                    ...
                    wpa_printf(MSG_MSGDUMP, "JSON: Number value: '%s' = '%d'", curr_token->name, curr_token->number);
                }
                else if (curr_token->parent && curr_token->parent->type == JSON_ARRAY &&
                         curr_token->parent->state == JSON_STARTED && curr_token->state == JSON_EMPTY)
                {
                    ...
                    curr_token->state  = JSON_COMPLETED;
                    curr_token->type   = JSON_NUMBER;
                    wpa_printf(MSG_MSGDUMP, "JSON: Number value: %d", curr_token->number);
                }
                else
                {
                    wpa_printf(MSG_DEBUG, "JSON: Invalid state for a number");
                    goto fail;
                }
                break;
            default:
                wpa_printf(MSG_DEBUG, "JSON: Unexpected JSON character: %c", *pos);
                goto fail;
        }
...
struct wpabuf *json_get_member_base64url(struct json_token *json, const char *name)
{
    ...
    if (!token || token->type != JSON_STRING)
        return NULL;
    buf = base64_url_decode(token->string, os_strlen(token->string), &buflen);
    if (!buf)
        return NULL;
...
struct wpabuf *json_get_member_base64(struct json_token *json, const char *name)
{
    struct json_token *token;
    unsigned char *buf;
    size_t buflen;
    ...
    if (!token || token->type != JSON_STRING)
        return NULL;
    buf = wpa_base64_decode(token->string, os_strlen(token->string), &buflen);
    if (!buf)
        return NULL;
    ret = wpabuf_alloc_ext_data(buf, buflen);
    if (!ret)
...
static void json_print_token(struct json_token *token, int depth, char *buf, size_t buflen)
{
    size_t len;
    int ret;
    ...
    if (!token)
        return;
    len = os_strlen(buf);
    ret = os_snprintf(buf + len, buflen - len, "[%d:%s:%s]", depth, json_type_str(token->type),
...
int json_add_string_escape(struct wpabuf *json, const char *name, const void *val, size_t len)
{
    char *tmp;
...
int json_add_base64url(struct wpabuf *json, const char *name, const void *val, size_t len)
{
    char *b64;
...
int json_add_base64(struct wpabuf *json, const char *name, const void *val, size_t len)
{
    char *b64;
