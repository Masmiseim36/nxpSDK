...
int uuid_bin2str(const u8 *bin, char *str, size_t max_len)
{
    ...
    len = os_snprintf(str, max_len,
                      ...
                      "%02x%02x-%02x%02x%02x%02x%02x%02x",
                      bin[0], bin[1], bin[2], bin[3], bin[4], bin[5], bin[6], bin[7], bin[8], bin[9], bin[10], bin[11],
...
int uuid_random(u8 *uuid)
{
    ...
    os_get_time(&t);
    if (os_get_random(uuid, UUID_LEN) < 0 || hmac_sha256(uuid, UUID_LEN, (const u8 *)&t, sizeof(t), hash) < 0)
        return -1;
