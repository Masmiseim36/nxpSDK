...
static void _wpa_hexdump(int level, const char *title, const u8 *buf, size_t len, int show, int only_syslog)
{
...
#ifdef CONFIG_ANDROID_LOG
    {
        ...
        __android_log_print(wpa_to_android_level(level), ANDROID_LOG_NAME, "%s - hexdump(len=%lu):%s%s", title,
                            (long unsigned int)len, display, len > slen ? " ..." : "");
...
static void _wpa_hexdump_ascii(int level, const char *title, const void *buf, size_t len, int show)
{
    ...
    if (wpa_debug_tracing_file != NULL)
    {
        fprintf(wpa_debug_tracing_file, WPAS_TRACE_PFX "%s - hexdump_ascii(len=%lu):", level, title,
                (unsigned long)len);
        if (buf == NULL)
        {
            fprintf(wpa_debug_tracing_file, " [NULL]\n");
...
int wpa_debug_open_file(const char *path)
{
    ...
    out_fd = open(path, O_CREAT | O_APPEND | O_WRONLY, S_IRUSR | S_IWUSR | S_IRGRP);
    if (out_fd < 0)
    {
        wpa_printf(MSG_ERROR, "%s: Failed to open output file descriptor, using standard output", __func__);
        return -1;
    }
    ...
    {
        wpa_printf(MSG_DEBUG, "%s: Failed to set FD_CLOEXEC - continue without: %s", __func__, strerror(errno));
    }
#endif /* __linux__ */
