...
xml_node_t *xml_node_create_root(
    struct xml_node_ctx *ctx, const char *ns_uri, const char *ns_prefix, xml_namespace_t **ret_ns, const char *name);
...
xml_node_t *xml_node_create_text(
    struct xml_node_ctx *ctx, xml_node_t *parent, xml_namespace_t *ns, const char *name, const char *value);
xml_node_t *xml_node_create_text_ns(
    struct xml_node_ctx *ctx, xml_node_t *parent, const char *ns_uri, const char *name, const char *value);
void xml_node_set_text(struct xml_node_ctx *ctx, xml_node_t *node, const char *value);
int xml_node_add_attr(
    struct xml_node_ctx *ctx, xml_node_t *node, xml_namespace_t *ns, const char *name, const char *value);
...
#define xml_node_for_each_child(ctx, child, parent) \
    for (child = xml_node_first_child(ctx, parent); child; child = xml_node_next_sibling(ctx, child))

#define xml_node_for_each_sibling(ctx, node) for (; node; node = xml_node_next_sibling(ctx, node))
