...
struct wps_data *wps_init(const struct wps_config *cfg)
{
    ...
    if (cfg->wps->ap && !cfg->registrar && cfg->wps->ap_nfc_dev_pw_id)
    {
        ...
        data->dev_password = os_memdup(wpabuf_head(cfg->wps->ap_nfc_dev_pw), wpabuf_len(cfg->wps->ap_nfc_dev_pw));
        if (data->dev_password == NULL)
        {
            os_free(data);
            return NULL;
    ...
    {
        data->new_ap_settings = os_memdup(cfg->new_ap_settings, sizeof(*data->new_ap_settings));
...
int wps_is_selected_pbc_registrar(const struct wpabuf *msg)
{
    ...
    if (wps_parse_msg(msg, &attr) < 0 || !attr.selected_registrar || *attr.selected_registrar == 0 ||
        !attr.dev_password_id || WPA_GET_BE16(attr.dev_password_id) != DEV_PW_PUSHBUTTON)
        return 0;
...
static int is_selected_pin_registrar(struct wps_parse_attr *attr)
{
    ...
    if (!attr->sel_reg_config_methods ||
        !(WPA_GET_BE16(attr->sel_reg_config_methods) & (WPS_CONFIG_LABEL | WPS_CONFIG_DISPLAY | WPS_CONFIG_KEYPAD)))
        return 0;
#endif /* CONFIG_WPS_STRICT */
...
struct wpabuf *wps_build_assoc_req_ie(enum wps_request_type req_type)
{
    ...
    if (wps_build_version(ie) || wps_build_req_type(ie, req_type) || wps_build_wfa_ext(ie, 0, NULL, 0, 0))
    {
        wpabuf_free(ie);
        return NULL;
    }
...
struct wpabuf *wps_build_assoc_resp_ie(void)
{
    ...
    if (wps_build_version(ie) || wps_build_resp_type(ie, WPS_RESP_AP) || wps_build_wfa_ext(ie, 0, NULL, 0, 0))
    {
        wpabuf_free(ie);
        return NULL;
    }
                                      ...
                                      const u8 *req_dev_types)
{
    ...
    if (wps_build_version(ie) || wps_build_req_type(ie, req_type) ||
        ...
        wps_build_primary_dev_type(dev, ie) || wps_build_rf_bands(dev, ie, 0) || wps_build_assoc_state(NULL, ie) ||
        wps_build_config_error(ie, WPS_CFG_NO_ERROR) || wps_build_dev_password_id(ie, pw_id) ||
        wps_build_manufacturer(dev, ie) || wps_build_model_name(dev, ie) || wps_build_model_number(dev, ie) ||
        wps_build_dev_name(dev, ie) || wps_build_wfa_ext(ie, req_type == WPS_REQ_ENROLLEE, NULL, 0, 0) ||
        wps_build_req_dev_type(dev, ie, num_req_dev_types, req_dev_types) || wps_build_secondary_dev_type(dev, ie))
    {
        wpabuf_free(ie);
        return NULL;
    }
