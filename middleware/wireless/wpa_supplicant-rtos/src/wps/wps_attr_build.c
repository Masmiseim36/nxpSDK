...
int wps_build_public_key(struct wps_data *wps, struct wpabuf *msg)
{
    ...
    else if ((wps->dev_pw_id >= 0x10 || wps->dev_pw_id == DEV_PW_NFC_CONNECTION_HANDOVER) &&
             (wps->wps->ap ||
              (wps->wps->ap_nfc_dh_pubkey && wps->wps->ap_nfc_dev_pw_id == DEV_PW_NFC_CONNECTION_HANDOVER &&
               wps->dev_pw_id == DEV_PW_NFC_CONNECTION_HANDOVER)) &&
             (wps->dev_pw_id == wps->wps->ap_nfc_dev_pw_id || wps->wps->ap_nfc_dh_pubkey))
    {
...
int wps_build_authenticator(struct wps_data *wps, struct wpabuf *msg)
{
    ...
    if (hmac_sha256_vector(wps->authkey, WPS_AUTHKEY_LEN, 2, addr, len, hash) < 0)
        return -1;
...
int wps_build_wfa_ext(
    struct wpabuf *msg, int req_to_enroll, const u8 *auth_macs, size_t auth_macs_count, u8 multi_ap_subelem)
{
    u8 *len;
...
int wps_build_auth_type_flags(struct wps_data *wps, struct wpabuf *msg)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "WPS: Testing - replace auth type 0x%x with 0x%x", auth_types, wps_force_auth_types);
...
int wps_build_encr_type_flags(struct wps_data *wps, struct wpabuf *msg)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "WPS: Testing - replace encr type 0x%x with 0x%x", encr_types, wps_force_encr_types);
...
int wps_build_key_wrap_auth(struct wps_data *wps, struct wpabuf *msg)
{
    ...
    if (hmac_sha256(wps->authkey, WPS_AUTHKEY_LEN, wpabuf_head(msg), wpabuf_len(msg), hash) < 0)
        return -1;
...
int wps_build_oob_dev_pw(
    struct wpabuf *msg, u16 dev_pw_id, const struct wpabuf *pubkey, const u8 *dev_pw, size_t dev_pw_len)
{
    size_t hash_len;
    const u8 *addr[1];
