...
static int wps_set_vendor_ext_wfa_subelem(struct wps_parse_attr *attr, u8 id, u8 len, const u8 *pos)
{
    ...
    switch (id)
    {
        ...
        case WFA_ELEM_MULTI_AP:
            if (len != 1)
            {
                wpa_printf(MSG_DEBUG, "WPS: Invalid Multi-AP Extension length %u", len);
                return -1;
            }
            attr->multi_ap_ext = *pos;
            wpa_printf(MSG_DEBUG, "WPS: Multi-AP Extension 0x%02x", attr->multi_ap_ext);
            break;
        default:
...
static int wps_set_attr(struct wps_parse_attr *attr, u16 type, const u8 *pos, u16 len)
{
    switch (type)
    {
        ...
        case ATTR_OOB_DEVICE_PASSWORD:
            if (len < WPS_OOB_PUBKEY_HASH_LEN + 2 || len > WPS_OOB_PUBKEY_HASH_LEN + 2 + WPS_OOB_DEVICE_PASSWORD_LEN ||
