...
int wps_derive_psk(struct wps_data *wps, const u8 *dev_passwd, size_t dev_passwd_len)
{
    ...
    if (hmac_sha256(wps->authkey, WPS_AUTHKEY_LEN, dev_passwd, (dev_passwd_len + 1) / 2, hash) < 0)
        return -1;
    os_memcpy(wps->psk1, hash, WPS_PSK_LEN);
    if (hmac_sha256(wps->authkey, WPS_AUTHKEY_LEN, dev_passwd + (dev_passwd_len + 1) / 2, dev_passwd_len / 2, hash) < 0)
        return -1;
...
void wps_fail_event(
    struct wps_context *wps, enum wps_msg_type msg, u16 config_error, u16 error_indication, const u8 *mac_addr)
{
...
struct wpabuf *wps_get_oob_cred(struct wps_context *wps, int rf_band, int channel)
{
    ...
    if (wps_build_cred(&data, plain) || (rf_band && wps_build_rf_bands_attr(plain, rf_band)) ||
        (channel && wps_build_ap_channel(plain, channel)) || wps_build_mac_addr(plain, wps->dev.mac_addr) ||
...
struct wpabuf *wps_build_nfc_pw_token(u16 dev_pw_id, const struct wpabuf *pubkey, const struct wpabuf *dev_pw)
{
    ...
    if (wps_build_oob_dev_pw(data, dev_pw_id, pubkey, wpabuf_head(dev_pw), wpabuf_len(dev_pw)) ||
        wps_build_wfa_ext(data, 0, NULL, 0, 0))
    {
        wpa_printf(MSG_ERROR,
                   "WPS: Failed to build NFC password "
                   "token");
...
char *wps_dev_type_bin2str(const u8 dev_type[WPS_DEV_TYPE_LEN], char *buf, size_t buf_len)
{
    ...
    ret = os_snprintf(buf, buf_len, "%u-%08lX-%u", WPA_GET_BE16(dev_type), WPA_GET_BE32(&dev_type[2]),
...
struct wpabuf *wps_build_wsc_ack(struct wps_data *wps)
{
    ...
    if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_WSC_ACK) || wps_build_enrollee_nonce(wps, msg) ||
...
struct wpabuf *wps_build_wsc_nack(struct wps_data *wps)
{
    ...
    if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_WSC_NACK) || wps_build_enrollee_nonce(wps, msg) ||
        wps_build_registrar_nonce(wps, msg) || wps_build_config_error(msg, wps->config_error) ||
...
struct wpabuf *wps_nfc_token_gen(
    int ndef, int *id, struct wpabuf **pubkey, struct wpabuf **privkey, struct wpabuf **dev_pw)
{
    struct wpabuf *pw;
    u16 val;
    ...
    if (random_get_bytes(wpabuf_put(pw, WPS_OOB_DEVICE_PASSWORD_LEN), WPS_OOB_DEVICE_PASSWORD_LEN) ||
...
struct wpabuf *wps_build_nfc_handover_req(struct wps_context *ctx, struct wpabuf *nfc_dh_pubkey)
{
    ...
    wpa_printf(MSG_DEBUG,
               "WPS: Building attributes for NFC connection "
               "handover request");

    if (nfc_dh_pubkey == NULL)
    {
        wpa_printf(MSG_DEBUG,
                   "WPS: No NFC OOB Device Password "
                   "configured");
        return NULL;
    }
    ...
    if (wps_build_oob_dev_pw(msg, DEV_PW_NFC_CONNECTION_HANDOVER, nfc_dh_pubkey, NULL, 0) ||
        wps_build_uuid_e(msg, ctx->uuid) || wps_build_wfa_ext(msg, 0, NULL, 0, 0))
    {
        wpabuf_free(msg);
        return NULL;
    }
                                          ...
                                          int freq)
{
    ...
    if (wps_build_oob_dev_pw(msg, DEV_PW_NFC_CONNECTION_HANDOVER, nfc_dh_pubkey, NULL, 0) || wps_build_ssid(msg, ctx) ||
        wps_build_ap_freq(msg, freq) || (bssid && wps_build_mac_addr(msg, bssid)) ||
        wps_build_wfa_ext(msg, 0, NULL, 0, 0))
    {
        wpabuf_free(msg);
        return NULL;
...
struct wpabuf *wps_build_nfc_handover_req_p2p(struct wps_context *ctx, struct wpabuf *nfc_dh_pubkey)
{
    ...
    if (wps_build_manufacturer(&ctx->dev, msg) || wps_build_model_name(&ctx->dev, msg) ||
        wps_build_model_number(&ctx->dev, msg) ||
        wps_build_oob_dev_pw(msg, DEV_PW_NFC_CONNECTION_HANDOVER, nfc_dh_pubkey, NULL, 0) ||
        wps_build_rf_bands(&ctx->dev, msg, 0) || wps_build_serial_number(&ctx->dev, msg) ||
        wps_build_uuid_e(msg, ctx->uuid) || wps_build_wfa_ext(msg, 0, NULL, 0, 0))
    {
        wpabuf_free(msg);
        return NULL;
                                              ...
                                              struct wpabuf *nfc_dev_pw)
{
    ...
    if (wps_build_manufacturer(&ctx->dev, msg) || wps_build_model_name(&ctx->dev, msg) ||
        wps_build_model_number(&ctx->dev, msg) ||
        wps_build_oob_dev_pw(msg, nfc_dev_pw_id, nfc_dh_pubkey, dev_pw, dev_pw_len) ||
        wps_build_rf_bands(&ctx->dev, msg, 0) || wps_build_serial_number(&ctx->dev, msg) ||
        wps_build_uuid_e(msg, ctx->uuid) || wps_build_wfa_ext(msg, 0, NULL, 0, 0))
    {
        wpabuf_free(msg);
