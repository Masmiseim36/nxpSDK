...
int wps_build_req_dev_type(struct wps_device_data *dev,
                           struct wpabuf *msg,
...
{
    if (wps_build_manufacturer(dev, msg) || wps_build_model_name(dev, msg) || wps_build_model_number(dev, msg) ||
        wps_build_serial_number(dev, msg) || wps_build_primary_dev_type(dev, msg) || wps_build_dev_name(dev, msg))
        return -1;
    return 0;
}
...
int wps_build_vendor_ext_m1(struct wps_device_data *dev, struct wpabuf *msg)
{
    ...
    {
        wpa_hexdump(MSG_DEBUG, "WPS:  * Vendor Extension M1", wpabuf_head_u8(dev->vendor_ext_m1),
...
int wps_build_vendor_ext(struct wps_device_data *dev, struct wpabuf *msg)
{
    ...
    for (i = 0; i < MAX_WPS_VENDOR_EXTENSIONS; i++)
    {
        if (dev->vendor_ext[i] == NULL)
            continue;
        wpa_hexdump(MSG_DEBUG, "WPS:  * Vendor Extension", wpabuf_head_u8(dev->vendor_ext[i]),
...
int wps_build_application_ext(struct wps_device_data *dev, struct wpabuf *msg)
{
    ...
    wpa_hexdump_buf(MSG_DEBUG, "WPS:  * Application Extension", dev->application_ext);
...
{
    if (wps_process_manufacturer(dev, attr->manufacturer, attr->manufacturer_len) ||
        wps_process_model_name(dev, attr->model_name, attr->model_name_len) ||
        wps_process_model_number(dev, attr->model_number, attr->model_number_len) ||
        wps_process_serial_number(dev, attr->serial_number, attr->serial_number_len) ||
