...
struct wps_registrar
{
    ...
    void (*enrollee_seen_cb)(void *ctx,
                             const u8 *addr,
                             const u8 *uuid_e,
...
struct wps_registrar *wps_registrar_init(struct wps_context *wps, const struct wps_registrar_config *cfg)
{
    ...
    {
        os_memcpy(reg->multi_ap_backhaul_ssid, cfg->multi_ap_backhaul_ssid, cfg->multi_ap_backhaul_ssid_len);
    ...
    if (cfg->multi_ap_backhaul_network_key)
    {
        reg->multi_ap_backhaul_network_key =
            os_memdup(cfg->multi_ap_backhaul_network_key, cfg->multi_ap_backhaul_network_key_len);
        if (reg->multi_ap_backhaul_network_key)
            reg->multi_ap_backhaul_network_key_len = cfg->multi_ap_backhaul_network_key_len;
    }
...
void wps_registrar_deinit(struct wps_registrar *reg)
{
    ...
    bin_clear_free(reg->multi_ap_backhaul_network_key, reg->multi_ap_backhaul_network_key_len);
    os_free(reg);
}
 ...
 */
int wps_registrar_add_pin(
    struct wps_registrar *reg, const u8 *addr, const u8 *uuid, const u8 *pin, size_t pin_len, int timeout)
{
                                ...
                                int p2p_wildcard)
{
    ...
    if (reg->enrollee_seen_cb && attr.uuid_e && attr.primary_dev_type && attr.request_type && !p2p_wildcard)
    {
        ...
        reg->enrollee_seen_cb(reg->cb_ctx, addr, attr.uuid_e, attr.primary_dev_type, WPA_GET_BE16(attr.config_methods),
                              WPA_GET_BE16(attr.dev_password_id), *attr.request_type, dev_name);
        os_free(dev_name);
    }
...
static void wps_cb_reg_success(
    struct wps_registrar *reg, const u8 *mac_addr, const u8 *uuid_e, const u8 *dev_pw, size_t dev_pw_len)
{
...
static void wps_cb_set_sel_reg(struct wps_registrar *reg)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               reg->selected_registrar, reg->wps->config_methods, reg->pbc, methods);

    reg->set_sel_reg_cb(reg->cb_ctx, reg->selected_registrar, reg->pbc ? DEV_PW_PUSHBUTTON : DEV_PW_DEFAULT, methods);
}

static int wps_cp_lookup_pskfile(struct wps_registrar *reg, const u8 *mac_addr, const u8 **psk)
{
...
static int wps_set_ie(struct wps_registrar *reg)
{
    ...
    if (wps_build_version(beacon) || wps_build_wps_state(reg->wps, beacon) ||
        wps_build_ap_setup_locked(reg->wps, beacon) || wps_build_selected_registrar(reg, beacon) ||
        wps_build_sel_reg_dev_password_id(reg, beacon) || wps_build_sel_reg_config_methods(reg, beacon) ||
        wps_build_sel_pbc_reg_uuid_e(reg, beacon) || (reg->dualband && wps_build_rf_bands(&reg->wps->dev, beacon, 0)) ||
        wps_build_wfa_ext(beacon, 0, auth_macs, count, 0) || wps_build_vendor_ext(&reg->wps->dev, beacon) ||
    ...
    if (wps_build_version(probe) || wps_build_wps_state(reg->wps, probe) ||
        wps_build_ap_setup_locked(reg->wps, probe) || wps_build_selected_registrar(reg, probe) ||
        wps_build_sel_reg_dev_password_id(reg, probe) || wps_build_sel_reg_config_methods(reg, probe) ||
        wps_build_resp_type(probe, reg->wps->ap ? WPS_RESP_AP : WPS_RESP_REGISTRAR) ||
        wps_build_uuid_e(probe, reg->wps->uuid) || wps_build_device_attrs(&reg->wps->dev, probe) ||
        wps_build_probe_config_methods(reg, probe) || (reg->dualband && wps_build_rf_bands(&reg->wps->dev, probe, 0)) ||
        wps_build_wfa_ext(probe, 0, auth_macs, count, 0) || wps_build_vendor_ext(&reg->wps->dev, probe) ||
...
static int wps_build_credential(struct wpabuf *msg, const struct wps_credential *cred)
{
    if (wps_build_cred_network_idx(msg, cred) || wps_build_cred_ssid(msg, cred) ||
        wps_build_cred_auth_type(msg, cred) || wps_build_cred_encr_type(msg, cred) ||
        wps_build_cred_network_key(msg, cred) || wps_build_mac_addr(msg, cred->mac_addr))
        return -1;
    return 0;
}
...
int wps_build_cred(struct wps_data *wps, struct wpabuf *msg)
{
    ...
    if (wps->peer_dev.multi_ap_ext == MULTI_AP_BACKHAUL_STA && reg->multi_ap_backhaul_ssid_len)
    {
        wpa_printf(MSG_DEBUG, "WPS: Use backhaul STA credentials");
        os_memcpy(wps->cred.ssid, reg->multi_ap_backhaul_ssid, reg->multi_ap_backhaul_ssid_len);
        ...
        if (reg->multi_ap_backhaul_network_key)
        {
            os_memcpy(wps->cred.key, reg->multi_ap_backhaul_network_key, reg->multi_ap_backhaul_network_key_len);
            wps->cred.key_len = reg->multi_ap_backhaul_network_key_len;
        }
        goto use_provided;
    }
    ...
    wpa_printf(MSG_DEBUG, "WPS: Own auth types 0x%x - masked Enrollee auth types 0x%x", wps->wps->auth_types,
               wps->auth_type);
    ...
    wpa_printf(MSG_DEBUG, "WPS: Own encr types 0x%x (rsn: 0x%x, wpa: 0x%x) - masked Enrollee encr types 0x%x",
               wps->wps->encr_types, wps->wps->encr_types_rsn, wps->wps->encr_types_wpa, wps->encr_type);
    ...
    if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->wps->ap && !wps->wps->registrar->disable_auto_conf)
    {
        ...
        os_free(wps->new_psk);
        wps->new_psk = (u8 *)wpa_base64_encode(r, sizeof(r), &wps->new_psk_len);
        if (wps->new_psk == NULL)
            return -1;
    ...
    {
        wpa_hexdump_key(MSG_DEBUG, "WPS: Use PSK from wpa_psk_file", pskfile_psk, PMK_LEN);
    ...
    else if ((!wps->wps->registrar->force_per_enrollee_psk || wps->wps->use_passphrase) && wps->wps->network_key)
    {
        wpa_printf(MSG_DEBUG, "WPS: Use passphrase format for Network key");
...
static struct wpabuf *wps_build_m2(struct wps_data *wps)
{
    ...
    if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M2) || wps_build_enrollee_nonce(wps, msg) ||
        wps_build_registrar_nonce(wps, msg) || wps_build_uuid_r(wps, msg) || wps_build_public_key(wps, msg) ||
        wps_derive_keys(wps) || wps_build_auth_type_flags(wps, msg) || wps_build_encr_type_flags(wps, msg) ||
        wps_build_conn_type_flags(wps, msg) || wps_build_config_methods_r(wps->wps->registrar, msg) ||
        wps_build_device_attrs(&wps->wps->dev, msg) ||
        wps_build_rf_bands(&wps->wps->dev, msg, wps->wps->rf_band_cb(wps->wps->cb_ctx)) ||
        wps_build_assoc_state(wps, msg) || wps_build_config_error(msg, WPS_CFG_NO_ERROR) ||
        wps_build_dev_password_id(msg, wps->dev_pw_id) || wps_build_os_version(&wps->wps->dev, msg) ||
        ...
        wps->nfc_pw_token->pw_id == DEV_PW_NFC_CONNECTION_HANDOVER)
    {
        ...
        if (plain == NULL || wps_build_cred(wps, plain) || wps_build_key_wrap_auth(wps, plain) ||
...
static struct wpabuf *wps_build_m2d(struct wps_data *wps)
{
    ...
    if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M2D) || wps_build_enrollee_nonce(wps, msg) ||
        wps_build_registrar_nonce(wps, msg) || wps_build_uuid_r(wps, msg) || wps_build_auth_type_flags(wps, msg) ||
        wps_build_encr_type_flags(wps, msg) || wps_build_conn_type_flags(wps, msg) ||
        wps_build_config_methods_r(wps->wps->registrar, msg) || wps_build_device_attrs(&wps->wps->dev, msg) ||
        wps_build_rf_bands(&wps->wps->dev, msg, wps->wps->rf_band_cb(wps->wps->cb_ctx)) ||
        wps_build_assoc_state(wps, msg) || wps_build_config_error(msg, err) ||
        wps_build_os_version(&wps->wps->dev, msg) || wps_build_wfa_ext(msg, 0, NULL, 0, 0))
    {
        wpabuf_free(msg);
        return NULL;
    }
...
static struct wpabuf *wps_build_m4(struct wps_data *wps)
{
    ...
    if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M4) || wps_build_enrollee_nonce(wps, msg) ||
        wps_build_r_hash(wps, msg) || wps_build_r_snonce1(wps, plain) || wps_build_key_wrap_auth(wps, plain) ||
        wps_build_encr_settings(wps, msg, plain) || wps_build_wfa_ext(msg, 0, NULL, 0, 0) ||
...
static struct wpabuf *wps_build_m6(struct wps_data *wps)
{
    ...
    if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M6) || wps_build_enrollee_nonce(wps, msg) ||
        wps_build_r_snonce2(wps, plain) || wps_build_key_wrap_auth(wps, plain) ||
        wps_build_encr_settings(wps, msg, plain) || wps_build_wfa_ext(msg, 0, NULL, 0, 0) ||
        wps_build_authenticator(wps, msg))
    {
        wpabuf_clear_free(plain);
...
static struct wpabuf *wps_build_m8(struct wps_data *wps)
{
    ...
    if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M8) || wps_build_enrollee_nonce(wps, msg) ||
        ((wps->wps->ap || wps->er) && wps_build_cred(wps, plain)) ||
        (!wps->wps->ap && !wps->er && wps_build_ap_settings(wps, plain)) || wps_build_key_wrap_auth(wps, plain) ||
        wps_build_encr_settings(wps, msg, plain) || wps_build_wfa_ext(msg, 0, NULL, 0, 0) ||
        wps_build_authenticator(wps, msg))
    {
        wpabuf_clear_free(plain);
...
static int wps_process_auth_type_flags(struct wps_data *wps, const u8 *auth)
{
     ...
     */
    if ((auth_types & (WPS_AUTH_WPA2 | WPS_AUTH_WPA2PSK | WPS_AUTH_WPAPSK)) == (WPS_AUTH_WPA2 | WPS_AUTH_WPAPSK))
    {
        wpa_printf(MSG_DEBUG, "WPS: Workaround - assume Enrollee supports WPA2PSK based on claimed WPA2 support");
...
static int wps_process_config_methods(struct wps_data *wps, const u8 *methods)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "%s%s%s%s%s%s%s%s%s",
               m, m & WPS_CONFIG_USBA ? " [USBA]" : "", m & WPS_CONFIG_ETHERNET ? " [Ethernet]" : "",
               m & WPS_CONFIG_LABEL ? " [Label]" : "", m & WPS_CONFIG_DISPLAY ? " [Display]" : "",
               m & WPS_CONFIG_EXT_NFC_TOKEN ? " [Ext NFC Token]" : "",
               m & WPS_CONFIG_INT_NFC_TOKEN ? " [Int NFC Token]" : "", m & WPS_CONFIG_NFC_INTERFACE ? " [NFC]" : "",
               m & WPS_CONFIG_PUSHBUTTON ? " [PBC]" : "", m & WPS_CONFIG_KEYPAD ? " [Keypad]" : "");
...
static int wps_registrar_p2p_dev_addr_match(struct wps_data *wps)
{
    ...
    if (os_memcmp(reg->p2p_dev_addr, wps->p2p_dev_addr, ETH_ALEN) != 0)
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   "filtering for PBC: expected " MACSTR " was " MACSTR " - indicate PBC session overlap",
...
static enum wps_process_res wps_process_m1(struct wps_data *wps, struct wps_parse_attr *attr)
{
    ...
    if (wps_process_uuid_e(wps, attr->uuid_e) || wps_process_mac_addr(wps, attr->mac_addr) ||
        ...
        wps_process_config_methods(wps, attr->config_methods) || wps_process_wps_state(wps, attr->wps_state) ||
        wps_process_device_attrs(&wps->peer_dev, attr) || wps_process_rf_bands(&wps->peer_dev, attr->rf_bands) ||
        wps_process_assoc_state(wps, attr->assoc_state) || wps_process_dev_password_id(wps, attr->dev_password_id) ||
        wps_process_config_error(wps, attr->config_error) || wps_process_os_version(&wps->peer_dev, attr->os_version))
        return WPS_FAILURE;

    if (wps->dev_pw_id < 0x10 && wps->dev_pw_id != DEV_PW_DEFAULT && wps->dev_pw_id != DEV_PW_P2PS_DEFAULT &&
        wps->dev_pw_id != DEV_PW_USER_SPECIFIED && wps->dev_pw_id != DEV_PW_MACHINE_SPECIFIED &&
    ...
    if (wps->dev_pw_id >= 0x10 || wps->dev_pw_id == DEV_PW_NFC_CONNECTION_HANDOVER)
    {
        ...
        wpa_printf(MSG_DEBUG, "WPS: Searching for NFC token match for id=%d (ctx %p registrar %p)", wps->dev_pw_id,
                   wps->wps, wps->wps->registrar);
     ...
     * used.
     */
    if (!wps->use_psk_key && wps->peer_dev.manufacturer && os_strncmp(wps->peer_dev.manufacturer, "Apple ", 6) == 0 &&
        wps->peer_dev.model_name && os_strcmp(wps->peer_dev.model_name, "AirPort") == 0)
    {
        wpa_printf(MSG_DEBUG,
...
static enum wps_process_res wps_process_m3(struct wps_data *wps, const struct wpabuf *msg, struct wps_parse_attr *attr)
{
    ...
    if (wps_process_registrar_nonce(wps, attr->registrar_nonce) ||
        wps_process_authenticator(wps, attr->authenticator, msg) || wps_process_e_hash1(wps, attr->e_hash1) ||
...
static enum wps_process_res wps_process_m5(struct wps_data *wps, const struct wpabuf *msg, struct wps_parse_attr *attr)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "attribute");
    if (wps_parse_msg(decrypted, &eattr) < 0 || wps_process_key_wrap_auth(wps, decrypted, eattr.key_wrap_auth) ||
...
static enum wps_process_res wps_process_m7(struct wps_data *wps, const struct wpabuf *msg, struct wps_parse_attr *attr)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "attribute");
    if (wps_parse_msg(decrypted, &eattr) < 0 || wps_process_key_wrap_auth(wps, decrypted, eattr.key_wrap_auth) ||
        wps_process_e_snonce2(wps, eattr.e_snonce2) || wps_process_ap_settings_r(wps, &eattr))
    {
        wpabuf_clear_free(decrypted);
        wps->state = SEND_WSC_NACK;
        return WPS_CONTINUE;
...
static enum wps_process_res wps_process_wsc_msg(struct wps_data *wps, const struct wpabuf *msg)
{
    ...
    if (*attr.msg_type != WPS_M1 &&
        (attr.registrar_nonce == NULL || os_memcmp(wps->nonce_r, attr.registrar_nonce, WPS_NONCE_LEN) != 0))
    {
...
static enum wps_process_res wps_process_wsc_done(struct wps_data *wps, const struct wpabuf *msg)
{
    ...
    if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->new_psk && wps->wps->ap &&
...
static void wps_registrar_sel_reg_add(struct wps_registrar *reg, struct subscription *s)
{
    ...
    wpa_hexdump(MSG_DEBUG, "WPS: Authorized MACs union", (u8 *)reg->authorized_macs_union,
...
static void wps_registrar_sel_reg_union(struct wps_registrar *reg)
{
    ...
    dl_list_for_each(s, &reg->wps->wps_upnp->subscriptions, struct subscription, list)
    {
        ...
        if (sa)
        {
            wpa_printf(MSG_DEBUG, "WPS: External Registrar %s:%d", inet_ntoa(sa->saddr.sin_addr),
...
void wps_registrar_selected_registrar_changed(struct wps_registrar *reg, u16 dev_pw_id)
{
    ...
    wpa_hexdump(MSG_DEBUG, "WPS: Authorized MACs union (start with own)", (u8 *)reg->authorized_macs_union,
                sizeof(reg->authorized_macs_union));
    if (reg->selected_registrar)
    {
        u16 methods;
...
int wps_registrar_get_info(struct wps_registrar *reg, const u8 *addr, char *buf, size_t buflen)
{
    ...
    ret = os_snprintf(buf + len, buflen - len,
                      ...
                      d->dev.device_name ? d->dev.device_name : "", d->dev.manufacturer ? d->dev.manufacturer : "",
                      d->dev.model_name ? d->dev.model_name : "", d->dev.model_number ? d->dev.model_number : "",
                                  ...
                                  size_t multi_ap_backhaul_network_key_len)
{
    ...
    {
        os_memcpy(reg->multi_ap_backhaul_ssid, multi_ap_backhaul_ssid, multi_ap_backhaul_ssid_len);
    ...
    {
        reg->multi_ap_backhaul_network_key =
            os_memdup(multi_ap_backhaul_network_key, multi_ap_backhaul_network_key_len);
        if (!reg->multi_ap_backhaul_network_key)
            return -1;
        reg->multi_ap_backhaul_network_key_len = multi_ap_backhaul_network_key_len;
    }
                                   ...
                                   int pk_hash_provided_oob)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "WPS: Unexpected NFC Password Token "
...
int wps_registrar_add_nfc_password_token(struct wps_registrar *reg, const u8 *oob_dev_pw, size_t oob_dev_pw_len)
{
    ...
    if (oob_dev_pw_len < WPS_OOB_PUBKEY_HASH_LEN + 2 ||
        oob_dev_pw_len > WPS_OOB_PUBKEY_HASH_LEN + 2 + WPS_OOB_DEVICE_PASSWORD_LEN)
        return -1;
