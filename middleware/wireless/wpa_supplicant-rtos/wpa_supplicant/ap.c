...
#ifdef CONFIG_P2P
static bool is_chanwidth160_supported(struct hostapd_hw_modes *mode, struct hostapd_config *conf)
{
    ...
    if (conf->ieee80211ax)
    {
        ...
        if (he_cap->phy_cap[HE_PHYCAP_CHANNEL_WIDTH_SET_IDX] &
            (HE_PHYCAP_CHANNEL_WIDTH_SET_80PLUS80MHZ_IN_5G | HE_PHYCAP_CHANNEL_WIDTH_SET_160MHZ_IN_5G))
            return true;
    }
#endif /* CONFIG_IEEE80211AX */
    if (mode->vht_capab & (VHT_CAP_SUPP_CHAN_WIDTH_160MHZ | VHT_CAP_SUPP_CHAN_WIDTH_160_80PLUS80MHZ))
        return true;
    return false;
}
#endif /* CONFIG_P2P */
                             ...
                             struct hostapd_hw_modes *mode)
{
    ...
    if (hostapd_get_oper_chwidth(conf))
        ieee80211_freq_to_channel_ext(ssid->frequency, 0, hostapd_get_oper_chwidth(conf), &conf->op_class,
                                      &conf->channel);

    if (hostapd_get_oper_chwidth(conf) == CHANWIDTH_80P80MHZ)
    {
        ieee80211_freq_to_chan(ssid->vht_center_freq2, &freq_seg_idx);
    ...
    if (!ssid->p2p_group)
    {
        if (!ssid->vht_center_freq1)
            goto no_vht;
        ieee80211_freq_to_chan(ssid->vht_center_freq1, &freq_seg_idx);
        hostapd_set_oper_centr_freq_seg0_idx(conf, freq_seg_idx);

        wpa_printf(MSG_DEBUG, "VHT seg0 index %d and seg1 index %d for AP", hostapd_get_oper_centr_freq_seg0_idx(conf),
    ...
    switch (hostapd_get_oper_chwidth(conf))
    {
        ...
        case CHANWIDTH_80P80MHZ:
            center_chan = wpas_p2p_get_vht80_center(wpa_s, mode, channel, conf->op_class);
            wpa_printf(MSG_DEBUG, "VHT center channel %u for 80 or 80+80 MHz bandwidth", center_chan);
            break;
        case CHANWIDTH_160MHZ:
            center_chan = wpas_p2p_get_vht160_center(wpa_s, mode, channel, conf->op_class);
            wpa_printf(MSG_DEBUG, "VHT center channel %u for 160 MHz bandwidth", center_chan);
            break;
        default:
            /*
            ...
            ieee80211_freq_to_channel_ext(ssid->frequency, 0, conf->vht_oper_chwidth, &conf->op_class, &conf->channel);
            center_chan = wpas_p2p_get_vht160_center(wpa_s, mode, channel, conf->op_class);
            if (center_chan && is_chanwidth160_supported(mode, conf))
            {
                wpa_printf(MSG_DEBUG, "VHT center channel %u for auto-selected 160 MHz bandwidth", center_chan);
            }
            else
            {
                hostapd_set_oper_chwidth(conf, CHANWIDTH_80MHZ);
                ieee80211_freq_to_channel_ext(ssid->frequency, 0, conf->vht_oper_chwidth, &conf->op_class,
                                              &conf->channel);
                center_chan = wpas_p2p_get_vht80_center(wpa_s, mode, channel, conf->op_class);
                wpa_printf(MSG_DEBUG, "VHT center channel %u for auto-selected 80 MHz bandwidth", center_chan);
            }
            break;
    }
    if (!center_chan)
    ...
    wpa_printf(MSG_DEBUG, "VHT seg0 index %d for P2P GO", hostapd_get_oper_centr_freq_seg0_idx(conf));
    return;
#endif /* CONFIG_P2P */

no_vht:
    wpa_printf(MSG_DEBUG, "No VHT higher bandwidth support for the selected channel %d", conf->channel);
    hostapd_set_oper_centr_freq_seg0_idx(conf, conf->channel + conf->secondary_channel * 2);
    hostapd_set_oper_chwidth(conf, CHANWIDTH_USE_HT);
}

static struct hostapd_hw_modes *wpa_supplicant_find_hw_mode(struct wpa_supplicant *wpa_s, enum hostapd_hw_mode hw_mode)
{
                                 ...
                                 struct hostapd_config *conf)
{
    ...
    he_chanwidth = mode->he_capab[wpas_mode_to_ieee80211_mode(ssid->mode)].phy_cap[HE_PHYCAP_CHANNEL_WIDTH_SET_IDX];
    is_chanwidth_40_80 = he_chanwidth & HE_PHYCAP_CHANNEL_WIDTH_SET_40MHZ_80MHZ_IN_5G;
    is_chanwidth_160   = he_chanwidth & HE_PHYCAP_CHANNEL_WIDTH_SET_160MHZ_IN_5G;

    wpa_printf(MSG_DEBUG, "Enable HE support (p2p_group=%d he_chwidth_cap=%d)", ssid->p2p_group, he_chanwidth);

    if (mode->he_capab[wpas_mode_to_ieee80211_mode(ssid->mode)].he_supported && ssid->he)
        conf->ieee80211ax = 1;

    if (is_chanwidth_40_80 && ssid->p2p_group && get_max_oper_chwidth_6ghz(ssid->max_oper_chwidth) >= 40)
    {
        conf->secondary_channel = wpas_p2p_get_sec_channel_offset_40mhz(wpa_s, mode, conf->channel);
        wpa_printf(MSG_DEBUG, "Secondary channel offset %d for P2P group", conf->secondary_channel);
...
int wpa_supplicant_conf_ap_ht(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid, struct hostapd_config *conf)
{
    conf->hw_mode =
        ieee80211_freq_to_channel_ext(ssid->frequency, 0, CHANWIDTH_USE_HT, &conf->op_class, &conf->channel);
    ...
    if (wpa_s->hw.modes)
    {
        ...
        wpa_printf(MSG_DEBUG, "Determining HT/VHT options based on driver capabilities (freq=%u chan=%u)",
                   ssid->frequency, conf->channel);

        mode = get_mode(wpa_s->hw.modes, wpa_s->hw.num_modes, conf->hw_mode, is_6ghz_freq(ssid->frequency));
        ...
        if (!mode && conf->hw_mode == HOSTAPD_MODE_IEEE80211G)
        {
            ...
            wpa_printf(MSG_INFO, "Try downgrade to IEEE 802.11b as 802.11g is not supported by the current hardware");
            mode = wpa_supplicant_find_hw_mode(wpa_s, conf->hw_mode);
        }

        if (!mode)
        {
            wpa_printf(MSG_ERROR, "No match between requested and supported hw modes found");
            return -1;
        }
        ...
        if (!ssid->ht)
        {
            wpa_printf(MSG_DEBUG, "HT not enabled in network profile");
            conf->ieee80211n = 0;
            conf->ht_capab   = 0;
            no_ht            = 1;
        }
        ...
        {
            wpa_printf(MSG_DEBUG, "Enable HT support (p2p_group=%d 11a=%d ht40_hw_capab=%d ssid->ht40=%d)",
                       ssid->p2p_group, conf->hw_mode == HOSTAPD_MODE_IEEE80211A,
                       !!(mode->ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET), ssid->ht40);
            conf->ieee80211n = 1;

            if (ssid->ht40 && (mode->ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET))
            ...
            {
                conf->secondary_channel = wpas_p2p_get_sec_channel_offset_40mhz(wpa_s, mode, conf->channel);
                wpa_printf(MSG_DEBUG, "HT secondary channel offset %d for P2P group", conf->secondary_channel);
            }
            else if (ssid->p2p_group && conf->secondary_channel && conf->hw_mode != HOSTAPD_MODE_IEEE80211A)
            {
                ...
                wpa_printf(MSG_DEBUG, "Disable HT secondary channel for P2P group on 2.4 GHz");
                conf->secondary_channel = 0;
            }
#endif /* CONFIG_P2P */

            if (!ssid->p2p_group && (mode->ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET))
            {
                conf->secondary_channel = ssid->ht40;
                wpa_printf(MSG_DEBUG, "HT secondary channel offset %d for AP", conf->secondary_channel);
            }
             ...
             */
            conf->ht_capab |=
                mode->ht_capab & (HT_CAP_INFO_GREEN_FIELD | HT_CAP_INFO_SHORT_GI20MHZ | HT_CAP_INFO_SHORT_GI40MHZ |
                                  HT_CAP_INFO_RX_STBC_MASK | HT_CAP_INFO_TX_STBC | HT_CAP_INFO_MAX_AMSDU_SIZE);
    ...
    if (conf->secondary_channel)
    {
        ...
        {
            if (iface == wpa_s || iface->wpa_state < WPA_AUTHENTICATING || (int)iface->assoc_freq != ssid->frequency)
                continue;
...
static int wpa_supplicant_conf_ap(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid, struct hostapd_config *conf)
{
    ...
    if (conf->hw_mode == HOSTAPD_MODE_IEEE80211G &&
        (ssid->mode == WPAS_MODE_P2P_GO || ssid->mode == WPAS_MODE_P2P_GROUP_FORMATION))
    {
    ...
    else if (ssid->wep_key_len[0] || ssid->wep_key_len[1] || ssid->wep_key_len[2] || ssid->wep_key_len[3])
    {
        ...
        for (i = 0; i < NUM_WEP_KEYS; i++)
        {
            if (ssid->wep_key_len[i] == 0)
                continue;
            wep->key[i] = os_memdup(ssid->wep_key[i], ssid->wep_key_len[i]);
            if (wep->key[i] == NULL)
                return -1;
    ...
    {
        wpa_printf(MSG_DEBUG, "P2P: Enable Interworking with access_network_type: %d",
        ...
        {
            wpa_printf(MSG_DEBUG, "P2P: Venue group: %d  Venue type: %d", wpa_s->conf->go_venue_group,
    ...
    if (ssid->mode == WPAS_MODE_P2P_GO || ssid->mode == WPAS_MODE_P2P_GROUP_FORMATION)
    {
        ...
        {
            wpa_printf(MSG_INFO, "CTWindow (%d) is bigger than beacon interval (%d) - avoid configuring it",
    ...
    if (bss->wpa_group_rekey < 86400 && (bss->wpa & 2) &&
        (bss->wpa_group == WPA_CIPHER_CCMP || bss->wpa_group == WPA_CIPHER_GCMP ||
         bss->wpa_group == WPA_CIPHER_CCMP_256 || bss->wpa_group == WPA_CIPHER_GCMP_256))
    {
        /*
    ...
    if (bss->ssid.security_policy == SECURITY_WPA_PSK &&
        (!(bss->rsn_pairwise & (WPA_CIPHER_CCMP | WPA_CIPHER_GCMP)) || !(bss->wpa & 2)))
...
no_wps:
    ...
    {
        bss->assocresp_elements = wpabuf_dup(wpa_s->conf->ap_assocresp_elements);
    }
...
static void ap_public_action_rx(void *ctx, const u8 *buf, size_t len, int freq)
{
    ...
    if (mgmt->u.action.category != WLAN_ACTION_PUBLIC)
        return;
    wpas_p2p_rx_action(wpa_s, mgmt->da, mgmt->sa, mgmt->bssid, mgmt->u.action.category, buf + IEEE80211_HDRLEN + 1,
...
static void ap_wps_event_cb(void *ctx, enum wps_event event, union wps_event_data *data)
{
    ...
    if (event == WPS_EV_FAIL)
    {
        ...
        if (wpa_s->p2pdev && wpa_s->p2pdev != wpa_s && wpa_s == wpa_s->global->p2p_group_formation)
        {
            /*
             ...
             * here if needed.
             */
            wpa_msg(wpa_s->p2pdev, MSG_INFO, WPS_EVENT_FAIL "msg=%d config_error=%d", fail->msg, fail->config_error);
        }
...
static int ap_probe_req_rx(
    void *ctx, const u8 *sa, const u8 *da, const u8 *bssid, const u8 *ie, size_t ie_len, int ssi_signal)
{
    struct wpa_supplicant *wpa_s = ctx;
    unsigned int freq            = 0;
...
static void wpas_ap_configured_cb(void *ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "AP interface setup completed - state %s", hostapd_state_text(wpa_s->ap_iface->state));
...
#endif /* CONFIG_ACS */
        if (acs || (wpa_s->assoc_freq && wpa_s->ap_iface->freq && (int)wpa_s->assoc_freq != wpa_s->ap_iface->freq))
        {
...
int wpa_supplicant_create_ap(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
    ...
    if ((ssid->mode == WPAS_MODE_AP || ssid->mode == WPAS_MODE_P2P_GO) && ssid->enable_edmg)
    {
        u8 primary_channel;

        if (ieee80211_freq_to_chan(ssid->frequency, &primary_channel) == NUM_HOSTAPD_MODES)
        {
            wpa_printf(MSG_WARNING, "EDMG: Failed to get the primary channel");
            return -1;
        }

        hostapd_encode_edmg_chan(ssid->enable_edmg, ssid->edmg_channel, primary_channel, &params.freq.edmg);
    }
    ...
    if (ieee80211_is_dfs(params.freq.freq, wpa_s->hw.modes, wpa_s->hw.num_modes))
    ...
    os_memcpy(wpa_s->ap_iface->conf->tx_queue, wpa_s->conf->tx_queue, sizeof(wpa_s->conf->tx_queue));
...
int wpa_supplicant_ap_wps_pin(
    struct wpa_supplicant *wpa_s, const u8 *bssid, const char *pin, char *buf, size_t buflen, int timeout)
{
    int ret, ret_len = 0;
...
int ap_ctrl_iface_wpa_get_status(struct wpa_supplicant *wpa_s, char *buf, size_t buflen, int verbose)
{
    ...
    ret = os_snprintf(pos, end - pos,
                      ...
                      "group_cipher=%s\n"
                      "key_mgmt=%s\n",
                      wpa_cipher_txt(conf->rsn_pairwise), wpa_cipher_txt(conf->wpa_group),
...
void wpas_ap_ch_switch(
    struct wpa_supplicant *wpa_s, int freq, int ht, int offset, int width, int cf1, int cf2, int finished)
{
    struct hostapd_iface *iface = wpa_s->ap_iface;
    ...
    hostapd_event_ch_switch(iface->bss[0], freq, ht, offset, width, cf1, cf2, finished);
}
...
int wpas_ap_wps_add_nfc_pw(struct wpa_supplicant *wpa_s, u16 pw_id, const struct wpabuf *pw, const u8 *pubkey_hash)
{
    ...
    if (wpa_s->p2pdev->conf->wps_nfc_dh_pubkey == NULL || wpa_s->p2pdev->conf->wps_nfc_dh_privkey == NULL)
    {
    ...
    return wps_registrar_add_nfc_pw_token(hapd->wps->registrar, pubkey_hash, pw_id, pw ? wpabuf_head(pw) : NULL,
...
int wpas_ap_pmksa_cache_list(struct wpa_supplicant *wpa_s, char *buf, size_t len)
{
    ...
    if (wpa_s->ap_iface)
    {
        ...
        {
            reply_len += hostapd_ctrl_iface_pmksa_list(wpa_s->ap_iface->bss[i], &buf[reply_len], len - reply_len);
        }
    }
    ...
    if (wpa_s->ifmsh)
    {
        ...
        reply_len += hostapd_ctrl_iface_pmksa_list(wpa_s->ifmsh->bss[0], &buf[reply_len], len - reply_len);
    }
...
int wpas_ap_pmksa_cache_list_mesh(struct wpa_supplicant *wpa_s, const u8 *addr, char *buf, size_t len)
{
    return hostapd_ctrl_iface_pmksa_list_mesh(wpa_s->ifmsh->bss[0], addr, &buf[0], len);
}
...
int wpas_ap_pmksa_cache_add_external(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    pmksa_cache = hostapd_ctrl_iface_pmksa_create_entry(wpa_s->own_addr, cmd);
    if (!pmksa_cache)
        return -1;
...
int wpas_ap_update_beacon(struct wpa_supplicant *wpa_s)
{
    ...
    if (wpa_s->conf->ap_assocresp_elements)
    {
        hapd->conf->assocresp_elements = wpabuf_dup(wpa_s->conf->ap_assocresp_elements);
    }
    ...
    if (wpa_s->conf->ap_vendor_elements)
    {
        hapd->conf->vendor_elements = wpabuf_dup(wpa_s->conf->ap_vendor_elements);
    }
...
void wpas_ap_event_dfs_radar_detected(struct wpa_supplicant *wpa_s, struct dfs_event *radar)
{
    ...
    hostapd_dfs_radar_detected(iface, radar->freq, radar->ht_enabled, radar->chan_offset, radar->chan_width, radar->cf1,
                               radar->cf2);
}
...
void wpas_ap_event_dfs_cac_started(struct wpa_supplicant *wpa_s, struct dfs_event *radar)
{
    ...
    hostapd_dfs_start_cac(iface, radar->freq, radar->ht_enabled, radar->chan_offset, radar->chan_width, radar->cf1,
                          radar->cf2);
}
...
void wpas_ap_event_dfs_cac_finished(struct wpa_supplicant *wpa_s, struct dfs_event *radar)
{
    ...
    hostapd_dfs_complete_cac(iface, 1, radar->freq, radar->ht_enabled, radar->chan_offset, radar->chan_width,
                             radar->cf1, radar->cf2);
}
...
void wpas_ap_event_dfs_cac_aborted(struct wpa_supplicant *wpa_s, struct dfs_event *radar)
{
    ...
    hostapd_dfs_complete_cac(iface, 0, radar->freq, radar->ht_enabled, radar->chan_offset, radar->chan_width,
                             radar->cf1, radar->cf2);
}

void wpas_ap_event_dfs_cac_nop_finished(struct wpa_supplicant *wpa_s, struct dfs_event *radar)
{
    struct hostapd_iface *iface = wpa_s->ap_iface;
    ...
    hostapd_dfs_nop_finished(iface, radar->freq, radar->ht_enabled, radar->chan_offset, radar->chan_width, radar->cf1,
                             radar->cf2);
}
#endif /* NEED_AP_MLME */
