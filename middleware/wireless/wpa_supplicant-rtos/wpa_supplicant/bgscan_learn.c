...
#define MAX_BSS 50
    ...
    for (i = 0; i < scan_res->num; i++)
    {
        ...
        {
            wpa_printf(MSG_DEBUG, "bgscan learn: Update BSS " MACSTR " freq %d -> %d", MAC2STR(res->bssid), bss->freq,
                       res->freq);
            bss->freq = res->freq;
        }
        else if (!bss)
...
static void bgscan_learn_notify_signal_change(
    void *priv, int above, int current_signal, int current_noise, int current_txrate)
{
    struct bgscan_learn_data *data = priv;
    int scan                       = 0;
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "bgscan learn: Start using short bgscan "
                   "interval");
        data->scan_interval = data->short_interval;
        os_get_reltime(&now);
