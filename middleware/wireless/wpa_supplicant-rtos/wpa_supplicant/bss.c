...
static void wpa_bss_anqp_free(struct wpa_bss_anqp *anqp)
{
    ...
    while ((elem = dl_list_first(&anqp->anqp_elems, struct wpa_bss_anqp_elem, list)))
    {
...
void wpa_bss_remove(struct wpa_supplicant *wpa_s, struct wpa_bss *bss, const char *reason)
{
        ...
        for (i = 0; i < wpa_s->last_scan_res_used; i++)
        {
            if (wpa_s->last_scan_res[i] == bss)
            {
                os_memmove(&wpa_s->last_scan_res[i], &wpa_s->last_scan_res[i + 1],
                           (wpa_s->last_scan_res_used - i - 1) * sizeof(struct wpa_bss *));
    ...
    wpa_s->num_bss--;
    wpa_dbg(wpa_s, MSG_DEBUG, "BSS: Remove id %u BSSID " MACSTR " SSID '%s' due to %s", bss->id, MAC2STR(bss->bssid),
...
struct wpa_bss *wpa_bss_get(struct wpa_supplicant *wpa_s, const u8 *bssid, const u8 *ssid, size_t ssid_len)
{
    ...
    {
        if (os_memcmp(bss->bssid, bssid, ETH_ALEN) == 0 && bss->ssid_len == ssid_len &&
...
static int wpa_bss_is_wps_candidate(struct wpa_supplicant *wpa_s, struct wpa_bss *bss)
{
#ifdef CONFIG_WPS
    ...
    for (ssid = wpa_s->conf->ssid; ssid; ssid = ssid->next)
    {
        if (!(ssid->key_mgmt & WPA_KEY_MGMT_WPS))
            continue;
        if (ssid->ssid_len &&
            (ssid->ssid_len != bss->ssid_len || os_memcmp(ssid->ssid, bss->ssid, ssid->ssid_len) != 0))
            continue;
...
static bool is_p2p_pending_bss(struct wpa_supplicant *wpa_s, struct wpa_bss *bss)
{
#ifdef CONFIG_P2P
    u8 addr[ETH_ALEN];

    if (os_memcmp(bss->bssid, wpa_s->pending_join_iface_addr, ETH_ALEN) == 0)
        return true;
...
static int wpa_bss_in_use(struct wpa_supplicant *wpa_s, struct wpa_bss *bss)
{
    ...
    if (wpa_s->current_bss && (bss->ssid_len != wpa_s->current_bss->ssid_len ||
                               os_memcmp(bss->ssid, wpa_s->current_bss->ssid, bss->ssid_len) != 0))
        return 0; /* SSID has changed */

    return !is_zero_ether_addr(bss->bssid) && (os_memcmp(bss->bssid, wpa_s->bssid, ETH_ALEN) == 0 ||
                                   ...
                                   struct os_reltime *fetch_time)
{
    ...
    if (!is_zero_ether_addr(bss->hessid))
        os_snprintf(extra, sizeof(extra), " HESSID " MACSTR, MAC2STR(bss->hessid));
    else
        extra[0] = '\0';
    wpa_dbg(wpa_s, MSG_DEBUG, "BSS: Add new id %u BSSID " MACSTR " SSID '%s' freq %d%s", bss->id, MAC2STR(bss->bssid),
            wpa_ssid_txt(ssid, ssid_len), bss->freq, extra);
...
static u32 wpa_bss_compare_res(const struct wpa_bss *old, const struct wpa_scan_res *new_res)
{
    ...
    if (old->ie_len == new_res->ie_len && os_memcmp(wpa_bss_ie_ptr(old), new_res + 1, old->ie_len) == 0)
        return changes;
...
void notify_bss_changes(struct wpa_supplicant *wpa_s, u32 changes, const struct wpa_bss *bss)
{
                                      ...
                                      struct os_reltime *fetch_time)
{
    ...
    if (bss->last_update_idx == wpa_s->bss_update_idx)
    {
         ...
         */
        wpa_printf(MSG_DEBUG, "BSS: " MACSTR " has multiple entries in the scan results - select the most current one",
                   MAC2STR(bss->bssid));
        calculate_update_time(fetch_time, res->age, &update_time);
        wpa_printf(MSG_DEBUG, "Previous last_update: %u.%06u (freq %d%s)", (unsigned int)bss->last_update.sec,
                   (unsigned int)bss->last_update.usec, bss->freq, (bss->flags & WPA_BSS_ASSOCIATED) ? " assoc" : "");
        wpa_printf(MSG_DEBUG, "New last_update: %u.%06u (freq %d%s)", (unsigned int)update_time.sec,
                   (unsigned int)update_time.usec, res->freq, (res->flags & WPA_SCAN_ASSOCIATED) ? " assoc" : "");
        if ((bss->flags & WPA_BSS_ASSOCIATED) ||
            (!(res->flags & WPA_SCAN_ASSOCIATED) && !os_reltime_before(&bss->last_update, &update_time)))
        {
            wpa_printf(MSG_DEBUG, "Ignore this BSS entry since the previous update looks more current");
            return bss;
        }
        wpa_printf(MSG_DEBUG, "Accept this BSS entry since it looks more current than the previous update");
    }

    changes = wpa_bss_compare_res(bss, res);
    if (changes & WPA_BSS_FREQ_CHANGED_FLAG)
        wpa_printf(MSG_DEBUG, "BSS: " MACSTR " changed freq %d --> %d", MAC2STR(bss->bssid), bss->freq, res->freq);
    ...
    if (wpa_bss_get_vendor_ie(bss, P2P_IE_VENDOR_TYPE) && !wpa_scan_get_vendor_ie(res, P2P_IE_VENDOR_TYPE))
    {
        ...
        wpa_dbg(wpa_s, MSG_DEBUG,
                "BSS: Do not update scan IEs for " MACSTR " since that would remove P2P IE information",
                MAC2STR(bss->bssid));
    }
    else
#endif /* CONFIG_P2P */
    ...
    if (info->num_ssids)
    {
        ...
        for (i = 0; i < info->num_ssids; i++)
        {
            ...
            if ((s->ssid == NULL || s->ssid_len == 0) ||
                (s->ssid_len == bss->ssid_len && os_memcmp(s->ssid, bss->ssid, bss->ssid_len) == 0))
            {
                found = 1;
                break;
            }
...
struct wpa_bss *wpa_bss_get_p2p_dev_addr(struct wpa_supplicant *wpa_s, const u8 *dev_addr)
{
    ...
    {
        u8 addr[ETH_ALEN];
        if (p2p_parse_dev_addr(wpa_bss_ie_ptr(bss), bss->ie_len, addr) != 0 || os_memcmp(addr, dev_addr, ETH_ALEN) != 0)
            continue;
        if (!found || os_reltime_before(&found->last_update, &bss->last_update))
            found = bss;
    }
    return found;
}
