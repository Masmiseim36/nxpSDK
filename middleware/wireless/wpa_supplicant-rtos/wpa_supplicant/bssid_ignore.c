 ...
 */
struct wpa_bssid_ignore *wpa_bssid_ignore_get(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
    ...
    if (wpa_s->current_ssid && wpa_s->current_ssid->was_recently_reconfigured)
    {
...
int wpa_bssid_ignore_add(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
    ...
    if (e)
    {
        ...
        if (e->count > 5)
            ...
            e->timeout_secs = 10;
        wpa_printf(MSG_INFO, "BSSID " MACSTR " ignore list count incremented to %d, ignoring for %d seconds",
    ...
    wpa_printf(MSG_DEBUG, "Added BSSID " MACSTR " into ignore list, ignoring for %d seconds", MAC2STR(bssid),
               e->timeout_secs);
...
int wpa_bssid_ignore_del(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
    ...
    while (e)
    {
        if (os_memcmp(e->bssid, bssid, ETH_ALEN) == 0)
        {
            ...
            wpa_printf(MSG_DEBUG, "Removed BSSID " MACSTR " from ignore list", MAC2STR(bssid));
            os_free(e);
            return 0;
        }
        prev = e;
...
void wpa_bssid_ignore_clear(struct wpa_supplicant *wpa_s)
{
    ...
    while (e)
    {
        prev = e;
        e    = e->next;
        wpa_printf(MSG_DEBUG, "Removed BSSID " MACSTR " from ignore list (clear)", MAC2STR(prev->bssid));
        os_free(prev);
    }
}
...
void wpa_bssid_ignore_update(struct wpa_supplicant *wpa_s)
{
    ...
    while (e)
    {
        if (os_reltime_expired(&now, &e->start, e->timeout_secs + 3600))
        {
            ...
            wpa_printf(MSG_INFO, "Removed BSSID " MACSTR " from ignore list (expired)", MAC2STR(to_delete->bssid));
            os_free(to_delete);
        }
        else
        {
