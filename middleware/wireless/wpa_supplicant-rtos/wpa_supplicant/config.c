...
set:
    ...
    if (data->param2)
        ...
        prev_len = 0;
    if ((*dst == NULL && tmp == NULL) || (*dst && tmp && prev_len == res_len && os_memcmp(*dst, tmp, res_len) == 0))
    {
                                       ...
                                       const char *value)
{
    ...
    {
        wpa_printf(MSG_ERROR, "Line %d: Invalid BSSID hint '%s'.", line, value);
        return -1;
    }
...
#ifndef NO_CONFIG_WRITE
static char *wpa_config_write_bssid_hint(const struct parse_data *data, struct wpa_ssid *ssid)
{
    char *value;
    int res;
...
static int wpa_config_parse_bssid_ignore(const struct parse_data *data,
                                         ...
                                         const char *value)
{
    return wpa_config_parse_addr_list(data, line, value, &ssid->bssid_ignore, &ssid->num_bssid_ignore, "bssid_ignore",
                                      1, 1);
}
...
static char *wpa_config_write_bssid_ignore(const struct parse_data *data, struct wpa_ssid *ssid)
{
    return wpa_config_write_addr_list(data, ssid->bssid_ignore, ssid->num_bssid_ignore, "bssid_ignore");
}
...
static int wpa_config_parse_bssid_accept(const struct parse_data *data,
                                         ...
                                         const char *value)
{
    return wpa_config_parse_addr_list(data, line, value, &ssid->bssid_accept, &ssid->num_bssid_accept, "bssid_accept",
                                      1, 1);
}
...
static char *wpa_config_write_bssid_accept(const struct parse_data *data, struct wpa_ssid *ssid)
{
    return wpa_config_write_addr_list(data, ssid->bssid_accept, ssid->num_bssid_accept, "bssid_accept");
}
...
static char *wpa_config_write_key_mgmt(const struct parse_data *data, struct wpa_ssid *ssid)
{
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sFT-EAP-SHA384", pos == buf ? "" : " ");
        if (os_snprintf_error(end - pos, ret))
        {
            end[-1] = '\0';
            return buf;
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sFILS-SHA256", pos == buf ? "" : " ");
        if (os_snprintf_error(end - pos, ret))
        {
            end[-1] = '\0';
            return buf;
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sFILS-SHA384", pos == buf ? "" : " ");
        if (os_snprintf_error(end - pos, ret))
        {
            end[-1] = '\0';
            return buf;
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sFT-FILS-SHA256", pos == buf ? "" : " ");
        if (os_snprintf_error(end - pos, ret))
        {
            end[-1] = '\0';
            return buf;
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sFT-FILS-SHA384", pos == buf ? "" : " ");
        if (os_snprintf_error(end - pos, ret))
        {
            end[-1] = '\0';
            return buf;
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sDPP", pos == buf ? "" : " ");
        if (os_snprintf_error(end - pos, ret))
        {
            end[-1] = '\0';
            return buf;
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sOWE", pos == buf ? "" : " ");
        if (os_snprintf_error(end - pos, ret))
        {
            end[-1] = '\0';
            return buf;
                                       ...
                                       const char *value)
{
    ...
    {
        wpa_printf(MSG_ERROR, "Line %d: not allowed group management cipher (0x%x).", line, val);
        return -1;
    }
...
#ifndef NO_CONFIG_WRITE
static char *wpa_config_write_group_mgmt(const struct parse_data *data, struct wpa_ssid *ssid)
{
...
static int wpa_config_parse_password(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
    ...
    if (ssid->eap.password && ssid->eap.password_len == 16 && os_memcmp(ssid->eap.password, hash, 16) == 0 &&
        (ssid->eap.flags & EAP_CONFIG_FLAGS_PASSWORD_NTHASH))
    {
                                             ...
                                             const char *value)
{
    ...
    if (os_strcmp(value, "NULL") == 0)
    {
        if (!ssid->eap.machine_password)
            return 1; /* Already unset */
        wpa_printf(MSG_DEBUG, "Unset configuration string 'machine_password'");
        bin_clear_free(ssid->eap.machine_password, ssid->eap.machine_password_len);
    ...
    if (os_strncmp(value, "hash:", 5) != 0)
    {
        ...
        if (!tmp)
        {
            wpa_printf(MSG_ERROR, "Line %d: failed to parse machine_password.", line);
            return -1;
        }
        wpa_hexdump_ascii_key(MSG_MSGDUMP, data->name, (u8 *)tmp, res_len);
    ...
    if (os_strlen(value + 5) != 2 * 16)
    {
        wpa_printf(MSG_ERROR, "Line %d: Invalid machine_password hash length (expected 32 hex digits)", line);
        return -1;
    }
    ...
    {
        os_free(hash);
        wpa_printf(MSG_ERROR, "Line %d: Invalid machine_password hash", line);
        return -1;
    }

    wpa_hexdump_key(MSG_MSGDUMP, data->name, hash, 16);

    if (ssid->eap.machine_password && ssid->eap.machine_password_len == 16 &&
...
static char *wpa_config_write_machine_password(const struct parse_data *data, struct wpa_ssid *ssid)
{
    char *buf;
    ...
    if (ssid->eap.flags & EAP_CONFIG_FLAGS_EXT_MACHINE_PASSWORD)
    {
        ...
        if (!buf)
            return NULL;
        os_memcpy(buf, "ext:", 4);
        os_memcpy(buf + 4, ssid->eap.machine_password, ssid->eap.machine_password_len);
        return buf;
    }
#endif /* CONFIG_EXT_PASSWORD */
    ...
    {
        return wpa_config_write_string(ssid->eap.machine_password, ssid->eap.machine_password_len);
    }
...
static int wpa_config_parse_p2p_client_list(const struct parse_data *data,
                                            ...
                                            const char *value)
{
    return wpa_config_parse_addr_list(data, line, value, &ssid->p2p_client_list, &ssid->num_p2p_clients,
                                      "p2p_client_list", 0, 0);
}
...
static int wpa_config_parse_mka_cak(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
    size_t len;

    len = os_strlen(value);
    if (len > 2 * MACSEC_CAK_MAX_LEN || (len != 2 * 16 && len != 2 * 32) || hexstr2bin(value, ssid->mka_cak, len / 2))
    {
        wpa_printf(MSG_ERROR, "Line %d: Invalid MKA-CAK '%s'.", line, value);
        return -1;
    }
    ssid->mka_cak_len = len / 2;
    ssid->mka_psk_set |= MKA_PSK_SET_CAK;

    wpa_hexdump_key(MSG_MSGDUMP, "MKA-CAK", ssid->mka_cak, ssid->mka_cak_len);
    return 0;
}

static int wpa_config_parse_mka_ckn(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
    size_t len;
    ...
    {
        wpa_printf(MSG_ERROR, "Line %d: Invalid MKA-CKN '%s'.", line, value);
        return -1;
    }
    ...
    wpa_hexdump_key(MSG_MSGDUMP, "MKA-CKN", ssid->mka_ckn, ssid->mka_ckn_len);
    return 0;
}

#ifndef NO_CONFIG_WRITE

static char *wpa_config_write_mka_cak(const struct parse_data *data, struct wpa_ssid *ssid)
{
...
static char *wpa_config_write_mka_ckn(const struct parse_data *data, struct wpa_ssid *ssid)
{
...
static int wpa_config_parse_ocv(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
    char *end;
    ...
    {
        wpa_printf(MSG_ERROR, "Line %d: Invalid ocv value '%s'.", line, value);
        return -1;
    }
...
#ifndef NO_CONFIG_WRITE
static char *wpa_config_write_ocv(const struct parse_data *data, struct wpa_ssid *ssid)
{
...
static int wpa_config_parse_peerkey(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
...
#ifndef NO_CONFIG_WRITE
static char *wpa_config_write_peerkey(const struct parse_data *data, struct wpa_ssid *ssid)
{
    return NULL;
}
#endif /* NO_CONFIG_WRITE */
...
#define _STRe(f, m) #f, wpa_config_parse_str, wpa_config_write_str, OFFSET(eap.m)
#endif /* NO_CONFIG_WRITE */
...
#else /* NO_CONFIG_WRITE */
#define _INT(f)     #f, wpa_config_parse_int, wpa_config_write_int, OFFSET(f), (void *)0
#define _INTe(f, m) #f, wpa_config_parse_int, wpa_config_write_int, OFFSET(eap.m), (void *)0
#endif /* NO_CONFIG_WRITE */
...
#else /* NO_CONFIG_WRITE */
#define _FUNC(f) #f, wpa_config_parse_##f, wpa_config_write_##f, NULL, NULL, NULL, NULL
#endif /* NO_CONFIG_WRITE */
...
static const struct parse_data ssid_fields[] = {
    ...
    {INT_RANGE(vht, 0, 1)},
    {INT_RANGE(ht40, -1, 1)},
    {INT_RANGE(max_oper_chwidth, CHANWIDTH_USE_HT, CHANWIDTH_80P80MHZ)},
    ...
    {STR_KEYe(machine_private_key_passwd, machine_cert.private_key_passwd)},
    ...
    {STRe(machine_domain_suffix_match, machine_cert.domain_suffix_match)},
...
#ifdef CONFIG_SAE
        if (os_strcmp(var, "ssid") == 0 || os_strcmp(var, "psk") == 0 || os_strcmp(var, "sae_password") == 0 ||
...
static int wpa_config_set_cred_roaming_consortiums(struct wpa_cred *cred, const char *value)
{
    ...
    for (pos = value;;)
    {
        ...
        if (!end && len == 0)
            break;
        if (len == 0 || (len & 1) != 0 || len / 2 > MAX_ROAMING_CONS_OI_LEN ||
            hexstr2bin(pos, roaming_consortiums[num_roaming_consortiums], len / 2) < 0)
        {
            wpa_printf(MSG_INFO, "Invalid roaming_consortiums entry: %s", pos);
            return -1;
        }
        ...
        {
            wpa_printf(MSG_INFO, "Too many roaming_consortiums OIs");
            return -1;
        }
    ...
    os_memcpy(cred->roaming_consortiums, roaming_consortiums, sizeof(roaming_consortiums));
    os_memcpy(cred->roaming_consortiums_len, roaming_consortiums_len, sizeof(roaming_consortiums_len));
...
int wpa_config_set_cred(struct wpa_cred *cred, const char *var, const char *value, int line)
{
    ...
    if (val == NULL || (os_strcmp(var, "excluded_ssid") != 0 && os_strcmp(var, "roaming_consortium") != 0 &&
                        os_strcmp(var, "required_roaming_consortium") != 0 && has_newline(val)))
    {
        wpa_printf(MSG_ERROR,
    ...
    if (os_strcmp(var, "roaming_consortiums") == 0)
    {
        ...
        if (res < 0)
            wpa_printf(MSG_ERROR, "Line %d: invalid roaming_consortiums", line);
        os_free(val);
        return res;
    }
...
char *wpa_config_get_cred_no_key(struct wpa_cred *cred, const char *var)
{
    ...
    if (os_strcmp(var, "roaming_consortiums") == 0)
    {
        ...
        if (!cred->num_roaming_consortiums)
            return NULL;
        buflen = cred->num_roaming_consortiums * MAX_ROAMING_CONS_OI_LEN * 2 + 1;
        buf    = os_malloc(buflen);
        if (!buf)
            return NULL;
        pos = buf;
        for (i = 0; i < cred->num_roaming_consortiums; i++)
        {
            if (i > 0)
                *pos++ = ',';
            pos += wpa_snprintf_hex(pos, buf + buflen - pos, cred->roaming_consortiums[i],
...
#undef ecw2cw
    ...
    config->disassoc_imminent_rssi_threshold = DEFAULT_DISASSOC_IMMINENT_RSSI_THRESHOLD;
...
static int wpa_config_process_initial_freq_list(const struct global_parse_data *data,
                                                struct wpa_config *config,
                                                int line,
                                                const char *value)
{
...
static int wpa_config_process_wps_vendor_ext_m1(const struct global_parse_data *data,
                                                struct wpa_config *config,
                                                int line,
                                                const char *pos)
{
...
#ifdef CONFIG_P2P
static int wpa_config_process_sec_device_type(const struct global_parse_data *data,
                                              struct wpa_config *config,
                                              int line,
                                              const char *pos)
{
...
static int wpa_config_process_p2p_pref_chan(const struct global_parse_data *data,
                                            struct wpa_config *config,
                                            int line,
                                            const char *pos)
{
    ...
    wpa_hexdump(MSG_DEBUG, "P2P: Preferred class/channel pairs", (u8 *)config->p2p_pref_chan,
...
static int wpa_config_process_p2p_no_go_freq(const struct global_parse_data *data,
                                             struct wpa_config *config,
                                             int line,
                                             const char *pos)
{
...
static int wpa_config_process_p2p_device_persistent_mac_addr(const struct global_parse_data *data,
                                                             struct wpa_config *config,
                                                             int line,
                                                             const char *pos)
{
    ...
    {
        wpa_printf(MSG_ERROR, "Line %d: Invalid p2p_device_persistent_mac_addr '%s'", line, pos);
        return -1;
    }
...
static int wpa_config_process_ap_vendor_elements(const struct global_parse_data *data,
                                                 struct wpa_config *config,
                                                 int line,
                                                 const char *pos)
{
...
static int wpa_config_process_ap_assocresp_elements(const struct global_parse_data *data,
                                                    struct wpa_config *config,
                                                    int line,
                                                    const char *pos)
{
    ...
    if (!tmp)
    {
        wpa_printf(MSG_ERROR, "Line %d: invalid ap_assocresp_elements", line);
        return -1;
    }
    wpabuf_free(config->ap_assocresp_elements);
...
static int wpa_config_process_no_ctrl_interface(const struct global_parse_data *data,
                                                struct wpa_config *config,
                                                int line,
                                                const char *pos)
{
...
static int wpa_config_get_int(
    const char *name, struct wpa_config *config, long offset, char *buf, size_t buflen, int pretty_print)
{
...
#ifdef CONFIG_P2P
static int wpa_config_get_ipv4(
    const char *name, struct wpa_config *config, long offset, char *buf, size_t buflen, int pretty_print)
{
...
#define IPV4(f)                #f, wpa_global_config_parse_ipv4, wpa_config_get_ipv4, OFFSET(f), NULL, NULL
...
static const struct global_parse_data global_fields[] = {
    ...
    {STR(autoscan), 0},
    {INT_RANGE(wps_nfc_dev_pw_id, 0x10, 0xffff), CFG_CHANGED_NFC_PASSWORD_TOKEN},
...
#ifdef CONFIG_MBO
    {STR(non_pref_chan), 0},
    {INT_RANGE(mbo_cell_capa, MBO_CELL_CAPA_AVAILABLE, MBO_CELL_CAPA_NOT_SUPPORTED), 0},
...
int wpa_config_process_global(struct wpa_config *config, char *pos, int line)
{
    ...
    if (i == NUM_GLOBAL_FIELDS)
    {
        ...
        if (os_strncmp(pos, "tx_queue_", 9) == 0)
        {
            ...
            *tmp++ = '\0';
            if (hostapd_config_tx_queue(config->tx_queue, pos, tmp))
            {
