...
static int wpa_supplicant_global_iface_interfaces(struct wpa_global *global, const char *input, char *buf, int len);
...
static int wpas_ctrl_iface_set_lci(struct wpa_supplicant *wpa_s, const char *cmd)
{
    struct wpabuf *lci;
...
static int wpas_ctrl_set_relative_rssi(struct wpa_supplicant *wpa_s, const char *cmd)
{
    int relative_rssi;
...
static int wpas_ctrl_set_relative_band_adjust(struct wpa_supplicant *wpa_s, const char *cmd)
{
    char *pos;
    int adjust_rssi;
...
static int wpas_ctrl_iface_set_ric_ies(struct wpa_supplicant *wpa_s, const char *cmd)
{
...
#ifdef CONFIG_TESTING_OPTIONS
static int wpas_ctrl_iface_set_dso(struct wpa_supplicant *wpa_s, const char *val)
{
    u8 bssid[ETH_ALEN];
    const char *pos                    = val;
    ...
    dl_list_for_each(tmp, &wpa_s->drv_signal_override, struct driver_signal_override, list)
    {
    ...
    if (sscanf(pos, "%d %d %d %d %d", &parsed.si_current_signal, &parsed.si_avg_signal, &parsed.si_avg_beacon_signal,
               &parsed.si_current_noise, &parsed.scan_level) != 5)
        return -1;
    ...
    *value++ = '\0';
    ...
    else if (os_strcasecmp(cmd, "EAPOL::portControl") == 0)
    {
        if (os_strcmp(value, "Auto") == 0)
            ...
            eapol_sm_notify_portControl(wpa_s->eapol, ForceUnauthorized);
    ...
    else if (os_strcasecmp(cmd, "dpp_pkex_ephemeral_key_override") == 0)
    {
        ...
        if (hex_len > 2 * sizeof(dpp_pkex_ephemeral_key_override))
            ret = -1;
        else if (hexstr2bin(value, dpp_pkex_ephemeral_key_override, hex_len / 2))
            ret = -1;
        else
    ...
    else if (os_strcasecmp(cmd, "dpp_protocol_key_override") == 0)
    {
        ...
        if (hex_len > 2 * sizeof(dpp_protocol_key_override))
            ret = -1;
        else if (hexstr2bin(value, dpp_protocol_key_override, hex_len / 2))
            ret = -1;
        else
    ...
    else if (os_strcasecmp(cmd, "ignore_assoc_disallow") == 0)
    {
        ...
        wpa_drv_ignore_assoc_disallow(wpa_s, wpa_s->ignore_assoc_disallow);
    }
    ...
    else if (os_strcasecmp(cmd, "extra_sae_rejected_groups") == 0)
    {
        ...
        while (pos && pos[0])
        {
            ...
            group = atoi(pos);
            wpa_printf(MSG_DEBUG, "TESTING: Extra rejection of SAE group %d", group);
            if (group)
                int_array_add_unique(&wpa_s->extra_sae_rejected_groups, group);
            pos = os_strchr(pos, ' ');
            if (!pos)
                break;
            pos++;
...
static int wpa_supplicant_ctrl_iface_get(struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
    ...
    {
        return wpa_snprintf_hex(buf, buflen, wpa_sm_get_anonce(wpa_s->wpa), WPA_NONCE_LEN);
    }
...
static int ctrl_iface_get_capability_tdls(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
    ...
    ret = os_snprintf(buf, buflen, "%s\r\n",
                      wpa_s->drv_flags & WPA_DRIVER_FLAGS_TDLS_SUPPORT ?
                          (wpa_s->drv_flags & WPA_DRIVER_FLAGS_TDLS_EXTERNAL_SETUP ? "EXTERNAL" : "INTERNAL") :
                          "UNSUPPORTED");
...
#undef SET_FREQ_SETTING
    ...
    wpa_printf(MSG_DEBUG,
               ...
               MAC2STR(peer), oper_class, freq_params.freq, freq_params.center_freq1, freq_params.center_freq2,
               freq_params.bandwidth, freq_params.sec_channel_offset, freq_params.ht_enabled ? " HT" : "",
...
static int wpa_supplicant_ctrl_iface_tdls_link_status(struct wpa_supplicant *wpa_s,
                                                      const char *addr,
                                                      char *buf,
                                                      size_t buflen)
{
...
static int wmm_ac_ctrl_addts(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    {
        if (sscanf(token, "tsid=%i", &params.tsid) == 1 || sscanf(token, "up=%i", &params.user_priority) == 1 ||
...
static int wpa_supplicant_ctrl_iface_wps_pbc(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    if (!cmd || os_strcmp(cmd, "any") == 0 || os_strncmp(cmd, "any ", 4) == 0)
    {
        _bssid = NULL;
#ifdef CONFIG_P2P
    }
...
static int wpa_supplicant_ctrl_iface_wps_nfc_config_token(struct wpa_supplicant *wpa_s,
                                                          char *cmd,
                                                          char *reply,
                                                          size_t max_len)
{
...
static int wpas_ctrl_nfc_get_handover_sel_wps(
    struct wpa_supplicant *wpa_s, char *reply, size_t max_len, int ndef, int cr, char *uuid)
{
    struct wpabuf *buf;
    int res;
...
#ifdef CONFIG_P2P
static int wpas_ctrl_nfc_get_handover_sel_p2p(
    struct wpa_supplicant *wpa_s, char *reply, size_t max_len, int ndef, int tag)
{
    struct wpabuf *buf;
    int res;
...
static int wpas_ctrl_nfc_report_handover(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    *pos2++ = '\0';
    ...
    wpa_printf(MSG_DEBUG, "NFC: Connection handover reported - role=%s type=%s req_len=%d sel_len=%d", role, type,
               (int)wpabuf_len(req), (int)wpabuf_len(sel));
...
#ifdef CONFIG_WPS_NFC
static int wpa_supplicant_ctrl_iface_wps_er_nfc_config_token(struct wpa_supplicant *wpa_s,
                                                             char *cmd,
                                                             char *reply,
                                                             size_t max_len)
{
...
static int wpa_supplicant_ctrl_iface_status(struct wpa_supplicant *wpa_s, const char *params, char *buf, size_t buflen)
{
...
#ifdef CONFIG_HS20
    if (wpa_s->current_bss && (hs20 = wpa_bss_get_vendor_ie(wpa_s->current_bss, HS20_IE_VENDOR_TYPE)) &&
        wpa_s->wpa_proto == WPA_PROTO_RSN && wpa_key_mgmt_wpa_ieee8021x(wpa_s->key_mgmt))
    {
        int release = 1;
        if (hs20[1] >= 5)
        {
    ...
    if (os_strcmp(params, "-NO_EVENTS"))
    {
        wpa_msg_ctrl(wpa_s, MSG_INFO, WPA_EVENT_STATE_CHANGE "id=%d state=%d BSSID=" MACSTR " SSID=%s",
                     wpa_s->current_ssid ? wpa_s->current_ssid->id : -1, wpa_s->wpa_state, MAC2STR(wpa_s->bssid),
        ...
        if (wpa_s->wpa_state == WPA_COMPLETED)
        {
            struct wpa_ssid *ssid = wpa_s->current_ssid;
            wpa_msg_ctrl(
                wpa_s, MSG_INFO, WPA_EVENT_CONNECTED "- connection to " MACSTR " completed %s [id=%d id_str=%s]",
                MAC2STR(wpa_s->bssid), "(auth)", ssid ? ssid->id : -1, ssid && ssid->id_str ? ssid->id_str : "");
        }
    }
#endif /* ANDROID */
...
static int wpa_supplicant_ctrl_iface_bssid_ignore(struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
    u8 bssid[ETH_ALEN];
...
static char *wpa_supplicant_ie_txt(char *pos, char *end, const char *proto, const u8 *ie, size_t ie_len)
{
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sFILS-SHA256", pos == start ? "" : "+");
        if (os_snprintf_error(end - pos, ret))
            return pos;
        pos += ret;
    }
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sFILS-SHA384", pos == start ? "" : "+");
        if (os_snprintf_error(end - pos, ret))
            return pos;
        pos += ret;
    }
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sFT-FILS-SHA256", pos == start ? "" : "+");
        if (os_snprintf_error(end - pos, ret))
            return pos;
        pos += ret;
    }
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sFT-FILS-SHA384", pos == start ? "" : "+");
        if (os_snprintf_error(end - pos, ret))
            return pos;
        pos += ret;
    }
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sOWE", pos == start ? "" : "+");
        if (os_snprintf_error(end - pos, ret))
            return pos;
        pos += ret;
    }
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sDPP", pos == start ? "" : "+");
        if (os_snprintf_error(end - pos, ret))
            return pos;
        pos += ret;
    }
                                                 ...
                                                 size_t buflen)
{
    ...
    ret = os_snprintf(pos, end - pos, MACSTR "\t%d\t%d\t", MAC2STR(bss->bssid), bss->freq, bss->level);
    if (os_snprintf_error(end - pos, ret))
        return -1;
    pos += ret;
    ...
    if (owe)
    {
        ret = os_snprintf(pos, end - pos, ie2 ? "[OWE-TRANS]" : "[OWE-TRANS-OPEN]");
        if (os_snprintf_error(end - pos, ret))
            return -1;
        pos += ret;
    }
...
static int wpa_supplicant_ctrl_iface_mesh_interface_add(struct wpa_supplicant *wpa_s,
                                                        char *cmd,
                                                        char *reply,
                                                        size_t max_len)
{
...
static int wpa_supplicant_ctrl_iface_mesh_peer_remove(struct wpa_supplicant *wpa_s, char *cmd)
{
    u8 addr[ETH_ALEN];
...
static int wpa_supplicant_ctrl_iface_mesh_peer_add(struct wpa_supplicant *wpa_s, char *cmd)
{
    u8 addr[ETH_ALEN];
    int duration;
    char *pos;
...
static int wpa_supplicant_ctrl_iface_mesh_link_probe(struct wpa_supplicant *wpa_s, char *cmd)
{
...
static int wpa_supplicant_ctrl_iface_update_network(struct wpa_supplicant *wpa_s,
                                                    struct wpa_ssid *ssid,
                                                    char *name,
                                                    char *value)
{
    ...
    if (os_strcmp(name, "bgscan") == 0)
    {
         ...
         * changes.
         */
        if (wpa_s->current_ssid == ssid && wpa_s->wpa_state == WPA_COMPLETED)
            wpa_supplicant_reset_bgscan(wpa_s);
        return 0;
    }
#endif /* CONFIG_BGSCAN */

    if (os_strcmp(name, "bssid") != 0 && os_strcmp(name, "bssid_hint") != 0 && os_strcmp(name, "priority") != 0)
    {
...
static int wpa_supplicant_ctrl_iface_get_network(struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
    ...
    *name++ = '\0';
    ...
    id = atoi(cmd);
    wpa_printf(MSG_EXCESSIVE, "CTRL_IFACE: GET_NETWORK id=%d name='%s'", id, name);
...
static int wpa_supplicant_ctrl_iface_dup_network(struct wpa_supplicant *wpa_s,
                                                 char *cmd,
                                                 struct wpa_supplicant *dst_wpa_s)
{
    ...
    *name++ = '\0';
    ...
    wpa_printf(MSG_DEBUG, "CTRL_IFACE: DUP_NETWORK ifname=%s->%s id=%d->%d name='%s'", wpa_s->ifname, dst_wpa_s->ifname,
               id_s, id_d, name);
    ...
    ret = wpa_supplicant_ctrl_iface_update_network(dst_wpa_s, ssid_d, name, value);
...
static int wpa_supplicant_ctrl_iface_list_creds(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
    ...
    while (cred)
    {
        ret = os_snprintf(pos, end - pos, "%d\t%s\t%s\t%s\t%s\r\n", cred->id, cred->realm ? cred->realm : "",
                          cred->username ? cred->username : "", cred->domain ? cred->domain[0] : "",
                          cred->imsi ? cred->imsi : "");
        if (os_snprintf_error(end - pos, ret))
            return pos - buf;
        pos += ret;
...
static const struct cipher_info ciphers[] = {{WPA_DRIVER_CAPA_ENC_CCMP_256, "CCMP-256", 0},
...
static int ctrl_iface_get_capability_pairwise(
    int res, bool strict, struct wpa_driver_capa *capa, char *buf, size_t buflen)
{
    int ret;
    char *pos, *end;
    size_t len;
...
static int ctrl_iface_get_capability_group(int res, bool strict, struct wpa_driver_capa *capa, char *buf, size_t buflen)
{
    int ret;
    char *pos, *end;
    size_t len;
...
static int ctrl_iface_get_capability_group_mgmt(
    int res, bool strict, struct wpa_driver_capa *capa, char *buf, size_t buflen)
{
    int ret;
    char *pos, *end;
...
static int ctrl_iface_get_capability_key_mgmt(
    int res, bool strict, struct wpa_driver_capa *capa, const char *iftype_str, char *buf, size_t buflen)
{
    int ret;
    ...
    if (key_mgmt & (WPA_DRIVER_CAPA_KEY_MGMT_WPA | WPA_DRIVER_CAPA_KEY_MGMT_WPA2))
    {
    ...
    if (key_mgmt & (WPA_DRIVER_CAPA_KEY_MGMT_WPA_PSK | WPA_DRIVER_CAPA_KEY_MGMT_WPA2_PSK))
    {
...
static int ctrl_iface_get_capability_proto(int res, bool strict, struct wpa_driver_capa *capa, char *buf, size_t buflen)
{
    int ret;
    char *pos, *end;
    size_t len;
...
static int ctrl_iface_get_capability_auth_alg(
    struct wpa_supplicant *wpa_s, int res, bool strict, struct wpa_driver_capa *capa, char *buf, size_t buflen)
{
    int ret;
    char *pos, *end;
    size_t len;
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sFILS_SK_WITHOUT_PFS", pos == buf ? "" : " ");
        if (os_snprintf_error(end - pos, ret))
            return pos - buf;
        pos += ret;
    }
    ...
    {
        ret = os_snprintf(pos, end - pos, "%sFILS_SK_WITH_PFS", pos == buf ? "" : " ");
        if (os_snprintf_error(end - pos, ret))
            return pos - buf;
        pos += ret;
    }
...
#ifdef CONFIG_PASN
    ret = os_snprintf(pos, end - pos, "%sPASN", pos == buf ? "" : " ");
    if (os_snprintf_error(end - pos, ret))
        return pos - buf;
    pos += ret;
...
static int ctrl_iface_get_capability_modes(int res, bool strict, struct wpa_driver_capa *capa, char *buf, size_t buflen)
{
    int ret;
    char *pos, *end;
    size_t len;
                                                    ...
                                                    size_t buflen)
{
    ...
    wpa_printf(MSG_DEBUG, "CTRL_IFACE: GET_CAPABILITY '%s'%s%s%s", field, iftype ? " iftype=" : "",
               iftype ? iftype : "", strict ? " strict" : "");
    ...
    if (os_strcmp(field, "key_mgmt") == 0)
        return ctrl_iface_get_capability_key_mgmt(res, strict, &capa, iftype, buf, buflen);
    ...
    {
#ifdef CONFIG_FILS_SK_PFS
        if (wpa_is_fils_supported(wpa_s) && wpa_is_fils_sk_pfs_supported(wpa_s))
        {
...
#ifdef CONFIG_SAE
    if (os_strcmp(field, "sae") == 0 && (wpa_s->drv_flags & WPA_DRIVER_FLAGS_SAE))
    {
#ifdef CONFIG_SAE_PK
    ...
    if (os_strcmp(field, "beacon_prot") == 0)
    {
        if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_BEACON_PROTECTION) ||
            (wpa_s->drv_flags2 & WPA_DRIVER_FLAGS2_BEACON_PROTECTION_CLIENT))
            res = os_snprintf(buf, buflen, "supported");
        else
            res = os_snprintf(buf, buflen, "not supported");
        if (os_snprintf_error(buflen, res))
...
static int print_bss_info(
    struct wpa_supplicant *wpa_s, struct wpa_bss *bss, unsigned long mask, char *buf, size_t buflen)
{
    size_t i;
    int ret;
    char *pos, *end;
    ...
    if ((mask & WPA_BSS_MASK_INTERNETW) && bss->anqp)
    {
        ...
        pos = anqp_add_hex(pos, end, "anqp_fils_realm_info", anqp->fils_realm_info);
#ifdef CONFIG_HS20
        ...
        pos = anqp_add_hex(pos, end, "hs20_operator_icon_metadata", anqp->hs20_operator_icon_metadata);
        pos = anqp_add_hex(pos, end, "hs20_osu_providers_nai_list", anqp->hs20_osu_providers_nai_list);
#endif /* CONFIG_HS20 */
        ...
        dl_list_for_each(elem, &anqp->anqp_elems, struct wpa_bss_anqp_elem, list)
        {
            ...
            {
                ret = os_snprintf(pos, end - pos, "protected-anqp-info[%u]=1\r\n", elem->infoid);
                if (os_snprintf_error(end - pos, ret))
                    return 0;
                pos += ret;
            }
...
static void wpa_supplicant_ctrl_iface_drop_sa(struct wpa_supplicant *wpa_s)
{
    ...
    wpa_drv_set_key(wpa_s, WPA_ALG_NONE, NULL, 0, 0, NULL, 0, NULL, 0, KEY_FLAG_GROUP);
    wpa_drv_set_key(wpa_s, WPA_ALG_NONE, NULL, 1, 0, NULL, 0, NULL, 0, KEY_FLAG_GROUP);
    wpa_drv_set_key(wpa_s, WPA_ALG_NONE, NULL, 2, 0, NULL, 0, NULL, 0, KEY_FLAG_GROUP);
    wpa_drv_set_key(wpa_s, WPA_ALG_NONE, NULL, 3, 0, NULL, 0, NULL, 0, KEY_FLAG_GROUP);
    wpa_drv_set_key(wpa_s, WPA_ALG_NONE, NULL, 4, 0, NULL, 0, NULL, 0, KEY_FLAG_GROUP);
    wpa_drv_set_key(wpa_s, WPA_ALG_NONE, NULL, 5, 0, NULL, 0, NULL, 0, KEY_FLAG_GROUP);

    wpa_drv_set_key(wpa_s, WPA_ALG_NONE, wpa_s->bssid, 0, 0, NULL, 0, NULL, 0, KEY_FLAG_PAIRWISE);
    if (wpa_sm_ext_key_id(wpa_s->wpa))
        wpa_drv_set_key(wpa_s, WPA_ALG_NONE, wpa_s->bssid, 1, 0, NULL, 0, NULL, 0, KEY_FLAG_PAIRWISE);
    /* MLME-SETPROTECTION.request(None) */
    wpa_drv_mlme_setprotection(wpa_s, wpa_s->bssid, MLME_SETPROTECTION_PROTECT_TYPE_NONE,
...
static int p2p_ctrl_find(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    return wpas_p2p_find(wpa_s, timeout, type, _dev_type != NULL, _dev_type, _dev_id, search_delay, seek_count, seek,
                         freq, include_6ghz);
}
...
static int p2ps_ctrl_parse_cpt_priority(const char *pos, u8 *cpt)
{
    ...
    for (i = 0; (token = cstr_token(pos, ": \t", &last)); i++)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "P2PS: CPT name list is too long, expected up to %d names",
...
static int p2p_ctrl_connect(struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
    ...
    new_pin = wpas_p2p_connect(wpa_s, addr, pin, wps_method, persistent_group, automatic, join, auth, go_intent, freq,
...
static int p2p_ctrl_group_add(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    if (group_id >= 0)
        return p2p_ctrl_group_add_persistent(wpa_s, group_id, freq, freq2, ht40, vht, max_oper_chwidth, he, edmg,
                                             allow_6ghz);

    return wpas_p2p_group_add(wpa_s, persistent, freq, freq2, ht40, vht, max_oper_chwidth, he, edmg, allow_6ghz);
}

static int p2p_ctrl_group_member(struct wpa_supplicant *wpa_s, const char *cmd, char *buf, size_t buflen)
{
    ...
    if (!wpa_s->global->p2p || !ssid || ssid->mode != WPAS_MODE_P2P_GO || hwaddr_aton(cmd, dev_addr))
        return -1;
...
static int p2p_ctrl_set(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    *param++ = '\0';
    ...
    if (os_strcmp(cmd, "override_pref_op_chan") == 0)
    {
        ...
        param++;
        chan = atoi(param);
        p2p_set_override_pref_op_chan(wpa_s->global->p2p, op_class, chan);
        return 0;
    }
...
#define MAX_ANQP_INFO_ID 100
    ...
    return anqp_send_req(wpa_s, dst_addr, freq, id, num_id, subtypes, mbo_subtypes);
}
...
static int gas_response_get(struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
    ...
    if (wpa_s->last_gas_resp && os_memcmp(addr, wpa_s->last_gas_addr, ETH_ALEN) == 0 &&
        dialog_token == wpa_s->last_gas_dialog_token)
        resp = wpa_s->last_gas_resp;
    else if (wpa_s->prev_gas_resp && os_memcmp(addr, wpa_s->prev_gas_addr, ETH_ALEN) == 0 &&
...
static int hs20_nai_home_realm_list(struct wpa_supplicant *wpa_s, const u8 *addr, const char *realm)
{
    ...
    ret = hs20_anqp_send_req(wpa_s, addr, BIT(HS20_STYPE_NAI_HOME_REALM_QUERY), buf, len, 0);
...
static int hs20_get_nai_home_realm_list(struct wpa_supplicant *wpa_s, char *dst)
{
    ...
    ret = hs20_anqp_send_req(wpa_s, dst_addr, BIT(HS20_STYPE_NAI_HOME_REALM_QUERY), buf, len, 0);
    os_free(buf);

    return ret;
}

static int get_hs20_icon(struct wpa_supplicant *wpa_s, char *cmd, char *reply, int buflen)
{
    u8 dst_addr[ETH_ALEN];
    int used;
    ...
    return hs20_get_icon(wpa_s, dst_addr, icon, atoi(poffset), atoi(psize), reply, buflen);
}
...
static int hs20_icon_request(struct wpa_supplicant *wpa_s, char *cmd, int inmem)
{
    ...
    wpa_s->fetch_osu_icon_in_progress = 0;
    return hs20_anqp_send_req(wpa_s, dst_addr, BIT(HS20_STYPE_ICON_REQUEST), (u8 *)icon, os_strlen(icon), inmem);
}
...
static int wpa_supplicant_ctrl_iface_autoscan(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    if (wpa_s->conf->autoscan == NULL)
        ...
        wpa_printf(MSG_DEBUG, "No autoscan update in state %s", wpa_supplicant_state_txt(state));
...
static int wpas_ctrl_iface_wnm_bss_query(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    wpa_printf(MSG_DEBUG, "CTRL_IFACE: WNM_BSS_QUERY query_reason=%d%s", query_reason, list ? " candidate list" : "");

    return wnm_send_bss_transition_mgmt_query(wpa_s, query_reason, btm_candidates, list);
}

static int wpas_ctrl_iface_coloc_intf_report(struct wpa_supplicant *wpa_s, char *cmd)
{
    struct wpabuf *elems;
    int ret;
...
static int wpas_ctrl_iface_signal_monitor(struct wpa_supplicant *wpa_s, const char *cmd)
{
    const char *pos;
    int threshold  = 0;
    int hysteresis = 0;
...
static int wpas_ctrl_iface_get_pref_freq_list(struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
    ...
    wpa_printf(MSG_DEBUG, "CTRL_IFACE: GET_PREF_FREQ_LIST iface_type=%d (%s)", iface_type, cmd);
...
static int wpas_ctrl_iface_driver_flags(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
    int ret, i;
    char *pos, *end;
...
static int wpas_ctrl_iface_driver_flags2(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
    int ret, i;
    char *pos, *end;
...
static int wpa_supplicant_pktcnt_poll(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
    ...
    ret = os_snprintf(buf, buflen, "TXGOOD=%lu\r\nTXBAD=%lu\r\nRXGOOD=%lu\r\n", sta.tx_packets, sta.tx_retry_failed,
                      sta.rx_packets);
    if (os_snprintf_error(buflen, ret))
        return -1;
    return ret;
...
static int wpa_supplicant_vendor_cmd(struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
    ...
    ret = wpa_drv_vendor_cmd(wpa_s, vendor_id, subcmd, data, data_len, nested_attr_flag, reply);
...
static void wpa_supplicant_ctrl_iface_flush(struct wpa_supplicant *wpa_s)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "BSS table not empty after flush: %u entries, current_bss=%p bssid=" MACSTR " pending_bssid=" MACSTR,
                   dl_list_len(&wpa_s->bss), wpa_s->current_bss, MAC2STR(wpa_s->bssid), MAC2STR(wpa_s->pending_bssid));
    }
...
void wpas_ctrl_radio_work_flush(struct wpa_supplicant *wpa_s)
{
    ...
    dl_list_for_each_safe(work, tmp, &wpa_s->radio->work, struct wpa_radio_work, list)
    {
        ...
        if (os_strncmp(work->type, "ext:", 4) != 0)
            continue;
        ework = work->ctx;
        wpa_dbg(wpa_s, MSG_DEBUG, "Flushing%s external radio work %u (%s)", work->started ? " started" : "", ework->id,
                ework->type);
        if (work->started)
            eloop_cancel_timeout(wpas_ctrl_radio_work_timeout, work, NULL);
        radio_work_done(work);
...
static int scan_id_list_parse(struct wpa_supplicant *wpa_s,
                              const char *value,
...
static void wpas_ctrl_scan(struct wpa_supplicant *wpa_s, char *params, char *reply, int reply_size, int *reply_len)
{
    ...
    if (params)
    {
        ...
        if (pos)
        {
            ...
            wpa_s->next_scan_bssid_wildcard_ssid = os_strstr(params, "wildcard_ssid=1") != NULL;
        }
...
{
    wpa_msg(wpa_s, MSG_INFO, "MGMT-TX-STATUS freq=%u dst=" MACSTR " src=" MACSTR " bssid=" MACSTR " result=%s", freq,
...
static int wpas_ctrl_iface_mgmt_tx(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    res = offchannel_send_action(wpa_s, freq, da, wpa_s->own_addr, bssid, buf, len, wait_time,
...
static int wpas_ctrl_iface_mgmt_rx_process(struct wpa_supplicant *wpa_s, char *cmd)
{
    char *pos, *param;
    size_t len;
    u8 *buf;
...
static int wpas_ctrl_iface_driver_scan_res(struct wpa_supplicant *wpa_s, char *param)
{
    ...
    if (pos)
    {
        ...
        if (hexstr2bin(pos, ((u8 *)(res + 1)) + res->ie_len, res->beacon_ie_len))
            goto fail;
    }
...
static int wpas_ctrl_iface_driver_event_assoc(struct wpa_supplicant *wpa_s, char *param)
{
    union wpa_event_data event;
...
static void wpas_data_test_rx(void *ctx, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    wpa_msg(wpa_s, MSG_INFO, "DATA-TEST-RX " MACSTR " " MACSTR "%s", MAC2STR(eth->ether_dhost),
...
static int wpas_ctrl_iface_data_test_config(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    wpa_s->l2_test = l2_packet_init(ifname, wpa_s->own_addr, ETHERTYPE_IP, wpas_data_test_rx, wpa_s, 1);
...
static int wpas_ctrl_iface_data_test_tx(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    if (l2_packet_send(wpa_s->l2_test, dst, ETHERTYPE_IP, &buf[2], sizeof(struct ether_header) + send_len) < 0)
        return -1;
...
static int wpas_ctrl_get_fail(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
#ifdef WPA_TRACE_BFD
...
static int wpas_ctrl_reset_pn(struct wpa_supplicant *wpa_s)
{
     ...
     * in the driver. */
    if (wpa_drv_set_key(wpa_s, wpa_s->last_tk_alg, wpa_s->last_tk_addr, wpa_s->last_tk_key_idx, 1, zero, 6, zero,
                        wpa_s->last_tk_len, KEY_FLAG_PAIRWISE_RX_TX) < 0)
        return -1;

    /* Set the previously configured key to reset its TSC/RSC */
    return wpa_drv_set_key(wpa_s, wpa_s->last_tk_alg, wpa_s->last_tk_addr, wpa_s->last_tk_key_idx, 1, zero, 6,
                           wpa_s->last_tk, wpa_s->last_tk_len, KEY_FLAG_PAIRWISE_RX_TX);
}
...
static int wpas_ctrl_iface_send_twt_setup(struct wpa_supplicant *wpa_s, const char *cmd)
{
    u8 dtok                = 1;
    int exponent           = 10;
    int mantissa           = 8192;
    ...
    return wpas_twt_send_setup(wpa_s, dtok, exponent, mantissa, min_twt, setup_cmd, twt, requestor, trigger, implicit,
                               flow_type, flow_id, protection, twt_channel, control);
}

static int wpas_ctrl_iface_send_twt_teardown(struct wpa_supplicant *wpa_s, const char *cmd)
{
    u8 flags = 0x1;
    const char *tok_s;
...
update_ies:
    ...
    if (frame == VENDOR_ELEM_PROBE_REQ || frame == VENDOR_ELEM_PROBE_REQ_P2P)
        wpa_supplicant_set_default_scan_ies(wpa_s);
...
static int wpas_ctrl_vendor_elem_get(struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
    ...
    return wpa_snprintf_hex(buf, buflen, wpabuf_head_u8(wpa_s->vendor_elem[frame]),
...
static int wpas_ctrl_iface_send_neighbor_rep(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    if (ssid_s)
    {
        ...
        {
            wpa_msg(wpa_s, MSG_INFO, "CTRL: Send Neighbor Report: bad SSID");
            return -1;
        }
    ...
    ret = wpas_rrm_send_neighbor_rep_request(wpa_s, ssid_p, lci, civic, wpas_ctrl_neighbor_rep_cb, wpa_s);
...
static int wpas_ctrl_iface_pmksa_get(struct wpa_supplicant *wpa_s, const char *cmd, char *buf, size_t buflen)
{
    ...
    for (entry = wpa_sm_pmksa_cache_head(wpa_s->wpa); entry; entry = entry->next)
    {
        if (entry->network_ctx != ssid)
            continue;

        pos2 = pos;
        ret  = os_snprintf(pos2, end - pos2, MACSTR " ", MAC2STR(entry->aa));
        if (os_snprintf_error(end - pos2, ret))
            break;
        pos2 += ret;

        pos2 += wpa_snprintf_hex(pos2, end - pos2, entry->pmkid, PMKID_LEN);
        ...
        pos2 += wpa_snprintf_hex(pos2, end - pos2, entry->pmk, entry->pmk_len);

        ret = os_snprintf(pos2, end - pos2, " %d %d %d %d", (int)(entry->reauth_time - now.sec),
                          (int)(entry->expiration - now.sec), entry->akmp, entry->opportunistic);
        if (os_snprintf_error(end - pos2, ret))
            break;
        pos2 += ret;
        ...
        {
            ret = os_snprintf(pos2, end - pos2, " %02x%02x", entry->fils_cache_id[0], entry->fils_cache_id[1]);
            if (os_snprintf_error(end - pos2, ret))
                break;
            pos2 += ret;
        }
...
static int wpas_ctrl_iface_pmksa_add(struct wpa_supplicant *wpa_s, char *cmd)
{
    struct rsn_pmksa_cache_entry *entry;
    struct wpa_ssid *ssid;
    char *pos, *pos2;
    ...
    if (entry->pmk_len < PMK_LEN || entry->pmk_len > PMK_LEN_MAX || hexstr2bin(pos, entry->pmk, entry->pmk_len) < 0)
        goto fail;
    ...
    if (sscanf(pos, "%d %d %d %d", &reauth_time, &expiration, &entry->akmp, &entry->opportunistic) != 4)
        goto fail;
    ...
    if (pos)
    {
        if (hexstr2bin(pos, entry->fils_cache_id, FILS_CACHE_ID_LEN) < 0)
            goto fail;
...
static int wpas_ctrl_iface_mesh_pmksa_get(struct wpa_supplicant *wpa_s, const char *cmd, char *buf, size_t buflen)
{
    u8 spa[ETH_ALEN];
...
static int wpas_ctrl_iface_mesh_pmksa_add(struct wpa_supplicant *wpa_s, char *cmd)
{
    /*
...
#ifdef CONFIG_FILS
static int wpas_ctrl_iface_fils_hlp_req_add(struct wpa_supplicant *wpa_s, const char *cmd)
{
...
{
    if (os_strcmp(cmd, "PING") == 0 || os_strncmp(cmd, "BSS ", 4) == 0 || os_strncmp(cmd, "GET_NETWORK ", 12) == 0 ||
        os_strncmp(cmd, "STATUS", 6) == 0 || os_strncmp(cmd, "STA ", 4) == 0 || os_strncmp(cmd, "STA-", 4) == 0)
        return MSG_EXCESSIVE;
    return MSG_DEBUG;
}

static int wpas_ctrl_iface_configure_mscs(struct wpa_supplicant *wpa_s, const char *cmd)
{
...
static int wpas_ctrl_iface_pasn_start(struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    while ((token = str_token(cmd, " ", &context)))
    {
        ...
        else if (os_strncmp(token, "comeback=", 9) == 0)
        {
            ...
            if (!comeback || hexstr2bin(token + 9, comeback, comeback_len))
                goto out;
        }
        else
        {
            wpa_printf(MSG_DEBUG, "CTRL: PASN Invalid parameter: '%s'", token);
            goto out;
        }
    }
    ...
    ret = wpas_pasn_auth_start(wpa_s, bssid, akmp, cipher, group, id, comeback, comeback_len);
out:
    os_free(comeback);
    return ret;
}

static int wpas_ctrl_iface_pasn_deauthenticate(struct wpa_supplicant *wpa_s, const char *cmd)
{
    u8 bssid[ETH_ALEN];

    if (os_strncmp(cmd, "bssid=", 6) != 0 || hwaddr_aton(cmd + 6, bssid))
    {
        wpa_printf(MSG_DEBUG, "CTRL: PASN_DEAUTH without valid BSSID");
        return -1;
    }
...
static int set_type4_frame_classifier(const char *cmd, struct type4_params *param)
{
    const char *pos, *end;
    ...
    if (pos)
    {
        ...
        if (param->ip_version == IPV4)
            ret = inet_pton(AF_INET, addr, &param->ip_params.v4.src_ip);
        else
            ret = inet_pton(AF_INET6, addr, &param->ip_params.v6.src_ip);

        if (ret != 1)
        {
            wpa_printf(MSG_ERROR, "Error converting src IP address to binary ret=%d", ret);
            return -1;
        }
    ...
    if (pos)
    {
        ...
        addr[alen] = '\0';
        if (param->ip_version == IPV4)
            ret = inet_pton(AF_INET, addr, &param->ip_params.v4.dst_ip);
        else
            ret = inet_pton(AF_INET6, addr, &param->ip_params.v6.dst_ip);

        if (ret != 1)
        {
            wpa_printf(MSG_ERROR, "Error converting dst IP address to binary ret=%d", ret);
            return -1;
        }
    ...
    else
    {
        ...
        if (pos)
        {
            ...
            if (end - pos != 6 || hexstr2bin(pos, param->ip_params.v6.flow_label, 3) ||
...
static int set_type10_frame_classifier(const char *cmd, struct type10_params *param)
{
    const char *pos, *end;
    size_t filter_len;
    ...
    if (!pos)
    {
        wpa_printf(MSG_ERROR, "Classifier parameter filter_value missing");
        return -1;
    }
    ...
    if (!pos)
    {
        wpa_printf(MSG_ERROR, "Classifier parameter filter_mask missing");
        goto free;
    }
    ...
    {
        wpa_printf(MSG_ERROR, "Filter mask length mismatch expected=%zu received=%zu", filter_len,
                   (size_t)(end - pos) / 2);
        goto free;
    }
...
static int scs_parse_type4(struct tclas_element *elem, const char *pos)
{
    ...
    os_memcpy(&elem->frame_classifier.type4_param, &type4_param, sizeof(struct type4_params));
    return 0;
}
...
static int scs_parse_type10(struct tclas_element *elem, const char *pos)
{
    ...
    os_memcpy(&elem->frame_classifier.type10_param, &type10_param, sizeof(struct type10_params));
    return 0;
}

static int wpas_ctrl_iface_configure_scs(struct wpa_supplicant *wpa_s, char *cmd)
{
    char *pos1, *pos;
    ...
    {
        wpa_printf(MSG_ERROR, "%s: SCS Request already in queue", __func__);
        return -1;
    }
    ...
    while (pos1)
    {
        ...
        if (next_scs_desc)
        {
            ...
            os_snprintf(temp, sizeof(temp), "scs_id=%d ", desc_elem.scs_id);
            if (os_strstr(next_scs_desc, temp))
            {
                wpa_printf(MSG_ERROR, "Multiple SCS descriptors configured with same SCSID(=%d)", desc_elem.scs_id);
                goto free_scs_desc;
            }
            pos1[next_scs_desc - pos1 - 1] = '\0';
        }

        dl_list_for_each(active_scs_desc, &wpa_s->active_scs_ids, struct active_scs_elem, list)
        {
        ...
        if (os_strstr(pos1, "add "))
        {
            ...
            if (scsid_active)
            {
                wpa_printf(MSG_ERROR, "SCSID %d already active", desc_elem.scs_id);
                return -1;
            }
        }
        else if (os_strstr(pos1, "remove"))
        {
            ...
            if (!scsid_active)
            {
                wpa_printf(MSG_ERROR, "SCSID %d not active", desc_elem.scs_id);
                return -1;
            }
            goto scs_desc_end;
        }
        ...
        if (!pos1)
        {
            wpa_printf(MSG_ERROR, "Intra-Access user priority not present");
            goto free_scs_desc;
        }
        ...
        if (val < 0 || val > 7)
        {
            wpa_printf(MSG_ERROR, "Intra-Access user priority invalid %d", val);
            goto free_scs_desc;
        }
        ...
        while (pos)
        {
            ...
            if (val != 4 && val != 10)
            {
                wpa_printf(MSG_ERROR, "classifier type invalid %d", val);
                goto free_scs_desc;
            }
            ...
            n = os_realloc(desc_elem.tclas_elems, (num_tclas_elem + 1) * sizeof(elem));
            if (!n)
                goto free_scs_desc;

            desc_elem.tclas_elems = n;
            os_memcpy((u8 *)desc_elem.tclas_elems + num_tclas_elem * sizeof(elem), &elem, sizeof(elem));
            num_tclas_elem++;
        ...
        if (desc_elem.num_tclas_elem > 1)
        {
            ...
            if (val != 0 && val != 1)
            {
                wpa_printf(MSG_ERROR, "tclas_processing invalid");
                goto free_scs_desc;
            }
    ...
    scs_desc_end:
        n1 = os_realloc(scs_data->scs_desc_elems, (num_scs_desc + 1) * sizeof(struct scs_desc_elem));
        if (!n1)
            goto free_scs_desc;

        scs_data->scs_desc_elems = n1;
        os_memcpy((u8 *)scs_data->scs_desc_elems + num_scs_desc * sizeof(desc_elem), &desc_elem, sizeof(desc_elem));
        num_scs_desc++;
...
static int wpas_ctrl_iface_send_dscp_resp(struct wpa_supplicant *wpa_s, const char *cmd)
{
    char *pos;
    ...
    while (pos)
    {
        ...
        if (!pos)
        {
            wpa_printf(MSG_ERROR, "DSCP: Status is not found for a policy");
            goto fail;
        }
...
static int wpas_ctrl_iface_send_dscp_query(struct wpa_supplicant *wpa_s, const char *cmd)
{
    char *pos;
...
char *wpa_supplicant_ctrl_iface_process(struct wpa_supplicant *wpa_s, char *buf, size_t *resp_len)
{
        ...
        if (wpa_debug_show_keys)
            ...
            wpa_dbg(wpa_s, MSG_DEBUG, "Control interface command '%s [REMOVED]'",
                    os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0 ?
                        WPA_CTRL_RSP :
                        (os_strncmp(buf, "SET_NETWORK ", 12) == 0 ? "SET_NETWORK" : "key-add"));
    }
...
#ifdef CONFIG_MACSEC
            reply_len += ieee802_1x_kay_get_mib(wpa_s->kay, reply + reply_len, reply_size - reply_len);
#endif /* CONFIG_MACSEC */
        }
    }
    ...
    {
        reply_len = wpas_ctrl_iface_pmksa_get(wpa_s, buf + 10, reply, reply_size);
    }
    ...
    {
        reply_len = wpas_ctrl_iface_mesh_pmksa_get(wpa_s, buf + 15, reply, reply_size);
    }
    ...
    {
        reply_len = wpa_supplicant_ctrl_iface_bssid_ignore(wpa_s, buf + 12, reply, reply_size);
    }
    else if (os_strncmp(buf, "BLACKLIST", 9) == 0)
    {
        /* deprecated backwards compatibility alias for BSSID_IGNORE */
        reply_len = wpa_supplicant_ctrl_iface_bssid_ignore(wpa_s, buf + 9, reply, reply_size);
    }
    ...
    {
        reply_len = wpa_supplicant_global_iface_interfaces(wpa_s->global, buf + 10, reply, reply_size);
    }
    ...
    {
        reply_len = wpas_ctrl_iface_driver_flags2(wpa_s, reply, reply_size);
#ifdef ANDROID
    }
    ...
    {
        sme_event_unprot_disconnect(wpa_s, wpa_s->bssid, NULL, WLAN_REASON_CLASS2_FRAME_FROM_NONAUTH_STA);
    }
    ...
    {
        reply_len = wpas_ctrl_iface_get_pref_freq_list(wpa_s, buf + 19, reply, reply_size);
#ifdef CONFIG_FILS
    }
    ...
    {
        reply_len = dpp_bootstrap_info(wpa_s->dpp, atoi(buf + 19), reply, reply_size);
    }
    else if (os_strncmp(buf, "DPP_BOOTSTRAP_SET ", 18) == 0)
    {
        if (dpp_bootstrap_set(wpa_s->dpp, atoi(buf + 18), os_strchr(buf + 18, ' ')) < 0)
            reply_len = -1;
    }
    ...
    {
        reply_len = dpp_configurator_get_key_id(wpa_s->dpp, atoi(buf + 25), reply, reply_size);
    }
...
static int wpa_supplicant_global_iface_add(struct wpa_global *global, char *cmd)
{
    ...
    do
    {
        ...
        if (os_strcmp(extra, "create") == 0)
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "INTERFACE_ADD unsupported interface type: '%s'", pos);
                return -1;
            }
        }
        else
    ...
    if (create_iface)
    {
        ...
        if (!global->ifaces)
            return -1;
        if (wpa_drv_if_add(global->ifaces, type, iface.ifname, NULL, NULL, NULL, mac_addr, NULL) < 0)
        {
            wpa_printf(MSG_ERROR, "CTRL_IFACE interface creation failed");
            return -1;
        }

        wpa_printf(MSG_DEBUG, "CTRL_IFACE interface '%s' created with MAC addr: " MACSTR, iface.ifname,
                   MAC2STR(mac_addr));
    }
...
static int wpas_global_ctrl_iface_dup_network(struct wpa_global *global, char *cmd)
{
...
static int wpas_global_ctrl_iface_fst_attach(struct wpa_global *global, char *cmd, char *buf, size_t reply_size)
{
...
static int wpas_global_ctrl_iface_fst_detach(struct wpa_global *global, char *cmd, char *buf, size_t reply_size)
{
    char ifname[IFNAMSIZ + 1];
    struct wpa_supplicant *wpa_s;
...
char *wpa_supplicant_global_ctrl_iface_process(struct wpa_global *global, char *buf, size_t *resp_len)
{
    ...
    {
        reply_len = wpa_supplicant_global_iface_interfaces(global, buf + 10, reply, reply_size);
#ifdef CONFIG_FST
    }
    else if (os_strncmp(buf, "FST-ATTACH ", 11) == 0)
    {
        reply_len = wpas_global_ctrl_iface_fst_attach(global, buf + 11, reply, reply_size);
    }
    else if (os_strncmp(buf, "FST-DETACH ", 11) == 0)
    {
        reply_len = wpas_global_ctrl_iface_fst_detach(global, buf + 11, reply, reply_size);
    }
