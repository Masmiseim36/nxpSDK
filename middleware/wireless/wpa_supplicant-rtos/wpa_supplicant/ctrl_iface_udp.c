                                            ...
                                            socklen_t fromlen)
{
    ...
    *head            = dst;
    ...
    wpa_printf(MSG_DEBUG, "CTRL_IFACE monitor attached %s:%d",
               inet_ntop(AF_INET6, &from->sin6_addr, addr, sizeof(*from)), ntohs(from->sin6_port));
#else  /* CONFIG_CTRL_IFACE_UDP_IPV6 */
                                            ...
                                            socklen_t fromlen)
{
    ...
    while (dst)
    {
        ...
        if (from->sin6_port == dst->addr.sin6_port &&
            !os_memcmp(&from->sin6_addr, &dst->addr.sin6_addr, sizeof(from->sin6_addr)))
        {
...
static void wpa_supplicant_ctrl_iface_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
    ...
    if (!buf)
        return;
    res = recvfrom(sock, buf, CTRL_IFACE_MAX_LEN, 0, (struct sockaddr *)&from, &fromlen);
    if (res < 0)
    {
...
static void wpa_supplicant_ctrl_iface_msg_cb(void *ctx, int level, enum wpa_msg_type type, const char *txt, size_t len)
{
    ...
    if (type != WPA_MSG_NO_GLOBAL && wpa_s->global->ctrl_iface)
    {
        ...
        if (priv->ctrl_dst)
        {
            wpa_supplicant_ctrl_iface_send(wpa_s, type != WPA_MSG_PER_INTERFACE ? NULL : wpa_s->ifname, priv->sock,
    ...
    wpa_supplicant_ctrl_iface_send(wpa_s, NULL, wpa_s->ctrl_iface->sock, &wpa_s->ctrl_iface->ctrl_dst, level, txt, len);
}
...
struct ctrl_iface_priv *wpa_supplicant_ctrl_iface_init(struct wpa_supplicant *wpa_s)
{
    ...
    if (pos)
    {
        ...
        if (port <= 0)
        {
            wpa_printf(MSG_ERROR, "Invalid ctrl_iface UDP port: %s", wpa_s->conf->ctrl_interface);
            goto fail;
        }
    }
                                           ...
                                           size_t len)
{
    ...
    if (ifname)
        os_snprintf(levelstr, sizeof(levelstr), "IFNAME=%s <%d>", ifname, level);
    else
    ...
    while (dst)
    {
        ...
        if (level >= dst->debug_level)
        {
            ...
            wpa_printf(MSG_DEBUG, "CTRL_IFACE monitor send %s:%d",
                       inet_ntop(AF_INET6, &dst->addr.sin6_addr, addr, sizeof(dst->addr)), ntohs(dst->addr.sin6_port));
#else  /* CONFIG_CTRL_IFACE_UDP_IPV6 */
            wpa_printf(MSG_DEBUG, "CTRL_IFACE monitor send %s:%d", inet_ntoa(dst->addr.sin_addr),
                       ntohs(dst->addr.sin_port));
#endif /* CONFIG_CTRL_IFACE_UDP_IPV6 */
            if (sendto(sock, sbuf, llen + len, 0, (struct sockaddr *)&dst->addr, sizeof(dst->addr)) < 0)
            {
...
struct ctrl_iface_global_priv *wpa_supplicant_global_ctrl_iface_init(struct wpa_global *global)
{
    ...
    if (pos)
    {
        ...
        if (port <= 0)
        {
            wpa_printf(MSG_ERROR, "Invalid global ctrl UDP port %s", global->params.ctrl_interface);
            goto fail;
        }
    }
...
try_again:
    ...
    {
        port++;
        if ((port - WPA_GLOBAL_CTRL_IFACE_PORT) < WPA_GLOBAL_CTRL_IFACE_PORT_LIMIT && !pos)
            goto try_again;
        wpa_printf(MSG_ERROR, "bind(AF_INET): %s", strerror(errno));
        goto fail;
    }
