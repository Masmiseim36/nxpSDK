...
void wpas_hs20_add_roam_cons_sel(struct wpabuf *buf, const struct wpa_ssid *ssid);

int hs20_anqp_send_req(
    struct wpa_supplicant *wpa_s, const u8 *dst, u32 stypes, const u8 *payload, size_t payload_len, int inmem);
void hs20_put_anqp_req(u32 stypes, const u8 *payload, size_t payload_len, struct wpabuf *buf);
void hs20_parse_rx_hs20_anqp_resp(
    struct wpa_supplicant *wpa_s, struct wpa_bss *bss, const u8 *sa, const u8 *data, size_t slen, u8 dialog_token);
...
int hs20_get_icon(struct wpa_supplicant *wpa_s,
                  const u8 *bssid,
                  ...
                  char *reply,
                  size_t buf_len);
int hs20_del_icon(struct wpa_supplicant *wpa_s, const u8 *bssid, const char *file_name);
