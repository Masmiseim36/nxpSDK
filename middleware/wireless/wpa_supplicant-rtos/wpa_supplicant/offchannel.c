...
static struct wpa_supplicant *wpas_get_tx_interface(struct wpa_supplicant *wpa_s, const u8 *src)
{
    ...
    if (os_memcmp(src, wpa_s->own_addr, ETH_ALEN) == 0)
    {
            ...
            *wpabuf_head_u8(wpa_s->pending_action_tx) != WLAN_ACTION_PUBLIC)
        {
             ...
             */
            wpa_printf(MSG_DEBUG, "P2P: Use GO interface %s instead of interface %s for Action TX",
...
static void wpas_send_action_cb(void *eloop_ctx, void *timeout_ctx)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               without_roc, wpa_s->pending_action_tx, !!wpa_s->pending_action_tx_done);
    ...
    if (wpa_s->off_channel_freq != wpa_s->pending_action_freq && wpa_s->pending_action_freq != 0 &&
        wpa_s->pending_action_freq != iface->assoc_freq)
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   "assoc_freq=%u)",
                   wpa_s->pending_action_freq, wpa_s->off_channel_freq, iface->assoc_freq);
    ...
    wpa_printf(MSG_DEBUG,
               "Off-channel: Sending pending Action frame to " MACSTR " using interface %s (pending_action_tx=%p)",
               MAC2STR(wpa_s->pending_action_dst), iface->ifname, wpa_s->pending_action_tx);
    res =
        wpa_drv_send_action(iface, wpa_s->pending_action_freq, 0, wpa_s->pending_action_dst, wpa_s->pending_action_src,
                            wpa_s->pending_action_bssid, wpabuf_head(wpa_s->pending_action_tx),
                            wpabuf_len(wpa_s->pending_action_tx), wpa_s->pending_action_no_cck);
    if (res)
    {
        wpa_printf(MSG_DEBUG,
                   "Off-channel: Failed to send the "
                   "pending Action frame");
        /*
         * Use fake TX status event to allow state machines to
         * continue.
         */
        offchannel_send_action_tx_status(wpa_s, wpa_s->pending_action_dst, wpabuf_head(wpa_s->pending_action_tx),
                                      ...
                                      enum offchannel_send_action_result result)
{
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "Off-channel: Ignore Action TX status - "
    ...
    if (data_len - wpabuf_len(wpa_s->pending_action_tx) != 24 ||
        os_memcmp(data + 24, wpabuf_head(wpa_s->pending_action_tx), wpabuf_len(wpa_s->pending_action_tx)) != 0)
    {
        wpa_printf(MSG_DEBUG,
                   "Off-channel: Ignore Action TX status - "
    ...
    wpa_printf(MSG_DEBUG, "Off-channel: Delete matching pending action frame (dst=" MACSTR " pending_action_tx=%p)",
    ...
    {
        wpa_s->pending_action_tx_status_cb(wpa_s, wpa_s->pending_action_freq, wpa_s->pending_action_dst,
...
{
    wpa_printf(MSG_DEBUG,
               "Off-channel: Send action frame: freq=%d dst=" MACSTR " src=" MACSTR " bssid=" MACSTR " len=%d", freq,
               MAC2STR(dst), MAC2STR(src), MAC2STR(bssid), (int)len);
    ...
    wpa_printf(MSG_DEBUG, "Off-channel: Stored pending action frame (dst=" MACSTR " pending_action_tx=%p)",
               MAC2STR(dst), wpa_s->pending_action_tx);
    wpa_hexdump_buf(MSG_MSGDUMP, "Pending TX frame", wpa_s->pending_action_tx);
    ...
    if (freq != 0 && wpa_s->drv_flags & WPA_DRIVER_FLAGS_OFFCHANNEL_TX)
    {
        ...
        ret = wpa_drv_send_action(iface, wpa_s->pending_action_freq, wait_time, wpa_s->pending_action_dst,
                                  wpa_s->pending_action_src, wpa_s->pending_action_bssid,
                                  wpabuf_head(wpa_s->pending_action_tx), wpabuf_len(wpa_s->pending_action_tx),
    ...
    {
        wpa_printf(MSG_DEBUG,
                   "Off-channel: Already on requested "
    ...
    wpa_printf(MSG_DEBUG,
               "Off-channel: Schedule Action frame to be "
...
{
    wpa_printf(MSG_DEBUG,
               "Off-channel: Action frame sequence done notification: pending_action_tx=%p drv_offchan_tx=%d "
               "action_tx_wait_time=%d off_channel_freq=%d roc_waiting_drv_freq=%d",
               wpa_s->pending_action_tx, !!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_OFFCHANNEL_TX),
               wpa_s->action_tx_wait_time, wpa_s->off_channel_freq, wpa_s->roc_waiting_drv_freq);
    wpabuf_free(wpa_s->pending_action_tx);
    wpa_s->pending_action_tx = NULL;
...
{
    wpa_printf(MSG_DEBUG, "Off-channel: Clear pending Action frame TX (pending_action_tx=%p", wpa_s->pending_action_tx);
    wpabuf_free(wpa_s->pending_action_tx);
    wpa_s->pending_action_tx = NULL;
}
