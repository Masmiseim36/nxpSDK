...
static int wpas_p2p_setup_freqs(struct wpa_supplicant *wpa_s,
                                int freq,
                                int *force_freq,
                                int *pref_freq,
                                int go,
...
static int wpas_p2p_join(struct wpa_supplicant *wpa_s,
                         const u8 *iface_addr,
                         const u8 *dev_addr,
...
static int wpas_get_6ghz_he_chwidth_capab(struct hostapd_hw_modes *mode)
{
    ...
    if (mode)
        he_capab = mode->he_capab[WPAS_MODE_INFRA].phy_cap[HE_PHYCAP_CHANNEL_WIDTH_SET_IDX];
    return he_capab;
}
...
static void wpas_p2p_set_own_freq_preference(struct wpa_supplicant *wpa_s, int freq)
{
    ...
    if (wpa_s->conf->p2p_ignore_shared_freq && freq > 0 && wpa_s->num_multichan_concurrent > 1 &&
...
static void wpas_p2p_scan_res_handled(struct wpa_supplicant *wpa_s)
{
    ...
    if (external_scan_running(wpa_s->radio))
    {
        ...
        wpa_printf(MSG_DEBUG, "P2P: Delay next P2P search by %d ms to let externally triggered scan complete", delay);
    }
...
static void wpas_p2p_scan_res_handler(struct wpa_supplicant *wpa_s, struct wpa_scan_results *scan_res)
{
    ...
    for (i = 0; i < scan_res->num; i++)
    {
        ...
        ies     = (const u8 *)(bss + 1);
        ies_len = bss->ie_len;
        if (bss->beacon_ie_len > 0 && !wpa_scan_get_vendor_ie(bss, P2P_IE_VENDOR_TYPE) &&
            wpa_scan_get_vendor_ie_beacon(bss, P2P_IE_VENDOR_TYPE))
        {
            wpa_printf(
                MSG_DEBUG,
                "P2P: Use P2P IE(s) from Beacon frame since no P2P IE(s) in Probe Response frames received for " MACSTR,
                MAC2STR(bss->bssid));
            ies     = ies + ies_len;
...
static void wpas_p2p_scan_res_fail_handler(struct wpa_supplicant *wpa_s)
{
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Failed to get scan results - try to continue");
    wpas_p2p_scan_res_handled(wpa_s);
}
...
static void wpas_p2p_trigger_scan_cb(struct wpa_radio_work *work, int deinit)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "P2P: Exclude 6 GHz channels - update the scan frequency list");
        wpa_add_scan_freqs_list(wpa_s, HOSTAPD_MODE_IEEE80211G, params, 0);
        wpa_add_scan_freqs_list(wpa_s, HOSTAPD_MODE_IEEE80211A, params, 0);
    }
...
static int wpas_p2p_scan(void *ctx,
                         ...
                         bool include_6ghz)
{
    ...
    wpa_s->wps->dev.p2p = 1;
    wps_ie = wps_build_probe_req_ie(pw_id, &wpa_s->wps->dev, wpa_s->wps->uuid, WPS_REQ_ENROLLEE, num_req_dev_types,
                                    req_dev_types);
    if (wps_ie == NULL)
        goto fail;
...
static unsigned int p2p_group_go_member_count(struct wpa_supplicant *wpa_s)
{
    ...
    for (wpa_s = wpa_s->global->ifaces; wpa_s; wpa_s = wpa_s->next)
    {
        for (s = wpa_s->conf->ssid; s; s = s->next)
        {
            wpa_printf(MSG_DEBUG, "P2P: sup:%p ssid:%p disabled:%d p2p:%d mode:%d", wpa_s, s, s->disabled, s->p2p_group,
                       s->mode);
...
{
    return !wpa_s->p2p_mgmt && wpa_s->current_ssid && !wpa_s->current_ssid->disabled &&
           wpa_s->current_ssid->p2p_group && wpa_s->current_ssid->p2p_persistent_group;
}

static unsigned int p2p_is_active_persistent_go(struct wpa_supplicant *wpa_s)
{
    return p2p_is_active_persistent_group(wpa_s) && wpa_s->current_ssid->mode == WPAS_MODE_P2P_GO;
}
...
{
    return p2p_is_active_persistent_group(wpa_s) && wpa_s->current_ssid->mode == WPAS_MODE_INFRA;
}

/* Find an interface for a P2P group where we are the P2P Client */
static struct wpa_supplicant *wpas_p2p_get_cli_group(struct wpa_supplicant *wpa_s)
{
    for (wpa_s = wpa_s->global->ifaces; wpa_s; wpa_s = wpa_s->next)
    {
...
static u8 p2ps_group_capability(void *ctx, u8 incoming, u8 role, unsigned int *force_freq, unsigned int *pref_freq)
{
    struct wpa_supplicant *wpa_s = ctx;
    struct wpa_ssid *s;
    ...
    if (force_freq && pref_freq &&
        !wpas_p2p_setup_freqs(wpa_s, 0, (int *)force_freq, (int *)pref_freq, 0, pref_freq_list, &size))
    ...
    wpa_printf(MSG_DEBUG, "P2P: GO(iface)=%p members=%u CLI(iface)=%p persistent(ssid)=%p", go_wpa_s, owned_members,
               cli_wpa_s, persistent_go);
...
grp_owner:
    if ((conncap & P2PS_SETUP_GROUP_OWNER) || (!incoming && (conncap & P2PS_SETUP_NEW)))
    {
        ...
        {
            p2p_set_intended_addr(wpa_s->global->p2p, wpa_s->p2p_mgmt ? wpa_s->parent->own_addr : wpa_s->own_addr);
        }
        else if (!s && !go_wpa_s)
        {
            if (wpas_p2p_add_group_interface(wpa_s, WPA_IF_P2P_GROUP) < 0)
            {
...
static int wpas_p2p_group_delete(struct wpa_supplicant *wpa_s, enum p2p_group_removal_reason removal_reason)
{
    ...
    if (wpa_s->cross_connect_in_use)
    {
        ...
        wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_CROSS_CONNECT_DISABLE "%s %s", wpa_s->ifname,
    ...
    {
        wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_GROUP_REMOVED "%s %s%s", wpa_s->ifname, gtype, reason);
    }
    ...
    if (eloop_cancel_timeout(wpas_p2p_group_formation_timeout, wpa_s->p2pdev, NULL) > 0)
    {
        wpa_printf(MSG_DEBUG,
...
static int wpas_p2p_persistent_group(struct wpa_supplicant *wpa_s, u8 *go_dev_addr, const u8 *ssid, size_t ssid_len)
{
    ...
    if (p2p == NULL)
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   MAC2STR(bssid));
        wpa_hexdump(MSG_DEBUG, "P2P: Probe Response IEs", wpa_bss_ie_ptr(bss), bss->ie_len);
        wpa_hexdump(MSG_DEBUG, "P2P: Beacon IEs", wpa_bss_ie_ptr(bss) + bss->ie_len, bss->beacon_ie_len);
        return 0;
    }
...
static int wpas_p2p_store_persistent_group(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid, const u8 *go_dev_addr)
{
    ...
    for (s = wpa_s->conf->ssid; s; s = s->next)
    {
        if (s->disabled == 2 && os_memcmp(go_dev_addr, s->bssid, ETH_ALEN) == 0 && s->ssid_len == ssid->ssid_len &&
...
static void wpas_p2p_add_persistent_group_client(struct wpa_supplicant *wpa_s, const u8 *addr)
{
    ...
    for (i = 0; s->p2p_client_list && i < s->num_p2p_clients; i++)
    {
        ...
        os_memmove(s->p2p_client_list + i * 2 * ETH_ALEN, s->p2p_client_list + (i + 1) * 2 * ETH_ALEN,
                                   ...
                                   const char *extra)
{
    ...
    wpa_msg_global_ctrl(wpa_s->p2pdev, MSG_INFO,
                        P2P_EVENT_GROUP_STARTED "%s %s ssid=\"%s\" freq=%d%s%s%s%s%s go_dev_addr=" MACSTR "%s%s",
                        wpa_s->ifname, go ? "GO" : "client", ssid_txt, freq, psk ? " psk=" : "", psk_txt,
                        passphrase ? " passphrase=\"" : "", passphrase ? passphrase : "", passphrase ? "\"" : "",
                        MAC2STR(go_dev_addr), persistent ? " [PERSISTENT]" : "", extra);
    wpa_printf(MSG_INFO, P2P_EVENT_GROUP_STARTED "%s %s ssid=\"%s\" freq=%d go_dev_addr=" MACSTR "%s%s", wpa_s->ifname,
               go ? "GO" : "client", ssid_txt, freq, MAC2STR(go_dev_addr), persistent ? " [PERSISTENT]" : "", extra);
}
...
static void wpas_group_formation_completed(struct wpa_supplicant *wpa_s, int success, int already_deleted)
{
    ...
    {
        wpa_msg(wpa_s, MSG_DEBUG, "P2P: Stop provisioning state due to failure");
        wpa_s->p2p_in_provisioning = 0;
    }
    wpa_s->p2p_in_invitation        = 0;
    ...
    {
        wpas_p2p_group_started(wpa_s, 1, ssid, ssid ? ssid->frequency : 0,
...
static void wpas_p2p_free_send_action_work(struct wpa_supplicant *wpa_s)
{
    ...
    wpa_printf(MSG_DEBUG,
               "P2P: Free Action frame radio work @%p (freq=%u dst=" MACSTR " src=" MACSTR " bssid=" MACSTR
               " wait_time=%u)",
               wpa_s->p2p_send_action_work, awork->freq, MAC2STR(awork->dst), MAC2STR(awork->src),
               MAC2STR(awork->bssid), awork->wait_time);
    wpa_hexdump(MSG_DEBUG, "P2P: Freeing pending Action frame", awork->buf, awork->len);
    os_free(awork);
...
static void wpas_p2p_action_tx_clear(struct wpa_supplicant *wpa_s)
{
    if (wpa_s->p2p_send_action_work)
    {
        ...
        wpa_printf(MSG_DEBUG, "P2P: Clear Action TX work @%p (wait_time=%u)", wpa_s->p2p_send_action_work,
                   awork->wait_time);
                                           ...
                                           enum offchannel_send_action_result result)
{
    ...
    if (result != OFFCHANNEL_SEND_ACTION_SUCCESS && wpa_s->pending_pd_before_join &&
        ...
        wpa_s->p2p_fallback_to_go_neg)
    {
        ...
        wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_FALLBACK_TO_GO_NEG "reason=no-ACK-to-PD-Req");
...
static void wpas_send_action_cb(struct wpa_radio_work *work, int deinit)
{
    ...
    if (offchannel_send_action(wpa_s, awork->freq, awork->dst, awork->src, awork->bssid, awork->buf, awork->len,
                               awork->wait_time, wpas_p2p_send_action_tx_status, 1) < 0)
    {
        os_free(awork);
        radio_work_done(work);
        return;
                                 ...
                                 unsigned int wait_time)
{
    ...
    if (radio_add_work(wpa_s, freq, "p2p-send-action", 1, wpas_send_action_cb, awork) < 0)
    {
        os_free(awork);
        return -1;
    }
...
static int wpas_send_action(void *ctx,
                            unsigned int freq,
                            const u8 *dst,
                            const u8 *src,
                            const u8 *bssid,
                            ...
                            int *scheduled)
{
    ...
    if (listen_freq != (int)freq && send_freq != (int)freq)
    {
        ...
        res = wpas_send_action_work(wpa_s, freq, dst, src, bssid, buf, len, wait_time);
        if (res == 0 && scheduled)
            *scheduled = 1;
        return res;
    }
...
static void wpas_start_wps_enrollee(struct wpa_supplicant *wpa_s, struct p2p_go_neg_results *res)
{
    ...
    wpa_printf(MSG_DEBUG, "P2P: Start WPS Enrollee for peer " MACSTR " dev_addr " MACSTR " wps_method %d",
               MAC2STR(res->peer_interface_addr), MAC2STR(res->peer_device_addr), res->wps_method);
    ...
    {
        wpas_wps_start_nfc(wpa_s, res->peer_device_addr, res->peer_interface_addr, wpa_s->p2pdev->p2p_oob_dev_pw,
...
static void wpas_p2p_add_psk_list(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
    ...
    persistent = wpas_p2p_get_persistent(wpa_s->p2pdev, NULL, ssid->ssid, ssid->ssid_len);
    if (persistent == NULL)
        return;
    ...
    dl_list_for_each(psk, &persistent->psk_list, struct psk_list_entry, list)
    {
        ...
        wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Add persistent group PSK entry for " MACSTR " psk=%d", MAC2STR(psk->addr),
                psk->p2p);
...
static void p2p_go_dump_common_freqs(struct wpa_supplicant *wpa_s)
{
    ...
    {
        res = os_snprintf(pos, end - pos, " %d", wpa_s->p2p_group_common_freqs[i]);
...
static void p2p_config_write(struct wpa_supplicant *wpa_s)
{
    ...
    if (wpa_s->p2pdev->conf->update_config && wpa_config_write(wpa_s->p2pdev->confname, wpa_s->p2pdev->conf))
        wpa_printf(MSG_DEBUG, "P2P: Failed to update configuration");
...
static void p2p_go_configured(void *ctx, void *data)
{
    ...
    if (!wpa_s->go_params)
    {
        wpa_printf(MSG_ERROR, "P2P: p2p_go_configured() called with wpa_s->go_params == NULL");
        return;
    }
    ...
    if (ssid && ssid->mode == WPAS_MODE_P2P_GO)
    {
        ...
        if (wpa_s->global->p2p_group_formation == wpa_s)
            wpa_s->global->p2p_group_formation = NULL;
        wpas_p2p_group_started(wpa_s, 1, ssid, ssid->frequency, params->passphrase[0] == '\0' ? params->psk : NULL,
                               params->passphrase, wpa_s->global->p2p_dev_addr, params->persistent_group, "");
        wpa_s->group_formation_reported = 1;
        ...
        if (wpa_s->p2pdev->p2ps_method_config_any)
        {
            ...
            else
            {
                ...
                wpa_supplicant_ap_wps_pin(wpa_s, wpa_s->p2pdev->p2ps_join_addr, "12345670", NULL, 0, 0);
            }
        ...
        {
            wpas_p2p_store_persistent_group(wpa_s->p2pdev, ssid, wpa_s->global->p2p_dev_addr);
            wpas_p2p_add_psk_list(wpa_s, ssid);
        }

        wpas_notify_p2p_group_started(wpa_s, ssid, params->persistent_group, 0, NULL);
        wpas_p2p_cross_connect_setup(wpa_s);
        wpas_p2p_set_group_idle_timeout(wpa_s);
        ...
        if (wpa_s->p2p_first_connection_timeout)
        {
            ...
            eloop_register_timeout(wpa_s->p2p_first_connection_timeout, 0, wpas_p2p_group_formation_timeout,
                                   wpa_s->p2pdev, NULL);
        }
    ...
    {
        if (wpa_s->p2pdev->p2p_oob_dev_pw_id != DEV_PW_NFC_CONNECTION_HANDOVER && !wpa_s->p2pdev->p2p_oob_dev_pw)
        {
            wpa_printf(MSG_DEBUG, "P2P: No NFC Dev Pw known");
            return;
        }
        wpas_ap_wps_add_nfc_pw(
            wpa_s, wpa_s->p2pdev->p2p_oob_dev_pw_id, wpa_s->p2pdev->p2p_oob_dev_pw,
            wpa_s->p2pdev->p2p_peer_oob_pk_hash_known ? wpa_s->p2pdev->p2p_peer_oob_pubkey_hash : NULL);
#endif /* CONFIG_WPS_NFC */
    }
                                         ...
                                         u8 *op_edmg_channel)
{
    ...
    hwmode = get_mode(wpa_s->hw.modes, wpa_s->hw.num_modes, HOSTAPD_MODE_IEEE80211AD, false);
    if (!hwmode)
    {
        wpa_printf(MSG_ERROR, "Unsupported AP mode: HOSTAPD_MODE_IEEE80211AD");
        return -1;
    }
    ...
    {
        if (ieee80211_chaninfo_to_channel(freq, chanwidth[i], 0, op_class, op_edmg_channel) < 0)
            continue;

        hostapd_encode_edmg_chan(1, *op_edmg_channel, 0, &edmg);
        if (edmg.channels && ieee802_edmg_is_allowed(hwmode->edmg, edmg))
        {
            wpa_printf(MSG_DEBUG, "Freq %u to EDMG channel %u at opclass %u", freq, *op_edmg_channel, *op_class);
            return 0;
        }
    }
...
int wpas_p2p_try_edmg_channel(struct wpa_supplicant *wpa_s, struct p2p_go_neg_results *params)
{
    ...
    if (wpas_p2p_freq_to_edmg_channel(wpa_s, freq, &op_class, &op_channel) == 0)
    {
        wpa_printf(MSG_DEBUG, "Freq %d will be used to set an EDMG connection (channel=%u opclass=%u)", freq,
                   op_channel, op_class);
        params->freq = freq;
        return 0;
    }
...
static void wpas_start_wps_go(struct wpa_supplicant *wpa_s, struct p2p_go_neg_results *params, int group_formation)
{
    ...
    if (params->edmg)
    {
        ...
        if (!wpas_p2p_freq_to_edmg_channel(wpa_s, params->freq, &op_class, &op_channel))
        {
        ...
        {
            wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Could not match EDMG channel, freq %d, for GO", params->freq);
        }
    }
    ...
    if (is_6ghz_freq(ssid->frequency) && is_p2p_6ghz_capable(wpa_s->global->p2p))
    {
...
#undef C
    ...
    if (s->wps_nfc_dh_privkey && s->wps_nfc_dh_pubkey && !d->wps_nfc_pw_from_config)
    {
...
static int wpas_p2p_add_group_interface(struct wpa_supplicant *wpa_s, enum wpa_driver_if_type type)
{
    ...
    wpa_printf(MSG_DEBUG, "P2P: Created pending virtual interface %s addr " MACSTR, wpa_s->pending_interface_name,
               MAC2STR(wpa_s->pending_interface_addr));
...
static struct wpa_supplicant *wpas_p2p_init_group_interface(struct wpa_supplicant *wpa_s, int go)
{
    ...
    if (wpa_s->conf->ctrl_interface == NULL && wpa_s->parent != wpa_s && wpa_s->p2p_mgmt &&
    ...
    if (wpa_s->conf->p2p_interface_random_mac_addr)
    {
        ...
        {
            wpa_msg(group_wpa_s, MSG_INFO, "Failed to set random MAC address");
        ...
        {
            wpa_msg(group_wpa_s, MSG_INFO, "Could not update MAC address information");
            wpa_supplicant_remove_iface(wpa_s->global, group_wpa_s, 0);
            return NULL;
        }
...
static void wpas_p2p_grpform_fail_after_wps(struct wpa_supplicant *wpa_s)
{
    ...
    eloop_cancel_timeout(wpas_p2p_group_formation_timeout, wpa_s->p2pdev, NULL);
    eloop_register_timeout(0, 0, wpas_p2p_group_formation_timeout, wpa_s->p2pdev, NULL);
...
static void wpas_go_neg_completed(void *ctx, struct p2p_go_neg_results *res)
{
    ...
    wpa_msg_global(wpa_s, MSG_INFO,
                   ...
                   "role=%s "
                   "freq=%d ht40=%d peer_dev=" MACSTR " peer_iface=" MACSTR " wps_method=%s",
                   res->role_go ? "GO" : "client", res->freq, res->ht40, MAC2STR(res->peer_device_addr),
                   MAC2STR(res->peer_interface_addr), p2p_wps_method_text(res->wps_method));
    wpas_notify_p2p_go_neg_completed(wpa_s, res);
    ...
    eloop_register_timeout(15 + res->peer_config_timeout / 100, (res->peer_config_timeout % 100) * 10000,
...
{
    struct wpa_supplicant *wpa_s = ctx;
    wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_REQUEST MACSTR " dev_passwd_id=%u go_intent=%u", MAC2STR(src),
...
static void wpas_dev_found(void *ctx, const u8 *addr, const struct p2p_peer_info *info, int new_device)
{
    ...
    if (info->p2ps_instance)
    {
        ...
        while (len)
        {
            ...
            wpa_msg_global(wpa_s, MSG_INFO,
                           ...
                           wps_dev_type_bin2str(info->pri_dev_type, devtype, sizeof(devtype)), info->device_name,
                           methods, info->dev_capab, info->group_capab, id, str,
    ...
    wpa_msg_global(wpa_s, MSG_INFO,
                   ...
                   MAC2STR(addr), MAC2STR(info->p2p_device_addr),
                   wps_dev_type_bin2str(info->pri_dev_type, devtype, sizeof(devtype)), info->device_name,
                   info->config_methods, info->dev_capab, info->group_capab, wfd_dev_info_hex ? " wfd_dev_info=0x" : "",
                   wfd_dev_info_hex ? wfd_dev_info_hex : "", info->vendor_elems ? " vendor_elems=1" : "", new_device);
...
{
    struct wpa_supplicant *wpa_s = ctx;
    return wpa_drv_send_mlme(wpa_s, wpabuf_head(buf), wpabuf_len(buf), 1, freq, 0);
}
...
{
    wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_PROV_DISC_SHOW_PIN MACSTR " %08d%s", MAC2STR(peer), generated_pin,
                   params);
}
...
static void wpas_prov_disc_req(void *ctx,
                               const u8 *peer,
                               u16 config_methods,
                               const u8 *dev_addr,
                               ...
                               size_t group_id_len)
{
    ...
    if (group_id)
    {
        for (group = wpa_s->global->ifaces; group; group = group->next)
        {
            ...
            if (s != NULL && s->mode == WPAS_MODE_P2P_GO && group_id_len - ETH_ALEN == s->ssid_len &&
    ...
    res = os_snprintf(params, sizeof(params),
                      ...
                      MAC2STR(dev_addr), wps_dev_type_bin2str(pri_dev_type, devtype, sizeof(devtype)), dev_name,
                      supp_config_methods, dev_capab, group_capab, group ? " group=" : "", group ? group->ifname : "");
    ...
    if (config_methods & WPS_CONFIG_DISPLAY)
    {
        if (wps_generate_pin(&generated_pin) < 0)
        {
            ...
            wpas_notify_p2p_provision_discovery(wpa_s, peer, 0 /* response */, P2P_PROV_DISC_INFO_UNAVAILABLE, 0, 0);
            return;
        }
    ...
    wpas_notify_p2p_provision_discovery(wpa_s, peer, 1 /* request */, P2P_PROV_DISC_SUCCESS, config_methods,
                                        generated_pin);
}
...
static void wpas_prov_disc_resp(void *ctx, const u8 *peer, u16 config_methods)
{
    ...
    if (wpa_s->pending_pd_before_join && (os_memcmp(peer, wpa_s->pending_join_dev_addr, ETH_ALEN) == 0 ||
    ...
    wpas_notify_p2p_provision_discovery(wpa_s, peer, 0 /* response */, P2P_PROV_DISC_SUCCESS, config_methods,
                                        generated_pin);
}

static void wpas_prov_disc_fail(void *ctx,
                                const u8 *peer,
                                ...
                                const char *deferred_session_resp)
{
    ...
    if (wpa_s->p2p_fallback_to_go_neg)
    {
        ...
        wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_FALLBACK_TO_GO_NEG "reason=PD-failed");
        wpas_p2p_fallback_to_go_neg(wpa_s, 0);
        return;
    }
    ...
    {
        wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_PROV_DISC_FAILURE " p2p_dev_addr=" MACSTR " status=%d adv_id=%x",
                       MAC2STR(peer), status, adv_id);
    }
    else
    {
        wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_PROV_DISC_FAILURE " p2p_dev_addr=" MACSTR " status=%d", MAC2STR(peer),
                       status);
    }

    wpas_notify_p2p_provision_discovery(wpa_s, peer, 0 /* response */, status, 0, 0);
}

static int freq_included(struct wpa_supplicant *wpa_s, const struct p2p_channels *channels, unsigned int freq)
{
    if ((channels == NULL || p2p_channels_includes_freq(channels, freq)) && wpas_p2p_go_is_peer_freq(wpa_s, freq))
        return 1;
    return 0;
}
...
static int wpas_sta_check_ecsa(struct hostapd_data *hapd, struct sta_info *sta, void *ctx)
{
...
static u8 wpas_invitation_process(void *ctx,
                                  const u8 *sa,
                                  const u8 *bssid,
                                  const u8 *go_dev_addr,
                                  const u8 *ssid,
                                  ...
                                  int dev_pw_id)
{
    ...
    if (!persistent_group)
    {
        wpa_printf(MSG_DEBUG, "P2P: Invitation from " MACSTR " to join an active group (SSID: %s)", MAC2STR(sa),
    ...
    for (s = wpa_s->conf->ssid; s; s = s->next)
    {
        if (s->disabled == 2 && os_memcmp(s->bssid, go_dev_addr, ETH_ALEN) == 0 && s->ssid_len == ssid_len &&
    ...
    if (!s)
    {
        wpa_printf(MSG_DEBUG, "P2P: Invitation from " MACSTR " requested reinvocation of an unknown group",
                   MAC2STR(sa));
...
accept_inv:
    ...
    if (best_freq > 0)
    {
        ...
        {
            wpa_printf(MSG_DEBUG,
                       "P2P: No extra channels available - trying to force channel to match a channel already used by "
...
static void wpas_invitation_received(void *ctx,
                                     const u8 *sa,
                                     const u8 *bssid,
                                     const u8 *ssid,
                                     size_t ssid_len,
                                     ...
                                     int op_freq)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "P2P: Invitation from peer " MACSTR " was accepted; op_freq=%d MHz, SSID=%s", MAC2STR(sa),
                   op_freq, wpa_ssid_txt(ssid, ssid_len));
        if (s)
        {
            int go = s->mode == WPAS_MODE_P2P_GO;
            if (go)
            {
                wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_ACCEPTED "sa=" MACSTR " persistent=%d freq=%d",
                               MAC2STR(sa), s->id, op_freq);
            }
            else
            {
                wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_ACCEPTED "sa=" MACSTR " persistent=%d",
                               MAC2STR(sa), s->id);
            }
            wpas_p2p_group_add_persistent(wpa_s, s, go, 0, op_freq, 0, wpa_s->conf->p2p_go_ht40,
                                          wpa_s->conf->p2p_go_vht, 0, wpa_s->conf->p2p_go_he, wpa_s->conf->p2p_go_edmg,
                                          NULL, go ? P2P_MAX_INITIAL_CONN_WAIT_GO_REINVOKE : 0, 1,
        ...
        else if (bssid)
        {
            ...
            wpa_msg_global(wpa_s, MSG_INFO,
                           P2P_EVENT_INVITATION_ACCEPTED "sa=" MACSTR " go_dev_addr=" MACSTR " bssid=" MACSTR
                                                         " unknown-network",
    ...
    if (!s)
    {
        ...
        {
            wpa_msg_global(wpa_s, MSG_INFO,
                           P2P_EVENT_INVITATION_RECEIVED "sa=" MACSTR " go_dev_addr=" MACSTR " bssid=" MACSTR
                                                         " unknown-network",
                           MAC2STR(sa), MAC2STR(go_dev_addr), MAC2STR(bssid));
        }
        else
        {
            wpa_msg_global(wpa_s, MSG_INFO,
                           P2P_EVENT_INVITATION_RECEIVED "sa=" MACSTR " go_dev_addr=" MACSTR " unknown-network",
                           MAC2STR(sa), MAC2STR(go_dev_addr));
        }
        wpas_notify_p2p_invitation_received(wpa_s, sa, go_dev_addr, bssid, 0, op_freq);
        return;
    }

    if (s->mode == WPAS_MODE_P2P_GO && op_freq)
    {
        wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RECEIVED "sa=" MACSTR " persistent=%d freq=%d",
                       MAC2STR(sa), s->id, op_freq);
    }
    else
    {
        wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RECEIVED "sa=" MACSTR " persistent=%d", MAC2STR(sa),
                       s->id);
    }
    wpas_notify_p2p_invitation_received(wpa_s, sa, go_dev_addr, bssid, s->id, op_freq);
}
...
static void wpas_remove_persistent_peer(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid, const u8 *peer, int inv)
{
    ...
    os_memmove(ssid->p2p_client_list + i * 2 * ETH_ALEN, ssid->p2p_client_list + (i + 1) * 2 * ETH_ALEN,
...
static void wpas_remove_persistent_client(struct wpa_supplicant *wpa_s, const u8 *peer)
{
    ...
    ssid = wpas_p2p_get_persistent(wpa_s->p2pdev, peer, ssid->ssid, ssid->ssid_len);
    wpas_remove_persistent_peer(wpa_s, ssid, peer, 1);
}

static void wpas_invitation_result(void *ctx,
                                   int status,
                                   const u8 *bssid,
                                   const struct p2p_channels *channels,
                                   const u8 *peer,
                                   ...
                                   int peer_oper_freq)
{
    ...
    if (wpa_s->pending_invite_ssid_id == -1)
    {
            ...
            group_if->current_ssid->mode == WPAS_MODE_P2P_GO)
        {
            ...
            if (group_if->p2p_go_csa_on_inv)
            {
                ...
                eloop_cancel_timeout(wpas_p2p_reconsider_moving_go, wpa_s, NULL);
                eloop_register_timeout(0, 50000, wpas_p2p_reconsider_moving_go, wpa_s, NULL);
            }
#endif /* CONFIG_TESTING_OPTIONS */
        }
        return;
    ...
    if (neg_freq > 0 && ssid->mode == WPAS_MODE_P2P_GO && freq_included(wpa_s, channels, neg_freq))
        freq = neg_freq;
    else if (peer_oper_freq > 0 && ssid->mode != WPAS_MODE_P2P_GO && freq_included(wpa_s, channels, peer_oper_freq))
        freq = peer_oper_freq;
    else
        freq = 0;

    wpa_printf(MSG_DEBUG, "P2P: Persistent group invitation success - op_freq=%d MHz SSID=%s", freq,
               wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
    wpas_p2p_group_add_persistent(wpa_s, ssid, ssid->mode == WPAS_MODE_P2P_GO, wpa_s->p2p_persistent_go_freq, freq,
                                  wpa_s->p2p_go_vht_center_freq2, wpa_s->p2p_go_ht40, wpa_s->p2p_go_vht,
                                  wpa_s->p2p_go_max_oper_chwidth, wpa_s->p2p_go_he, wpa_s->p2p_go_edmg, channels,
                                  ssid->mode == WPAS_MODE_P2P_GO ? P2P_MAX_INITIAL_CONN_WAIT_GO_REINVOKE : 0, 1,
                                  is_p2p_allow_6ghz(wpa_s->global->p2p));
}
...
static enum chan_allowed has_channel(
    struct wpa_global *global, struct hostapd_hw_modes *mode, u8 op_class, u8 chan, int *flags)
{
    int i;
    unsigned int freq;
...
static int wpas_p2p_get_center_80mhz(
    struct wpa_supplicant *wpa_s, struct hostapd_hw_modes *mode, u8 channel, const u8 *center_channels, size_t num_chan)
{
    size_t i;
...
static const u8 center_channels_5ghz_80mhz[] = {42, 58, 106, 122, 138, 155, 171};
static const u8 center_channels_6ghz_80mhz[] = {7, 23, 39, 55, 71, 87, 103, 119, 135, 151, 167, 183, 199, 215};

static enum chan_allowed wpas_p2p_verify_80mhz(
    struct wpa_supplicant *wpa_s, struct hostapd_hw_modes *mode, u8 op_class, u8 channel, u8 bw)
{
    u8 center_chan;
    int i, flags;
    ...
    center_chan = wpas_p2p_get_center_80mhz(wpa_s, mode, channel, chans, num_chans);
    if (!center_chan)
        return NOT_ALLOWED;
    if (!wpa_s->p2p_go_allow_dfs && !is_6ghz && center_chan >= 58 && center_chan <= 138)
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        res = has_channel(wpa_s->global, mode, op_class, adj_chan, &flags);
        if (res == NOT_ALLOWED)
            return NOT_ALLOWED;
        if (res == RADAR)
            ret = RADAR;
        ...
        else if (is_6ghz && (!(wpas_get_6ghz_he_chwidth_capab(mode) & HE_PHYCAP_CHANNEL_WIDTH_SET_40MHZ_80MHZ_IN_5G)))
        {
            return NOT_ALLOWED;
        }
    }
...
static int wpas_p2p_get_center_160mhz(
    struct wpa_supplicant *wpa_s, struct hostapd_hw_modes *mode, u8 channel, const u8 *center_channels, size_t num_chan)
{
    unsigned int i;
    ...
    for (i = 0; i < num_chan; i++)
         ...
         */
        if (channel >= center_channels[i] - 14 && channel <= center_channels[i] + 14)
            return center_channels[i];
...
static const u8 center_channels_6ghz_160mhz[] = {15, 47, 79, 111, 143, 175, 207};
    ...
    struct wpa_supplicant *wpa_s, struct hostapd_hw_modes *mode, u8 op_class, u8 channel, u8 bw)
{
    ...
    center_chan = wpas_p2p_get_center_160mhz(wpa_s, mode, channel, chans, num_chans);
    if (!center_chan)
        return NOT_ALLOWED;
        ...
        if (!is_6ghz_op_class(op_class))
        {
            ...
            if (i == 7 && !(flags & HOSTAPD_CHAN_VHT_150_10))
                return NOT_ALLOWED;
                 ...
                 (!(wpas_get_6ghz_he_chwidth_capab(mode) & HE_PHYCAP_CHANNEL_WIDTH_SET_160MHZ_IN_5G)))
        {
            return NOT_ALLOWED;
        }
    }
...
static enum chan_allowed wpas_p2p_verify_edmg(struct wpa_supplicant *wpa_s, struct hostapd_hw_modes *mode, u8 channel)
{
    struct ieee80211_edmg_config edmg;
...
static enum chan_allowed wpas_p2p_verify_channel(
    struct wpa_supplicant *wpa_s, struct hostapd_hw_modes *mode, u8 op_class, u8 channel, u8 bw)
{
    int flag = 0;
    ...
    if (bw == BW40MINUS)
    {
        if (!(flag & HOSTAPD_CHAN_HT40MINUS))
            return NOT_ALLOWED;
        res2 = has_channel(wpa_s->global, mode, op_class, channel - 4, NULL);
    }
    else if (bw == BW40PLUS)
    {
    ...
    {
        res2 = wpas_p2p_verify_80mhz(wpa_s, mode, op_class, channel, bw);
    }
    else if (bw == BW160)
    {
        res2 = wpas_p2p_verify_160mhz(wpa_s, mode, op_class, channel, bw);
    }
                                   ...
                                   bool p2p_disable_6ghz)
{
    ...
    for (op = 0; global_op_class[op].op_class; op++)
    {
        ...
        if (o->p2p == NO_P2P_SUPP || (is_6ghz_op_class(o->op_class) && p2p_disable_6ghz))
            continue;

        mode = get_mode(wpa_s->hw.modes, wpa_s->hw.num_modes, o->mode, is_6ghz_op_class(o->op_class));
        if (mode == NULL)
            continue;
        ...
        for (ch = o->min_chan; ch <= o->max_chan; ch += o->inc)
        {
             ...
             * incrementation */
            if ((o->op_class >= 128 && o->op_class <= 130) && ch < 149 && ch + o->inc > 149)
                ch = 149;

            res = wpas_p2p_verify_channel(wpa_s, mode, o->op_class, ch, o->bw);
            if (res == ALLOWED)
            {
                if (reg == NULL)
                {
            ...
            else if (res == NO_IR && wpa_s->conf->p2p_add_cli_chan)
            {
                ...
                if (cli_reg->channels == P2P_MAX_REG_CLASS_CHANNELS)
                    continue;
...
int wpas_p2p_get_sec_channel_offset_40mhz(struct wpa_supplicant *wpa_s, struct hostapd_hw_modes *mode, u8 channel)
{
    int op;
    ...
    for (op = 0; global_op_class[op].op_class; op++)
    {
        ...
        if ((o->p2p == NO_P2P_SUPP && (!is_dfs_global_op_class(o->op_class) || !wpa_s->p2p_go_allow_dfs)) ||
            (is_6ghz_op_class(o->op_class) && wpa_s->conf->p2p_6ghz_disable))
            continue;

        if (is_6ghz_op_class(o->op_class) && o->bw == BW40 && get_6ghz_sec_channel(channel) < 0)
            chan = channel - 4;

        for (ch = o->min_chan; ch <= o->max_chan; ch += o->inc)
        {
            if (o->mode != HOSTAPD_MODE_IEEE80211A || (o->bw != BW40PLUS && o->bw != BW40MINUS && o->bw != BW40) ||
                ch != chan)
                continue;
            ret = wpas_p2p_verify_channel(wpa_s, mode, o->op_class, ch, o->bw);
            if (ret == ALLOWED)
            {
...
int wpas_p2p_get_vht80_center(struct wpa_supplicant *wpa_s, struct hostapd_hw_modes *mode, u8 channel, u8 op_class)
{
    const u8 *chans;
    size_t num_chans;
    enum chan_allowed ret;
    ...
    return wpas_p2p_get_center_80mhz(wpa_s, mode, channel, chans, num_chans);
}

int wpas_p2p_get_vht160_center(struct wpa_supplicant *wpa_s, struct hostapd_hw_modes *mode, u8 channel, u8 op_class)
{
    const u8 *chans;
    size_t num_chans;
    enum chan_allowed ret;
    ...
    return wpas_p2p_get_center_160mhz(wpa_s, mode, channel, chans, num_chans);
}
...
int wpas_p2p_add_p2pdev_interface(struct wpa_supplicant *wpa_s, const char *conf_p2p_dev)
{
    ...
    ret = wpa_drv_if_add(wpa_s, WPA_IF_P2P_DEVICE, ifname, if_addr, NULL, force_name, wpa_s->pending_interface_addr,
                         NULL);
    if (ret < 0)
    {
...
static int wpas_get_persistent_group(void *ctx,
                                     const u8 *addr,
                                     const u8 *ssid,
                                     size_t ssid_len,
                                     u8 *go_dev_addr,
...
static int wpas_get_go_info(
    void *ctx, u8 *intended_addr, u8 *ssid, size_t *ssid_len, int *group_iface, unsigned int *freq)
{
    struct wpa_supplicant *wpa_s = ctx;
...
static int wpas_remove_stale_groups(void *ctx, const u8 *peer, const u8 *go, const u8 *ssid, size_t ssid_len)
{
    ...
    {
        if (go && ssid && ssid_len && s->ssid_len == ssid_len && os_memcmp(go, s->bssid, ETH_ALEN) == 0 &&
            os_memcmp(ssid, s->ssid, ssid_len) == 0)
            break;
        ...
        {
            wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Remove stale persistent group id=%d", s->id);
...
static void wpas_p2ps_get_feat_cap_str(char *buf, size_t buf_len, const u8 *feat_cap, size_t feat_cap_len)
{
    ...
    ret = wpa_snprintf_hex(&buf[sizeof(pref) - 1], buf_len - sizeof(pref) + 1, feat_cap, feat_cap_len);

    if (ret != (2 * (int)feat_cap_len))
        wpa_printf(MSG_WARNING, "P2PS feature_cap bytes truncated");
}

static void wpas_p2ps_prov_complete(void *ctx,
                                    u8 status,
                                    const u8 *dev,
                                    const u8 *adv_mac,
                                    const u8 *ses_mac,
                                    ...
                                    size_t group_ssid_len)
{
    ...
    wpas_p2ps_get_feat_cap_str(feat_cap_str, sizeof(feat_cap_str), feat_cap, feat_cap_len);
    ...
    if (prov_start)
    {
        if (session_info == NULL)
        {
            wpa_msg_global(wpa_s, MSG_INFO,
                           ...
                           " adv_id=%x conncap=%x"
                           " adv_mac=" MACSTR " session=%x mac=" MACSTR " dev_passwd_id=%d%s",
                           MAC2STR(dev), adv_id, conncap, MAC2STR(adv_mac), ses_id, MAC2STR(ses_mac), passwd_id,
                           feat_cap_str);
        }
        else
        {
            wpa_msg_global(wpa_s, MSG_INFO,
                           P2P_EVENT_P2PS_PROVISION_START MACSTR
                           " adv_id=%x conncap=%x"
                           " adv_mac=" MACSTR " session=%x mac=" MACSTR " dev_passwd_id=%d info='%s'%s",
                           MAC2STR(dev), adv_id, conncap, MAC2STR(adv_mac), ses_id, MAC2STR(ses_mac), passwd_id,
                           session_info, feat_cap_str);
        }
        return;
    }
    ...
    if (status && status != P2P_SC_SUCCESS_DEFERRED)
    {
        ...
        if (persistent_go && !persistent_go->num_p2p_clients)
        {
            ...
            wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Remove empty persistent group id=%d", persistent_go->id);
        ...
        wpa_msg_global(wpa_s, MSG_INFO,
                       ...
                       MAC2STR(dev), status, adv_id, MAC2STR(adv_mac), ses_id, MAC2STR(ses_mac), feat_cap_str);
        return;
    }
    ...
    if (persist_ssid && s && s->mode != WPAS_MODE_P2P_GO && is_zero_ether_addr(grp_mac))
    {
    ...
    for (;;)
    {
        ...
        if (s && s->ssid_len == stale->ssid_len && os_memcmp(stale->bssid, s->bssid, ETH_ALEN) == 0 &&
        ...
        {
            wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Remove stale persistent group id=%d", stale->id);
        ...
        else
        {
            ...
            for (i = 0; i < stale->num_p2p_clients; i++)
            {
                ...
                {
                    os_memmove(stale->p2p_client_list + i * ETH_ALEN, stale->p2p_client_list + (i + 1) * ETH_ALEN,
    ...
    if (s)
    {
        ...
        wpa_msg_global(wpa_s, MSG_INFO,
                       P2P_EVENT_P2PS_PROVISION_DONE MACSTR
                       " status=%d"
                       " adv_id=%x adv_mac=" MACSTR " session=%x mac=" MACSTR " persist=%d%s",
                       MAC2STR(dev), status, adv_id, MAC2STR(adv_mac), ses_id, MAC2STR(ses_mac), s->id, feat_cap_str);
        return;
    }
    ...
    if (conncap == P2PS_SETUP_GROUP_OWNER)
    {
        ...
        if (!go_wpa_s)
        {
            ...
            if (!go_ifname[0])
            {
                wpas_p2ps_prov_complete(wpa_s, P2P_SC_FAIL_UNKNOWN_GROUP, dev, adv_mac, ses_mac, grp_mac, adv_id,
                                        ses_id, 0, 0, NULL, 0, 0, 0, NULL, NULL, 0, 0, NULL, 0);
                return;
            }
            ...
            if (response_done && persistent_go)
            {
                wpas_p2p_group_add_persistent(
                    wpa_s, persistent_go, 0, 0, freq, 0, 0, 0, 0, 0, 0, NULL,
                    persistent_go->mode == WPAS_MODE_P2P_GO ? P2P_MAX_INITIAL_CONN_WAIT_GO_REINVOKE : 0, 0, false);
            }
            else if (response_done)
            {
                wpas_p2p_group_add(wpa_s, 1, freq, 0, 0, 0, 0, 0, 0, false);
            }
        ...
        else if (passwd_id == DEV_PW_P2PS_DEFAULT)
        {
            ...
            {
                wpa_dbg(go_wpa_s, MSG_DEBUG, "P2P: Setting PIN-1 for " MACSTR, MAC2STR(grp_mac));
        ...
        wpa_msg_global(wpa_s, MSG_INFO,
                       P2P_EVENT_P2PS_PROVISION_DONE MACSTR
                       " status=%d conncap=%x"
                       " adv_id=%x adv_mac=" MACSTR " session=%x mac=" MACSTR " dev_passwd_id=%d go=%s%s",
                       MAC2STR(dev), status, conncap, adv_id, MAC2STR(adv_mac), ses_id, MAC2STR(ses_mac), passwd_id,
                       go_ifname, feat_cap_str);
        return;
    }
    ...
    if (conncap == P2PS_SETUP_CLIENT)
    {
        ...
        wpa_msg_global(wpa_s, MSG_INFO,
                       P2P_EVENT_P2PS_PROVISION_DONE MACSTR
                       " status=%d conncap=%x"
                       " adv_id=%x adv_mac=" MACSTR " session=%x mac=" MACSTR " dev_passwd_id=%d join=" MACSTR "%s%s%s",
                       MAC2STR(dev), status, conncap, adv_id, MAC2STR(adv_mac), ses_id, MAC2STR(ses_mac), passwd_id,
                       MAC2STR(grp_mac), feat_cap_str, group_ssid ? " group_ssid=" : "", ssid_hex);
    }
    else
    {
        wpa_msg_global(wpa_s, MSG_INFO,
                       P2P_EVENT_P2PS_PROVISION_DONE MACSTR
                       " status=%d conncap=%x"
                       " adv_id=%x adv_mac=" MACSTR " session=%x mac=" MACSTR " dev_passwd_id=%d%s",
                       MAC2STR(dev), status, conncap, adv_id, MAC2STR(adv_mac), ses_id, MAC2STR(ses_mac), passwd_id,
                       feat_cap_str);
    }
}
...
static int wpas_prov_disc_resp_cb(void *ctx)
{
    ...
    {
        wpas_p2p_group_add_persistent(
            wpa_s, persistent_go, 0, 0, 0, 0, 0, 0, 0, 0, 0, NULL,
            persistent_go->mode == WPAS_MODE_P2P_GO ? P2P_MAX_INITIAL_CONN_WAIT_GO_REINVOKE : 0, 0,
            is_p2p_allow_6ghz(wpa_s->global->p2p));
    }
    else
    {
        wpas_p2p_group_add(wpa_s, 1, freq, 0, 0, 0, 0, 0, 0, is_p2p_allow_6ghz(wpa_s->global->p2p));
    }

    return 1;
}

static int wpas_p2p_get_pref_freq_list(void *ctx, int go, unsigned int *len, unsigned int *freq_list)
{
    struct wpa_supplicant *wpa_s = ctx;

    return wpa_drv_get_pref_freq_list(wpa_s, go ? WPA_IF_P2P_GO : WPA_IF_P2P_CLIENT, len, freq_list);
}
...
int wpas_p2p_mac_setup(struct wpa_supplicant *wpa_s)
{
    ...
    if (wpa_s->conf->p2p_device_random_mac_addr == 2)
    {
        if (is_zero_ether_addr(wpa_s->conf->p2p_device_persistent_mac_addr) && !is_zero_ether_addr(wpa_s->own_addr))
        {
            os_memcpy(wpa_s->conf->p2p_device_persistent_mac_addr, wpa_s->own_addr, ETH_ALEN);
        }
        return 0;
    }
    ...
    if (!wpa_s->conf->ssid)
    {
        ...
        {
            wpa_msg(wpa_s, MSG_INFO, "Failed to generate random MAC address");
            return -EINVAL;
        }

        /* Store generated MAC address. */
        os_memcpy(wpa_s->conf->p2p_device_persistent_mac_addr, addr, ETH_ALEN);
    }
    else
    {
        ...
        if (is_zero_ether_addr(wpa_s->conf->p2p_device_persistent_mac_addr))
            return 0;
        os_memcpy(addr, wpa_s->conf->p2p_device_persistent_mac_addr, ETH_ALEN);
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
    ...
    {
        wpa_msg(wpa_s, MSG_ERROR, "Failed to initialize P2P random MAC address.");
        return -1;
    }
    ...
    if (wpas_p2p_setup_channels(wpa_s, &p2p.channels, &p2p.cli_channels, p2p.p2p_6ghz_disable))
    {
    ...
    else
    {
         ...
         * channel.
         */
        if (p2p_config_get_random_social(&p2p, &p2p.reg_class, &p2p.channel, &global->p2p_go_avoid_freq,
                                         &global->p2p_disallow_freq) != 0)
        {
    ...
    else
    {
         ...
         */
        if (p2p_config_get_random_social(&p2p, &p2p.op_reg_class, &p2p.op_channel, NULL, NULL) != 0)
        {
            wpa_printf(MSG_INFO, "P2P: Failed to select random social channel as operation channel");
            p2p.op_reg_class = 0;
            p2p.op_channel   = 0;
                                 ...
                                 unsigned int pref_freq)
{
    ...
    return p2p_connect(wpa_s->global->p2p, peer_addr, wps_method, go_intent, own_interface_addr, force_freq,
                       persistent_group, ssid ? ssid->ssid : NULL, ssid ? ssid->ssid_len : 0,
                       wpa_s->p2p_pd_before_go_neg, pref_freq, wps_method == WPS_NFC ? wpa_s->p2p_oob_dev_pw_id : 0);
}
                                ...
                                unsigned int pref_freq)
{
    ...
    return p2p_authorize(wpa_s->global->p2p, peer_addr, wps_method, go_intent, own_interface_addr, force_freq,
                         persistent_group, ssid ? ssid->ssid : NULL, ssid ? ssid->ssid_len : 0, pref_freq,
...
static void wpas_p2p_check_join_scan_limit(struct wpa_supplicant *wpa_s)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "P2P: Failed to find GO " MACSTR " for join operationg - stop join attempt",
                   MAC2STR(wpa_s->pending_join_iface_addr));
        eloop_cancel_timeout(wpas_p2p_join_scan, wpa_s, NULL);
        if (wpa_s->p2p_auto_pd)
        {
            wpa_s->p2p_auto_pd = 0;
            wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_PROV_DISC_FAILURE " p2p_dev_addr=" MACSTR " status=N/A",
                           MAC2STR(wpa_s->pending_join_dev_addr));
            return;
        }
        if (wpa_s->p2p_fallback_to_go_neg)
        {
            wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Join operation failed - fall back to GO Negotiation");
            wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_FALLBACK_TO_GO_NEG "reason=join-failed");
            wpas_p2p_fallback_to_go_neg(wpa_s, 0);
            return;
        }
        wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_GROUP_FORMATION_FAILURE);
...
static int wpas_p2p_peer_go(struct wpa_supplicant *wpa_s, const u8 *peer_dev_addr)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               bss->last_update.sec, bss->last_update.usec, updated ? "" : "not ");
...
static void wpas_p2p_scan_res_join(struct wpa_supplicant *wpa_s, struct wpa_scan_results *scan_res)
{
    ...
    wpa_printf(MSG_DEBUG, "P2P: Scan results received (%d BSS) for %sjoin", scan_res ? (int)scan_res->num : -1,
    ...
    if (wpa_s->p2p_auto_pd)
    {
        ...
        if (join == 0 && wpa_s->auto_pd_scan_retry < P2P_AUTO_PD_SCAN_ATTEMPTS)
        {
            ...
            if (bss)
            {
                ...
                wpa_printf(MSG_DEBUG,
                           ...
                           wpa_s->auto_pd_scan_retry, MAC2STR(wpa_s->pending_join_dev_addr), freq);
        ...
        if (p2p_prov_disc_req(wpa_s->global->p2p, wpa_s->pending_join_dev_addr, NULL, wpa_s->pending_pd_config_methods,
                              join, 0, wpa_s->user_initiated_pd) < 0)
        {
            wpa_s->p2p_auto_pd = 0;
            wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_PROV_DISC_FAILURE " p2p_dev_addr=" MACSTR " status=N/A",
                           MAC2STR(wpa_s->pending_join_dev_addr));
    ...
    if (wpa_s->p2p_auto_join)
    {
        ...
        if (join < 0)
        {
            ...
            wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_FALLBACK_TO_GO_NEG "reason=peer-not-running-GO");
            wpas_p2p_connect(wpa_s, wpa_s->pending_join_dev_addr, wpa_s->p2p_pin, wpa_s->p2p_wps_method,
                             wpa_s->p2p_persistent_group, 0, 0, 0, wpa_s->p2p_go_intent, wpa_s->p2p_connect_freq,
                             wpa_s->p2p_go_vht_center_freq2, wpa_s->p2p_persistent_id, wpa_s->p2p_pd_before_go_neg,
                             wpa_s->p2p_go_ht40, wpa_s->p2p_go_vht, wpa_s->p2p_go_max_oper_chwidth, wpa_s->p2p_go_he,
                             wpa_s->p2p_go_edmg, NULL, 0, is_p2p_allow_6ghz(wpa_s->global->p2p));
            return;
        }
        ...
        if (!join)
        {
            wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_FALLBACK_TO_GO_NEG_ENABLED);
            wpa_s->p2p_fallback_to_go_neg = 1;
        }
    }

    freq = p2p_get_oper_freq(wpa_s->global->p2p, wpa_s->pending_join_iface_addr);
    if (freq < 0 && p2p_get_interface_addr(wpa_s->global->p2p, wpa_s->pending_join_dev_addr, iface_addr) == 0 &&
        ...
        !wpa_bss_get_bssid(wpa_s, wpa_s->pending_join_iface_addr))
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   "address for join from " MACSTR " to " MACSTR " based on newly discovered P2P peer entry",
    ...
    {
        wpa_printf(MSG_DEBUG, "P2P: Trying to find target GO BSS entry based on BSSID " MACSTR " and SSID %s",
                   MAC2STR(wpa_s->pending_join_iface_addr),
    ...
    {
        wpa_printf(MSG_DEBUG, "P2P: Trying to find target GO BSS entry based on BSSID " MACSTR,
                   MAC2STR(wpa_s->pending_join_iface_addr));
    ...
    if (bss)
    {
        ...
        if (p2p_parse_dev_addr(wpa_bss_ie_ptr(bss), bss->ie_len, dev_addr) == 0 &&
            os_memcmp(wpa_s->pending_join_dev_addr, wpa_s->pending_join_iface_addr, ETH_ALEN) == 0 &&
    ...
    if (freq > 0)
    {
        ...
        {
            wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_GROUP_FORMATION_FAILURE "reason=FREQ_CONFLICT");
        ...
        if (p2p_prov_disc_req(wpa_s->global->p2p, wpa_s->pending_join_dev_addr, NULL, method, 1, freq,
                              wpa_s->user_initiated_pd) < 0)
        {
            wpa_printf(MSG_DEBUG,
...
static void wpas_p2p_join_scan_req(struct wpa_supplicant *wpa_s, int freq, const u8 *ssid, size_t ssid_len)
{
    ...
    else if (wpa_s->conf->p2p_6ghz_disable || !is_p2p_allow_6ghz(wpa_s->global->p2p))
    {
        wpa_printf(MSG_DEBUG, "P2P: 6 GHz disabled - update the scan frequency list");
        wpa_add_scan_freqs_list(wpa_s, HOSTAPD_MODE_IEEE80211G, &params, 0);
        wpa_add_scan_freqs_list(wpa_s, HOSTAPD_MODE_IEEE80211A, &params, 0);
    }
...
static int wpas_p2p_join(struct wpa_supplicant *wpa_s,
                         ...
                         size_t ssid_len)
{
    wpa_printf(MSG_DEBUG, "P2P: Request to join existing group (iface " MACSTR " dev " MACSTR " op_freq=%d)%s",
               MAC2STR(iface_addr), MAC2STR(dev_addr), op_freq, auto_join ? " (auto_join)" : "");
    if (ssid && ssid_len)
    {
...
static int wpas_p2p_join_start(struct wpa_supplicant *wpa_s, int freq, const u8 *ssid, size_t ssid_len)
{
    ...
    else
    {
        ...
        if (bss)
        {
            ...
            wpa_printf(MSG_DEBUG, "P2P: Join target GO operating frequency from BSS table: %d MHz (SSID %s)", bss->freq,
        ...
        else if (ssid && ssid_len)
        {
            res.ssid_len = ssid_len;
            os_memcpy(res.ssid, ssid, ssid_len);
            wpa_printf(MSG_DEBUG, "P2P: Join target GO (SSID %s)", wpa_ssid_txt(ssid, ssid_len));
        }
    }
                                ...
                                unsigned int *num_pref_freq)
{
    ...
    *num_pref_freq = 0;
    ...
    wpa_printf(MSG_DEBUG, "P2P: Setup freqs: freq=%d num_MCC=%d shared_freqs=%u num_unused=%d", freq,
               wpa_s->num_multichan_concurrent, num, num_unused);
    ...
    if (freq > 0)
    {
        ...
        if (!ret)
        {
                ...
                ieee80211_is_dfs(freq, wpa_s->hw.modes, wpa_s->hw.num_modes))
            {
                 ...
                 */
                wpa_printf(MSG_DEBUG,
                           "P2P: The forced channel for GO (%u MHz) is DFS, and DFS is offloaded to the driver", freq);
            }
            else
            {
    ...
    if (!wpa_s->conf->num_p2p_pref_chan && *pref_freq == 0)
    {
        ...
        res = wpa_drv_get_pref_freq_list(wpa_s, iface_type, &max_pref_freq, pref_freq_list);
        if (!is_p2p_allow_6ghz(wpa_s->global->p2p))
            max_pref_freq = p2p_remove_6ghz_channels(pref_freq_list, max_pref_freq);

        if (!res && max_pref_freq > 0)
        {
            *num_pref_freq = max_pref_freq;
            i              = 0;
            while (i < *num_pref_freq && (!p2p_supported_freq(wpa_s->global->p2p, pref_freq_list[i]) ||
                                          wpas_p2p_disallowed_freq(wpa_s->global, pref_freq_list[i])))
            {
                wpa_printf(MSG_DEBUG, "P2P: preferred_freq_list[%d]=%d is disallowed", i, pref_freq_list[i]);
                i++;
            }
            if (i != *num_pref_freq)
            {
                best_freq = pref_freq_list[i];
                wpa_printf(MSG_DEBUG, "P2P: Using preferred_freq_list[%d]=%d", i, best_freq);
            }
            else
            {
                wpa_printf(MSG_DEBUG, "P2P: All driver preferred frequencies are disallowed for P2P use");
                *num_pref_freq = 0;
            }
        }
        else
...
static bool is_p2p_6ghz_supported(struct wpa_supplicant *wpa_s, const u8 *peer_addr)
{
    if (wpa_s->conf->p2p_6ghz_disable || !get_mode(wpa_s->hw.modes, wpa_s->hw.num_modes, HOSTAPD_MODE_IEEE80211A, true))
        return false;
...
static int wpas_p2p_check_6ghz(struct wpa_supplicant *wpa_s, const u8 *peer_addr, bool allow_6ghz, int freq)
{
    if (allow_6ghz && is_p2p_6ghz_supported(wpa_s, peer_addr))
    {
        wpa_printf(MSG_DEBUG, "P2P: Allow connection on 6 GHz channels");
                     ...
                     bool allow_6ghz)
{
    ...
    if (join || auto_join)
    {
        ...
        if (wpas_p2p_join(wpa_s, iface_addr, dev_addr, wps_method, auto_join, freq, group_ssid, group_ssid_len) < 0)
            return -1;
        return ret;
    }

    size = P2P_MAX_PREF_CHANNELS;
    res  = wpas_p2p_setup_freqs(wpa_s, freq, &force_freq, &pref_freq, go_intent == 15, pref_freq_list, &size);
    if (res)
        return res;
    wpas_p2p_set_own_freq_preference(wpa_s, force_freq ? force_freq : pref_freq);
    ...
    if (auth)
    {
        if (wpas_p2p_auth_go_neg(wpa_s, peer_addr, wps_method, go_intent, if_addr, force_freq, persistent_group, ssid,
                                 pref_freq) < 0)
            return -1;
        return ret;
    }

    if (wpas_p2p_start_go_neg(wpa_s, peer_addr, wps_method, go_intent, if_addr, force_freq, persistent_group, ssid,
                              pref_freq) < 0)
    {
        if (wpa_s->create_p2p_iface)
            wpas_p2p_remove_pending_group_interface(wpa_s);
...
void wpas_p2p_remain_on_channel_cb(struct wpa_supplicant *wpa_s, unsigned int freq, unsigned int duration)
{
    if (wpa_s->global->p2p_disabled || wpa_s->global->p2p == NULL)
        return;
    wpa_printf(MSG_DEBUG,
               "P2P: remain-on-channel callback (off_channel_freq=%u pending_listen_freq=%d roc_waiting_drv_freq=%d "
               "freq=%u duration=%u)",
               wpa_s->off_channel_freq, wpa_s->pending_listen_freq, wpa_s->roc_waiting_drv_freq, freq, duration);
    ...
    else
    {
        wpa_printf(
            ...
            wpa_s->off_channel_freq, wpa_s->pending_listen_freq, freq, duration);
    }
}
...
int wpas_p2p_group_remove(struct wpa_supplicant *wpa_s, const char *ifname)
{
    ...
    if (os_strcmp(ifname, "*") == 0)
    {
        ...
        if (!calling_wpa_s_group_removed && (calling_wpa_s->p2p_group_interface != NOT_P2P_GROUP_INTERFACE ||
                                             (calling_wpa_s->current_ssid && calling_wpa_s->current_ssid->p2p_group)))
        {
            wpa_printf(MSG_DEBUG, "Remove calling_wpa_s P2P group");
            wpas_p2p_disconnect_safely(calling_wpa_s, calling_wpa_s);
        }
...
static int wpas_p2p_select_go_freq(struct wpa_supplicant *wpa_s, int freq)
{
    ...
    if (!wpa_s->conf->num_p2p_pref_chan && !freq)
    {
        ...
        res = wpa_drv_get_pref_freq_list(wpa_s, WPA_IF_P2P_GO, &size, pref_freq_list);
        if (!is_p2p_allow_6ghz(wpa_s->global->p2p))
        ...
        {
            i = 0;
            while (i < size && (!p2p_supported_freq(wpa_s->global->p2p, pref_freq_list[i]) ||
                                wpas_p2p_disallowed_freq(wpa_s->global, pref_freq_list[i])))
            {
                wpa_printf(MSG_DEBUG, "P2P: preferred_freq_list[%d]=%d is disallowed", i, pref_freq_list[i]);
                i++;
...
static int wpas_p2p_supported_freq_go(struct wpa_supplicant *wpa_s, const struct p2p_channels *channels, int freq)
{
    if (!wpas_p2p_disallowed_freq(wpa_s->global, freq) && p2p_supported_freq_go(wpa_s->global->p2p, freq) &&
                                   ...
                                   const struct p2p_channels *channels)
{
    ...
    if (wpa_s->current_ssid && wpa_s->current_ssid->mode == WPAS_MODE_P2P_GO && wpa_s->wpa_state == WPA_COMPLETED)
    {
        ...
        for (i = 0; i < num; i++)
        {
            if (freqs[i].freq == wpa_s->current_ssid->frequency && freqs[i].flags == 0)
            {
                ignore_no_freqs = 1;
                break;
            }
    ...
    if (freq)
    {
        if (wpas_p2p_disallowed_freq(wpa_s->global, freq) || !freq_included(wpa_s, channels, freq))
        {
            wpa_printf(MSG_DEBUG, "P2P: Forced GO freq %d MHz disallowed", freq);
            goto fail;
        }
        if (!p2p_supported_freq_go(wpa_s->global->p2p, freq))
        {
                ...
                ieee80211_is_dfs(freq, wpa_s->hw.modes, wpa_s->hw.num_modes))
            {
                 ...
                 * to the driver, allow P2P GO to use it.
                 */
                wpa_printf(MSG_DEBUG, "P2P: %s: The forced channel for GO (%u MHz) requires DFS and DFS is offloaded",
                           __func__, freq);
            }
            else
            {
        ...
        {
            wpa_printf(MSG_DEBUG, "P2P: Cannot force GO on freq (%d MHz) as all the channels are in use", freq);
            goto fail;
        }

        wpa_printf(MSG_DEBUG, "P2P: force GO freq (%d MHz) on a free channel", freq);
        params->freq = freq;
        goto success;
    }
    ...
    if (num && (!wpa_s->conf->p2p_ignore_shared_freq || !unused_channels))
    {
        ...
        for (i = 0; i < num; i++)
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "P2P: Use shared freq (%d MHz) for GO", freqs[i].freq);
    ...
    if (wpa_s->conf->p2p_oper_reg_class == 81 && wpa_s->conf->p2p_oper_channel >= 1 &&
        wpa_s->conf->p2p_oper_channel <= 11 &&
        wpas_p2p_supported_freq_go(wpa_s, channels, 2407 + 5 * wpa_s->conf->p2p_oper_channel))
    {
    ...
    if ((wpa_s->conf->p2p_oper_reg_class == 115 || wpa_s->conf->p2p_oper_reg_class == 116 ||
         wpa_s->conf->p2p_oper_reg_class == 117 || wpa_s->conf->p2p_oper_reg_class == 124 ||
         wpa_s->conf->p2p_oper_reg_class == 125 || wpa_s->conf->p2p_oper_reg_class == 126 ||
         wpa_s->conf->p2p_oper_reg_class == 127) &&
        wpas_p2p_supported_freq_go(wpa_s, channels, 5000 + 5 * wpa_s->conf->p2p_oper_channel))
    {
    ...
    if (wpa_s->hw.modes && wpa_s->p2p_group_common_freqs)
    {
        for (i = 0; i < wpa_s->p2p_group_common_freqs_num; i++)
        {
            ...
            hwmode   = get_mode(wpa_s->hw.modes, wpa_s->hw.num_modes, mode, is_6ghz_freq(cand));
            if (!hwmode || wpas_p2p_verify_channel(wpa_s, hwmode, op_class, chan, BW80) != ALLOWED)
                continue;
            if (wpas_p2p_supported_freq_go(wpa_s, channels, cand))
            {
                params->freq = cand;
                wpa_printf(MSG_DEBUG, "P2P: Use freq %d MHz common with the peer and allowing VHT80", params->freq);
                goto success;
            }
        }
    }
    ...
    if (wpa_s->current_ssid && wpa_s->hw.modes && wpa_s->p2p_group_common_freqs)
    {
        for (i = 0; i < wpa_s->p2p_group_common_freqs_num; i++)
        {
            enum hostapd_hw_mode mode;
            ...
            cand    = wpa_s->p2p_group_common_freqs[i];
            mode    = ieee80211_freq_to_chan(cand, &chan);
            hwmode  = get_mode(wpa_s->hw.modes, wpa_s->hw.num_modes, mode, is_6ghz);
            if (!wpas_same_band(wpa_s->current_ssid->frequency, cand) || !hwmode)
                continue;
            if (is_6ghz && wpas_p2p_verify_channel(wpa_s, hwmode, 132, chan, BW40) == ALLOWED)
                supported = true;

            if (!is_6ghz &&
                ieee80211_freq_to_channel_ext(cand, -1, CHANWIDTH_USE_HT, &op_class, &chan) != NUM_HOSTAPD_MODES &&
                wpas_p2p_verify_channel(wpa_s, hwmode, op_class, chan, BW40MINUS) == ALLOWED)
                supported = true;

            if (!supported && !is_6ghz &&
                ieee80211_freq_to_channel_ext(cand, 1, CHANWIDTH_USE_HT, &op_class, &chan) != NUM_HOSTAPD_MODES &&
                wpas_p2p_verify_channel(wpa_s, hwmode, op_class, chan, BW40PLUS) == ALLOWED)
                supported = true;
    ...
    if (wpa_s->current_ssid && wpa_s->p2p_group_common_freqs)
    {
        ...
        {
            cand = wpa_s->p2p_group_common_freqs[i];
            if (!wpas_same_band(wpa_s->current_ssid->frequency, cand))
                continue;
            if (wpas_p2p_supported_freq_go(wpa_s, channels, cand))
            {
                params->freq = cand;
                wpa_printf(MSG_DEBUG, "P2P: Use freq %d MHz common with the peer and maintaining same band",
                           params->freq);
                goto success;
            }
        }
    ...
    if (wpa_s->p2p_group_common_freqs)
    {
        for (i = 0; i < wpa_s->p2p_group_common_freqs_num; i++)
        {
            ...
            {
                params->freq = cand;
                wpa_printf(MSG_DEBUG, "P2P: Use freq %d MHz common with the peer", params->freq);
                goto success;
            }
        }
    }
...
static struct wpa_supplicant *wpas_p2p_get_group_iface(struct wpa_supplicant *wpa_s, int addr_allocated, int go)
{
    ...
    if (!wpas_p2p_create_iface(wpa_s))
    {
        ...
        wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Use primary interface for group operations");
                       ...
                       bool allow_6ghz)
{
    ...
    if (wpas_p2p_init_go_params(wpa_s, &params, freq, vht_center_freq2, ht40, vht, max_oper_chwidth, he, edmg, NULL))
        return -1;
...
static int wpas_start_p2p_client(
                                  ...
                                  bool allow_6ghz)
{
    ...
    if (wpas_get_p2p_group(wpa_s, ssid->ssid, ssid->ssid_len, &go) && go == (ssid->mode == WPAS_MODE_P2P_GO))
    {
        wpa_printf(MSG_DEBUG,
                   ...
                   "already running");
        if (go == 0 && eloop_cancel_timeout(wpas_p2p_group_formation_timeout, wpa_s->p2pdev, NULL))
        {
            /*
    ...
    else if (ssid->mode == WPAS_MODE_INFRA)
    {
        ...
        if (freq <= 0 || !freq_included(wpa_s, channels, freq))
        {
            ...
            os_get_reltime(&now);
            if (bss && !os_reltime_expired(&now, &bss->last_update, 5) && freq_included(wpa_s, channels, bss->freq))
                freq = bss->freq;
            else
                freq = 0;
        }
    ...
    if (wpas_p2p_init_go_params(wpa_s, &params, freq, vht_center_freq2, ht40, vht, max_oper_chwidth, he, edmg,
                                channels))
        return -1;
...
struct p2p_group *wpas_p2p_group_init(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
    ...
    if (wpa_s->global->p2p_disabled || wpa_s->global->p2p == NULL || !ssid->p2p_group)
        return NULL;
...
void wpas_p2p_wps_success(struct wpa_supplicant *wpa_s, const u8 *peer_addr, int registrar)
{
    ...
    if (ssid && ssid->mode == WPAS_MODE_INFRA)
    {
         ...
         */
        wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Re-start group formation timeout (%d seconds) as client for initial connection",
    ...
    else if (ssid)
    {
         ...
         */
        wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Re-start group formation timeout (%d seconds) as GO for initial connection",
                P2P_MAX_INITIAL_CONN_WAIT_GO);
        eloop_register_timeout(P2P_MAX_INITIAL_CONN_WAIT_GO, 0, wpas_p2p_group_formation_timeout, wpa_s->p2pdev, NULL);
        /*
...
void wpas_p2p_wps_failed(struct wpa_supplicant *wpa_s, struct wps_event_fail *fail)
{
    ...
    if (wpa_s == wpa_s->global->p2p_group_formation)
    {
        ...
        eloop_deplete_timeout(0, 50000, wpas_p2p_group_formation_timeout, wpa_s->p2pdev, NULL);
    }
}
...
int wpas_p2p_prov_disc(struct wpa_supplicant *wpa_s,
                       const u8 *peer_addr,
                       ...
                       struct p2ps_provision *p2ps_prov)
{
    ...
    {
        p2ps_prov->conncap = p2ps_group_capability(wpa_s, P2PS_SETUP_NONE, p2ps_prov->role, &p2ps_prov->force_freq,
                                                   &p2ps_prov->pref_freq);

        wpa_printf(MSG_DEBUG, "P2P: %s conncap: %d - ASP parsed: %x %x %d %s", __func__, p2ps_prov->conncap,
                   p2ps_prov->adv_id, p2ps_prov->conncap, p2ps_prov->status, p2ps_prov->info);
    ...
    if (use == WPAS_P2P_PD_AUTO)
    {
        ...
        wpa_s->p2p_join_scan_count = 0;
        os_get_reltime(&wpa_s->p2p_auto_started);
        wpa_printf(MSG_DEBUG, "P2P: Auto PD started at %lld.%06lld", wpa_s->p2p_auto_started.sec,
    ...
    return p2p_prov_disc_req(wpa_s->global->p2p, peer_addr, p2ps_prov, config_methods, use == WPAS_P2P_PD_FOR_JOIN, 0,
                             1);
}
...
int wpas_p2p_find(struct wpa_supplicant *wpa_s,
                  ...
                  bool include_6ghz)
{
    ...
    if (wpa_s->global->p2p_disabled || wpa_s->global->p2p == NULL || wpa_s->p2p_in_provisioning)
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Reject p2p_find operation%s%s",
                (wpa_s->global->p2p_disabled || !wpa_s->global->p2p) ? " (P2P disabled)" : "",
                wpa_s->p2p_in_provisioning ? " (p2p_in_provisioning)" : "");
        return -1;
    }

    wpa_supplicant_cancel_sched_scan(wpa_s);

    return p2p_find(wpa_s->global->p2p, timeout, type, num_req_dev_types, req_dev_types, dev_id, search_delay, seek_cnt,
                    seek_string, freq, include_6ghz);
}
...
int wpas_p2p_probe_req_rx(struct wpa_supplicant *wpa_s,
                          const u8 *addr,
                          const u8 *dst,
                          const u8 *bssid,
                          const u8 *ie,
                          ...
                          int ssi_signal)
{
    ...
    switch (p2p_probe_req_rx(wpa_s->global->p2p, addr, dst, bssid, ie, ie_len, rx_freq, wpa_s->p2p_lo_started))
    {
...
void wpas_p2p_rx_action(struct wpa_supplicant *wpa_s,
                        const u8 *da,
                        const u8 *sa,
                        const u8 *bssid,
                        u8 category,
...
int wpas_p2p_invite(struct wpa_supplicant *wpa_s,
                    const u8 *peer_addr,
                    struct wpa_ssid *ssid,
                    const u8 *go_dev_addr,
                    int freq,
                    ...
                    bool allow_6ghz)
{
    ...
    size = P2P_MAX_PREF_CHANNELS;
    res = wpas_p2p_setup_freqs(wpa_s, freq, &force_freq, &pref_freq, role == P2P_INVITE_ROLE_GO, pref_freq_list, &size);
    if (res)
        return res;
    ...
    if (wpa_s->parent->conf->p2p_ignore_shared_freq && no_pref_freq_given && pref_freq > 0 &&
        wpa_s->num_multichan_concurrent > 1 && wpas_p2p_num_unused_channels(wpa_s) > 0)
    {
        wpa_printf(
            MSG_DEBUG,
    ...
    return p2p_invite(wpa_s->global->p2p, peer_addr, role, bssid, ssid->ssid, ssid->ssid_len, force_freq, go_dev_addr,
                      1, pref_freq, -1);
}

/* Invite to join an active group */
int wpas_p2p_invite_group(
    struct wpa_supplicant *wpa_s, const char *ifname, const u8 *peer_addr, const u8 *go_dev_addr, bool allow_6ghz)
{
    struct wpa_global *global = wpa_s->global;
    enum p2p_invite_role role;
    u8 *bssid = NULL;
    ...
    persistent =
        ssid->p2p_persistent_group && wpas_p2p_get_persistent(wpa_s->p2pdev, peer_addr, ssid->ssid, ssid->ssid_len);
    ...
    size = P2P_MAX_PREF_CHANNELS;
    res  = wpas_p2p_setup_freqs(wpa_s, freq, &force_freq, &pref_freq, role == P2P_INVITE_ROLE_ACTIVE_GO, pref_freq_list,
                               &size);
    if (res)
        return res;
...
void wpas_p2p_completed(struct wpa_supplicant *wpa_s)
{
    ...
    wpa_s->show_group_started = 0;
    if (!wpa_s->p2p_go_group_formation_completed && wpa_s->global->p2p_group_formation == wpa_s)
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Marking group formation completed on client on data connection");
        wpa_s->p2p_go_group_formation_completed = 1;
        wpa_s->global->p2p_group_formation      = NULL;
        wpa_s->p2p_in_provisioning              = 0;
        wpa_s->p2p_in_invitation                = 0;
    ...
    wpas_p2p_group_started(wpa_s, 0, ssid, freq, ssid->passphrase == NULL && ssid->psk_set ? ssid->psk : NULL,
...
int wpas_p2p_presence_req(struct wpa_supplicant *wpa_s, u32 duration1, u32 interval1, u32 duration2, u32 interval2)
{
    ...
    ret = p2p_presence_req(wpa_s->global->p2p, wpa_s->bssid, wpa_s->own_addr, wpa_s->assoc_freq, duration1, interval1,
                           duration2, interval2);
    if (ret == 0)
...
int wpas_p2p_deauth_notif(
    struct wpa_supplicant *wpa_s, const u8 *bssid, u16 reason_code, const u8 *ie, size_t ie_len, int locally_generated)
{
    if (wpa_s->global->p2p_disabled || wpa_s->global->p2p == NULL)
        return 0;
    ...
    if (reason_code == WLAN_REASON_DEAUTH_LEAVING && !locally_generated && wpa_s->current_ssid &&
        wpa_s->current_ssid->p2p_group && wpa_s->current_ssid->mode == WPAS_MODE_INFRA)
    {
        wpa_printf(MSG_DEBUG,
...
void wpas_p2p_update_config(struct wpa_supplicant *wpa_s)
{
    ...
    if (wpa_s->conf->changed_parameters & CFG_CHANGED_P2P_SSID_POSTFIX)
    {
        p2p_set_ssid_postfix(p2p, (u8 *)wpa_s->conf->p2p_ssid_postfix,
                             wpa_s->conf->p2p_ssid_postfix ? os_strlen(wpa_s->conf->p2p_ssid_postfix) : 0);
    }
...
int wpas_p2p_set_cross_connect(struct wpa_supplicant *wpa_s, int enabled)
{
    ...
    if (!enabled)
    {
        ...
        for (iface = wpa_s->global->ifaces; iface; iface = iface->next)
        {
            ...
            wpa_msg_global(iface->p2pdev, MSG_INFO, P2P_EVENT_CROSS_CONNECT_DISABLE "%s %s", iface->ifname,
...
static void wpas_p2p_enable_cross_connect(struct wpa_supplicant *uplink)
{
    ...
    for (iface = uplink->global->ifaces; iface; iface = iface->next)
    {
        ...
        wpa_msg_global(iface->p2pdev, MSG_INFO, P2P_EVENT_CROSS_CONNECT_ENABLE "%s %s", iface->ifname,
                       iface->cross_connect_uplink);
    }
}
...
{
    if (wpa_s->ap_iface || wpa_s->current_ssid == NULL || wpa_s->current_ssid->mode != WPAS_MODE_INFRA ||
...
static void wpas_p2p_cross_connect_setup(struct wpa_supplicant *wpa_s)
{
    ...
    for (iface = wpa_s->global->ifaces; iface; iface = iface->next)
    {
        ...
        if (iface->ap_iface || iface->current_ssid == NULL || iface->current_ssid->mode != WPAS_MODE_INFRA ||
            iface->cross_connect_disallowed || iface->wpa_state != WPA_COMPLETED)
            break;

        wpa_s->cross_connect_in_use = 1;
        wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_CROSS_CONNECT_ENABLE "%s %s", wpa_s->ifname,
                       wpa_s->cross_connect_uplink);
        break;
    }
}
...
void wpas_p2p_update_channel_list(struct wpa_supplicant *wpa_s, enum wpas_p2p_channel_update_trig trig)
{
    ...
    os_memset(&cli_chan, 0, sizeof(cli_chan));
    if (wpas_p2p_setup_channels(wpa_s, &chan, &cli_chan, is_p2p_6ghz_disabled(wpa_s->global->p2p)))
    {
        wpa_printf(MSG_ERROR,
...
int wpas_p2p_cancel(struct wpa_supplicant *wpa_s)
{
    ...
    {
        if (wpa_s == global->p2p_group_formation &&
            (wpa_s->p2p_in_provisioning || wpa_s->parent->pending_interface_type == WPA_IF_P2P_CLIENT))
        {
            wpa_printf(MSG_DEBUG,
...
void wpas_p2p_network_removed(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
    if (wpa_s->p2p_in_provisioning && ssid->p2p_group &&
        eloop_cancel_timeout(wpas_p2p_group_formation_timeout, wpa_s->p2pdev, NULL) > 0)
    {
        /**
...
int wpas_p2p_scan_no_go_seen(struct wpa_supplicant *wpa_s)
{
    ...
    wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_FALLBACK_TO_GO_NEG "reason=GO-not-found");
...
static int wpas_p2p_remove_psk_entry(struct wpa_supplicant *wpa_s, struct wpa_ssid *s, const u8 *addr, int iface_addr)
{
    ...
    {
        if ((iface_addr && !psk->p2p && os_memcmp(addr, psk->addr, ETH_ALEN) == 0) ||
            (!iface_addr && psk->p2p && os_memcmp(addr, psk->addr, ETH_ALEN) == 0))
        {
            wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Remove persistent group PSK list entry for " MACSTR " p2p=%u",
...
void wpas_p2p_new_psk_cb(
    struct wpa_supplicant *wpa_s, const u8 *mac_addr, const u8 *p2p_dev_addr, const u8 *psk, size_t psk_len)
{
    struct wpa_ssid *ssid = wpa_s->current_ssid;
    struct wpa_ssid *persistent;
    ...
    if (p2p_dev_addr)
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "P2P: New PSK for addr=" MACSTR " p2p_dev_addr=" MACSTR, MAC2STR(mac_addr),
    ...
    if (dl_list_len(&persistent->psk_list) > P2P_MAX_STORED_CLIENTS &&
        (last = dl_list_last(&persistent->psk_list, struct psk_list_entry, list)))
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Remove oldest PSK entry for " MACSTR " (p2p=%u) to make room for a new one",
                MAC2STR(last->addr), last->p2p);
        dl_list_del(&last->list);
        os_free(last);
    }

    wpas_p2p_remove_psk_entry(wpa_s->p2pdev, persistent, p2p_dev_addr ? p2p_dev_addr : mac_addr, p2p_dev_addr == NULL);
    if (p2p_dev_addr)
    {
...
static void wpas_p2p_remove_client_go(struct wpa_supplicant *wpa_s, const u8 *peer, int iface_addr)
{
    ...
    while (psk)
    {
        ...
        {
            wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Remove operating group PSK entry for " MACSTR " iface_addr=%d",
    ...
    if (sta)
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Disconnect peer " MACSTR " (iface_addr=%d) from group", MAC2STR(peer),
                iface_addr);
...
int wpas_p2p_handle_frequency_conflicts(struct wpa_supplicant *wpa_s, int freq, struct wpa_ssid *ssid)
{
    ...
    for (iface = wpa_s->global->ifaces; iface; iface = iface->next)
    {
        if (!iface->current_ssid || iface->current_ssid->frequency == freq ||
            (iface->p2p_group_interface == NOT_P2P_GROUP_INTERFACE && !iface->current_ssid->p2p_group))
            continue;
...
int wpas_p2p_4way_hs_failed(struct wpa_supplicant *wpa_s)
{
    ...
    if (wpa_s->p2p_last_4way_hs_fail && wpa_s->p2p_last_4way_hs_fail == ssid)
    {
        ...
        wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Two 4-way handshake failures for a P2P group - go_dev_addr=" MACSTR,
                MAC2STR(go_dev_addr));
        persistent = wpas_p2p_get_persistent(wpa_s->p2pdev, go_dev_addr, ssid->ssid, ssid->ssid_len);
        if (persistent == NULL || persistent->mode != WPAS_MODE_INFRA)
        {
            wpa_dbg(wpa_s, MSG_DEBUG, "P2P: No matching persistent group stored");
            goto disconnect;
        }
        wpa_msg_global(wpa_s->p2pdev, MSG_INFO, P2P_EVENT_PERSISTENT_PSK_FAIL "%d", persistent->id);
    disconnect:
        wpa_s->p2p_last_4way_hs_fail = NULL;
        /*
...
static int wpas_p2p_cli_freq(struct wpa_supplicant *wpa_s, struct wpa_ssid **ssid, u8 *go_dev_addr)
{
    ...
    for (iface = wpa_s->global->ifaces; iface; iface = iface->next)
    {
        if (iface->wpa_state < WPA_ASSOCIATING || iface->current_ssid == NULL || iface->assoc_freq == 0 ||
            !iface->current_ssid->p2p_group || iface->current_ssid->mode != WPAS_MODE_INFRA)
            continue;
        if (ssid)
...
struct wpabuf *wpas_p2p_nfc_handover_req(struct wpa_supplicant *wpa_s, int ndef)
{
    ...
    if (wpa_s->conf->wps_nfc_dh_pubkey == NULL &&
        wps_nfc_gen_dh(&wpa_s->conf->wps_nfc_dh_pubkey, &wpa_s->conf->wps_nfc_dh_privkey) < 0)
    {
    ...
    if (cli_freq == 0)
    {
        ...
        wsc = NULL;
    p2p = p2p_build_nfc_handover_req(wpa_s->global->p2p, cli_freq, go_dev_addr, ssid ? ssid->ssid : NULL,
...
struct wpabuf *wpas_p2p_nfc_handover_sel(struct wpa_supplicant *wpa_s, int ndef, int tag)
{
    ...
    if (cli_freq == 0)
    {
        wsc = wps_build_nfc_handover_sel_p2p(wpa_s->parent->wps,
                                             ...
                                             wpa_s->conf->wps_nfc_dh_pubkey, tag ? wpa_s->conf->wps_nfc_dev_pw : NULL);
    }
    else
        wsc = NULL;
    p2p = p2p_build_nfc_handover_sel(wpa_s->global->p2p, cli_freq, go_dev_addr, ssid ? ssid->ssid : NULL,
                                     ssid ? ssid->ssid_len : 0);
...
static int wpas_p2p_nfc_join_group(struct wpa_supplicant *wpa_s, struct p2p_nfc_params *params)
{
    ...
    return wpas_p2p_connect(wpa_s, params->peer->p2p_device_addr, NULL, WPS_NFC, 0, 0, 1, 0, wpa_s->conf->p2p_go_intent,
                            params->go_freq, wpa_s->p2p_go_vht_center_freq2, -1, 0, 1, 1,
                            wpa_s->p2p_go_max_oper_chwidth, wpa_s->p2p_go_he, wpa_s->p2p_go_edmg,
                            params->go_ssid_len ? params->go_ssid : NULL, params->go_ssid_len, false);
}
...
static int wpas_p2p_nfc_auth_join(struct wpa_supplicant *wpa_s, struct p2p_nfc_params *params, int tag)
{
    ...
    wpa_s->global->p2p_invite_group = wpa_s;
    persistent                      = ssid->p2p_persistent_group &&
                 wpas_p2p_get_persistent(wpa_s->p2pdev, params->peer->p2p_device_addr, ssid->ssid, ssid->ssid_len);
    wpa_s->p2pdev->pending_invite_ssid_id = -1;

    return p2p_invite(wpa_s->global->p2p, params->peer->p2p_device_addr, P2P_INVITE_ROLE_ACTIVE_GO, wpa_s->own_addr,
                      ssid->ssid, ssid->ssid_len, ssid->frequency, wpa_s->global->p2p_dev_addr, persistent, 0,
...
static int wpas_p2p_nfc_init_go_neg(struct wpa_supplicant *wpa_s, struct p2p_nfc_params *params, int forced_freq)
{
    wpa_printf(MSG_DEBUG,
               ...
               "connection handover");
    return wpas_p2p_connect(wpa_s, params->peer->p2p_device_addr, NULL, WPS_NFC, 0, 0, 0, 0, wpa_s->conf->p2p_go_intent,
                            forced_freq, wpa_s->p2p_go_vht_center_freq2, -1, 0, 1, 1, wpa_s->p2p_go_max_oper_chwidth,
                            wpa_s->p2p_go_he, wpa_s->p2p_go_edmg, NULL, 0, false);
}
...
static int wpas_p2p_nfc_resp_go_neg(struct wpa_supplicant *wpa_s, struct p2p_nfc_params *params, int forced_freq)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               "connection handover");
    res = wpas_p2p_connect(wpa_s, params->peer->p2p_device_addr, NULL, WPS_NFC, 0, 0, 0, 1, wpa_s->conf->p2p_go_intent,
                           forced_freq, wpa_s->p2p_go_vht_center_freq2, -1, 0, 1, 1, wpa_s->p2p_go_max_oper_chwidth,
                           wpa_s->p2p_go_he, wpa_s->p2p_go_edmg, NULL, 0, false);
    if (res)
        return res;
...
static int wpas_p2p_nfc_connection_handover(
    struct wpa_supplicant *wpa_s, const struct wpabuf *data, int sel, int tag, int forced_freq)
{
    const u8 *pos, *end;
    u16 len, id;
    ...
    if (params.next_step == PEER_CLIENT)
    {
        if (!is_zero_ether_addr(params.go_dev_addr))
        {
            wpa_msg(wpa_s, MSG_INFO,
                    P2P_EVENT_NFC_PEER_CLIENT "peer=" MACSTR " freq=%d go_dev_addr=" MACSTR " ssid=\"%s\"",
                    MAC2STR(params.peer->p2p_device_addr), params.go_freq, MAC2STR(params.go_dev_addr),
                    wpa_ssid_txt(params.go_ssid, params.go_ssid_len));
        }
        else
        {
            wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_NFC_PEER_CLIENT "peer=" MACSTR " freq=%d",
    ...
    if (tag)
    {
        ...
        wpa_printf(MSG_DEBUG,
                   ...
                   id);
        wpa_hexdump_key(MSG_DEBUG, "P2P: Peer OOB Device Password", params.oob_dev_pw + WPS_OOB_PUBKEY_HASH_LEN + 2,
                        params.oob_dev_pw_len - WPS_OOB_PUBKEY_HASH_LEN - 2);
        wpa_s->p2p_oob_dev_pw_id = id;
        wpa_s->p2p_oob_dev_pw    = wpabuf_alloc_copy(params.oob_dev_pw + WPS_OOB_PUBKEY_HASH_LEN + 2,
                                                  params.oob_dev_pw_len - WPS_OOB_PUBKEY_HASH_LEN - 2);
...
int wpas_p2p_nfc_report_handover(
    struct wpa_supplicant *wpa_s, int init, const struct wpabuf *req, const struct wpabuf *sel, int forced_freq)
{
    struct wpabuf *tmp;
    int ret;
...
int wpas_p2p_nfc_tag_enabled(struct wpa_supplicant *wpa_s, int enabled)
{
    ...
    if (wpa_s->conf->wps_nfc_dh_pubkey == NULL || wpa_s->conf->wps_nfc_dh_privkey == NULL ||
        wpa_s->conf->wps_nfc_dev_pw == NULL || wpa_s->conf->wps_nfc_dev_pw_id < 0x10)
    {
        wpa_printf(MSG_DEBUG,
...
static int wpas_p2p_move_go_csa(struct wpa_supplicant *wpa_s)
{
     ...
     */
    if (wpas_p2p_init_go_params(wpa_s, &params, 0, 0, 0, 0, 0, 0, 0, NULL))
    {
    ...
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "P2P CSA: Selected same frequency - not moving GO");
        return 0;
    }
    ...
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "P2P CSA: CSA to a different band is not supported");
        ret = -1;
        goto out;
    }
    ...
    if (conf->ieee80211ac)
    {
        ...
        if (ieee80211_freq_to_channel_ext(params.freq, conf->secondary_channel, conf->vht_oper_chwidth, &opclass,
        ...
        if (conf->vht_oper_centr_freq_seg0_idx)
            freq1 = ieee80211_chan_to_freq(NULL, opclass, conf->vht_oper_centr_freq_seg0_idx);

        if (conf->vht_oper_centr_freq_seg1_idx)
            freq2 = ieee80211_chan_to_freq(NULL, opclass, conf->vht_oper_centr_freq_seg1_idx);
...
static void wpas_p2p_move_go_no_csa(struct wpa_supplicant *wpa_s)
{
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Move GO from freq=%d MHz", current_ssid->frequency);
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P: New freq selected for the GO (%u MHz)", params.freq);

    if (params.freq && !p2p_supported_freq_go(wpa_s->global->p2p, params.freq))
    {
        wpa_printf(MSG_DEBUG, "P2P: Selected freq (%u MHz) is not valid for P2P", params.freq);
        wpas_p2p_group_delete(wpa_s, P2P_GROUP_REMOVAL_GO_LEAVE_CHANNEL);
        return;
    }
                                            ...
                                            unsigned int num)
{
    ...
    for (i = 0, invalid_freq = 0; i < num; i++)
    {
        if (freqs[i].freq == freq)
        {
            ...
            if (!p2p_supported_freq_go(wpa_s->global->p2p, freq) && !dfs_offload)
            {
                wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Freq=%d MHz no longer valid for GO", freq);
                invalid_freq = 1;
            }
        }
        ...
        else if (!p2p_supported_freq(wpa_s->global->p2p, freqs[i].freq) && !dfs_offload)
        {
        ...
        else if (wpa_s->conf->p2p_go_freq_change_policy == P2P_GO_FREQ_MOVE_SCM_PEER_SUPPORTS &&
                 wpas_p2p_go_is_peer_freq(wpa_s, freqs[i].freq))
        {
            policy_move = 1;
        }
        else if ((wpa_s->conf->p2p_go_freq_change_policy == P2P_GO_FREQ_MOVE_SCM_ECSA) &&
                 wpas_p2p_go_is_peer_freq(wpa_s, freqs[i].freq))
        {
            if (!p2p_get_group_num_members(wpa_s->p2p_group))
            {
            ...
            else if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_AP_CSA) && wpas_p2p_go_clients_support_ecsa(wpa_s))
            {
                u8 chan;
                 ...
                 */
                if (wpa_s->ap_iface->current_mode->mode == ieee80211_freq_to_chan(freqs[i].freq, &chan))
                    policy_move = 1;
            }
        }
    }

    wpa_dbg(wpa_s, MSG_DEBUG, "P2P: GO move: invalid_freq=%u, policy_move=%u, flags=0x%X", invalid_freq, policy_move,
            flags);
    ...
    if (!invalid_freq || policy_move)
    {
        ...
        if (wpas_p2p_in_progress(wpa_s))
        {
            ...
            eloop_cancel_timeout(wpas_p2p_reconsider_moving_go, wpa_s, NULL);
            eloop_register_timeout(P2P_RECONSIDER_GO_MOVE_DELAY, 0, wpas_p2p_reconsider_moving_go, wpa_s, NULL);
            return;
        }
    }
    ...
    if (hostapd_csa_in_progress(wpa_s->ap_iface))
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Not initiating a GO frequency change - CSA is in progress");
        return;
    }
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Move GO from freq=%d MHz in %d secs", freq, timeout);
    eloop_cancel_timeout(wpas_p2p_move_go, wpa_s, NULL);
                                         ...
                                         enum wpas_p2p_channel_update_trig trig)
{
    ...
    dl_list_for_each(ifs, &wpa_s->radio->ifaces, struct wpa_supplicant, radio_list)
    {
        if (ifs->current_ssid == NULL || ifs->current_ssid->mode != WPAS_MODE_P2P_GO)
            continue;
         ...
         * to move it.
         */
        if (wpa_s == ifs && (trig == WPAS_P2P_CHANNEL_UPDATE_STATE_CHANGE || trig == WPAS_P2P_CHANNEL_UPDATE_CS))
        {
...
int wpas_p2p_lo_start(
    struct wpa_supplicant *wpa_s, unsigned int freq, unsigned int period, unsigned int interval, unsigned int count)
{
    struct p2p_data *p2p = wpa_s->global->p2p;
    u8 *device_types;
    ...
    if (wpa_s->global->p2p == NULL || !(wpa_s->drv_flags & WPA_DRIVER_FLAGS_P2P_LISTEN_OFFLOAD))
    {
    ...
    ret = wpa_drv_p2p_lo_start(wpa_s, freq, period, interval, count, device_types, dev_types_len, wpabuf_mhead_u8(buf),
                               wpabuf_len(buf));
    if (ret < 0)
