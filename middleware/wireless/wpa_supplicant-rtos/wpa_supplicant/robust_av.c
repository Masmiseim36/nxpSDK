...
void wpas_populate_mscs_descriptor_ie(struct robust_av_data *robust_av, struct wpabuf *buf)
{
    u8 *len, *len1;
    ...
    if (robust_av->request_type != SCS_REQ_REMOVE)
    {
        ...
        /* Frame classifier */
        wpabuf_put_data(buf, robust_av->frame_classifier, robust_av->frame_classifier_len);
...
static int wpas_populate_type4_classifier(struct type4_params *type4_param, struct wpabuf *buf)
{
    ...
    if (type4_param->ip_version == IPV4)
    {
        ...
        wpabuf_put_data(buf, &type4_param->ip_params.v4.src_ip.s_addr, 4);
        wpabuf_put_data(buf, &type4_param->ip_params.v4.dst_ip.s_addr, 4);
    ...
    else
    {
        ...
        wpabuf_put_data(buf, &type4_param->ip_params.v6.src_ip.s6_addr, 16);
        wpabuf_put_data(buf, &type4_param->ip_params.v6.dst_ip.s6_addr, 16);
...
static int wpas_populate_type10_classifier(struct type10_params *type10_param, struct wpabuf *buf)
{
    /* classifier parameters */
    wpabuf_put_u8(buf, type10_param->prot_instance);
    wpabuf_put_u8(buf, type10_param->prot_number);
    wpabuf_put_data(buf, type10_param->filter_value, type10_param->filter_len);
    wpabuf_put_data(buf, type10_param->filter_mask, type10_param->filter_len);
    return 0;
}

static int wpas_populate_scs_descriptor_ie(struct scs_desc_elem *desc_elem, struct wpabuf *buf)
{
    u8 *len, *len1;
    ...
    for (i = 0; i < desc_elem->num_tclas_elem; i++, tclas_elem++)
    {
        ...
        {
            case 4:
                ret = wpas_populate_type4_classifier(&tclas_elem->frame_classifier.type4_param, buf);
                break;
            case 10:
                ret = wpas_populate_type10_classifier(&tclas_elem->frame_classifier.type10_param, buf);
                break;
            default:
                return -1;
        }
        ...
        if (ret == -1)
        {
            wpa_printf(MSG_ERROR, "Failed to populate frame classifier");
            return -1;
        }
...
int wpas_send_mscs_req(struct wpa_supplicant *wpa_s)
{
    ...
    {
        wpa_dbg(wpa_s, MSG_INFO, "AP does not support MSCS - could not send MSCS Req");
        return -1;
    }

    if (!wpa_s->mscs_setup_done && wpa_s->robust_av.request_type != SCS_REQ_ADD)
    {
        wpa_msg(wpa_s, MSG_INFO, "MSCS: Failed to send MSCS Request: request type invalid");
        return -1;
    }
    ...
    ret = wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, wpa_s->bssid, wpa_s->own_addr, wpa_s->bssid,
...
static size_t tclas_elem_len(const struct tclas_element *elem)
{
    ...
    if (elem->classifier_type == 4)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "%s: Incorrect IP version %d", __func__, ip_ver);
            return 0;
        }
    }
    ...
    {
        wpa_printf(MSG_ERROR, "%s: Incorrect classifier type %u", __func__, elem->classifier_type);
        return 0;
    }
...
static struct wpabuf *allocate_scs_buf(struct scs_desc_elem *desc_elem, unsigned int num_scs_desc)
{
    struct wpabuf *buf;
    size_t buf_len = 0;
    unsigned int i, j;
    ...
    for (i = 0; i < num_scs_desc; i++, desc_elem++)
    {
        ...
        if (!tclas_elem)
        {
            wpa_printf(MSG_ERROR, "%s: TCLAS element null", __func__);
            return NULL;
        }
...
static void scs_request_timer(void *eloop_ctx, void *timeout_ctx)
{
    ...
    dl_list_for_each_safe(scs_desc, prev, &wpa_s->active_scs_ids, struct active_scs_elem, list)
    {
        u8 bssid[ETH_ALEN] = {0};
        const u8 *src;
        ...
        wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_SCS_RESULT "bssid=" MACSTR " SCSID=%u status_code=timedout", MAC2STR(src),
                scs_desc->scs_id);

        dl_list_del(&scs_desc->list);
        wpa_printf(MSG_INFO, "%s: SCSID %d removed after timeout", __func__, scs_desc->scs_id);
        os_free(scs_desc);
    }
...
int wpas_send_scs_req(struct wpa_supplicant *wpa_s)
{
    ...
    {
        wpa_dbg(wpa_s, MSG_INFO, "AP does not support SCS - could not send SCS Request");
        return -1;
    }
    ...
    buf = allocate_scs_buf(desc_elem, wpa_s->scs_robust_av_req.num_scs_desc);
    if (!buf)
        return -1;
    ...
    for (i = 0; i < wpa_s->scs_robust_av_req.num_scs_desc; i++, desc_elem++)
    {
        /* SCS Descriptor element */
     ...
     * the cache.
     */
    eloop_register_timeout(SCS_RESP_TIMEOUT, 0, scs_request_timer, wpa_s, NULL);
...
void free_up_scs_desc(struct scs_robust_av_data *data)
{
    ...
    {
        if (desc_data->request_type == SCS_REQ_REMOVE || !desc_data->tclas_elems)
            continue;
...
void wpas_handle_robust_av_recv_action(struct wpa_supplicant *wpa_s, const u8 *src, const u8 *buf, size_t len)
{
    u8 dialog_token;
    u16 status_code;
    ...
    {
        wpa_printf(MSG_INFO, "MSCS: Drop received frame due to dialog token mismatch: received:%u expected:%u",
                   dialog_token, wpa_s->robust_av.dialog_token);
        return;
    }

    status_code = WPA_GET_LE16(buf);
    wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_MSCS_RESULT "bssid=" MACSTR " status_code=%u", MAC2STR(src), status_code);
    wpa_s->mscs_setup_done = status_code == WLAN_STATUS_SUCCESS;
}

void wpas_handle_assoc_resp_mscs(struct wpa_supplicant *wpa_s, const u8 *bssid, const u8 *ies, size_t ies_len)
{
     ...
     */
    mscs_status = get_ie(&mscs_desc_ie[10], mscs_desc_ie[1] - 8, MCSC_SUBELEM_STATUS);
    if (!mscs_status || mscs_status[1] < 2)
        return;

    status = WPA_GET_LE16(mscs_status + 2);
    wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_MSCS_RESULT "bssid=" MACSTR " status_code=%u", MAC2STR(bssid), status);
...
static void wpas_wait_for_dscp_req_timer(void *eloop_ctx, void *timeout_ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "QM: Wait time over for sending DSCP request - allow DSCP query");
    wpa_s->wait_for_dscp_req = 0;
    wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_DSCP_POLICY "request_wait end");
}

void wpas_handle_assoc_resp_qos_mgmt(struct wpa_supplicant *wpa_s, const u8 *ies, size_t ies_len)
{
    const u8 *wfa_capa;
    ...
    if (!wfa_capa || wfa_capa[1] < 6 || wfa_capa[6] < 1 || !(wfa_capa[7] & WFA_CAPA_QM_DSCP_POLICY))
        return; /* AP does not enable QM DSCP Policy */

    wpa_s->connection_dscp   = 1;
    wpa_s->wait_for_dscp_req = !!(wfa_capa[7] & WFA_CAPA_QM_UNSOLIC_DSCP);
    if (!wpa_s->wait_for_dscp_req)
        return;
    ...
    eloop_register_timeout(DSCP_REQ_TIMEOUT, 0, wpas_wait_for_dscp_req_timer, wpa_s, NULL);
}

void wpas_handle_robust_av_scs_recv_action(struct wpa_supplicant *wpa_s, const u8 *src, const u8 *buf, size_t len)
{
    u8 dialog_token;
    ...
    {
        wpa_printf(MSG_INFO, "SCS: Drop received response due to no ongoing request");
        return;
    }
    ...
    {
        wpa_printf(MSG_INFO, "SCS: Drop received frame due to dialog token mismatch: received:%u expected:%u",
    ...
    {
        wpa_printf(MSG_INFO, "SCS: Drop received frame due to invalid count: %u (remaining %zu octets)", count, len);
        return;
    }
    ...
    for (i = 0; i < count; i++)
    {
        ...
        dl_list_for_each(scs_desc, &wpa_s->active_scs_ids, struct active_scs_elem, list)
        {
        ...
        wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_SCS_RESULT "bssid=" MACSTR " SCSID=%u status_code=%u", MAC2STR(src), id,
                status);
    }
    ...
    dl_list_for_each_safe(scs_desc, prev, &wpa_s->active_scs_ids, struct active_scs_elem, list)
    {
        if (scs_desc->status != SCS_DESC_SUCCESS)
        {
            wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_SCS_RESULT "bssid=" MACSTR " SCSID=%u status_code=response_not_received",
...
static void wpas_clear_active_scs_ids(struct wpa_supplicant *wpa_s)
{
    ...
    while ((scs_elem = dl_list_first(&wpa_s->active_scs_ids, struct active_scs_elem, list)))
    {
...
static int write_ipv4_info(char *pos, int total_len, const struct ipv4_params *v4)
{
    int res, rem_len;
    ...
    if (v4->param_mask & BIT(1))
    {
        ...
        {
            wpa_printf(MSG_ERROR, "QM: Failed to set IPv4 source address");
            return -1;
        }
    ...
    if (v4->param_mask & BIT(2))
    {
        ...
        {
            wpa_printf(MSG_ERROR, "QM: Failed to set IPv4 destination address");
            return -1;
        }
...
static int write_ipv6_info(char *pos, int total_len, const struct ipv6_params *v6)
{
    int res, rem_len;
    ...
    if (v6->param_mask & BIT(1))
    {
        ...
        {
            wpa_printf(MSG_ERROR, "QM: Failed to set IPv6 source addr");
            return -1;
        }
    ...
    if (v6->param_mask & BIT(2))
    {
        ...
        {
            wpa_printf(MSG_ERROR, "QM: Failed to set IPv6 destination addr");
            return -1;
        }
    ...
    {
        res = os_snprintf(pos, rem_len, " protocol=%d", v6->next_header);
        if (os_snprintf_error(rem_len, res))
            return -1;
...
static int set_frame_classifier_type4_ipv4(struct dscp_policy_data *policy)
{
    ...
    {
        wpa_printf(MSG_ERROR, "QM: Received IPv4 frame classifier with insufficient length %d",
    ...
    if (classifier_mask & BIT(1))
    {
        ...
        os_memcpy(&type4_param->ip_params.v4.src_ip, &frame_classifier[3], 4);
    }
    ...
    if (classifier_mask & BIT(2))
    {
        ...
        {
            wpa_printf(MSG_ERROR, "QM: IPv4: Both domain name and destination IP address not expected");
            return -1;
        }

        type4_param->ip_params.v4.param_mask |= BIT(2);
        os_memcpy(&type4_param->ip_params.v4.dst_ip, &frame_classifier[7], 4);
    }
    ...
    if (classifier_mask & BIT(3))
    {
        ...
        type4_param->ip_params.v4.src_port = WPA_GET_BE16(&frame_classifier[11]);
    }
    ...
    if (classifier_mask & BIT(4))
    {
        ...
        {
            wpa_printf(MSG_ERROR, "QM: IPv4: Both port range and destination port not expected");
            return -1;
        }

        type4_param->ip_params.v4.param_mask |= BIT(4);
        type4_param->ip_params.v4.dst_port = WPA_GET_BE16(&frame_classifier[13]);
    }
...
static int set_frame_classifier_type4_ipv6(struct dscp_policy_data *policy)
{
    ...
    {
        wpa_printf(MSG_ERROR, "QM: Received IPv6 frame classifier with insufficient length %d",
                   policy->frame_classifier_len);
        return -1;
    }
    ...
    if (classifier_mask & BIT(1))
    {
        ...
        os_memcpy(&type4_param->ip_params.v6.src_ip, &frame_classifier[3], 16);
    }
    ...
    if (classifier_mask & BIT(2))
    {
        if (policy->domain_name)
        {
            wpa_printf(MSG_ERROR, "QM: IPv6: Both domain name and destination IP address not expected");
            return -1;
        }
        type4_param->ip_params.v6.param_mask |= BIT(2);
        os_memcpy(&type4_param->ip_params.v6.dst_ip, &frame_classifier[19], 16);
    }
    ...
    if (classifier_mask & BIT(3))
    {
        ...
        type4_param->ip_params.v6.src_port = WPA_GET_BE16(&frame_classifier[35]);
    }
    ...
    if (classifier_mask & BIT(4))
    {
        if (policy->port_range_info)
        {
            wpa_printf(MSG_ERROR, "IPv6: Both port range and destination port not expected");
            return -1;
        }

        type4_param->ip_params.v6.param_mask |= BIT(4);
        type4_param->ip_params.v6.dst_port = WPA_GET_BE16(&frame_classifier[37]);
    }
...
static int wpas_set_frame_classifier_params(struct dscp_policy_data *policy)
{
    ...
    {
        wpa_printf(MSG_ERROR, "QM: Received frame classifier with insufficient length %d", frame_classifier_len);
        return -1;
    }
    ...
    {
        wpa_printf(MSG_ERROR, "QM: Received frame classifier with invalid classifier type %d", frame_classifier[0]);
        return -1;
    }
    ...
    {
        wpa_printf(MSG_ERROR, "QM: Received frame classifier without IP version");
        return -1;
    }
    ...
    if (frame_classifier[2] == 4)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "QM: Failed to set IPv4 parameters");
            return -1;
        }
    ...
    else if (frame_classifier[2] == 6)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "QM: Failed to set IPv6 parameters");
            return -1;
        }
    ...
    {
        wpa_printf(MSG_ERROR, "QM: Received unknown IP version %d", frame_classifier[2]);
        return -1;
    }
...
static void wpas_add_dscp_policy(struct wpa_supplicant *wpa_s, struct dscp_policy_data *policy)
{
    int ip_ver = 0, res;
    char policy_str[1000], *pos;
    int len;

    if (!policy->frame_classifier && !policy->domain_name && !policy->port_range_info)
    {
        wpa_printf(MSG_ERROR, "QM: Invalid DSCP policy - no attributes present");
        goto fail;
    }
    ...
    if (policy->frame_classifier)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "QM: Failed to set frame classifier parameters");
            goto fail;
        }
        ...
        if (res <= 0)
        {
            wpa_printf(MSG_ERROR, "QM: Failed to write IP parameters");
            goto fail;
        }
    ...
    if (policy->port_range_info)
    {
        res = os_snprintf(pos, len, " start_port=%u end_port=%u", policy->start_port, policy->end_port);
        if (os_snprintf_error(len, res))
        {
            wpa_printf(MSG_ERROR, "QM: Failed to write port range attributes for policy id = %d", policy->policy_id);
            goto fail;
        }
    ...
    if (policy->domain_name)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "QM: Domain name length higher than max expected");
            goto fail;
        }
        os_memcpy(domain_name_str, policy->domain_name, policy->domain_name_len);
        ...
        if (os_snprintf_error(len, res))
        {
            wpa_printf(MSG_ERROR, "QM: Failed to write domain name attribute for policy id = %d", policy->policy_id);
            goto fail;
        }
    }

    wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_DSCP_POLICY "add policy_id=%u dscp=%u ip_version=%d%s", policy->policy_id,
            policy->dscp, ip_ver, policy_str);
    return;
fail:
    wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_DSCP_POLICY "reject policy_id=%u", policy->policy_id);
}
...
static void wpas_fill_dscp_policy(struct dscp_policy_data *policy, u8 attr_id, u8 attr_len, const u8 *attr_data)
{
    switch (attr_id)
    {
        case QM_ATTR_PORT_RANGE:
            if (attr_len < 4)
            {
                wpa_printf(MSG_ERROR, "QM: Received Port Range attribute with insufficient length %d", attr_len);
                break;
            }
        ...
        case QM_ATTR_DSCP_POLICY:
            if (attr_len < 3)
            {
                wpa_printf(MSG_ERROR, "QM: Received DSCP Policy attribute with insufficient length %d", attr_len);
                return;
            }
        ...
        case QM_ATTR_TCLAS:
            if (attr_len < 1)
            {
                wpa_printf(MSG_ERROR, "QM: Received TCLAS attribute with insufficient length %d", attr_len);
                return;
            }
        ...
        case QM_ATTR_DOMAIN_NAME:
            if (attr_len < 1)
            {
                wpa_printf(MSG_ERROR, "QM: Received domain name attribute with insufficient length %d", attr_len);
                return;
            }
            ...
            break;
        default:
            wpa_printf(MSG_ERROR, "QM: Received invalid QoS attribute %d", attr_id);
            break;
    }
}

void wpas_handle_qos_mgmt_recv_action(struct wpa_supplicant *wpa_s, const u8 *src, const u8 *buf, size_t len)
{
    int rem_len;
    ...
    {
        wpa_printf(MSG_ERROR, "QM: Ignore DSCP Policy frame since the capability is not enabled");
        return;
    }

    if (!pmf_in_use(wpa_s, src))
    {
        wpa_printf(MSG_ERROR, "QM: Ignore DSCP Policy frame since PMF is not in use");
        return;
    }
    ...
    {
        wpa_printf(MSG_ERROR, "QM: Received unexpected QoS action frame %d", buf[0]);
        return;
    }

    if (len < 3)
    {
        wpa_printf(MSG_ERROR, "Received QoS Management DSCP Policy Request frame with invalid length %zu", len);
        return;
    }
    ...
    wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_DSCP_POLICY "request_start%s%s", reset ? " clear_all" : "", more ? " more" : "");
    ...
    while (rem_len > 2)
    {
        ...
        if (rem_len < 6 || qos_ie[0] != WLAN_EID_VENDOR_SPECIFIC || qos_ie[1] < 4 ||
        ...
        {
            wpas_fill_dscp_policy(&policy, attr[0], attr[1], &attr[2]);
        ...
        if (!policy.dscp_info)
        {
            wpa_printf(MSG_ERROR, "QM: Received QoS IE without DSCP Policy attribute");
            continue;
        }
        ...
        if (policy.req_type == DSCP_POLICY_REQ_ADD)
            ...
            wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_DSCP_POLICY "remove policy_id=%u", policy.policy_id);
        else
            wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_DSCP_POLICY "reject policy_id=%u", policy.policy_id);
    }
...
int wpas_send_dscp_response(struct wpa_supplicant *wpa_s, struct dscp_resp_data *resp_data)
{
    struct wpabuf *buf = NULL;
    size_t buf_len;
    int ret         = -1, i;
    ...
    if (wpa_s->wpa_state != WPA_COMPLETED || !wpa_s->current_ssid)
    {
        wpa_printf(MSG_ERROR, "QM: Failed to send DSCP response - not connected to AP");
        return -1;
    }
    ...
    if (!buf)
    {
        wpa_printf(MSG_ERROR, "QM: Failed to allocate DSCP policy response");
        return -1;
    }
    ...
    wpabuf_put_u8(buf, resp_data->solicited ? wpa_s->dscp_req_dialog_token : 0);
...
int wpas_send_dscp_query(struct wpa_supplicant *wpa_s, const char *domain_name, size_t domain_name_length)
{
    struct wpabuf *buf = NULL;
