...
static int wpa_setup_mac_addr_rand_params(struct wpa_driver_scan_params *params, const u8 *mac_addr)
{
    u8 *tmp;
...
static void wpa_supplicant_assoc_try(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
    ...
    while (ssid)
    {
        if (!wpas_network_disabled(wpa_s, ssid))
        {
            ...
            if (!min_temp_disabled || temp_disabled < min_temp_disabled)
...
static void wpas_trigger_scan_cb(struct wpa_radio_work *work, int deinit)
{
    ...
    if ((wpa_s->mac_addr_rand_enable & MAC_ADDR_RAND_SCAN) && wpa_s->wpa_state <= WPA_SCANNING)
    ...
    if (ret)
    {
        int retry = wpa_s->last_scan_req != MANUAL_SCAN_REQ && !wpa_s->beacon_rep_data.token;
...
static int wpa_supplicant_start_sched_scan(struct wpa_supplicant *wpa_s, struct wpa_driver_scan_params *params)
{
    int ret;
...
#ifdef CONFIG_MBO
static void wpas_fils_req_param_add_max_channel(struct wpa_supplicant *wpa_s, struct wpabuf **ie)
{
    if (wpabuf_resize(ie, 5))
    {
        wpa_printf(MSG_DEBUG, "Failed to allocate space for FILS Request Parameters element");
        return;
    }
...
void wpa_supplicant_set_default_scan_ies(struct wpa_supplicant *wpa_s)
{
    ...
    if (wpa_s->vendor_elem[frame_id])
    {
        ...
        if (len > 0 && wpabuf_resize(&default_ies, len) == 0)
            wpabuf_put_buf(default_ies, wpa_s->vendor_elem[frame_id]);
    }

    if (default_ies)
        wpa_drv_set_default_scan_ies(wpa_s, wpabuf_head(default_ies), wpabuf_len(default_ies));
...
static struct wpabuf *wpa_supplicant_extra_ies(struct wpa_supplicant *wpa_s)
{
    ...
    if (wps)
    {
        ...
        wps_ie = wps_build_probe_req_ie(wps == 2 ? DEV_PW_PUSHBUTTON : DEV_PW_DEFAULT, &wpa_s->wps->dev,
...
#ifdef CONFIG_FST
    if (wpa_s->fst_ies && wpabuf_resize(&extra_ie, wpabuf_len(wpa_s->fst_ies)) == 0)
                            ...
                            bool is_6ghz)
{
    ...
    freqs = os_realloc(params->freqs, (num_chans + mode->num_channels + 1) * sizeof(int));
    if (!freqs)
        return -1;
...
static void wpa_setband_scan_freqs(struct wpa_supplicant *wpa_s, struct wpa_driver_scan_params *params)
{
    ...
    if (wpa_s->setband_mask & WPA_SETBAND_6G)
        wpa_add_scan_freqs_list(wpa_s, HOSTAPD_MODE_IEEE80211A, params, true);
}
                              ...
                              size_t ssid_len)
{
    ...
    {
        if (params->ssids[j].ssid_len == ssid_len && params->ssids[j].ssid &&
    ...
    wpa_printf(MSG_DEBUG, "Scan SSID (manual request): %s", wpa_ssid_txt(ssid, ssid_len));
                                  ...
                                  size_t max_ssids)
{
    ...
    wpa_printf(MSG_DEBUG, "OWE: Look for transition mode AP. ssid=%s", wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
    ...
    dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list)
    {
        ...
        if (bss->ssid_len != ssid->ssid_len || os_memcmp(bss->ssid, ssid->ssid, ssid->ssid_len) != 0)
            continue;
        ...
        if ((size_t)(end - pos) < owe_ssid_len || owe_ssid_len > SSID_MAX_LEN)
            return;

        wpa_printf(MSG_DEBUG, "OWE: scan_ssids: transition mode OWE ssid=%s", wpa_ssid_txt(owe_ssid, owe_ssid_len));

        wpa_add_scan_ssid(wpa_s, params, max_ssids, owe_ssid, owe_ssid_len);
        return;
    }
#endif /* CONFIG_OWE */
}
...
static void wpa_set_scan_ssids(struct wpa_supplicant *wpa_s, struct wpa_driver_scan_params *params, size_t max_ssids)
{
    ...
    for (i = 0; i < wpa_s->scan_id_count; i++)
    {
        ...
        if (!ssid)
            continue;
        if (ssid->scan_ssid)
            wpa_add_scan_ssid(wpa_s, params, max_ssids, ssid->ssid, ssid->ssid_len);
        /*
    ...
    else
    {
        ...
        for (tssid = wpa_s->conf->ssid; wpa_s->last_scan_req != MANUAL_SCAN_REQ && tssid; tssid = tssid->next)
        {
            if (wpas_network_disabled(wpa_s, tssid))
                continue;
            if (((params.freqs || !freqs_set) && tssid->scan_freq) && int_array_len(params.freqs) < 100)
            {
...
ssid_list_set:
    ...
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "Limit select_network scan to specified channels");
    ...
    if (wpa_s->last_scan_req == INITIAL_SCAN_REQ && wpa_s->conf->initial_freq_list && !params.freqs)
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "Optimize scan based on conf->initial_freq_list");
    ...
    if (!is_zero_ether_addr(wpa_s->next_scan_bssid))
    {
        ...
        if (!wpa_s->next_scan_bssid_wildcard_ssid && bss && bss->ssid_len && params.num_ssids == 1 &&
            params.ssids[0].ssid_len == 0)
        {
            params.ssids[0].ssid     = bss->ssid;
            params.ssids[0].ssid_len = bss->ssid_len;
            wpa_dbg(wpa_s, MSG_DEBUG, "Scan a previously specified BSSID " MACSTR " and SSID %s", MAC2STR(params.bssid),
                    wpa_ssid_txt(bss->ssid, bss->ssid_len));
        }
        else
        {
            wpa_dbg(wpa_s, MSG_DEBUG, "Scan a previously specified BSSID " MACSTR, MAC2STR(params.bssid));
        }
    }
...
scan:
     ...
     */
    if (wpa_s->scan_for_connection && wpa_s->last_scan_req == NORMAL_SCAN_REQ && !scan_params->freqs && !params.freqs &&
        wpas_is_p2p_prioritized(wpa_s) && wpa_s->p2p_group_interface == NOT_P2P_GROUP_INTERFACE &&
    ...
    if (!params.freqs && (wpa_s->p2p_in_invitation || wpa_s->p2p_in_provisioning) &&
        !is_p2p_allow_6ghz(wpa_s->global->p2p) && is_6ghz_supported(wpa_s))
    {
        int i;
...
void wpa_supplicant_req_scan(struct wpa_supplicant *wpa_s, int sec, int usec)
{
    ...
    {
        wpa_dbg(wpa_s, MSG_DEBUG,
                "Ignore new scan request for %d.%06d sec since an earlier request is scheduled to trigger sooner", sec,
                usec);
    }
    else
    {
...
static void wpa_scan_set_relative_rssi_params(struct wpa_supplicant *wpa_s, struct wpa_driver_scan_params *params)
{
    if (wpa_s->wpa_state != WPA_COMPLETED || !(wpa_s->drv_flags & WPA_DRIVER_FLAGS_SCHED_SCAN_RELATIVE_RSSI) ||
...
int wpa_supplicant_req_sched_scan(struct wpa_supplicant *wpa_s)
{
    ...
    if (wpa_s->normal_scans < 3 &&
        (need_ssids <= wpa_s->max_scan_ssids || wpa_s->max_scan_ssids >= (int)max_sched_scan_ssids))
    {
        /*
...
scan:
     ...
     */
    if (wpa_s->sched_scan_plans_num == 1 || (wpa_s->sched_scan_plans_num && !ssid && wpa_s->first_sched_scan))
    {
    ...
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "Too many SSIDs. Default to using single scheduled_scan plan");
        params.sched_scan_plans     = &wpa_s->sched_scan_plans[wpa_s->sched_scan_plans_num - 1];
    ...
    else
    {
        ...
        {
            wpa_printf(MSG_WARNING, "Scan interval too long(%u), use the maximum allowed(%u)", scan_plan.interval,
    ...
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "Starting sched scan after %u seconds: interval %u timeout %d",
                params.sched_scan_start_delay, params.sched_scan_plans[0].interval, wpa_s->sched_scan_timeout);
    }
    else
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "Starting sched scan after %u seconds (no timeout)", params.sched_scan_start_delay);
    }

    wpa_setband_scan_freqs(wpa_s, scan_params);

    if ((wpa_s->mac_addr_rand_enable & MAC_ADDR_RAND_SCHED_SCAN) && wpa_s->wpa_state <= WPA_SCANNING)
    ...
    if (ssid || !wpa_s->first_sched_scan)
    {
        ...
        if ((unsigned int)wpa_s->sched_scan_timeout < params.sched_scan_plans[0].interval ||
            params.sched_scan_plans[0].interval > wpa_s->max_sched_scan_plan_interval)
        {
...
const u8 *wpa_scan_get_vendor_ie_beacon(const struct wpa_scan_res *res, u32 vendor_type)
{
    ...
    for_each_element_id(elem, WLAN_EID_VENDOR_SPECIFIC, ies, res->beacon_ie_len)
    {
        if (elem->datalen >= 4 && vendor_type == WPA_GET_BE32(elem->data))
            return &elem->id;
    }
...
struct wpabuf *wpa_scan_get_vendor_ie_multi(const struct wpa_scan_res *res, u32 vendor_type)
{
    ...
    while (end - pos > 1)
    {
        ...
        if (len > end - pos - 2)
            break;
        pos += 2;
        if (ie == WLAN_EID_VENDOR_SPECIFIC && len >= 4 && vendor_type == WPA_GET_BE32(pos))
...
#define MIN(a, b) a < b ? a : b
    ...
    if (snr_a && snr_b && (abs(snr_b - snr_a) < 7 || is_6ghz_freq(wa->freq) || is_6ghz_freq(wb->freq)))
    {
    ...
    for (i = 0; i < scan_res->num; i++)
    {
        ...
        if (r->flags & WPA_SCAN_LEVEL_DBM)
        {
            ...
            wpa_printf(MSG_EXCESSIVE,
                       ...
                       MAC2STR(r->bssid), r->freq, r->qual, r->noise, noise_valid ? "" : "~", r->level, r->snr,
                       r->snr >= GREAT_SNR ? "*" : "", r->flags, r->age, r->est_throughput);
        }
        else
        {
            wpa_printf(MSG_EXCESSIVE,
                       ...
                       MAC2STR(r->bssid), r->freq, r->qual, r->noise, r->level, r->flags, r->age, r->est_throughput);
        }
        pos = (u8 *)(r + 1);
        if (r->ie_len)
...
void filter_scan_res(struct wpa_supplicant *wpa_s, struct wpa_scan_results *res)
{
    size_t i, j;
...
void scan_snr(struct wpa_scan_res *res)
{
    if (res->flags & WPA_SCAN_NOISE_INVALID)
    {
        res->noise = is_6ghz_freq(res->freq) ?
                         ...
                         (IS_5GHZ(res->freq) ? DEFAULT_NOISE_FLOOR_5GHZ : DEFAULT_NOISE_FLOOR_2GHZ);
    }
...
static unsigned int interpolate_rate(int snr, int snr0, int snr1, int rate0, int rate1)
{
    return rate0 + (snr - snr0) * (rate1 - rate0) / (snr1 - snr0);
}

static unsigned int max_rate(const struct minsnr_bitrate_entry table[], int snr, bool vht)
{
    const struct minsnr_bitrate_entry *prev, *entry = table;

    while ((entry->minsnr != -1) && (snr >= entry->minsnr) && (vht || entry - table <= vht_mcs))
        entry++;
    if (entry == table)
    ...
    if (entry->minsnr == -1 || (!vht && entry - table > vht_mcs))
        return prev->bitrate;
    return interpolate_rate(snr, prev->minsnr, entry->minsnr, prev->bitrate, entry->bitrate);
}
...
static unsigned int max_he_rate(const struct minsnr_bitrate_entry table[], int snr)
{
    const struct minsnr_bitrate_entry *prev, *entry = table;
...
unsigned int wpas_get_est_tpt(
    const struct wpa_supplicant *wpa_s, const u8 *ies, size_t ies_len, int rate, int snr, int freq)
{
    ...
    hw_mode = get_mode_with_freq(wpa_s->hw.modes, wpa_s->hw.num_modes, freq);
    ...
    if (hw_mode && (hw_mode->ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET))
    {
    ...
    if (hw_mode && hw_mode->vht_capab)
    {
        ...
        if (ie)
        {
            ...
            if (ie && ie[1] >= 3)
            {
                ...
                if (cw)
                    vht80 = true;
                if (cw == 2 || (cw == 3 && (seg1 > 0 && abs(seg1 - seg0) == 16)))
                    vht160 = true;
                if (cw == 1 && ((seg1 > 0 && abs(seg1 - seg0) == 8) || (seg1 > 0 && abs(seg1 - seg0) == 16)))
                    vht160 = true;
            }
            ...
            if (vht160 &&
                (hw_mode->vht_capab & (VHT_CAP_SUPP_CHAN_WIDTH_160MHZ | VHT_CAP_SUPP_CHAN_WIDTH_160_80PLUS80MHZ)))
            {
    ...
    if (hw_mode && hw_mode->he_capab[IEEE80211_MODE_INFRA].he_supported)
    {
        ...
        cw = he->he_phy_capab_info[HE_PHYCAP_CHANNEL_WIDTH_SET_IDX] & own_he->phy_cap[HE_PHYCAP_CHANNEL_WIDTH_SET_IDX];
        if (cw &
            (IS_2P4GHZ(freq) ? HE_PHYCAP_CHANNEL_WIDTH_SET_40MHZ_IN_2G : HE_PHYCAP_CHANNEL_WIDTH_SET_40MHZ_80MHZ_IN_5G))
        {
        ...
        if (!IS_2P4GHZ(freq) && (cw & HE_PHYCAP_CHANNEL_WIDTH_SET_40MHZ_80MHZ_IN_5G))
        {
        ...
        if (!IS_2P4GHZ(freq) &&
            (cw & (HE_PHYCAP_CHANNEL_WIDTH_SET_160MHZ_IN_5G | HE_PHYCAP_CHANNEL_WIDTH_SET_80PLUS80MHZ_IN_5G)))
        {
...
void scan_est_throughput(struct wpa_supplicant *wpa_s, struct wpa_scan_res *res)
{
    ...
    if (!ie_len)
        ie_len = res->beacon_ie_len;
    res->est_throughput = wpas_get_est_tpt(wpa_s, ies, ie_len, rate, snr, res->freq);
...
struct wpa_driver_scan_params *wpa_scan_clone_params(const struct wpa_driver_scan_params *src)
{
    ...
    for (i = 0; i < src->num_ssids; i++)
    {
        if (src->ssids[i].ssid)
        {
            n = os_memdup(src->ssids[i].ssid, src->ssids[i].ssid_len);
            if (n == NULL)
                goto failed;
    ...
    if (src->filter_ssids)
    {
        params->filter_ssids = os_memdup(src->filter_ssids, sizeof(*params->filter_ssids) * src->num_filter_ssids);
    ...
    if (src->sched_scan_plans_num > 0)
    {
        params->sched_scan_plans =
            os_memdup(src->sched_scan_plans, sizeof(*src->sched_scan_plans) * src->sched_scan_plans_num);
    ...
    if (src->mac_addr_rand && wpa_setup_mac_addr_rand_params(params, src->mac_addr))
        goto failed;
...
int wpas_start_pno(struct wpa_supplicant *wpa_s)
{
    ...
    if ((wpa_s->mac_addr_rand_enable & MAC_ADDR_RAND_PNO) && wpa_s->wpa_state <= WPA_SCANNING)
...
int wpas_mac_addr_rand_scan_set(struct wpa_supplicant *wpa_s, unsigned int type, const u8 *addr, const u8 *mask)
{
    ...
    {
        wpa_printf(MSG_INFO, "scan: MAC randomization type %u != supported=%u", type, wpa_s->mac_addr_rand_supported);
        return -1;
    }
...
int wpas_mac_addr_rand_scan_get_mask(struct wpa_supplicant *wpa_s, unsigned int type, u8 *mask)
{
    const u8 *to_copy;
    ...
    {
        wpa_printf(MSG_DEBUG, "scan: Invalid MAC randomization type=0x%x", type);
        return -1;
    }
...
int wpas_abort_ongoing_scan(struct wpa_supplicant *wpa_s)
{
    ...
    dl_list_for_each(work, &radio->work, struct wpa_radio_work, list)
    {
        if (work->wpa_s != wpa_s || !work->started ||
            (os_strcmp(work->type, "scan") != 0 && os_strcmp(work->type, "p2p-scan") != 0))
            continue;
...
int wpas_sched_scan_plans_set(struct wpa_supplicant *wpa_s, const char *cmd)
{
    ...
    while ((token = cstr_token(cmd, " ", &context)))
    {
        ...
        if (scan_plan->interval > wpa_s->max_sched_scan_plan_interval)
        {
            wpa_printf(MSG_WARNING, "scan plan %u: Scan interval too long(%u), use the maximum allowed(%u)", num,
                       scan_plan->interval, wpa_s->max_sched_scan_plan_interval);
            scan_plan->interval = wpa_s->max_sched_scan_plan_interval;
        }
        ...
        {
            wpa_printf(MSG_ERROR, "scan plan %u: Number of iterations cannot be zero", num);
            goto fail;
        }

        if (scan_plan->iterations > wpa_s->max_sched_scan_plan_iterations)
        {
            wpa_printf(MSG_WARNING, "scan plan %u: Too many iterations(%u), use the maximum allowed(%u)", num,
                       scan_plan->iterations, wpa_s->max_sched_scan_plan_iterations);
            scan_plan->iterations = wpa_s->max_sched_scan_plan_iterations;
        }

        wpa_printf(MSG_DEBUG, "scan plan %u: interval=%u iterations=%u", num, scan_plan->interval,
    ...
    {
        wpa_printf(MSG_ERROR, "All scan plans but the last must specify a number of iterations");
        goto fail;
    }

    wpa_printf(MSG_DEBUG, "scan plan %u (last plan): interval=%u", num, scan_plans[num - 1].interval);

    if (num > wpa_s->max_sched_scan_plans)
    {
        wpa_printf(MSG_WARNING, "Too many scheduled scan plans (only %u supported)", wpa_s->max_sched_scan_plans);
        wpa_printf(MSG_WARNING, "Use only the first %u scan plans, and the last one (in infinite loop)",
                   wpa_s->max_sched_scan_plans - 1);
        os_memcpy(&scan_plans[wpa_s->max_sched_scan_plans - 1], &scan_plans[num - 1], sizeof(*scan_plans));
