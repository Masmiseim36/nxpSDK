                                                ...
                                                bool *ret_use_pk)
{
    ...
    if (reuse && wpa_s->sme.sae.tmp && os_memcmp(bssid, wpa_s->sme.sae.tmp->bssid, ETH_ALEN) == 0)
    {
        wpa_printf(MSG_DEBUG, "SAE: Reuse previously generated PWE on a retry with the same AP");
    ...
    if (!bss)
    {
        wpa_printf(MSG_DEBUG, "SAE: BSS not available, update scan result to get BSS");
    ...
    {
        wpa_printf(MSG_DEBUG, "SAE: Cannot use PK with the selected AP");
        return NULL;
    }
#endif /* CONFIG_SAE_PK */
    ...
    if (use_pt || wpa_s->conf->sae_pwe == 1 || wpa_s->conf->sae_pwe == 2)
    {
        ...
        if ((wpa_s->conf->sae_pwe == 1 || ssid->sae_password_id) && wpa_s->conf->sae_pwe != 3 && !use_pt)
        {
            wpa_printf(MSG_DEBUG, "SAE: Cannot use H2E with the selected AP");
            return NULL;
        }
    }

    if (use_pt && sae_prepare_commit_pt(&wpa_s->sme.sae, ssid->pt, wpa_s->own_addr, bssid,
                                        wpa_s->sme.sae_rejected_groups, NULL) < 0)
        return NULL;
    if (!use_pt && sae_prepare_commit(wpa_s->own_addr, bssid, (u8 *)password, os_strlen(password), &wpa_s->sme.sae) < 0)
    {
    ...
    if (wpa_s->sme.sae.tmp)
    {
        ...
        if (use_pt && use_pk)
            wpa_s->sme.sae.pk = 1;
#ifdef CONFIG_SAE_PK
        os_memcpy(wpa_s->sme.sae.tmp->own_addr, wpa_s->own_addr, ETH_ALEN);
...
reuse_data:
    ...
    if (sae_write_commit(&wpa_s->sme.sae, buf, wpa_s->sme.sae_token, ssid->sae_password_id) < 0)
    {
        wpabuf_free(buf);
        return NULL;
    }
...
static struct wpabuf *sme_auth_build_sae_confirm(struct wpa_supplicant *wpa_s, int external)
{
    struct wpabuf *buf;
...
static void sme_auth_handle_rrm(struct wpa_supplicant *wpa_s, struct wpa_bss *bss)
{
    ...
    *pos |=
        WLAN_RRM_CAPS_BEACON_REPORT_PASSIVE | WLAN_RRM_CAPS_BEACON_REPORT_ACTIVE | WLAN_RRM_CAPS_BEACON_REPORT_TABLE;
...
static void sme_send_authentication(struct wpa_supplicant *wpa_s, struct wpa_bss *bss, struct wpa_ssid *ssid, int start)
{
    ...
    if (wpa_key_mgmt_sae(ssid->key_mgmt))
    {
        ...
        else if (wpa_parse_wpa_ie(rsn, 2 + rsn[1], &ied) == 0 && (ssid->key_mgmt & WPA_KEY_MGMT_DPP) &&
...
#endif /* CONFIG_FILS */
        if (pmksa_cache_set_current(wpa_s->wpa, NULL, bss->bssid, wpa_s->current_ssid, try_opportunistic, cache_id,
                                    0) == 0)
    ...
    else if (wpa_bss_get_vendor_ie(bss, OSEN_IE_VENDOR_TYPE) && (ssid->key_mgmt & WPA_KEY_MGMT_OSEN))
    {
    ...
    {
        wpa_ie = os_memdup(wpa_s->sme.assoc_req_ie, wpa_s->sme.assoc_req_ie_len);
        if (wpa_ie)
        {
    ...
    if (md && (!wpa_key_mgmt_ft(ssid->key_mgmt) || !wpa_key_mgmt_ft(wpa_s->key_mgmt)))
        md = NULL;
    if (md)
    {
    ...
    if (md)
    {
        ...
        if (wpa_s->sme.prev_bssid_set && wpa_s->sme.ft_used && os_memcmp(md, wpa_s->sme.mobility_domain, 2) == 0 &&
    ...
    if (wpa_s->fst_ies)
    {
        ...
        if (wpa_s->sme.assoc_req_ie_len + fst_ies_len <= sizeof(wpa_s->sme.assoc_req_ie))
        {
            os_memcpy(wpa_s->sme.assoc_req_ie + wpa_s->sme.assoc_req_ie_len, wpabuf_head(wpa_s->fst_ies), fst_ies_len);
    ...
    wpa_s->sme.assoc_req_ie_len +=
        wpas_supp_op_class_ie(wpa_s, ssid, bss, wpa_s->sme.assoc_req_ie + wpa_s->sme.assoc_req_ie_len,
    ...
    if (wpa_s->rsnxe_override_assoc &&
        wpabuf_len(wpa_s->rsnxe_override_assoc) <= sizeof(wpa_s->sme.assoc_req_ie) - wpa_s->sme.assoc_req_ie_len)
    {
        wpa_printf(MSG_DEBUG, "TESTING: RSNXE AssocReq override");
        os_memcpy(wpa_s->sme.assoc_req_ie + wpa_s->sme.assoc_req_ie_len, wpabuf_head(wpa_s->rsnxe_override_assoc),
                  wpabuf_len(wpa_s->rsnxe_override_assoc));
        wpa_s->sme.assoc_req_ie_len += wpabuf_len(wpa_s->rsnxe_override_assoc);
    }
    else
#endif /* CONFIG_TESTING_OPTIONS */
        if (wpa_s->rsnxe_len > 0 && wpa_s->rsnxe_len <= sizeof(wpa_s->sme.assoc_req_ie) - wpa_s->sme.assoc_req_ie_len &&
            !omit_rsnxe)
    {
        os_memcpy(wpa_s->sme.assoc_req_ie + wpa_s->sme.assoc_req_ie_len, wpa_s->rsnxe, wpa_s->rsnxe_len);
    ...
    if (wpa_ie)
    {
        ...
        if (len > wpa_ie_len)
        {
            os_memcpy(wpa_s->sme.assoc_req_ie + wpa_s->sme.assoc_req_ie_len, wpa_ie, wpa_ie_len);
    ...
    if (!wpa_s->disable_mbo_oce && mbo_ie)
    {
        ...
        len = wpas_mbo_ie(wpa_s, wpa_s->sme.assoc_req_ie + wpa_s->sme.assoc_req_ie_len,
                          sizeof(wpa_s->sme.assoc_req_ie) - wpa_s->sme.assoc_req_ie_len,
    ...
    if (!skip_auth && params.auth_alg == WPA_AUTH_ALG_SAE &&
        pmksa_cache_set_current(wpa_s->wpa, NULL, bss->bssid, ssid, 0, NULL,
                                wpa_s->key_mgmt == WPA_KEY_MGMT_FT_SAE ? WPA_KEY_MGMT_FT_SAE : WPA_KEY_MGMT_SAE) == 0)
    {
        wpa_dbg(wpa_s, MSG_DEBUG,
    ...
    {
        if (start)
            resp = sme_auth_build_sae_commit(wpa_s, ssid, bss->bssid, 0, start == 2, NULL, NULL);
        else
     ...
     * networks). */
    if (params.auth_alg == WPA_AUTH_ALG_OPEN && wpa_key_mgmt_fils(ssid->key_mgmt))
    {
        const u8 *indic;
        u16 fils_info;
        ...
        {
            wpa_printf(MSG_DEBUG,
                       "SME: " MACSTR " does not support FILS SK without PFS - cannot use FILS authentication with it",
                       MAC2STR(bss->bssid));
            goto no_fils;
        }
        ...
        {
            wpa_printf(MSG_DEBUG,
                       "SME: " MACSTR " does not support FILS SK with PFS - cannot use FILS authentication with it",
                       MAC2STR(bss->bssid));
            goto no_fils;
        }
        ...
        if (wpa_s->last_con_fail_realm &&
            ...
            os_memcmp(realm, wpa_s->last_con_fail_realm, realm_len) == 0)
        {
            wpa_printf(
                MSG_DEBUG,
        ...
        if (pmksa_cache_set_current(wpa_s->wpa, NULL, bss->bssid, ssid, 0, wpa_bss_get_fils_cache_id(bss), 0) == 0)
            wpa_printf(MSG_DEBUG, "SME: Try to use FILS with PMKSA caching");
        ...
        if (resp)
        {
            ...
            if (ssid->fils_dh_group)
                wpa_printf(MSG_DEBUG, "SME: Try to use FILS SK authentication with PFS (DH Group %u)",
                           ssid->fils_dh_group);
            else
                wpa_printf(MSG_DEBUG, "SME: Try to use FILS SK authentication without PFS");
            auth_alg             = ssid->fils_dh_group ? WPA_AUTH_ALG_FILS_SK_PFS : WPA_AUTH_ALG_FILS;
...
no_fils:
    ...
    wpa_msg(wpa_s, MSG_INFO, "SME: Trying to authenticate with " MACSTR " (SSID='%s' freq=%d MHz)",
...
static void sme_auth_start_cb(struct wpa_radio_work *work, int deinit)
{
        ...
        wpas_network_disabled(wpa_s, cwork->ssid))
    {
        ...
        wpa_msg_ctrl(wpa_s, MSG_INFO, WPA_EVENT_AUTH_REJECT);
        wpas_connect_work_done(wpa_s);
        return;
    }
...
void sme_authenticate(struct wpa_supplicant *wpa_s, struct wpa_bss *bss, struct wpa_ssid *ssid)
{
    ...
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "SME: Reject sme_authenticate() in favor of explicit roam request");
        return;
    }
#ifdef CONFIG_WNM
    if (wpa_s->bss_trans_mgmt_in_progress)
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "SME: Reject sme_authenticate() in favor of BSS transition management request");
        return;
    }
#endif /* CONFIG_WNM */
                                       ...
                                       u16 status_code)
{
    ...
    resp = wpabuf_put(buf, offsetof(struct ieee80211_mgmt, u.auth.variable));

    resp->frame_control = host_to_le16((WLAN_FC_TYPE_MGMT << 2) | (WLAN_FC_STYPE_AUTH << 4));
...
static int sme_external_auth_send_sae_commit(struct wpa_supplicant *wpa_s, const u8 *bssid, struct wpa_ssid *ssid)
{
    ...
    resp = sme_auth_build_sae_commit(wpa_s, ssid, bssid, 1, 0, &use_pt, &use_pk);
    if (!resp)
    {
    ...
    sme_external_auth_build_buf(buf, resp, wpa_s->own_addr, bssid, 1, wpa_s->sme.seq_num, status);
...
static void sme_send_external_auth_status(struct wpa_supplicant *wpa_s, u16 status)
{
...
static int sme_handle_external_auth_start(struct wpa_supplicant *wpa_s, union wpa_event_data *data)
{
    struct wpa_ssid *ssid;
    ...
    {
        if (!wpas_network_disabled(wpa_s, ssid) && ssid_str_len == ssid->ssid_len &&
            os_memcmp(ssid_str, ssid->ssid, ssid_str_len) == 0 &&
            (ssid->key_mgmt & (WPA_KEY_MGMT_SAE | WPA_KEY_MGMT_FT_SAE)))
            break;
    }
    if (!ssid || sme_external_auth_send_sae_commit(wpa_s, data->external_auth.bssid, ssid) < 0)
        return -1;

    return 0;
}

static void sme_external_auth_send_sae_confirm(struct wpa_supplicant *wpa_s, const u8 *da)
{
    struct wpabuf *resp, *buf;
    ...
    wpa_s->sme.seq_num++;
    sme_external_auth_build_buf(buf, resp, wpa_s->own_addr, da, 2, wpa_s->sme.seq_num, WLAN_STATUS_SUCCESS);
    wpa_drv_send_mlme(wpa_s, wpabuf_head(buf), wpabuf_len(buf), 1, 0, 0);
    wpabuf_free(resp);
    wpabuf_free(buf);
}
...
static int sme_check_sae_rejected_groups(struct wpa_supplicant *wpa_s, const struct wpabuf *groups)
{
    size_t i, count;
    const u8 *pos;
    ...
    for (i = 0; i < count; i++)
    {
        ...
        wpa_printf(MSG_DEBUG, "SAE: Rejected group %u is %s", group, enabled ? "enabled" : "disabled");
        if (enabled)
            return 1;
    }
...
static int sme_sae_auth(struct wpa_supplicant *wpa_s,
                        u16 auth_transaction,
                        u16 status_code,
                        const u8 *data,
                        size_t len,
                        ...
                        const u8 *sa)
{
    ...
    if (auth_transaction == 1 && status_code == WLAN_STATUS_ANTI_CLOGGING_TOKEN_REQ &&
        wpa_s->sme.sae.state == SAE_COMMITTED && (external || wpa_s->current_bss) && wpa_s->current_ssid)
    {
        ...
        wpa_hexdump(MSG_DEBUG, "SME: SAE anti-clogging token request", data, len);
        if (len < sizeof(le16))
        {
        ...
        if (h2e)
        {
            if (token_len < 3)
            {
                wpa_dbg(wpa_s, MSG_DEBUG, "SME: Too short SAE anti-clogging token container");
                return -1;
            }
            if (token_pos[0] != WLAN_EID_EXTENSION || token_pos[1] == 0 || token_pos[1] > token_len - 2 ||
                token_pos[2] != WLAN_EID_EXT_ANTI_CLOGGING_TOKEN)
            {
                wpa_dbg(wpa_s, MSG_DEBUG, "SME: Invalid SAE anti-clogging token container header");
                return -1;
            }
        ...
        if (!external)
            sme_send_authentication(wpa_s, wpa_s->current_bss, wpa_s->current_ssid, 2);
        else
            sme_external_auth_send_sae_commit(wpa_s, wpa_s->sme.ext_auth_bssid, wpa_s->current_ssid);
        return 0;
    }
        ...
        wpa_s->sme.sae.state == SAE_COMMITTED && (external || wpa_s->current_bss) && wpa_s->current_ssid)
    {
        ...
        int_array_add_unique(&wpa_s->sme.sae_rejected_groups, wpa_s->sme.sae.group);
        wpa_s->sme.sae_group_index++;
        if (sme_set_sae_group(wpa_s) < 0)
    ...
    if (auth_transaction == 1 && status_code == WLAN_STATUS_UNKNOWN_PASSWORD_IDENTIFIER)
    {
        const u8 *bssid = sa ? sa : wpa_s->pending_bssid;

        wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_SAE_UNKNOWN_PASSWORD_IDENTIFIER MACSTR, MAC2STR(bssid));
        return -1;
    }
        ...
        status_code != WLAN_STATUS_SAE_PK)
    {
        ...
        wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_AUTH_REJECT MACSTR " auth_type=%u auth_transaction=%u status_code=%u",
                MAC2STR(bssid), WLAN_AUTH_SAE, auth_transaction, status_code);
        return -1;
    }
    ...
    if (auth_transaction == 1)
    {
        ...
        if ((!external && wpa_s->current_bss == NULL) || wpa_s->current_ssid == NULL)
            return -1;
        if (wpa_s->sme.sae.state != SAE_COMMITTED)
        {
            wpa_printf(MSG_DEBUG, "SAE: Ignore commit message while waiting for confirm");
            return 0;
        }
        if (wpa_s->sme.sae.h2e && status_code == WLAN_STATUS_SUCCESS)
        {
            wpa_printf(MSG_DEBUG, "SAE: Unexpected use of status code 0 in SAE commit when H2E was expected");
            return -1;
        }
        if ((!wpa_s->sme.sae.h2e || wpa_s->sme.sae.pk) && status_code == WLAN_STATUS_SAE_HASH_TO_ELEMENT)
        {
            wpa_printf(MSG_DEBUG, "SAE: Unexpected use of status code for H2E in SAE commit when H2E was not expected");
            return -1;
        }
        if (!wpa_s->sme.sae.pk && status_code == WLAN_STATUS_SAE_PK)
        {
            wpa_printf(MSG_DEBUG, "SAE: Unexpected use of status code for PK in SAE commit when PK was not expected");
            return -1;
        }
        ...
        res = sae_parse_commit(&wpa_s->sme.sae, data, len, NULL, NULL, groups,
                               status_code == WLAN_STATUS_SAE_HASH_TO_ELEMENT || status_code == WLAN_STATUS_SAE_PK);
        ...
        if (wpa_s->sme.sae.tmp && sme_check_sae_rejected_groups(wpa_s, wpa_s->sme.sae.tmp->peer_rejected_groups))
            return -1;
...
static int sme_sae_set_pmk(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
    ...
    if (wpa_s->conf->sae_pmkid_in_assoc)
    {
         ...
         * into RSNE. */
        if (wpa_s->sme.assoc_req_ie_len + 2 + PMKID_LEN > sizeof(wpa_s->sme.assoc_req_ie))
        {
            wpa_msg(wpa_s, MSG_WARNING, "RSN: Not enough room for inserting own PMKID into RSNE");
            return -1;
        }
        if (wpa_insert_pmkid(wpa_s->sme.assoc_req_ie, &wpa_s->sme.assoc_req_ie_len, wpa_s->sme.sae.pmkid) < 0)
            return -1;
        wpa_hexdump(MSG_DEBUG, "SME: Updated Association Request IEs", wpa_s->sme.assoc_req_ie,
                    wpa_s->sme.assoc_req_ie_len);
    }
...
void sme_external_auth_mgmt_rx(struct wpa_supplicant *wpa_s, const u8 *auth_frame, size_t len)
{
    ...
    if (le_to_host16(header->u.auth.auth_alg) == WLAN_AUTH_SAE)
    {
        ...
        res =
            sme_sae_auth(wpa_s, le_to_host16(header->u.auth.auth_transaction), le_to_host16(header->u.auth.status_code),
                         header->u.auth.variable, len - auth_length, 1, header->sa);
        if (res < 0)
        {
            /* Notify failure to the driver */
            sme_send_external_auth_status(wpa_s, WLAN_STATUS_UNSPECIFIED_FAILURE);
...
void sme_event_auth(struct wpa_supplicant *wpa_s, union wpa_event_data *data)
{
    ...
    wpa_dbg(wpa_s, MSG_DEBUG,
            "SME: Authentication response: peer=" MACSTR " auth_type=%d auth_transaction=%d status_code=%d",
            MAC2STR(data->auth.peer), data->auth.auth_type, data->auth.auth_transaction, data->auth.status_code);
    ...
    {
        int res;
        res = sme_sae_auth(wpa_s, data->auth.auth_transaction, data->auth.status_code, data->auth.ies,
    ...
    if (data->auth.status_code != WLAN_STATUS_SUCCESS)
    {
        ...
        wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_AUTH_REJECT MACSTR " auth_type=%u auth_transaction=%u status_code=%u%s%s",
                MAC2STR(data->auth.peer), data->auth.auth_type, data->auth.auth_transaction, data->auth.status_code,
                ie_txt ? " ie=" : "", ie_txt ? ie_txt : "");
        os_free(ie_txt);
        ...
        if (data->auth.status_code != WLAN_STATUS_NOT_SUPPORTED_AUTH_ALG ||
            wpa_s->sme.auth_alg == data->auth.auth_type || wpa_s->current_ssid->auth_alg == WPA_AUTH_ALG_LEAP)
        {
            wpas_connection_failed(wpa_s, wpa_s->pending_bssid);
            wpa_supplicant_set_state(wpa_s, WPA_DISCONNECTED);
            return;
    ...
    if (data->auth.auth_type == WLAN_AUTH_FT)
    {
        ...
        if (wpa_ft_process_response(wpa_s->wpa, data->auth.ies, data->auth.ies_len, 0, data->auth.peer, ric_ies,
                                    ric_ies_len) < 0)
        {
            wpa_dbg(wpa_s, MSG_DEBUG, "SME: FT Authentication response processing failed");
            wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_DISCONNECTED "bssid=" MACSTR " reason=%d locally_generated=1",
...
#ifdef CONFIG_FILS
    if (data->auth.auth_type == WLAN_AUTH_FILS_SK || data->auth.auth_type == WLAN_AUTH_FILS_SK_PFS)
    {
        u16 expect_auth_type;

        expect_auth_type = wpa_s->sme.auth_alg == WPA_AUTH_ALG_FILS_SK_PFS ? WLAN_AUTH_FILS_SK_PFS : WLAN_AUTH_FILS_SK;
        if (data->auth.auth_type != expect_auth_type)
        {
            wpa_dbg(wpa_s, MSG_DEBUG, "SME: FILS Authentication response used different auth alg (%u; expected %u)",
        ...
        if (fils_process_auth(wpa_s->wpa, wpa_s->pending_bssid, data->auth.ies, data->auth.ies_len) < 0)
        {
            wpa_dbg(wpa_s, MSG_DEBUG, "SME: FILS Authentication response processing failed");
            wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_DISCONNECTED "bssid=" MACSTR " reason=%d locally_generated=1",
                    MAC2STR(wpa_s->pending_bssid), WLAN_REASON_DEAUTH_LEAVING);
            wpas_connection_failed(wpa_s, wpa_s->pending_bssid);
            wpa_supplicant_mark_disassoc(wpa_s);
...
void sme_associate(struct wpa_supplicant *wpa_s, enum wpas_mode mode, const u8 *bssid, u16 auth_type)
{
...
#ifdef CONFIG_FILS
    if (auth_type == WLAN_AUTH_FILS_SK || auth_type == WLAN_AUTH_FILS_SK_PFS)
    {
        struct wpabuf *buf;
        ...
        {
            hlp[num_hlp] = wpabuf_alloc(2 * ETH_ALEN + 6 + wpabuf_len(req->pkt));
            if (!hlp[num_hlp])
                break;
            wpabuf_put_data(hlp[num_hlp], req->dst, ETH_ALEN);
            wpabuf_put_data(hlp[num_hlp], wpa_s->own_addr, ETH_ALEN);
            wpabuf_put_data(hlp[num_hlp], "\xaa\xaa\x03\x00\x00\x00", 6);
        ...
        buf = fils_build_assoc_req(wpa_s->wpa, &params.fils_kek, &params.fils_kek_len, &snonce, &anonce,
                                   (const struct wpabuf **)hlp, num_hlp);
        for (i = 0; i < num_hlp; i++)
            wpabuf_free(hlp[i]);
        if (!buf)
            return;
        wpa_hexdump(MSG_DEBUG, "FILS: assoc_req before FILS elements", wpa_s->sme.assoc_req_ie,
                    wpa_s->sme.assoc_req_ie_len);
#ifdef CONFIG_IEEE80211R
        if (wpa_key_mgmt_ft(wpa_s->key_mgmt))
        {
            ...
            remove_ie(wpa_s->sme.assoc_req_ie, &wpa_s->sme.assoc_req_ie_len, WLAN_EID_RSN);
            wpa_hexdump(MSG_DEBUG, "FILS: assoc_req after RSNE removal", wpa_s->sme.assoc_req_ie,
                        wpa_s->sme.assoc_req_ie_len);
            remove_ie(wpa_s->sme.assoc_req_ie, &wpa_s->sme.assoc_req_ie_len, WLAN_EID_MOBILITY_DOMAIN);
            wpa_hexdump(MSG_DEBUG, "FILS: assoc_req after MDE removal", wpa_s->sme.assoc_req_ie,
                        wpa_s->sme.assoc_req_ie_len);
        }
#endif /* CONFIG_IEEE80211R */
        /* TODO: Make wpa_s->sme.assoc_req_ie use dynamic allocation */
        if (wpa_s->sme.assoc_req_ie_len + wpabuf_len(buf) > sizeof(wpa_s->sme.assoc_req_ie))
        {
            wpa_printf(MSG_ERROR, "FILS: Not enough buffer room for own AssocReq elements");
            wpabuf_free(buf);
            return;
        }
        os_memcpy(wpa_s->sme.assoc_req_ie + wpa_s->sme.assoc_req_ie_len, wpabuf_head(buf), wpabuf_len(buf));
        wpa_s->sme.assoc_req_ie_len += wpabuf_len(buf);
        wpabuf_free(buf);
        wpa_hexdump(MSG_DEBUG, "FILS: assoc_req after FILS elements", wpa_s->sme.assoc_req_ie,
                    wpa_s->sme.assoc_req_ie_len);
...
#ifdef CONFIG_OWE
#ifdef CONFIG_TESTING_OPTIONS
    if (get_ie_ext(wpa_s->sme.assoc_req_ie, wpa_s->sme.assoc_req_ie_len, WLAN_EID_EXT_OWE_DH_PARAM))
    {
        wpa_printf(MSG_INFO, "TESTING: Override OWE DH element");
    }
    else
#endif /* CONFIG_TESTING_OPTIONS */
        if (auth_type == WLAN_AUTH_OPEN && wpa_s->key_mgmt == WPA_KEY_MGMT_OWE)
    {
        ...
        if (!owe_ie)
        {
            wpa_printf(MSG_ERROR, "OWE: Failed to build IE for Association Request frame");
            return;
        }
        if (wpa_s->sme.assoc_req_ie_len + wpabuf_len(owe_ie) > sizeof(wpa_s->sme.assoc_req_ie))
        {
            wpa_printf(MSG_ERROR, "OWE: Not enough buffer room for own Association Request frame elements");
            wpabuf_free(owe_ie);
            return;
        }
        os_memcpy(wpa_s->sme.assoc_req_ie + wpa_s->sme.assoc_req_ie_len, wpabuf_head(owe_ie), wpabuf_len(owe_ie));
...
#ifdef CONFIG_DPP2
    if (DPP_VERSION > 1 && wpa_s->key_mgmt == WPA_KEY_MGMT_DPP && ssid && ssid->dpp_netaccesskey &&
        ssid->dpp_pfs != 2 && !ssid->dpp_pfs_fallback)
    {
        ...
        if (wpa_s->sme.assoc_req_ie_len + wpabuf_len(wpa_s->dpp_pfs->ie) > sizeof(wpa_s->sme.assoc_req_ie))
        {
            wpa_printf(MSG_ERROR, "DPP: Not enough buffer room for own Association Request frame elements");
            dpp_pfs_free(wpa_s->dpp_pfs);
            wpa_s->dpp_pfs = NULL;
            goto pfs_fail;
        }
        os_memcpy(wpa_s->sme.assoc_req_ie + wpa_s->sme.assoc_req_ie_len, wpabuf_head(wpa_s->dpp_pfs->ie),
...
pfs_fail:
    ...
    if (wpa_bss_ext_capab(wpa_s->current_bss, WLAN_EXT_CAPAB_MSCS) && wpa_s->robust_av.valid_config)
    {
        ...
        if ((*wpa_ie_len + wpabuf_len(mscs_ie)) <= max_ie_len)
        {
            ...
            os_memcpy(wpa_s->sme.assoc_req_ie + *wpa_ie_len, wpabuf_head(mscs_ie), mscs_ie_len);
...
mscs_fail:
    ...
    if (ssid && ssid->multi_ap_backhaul_sta)
    {
        ...
        multi_ap_ie_len =
            add_multi_ap_ie(wpa_s->sme.assoc_req_ie + wpa_s->sme.assoc_req_ie_len,
                            sizeof(wpa_s->sme.assoc_req_ie) - wpa_s->sme.assoc_req_ie_len, MULTI_AP_BACKHAUL_STA);
    ...
    wpa_hexdump(MSG_DEBUG, "SME: Association Request IEs", params.wpa_ie, params.wpa_ie_len);
    ...
    if (auth_type == WLAN_AUTH_FT && wpa_s->sme.ft_ies &&
        get_ie(wpa_s->sme.ft_ies, wpa_s->sme.ft_ies_len, WLAN_EID_RIC_DATA))
    {
        ...
        wpa_printf(MSG_INFO, "SME: Linux kernel workaround - do not try to include additional IEs with RIC");
    ...
    else if (auth_type == WLAN_AUTH_FT && wpa_s->sme.ft_ies)
    {
        ...
        remove_ie(wpa_s->sme.assoc_req_ie, &wpa_s->sme.assoc_req_ie_len, WLAN_EID_RSN);
        remove_ie(wpa_s->sme.assoc_req_ie, &wpa_s->sme.assoc_req_ie_len, WLAN_EID_MOBILITY_DOMAIN);
        remove_ie(wpa_s->sme.assoc_req_ie, &wpa_s->sme.assoc_req_ie_len, WLAN_EID_FAST_BSS_TRANSITION);
        rm_en = get_ie(wpa_s->sme.assoc_req_ie, wpa_s->sme.assoc_req_ie_len, WLAN_EID_RRM_ENABLED_CAPABILITIES);
        if (rm_en)
        {
            ...
            remove_ie(wpa_s->sme.assoc_req_ie, &wpa_s->sme.assoc_req_ie_len, WLAN_EID_RRM_ENABLED_CAPABILITIES);
        }
        wpa_hexdump(MSG_DEBUG, "SME: Association Request IEs after FT IE removal", wpa_s->sme.assoc_req_ie,
                    wpa_s->sme.assoc_req_ie_len);
        if (wpa_s->sme.assoc_req_ie_len + wpa_s->sme.ft_ies_len + rm_en_len > sizeof(wpa_s->sme.assoc_req_ie))
        {
            wpa_printf(MSG_ERROR, "SME: Not enough buffer room for FT IEs in Association Request frame");
            os_free(rm_en_dup);
            return;
        }

        os_memmove(wpa_s->sme.assoc_req_ie + wpa_s->sme.ft_ies_len + rm_en_len, wpa_s->sme.assoc_req_ie,
                   wpa_s->sme.assoc_req_ie_len);
        ...
        wpa_s->sme.assoc_req_ie_len += wpa_s->sme.ft_ies_len + rm_en_len;
        params.wpa_ie     = wpa_s->sme.assoc_req_ie;
        params.wpa_ie_len = wpa_s->sme.assoc_req_ie_len;
        wpa_hexdump(MSG_DEBUG, "SME: Association Request IEs after FT override", params.wpa_ie, params.wpa_ie_len);
    }
#endif /* CONFIG_IEEE80211R */
    params.mode                  = mode;
    ...
    wpa_msg(wpa_s, MSG_INFO, "Trying to associate with " MACSTR " (SSID='%s' freq=%d MHz)", MAC2STR(params.bssid),
            params.ssid ? wpa_ssid_txt(params.ssid, params.ssid_len) : "", params.freq.freq);

    wpa_supplicant_set_state(wpa_s, WPA_ASSOCIATING);

    if (params.wpa_ie == NULL || ieee802_11_parse_elems(params.wpa_ie, params.wpa_ie_len, &elems, 0) < 0)
    {
    ...
    else if (elems.osen)
    {
        ...
        wpa_sm_set_assoc_wpa_ie(wpa_s->wpa, NULL, 0);
    if (elems.rsnxe)
        wpa_sm_set_assoc_rsnxe(wpa_s->wpa, elems.rsnxe - 2, elems.rsnxe_len + 2);
    else
        wpa_sm_set_assoc_rsnxe(wpa_s->wpa, NULL, 0);
    ...
    if (params.wpa_ie)
        wpa_s->last_assoc_req_wpa_ie = wpabuf_alloc_copy(params.wpa_ie, params.wpa_ie_len);
#endif /* CONFIG_TESTING_OPTIONS */
}
...
static void sme_send_2040_bss_coex(struct wpa_supplicant *wpa_s, const u8 *chan_list, u8 num_channels, u8 num_intol)
{
    ...
    wpa_printf(MSG_DEBUG, "SME: Send 20/40 BSS Coexistence to " MACSTR " (num_channels=%u num_intol=%u)",
    ...
    buf = wpabuf_alloc(2 + /* action.category + action_code */
                       sizeof(struct ieee80211_2040_bss_coex_ie) + sizeof(struct ieee80211_2040_intol_chan_report) +
                       num_channels);
    if (buf == NULL)
        return;
    ...
    if (wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, wpa_s->bssid, wpa_s->own_addr, wpa_s->bssid, wpabuf_head(buf),
                            wpabuf_len(buf), 0) < 0)
    {
...
int sme_proc_obss_scan(struct wpa_supplicant *wpa_s)
{
    ...
    dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list)
    {
        ...
        wpa_printf(MSG_DEBUG, "SME OBSS scan BSS " MACSTR " freq=%u chan=%u ht_cap=0x%x", MAC2STR(bss->bssid),
                   bss->freq, channel, ht_cap);
...
void sme_sched_obss_scan(struct wpa_supplicant *wpa_s, int enable)
{
     ...
     */
    if (!((wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME) || (wpa_s->drv_flags & WPA_DRIVER_FLAGS_OBSS_SCAN)) ||
...
static void sme_send_sa_query_req(struct wpa_supplicant *wpa_s, const u8 *trans_id)
{
    ...
    if (wpa_sm_ocv_enabled(wpa_s->wpa))
    {
        ...
        {
            wpa_printf(MSG_WARNING, "Failed to get channel info for OCI element in SA Query Request frame");
            return;
        }
        ...
        {
            wpa_printf(MSG_INFO, "TEST: Override SA Query Request OCI frequency %d -> %d MHz", ci.frequency,
    ...
    if (wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, wpa_s->bssid, wpa_s->own_addr, wpa_s->bssid, req, req_len, 0) <
        0)
        wpa_msg(wpa_s, MSG_INFO,
...
void sme_event_ch_switch(struct wpa_supplicant *wpa_s)
{
    ...
    if (wpa_s->wpa_state != WPA_COMPLETED || !wpa_sm_ocv_enabled(wpa_s->wpa))
        return;

    wpa_dbg(wpa_s, MSG_DEBUG, "SME: Channel switch completed - trigger new SA Query to verify new operating channel");
    sme_stop_sa_query(wpa_s);
...
static void sme_process_sa_query_request(struct wpa_supplicant *wpa_s, const u8 *sa, const u8 *data, size_t len)
{
    u8 resp[2 + WLAN_SA_QUERY_TR_ID_LEN + OCV_OCI_EXTENDED_LEN];
    u8 resp_len = 2 + WLAN_SA_QUERY_TR_ID_LEN;

    wpa_dbg(wpa_s, MSG_DEBUG, "SME: Sending SA Query Response to " MACSTR, MAC2STR(wpa_s->bssid));
    ...
    if (wpa_sm_ocv_enabled(wpa_s->wpa))
    {
        ...
        if (wpa_drv_channel_info(wpa_s, &ci) != 0)
        {
            wpa_printf(MSG_WARNING, "Failed to get channel info for OCI element in SA Query Response frame");
            return;
        }
        ...
        {
            wpa_printf(MSG_INFO, "TEST: Override SA Query Response OCI frequency %d -> %d MHz", ci.frequency,
    ...
    if (wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, wpa_s->bssid, wpa_s->own_addr, wpa_s->bssid, resp, resp_len,
                            0) < 0)
        wpa_msg(wpa_s, MSG_INFO, "SME: Failed to send SA Query Response");
}

static void sme_process_sa_query_response(struct wpa_supplicant *wpa_s, const u8 *sa, const u8 *data, size_t len)
{
    int i;
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "SME: Received SA Query response from " MACSTR " (trans_id %02x%02x)", MAC2STR(sa),
            data[1], data[2]);
    ...
    {
        if (os_memcmp(wpa_s->sme.sa_query_trans_id + i * WLAN_SA_QUERY_TR_ID_LEN, data + 1, WLAN_SA_QUERY_TR_ID_LEN) ==
            0)
            break;
    }
...
void sme_sa_query_rx(struct wpa_supplicant *wpa_s, const u8 *da, const u8 *sa, const u8 *data, size_t len)
{
    if (len < 1 + WLAN_SA_QUERY_TR_ID_LEN)
        return;
    if (is_multicast_ether_addr(da))
    {
        wpa_printf(MSG_DEBUG, "IEEE 802.11: Ignore group-addressed SA Query frame (A1=" MACSTR " A2=" MACSTR ")",
                   MAC2STR(da), MAC2STR(sa));
        return;
    }

    wpa_dbg(wpa_s, MSG_DEBUG, "SME: Received SA Query frame from " MACSTR " (trans_id %02x%02x)", MAC2STR(sa), data[1],
            data[2]);
    ...
    if (wpa_sm_ocv_enabled(wpa_s->wpa))
    {
        ...
        if (ieee802_11_parse_elems(data + 1 + WLAN_SA_QUERY_TR_ID_LEN, len - 1 - WLAN_SA_QUERY_TR_ID_LEN, &elems, 1) ==
            ParseFailed)
        {
            wpa_printf(MSG_DEBUG, "SA Query: Failed to parse elements");
            return;
        }

        if (wpa_drv_channel_info(wpa_s, &ci) != 0)
        {
            wpa_printf(MSG_WARNING, "Failed to get channel info to validate received OCI in SA Query Action frame");
            return;
        }
        ...
        if (ocv_verify_tx_params(elems.oci, elems.oci_len, &ci, channel_width_to_int(ci.chanwidth), ci.seg1_idx) !=
            OCI_SUCCESS)
        {
            wpa_msg(wpa_s, MSG_INFO, OCV_FAILURE "addr=" MACSTR " frame=saquery%s error=%s", MAC2STR(sa),
                    data[0] == WLAN_SA_QUERY_REQUEST ? "req" : "resp", ocv_errorstr);
            return;
        }
    }
#endif /* CONFIG_OCV */
