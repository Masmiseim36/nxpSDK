...
static int wmm_ac_add_ts(struct wpa_supplicant *wpa_s, const u8 *addr, const struct wmm_tspec_element *tspec)
{
    ...
    if (dir != WMM_AC_DIR_DOWNLINK)
    {
        ...
        wpa_printf(MSG_DEBUG, "WMM AC: Add TS: addr=" MACSTR " TSID=%u admitted time=%u, ret=%d", MAC2STR(addr), tsid,
                   admitted_time, ret);
        if (ret < 0)
        {
            os_free(_tspec);
    ...
    wpa_msg(wpa_s, MSG_INFO, WMM_AC_EVENT_TSPEC_ADDED "tsid=%d addr=" MACSTR " admitted_time=%d", tsid, MAC2STR(addr),
            admitted_time);
...
static void wmm_ac_addts_req_timeout(void *eloop_ctx, void *timeout_ctx)
{
    ...
    wpa_printf(MSG_DEBUG, "Timeout getting ADDTS response (tsid=%d up=%d)", wmm_ac_get_tsid(&addts_req->tspec),
...
static int wmm_ac_send_addts_request(struct wpa_supplicant *wpa_s, const struct wmm_ac_addts_request *req)
{
    ...
    ret = wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, req->address, wpa_s->own_addr, wpa_s->bssid,
...
static int wmm_ac_send_delts(struct wpa_supplicant *wpa_s, const struct wmm_tspec_element *tspec, const u8 *address)
{
    ...
    ret = wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, address, wpa_s->own_addr, wpa_s->bssid, wpabuf_head(buf),
                              wpabuf_len(buf), 0);
    if (ret)
...
static struct wmm_ac_addts_request *wmm_ac_build_addts_req(struct wpa_supplicant *wpa_s,
...
#define PARAM_IN_RANGE(field, min_value, max_value) param_in_range(#field, params->field, min_value, max_value)

    if (!PARAM_IN_RANGE(tsid, 0, WMM_AC_MAX_TID) || !PARAM_IN_RANGE(user_priority, 0, WMM_AC_MAX_USER_PRIORITY) ||
        !PARAM_IN_RANGE(nominal_msdu_size, 1, WMM_AC_MAX_NOMINAL_MSDU) || !PARAM_IN_RANGE(mean_data_rate, 1, -1) ||
        !PARAM_IN_RANGE(minimum_phy_rate, 1, -1) ||
        !PARAM_IN_RANGE(surplus_bandwidth_allowance, WMM_AC_MIN_SBA_UNITY, -1))
        return 0;
#undef PARAM_IN_RANGE

    if (!(params->direction == WMM_TSPEC_DIRECTION_UPLINK || params->direction == WMM_TSPEC_DIRECTION_DOWNLINK ||
...
static struct wmm_ac_assoc_data *wmm_ac_process_param_elem(struct wpa_supplicant *wpa_s, const u8 *ies, size_t ies_len)
{
    ...
    wpa_printf(MSG_DEBUG, "WMM AC: AC mandatory: AC_BE=%u AC_BK=%u AC_VI=%u AC_VO=%u",
               assoc_data->ac_params[WMM_AC_BE].acm, assoc_data->ac_params[WMM_AC_BK].acm,
               assoc_data->ac_params[WMM_AC_VI].acm, assoc_data->ac_params[WMM_AC_VO].acm);
...
void wmm_ac_notify_assoc(struct wpa_supplicant *wpa_s,
                         const u8 *ies,
                         size_t ies_len,
                         const struct wmm_params *wmm_params)
{
...
int wpas_wmm_ac_addts(struct wpa_supplicant *wpa_s, struct wmm_ac_ts_setup_params *params)
{
    ...
    wpa_printf(MSG_DEBUG, "WMM AC: TS setup request (addr=" MACSTR " tsid=%u user priority=%u direction=%d)",
               MAC2STR(wpa_s->bssid), params->tsid, params->user_priority, params->direction);
...
static void wmm_ac_handle_addts_resp(struct wpa_supplicant *wpa_s,
                                     const u8 *sa,
                                     ...
                                     const struct wmm_tspec_element *tspec)
{
    ...
    if (os_memcmp(req->address, sa, ETH_ALEN) != 0 || tsid != wmm_ac_get_tsid(&req->tspec) ||
        up != wmm_ac_get_user_priority(&req->tspec) || dir != wmm_ac_get_direction(&req->tspec))
    {
    ...
    wpa_printf(MSG_DEBUG,
               "WMM AC: adding a new TS with TSID=%u address=" MACSTR " medium time=%u access category=%d dir=%d ",
...
void wmm_ac_rx_action(struct wpa_supplicant *wpa_s, const u8 *da, const u8 *sa, const u8 *data, size_t len)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "WMM AC: frame destination addr=" MACSTR " is other than ours, ignoring frame",
                   MAC2STR(da));
        return;
    }
...
int wpas_wmm_ac_status(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
    ...
    for (ac = 0; ac < WMM_AC_NUM; ac++)
    {
        ...
        pos += wpa_scnprintf(buf + pos, buflen - pos, "%s: acm=%d uapsd=%d\n", get_ac_str(ac),
                             assoc_info->ac_params[ac].acm, assoc_info->ac_params[ac].uapsd);
