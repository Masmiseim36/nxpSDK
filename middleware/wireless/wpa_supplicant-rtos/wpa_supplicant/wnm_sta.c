...
static int ieee80211_11_set_tfs_ie(
    struct wpa_supplicant *wpa_s, const u8 *addr, const u8 *buf, u16 buf_len, enum wnm_oper oper)
{
    u16 len = buf_len;
...
int ieee802_11_send_wnmsleep_req(struct wpa_supplicant *wpa_s, u8 action, u16 intval, struct wpabuf *tfs_req)
{
    ...
    if (action == WNM_SLEEP_MODE_EXIT && wpa_sm_ocv_enabled(wpa_s->wpa))
    {
        ...
        {
            wpa_printf(MSG_WARNING, "Failed to get channel info for OCI element in WNM-Sleep Mode frame");
            os_free(wnmsleep_ie);
            os_free(wnmtfs_ie);
            return -1;
        }
        ...
        if (!oci_ie)
        {
            wpa_printf(MSG_WARNING, "Failed to allocate buffer for for OCI element in WNM-Sleep Mode frame");
            os_free(wnmsleep_ie);
            os_free(wnmtfs_ie);
            return -1;
        }
    ...
    if (oci_ie_len > 0)
    {
        os_memcpy(mgmt->u.action.u.wnm_sleep_req.variable + wnmsleep_ie_len + wnmtfs_ie_len, oci_ie, oci_ie_len);
    }
#endif /* CONFIG_OCV */

    len = 1 + sizeof(mgmt->u.action.u.wnm_sleep_req) + wnmsleep_ie_len + wnmtfs_ie_len + oci_ie_len;
...
static void wnm_sleep_mode_exit_success(struct wpa_supplicant *wpa_s, const u8 *frm, u16 key_len_total)
{
    ...
    {
        wpa_msg(wpa_s, MSG_INFO, "WNM: Ignore Key Data in WNM-Sleep Mode Response - PMF not enabled");
        return;
    }
    ...
    while (end - ptr > 1)
    {
        ...
        else if (*ptr == WNM_SLEEP_SUBELEM_BIGTK)
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "WNM: Too short BIGTK subelem");
                break;
            }
            wpa_wnmsleep_install_key(wpa_s->wpa, WNM_SLEEP_SUBELEM_BIGTK, ptr);
...
static void ieee802_11_rx_wnmsleep_resp(struct wpa_supplicant *wpa_s, const u8 *frm, int len)
{
    ...
    while (pos - frm + 1 < len)
    {
        ...
        else if (*pos == WLAN_EID_EXTENSION && ie_len >= 1 && pos[2] == WLAN_EID_EXT_OCV_OCI)
        {
            oci_ie     = pos + 3;
            oci_ie_len = ie_len - 1;
#endif /* CONFIG_OCV */
...
#ifdef CONFIG_OCV
    if (wnmsleep_ie->action_type == WNM_SLEEP_MODE_EXIT && wpa_sm_ocv_enabled(wpa_s->wpa))
    {
        struct wpa_channel_info ci;

        if (wpa_drv_channel_info(wpa_s, &ci) != 0)
        {
            wpa_msg(wpa_s, MSG_WARNING, "Failed to get channel info to validate received OCI in WNM-Sleep Mode frame");
            return;
        }

        if (ocv_verify_tx_params(oci_ie, oci_ie_len, &ci, channel_width_to_int(ci.chanwidth), ci.seg1_idx) !=
            OCI_SUCCESS)
        {
...
static void wnm_parse_neighbor_report_elem(struct neighbor_report *rep, u8 id, u8 elen, const u8 *pos)
{
    switch (id)
    {
        ...
        case WNM_NEIGHBOR_BSS_TERMINATION_DURATION:
            if (elen < 10)
            {
                wpa_printf(MSG_DEBUG, "WNM: Too short BSS termination duration");
                break;
            }
...
#ifdef CONFIG_MBO
static struct wpa_bss *get_mbo_transition_candidate(struct wpa_supplicant *wpa_s,
                                                    enum mbo_transition_reject_reason *reason)
{
    ...
    for (i = 0; i < info->num; i++)
    {
        ...
        {
            target = wpa_bss_get_bssid(wpa_s, info->candidates[i].bssid);
            goto end;
        }
    }
    ...
    if (wpa_s->wnm_mode & WNM_BSS_TM_REQ_DISASSOC_IMMINENT)
    {
        ...
        {
            target = wpa_bss_get_bssid(wpa_s, info->candidates[i].bssid);
            if (target && (target->level < wpa_s->conf->disassoc_imminent_rssi_threshold))
                continue;
            goto end;
        }
    }
    ...
    if (reason)
    {
        for (i = 0; i < info->num; i++)
        {
            if (first_candidate_bssid && os_memcmp(first_candidate_bssid, info->candidates[i].bssid, ETH_ALEN) == 0)
            {
                                                     ...
                                                     enum mbo_transition_reject_reason *reason)
{
    ...
    for (i = 0; i < wpa_s->wnm_num_neighbor_report; i++)
    {
        ...
        if (!target)
        {
            wpa_printf(MSG_DEBUG, "Candidate BSS " MACSTR " (pref %d) not found in scan results", MAC2STR(nei->bssid),
        ...
        if (age_secs)
        {
            ...
            if (os_get_reltime(&now) == 0 && os_reltime_expired(&now, &target->last_update, age_secs))
            {
                wpa_printf(MSG_DEBUG, "Candidate BSS is more than %lld seconds old", age_secs);
                continue;
            }
        }
        ...
        if (bss->ssid_len != target->ssid_len || os_memcmp(bss->ssid, target->ssid, bss->ssid_len) != 0)
        {
             ...
             */
            wpa_printf(MSG_DEBUG, "Candidate BSS " MACSTR " (pref %d) in different ESS", MAC2STR(nei->bssid),
                       nei->preference_present ? nei->preference : -1);
            continue;
        }

        if (wpa_s->current_ssid && !wpa_scan_res_match(wpa_s, 0, target, wpa_s->current_ssid, 1, 0))
        {
            wpa_printf(MSG_DEBUG, "Candidate BSS " MACSTR " (pref %d) does not match the current network profile",
        ...
        {
            wpa_printf(MSG_DEBUG, "MBO: Candidate BSS " MACSTR " retry delay is not over yet", MAC2STR(nei->bssid));
            continue;
        }

        if (target->level < bss->level && target->level < -80)
        {
            wpa_printf(MSG_DEBUG, "Candidate BSS " MACSTR " (pref %d) does not have sufficient signal level (%d)",
                       MAC2STR(nei->bssid), nei->preference_present ? nei->preference : -1, target->level);
            continue;
        }
    ...
    if (target)
    {
        wpa_printf(MSG_DEBUG, "WNM: Found an acceptable preferred transition candidate BSS " MACSTR " (RSSI %d)",
...
static int wnm_add_nei_rep(
    struct wpabuf **buf, const u8 *bssid, u32 bss_info, u8 op_class, u8 chan, u8 phy_type, u8 pref)
{
    if (wpabuf_len(*buf) + 18 > IEEE80211_MAX_MMPDU_SIZE - IEEE80211_HDRLEN)
    {
        wpa_printf(MSG_DEBUG, "WNM: No room in frame for Neighbor Report element");
        return -1;
    }

    if (wpabuf_resize(buf, 18) < 0)
    {
        wpa_printf(MSG_DEBUG, "WNM: Failed to allocate memory for Neighbor Report element");
        return -1;
    }
...
static int wnm_nei_rep_add_bss(struct wpa_supplicant *wpa_s, struct wpa_bss *bss, struct wpabuf **buf, u8 pref)
{
    const u8 *ie;
    u8 op_class, chan;
    ...
    if (ie && ie[1] >= 1)
    {
        ...
        if (vht_oper->vht_op_info_chwidth == CHANWIDTH_80MHZ || vht_oper->vht_op_info_chwidth == CHANWIDTH_160MHZ ||
            vht_oper->vht_op_info_chwidth == CHANWIDTH_80P80MHZ)
            vht = vht_oper->vht_op_info_chwidth;
    }

    if (ieee80211_freq_to_channel_ext(bss->freq, sec_chan, vht, &op_class, &chan) == NUM_HOSTAPD_MODES)
    {
        wpa_printf(MSG_DEBUG, "WNM: Cannot determine operating class and channel");
        return -2;
    }

    phy_type = ieee80211_get_phy_type(bss->freq, (ht_oper != NULL), (vht_oper != NULL));
    if (phy_type == PHY_TYPE_UNSPECIFIED)
    {
        wpa_printf(MSG_DEBUG, "WNM: Cannot determine BSS phy type for Neighbor Report");
        return -2;
    }

    info = wnm_get_bss_info(wpa_s, bss);

    return wnm_add_nei_rep(buf, bss->bssid, info, op_class, chan, phy_type, pref);
}
...
static void wnm_add_cand_list(struct wpa_supplicant *wpa_s, struct wpabuf **buf)
{
    ...
    wpa_printf(MSG_DEBUG, "WNM: Add candidate list to BSS Transition Management Response frame");
    ...
    wpa_hexdump_buf(MSG_DEBUG, "WNM: BSS Transition Management Response candidate list", *buf);
}
                                              ...
                                              const u8 *target_bssid)
{
    ...
    if (!buf)
    {
        wpa_printf(MSG_DEBUG, "WNM: Failed to allocate memory for BTM response");
        return;
    }
...
#ifdef CONFIG_MBO
    if (status != WNM_BSS_TM_ACCEPT && wpa_bss_get_vendor_ie(wpa_s->current_bss, MBO_IE_VENDOR_TYPE))
    {
        u8 mbo[10];
        size_t ret;

        ret = wpas_mbo_ie_bss_trans_reject(wpa_s, mbo, sizeof(mbo), reason);
        if (ret)
        {
            if (wpabuf_resize(&buf, ret) < 0)
            {
                wpabuf_free(buf);
                wpa_printf(MSG_DEBUG, "WNM: Failed to allocate memory for MBO IE");
                return;
            }
                               ...
                               int after_new_scan)
{
    ...
    wpa_dbg(wpa_s, MSG_DEBUG,
            "WNM: Transition to BSS " MACSTR " based on BSS Transition Management Request (old BSSID " MACSTR
            " after_new_scan=%d)",
    ...
    {
        wpa_s->wnm_reply = 0;
        wpa_printf(MSG_DEBUG, "WNM: Sending successful BSS Transition Management Response");
        wnm_send_bss_transition_mgmt_resp(wpa_s, wpa_s->wnm_dialog_token, WNM_BSS_TM_ACCEPT,
                                          MBO_TRANSITION_REJECT_REASON_UNSPECIFIED, 0, bss->bssid);
    }
...
int wnm_scan_process(struct wpa_supplicant *wpa_s, int reply_on_fail)
{
    ...
    enum mbo_transition_reject_reason reason = MBO_TRANSITION_REJECT_REASON_UNSPECIFIED;

    if (!wpa_s->wnm_neighbor_report_elements)
        return 0;

    wpa_dbg(wpa_s, MSG_DEBUG, "WNM: Process scan results for BSS Transition Management");
...
send_bss_resp_fail:
    ...
    {
        wpa_s->wnm_reply = 0;
        wnm_send_bss_transition_mgmt_resp(wpa_s, wpa_s->wnm_dialog_token, status, reason, 0, NULL);
    }
    wnm_deallocate_memory(wpa_s);
...
static void wnm_sort_cand_list(struct wpa_supplicant *wpa_s)
{
    if (!wpa_s->wnm_neighbor_report_elements)
        return;
    qsort(wpa_s->wnm_neighbor_report_elements, wpa_s->wnm_num_neighbor_report, sizeof(struct neighbor_report),
          cand_pref_compar);
}
...
static void wnm_dump_cand_list(struct wpa_supplicant *wpa_s)
{
    ...
    for (i = 0; i < wpa_s->wnm_num_neighbor_report; i++)
    {
        ...
        nei = &wpa_s->wnm_neighbor_report_elements[i];
        wpa_printf(MSG_DEBUG, "%u: " MACSTR " info=0x%x op_class=%u chan=%u phy=%u pref=%d freq=%d", i,
                   MAC2STR(nei->bssid), nei->bssid_info, nei->regulatory_class, nei->channel_number, nei->phy_type,
...
static void wnm_set_scan_freqs(struct wpa_supplicant *wpa_s)
{
    ...
    for (i = 0; i < wpa_s->wnm_num_neighbor_report; i++)
    {
        ...
        if (nei->freq <= 0)
        {
            wpa_printf(MSG_DEBUG, "WNM: Unknown neighbor operating frequency for " MACSTR " - scan all channels",
                       MAC2STR(nei->bssid));
            os_free(freqs);
            return;
        }
...
static int wnm_fetch_scan_results(struct wpa_supplicant *wpa_s)
{
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "WNM: Fetch current scan results from the driver for checking transition candidates");
    ...
    for (i = 0; i < wpa_s->wnm_num_neighbor_report; i++)
    {
        ...
        for (j = 0; j < scan_res->num; j++)
        {
            ...
            res = scan_res->res[j];
            if (os_memcmp(nei->bssid, res->bssid, ETH_ALEN) != 0 || res->age > WNM_SCAN_RESULT_AGE * 1000)
                continue;
            bss     = wpa_s->current_bss;
            ssid_ie = wpa_scan_get_ie(res, WLAN_EID_SSID);
            if (bss && ssid_ie && ssid_ie[1] &&
                (bss->ssid_len != ssid_ie[1] || os_memcmp(bss->ssid, ssid_ie + 2, bss->ssid_len) != 0))
    ...
    if (!found)
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "WNM: No transition candidate matches existing scan results");
        return 0;
    }
    ...
    if (!bss)
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "WNM: Comparison of scan results against transition candidates did not find matches");
        return 0;
    }
...
static void ieee802_11_rx_bss_trans_mgmt_req(struct wpa_supplicant *wpa_s, const u8 *pos, const u8 *end, int reply)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               wpa_s->wnm_dialog_token, wpa_s->wnm_mode, wpa_s->wnm_dissoc_timer, valid_int);

#if defined(CONFIG_MBO) && defined(CONFIG_TESTING_OPTIONS)
    if (wpa_s->reject_btm_req_reason)
    {
        wpa_printf(MSG_INFO, "WNM: Testing - reject BSS Transition Management Request: reject_btm_req_reason=%d",
                   wpa_s->reject_btm_req_reason);
        wnm_send_bss_transition_mgmt_resp(wpa_s, wpa_s->wnm_dialog_token, wpa_s->reject_btm_req_reason,
    ...
    if (wpa_s->wnm_mode & WNM_BSS_TM_REQ_ESS_DISASSOC_IMMINENT)
    {
        ...
        wpa_msg(wpa_s, MSG_INFO, ESS_DISASSOC_IMMINENT "%d %u %s", wpa_sm_pmf_enabled(wpa_s->wpa),
    ...
    if (wpa_s->wnm_mode & WNM_BSS_TM_REQ_PREF_CAND_LIST_INCLUDED)
    {
        ...
        while (end - pos >= 2 && wpa_s->wnm_num_neighbor_report < WNM_MAX_NEIGHBOR_REPORT)
        {
            ...
            if (tag == WLAN_EID_NEIGHBOR_REPORT)
            {
                ...
                {
                    rep->is_first = 1;
                    wpa_printf(MSG_DEBUG, "WNM: First transition candidate is " MACSTR, MAC2STR(rep->bssid));
                }
#endif /* CONFIG_MBO */
            }
        ...
        {
            wpa_printf(MSG_DEBUG, "WNM: Candidate list included bit is set, but no candidates found");
            wnm_send_bss_transition_mgmt_resp(wpa_s, wpa_s->wnm_dialog_token, WNM_BSS_TM_REJECT_NO_SUITABLE_CANDIDATES,
                                              MBO_TRANSITION_REJECT_REASON_UNSPECIFIED, 0, NULL);
            return;
        }
        ...
        {
            os_memcpy(wpa_s->next_scan_bssid, wpa_s->wnm_neighbor_report_elements[0].bssid, ETH_ALEN);
            wpa_printf(MSG_DEBUG, "WNM: Scan only for a specific BSSID since there is only a single candidate " MACSTR,
    ...
    else if (reply)
    {
        ...
        wnm_send_bss_transition_mgmt_resp(wpa_s, wpa_s->wnm_dialog_token, status,
                                          MBO_TRANSITION_REJECT_REASON_UNSPECIFIED, 0, NULL);
    }
}
                                       ...
                                       int cand_list)
{
    ...
    wpa_printf(MSG_DEBUG, "WNM: Send BSS Transition Management Query to " MACSTR " query_reason=%u%s",
               MAC2STR(wpa_s->bssid), query_reason, cand_list ? " candidate list" : "");
    ...
    if (btm_candidates)
    {
        ...
        ret = ieee802_11_parse_candidate_list(btm_candidates, wpabuf_put(buf, 0), max_len);
        if (ret < 0)
        {
            wpabuf_free(buf);
            return ret;
...
static void ieee802_11_rx_wnm_notif_req_wfa(struct wpa_supplicant *wpa_s, const u8 *sa, const u8 *data, int len)
{
    ...
    while (end - pos > 1)
    {
...
#ifdef CONFIG_HS20
        if (ie == WLAN_EID_VENDOR_SPECIFIC && ie_len >= 5 && WPA_GET_BE24(pos) == OUI_WFA &&
        ...
        if (ie == WLAN_EID_VENDOR_SPECIFIC && ie_len >= 8 && WPA_GET_BE24(pos) == OUI_WFA &&
            ...
            pos[3] == HS20_WNM_T_C_ACCEPTANCE)
        {
            ...
            pos += 4;
            url_len = *pos++;
            wpa_printf(MSG_DEBUG, "WNM: HS 2.0 Terms and Conditions Acceptance (URL Length %u)", url_len);
            if (url_len > ie_end - pos)
                break;
            url = os_malloc(url_len + 1);
            if (!url)
...
static void ieee802_11_rx_wnm_coloc_intf_req(struct wpa_supplicant *wpa_s, const u8 *sa, const u8 *frm, int len)
{
    ...
    if (wpa_s->wpa_state != WPA_COMPLETED || os_memcmp(sa, wpa_s->bssid, ETH_ALEN) != 0)
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "WNM: Collocated Interference Request frame not from current AP - ignore it");
        return;
    }

    wpa_msg(wpa_s, MSG_INFO, COLOC_INTF_REQ "%u %u %u", dialog_token, auto_report, timeout);
...
void ieee802_11_rx_wnm_action(struct wpa_supplicant *wpa_s, const struct ieee80211_mgmt *mgmt, size_t len)
{
    ...
    switch (act)
    {
        ...
        case WNM_COLLOCATED_INTERFERENCE_REQ:
            ieee802_11_rx_wnm_coloc_intf_req(wpa_s, mgmt->sa, pos, end - pos);
            break;
        default:
...
int wnm_send_coloc_intf_report(struct wpa_supplicant *wpa_s, u8 dialog_token, const struct wpabuf *elems)
{
    struct wpabuf *buf;
    int ret;
    ...
    wpa_printf(MSG_DEBUG, "WNM: Send Collocated Interference Report to " MACSTR " (dialog token %u)",
...
void wnm_set_coloc_intf_elems(struct wpa_supplicant *wpa_s, struct wpabuf *elems)
{
    wpabuf_free(wpa_s->coloc_intf_elems);
    ...
    if (wpa_s->conf->coloc_intf_reporting && wpa_s->coloc_intf_elems && wpa_s->coloc_intf_dialog_token &&
        (wpa_s->coloc_intf_auto_report == 1 || wpa_s->coloc_intf_auto_report == 3))
    {
        /* TODO: Check that there has not been less than
         * wpa_s->coloc_intf_timeout * 200 TU from the last report.
         */
        wnm_send_coloc_intf_report(wpa_s, wpa_s->coloc_intf_dialog_token, wpa_s->coloc_intf_elems);
    }
}
