...
int wpa_set_wep_keys(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
    ...
    for (i = 0; i < NUM_WEP_KEYS; i++)
    {
        ...
        set = 1;
        wpa_drv_set_key(wpa_s, WPA_ALG_WEP, NULL, i, i == ssid->wep_tx_keyidx, NULL, 0, ssid->wep_key[i],
                        ssid->wep_key_len[i],
                        i == ssid->wep_tx_keyidx ? KEY_FLAG_GROUP_RX_TX_DEFAULT : KEY_FLAG_GROUP_RX_TX);
    }
...
int wpa_supplicant_set_wpa_none_key(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
    ...
    ret = wpa_drv_set_key(wpa_s, alg, NULL, 0, 1, seq, 6, key, keylen, KEY_FLAG_GROUP_RX_TX_DEFAULT);
...
static void wpa_supplicant_timeout(void *eloop_ctx, void *timeout_ctx)
{
    ...
    if (!is_zero_ether_addr(wpa_s->pending_bssid) &&
        (wpa_s->wpa_state == WPA_AUTHENTICATING || wpa_s->wpa_state == WPA_ASSOCIATING))
...
void wpas_auth_timeout_restart(struct wpa_supplicant *wpa_s, int sec_diff)
{
    ...
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "Authentication timeout restart: %d sec", new_sec);
        eloop_cancel_timeout(wpa_supplicant_timeout, wpa_s, NULL);
        eloop_register_timeout(new_sec, 0, wpa_supplicant_timeout, wpa_s, NULL);
    }
}
...
void wpa_supplicant_initiate_eapol(struct wpa_supplicant *wpa_s)
{
    ...
    eapol_conf.eap_disabled = !wpa_key_mgmt_wpa_ieee8021x(wpa_s->key_mgmt) &&
                              wpa_s->key_mgmt != WPA_KEY_MGMT_IEEE8021X_NO_WPA && wpa_s->key_mgmt != WPA_KEY_MGMT_WPS;
...
static void remove_bss_tmp_disallowed_entry(struct wpa_supplicant *wpa_s, struct wpa_bss_tmp_disallowed *bss)
{
...
void free_bss_tmp_disallowed(struct wpa_supplicant *wpa_s)
{
    ...
    dl_list_for_each_safe(bss, prev, &wpa_s->bss_tmp_disallowed, struct wpa_bss_tmp_disallowed, list)
...
void wpas_flush_fils_hlp_req(struct wpa_supplicant *wpa_s)
{
    ...
    while ((req = dl_list_first(&wpa_s->fils_hlp_req, struct fils_hlp_req, list)) != NULL)
    {
...
void wpas_clear_driver_signal_override(struct wpa_supplicant *wpa_s)
{
    ...
    while ((dso = dl_list_first(&wpa_s->drv_signal_override, struct driver_signal_override, list)))
    {
...
static void wpa_supplicant_cleanup(struct wpa_supplicant *wpa_s)
{
...
#ifdef CONFIG_MESH
    {
        ...
        while ((entry = dl_list_last(&wpa_s->mesh_external_pmksa_cache, struct external_pmksa_cache, list)) != NULL)
        {
...
void wpa_clear_keys(struct wpa_supplicant *wpa_s, const u8 *addr)
{
    ...
    for (i = 0; i < max; i++)
    {
        if (wpa_s->keys_cleared & BIT(i))
            continue;
        wpa_drv_set_key(wpa_s, WPA_ALG_NONE, NULL, i, 0, NULL, 0, NULL, 0, KEY_FLAG_GROUP);
    }
    /* Pairwise Key ID 1 for Extended Key ID is tracked in bit 15 */
    if (~wpa_s->keys_cleared & (BIT(0) | BIT(15)) && addr && !is_zero_ether_addr(addr))
    {
        if (!(wpa_s->keys_cleared & BIT(0)))
            wpa_drv_set_key(wpa_s, WPA_ALG_NONE, addr, 0, 0, NULL, 0, NULL, 0, KEY_FLAG_PAIRWISE);
        if (!(wpa_s->keys_cleared & BIT(15)))
            wpa_drv_set_key(wpa_s, WPA_ALG_NONE, addr, 1, 0, NULL, 0, NULL, 0, KEY_FLAG_PAIRWISE);
        /* MLME-SETPROTECTION.request(None) */
        wpa_drv_mlme_setprotection(wpa_s, addr, MLME_SETPROTECTION_PROTECT_TYPE_NONE,
...
static void dhcp_war(struct wpa_supplicant *wpa_s)
{
    ...
    res = l2_packet_send(wpa_s->l2, wpa_s->bssid, ntohs(0x8989), (const u8 *)buf, len);
...
void wpa_supplicant_set_state(struct wpa_supplicant *wpa_s, enum wpa_states state)
{
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "State: %s -> %s", wpa_supplicant_state_txt(wpa_s->wpa_state),
            wpa_supplicant_state_txt(state));

    if (state == WPA_COMPLETED && os_reltime_initialized(&wpa_s->roam_start))
    {
    ...
    else if (state == WPA_DISCONNECTED && os_reltime_initialized(&wpa_s->roam_start))
    {
        wpa_s->roam_start.sec  = 0;
        wpa_s->roam_start.usec = 0;
     ...
     * set to 1.
     */
    if (wpa_s->conf->p2p_cli_probe && wpa_s->current_ssid && wpa_s->current_ssid->mode == WPAS_MODE_INFRA &&
...
#ifdef CONFIG_SME
        if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME) && wpa_auth_alg_fils(wpa_s->sme.auth_alg))
            fils_hlp_sent = 1;
#endif /* CONFIG_SME */
        if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME) && wpa_auth_alg_fils(wpa_s->auth_alg))
            fils_hlp_sent = 1;

#if defined(CONFIG_CTRL_IFACE) || !defined(CONFIG_NO_STDOUT_DEBUG)
        wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_CONNECTED "- Connection to " MACSTR " completed [id=%d id_str=%s%s]",
                MAC2STR(wpa_s->bssid), ssid ? ssid->id : -1, ssid && ssid->id_str ? ssid->id_str : "",
...
int wpa_supplicant_reload_configuration(struct wpa_supplicant *wpa_s)
{
    ...
    if (wpa_s->confanother && !wpa_config_read(wpa_s->confanother, conf))
    {
        wpa_msg(wpa_s, MSG_ERROR, "Failed to parse the configuration file '%s' - exiting", wpa_s->confanother);
        os_free(conf);
        return -1;
    }
...
static int matching_ciphers(struct wpa_ssid *ssid, struct wpa_ie_data *ie, int freq)
{
    if (!ie->has_group)
        ie->group_cipher = wpa_default_rsn_cipher(freq);
    if (!ie->has_pairwise)
        ie->pairwise_cipher = wpa_default_rsn_cipher(freq);
    return (ie->group_cipher & ssid->group_cipher) && (ie->pairwise_cipher & ssid->pairwise_cipher);
}

void wpas_set_mgmt_group_cipher(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid, struct wpa_ie_data *ie)
{
    int sel;
 ...
 * available).
 */
int wpa_supplicant_set_suites(
    struct wpa_supplicant *wpa_s, struct wpa_bss *bss, struct wpa_ssid *ssid, u8 *wpa_ie, size_t *wpa_ie_len)
{
    ...
    if (bss_rsn && (ssid->proto & WPA_PROTO_RSN) && wpa_parse_wpa_ie(bss_rsn, 2 + bss_rsn[1], &ie) == 0 &&
        matching_ciphers(ssid, &ie, bss->freq) && (ie.key_mgmt & ssid->key_mgmt))
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "RSN: using IEEE 802.11i/D9.0");
        proto = WPA_PROTO_RSN;
    }
    else if (bss_wpa && (ssid->proto & WPA_PROTO_WPA) && wpa_parse_wpa_ie(bss_wpa, 2 + bss_wpa[1], &ie) == 0 &&
             (ie.group_cipher & ssid->group_cipher) && (ie.pairwise_cipher & ssid->pairwise_cipher) &&
             (ie.key_mgmt & ssid->key_mgmt))
    {
    ...
    else if (bss_osen && (ssid->proto & WPA_PROTO_OSEN) && wpa_parse_wpa_ie(bss_osen, 2 + bss_osen[1], &ie) == 0 &&
             (ie.group_cipher & ssid->group_cipher) && (ie.pairwise_cipher & ssid->pairwise_cipher) &&
             (ie.key_mgmt & ssid->key_mgmt))
    {
    ...
    else if (bss_rsn && (ssid->proto & WPA_PROTO_OSEN) && wpa_parse_wpa_ie(bss_rsn, 2 + bss_rsn[1], &ie) == 0 &&
             (ie.group_cipher & ssid->group_cipher) && (ie.pairwise_cipher & ssid->pairwise_cipher) &&
             (ie.key_mgmt & ssid->key_mgmt))
    {
    ...
    else if (bss)
    {
        ...
        wpa_dbg(wpa_s, MSG_DEBUG, "WPA: ssid proto=0x%x pairwise_cipher=0x%x group_cipher=0x%x key_mgmt=0x%x",
                ssid->proto, ssid->pairwise_cipher, ssid->group_cipher, ssid->key_mgmt);
        wpa_dbg(wpa_s, MSG_DEBUG, "WPA: BSS " MACSTR " ssid='%s'%s%s%s", MAC2STR(bss->bssid),
                wpa_ssid_txt(bss->ssid, bss->ssid_len), bss_wpa ? " WPA" : "", bss_rsn ? " RSN" : "",
                bss_osen ? " OSEN" : "");
        if (bss_rsn)
        {
            ...
            {
                wpa_dbg(wpa_s, MSG_DEBUG, "RSN: pairwise_cipher=0x%x group_cipher=0x%x key_mgmt=0x%x",
        ...
        if (bss_wpa)
        {
            ...
            {
                wpa_dbg(wpa_s, MSG_DEBUG, "WPA: pairwise_cipher=0x%x group_cipher=0x%x key_mgmt=0x%x",
                        ie.pairwise_cipher, ie.group_cipher, ie.key_mgmt);
            }
        }
        return -1;
    ...
    else
    {
        ...
        if (wpa_supplicant_suites_from_ai(wpa_s, ssid, &ie) < 0)
        {
            ...
            {
                if (ssid->group_mgmt_cipher & WPA_CIPHER_BIP_GMAC_256)
                    ie.mgmt_group_cipher = WPA_CIPHER_BIP_GMAC_256;
                else if (ssid->group_mgmt_cipher & WPA_CIPHER_BIP_CMAC_256)
                    ie.mgmt_group_cipher = WPA_CIPHER_BIP_CMAC_256;
                else if (ssid->group_mgmt_cipher & WPA_CIPHER_BIP_GMAC_128)
                    ie.mgmt_group_cipher = WPA_CIPHER_BIP_GMAC_128;
                else
                    ie.mgmt_group_cipher = WPA_CIPHER_AES_128_CMAC;
            }
#ifdef CONFIG_OWE
            if ((ssid->key_mgmt & WPA_KEY_MGMT_OWE) && !ssid->owe_only && !bss_wpa && !bss_rsn && !bss_osen)
            {
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "WPA: AP group 0x%x network profile group 0x%x; available group 0x%x", ie.group_cipher,
            ssid->group_cipher, sel);
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "WPA: AP pairwise 0x%x network profile pairwise 0x%x; available pairwise 0x%x",
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "WPA: AP key_mgmt 0x%x network profile key_mgmt 0x%x; available key_mgmt 0x%x",
    ...
    else if ((sel & WPA_KEY_MGMT_FT_IEEE8021X_SHA384) && os_strcmp(wpa_supplicant_get_eap_mode(wpa_s), "LEAP") != 0)
    {
        wpa_s->key_mgmt = WPA_KEY_MGMT_FT_IEEE8021X_SHA384;
        wpa_dbg(wpa_s, MSG_DEBUG, "WPA: using KEY_MGMT FT/802.1X-SHA384");
        if (!ssid->ft_eap_pmksa_caching && pmksa_cache_get_current(wpa_s->wpa))
        {
            /* PMKSA caching with FT may have interoperability
             * issues, so disable that case by default for now. */
            wpa_dbg(wpa_s, MSG_DEBUG, "WPA: Disable PMKSA caching for FT/802.1X connection");
            pmksa_cache_clear_current(wpa_s->wpa);
        }
#endif /* CONFIG_SHA384 */
#endif /* CONFIG_IEEE80211R */
    ...
    else if ((sel & WPA_KEY_MGMT_FT_IEEE8021X) && os_strcmp(wpa_supplicant_get_eap_mode(wpa_s), "LEAP") != 0)
    {
    ...
    if (!(ie.capabilities & WPA_CAPABILITY_MFPC) && wpas_get_ssid_pmf(wpa_s, ssid) == MGMT_FRAME_PROTECTION_REQUIRED)
    {
        wpa_msg(wpa_s, MSG_INFO, "RSN: Management frame protection required but the selected AP does not enable it");
        return -1;
    }

    wpas_set_mgmt_group_cipher(wpa_s, ssid, &ie);
#ifdef CONFIG_OCV
    if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME) || (wpa_s->drv_flags2 & WPA_DRIVER_FLAGS2_OCV))
    ...
    wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_SAE_PK,
                     wpa_key_mgmt_sae(ssid->key_mgmt) && ssid->sae_pk != SAE_PK_MODE_DISABLED &&
...
#endif /* CONFIG_SAE_PK */
#ifdef CONFIG_TESTING_OPTIONS
    wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_FT_RSNXE_USED, wpa_s->ft_rsnxe_used);
    wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_OCI_FREQ_EAPOL, wpa_s->oci_freq_override_eapol);
    wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_OCI_FREQ_EAPOL_G2, wpa_s->oci_freq_override_eapol_g2);
    wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_OCI_FREQ_FT_ASSOC, wpa_s->oci_freq_override_ft_assoc);
    wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_OCI_FREQ_FILS_ASSOC, wpa_s->oci_freq_override_fils_assoc);
#endif /* CONFIG_TESTING_OPTIONS */

    /* Extended Key ID is only supported in infrastructure BSS so far */
    if (ssid->mode == WPAS_MODE_INFRA && wpa_s->conf->extended_key_id && (ssid->proto & WPA_PROTO_RSN) &&
        ssid->pairwise_cipher & (WPA_CIPHER_CCMP | WPA_CIPHER_CCMP_256 | WPA_CIPHER_GCMP | WPA_CIPHER_GCMP_256) &&
        (wpa_s->drv_flags & WPA_DRIVER_FLAGS_EXTENDED_KEY_ID))
    {
        int use_ext_key_id = 0;

        wpa_msg(wpa_s, MSG_DEBUG, "WPA: Enable Extended Key ID support");
        wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_EXT_KEY_ID, wpa_s->conf->extended_key_id);
        if (bss_rsn && wpa_s->conf->extended_key_id && wpa_s->pairwise_cipher != WPA_CIPHER_TKIP &&
            (ie.capabilities & WPA_CAPABILITY_EXT_KEY_ID_FOR_UNICAST))
            use_ext_key_id = 1;
        wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_USE_EXT_KEY_ID, use_ext_key_id);
    }
    else
    {
    ...
    if (wpa_sm_set_assoc_rsnxe_default(wpa_s->wpa, wpa_s->rsnxe, &wpa_s->rsnxe_len))
    {
    ...
    else if (wpa_key_mgmt_wpa_psk(ssid->key_mgmt))
    {
        ...
        sae_only = (ssid->key_mgmt & (WPA_KEY_MGMT_PSK | WPA_KEY_MGMT_FT_PSK | WPA_KEY_MGMT_PSK_SHA256)) == 0;

        if (ssid->psk_set && !sae_only)
        {
            wpa_hexdump_key(MSG_MSGDUMP, "PSK (set in config)", ssid->psk, PMK_LEN);
            wpa_sm_set_pmk(wpa_s->wpa, ssid->psk, PMK_LEN, NULL, NULL);
            psk_set = 1;
        }

        if (wpa_key_mgmt_sae(ssid->key_mgmt) && (ssid->sae_password || ssid->passphrase))
            psk_set = 1;

#ifndef CONFIG_NO_PBKDF2
        if (bss && ssid->bssid_set && ssid->ssid_len == 0 && ssid->passphrase && !sae_only)
        {
            u8 psk[PMK_LEN];
        ...
        if (ssid->ext_psk && !sae_only)
        {
                ...
                if (wpabuf_len(pw) == 2 * PMK_LEN)
            {
                ...
                wpa_hexdump_key(MSG_MSGDUMP, "PSK (from external PSK)", psk, PMK_LEN);
                wpa_sm_set_pmk(wpa_s->wpa, psk, PMK_LEN, NULL, NULL);
                psk_set = 1;
                os_memset(psk, 0, sizeof(psk));
            }
    ...
    if (ssid->mode != WPAS_MODE_IBSS && !(wpa_s->drv_flags & WPA_DRIVER_FLAGS_WIRED) &&
    ...
    {
        wpa_msg(wpa_s, MSG_INFO, "Disable PTK0 rekey support - replaced with reconnect");
...
int wpas_update_random_addr(struct wpa_supplicant *wpa_s, int style)
{
    ...
    os_get_reltime(&now);
    if (wpa_s->last_mac_addr_style == style && wpa_s->last_mac_addr_change.sec != 0 &&
        !os_reltime_expired(&now, &wpa_s->last_mac_addr_change, wpa_s->conf->rand_addr_lifetime))
    {
...
static void wpa_s_setup_sae_pt(struct wpa_config *conf, struct wpa_ssid *ssid)
{
    ...
    ssid->pt = sae_derive_pt(groups, ssid->ssid, ssid->ssid_len, (const u8 *)password, os_strlen(password),
...
static void wpa_s_clear_sae_rejected(struct wpa_supplicant *wpa_s)
{
    ...
    if (wpa_s->extra_sae_rejected_groups)
    {
        ...
        for (i = 0; groups[i]; i++)
        {
            wpa_printf(MSG_DEBUG, "TESTING: Indicate rejection of an extra SAE group %d", groups[i]);
            int_array_add_unique(&wpa_s->sme.sae_rejected_groups, groups[i]);
        }
    }
#endif /* CONFIG_TESTING_OPTIONS */
...
void wpa_supplicant_associate(struct wpa_supplicant *wpa_s, struct wpa_bss *bss, struct wpa_ssid *ssid)
{
    ...
    {
        wpa_msg(wpa_s, MSG_INFO, "IBSS RSN not supported in the build");
        return;
    }
#endif /* CONFIG_IBSS_RSN */
    ...
    if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME) && ssid->mode == WPAS_MODE_INFRA)
    {
...
static int drv_supports_vht(struct wpa_supplicant *wpa_s, const struct wpa_ssid *ssid)
{
...
static bool ibss_mesh_is_80mhz_avail(int channel, struct hostapd_hw_modes *mode)
{
    ...
    for (i = channel; i < channel + 16; i += 4)
    {
        ...
        if (!chan || chan->flag & (HOSTAPD_CHAN_DISABLED | HOSTAPD_CHAN_NO_IR))
            return false;
    }
...
void ibss_mesh_setup_freq(struct wpa_supplicant *wpa_s, const struct wpa_ssid *ssid, struct hostapd_freq_params *freq)
{
    ...
    int bw80[]                            = {5180, 5260, 5500, 5580, 5660, 5745, 5955, 6035, 6115, 6195,
    ...
    if (wpa_s->group_cipher == WPA_CIPHER_WEP40 || wpa_s->group_cipher == WPA_CIPHER_WEP104 ||
    ...
    is_24ghz = hw_mode == HOSTAPD_MODE_IEEE80211G || hw_mode == HOSTAPD_MODE_IEEE80211B;
...
skip_ht40:
#endif /* CONFIG_HT_OVERRIDES */
    wpa_printf(MSG_DEBUG, "IBSS/mesh: setup freq channel %d, sec_channel_offset %d", freq->channel,
    ...
    if (ssid->mode == WPAS_MODE_IBSS && !(wpa_s->drv_flags & WPA_DRIVER_FLAGS_VHT_IBSS))
        return;
...
skip_to_6ghz:
    ...
    {
        if (freq->freq >= bw80[j] && freq->freq < bw80[j] + 80)
            break;
    }

    if (j == ARRAY_SIZE(bw80) || ieee80211_freq_to_chan(bw80[j], &channel) == NUM_HOSTAPD_MODES)
        return;
...
#endif /* CONFIG_HE_OVERRIDES */
    if (hostapd_set_freq_params(&vht_freq, mode->mode, freq->freq, freq->channel, ssid->enable_edmg, ssid->edmg_channel,
                                freq->ht_enabled, vht_freq.vht_enabled, freq->he_enabled, freq->sec_channel_offset,
                                chwidth, seg0, seg1, vht_caps, &mode->he_capab[ieee80211_mode]) != 0)
        return;

    *freq = vht_freq;

    wpa_printf(MSG_DEBUG, "IBSS: VHT setup freq cf1 %d, cf2 %d, bw %d", freq->center_freq1, freq->center_freq2,
               freq->bandwidth);
}

#ifdef CONFIG_FILS
static size_t wpas_add_fils_hlp_req(struct wpa_supplicant *wpa_s, u8 *ie_buf, size_t ie_buf_len)
{
    struct fils_hlp_req *req;
    ...
    dl_list_for_each(req, &wpa_s->fils_hlp_req, struct fils_hlp_req, list)
    {
        ...
        {
            wpa_printf(MSG_ERROR, "FILS: Cannot fit HLP - rem_len=%lu to_fill=%lu", (unsigned long)rem_len,
...
{
    return (((wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME) && (wpa_s->drv_flags & WPA_DRIVER_FLAGS_SUPPORT_FILS)) ||
            (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME) && (wpa_s->drv_flags & WPA_DRIVER_FLAGS_FILS_SK_OFFLOAD)));
}
...
static int wpas_populate_wfa_capa(
    struct wpa_supplicant *wpa_s, struct wpa_bss *bss, u8 *wpa_ie, size_t wpa_ie_len, size_t max_wpa_ie_len)
{
    ...
    {
        wpa_hexdump_buf(MSG_MSGDUMP, "WFA Capabilities element", wfa_ie);
        os_memcpy(wpa_ie + wpa_ie_len, wpabuf_head(wfa_ie), wfa_ie_len);
...
static u8 *wpas_populate_assoc_ies(struct wpa_supplicant *wpa_s,
                                   struct wpa_bss *bss,
                                   struct wpa_ssid *ssid,
                                   ...
                                   enum wpa_drv_update_connect_params_mask *mask)
{
    ...
    dl_list_for_each(req, &wpa_s->fils_hlp_req, struct fils_hlp_req, list)
    {
        max_wpa_ie_len += 3 + 2 * ETH_ALEN + 6 + wpabuf_len(req->pkt) + 2 + 2 * wpabuf_len(req->pkt) / 255;
    }
#endif /* CONFIG_FILS */
    ...
    if (!wpa_ie)
    {
        wpa_printf(MSG_ERROR, "Failed to allocate connect IE buffer for %lu bytes", (unsigned long)max_wpa_ie_len);
        return NULL;
    }

    if (bss && (wpa_bss_get_vendor_ie(bss, WPA_IE_VENDOR_TYPE) || wpa_bss_get_rsne(wpa_s, bss, ssid, false)) &&
        wpa_key_mgmt_wpa(ssid->key_mgmt))
    {
        int try_opportunistic;
        const u8 *cache_id = NULL;

        try_opportunistic = (ssid->proactive_key_caching < 0 ? wpa_s->conf->okc : ssid->proactive_key_caching) &&
                            (ssid->proto & WPA_PROTO_RSN);
#ifdef CONFIG_FILS
        if (wpa_key_mgmt_fils(ssid->key_mgmt))
            cache_id = wpa_bss_get_fils_cache_id(bss);
#endif /* CONFIG_FILS */
        if (pmksa_cache_set_current(wpa_s->wpa, NULL, bss->bssid, ssid, try_opportunistic, cache_id, 0) == 0)
        {
    ...
    else if (bss && wpa_bss_get_vendor_ie(bss, OSEN_IE_VENDOR_TYPE) && (ssid->key_mgmt & WPA_KEY_MGMT_OSEN))
    {
    ...
    if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_FILS_SK_OFFLOAD) && ssid->eap.erp && wpa_key_mgmt_fils(wpa_s->key_mgmt) &&
        eapol_sm_get_erp_info(wpa_s->eapol, &ssid->eap, &username, &username_len, &realm, &realm_len, &next_seq_num,
                              &rrk, &rrk_len) == 0 &&
        (!wpa_s->last_con_fail_realm || wpa_s->last_con_fail_realm_len != realm_len ||
    ...
    else if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_FILS_SK_OFFLOAD) && ssid->eap.erp &&
             wpa_key_mgmt_fils(wpa_s->key_mgmt) && pmksa_cached)
    {
        algs = WPA_AUTH_ALG_FILS;
    }
#endif /* CONFIG_FILS */
    ...
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "SAE: Use WPA_AUTH_ALG_OPEN for PMKSA caching attempt");
    ...
    if (bss)
    {
        wpa_ie_len += wpas_supp_op_class_ie(wpa_s, ssid, bss, wpa_ie + wpa_ie_len, max_wpa_ie_len - wpa_ie_len);
    }
    ...
    if (!bss || wpa_bss_get_ie(bss, WLAN_EID_EXT_CAPAB))
    {
        ...
        if (ext_capab_len > 0 && wpa_ie_len + ext_capab_len <= max_wpa_ie_len)
        {
            u8 *pos = wpa_ie;
    ...
    if (is_hs20_network(wpa_s, ssid, bss))
    {
        ...
        if (hs20)
        {
            ...
            wpas_hs20_add_indication(hs20, pps_mo_id, get_hs20_version(bss));
    ...
    if (!wpa_s->disable_mbo_oce && mbo_ie)
    {
        ...
        len = wpas_mbo_ie(wpa_s, wpa_ie + wpa_ie_len, max_wpa_ie_len - wpa_ie_len,
                          !!mbo_attr_from_mbo_ie(mbo_ie, OCE_ATTR_ID_CAPA_IND));
        if (len >= 0)
            wpa_ie_len += len;
    }
#endif /* CONFIG_MBO */
    ...
    if (algs == WPA_AUTH_ALG_FILS)
    {
        ...
        len = wpas_add_fils_hlp_req(wpa_s, wpa_ie + wpa_ie_len, max_wpa_ie_len - wpa_ie_len);
        wpa_ie_len += len;
    }
#endif /* CONFIG_FILS */
...
#endif /* CONFIG_TESTING_OPTIONS */
        if (algs == WPA_AUTH_ALG_OPEN && ssid->key_mgmt == WPA_KEY_MGMT_OWE)
    {
        ...
        else if (wpa_s->assoc_status_code == WLAN_STATUS_FINITE_CYCLIC_GROUP_NOT_SUPPORTED)
        {
        ...
        if (owe_ie && wpabuf_len(owe_ie) <= max_wpa_ie_len - wpa_ie_len)
        {
            os_memcpy(wpa_ie + wpa_ie_len, wpabuf_head(owe_ie), wpabuf_len(owe_ie));
...
#ifdef CONFIG_DPP2
    if (DPP_VERSION > 1 && wpa_sm_get_key_mgmt(wpa_s->wpa) == WPA_KEY_MGMT_DPP && ssid->dpp_netaccesskey &&
        ssid->dpp_pfs != 2 && !ssid->dpp_pfs_fallback)
    {
        ...
        wpa_s->dpp_pfs = dpp_pfs_init(ssid->dpp_netaccesskey, ssid->dpp_netaccesskey_len);
        if (!wpa_s->dpp_pfs)
        {
        ...
        if (wpabuf_len(wpa_s->dpp_pfs->ie) <= max_wpa_ie_len - wpa_ie_len)
        {
            os_memcpy(wpa_ie + wpa_ie_len, wpabuf_head(wpa_s->dpp_pfs->ie), wpabuf_len(wpa_s->dpp_pfs->ie));
...
pfs_fail:
    ...
    if (bss && wpa_key_mgmt_ft(wpa_sm_get_key_mgmt(wpa_s->wpa)))
    {
        ...
        if (mdie && mdie[1] >= MOBILITY_DOMAIN_ID_LEN)
        {
            ...
            if (os_memcmp(md, wpa_md, MOBILITY_DOMAIN_ID_LEN) == 0)
            {
                /* Add mobility domain IE */
                len = wpa_ft_add_mdie(wpa_s->wpa, wpa_ie + wpa_ie_len, max_wpa_ie_len - wpa_ie_len, mdie);
                wpa_ie_len += len;
            }
#ifdef CONFIG_SME
            if (len > 0 && wpa_s->sme.ft_used && wpa_sm_has_ptk(wpa_s->wpa))
            {
...
#ifdef CONFIG_TESTING_OPTIONS
    if (wpa_s->rsnxe_override_assoc && wpabuf_len(wpa_s->rsnxe_override_assoc) <= max_wpa_ie_len - wpa_ie_len)
    {
        wpa_printf(MSG_DEBUG, "TESTING: RSNXE AssocReq override");
        os_memcpy(wpa_ie + wpa_ie_len, wpabuf_head(wpa_s->rsnxe_override_assoc),
                  wpabuf_len(wpa_s->rsnxe_override_assoc));
        wpa_ie_len += wpabuf_len(wpa_s->rsnxe_override_assoc);
    }
    else
#endif /* CONFIG_TESTING_OPTIONS */
        if (wpa_s->rsnxe_len > 0 && wpa_s->rsnxe_len <= max_wpa_ie_len - wpa_ie_len)
    {
    ...
    if (wpa_s->disable_mscs_support)
        goto mscs_end;
#endif /* CONFIG_TESTING_OPTIONS */
    if (wpa_bss_ext_capab(bss, WLAN_EXT_CAPAB_MSCS) && wpa_s->robust_av.valid_config)
    {
        ...
        if (!mscs_ie)
        {
            wpa_printf(MSG_INFO, "MSCS: Failed to allocate MSCS IE");
            goto mscs_end;
        }
        ...
        if ((wpa_ie_len + wpabuf_len(mscs_ie)) <= max_wpa_ie_len)
        {
            ...
            os_memcpy(wpa_ie + wpa_ie_len, wpabuf_head(mscs_ie), mscs_ie_len);
...
mscs_end:
    ...
    wpa_ie_len = wpas_populate_wfa_capa(wpa_s, bss, wpa_ie, wpa_ie_len, max_wpa_ie_len);

    if (ssid->multi_ap_backhaul_sta)
    {
        size_t multi_ap_ie_len;

        multi_ap_ie_len = add_multi_ap_ie(wpa_ie + wpa_ie_len, max_wpa_ie_len - wpa_ie_len, MULTI_AP_BACKHAUL_STA);
...
static void wpas_update_owe_connect_params(struct wpa_supplicant *wpa_s)
{
    ...
    wpa_ie = wpas_populate_assoc_ies(wpa_s, wpa_s->current_bss, wpa_s->current_ssid, &params, NULL);
    if (!wpa_ie)
        return;
...
static void wpas_update_fils_connect_params(struct wpa_supplicant *wpa_s)
{
    ...
    os_memset(&params, 0, sizeof(params));
    wpa_ie = wpas_populate_assoc_ies(wpa_s, wpa_s->current_bss, wpa_s->current_ssid, &params, &mask);
    if (!wpa_ie)
        return;
 ...
 */
static struct ieee80211_edmg_config get_edmg_intersection(struct ieee80211_edmg_config a,
                                                          ...
                                                          u8 primary_channel)
{
    ...
    if (!result.channels)
    {
        wpa_printf(MSG_DEBUG, "EDMG not possible: cannot intersect channels 0x%x and 0x%x", a.channels, b.channels);
        goto fail;
    }

    if (!(result.channels & BIT(primary_channel - 1)))
    {
        wpa_printf(MSG_DEBUG, "EDMG not possible: the primary channel %d is not one of the intersected channels 0x%x",
    ...
    {
        wpa_printf(MSG_DEBUG, "EDMG not possible: not enough contiguous channels %d for supporting CB1 or CB2",
                   max_contiguous);
        goto fail;
    }
...
static struct ieee80211_edmg_config get_supported_edmg(struct wpa_supplicant *wpa_s,
...
void wpas_update_mbo_connect_params(struct wpa_supplicant *wpa_s)
{
    ...
    if (wpa_s->wpa_state != WPA_COMPLETED || !wpa_s->current_ssid || !wpa_s->current_bss ||
...
static void wpas_start_assoc_cb(struct wpa_radio_work *work, int deinit)
{
    ...
    if (cwork->bss_removed || !wpas_valid_bss_ssid(wpa_s, bss, ssid) || wpas_network_disabled(wpa_s, ssid))
    {
    ...
    else if ((ssid->ssid == NULL || ssid->ssid_len == 0) && wpa_s->conf->ap_scan == 2 &&
    ...
    if (wpa_s->key_mgmt == WPA_KEY_MGMT_IEEE8021X_NO_WPA)
    {
        if ((ssid->eapol_flags & (EAPOL_FLAG_REQUIRE_KEY_UNICAST | EAPOL_FLAG_REQUIRE_KEY_BROADCAST)) == 0 &&
            !wep_keys_set)
        {
            use_crypt = 0;
        }
    ...
    if (bss)
    {
        ...
        if (!wpas_driver_bss_selection(wpa_s) || ssid->bssid_set || wpa_s->key_mgmt == WPA_KEY_MGMT_WPS)
        {
            wpa_printf(MSG_DEBUG,
                       "Limit connection to BSSID " MACSTR
                       " freq=%u MHz based on scan results "
                       "(bssid_set=%d wps=%d)",
                       MAC2STR(bss->bssid), bss->freq, ssid->bssid_set, wpa_s->key_mgmt == WPA_KEY_MGMT_WPS);
    ...
    if (bss && ssid->enable_edmg)
        edmg_ie_oper = wpa_bss_get_ie_ext(bss, WLAN_EID_EXT_EDMG_OPERATION);
    else
        edmg_ie_oper = NULL;

    if (edmg_ie_oper)
    {
        params.freq.edmg.channels  = wpa_ie_get_edmg_oper_chans(edmg_ie_oper);
        params.freq.edmg.bw_config = wpa_ie_get_edmg_oper_chan_width(edmg_ie_oper);
        wpa_printf(MSG_DEBUG, "AP supports EDMG channels 0x%x, bw_config %d", params.freq.edmg.channels,
        ...
        if (ssid->edmg_channel)
        {
            ...
            hw_mode = ieee80211_freq_to_chan(bss->freq, &primary_channel);
            if (hw_mode == NUM_HOSTAPD_MODES)
                goto edmg_fail;

            hostapd_encode_edmg_chan(ssid->enable_edmg, ssid->edmg_channel, primary_channel, &configured_edmg);

            if (ieee802_edmg_is_allowed(params.freq.edmg, configured_edmg))
            {
                params.freq.edmg = configured_edmg;
                wpa_printf(MSG_DEBUG, "Use EDMG channel %d for connection", ssid->edmg_channel);
            }
            else
            {
            edmg_fail:
                ...
                wpa_printf(MSG_WARNING, "EDMG channel %d not supported by AP, fallback to DMG", ssid->edmg_channel);
            }
        }

        if (params.freq.edmg.channels)
        {
            wpa_printf(MSG_DEBUG, "EDMG before: channels 0x%x, bw_config %d", params.freq.edmg.channels,
                       params.freq.edmg.bw_config);
            params.freq.edmg = get_supported_edmg(wpa_s, &params.freq, params.freq.edmg);
            wpa_printf(MSG_DEBUG, "EDMG after: channels 0x%x, bw_config %d", params.freq.edmg.channels,
                       params.freq.edmg.bw_config);
        }
    }
    ...
    if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_4WAY_HANDSHAKE_PSK) &&
        (params.key_mgmt_suite == WPA_KEY_MGMT_PSK || params.key_mgmt_suite == WPA_KEY_MGMT_FT_PSK))
    {
    ...
    if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_4WAY_HANDSHAKE_8021X) &&
        (params.key_mgmt_suite == WPA_KEY_MGMT_IEEE8021X || params.key_mgmt_suite == WPA_KEY_MGMT_IEEE8021X_SHA256 ||
    ...
    if (wpa_s->conf->key_mgmt_offload)
    {
        if (params.key_mgmt_suite == WPA_KEY_MGMT_IEEE8021X || params.key_mgmt_suite == WPA_KEY_MGMT_IEEE8021X_SHA256 ||
            ...
            params.req_key_mgmt_offload =
                ssid->proactive_key_caching < 0 ? wpa_s->conf->okc : ssid->proactive_key_caching;
        else
    ...
    if (params.mgmt_frame_protection != NO_MGMT_FRAME_PROTECTION && bss)
    {
        ...
        else if (!rsn && (ssid->key_mgmt & WPA_KEY_MGMT_OWE) && !ssid->owe_only)
        {
    ...
    if (wpa_s->reassoc_same_ess && !is_zero_ether_addr(prev_bssid) && wpa_s->current_ssid)
    ...
    else if (ssid->mode == WPAS_MODE_IBSS && wpa_s->key_mgmt != WPA_KEY_MGMT_NONE &&
             wpa_s->key_mgmt != WPA_KEY_MGMT_WPA_NONE)
    {
        /*
         * RSN IBSS authentication is per-STA and we can disable the
 ...
 * current AP.
 */
void wpa_supplicant_deauthenticate(struct wpa_supplicant *wpa_s, u16 reason_code)
{
    u8 *addr = NULL;
    ...
    wpa_dbg(wpa_s, MSG_DEBUG,
            "Request to deauthenticate - bssid=" MACSTR " pending_bssid=" MACSTR " reason=%d (%s) state=%s",
            MAC2STR(wpa_s->bssid), MAC2STR(wpa_s->pending_bssid), reason_code, reason2str(reason_code),
    ...
    if (wpa_s->ifmsh)
    {
        ...
        wpas_notify_mesh_group_removed(wpa_s, mconf->meshid, mconf->meshid_len, reason_code);
...
void wpa_supplicant_enable_network(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
    ...
    if (wpa_s->reassociate && !wpa_s->disconnected &&
        (!wpa_s->current_ssid || wpa_s->wpa_state == WPA_DISCONNECTED || wpa_s->wpa_state == WPA_SCANNING))
    {
        if (wpa_s->sched_scanning)
        {
            wpa_printf(MSG_DEBUG,
...
int wpas_remove_cred(struct wpa_supplicant *wpa_s, struct wpa_cred *cred)
{
    ...
    while (ssid)
    {
        ...
        if (ssid->parent_cred == cred)
        {
            ...
            if (wpa_supplicant_remove_network(wpa_s, ssid->id) == -1)
            {
                wpa_printf(MSG_DEBUG, "Could not find network id=%d", ssid->id);
            }
        }
...
int wpas_remove_all_creds(struct wpa_supplicant *wpa_s)
{
    ...
    while (cred)
    {
        ...
        if (res < 0)
        {
            wpa_printf(MSG_DEBUG, "Removal of all credentials failed - failed to remove credential id=%d", prev->id);
            ret = -1;
        }
    }
...
int wpa_supplicant_set_ap_scan(struct wpa_supplicant *wpa_s, int ap_scan)
{
    ...
    {
        wpa_printf(MSG_INFO,
                   "Note: nl80211 driver interface is not designed to be used with ap_scan=2; this can result in "
                   "connection failures");
    }

#ifdef ANDROID
    if (ap_scan == 2 && ap_scan != wpa_s->conf->ap_scan && wpa_s->wpa_state >= WPA_ASSOCIATING &&
                                    ...
                                    int debug_show_keys)
{
    ...
    if (debug_level != MSG_EXCESSIVE && debug_level != MSG_MSGDUMP && debug_level != MSG_DEBUG &&
        debug_level != MSG_INFO && debug_level != MSG_WARNING && debug_level != MSG_ERROR)
        return -1;
...
#ifdef CONFIG_OWE
static int owe_trans_ssid_match(struct wpa_supplicant *wpa_s,
                                const u8 *bssid,
                                const u8 *entry_ssid,
                                size_t entry_ssid_len)
{
    ...
    return entry_ssid_len == ssid_len && os_memcmp(pos, entry_ssid, ssid_len) == 0;
}
#endif /* CONFIG_OWE */
...
struct wpa_ssid *wpa_supplicant_get_ssid(struct wpa_supplicant *wpa_s)
{
    ...
    while (entry)
    {
        if (!wpas_network_disabled(wpa_s, entry) &&
            ((ssid_len == entry->ssid_len && (!entry->ssid || os_memcmp(ssid, entry->ssid, ssid_len) == 0)) || wired) &&
            (!entry->bssid_set || os_memcmp(bssid, entry->bssid, ETH_ALEN) == 0))
            return entry;
#ifdef CONFIG_WPS
        if (!wpas_network_disabled(wpa_s, entry) && (entry->key_mgmt & WPA_KEY_MGMT_WPS) &&
...
#ifdef CONFIG_OWE
        if (!wpas_network_disabled(wpa_s, entry) && owe_trans_ssid_match(wpa_s, bssid, entry->ssid, entry->ssid_len) &&
            (!entry->bssid_set || os_memcmp(bssid, entry->bssid, ETH_ALEN) == 0))
            return entry;
#endif /* CONFIG_OWE */

        if (!wpas_network_disabled(wpa_s, entry) && entry->bssid_set && entry->ssid_len == 0 &&
...
void wpa_supplicant_rx_eapol(void *ctx, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "Drop received EAPOL frame as we are disconnecting");
        return;
    }

#ifdef CONFIG_TESTING_OPTIONS
    wpa_msg_ctrl(wpa_s, MSG_INFO, "EAPOL-RX " MACSTR " %zu", MAC2STR(src_addr), len);
    ...
    if (wpa_s->eapol_received == 0 &&
        (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_4WAY_HANDSHAKE_PSK) || !wpa_key_mgmt_wpa_psk(wpa_s->key_mgmt) ||
         wpa_s->wpa_state != WPA_COMPLETED) &&
        (wpa_s->current_ssid == NULL || wpa_s->current_ssid->mode != WPAS_MODE_IBSS))
    {
...
#ifdef CONFIG_WPS
        if (wpa_s->current_ssid && wpa_s->current_bss && (wpa_s->current_ssid->key_mgmt & WPA_KEY_MGMT_WPS) &&
    ...
    if (!wpa_key_mgmt_wpa_psk(wpa_s->key_mgmt) && wpa_s->key_mgmt != WPA_KEY_MGMT_OWE &&
        wpa_s->key_mgmt != WPA_KEY_MGMT_DPP && eapol_sm_rx_eapol(wpa_s->eapol, src_addr, buf, len) > 0)
        return;
    wpa_drv_poll(wpa_s);
    if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_4WAY_HANDSHAKE_PSK))
...
{
    if ((!wpa_s->p2p_mgmt || !(wpa_s->drv_flags & WPA_DRIVER_FLAGS_DEDICATED_P2P_DEVICE)) &&
        !(wpa_s->drv_flags & WPA_DRIVER_FLAGS_P2P_DEDICATED_INTERFACE))
    {
        l2_packet_deinit(wpa_s->l2);
        wpa_s->l2 = l2_packet_init(wpa_s->ifname, wpa_drv_get_mac_addr(wpa_s), ETH_P_EAPOL,
                                   wpas_eapol_needs_l2_packet(wpa_s) ? wpa_supplicant_rx_eapol : NULL, wpa_s, 0);
        if (wpa_s->l2 == NULL)
            return -1;

        if (l2_packet_set_packet_filter(wpa_s->l2, L2_PACKET_FILTER_PKTTYPE))
...
static void wpa_supplicant_rx_eapol_bridge(void *ctx, const u8 *src_addr, const u8 *buf, size_t len)
{
    ...
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "RX EAPOL from " MACSTR " to " MACSTR " (bridge - not for this interface - ignore)",
                MAC2STR(src_addr), MAC2STR(eth->h_dest));
        return;
    }

    wpa_dbg(wpa_s, MSG_DEBUG, "RX EAPOL from " MACSTR " to " MACSTR " (bridge)", MAC2STR(src_addr),
            MAC2STR(eth->h_dest));
    wpa_supplicant_rx_eapol(wpa_s, src_addr, buf + sizeof(*eth), len - sizeof(*eth));
}

int wpa_supplicant_update_bridge_ifname(struct wpa_supplicant *wpa_s, const char *bridge_ifname)
{
    if (wpa_s->wpa_state > WPA_SCANNING)
        return -EBUSY;

    if (bridge_ifname && os_strlen(bridge_ifname) >= sizeof(wpa_s->bridge_ifname))
        return -EINVAL;
    ...
    os_strlcpy(wpa_s->bridge_ifname, bridge_ifname, sizeof(wpa_s->bridge_ifname));

    if (wpa_s->bridge_ifname[0])
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "Receiving packets from bridge interface '%s'", wpa_s->bridge_ifname);
        wpa_s->l2_br = l2_packet_init_bridge(wpa_s->bridge_ifname, wpa_s->ifname, wpa_s->own_addr, ETH_P_EAPOL,
                                             wpa_supplicant_rx_eapol_bridge, wpa_s, 1);
        if (!wpa_s->l2_br)
        {
            wpa_msg(wpa_s, MSG_ERROR, "Failed to open l2_packet connection for the bridge interface '%s'",
                    wpa_s->bridge_ifname);
            goto fail;
        }
    }
                           ...
                           int tx_stbc)
{
    ...
    {
        wpa_msg(wpa_s, MSG_ERROR, "tx_stbc: %d out of range. Must be 0-1 or -1", tx_stbc);
        return -EINVAL;
    }
                           ...
                           int rx_stbc)
{
    ...
    {
        wpa_msg(wpa_s, MSG_ERROR, "rx_stbc: %d out of range. Must be 0-3 or -1", rx_stbc);
        return -EINVAL;
    }
...
#ifdef CONFIG_VHT_OVERRIDES
void wpa_supplicant_apply_vht_overrides(struct wpa_supplicant *wpa_s,
                                        struct wpa_ssid *ssid,
                                        struct wpa_driver_associate_params *params)
{
    struct ieee80211_vht_capabilities *vhtcaps;
    ...
    if (ssid->disable_sgi)
    {
        ...
        wpa_msg(wpa_s, MSG_DEBUG, "disable-sgi override specified, vht-caps: 0x%x", vhtcaps->vht_capabilities_info);
    }
    ...
    if (ssid->vht_capa_mask & VHT_CAP_MAX_A_MPDU_LENGTH_EXPONENT_MAX)
    {
        ...
        max_ampdu =
            (ssid->vht_capa & VHT_CAP_MAX_A_MPDU_LENGTH_EXPONENT_MAX) >> VHT_CAP_MAX_A_MPDU_LENGTH_EXPONENT_MAX_SHIFT;
...
#define OVERRIDE_MCS(i)                                                                                 \
    ...
    {                                                                                                   \
        vhtcaps_mask->vht_supported_mcs_set.tx_map |= host_to_le16(3 << 2 * (i - 1));                   \
        vhtcaps->vht_supported_mcs_set.tx_map |= host_to_le16(ssid->vht_tx_mcs_nss_##i << 2 * (i - 1)); \
    }                                                                                                   \
    if (ssid->vht_rx_mcs_nss_##i >= 0)                                                                  \
    {                                                                                                   \
        vhtcaps_mask->vht_supported_mcs_set.rx_map |= host_to_le16(3 << 2 * (i - 1));                   \
        vhtcaps->vht_supported_mcs_set.rx_map |= host_to_le16(ssid->vht_rx_mcs_nss_##i << 2 * (i - 1)); \
    }
...
#ifdef CONFIG_HE_OVERRIDES
void wpa_supplicant_apply_he_overrides(struct wpa_supplicant *wpa_s,
                                       struct wpa_ssid *ssid,
                                       struct wpa_driver_associate_params *params)
{
    if (!ssid)
...
static const u8 *wpas_fst_get_bssid_cb(void *ctx)
{
    ...
    return (is_zero_ether_addr(wpa_s->bssid) || wpa_s->wpa_state != WPA_COMPLETED) ? NULL : wpa_s->bssid;
}

static void wpas_fst_get_channel_info_cb(void *ctx, enum hostapd_hw_mode *hw_mode, u8 *channel)
{
    struct wpa_supplicant *wpa_s = ctx;
...
static int wpas_fst_send_action_cb(void *ctx, const u8 *da, struct wpabuf *data)
{
    ...
    {
        wpa_printf(MSG_INFO, "FST:%s:bssid=" MACSTR " != da=" MACSTR, __func__, MAC2STR(wpa_s->bssid), MAC2STR(da));
        return -1;
    }
    return wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, wpa_s->bssid, wpa_s->own_addr, wpa_s->bssid,
...
static void wpas_fst_update_mb_ie_cb(void *ctx, const u8 *addr, const u8 *buf, size_t size)
{
    struct wpa_supplicant *wpa_s = ctx;
...
static const u8 *wpas_fst_get_peer_first(void *ctx, struct fst_get_peer_ctx **get_ctx, bool mb_only)
{
    struct wpa_supplicant *wpa_s = ctx;
...
static const u8 *wpas_fst_get_peer_next(void *ctx, struct fst_get_peer_ctx **get_ctx, bool mb_only)
{
    return NULL;
}

void fst_wpa_supplicant_fill_iface_obj(struct wpa_supplicant *wpa_s, struct fst_wpa_obj *iface_obj)
{
    iface_obj->ctx              = wpa_s;
...
static void radio_work_free(struct wpa_radio_work *work)
{
    ...
    if (work->started)
    {
        ...
        wpa_dbg(work->wpa_s, MSG_DEBUG, "radio_work_free('%s'@%p): num_active_works --> %u", work->type, work,
                work->wpa_s->radio->num_active_works);
    }
...
{
    return os_strcmp(work->type, "sme-connect") == 0 || os_strcmp(work->type, "connect") == 0;
}

static int radio_work_is_scan(struct wpa_radio_work *work)
{
    return os_strcmp(work->type, "scan") == 0 || os_strcmp(work->type, "p2p-scan") == 0;
}
...
static struct wpa_radio_work *radio_work_get_next_work(struct wpa_radio *radio)
{
    ...
    if (!active_work)
    {
        ...
        dl_list_for_each(tmp, &radio->work, struct wpa_radio_work, list)
        {
            if (os_strcmp(tmp->type, "scan") == 0 && external_scan_running(radio) &&
                (((struct wpa_driver_scan_params *)tmp->ctx)->only_new_results || tmp->wpa_s->clear_driver_scan_cache))
                continue;
            return tmp;
        }
        return NULL;
    ...
    if (radio_work_is_connect(active_work))
    {
         ...
         */
        wpa_dbg(active_work->wpa_s, MSG_DEBUG, "Do not parallelize radio work with %s", active_work->type);
        return NULL;
    }
    ...
    dl_list_for_each(tmp, &radio->work, struct wpa_radio_work, list)
    {
        ...
        if (active_work->wpa_s == tmp->wpa_s && radio_work_is_scan(active_work) && radio_work_is_scan(tmp))
        {
            wpa_dbg(active_work->wpa_s, MSG_DEBUG, "Do not start work '%s' when another work '%s' is already scheduled",
         ...
         * on different bands.
         */
        if (os_strcmp(active_work->type, tmp->type) != 0 && (active_work->bands != tmp->bands))
        {
            /*
            ...
            wpa_dbg(active_work->wpa_s, MSG_DEBUG, "active_work:%s new_work:%s", active_work->type, tmp->type);
            return tmp;
        }
    }
...
static void radio_start_next_work(void *eloop_ctx, void *timeout_ctx)
{
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "Starting radio work '%s'@%p after %lld.%06lld second wait", work->type, work, diff.sec,
            diff.usec);
    work->started = 1;
    work->time    = now;
    ...
    if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_OFFCHANNEL_SIMULTANEOUS) && radio->num_active_works < MAX_ACTIVE_WORKS)
...
void radio_remove_pending_work(struct wpa_supplicant *wpa_s, void *ctx)
{
    ...
    dl_list_for_each(work, &radio->work, struct wpa_radio_work, list)
    {
        if (work->ctx != ctx)
            continue;
        wpa_dbg(wpa_s, MSG_DEBUG, "Free pending radio work '%s'@%p%s", work->type, work,
                   ...
                   void *ctx)
{
    ...
    if (freq)
        ...
        work->bands = wpas_get_bands(wpa_s, ((struct wpa_driver_scan_params *)ctx)->freqs);
    else
    ...
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "Try to schedule a radio work (num_active_works=%u)", radio->num_active_works);
        radio_work_check_next(wpa_s);
    }
...
void radio_work_done(struct wpa_radio_work *work)
{
    ...
    os_get_reltime(&now);
    os_reltime_sub(&now, &work->time, &diff);
    wpa_dbg(wpa_s, MSG_DEBUG, "Radio work '%s'@%p %s in %lld.%06lld seconds", work->type, work,
            started ? "done" : "canceled", diff.sec, diff.usec);
    radio_work_free(work);
    if (started)
        radio_work_check_next(wpa_s);
}
...
static int wpas_init_driver(struct wpa_supplicant *wpa_s, const struct wpa_interface *iface)
{
...
{
    wpa_printf(MSG_DEBUG, "GAS: TX status: freq=%u dst=" MACSTR " result=%s", freq, MAC2STR(dst),
               result == OFFCHANNEL_SEND_ACTION_SUCCESS ?
                   "SUCCESS" :
                   (result == OFFCHANNEL_SEND_ACTION_NO_ACK ? "no-ACK" : "FAILED"));
    gas_server_tx_status(wpa_s->gas_server, dst, data, data_len, result == OFFCHANNEL_SEND_ACTION_SUCCESS);
}

static void wpas_gas_server_tx(void *ctx, int freq, const u8 *da, struct wpabuf *buf, unsigned int wait_time)
{
    struct wpa_supplicant *wpa_s = ctx;
    ...
    offchannel_send_action(wpa_s, freq, da, wpa_s->own_addr, broadcast, wpabuf_head(buf), wpabuf_len(buf), wait_time,
                           wpas_gas_server_tx_status, 0);
}
...
static int wpa_supplicant_init_iface(struct wpa_supplicant *wpa_s, const struct wpa_interface *iface)
{
    ...
    wpa_printf(MSG_DEBUG,
               ...
               iface->ifname, iface->confname ? iface->confname : "N/A", iface->driver ? iface->driver : "default",
    ...
    if (iface->confname)
    {
        ...
        if (wpa_s->confanother && !wpa_config_read(wpa_s->confanother, wpa_s->conf))
        {
            wpa_printf(MSG_ERROR, "Failed to read or parse configuration '%s'.", wpa_s->confanother);
            return -1;
        }
    ...
    wpa_s->hw.modes = wpa_drv_get_hw_feature_data(wpa_s, &wpa_s->hw.num_modes, &wpa_s->hw.flags, &dfs_domain);
    if (wpa_s->hw.modes)
    {
        u16 i;
    ...
    if (capa_res == 0)
    {
        ...
        wpa_drv_get_ext_capa(wpa_s, WPA_IF_STATION);
        if (wpa_s->extended_capa && wpa_s->extended_capa_len >= 3 && wpa_s->extended_capa[2] & 0x40)
    ...
    if (wpa_s->conf->fst_group_id)
    {
        ...
        if (!wpa_s->fst)
        {
            wpa_msg(wpa_s, MSG_ERROR, "FST: Cannot attach iface %s to group %s", wpa_s->ifname, cfg.group_id);
            return -1;
        }
    }
#endif /* CONFIG_FST */
    ...
    if ((!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_DEDICATED_P2P_DEVICE) || wpa_s->p2p_mgmt) &&
    ...
    {
        size_t len;
        wpa_s->mnc_len = eapol_sm_get_eap_proxy_imsi(wpa_s->eapol, -1, wpa_s->imsi, &len);
        if (wpa_s->mnc_len > 0)
        {
            wpa_s->imsi[len] = '\0';
    ...
    if (!wpa_s->disable_mbo_oce && wpa_s->conf->oce)
    {
        ...
        if ((wpa_s->conf->oce & OCE_STA_CFON) && (wpa_s->drv_flags & WPA_DRIVER_FLAGS_OCE_STA_CFON))
        {
            ...
            wpa_printf(MSG_ERROR, "OCE STA-CFON feature is not yet supported");
        }
    }
...
static void wpa_supplicant_deinit_iface(struct wpa_supplicant *wpa_s, int notify, int terminate)
{
    ...
    if (wpa_s->drv_priv)
    {
         ...
         */
        if (!wpa_drv_get_wowlan(wpa_s) || wpa_s->conf->wowlan_disconnect_on_deinit)
        {
            wpa_supplicant_deauthenticate(wpa_s, WLAN_REASON_DEAUTH_LEAVING);
        ...
        {
            wpa_msg(wpa_s, MSG_INFO, "Do not deauthenticate as part of interface deinit since WoWLAN is enabled");
        }
    }
 ...
 */
struct wpa_interface *wpa_supplicant_match_iface(struct wpa_global *global, const char *ifname)
{
    int i;
...
struct wpa_global *wpa_supplicant_init(struct wpa_params *params)
{
    ...
    {
        global->params.match_ifaces = os_calloc(params->match_iface_count, sizeof(struct wpa_interface));
...
int wpa_supplicant_run(struct wpa_global *global)
{
    ...
    if (global->params.daemonize && (wpa_supplicant_daemon(global->params.pid_file) || eloop_sock_requeue()))
        return -1;
...
void wpa_supplicant_update_config(struct wpa_supplicant *wpa_s)
{
    ...
    if (wpa_s->conf->changed_parameters & CFG_CHANGED_WOWLAN_TRIGGERS)
    {
        ...
        if (res == 0 && wpas_set_wowlan_triggers(wpa_s, &capa) < 0)
            wpa_printf(MSG_ERROR, "Failed to update wowlan_triggers to '%s'", wpa_s->conf->wowlan_triggers);
    }
     ...
     * this is the case.
     */
    if ((wpa_s->conf->changed_parameters & CFG_CHANGED_BGSCAN) && wpa_s->current_ssid && !wpa_s->current_ssid->bgscan &&
        wpa_s->wpa_state == WPA_COMPLETED)
        wpa_supplicant_reset_bgscan(wpa_s);
#endif /* CONFIG_BGSCAN */
...
static int *get_bss_freqs_in_ess(struct wpa_supplicant *wpa_s)
{
    ...
    dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list)
    {
        if (bss == cbss)
            continue;
        if (bss->ssid_len == cbss->ssid_len && os_memcmp(bss->ssid, cbss->ssid, bss->ssid_len) == 0 &&
...
void wpas_connection_failed(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
    ...
    wpa_dbg(wpa_s, MSG_DEBUG, "Consecutive connection failures: %d --> request scan in %d ms",
...
void fils_pmksa_cache_flush(struct wpa_supplicant *wpa_s)
{
    ...
    if (eapol_sm_get_erp_info(wpa_s->eapol, &ssid->eap, &username, &username_len, &realm, &realm_len, &next_seq_num,
                              &rrk, &rrk_len) != 0 ||
        !realm)
    {
        wpa_dbg(wpa_s, MSG_DEBUG, "FILS: Drop external PMKSA cache entry");
...
void fils_connection_failure(struct wpa_supplicant *wpa_s)
{
    ...
    wpa_hexdump_ascii(MSG_DEBUG, "FILS: Store last connection failure realm", realm, realm_len);
    os_free(wpa_s->last_con_fail_realm);
...
int wpas_network_disabled(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
    ...
    if (wpa_key_mgmt_wpa_psk(ssid->key_mgmt) && !ssid->psk_set && (!ssid->passphrase || ssid->ssid_len != 0) &&
        !ssid->ext_psk && !(wpa_key_mgmt_sae(ssid->key_mgmt) && ssid->sae_password) && !ssid->mem_only_psk)
        return 1;
...
int wpas_get_ssid_pmf(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
    if (ssid == NULL || ssid->ieee80211w == MGMT_FRAME_PROTECTION_DEFAULT)
    {
        ...
        if (ssid && (ssid->key_mgmt & ~(WPA_KEY_MGMT_NONE | WPA_KEY_MGMT_WPS | WPA_KEY_MGMT_IEEE8021X_NO_WPA)) == 0)
        {
            /*
...
void wpas_auth_failed(struct wpa_supplicant *wpa_s, char *reason)
{
    ...
    wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_TEMP_DISABLED "id=%d ssid=\"%s\" auth_failures=%u duration=%d reason=%s",
            ssid->id, wpa_ssid_txt(ssid->ssid, ssid->ssid_len), ssid->auth_failures, dur, reason);
}
...
int get_shared_radio_freqs_data(struct wpa_supplicant *wpa_s, struct wpa_used_freq_data *freqs_data, unsigned int len)
{
    ...
    dl_list_for_each(ifs, &wpa_s->radio->ifaces, struct wpa_supplicant, radio_list)
    {
        ...
        {
            freqs_data[i].flags |=
                ifs->current_ssid->p2p_group ? WPA_FREQ_USED_BY_P2P_CLIENT : WPA_FREQ_USED_BY_INFRA_STATION;
        }
    }
...
struct wpa_supplicant *wpas_vendor_elem(struct wpa_supplicant *wpa_s, enum wpa_vendor_elem_frame frame)
{
    switch (frame)
    {
#ifdef CONFIG_P2P
...
int wpas_vendor_elem_remove(struct wpa_supplicant *wpa_s, int frame, const u8 *elem, size_t len)
{
    u8 *ie, *end;
                                  ...
                                  bool is_6ghz)
{
    ...
    {
        if (modes[i].mode != mode || !modes[i].num_channels || !modes[i].channels)
            continue;
...
struct hostapd_hw_modes *get_mode_with_freq(struct hostapd_hw_modes *modes, u16 num_modes, int freq)
{
    int i, j;
...
static struct wpa_bss_tmp_disallowed *wpas_get_disallowed_bss(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
    struct wpa_bss_tmp_disallowed *bss;

    dl_list_for_each(bss, &wpa_s->bss_tmp_disallowed, struct wpa_bss_tmp_disallowed, list)
    {
...
static int wpa_set_driver_tmp_disallow_list(struct wpa_supplicant *wpa_s)
{
    ...
    if (!bssids)
        return -1;
    dl_list_for_each(tmp, &wpa_s->bss_tmp_disallowed, struct wpa_bss_tmp_disallowed, list)
    {
        os_memcpy(&bssids[num_bssid * ETH_ALEN], tmp->bssid, ETH_ALEN);
        num_bssid++;
    }
...
void wpa_bss_tmp_disallow(struct wpa_supplicant *wpa_s, const u8 *bssid, unsigned int sec, int rssi_threshold)
{
    struct wpa_bss_tmp_disallowed *bss;
...
finish:
    ...
    eloop_register_timeout(sec, 0, wpa_bss_tmp_disallow_timeout, wpa_s, bss);
}

int wpa_is_bss_tmp_disallowed(struct wpa_supplicant *wpa_s, struct wpa_bss *bss)
{
    struct wpa_bss_tmp_disallowed *disallowed = NULL, *tmp, *prev;

    dl_list_for_each_safe(tmp, prev, &wpa_s->bss_tmp_disallowed, struct wpa_bss_tmp_disallowed, list)
    {
    ...
    if (disallowed->rssi_threshold != 0 && bss->level > disallowed->rssi_threshold)
    {
...
int wpas_enable_mac_addr_randomization(struct wpa_supplicant *wpa_s, unsigned int type, const u8 *addr, const u8 *mask)
{
    if ((addr && !mask) || (!addr && mask))
    {
        wpa_printf(MSG_INFO, "MAC_ADDR_RAND_SCAN invalid addr/mask combination");
        return -1;
    }

    if (addr && mask && (!(mask[0] & 0x01) || (addr[0] & 0x01)))
    {
        wpa_printf(MSG_INFO, "MAC_ADDR_RAND_SCAN cannot allow multicast address");
        return -1;
    }

    if (type & MAC_ADDR_RAND_SCAN)
    {
        if (wpas_mac_addr_rand_scan_set(wpa_s, MAC_ADDR_RAND_SCAN, addr, mask))
            return -1;
    }

    if (type & MAC_ADDR_RAND_SCHED_SCAN)
    {
        if (wpas_mac_addr_rand_scan_set(wpa_s, MAC_ADDR_RAND_SCHED_SCAN, addr, mask))
            return -1;
    ...
    {
        if (wpas_mac_addr_rand_scan_set(wpa_s, MAC_ADDR_RAND_PNO, addr, mask))
            return -1;
...
int wpas_disable_mac_addr_randomization(struct wpa_supplicant *wpa_s, unsigned int type)
{
...
int wpa_drv_signal_poll(struct wpa_supplicant *wpa_s, struct wpa_signal_info *si)
{
    ...
    if (res == 0)
    {
        ...
        dl_list_for_each(dso, &wpa_s->drv_signal_override, struct driver_signal_override, list)
        {
            if (os_memcmp(wpa_s->bssid, dso->bssid, ETH_ALEN) != 0)
                continue;
            wpa_printf(MSG_DEBUG,
                       "Override driver signal_poll information: current_signal: %d->%d avg_signal: %d->%d "
                       "avg_beacon_signal: %d->%d current_noise: %d->%d",
                       si->current_signal, dso->si_current_signal, si->avg_signal, dso->si_avg_signal,
                       si->avg_beacon_signal, dso->si_avg_beacon_signal, si->current_noise, dso->si_current_noise);
    ...
    for (idx = 0; scan_res && idx < scan_res->num; idx++)
    {
        ...
        dl_list_for_each(dso, &wpa_s->drv_signal_override, struct driver_signal_override, list)
        {
            if (os_memcmp(res->bssid, dso->bssid, ETH_ALEN) != 0)
                continue;
            wpa_printf(MSG_DEBUG, "Override driver scan signal level %d->%d for " MACSTR, res->level, dso->scan_level,
                       MAC2STR(res->bssid));
