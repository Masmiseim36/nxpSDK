...
#if defined(IEEE8021X_EAPOL) || !defined(CONFIG_NO_WPA)
static u8 *wpa_alloc_eapol(
    const struct wpa_supplicant *wpa_s, u8 type, const void *data, u16 data_len, size_t *msg_len, void **data_pos)
{
 ...
 */
int wpa_ether_send(struct wpa_supplicant *wpa_s, const u8 *dest, u16 proto, const u8 *buf, size_t len)
{
    ...
    {
        int encrypt = wpa_s->wpa && wpa_sm_has_ptk_installed(wpa_s->wpa);
...
static int wpa_supplicant_eapol_send(void *ctx, int type, const u8 *buf, size_t len)
{
    ...
    if (wpa_key_mgmt_wpa_psk(wpa_s->key_mgmt) || wpa_s->key_mgmt == WPA_KEY_MGMT_OWE ||
        wpa_s->key_mgmt == WPA_KEY_MGMT_DPP || wpa_s->key_mgmt == WPA_KEY_MGMT_NONE)
    {
    ...
    if (pmksa_cache_get_current(wpa_s->wpa) && type == IEEE802_1X_TYPE_EAPOL_START)
    {
        ...
        {
            wpa_printf(MSG_DEBUG,
                       "RSN: Timeout on waiting for the AP to initiate 4-way handshake for PMKSA caching or EAP "
                       "authentication - try to force it to start EAP authentication");
        }
        else
        {
    ...
    if (is_zero_ether_addr(wpa_s->bssid))
    {
        ...
        {
            dst = bssid;
            wpa_printf(MSG_DEBUG, "Using current BSSID " MACSTR " from the driver as the EAPOL destination",
                       MAC2STR(dst));
        }
        else
        {
            ...
            wpa_printf(MSG_DEBUG,
                       ...
                       " last received EAPOL frame " MACSTR
                       " as "
...
static void wpa_supplicant_eapol_cb(struct eapol_sm *eapol, enum eapol_supp_result result, void *ctx)
{
    ...
    if (result != EAPOL_SUPP_RESULT_SUCCESS || !(wpa_s->drv_flags & WPA_DRIVER_FLAGS_4WAY_HANDSHAKE_8021X))
        return;
    ...
    if (wpa_drv_set_key(wpa_s, 0, NULL, 0, 0, NULL, 0, pmk, pmk_len, KEY_FLAG_PMK))
    {
...
static int wpa_get_beacon_ie(struct wpa_supplicant *wpa_s)
{
    ...
    dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list)
    {
        if (os_memcmp(bss->bssid, wpa_s->bssid, ETH_ALEN) != 0)
            continue;
        if (ssid == NULL ||
            ((bss->ssid_len == ssid->ssid_len && os_memcmp(bss->ssid, ssid->ssid, ssid->ssid_len) == 0) ||
             ssid->ssid_len == 0))
        {
            curr = bss;
            break;
...
#ifdef CONFIG_OWE
        if (ssid && (ssid->key_mgmt & WPA_KEY_MGMT_OWE) && (bss->flags & WPA_BSS_OWE_TRANSITION))
        {
            curr = bss;
            break;
        }
                                  ...
                                  enum key_flag key_flag)
{
    ...
    if (key_idx > 0 && addr && is_broadcast_ether_addr(addr) && alg != WPA_ALG_NONE &&
...
#ifdef CONFIG_TESTING_OPTIONS
    if (addr && !is_broadcast_ether_addr(addr) && !(key_flag & KEY_FLAG_MODIFY))
    {
...
#endif /* CONFIG_TESTING_OPTIONS */
    return wpa_drv_set_key(wpa_s, alg, addr, key_idx, set_tx, seq, seq_len, key, key_len, key_flag);
}
...
static struct wpa_ssid *wpas_get_network_ctx(struct wpa_supplicant *wpa_s, void *network_ctx)
{
...
static int wpa_supplicant_add_pmkid(void *_wpa_s,
                                    void *network_ctx,
                                    const u8 *bssid,
                                    const u8 *pmkid,
                                    ...
                                    int akmp)
{
    ...
    if (ssid)
    {
        wpa_msg(wpa_s, MSG_INFO, PMKSA_CACHE_ADDED MACSTR " %d", MAC2STR(bssid), ssid->id);
        if ((akmp == WPA_KEY_MGMT_FT_IEEE8021X || akmp == WPA_KEY_MGMT_FT_IEEE8021X_SHA384) &&
...
static int wpa_supplicant_remove_pmkid(
    void *_wpa_s, void *network_ctx, const u8 *bssid, const u8 *pmkid, const u8 *fils_cache_id)
{
    struct wpa_supplicant *wpa_s = _wpa_s;
    struct wpa_ssid *ssid;
    struct wpa_pmkid_params params;
    ...
    ssid = wpas_get_network_ctx(wpa_s, network_ctx);
    if (ssid)
        wpa_msg(wpa_s, MSG_INFO, PMKSA_CACHE_REMOVED MACSTR " %d", MAC2STR(bssid), ssid->id);
    if (ssid && fils_cache_id)
    {
        params.ssid          = ssid->ssid;
        params.ssid_len      = ssid->ssid_len;
...
static int wpa_supplicant_send_ft_action(void *ctx, u8 action, const u8 *target_ap, const u8 *ies, size_t ies_len)
{
    ...
    *pos++ = action;
    ...
    ret = wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, wpa_s->bssid, wpa_s->own_addr, wpa_s->bssid, data, data_len,
                              0);
    os_free(data);
...
static int wpa_supplicant_send_tdls_mgmt(void *ctx,
                                         const u8 *dst,
                                         u8 action_code,
                                         u8 dialog_token,
                                         u16 status_code,
...
static int wpa_supplicant_tdls_peer_addset(void *ctx,
                                           const u8 *peer,
                                           int add,
                                           u16 aid,
                                           u16 capability,
...
static int wpa_supplicant_tdls_enable_channel_switch(void *ctx,
                                                     const u8 *addr,
                                                     u8 oper_class,
    ...
    for (ssid = wpa_s->conf->ssid; ssid; ssid = ssid->next)
    {
        ...
        for (i = 0; eap_methods[i].method != EAP_TYPE_NONE; i++)
        {
            if (eap_methods[i].vendor == EAP_VENDOR_IETF &&
                (eap_methods[i].method == EAP_TYPE_SIM || eap_methods[i].method == EAP_TYPE_AKA ||
...
static void wpa_supplicant_eap_proxy_notify_sim_status(void *ctx, enum eap_proxy_sim_state sim_state)
{
    struct wpa_supplicant *wpa_s = ctx;
...
static void wpa_supplicant_cert_cb(void *ctx, struct tls_cert_data *cert, const char *cert_hash)
{
    struct wpa_supplicant *wpa_s = ctx;
...
static int wpa_supplicant_eap_auth_start_cb(void *ctx)
{
    ...
    if (!wpa_s->new_connection && wpa_s->deny_ptk0_rekey && !wpa_sm_ext_key_id_active(wpa_s->wpa))
    {
        wpa_msg(wpa_s, MSG_INFO, "WPA: PTK0 rekey not allowed, reconnecting");
        wpa_supplicant_reconnect(wpa_s);
        return -1;
    }
    return 0;
...
int wpa_supplicant_init_eapol(struct wpa_supplicant *wpa_s)
{
    ...
    ctx->eap_proxy_notify_sim_status = wpa_supplicant_eap_proxy_notify_sim_status;
#endif /* CONFIG_EAP_PROXY */
...
static void wpa_supplicant_set_rekey_offload(
    void *ctx, const u8 *kek, size_t kek_len, const u8 *kck, size_t kck_len, const u8 *replay_ctr)
{
    struct wpa_supplicant *wpa_s = ctx;
...
static int wpa_supplicant_key_mgmt_set_pmk(void *ctx, const u8 *pmk, size_t pmk_len)
{
    ...
    if (wpa_s->conf->key_mgmt_offload && (wpa_s->drv_flags & WPA_DRIVER_FLAGS_KEY_MGMT_OFFLOAD))
        return wpa_drv_set_key(wpa_s, 0, NULL, 0, 0, NULL, 0, pmk, pmk_len, KEY_FLAG_PMK);
    else
        return 0;
}

static void wpa_supplicant_fils_hlp_rx(void *ctx, const u8 *dst, const u8 *src, const u8 *pkt, size_t pkt_len)
{
    struct wpa_supplicant *wpa_s = ctx;
    char *hex;
    size_t hexlen;
    ...
    wpa_msg(wpa_s, MSG_INFO, FILS_HLP_RX "dst=" MACSTR " src=" MACSTR " frame=%s", MAC2STR(dst), MAC2STR(src), hex);
    os_free(hex);
}
...
static void disable_wpa_wpa2(struct wpa_ssid *ssid)
{
    ...
    ssid->key_mgmt &= ~(WPA_KEY_MGMT_PSK | WPA_KEY_MGMT_FT_PSK | WPA_KEY_MGMT_PSK_SHA256);
    ssid->group_cipher &= ~WPA_CIPHER_TKIP;
    if (!(ssid->group_cipher & (WPA_CIPHER_CCMP | WPA_CIPHER_GCMP | WPA_CIPHER_GCMP_256 | WPA_CIPHER_CCMP_256)))
...
static void wpa_supplicant_transition_disable(void *_wpa_s, u8 bitmap)
{
...
#ifdef CONFIG_SAE
    if ((bitmap & TRANSITION_DISABLE_WPA3_PERSONAL) && wpa_key_mgmt_sae(wpa_s->key_mgmt) &&
        (ssid->key_mgmt & (WPA_KEY_MGMT_SAE | WPA_KEY_MGMT_FT_SAE)) &&
        (ssid->ieee80211w != MGMT_FRAME_PROTECTION_REQUIRED || (ssid->group_cipher & WPA_CIPHER_TKIP)))
    {
        wpa_printf(MSG_DEBUG, "WPA3-Personal transition mode disabled based on AP notification");
        disable_wpa_wpa2(ssid);
        changed = 1;
    }

    if ((bitmap & TRANSITION_DISABLE_SAE_PK) && wpa_key_mgmt_sae(wpa_s->key_mgmt) &&
#ifdef CONFIG_SME
        wpa_s->sme.sae.state == SAE_ACCEPTED && wpa_s->sme.sae.pk &&
#endif /* CONFIG_SME */
        (ssid->key_mgmt & (WPA_KEY_MGMT_SAE | WPA_KEY_MGMT_FT_SAE)) &&
        (ssid->sae_pk != SAE_PK_MODE_ONLY || ssid->ieee80211w != MGMT_FRAME_PROTECTION_REQUIRED ||
         (ssid->group_cipher & WPA_CIPHER_TKIP)))
    {
        wpa_printf(MSG_DEBUG, "SAE-PK: SAE authentication without PK disabled based on AP notification");
        disable_wpa_wpa2(ssid);
    ...
    if ((bitmap & TRANSITION_DISABLE_WPA3_ENTERPRISE) && wpa_key_mgmt_wpa_ieee8021x(wpa_s->key_mgmt) &&
        (ssid->key_mgmt & (WPA_KEY_MGMT_IEEE8021X | WPA_KEY_MGMT_FT_IEEE8021X | WPA_KEY_MGMT_IEEE8021X_SHA256)) &&
        (ssid->ieee80211w != MGMT_FRAME_PROTECTION_REQUIRED || (ssid->group_cipher & WPA_CIPHER_TKIP)))
    {
        disable_wpa_wpa2(ssid);
        changed = 1;
    ...
    if ((bitmap & TRANSITION_DISABLE_ENHANCED_OPEN) && wpa_s->key_mgmt == WPA_KEY_MGMT_OWE &&
...
static void wpa_supplicant_store_ptk(void *ctx, u8 *addr, int cipher, u32 life_time, const struct wpa_ptk *ptk)
{
    struct wpa_supplicant *wpa_s = ctx;
...
void wpa_supplicant_rsn_supp_set_config(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
    ...
    if (ssid)
    {
        ...
        if (wpa_key_mgmt_fils(wpa_s->key_mgmt))
            conf.fils_cache_id = wpa_bss_get_fils_cache_id(wpa_s->current_bss);
#endif /* CONFIG_FILS */
