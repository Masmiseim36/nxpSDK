...
int wpas_wps_eapol_cb(struct wpa_supplicant *wpa_s)
{
    ...
    if (!wpa_s->wps_success && wpa_s->current_ssid && eap_is_wps_pin_enrollee(&wpa_s->current_ssid->eap))
    {
        ...
        wpa_s->reassociate = 1;
        wpa_supplicant_req_scan(wpa_s, wpa_s->bssid_ignore_cleared ? 5 : 0, 0);
                                         ...
                                         const struct wps_credential *cred)
{
    ...
    if (ccmp && !(ssid->pairwise_cipher & WPA_CIPHER_CCMP) && (ssid->pairwise_cipher & WPA_CIPHER_TKIP) &&
        (capa.key_mgmt_iftype[iftype] & WPA_DRIVER_CAPA_KEY_MGMT_WPA2_PSK))
    {
        wpa_printf(MSG_DEBUG,
...
static void wpas_wps_remove_dup_network(struct wpa_supplicant *wpa_s, struct wpa_ssid *new_ssid)
{
    ...
    for (ssid = wpa_s->conf->ssid, next = ssid ? ssid->next : NULL; ssid; ssid = next, next = ssid ? ssid->next : NULL)
    {
        ...
        if (ssid->auth_alg != new_ssid->auth_alg || ssid->key_mgmt != new_ssid->key_mgmt ||
...
static int wpa_supplicant_wps_cred(void *ctx, const struct wps_credential *cred)
{
    ...
    if (ssid && (ssid->key_mgmt & WPA_KEY_MGMT_WPS))
    {
        ...
        if (ssid->eap.identity && ssid->eap.identity_len == WSC_ID_REGISTRAR_LEN &&
            os_memcmp(ssid->eap.identity, WSC_ID_REGISTRAR, WSC_ID_REGISTRAR_LEN) == 0)
            registrar = 1;
#endif /* CONFIG_WPS_REG_DISABLE_OPEN */
    ...
    switch (cred->encr_type)
    {
        ...
        case WPS_ENCR_AES:
            ...
            if (wpa_s->drv_capa_known && (wpa_s->drv_enc & WPA_DRIVER_CAPA_ENC_GCMP_256))
            {
                ssid->pairwise_cipher |= WPA_CIPHER_GCMP_256;
                ssid->group_cipher |= WPA_CIPHER_GCMP_256;
            }
            if (wpa_s->drv_capa_known && (wpa_s->drv_enc & WPA_DRIVER_CAPA_ENC_CCMP_256))
            {
...
{
    wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_M2D "dev_password_id=%d config_error=%d", m2d->dev_password_id,
            m2d->config_error);
    wpas_notify_wps_event_m2d(wpa_s, m2d);
#ifdef CONFIG_P2P
    if (wpa_s->p2pdev && wpa_s->p2pdev != wpa_s)
    {
        wpa_msg(wpa_s->p2pdev, MSG_INFO, WPS_EVENT_M2D "dev_password_id=%d config_error=%d", m2d->dev_password_id,
                m2d->config_error);
    }
...
static void wpa_supplicant_wps_event_fail(struct wpa_supplicant *wpa_s, struct wps_event_fail *fail)
{
    ...
    {
        wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_FAIL "msg=%d config_error=%d reason=%d (%s)", fail->msg, fail->config_error,
                fail->error_indication, wps_ei_str(fail->error_indication));
        if (wpa_s->p2pdev && wpa_s->p2pdev != wpa_s)
            wpa_msg(wpa_s->p2pdev, MSG_INFO, WPS_EVENT_FAIL "msg=%d config_error=%d reason=%d (%s)", fail->msg,
                    fail->config_error, fail->error_indication, wps_ei_str(fail->error_indication));
    }
    else
    {
...
{
    return eloop_is_timeout_registered(wpas_wps_reenable_networks_cb, wpa_s, NULL);
}
...
static void wpa_supplicant_wps_event_er_ap_add(struct wpa_supplicant *wpa_s, struct wps_event_er_ap *ap)
{
    ...
    wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_ER_AP_ADD "%s " MACSTR " pri_dev_type=%s wps_state=%d |%s|%s|%s|%s|%s|%s|",
            uuid_str, MAC2STR(ap->mac_addr), dev_type, ap->wps_state, ap->friendly_name ? ap->friendly_name : "",
            ap->manufacturer ? ap->manufacturer : "", ap->model_description ? ap->model_description : "",
            ap->model_name ? ap->model_name : "", ap->manufacturer_url ? ap->manufacturer_url : "",
                                                     ...
                                                     struct wps_event_er_enrollee *enrollee)
{
    ...
    wpa_msg(wpa_s, MSG_INFO,
            WPS_EVENT_ER_ENROLLEE_ADD "%s " MACSTR
                                      ...
                                      "|%s|%s|%s|%s|%s|",
            uuid_str, MAC2STR(enrollee->mac_addr), enrollee->m1_received, enrollee->config_methods,
            enrollee->dev_passwd_id, dev_type, enrollee->dev_name ? enrollee->dev_name : "",
            enrollee->manufacturer ? enrollee->manufacturer : "", enrollee->model_name ? enrollee->model_name : "",
...
static void wpa_supplicant_wps_event_er_ap_settings(struct wpa_supplicant *wpa_s,
                                                    struct wps_event_er_ap_settings *ap_settings)
{
    ...
    wpa_msg_ctrl(wpa_s, MSG_INFO,
                 ...
                 "key=%s",
                 uuid_str, wpa_ssid_txt(cred->ssid, cred->ssid_len), cred->auth_type, cred->encr_type, key_str);
}

static void wpa_supplicant_wps_event_er_set_sel_reg(struct wpa_supplicant *wpa_s,
    ...
    switch (ev->state)
    {
        case WPS_ER_SET_SEL_REG_START:
            wpa_msg(wpa_s, MSG_DEBUG,
                    ...
                    uuid_str, ev->sel_reg, ev->dev_passwd_id, ev->sel_reg_config_methods);
            break;
                                             ...
                                             const u8 *bssid)
{
    ...
    wpa_config_set_network_defaults(ssid);
    ssid->temporary = 1;
    if (wpa_config_set(ssid, "key_mgmt", "WPS", 0) < 0 || wpa_config_set(ssid, "eap", "WSC", 0) < 0 ||
...
int wpas_wps_start_pbc(struct wpa_supplicant *wpa_s, const u8 *bssid, int p2p_group, int multi_ap_backhaul_sta)
{
    struct wpa_ssid *ssid;
    char phase1[32];
    ...
    if (p2p_group && wpa_s->go_params && wpa_s->go_params->ssid_len)
    {
        ...
        if (ssid->ssid)
        {
            ...
            if (wpa_s->go_params->edmg && wpas_p2p_try_edmg_channel(wpa_s, wpa_s->go_params) == 0)
                ssid->enable_edmg = 1;
...
#endif /* CONFIG_P2P */
    os_snprintf(phase1, sizeof(phase1), "pbc=1%s", multi_ap_backhaul_sta ? " multi_ap=1" : "");
                       ...
                       struct wps_new_ap_settings *settings)
{
    ...
    if (settings)
    {
        res = os_snprintf(pos, end - pos,
                          ...
                          settings->ssid_hex, settings->auth, settings->encr, settings->key_hex);
        if (os_snprintf_error(end - pos, res))
            return -1;
        pos += res;
    }
...
static int wpas_wps_new_psk_cb(void *ctx, const u8 *mac_addr, const u8 *p2p_dev_addr, const u8 *psk, size_t psk_len)
{
    ...
    {
        wpa_printf(MSG_DEBUG, "Received new WPA/WPA2-PSK from WPS for STA " MACSTR " P2P Device Addr " MACSTR,
...
static void wpas_wps_pin_needed_cb(void *ctx, const u8 *uuid_e, const struct wps_device_data *dev)
{
    ...
    len = os_snprintf(txt, sizeof(txt), "WPS-EVENT-PIN-NEEDED %s " MACSTR " [%s|%s|%s|%s|%s|%s]", uuid,
                      MAC2STR(dev->mac_addr), dev->device_name, dev->manufacturer, dev->model_name, dev->model_number,
                      dev->serial_number, wps_dev_type_bin2str(dev->pri_dev_type, devtype, sizeof(devtype)));
...
{
    if ((config_methods & (WPS_CONFIG_DISPLAY | WPS_CONFIG_VIRT_DISPLAY | WPS_CONFIG_PHY_DISPLAY)) ==
        WPS_CONFIG_DISPLAY)
    {
        wpa_printf(MSG_INFO,
    ...
    if ((config_methods & (WPS_CONFIG_PUSHBUTTON | WPS_CONFIG_VIRT_PUSHBUTTON | WPS_CONFIG_PHY_PUSHBUTTON)) ==
...
int wpas_wps_ssid_bss_match(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid, struct wpa_bss *bss)
{
    ...
    if (eap_is_wps_pin_enrollee(&ssid->eap))
    {
        ...
        if (!wps_is_addr_authorized(wps_ie, wpa_s->own_addr, 1))
        {
            ...
            {
                wpa_printf(MSG_DEBUG, "   skip - WPS AP without active PIN Registrar (scan_runs=%d age=%d)",
...
int wpas_wps_ssid_wildcard_ok(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid, struct wpa_bss *bss)
{
    ...
    else if (eap_is_wps_pin_enrollee(&ssid->eap))
    {
        wps_ie = wpa_bss_get_vendor_ie_multi(bss, WPS_IE_VENDOR_TYPE);
        if (wps_ie &&
            (wps_is_addr_authorized(wps_ie, wpa_s->own_addr, 1) || wpa_s->scan_runs >= WPS_PIN_SCAN_IGNORE_SEL_REG))
        {
...
int wpas_wps_scan_pbc_overlap(struct wpa_supplicant *wpa_s, struct wpa_bss *selected, struct wpa_ssid *ssid)
{
    ...
    if (!is_zero_ether_addr(ssid->bssid))
        wpa_printf(MSG_DEBUG, "WPS: Network profile limited to accept only a single BSSID " MACSTR,
                   MAC2STR(ssid->bssid));
    ...
    for (i = 0; i < wpa_s->num_wps_ap; i++)
    {
        ...
        if (!is_zero_ether_addr(ssid->bssid) && os_memcmp(ap->bssid, ssid->bssid, ETH_ALEN) != 0)
        {
            wpa_printf(MSG_DEBUG, "WPS: Ignore another BSS " MACSTR " in active PBC mode due to local BSSID limitation",
                       MAC2STR(ap->bssid));
            continue;
        }
        ...
        {
            ret = 1; /* PBC overlap */
            wpa_msg(wpa_s, MSG_INFO, "WPS: PBC overlap detected: " MACSTR " and " MACSTR, MAC2STR(selected->bssid),
                    MAC2STR(ap->bssid));
            break;
        }
...
int wpas_wps_er_config(struct wpa_supplicant *wpa_s,
                       const char *uuid,
                       const char *pin,
                       struct wps_new_ap_settings *settings)
{
...
struct wpabuf *wpas_wps_nfc_token(struct wpa_supplicant *wpa_s, int ndef)
{
    ...
    {
        return wps_nfc_token_build(ndef, wpa_s->conf->wps_nfc_dev_pw_id, wpa_s->conf->wps_nfc_dh_pubkey,
                                   wpa_s->conf->wps_nfc_dev_pw);
    }

    return wps_nfc_token_gen(ndef, &wpa_s->conf->wps_nfc_dev_pw_id, &wpa_s->conf->wps_nfc_dh_pubkey,
                             &wpa_s->conf->wps_nfc_dh_privkey, &wpa_s->conf->wps_nfc_dev_pw);
}

int wpas_wps_start_nfc(struct wpa_supplicant *wpa_s,
                       ...
                       int freq)
{
    ...
    return wpas_wps_start_dev_pw(wpa_s, go_dev_addr, bssid, dev_pw ? pw : NULL, p2p_group, dev_pw_id, peer_pubkey_hash,
                                 ssid, ssid_len, freq);
}
...
{
    return wps_registrar_add_nfc_password_token(wpa_s->wps->registrar, attr->oob_dev_password,
...
static int wpas_wps_nfc_rx_handover_sel(struct wpa_supplicant *wpa_s, const struct wpabuf *data)
{
    ...
    if (wsc_len < wpabuf_len(wps) - 2)
    {
        wpa_hexdump(MSG_DEBUG, "WPS: Ignore extra data after WSC attributes", pos + wsc_len,
                    wpabuf_len(wps) - 2 - wsc_len);
    }
    ...
    if (attr.ap_channel)
    {
        ...
        if (chan >= 1 && chan <= 13 && (attr.rf_bands == NULL || *attr.rf_bands & WPS_RF_24GHZ))
            ...
            freq = 5000 + 5 * chan;
        else if (chan >= 1 && chan <= 6 && (attr.rf_bands == NULL || *attr.rf_bands & WPS_RF_60GHZ))
    ...
    ret = wpas_wps_start_nfc(wpa_s, NULL, bssid, NULL, dev_pw_id, 0, attr.oob_dev_password, attr.ssid, attr.ssid_len,
                             freq);
...
int wpas_er_wps_nfc_report_handover(struct wpa_supplicant *wpa_s, const struct wpabuf *req, const struct wpabuf *sel)
{
    ...
    ret = wps_registrar_add_nfc_pw_token(wpa_s->wps->registrar, attr.oob_dev_password, DEV_PW_NFC_CONNECTION_HANDOVER,
                                         NULL, 0, 1);
...
static void wpas_wps_dump_ap_info(struct wpa_supplicant *wpa_s)
{
    ...
    {
        struct wps_ap_info *ap     = &wpa_s->wps_ap[i];
        struct wpa_bssid_ignore *e = wpa_bssid_ignore_get(wpa_s, ap->bssid);
        ...
        wpa_printf(MSG_DEBUG,
                   ...
                   ap->last_attempt.sec > 0 ? (int)now.sec - (int)ap->last_attempt.sec : -1, e ? e->count : 0);
    }
}
