...
/*!=============================================================================
\file       bdb_nf.c
\brief      BDB Network Formation implementation
==============================================================================*/
...
/****************************************************************************
 *
 * NAME: BDB_vNfGetNwkAddress
 *
 * DESCRIPTION:
 *  BDB Network formation return of network address
 *
 * PARAMETERS:      Name            RW  Usage
 * 
 * RETURNS:
...
{
    return sStartParams.sNwkParams.u16NwkAddr;
}
/****************************************************************************
 * NAME: BDB_vNfGetPanID
 *
 * DESCRIPTION:
 *  BDB Network formation return of PAN ID
 *
 * PARAMETERS:      Name            RW  Usage
 *
 * RETURNS:
 *  uint16          Value of PAN ID
 *
 ****************************************************************************/
PUBLIC uint16 BDB_vNfGetPanID(void)
{
    return sStartParams.sNwkParams.u16PanId;
}

/****************************************************************************
 *
 * NAME: BDB_vNfSetNwkAddress
 *
 * DESCRIPTION:
 *  BDB Network formation set of network address
 *
 * PARAMETERS:      Name            RW  Usage
 *                  nwkAddr         R   Network address to be set
 * RETURNS:
 *  void
 *
 ****************************************************************************/
...
{
    sStartParams.sNwkParams.u16NwkAddr = nwkAddr;
    ZPS_vNwkNibSetNwkAddr(ZPS_pvAplZdoGetNwkHandle(), sStartParams.sNwkParams.u16NwkAddr);
}
 
/****************************************************************************
 * NAME: BDB_vNfSetPanID
 *
 * DESCRIPTION:
 *  BDB Network formation set of PAN ID
 *
 * PARAMETERS:      Name            RW  Usage
 *                  panId           R   PAN ID to be set
 * RETURNS:
 *  void
 *
 ****************************************************************************/
...
{
    sStartParams.sNwkParams.u16PanId = panId;
    ZPS_vNwkNibSetPanId (ZPS_pvAplZdoGetNwkHandle(), sStartParams.sNwkParams.u16PanId);
}
...
PRIVATE void vNfDiscoverNwk()
{
    ...
    if (sStartParams.sNwkParams.u16NwkAddr == 0)
    {
    ...
    if(u8Status == BDB_TC_STATUS_STD_UNSEC_JOIN)
    {
        for (i = 0; i < u32KeyTblSize; i++)
        {
            if (u64DeviceAddress == ZPS_u64NwkNibGetMappedIeeeAddr( ZPS_pvAplZdoGetNwkHandle(), psKeyTbl[i].u16ExtAddrLkup ) )
            {
                ...
                FLib_MemSet(psKeyTbl[i].au8LinkKey, 0, ZPS_SEC_KEY_LENGTH);
#else
