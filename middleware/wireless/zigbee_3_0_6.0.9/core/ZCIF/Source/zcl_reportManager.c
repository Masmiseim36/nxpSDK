                                ...
                                tsZCL_AttributeReportingConfigurationRecord    *psAttributeReportingRecord)
{
...
#ifndef ZCLIP_ENABLED /* ZCLIP allows to have the same attribute in multiple reports, so skip the check below */
    ...
    // ZCLIP: Block moved down so that first report is not triggered right after creation, but after attribute change
    // Possible specs divergence here (ZCLIP vs ZCL)
    // store new value
    ...
    FLib_MemCpy(&psHeadReportRecord->sAttributeReportingConfigurationRecord, psAttributeReportingRecord, sizeof(tsZCL_AttributeReportingConfigurationRecord));
