                        ...
                        tsZCL_Address              *psDestinationAddress)
{
    ...
    eZCL_SearchForClusterEntry(u8SourceEndPointId,
        u16ClusterId,
        FALSE,
        &psClusterInstance);
    ...
    if((u8SourceEndPointId == ZCL_GP_PROXY_ENDPOINT_ID)&&(psClusterInstance == NULL))
    {
        ...
        {
            eZCL_SearchForClusterEntry(u8DestinationEndPointId,
                  u16ClusterId,
                  TRUE,
                  &psClusterInstance);
    ...
    switch(psDestinationAddress->eAddressMode)
    {
        ...
        case(E_ZCL_AM_GROUP):
        {
            ...
            eStatus = eGP_FindGpCluster(psZCL_Common->u8GreenPowerMappedEpId, TRUE,
        ...
        case(E_ZCL_AM_BROADCAST):
        {
            ...
            uint8_t u8BroadcastMode = (uint8_t) psDestinationAddress->uAddress.eBroadcastMode;
            
            if ( u8BroadcastMode == 0xFC )
              u8BroadcastMode = ZPS_E_APL_AF_BROADCAST_ZC_ZR;
            
            if( u8BroadcastMode == 0xFD )
              u8BroadcastMode = ZPS_E_APL_AF_BROADCAST_RX_ON;
                    ...
                    uint8                      *pu8PDU)
{
    ...
    while(u8NoOfBufferedCopied < u8NoOfBuffersRequired)
    {
        ...
        {
            FLib_MemCpy(psBindServerBufferedPDURecord->sBindServerBufferedPDUInfo.au8PDU,
...
PUBLIC void vZCL_BindServerTransmissionTimerCallback(void)
{
    ...
    if(psBindServerBufferedPDURecord != NULL)
    {
        ...
        while(u8NoOfBufferedCopied < u8NoOfBuffersRequiredToCopyPDU)
        {
            ...
            {
                FLib_MemCpy(pu8PDU, psBindServerBufferedPDURecord->sBindServerBufferedPDUInfo.au8PDU,
