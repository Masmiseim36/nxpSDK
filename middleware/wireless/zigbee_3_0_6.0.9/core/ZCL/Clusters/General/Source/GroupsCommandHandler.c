    ...
    if(psClusterInstance->bIsServer == TRUE)
    {
    	uint16 u16ActualQuantity;
    	  tsZCL_RxPayloadItem asPayloadDefinition[] = {
    	    ...
    	    {1, &u16ActualQuantity, E_ZCL_TLV,  0},
#endif
    	                                                };
...
#if (CLD_GROUPS_MAX_GROUP_NAME_LENGTH != 0)
...
#if (defined(ZCLIP_ENABLED) && defined(ZCLIP_GROUPS_SERVER_CMD_EXTENSIONS))
    uint8 u8AddrAssignmentMode;
    uint8 ip6MulticastAddress[17];
    uint16 u16Port;
    tsZCL_Tlv sAddrAssignmentModeTlv = {
            E_CLD_GROUPS_CMD_TLV_TAG_ADDR_ASSIGNMENT_MODE,
            sizeof(uint8),
            0,
            &u8AddrAssignmentMode
    };
    tsZCL_Tlv sIpv6MulticastAddrTlv = {
            E_CLD_GROUPS_CMD_TLV_TAG_MULTICAST_IPV6_ADDR,
            sizeof(ip6MulticastAddress),
            0,
            ip6MulticastAddress
    };
    tsZCL_Tlv sGroupPortTlv = {
            E_CLD_GROUPS_CMD_TLV_TAG_PORT,
            sizeof(uint16),
            0,
            (uint8 *)&u16Port
    };
    tsZCL_Tlv psTlvlist[] = {sAddrAssignmentModeTlv, sIpv6MulticastAddrTlv, sGroupPortTlv};
#endif
...
#else
    sPayload.sGroupName.pu8Data = NULL;
    sPayload.sGroupName.u8MaxLength = CLD_GROUPS_MAX_GROUP_NAME_LENGTH;
#endif
    
#if (defined(ZCLIP_ENABLED) && defined(ZCLIP_GROUPS_SERVER_CMD_EXTENSIONS))
    sPayload.psTlvList = psTlvlist;
    sPayload.u8TlvCount = (sizeof(psTlvlist)/sizeof(tsZCL_Tlv));
#endif
...
#if (defined(ZCLIP_ENABLED) && defined(ZCLIP_GROUPS_SERVER_CMD_EXTENSIONS))
    pasPayloadDefinition[2].pvDestination = sPayload.psTlvList;
    pasPayloadDefinition[2].u16MaxQuantity = sPayload.u8TlvCount;
#endif
...
#if (defined(ZCLIP_ENABLED) && defined(ZCLIP_GROUPS_SERVER_CMD_EXTENSIONS))
        for(x = 0; x < sPayload.u8TlvCount; x++)
        {
            tsZCL_Tlv *zclTlv = &sPayload.psTlvList[x];
            if (zclTlv != NULL)
            {
                int i;
                DBG_vPrintf(TRACE_GROUPS, " \nTLV TAG=%02x LEN=%04x VALUE=",
                                                    zclTlv->eTag,
                                                    zclTlv->u16Length);

                for(i = 0; i < zclTlv->u16Length; i++)
                {
                    DBG_vPrintf(TRACE_GROUPS, "%02x", zclTlv->pu8Data[i]);
                }
            }
        }
#endif
...
#ifdef ZCLIP_ENABLED
        /* Zigbee and ZCLIP spec do not specify INVALID_FIELD as a valid status for AddGroup command
         * However, non ZCLIP implementation used to return this code.
         */
...
#ifdef ZCLIP_ENABLED
    pasPayloadDefinition[0].eTag = E_ZCL_PT_ARRAY_COUNT;
#endif
...
#ifdef ZCLIP_ENABLED
    pasPayloadDefinition[1].eTag = E_ZCL_PT_ARRAY_ELEMS;
#endif
    eStatus = eZCL_CustomCommandReceive(pZPSevent,
            &u8TransactionSequenceNumber,
            pasPayloadDefinition,
...
#ifdef ZCLIP_ENABLED
        if (sResponse.u8GroupCount == 0)
        {
            /* If no group found do not reply according to the zcl spec */
            return eStatus;
        }
#endif
...
#ifdef ZCLIP_ENABLED
	pasPayloadDefinition[1].eTag = E_ZCL_PT_ARRAY_COUNT;
#endif
	pasPayloadDefinition[2].pvDestination = sResponse.pi16GroupList;
#ifdef ZCLIP_ENABLED
    pasPayloadDefinition[2].eTag = E_ZCL_PT_ARRAY_ELEMS;
#endif
