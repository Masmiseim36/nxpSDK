    ...
    for(i = 0; i < u8NumEndpoints; i++)
    {
        ...
        if(psCustomDataStructure->sControlParameters.bFastPollMode)
        {
            ...
            if(psCustomDataStructure->sControlParameters.u16CurrentFastPollTimeout > 0){
                ...
                FLib_MemSet(psCustomDataStructure->sControlParameters.asCheckinEntry,0,sizeof(tsCLD_PollControlCheckinEntry) * CLD_POLL_CONTROL_NUMBER_OF_MULTIPLE_CLIENTS);
            }   
 ...
 ** bool                                    *pbEntryFound               Whether Entry is present or not
 **
 ** RETURN:
 ** uint8 
 ** 
