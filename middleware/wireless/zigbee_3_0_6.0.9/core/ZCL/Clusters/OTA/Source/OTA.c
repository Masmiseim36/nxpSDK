...
uint8 au8OTAAttributeControlBits[(sizeof(asOTAClusterAttributeDefinitions) / sizeof(tsZCL_AttributeDefinition))];
...
/****************************************************************************/
#ifdef OTA_CLIENT
static void vOTA_InitHeaderIfRequired(void);
#endif

#ifdef OTA_SERVER
#ifdef ZCLIP_ENABLED
PRIVATE void vOTA_BuildFileUri(char * uri, uint32 maxLength, uint16 u16ManufacturerCode, uint16 u16ImageType, uint32 u32FileVersion);
#endif
#if OTA_MAX_SIMULTANEOUS_CLIENTS
PRIVATE otaClientSessionInfo_t mOtaClientSessionInfoTable[OTA_MAX_SIMULTANEOUS_CLIENTS];
#endif
#endif
...
tsNvmDefs  sNvmDefsStruct;
 ...
 /* Please consider that in the current version 
    the OTA header is stored in RAM */
uint8 au8OtaHeader[OTA_MAX_HEADER_SIZE] /* __attribute__ ((section (".ro_ota_header"))) */ =
       {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
        0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
#ifdef ZCLIP_ENABLED
        0x4E, 0x58, 0x50, 0x20, 0x5A, 0x43, 0x4C, 0x49, 0x50, 0x20,
        0x49, 0x6D, 0x61, 0x67, 0x65, 0x20, 0x46, 0x69, 0x6C, 0x65,
                ...
                tsOTA_Common                       *psCustomDataStruct)
{
    ...
    FLib_MemSet(&psCustomDataStruct->sOTACallBackMessage.uMessage,0,sizeof(psCustomDataStruct->sOTACallBackMessage.uMessage));
    ...
    #endif
#if OTA_MAX_SIMULTANEOUS_CLIENTS
        for(int i=0; i<OTA_MAX_SIMULTANEOUS_CLIENTS; i++)
        {
            mOtaClientSessionInfoTable[i].remoteAddr = 0xffff;
            mOtaClientSessionInfoTable[i].timeStamp = 0;
        }
#endif
#endif
                           ...
                           == E_ZCL_SUCCESS)
    {
        ...
        else
        {
            ...
            if(bIsServer)
            {
                ...
                FLib_MemCpy(au8Header, psCustomData->sOTACallBackMessage.au8ServerOTAHeader, u16HeaderSize);
#endif
            }
            else
            {
                ...
                FLib_MemCpy(au8Header, psCustomData->sOTACallBackMessage.sPersistedData.au8Header, u16HeaderSize);
#endif
            }
        }
                                   ...
                                   bool_t               bReset)
{
                           ...
                           == E_ZCL_SUCCESS)
    {
        if(bReset)
        {
            FLib_MemCpy(&psCustomData->sOTACallBackMessage.sPersistedData,psOTAData, sizeof(tsOTA_PersistedData) );
...
#ifdef OTA_CLD_ATTR_CURRENT_FILE_VERSION
    sOta.u32CurrentFileVersion = OTA_FILE_VERSION_DEFAULT;
#endif

#ifdef OTA_CLD_ATTR_CURRENT_ZIGBEE_STACK_VERSION
    sOta.u16CurrentStackVersion = OTA_STACK_VERSION;
#endif

#ifdef OTA_CLD_ATTR_DOWNLOADED_FILE_VERSION
    sOta.u32DownloadedFileVersion = OTA_FILE_VERSION_DEFAULT;
#endif

#ifdef OTA_CLD_ATTR_DOWNLOADED_ZIGBEE_STACK_VERSION
    sOta.u16DownloadedStackVersion = OTA_STACK_VERSION;
#endif
...
#ifdef OTA_CLD_ATTR_IMAGE_TYPE
    sOta.u16ImageType = OTA_IMAGE_TYPE_DEFAULT;
#endif
...
#else
        psCustomData->sOTACallBackMessage.sPersistedData.serverAddress.uAddress.u16DestinationAddress = u16ShortAddress;
#endif
                                            ...
                                            tsCLD_PR_Ota  *psOTAData)
{
    ...
    if((eStatus = eOtaFindCluster(u8Endpoint,
                         &psEndPointDefinition,
                           ...
                           == E_ZCL_SUCCESS)
    {
        FLib_MemCpy (&psCustomData->sOTACallBackMessage.aServerPrams[u8ImageIndex],psOTAData, sizeof(tsCLD_PR_Ota));
    }
    return eStatus;
}
                                          ...
                                          tsCLD_PR_Ota    *psOTAData)
{
    ...
    if((eStatus = eOtaFindCluster(u8Endpoint,
                         &psEndPointDefinition,
                           ...
                           == E_ZCL_SUCCESS)
    {
        FLib_MemCpy (psOTAData, &psCustomData->sOTACallBackMessage.aServerPrams[u8ImageIndex], sizeof(tsCLD_PR_Ota));
    }
    return eStatus;
}
/****************************************************************************
                           ...
                           == E_ZCL_SUCCESS)
    {
        ...
        if(bIsImageOnCoProcessorMedia)
        {
            ...
            for(i = 0; i < OTA_MAX_CO_PROCESSOR_IMAGES; i++)
            {
                ...
                {
                    FLib_MemCpy(&sOTAHeader[i], &psOTA_CoProcessorOTAHeader->sOTA_ImageHeader[i],
...
#ifdef ZCLIP_ENABLED
                    vOTA_BuildFileUri((char *) psCustomData->sOTACallBackMessage.aServerPrams[OTA_MAX_IMAGES_PER_ENDPOINT+i].sFileUri.uri,
                                      OTA_MAX_OTA_FILE_URI_LENGTH,
                                      sOTAHeader[i].u16ManufacturerCode,
                                      sOTAHeader[i].u16ImageType,
                                      sOTAHeader[i].u32FileVersion);
#endif
                }
                else
                {
        ...
        {
            {
                ...
                u32FlashOffset = u8CurrentSector*sNvmDefsStruct.u32SectorSize;
...
#ifdef ZCLIP_ENABLED
                    vOTA_BuildFileUri((char *) psCustomData->sOTACallBackMessage.aServerPrams[i].sFileUri.uri,
                                      OTA_MAX_OTA_FILE_URI_LENGTH,
                                      sImageHeader.u16ManufacturerCode,
                                      sImageHeader.u16ImageType,
                                      sImageHeader.u32FileVersion);
#endif
                           ...
                           == E_ZCL_SUCCESS)
    {
        ...
        if(bIsServerImage)
        {
            ...
            if(!psCustomData->sOTACallBackMessage.bIsOTAHeaderCopied)
            {
                ...
                {
                    FLib_MemCpy(&psCustomData->sOTACallBackMessage.au8ServerOTAHeader[u32FileOffSet], pu8UpgradeBlockData, u8UpgradeBlockDataLength);
                }
                else
                {
                    uint8 u8DataOverFlow = OTA_MIN_HEADER_SIZE - (uint8)u32FileOffSet;
                    FLib_MemCpy(&psCustomData->sOTACallBackMessage.au8ServerOTAHeader[u32FileOffSet], pu8UpgradeBlockData, u8DataOverFlow);
                            ...
                            u16OTAHeaderLength <= OTA_MAX_HEADER_SIZE)
                    {
                        if( (u32FileOffSet + u8UpgradeBlockDataLength) <= (uint32)(u16OTAHeaderLength + OTA_TAG_HEADER_SIZE))
                        {
                            ...
                            FLib_MemCpy(&psCustomData->sOTACallBackMessage.au8ServerOTAHeader[OTA_MIN_HEADER_SIZE], pu8UpgradeBlockData+u8DataOverFlow, u8HeaderOverFlow);
                        }
                        else
                        {
                            ...
                            FLib_MemCpy(&psCustomData->sOTACallBackMessage.au8ServerOTAHeader[u32FileOffSet],
                                        pu8UpgradeBlockData, u8ServerOTAHeaderRemBytesWrite);


                            /* Get Starting Image Offset */
                            u32FlashOffset = (psCustomData->sOTACallBackMessage.u8ImageStartSector[u8ImageIndex] *
                                    sNvmDefsStruct.u32SectorSize);
                    ...
                    sOTA_ImageHeader.u16ImageType == sOtaGetHeader(&au8OtaHeader[0]).u16ImageType)
            {
                ...
                if(u32Offset > 0)
                {
                    ...
                    if (bEncExternalFlash)
                    {
                        ...
                        FLib_MemCpy(au8TempBuffer, pu8MacAddress, 8);
                        ...
                        */
                        ...
                        FLib_MemCpy(au8TempBuffer, pu8LinkKey, 16);
                        ...
                        FLib_MemCpy(au8TempBuffer, pu8Cert, 48);
                        ...
                        FLib_MemCpy(au8TempBuffer, pu8PrvKey, 21);
                    ...
                    if(u8SizeOfCustomData > 0)
                    {
                        ...
                        if (bEncExternalFlash)
                        {
                            ...
                            u32WriteOffset += (psCustomData->sOTACallBackMessage.u8ImageStartSector[u8ImageIndex] * sNvmDefsStruct.u32SectorSize);


                            for(u8LoopCount = 0; u8LoopCount < u8NoOf16BytesBlocks; u8LoopCount++)
                            {

                                FLib_MemCpy(au8TempBuffer, (pu8CustData+(16*u8LoopCount)), 16);
                            ...
                            FLib_MemCpy(au8TempBuffer, (pu8CustData+(16*u8LoopCount)), u8NoOf16BytesBlocks);
                            for(u8LoopCount = 0; u8LoopCount < u8NoOf16BytesBlocks; u8LoopCount++)
                                                ...
                                                uint8     u8ImageIndex)
{
                           ...
                           == E_ZCL_SUCCESS)
    {
        ...
        u32FlashOffset = psCustomData->sOTACallBackMessage.u8ImageStartSector[u8ImageIndex] * sNvmDefsStruct.u32SectorSize;
                                                ...
                                                tsOTA_WaitForData *sWaitForDataParams)
{
                           ...
                           == E_ZCL_SUCCESS)
    {
        ...
        FLib_MemCpy(&psCustomData->sOTACallBackMessage.sWaitForDataParams.sWaitForDataPyld,sWaitForDataParams, sizeof(tsOTA_WaitForData));
...
/****************************************************************************
 **
 ** NAME:       vOtaUpgManServerCheckClient
 **
 ** DESCRIPTION:
 ** Check if the server can process this client block request
 ** PARAMETERS:                           Name                           Usage
 ** uint16                             clientAddr                      network address of the client
 ** timeStamp                          timeStamp                       ZCL time stamp for the request
 **
 ** RETURN:
 ** bool_t
 ****************************************************************************/
PUBLIC bool_t vOtaUpgManServerCheckClient(uint16 clientAddr, uint32 timeStamp)
{
    uint8_t idxInfoTable;
    uint8_t firstExpiredEntry = OTA_MAX_SIMULTANEOUS_CLIENTS;

    for (idxInfoTable = 0; idxInfoTable < OTA_MAX_SIMULTANEOUS_CLIENTS; idxInfoTable++)
    {
        if (clientAddr == mOtaClientSessionInfoTable[idxInfoTable].remoteAddr)
        {
            mOtaClientSessionInfoTable[idxInfoTable].timeStamp = timeStamp;
            return TRUE;
        }

        if (((mOtaClientSessionInfoTable[idxInfoTable].timeStamp == 0) ||
                (mOtaClientSessionInfoTable[idxInfoTable].timeStamp + OTA_CLIENT_SESSION_EXPIRATION < timeStamp)) &&
                (firstExpiredEntry == OTA_MAX_SIMULTANEOUS_CLIENTS))
        {
            firstExpiredEntry = idxInfoTable;
        }
    }

    if (firstExpiredEntry < OTA_MAX_SIMULTANEOUS_CLIENTS)
    {
        mOtaClientSessionInfoTable[firstExpiredEntry].remoteAddr = clientAddr;
        mOtaClientSessionInfoTable[firstExpiredEntry].timeStamp = timeStamp;
        return TRUE;
    }
...
PUBLIC  tsOTA_ImageHeader sOtaGetHeader(uint8 *pu8HeaderBytes)
{
    ...
    FLib_MemSet(&sTempStruct, 0, sizeof(sTempStruct));
    ...
    else if((sTempStruct.u16HeaderControlField & 0x2 ))
    {
    ...
    else if(sTempStruct.u16HeaderControlField&0x4)
    {
                                              ...
                                              uint8    *pu8CAPublicKey)
{
    ...
    {
        FLib_MemCpy(psCustomData->sOTACallBackMessage.au8CAPublicKey, pu8CAPublicKey, 22);
...
PUBLIC  bool_t bOtaIsImageValid(uint8_t  *pu8Data)
{
    ...
    return TRUE;
        ...
        uint8_t au8Values[OTA_FLS_MAGIC_NUMBER_LENGTH] = {0x12,0x34,0x56,0x78,0x11,0x22,0x33,0x44,0x55,0x66,0x77,0x88}; // TODO fill the magic value
    #else
        uint8_t au8Values[OTA_FLS_MAGIC_NUMBER_LENGTH] = {0x78,0x56,0x34,0x12,0x44,0x33,0x22,0x11,0x88,0x77,0x66,0x55}; // TODO fill the magic value
    #endif
                                ...
                                uint8                     u8ImageLocation)
{
    ...
    FLib_MemSet(uBuf.au8, 0, AESSW_BLK_SIZE);
    FLib_MemSet(puHash->au8, 0, AESSW_BLK_SIZE);
    ...
    else
    {
        ...
        if(bIsServer)
        {
            ...
            FLib_MemCpy(au8Buffer, psCustomData->sOTACallBackMessage.au8ServerOTAHeader, u16HeaderSize);
#endif
        }
        else
        {
            ...
            FLib_MemCpy(au8Buffer, psCustomData->sOTACallBackMessage.sPersistedData.au8Header, u16HeaderSize);
#endif
        }
    }
    ...
    /* Copy Tag Id */
    FLib_MemCpy(au8Buffer+u16HeaderSize, au8Tag, OTA_TAG_HEADER_SIZE);

    if(bHeaderPresent)
    {
        u32FlashOffset = (psCustomData->sOTACallBackMessage.u8ImageStartSector[u8ImageLocation] *
                    sNvmDefsStruct.u32SectorSize) + u16HeaderSize + OTA_TAG_HEADER_SIZE;
    ...
    else
    {
        if(u32TagLength)
        {
            ...
            /* Copy  Magic Number */
            FLib_MemCpy(au8Buffer+u16HeaderSize+OTA_TAG_HEADER_SIZE, au8MagicFlashNumbers, OTA_FLS_MAGIC_NUMBER_LENGTH);

            u32FlashOffset = (psCustomData->sOTACallBackMessage.u8ImageStartSector[u8ImageLocation] *
                                    sNvmDefsStruct.u32SectorSize) + OTA_FLS_MAGIC_NUMBER_LENGTH;
    ...
    {
        FLib_MemCpy(uBuf.au8, (au8Buffer+(i*AESSW_BLK_SIZE)), AESSW_BLK_SIZE);
    ...
    FLib_MemCpy(uBuf.au8, (au8Buffer+(i*AESSW_BLK_SIZE)), iBytes);
                                  ...
                                  uint8                     u8UpgradeImageIndex)
{
    ...
    FLib_MemCpy(au8ReadBuffer1, pu8MacAddress, 8);
    ...
    FLib_MemCpy(au8ReadBuffer1, pu8LinkKey, 16);
    ...
    if(u8SizeOfCustomData > 0)
    {
        ...
        for(u8LoopCount = 0; u8LoopCount < u8NoOf16BytesBlocks; u8LoopCount++)
        {
            FLib_MemCpy(au8ReadBuffer1, (pu8CustData+(16*u8LoopCount)), 16);
        ...
        /* read remaining data */
        u8NoOf16BytesBlocks = u8SizeOfCustomData%16;
        FLib_MemCpy(au8ReadBuffer1, (pu8CustData+(16*u8LoopCount)), u8NoOf16BytesBlocks);
                                                ...
                                                tsOTA_ImageHeader *psOTAHeader)
{
    ...
    uint16 u16HeaderLength;
    uint32 u32HeaderType, u32FlashOffset;
...
PUBLIC void vOTA_EncodeString(tsReg128 *psKey, uint8 *pu8Iv,uint8 * pu8DataOut)
{
    tsReg128 sIv,sDataOut;
        ...
        FLib_MemCpy(&uTempData,pu8Iv,sizeof(AESSW_Block_u));
        vSwipeEndian(&uTempData,&sIv,TRUE);
    #else
        FLib_MemCpy(&sIv,pu8Iv,0x10);
    #endif

    FLib_MemCpy(&sDataOut,pu8DataOut,0x10);

    #ifndef LITTLE_ENDIAN_PROCESSOR
        FLib_MemCpy(pu8DataOut,&sDataOut,0x10);
    #else
        vSwipeEndian(&uTempData,&sDataOut,FALSE);
        FLib_MemCpy(pu8DataOut,&uTempData,sizeof(AESSW_Block_u));
    #endif    
}

#ifdef ZCLIP_ENABLED
#if defined(OTA_CLIENT)
PUBLIC void vOTAGetServerAddress(tsZCL_Address *sZCL_Address, tsOTA_Common *psOTA_Common)
{
    *sZCL_Address = psOTA_Common->sOTACallBackMessage.sPersistedData.serverAddress;
#if OTA_ACKS_ON == TRUE
...
/****************************************************************************
 **
 ** NAME:       vOTA_InitHeaderIfRequired
 **
 ** DESCRIPTION:
 ** This function is used to initialize OTA header with 
 **      default data if the header is invalid 
 ** PARAMETERS:
 ** RETURN:
 ** void
 ****************************************************************************/
uint16_t gCRC_Global = 0;
static void vOTA_InitHeaderIfRequired(void)
{
    tsOTA_ImageHeader sOtaHeader;
    uint8_t index = 0;
    ...
    if(sOtaHeader.u32FileIdentifier != OTA_FILE_IDENTIFIER)
    {
       
       /* header invalid */
        sOtaHeader.u32FileIdentifier = OTA_FILE_IDENTIFIER;
        sOtaHeader.u16HeaderVersion = OTA_FILE_HEADER_VERSION;
        sOtaHeader.u16HeaderControlField = 0x00;
        sOtaHeader.u16ImageType = OTA_IMAGE_TYPE_DEFAULT;
        sOtaHeader.u16ManufacturerCode = CLD_OTA_MANF_ID_VALUE;
        sOtaHeader.u32FileVersion =  OTA_FILE_VERSION_DEFAULT;
        sOtaHeader.u16StackVersion =  OTA_STACK_VERSION;
        sOtaHeader.u8SecurityCredVersion = 0x00;
        sOtaHeader.u16MinimumHwVersion = OTA_FILE_HW_VERSION_MIN;
        sOtaHeader.u16MaxHwVersion = OTA_FILE_HW_VERSION_MAX;
          
       /* set default header information */
        FLib_MemCpy(&au8OtaHeader[index], &sOtaHeader.u32FileIdentifier, sizeof(uint32_t));
        index+= sizeof(uint32_t);
        FLib_MemCpy(&au8OtaHeader[index], &sOtaHeader.u16HeaderVersion, sizeof(uint16_t));
        index+= sizeof(uint16_t);
        /* skip header lenght */
        index+= sizeof(uint16_t);
        FLib_MemCpy(&au8OtaHeader[index], &sOtaHeader.u16HeaderControlField, sizeof(uint16_t));
        index+= sizeof(uint16_t);       
        FLib_MemCpy(&au8OtaHeader[index], &sOtaHeader.u16ManufacturerCode, sizeof(uint16_t));
        index+= sizeof(uint16_t);           
        FLib_MemCpy(&au8OtaHeader[index], &sOtaHeader.u16ImageType, sizeof(uint16_t));
        index+= sizeof(uint16_t);         
        vReverseMemcpy(&au8OtaHeader[index], (uint8 *)&sOtaHeader.u32FileVersion, sizeof(uint32_t));
        index+= sizeof(uint32_t);       
        FLib_MemCpy(&au8OtaHeader[index], &sOtaHeader.u16StackVersion, sizeof(uint16_t));
        index+= sizeof(uint16_t);         
        /* skip header string, total image size */
        index+= OTA_HEADER_STRING_SIZE + sizeof(uint32_t);   
        au8OtaHeader[index] = sOtaHeader.u8SecurityCredVersion;
        /* skip upgrade File destination */
        index+= sizeof(uint64);   
        FLib_MemCpy(&au8OtaHeader[index], &sOtaHeader.u16MinimumHwVersion, sizeof(uint16_t));
        index+= sizeof(uint16_t);      
        FLib_MemCpy(&au8OtaHeader[index], &sOtaHeader.u16MaxHwVersion, sizeof(uint16_t));
        index+= sizeof(uint16_t);       
    }    
}
#endif
...
PRIVATE void vOTA_BuildFileUri(char * uri, uint32 maxLength, uint16 u16ManufacturerCode, uint16 u16ImageType, uint32 u32FileVersion)
{
    FLib_MemSet((void *) uri, 0, maxLength + 1);
    snprintf(uri, maxLength + 1, "%.*s/%.4x-%.4x-%.8x", CLD_OTA_FILE_URI_ROOT_LENGTH, CLD_OTA_FILE_URI_ROOT, u16ManufacturerCode, u16ImageType, u32FileVersion);
}
#endif
#endif
