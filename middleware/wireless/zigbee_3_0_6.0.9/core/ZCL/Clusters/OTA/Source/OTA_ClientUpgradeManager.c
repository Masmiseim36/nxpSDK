...
/*!
\file       OTA_ClientUpgradeManager.c
\brief      Over The Air Upgrade client manager
*/
...
#ifdef OTA_CLIENT
#ifdef ZCLIP_ENABLED
PRIVATE void vOTAStoreFileUri(tsOTA_Common *psOTA_Common, tsZCL_CharacterString *psFileUri);
#endif
...
#ifdef OTA_CLIENT
PRIVATE bool_t bOTA_ProcessOtherTagThanUpgradeImage(       
                                 tsOTA_Common                *psOTA_Common, 
                                 uint32_t                    u32Overflow, 
                                 uint8_t                     *pu8DataSize, 
                                 uint16_t                    *pu16TagId,
                                 uint32_t                    *pu32TagLenth
        );
PRIVATE bool_t bOTA_ReceiveNewTag(       
                                 tsOTA_Common *psOTA_Common, 
                                 uint8_t u8IndxData,
                                 uint16_t *pu16TagId,
                                 uint32_t *pu32TagLenth
...
PRIVATE void vOTA_UpdateSignature(
                                  tsOTA_Common *psOTA_Common, 
                                  uint8_t *pData,
                                  uint16_t u16LenData
        );
...
uint32_t gCrcValue = 0;
uint32_t gCrcByteProcessed = 0;
...
PRIVATE void vOTAStoreFileUri(tsOTA_Common *psOTA_Common, tsZCL_CharacterString *psFileUri)
{
    if (psOTA_Common->sOTACallBackMessage.sPersistedData.pFileUri != NULL) MEM_BufferFree(psOTA_Common->sOTACallBackMessage.sPersistedData.pFileUri);
    psOTA_Common->sOTACallBackMessage.sPersistedData.pFileUri = (char *) MEM_BufferAlloc(psFileUri->u8Length + 1);
    FLib_MemCpy((void *) psOTA_Common->sOTACallBackMessage.sPersistedData.pFileUri, (const void *) psFileUri->pu8Data, (uint32_t) psFileUri->u8Length);
    *(psOTA_Common->sOTACallBackMessage.sPersistedData.pFileUri+psFileUri->u8Length) = '\0';
}
#endif
                                    ...
                                    tsZCL_EndPointDefinition    *psEndPointDefinition)
{
    ...
    uint8 u8DstEndpoint,u8SrcEndpoint, u8NextImageLocation;

    bool_t bProceed = FALSE, bPollRequired = FALSE;
    tsOTA_Common *psOTA_Common = (tsOTA_Common *)pvStruct;
    tsOTA_ImageHeader sOTAHeader = sOtaGetHeader(&au8OtaHeader[0]);
...
#else
    vOTAGetServerAddress(&sZCL_Address, psOTA_Common);
#endif
    ...
    switch(psOTA_Common->sOTACallBackMessage.eEventId)
    {
        ...
        {
            vOtaHandleTimedBlockRequest(psOTA_Common, &bProceed,&sOTAHeader,psEndPointDefinition);
        ...
        case E_CLD_OTA_COMMAND_IMAGE_NOTIFY:
        {
            ...
            uint8 u8LoopCount;
            bool bSendReqForAllImages = FALSE;
                    ...
                    else
                    {
                        ...
                        for(u8LoopCount = 0; u8LoopCount < OTA_TOTAL_ACTIVE_IMAGES; u8LoopCount++)
                        {
                            ...
                            if(sReceivedFrame.u16ImageType == sOTAHeader.u16ImageType)
                            {
                                ...
                                bool bIsUnicast = OTA_IS_UNICAST(psOTA_Common->sReceiveEventAddress.u8DstAddrMode,psOTA_Common->sReceiveEventAddress.uDstAddress.u16Addr);
...
#endif
                                    eOTA_ClientQueryNextImageRequest(u8SrcEndpoint,u8DstEndpoint,&sZCL_Address,&sQueryImage);
                                    bPollRequired = TRUE;
                                }
                                break;
                            }
        ...
        case E_CLD_OTA_COMMAND_QUERY_NEXT_IMAGE_RESPONSE:
        {
            ...
            uint32 u32MaxImage = sNvmDefsStruct.u32SectorSize * psOTA_Common->sOTACallBackMessage.u8MaxNumberOfSectors;
...
#ifdef ZCLIP_ENABLED
            sImageResponse.u16ManufacturerCode = psOTA_Common->sOTACallBackMessage.sPersistedData.u16ManufacturerCode;
            sImageResponse.u16ImageType = psOTA_Common->sOTACallBackMessage.sPersistedData.u16ImageType;
#endif
            ...
            {
#ifdef ZCLIP_ENABLED
                vOTAStoreFileUri(psOTA_Common, &sImageResponse.sFileUri);
#endif
                ...
                for(u8LoopCount = 0; u8LoopCount < OTA_TOTAL_ACTIVE_IMAGES; u8LoopCount++)
                {
                            ...
                            sImageResponse.u16ImageType == sOTAHeader.u16ImageType)
                    {
                        ...
                        {
                            if((u32MaxImage >= sImageResponse.u32ImageSize)||(u8LoopCount != 0) || (true)) /* TO BE MODIFIED */
                            {
                                ...
                                vOtaRequestNextBlock(u8SrcEndpoint,u8DstEndpoint,&sBlock, psEndPointDefinition,psOTA_Common, &sZCL_Address);
                                psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u32FileOffset = 0;
#if OTA_CLIENT_SIGNATURE == OTA_SIGNATURE_CRC    
                                psOTA_Common->sOTACallBackMessage.sPersistedData.u32CrcValue = 0x00;
                                psOTA_Common->sOTACallBackMessage.sPersistedData.u32CrcByteProcessed = 0x00;
#endif                             
        ...
        case E_CLD_OTA_COMMAND_BLOCK_RESPONSE:
        {
               ...
               )
            {
                ...
                vOtaFlashLockWrite(psEndPointDefinition,
                                   psOTA_Common,
                                   ((sBlockResponse.uMessage.sBlockPayloadSuccess.u32FileOffset +
                                     sBlockResponse.uMessage.sBlockPayloadSuccess.u8DataSize) > OTA_MIN_HEADER_SIZE)?1:0,
                                   (uint16)sBlockResponse.uMessage.sBlockPayloadSuccess.u8DataSize,
                                   sBlockResponse.uMessage.sBlockPayloadSuccess.pu8Data);
#endif
                    ...
                    sBlockResponse.uMessage.sBlockPayloadSuccess.u8DataSize) > OTA_MIN_HEADER_SIZE)
                {
                    ...
                    if(sBlockResponse.uMessage.sBlockPayloadSuccess.u32FileOffset <= OTA_MIN_HEADER_SIZE)
                    {
                        ...
                        u32MinOverFlow = sBlockResponse.uMessage.sBlockPayloadSuccess.u8DataSize -
                                    u32MinOverFlow;
                        FLib_MemCpy(&psOTA_Common->sOTACallBackMessage.sPersistedData.au8Header[sBlockResponse.uMessage.sBlockPayloadSuccess.u32FileOffset],
                               &sBlockResponse.uMessage.sBlockPayloadSuccess.pu8Data[0], u32MinOverFlow);
                            ...
                            sBlockResponse.uMessage.sBlockPayloadSuccess.u8DataSize) > u16FieldLength)
                        {
                            ...
                            if(sBlockResponse.uMessage.sBlockPayloadSuccess.u32FileOffset <= u16FieldLength)
                            {
                                ...
                                u32FileOverflow = sBlockResponse.uMessage.sBlockPayloadSuccess.u8DataSize -
                                                u32FileOverflow ;
                                FLib_MemCpy(&psOTA_Common->sOTACallBackMessage.sPersistedData.au8Header[sBlockResponse.uMessage.sBlockPayloadSuccess.u32FileOffset], 
                                       &sBlockResponse.uMessage.sBlockPayloadSuccess.pu8Data[0], u32FileOverflow);
    
                                /* update signature - OTA header completly received  */
                                vOTA_UpdateSignature(psOTA_Common, &psOTA_Common->sOTACallBackMessage.sPersistedData.au8Header[0], u16FieldLength);
                                 ...
                                 sBlockResponse.uMessage.sBlockPayloadSuccess.u8DataSize) > u32TempChkLength)
                            {
                                ...
                                if(sBlockResponse.uMessage.sBlockPayloadSuccess.u32FileOffset <= (uint32)(u16FieldLength+OTA_TAG_HEADER_SIZE))
                                {
                                    ...
                                    {
                                        psOTA_Common->sOTACallBackMessage.sPersistedData.u8PassiveTag[(psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u32FileOffset - u16FieldLength) + i] =
                                            sBlockResponse.uMessage.sBlockPayloadSuccess.pu8Data[u32FileOverflow+i];

                                    }
                                    
                                    /* update signature - image tag information   */
                                    vOTA_UpdateSignature(psOTA_Common, &sBlockResponse.uMessage.sBlockPayloadSuccess.pu8Data[u32FileOverflow], u32TagIdOverflow);
                                ...
                                psOTA_Common->sOTACallBackMessage.uMessage.sImageIdentification.pu8Current = sOTAHeader.stHeaderString;
                                psOTA_Common->sOTACallBackMessage.uMessage.sImageIdentification.puNotified = &(psOTA_Common->sOTACallBackMessage.sPersistedData.au8Header[20]);
                                
                                for (i=0;i<OTA_HEADER_STRING_SIZE;i++)
                                {
                                ...
                                //eOtaSetCallBackMessage(psOTA_Common, E_CLD_OTA_INTERNAL_COMMAND_VERIFY_STRING);
                                //psEndPointDefinition->pCallBackFunctions(&psOTA_Common->sOTACustomCallBackEvent);
                                ...
                                if(psOTA_Common->sOTACallBackMessage.uMessage.sImageIdentification.eStatus != E_ZCL_SUCCESS)
                                {
                                    ...
                                    psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u16ImageType = sBlockResponse.uMessage.sBlockPayloadSuccess.u16ImageType;
                                    psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u16ManfId = sBlockResponse.uMessage.sBlockPayloadSuccess.u16ManufacturerCode;
                                    vOtaAbortDownload(psOTA_Common, psEndPointDefinition);
                                    return;
                                }
                                /*String compare Done !!*/
#endif                                
                                FLib_MemCpy(psOTA_Common->sOTACallBackMessage.sPersistedData.u8ActiveTag,
                                ...
                                if(u16TagId == OTA_TAG_ID_UPGRADE_IMAGE)
                                {
                                    ...
                                    uint32 u32StartLocation = psOTA_Common->sOTACallBackMessage.u8ImageStartSector[u8NextImageLocation]
                                                    * sNvmDefsStruct.u32SectorSize + gBootData_Image_Offset_c; /* Add gBootData_Image_Offset_c because image is not stored directly at the beggining in external flash */
                                    ...
                                    for(i=0; i< psOTA_Common->sOTACallBackMessage.u8MaxNumberOfSectors; i++)
                                    {
                                        ...
                                        vOtaFlashLockErase(psEndPointDefinition, psOTA_Common, (uint8)(psOTA_Common->sOTACallBackMessage.u8ImageStartSector[u8NextImageLocation]+i));
                                    }

                                    if( !psOTA_Common->sOTACallBackMessage.sPersistedData.bIsNullImage)
                                    {  /* if image is for own device copy from  OTA_FLS_MAGIC_NUMBER_LENGTH onwards */
                                        if(u32DataOverflow > OTA_FLS_MAGIC_NUMBER_LENGTH)
                                        {
#ifndef OTA_INTERNAL_STORAGE
                                            u32WriteValue = u32DataOverflow - OTA_FLS_MAGIC_NUMBER_LENGTH;                                           
                                            if(OTA_PushImageChunk((sBlockResponse.uMessage.sBlockPayloadSuccess.pu8Data + (u32TagIdOverflow + u32FileOverflow)), 
                                                       (uint16)u32WriteValue,
                                                       NULL, 
                                                       NULL))
                                            {
                                                /* flash error */
                                                DBG_vPrintf(TRACE_INT_FLASH, "WRITE1 -> Error writting data \n");
                                                return;
                                            }
                                    
                                            /* update signature - image data   */
                                            vOTA_UpdateSignature(psOTA_Common, (sBlockResponse.uMessage.sBlockPayloadSuccess.pu8Data + (u32TagIdOverflow + u32FileOverflow)), u32WriteValue);                                            
                                            
#else                                       
                                            
                                            // internal
                                            uint32 u32Temp;
                                            u32WriteValue = u32DataOverflow - OTA_FLS_MAGIC_NUMBER_LENGTH - 4;
                                            ...
                                            vOtaProcessInternalEncryption(
                                                    sBlockResponse.uMessage.sBlockPayloadSuccess.pu8Data + (u32TagIdOverflow+u32FileOverflow+OTA_FLS_MAGIC_NUMBER_LENGTH+4 ),
                                                         u32StartLocation + OTA_FLS_MAGIC_NUMBER_LENGTH + 4,
                                                         u32WriteValue,
                                                         u32StartLocation );
...
#else
                                        psOTA_Common->sOTACallBackMessage.sPersistedData.u32CurrentFlashOffset = u32StartLocation + u32WriteValue + OTA_FLS_MAGIC_NUMBER_LENGTH + 4;
                                        psOTA_Common->sOTACallBackMessage.sPersistedData.u32TagDataWritten = u32DataOverflow;
                                        ...
                                        psOTA_Common->sOTACallBackMessage.sPersistedData.u8Buf[0] = *(sBlockResponse.uMessage.sBlockPayloadSuccess.pu8Data + (u32TagIdOverflow+u32FileOverflow+OTA_FLS_MAGIC_NUMBER_LENGTH)  );
                                        psOTA_Common->sOTACallBackMessage.sPersistedData.u8Buf[1] = *(sBlockResponse.uMessage.sBlockPayloadSuccess.pu8Data + (u32TagIdOverflow+u32FileOverflow+OTA_FLS_MAGIC_NUMBER_LENGTH + 1 ) );
                                        psOTA_Common->sOTACallBackMessage.sPersistedData.u8Buf[2] = *(sBlockResponse.uMessage.sBlockPayloadSuccess.pu8Data + (u32TagIdOverflow+u32FileOverflow+OTA_FLS_MAGIC_NUMBER_LENGTH + 2) );
                                        psOTA_Common->sOTACallBackMessage.sPersistedData.u8Buf[3] = *(sBlockResponse.uMessage.sBlockPayloadSuccess.pu8Data + (u32TagIdOverflow+u32FileOverflow+OTA_FLS_MAGIC_NUMBER_LENGTH + 3) );
#endif
                                    }
                                    else
                                    {
                                    ...
                                    FLib_MemCpy((uint8*)&psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u16DownloadedStackVersion,
                                            &psOTA_Common->sOTACallBackMessage.sPersistedData.au8Header[19],
                                            sizeof(uint16));
                ...
                else
                {
                    ...
                    FLib_MemCpy(&psOTA_Common->sOTACallBackMessage.sPersistedData.au8Header[psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u32FileOffset],
                           &sBlockResponse.uMessage.sBlockPayloadSuccess.pu8Data[0], sBlockResponse.uMessage.sBlockPayloadSuccess.u8DataSize);
...
// Download Complete
        ...
        case E_CLD_OTA_COMMAND_QUERY_SPECIFIC_FILE_RESPONSE:
        {
...
#ifdef ZCLIP_ENABLED
            sQSpFileResponse.u16ManufacturerCode = psOTA_Common->sOTACallBackMessage.sPersistedData.u16ManufacturerCode;
            sQSpFileResponse.u16ImageType = psOTA_Common->sOTACallBackMessage.sPersistedData.u16ImageType;
#endif
            ...
            {
#ifdef ZCLIP_ENABLED
                vOTAStoreFileUri(psOTA_Common, &sQSpFileResponse.sFileUri);
#endif
                ...
                vOtaRequestNextBlock(u8SrcEndpoint,u8DstEndpoint,&sBlockSp, psEndPointDefinition,psOTA_Common, &sZCL_Address);
                psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u32FileOffset = 0;
#if OTA_CLIENT_SIGNATURE == OTA_SIGNATURE_CRC    
                psOTA_Common->sOTACallBackMessage.sPersistedData.u32CrcValue = 0x00;
                psOTA_Common->sOTACallBackMessage.sPersistedData.u32CrcByteProcessed = 0x00;
        ...
        case E_CLD_OTA_INTERNAL_COMMAND_RCVD_DEFAULT_RESPONSE:
            ...
            break;
        default:
            psOTA_Common->sOTACallBackMessage.sPersistedData.u32RequestBlockRequestTime = 0; // To stop timer events sent to state machine from previous states.
            break;
                                           ...
                                           tsZCL_EndPointDefinition    *psEndPointDefinition)
{
    ...
    bool_t bPollRequired = FALSE, bRequestMore = TRUE ,bPKTProcessed = FALSE;
    tsZCL_Address sZCL_Address;
    tsOTA_Common *psOTA_Common = (tsOTA_Common *)pvStruct;
    ...
    switch(psOTA_Common->sOTACallBackMessage.eEventId)
    {
        ...
        case E_CLD_OTA_COMMAND_BLOCK_RESPONSE:
        {
                ...
                ( !psOTA_Common->sOTACallBackMessage.sPersistedData.bIsSpecificFile) )
            {
...
#endif
                {
                    ...
                    vOtaFlashLockWrite(psEndPointDefinition,
                                       psOTA_Common,
                                       1,
                                       (uint16)sResponse.uMessage.sBlockPayloadSuccess.u8DataSize,
                                       sResponse.uMessage.sBlockPayloadSuccess.pu8Data);
#endif

                    if ( ( sResponse.uMessage.sBlockPayloadSuccess.u32FileOffset +
                                    sResponse.uMessage.sBlockPayloadSuccess.u8DataSize > u32TotalSize) ||
                         ( sResponse.uMessage.sBlockPayloadSuccess.u8DataSize > OTA_MAX_BLOCK_SIZE) )
                    {
                    ...
                    else if ( sResponse.uMessage.sBlockPayloadSuccess.u32FileOffset ==
                             psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u32FileOffset )
                    {
                        /* rcvd file offset equal persisted file offset */
                        uint16 u16TagId;
                        uint32 u32TagLength;
                        uint8 u8DataSize = sResponse.uMessage.sBlockPayloadSuccess.u8DataSize;
                        ...
                        {
                            /* we expect at least the image and got less hence can't allow this*/
                            vOtaAbortDownload(psOTA_Common, psEndPointDefinition);
                            return;
                        }
                        if ( (psOTA_Common->sOTACallBackMessage.sPersistedData.u32TagDataWritten + u8DataSize) <= u32TagLength )
                        {
                            ...
                            if ( u16TagId == OTA_TAG_ID_UPGRADE_IMAGE )
                            {
                                ...
                                uint32 u32ValueToWrite, u32ValueToIgnore;
                                ...
                                if( psOTA_Common->sOTACallBackMessage.sPersistedData.u32TagDataWritten > OTA_FLS_MAGIC_NUMBER_LENGTH)
                                {
...
#ifdef INTERNAL_ENCRYPTED
                                    vOtaProcessInternalEncryption( sResponse.uMessage.sBlockPayloadSuccess.pu8Data,
                                            psOTA_Common->sOTACallBackMessage.sPersistedData.u32CurrentFlashOffset,
                                            (uint32)u8DataSize,
                                            psOTA_Common->sOTACallBackMessage.u8ImageStartSector[psOTA_Common->sOTACallBackMessage.u8NextFreeImageLocation]
                                    ...
                                    if(OTA_PushImageChunk(sResponse.uMessage.sBlockPayloadSuccess.pu8Data, 
                                                       (uint16)u8DataSize, 
                                                       NULL, 
                                                       NULL))
                                    {
                                        /* flash error */
                                        DBG_vPrintf(TRACE_INT_FLASH, "WRITE5 -> Error writting data \n");
                                        return;
                                    }
                                    
                                    /* update signature - - image data   */
                                    vOTA_UpdateSignature(psOTA_Common, sResponse.uMessage.sBlockPayloadSuccess.pu8Data,u8DataSize);                                       
                                ...
                                else
                                {
                                                ...
                                                > OTA_FLS_MAGIC_NUMBER_LENGTH)
                                    {
                                        ...
                                        psOTA_Common->sOTACallBackMessage.sPersistedData.u32CurrentFlashOffset += u32ValueToIgnore;

                                         DBG_vPrintf(TRACE_INT_FLASH, "WRITE4 -> persit Loc %08x, u32ValueToWrite %d \n",
                                                            psOTA_Common->sOTACallBackMessage.sPersistedData.u32CurrentFlashOffset,
                                                            u32ValueToWrite);
                                        if(OTA_PushImageChunk(sResponse.uMessage.sBlockPayloadSuccess.pu8Data+u32ValueToIgnore, 
                                        ...
                                        /* update signature - image data */
                                        vOTA_UpdateSignature(psOTA_Common, sResponse.uMessage.sBlockPayloadSuccess.pu8Data+u32ValueToIgnore, u32ValueToWrite);                                            
                            ...
                            else if (!bOTA_ProcessOtherTagThanUpgradeImage( psOTA_Common, 0, &u8DataSize, &u16TagId, &u32TagLength))
                            {
                                /* u16TagId != OTA_TAG_ID_UPGRADE_IMAGE, OTA_TAG_ID_BITMAP_ELEMENT, OTA_TAG_ID_CRC_ELEMENT */
                                /* should never happen */
                                vOtaAbortDownload(psOTA_Common, psEndPointDefinition);
                                return;
                            }
                        ...
                        else
                        {
                            ...
                            if(u16TagId == OTA_TAG_ID_UPGRADE_IMAGE)
                            {
                                ...
                                if (u32CurrOverflow > 0 )
                                {
                                    ...
                                    /* update signature - image data */
                                    vOTA_UpdateSignature(psOTA_Common, sResponse.uMessage.sBlockPayloadSuccess.pu8Data,u32CurrOverflow);                                            
                                ...
                                psOTA_Common->sOTACallBackMessage.uMessage.sBlockResponseEvent.pu8Data = sResponse.uMessage.sBlockPayloadSuccess.pu8Data+u32CurrOverflow;
                               
                                if((psOTA_Common->sOTACallBackMessage.sPersistedData.u32TagDataWritten == u32TagLength) &&
                                  (u8DataSize > OTA_TAG_HEADER_SIZE))
                                {
                                    /* move to the next tag */
                                    if (!bOTA_ReceiveNewTag(psOTA_Common, 0, &u16TagId, &u32TagLength))
                                    {
                                        DBG_vPrintf(TRACE_VERIF, "Send Upgrade End Response - new tag error \n");
                                        vOtaAbortDownload(psOTA_Common, psEndPointDefinition);
                                        return;
                                    }
                                }
                            ...
                            if(!bOTA_ProcessOtherTagThanUpgradeImage(psOTA_Common, u32CurrOverflow, &u8DataSize, &u16TagId, &u32TagLength))
                            {
                                /* send event to application */
                                eOtaSetEventTypeAndGiveCallBack(psOTA_Common, E_CLD_OTA_BLOCK_RESPONSE_TAG_OTHER_THAN_UPGRADE_IMAGE_BITMAP_CRC,psEndPointDefinition); 
                            }
                           ...
                           for (i=0;i<u8DataSize; i++)
                               DBG_vPrintf(TRACE_OTA_DEBUG,"%x", sResponse.uMessage.sBlockPayloadSuccess.pu8Data[i]);
                            DBG_vPrintf(TRACE_OTA_DEBUG,"\n");
#endif
                      ...
                      (psOTA_Common->sOTACallBackMessage.sPersistedData.bIsCoProcessorImage) )
            {
                ...
                {
                    if((sResponse.uMessage.sBlockPayloadSuccess.u32FileOffset +
                            sResponse.uMessage.sBlockPayloadSuccess.u8DataSize > psOTA_Common->sOTACallBackMessage.sPersistedData.u32CoProcessorImageSize) ||
                        (sResponse.uMessage.sBlockPayloadSuccess.u8DataSize > OTA_MAX_BLOCK_SIZE))
                    {
                        vOtaAbortDownload(psOTA_Common, psEndPointDefinition);
                        ...
                        psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u32FileOffset)
                    {
                        ...
                        psOTA_Common->sOTACallBackMessage.uMessage.sImageBlockResponsePayload.uMessage.sBlockPayloadSuccess = sResponse.uMessage.sBlockPayloadSuccess;
                        psOTA_Common->sOTACallBackMessage.uMessage.sImageBlockResponsePayload.u8Status = OTA_STATUS_SUCCESS;
                        psEndPointDefinition->pCallBackFunctions(&psOTA_Common->sOTACustomCallBackEvent);
                        psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u32FileOffset += sResponse.uMessage.sBlockPayloadSuccess.u8DataSize;
                        DBG_vPrintf(TRACE_INT_FLASH, "OFFSET11 -> %08x, added %d\n",
                                psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u32FileOffset,
                                                               sResponse.uMessage.sBlockPayloadSuccess.u8DataSize);
                      ...
                      (psOTA_Common->sOTACallBackMessage.sPersistedData.bIsSpecificFile) )
            {
                ...
                {
                    if((sResponse.uMessage.sBlockPayloadSuccess.u32FileOffset+
                        sResponse.uMessage.sBlockPayloadSuccess.u8DataSize > psOTA_Common->sOTACallBackMessage.sPersistedData.u32SpecificFileSize) ||
                        (sResponse.uMessage.sBlockPayloadSuccess.u8DataSize > OTA_MAX_BLOCK_SIZE))
                    {
                        vOtaAbortDownload(psOTA_Common, psEndPointDefinition);
    ...
    if(bPollRequired)
    {
        if(psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u32FileOffset > psOTA_Common->sOTACallBackMessage.sPersistedData.u32Step)
        {
            ...
            eOtaSetEventTypeAndGiveCallBack(psOTA_Common, E_CLD_OTA_INTERNAL_COMMAND_SAVE_CONTEXT,psEndPointDefinition);
            psOTA_Common->sOTACallBackMessage.sPersistedData.u32Step = ((psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u32FileOffset+1023) > psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u32FileOffset)?
                    psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u32FileOffset+1024 : (0xFFFFFFFF - psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u32FileOffset -1);
        }
        /* request has been sent make sure we poll for the response too otherwise might miss it
            only valid for end device*/
        eOtaSetEventTypeAndGiveCallBack(psOTA_Common, E_CLD_OTA_INTERNAL_COMMAND_POLL_REQUIRED,psEndPointDefinition);
...
{
    tsOTA_Common *psOTA_Common = (tsOTA_Common *)pvStruct;
    bool bPollRequired = FALSE, bCountDown =  FALSE, bWaitToUpgrade = FALSE;

    tsOTA_ImageHeader sOTAHeader;
    tsOTA_ImageHeader sOTACurrentHeader = sOtaGetHeader(au8OtaHeader);
...
#else
    vOTAGetServerAddress(&sZCLAddress, psOTA_Common);
#endif
                                                    ...
                                                    tsOTA_ImageBlockResponsePayload *psBlockResponse)
{
    ...
    if(psBlockResponse->uMessage.sWaitForData.u32CurrentTime == psBlockResponse->uMessage.sWaitForData.u32RequestTime)
    {
        ...
        {
            if(psBlockResponse->uMessage.sWaitForData.u16BlockRequestDelayMs <= OTA_MIN_TIMER_MS_RESOLUTION)
            {
                psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u16MinBlockRequestDelay = OTA_MIN_TIMER_MS_RESOLUTION;
            }
            else
            {
    ...
    {
        psOTA_Common->sOTACallBackMessage.sPersistedData.u32RequestBlockRequestTime = u32ZCL_GetUTCTime() + (psBlockResponse->uMessage.sWaitForData.u32RequestTime - psBlockResponse->uMessage.sWaitForData.u32CurrentTime);
    }
    psOTA_Common->sOTACallBackMessage.sPersistedData.u8Retry = 0;
}
         ...
         uint32                      u32ImageStartAddress)
{
    ...
    DBG_vPrintf(TRACE_INT_DECRYPT, "Flash Addr %08x u32EncryptionStart %08x\n",
            u32StartAddress, u32EncryptionStart);
    ...
    if (u32EncryptedToWrite > 0)
    {
        ...
        #else
            FLib_MemCpy((uint8*)&u32IVCount,&au8Iv[12],sizeof(uint32));
        #endif
...
/****************************************************************************
 **
 ** NAME:       bOTA_ProcessOtherTagThanUpgradeImage
 **
 ** DESCRIPTION:
 ** Process other tag than upgrade image
 ** PARAMETERS:                           Name                           Usage
 ** tsOTA_Common                       *psOTA_Common                   OTA custom data
 ** uint32_t                            u32Overflow                    overflow data size
 ** uint8_t                             pu8DataSize                    pointer to data size
 ** uint8_t                             pu16TagId                      pointer to tagId
 ** pu32TagLenth                        pu32TagLenth                   pointer to tag length
 ** RETURN:
 ** bool_t
 ****************************************************************************/
PRIVATE bool_t bOTA_ProcessOtherTagThanUpgradeImage
...
)
{
    ...
    uint8_t u8IndxData = 0;
    uint8_t tagDataWritten = 0;
    uint8_t u8TempDataSize = *pu8DataSize;
   
    while(u8TempDataSize)
    {
        tagDataWritten = (uint8_t)psOTA_Common->sOTACallBackMessage.sPersistedData.u32TagDataWritten;
        
        if((tagDataWritten == 0x00) &&
           (u8TempDataSize < OTA_TAG_HEADER_SIZE))
        {
            /* data size is not enough to get tag information */
            *pu8DataSize  -= u8TempDataSize;
            /* wait for a new block response */
            return TRUE;
        }
        
        if(!u32Overflow)
        {
            u8TempDataSize = (tagDataWritten == 0) ? (u8TempDataSize - OTA_TAG_HEADER_SIZE):u8TempDataSize;
            u32Overflow = (u8TempDataSize > *pu32TagLenth) ? (*pu32TagLenth): u8TempDataSize;
        }

        if(tagDataWritten == 0x00) 
        {
            u8IndxData += OTA_TAG_HEADER_SIZE;
        }
        
        if ( *pu16TagId == OTA_TAG_ID_BITMAP_ELEMENT )
        {  
            /* get bitmap information */
            FLib_MemCpy(&psOTA_Common->sOTACallBackMessage.sPersistedData.au8Bitmap[tagDataWritten], 
                   &psOTA_Common->sOTACallBackMessage.uMessage.sBlockResponseEvent.pu8Data[u8IndxData], u32Overflow);
            
            /* update signature - bitmap information */
            vOTA_UpdateSignature(psOTA_Common, &psOTA_Common->sOTACallBackMessage.uMessage.sBlockResponseEvent.pu8Data[u8IndxData], u32Overflow);     
            bStatus = TRUE;
            
        }
        else if ( *pu16TagId == OTA_TAG_ID_CRC_ELEMENT )
        {
            bStatus = TRUE;  
            
            if(u32Overflow <= sizeof(uint32_t))
            {
                FLib_MemCpy(&psOTA_Common->sOTACallBackMessage.sPersistedData.au8CrcValueReceived[tagDataWritten], 
                       &psOTA_Common->sOTACallBackMessage.uMessage.sBlockResponseEvent.pu8Data[u8IndxData], u32Overflow);
            }
            else
            {
                DBG_vPrintf(TRACE_VERIF, "OTA_TAG_ID_CRC_ELEMENT - wrong CRC size \n");
                break;
            }
        }
        
        /* update parameters */
        psOTA_Common->sOTACallBackMessage.sPersistedData.u32TagDataWritten += u32Overflow;
        u8TempDataSize -= (uint8_t)u32Overflow;
        u8IndxData += u32Overflow;                   
        u32Overflow = 0;
        
        
        if((psOTA_Common->sOTACallBackMessage.sPersistedData.u32TagDataWritten == *pu32TagLenth) &&
            (u8TempDataSize > OTA_TAG_HEADER_SIZE))
        {
            /* move to the next tag */
            if(!bOTA_ReceiveNewTag(psOTA_Common, u8IndxData, pu16TagId, pu32TagLenth))
            {
                DBG_vPrintf(TRACE_VERIF, "new tag error \n");
                break;
            }
            u32Overflow = 0;
        }          
...
)         
{
    ...
    FLib_MemCpy(psOTA_Common->sOTACallBackMessage.sPersistedData.u8ActiveTag,
           &psOTA_Common->sOTACallBackMessage.uMessage.sBlockResponseEvent.pu8Data[u8IndxData], OTA_TAG_HEADER_SIZE);
                                    
    /* update tag variables */
    vOTA_GetTagIdandLengh( pu16TagId, pu32TagLenth,
                          &psOTA_Common->sOTACallBackMessage.sPersistedData.u8ActiveTag[0]);                                    
                                    
    if(*pu16TagId != OTA_TAG_ID_CRC_ELEMENT)
    {
        /* update signature - tag information */
        vOTA_UpdateSignature(psOTA_Common, &psOTA_Common->sOTACallBackMessage.uMessage.sBlockResponseEvent.pu8Data[u8IndxData], OTA_TAG_HEADER_SIZE);
    }
    ...
    /* validate size */
    vReverseMemcpy((uint8*)&u32TotalSize,&psOTA_Common->sOTACallBackMessage.sPersistedData.au8Header[52],sizeof(uint32));
    if((*pu32TagLenth + psOTA_Common->sOTACallBackMessage.sPersistedData.sAttributes.u32FileOffset + 
         OTA_TAG_HEADER_SIZE) > u32TotalSize)
    {
         DBG_vPrintf(TRACE_VERIF, "New tag exceed total size \n");
         bStatus = FALSE;
    }  
    return bStatus;
}
...
/****************************************************************************
 **
 ** NAME:       vOTA_UpdateSignature
 **
 ** DESCRIPTION:
 ** This function is used to update signature based on incomming data
 ** PARAMETERS:                           Name                           Usage
 ** tsOTA_Common                       *psOTA_Common                   OTA custom data
 ** uint8_t                             pu16TagId                      pointer to tagId
 ** pu32TagLenth                        pu32TagLenth                   pointer to tag length
...
{       
#if OTA_CLIENT_SIGNATURE == OTA_SIGNATURE_CRC
    psOTA_Common->sOTACallBackMessage.sPersistedData.u32CrcValue = OTA_CrcCompute(pData, u16LenData, 
                                                                                  psOTA_Common->sOTACallBackMessage.sPersistedData.u32CrcValue);
    psOTA_Common->sOTACallBackMessage.sPersistedData.u32CrcByteProcessed += u16LenData;
#endif
}
#endif /* OTA_CLIENT */
