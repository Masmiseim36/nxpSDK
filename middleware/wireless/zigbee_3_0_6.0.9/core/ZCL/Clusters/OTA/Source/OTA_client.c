...
/*!
\file       OTA_client.c
\brief      Over The Air Upgrade client
*/
...
/****************************************************************************/
#ifdef OTA_CLIENT 
PRIVATE  bool_t bOTA_ValidateSignature( tsOTA_Common *psOTA_Common);
#endif
/****************************************************************************/
/***        Exported Variables                                            ***/
/****************************************************************************/
...
//    tsZCL_TxPayloadItem asActPayloadDefinition[5] = {0};
...
#ifdef ZCLIP_ENABLED
            u8SequenceNumber = psCustomData->sOTACustomCallBackEvent.u8TransactionSequenceNumber;
#endif
    ...
    {
#ifdef ZCLIP_ENABLED
        psCustomData->sOTACallBackMessage.sPersistedData.u16ManufacturerCode = psQueryImageRequest->u16ManufacturerCode;
        psCustomData->sOTACallBackMessage.sPersistedData.u16ImageType = psQueryImageRequest->u16ImageType;
#endif
                    ...
                    tsOTA_QuerySpecificFileRequestPayload *psQuerySpecificFileRequestPayload)
{
    ...
    {
#ifdef ZCLIP_ENABLED
        psCustomData->sOTACallBackMessage.sPersistedData.u16ManufacturerCode = psQuerySpecificFileRequestPayload->u16ManufacturerCode;
        psCustomData->sOTACallBackMessage.sPersistedData.u16ImageType = psQuerySpecificFileRequestPayload->u16ImageType;
#endif
        eOtaClientReqSeqNoUpdate(u8SourceEndpoint,u8SequenceNumber);
    }
...
#else
        vOTAGetServerAddress(&sZCL_Address, psCustomData);
#endif
...
PUBLIC  teZCL_Status eOTA_ClientSwitchToNewImage(uint8 u8SourceEndPointId)
{
                                  ...
                                  == E_ZCL_SUCCESS)
    {
        if(!psOTA_Common->sOTACallBackMessage.sPersistedData.bIsNullImage)
        {
            ...
            eStatus = OTA_CommitImage(psOTA_Common->sOTACallBackMessage.sPersistedData.au8Bitmap);
	    OTA_SetNewImageFlag();
            vOtaSwitchLoads(); 
        }
    }
                                                                  ...
                                                                  teZCL_Status eImageVerificationStatus)
{
...
#endif
#endif /* defined JENNIC_CHIP_FAMILY */
                                  ...
                                  == E_ZCL_SUCCESS)
    {
...
#else
        FLib_MemCpy(au8Value, &psOTA_Common->sOTACallBackMessage.sPersistedData.au8Header[0], OTA_AES_BLOCK_SIZE);
#endif
        ...
        if(u32Id == OTA_FILE_IDENTIFIER && (eImageVerificationStatus == E_ZCL_SUCCESS))
        {
...
#else
                    FLib_MemSet(au8TempBuffer, 0, 16);
                    FLib_MemCpy(au8TempBuffer, pu8MacAddress, 8);
                    u32Offset = pu8MacAddress - pu8Start;
                    u32Offset += (psOTA_Common->sOTACallBackMessage.u8ImageStartSector[psOTA_Common->sOTACallBackMessage.u8NextFreeImageLocation] * sNvmDefsStruct.u32SectorSize);
            ...
            /* validate CRC */
            bValid = bOTA_ValidateSignature(psOTA_Common)? TRUE: FALSE;
#endif /* (defined JENNIC_CHIP_FAMILY)*/ 
...
/****************************************************************************
 **
 ** NAME:       bOTA_ValidateSignature
 **
 ** DESCRIPTION:
 ** This function is used to validate signature
 **
 ** PARAMETERS:                         Name                           Usage
 ** tsOTA_Common                                    Source EP Id
 ** uint8                           u8DstEndpoint                 Destination EP Id
 ** teZCL_Status                    eImageVerificationStatus      image verification status
 **
 ** RETURN:
...
PRIVATE  bool_t bOTA_ValidateSignature( tsOTA_Common *psOTA_Common)
{
    ...
    FLib_MemCpy(&u32CrcReceived, &psOTA_Common->sOTACallBackMessage.sPersistedData.au8CrcValueReceived[0], sizeof(uint32_t));
    
    bStatus = (u32CrcReceived == psOTA_Common->sOTACallBackMessage.sPersistedData.u32CrcValue)? TRUE:FALSE;
