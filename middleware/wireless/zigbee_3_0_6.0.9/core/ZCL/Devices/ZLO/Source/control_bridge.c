...
/*!=============================================================================
\file       control_bridge.c
\brief      Zigbee Control bridge functions.
==============================================================================*/
                                                         ...
                                                         tsZLO_ControlBridgeDevice     *psDeviceInfo )
{
    ...
    /* Optional server */
        ...
        /* Create an instance of a Diagnostic cluster as a server */
        if ( eCLD_DiagnosticsCreateDiagnostics ( &psDeviceInfo->sClusterInstance.sDiagnosticsServer,
                                              TRUE,
                                              &sCLD_Diagnostics,
    ...
    /* Optional CLient */
        ...
        if( eSE_SMCreate( u8EndPointIdentifier,                                    // uint8 u8Endpoint
                      ...
                      au8SimpleMeteringServerAttributeControlBits,                  // uint8 *pu8AttributeControlBits
        ...
        if ( eCLD_DiagnosticsCreateDiagnostics ( &psDeviceInfo->sClusterInstance.sDiagnosticsClient,
                                              FALSE,
                                              &sCLD_Diagnostics,
                                              &psDeviceInfo->sDiagnosticsClientCluster,
                                              &au8DiagnosticsClusterAttributeControlBits[0] ) != E_ZCL_SUCCESS )
        {
            // Need to convert from cluster specific to ZCL return type so we lose the extra information of the return code
            return E_ZCL_FAIL;
