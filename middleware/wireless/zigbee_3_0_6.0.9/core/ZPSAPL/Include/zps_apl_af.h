...
typedef enum {
    ...
    ZPS_EVENT_NWK_DISCOVERY_ACTIVE_COMPLETE,            /* 31, 0x1F */
...
typedef union
{
    ...
    ZPS_tsAfNwkFcOverflowIndEvent       sNwkFcOverflowIndEvent;
} ZPS_tuAfEventData;
...
PUBLIC void ZPS_vRunningCRC32 ( uint8_t    u8Byte, uint32_t *pu32Result );
PUBLIC void ZPS_vFinalCrc32 (uint32_t *pu32Result);
...
PUBLIC ZPS_tsAplAfSimpleDescriptor* zps_psGetSimpleDescriptor (void *pvApl, uint8_t u8Index);
...
PUBLIC void zps_vSetLocalPollInterval (void *pvApl, uint16_t u16PollInterval);
PUBLIC void zps_vSetLocalMaxNumPollFailures (void *pvApl, uint16_t u8PollFailure);
PUBLIC void zps_vSetLocalRxOnWhenIdle (void *pvApl, bool_t bSet);
PUBLIC void zps_vSetLocalScanDuration (void *pvApl, uint8_t u8ScanDuration);
...
#endif
PUBLIC ZPS_teStatus zps_eApsAfAddInOutClusters(void *pvApl,
                                               uint8 u8Endpoint,
                                               uint8 u8InClusterCount,
                                               uint8 u8OutClusterCount,
                                               uint16 *pu16InClusterList,
...
ZPS_APL_INLINE ZPS_tsAplAfSimpleDescriptor* ZPS_psGetSimpleDescriptor (uint8_t u8Index) ZPS_APL_ALWAYS_INLINE;
ZPS_APL_INLINE ZPS_tsAplAfSimpleDescriptor* ZPS_psGetSimpleDescriptor (uint8_t u8Index)
{
...
ZPS_APL_INLINE void ZPS_vSetLocalPollInterval(uint16_t u16PollInterval) ZPS_APL_ALWAYS_INLINE;
ZPS_APL_INLINE void ZPS_vSetLocalPollInterval(uint16_t u16PollInterval)
{
    zps_vSetLocalPollInterval(ZPS_pvAplZdoGetAplHandle(), u16PollInterval);
}


ZPS_APL_INLINE void ZPS_vSetLocalMaxNumPollFailures(uint16_t u8PollFailure) ZPS_APL_ALWAYS_INLINE;
ZPS_APL_INLINE void ZPS_vSetLocalMaxNumPollFailures(uint16_t u8PollFailure)
{
    zps_vSetLocalMaxNumPollFailures(ZPS_pvAplZdoGetAplHandle(), u8PollFailure);
}

ZPS_APL_INLINE void ZPS_vSetLocalRxOnWhenIdle(bool_t bSet) ZPS_APL_ALWAYS_INLINE;
ZPS_APL_INLINE void ZPS_vSetLocalRxOnWhenIdle(bool_t bSet)
{
     zps_vSetLocalRxOnWhenIdle(ZPS_pvAplZdoGetAplHandle(),bSet);
}

ZPS_APL_INLINE void ZPS_vSetLocalScanDuration(uint8_t u8ScanDuration) ZPS_APL_ALWAYS_INLINE;
ZPS_APL_INLINE void ZPS_vSetLocalScanDuration(uint8_t u8ScanDuration)
{
