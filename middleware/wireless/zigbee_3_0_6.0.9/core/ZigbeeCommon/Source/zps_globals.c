...
#endif
#if ZPS_ZDO_ENABLE_ZDO_MGMT_NWK_UPDATE_SERVER
PRIVATE uint8 s_sMgmtNWKUpdateServerContext[88];
#endif
#if ZPS_ZDO_ENABLE_ZDO_BIND_UNBIND_SERVER
...
#endif
#if ZPS_ZDO_ENABLE_ZDO_BIND_REQUEST_SERVER
PRIVATE uint8 s_sBindRequestServerContext[84];
...
#if defined(__ICCARM__)
PRIVATE uint8                   s_sNwkContext[1920];
#else
PRIVATE uint8                   s_sNwkContext[1920] __attribute__ ((aligned (4)));
#endif
...
#ifdef ZPS_FRQAG
PRIVATE uint32                  s_au32RxPacketCount[3];
PRIVATE uint32                  s_au32TxPacketCount[3];
#endif
