...
/*!*************************************************************************************************
\fn		memStatus_t AddInOutClustersRequest(AddInOutClustersRequest_t *req, void *arg, uint8_t fsciInterface)
\brief	None
...
memStatus_t AddInOutClustersRequest(AddInOutClustersRequest_t *req, void *arg, uint8_t fsciInterface)
{
	...
	/* Compute the size of the request */
	msgLen += sizeof(uint8_t);  // EndPoint
	msgLen += sizeof(uint8_t);  // InClusterCount
	msgLen += sizeof(uint8_t);  // OutClusterCount
	msgLen += req->InClusterCount * sizeof(uint16_t);  // InputClustersList
	msgLen += req->OutClusterCount * sizeof(uint16_t);  // OutputClustersList
	...
	/* Serialize */
	pMsg[idx] = req->EndPoint; idx++;
	pMsg[idx] = req->InClusterCount; idx++;
	pMsg[idx] = req->OutClusterCount; idx++;
	FLib_MemCpy(pMsg + idx, req->InputClustersList, req->InClusterCount * sizeof(uint16_t)); idx += req->InClusterCount * sizeof(uint16_t);
	FLib_MemCpy(pMsg + idx, req->OutputClustersList, req->OutClusterCount * sizeof(uint16_t)); idx += req->OutClusterCount * sizeof(uint16_t);

	/* Send the request */
	FSCI_transmitPayload(arg, 0x37, 0x2E, pMsg, msgLen, fsciInterface);
	MEM_BufferFree(pMsg);
	return MEM_SUCCESS_c;
}

/*!*************************************************************************************************
\fn		memStatus_t FindAndBindRequest(FindAndBindRequest_t *req, void *arg, uint8_t fsciInterface)
\brief	None
...
memStatus_t FindAndBindRequest(FindAndBindRequest_t *req, void *arg, uint8_t fsciInterface)
{
	...
	FSCI_transmitPayload(arg, 0x37, 0x2F, req, sizeof(FindAndBindRequest_t), fsciInterface);
	return MEM_SUCCESS_c;
}
...
/*!*************************************************************************************************
\fn		memStatus_t MoveToClosestFrequency(MoveToClosestFrequency_t *req, void *arg, uint8_t fsciInterface)
\brief	None
...
memStatus_t MoveToClosestFrequency(MoveToClosestFrequency_t *req, void *arg, uint8_t fsciInterface)
{
	...
	FSCI_transmitPayload(arg, 0x37, 0x85, req, sizeof(MoveToClosestFrequency_t), fsciInterface);
	return MEM_SUCCESS_c;
}
...
/*!*************************************************************************************************
\fn		memStatus_t DiagnosticRequest(DiagnosticRequest_t *req, void *arg, uint8_t fsciInterface)
...
memStatus_t DiagnosticRequest(DiagnosticRequest_t *req, void *arg, uint8_t fsciInterface)
{
	...
	{
		case DiagnosticRequest_AddressMode_TargetShort:
			break;

		case DiagnosticRequest_AddressMode_TargetIEEE:
			break;

		case DiagnosticRequest_AddressMode_TargetBroadcast:
			break;
	}	// TargetAddressValue
	msgLen += sizeof(uint8_t);  // SourceEndPoint
	msgLen += sizeof(uint8_t);  // DestinationEndPoint
	msgLen += sizeof(uint8_t);  // CommandID

	switch (req->CommandID)
	{
		case DiagnosticRequest_CommandID_DelayRequest:
			msgLen += sizeof(uint8_t);  // SequenceNumber
			msgLen += sizeof(uint8_t);  // PayloadSize
			msgLen += req->AttributeValue.DelayRequest.PayloadSize;  // Payload
			break;

		case DiagnosticRequest_CommandID_DelayResults:
			break;
	}	// AttributeValue
	...
	pMsg[idx] = req->SourceEndPoint; idx++;
	pMsg[idx] = req->DestinationEndPoint; idx++;
	pMsg[idx] = req->CommandID; idx++;

	switch (req->CommandID)
	{
		case DiagnosticRequest_CommandID_DelayRequest:
			pMsg[idx] = req->AttributeValue.DelayRequest.SequenceNumber; idx++;
			pMsg[idx] = req->AttributeValue.DelayRequest.PayloadSize; idx++;
			FLib_MemCpy(pMsg + idx, req->AttributeValue.DelayRequest.Payload, req->AttributeValue.DelayRequest.PayloadSize); idx += req->AttributeValue.DelayRequest.PayloadSize;
			break;
	...
	/* Send the request */
	FSCI_transmitPayload(arg, 0x37, 0x99, pMsg, msgLen, fsciInterface);
	MEM_BufferFree(pMsg);
	return MEM_SUCCESS_c;
}
