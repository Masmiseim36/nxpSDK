...
static memStatus_t Load_SocketCloseConfirm(thrEvtContainer_t *container, uint8_t *pPayload);
#endif  /* SOCKET_ENABLE */
...
static memStatus_t Load_THR_SetParentPriorityConfirm(thrEvtContainer_t *container, uint8_t *pPayload);
...
static memStatus_t Load_THR_GetDeviceInfoConfirm(thrEvtContainer_t *container, uint8_t *pPayload);
...
static memStatus_t Load_THR_SetRouterPromoteMaxJitterConfirm(thrEvtContainer_t *container, uint8_t *pPayload);
static memStatus_t Load_THR_SetThresholdConfirm(thrEvtContainer_t *container, uint8_t *pPayload);
static memStatus_t Load_THR_SetThreshold32Confirm(thrEvtContainer_t *container, uint8_t *pPayload);
...
static memStatus_t Load_NWKU_FirewallEnableConfirm(thrEvtContainer_t *container, uint8_t *pPayload);
static memStatus_t Load_NWKU_FirewallRuleAddConfirm(thrEvtContainer_t *container, uint8_t *pPayload);
static memStatus_t Load_NWKU_FirewallRuleRemoveConfirm(thrEvtContainer_t *container, uint8_t *pPayload);
static memStatus_t Load_NWKU_FirewallTableShowResponse(thrEvtContainer_t *container, uint8_t *pPayload);
...
static const thrEvtHandler_t evtHandlerTbl[] =
{
	...
	{SocketCloseConfirm_FSCI_ID, Load_SocketCloseConfirm},
#endif  /* SOCKET_ENABLE */
	...
	{THR_SetParentPriorityConfirm_FSCI_ID, Load_THR_SetParentPriorityConfirm},
	...
	{THR_GetDeviceInfoConfirm_FSCI_ID, Load_THR_GetDeviceInfoConfirm},
	...
	{THR_SetRouterPromoteMaxJitterConfirm_FSCI_ID, Load_THR_SetRouterPromoteMaxJitterConfirm},
	{THR_SetThresholdConfirm_FSCI_ID, Load_THR_SetThresholdConfirm},
	{THR_SetThreshold32Confirm_FSCI_ID, Load_THR_SetThreshold32Confirm},
	...
	{NWKU_FirewallEnableConfirm_FSCI_ID, Load_NWKU_FirewallEnableConfirm},
	{NWKU_FirewallRuleAddConfirm_FSCI_ID, Load_NWKU_FirewallRuleAddConfirm},
	{NWKU_FirewallRuleRemoveConfirm_FSCI_ID, Load_NWKU_FirewallRuleRemoveConfirm},
	{NWKU_FirewallTableShowResponse_FSCI_ID, Load_NWKU_FirewallTableShowResponse},
...
/*!*************************************************************************************************
\fn		static memStatus_t Load_SocketCloseConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
\brief	Confirmation of the socket close request.
***************************************************************************************************/
static memStatus_t Load_SocketCloseConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
{
	SocketCloseConfirm_t *evt = &(container->Data.SocketCloseConfirm);

	/* Store (OG, OC) in ID */
	container->id = SocketCloseConfirm_FSCI_ID;

	FLib_MemCpy(evt, pPayload, sizeof(SocketCloseConfirm_t));
...
/*!*************************************************************************************************
\fn		static memStatus_t Load_THR_SetParentPriorityConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
\brief	Confirmation of the THR_SetParentPriority Request
***************************************************************************************************/
static memStatus_t Load_THR_SetParentPriorityConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
{
	THR_SetParentPriorityConfirm_t *evt = &(container->Data.THR_SetParentPriorityConfirm);

	/* Store (OG, OC) in ID */
	container->id = THR_SetParentPriorityConfirm_FSCI_ID;

	FLib_MemCpy(evt, pPayload, sizeof(THR_SetParentPriorityConfirm_t));
...
/*!*************************************************************************************************
\fn		static memStatus_t Load_THR_GetDeviceInfoConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
\brief	Confirmation of the get device info command.
***************************************************************************************************/
static memStatus_t Load_THR_GetDeviceInfoConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
{
	THR_GetDeviceInfoConfirm_t *evt = &(container->Data.THR_GetDeviceInfoConfirm);

	/* Store (OG, OC) in ID */
	container->id = THR_GetDeviceInfoConfirm_FSCI_ID;

	FLib_MemCpy(evt, pPayload, sizeof(THR_GetDeviceInfoConfirm_t));
...
static memStatus_t Load_THR_MgmtDiagnosticGetRspIndication(thrEvtContainer_t *container, uint8_t *pPayload)
{
	...
	evt->NoOfTLVs = pPayload[idx]; idx++;

	if (evt->NoOfTLVs > 0)
	{
		evt->Payload = MEM_BufferAlloc(evt->NoOfTLVs * sizeof(evt->Payload[0]));
	...
	for (uint32_t i = 0; i < evt->NoOfTLVs; i++)
	{
		...
		switch (evt->Payload[i].TlvId)
		{
			...
			case THR_MgmtDiagnosticGetRspIndication_Payload_TlvId_IeeeAddress:
				FLib_MemCpy(&(evt->Payload[i].TlvData.IeeeAddress), pPayload + idx, sizeof(evt->Payload[i].TlvData.IeeeAddress)); idx += sizeof(evt->Payload[i].TlvData.IeeeAddress);
				break;
		}
	}
...
/*!*************************************************************************************************
\fn		static memStatus_t Load_THR_SetRouterPromoteMaxJitterConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
\brief	Set router promotion max jitter confirm
***************************************************************************************************/
static memStatus_t Load_THR_SetRouterPromoteMaxJitterConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
{
	THR_SetRouterPromoteMaxJitterConfirm_t *evt = &(container->Data.THR_SetRouterPromoteMaxJitterConfirm);

	/* Store (OG, OC) in ID */
	container->id = THR_SetRouterPromoteMaxJitterConfirm_FSCI_ID;

	FLib_MemCpy(evt, pPayload, sizeof(THR_SetRouterPromoteMaxJitterConfirm_t));
...
/*!*************************************************************************************************
\fn		static memStatus_t Load_THR_SetThreshold32Confirm(thrEvtContainer_t *container, uint8_t *pPayload)
\brief	Thread set threshold confirm (1 byte)
***************************************************************************************************/
static memStatus_t Load_THR_SetThreshold32Confirm(thrEvtContainer_t *container, uint8_t *pPayload)
{
	THR_SetThreshold32Confirm_t *evt = &(container->Data.THR_SetThreshold32Confirm);

	/* Store (OG, OC) in ID */
	container->id = THR_SetThreshold32Confirm_FSCI_ID;

	FLib_MemCpy(evt, pPayload, sizeof(THR_SetThreshold32Confirm_t));
...
static memStatus_t Load_THR_GetNeighborInfoConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
{
	...
	switch (evt->Status)
	{
		...
		case THR_GetNeighborInfoConfirm_Status_NeighborNotFound:
			FLib_MemCpy(&(evt->NeighborInfo.NeighborNotFound), pPayload + idx, sizeof(evt->NeighborInfo.NeighborNotFound)); idx += sizeof(evt->NeighborInfo.NeighborNotFound);
			break;
	}
...
/*!*************************************************************************************************
\fn		static memStatus_t Load_NWKU_FirewallEnableConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
\brief	Enable/Disable IPv6 Firewall
***************************************************************************************************/
static memStatus_t Load_NWKU_FirewallEnableConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
{
	NWKU_FirewallEnableConfirm_t *evt = &(container->Data.NWKU_FirewallEnableConfirm);

	/* Store (OG, OC) in ID */
	container->id = NWKU_FirewallEnableConfirm_FSCI_ID;

	FLib_MemCpy(evt, pPayload, sizeof(NWKU_FirewallEnableConfirm_t));

	return MEM_SUCCESS_c;
}

/*!*************************************************************************************************
\fn		static memStatus_t Load_NWKU_FirewallRuleAddConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
\brief	Add new IPv6 Firewall rule
***************************************************************************************************/
static memStatus_t Load_NWKU_FirewallRuleAddConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
{
	NWKU_FirewallRuleAddConfirm_t *evt = &(container->Data.NWKU_FirewallRuleAddConfirm);

	/* Store (OG, OC) in ID */
	container->id = NWKU_FirewallRuleAddConfirm_FSCI_ID;

	FLib_MemCpy(evt, pPayload, sizeof(NWKU_FirewallRuleAddConfirm_t));

	return MEM_SUCCESS_c;
}

/*!*************************************************************************************************
\fn		static memStatus_t Load_NWKU_FirewallRuleRemoveConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
\brief	Add new IPv6 Firewall rule
***************************************************************************************************/
static memStatus_t Load_NWKU_FirewallRuleRemoveConfirm(thrEvtContainer_t *container, uint8_t *pPayload)
{
	NWKU_FirewallRuleRemoveConfirm_t *evt = &(container->Data.NWKU_FirewallRuleRemoveConfirm);

	/* Store (OG, OC) in ID */
	container->id = NWKU_FirewallRuleRemoveConfirm_FSCI_ID;

	FLib_MemCpy(evt, pPayload, sizeof(NWKU_FirewallRuleRemoveConfirm_t));

	return MEM_SUCCESS_c;
}

/*!*************************************************************************************************
\fn		static memStatus_t Load_NWKU_FirewallTableShowResponse(thrEvtContainer_t *container, uint8_t *pPayload)
\brief	Displays the list of IPv6 Firewall rules.
***************************************************************************************************/
static memStatus_t Load_NWKU_FirewallTableShowResponse(thrEvtContainer_t *container, uint8_t *pPayload)
{
	NWKU_FirewallTableShowResponse_t *evt = &(container->Data.NWKU_FirewallTableShowResponse);

	uint32_t idx = 0;

	/* Store (OG, OC) in ID */
	container->id = NWKU_FirewallTableShowResponse_FSCI_ID;

	evt->CountEntries = pPayload[idx]; idx++;

	if (evt->CountEntries > 0)
	{
		evt->Rule = MEM_BufferAlloc(evt->CountEntries * sizeof(evt->Rule[0]));
	...
	for (uint32_t i = 0; i < evt->CountEntries; i++)
	{
		evt->Rule[i].RuleEntryNumber = pPayload[idx]; idx++;
		FLib_MemCpy(evt->Rule[i].SourcePrefix, pPayload + idx, 16); idx += 16;
		FLib_MemCpy(evt->Rule[i].DestinationPrefix, pPayload + idx, 16); idx += 16;
		FLib_MemCpy(&(evt->Rule[i].SourcePort), pPayload + idx, sizeof(evt->Rule[i].SourcePort)); idx += sizeof(evt->Rule[i].SourcePort);
		FLib_MemCpy(&(evt->Rule[i].DestinationPort), pPayload + idx, sizeof(evt->Rule[i].DestinationPort)); idx += sizeof(evt->Rule[i].DestinationPort);
		evt->Rule[i].SourcePrefixLength = pPayload[idx]; idx++;
		evt->Rule[i].DestinationPrefixLength = pPayload[idx]; idx++;
		evt->Rule[i].SecurityLevel = pPayload[idx]; idx++;
		evt->Rule[i].InterfaceID = pPayload[idx]; idx++;
		evt->Rule[i].Rule = pPayload[idx]; idx++;
	}
